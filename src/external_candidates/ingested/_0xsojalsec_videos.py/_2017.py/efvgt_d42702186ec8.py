...
def get_confetti_animations(num_confetti_squares):
    ...
    confetti_spirils = [
        ...
        for a, square in zip(
            np.linspace(0, 0.5, num_confetti_squares), confetti_squares
        )
    ]
...
class Anniversary(TeacherStudentsScene):
    ...
    def get_party_hats(self):
        hats = VGroup(
            *[
                PartyHat(pi_creature=pi, height=0.5 * pi.get_height())
...
class SymmetriesOfSquare(ThreeDScene):
    ...
    def rotate_square(
        ...
        **kwargs,
        ...
        rotation = Rotate(
            square, angle=angle, axis=axis, about_point=square.get_center(), **kwargs
        )
    ...
    def get_rotation_arcs(self, square, angle, angle_buff=SMALL_BUFF):
        ...
        arcs = VGroup(
            arc, *[arc.copy().rotate(i * np.pi / 2) for i in angle_multiple_range]
        )
...
class GroupOfCubeSymmetries(ThreeDScene):
    ...
    def get_cube(self):
        ...
        cube.apply_function(
            lambda p: np.dot(p, pose_matrix.T), maintain_smoothness=False
        )
        return cube

    def get_pose_matrix(self):
        return np.dot(
            rotation_matrix(np.pi / 8, UP), rotation_matrix(np.pi / 24, RIGHT)
        )
...
class AddSquareSymmetries(SymmetriesOfSquare):
    def construct(self):
        ...
        self.play(FadeOut(arcs))
        self.flip_square(
            square=square, axis=UP, added_anims=list(map(FadeIn, equation[2:4]))
        )
        self.wait()
        ...
        self.play(
            square.next_to, equals, LEFT, LARGE_BUFF, alt_square.restore, Write(equals)
        )
        self.flip_square(
            square=alt_square,
            axis=UP + RIGHT,
    ...
    def construct(self):
        ...
        def rotate(circle, angle, arc, terms):
            self.play(
                ...
                ShowCreation(
                    arc, rate_func=lambda t: (angle / (2 * np.pi)) * smooth(t)
                ),
                Write(VGroup(*terms)),
                run_time=2,
            )
        ...
        self.play(
            circle.next_to, equals, LEFT, LARGE_BUFF, alt_circle.restore, Write(equals)
        )
...
class AddCubeSymmetries(GroupOfCubeSymmetries):
    ...
    def construct(self):
        ...
        equation = cube1, plus, cube2, equals, cube3 = VGroup(
            cube, OldTex("+"), cube.copy(), OldTex("="), cube.copy()
        )
    ...
    def rotate_cube(self, cube, angle, axis, add_arrows=True, **kwargs):
        ...
        if add_arrows:
            arrows = VGroup(
                *[
                    Arc(
                        start_angle=np.pi / 12 + a, angle=5 * np.pi / 6, color=YELLOW
                    ).add_tip()
                    for a in (0, np.pi)
                ]
            )
            ...
            arrows.apply_function(
                lambda p: np.dot(p, z_to_axis.T), maintain_smoothness=False
            )
            arrows.move_to(cube)
...
class DihedralGroupStructure(SymmetriesOfSquare):
    ...
    def file_away_sum(self, sum_expression):
        ...
        y = interpolate(
            FRAME_Y_RADIUS - LARGE_BUFF, -FRAME_Y_RADIUS + LARGE_BUFF, y_prop
        )
...
class ThisIsAVeryGeneralIdea(Scene):
    def construct(self):
        ...
        self.play(
            *it.chain(
                list(map(ShowCreation, sub_lines)), list(map(Write, sub_categories))
            )
        )
        self.wait()
...
class AdditiveGroupOfReals(Scene):
    ...
    def show_example_slides_labeled(self, *nums, **kwargs):
        for num in nums:
            ...
            vect = self.number_line.number_to_point(
                num
        ...
        for num_pair in num_pairs:
            ...
            for num in num_pair:
                ...
                if len(num_mobs) > 0:
                    ...
                    if (
                        x < last_num_mob.get_right()[0]
                        and x > last_num_mob.get_left()[0]
                    ):
...
class AdditiveGroupOfComplexNumbers(ComplexTransformationScene):
    ...
    def slide(self, z, *added_anims, **kwargs):
        ...
        self.play(
            ApplyMethod(self.plane.shift, self.z_to_point(z), **kwargs), *added_anims
        )
...
class MultiplicativeGroupOfReals(AdditiveGroupOfReals):
    ...
    def write_group_name(self):
        ...
        VGroup(*new_title[-len("positiverealnumbers") :]).set_color(
            self.positive_reals_color
        )
...
class MultiplicativeGroupOfComplexNumbers(AdditiveGroupOfComplexNumbers):
    ...
    def show_break_down(self):
        ...
        rotation_actions = [
            np.exp(complex(0, angle)) for angle in np.linspace(0, 2 * np.pi, 4)[1:]
        ]
...
class ExponentsAsRepeatedMultiplication(TeacherStudentsScene):
    ...
    def show_repeated_multiplication(self):
        ...
        self.play(
            *[
                ...
                for term, mob in zip(
                    general_equation, [lhs, three_twos.exp, five_twos.exp]
                )
            ]
        )
        self.wait(2)
    ...
    def show_non_counting_exponents(self):
        ...
        half_expression = OldTex(
            "\\big(", "2^{1/2}", "\\big)", "\\big(2^{1/2}\\big) = 2^{1}"
        )
        neg_one_expression = OldTex(
            "\\big(", "2^{-1}", "\\big)", "\\big( 2^{1} \\big) = 2^{0}"
        )
        ...
        self.wait()
        self.play(
            Transform(neg_power, neg_one_expression[1]), Write(neg_one_expression)
        )
        self.wait(2)
        self.play(
        ...
        arrow = Arrow(
            group_theory_words, self.exponential_rule, color=WHITE, buff=SMALL_BUFF
        )
...
class ExponentsAsHomomorphism(Scene):
    ...
    def talk_through_composition(self):
        ...
        self.show_composition(
            *inputs, parallel_anims=list(map(MoveToTarget, self.adders))
        )
        self.play(
        ...
        self.show_composition(
            *inputs, parallel_anims=list(map(MoveToTarget, self.multipliers))
        )
        self.wait()
...
class ComplexExponentiationAdderHalf(
    ComplexExponentiationAbstract, AdditiveGroupOfComplexNumbers
):
    CONFIG = {
        "group_type": "Additive",
        "color": GREEN_B,
...
class ComplexExponentiationMultiplierHalf(
    ComplexExponentiationAbstract, MultiplicativeGroupOfComplexNumbers
):
    CONFIG = {
...
class ExpComplexHomomorphismPreviewAdderHalf(
    ExpComplexHomomorphismPreviewAbstract, ComplexExponentiationAdderHalf
):
    def construct(self):
...
class ECLPromo(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait(10)
        self.play(
            self.pi_creature.change_mode, "pondering", self.pi_creature.look, DOWN
        )
        self.wait(10)
