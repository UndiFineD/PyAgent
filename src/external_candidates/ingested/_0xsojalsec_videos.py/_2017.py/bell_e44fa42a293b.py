...
class PhotonPassesCompletelyOrNotAtAll(DirectionOfPolarizationScene):
    ...
    def construct(self):
        ...
        passing_photon = WavePacket(
            run_time=2, get_filtered=False, em_wave=self.em_wave.copy()
        )
        ...
        filtered_photon = WavePacket(
            run_time=2, get_filtered=True, em_wave=lower_em_wave.copy()
        )
...
class MoreFiltersMoreLight(FilterScene):
    CONFIG = {
        ...
        "pol_filter_configs": [
            {"include_arrow_label": False, "filter_angle": angle}
...
class ShowVariousFilterPairsWithPhotonsOverTime(PhotonsThroughPerpendicularFilters):
    CONFIG = {
        ...
        "pol_filter_configs": [
            {"filter_angle": angle} for angle in (0, 0, np.pi / 2, np.pi / 4, np.pi / 8)
        ],
        "apply_filter": False,
    }
    ...
    def add_filters(self):
        ...
        self.play(
            ReplacementTransform(
                self.pol_filters[0].copy().set_fill(BLACK, 1), self.pol_filters[1]
            )
        )
        self.move_camera(
            theta=-0.65 * np.pi,
    ...
    def show_photons(self, n_photons=5):
        ...
        photons = [
            ...
            for q in np.arange(0, 1, 1.0 / n_photons)
        ]
...
class ShowVariousFilterPairs(ShowVariousFilterPairsWithPhotonsOverTime):
    ...
    def add_top_filters(self):
        ...
        self.move_camera(
            0.9 * np.pi / 2, -0.6 * np.pi, added_anims=[pf.restore for pf in pair]
        )
...
class ForgetPreviousActions(ShowVariousFilterPairs):
    ...
    def construct(self):
        ...
        prob_words = OldTexText(
            "Probabilities depend only\\\\", "on this angle difference"
        )
...
class IntroduceLabeledFilters(ShowVariousFilterPairs):
    CONFIG = {
        ...
        "pol_filter_configs": [
            {"filter_angle": angle} for angle in [0, np.pi / 8, np.pi / 4]
        ],
    ...
    def comment_on_those_blocked_by_B(self):
        ...
        arrow1 = Arrow(
            2 * LEFT, ORIGIN, path_arc=0.8 * np.pi, color=GREEN, stroke_width=5, buff=0
        )
        arrow2 = arrow1.copy()
    ...
    def show_sum(self):
        ...
        self.play(
            fifty.next_to, equals, RIGHT, 2 * SMALL_BUFF, Write(q_mark), FadeIn(randy)
        )
...
class VennDiagramProofByContradiction(Scene):
    ...
    def show_100_photons(self):
        ...
        hundred, photon_word, s = words = OldTexText(
            "100 ", "Photon", "s", arg_separator=""
        )
        words.next_to(eyes, UP)

        self.play(
            ShowCreation(photon), FadeIn(photon.eyes), Write(photon_word, run_time=1.5)
        )
    ...
    def put_all_photons_in_A(self):
        ...
        A.label.target.next_to(
            A.target.point_from_proportion(0.1), UP + RIGHT, SMALL_BUFF
        )
    ...
    def separate_by_C(self):
        ...
        C.label.target.next_to(
            C.target.point_from_proportion(0), DOWN + RIGHT, buff=SMALL_BUFF
        )
    ...
    def show_two_relevant_subsets(self):
        ...
        self.play(
            *[
                ApplyMethod(
                    m.scale, 0.7, method_kwargs={"about_point": FRAME_Y_RADIUS * DOWN}
                )
                for m in everything
            ]
        )
        ...
        self.play(
            morty.change, "confused", should_be_50, Write(contradiction, run_time=1)
        )
...
class ReEmphasizeVennDiagram(VennDiagramProofByContradiction):
    ...
    def shift_B_circle(self):
        ...
        words.shift(RIGHT)
        arrow = Arrow(
            words.get_bottom(), A.get_top() + MED_SMALL_BUFF * RIGHT, color=RED
        )
