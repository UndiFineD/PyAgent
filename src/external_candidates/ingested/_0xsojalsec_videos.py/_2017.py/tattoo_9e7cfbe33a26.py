...
class Introduce(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            "Something different today!", target_mode="hooray", run_time=2
        )
...
class ReactionsToTattoo(PiCreatureScene):
    def construct(self):
        ...
        for mode in modes:
            self.play(
                self.pi_creature.change_mode, mode, self.pi_creature.look, UP + RIGHT
            )
            self.wait(2)
        self.play(
...
class ExplainTrigFunctionDistances(TrigRepresentationsScene, PiCreatureScene):
    ...
    def show_tangent_and_cotangent(self):
        ...
        self.wait()
        self.play(
            *list(map(FadeOut, [tan_frac, cot_frac, self.sin_group, self.cos_group]))
        )
        self.wait()
    ...
    def show_secant_and_cosecant(self):
        ...
        sec_dot, csc_dot = [
            Dot(line.get_end(), color=line.get_color()) for line in (sec_line, csc_line)
        ]
    ...
    def summarize_full_group(self):
        ...
        for func_name in ["sin", "tan", "sec", "cos", "cot", "csc"]:
            ...
            if func_name in ["sin", "cos"]:
                ...
                self.play(
                    ShowCreation(line), GrowFromCenter(brace), Write(text, run_time=1)
                )
...
class Credit(Scene):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(morty, "Book recommendation!", target_mode="surprised")
        )
