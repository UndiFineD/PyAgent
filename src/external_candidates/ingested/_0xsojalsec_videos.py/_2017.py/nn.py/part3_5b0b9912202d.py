...
class InterpretGradientComponents(GradientNudging):
    ...
    def add_cost(self):
        ...
        self.set_variables_as_attrs(
            cost, cost_expression, cost_group, network_rect=rect
        )
    ...
    def change_weights_repeatedly(self):
        ...
        for x in range(self.n_steps):
            self.move_grad_terms_into_position(
                grad_terms.copy(), *self.get_weight_adjustment_anims(edges, cost)
            )
    ...
    def isolate_particular_weights(self):
        ...
        self.set_variables_as_attrs(
            edge1, edge2, new_w_terms, new_decimals=VGroup(d1, d2)
        )
    ...
    def tweak_individual_weights(self):
        ...
        number_line_template = NumberLine(
            x_min=-1, x_max=1, tick_frequency=0.25, big_tick_numbers=[], color=WHITE
        )
        ...
        groups = [
            VGroup(d, d.arrow, edge, w) for d, edge, w in zip(decimals, edges, [wn, wk])
        ]
        for group in groups:
            group.save_state()
    ...
    def get_weight_adjustment_anims(self, edges, cost):
        ...
        return [
            ...
            ChangingDecimal(
                cost, lambda a: interpolate(start_cost, target_cost, a), run_time=1.5
            ),
        ]
...
class GetLostInNotation(PiCreatureScene):
    def construct(self):
        ...
        equations = VGroup(
            ...
            OldTex(
                "{\\partial", "C", "\\over \\partial", "b", "_j^l} =", "\\delta", "_j^l"
            ),
            OldTex(
                "{\\partial",
                "C",
...
class ShowAveragingCost(PreviewLearning):
    ...
    def setup_diff_words(self):
        ...
        double_arrow = DoubleArrow(
            self.network_mob.output_labels, last_layer_copy, color=RED
        )
    ...
    def show_many_examples(self):
        ...
        n_examples_per_adjustment = int(
            self.examples_per_adjustment_time / self.time_per_example
        )
...
class WalkThroughTwoExample(ShowAveragingCost):
    ...
    def show_activation_formula(self):
        ...
        self.wait()
        self.play(
            ReplacementTransform(prev_neurons.copy(), a_terms, path_arc=np.pi / 2)
        )
        self.wait()
    ...
    def three_ways_to_increase(self):
        ...
        for words, mob in zip(increase_words, mobs):
            self.play(
                Write(words[0], run_time=1), ReplacementTransform(mob.copy(), words[1])
            )
            self.wait()
    ...
    def note_connections_to_brightest_neurons(self):
        ...
        self.set_variables_as_attrs(
            bright_neurons, bright_edges, in_proportion_to_a=added_words
        )
    ...
    def fire_together_wire_together(self):
        ...
        def get_edge_animation():
            return LaggedStartMap(
                ShowCreationThenDestruction, bright_edges, lag_ratio=0.7
            )

        neuron_arrows = VGroup(
            *[Vector(MED_LARGE_BUFF * RIGHT).next_to(n, LEFT) for n in bright_neurons]
        )
        ...
        self.play(
            GrowArrow(two_neuron_arrow), morty.change, "raise_right_hand", two_neuron
        )
        self.play(
    ...
    def show_desired_increase_to_previous_neurons(self):
        ...
        for s_edges in positive_edges, negative_edges:
            s_edges.alt_position = VGroup(
                *[Line(LEFT, RIGHT, color=s_edge.get_color()) for s_edge in s_edges]
            )
    ...
    def show_other_output_neurons(self):
        ...
        rect = SurroundingRectangle(
            VGroup(self.network_mob.layers[-1], self.network_mob.output_labels)
        )
...
class ConstructGradientFromAllTrainingExamples(Scene):
    ...
    def setup_grid(self):
        h_lines = VGroup(
            *[Line(LEFT, RIGHT).scale(0.85 * FRAME_X_RADIUS) for x in range(6)]
        )
    ...
    def setup_weights(self):
        weights = VGroup(
            *list(map(Tex, ["w_0", "w_1", "w_2", "\\vdots", "w_{13{,}001}"]))
        )
    ...
    def show_all_examples_requesting_changes(self):
        ...
        examples = VGroup(
            *[self.get_example(t[0], j) for t, j in zip(data, it.count(1))]
        )
        h_dots = OldTex("\\dots")
        h_dots.next_to(examples, RIGHT, MED_LARGE_BUFF)
        more_h_dots = VGroup(
            *[
                OldTex("\\dots").move_to(self.get_grid_position(i, self.n_examples))
                for i in range(5)
            ]
        )
        ...
        more_v_dots = VGroup(
            *[
                self.dots.copy().move_to(self.get_grid_position(3, j))
    ...
    def average_together(self):
        ...
        for row in self.change_rows:
            ...
            rect.arrow = arrow
            average = self.get_colored_decimal(
                3 * np.mean([m.number for m in row if isinstance(m, DecimalNumber)])
            )
        ...
        self.play(
            ...
            ReplacementTransform(
                self.change_rows[0].copy(), self.change_rows[0].target
            ),
            *look_at_anims(averages[0]),
        )
        self.play(GrowArrow(arrow_to_averages))
        self.play(
            LaggedStartMap(ShowCreation, VGroup(*rects[1:])), *look_at_anims(rects[1])
        )
        self.play(
            LaggedStartMap(
                ReplacementTransform,
...
class OrganizeDataIntoMiniBatches(Scene):
    ...
    def divide_into_minibatches(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, rects, lag_ratio=0.7, rate_func=there_and_back)
        )
        self.wait()
    ...
    def one_step_per_batch(self):
        ...
        text = brace.get_text(
            "Compute gradient descent step (using backprop)", buff=SMALL_BUFF
        )
...
class SwimmingInTerms(TeacherStudentsScene):
    def construct(self):
        ...
        terms.to_edge(UP)
        self.play(
            LaggedStartMap(FadeIn, terms), self.change_students(*["horrified"] * 3)
        )
        self.wait()
        self.play(
            terms[-1].next_to,
...
class SimplestNetworkExample(PreviewLearning):
    ...
    def label_neurons(self):
        ...
        for neuron, superscript in zip(neurons, superscripts):
            ...
            arrow = Arrow(
                label[0].get_top(), neuron.get_bottom(), buff=SMALL_BUFF, color=WHITE
            )
            ...
            self.play(
                ...
                UpdateFromFunc(
                    decimal, lambda d: d.set_fill(WHITE if d.number < 0.8 else BLACK)
                ),
                run_time=2,
                rate_func=there_and_back,
            )
        ...
        self.play(
            LaggedStartMap(
                ShowCreation, superscript_rects, lag_ratio=0.8, run_time=1.5
            ),
        ...
        self.set_variables_as_attrs(
            a_labels, a_label_arrows, decimals, last_neurons=neurons
        )
    ...
    def show_cost(self):
        ...
        C0, a, y = [
            cost_equation.get_part_by_tex(tex) for tex in ("C_0", "a^{(L)}", "y")
        ]
        y.set_color(YELLOW)
    ...
    def break_into_computational_graph(self):
        network_early_layers = VGroup(
            *it.chain(self.network_mob.layers[:2], self.network_mob.edge_groups[:2])
        )
        ...
        top_lines = VGroup(
            *[
                Line(term.get_bottom(), zL.target.get_top(), buff=SMALL_BUFF)
        ...
        self.play(
            network_early_layers.fade, 1, ShowCreation(z_to_a_line), z_to_a_line.flash
        )
        self.wait()
    ...
    def show_number_lines(self):
        comp_graph = self.comp_graph
        wL, aLm1, bL, zL, aL, C0 = [
            getattr(comp_graph, attr) for attr in ["wL", "aLm1", "bL", "zL", "aL", "C0"]
        ]
    ...
    def ask_about_w_sensitivity(self):
        ...
        C0_line = Line(
            C0.dot.get_center(), C0.dot.get_center() + MED_SMALL_BUFF * RIGHT
        )
    ...
    def show_chain_of_events(self):
        comp_graph = self.comp_graph
        wL, zL, aL, C0 = [
            getattr(comp_graph, attr) for attr in ["wL", "zL", "aL", "C0"]
        ]
        del_wL = self.del_wL
        del_C0 = self.del_C0
    ...
    def show_chain_rule(self):
        dC_dw = self.dC_dw
        del_syms = [
            getattr(self, attr) for attr in ("del_wL", "del_zL", "del_aL", "del_C0")
        ]
    ...
    def indicate_everything_on_screen(self):
        ...
        everything = VGroup(
            *[m for m in everything.family_members_with_points() if not m.is_subpath]
        )
        self.play(
            LaggedStartMap(
                Indicate, everything, rate_func=wiggle, lag_ratio=0.2, run_time=5
            )
        )
        self.wait()
    ...
    def prepare_for_derivatives(self):
        ...
        Transform(
            VGroup(*cost_equation.target[1:index]), VectorizedPoint(cost_eq.get_left())
        ).update(1)
    ...
    def compute_derivatives(self):
        ...
        moving_decimals = VGroup(
            self.decimals[0].copy(), self.desired_output_decimal.copy()
        )
        minus = OldTex("-")
    ...
    def show_average_derivative(self):
        ...
        VGroup(full_derivative, lhs_brace, lhs_text, rhs_brace, rhs_text).to_corner(
            DOWN + LEFT
        )
    ...
    def transition_to_derivative_wrt_b(self):
        ...
        comp_graph = self.comp_graph
        wL, bL, zL, aL, C0 = [
            getattr(comp_graph, attr) for attr in ["wL", "bL", "zL", "aL", "C0"]
        ]
    ...
    def show_derivative_wrt_b(self):
        ...
        dz_db.set_color_by_tex_to_color_map(
            {"partial z": self.z_color, "partial b": MAROON_B}
        )
        dz_db.replace(dz_dw)
        ...
        self.wait()
        self.play(
            ReplacementTransform(b_in_z_formula.copy(), one), FadeOut(b_in_z_rect)
        )
        self.play(
            ShowCreation(arrow),
...
class IsntThatOverSimplified(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "Isn't that over-simplified?", target_mode="raise_right_hand", run_time=1
        )
...
class GeneralFormulas(SimplestNetworkExample):
    ...
    def show_all_a_labels(self):
        ...
        self.play(
            *it.chain(
                list(map(Write, start_labels)), [GrowArrow(a[0]) for a in start_arrows]
            )
        )
        self.wait()
        self.play(
        ...
        for x, layer in enumerate(self.network_mob.layers[-2:]):
            for y, neuron in enumerate(layer.neurons):
                if (x == 0 and y == 1) or (x == 1 and y == 0):
                    ...
                    rects.add(
                        SurroundingRectangle(
                            neuron.label.target[-1], buff=0.5 * SMALL_BUFF
                        )
                    )
                else:
    ...
    def show_cost(self):
        ...
        to_fade_in = VGroup(
            *[m for m in cost_equation if m not in [aj.target, yj.target]]
        )
    ...
    def show_values_between_weight_and_cost(self):
        ...
        self.play(
            Write(VGroup(*[m for m in z_formula if m not in [w_part, aLm1_part]]))
        )
        self.wait()
    ...
    def show_weight_chain_rule(self):
        ...
        terms = VGroup(
            *[VGroup(*chain_rule[i : i + 3]) for i in range(4, len(chain_rule), 3)]
        )
        rects = VGroup(
            *[SurroundingRectangle(term, buff=0.5 * SMALL_BUFF) for term in terms]
        )
        ...
        self.play(
            LaggedStartMap(
                ShowCreationThenDestruction, rects, lag_ratio=0.7, run_time=3
            )
        )
        self.wait()
...
class LayersOfComplexity(Scene):
    ...
    def get_chain_rule_equations(self):
        ...
        result = VGroup(
            w_deriv, lil_rect, arrow, big_rect, a_deriv, or_word, last_a_deriv
        )
...
class SponsorFrame(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            morty.change, "hooray", t_shirt_words, Write(t_shirt_words, run_time=2)
        )
        self.wait()
        ...
        self.play(
            LaggedStartMap(Indicate, url, rate_func=wiggle, color=PINK, run_time=3)
        )
        self.wait(3)
