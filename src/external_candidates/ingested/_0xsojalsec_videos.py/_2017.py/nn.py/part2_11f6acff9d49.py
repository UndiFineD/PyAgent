...
class PreviewLearning(NetworkScene):
    ...
    def show_training(self):
        ...
        for train_in, train_out in training_data[: self.n_examples]:
            ...
            self.backprop_one_example(
                train_in, train_out, FadeOut(image), self.network_mob.layers.restore
            )
        ...
        for i, nb, nw, delta_neurons, neurons, delta_edges, edges in reversed(tups):
            ...
            if i == 2:
                delta_edges.submobjects = [
                    delta_edges[j] for j in np.argsort(shown_nw.T.flatten())
                ]
            ...
            self.play(
                ShowCreation(delta_edges, lag_ratio=0),
    ...
    def get_edge_animation(self):
        ...
        return LaggedStartMap(
            ApplyFunction,
            edges,
            lambda mob: (lambda m: m.rotate(np.pi / 12).set_color(YELLOW), mob),
            rate_func=wiggle,
        )
...
class TrainingVsTestData(Scene):
    ...
    def get_examples(self):
        ...
        group = Group(
            *it.starmap(get_training_image_group, self.data[ci : ci + self.n_examples])
        )
        group.arrange(DOWN)
        group.scale(0.5)
        return group
    ...
    def subdivide_into_training_and_testing(self):
        ...
        self.wait()
        self.play(
            MoveToTarget(test_examples), GrowFromCenter(test_brace), FadeIn(test_words)
        )
...
class MNistDescription(Scene):
    ...
    def construct(self):
        ...
        training_data_groups = [
            training_data[i * epc : (i + 1) * epc] for i in range(self.n_grids)
        ]

        for i, td_group in enumerate(training_data_groups):
            print(i)
            group = Group(
                *[self.get_digit_pair(v_in, v_out) for v_in, v_out in td_group]
            )
...
class FunctionMinmization(GraphScene):
    ...
    def construct(self):
        ...
        dots = VGroup(
            *[Dot().move_to(self.input_to_graph_point(x, graph)) for x in range(10)]
        )
...
class ChangingDecimalWithColor(ChangingDecimal):
    def interpolate_mobject(self, alpha):
        ...
        self.decimal_number.set_fill(
            interpolate_color(BLACK, WHITE, 0.5 + num * 0.5), opacity=1
        )
...
class IntroduceCostFunction(PreviewLearning):
    ...
    def isolate_one_neuron(self):
        network_mob = self.network_mob
        neurons = VGroup(
            *it.chain(*[layer.neurons for layer in network_mob.layers[1:]])
        )
    ...
    def reminder_of_weights_and_bias(self):
        ...
        weighted_edges = VGroup(
            *[self.color_edge_randomly(edge.copy(), exp=1) for edge in neuron.edges_in]
        )
        ...
        self.wait()
        self.play(
            ReplacementTransform(w_labels.copy(), weighted_edges, remover=True),
    ...
    def feed_in_example(self):
        ...
        self.play(
            MoveToTarget(neurons, lag_ratio=0.5, remover=True),
            layer0.neurons.set_fill,
            None,
            0,
        )
    ...
    def need_a_cost_function(self):
        ...
        double_arrow = DoubleArrow(
            layer.get_right(), desired_layer.get_left(), color=RED
        )
    ...
    def fade_all_but_last_layer(self):
        network_mob = self.network_mob
        to_fade = VGroup(
            *it.chain(*list(zip(network_mob.layers[:-1], network_mob.edge_groups)))
        )
    ...
    def break_down_cost_function(self):
        ...
        self.play(
            *[
                ReplacementTransform(VGroup(*l.neurons[:10]).copy(), dg)
    ...
    def show_small_cost_output(self):
        ...
        arrows = VGroup(
            *[Arrow(ORIGIN, LEFT).next_to(d, LEFT, MED_LARGE_BUFF) for d in decimals]
        )
        ...
        for arrow, d1, d2 in zip(arrows, *self.decimal_groups):
            ...
            term_updates.append(
                ChangingDecimalWithColor(
                    term, term.num_update_func, num_decimal_places=4
                )
            )
        ...
        def sum_update(alpha):
            return sum(
                [(d1.number - d2.number) ** 2 for d1, d2 in zip(*self.decimal_groups)]
            )
        ...
        update_decimals = [
            ChangingDecimalWithColor(
                decimal, generate_decimal_update_func(decimal.number, t)
            )
        ...
        self.play(
            *it.chain(update_decimals, term_updates, [neurons.restore]), run_time=2
        )
        self.wait()
        self.play(
            cost_group.restore,
    ...
    def average_over_all_training_data(self):
        ...
        for in_vect, out_vect in training_data[: self.n_training_examples]:
            ...
            new_decimal_groups = VGroup(
                *[self.num_vect_to_decimals(v) for v in (random_v, out_vect)]
            )
    ...
    def num_vect_to_decimals(self, num_vect):
        return VGroup(
            *[DecimalNumber(n).set_fill(opacity=0.5 * n + 0.5) for n in num_vect]
        )
...
class YellAtNetwork(PiCreatureScene, PreviewLearning):
    ...
    def construct(self):
        ...
        self.play(
            FadeIn(desired),
            RemovePiCreatureBubble(randy, target_mode="sassy", look_at=desired),
...
class EmphasizeComplexityOfCostFunction(IntroduceCostFunction):
    ...
    def show_network_as_a_function(self):
        ...
        self.activate_network(
            in_vect, GrowArrow(arrow), FadeIn(output), FadeIn(output_words[1])
        )
        self.wait()
    ...
    def show_cost_function(self):
        ...
        training_data, validation_data, test_data = load_data_wrapper()
        training_examples = Group(
            *list(map(self.get_training_pair_mob, training_data[: self.n_examples]))
        )
        ...
        self.play(
            ...
            self.underline.stretch_to_fit_width,
            new_title.get_width(),
        )
        self.play(
            ApplyMethod(
                parameter_words[1].move_to,
                input_words[1],
                LEFT,
                path_arc=np.pi,
            ),
...
class NetworkGrowthMindset(YellAtNetwork):
    def construct(self):
        ...
        self.pi_creature_says(
            "But we can do better! \\\\ Growth mindset!", target_mode="hooray"
        )
...
class SingleVariableCostFunction(GraphScene):
    ...
    def show_graph(self):
        ...
        self.play(
            ...
            self.input_to_graph_point(5, graph),
            RIGHT,
            ShowCreation(graph),
        )
        self.wait()
    ...
    def find_exact_solution(self):
        ...
        line = self.get_vertical_line_to_graph(
            x, graph, line_class=DashedLine, color=YELLOW
        )
    ...
    def take_steps(self):
        ...
        def update_line(line):
            ...
            line.put_start_and_end_on_with_projection(
                self.coords_to_point(x, 0), self.input_to_graph_point(x, graph)
            )
            return line
...
class TwoVariableInputSpace(Scene):
    ...
    def ask_about_direction(self):
        ...
        dot.fade(1)
        arrows = VGroup(
            *[Arrow(ORIGIN, vect).shift(point) for vect in compass_directions(8)]
        )
        arrows.set_color(WHITE)
        question = OldTexText(
...
class ShowFullCostFunctionGradient(PreviewLearning):
    ...
    def organize_weights_as_column_vector(self):
        ...
        decimals = VGroup(
            *[DecimalNumber(num).set_color(BLUE_D if num > 0 else RED) for num in nums]
        )
        dots = OldTex("\\vdots")
        ...
        edges_target = VGroup(
            *it.chain(decimals[:3], [dots] * (len(edges) - 6), decimals[-3:])
        )
        ...
        self.play(
            FadeOut(layers),
            edges.space_out_submobjects,
            1.2,
        )
        self.play(
            ReplacementTransform(edges, edges_target, run_time=2, lag_ratio=0.5),
    ...
    def show_gradient(self):
        ...
        for od, nd in zip(old_decimals, new_decimals):
            ...
            self.play(
                Transform(
                    nd, VectorizedPoint(od.get_center()), lag_ratio=0.5, remover=True
                ),
                ChangingDecimal(od, lambda a: interpolate(od_num, od_num + nd_num, a)),
            )
        self.wait()
...
class HowMinimizingCostMeansBetterTrainingPerformance(IntroduceCostFunction):
    ...
    def improve_last_layer(self):
        ...
        self.play(
            *[ChangingDecimal(d, update) for d, update in zip(decimals, updates)]
            + [
                UpdateFromFunc(
                    d,
...
class NonSpatialGradientIntuition(Scene):
    ...
    def show_sign_interpretation(self):
        ...
        self.play(
            LaggedStartMap(MoveToTarget, ws), LaggedStartMap(FadeIn, direction_phrases)
        )
        self.wait(2)
    ...
    def show_magnitude_interpretation(self):
        ...
        for phrase, decimal in zip(direction_phrases, decimals):
            ...
            rect = SurroundingRectangle(
                VGroup(*decimal[-4:]), buff=SMALL_BUFF, color=GREY_B
            )
            rect.target = words
            rects.add(rect)
...
class SomeConnectionsMatterMoreThanOthers(PreviewLearning):
    ...
    def fade_edges(self):
        ...
        self.play(
            *[
                ApplyMethod(
                    edge.set_stroke, BLACK, 0, rate_func=lambda a: 0.5 * smooth(a)
                )
                for edge in edges
            ]
        )
...
class SpinningVectorWithLabel(Scene):
    def construct(self):
        ...
        self.play(
            ...
            UpdateFromFunc(label, lambda m: m.next_to(vector.get_end(), UP + RIGHT)),
            cd1,
            cd2,
        )
        self.wait()
...
class GradientNudging(PreviewLearning):
    ...
    def add_gradient(self):
        ...
        self.set_variables_as_attrs(
            grad_lhs=lhs, grad_vect=vect, grad_arg_words=words, grad_arg_brace=brace
        )
    ...
    def change_weights_repeatedly(self):
        ...
        arrows = VGroup(
            *[
                Arrow(words.get_bottom(), edge_group.get_top(), color=WHITE)
    ...
    def get_decimal_change_anims(self, decimals):
        ...
        return [
            ChangingDecimal(decimal, generate_change_func(decimal.number, change))
    ...
    def construct(self):
        ...
        for x in 0.7, 0.35, 0.97, 0.23, 0.54:
            ...
            self.play(
                ...
                None,
                x,
                ChangingDecimal(decimal, lambda a: interpolate(curr_num, x, a)),
                decimal_color_anim,
            )
            self.wait()
...
class TestPerformance(PreviewLearning):
    ...
    def run_through_examples(self):
        title = self.title
        rects = [
            SurroundingRectangle(VGroup(neuron, label), buff=0.5 * SMALL_BUFF)
            for neuron, label in zip(
                self.network_mob.layers[-1].neurons, self.network_mob.output_labels
            )
        ]
        ...
        for test_in, test_out in ProgressDisplay(list(self.test_data)):
            ...
            group.arrange(RIGHT)
            group.shift(
                self.title.get_bottom() + MED_SMALL_BUFF * DOWN - image.get_top()
            )
            ...
            new_decimal = DecimalNumber(
                float(self.n_correct) / self.total, **self.decimal_kwargs
            )
...
class InterpretFirstWeightMatrixRows(TestPerformance):
    ...
    def show_all_pixel_array(self):
        ...
        self.play(
            VGroup(*neurons[1:]).set_stroke,
            None,
            0.5,
            FadeIn(self.words),
            neurons[0].edges_in.set_stroke,
            None,
            2,
            *[
        ...
        for neuron, pixel_array in zip(neurons, self.pixel_arrays):
            if last_neuron:
                self.play(
                    last_neuron.edges_in.set_stroke,
                    None,
                    0.25,
                    last_neuron.set_stroke,
                    None,
                    0.5,
                    neuron.set_stroke,
                    None,
...
class InputRandomData(TestPerformance):
    ...
    def show_random_image(self):
        ...
        self.play(
            ShowCreation(rect),
            LaggedStartMap(DrawBorderThenFill, image, stroke_width=0.5),
        )
...
class CannotDraw(PreviewLearning):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(randy, "Draw a \\\\ 5 for me", look_at=eyes, run_time=1)
        )
...
class SomethingToImproveUpon(PiCreatureScene, TestPerformance):
    ...
    def show_path(self):
        ...
        line.shift(UP)
        dots = VGroup(
            *[Dot(line.point_from_proportion(a)) for a in np.linspace(0, 1, 5)]
        )
    ...
    def old_news(self):
        ...
        self.play(
            Write(words, run_time=1), GrowArrow(arrow), morty.change_mode, "hesitant"
        )
