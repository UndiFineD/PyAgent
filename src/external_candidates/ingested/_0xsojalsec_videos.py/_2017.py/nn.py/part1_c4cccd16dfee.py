...
class PixelsFromVect(PixelsAsSquares):
    def __init__(self, vect, **kwargs):
        PixelsAsSquares.__init__(
            self, ImageMobject(layer_to_image_array(vect)), **kwargs
        )
...
class ExampleThrees(PiCreatureScene):
    ...
    def show_initial_three(self):
        ...
        self.play(
            ...
            "sassy",
            *it.chain(
                list(map(GrowFromCenter, braces)), list(map(FadeIn, brace_labels))
            ),
        )
        self.wait()
        self.play(
    ...
    def show_alternate_threes(self):
        ...
        self.play(
            randy.change,
            "plain",
            *list(map(FadeOut, [self.bubble, self.arrow, self.real_three]))
            + [MoveToTarget(three)],
        )
    ...
    def show_alternate_digits(self):
        ...
        self.play(
            randy.change, "sassy", Transform(three, alt_mobs[0]), ShowCreation(cross)
        )
        self.wait()
...
class WriteAProgram(Scene):
    def construct(self):
        ...
        for n in 8, 1, 5, 3:
            self.play(
                rect.move_to, choices[n], MaintainPositionRelativeTo(q_mark, rect)
            )
            self.wait(1)
        choice = choices[3]
...
class PreviewMNistNetwork(NetworkScene):
    ...
    def get_image_to_layer_one_animation(self, image, start_neurons):
        image_mover = VGroup(
            *[pixel.copy() for pixel in image if pixel.fill_rgb[0] > 0.1]
        )
        return Transform(
            image_mover,
            start_neurons,
...
class BreakDownName(Scene):
    ...
    def ask_questions(self):
        ...
        self.play(
            Write(q2, run_time=1), ShowCreation(a2), name[1].set_color, q2.get_color()
        )
        self.wait(2)
    ...
    def show_neuron(self):
        ...
        self.play(
            ReplacementTransform(self.neuron_word, neuron_word), ShowCreation(neuron)
        )
...
class DiscussChoiceForHiddenLayers(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            FadeOut(rects), ReplacementTransform(two_words, sixteen_words), neurons_anim
        )
        self.wait()
...
class AskAboutPropogationAndTraining(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "How does one layer \\\\ influence the next?", index=0, run_time=1
        )
        self.wait()
...
class AskAboutLayers(PreviewMNistNetwork):
    def construct(self):
        ...
        arrows = VGroup(
            *[
                Arrow(question[1].get_bottom(), group.get_top())
        ...
        self.play(
            ...
            LaggedStartMap(
                GrowFromPoint, arrows, lambda a: (a, a.get_start()), run_time=2
            ),
        )
        self.wait()
...
class BreakUpMacroPatterns(IntroduceEachLayer):
    ...
    def show_what_learning_is_required(self):
        ...
        for x in range(3):
            self.play(
                LaggedStartMap(ShowCreationThenDestruction, edge_group, run_time=3)
            )
            self.wait()
    ...
    def get_equation(self, *mobs):
        equation = VGroup(
            mobs[0],
            OldTex("=").scale(2),
            *list(it.chain(*[[m, OldTex("+").scale(2)] for m in mobs[1:-1]]))
            + [mobs[-1]],
        )
...
class HowWouldYouRecognizeSubcomponent(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "Okay, but recognizing loops \\\\", "is just as hard!", target_mode="sassy"
        )
...
class BreakUpMicroPatterns(BreakUpMacroPatterns):
    ...
    def break_down_loop(self):
        ...
        self.wait()
        self.play(
            LaggedStartMap(ApplyMethod, edges, lambda e: (e.restore,), run_time=4)
        )
        self.wait()
        self.play(
...
class SecondLayerIsLittleEdgeLayer(IntroduceEachLayer):
    ...
    def setup_activations_and_nines(self):
        ...
        nine_array, loop_array, line_array = [
            np.array(im)[:, :, 0] / 255.0 for im in images
        ]
    ...
    def show_propogation(self):
        ...
        active_layers = [
            network_mob.get_active_layer(i, a) for i, a in enumerate(activations)
        ]

        def activate_layer(i):
            self.play(
                ShowCreationThenDestruction(
                    edge_groups[i - 1], run_time=2, lag_ratio=0.5
                ),
    ...
    def set_neurons_target(self, neurons, layer):
        ...
        Transform(
            VGroup(*neurons.target[n:-n]), VectorizedPoint(layer.get_center())
        ).update(1)
...
class ManyTasksBreakDownLikeThis(TeacherStudentsScene):
    def construct(self):
        ...
        sequence = VGroup(
            audio, get_arrow(), letters, get_arrow(), syllables, get_arrow(), word
        )
    ...
    def get_wave_form(self):
        func = lambda x: abs(
            sum([(1.0 / n) * np.sin((n + 3) * x) for n in range(1, 5)])
        )
        result = VGroup(
            *[Line(func(x) * DOWN, func(x) * UP) for x in np.arange(0, 4, 0.1)]
        )
...
class AskAboutWhatEdgesAreDoing(IntroduceEachLayer):
    ...
    def add_question(self):
        ...
        arrows = VGroup(
            *[
                Arrow(question.get_bottom(), edge_group.get_top())
...
class IntroduceWeights(IntroduceEachLayer):
    ...
    def ask_about_parameters(self):
        ...
        pixel_updates = [
            UpdateFromAlphaFunc(pixel, lambda p, a: p.set_fill(opacity=p.func(a)))
            for pixel in pixels
        ]
    ...
    def show_weights(self):
        ...
        self.play(
            MoveToTarget(edges),
            *[
                ChangingDecimal(decimal, get_alpha_func(r))
    ...
    def show_weighted_sum(self):
        ...
        weighted_sum = VGroup(
            *it.chain(*[[w, a, OldTex("+")] for w, a in zip(w_labels, a_labels)])
        )
        weighted_sum.add(
            OldTex("\\cdots"),
            OldTex("+"),
    ...
    def add_negative_weights_around_the_edge(self):
        ...
        self.play(
            *[
                ApplyMethod(
                    weight_grid[28 * y + x].set_fill, self.negative_weights_color, 0.5
                )
                for y in (6, 10)
    ...
    def get_pixels_to_detect(self, pixels):
        ...
        return VGroup(
            *it.chain(
                *[pixels[d * n + d / 2 - 4 : d * n + d / 2 + 4] for n in range(7, 10)]
            )
        )
    ...
    def get_surrounding_pixels_for_edge(self, pixels):
        d = int(np.sqrt(len(pixels)))
        return VGroup(
            *it.chain(*[pixels[d * n + d / 2 - 4 : d * n + d / 2 + 4] for n in (6, 10)])
        )
...
class MotivateSquishing(Scene):
    ...
    def add_weighted_sum(self):
        weighted_sum = OldTex(
            *it.chain(
                *[["w_%d" % d, "a_%d" % d, "+"] for d in range(1, 5)]
                + [["\\cdots", "+", "w_n", "a_n"]]
            )
        )
...
class ContinualEdgeUpdate(VGroup):
    ...
    def __init__(self, network_mob, **kwargs):
        ...
        for edge in edges:
            ...
            edge.widths = [
                msw * random.random() ** self.stroke_width_exp for x in range(n_cycles)
            ]
...
class ShowRemainingNetwork(IntroduceWeights):
    ...
    def count_in_biases(self):
        ...
        arrows = VGroup(
            *[
                Arrow(words.get_left(), neuron.get_center(), color=BLUE)
                for neuron in neurons
            ]
        )
    ...
    def show_remaining_layers(self):
        ...
        weights_count.target[-1].next_to(
            VGroup(weights_count.prefix, added_weights), DOWN
        )
        ...
        neurons = VGroup(
            *it.chain(*[layer.neurons for layer in network_mob.layers[2:]])
        )
...
class ImagineSettingByHand(Scene):
    def construct(self):
        ...
        network_mob = NetworkMobject(
            Network(sizes=[8, 6, 6, 4]), neuron_stroke_color=WHITE
        )
...
class IntroduceWeightMatrix(NetworkScene):
    ...
    def activate_first_layer(self):
        ...
        a_labels = VGroup(
            *[OldTex("a^{(0)}_%d" % d) for d in range(len(layer.neurons))]
        )
    ...
    def show_first_neuron_weighted_sum(self):
        ...
        self.play(
            Write(a1_label), Write(equals), neuron.set_fill, None, 0.3, run_time=1
        )
    ...
    def organize_activations_into_column(self):
        ...
        self.play(
            LaggedStartMap(
                Indicate, self.a_labels, rate_func=there_and_back, run_time=1
            )
        )
        self.play(
            MoveToTarget(a_labels), Transform(pre_brackets, post_bracketes), run_time=2
        )
        self.wait()
        self.play(
            *[
    ...
    def connect_weighted_sum_to_matrix_multiplication(self):
        ...
        self.play(
            ReplacementTransform(
                VGroup(top_row_rect, column_rect).copy(), result_terms[0]
            )
        )
        ...
        self.play(
            *list(map(FadeOut, [result_terms, result_brackets, equals, column_rect]))
        )

    def show_meaning_of_lower_rows(self, arrow, brace, row_rect, result_terms):
        n1, n2, nk = neurons = VGroup(
            *[self.network_mob.layers[1].neurons[i] for i in (0, 1, -1)]
        )
        for n in neurons:
            n.save_state()
    ...
    def write_clean_final_expression(self):
        ...
        expression.set_color_by_tex_to_color_map(
            {"sigma": YELLOW, "big": YELLOW, "W": GREEN, "\\textbf{b}": BLUE}
        )
        ...
        self.play(
            ReplacementTransform(
                VGroup(
                    self.top_matrix_row, self.lower_matrix_rows, self.matrix_brackets
                ).copy(),
                VGroup(W),
            )
        )
        ...
        self.play(
            ReplacementTransform(
                VGroup(self.b_column, self.b_brackets).copy(), VGroup(VGroup(b))
            ),
            ReplacementTransform(self.bias_plus.copy(), plus),
        )
        self.play(
            ReplacementTransform(self.big_sigma_group.copy(), VGroup(sigma, lp, rp))
        )
        self.wait()
...
class EoLA3Wrapper(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            ShowCreation(rect), FadeIn(title), morty.change, "raise_right_hand", rect
        )
        self.wait(4)
...
class NeuronIsFunction(MoreHonestMNistNetworkPreview):
    ...
    def fade_network_back_in(self):
        anims = [
            FadeIn(mob, run_time=2, lag_ratio=0.5)
...
class IntroduceReLU(IntroduceSigmoid):
    ...
    def old_school(self):
        ...
        self.play(
            ShowCreation(
                self.sigmoid_graph, rate_func=lambda t: smooth(1 - t), remover=True
            )
        )
