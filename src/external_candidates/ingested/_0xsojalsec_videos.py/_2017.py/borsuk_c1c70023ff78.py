# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\borsuk.py
...
class Jewel(VMobject):
    ...
    def init_points(self):
        ...
        self.submobjects.sort(key=lambda m: -m1.get_center()[2])
        return self
...
class Necklace(VMobject):
    ...
    def get_default_colors(self):
        result = list(it.chain(*[num * [color] for num, color in self.default_colors]))
        random.shuffle(result)
        return result

    def init_points(self):
        jewels = VGroup(*[Jewel(color=color) for color in self.colors])
...
class IntroduceStolenNecklaceProblem(ThreeDScene):
    ...
    def add_thieves(self):
        thieves = VGroup(Randolph(), Mortimer())
    ...
    def write_title(self):
        ...
        title.to_edge(UP)
        self.play(
            Write(title), *[ApplyMethod(pi.look_at, title) for pi in self.thieves]
        )
        self.title = title
    ...
    def introduce_necklace(self):
        ...
        self.play(
            FadeIn(necklace, lag_ratio=0.5, run_time=3),
            *it.chain(
                *[
        ...
        self.play(
            *[
                ApplyMethod(jewel.rotate, np.pi / 6, UP, rate_func=there_and_back)
                for jewel in jewels
            ]
        )
        self.play(Blink(self.thieves[0]))
        for jewel_type in jewel_types:
            self.play(jewel_type.shift, 0.2 * UP, rate_func=wiggle)
        self.wait()
        ...
        for jewel_type, label in zip(jewel_types, enumeration_labels):
            jewel_type.submobjects.sort(key=lambda m: m1.get)
            ...
            self.play(MoveToTarget(jewel_type), Write(label))
    ...
    def divvy_by_cutting_all(self):
        ...
        for thief, vect in zip(self.thieves, [LEFT, RIGHT]):
            ...
            for label in enumeration_labels:
                tex, jewel = label
                num = int(tex.get_tex())
                half_label = VGroup(OldTex(str(num / 2)), jewel.copy())
                half_label.arrange()
                half_labels.add(half_label)
            half_labels.arrange(DOWN)
            half_labels.set_height(thief.get_height())
            half_labels.next_to(thief, vect, buff=MED_LARGE_BUFF, aligned_edge=DOWN)
            both_half_labels.add(half_labels)

        for half_labels in both_half_labels:
            self.play(ReplacementTransform(enumeration_labels.copy(), half_labels))
        ...
        for type_index, jewel_type in enumerate(jewel_types):
            ...
            self.play(
                ...
                *[ApplyMethod(thief.look_at, jewel_type_copy) for thief in thieves],
            )
        self.play(
            *it.chain(
                *[
        ...
        self.wait()
        self.play(*[jewel_type.restore for jewel_type in jewel_types])
        self.play(
            *it.chain(
                *[
    ...
    def divvy_with_n_cuts(
        ...
        jewel_types = self.jewel_types
        jewels = sorted(
            necklace.jewels, lambda m1, m2: cmp(m1.get_center()[0], m2.get_center()[0])
        )
        slice_indices, binary_choices = self.find_slice_indices(jewels, jewel_types)
        subgroups = [
            VGroup(*jewels[i1:i2]) for i1, i2 in zip(slice_indices, slice_indices[1:])
        ]
        ...
        for group in strand_groups:
            group.save_state()
        self.play(
            strand_groups[0].shift,
            UP / 2.0,
            strand_groups[1].shift,
            DOWN / 2.0,
        )
        if with_thieves:
            self.play(
        ...
        if show_matching_after_divvying:
            for jewel_type in jewel_types:
                self.play(
                    *[ApplyMethod(jewel.scale, 1.5) for jewel in jewel_type],
    ...
    def shuffle_jewels(self, jewels, run_time=2, path_arc=np.pi / 2, **kwargs):
        ...
        self.play(
            Transform(
                jewels, target_group, run_time=run_time, path_arc=path_arc, **kwargs
            )
        )
...
class ThingToProve(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            Write(bottom_words, run_time=2), self.pi_creature.change_mode, "pondering"
        )
        self.wait(3)
...
class FiveJewelCase(IntroduceStolenNecklaceProblem):
    ...
    def add_title(self):
        ...
        title = OldTexText("%d jewel types, %d cuts" % (n_cuts, n_cuts))
        title.to_edge(UP)
        self.add(title)
...
class RepeatedShuffling(IntroduceStolenNecklaceProblem):
    ...
    def construct(self):
        ...
        for x in range(self.num_shuffles):
            self.shuffle_jewels(jewels)
            self.divvy_with_n_cuts(show_matching_after_divvying=False)
...
class TemperaturePressurePlane(GraphScene):
    ...
    def draw_arrow(self):
        ...
        arrow = Arrow(
            square.get_right(), self.coords_to_point(*self.example_point_coords)
        )
...
class WalkEquatorPostTransform(GraphScene):
    ...
    def walk_equator(self):
        ...
        self.play(
            self.get_arc_walk_dot_movement(
                equator, dots, rate_func=lambda t: 2 * proportion * smooth(t)
            )
        )
        ...
        dots.save_state()
        equator.save_state()
        self.play(equator.fade, dots.fade)
    ...
    def walk_tilted_equator(self):
        ...
        self.play(
            self.get_arc_walk_dot_movement(
                tilted_eq, dots, rate_func=lambda t: 2 * proportion * smooth(t)
            )
        )
        v_line = DashedLine(FRAME_Y_RADIUS * UP, FRAME_Y_RADIUS * DOWN)
        v_line.shift(dots.get_center()[0] * RIGHT)
    ...
    def draw_transverse_curve(self):
        ...
        equator_transform = Succession(
            *[Transform(equator, arc, rate_func=linear) for arc in target_arcs]
        )
        dots_transform = Succession(
            *[Transform(dots, target, rate_func=linear) for target in target_dots]
        )
    ...
    def walk_transverse_curve(self):
        ...
        for x in range(2):
            self.play(UpdateFromAlphaFunc(dots, dot_update), run_time=4)
        self.play(
            UpdateFromAlphaFunc(dots, dot_update),
            run_time=4,
            rate_func=lambda t: 0.455 * smooth(t),
        )
        self.play(dots.set_color, YELLOW, dots.scale, 1.2, rate_func=there_and_back)
        self.wait()
    ...
    def sphere_to_plane(self, point):
        x, y, z = point
        return np.array([x - 2 * x * z + y + 1, y + 0.5 * y * np.cos(z * np.pi), 0])
    ...
    def get_great_arc_images(self):
        curves = VGroup(
            *[
                ParametricCurve(lambda t: self.sphere_point(t, s)).apply_function(
                    self.sphere_to_plane
                )
                for s in np.arange(0, 1, 1.0 / self.num_great_arcs)
                # for s in [0]
            ]
        )
        ...
        for curve in curves:
            antipodal_x_diff = (
                lambda x: curve.point_from_proportion(x + 0.5)[0]
    ...
    def get_transverse_curve(self, gerat_arc_images):
        points = list(
            it.chain(
                *[
                    [curve.x_collision_points[i] for curve in gerat_arc_images]
                    for i in (0, 1)
                ]
            )
        )
        ...
        first_half = full_curve.copy().pointwise_become_partial(full_curve, 0, 0.5)
...
class MentionGenerality(TeacherStudentsScene, ThreeDScene):
    def construct(self):
        ...
        self.play(
            self.teacher.change_mode, "raise_right_hand", self.teacher.look_at, arrow
        )
        self.play(
            FadeIn(necklace, run_time=2, lag_ratio=0.5),
            Write(arrow),
            *[ApplyMethod(pi.look_at, arrow) for pi in self.get_pi_creatures()],
        )
        self.play_student_changes("pondering", "erm", "confused")
        self.wait()
        self.play(*[ApplyMethod(pi.look_at, arrow) for pi in self.get_pi_creatures()])
...
class PointsIn3D(Scene):
    ...
    def construct(self):
        sphere_def = OldTexText(
            "\\doublespacing Sphere in 3D: All",
            "$(x_1, x_2, x_3)$\\\\",
            "such that",
            "$x_1^2 + x_2^2 + x_3^2 = 1$",
            alignment="",
        ...
        point_ex.scale(0.8)
        point_ex.next_to(sphere_def[1], UP + RIGHT, buff=1.5 * LARGE_BUFF)
...
class WritePointCoordinates(Scene):
    ...
    def construct(self):
        ...
        arrow = Arrow(-self.corner, self.corner, stroke_width=8, color=MAROON_B)
    ...
    def get_coords(self):
        coords = OldTex("(", "0.41", ", ", "-0.58", ", ", "0.71", ")", arg_separator="")
...
class GeneralizeBorsukUlam(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(arrow), Write(f))
        self.play(Write(output_space))
        self.wait()
        self.play(FadeIn(lhs))
        self.play(ReplacementTransform(lhs.copy(), rhs), Write(eq))
        self.wait()

    def get_condition(self):
        squares = list(map(Tex, ["x_%d^2" % d for d in range(1, 1 + self.n_dims)]))
    ...
    def get_tuple(self):
        mid_parts = list(it.chain(*[["x_%d" % d, ","] for d in range(1, self.n_dims)]))
    ...
    def get_negative_tuple(self):
        mid_parts = list(
            it.chain(*[["-", "x_%d" % d, ","] for d in range(1, self.n_dims)])
        )
        tup = OldTex(*["("] + mid_parts + ["-", "x_%d" % self.n_dims, ")"])
    ...
    def get_sphere_set(self):
        ...
        condition.next_to(tup, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
...
class MakeTwoJewelCaseContinuous(IntroduceStolenNecklaceProblem):
    ...
    def introduce_necklace(self):
        ...
        self.play(FadeIn(self.necklace, lag_ratio=0.5))
        self.shuffle_jewels(self.necklace.jewels)
        jewel_types = self.get_jewels_organized_by_type(self.necklace.jewels)
        self.wait()
    ...
    def identify_necklace_with_unit_interval(self):
        interval = UnitInterval(
            tick_frequency=1.0 / sum(self.num_per_jewel),
            tick_size=0.2,
            big_tick_numbers=[],
        )
    ...
    def color_necklace(self):
        ...
        jewels.submobjects.sort(key=lambda m: m.get_center()[0])
        ...
        segments.submobjects.sort(key=lambda m: m.get_center()[0])
        ...
        for group in segment_types:
            length_tex = OldTex("\\frac{%d}{%d}" % (len(group), len(jewels)))
            ...
            self.wait()
            self.play(group.shift, DOWN, FadeOut(length_tex))
    ...
    def color_necklace_by_indices(self, *indices):
        chain = self.necklace.chain
        jewels = VGroup(*[self.necklace.jewels[i] for i in indices])
        ...
        self.play(ReplacementTransform(jewels, segments, lag_ratio=0.5, run_time=2))
        self.wait()
        return segments
    ...
    def find_continuous_fair_division(self):
        ...
        slice_indices, ignore = self.find_slice_indices(
            self.necklace.jewels, self.jewel_types
        )
        cut_proportions = [
            sorted([random.random(), random.random()])
            for x in range(self.num_continuous_division_searches)
        ]
        cut_proportions.append([float(i) / n_jewels for i in slice_indices[1:-1]])
        cut_points = [
            list(map(chain.point_from_proportion, pair)) for pair in cut_proportions
        ]
    ...
    def set_color_continuous_groups(self):
        ...
        weight_description = VGroup(
            *[
                VGroup(
                    OldTex(
                        "\\frac{%d}{%d}" % (len(jewel_type) / 2, len(self.segments))
                    ),
    ...
    def mention_equivalence_to_discrete_case(self):
        ...
        self.wait()
        self.play(*list(map(FadeOut, [morty, morty.bubble, morty.bubble.content])))
    ...
    def shift_divide_off_tick_marks(self):
        ...
        label = VGroup(
            OldTex("5\\left( 1/18 \\right)"), Jewel(color=self.jewel_colors[1])
        ).arrange()
        ...
        self.wait()
        self.play(emerald_segments.restore, *list(map(FadeOut, [brace, label])))
...
class ThinkAboutTheChoices(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(
            *["pondering"] * 3, look_at=FRAME_X_RADIUS * RIGHT + FRAME_Y_RADIUS * DOWN
        )
        self.wait(3)
...
class ChoicesInNecklaceCutting(ReconfigurableScene):
    ...
    def add_necklace(self):
        width, colors, num_per_color = [
            ...
            for key in ["necklace_width", "jewel_colors", "num_per_jewel"]
        ]
        color_list = list(
            it.chain(*[num * [color] for num, color in zip(num_per_color, colors)])
        )
        random.shuffle(color_list)

        interval = UnitInterval(
            tick_frequency=1.0 / sum(num_per_color),
            tick_size=0.2,
            big_tick_numbers=[],
        )
        ...
        for l_tick, r_tick, color in zip(tick_marks, tick_marks[1:], color_list):
            segment = Line(l_tick.get_center(), r_tick.get_center(), color=color)
    ...
    def choose_places_to_cut(self):
        v_lines = VGroup(*[DashedLine(UP, DOWN) for x in range(2)])
        final_num_pair = np.cumsum([1.0 / d for d in self.denoms[:2]])
        ...
        point_pairs = [
            list(map(self.interval.number_to_point, num_pair)) for num_pair in num_pairs
        ]
        ...
        self.division_points = list(
            it.chain(
                [self.interval.get_left()], point_pairs[-1], [self.interval.get_right()]
            )
        )
    ...
    def show_three_numbers_adding_to_one(self):
        ...
        for char, denom, brace in zip("abc", self.denoms, braces):
            ...
            VGroup(brace.label, brace.concrete_label).set_color(YELLOW)

        words = OldTexText("1) Choose", "$a$, $b$, $c$", "so that", "$a+b+c = 1$")
        ...
        self.play(
            *it.chain(
                *[[GrowFromCenter(brace), Write(brace.label)] for brace in braces]
            )
        )
        self.play(Write(words))
        self.wait(2)
        ...
        self.play(
            *list(
                map(FadeOut, list(braces) + [brace.concrete_label for brace in braces])
            )
        )
    ...
    def make_binary_choice(self):
        ...
        words.next_to(
            self.choice_one_words, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
        ...
        for binary_choices in it.product(*[[0, 1]] * 3):
            self.play(
                *[
                ...
                + [
                    Transform(curr_arrow, arrow_pair[choice], path_arc=np.pi)
    ...
    def get_groups(self, indices=None):
        ...
        for group in groups:
            group.target_points = [
                group.get_center() + self.thief_box_offset * vect for vect in (UP, DOWN)
            ]
    ...
    def get_boxes_and_labels(self):
        ...
        boxes = VGroup(
            *[box.copy().shift(self.thief_box_offset * vect) for vect in (UP, DOWN)]
        )
        labels = VGroup(
            *[
                OldTexText("Thief %d" % (i + 1)).next_to(box, UP, aligned_edge=RIGHT)
    ...
    def get_choice_arrow_pairs(self, groups):
        ...
        arrow_pairs = [[arrow.copy(), arrow.copy().rotate(np.pi)] for group in groups]
        pre_arrow_points = [VectorizedPoint(group.get_center()) for group in groups]
    ...
    def wiggle_v_lines(self):
        self.play(
            *it.chain(
                *[
                    [line.rotate, np.pi / 12, vect, line.set_color, RED]
...
class ChoicesForSpherePoint(GeneralizeBorsukUlam):
    ...
    def initialize_words(self):
        choice_one_words = OldTexText(
            "1) Choose", "$x^2$, $y^2$, $z^2$", "so that", "$x^2+y^2+z^2 = 1$"
        )
        for i in 1, 3:
        ...
        choice_two_words = OldTexText("2) Make a binary choice for each one")
        choice_two_words.next_to(
            choice_one_words, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
    ...
    def show_example_choices(self):
        ...
        choices.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=LEFT)
        choices.set_height(FRAME_Y_RADIUS)
        choices.to_edge(LEFT)
        choices.shift(DOWN)

        self.play(FadeIn(choices, run_time=2, lag_ratio=0.5))
        self.wait()
    ...
    def show_binary_choices(self):
        for choice in self.choices:
            ...
            sqrts = VGroup(
                *[
                    OldTex(var_tex + "=" + sign + "\\sqrt{%s}" % frac_tex)
                    for sign in ["+", "-"]
                ]
            )
            for sqrt in sqrts:
...
class NecklaceDivisionSphereAssociation(ChoicesInNecklaceCutting):
    ...
    def add_sphere_point_label(self):
        label = OldTexText("$(x, y, z)$", "such that", "$x^2 + y^2 + z^2 = 1$")
        ...
        arrow = Arrow(label.get_left(), ghost_sphere_point, color=WHITE)
    ...
    def add_braces(self):
        points = self.division_points
        braces = [
            Brace(Line(p1 + SMALL_BUFF * RIGHT / 2, p2 + SMALL_BUFF * LEFT / 2), UP)
            for p1, p2 in zip(points, points[1:])
        ]
        for char, brace, color, denom in zip(
            "xyz", braces, self.xyz_colors, self.denoms
        ):
            brace.label = brace.get_text(
                "$%s^2$" % char, "$= 1/%d$" % denom, buff=SMALL_BUFF
            )
            brace.label.set_color(color)
            if brace.label.get_right()[0] > brace.get_right()[0]:
                brace.label.next_to(brace, UP, buff=SMALL_BUFF, aligned_edge=RIGHT)
    ...
    def show_binary_choice_association(self):
        ...
        for brace, denom, group, final_choice in quads:
            ...
            choices = [
                OldTex(char, "=", sign, "\\sqrt{\\frac{1}{%d}}" % denom)
                for sign in ("+", "-")
            ]
            ...
            self.play(
                ReplacementTransform(
                    VGroup(brace.label[0], brace, brace.label[1]), choices[0]
                ),
                group.move_to,
                group.target_points[0],
            )
            self.wait()
            self.play(Transform(*choices), group.move_to, group.target_points[1])
            self.wait()
            if final_choice == 0:
                self.play(
            ...
            self.swapping_anims += [
                ...
                group.move_to,
                group.target_points[1 - final_choice],
            ]
    ...
    def ask_about_antipodal_pairs(self):
        ...
        antipodal_tex = OldTex("(x, y, z) \\rightarrow (-x, -y, -z)")
...
class TotalLengthOfEachJewelEquals(NecklaceDivisionSphereAssociation, ThreeDScene):
    ...
    def find_fair_division(self):
        segments, tick_marks = self.necklace
        segments.sort()
        segment_colors = [segment.get_color() for segment in segments]
        indices = self.get_fair_division_indices(segment_colors)
        groups = self.get_groups([0] + list(np.array(indices) + 1) + [len(segments)])
        self.add(*groups)
        binary_choice = [0, 1, 0]
    ...
    def get_fair_division_indices(self, colors):
        ...
        type_to_count = dict([(color, sum(colors == color)) for color in color_types])
    ...
    def demonstrate_fair_division(self):
        segments, tick_marks = self.necklace
        color_types = list(
            map(Color, set([segment.get_color().get_hex_l() for segment in segments]))
        )
        top_segments = VGroup(
            *it.chain(
                self.groups[0][0],
        ...
        for color in color_types:
            ...
            for i, group in enumerate(monochrome_groups):
                ...
                label = VGroup(
                    OldTexText("Thief %d" % (i + 1)), Jewel(color=group[0].get_color())
                )
                label.arrange()
            ...
            for group, label in zip(monochrome_groups, labels):
                ...
                self.wait()
            self.play(FadeOut(labels), *[group.restore for group in monochrome_groups])
        self.wait()
...
class ExclaimBorsukUlam(TeacherStudentsScene):
    def construct(self):
        self.student_says("Borsuk-Ulam!", target_mode="hooray")
        self.play(
...
class ShowFunctionDiagram(TotalLengthOfEachJewelEquals, ReconfigurableScene):
    ...
    def add_number_pair(self):
        plane_classes = [
            JewelPairPlane(skip_animations=True, thief_number=x) for x in (1, 2)
        ]
        ...
        arrow = Arrow(
            self.necklace.get_corner(DOWN + RIGHT), self.example_coords, color=YELLOW
        )
        ...
        self.transition_to_alt_config(make_up_fair_division_indices=True)
        self.wait()
        t1_plane.save_state()
        self.play(Transform(*planes, path_arc=np.pi), Animation(arrow))
        self.wait(2)
        self.play(ApplyMethod(t1_plane.restore, path_arc=np.pi), Animation(arrow))
        self.wait()
    ...
    def add_sphere_arrow(self):
        ...
        to_plane_arrow = Arrow(
            up_down_arrow.get_bottom() + DOWN + RIGHT, self.example_coords, color=YELLOW
        )
...
class FourDBorsukUlam(GeneralizeBorsukUlam, PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(lhs), self.pi_creature.change_mode, "raise_right_hand")
        self.play(ReplacementTransform(lhs.copy(), rhs), Write(eq))
        self.wait(2)
...
class CircleToSphereToQMarks(Scene):
    def construct(self):
        ...
        shapes = [Circle(color=BLUE, radius=0.5), VectorizedPoint(), OldTex("???")]
        for d, mode, shape in zip(it.count(2), modes, shapes):
            randy = Randolph(mode=mode)
            randy.scale(0.7)
            bubble = randy.get_bubble(height=3, width=4, direction=LEFT)
            bubble.pin_to(randy)
            ...
            pi_groups.add(VGroup(randy, bubble, shape, title, arrow))
...
class RotatingThreeDSphereProjection(Scene):
    ...
    def construct(self):
        ...
        self.play(Rotating(sphere, axis=RIGHT + OUT, run_time=10))
