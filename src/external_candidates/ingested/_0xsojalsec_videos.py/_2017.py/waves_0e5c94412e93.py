# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\waves.py
import warnings
    ...
    def interpolate_mobject(self, alpha):
        ...
        for vect in self.vects:
            ...
            distance_from_packet = np.dot(
                tail - packet_center, em_wave.propogation_direction
            )
...
class FilterScene(ThreeDScene):
    CONFIG = {
        ...
        "pol_filter_configs": [{}],
        "EMWave_config": {"start_point": FRAME_X_RADIUS * LEFT + DOWN + OUT},
        "axes_config": {},
...
class DirectionOfPolarizationScene(FilterScene):
    ...
    def change_polarization_direction(self, angle, **kwargs):
        ...
        self.play(
            ApplyMethod(self.reference_line.rotate, angle, **kwargs), *added_anims
        )
...
class IntroduceElectricField(PiCreatureScene):
    ...
    def let_particle_wander(self):
        ...
        particles = VGroup(
            self.particle, *[self.particle.copy().move_to(point) for point in points]
        )
        for particle in particles:
            particle.velocity = np.zeros(3)

        self.play(
            FadeOut(self.force_vector), LaggedStartMap(FadeIn, VGroup(*particles[1:]))
        )
...
class CurlRelationBetweenFields(ThreeDScene):
    ...
    def loop_in_M(self):
        M_vects = VGroup(
            *[
                Vector(
                    rotate_vector(vect, np.pi / 2), normal_vector=IN, color=M_COLOR
                ).shift(vect)
        ...
        self.play(
            ShowCreation(M_vects, run_time=2), *list(map(FadeOut, self.E_vects[1:]))
        )
        self.wait()
        self.play(
            E_vect.rotate,
...
class ShowVectorEquation(Scene):
    ...
    def write_components(self):
        x, y = components = VGroup(
            OldTex("\\cos(", "2\\pi", "f_x", "t", "+ ", "\\phi_x", ")"), OldTex("0", "")
        )
    ...
    def show_graph(self):
        ...
        self.play(
            ReplacementTransform(
                self.components[0].get_part_by_tex("f_x").copy(), fx_equals_1
            ),
        )
    ...
    def add_kets(self):
        ...
        arrows = VGroup(
            *[Arrow(kets_word.get_top(), ket, color=ket.get_color()) for ket in kets]
        )
...
class ShowTipToTailSum(ShowVectorEquation):
    ...
    def add_kets(self):
        h_ket, v_ket = kets = VGroup(
            *[
                OldTex("\\cos(", "2\\pi", "f", "t", ")", "|\\!\\%sarrow\\rangle" % s)
                for s in ("right", "up")
            ]
        )
        for ket in kets:
    ...
    def add_amplitudes(self):
        ...
        h_A_mob, v_A_mob = A_mobs = VGroup(
            *[OldTex(str(A)).add_background_rectangle() for A in [h_A, v_A]]
        )
        A_mobs.scale(0.8)
                ...
                [
                    UpdateFromAlphaFunc(
                        ...
                        self.generate_A_update(
                            ov, A * np.array(ov.A_vect), np.array(ov.A_vect)
                        ),
                    )
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                ...
                self.generate_phi_update(
                    v_ov, np.array([0, np.pi / 2, 0]), np.array(v_ov.phi_vect)
                ),
            ),
        )
    ...
    def get_superposition_tex(self, angle, s1, s2):
        ...
        def generate_decimal_update(trig_func):
            def update_decimal(decimal):
                ...
                families = [
                    mob.family_members_with_points() for mob in (decimal, new_decimal)
                ]
...
class EnergyOfWavesTeacherPortion(TeacherStudentsScene):
    ...
    def show_energy_equation(self):
        ...
        energy = OldTex(
            "\\frac{\\text{Energy}}{\\text{Volume}}", "=", "\\epsilon_0", "A", "^2"
        )
    ...
    def show_both_ways_of_thinking_about_it(self):
        ...
        for b, s in (b1, s1), (b2, s2):
            self.play(
                ShowCreation(b), Write(b.content, run_time=2), s.change, "thinking"
            )
            self.wait(2)
        self.play_student_changes(
            *["plain"] * 3,
...
class ShootPhotonThroughFilter(DirectionOfPolarizationScene):
    ...
    def expect_half_energy_to_be_absorbed(self):
        ...
        self.move_camera(
            ...
            + [
                ApplyMethod(
                    v.rotate, -np.pi / 2, method_kwargs={"axis": v.get_vector()}
                )
        ...
        self.play(
            ...
            self.superposition_tex.h_rect.set_stroke,
            RED,
            3,
            *list(map(GrowFromCenter, lines))
            + [Animation(self.pol_filter), Animation(self.frozen_photon.mobject)],
        )
        self.wait(2)
        self.move_camera(
            phi=0.8 * np.pi / 2,
            ...
            + [
                ApplyMethod(v.rotate, np.pi / 2, method_kwargs={"axis": v.get_vector()})
                for v in self.frozen_photon.mobject
            ],
        )
        self.play(
            ...
            self.superposition_tex.h_rect.fade,
            1,
...
class YouCanSeeTheCollapse(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            "You can literally \\\\ \\emph{see} the collapse", target_mode="hooray"
        )
...
class ThreeFilters(ShootPhotonThroughFilter):
    ...
    def fifty_percent_pass_second(self):
        arrow = Arrow(
            ORIGIN, 3 * RIGHT, use_rectangular_stem=False, path_arc=-0.8 * np.pi
        )
        label = OldTex("50\\%")
    ...
    def show_changed_to_diagonal(self):
        ...
        self.move_camera(
            ...
            + [
                Rotate(v, np.pi / 2, axis=v.get_vector(), in_place=True, **kwargs)
...
class PhotonAtSlightAngle(ThreeFilters):
    ...
    def write_components(self):
        ...
        self.play(
            ...
            ReplacementTransform(h_arrow.copy(), state.get_part_by_tex("rightarrow")),
        ...
        self.play(
            ...
            ReplacementTransform(v_arrow.copy(), state.get_part_by_tex("uparrow")),
        )
    ...
    def probabalistic_passing(self):
        ...
        self.play(
            ShowCreation(arrow), Write(label, run_time=1), ShowCreation(l1, **kwargs)
        )
        self.play(
    ...
    def get_blocked_photon(self, **kwargs):
        return self.get_photon(
            filter_distance=FRAME_X_RADIUS + 3, get_filtered=True, **kwargs
        )
...
class CompareWaveEquations(TeacherStudentsScene):
    ...
    def add_equation(self):
        ...
        self.play_student_changes(
            *["pondering"] * 3,
            look_at=psi_rect,
            added_anims=[ShowCreation(psi_rect), Write(state_words, run_time=1)],
            run_time=1,
        )
    ...
    def show_complex_plane(self):
        new_alpha, new_beta = terms = [
            self.equation.get_part_by_tex(tex).copy() for tex in ("alpha", "beta")
        ]
        for term in terms:
            term.save_state()
    ...
    def show_interpretations(self):
        ...
        self.play(
            FadeOut(self.state_words), Write(c_words), self.teacher.change, "happy"
        )
