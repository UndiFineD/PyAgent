# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\qa_round_two.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
class PowersOfTwo(Scene):
    def construct(self):
        powers_of_2 = VGroup(
            *[OldTex("2^{%d}" % n, "=", "{:,}".format(2**n)) for n in range(20)]
        )
        ...
        self.play(mob.space_out_submobjects, 1.1, mob.to_edge, RIGHT)
        ...
        self.play(two_to_ten.restore, *list(map(ShowCreation, lines)))

        curr_po2_outline = curr_po2[-1].copy()
        curr_po2_outline.set_fill(opacity=0)
        curr_po2_outline.set_stroke(width=2)
        curr_po2_outline.set_color_by_gradient(YELLOW, RED, PINK, PURPLE, BLUE, GREEN)
...
class PiHoldingScreen(PiCreatureScene):
    def construct(self):
        ...
        self.play(ShowCreation(screen), morty.change, "raise_right_hand", screen)
        self.wait(10)
        self.play(morty.change, "hooray", words[0], Write(words[0]))
        self.wait(10)
        self.play(morty.change, "pondering", words[1], Transform(words[0], words[1]))
        self.wait(10)
