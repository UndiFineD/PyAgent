...
class SliderScene(Scene):
    ...
    def setup(self):
        ...
        sliders = VGroup(
            *[Slider(center_value=cv, **self.slider_config) for cv in self.center_point]
        )
...
class BackAndForth(Scene):
    def construct(self):
        ...
        self.wait(3)
        for tup, eq, to_draw in (pair, circle_eq, circle), (
            triplet,
            sphere_eq,
            VMobject(),
        ):
...
class AskAboutLongerLists(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(
            *["confused"] * 3, look_at=self.teacher.get_top() + 2 * UP
        )
        ...
        self.play(
            *[
                ...
                for num, dial, a in zip(
                    numbers, dial_copies, np.linspace(0, 0.5, len(numbers))
                )
            ]
        )
...
class TwoDimensionalCase(Introduce4DSliders):
    ...
    def setup(self):
        ...
        for number in self.sliders[0].numbers:
            ...
            number.move_to(
                center_of_mass(
                    [slider.number_to_point(value) for slider in self.sliders]
                )
            )
    ...
    def comment_on_cheap_vs_expensive_real_estate(self):
        ...
        for slider in self.sliders:
            for x1, x2 in (-0.5, 0.5), (0.75, 1.0), (-1.0, -0.75):
                ...
                rect = Rectangle(
                    stroke_width=0, fill_opacity=0.5, width=0.25, height=(p2 - p1)[1]
                )
...
class TwoDBoxExample(Scene):
    ...
    def compute_radius(self):
        ...
        self.play(
            Write(root_2_value, run_time=1), *list(map(FadeOut, [bottom_one, side_one]))
        )
        self.wait()
        self.play(ShowCreation(corner_radius))
        self.play(
            Rotating(corner_radius, about_point=c0_center, run_time=2, rate_func=smooth)
        )
...
class GeneralizePythagoreanTheoremBeyondTwoD(ThreeDScene):
    def construct(self):
        ...
        diag = Line(
            rect.get_corner(DOWN + LEFT), rect.get_corner(UP + RIGHT), color=YELLOW
        )
...
class ThreeDBoxFormulas(Scene):
    def construct(self):
        ...
        inner_r = OldTex(
            "\\text{Inner radius}", "&=", "\\sqrt{3} - 1\\\\", "&\\approx", "0.73"
        )
...
class TwoDBoxWithSliders(TwoDimensionalCase):
    ...
    def setup(self):
        ...
        corner_circles = VGroup(
            *[
                self.circle.copy()
                .move_to(self.plane.coords_to_point(*coords))
                .set_color(GREY)
    ...
    def recenter_circle(self):
        ...
        self.play(
            ...
            *[
                ApplyMethod(
                    mob.shift,
                    slider.number_to_point(0)
                    - slider.number_to_point(slider.center_value),
                )
                for slider in self.sliders
        ...
        self.play(
            ...
            *[
                ApplyMethod(
                    mob.shift, slider.number_to_point(x) - slider.number_to_point(0)
                )
    ...
    def write_x_and_y_real_estate(self):
        ...
        x_brace, y_brace = [
            Brace(slider.real_estate_ticks, RIGHT) for slider in self.sliders
        ]
    ...
    def swap_with_top_right_circle(self):
        ...
        self.play(
            ...
            *[
                ApplyMethod(
                    mob.shift, slider.number_to_point(1) - slider.number_to_point(-1)
                )
                for mob in (slider.real_estate_ticks, slider.dial)
            ],
        )
...
class ThreeDBoxExampleWithSliders(SliderScene):
    ...
    def point_out_closest_point(self):
        ...
        re_words = OldTexText(
            "$x$, $y$ and $z$ each have \\\\", "$\\frac{1}{3}$", "units of real estate"
        )
        ...
        new_arrow = Arrow(
            re_words.get_corner(DOWN + RIGHT), re_line.get_left(), color=BLUE
        )
    ...
    def compare_to_halfway_point(self):
        ...
        ghost_dials = VGroup(
            *[slider.dial.copy().set_fill(WHITE, 0.5) for slider in self.sliders]
        )
        ...
        self.play(
            FadeOut(self.arrow), Write(half_label, run_time=1), ShowCreation(half_line)
        )
        self.wait()
...
class FourDBoxExampleWithSliders(ThreeDBoxExampleWithSliders):
    ...
    def list_corner_coordinates(self):
        ...
        coordinate_mobs = VGroup(
            *[OldTex("(%d, %d, %d, %d)" % tup) for tup in coordinates]
        )
...
class TwoDInnerSphereTouchingBox(TwoDBoxWithSliders, PiCreatureScene):
    ...
    def construct(self):
        little_inner_circle, big_inner_circle = [
            Circle(radius=radius * self.plane.x_unit_size, color=GREEN).move_to(
                self.plane.coords_to_point(0, 0)
            )
        ...
        self.wait()
        self.play(
            LaggedStartMap(DrawBorderThenFill, tangency_points, rate_func=double_smooth)
        )
...
class FiveDBoxExampleWithSliders(FourDBoxExampleWithSliders):
    ...
    def show_halfway_point(self):
        ...
        curr_vector = self.get_vector()
        ghost_dials = VGroup(
            *[slider.dial.copy().set_fill(WHITE, 0.75) for slider in self.sliders]
        )
    ...
    def reframe_as_inner_sphere_point(self):
        ...
        self.play(
            Animation(self.sliders),
            LaggedStartMap(
                ApplyMethod, re_ticks, lambda m: (m.shift, shift_vect), path_arc=np.pi
            ),
...
class SkipAheadTo10(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            "Let's skip ahead \\\\ to 10 dimensions", target_mode="hooray"
        )
...
class ThreeDOuterBoundingBoxWords(Scene):
    def construct(self):
        words = OldTexText(
            "$4 \\!\\times\\! 4\\!\\times\\! 4$ outer\\\\", "bounding box"
        )
...
class CoordinateFree(PiCreatureScene):
    def construct(self):
        ...
        circles = VGroup(
            *[
                Circle(color=YELLOW).move_to(plane.coords_to_point(*coords))
                for coords in it.product(*2 * [[-1, 1]])
            ]
        )
        inner_circle = Circle(radius=np.sqrt(2) - 1, color=GREEN).move_to(
        ...
        self.play(
            PiCreatureSays(
                self.pi_creature, "Lose the \\\\ coordinates!", target_mode="hooray"
            )
        )
...
class Skeptic(TeacherStudentsScene, SliderScene):
    ...
    def construct(self):
        ...
        all_real_estate_ticks = VGroup(
            *it.chain(*[slider.real_estate_ticks for slider in self.sliders])
        )
        ...
        bubble = self.teacher.get_bubble(
            "It's much \\\\ more playful!", bubble_type=SpeechBubble
        )
        bubble.resize_to_content()
        VGroup(bubble, bubble.content).next_to(self.teacher, UP + LEFT)
        self.play(
            self.teacher.change, "hooray", ShowCreation(bubble), Write(bubble.content)
        )
        self.wait(3)
        self.play(
            RemovePiCreatureBubble(
                self.teacher, target_mode="raise_right_hand", look_at=self.sliders
            ),
...
class Announcements(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            *[ApplyMethod(pi.change, "hooray", items) for pi in self.pi_creatures]
        )
...
class Podcast(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                ApplyMethod, self.pi_creatures, lambda pi: (pi.change, "hooray", title)
            ),
            Write(title),
        )
        self.wait(5)
...
class TenDThumbnail(Scene):
    def construct(self):
        ...
        circles = VGroup(
            *[Circle(radius=r).move_to(corner) for corner in square.get_vertices()]
        )
