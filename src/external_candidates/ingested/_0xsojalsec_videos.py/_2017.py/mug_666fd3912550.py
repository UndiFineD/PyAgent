# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\mug.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
class HappyHolidays(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(DrawBorderThenFill, hats), Animation(self.pi_creatures)
        )
        self.play_student_changes(
            "happy",
            "wave_2",
...
class UtilitiesPuzzleScene(Scene):
    ...
    def setup_configuration(self):
        ...
        bounding_box = SurroundingRectangle(
            objects, buff=MED_LARGE_BUFF, stroke_width=0
        )
...
class AboutToyPuzzles(UtilitiesPuzzleScene, TeacherStudentsScene, ThreeDScene):
    ...
    def lay_out_puzzle(self):
        ...
        self.play(
            LaggedStartMap(ShowCreation, lines, run_time=3), Animation(self.objects)
        )
        self.play(
    ...
    def point_to_abstractions(self):
        ...
        arrow_from_eulers = Arrow(
            eulers.get_bottom(), tda.get_corner(UP + LEFT), color=WHITE
        )
        ...
        arrow_to_eulers = Arrow(
            self.houses[2].get_corner(UP + LEFT), eulers.get_bottom(), color=WHITE
        )
...
class ThisPuzzleIsHard(UtilitiesPuzzleScene, PiCreatureScene):
    ...
    def failed_attempts(self):
        ...
        line_sets = [
            ...
            [
                ...
                self.get_line(0, 1),
                self.get_line(1, 2),
                self.get_line(2, 0, bb.get_bottom(), bb.get_corner(DOWN + LEFT)),
                self.get_line(0, 2, bb.get_top()),
                self.get_line(
                    2,
                    1,
    ...
    def ask_meta_puzzle(self):
        ...
        self.play(
            LaggedStartMap(ShowCreation, straight_lines, run_time=2, lag_ratio=0.8),
            Blink(randy),
        )
        self.play(
            Transform(straight_lines, almost_solution_lines, run_time=3, lag_ratio=0.5)
        )
        self.wait()
...
class IntroduceGraph(PiCreatureScene):
    def construct(self):
        ...
        dots = VGroup(
            *[Dot(color=pi.get_color()).scale(2).move_to(pi) for pi in pi_creatures]
        )
        lines = VGroup(
            *[
        ...
        self.play(
            ...
            LaggedStartMap(
                ApplyMethod, pi_creatures, lambda pi: (pi.change, "pondering", lines)
            ),
        )
        ...
        self.play(
            LaggedStartMap(
                ApplyMethod, lines, lambda l: (l.rotate, np.pi / 12), rate_func=wiggle
            )
        )
        self.play(
            FadeIn(edge_word),
...
class IsK33Planar(UtilitiesPuzzleScene):
    def construct(self):
        ...
        fancy_name = brace.get_text(
            "``Complete bipartite graph $K_{3, 3}$''", buff=SMALL_BUFF
        )
        ...
        self.play(
            ReplacementTransform(
                straight_lines, almost_solution_lines, run_time=3, lag_ratio=0.5
            )
        )
        self.wait(2)
...
class TwoKindsOfViewers(PiCreatureScene, UtilitiesPuzzleScene):
    def construct(self):
        ...
        self.pi_creature_thinks(
            pi1, "", bubble_config={"width": 3, "height": 2}, target_mode="thinking"
        )
        ...
        ### Out of thin air
        self.play(
            *list(map(FadeOut, [non_eulers, pi1, pi2, pi1.bubble, know_eulers, dont]))
        )
...
class NewRegionClosedOnlyForNodesWithEdges(UtilitiesPuzzleScene):
    def construct(self):
        ...
        self.remove(region)
        self.play(
            ShowCreation(lines[-1], rate_func=lambda t: smooth(1 - 2 * t * (1 - t)))
        )
        self.add(region, lines, objects)
        self.wait()
        self.remove(region)
...
class LightUpNodes(IntroduceRegions):
    ...
    def get_plus_one_anim(self, count):
        ...
        return UpdateFromAlphaFunc(
            plus_one,
            lambda m, a: ((grow if a < 0.5 else move).update(2 * a % 1)),
            remover=True,
    ...
    def get_lit_vertex_animations(self, vertex, run_time=1, squish_range=(0, 1)):
        ...
        lines = VGroup(
            *[line.copy().rotate(angle) for angle in np.arange(0, 2 * np.pi, np.pi / 4)]
        )
...
class ShowRule(TeacherStudentsScene):
    def construct(self):
        ...
        arrows = VGroup(
            *[
                Arrow(
                    new_edge.get_right(), mob.get_left(), color=WHITE, buff=SMALL_BUFF
                )
...
class ConcludeFiveRegions(LightUpNodes):
    ...
    def describe_start_setup(self):
        ...
        self.play(
            Indicate(self.v_count), *self.get_lit_vertex_animations(self.utilities[0])
        )
        self.play(
            FadeIn(
                full_screen_rect,
    ...
    def count_edges_per_region(self):
        ...
        for region in list(regions[:-1]) + [back_region]:
            ...
            for edge in region.edges:
                ...
                self.play(
                    ShowCreation(edge), FadeOut(count), FadeIn(new_count), run_time=0.5
                )
                count = new_count
...
class EulersFormulaForGeneralPlanarGraph(LightUpNodes, ThreeDScene):
    ...
    def show_creation_of_graph(self):
        ...
        edges = VGroup(
            *[VGroup(*[Line(p1, p2, color=WHITE) for p2 in points]) for p1 in points]
        )
...
class YouGaveFriendsAnImpossiblePuzzle(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(
            "angry", "sassy", "angry", added_anims=[self.teacher.change, "happy"]
        )
        self.wait(2)
