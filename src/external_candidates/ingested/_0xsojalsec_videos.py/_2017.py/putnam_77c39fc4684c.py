# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\putnam.py
...
class IntroducePutnam(Scene):
    ...
    def construct(self):
        ...
        question_groups = VGroup(
            *[
                VGroup(*[OldTexText("%s%d)" % (c, i)) for i in range(1, 7)]).arrange(
        ...
        self.play(
            LaggedStartMap(
                DrawBorderThenFill, out_of_tens, run_time=3, stroke_color=YELLOW
            )
        )
        self.wait()
        self.play(
...
class NatureOf5sAnd6s(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            UP + LEFT,
            FadeIn(self.teacher),
            self.change_students(*["horrified"] * 3, look_at=test),
        )
        self.wait()
...
class TwoDCase(Scene):
    ...
    def ask_about_probability_p3_lands_in_this_arc(self):
        ...
        self.play(
            question.next_to, answer, UP, LARGE_BUFF, Write(equals), FadeIn(answer)
        )
    ...
    def get_point_mob_labels(self):
        point_labels = VGroup(
            *[OldTex("P_%d" % (i + 1)) for i in range(len(self.point_mobs))]
        )
    ...
    def get_point_mob_theta_change_anim(self, point_mob, d_theta):
        ...
        def update_point(point_mob, alpha):
            ...
            point_mob.move_to(
                self.center + self.radius * (np.cos(theta) * RIGHT + np.sin(theta) * UP)
            )
            return point_mob
    ...
    def change_point_mobs_to_angles(self, target_angles, *added_anims, **kwargs):
        ...
        self.change_point_mobs(
            [ta - a for a, ta in zip(angles, target_angles)], *added_anims, **kwargs
        )
    ...
    def have_p3_jump_around_randomly(self, n_jumps, wait_time=0.75, run_time=0):
        for x in range(n_jumps):
            self.change_point_mobs(
                [0, 0, 2 * np.pi * random.random()], run_time=run_time
            )
            self.wait(wait_time)
...
class AverageSizeOfSphericalTriangleSectionSupplement(Scene):
    def construct(self):
        words = OldTexText(
            "Average size of \\\\", "this section", "?", arg_separator=""
        )
...
class ContrastTwoRandomProcesses(TwoDCase):
    ...
    def construct(self):
        ...
        right_circles = VGroup(
            *[VGroup(circle, *point_label_groups[: i + 1]).copy() for i in range(3)]
        )
        left_circles = VGroup(
...
class ThreeDAnswer(Scene):
    def construct(self):
        words = OldTexText(
            "Probability that the tetrahedron contains center:", "$\\frac{1}{8}$"
        )
        words.set_width(FRAME_WIDTH - 1)
        words.to_edge(DOWN)
...
class BrilliantPuzzle(PiCreatureScene):
    ...
    def construct(self):
        ...
        for s1, s2 in adjacent_pairs(self.students):
            arrow = Arrow(
                s1.get_center(), s2.get_center(), path_arc=np.pi / 2, buff=0.8
            )
        ...
        self.play(
            ...
            LaggedStartMap(
                ApplyMethod, students, lambda m: (m.change, "horrified", m.test)
            ),
        )
        self.wait()
        self.play(
            LaggedStartMap(ApplyMethod, students, lambda m: (m.change, "conniving"))
        )
    ...
    def create_pi_creatures(self):
        self.students = VGroup(
            *[
                PiCreature(color=random.choice([BLUE_C, BLUE_D, BLUE_E, GREY_BROWN]))
                .scale(0.25)
                .move_to(3 * vect)
...
class AddedPromoWords(Scene):
    def construct(self):
        words = OldTexText(
            "First", "$2^8$", "vistors get", "$(e^\\pi - \\pi)\\%$", "off"
        )
        words.set_width(FRAME_WIDTH - 1)
        words.to_edge(DOWN)
