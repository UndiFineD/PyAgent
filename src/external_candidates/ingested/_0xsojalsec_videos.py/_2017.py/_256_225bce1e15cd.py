...
class BreakUp2To256(PiCreatureScene):
    ...
    def reorganize_four_billions(self):
        target = VGroup(
            *[
                OldTexText("$\\big($", "4 Billion", "$\\big)$", arg_separator="")
                for x in range(8)
            ]
        )
        ...
        four_billions, to_fade = [
            VGroup(*[se[i] for se in self.subexpressions]) for i in range(2)
        ]
...
class MainBreakdown(Scene):
    ...
    def show_gpu(self):
        ...
        self.play(
            Write(rate_words, run_time=2), get_passing_flash(), Animation(hash_names)
        )
        self.play(get_passing_flash(), Animation(hash_names))
    ...
    def kilo_google(self):
        ...
        google.next_to(
            self.group_of_four_billion_things, UP, buff=LARGE_BUFF, aligned_edge=LEFT
        )
        google.shift(RIGHT)
    ...
    def four_billion_galxies(self):
        ...
        name = OldTexText(
            "Giga", "Galactic \\\\", " Super", " Computer", arg_separator=""
        )
        for word in name:
            word[0].set_color(BLUE)
    ...
    def show_time_scale(self):
        ...
        for fb, words in (fb1, seconds_to_years), (fb2, years_to_eons):
            self.play(
                fb.scale, 1.3, fb.next_to, words, LEFT, fb.set_color, BLUE, Write(words)
            )
            self.wait()
    ...
    def create_four_billion_copies(self, index, mobject):
        ...
        group = VGroup(
            *[
                VGroup(
                    *[mobject.copy().set_height(0.25) for x in range(self.n_group_rows)]
...
class QAndA(PiCreatureScene):
    ...
    def show_powers_of_two(self):
        ...
        dots = VGroup(
            *[
                VGroup(*[Dot() for x in range(rows)]).arrange(DOWN, buff=SMALL_BUFF)
                for y in range(cols)
            ]
        ...
        powers_of_two = VGroup(
            *[Integer(2**i) for i in range(int(np.log2(rows * cols)) + 1)]
        )
        ...
        for i, power_of_two in enumerate(powers_of_two):
            ...
            self.play(
                ...
                Transform(
                    curr_power, power_of_two, rate_func=squish_rate_func(smooth, 0, 0.5)
                ),
            )
        self.wait()
