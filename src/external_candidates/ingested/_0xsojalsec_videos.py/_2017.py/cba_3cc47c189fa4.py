# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\cba.py
...
class LayersOfAbstraction(EnumerableSaveScene):
    ...
    def get_layers(self):
        ...
        pi_chart = VGroup(
            *[Sector(start_angle=a, angle=TAU / 3) for a in np.arange(0, TAU, TAU / 3)]
        )
        ...
        # Layer 2: Algebra
        algebra = VGroup(OldTex("x^2 - 1 = (x + 1)(x - 1)"))
        ...
        vector_spaces = VGroup(
            ...
            OldTex(
                "s(\\textbf{v} + \\textbf{w}) =" "s\\textbf{v} + s\\textbf{w}",
                tex_to_color_map=t2c_map,
            ),
        )
        ...
        inner_product = OldTex("\\langle f, g \\rangle =" "\\int f(x)g(x)dx")
...
class DifferenceOfSquares(Scene):
    def construct(self):
        ...
        self.play(
            ReplacementTransform(squares[:-3].copy().set_fill(opacity=0), new_squares),
            ShowCreation(arrow),
            lag_ratio=0,
        )
...
class Lightbulbs(EnumerableSaveScene):
    def construct(self):
        ...
        top_lines = VGroup(
            *[Line(d.get_center(), top_dot.get_center(), buff=dot_radius) for d in dots]
        )
