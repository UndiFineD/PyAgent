# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\fractal_dimension.py
...
class Norway(Britain):
    CONFIG = {"file_name": "Norway", "mark_paths_closed": False}
...
class KochTest(Scene):
    def construct(self):
        ...
        self.play(koch.scale, 3, koch.get_left(), koch.set_stroke, None, 4)
        self.wait()
...
class ZoomInOnFractal(PiCreatureScene):
    ...
    def zoom_in(self, fractal):
        ...
        for x in range(self.num_zooms):
            ...
            everything.target.shift(grower_target.get_center() - grower.get_center())
            everything.target.scale(grower_target.get_height() / grower.get_height())
...
class WhatAreFractals(TeacherStudentsScene):
    def construct(self):
        self.student_says("But what \\emph{is} a fractal?", index=2, width=6)
        ...
        self.play(
            ...
            FadeIn(picture),
            *[ApplyMethod(pi.look_at, name) for pi in self.get_pi_creatures()],
        )
        self.wait(2)
...
class IntroduceVonKochCurve(Scene):
    ...
    def get_snowflake(self):
        ...
        curves = VGroup(
            *[
                KochCurve(order=self.order, stroke_width=self.stroke_width)
                for x in range(3)
            ]
        )
        for index, curve in enumerate(curves):
            width = curve.get_width()
            curve.move_to((np.sqrt(3) / 6) * width * UP, DOWN)
    ...
    def isolate_one_curve(self, snowflake):
        self.play(
            *[ApplyMethod(curve.shift, curve.get_center() / 2) for curve in snowflake]
        )
        self.wait()
        self.play(snowflake.scale, 2.1, snowflake.next_to, UP, DOWN)
        self.remove(*snowflake[1:])
        return snowflake[0]

    def zoom_in_on(self, curve):
        larger_curve = KochCurve(order=self.order + 1, stroke_width=self.stroke_width)
        ...
        self.play(sub_portion.set_color, YELLOW, rate_func=there_and_back)
        self.wait()
...
class IntroduceSierpinskiTriangle(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(sierp, run_time=2, lag_ratio=0.5))
        self.wait()
        self.play(
            self.pi_creature.change_mode,
            "pondering",
            *[ApplyMethod(submob.shift, submob.get_center()) for submob in sierp],
        )
        self.wait()
        for submob in sierp:
...
class SelfSimilarFractalsAsSubset(Scene):
    ...
    def add_self_similar_fractals(self):
        ...
        self.play(Write(title), ShowCreation(fractals), run_time=3)
    ...
    def add_general_fractals(self):
        ...
        self.play(britain.set_fill, BLUE, 1, britain.set_stroke, None, 0, run_time=2)
...
class ConstrastSmoothAndFractal(Scene):
    ...
    def construct(self):
        ...
        self.play(*[ShowCreation(mob, run_time=3) for mob in britains])
        self.play(
            britains.set_fill,
            BLUE,
            1,
        ...
        self.play(
            ApplyMethod(
                smooth_britain.scale, self.scale_factor, smooth_britain.zoom_point
            ),
            Animation(v_line),
            Animation(background_rectangle),
            ApplyMethod(britain.scale, self.scale_factor, britain.zoom_point),
            Animation(smooth),
            Animation(fractal),
            run_time=10,
        )
...
class InfiniteKochZoom(Scene):
    ...
    def get_curve(self, order):
        ...
        koch_curve.shift(self.left_point - koch_curve.get_points()[0])
        return koch_curve
...
class ExamplesOfDimension(Scene):
    def construct(self):
        labels = VGroup(
            *[OldTexText("%s-dimensional" % s) for s in ("1.585", "1.262", "1.21")]
        )
        fractals = VGroup(
            *[
                Sierpinski(order=7),
        ...
        for fractal, label, vect in zip(fractals, labels, [DOWN, UP, DOWN]):
            ...
            self.play(ShowCreation(fractal), Write(label), run_time=3)
            self.wait()
        self.wait()
...
class FractalDimensionIsNonsense(Scene):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(mathy, "It's 1.585-dimensional!", target_mode="hooray"),
            morty.change_mode,
            "hesitant",
        )
...
class DimensionForNaturalNumbers(Scene):
    def construct(self):
        labels = VGroup(*[OldTexText("%d-dimensional" % d) for d in (1, 2, 3)])
        ...
        self.play(Write(labels[0]), ShowCreation(line))
        self.wait()
...
class OfCourseItsMadeUp(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.teacher_says("""But it's useful!""", target_mode="hooray")
...
class FourSelfSimilarShapes(Scene):
    ...
    def get_titles(self):
        titles = VGroup(*list(map(TexText, ["Line", "Square", "Cube", "Sierpinski"])))
    ...
    def get_shapes(self, titles):
        line = VGroup(Line(LEFT, ORIGIN), Line(ORIGIN, RIGHT))
    ...
    def mention_measurements(self):
        ...
        for label, shape in zip(labels, self.shapes_copy):
            ...
            self.play(Write(label, run_time=1), shape[0].set_color, YELLOW)
            self.wait()
...
class GeneralWordForMeasurement(Scene):
    def construct(self):
        ...
        words = VGroup(*list(map(TexText, ["Length", "Area", "Volume"])))
...
class ImagineShapesAsMetal(FourSelfSimilarShapes):
    def construct(self):
        ...
        for shape, title in zip(shapes, titles):
            self.play(MoveToTarget(title), MoveToTarget(shape))
            self.wait()
        self.wait()

        for shape in shapes:
            self.play(
                shape.scale, 0.5, shape.get_top(), run_time=3, rate_func=there_and_back
            )
        self.wait()
...
class ScaledLineMass(Scene):
    ...
    def construct(self):
        ...
        title.to_edge(UP)
        scaling_factor_label = OldTexText("Scaling factor:", "$\\frac{1}{2}$")
        ...
        mass_scaling_label.next_to(
            scaling_factor_label, DOWN, aligned_edge=LEFT, buff=LARGE_BUFF
        )
        ...
        for submob in shape_copy:
            submob.scale(1.0 / self.break_up_factor)
        ...
        self.play(GrowFromCenter(big_brace), Write(big_brace_text))
        self.wait()

        self.play(shape.copy().replace, shape_copy[0])
        self.remove(*self.get_mobjects_from_last_animation())
        self.add(shape_copy[0])
        self.play(GrowFromCenter(little_brace), Write(little_brace_text))
        self.wait()

        self.play(Write(mass_scaling_label[1], run_time=1))
        self.wait()
        self.play(FadeIn(VGroup(*shape_copy[1:]), lag_ratio=0.5))
        self.wait()
        self.play(Transform(shape_copy.copy(), shape))
        self.wait()

    def get_shape(self):
        return VGroup(Line(LEFT, ORIGIN), Line(ORIGIN, RIGHT)).set_color(BLUE)
...
class ScaledSquareMass(ScaledLineMass):
    ...
    def get_shape(self):
        return VGroup(
            *[
                Square(stroke_width=0, fill_color=BLUE, fill_opacity=0.7).shift(vect)
                for vect in compass_directions(start_vect=DOWN + LEFT)
            ]
        )
...
class DefineTwoDimensional(PiCreatureScene):
    ...
    def add_title(self):
        title = OldTexText(self.dimension, "-dimensional", arg_separator="")
    ...
    def add_width_mass_labels(self):
        ...
        bottom_length = OldTexText(
            "Length: ", "$%s$" % self.scalar, "$L$", arg_separator=""
        )
        bottom_mass = OldTexText(
            "Mass: ", "$%s^%s$" % (self.scalar, self.dimension), "$M$", arg_separator=""
        )
        self.dimension_in_exp = VGroup(*bottom_mass[1][-len(self.dimension) :])
    ...
    def perform_scaling(self):
        ...
        self.play(group.shift, (group.get_top()[1] + self.bottom_shape_buff) * DOWN)
...
class DefineThreeDimensional(DefineTwoDimensional):
    ...
    def get_shape(self):
        return Square(stroke_width=0, fill_opacity=0)
...
class DefineSierpinskiDimension(DefineTwoDimensional):
    ...
    def add_one_third(self):
        equation = OldTexText(
            "$= \\left(\\frac{1}{3}\\right)$", "$M$", arg_separator=""
        )
        ...
        self.distilled_equation = VGroup(self.bottom_mass[1], equation[0]).copy()
    ...
    def isolate_equation(self):
        ...
        self.play(
            *[ApplyMethod(mob.fade, 0.5) for mob in self.get_mobjects()]
            + [Animation(mob) for mob in keepers_copies]
        )
...
class ShowSierpinskiCurve(Scene):
    ...
    def construct(self):
        ...
        for order in range(3, self.max_order + 1):
            self.play(Transform(curve, self.get_curve(order), run_time=2))
            self.wait()
...
class LengthAndAreaOfSierpinski(ShowSierpinskiCurve):
    ...
    def construct(self):
        ...
        for x in range(self.n_iterations):
            ...
            new_sierp = self.get_sierpinski(order=self.sierpinski_start_order + x + 1)
            self.play(
                Transform(curve, new_curve), Transform(sierp, new_sierp), run_time=2
            )
...
class DimensionOfKoch(Scene):
    ...
    def add_labels(self):
        ...
        mass_scaling = OldTexText(
            "Mass scaling factor:", "$\\frac{1}{%d}$" % self.mass_scaling_factor
        )
    ...
    def show_dimension(self):
        ...
        simpler_formula = OldTex(
            str(self.scaling_factor), "^D", "=", str(self.mass_scaling_factor)
        )
        ...
        log_expression = OldTex(
            "D = \\log_%d(%d) %s"
            % (self.scaling_factor, self.mass_scaling_factor, self.dimension_rhs)
        )
        ...
        log_expression.next_to(
            simpler_formula, DOWN, aligned_edge=LEFT, buff=MED_LARGE_BUFF
        )
...
class DimensionOfQuadraticKoch(DimensionOfKoch):
    ...
    def get_curve(self, order):
        curve = self.curve_class(order=order, monochromatic=True)
        curve.set_width(self.koch_curve_width)
    ...
    def set_color_curve_subparts(self):
        n_parts = self.num_subparts
        colored_curve = self.curve_class(order=self.koch_curve_order, stroke_width=1)
        colored_curve.replace(self.curve)
        length = len(colored_curve)
        broken_curve = VGroup(
            *[
                VGroup(
                    *colored_curve[i * length / n_parts : (i + 1) * length / n_parts]
                )
                for i in range(n_parts)
            ]
        )
        ...
        self.play(FadeOut(self.curve), FadeIn(colored_curve))
        self.play(
            ApplyFunction(
...
class SeparateFractals(Scene):
    def construct(self):
        ...
        curves.save_state()
        self.play(*[ApplyFunction(lambda m: break_up(m, 2), curve) for curve in curves])
        self.wait(2)
...
class ShowDiskScaling(Scene):
    ...
    def show_non_self_similar_shapes(self):
        title = OldTexText("Most shapes are not self-similar")
        title.to_edge(UP)
        self.add(title)
        ...
        self.disk = disk
        self.to_fade = VGroup(title, hexagon, blob, britain)
    ...
    def isolate_disk(self):
        ...
        radius = Line(disk.get_center(), disk.get_right(), color=YELLOW)
    ...
    def write_mass_scaling_factor(self):
        mass_scaling = OldTexText("Mass scaling factor: $2^2 = 4$")
...
class WhatDoYouMeanByMass(TeacherStudentsScene):
    def construct(self):
        self.student_says("What do you mean \\\\ by mass?", target_mode="sassy")
...
class BoxCountingScene(Scene):
    ...
    def get_highlighted_boxes(self, vmobject):
        ...
        if vmobject.stroke_width > 0:
            for submob in vmobject.family_members_with_points():
                ...
                points += [submob.point_from_proportion(alpha) for alpha in alphas]
    ...
    def get_boxes(self, points):
        ...
        unique_rounded_points = np.vstack({tuple(row) for row in rounded_points})
...
class BoxCountingWithDisk(BoxCountingScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(corner_rect), FadeIn(counting_label))
        ...
        self.play(
            PiCreatureSays(
                randy, "Is it?", target_mode="sassy", bubble_config={"direction": LEFT}
            )
        )
...
class PlotDiskBoxCounting(GraphScene):
    ...
    def plot_points(self):
        ...
        data_dots = VGroup(
            *[Dot(point, radius=0.05, color=YELLOW) for point in data_points]
        )
...
class FineGridSameAsLargeScaling(BoxCountingScene):
    CONFIG = {"box_width": 0.25 / 6, "scale_factor": 6}
...
class BoxCountingWithBritain(BoxCountingScene):
    ...
    def show_box_counting(self):
        self.add(self.get_grid())
        britain = Britain(stroke_width=2, fill_opacity=0)
    ...
    def show_formula(self):
        ...
        """)
        equation.next_to(corner_rect.get_corner(UP + LEFT), DOWN + RIGHT)
        ...
        simpler_eq.target[-1].next_to(
            simpler_eq.target[-2].get_corner(UP + RIGHT), RIGHT, buff=SMALL_BUFF
        )

        self.play(FadeIn(corner_rect), Write(equation))
        self.wait(2)
        self.play(FadeIn(simpler_eq))
        self.wait()
        self.play(FadeOut(equation), Animation(simpler_eq))
        ...
        self.wait()
        self.play(Transform(rewired_log_expression1, log_expression2, run_time=2))
        self.wait(2)

        self.final_expression = VGroup(simpler_eq, rewired_log_expression1)
...
class IntroduceLogLogPlot(GraphScene):
    ...
    def construct(self):
        ...
        graph = self.graph_function(lambda x: self.y_intercept + self.dimension * x)
        self.remove(graph)
        ...
        self.play(ShowCreation(graph), Animation(expression))
        self.wait()
...
class DefineFractal(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(3)
        self.teacher_says("Kind of...", target_mode="sassy")
        self.play_student_changes(*["pondering"] * 3)
        self.play(*[ApplyMethod(pi.look, DOWN) for pi in self.get_pi_creatures()])
        self.wait(3)


class RoughnessAndFractionalDimension(Scene):
    def construct(self):
        title = OldTexText("Non-integer dimension $\\Leftrightarrow$ Roughness")
        title.to_edge(UP)
        self.add(title)
        ...
        self.play(Blink(randy))
        self.play(Transform(randy, target, run_time=2), Transform(one, new_dim))
        self.wait()
        self.play(Blink(randy))
...
class DifferentSlopesAtDifferentScales(IntroduceLogLogPlot):
    def construct(self):
        ...
        self.x_axis_label_mob[-2].set_color(BLUE)
        self.y_axis_label_mob[-2].set_color(YELLOW)
        self.graph_function(lambda x: 0.01 * (x - 5) ** 3 + 0.3 * x + 3)
        self.remove(self.graph)
        ...
        words.to_edge(RIGHT)
        arrows = VGroup(
            *[Arrow(words.get_left(), self.input_to_graph_point(x)) for x in (1, 7, 12)]
        )
        ...
        self.play(ShowCreation(self.graph))
        self.wait()
        self.play(Write(words), ShowCreation(arrows))
        self.wait()
...
class HoldUpCoilExample(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            "raise_right_hand",
            self.get_teacher().look_at,
            point,
        )
        self.play(*[ApplyMethod(pi.look_at, point) for pi in self.get_students()])
        self.wait(5)
...
class SmoothHilbertZoom(Scene):
    def construct(self):
        hilbert = HilbertCurve(order=7, color=MAROON_B, monochromatic=True)
        hilbert.make_smooth()
        self.add(hilbert)
        ...
        self.play(
            ...
            Transform(two_d_title, one_d_title, rate_func=squish_rate_func(smooth)),
            run_time=3,
        )
        self.wait()
...
class ListDimensionTypes(PiCreatureScene):
    ...
    def construct(self):
        ...
        for text in types:
            self.play(
                Write(text, run_time=1), self.pi_creature.change_mode, "pondering"
            )
        self.wait(3)


class ZoomInOnBritain(Scene):
    CONFIG = {"zoom_factor": 1000}
    ...
    def construct(self):
        ...
        britain.set_points_as_corners(
            reduce(
                lambda a1, a2: np.append(a1, a2, axis=0),
                [
                    anchors[: key_value - num_neighbors_kept : thinning_factor, :],
                    anchors[
                        key_value - num_neighbors_kept : key_value + num_neighbors_kept,
                        :,
                    ],
                    anchors[key_value + num_neighbors_kept :: thinning_factor, :],
                ],
        ...
        self.add(britain)
        self.wait()
        self.play(britain.scale, self.zoom_factor, point, run_time=10)
        self.wait()
...
class FromHandwavyToQuantitative(Scene):
    def construct(self):
        ...
        self.play(PiCreatureSays(randy, "Fractals are rough", target_mode="shruggie"))
        self.play(morty.change_mode, "sassy")
        self.play(Blink(morty))
        self.play(
            PiCreatureSays(
                morty, "We can make \\\\ that quantitative!", target_mode="hooray"
            ),
...
class WhatSlopeDoesLogLogPlotApproach(IntroduceLogLogPlot):
    ...
    def construct(self):
        ...
        data_points = [
            self.coords_to_point(x, cum_sum) for x, cum_sum in zip(x_range, cum_sums)
        ]
        data_dots = VGroup(
            *[Dot(point, radius=0.025, color=YELLOW) for point in data_points]
        )

        words = OldTexText(self.words)
        p1, p2 = [
            data_dots[int(alpha * len(data_dots))].get_center() for alpha in (0.3, 0.5)
        ]
        ...
        self.play(Write(words), morty.change_mode, "speaking")
        self.play(Blink(morty))
        self.wait()
...
class BritainBoxCountHighZoom(BoxCountingWithBritain):
    def construct(self):
        ...
        for x in range(2):
            self.play(britain.scale, 10, britain.point_from_proportion(0.3), run_time=2)
            if x == 0:
                a, b = 0.2, 0.5
            else:
                a, b = 0.25, 0.35
...
class IfBritainWasEventuallySmooth(Scene):
    def construct(self):
        ...
        self.add(britain)
        self.wait()
        self.play(britain.scale, 200, point, run_time=10)
        self.wait()
...
class SmoothBritainLogLogPlot(IntroduceLogLogPlot):
    ...
    def construct(self):
        self.setup_axes()
        self.graph_function(lambda x: (1 + np.exp(-x / 5.0)) * x)
        self.remove(self.graph)

        p1, p2, p3, p4 = [self.input_to_graph_point(x) for x in (1, 2, 7, 8)]
        ...
        slope_lines1.set_points_as_corners([p1, interim_point1, p2])
        slope_lines2.set_points_as_corners([p3, interim_point2, p4])
        ...
        self.play(ShowCreation(self.graph))
        self.wait()
        self.play(ShowCreation(slope_lines1), Write(slope_label1))
        self.wait()
        self.play(ShowCreation(slope_lines2), Write(slope_label2))
        self.wait()
...
class CompareBritainAndNorway(Scene):
    def construct(self):
        ...
        britain = Britain(fill_opacity=0, stroke_width=2)
        ...
        self.play(*list(map(ShowCreation, [norway, britain])), run_time=3)
        self.wait()
        self.play(
            *it.chain(
                *[
                    [mob.set_stroke, None, 0, mob.set_fill, BLUE, 1]
...
class FractalNonFractalFlowChart(Scene):
    def construct(self):
        ...
        yes_arrow = Arrow(is_fractal.get_bottom(), nature.get_top())
        no_arrow = Arrow(is_fractal.get_bottom(), man_made.get_top())
        ...
        for word, arrow, answer in (yes, yes_arrow, nature), (no, no_arrow, man_made):
            self.play(ShowCreation(arrow), Write(word, run_time=1))
