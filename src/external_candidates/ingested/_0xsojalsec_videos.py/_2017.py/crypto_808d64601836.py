# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\crypto.py
import binascii
...
def bit_string_to_mobject(bit_string):
    line = OldTex("0" * 32)
    pre_result = VGroup(*[line.copy() for row in range(8)])
...
class TenDollarBill(VGroup):
    ...
    def __init__(self, **kwargs):
        ...
        pi = Randolph(mode="pondering", color=GREEN_B)
...
class AskQuestion(Scene):
    ...
    def construct(self):
        strings = ["What", "does", "it", "mean   ", "to", "have    ", "a", "Bitcoin?"]
...
class ListOfAttributes(Scene):
    def construct(self):
        ...
        self.play(logo.to_corner, UP + LEFT, Write(digital, run_time=2))
        for building in buildings:
...
class UnknownAuthor(Scene):
    CONFIG = {"camera_config": {"background_image": "bitcoin_paper"}}
    ...
    def construct(self):
        ...
        self.play(Write(question), ShowCreation(arrow))
        self.wait()
...
class DisectQuestion(TeacherStudentsScene):
    ...
    def hold_up_question(self):
        question = OldTexText("What does it mean to", "have", "a", "Bitcoin?")
        question.set_color_by_tex("have", YELLOW)
        ...
        self.play(self.teacher.change, "raise_right_hand", question.restore)
    ...
    def list_topics(self):
        ...
        for word in topics:
            ...
            self.play_student_changes(
                *["confused"] * 3, added_anims=anims, look_at=word
            )
    ...
    def isolate_you(self):
        ...
        self.play(Write(words), ShowCreation(arrow), you.change, "erm", words)
        self.play(Blink(you))
        self.wait()
...
class CryptocurrencyEquation(Scene):
    def construct(self):
        parts = OldTexText("Ledger", "- Trust", "+ Cryptography", "= Cryptocurrency")
...
class ListRecentCurrencies(Scene):
    def construct(self):
        ...
        names = list(
            map(
                TexText,
                ["Bitcoin", "Ethereum", "Ripple", "Litecoin", "Ethereum Classic"],
            )
        )
...
class Hype(TeacherStudentsScene):
    def construct(self):
        ...
        phrases = list(
            map(TexText, ["I want some!", "I'll get rich, right?", "Buy them all!"])
        )
...
class NoCommentOnSpeculation(TeacherStudentsScene):
    def construct(self):
        ...
        graph.set_points_as_corners(
            [time * RIGHT + price * UP for time, price in zip(times, prices)]
        )
        ...
        group.add(cross)
        self.play(group.shift, FRAME_WIDTH * RIGHT, self.teacher.change, "happy")
        self.wait()

        self.student_says("But...what are they?", index=0, target_mode="confused")
        self.wait(2)
        self.play(
        ...
        self.wait()
        self.play(VGroup(mining_graphic, mining_cross).shift, FRAME_WIDTH * RIGHT)
...
class LedgerScene(PiCreatureScene):
    ...
    def get_ledger(self):
        ...
        rect = Rectangle(width=self.ledger_width, height=self.ledger_height)
    ...
    def add_line_to_ledger(self, string_or_mob):
        ...
        if self.enumerate_lines:
            ...
            mob.add_to_back(num)
        mob.next_to(items[-1], DOWN, buff=MED_SMALL_BUFF, aligned_edge=LEFT)
    ...
    def animate_payment_addition(self, *args, **kwargs):
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(*it.chain(*line)), run_time=1))
    ...
    def get_network(self):
        ...
        lines = VGroup(
            *[
                Line(
                    VGroup(pi1, pi1.label),
                    VGroup(pi2, pi2.label),
                    buff=MED_SMALL_BUFF,
                    stroke_width=2,
                )
...
class LayOutPlan(LedgerScene):
    ...
    def ask_question(self):
        ...
        self.play(DrawBorderThenFill(btc), Write(group[1], run_time=2))
        self.wait()
        self.play(group.scale, 0.7, group.next_to, ORIGIN, RIGHT, group.to_edge, UP)
    ...
    def become_skeptical(self):
        ...
        self.play(*[ApplyMethod(pi.change_mode, "sassy") for pi in creatures])
        for k in range(3):
            self.play(
                *[
                    ApplyMethod(creatures[i].look_at, creatures[k * (i + 1) % 4])
                    for i in range(4)
                ]
            )
        self.wait(2)
...
class IntroduceLedgerSystem(LedgerScene):
    ...
    def add_ledger(self):
        ...
        self.play(
            Write(ledger),
            *[ApplyMethod(pi.change, "pondering", ledger) for pi in self.pi_creatures],
        )
    ...
    def tally_it_all_up(self):
        ...
        for name in names:
            ...
            if name == "you":
                verb = verb[:-1]
            result = OldTexText(verb, "\\$%d" % abs(amount))
        ...
        self.play(GrowFromCenter(brace), FadeIn(tally_up))
        self.play(
            LaggedStartMap(FadeIn, results),
            *[ApplyMethod(pi.change, "happy") for pi in creditors]
            + [ApplyMethod(pi.change, "plain") for pi in debtors],
        )
        self.wait()
        ...
        self.wait()
        self.play(ReplacementTransform(debtor_cash, creditor_cash))
        self.wait(2)
...
class IntroduceSignatures(LedgerScene):
    ...
    def add_transactions(self):
        transactions = VGroup(
            *[self.add_payment_line_to_ledger(*payment) for payment in self.payments]
        )
    ...
    def add_signatures(self):
        signatures = VGroup(
            *[get_cursive_name(payments[0].capitalize()) for payments in self.payments]
        )
...
class AskHowDigitalSignaturesArePossible(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(ReplacementTransform(bits.copy(), bits_copy, path_arc=np.pi / 2))
...
class DescribeDigitalSignatures(LedgerScene):
    ...
    def reorganize_pi_creatures(self):
        ...
        creature_groups = VGroup(
            *[VGroup(pi, pi.label).scale(1.7) for pi in self.pi_creatures]
        )
    ...
    def generate_key_pairs(self):
        ...
        public_key_strings = [
            bin(256 + ord(name[0].capitalize()))[3:] for name in names
        ]
        private_key_strings = [bin(hash(name))[2:10] for name in names]
        public_keys, private_keys = [
            VGroup(*[OldTexText(key_name + ":", " $%s\\dots$" % key) for key in keys])
        ...
        self.play(ReplacementTransform(VGroup(VGroup(public.copy())), public_keys))
        self.play(ReplacementTransform(VGroup(VGroup(private.copy())), private_keys))
        self.wait()
    ...
    def keep_secret_key_secret(self):
        ...
        locks = VGroup(*[lock.copy().next_to(rect, LEFT, SMALL_BUFF) for rect in rects])
    ...
    def show_handwritten_signatures(self):
        ...
        rect = SurroundingRectangle(lines, color=GREY_B, buff=MED_SMALL_BUFF)
        document = VGroup(rect, lines)
        documents = VGroup(*[document.copy() for x in range(2)])
        ...
        self.play(FadeOut(self.title), LaggedStartMap(FadeIn, documents, run_time=1))
    ...
    def show_digital_signatures(self):
        rect = SurroundingRectangle(
            VGroup(self.public_keys[0], self.private_key_rects[0], self.locks[0])
        )
        ...
        arrows = VGroup(
            *[
                Arrow(
                    rect.get_corner(UP), sig.get_bottom(), tip_length=0.15, color=WHITE
                )
        ...
        words = VGroup(
            *list(
                map(TexText, ["Different messages", "Completely different signatures"])
            )
        )
    ...
    def show_signing_functions(self):
        sign = OldTexText(
            "Sign(", "Message", ", ", "sk", ") = ", "Signature", arg_separator=""
        )
        ...
        self.play(ReplacementTransform(message.copy(), VGroup(sign.message)))
        self.wait()
        self.play(ReplacementTransform(private_key.copy(), sign.sk))
        self.wait()
        self.play(
            ReplacementTransform(
                VGroup(sign.sk, sign.message).copy(), VGroup(sign.signature)
            )
        )
        self.wait()
        ...
        self.play(
            *[
                FadeIn(part)
                for part in verify
                if part not in [verify.message, verify.signature, verify.pk, verify[-1]]
            ]
        )
        self.wait()
        self.play(
            ReplacementTransform(sign.message.copy(), verify.message),
            ReplacementTransform(sign.signature.copy(), verify.signature),
        )
        self.wait()
        self.play(ReplacementTransform(public_key.copy(), VGroup(verify.pk)))
        self.wait()
...
class TryGuessingDigitalSignature(Scene):
    def construct(self):
        ...
        self.add(verify)
        self.play(GrowFromCenter(brace), FadeIn(zeros, lag_ratio=0.5, run_time=3))
        self.wait()
        for n in range(2**10):
            last_row = zeros[-1]
            binary = bin(n)[2:]
...
class WriteTwoTo256PossibleSignatures(Scene):
    def construct(self):
        words = OldTexText("$2^{256}$", "possible\\\\", "signatures")
        words.scale(2)
...
class FeelConfidentWithVerification(PiCreatureScene):
    ...
    def show_verification(self):
        ...
        self.play(ReplacementTransform(verify.copy(), rhs))
        self.wait()
        self.play(self.pi_creature.change, "happy")
        self.play(Write(sk_group))
        self.play(ShowCreation(lock_box), DrawBorderThenFill(lock, run_time=1))
        self.wait(2)
...
class IncludeTransactionNumber(LedgerScene):
    ...
    def add_signed_payment(self):
        ...
        self.play(signature.restore, self.alice.change, "raise_left_hand")
        self.wait()
    ...
    def fail_to_sign_new_transaction(self):
        ...
        self.play(self.bob.change, "confused", Write(q_marks))
        self.play(ShowCreation(cross))
        self.wait()
    ...
    def add_ids(self):
        ...
        self.play(Write(numbers), self.alice.change, "thinking")
...
class ProtocolWithDigitalSignatures(InitialProtocol):
    def construct(self):
        ...
        new_item = self.get_new_item("Only signed transactions are valid")
...
class CharlieRacksUpDebt(SignedLedgerScene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, lines, run_time=3, lag_ratio=0.25))
        self.play(
            *[
        ...
        self.play(self.charlie.shift, FRAME_X_RADIUS * RIGHT, rate_func=running_start)
        self.play(
            *[
    ...
    def construct(self):
        ...
        self.play(
            PiCreatureBubbleIntroduction(
                randy,
                "You don't \\emph{actually} \\\\" + "need to settle up $\\dots$",
...
class LedgerWithInitialBuyIn(SignedLedgerScene):
    ...
    def everyone_buys_in(self):
        ...
        moneys = VGroup(*[OldTex("\\$100") for pi in self.pi_creatures])
        ...
        self.play(
            LaggedStartMap(ApplyMethod, moneys, lambda m: (m.restore,)),
            self.charlie.change,
            "raise_right_hand",
            self.you.change,
            "raise_right_hand",
    ...
    def add_initial_lines(self):
        ...
        for name in self.get_names():
            new_line = OldTexText(
                name.capitalize(), "get" if name == "you" else "gets", "\\$100"
            )
            new_line.set_color_by_tex(name.capitalize(), self.get_color_from_name(name))
    ...
    def add_charlie_payments(self):
        ...
        new_lines = VGroup(
            *[self.add_payment_line_to_ledger(*payment) for payment in payments]
        )
    ...
    def point_out_charlie_is_broke(self):
        charlie_lines = VGroup(
            *[VGroup(*self.ledger.content[i][1:5]) for i in (3, 5, 6, 7)]
        )
        rects = VGroup(*[SurroundingRectangle(line) for line in charlie_lines])
    ...
    def running_balance(self):
        ...
        signatures = VGroup(*[self.ledger.content[i][5] for i in (5, 6, 7)])
        ...
        lines = VGroup(*list(map(TexText, ["\\$100", "\\$50", "\\$0", "Overdrawn"])))
        ...
        for line, c_line in zip(lines, charlie_lines):
            ...
            line.shift((c_line.get_center() - line.get_center())[1] * UP)
            arrow = Arrow(c_line, line)
            arrows.add(arrow)

        self.pi_creatures.remove(self.alice, self.charlie)
        self.play(FadeOut(signatures), FadeIn(balance))
        self.play(
...
class RemovedConnectionBetweenLedgerAndCash(TeacherStudentsScene):
    def construct(self):
        ledger = Rectangle(height=2, width=1.5, color=WHITE)
        ...
        self.add(group)
        self.play(Animation(group), self.teacher.change, "raise_right_hand")
        self.play(arrow.shift, 2 * UP, arrow.set_fill, None, 0)
        self.play(ledger.shift, LEFT, cash.shift, RIGHT)
        self.play_student_changes(
            *["pondering"] * 3,
            look_at=ledger,
...
class RenameToLedgerDollars(LedgerScene):
    ...
    def jump_in_to_middle(self):
        ...
        self.ledger.content.add(*[point.copy() for x in range(103)])
    ...
    def add_payments_in_dollars(self):
        ...
        self.play(LaggedStartMap(FadeIn, lines, run_time=4, lag_ratio=0.3))
        self.wait()
...
class ExchangeCashForLedgerDollars(LedgerScene):
    ...
    def add_title(self):
        ...
        title = OldTexText("Exchange", "LD", "for", "\\$\\$\\$")
    ...
    def add_bob_pays_alice_line(self):
        line = self.add_payment_line_to_ledger("Bob", "Alice", 10)
        line.save_state()
        line.scale(0.01)
...
class BitcoinIsALedger(Scene):
    ...
    def add_btc_to_ledger(self):
        ...
        self.play(DrawBorderThenFill(logo))
        self.wait()
        self.play(Write(arrow), Write(ledger))
        self.wait()

        self.btc_to_ledger = group

    def add_currency_to_tx_history(self):
        equation = OldTexText("Currency", "=", "Transaction history")
        equation.set_color_by_tex("Currency", BITCOIN_COLOR)
        equation.shift(
            self.btc_to_ledger[1].get_center() - equation[1].get_center() + 2 * UP
        )
    ...
    def get_ledger(self):
        ...
        lines = VGroup(*[Line(LEFT, RIGHT) for x in range(8)])
...
class TransitionToDistributedLedger(DistributedLedgerScene):
    ...
    def distribute_ledger(self):
        ...
        self.play(ReplacementTransform(VGroup(ledger), distribute_ledgers))
        self.play(
            *[
        ...
        self.play(group.set_height, FRAME_HEIGHT - 2, group.center)
        self.wait(2)
    ...
    def broadcast_transaction(self):
        ...
        payment_copies = VGroup(
            *[payment.copy().next_to(pi, UP) for pi in self.pi_creatures]
        )
        payment = payment_copies[0]

        self.play(
            self.alice.change, "raise_right_hand", payment, Write(payment, run_time=2)
        )
        self.wait()
        self.play(
            ReplacementTransform(
    ...
    def ask_about_ledger_consistency(self):
        ledgers = VGroup(*[pi.ledger for pi in self.pi_creatures])
        ...
        question.to_edge(UP)
        arrows = VGroup(
            *[Arrow(question.get_bottom(), ledger.get_top()) for ledger in ledgers]
        )
...
class BobDoubtsBroadcastTransaction(DistributedLedgerScene):
    ...
    def bob_receives_transaction(self):
        ...
        payment = OldTexText("Alice", "pays", "Bob", "10 LD")
        payment.set_color_by_tex("Alice", self.alice.get_color())
        payment.set_color_by_tex("Bob", self.bob.get_color())
        payment.set_color_by_tex("LD", YELLOW)
        ...
        self.play(payment.scale, 0.3, payment.next_to, bob.ledger[-1], DOWN, SMALL_BUFF)

    def bob_tries_to_pay_charlie(self):
        bob, charlie = self.bob, self.charlie
        chralie_group = VGroup(charlie, charlie.label, charlie.ledger)

        self.play(
            PiCreatureSays(bob, "Did you hear that?", target_mode="sassy"),
...
class TrustComputationalWorkSupplement(Scene):
    def construct(self):
        words = OldTexText("Main tool: ", "Cryptographic hash functions")
...
class FraudIsInfeasible(Scene):
    def construct(self):
        words = OldTexText("Fraud", "$\\Leftrightarrow$", "Computationally infeasible")
    ...
    def construct(self):
        ...
        self.teacher_says(
            ...
            added_anims=[
                ...
                for pi, mode in zip(self.students, ["hooray", "sad", "erm"])
            ],
        )
        self.wait()
        self.play(
            ...
            RemovePiCreatureBubble(
                self.teacher, target_mode="hooray", look_at=lightbulb
            ),
        )
        self.play_student_changes(
            *["pondering"] * 3, added_anims=[LaggedStartMap(FadeIn, lightbulb)]
        )
        self.play(
            LaggedStartMap(
                ApplyMethod,
...
class IntroduceSHA256(Scene):
    ...
    def introduce_evaluation(self):
        ...
        for message in messages:
            lhs = OldTexText("SHA256", "(``", message, "'') =", arg_separator="")
        ...
        for brace, text in brace_text_pairs:
            if brace is digest_brace:
                self.play(LaggedStartMap(FadeIn, digest, run_time=4, lag_ratio=0.05))
                self.wait()
        ...
        for new_group in groups[1:]:
            ...
            self.play(
                ...
                message_brace.stretch_to_fit_width,
                new_message.get_width(),
                message_brace.next_to,
                new_message,
                DOWN,
            ...
            self.play(
                Transform(
                    digest, new_digest, run_time=2, lag_ratio=0.5, path_arc=np.pi / 2
                )
            )
        ...
        new_sha_text = OldTexText("Cryptographic", "hash function")
    ...
    def issue_challenge(self):
        ...
        self.play(
            Transform(
                self.digest, new_digest, run_time=2, lag_ratio=0.5, path_arc=np.pi / 2
            ),
            Transform(self.digest_text, desired_output_text),
        )
        self.play(FadeOut(self.message), Transform(self.message_text, q_marks))
        self.wait()

    def shift_everything_down(self):
        everything = VGroup(*self.get_top_level_mobjects())
        self.play(everything.scale, 0.85, everything.to_edge, DOWN)
    ...
    def guess_and_check(self):
        ...
        for new_group in groups[1:]:
            ...
            self.play(Transform(group[1], new_group[1], run_time=1, lag_ratio=0.5))
...
class SHA256ToProofOfWork(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(Write(sha, run_time=1), self.teacher.change, "raise_right_hand")
        self.play(ShowCreation(arrow))
...
class IntroduceNonceOnTrasactions(LedgerScene):
    ...
    def hash_with_nonce(self):
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(special_word, nonce_brace, nonce)))
        self.wait()
        self.play(
            nonce.next_to,
            ledger.content,
        ...
        decomposed_ledger = VGroup(
            *[m for m in ledger.family_members_with_points() if not m.is_subpath]
        )
        self.play(ShowCreation(arrow), FadeIn(sha))
        self.play(
            LaggedStartMap(
                ApplyMethod,
                decomposed_ledger,
        ...
        self.play(
            LaggedStartMap(
                Transform, decomposed_ledger.copy(), lambda m: (m, point), run_time=1
            )
        )
        ...
        self.add(digest)
        self.play(GrowFromCenter(zeros_brace), Write(zeros_words, run_time=1))
        self.play(LaggedStartMap(ApplyMethod, zeros, lambda m: (m.set_color, YELLOW)))
        self.wait(2)
    ...
    def guess_and_check(self):
        ...
        for x in range(1, 13):
            ...
            self.play(Transform(self.nonce, nonce, run_time=1 if x == 1 else 0.3))
            self.play(Transform(self.digest, digest, run_time=1, lag_ratio=0.5))
        self.wait()
        self.play(self.nonce.restore)
        self.play(self.digest.restore, lag_ratio=0.5, run_time=2)
    ...
    def name_proof_of_work(self):
        ...
        arrow = Arrow(
            words.get_top(), self.nonce.get_bottom(), color=WHITE, tip_length=0.15
        )
    ...
    def change_ledger(self):
        ...
        self.play(dot.move_to, new_digest, dot.set_fill, None, 0)
        self.play(
            Transform(
                self.digest,
                new_digest,
...
class ShowSomeBroadcasting(DistributedLedgerScene):
    def construct(self):
        ...
        lines.add(*[line.copy().rotate(np.pi) for line in lines])
        ...
        for pi in self.pi_creatures:
            ...
            self.play(
                *it.chain(
                    *[[dot.move_to, dot.ledger, dot.set_fill, None, 0] for dot in dots]
                )
            )
...
class IntroduceBlockChain(Scene):
    ...
    def setup(self):
        ls = LedgerScene()
        self.names = [name.capitalize() for name in ls.get_names()]
        self.name_colors = [ls.get_color_from_name(name) for name in self.names]
    ...
    def divide_ledger_into_blocks(self):
        blocks = VGroup(*[self.get_block() for x in range(3)])
    ...
    def show_proofs_of_work(self):
        ...
        self.play(Transform(proofs_of_work, new_proofs_of_work, lag_ratio=0.5))
        self.play(ShowCreation(arrows), Write(sha_words), run_time=2)
        ...
        for group in signatures, proofs_of_work:
            ...
            self.wait()
        self.play(proofs_of_work.restore, FadeOut(sha_words))
    ...
    def chain_blocks_together(self):
        ...
        prev_hashes = VGroup(*[block.prev_hash for block in blocks])

        prev_hashes.add(prev_hashes[-1].copy().shift(block_spacing).fade(1))
    ...
    def propagate_hash_change(self):
        ...
        for block, prev_hash in zip(self.blocks, prev_hashes[1:]):
            ...
            self.play(ShowCreation(rect))
            self.play(MoveToTarget(rect), prev_hash.set_color, RED)

    def redo_proof_of_work(self):
        proofs_of_work = VGroup(*[block.proof_of_work for block in self.blocks])
        ...
        for proof_of_work, prev_hash in zip(proofs_of_work, hashes):
            num_pow_group = VGroup(
                *[Integer(random.randint(10**9, 10**10)) for x in range(50)]
            )
    ...
    def write_block_chain(self):
        ...
        self.play(ShowCreation(cross), Write(block_chain))
        self.wait(2)
    ...
    def get_block(self):
        ...
        h_line1, h_line2 = [
            Line(rect.get_left(), rect.get_right()).shift(
                0.3 * rect.get_height() * vect
            )
            for vect in (UP, DOWN)
        ]
        ...
        for x in range(2):
            hashes = [
                hash("%d %d" % (seed, self.transaction_counter)) for seed in range(3)
            ]
            payment = OldTexText(
        ...
        proof_of_work.move_to(VGroup(h_line2, VectorizedPoint(rect.get_bottom())))

        prev_hash = OldTexText("Prev hash")
        prev_hash.scale(0.8)
        prev_hash.set_color(self.prev_hash_color)
        prev_hash.move_to(VGroup(h_line1, VectorizedPoint(rect.get_top())))
...
class DistributedBlockChainScene(DistributedLedgerScene):
    ...
    def get_distributed_ledgers(self):
        ...
        for pi in self.pi_creatures:
            ...
            block_chain.next_to(VGroup(pi, pi.label), vect, SMALL_BUFF)
    ...
    def get_block_chain(self):
        blocks = VGroup(*[self.get_block() for x in range(self.n_blocks)])
        ...
        for b1, b2 in zip(blocks, blocks[1:]):
            ...
            target_point = interpolate(b2.get_left(), b2.get_corner(UP + LEFT), 0.8)
...
class FromBankToDecentralizedSystem(DistributedBlockChainScene):
    ...
    def show_block_chains(self):
        ...
        self.play(
            ...
            *[ApplyMethod(pi.change, "pondering", pi.ledger) for pi in creatures],
        )
        self.wait()
    ...
    def set_aside_everything(self):
        ...
        LedgerScene.add_payment_line_to_ledger(
            self,
            "Alice",
            "Bob",
            "40",
        ...
        self.play(
            ...
            everything.scale,
            0.1,
            everything.to_corner,
            DOWN + LEFT,
        )
        self.play(Write(ledger), digital_signature.next_to, ORIGIN, RIGHT)
        self.wait(2)
...
class IntroduceBlockCreator(DistributedBlockChainScene):
    ...
    def broadcast_transactions(self):
        ...
        for from_name, to_name, amount in payment_parts:
            ...
            payment = OldTexText(from_name, verb, to_name, "%d LD" % amount)
            payment.set_color_by_tex("LD", YELLOW)
            for name in self.get_names():
                payment.set_color_by_tex(
                    name.capitalize(), self.get_color_from_name(name)
                )
            payment.scale(0.7)
        ...
        payment_targets.next_to(self.block_creator_labels, RIGHT, MED_LARGE_BUFF)
    ...
    def collect_transactions(self):
        ...
        self.play(
            FadeIn(block), MoveToTarget(payments), creator.change, "raise_right_hand"
        )
        self.wait()
        block.add(payments)
    ...
    def add_block_reward(self):
        ...
        new_transaction = OldTexText(
            self.block_creator_labels[0].get_tex(), "gets", "10 LD"
        )
        ...
        self.play(MoveToTarget(payments), Write(new_transaction))
    ...
    def broadcast_block(self):
        ...
        anims.append(Transform(VGroup(self.block), block_target_group))
        anims.append(Broadcast(self.block, n_circles=4))
        anims.append(FadeOut(self.hash_group))
        anims.append(ApplyMethod(self.block_creators[0].change, "happy"))
        ...
        self.play(
            *it.chain(
                arrow_creations,
                [
                    ApplyMethod(pi.change, "hooray", pi.block_chain.get_right())
                    for pi in self.pi_creatures
                ],
            )
        )
...
class MiningIsALottery(IntroduceBlockCreator):
    ...
    def create_pi_creatures(self):
        ...
        miners = VGroup(*[PiCreature(color=GREY) for n in range(self.n_miners)])
        miners.scale(0.5)
    ...
    def make_guesses(self):
        ...
        self.play(
            ...
            *[
                ApplyMethod(
                    VGroup(self.blocks[i], self.arrows[i], nonces[i], digests[i]).fade,
                    0.7,
                )
        ...
        self.play(*[ApplyMethod(loser.change, "angry", winner) for loser in losers])
        self.wait(2)
    ...
    def get_payments(self, miner):
        ...
        for line in lines:
            line.set_height(0.5)
        lines.arrange(DOWN, buff=SMALL_BUFF, aligned_edge=LEFT)
...
class TwoBlockChains(DistributedBlockChainScene):
    ...
    def defer_to_longer(self):
        randy = self.randy
        self.n_blocks -= 1
        block_chains = VGroup(self.block_chain, self.get_block_chain().scale(1.5))
        ...
        self.play(
            ...
            Broadcast(corner),
            ApplyMethod(
                chain.restore, rate_func=squish_rate_func(smooth, 0.3, 0.7), run_time=3
            ),
        )
        self.play(Write(conflicting), *list(map(ShowCreation, arrows)))
        self.wait()
        ...
        self.to_fade = VGroup(conflicting, arrows, longer_chain_rect, checkmark)
    ...
    def break_tie(self):
        ...
        block_chains.target.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        block_chains.target.next_to(randy, UP)
        block_chains.target.to_edge(LEFT)

        self.play(MoveToTarget(block_chains), FadeOut(to_fade), run_time=1)
        ...
        self.play(
            ShowCreation(rect), Write(checkmark), randy.change, "happy", arrow_block
        )
        self.wait(2)
...
class ReplaceCentralAuthorityWithWork(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(cross))
        central.add(cross)
        self.play(central.shift, DOWN, Write(work))
        self.wait()
...
class AskAboutTrustingWork(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(3)
        self.teacher_says("Well, let's try\\\\ fooling someone", target_mode="speaking")
        self.wait(2)
...
class DoubleSpendingAttack(DistributedBlockChainScene):
    ...
    def send_to_bob(self):
        ...
        block.target.replace(self.bob.block_chain.blocks[-1], stretch=True)
        arrow = self.bob.block_chain.arrows[-1].copy()
        VGroup(arrow, block.target).next_to(
            self.bob.block_chain.blocks[-1], RIGHT, buff=0
        )

        self.play(MoveToTarget(block), self.alice.change, "happy")
        self.play(ShowCreation(arrow))
        self.wait()
    ...
    def get_fraud_block(self):
        ...
        for word, vect, color in tuples:
            ...
            mob.next_to(
                block.get_edge_center(vect), -vect, buff=0.06 * block.get_height()
            )
            content.add(mob)
        ...
        payment.set_color_by_tex("LD", YELLOW)
        payments = VGroup(OldTex("\\vdots"), payment, OldTex("\\vdots"))
...
class AliceRacesOtherMiners(DoubleSpendingAttack):
    ...
    def initialize_characters(self):
        ...
        miners = VGroup(*[PiCreature(color=GREY) for x in range(self.n_miners)])
        ...
        for miner in miners:
            ...
            for mob, vect in (proof_of_work, DOWN), (prev_hash, UP):
                ...
                mob.next_to(
                    block.get_edge_center(vect), -vect, buff=0.05 * block.get_height()
                )
                block.add(mob)
            block.proof_of_work = proof_of_work
            block.prev_hash = prev_hash
    ...
    def find_proof_of_work(self):
        ...
        self.play(Indicate(fraud_block.proof_of_work), self.alice.change, "hooray")
        self.wait()
        ...
        VGroup(arrow, block.target).next_to(chain.blocks[-1], RIGHT, buff=0)
    ...
    def receive_broadcast_from_other_miners(self):
        ...
        self.play(Indicate(proof_of_work), winner.change, "hooray")
        ...
        self.play(
            ...
            *[
                MoveToTarget(
                    mover, run_time=3, rate_func=squish_rate_func(smooth, 0.3, 0.8)
                )
                for mover in movers
            ],
        )
        ...
        self.wait(2)
        self.play(self.alice.change, "pondering", FadeIn(self.fraud_block))
        self.proofs_of_work.add(self.fraud_block.proof_of_work)
    ...
    def race_between_alice_and_miners(self):
        ...
        for winner in winners:
            ...
            VGroup(arrow, block_copy.target).next_to(prev_block, RIGHT, buff=0)
            anims = [
    ...
    def wait(self, time=1):
        self.play(Animation(VGroup(*self.foreground_mobjects)), run_time=time)
...
class WhenToTrustANewBlock(DistributedBlockChainScene):
    def construct(self):
        ...
        word = words[0]
        self.play(FadeIn(word), ShowCreation(arrow))
        for new_word in words[1:]:
            kwargs = {"run_time": 3, "rate_func": squish_rate_func(smooth, 0.7, 1)}
            self.add_new_block(
                Transform(word, new_word, **kwargs),
                ApplyMethod(arrow.set_color, new_word.get_color(), **kwargs),
            )
        self.wait(2)
...
class MainIdeas(Scene):
    def construct(self):
        ...
        ideas.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
...
class VariableProofOfWork(WhenToTrustANewBlock):
    ...
    def change_requirement(self):
        digest = self.digest
        requirement = OldTexText("Must start with \\\\", "60", "zeros")
    ...
    def add_more_miners(self):
        ...
        target[1].scale(0.5, about_point=miner.get_right())
        copies = VGroup(*[target.copy() for x in range(self.n_miners - 1)])
    ...
    def change_challenge(self, n_zeros):
        ...
        self.play(
            ...
            Transform(digest, new_digest, lag_ratio=0.5),
...
class CompareBlockTimes(Scene):
    def construct(self):
        ...
        self.add(title, h_line)
        self.play(FadeIn(examples[0]), DrawBorderThenFill(logos[0]))
        self.wait()
        self.play(
            *[LaggedStartMap(FadeIn, VGroup(*group[1:])) for group in (examples, logos)]
        )
        self.wait(2)
...
class BlockRewards(Scene):
    def construct(self):
        ...
        rewards.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        rewards.next_to(h_line, DOWN)
        for reward in rewards:
            reward[1].set_color(YELLOW)

        footnote = OldTexText("$^*$ Extrapolating from the 25 BTC reward period")
        footnote.scale(0.5)
        footnote.to_corner(DOWN + RIGHT)

        self.play(LaggedStartMap(FadeIn, rewards, run_time=4, lag_ratio=0.5))
...
class TransactionFeeExample(PiCreatureScene):
    def construct(self):
        ...
        signature = OldTexText("$\\langle$Alice's digital signature$\\rangle$")
        signature.set_color(BLUE_C)
        ...
        incentive_words = OldTexText("Incentivizes miner \\\\ to include")
        incentive_words.next_to(rect, DOWN, buff=1.5)
        incentive_words.shift(2 * RIGHT)
        arrow = Arrow(incentive_words.get_top(), rect.get_bottom(), buff=MED_LARGE_BUFF)
        ...
        self.play(Write(signature))
        self.play(ShowCreation(rect), alice.change_mode, "happy")
        self.wait()
        self.play(
            Write(incentive_words), ShowCreation(arrow), alice.change, "pondering"
        )
        self.wait(2)
...
class ShowBitcoinBlockSize(LedgerScene):
    ...
    def construct(self):
        ...
        payments_rect = SurroundingRectangle(payments, color=WHITE, buff=MED_LARGE_BUFF)
...
class CurrentAverageFees(Scene):
    def construct(self):
        fees = OldTexText(
            "Current average fees: ", "$\\sim 0.0013$ BTC", "$\\approx$", "\\$3.39"
        )
...
class TopicsNotCovered(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(title.restore, self.teacher.change_mode, "raise_right_hand")
        for topic in topics:
            self.play_student_changes(
                "confused",
                "thinking",
    ...
    def interpolate_mobject(self, alpha):
        ...
        if alpha < 1.0 / 3:
            ...
            if not self.changed_symbols_yet:
                new_left = random.choice(exchange.cryptocurrencies).copy()
                new_left.move_to(exchange.right)
                new_right = random.choice(exchange.currencies).copy()
...
class ShowManyExchanges(Scene):
    ...
    def construct(self):
        ...
        exchange = VGroup(
            *[
                Arrow(p1, p2, path_arc=np.pi, buff=MED_LARGE_BUFF)
                for p1, p2 in [(LEFT, RIGHT), (RIGHT, LEFT)]
            ]
        ).set_color(WHITE)
        exchanges = VGroup(
            *[
                VGroup(*[exchange.copy() for x in range(3)]).arrange(
                    RIGHT, buff=2 * LARGE_BUFF
                )
                for y in range(3)
            ]
        ...
        for t in ProgressDisplay(times):
            ...
            self.update_frame(self.extract_mobject_family_members(exchanges))
...
class ProtocolLabs(PiCreatureScene):
    def construct(self):
        ...
        self.wait()
        self.play(logo.scale, 0.5, logo.to_corner, UP + LEFT, ShowCreation(screen_rect))
