...
class PoolTableReflections(InteractiveScene):
    def construct(self):
        ...
        table_group2[2].add_updater(
            lambda m: m.move_to(get_reflection(ball.get_center()))
        )
        table_group3[2].add_updater(
            lambda m: m.move_to(get_reflection(ball.get_center(), dims=[0, 1]))
        )
        ...
        def line_to_trajectory(line, n_reflections=2):
            ...
            if n_reflections == 1:
                trajectory.set_points_as_corners(
                    [line.get_start(), p1, get_reflection(line.get_end(), [1])]
                )
            else:
                trajectory.set_points_as_corners(
                    [
        ...
        self.play(
            ...
            UpdateFromFunc(
                trajectory, lambda m: m.become(line_to_trajectory(straight_line))
            ),
            UpdateFromFunc(
                trajectory, lambda m: m.become(line_to_trajectory(straight_line))
            ),
        ...
        def n_to_lines(n):
            return VGroup(
                *(get_diamond_line(x, n - x) for x in range(70, 0, -10))
            ).set_stroke(YELLOW)
        ...
        for line in lines:
            ...
            traj.line = line
            traj.add_updater(
                lambda t: t.become(line_to_trajectory(t.line)).set_color(BLUE)
            )
            trajs.add(traj)
