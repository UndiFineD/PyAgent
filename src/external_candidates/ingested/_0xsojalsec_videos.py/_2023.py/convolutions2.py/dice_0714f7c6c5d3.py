...
class SumAlongDiagonal(InteractiveScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            LaggedStartMap(FadeOut, VGroup(morty, morty.bubble, morty.bubble.content))
        )
        ...
        marginal_labels = VGroup(
            *(VGroup(*(bar[2] for bar in group)).copy() for group in bar_groups)
        )
        ...
        for n, square in enumerate(grid):
            ...
            prob = DecimalNumber(
                margin1.get_value() * margin2.get_value(), num_decimal_places=3
            )
        ...
        self.play(
            ...
            self.frame.animate.reorient(12, 65, 0)
            .move_to([-0.48, 0.37, 0.77])
            .set_height(9.43),
            run_time=3,
        )
        self.add(full_table, *bars_3d)
        self.play(
            self.frame.animate.reorient(43, 66, 0)
            .move_to([-0.06, 0.21, -0.29])
            .set_height(10.59),
            run_time=7,
        )
        self.wait()
        ...
        self.play(
            ...
            self.frame.animate.reorient(4, 65, 0)
            .move_to([0.64, 0.8, 0.69])
            .set_height(10.59),
        ...
        for diagonal in diagonals:
            ...
            rect.round_corners()
            rects.add(rect)
        VGroup(rects, diagonals).rotate(
            -45 * DEGREES, about_point=diagonals.get_center()
        )
        ...
        # Show 3d grid again
        self.play(
            self.frame.animate.reorient(27, 66, 0)
            .move_to([-0.16, 1.41, 0.77])
            .set_height(9.36),
            FadeIn(bars_3d),
        ...
        self.play(
            self.frame.animate.reorient(36, 46, 0)
            .move_to([-0.56, 0.55, 1.22])
            .set_height(7.71),
        ...
        self.play(
            self.frame.animate.reorient(40, 61, 0)
            .move_to([1.69, 0.33, -0.73])
            .set_height(12.96),
        ...
        self.play(
            self.frame.animate.reorient(40, 85, 0)
            .move_to([3.05, 1.93, 0.77])
            .set_height(14.93),
    ...
    def show_repeated_samples(
        ...
        markers = VGroup(
            *(VGroup(VectorizedPoint(bucket.get_bottom())) for bucket in buckets)
        )
...
class ConvolveDiscreteDistributions(SumAlongDiagonal):
    ...
    def construct(self):
        ...
        for n in [*range(6), *range(4, -1, -1)]:
            ...
            self.play(
                FadeIn(rect), FadeOut(last_rect), MoveToTarget(bar_groups), run_time=0.5
            )
            self.wait(0.5)
            last_rect = rect
        ...
        x_args = VGroup(
            *(
                Tex(f"({x}) = {np.round(dist1[x - 1], 2)}").next_to(
                    PX, RIGHT, SMALL_BUFF
                )
                for x in range(1, 7)
            )
        ...
        self.play(
            LaggedStartMap(
                FadeOut, VGroup(func_group, func_group_Y, func_group_XY, x_args[-1])
            )
        )
        ...
        self.wait()
        self.play(
            conv_rect.animate.become(
                SurroundingRectangle(conv_def["*"], buff=0.05, stroke_width=1)
            )
        )
        self.wait()
        ...
        for part in ex_rhs[re.compile(R"P[^+]*P[^+]*\)")]:
            self.play(
                example_box.animate.replace(part, stretch=True)
                .scale(1.1)
                .set_stroke(width=2),
                run_time=0.5,
            )
        ...
        y_arrow.save_state()
        self.play(
            s_arrow.animate.scale(0.75)
            .rotate(90 * DEGREES)
            .next_to(xeq1, LEFT, SMALL_BUFF),
            y_arrow.animate.scale(0.75)
            .rotate(-90 * DEGREES)
            .next_to(xeq1, RIGHT, SMALL_BUFF),
        )
        self.wait()
        self.play(
        ...
        summands = VGroup(
            *(example[Rf"P_X({x}) \cdot P_Y({4 - x})"] for x in range(1, 7))
        )
        plusses = example["+"]
        ...
        self.play(
            ...
            sum_bar_group.animate.scale(0.5).next_to(
                bar_groups.target, DOWN, buff=0.75, aligned_edge=LEFT
            ),
        )
...
class ShowConvolutionOfLists(SumAlongDiagonal):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn, func_words, shift=0.5 * DOWN, scale=0.5, lag_ratio=0.5
            ),
...
class RepeatedDiscreteConvolutions(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(5):
            ...
            self.wait()
            self.play(
                new_bars.animate.move_to(top_bars, DL).set_anim_args(
    ...
    def get_bar_value_labels(self, bars, min_value=1):
        values = VGroup(
            *(Integer(x + min_value, font_size=16) for x in range(len(bars)))
        )
    ...
    def flip_bar_group(self, bar_group):
        ...
        self.play(
            ...
            *(
                MaintainPositionRelativeTo(VGroup(value, prob), bar)
