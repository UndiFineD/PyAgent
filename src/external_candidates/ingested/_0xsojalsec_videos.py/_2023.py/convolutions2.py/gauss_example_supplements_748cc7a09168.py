# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\convolutions2\gauss_example_supplements.py
...
class MultipleBellishCurves(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            words.animate.match_width(plot.saved_state).next_to(
                plot.saved_state, UP, SMALL_BUFF
            ),
            GrowArrow(arrow),
        ...
        self.play(
            LaggedStart(
                MoveToTarget(l0),
                VGroup(arrow, question).animate.next_to(
                    l0.target, RIGHT, aligned_edge=DOWN
                ),
...
class LastFewVideos(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, thumbnails, shift=UP, lag_ratio=0.7, run_time=4)
        )
        self.wait()
...
class AddingCopiesOfAVariable(InteractiveScene):
    def construct(self):
        ...
        self.play(
            TransformMatchingTex(expr, expr2), FadeIn(lim, 0.25 * UP, time_span=(1, 2))
        )
        self.wait()
...
class PreviewExplicitCalculation(InteractiveScene):
    def construct(self):
        ...
        conv_eq = Tex(
            R"[{f} * {g}](s) = \int_{-\infty}^\infty {f}(x){g}(s - x)dx", **tex_kw
        )
        ...
        gauss_color = RED
        gaussian1 = Tex(
            R"f(x) = {1 \over \sigma_1 \sqrt{2 \pi}} e^{-x^2 / 2 \sigma_1^2}", **tex_kw
        )
        gaussian2 = Tex(
            R"g(y) = {1 \over \sigma_2 \sqrt{2 \pi}} e^{-y^2 / 2 \sigma_2^2}", **tex_kw
        )
        gaussian1.match_y(conv_eq)
        gaussian1.to_edge(RIGHT)
        gaussian2.next_to(gaussian1, DOWN, LARGE_BUFF)
        gauss_arrow = Arrow(
            gauss_word.get_bottom(), gaussian1.get_top() + SMALL_BUFF * UP
        )
            ...
            LaggedStart(
                *(
                    TransformMatchingShapes(
                        conv_eq[9:13].copy(), full_expr[R"\int_{-\infty}^\infty"][0]
                    ),
                    TransformMatchingShapes(
                    ...
                    TransformMatchingShapes(
                        gaussian1[13:].copy(), full_expr[R"e^{-x^2 / 2\sigma_1^2}"][0]
                    ),
                    TransformMatchingShapes(
                        gaussian2[13:].copy(),
                        full_expr[R"e^{-(s-x)^2 / 2\sigma_2^2}"][0],
                    ),
    ...
    def construct(self):
        ...
        self.play(
            TransformMatchingTex(
                rhs2.copy(), rhs3, key_map={"x^2 + y^2": "r^2"}, run_time=1
            ),
...
class SliceLineAnnotations(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeOut, VGroup(words, conv, conv_eq, area, s_eq, decimal_rhs)
            ),
...
class YIntegralAnnotations(InteractiveScene):
    def construct(self):
        # Setup
        kw = dict(t2c={"{s}": RED, "{y}": YELLOW, "{x}": BLUE, R"\text{Area}": TEAL})
        integrals = VGroup(
            Tex(
                R"\text{Area} = \int_{\text{-}\infty}^\infty e^{-{x}^2} \cdot e^{-{y}^2} \, d{y}",
                **kw,
            ),
            Tex(
                R"\text{Area} = \int_{\text{-}\infty}^\infty e^{-({s} / \sqrt{2})^2} \cdot e^{-{y}^2} \, d{y}",
                **kw,
            ),
            Tex(
                R"\text{Area} = e^{-({s} / \sqrt{2})^2} \int_{\text{-}\infty}^\infty e^{-{y}^2} \, d{y}",
                **kw,
            ),
        ...
        s_rect1 = SurroundingRectangle(
            integrals[1][R"e^{-({s} / \sqrt{2})^2}"], buff=0.05
        )
        s_rect2 = SurroundingRectangle(
            integrals[2][R"e^{-({s} / \sqrt{2})^2}"], buff=0.05
        )
        ...
        conv_eqs = VGroup(
            ...
            Tex(
                R"[f * g]({s}) = \text{Area} / \sqrt{2} = e^{-{s}^2 / 2} \sqrt{\pi} / \sqrt{2}",
                **kw,
            ),
            Tex(
                R"[f * g]({s}) = \text{Area} / \sqrt{2} = e^{-{s}^2 / 2} \sqrt{\pi \over 2}",
                **kw,
            ),
        )
...
class OscillatingGraphValue(InteractiveScene):
    def construct(self):
        ...
        v_line = always_redraw(
            lambda: axes.get_v_line_to_graph(get_s(), graph, line_func=Line).set_stroke(
                WHITE, 1
            )
        )
        ...
        for _ in range(2):
            self.play(
                s_tracker.animate.set_value(3), rate_func=there_and_back, run_time=24
            )
...
class ShowGaussianConvolutionsAsEquations(InteractiveScene):
    def construct(self):
            ...
            *(
                NumberPlane(
                    ...
                    faded_line_style=dict(
                        stroke_width=1, stroke_opacity=0.2, stroke_color=GREY_D
                    ),
                )
                for x in range(6)
            )
        ...
        # Equations
        kw = dict(
            font_size=30, t2c={"x": BLUE, "y": YELLOW, "{s}": TEAL, R"\sigma": RED}
        )
        equations = VGroup(
            Tex(R"e^{-x^2}", **kw),
            Tex(R"e^{-y^2}", **kw),
            ...
            Tex(
                R"\frac{1}{\sqrt{2}\sigma \sqrt{2\pi}} e^{-\frac{1}{2} {s}^2 / 2 \sigma^2}",
                **kw,
            ),
        )
            ...
            *(
                NumberPlane(
                    ...
                    background_line_style=dict(
                        stroke_color=GREY_A, stroke_width=1, stroke_opacity=0.5
                    ),
                    faded_line_style=dict(
                        stroke_color=GREY_A, stroke_width=1, stroke_opacity=0.25
                    ),
                    width=5,
                    height=5,
                )
...
class WhatsTheBigdeal(TeacherStudentsScene):
    def construct(self):
        ...
        # But are they?
        common_words = (
            stds[2].bubble.content["Normal distributions\nare very common"][0].copy()
        )
...
class StepsToProof(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn,
                VGroup(steps[0]["Step 1:"], steps[1]["Step 2:"]),
                shift=0.5 * UP,
                lag_ratio=0.5,
            )
        )
...
class HerschelMaxwellWords(InteractiveScene):
    def construct(self):
        ...
        ideas = VGroup(
            ...
            TexText(
                R"Why $\pi$ is in this formula", font_size=60, t2c={R"\pi": YELLOW}
            ),
        )
        for idea in ideas:
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "pondering", "hooray", "well", look_at=self.teacher.eyes
            ),
        )
        self.play(
            self.change_students(
...
class DrawQRCode(InteractiveScene):
    def construct(self):
        ...
        code = SVGMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/convolutions2/gauss_example_supplements/images/SubstackQR2.svg"
        )
        code.remove(*code[:2])
        ...
        self.play(
            ...
            Write(
                code, stroke_color=RED_E, stroke_width=2, lag_ratio=0.005, run_time=5
            ),
        )
        self.wait()
