# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\convolutions2\continuous.py
import scipy.stats
...
class TransitionToContinuousProbability(InteractiveScene):
    def construct(self):
        ...
        bars = axes.get_riemann_rectangles(
            graph, dx=1, x_range=(0, 6), input_sample_type="right"
        )
        ...
        bound_labels = Tex("ab")
        bound_labels[0].add_updater(
            lambda m: m.move_to(v_lines[0], DOWN).shift(0.5 * DOWN)
        )
        bound_labels[1].add_updater(
            lambda m: m.move_to(v_lines[1], DOWN).shift(0.5 * DOWN)
        )
        bound_labels.add_updater(
            lambda m: m.set_opacity(sub_area_opacity_tracker.get_value())
        )
        ...
        self.wait()
        self.play(
            TransformMatchingStrings(long_name, short_name, lag_ratio=0.01, run_time=1)
        )
        self.wait()
...
class CompareFormulas(InteractiveScene):
    def construct(self):
        ...
        discrete_diagram = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/convolutions2/dice/DiscreteDistributionSum.png"
        )
        ...
        graphs = VGroup(
            all_axes[0].get_graph(wedge_func, use_smoothing=False).set_stroke(BLUE),
        ...
        self.play(
            TransformMatchingTex(
                disc_formula.copy(), cont_formula, lag_ratio=-0.001, path_arc=-0.1 * PI
            )
        )
        self.wait()
        ...
        graphs = VGroup(
            *(
                ...
                for axes, func, color in zip(
                    all_axes, self.get_pdfs(), self.graph_colors
                )
            )
        )
    ...
    def animate_samples(
        ...
        if len(plots) > 2:
            self.play(
                LaggedStart(
                    Transform(
                        dots[0].copy(), dots[2].copy().set_opacity(0.5), remover=True
                    ),
                    Transform(
                        dots[1].copy(), dots[2].copy().set_opacity(0.5), remover=True
                    ),
...
class SampleTwoNormals(RepeatedSamplesFromContinuousDistributions):
    ...
    def construct(self):
        ...
        if self.annotations:
            ...
            area = always_redraw(
                lambda: axes.get_area_under_graph(graph, bound_tracker.get_value())
            )
    ...
    def get_normal_parameter_labels(
        self, plot, mean, sigma, font_size=18, color=GREY_A
    ):
...
class AddTwoGammaDistributions(RepeatedSamplesFromContinuousDistributions):
    ...
    def construct(self):
        ...
        # Initial samples
        self.repeated_samples(
            plots, 40, animate=False, time_between_samples=0.1, time_before_fade=0.5
        )
        ...
        graph_groups = VGroup(
            *(VGroup(plot[1], label).copy() for plot, label in zip(plots, graph_labels))
        )
        ...
        equation = VGroup(
            lp.copy(),
            graph_groups.target[0],
            rp.copy(),
            Tex("*", **kw),
            lp.copy(),
            graph_groups.target[1],
            rp.copy(),
            Tex("=", **kw),
        ...
        self.repeated_samples(
            plots, 50, animate=False, time_between_samples=0.1, time_before_fade=0.5
        )
...
class Convolutions(InteractiveScene):
    ...
    def setup(self):
        ...
        # Add f(x)
        f_graph = self.f_graph = f_axes.get_graph(
            self.f, x_range=(x_min, x_max, self.f_graph_x_step)
        )
        ...
        self.get_s = get_s
        g_graph = self.g_graph = g_axes.get_graph(
            lambda x: 0, x_range=(x_min, x_max, self.g_graph_x_step)
        )
        ...
        if self.g_is_rect:
            ...
            g_axes.bind_graph_to_func(
                g_graph, lambda x: self.g(get_s() - x), jagged=self.jagged_product
            )
        ...
        fg_axes.bind_graph_to_func(
            pos_graph, lambda x: np.clip(prod_func(x), 0, np.inf), **kw
        )
        fg_axes.bind_graph_to_func(
            neg_graph, lambda x: np.clip(prod_func(x), -np.inf, 0), **kw
        )
    ...
    def get_conv_s_indicator(self):
        ...
        conv_s_indicator.add_updater(
            lambda m: m.shift(conv_axes.c2p(get_s(), 0) - g_axes.c2p(get_s(), 0))
        )
...
class ProbConvolutions(Convolutions):
    ...
    def construct(self):
        ...
        f_axes, g_axes, fg_axes, conv_axes = self.all_axes
        f_graph, g_graph, prod_graphs, conv_graph = (
            self.f_graph,
            self.g_graph,
            self.prod_graphs,
            self.conv_graph,
        )
        f_label, g_label, fg_label, conv_label = (
            self.f_label,
            self.g_label,
            self.fg_label,
            self.conv_label,
        ...
        x_brace, y_brace, s_brace = braces = VGroup(
            *(Brace(term, UP, SMALL_BUFF) for term in [x_term, y_term, s_term])
        )
        ...
        x_line = always_redraw(
            lambda: f_axes.get_v_line_to_graph(
                get_x(), f_graph, line_func=Line, color=WHITE
            )
        )
        y_line = always_redraw(
            lambda: g_axes.get_v_line_to_graph(
        ...
        self.play(
            *(ReplacementTransform(int_rect.copy(), x_rect) for x_rect in x_rects)
        )
        self.wait()
        ...
        self.play(
            LaggedStart(
                *map(FadeOut, [x_line, x_dot, y_line, y_dot, *equation, *braces])
            ),
            lag_ratio=0.2,
        )
        ...
        self.play(
            LaggedStart(
                ...
                TransformFromCopy(VGroup(*f_label, *g_label), fg_label),
                FadeOut(fg_rect),
                run_time=2,
            )
        )
    ...
    def highlight_several_regions(self, highlighted_xs=None, s=0, reference=None):
        ...
        for x in highlighted_xs:
            ...
            label = Tex(
                Rf"p_X({x_tex}) \cdot p_Y({diff_tex})",
...
class MovingAverageAsConvolution(Convolutions):
    ...
    def construct(self):
        ...
        top_line.add_updater(
            lambda l: l.put_start_and_end_on(*self.g_graph.get_anchors()[4:6])
        )
        side_line.add_updater(
            lambda l: l.put_start_and_end_on(*self.g_graph.get_anchors()[2:4])
        )
        ...
        rect.set_gloss(1)
        rect.add_updater(
            lambda m: m.set_width(g_axes.x_axis.unit_size / get_k1(), stretch=True)
        )
        rect.add_updater(
            lambda m: m.set_height(g_axes.y_axis.unit_size * get_k2(), stretch=True)
        )
        ...
        self.play(
            Write(area_label, stroke_color=WHITE), ShowCreation(arrow), FadeIn(rect)
        )
        self.wait()
...
class RepeatedConvolution(MovingAverageAsConvolution):
    ...
    def construct(self):
        ...
        def update_prod_graph(prod_graph):
            ...
            prod_samples = np.array(
                [f_sample * self.g(s - x) for f_sample, x in zip(f_samples, x_samples)]
            )
    ...
    def create_convolution(self, x_samples, f_samples, g_samples, conv_axes):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                endpoint_dot,
                lambda m, a: m.move_to(conv_graph.get_end()).set_opacity(min(6 * a, 1)),
            ),
    ...
    def swap_labels(self, n, conv_graph):
        ...
        if hasattr(self, "rescaled_graphs"):
            anims.append(
                self.rescaled_graphs.animate.set_stroke(width=0.5, opacity=0.5)
            )
...
class MovingAverageOfRectFuncs(Convolutions):
    ...
    def construct(self):
        ...
        self.conv_graph.set_height(
            0.5 * g_axes.y_axis.get_height(), about_edge=DOWN, stretch=True
        )
...
class RectConvolutionsNewNotation(MovingAverageOfRectFuncs):
    def construct(self):
        ...
        all_axes = axes1, axes2, axes3 = VGroup(
            *(Axes((x_min, x_max, 0.5), (0, 5), width=3.75, height=4) for x in range(3))
        )
        all_axes.arrange(RIGHT, buff=LARGE_BUFF, aligned_edge=DOWN)
        for axes in all_axes:
            axes.x_axis.add_numbers(font_size=12, num_decimal_places=1)
        axes2.y_axis.add_numbers(
            font_size=12, num_decimal_places=0, direction=DL, buff=0.05
        )
        ...
        rect_defs = VGroup(
            self.get_rect_func_def(), *(self.get_rect_k_def(k) for k in k_range)
        )
        ...
        conv_labels = VGroup(
            ...
            Tex(
                R"\big[\text{rect} * \text{rect}_3 * \text{rect}_5 * \text{rect}_7 \big](x)"
            ),
        )
        ...
        for n in range(2):
            ...
            self.play(
                Transform(
                    left_conv.copy(),
                    conv_labels[n + 1][0][1:r],
                    remover=True,
                    path_arc=-PI / 3,
                ),
                Transform(
                    rect_defs[2][:5].copy(),
                    conv_labels[n + 1][0][r + 1 : r + 6],
                    remover=True,
                    path_arc=-PI / 3,
                ),
