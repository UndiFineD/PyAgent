# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\convolutions2\diagonal_slices.py
...
class Introduce3DGraph(InteractiveScene):
    ...
    def construct(self):
        ...
        f_axes, g_axes = all_axes = VGroup(
            *(Axes((-2, 2), (0, 1, 0.5), width=5, height=2) for n in range(2))
        )
        ...
        f_label, g_label = func_labels = VGroup(
            Tex("f(x)", font_size=36), Tex("g(y)", font_size=36)
        )
        ...
        x_label.add_updater(
            lambda m: m.set_value(get_x())
            .next_to(x_indicator, DOWN, SMALL_BUFF)
            .fix_in_frame()
        )
        y_label.add_updater(
            lambda m: m.set_value(get_y())
            .next_to(y_indicator, DOWN, SMALL_BUFF)
            .fix_in_frame()
        )
        ...
        self.play(
            ...
            self.frame.animate.reorient(-27, 78, 0)
            .move_to([0.36, -0.62, 0.71])
            .set_height(5.66)
            .set_anim_args(run_time=4),
        )
        surface.always_sort_to_camera(self.camera)
        self.play(
            self.frame.animate.reorient(68, 77, 0)
            .move_to([-0.13, -1.12, -0.27])
            .set_height(9.37),
            run_time=5,
        )
        ...
        self.play(
            self.frame.animate.reorient(3, 83, 0)
            .move_to([1.09, -0.82, -0.54])
            .set_height(6.91),
            run_time=4,
        )
        self.wait()
        self.play(
            self.frame.animate.reorient(89, 95, 0)
            .move_to([0.63, -2.19, 2.56])
            .set_height(9.41),
            run_time=4,
        )
        self.wait()
        self.play(
            self.frame.animate.reorient(69, 75, 0)
            .move_to([1.07, -1.37, -0.19])
            .set_height(7.64),
            run_time=5,
        )
...
class DiagonalSlices(Introduce3DGraph):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            self.frame.animate.reorient(-22, 74, 0)
            .move_to([-0.12, -0.16, 0.04])
            .set_height(5.45),
            run_time=3,
        )
        self.wait()
        ...
        self.play(
            ...
            self.frame.animate.reorient(-45, 75, 0)
            .move_to([0.18, -0.14, 0.49])
            .set_height(3.0),
            run_time=6,
        )
        self.play(
            s_tracker.animate.set_value(-2.0),
            self.frame.animate.reorient(-5, 66, 0)
            .move_to([-0.03, -0.18, 0.14])
            .set_height(6.35),
            run_time=20,
        )
        self.play(
            s_tracker.animate.set_value(2.0),
            self.frame.animate.reorient(16, 73, 0)
            .move_to([-0.03, -0.18, 0.14])
            .set_height(6.35),
            run_time=15,
        )
    ...
    def add_surface_group(self, axes, get_s):
        ...
        surface = axes.get_graph(
            lambda x, y: self.f(x) * self.g(y), resolution=self.graph_resolution
        )
...
class SyncedSlices(DiagonalSlices):
    ...
    def construct(self):
        ...
        self.play(
            s_tracker.animate.set_value(-1.5),
            self.frame.animate.reorient(-28, 77, 0)
            .move_to([0.17, -0.28, 0.25])
            .set_height(5.29),
            run_time=20,
        )
        self.play(
            s_tracker.animate.set_value(1.5),
            self.frame.animate.reorient(-10, 73, 0)
            .move_to([0.15, -0.28, 0.22])
            .set_height(5.04),
            run_time=20,
        )
        self.play(
            s_tracker.animate.set_value(-0.5),
            self.frame.animate.reorient(-39, 79, 0)
            .move_to([0.15, -0.28, 0.21])
            .set_height(5.04),
            run_time=20,
        )
...
class SyncedSlicesExpAndRect(SyncedSlices):
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.frame.animate.reorient(-38, 72, 0)
            .move_to([0.53, -0.5, 0.34])
            .set_height(6.75),
            run_time=15,
        )
        self.play(
            s_tracker.animate.set_value(-1.5),
            self.frame.animate.reorient(-45, 80, 0)
            .move_to([-1.75, 0.32, 0.2])
            .set_height(5.04),
            run_time=10,
        )
        self.play(
            s_tracker.animate.set_value(-3.0),
            self.frame.animate.reorient(-28, 73, 0)
            .move_to([-1.75, 0.32, 0.2])
            .set_height(5.04),
            run_time=10,
        )
        self.play(
            s_tracker.animate.set_value(1.0),
            self.frame.animate.reorient(-39, 65, 0)
            .move_to([0.35, -0.53, 0.06])
            .set_height(6.38),
            run_time=20,
        )
...
class SyncedSlicesGaussian(SyncedSlices):
    ...
    def construct(self):
        ...
        self.play(
            s_tracker.animate.set_value(1.5),
            self.frame.animate.reorient(-7, 61, 0)
            .move_to([0.5, -0.06, 0.4])
            .set_height(3.85),
            run_time=10,
        )
        self.play(
...
class AnalyzeStepAlongDiagonalLine(DiagonalSlices):
    ...
    def construct(self):
        ...
        VGroup(brace, segment).scale(1 / 3, about_point=center).rotate(
            -45 * DEGREES, about_point=center
        )
        ...
        dx_line = DashedLine(
            segment.get_corner(UL), segment.get_corner(UR), dash_length=0.01
        )
        dy_line = DashedLine(
            segment.get_corner(UR), segment.get_corner(DR), dash_length=0.01
        )
        ...
        self.play(
            ...
            self.frame.animate.reorient(-42, 79, 0)
            .move_to([-0.15, 0.73, 1.18])
            .set_height(3.50),
            run_time=2,
        )
        ...
        for x in np.arange(-2, 2, dx):
            ...
            if not (-2 <= y <= 2):
                continue
            rect = Rectangle(
                width=math.sqrt(2) * x_unit * dx, height=self.f(x) * self.g(y) * z_unit
            )
...
class RotateGaussianSlice(RotationalSymmetryOfGaussian):
    def construct(self):
        ...
        surface.add_updater(
            lambda m: m.set_clip_plane(clip_vect.get_center(), -s_tracker.get_value())
        )
        ...
        # Ambient rotation
        self.play(
            frame.animate.reorient(-35, 59, 0)
            .move_to([-0.0, 0.22, 0.4])
            .set_height(5.94),
            run_time=8,
        )
        self.play(
