    ...
    def construct(self):
        ...
        self.play(
            self.change_students(
                "happy", "erm", "hesitant", look_at=self.screen, lag_ratio=0.2
            )
        )
        self.wait(3)
        ...
        self.wait(2)
        self.play(
            self.change_students("maybe", "hesitant", "well", look_at=self.screen)
        )
        self.wait(4)
...
class AlreadyCoveredConvolutions(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            morty.change("guilty"),
            self.change_students(
                "hesitant", "pondering", "hesitant", look_at=morty.eyes
            ),
        )
        self.wait()
        self.look_at(
...
class PauseAndPonder(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.change_students(
                "thinking", "pondering", "pondering", look_at=self.screen
            )
        )
        self.wait(5)
...
class IsntThatOvercomplicated(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            morty.says(
                "It's fun!", mode="hooray", bubble_config=dict(width=2, height=1.5)
            ),
            stds[1].debubble(),
        )
        self.play(
            self.change_students("hesitant", "tease", "pondering", look_at=self.screen)
        )
        self.wait(3)
        ...
        for brace, func, color in zip(all_braces, funcs, colors):
            ...
            if brace is expr_braces[1]:
                ...
                graph = axes.get_graph(
                    func, x_range=(*x_range, 0.025), use_smoothing=False
                )
...
class IndicatingRectangle(InteractiveScene):
    def construct(self):
        self.play(
            FlashAround(Rectangle(9, 3), run_time=2, time_width=1.5, stroke_width=8)
        )
...
class Sqrt2Correction(InteractiveScene):
    def construct(self):
        # Test
        words = TexText(
            R"Technically, this shape is $\sqrt{2}$ times wider than this shape"
        )
...
class SimpleQuestion(InteractiveScene):
    def construct(self):
        ...
        self.play(
            TransformFromCopy(equations[0], equations[1]), run_time=2, lag_ratio=0.02
        )
        self.wait(2)
...
class ComplainAboutCalculation(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            morty.change("raise_right_hand", look_at=self.screen),
            self.change_students(
                "hesitant", "pondering", "guilty", look_at=self.screen
            ),
        )
        self.wait(2)
        self.play(
            self.change_students(
                "horrified", "pleading", "concentrating", look_at=2 * UR
            )
        )
...
class OrdinaryApproach(InteractiveScene):
    def construct(self):
        ...
        brace = Brace(
            equations[0], RIGHT, tex_string=R"\underbrace{\qquad\qquad\qquad}"
        )
        brace.stretch(1.5, 1)
