# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\SoME3\main.py
...
class ThumbnailProgression(InteractiveScene):
    def construct(self):
        ...
        thumbnails = Group(
            *(ImageMobject(os.path.join(thumbnail_dir, name)) for name in names)
        )
        framed_thumbnails = Group(
            *(
                Group(SurroundingRectangle(tn, buff=0).set_stroke(WHITE, 3), tn)
        ...
        self.play(
            LaggedStart(
                *(FadeIn(covers[indices.index(n)]) for n in range(len(indices))),
                run_time=4,
                lag_ratio=0.1,
            )
        )
        ...
        self.play(
            LaggedStart(
                *(Restore(cover, path_arc=30 * DEGREES) for cover in covers[1:])
            ),
            lag_ratio=0.01,
            run_time=2,
        )
        ...
        winner_ftns = Group(
            *(framed_thumbnails[names.index(winner)] for winner in winners)
        )
...
class MetaThumbnail(InteractiveScene):
    def construct(self):
        ...
        ftns = Group(
            *(
                Group(SurroundingRectangle(tn, buff=0).set_stroke(GREY_A, 2), tn)
                for tn in thumbnails
            )
        )
...
class GenerallyGreat(InteractiveScene):
    def construct(self):
        ...
        qualities2 = VGroup(*map(Text, ["Pretty good"] * len(audiences))).set_color(
            YELLOW
        )
        qualities3 = VGroup(
    ...
    def construct(self):
        ...
        pieces = Group(
            *(
                Group(
                    Text(name),
                    ImageMobject(os.path.join(thumbnail_dir, name), height=4),
                ).arrange(UP)
                for name in names
            )
        )
...
class Credits(InteractiveScene):
    def construct(self):
        ...
        logo = SVGMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/SoME3/main/images/janestreet_logo/LOGO_stacked_w_registration_white.svg"
        )
