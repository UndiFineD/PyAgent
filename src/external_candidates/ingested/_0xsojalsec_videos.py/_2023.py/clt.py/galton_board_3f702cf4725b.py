# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\clt\galton_board.py
...
class GaltonBoard(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(1, self.n_rows + 1):
            split_labels = VGroup(
                *(get_peg_label(n, k, split=True) for k in range(n + 1))
            )
            unsplit_labels = VGroup(
                *(get_peg_label(n, k, split=False) for k in range(n + 1))
            )
            anims = [
    ...
    def get_ball_arrows(self, ball, labels, sub_labels=[], colors=[RED, BLUE]):
        ...
        for vect, color, label in zip([LEFT, RIGHT], colors, labels):
            arrow = Vector(
                0.5 * self.spacing * vect, tip_width_ratio=3, stroke_color=color
            )
    ...
    def add_single_clink_sound(self, time_offset=0, gain=-20):
        self.add_sound(
            sound_file=self.clink_sound.replace(
                "click", "click" + str(random.randint(1, 12))
            ),
    ...
    def drop_n_balls(self, n, pegs, buckets, lr_factor=1, sound=False):
        ...
        if sound:
            ...
            for time, traj in zip(start_times, trajs):
                self.add_falling_clink_sounds(
                    traj[1], time + 0.00 * random.random(), gain=-30
                )
