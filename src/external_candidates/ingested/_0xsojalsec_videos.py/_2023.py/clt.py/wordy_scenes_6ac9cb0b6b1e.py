...
class ErdosKac(InteractiveScene):
    def construct(self):
        ...
        stacks = VGroup(
            *(VGroup(VectorizedPoint(number_line.n2p(x - 0.5))) for x in range(1, 20))
        )
        self.add(stacks)
        ...
        sd = math.sqrt(mean)
        curve = FunctionGraph(
            lambda x: 10 * gauss_func(x, mean, sd), x_range=(0, 10, 0.25)
        )
...
class PopulationHeights(InteractiveScene):
    ...
    def get_pi(self, height=1):
        pi = Randolph(
            mode=random.choice(["happy", "hooray", "pondering", "tease"]), height=1
        )
...
class SumsOfSizeFive(TeacherStudentsScene):
    def construct(self):
        ...
        def get_sum():
            values = [
                Integer(random.choice([-1, 1]), include_sign=True) for x in range(5)
            ]
            lhs = Tex(R"\text{Sum} = ")
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "confused", "thinking", "hesitant", look_at=self.screen
            ),
        )
        self.wait(2)
...
class WhatElseDoYouNotice(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.play(
            self.change_students("erm", "pondering", "tease", look_at=self.screen)
        )
        self.wait(2)
...
class ExampleQuestion(TeacherStudentsScene):
    def construct(self):
        ...
        dice = VGroup(
            *(
                DieFace(value, fill_color=BLUE_E, stroke_width=1, dot_color=WHITE)
        ...
        num.add_updater(
            lambda m: m.set_value(sum(die_values[: int(n_tracker.get_value())]))
        )
...
class ExampleQuestionNoPiCreatures(InteractiveScene):
    def construct(self):
        ...
        for x in range(5):
            ...
            self.play(
                ShowIncreasingSubsets(dice, run_time=2), FadeOut(old_dice, run_time=0.5)
            )
            self.wait()
...
class ThreeAssumptions(InteractiveScene):
    def construct(self):
        ...
        iid_words = TexText(
            R"i.i.d. $\rightarrow$ independent and identically distributed"
        )
        ...
        gen_words = TexText(
            R"These can be relaxed (see Lindeberg CLT, $\alpha$-mixing, etc.)"
        )
        gen_words.scale(0.7)
