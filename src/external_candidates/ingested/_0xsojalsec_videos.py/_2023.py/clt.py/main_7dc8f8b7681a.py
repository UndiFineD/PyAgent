# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\clt\main.py
...
class RandomDieRolls(InteractiveScene):
    def construct(self):
        ...
        faces = VGroup(
            *(DieFace(n, fill_color=BLUE_E, dot_color=WHITE) for n in range(1, 7))
        )

        for x in range(n_runs):
            random_faces = VGroup(
                *(random.choice(faces).copy() for x in range(n_movements_per_run))
            )
...
class DiceSumDistributions(InteractiveScene):
    ...
    def construct(self):
        ...
        plots = VGroup(
            *(
                self.get_sum_distribution_plot(n, y_max=(0.2 if n > 1 else 0.5))
        ...
        self.play(
            MoveToTarget(plot1),
            LaggedStart(
                *(TransformFromCopy(VGroup(dice[n]), die_groups[n]) for n in range(6)),
                lag_ratio=0.1,
            ),
            Write(grid),
            run_time=3,
        ...
        highlights = [
            VGroup(columns[n - 2], prob_labels[n - 2]).copy() for n in [6, 10]
        ]
        last = VMobject()
        ...
        for n in range(1, len(plots) - 1):
            ...
            self.play(
                LaggedStart(
                    ...
                    FadeTransform(
                        plots[n].copy().set_opacity(0), plots[n + 1], run_time=2
                    ),
        ...
        # Flash through
        plot_groups = VGroup(
            *(VGroup(plot, label) for plot, label in zip(plots, labels))
        )
        ...
        self.play(
            ...
            LaggedStart(*(FadeOut(label[1]) for label in mean_labels)),
            run_time=3,
        )
        self.wait()
        ...
        graph_copies = VGroup(
            *(
                graph.copy().replace(bars[n:], dim_to_match=1).set_stroke(width=1.5)
                for n, bars in zip(it.count(1), all_bars[1:])
            )
        )
        self.play(
            LaggedStart(
                *(TransformFromCopy(graph, graph_copy) for graph_copy in graph_copies)
            ),
            run_time=3,
        )
        self.wait()
    ...
    def get_mean_labels(self, plots, color=PINK, num_decimal_places=1):
        ...
        for n, plot in zip(it.count(1), plots):
            axes = plot[0]
            label = Tex(Rf"{n}\mu = {np.round(n * mu, num_decimal_places)}")
            if n == 1:
    ...
    def get_sd_labels(self, plots, color=RED):
        ...
        for n, plot in zip(it.count(1), plots):
            ...
            if n == 1:
                sigma_label = Tex(Rf"\sigma")
            else:
                sigma_label = Tex(Rf"\sqrt{n} \cdot \sigma")
            ...
            LaggedStart(
                *(
                    bars.animate.stretch(
                        1 / get_factor(n), 0, about_point=lines.get_center()
        ...
        arrows = VGroup(
            *(
                FillArrow(ORIGIN, length * vect, **kw).move_to(
                    center - 3.0 * sigma * bar_width * vect, -vect
                )
    ...
    def show_sample_sum(self, plot, distribution, n, low_plot=None):
        ...
        self.wait()
        self.play(
            LaggedStartMap(FadeOut, VGroup(tips, sum_expr, low_bar_highlight)),
            run_time=0.5,
        )
...
class ExpDistSumDistributions(DiceSumDistributions):
    ...
    def construct(self):
        ...
        for dist in distributions:
            ...
            plots = VGroup(
                *(
                    self.get_sum_distribution_plot(n, y_max=(0.2 if n > 1 else 0.4))
                    for n in range(1, self.n_examples + 1)
                )
            )
            plots.scale(0.9)
        ...
        self.add(plots)
        self.play(
            frame.animate.set_height(plots.get_height() + 2)
            .move_to(plots, LEFT)
            .shift(LEFT),
            run_time=4,
        )
        ...
        arrows = VGroup(
            *(
                FillArrow(axes.c2p(0, 0), axes.c2p(n * mu, 0))
                .move_to(axes.y_axis.pfp(0.75), LEFT)
                .scale(0.7)
        ...
        axes.y_axis.add_numbers(
            np.arange(0.25, 1.25, 0.25), font_size=36, num_decimal_places=2
        )
        ...
        self.play(
            LaggedStart(*(TransformFromCopy(graph, curve) for curve in target_curves))
        )
        self.wait()
...
class MeanAndStandardDeviation(InteractiveScene):
    def construct(self):
        ...
        chart = get_die_distribution_chart(
            dist, y_range=(0, 0.5, 0.1), die_config=die_config
        )
        chart.to_edge(LEFT)
        self.add(*chart)
        ...
        # Mean line
        tex_kw = dict(t2c={R"\mu": PINK, R"\sigma": RED}, font_size=48)
        ...
        sd_lines[0].add_updater(
            lambda m: m.move_to(axes.c2p(get_mu() - get_sigma() - 0.5, 0), DOWN)
        )
        sd_lines[1].add_updater(
            lambda m: m.move_to(axes.c2p(get_mu() + get_sigma() - 0.5, 0), DOWN)
        )
        ...
        labels = VGroup(
            *(Tex(Rf"P({n}) \cdot ({n} - \mu)^2", **tex_kw) for n in range(1, 7))
        )
        labels.scale(0.5)
        for label, square in zip(labels, squares):
            label.square = square
            label.add_updater(
                lambda m: m.set_width(0.7 * m.square.get_width()).next_to(
                    m.square.get_bottom(), UP, SMALL_BUFF
                )
            )
    ...
    def construct(self):
        ...
        def decay_anim(graph):
            ...
            return AnimationGroup(
                ...
                UpdateFromAlphaFunc(
                    VectorizedPoint(),
                    lambda m, a: dot.set_radius(
                        0.2 * clip(10 * there_and_back(a), 0, 1)
                    ),
                    remover=True,
                ),
                run_time=1.5,
        ...
        self.play(
            LaggedStart(
                *(decay_anim(graph) for x in range(8)), lag_ratio=0.15, run_time=4
            )
        )
        ...
        self.play(
            TransformMatchingTex(formulas[1], abs_formula), Transform(graph, abs_graph)
        )
        self.wait()
        ...
        axes.bind_graph_to_func(
            graph, lambda x: np.exp(-get_c() * np.log(get_base()) * x**2)
        )
        ...
        pis, twos, threes = [
            VGroup(*(Tex(tex, **kw).move_to(e, DR) for e in es))
            for tex in [R"\pi", "2", "3"]
        ]
        ...
        form_with_sigma = Tex(R"e^{-{1 \over 2} \left({x / \sigma}\right)^2}", **kw)
        ...
        v_lines = Line(axes.c2p(0, 0), axes.c2p(0, 0.75 * axes.y_axis.x_max)).replicate(
            2
        )
        ...
        self.play(
            Restore(v_lines), ShowCreation(arrows, lag_ratio=0), FadeIn(sigma_labels)
        )
        ...
        sigma_forms = [
            ...
            Tex(
                R"{1 \over \sigma \sqrt{2}} {1 \over \sqrt{\pi}} e^{-{1 \over 2}(x / \sigma)^2}",
                **kw,
            ),
        ...
        self.play(
            FlashAround(sigma_forms[2][R"{1 \over \sigma \sqrt{2\pi}}"], run_time=3)
        )
        self.wait()
        ...
        final_form[: final_form.submobjects.index(final_form["e^"][0][0])].scale(
            0.75, about_edge=RIGHT
        )
    ...
    def get_axes(
        ...
        axes = NumberPlane(
            ...
            background_line_style=dict(
                stroke_color=GREY_A, stroke_width=2, stroke_opacity=0.5
            ),
        )
...
class LimitingDistributions(InteractiveScene):
    ...
    def construct(self):
        ...
        top_mean_sd_labels = self.get_mean_sd_labels(R"\mu", R"\sigma", mean, sd)
        ...
        self.play(
            TransformMatchingTex(mu_label[1].copy(), top_mean_sd_labels[0]), run_time=1
        )
        self.wait()
        ...
        self.play(
            TransformMatchingTex(sigma_label[2].copy(), top_mean_sd_labels[1]),
            run_time=1,
        )
        self.wait()
        ...
        self.wait()
        self.play(
            LaggedStartMap(FadeOut, VGroup(graph, *low_lines, formula), lag_ratio=0.5)
        )
        ...
        self.play(
            ...
            s_plot.bars.animate.shift(
                (target_x - low_lines[0].get_x() - x_shift) * RIGHT
            ),
        ...
        self.play(
            stretch_group.animate.stretch(1 / stretch_factor, dim=1, about_edge=DOWN)
        )
        self.wait()
        ...
        self.play(
            FadeOut(full_screen_rect), top_rect.animate.set_opacity(1), FadeIn(bar)
        )
        self.wait()
        self.play(
        ...
        self.change_distribution(
            alt_dists, top_plot, ss_plot, top_mean_sd_labels, get_N()
        )
        ...
        rect.set_stroke(BLUE, 2)
        statement = Tex(
            R"\lim_{N \to \infty} P(a < \text{This value} < b) = \int_a^b "
            + label.get_string()
            + "dx",
            font_size=36,
        )
        ...
        self.wait()
        self.play(FadeIn(statement[R"P(a < \text{This value} < b)"], 0.5 * DOWN))
        ...
        self.play(
            LaggedStart(
                *(Indicate(bar, color=BLUE) for bar in bars[mid_i - 5 : mid_i + 10])
            )
        )
        self.wait()

    def get_top_distribution_plot(
        self, dist, width=5, height=2.5, y_range=(0, 0.5, 0.1)
    ):
        # Axes and bars
        x_min = self.x_min
    ...
    def get_sum_label(self, n: str | int, dots=True, scaled: bool = False):
        ...
        if scaled:
            sum_tex = (
                ...
                + sum_tex
                + ")"
                + Rf" - {n_str} \cdot \mu \over \sigma \cdot \sqrt{{{n_str}}}"
            )
    ...
    def get_normal_plot_axes(self):
        sum_axes = Axes(
            (-6, 6), self.normal_y_range, width=12, height=self.normal_axes_height
        )
    ...
    def get_sum_plot(
        ...
        axes.x_axis.numbers.shift(0.5 * axes.x_axis.get_unit_size() * LEFT)
        axes.y_axis.add_numbers(
            num_decimal_places=len(str(y_range[2])) - 2, font_size=24, excluding=[0]
        )
    ...
    def change_distribution(
        ...
        for dist in alt_dists:
            ...
            self.play(
                ...
                UpdateFromFunc(
                    ss_plot.bars,
                    lambda m: m.set_submobjects(
                        self.get_scaled_sum_plot(get_dist(top_plot.bars), n).bars
                    ),
                ),
...
class HowVarianceAdds(LimitingDistributions):
    def construct(self):
        ...
        sum_plot = self.get_top_distribution_plot(
            sum_dist, y_range=(0, 0.301, 0.1), width=10
        )
        ...
        var_form = Tex(R"\text{Var}(X + Y) = \text{Var}(X) + \text{Var}(Y)", **label_kw)
        var_form.scale(0.75)
        ...
        self.play(
            LaggedStart(
                TransformMatchingShapes(
                    sum_label.copy(), var_form[R"\text{Var}(X + Y) = "]
                ),
                TransformMatchingShapes(
                    top_labels[0].copy(), var_form[R"\text{Var}(X)"]
                ),
                TransformMatchingShapes(
                    top_labels[1].copy(), var_form[R"+ \text{Var}(Y)"]
                ),
        ...
        # Sigma equation
        sigma_form = Tex(R"\sigma_{X + Y}^2 = \sigma_X^2 + \sigma_Y^2", **label_kw)
        ...
        new_sum_plot = self.get_top_distribution_plot(
            sum_dist, y_range=(0, 0.2, 0.1), width=10
        )
        new_sum_plot.axes.x_axis.remove(new_sum_plot.axes.x_axis.numbers)
        new_sum_plot.to_corner(DL)
        new_sum_label = Tex(R"X_1 + \cdots + X_n", t2c={"X_1": BLUE, "X_n": BLUE})
        ...
        self.play(
            ...
            FadeOut(sum_label),
            rules.animate.scale(0.5)
            .next_to(new_sum_plot.axes.c2p(0, 0), UP)
            .to_edge(RIGHT),
            FadeIn(new_sum_plot),
        ...
        new_sigma_form = Tex(
            R"\sigma_{X_1 + \cdots + X_n} = \sqrt{n} \cdot \sigma_{X_1}", t2c=t2c
        )
    ...
    def construct(self):
        ...
        for n in range(6, len(full_dist)):
            ...
            plot = self.get_top_distribution_plot(
                dist, width=min(n * (5 / 6), 12), height=4, y_range=(0, 0.301, 0.1)
            )
...
class AnalyzeHundreDiceQuestion(LimitingDistributions):
    ...
    def construct(self):
        ...
        top_mean = Tex(R"\mu = \frac{1}{6}\big(1 + 2 + 3 + 4 + 5 + 6\big) = 3.5", **kw)
        top_var = Tex(
        ...
        self.play(
            Write(top_sd),
            Transform(
                top_var[R"\text{Var}(X)"].copy(),
                top_sd[R"\text{Var}(X)"].copy(),
                remover=True,
            ),
        )
        self.wait()
        ...
        self.wait()
        self.play(
            TransformMatchingTex(top_sd_simple.copy(), low_sd, key_map={"1.71": "17.1"})
        )
        sd_label.save_state()
        ...
        self.wait()
        self.play(
            LaggedStartMap(FadeIn, bound_equations, shift=0.5 * UP, lag_ratio=0.75)
        )
        self.wait()
        ...
        self.wait()
        self.play(
            LaggedStartMap(FadeOut, VGroup(*bound_equations, rhs_rect), shift=DOWN)
        )
        ...
        dice = VGroup(
            *(DieFace(random.randint(1, 6), **dot_config) for x in range(100))
        )
...
class Thumbnail(LimitingDistributions):
    def construct(self):
        ...
        for dist in dists:
            top_plot = self.get_top_distribution_plot(
                dist, width=3, height=2, y_range=(0, 0.401, 0.1)
            )
        ...
        arrows = VGroup(
            *(
                ...
                for tp, vect in zip(
                    top_plots, np.linspace(2 * LEFT, 2 * RIGHT, len(top_plots))
                )
            )
        )
