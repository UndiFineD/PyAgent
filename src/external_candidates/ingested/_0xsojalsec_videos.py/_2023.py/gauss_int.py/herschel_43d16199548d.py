# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\gauss_int\herschel.py
...
class TwoDGaussianAsADistribution(InteractiveScene):
    ...
    def construct(self):
        ...
        for sigma in [0.8, 1.0, 0.6]:
            graph = ParametricSurface(
                lambda u, v: [u, v, func(u, v, sigma)], u_range=(-2, 2), v_range=(-2, 2)
            )
    ...
    def get_plane(self):
        plane = NumberPlane(
            ...
            background_line_style=dict(
                stroke_color=GREY, stroke_width=1, stroke_opacity=1
            ),
        )
        plane.set_height(7)
    ...
    def add_random_points_anim(self, plane):
        ...
        dots = Group(
            *(GlowDot(plane.c2p(x, y), glow_factor=4.0, radius=0.3) for x, y in coords)
        )

        anim = LaggedStart(
            *(FadeIn(dot, rate_func=there_and_back) for dot in dots),
            lag_ratio=1 / self.n_dots_per_moment,
            run_time=self.n_points / self.n_dots_per_moment,
        )
...
class IndependentCoordinates(TwoDGaussianAsADistribution):
    ...
    def construct(self):
        ...
        for n in range(self.n_iterations):
            ...
            self.play(*map(ShowCreation, lines), FadeIn(dot), run_time=0.5)
            self.wait(0.5)
            ...
            # self.wait()
            self.play(
                LaggedStartMap(FadeOut, Group(x_tip, y_tip, lines, dot)), run_time=1
            )
    ...
    def construct(self):
        ...
        self.play(
            ...
            ReplacementTransform(
                coord_label, new_coord_label, path_arc=-angle, time_span=(1, 2)
            ),
            run_time=3,
        )
        self.wait()
...
class ManyDifferentFs(InteractiveScene):
    def construct(self):
        ...
        curve = curves[0]
        name = func_names[0]
        self.play(ShowCreation(curve), FadeIn(name, 0.5 * UP))
        self.wait()
...
class RationalNumbers(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(Rotate(line, -90 * DEGREES) for line in lines),
...
class TwoProperties(InteractiveScene):
    def construct(self):
        ...
        prop_boxes = VGroup(
            *(
                SurroundingRectangle(prop[1])
                .set_fill(GREY_E, 1)
                .set_stroke(RED, 1, 0.5)
                for prop in properties
            )
        )
        ...
        kw = dict(t2c={"x": BLUE, "y": YELLOW, R"\sigma": RED, "{r}": RED})
        form1, form2, form3 = forms = VGroup(
            Tex(R"f_2(x, y) = e^{-(x^2 + y^2)}", **kw),
            Tex(R"f_2(x, y) = e^{-(x^2 + y^2) / 2 \sigma^2}", **kw),
            Tex(
                R"f_2(x, y) = {1 \over 4 \sigma^2 \pi} e^{-(x^2 + y^2) / 2 \sigma^2}",
                **kw,
            ),
        )
        ...
        form1.save_state()
        self.play(Write(implies), FadeIn(form1, DOWN))
        self.wait()
        ...
        full_radial_rhs.shift(
            (radial_rhs["="].get_y() - full_radial_rhs["="].get_y()) * UP
        )
        ...
        self.wait()
        self.play(
            lhs_rect.animate.replace(lhs[1], stretch=True)
            .set_stroke(width=1)
            .scale(1.1)
        )
        ...
        self.play(ShowCreation(f_rect), FadeIn(f_words, lag_ratio=0.1))
        self.wait(2)
        ...
        self.play(
            FlashUnder(phrase, color=TEAL, buff=0), phrase.animate.set_color(TEAL)
        )
        self.wait()
        ...
        g_box = SurroundingRectangle(factored_rhs["g(x)"], buff=0.05).set_stroke(
            BLUE, 2
        )
        h_box = SurroundingRectangle(factored_rhs["h(y)"], buff=0.05).set_stroke(
            YELLOW, 2
        )
        ...
        self.play(TransformMatchingShapes(lhs.copy(), factored_rhs))
        self.wait()
        self.play(ShowCreation(g_box), FadeIn(g_words))
        self.wait()
        self.play(
        ...
        self.wait()
        self.play(ShowCreation(const_rect), FadeIn(const_words))
        self.wait()
        ...
        self.play(TransformFromCopy(VGroup(radial_rhs[1], *simpler_rhs[:2]), f_eq_g))
        self.wait()
        ...
        self.play(Write(func_eq_name), GrowArrow(arrow))
        self.wait()
        ...
        self.play(FadeIn(example_box), FadeIn(example_word, 0.5 * DOWN))
        self.play(
        ...
        self.play(
            TransformMatchingTex(
                h_def.copy(), h_def2, key_map={R"\sqrt": "^2"}, run_time=1
            )
        )
        self.wait()
        self.play(TransformFromCopy(h_def, example_h))
        self.wait(2)
        ...
        self.play(
            TransformMatchingShapes(VGroup(*full_radial_rhs[1:], *f_rhs).copy(), h_eq)
        )
        self.wait()
        ...
        five_eq = Tex(
            R"h(5) &= h(1 + 1 + 1 + 1 + 1) \\ &= h(1)h(1)h(1)h(1)h(1) = h(1)^5"
        )
        ...
        sum_tex = sum_brace.get_tex(R"n \text{ times}", buff=SMALL_BUFF).scale(
            0.5, about_edge=DOWN
        )
        prod_brace = Brace(n_eq[R"h(1) \cdots h(1)"], UP, SMALL_BUFF)
        prod_tex = prod_brace.get_tex(R"n \text{ times}", buff=SMALL_BUFF).scale(
            0.5, about_edge=DOWN
        )
        ...
        exercise = TexText(
            R"Exercise: Show this is also true for rational inputs, $p / q$"
        )
        exercise["p / q"].set_color(RED)
        hint = TexText(
            R"Hint, think about $h\left(\frac{p}{q} + \cdots + \frac{p}{q} \right)$"
        )
        ...
        # Continuity
        assumption = TexText(
            R"Assuming $f$ (and hence also $h$) \\ is continuous...", font_size=36
        )
        ...
        self.wait()
        self.play(GrowArrow(arrow), TransformMatchingTex(rational_eq, hx_eq))
        ...
        self.play(
            Write(implies3), TransformMatchingTex(hx_eq2.copy(), f_form, run_time=1)
        )
        self.wait()
    ...
    def construct(self):
        ...
        def get_graph(c):
            ...
            x_slice = ParametricCurve(
                lambda t: axes.c2p(t, 0, np.exp(c * t**2)), t_range=(-4, 4, 0.1)
            )
    ...
    def construct(self):
        ...
        def update_area(area):
            area.set_points_as_corners(
                [plane.c2p(-4, 0), *graph.get_anchors(), plane.c2p(4, 0)]
            )
...
class OldTwoKeyProperties(InteractiveScene):
    def construct(self):
        # Setup equations
        kw = dict(t2c={"x": BLUE, "y": YELLOW, "{r}": RED, "{c}": PINK})
        ...
        self.play(
            LaggedStart(
                Write(factored["="]),
                TransformFromCopy(
                    one_var["f_1(x)"], factored["f_1(x)"], path_arc=PI / 4
                ),
                TransformFromCopy(
                    one_var["f_1(x)"], factored["f_1(y)"], path_arc=PI / 4
                ),
                run_time=3,
                lag_ratio=0.5,
            )
        ...
        self.play(FadeOut(question, DOWN), FadeOut(arrow, DOWN), Write(h_def))
        self.wait()
        ...
        self.wait()
        self.play(Rotate(implies, PI), FadeIn(assumption, lag_ratio=0.1))
        self.wait()

        self.play(ShowCreation(b_rect), Write(b_words, run_time=1))
        self.wait()
        c = h_exp2["{c}"][0][0]
        c_copy = c.copy()
        c.set_opacity(0)
        ...
        self.play(
            TransformMatchingTex(h_exp2.copy(), f_eq), Write(implies2, run_time=1)
        )
        self.wait()
        self.play(
            ShowCreation(rect, run_time=2),
