# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\gauss_int\supplements.py
...
class WignerZoom(InteractiveScene):
    def construct(self):
        im1 = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/clt/artwork/wigner-speech/wigner-speech-transition-3.jpg"
        )
...
class WignerReverseZoom(InteractiveScene):
    def construct(self):
        # Test
        im1 = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/clt/artwork/wigner-speech/wigner-speech-transition-4.jpg"
        )
        im1.set_height(FRAME_HEIGHT)
        self.add(im1)
...
class CirclesToPopulation(InteractiveScene):
    def construct(self):
        ...
        self.add(circle)
        self.play(ShowCreation(arrow), FadeIn(title, DR))
        self.wait()
...
class ThreeStepPlan(InteractiveScene):
    def construct(self):
        # Steps
        step_titles = VGroup(*(Text(f"Step {n}", font_size=48) for n in [1, 2, 3]))
        kw = dict(
            t2c={"e^{-{x}^2}": BLUE, R"\sqrt{\pi}": TEAL},
            color=GREY_A,
            font_size=36,
            alignment="",
        )
        step_contents = VGroup(
            ...
            TexText(
                R"Explain where $e^{-{x}^2}$ comes from \\ in the first place", **kw
            ),
        ...
        step_rects = VGroup(
            *(SurroundingRectangle(content) for content in step_contents)
        )
        ...
        for rect, content in zip(step_rects, step_contents):
            ...
            self.play(
                rect.animate.stretch(0, 0, about_edge=RIGHT).set_stroke(opacity=0)
            )
            self.remove(rect)
            self.wait()
        ...
        for step in steps:
            ...
            for other_step in steps:
                if other_step is not step:
                    anims.append(
                        other_step.animate.restore()
                        .scale(0.75, about_edge=LEFT)
                        .set_opacity(0.5)
                    )
            self.play(*anims)
            self.wait()
...
class ManyBulgingFunctions(InteractiveScene):
    def construct(self):
        # Test
        axes = Axes(
            (-3, 3), (0, 1), width=3, height=1, axis_config=dict(tick_size=0.05)
        )
        ...
        func_labels = VGroup(
            ...
            Tex(
                R"e^{-1/e} \left(\left|x\right|+\frac{1}{e}\right)^{-\left(\left|x\right|+\frac{1}{e}\right)}",
                font_size=36,
            ),
...
class WarningCalculus(InteractiveScene):
    def construct(self):
        ...
        warning = VGroup(
            Text("Warning", font_size=120), Text("Calculus ahead", font_size=90)
        )
...
class MysteryConstant(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(words), FadeIn(eq_C, LEFT), GrowArrow(arrow))
        self.wait()
...
class CompareThreeIntegrals(InteractiveScene):
    def construct(self):
        ...
        # Titles
        kw = dict(font_size=48, t2c={"C": PINK, "{x}": BLUE, "{y}": YELLOW, "{r}": RED})
        titles = VGroup(
            TexText("Area = $C$", **kw),
        ...
        integrals = VGroup(
            ...
            Tex(
                R"\int_{-\infty}^\infty \int_{-\infty}^\infty e^{-{y}^2} e^{-{x}^2}\,d{x}\,d{y} = C^2",
                **kw,
            ),
        ...
        for title, integral in zip(titles[:2], integrals[:2]):
            self.play(FadeIn(title, 0.5 * UP), FadeIn(integral, 0.5 * DOWN))
        self.wait(2)
...
class GaussianQuestion(InteractiveScene):
    def construct(self):
        ...
        self.play(ShowCreation(curve), FadeIn(func), Write(question))
...
class ImplicationPIP(InteractiveScene):
    def construct(self):
        ...
        self.add(screens)
        self.wait()
        self.play(ShowCreation(arrow), FadeIn(words, lag_ratio=0.1))
        self.wait()


class MaxwellsEquations(InteractiveScene):
    def construct(self):
        kw = dict(t2c={R"\mathbf{E}": RED, R"\mathbf{B}": TEAL})
        equations = VGroup(
...
class DefiningProperty(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(FlashAround(prop[0], color=TEAL, time_width=1, run_time=2))
        self.wait()
...
class NextVideo(InteractiveScene):
    def construct(self):
        ...
        images = Group(
            ImageMobject(
                "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2022/convolutions/discrete/images/EquationThumbnail.png"
            ),
            ImageMobject(
                "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/clt/main/images/Thumbnail.png"
            ),
            ImageMobject(
                "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/clt/integral/images/ProofSummary.jpg"
            ),
        )
        images.set_height(3)
        titles = VGroup(
        ...
        self.play(morty.animate.look_at(thumbnails[1]), FadeIn(thumbnails[1], DOWN))
        ...
        bubble2 = morty.target.get_bubble(
            words2, bubble_type=SpeechBubble, width=4, height=1
        )
        ...
        bubble3 = morty.target.get_bubble(
            words3, bubble_type=SpeechBubble, width=4, height=1
        )
...
class NDimensionalBallGrid(InteractiveScene):
    def construct(self):
        ...
        one_ball = VGroup(line, Dot(line.get_start()), Dot(line.get_end()))
...
class Thanks(InteractiveScene):
    def construct(self):
        ...
        self.play(morty.change("gracious"), Write(message))
        self.play(Blink(morty))
        self.wait()
        self.play(Blink(morty))
        self.wait(2)
