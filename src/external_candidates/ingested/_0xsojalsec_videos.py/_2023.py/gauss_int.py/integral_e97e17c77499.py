    ...
    def construct(self):
        ...
        axes = NumberPlane(
            ...
            background_line_style=dict(
                stroke_color=GREY_C, stroke_width=2, stroke_opacity=0.5
            ),
        )
        ...
        for dx in [0.1, 0.075, 0.05, 0.03, 0.02, 0.01, 0.005]:
            ...
            self.play(
                ...
                brace.animate.set_width(
                    dx * axes.x_axis.get_unit_size(), about_edge=LEFT
                ),
        ...
        planes = VGroup(
            *(
                NumberPlane(
                    ...
                    height=2.75,
                    background_line_style=dict(
                        stroke_color=GREY, stroke_width=1, stroke_opacity=1.0
                    ),
                    faded_line_ratio=3,
                )
                for x in range(2)
...
class GaussianIntegral(ThreeDScene, InteractiveScene):
    ...
    def get_axes(
        ...
        axes = ThreeDAxes(
            x_range, y_range, z_range, width=width, height=height, depth=depth
        )
    ...
    def get_dynamic_cylinder(self, axes, r_init=1):
        ...
        cylinder.add_updater(
            lambda m: self.set_cylinder_r(m, axes, r_tracker.get_value())
        )
    ...
    def get_thick_cylinder(self, cylinder, delta_r):
        ...
        annulus = ParametricSurface(
            lambda u, v: (radius + u * delta_r)
            * np.array([math.cos(v), math.sin(v), 0]),
            u_range=(0, 1),
            v_range=(0, TAU),
        )
...
class CylinderSlices(GaussianIntegral):
    def construct(self):
        ...
        self.play(
            *(
                VShowPassingFlash(
                    line.insert_n_curves(20).set_stroke(width=8), time_width=1.5
                )
        ...
        self.play(
            ...
            ShowCreation(circle),
            frame.animate.reorient(30, 60)
            .move_to(ORIGIN)
            .set_height(8)
            .set_field_of_view(45 * DEGREES),
            Restore(graph_mesh),
            run_time=7,
        )
        self.wait()
        ...
        delta_r = 0.1
        cylinders = Group(
            *(self.get_cylinder(axes, r, opacity=0.5) for r in np.arange(0, 3, delta_r))
        )
        ...
        self.play(
            ...
            functions.animate.shift(1.5 * DOWN)
            .scale(0.7, about_edge=DL)
        ...
        height_line = Line(
            cylinder.get_corner(IN + DOWN), cylinder.get_corner(OUT + DOWN)
        )
        ...
        self.play(
            FadeIn(height_brace),
            height_word.animate.scale(0.75).next_to(
                height_brace, DOWN, SMALL_BUFF, aligned_edge=LEFT
            ),
        ...
        thick_cylinder = self.get_thick_cylinder(
            cylinder, delta_r * axes.x_axis.get_unit_size()
        )
        thin_cylinder = self.get_thick_cylinder(
            cylinder, 0.1 * delta_r * axes.x_axis.get_unit_size()
        )
        ...
        self.play(
            ...
            UpdateFromFunc(
                dr_brace[-1], lambda m: m.next_to(dr_brace[:-1], OUT, SMALL_BUFF)
            ),
            run_time=3,
        )
    ...
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeOut,
                VGroup(
                    integrand_rect, arrow, arrow_label, anti_derivative, *rects, *values
                ),
            ),
...
class CartesianSlices(GaussianIntegral):
    def construct(self):
        ...
        graph.add_updater(
            lambda m: m.set_clip_plane(UP, -y_tracker.get_value() * y_unit)
        )
        ...
        coef.add_updater(
            lambda m: m.set_value(math.exp(-get_y() ** 2)).rotate(90 * DEGREES, RIGHT)
        )
        ...
        self.play(
            *(
                ...
                for mob in [
                    ...
                    .set_fill(opacity=0)
                    .shift(0.02 * OUT),
                    Line(*axes.x_axis.get_start_and_end())
                    .set_stroke(YELLOW, 8)
                    .insert_n_curves(40),
                ]
            ),
        ...
        def get_area_label():
            ...
            area_label.move_to(
                interpolate(x_slice.get_zenith(), x_slice.get_nadir(), 0.66)
            )
            area_label.shift(0.1 * DOWN)
            return area_label

        self.play(
            FadeIn(get_area_label(), run_time=3, rate_func=there_and_back_with_pause)
        )
        ...
        for value in [-0.5, -0.75, -1]:
            ...
            self.wait(0.25)
            self.play(
                slice_copy.animate.match_depth(
                    x_slice, stretch=True, about_edge=IN
                ).set_opacity(0)
            )
            self.wait()
        ...
        self.play(
            ...
            frame.animate.reorient(-83, 72, 0)
            .set_height(8)
            .center()
        ...
        frame.add_updater(
            lambda m: m.set_theta(theta0 + -0.2 * math.sin(0.1 * (self.time - t0)))
        )
        self.wait(10)
        ...
        line = Line(
            x_slices[mid_index].get_zenith(), x_slices[mid_index + 1].get_zenith()
        )
...
class CartesianSliceOverlay(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            TransformMatchingTex(integral1.copy(), integral2, path_arc=30 * DEGREES)
        )
        self.wait()
