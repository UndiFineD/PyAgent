# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\clt_proof\main.py
...
class ImagineProving(InteractiveScene):
    def construct(self):
        ...
        self.play(
            Write(yourself), ShowCreation(arrow), randy.change("pondering", screen)
        )
...
class LookingBeyondExpectationAndVariance(InteractiveScene):
    def construct(self):
        ...
        moment_boxes = VGroup(
            *(SurroundingRectangle(moment, buff=0.05) for moment in moments)
        )
        ...
        self.wait()
        self.play(
            ShowCreation(second_moment_rect), FlashAround(second_moment, run_time=1.5)
        )
        self.play(
        ...
        points = np.linspace(
            moment_name.get_corner(DL), moment_name.get_corner(DR), len(moments)
        )
        arrows = VGroup(
            *(
...
class DefineMGF(InteractiveScene):
    def construct(self):
        ...
        lines = VGroup(
            Tex(
                R"\mathds{E}\left[1 + tX + \frac{t^2}{2} X^2 + \cdots + \frac{t^k}{k!} X^k + \cdots\right]",
                **kw,
            ),
            Tex(
                R"1 + \mathds{E}[X]t + \mathds{E}[X^2] \frac{t^2}{2} + \cdots + \mathds{E}[X^k] \frac{t^k}{k!} + \cdots",
                **kw,
            ),
        )
        ...
        x_words, t_words = [
            Text(text, font_size=36) for text in ["Random variable", "Some real number"]
        ]
        ...
        exp_rects = VGroup(
            *(SurroundingRectangle(m, buff=0.05) for m in [exp_term, exp_series])
        )
        ...
        self.play(FadeIn(gf_expl, lag_ratio=0.1))
        self.wait()
...
class DirectMGFInterpretation(InteractiveScene):
    ...
    def construct(self):
        ...
        axes.x_axis.add(Tex("x").next_to(axes.x_axis.get_right(), UR, SMALL_BUFF))
        ...
        def get_sample_group(samples):
            ...
            v_lines = VGroup(*(Line(xp, gp) for xp, gp in zip(x_points, graph_points)))
            h_lines = VGroup(*(Line(gp, yp) for gp, yp in zip(graph_points, y_points)))
        ...
        sample_arrows = VGroup(
            *(Vector(0.5 * DOWN).next_to(dot, UP, buff=0) for dot in x_dots)
        )
        ...
        for _ in range(15):
            ...
            self.play(t_tracker.animate.set_value(target_t), run_time=7)

        for target_t in [1.5, 0.5, 1.2, 0.2, 2.0, 1.0]:
            self.play(t_tracker.animate.set_value(target_t), run_time=5)
        ...
        for i in [0, 1]:
            self.play(
                TransformMatchingTex(
                    equation[i].copy(), equation[i + 1], path_arc=-45 * DEGREES
                )
            )
            self.wait()
...
class DefineCGF(InteractiveScene):
    def construct(self):
        ...
        self.add(mgf_eq)
        self.wait()
        self.play(TransformMatchingTex(mgf_eq.copy(), log_eq))
        self.wait()
        ...
        self.play(ShowCreation(rect), gen_func.animate.set_color(YELLOW))
        self.wait()
...
class ExpandCGF(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeOut, VGroup(axes, graph, approx, rect, taylor_label))
        )
        ...
        self.play(k_eq.animate.align_to(mid_eq, UP), FadeOut(mid_eq, UP))
        self.wait()
        ...
        # Second order term
        full_second = Tex(
            R"\left(\mathds{E}[X^2] - \mathds{E}[X]^2 \right) \frac{t^2}{2}", **kw
        )
        plus = expansion["+"][0]
        ...
        self.wait()
        self.play(TransformFromCopy(term21, full_second[R"- \mathds{E}[X]^2"][0]))
        self.wait()
        ...
        self.play(
            VGroup(expanded_var, rect).animate.next_to(
                arrow.get_start(), UL, SMALL_BUFF
            ),
            FadeIn(var_term),
            GrowArrow(arrow),
            ReplacementTransform(
...
class CumulantsOfScaledSum(InteractiveScene):
    def construct(self):
        ...
        denom = lhs[-5:-1]
        self.play(FlashAround(denom), denom.animate.set_color(YELLOW))
        self.wait()
        ...
        cumulant_of_clt_var_equation = VGroup(lhs, rhs[re.compile(r"=.*\^m")], nK)
        full_rect = SurroundingRectangle(cumulant_of_clt_var_equation, buff=0.5)
        full_rect.set_stroke(TEAL, 3)
        words = TexText(
            f"Excellent! This is a complete(?) description of ${clt_var_tex}$"
        )
...
class ConnectingArrow(InteractiveScene):
    def construct(self):
        ...
        label = TexText(
            R"$m^{\text{th}}$ cumulant of $\frac{X_1 + \cdots + X_n}{\sigma \sqrt{n}}$"
        )
        label.set_fill(RED_E)
        label.set_stroke(BLACK, 2, background=True)
        label.next_to(arrow.pfp(0.5), RIGHT, SMALL_BUFF)

        self.play(ShowCreation(arrow), FadeIn(tip, time_span=(0.8, 1.0)))
