# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\optics_puzzles\adding_waves.py
...
class WhiteLightAsASum(InteractiveScene):
    def construct(self):
        ...
        spectral_axes = VGroup(
            *(ThreeDAxes((0, x_max), (-1, 1), (-1, 1)) for _ in range(n_colors))
        )
        ...
        self.play(
            frame.animate.reorient(50, 65, 0)
            .move_to([0.6, 2.9, -0.16])
            .set_height(11)
        ...
        self.play(
            self.frame.animate.reorient(19, 65, 0)
            .move_to([0.6, 2.9, -0.16])
            .set_height(11.00),
            run_time=8,
        )
        ...
        arrows = VGroup(
            *(
                Arrow(
                    sa.get_left(),
                    interpolate(
                        white_axes.get_corner(UR), white_axes.get_corner(DR), alpha
                    ),
                    buff=0.3,
...
class AddTwoSineWaves(InteractiveScene):
    def construct(self):
        ...
        wave3 = axes3.get_graph(
            lambda x: wave1.func(x) + wave2.func(x), color=GREEN, bind=True
        )
        ...
        t2c = {R"\omega": GOLD}
        top_words = TexText(
            R"If both have matching \\ frequencies, $\omega$...", font_size=36, t2c=t2c
        )
        ...
        top_arrows = VGroup(
            *(Arrow(top_words.get_right(), lock.get_left()) for lock in omega_locks)
        )
        ...
        sum_sine = Tex(
            R"= 1.00\sin(\omega t +2.00)", t2c={R"\omega": GOLD}, font_size=36
        )
        ...
        for wave, phasor in [(wave1, phasor1), (wave2, phasor2)]:
            ...
            phasor.A_label.add_updater(
                lambda m: m.set_height(min(max_A_height, m.vector.get_length()))
            )
            phasor.A_label.add_updater(
                lambda m: m.next_to(
                    m.vector.pfp(0.5),
            ...
            phasor.phi_label.add_updater(
                lambda m: m.set_height(min(max_phi_height, m.arc.get_height()))
            )
            phasor.phi_label.add_updater(
                lambda m: m.move_to(m.plane.n2p(1.5 * m.plane.p2n(m.arc.pfp(0.5))))
            )
            ...
            self.play(
                wave.trackers[2].animate.set_value(
                    wave.trackers[2].get_value() + PI / 2
                ),
                run_time=4,
                rate_func=there_and_back,
            )
        ...
        sum_plane = ComplexPlane(
            ...
            faded_line_style=dict(
                stroke_color=GREY_B, stroke_width=0.5, stroke_opacity=0.25
            ),
            faded_line_ratio=4,
        )
        ...
        sum_output_indicator = self.get_output_indicator(
            axes3, wave3, phasor1.t_tracker
        )
        ...
        sum_arc = Arc(
            angle=sum_parameters[1].get_value(), radius=0.5, arc_center=sum_plane.n2p(0)
        )
        ...
        self.wait()
        self.play(
            TransformFromCopy(sum_parameters[1], sum_phi_copy), ShowCreation(sum_arc)
        )
        self.wait()
        self.play(
            *map(
        ...
        self.wait()
        self.play(*self.get_wave_change_animations(wave2, "phi", phi0 - PI), run_time=3)
        self.wait()
    ...
    def get_axes(self, x_range=(0, 8), y_range=(-1, 1), width=11, height=1.25):
        result = Axes(x_range, y_range, width=width, height=height)
    ...
    def get_variable_wave(
        ...
        wave = axes.get_graph(func, stroke_color=color, bind=True)
        labels = VGroup(
            *(
    ...
    def get_phasor(self, axes, wave_group, plane_height=2.0):
        ...
        plane = ComplexPlane(
            ...
            background_line_style=dict(stroke_color=GREY_B, stroke_width=1),
            faded_line_style=dict(
                stroke_color=GREY_B, stroke_width=0.5, stroke_opacity=0.5
            ),
            faded_line_ratio=4,
        )
        ...
        vector.add_updater(
            lambda m: m.put_start_and_end_on(plane.n2p(0), plane.n2p(get_z0()))
        )
        ...
        rot_vect.add_updater(
            lambda m: m.put_start_and_end_on(plane.n2p(0), plane.n2p(get_z()))
        )
        ...
        y_line.add_updater(
            lambda m: m.put_start_and_end_on(rot_vect.get_end(), y_dot.get_center())
        )
    ...
    def get_output_indicator(self, axes, graph, t_tracker):
        ...
        v_line.add_updater(
            lambda m: m.put_start_and_end_on(axes.x_axis.n2p(get_t()), dot.get_center())
        )
    ...
    def change_wave_parameter(self, wave_group, parameter, value, run_time=2):
        self.play(
            *self.get_wave_change_animations(
                wave_group,
                parameter,
                value,
            ),
...
class WavePlusLayerInfluence(InteractiveScene):
    ...
    def construct(self):
        ...
        vect_wave2 = GraphAsVectorField(
            mid_axes, wave2_func, stroke_color=wave2.get_color(), **field_kw
        )
        vect_wave3 = GraphAsVectorField(
            low_axes, sum_func, stroke_color=wave3.get_color(), **field_kw
        )
        ...
        for x in layer_xs:
            ...
            charges.add_updater(
                lambda m: m.move_to(
                    mid_axes.c2p(
                        m.x,
                        m.amplitude_tracker.get_value()
                        * wave1.xt_to_yz(m.x, wave1.time)[0],
                        0,
                    )
                )
            )
        ...
        buff = 0.1
        glass.set_shape(*(dim + buff for dim in layers.get_shape()))
        ...
        self.play(
            LaggedStart(
                *(layer.amplitude_tracker.animate.set_value(0.1) for layer in layers),
                lag_ratio=0,
                time_span=(0, 2),
            ),
            LaggedStart(
                *(FadeIn(layer, suspend_mobject_updating=False) for layer in layers),
                lag_ratio=0,
                time_span=(0, 2),
            ),
        )
        self.play(self.frame.animate.reorient(-90, -40, 90), FadeOut(glass), run_time=9)
        self.wait(3)
        ...
        self.play(VFadeIn(wave2_group), FadeIn(mid_axes), run_time=1)
        self.wait(5)
        self.play(
        ...
        self.play(
            ...
            *(label.animate.set_x(FRAME_WIDTH - 2.5, RIGHT) for label in labels),
            run_time=2,
        )
        self.wait(9)
        ...
        self.play(wave1_copy.animate.match_y(wave3), run_time=2)
        self.wait()
        ...
        for stretch_factor in [3.0, 0.5, 2.0, 0.5, 1.0]:
            ...
            self.play(
                ...
                wave2_scale_tracker.animate.set_value(
                    stretch * wave2_scale_tracker.get_value()
                ),
