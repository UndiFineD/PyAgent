...
class DrivenHarmonicOscillator(InteractiveScene):
    def construct(self):
        ...
        self.play(
            frame.animate.reorient(-90, 0, 90)
            .center()
            .set_focal_distance(100)
            .set_anim_args(time_span=(0, 4)),
            charges.animate.center()
            .set_height(charges_target_height)
            .set_radius(radius)
            .shift(in_shift),
            run_time=6,
        )
        ...
        z_axis_field = TimeVaryingVectorField(
            time_func, height=0, width=0, depth=16, z_density=5, **field_config
        )
        ...
        self.play(
            ...
            full_field_opacity_mult.animate.set_value(1).set_anim_args(
                time_span=(2, 4)
            ),
        ...
        self.play(
            ...
            full_field_opacity_mult.animate.set_value(0).set_anim_args(
                time_span=(8, 10)
            ),
        ...
        plot_rect, plot_axes, plot = self.get_plot_group(
            lambda: 2 * sho.get_y(), width=FRAME_WIDTH - 1, max_t=20
        )
        ...
        self.play(
            omega0_eq_copy.animate.set_height(0.4)
            .move_to(plot_group1, UR)
            .shift(SMALL_BUFF * DL)
        )
        self.wait()
        ...
        self.add(
            BackgroundRectangle(close_freq_words).set_fill(BLACK, 1), close_freq_words
        )
...
class JigglesInCalcite(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            frame.animate.reorient(1, 84, 0)
            .move_to([-0.08, -0.16, -0.53])
            .set_height(9),
            run_time=3,
        )
        self.wait()
        ...
        springs = VGroup(
            ...
            *(
                Spring(
                    calcium, calcium.get_center() + spring_length * h_vect, edge=h_vect
                )
        ...
        def time_func(points, time):
            result = np.zeros(points.shape)
            result[:, self.polarization_direction] = F_max * np.cos(
                wave_number * points[:, 2] - omega * time
            )
            return result
