# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\optics_puzzles\bending_waves.py
...
class WavesByOpacity(ScalarFieldByOpacity):
    def __init__(
        self, wave: VectorField, vects_to_opacities=lambda v: np.tanh(v[:, 2]), **kwargs
    ):
        super().__init__(
            opacity_func=lambda p: vects_to_opacities(wave.func(p)), **kwargs
        )
...
class SnellsLaw(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            tank.animate.move_to(beam.pfp(0.49)).set_anim_args(
        ...
        self.play(
            tank.animate.rotate(get_theta2() - get_theta1()).move_to(
                hit_point + 0.3 * UP + 0.1 * LEFT
            ),
            rate_func=linear,
            run_time=1.5,
        )
        self.play(
            tank.animate.move_to(beam.pfp(0.65)).set_anim_args(
        ...
        # Write Snell's law
        law = Tex(
            R"{\sin(\theta_1) \over v_1} = {\sin(\theta_2) \over v_2}", font_size=52
        )
        ...
        self.play(
            LaggedStart(
                *(get_dot_anim() for x in range(50)),
                lag_ratio=0.02,
                run_time=20,
            )
        )
    ...
    def get_beam(self, angle_tracker, hit_point, stroke_color=YELLOW, stroke_width=3):
        ...
        def update_beam(beam):
            ...
            beam.set_points_as_corners(
                [
                    hit_point
                    + 0.6 * FRAME_HEIGHT * rotate_vector(UP, theta1) / math.cos(theta1),
                    hit_point,
                    hit_point
                    + 0.6
                    * FRAME_HEIGHT
                    * rotate_vector(DOWN, theta2)
                    / math.cos(theta2),
                ]
            )
...
class WavesIntoAngledMedium(InteractiveScene):
    ...
    def get_medium(
        ...
        medium.rotate(
            angle_of_vector(self.interface_normal), about_point=self.interface_origin
        )
        return medium
    ...
    def get_wave_dots(
        self, wave, density=20, offset=0.2, color=WHITE, max_opacity=1.0, **kwargs
    ):
        return WavesByOpacity(
            wave,
            density=density,
            vects_to_opacities=lambda v: max_opacity
            * np.tanh(v[:, 2] - offset * self.amplitude),
            color=color,
            **kwargs,
        )
...
class TransitionToOverheadView(WavesIntoAngledMedium):
    ...
    def construct(self):
        ...
        self.play(
            ...
            braces.animate.rotate(90 * DEGREES, RIGHT, about_point=ORIGIN).shift(
                0.3 * OUT
            ),
            run_time=5,
        )
...
class AngledMediumSingleFront(AngledMedium):
    def get_wave_dots(
        self, wave, density=20, offset=0.2, color=RED, max_opacity=1.0, **kwargs
    ):
        return super().get_wave_dots(
            wave, density, offset, color, max_opacity, **kwargs
        )
...
class LineGame(InteractiveScene):
    def construct(self):
        ...
        low_lines = always_redraw(
            lambda: self.get_line_group(
                interface,
                rotate_vector(dial.get_vector(), -PI / 2),
                small_spacing,
                line_color=GREEN,
                dot_color=YELLOW,
            )
    ...
    def highlight_intersection_points(self, line_group, color, arrow_direction=LEFT):
        ...
        self.play(
            LaggedStartMap(
                Indicate, line_group[0], scale_factor=2, color=color, lag_ratio=0.2
            ),
            LaggedStartMap(
                FlashAround, line_group[0], buff=0.2, color=color, lag_ratio=0.2
            ),
            run_time=4,
        )
...
class Prism(InteractiveScene):
    def construct(self):
        ...
        self.play(
            FadeOut(flat_prism),
            FadeIn(prism),
            frame.animate.reorient(-90, 35, 90)
            .set_height(15)
            .move_to(RIGHT + 4 * DOWN),
            run_time=4,
        )
        ...
        for vect, alpha in zip([5 * DOWN, 3 * DOWN, 4 * DOWN], [0.3, 0.3, 0.5]):
            self.play(
                in_beam.animate.put_start_and_end_on(
                    left_side + vect, in_edge.pfp(alpha)
                ),
                # run_time=5
            )
        ...
        self.play(
            ...
            in_beam.animate.put_start_and_end_on(
                left_side + 2 * DOWN, in_edge.get_center()
            ),
    ...
    def get_beams(
        self, min_index, max_index, n_beams, in_beam, in_edge: Line, out_edge: Line
    ):
        ...
        out_points = np.array(
            [
                find_intersection(
                    in_point, vect2, out_edge.get_start(), out_edge.get_vector()
                )
                for vect2 in vect2s
            ]
        )
