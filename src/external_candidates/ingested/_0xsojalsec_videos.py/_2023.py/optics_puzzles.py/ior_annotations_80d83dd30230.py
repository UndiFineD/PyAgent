# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\optics_puzzles\ior_annotations.py
...
class HoldUpAlbumCover(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        true_prism = Group(
            SurroundingRectangle(prism_image, buff=0).set_stroke(WHITE, 10), prism_image
        )
...
class DefineIndexOfRefraction(InteractiveScene):
    def construct(self):
        ...
        equation = Tex(
            Rf"{{{num_words} \over {den_words}}} \approx {ior}", font_size=36
        )
...
class LookAtScrolling(InteractiveScene):
    ...
    def construct(self):
        ...
        for _ in range(4):
            self.play(morty.change(random.choice(["tease", "pondering"]), 3 * UP))
...
class WhySlowAtAll(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.play(
            self.change_students("hesitant", "maybe", "pondering", look_at=self.screen)
        )
        self.wait(3)
...
class TankAnnotations(InteractiveScene):
    def construct(self):
        ...
        self.play(Flash(point), FadeIn(words, run_time=0.5))
        self.wait()
...
class NewQuestion(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(FlashUnder(q2["phase"]), q2["phase"].animate.set_color(YELLOW))
        self.wait()
...
class HoldUpLastVideo(TeacherStudentsScene):
    def construct(self):
        ...
        image = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/barber_pole/Thumbnails/Part2Thumbnail4.png"
        )
        tn = Group(
        ...
        self.play(FadeIn(label, lag_ratio=0.1), GrowArrow(arrow))
        self.wait(4)
...
class AnnoateWaveSumPhaseShift(InteractiveScene):
    def construct(self):
        ...
        arrow = Arrow(lines2[1].get_top(), lines1[1].get_bottom(), buff=0.2)
    ...
    def construct(self):
        ...
        plane = ComplexPlane(
            ...
            faded_line_style=dict(
                stroke_width=1, stroke_opacity=0.3, stroke_color=BLUE
            ),
        )
        plane.set_height(7)
        self.add(plane)
        ...
        vects = VGroup(*(Vector(plane.n2p(z), stroke_width=2) for z in zs))
        ...
        self.add(big_vect)
        self.play(ShowIncreasingSubsets(vects, rate_func=linear, run_time=8))
        self.wait()
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "pondering", "pondering", "pondering", look_at=true_law
            ),
        )
        self.wait(2)
        self.play(
            ShowCreation(rect),
            true_law[R" + c_2 x^2 + c_3 x^3 + c_4 x^4 + \cdots"].animate.set_opacity(
                0.2
            ),
            morty.change("tease"),
        )
...
class FrequencyVsAngularFrequency(TeacherStudentsScene):
    def construct(self):
        ...
        morty.change_mode("raise_right_hand")
        self.play(
            ShowCreation(cross), self.change_students("sassy", "hesitant", "angry")
        )
        rf_words.add(cross)
        ...
        plane = ComplexPlane(
            ...
            faded_line_style=dict(
                stroke_width=0.5, stroke_opacity=0.5, stroke_color=BLUE
            ),
        )
        plane.set_height(4)
        plane.to_edge(DOWN)
        ...
        vect.add_updater(
            lambda m: m.put_start_and_end_on(
                plane.n2p(0), plane.n2p(np.exp(complex(0, angle_tracker.get_value())))
            )
        )
        ...
        dot = GlowDot()
        dot.add_updater(
            lambda m: m.move_to(plane.n2p(math.cos(angle_tracker.get_value())))
        )
        line = Line(UP, DOWN)
        line.set_stroke(YELLOW, 1)
        line.add_updater(
            lambda m: m.put_start_and_end_on(vect.get_end(), dot.get_center())
        )

        self.play(MoveToTarget(plane), FadeIn(axes, 2 * LEFT))
...
class PiCreaturesReactingToEquation(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(3)
        self.play(
            self.change_students("thinking", "pondering", "pondering", look_at=point)
        )
        self.wait(3)
        self.play(
...
class HighlightExpression(InteractiveScene):
    def construct(self):
        ...
        rect2 = SurroundingRectangle(
            expr[
                R"\frac{q ||\vec{\textbf{E}}_0||}{m\left(\omega_r^2-\omega_l^2\right)}"
            ]
        )
...
class GuessSteadyState(InteractiveScene):
    def construct(self):
        ...
        solution = Tex(
            Rf"\vec{{\textbf{{x}}}}(t) = {amp_tex} \cos(\omega_l t)", t2c=EQUATION_T2C
        )
        solution.move_to(np.array([3.85, 0.65, 0.0]))

        self.play(
            rect.animate.set_height(
                FRAME_HEIGHT, about_edge=UP, stretch=True
            ).set_anim_args(run_time=2),
            FadeOut(VGroup(prompt[0], A_arrow, A_word, freq_arrow, freq_word)),
            TransformMatchingTex(
                prompt[1],
                solution,
                matched_pairs=[
                    (
                        prompt[1][R"\vec{\textbf{x}}(t)"],
                        solution[R"\vec{\textbf{x}}(t)"],
                    ),
...
class StrongerResonanceStrongerPhaseShift(InteractiveScene):
    def construct(self):
        words = TexText(
            R"Stronger resonance $\rightarrow$ Bigger phase kick", font_size=60
        )
        words.to_edge(UP)
        self.add(words)
...
class QuestionsFromPatrons(InteractiveScene):
    def construct(self):
        ...
        self.play(
            FadeIn(back_rect), FadeIn(grey_rect), DrawBorderThenFill(highlight_rect)
        )
        self.wait()
        ...
        for question in questions:
            ...
            question.target[0].scale(0, about_point=question[1].get_left()).set_opacity(
                0
            )
...
class HoldUpMainVideo(TeacherStudentsScene):
    def construct(self):
        # Show previous video
        thumbnail = get_bordered_thumbnail(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/barber_pole/Thumbnails/PrismThumbnail2.png"
        )
        ...
        self.play(
            ...
            self.change_students(
                "pondering", "hesitant", "pondering", look_at=thumbnail.target
            ),
...
class HoldUpBarberPoleVideos(HoldUpMainVideo):
    def construct(self):
        ...
        self.wait()
        self.play(self.students[2].change("tease", thumbnails))
        self.wait(2)
...
class MissingDetails(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn, details, shift=0.5 * DOWN, lag_ratio=0.35, run_time=3
            )
        )
        self.wait()
...
class KickForwardOrBackCondition(InteractiveScene):
    def construct(self):
        ...
        self.play(GrowArrow(arrow), FadeIn(words[1], RIGHT))
        self.wait()
...
class ThisReallyDoesHappen(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.change_students("sassy", "confused", "pleading", look_at=self.screen)
        )
        self.wait(4)
...
class CausalInfluence(InteractiveScene):
    def construct(self):
        words = TexText(
            R"No causal influence can \\ propagate faster than $c$", font_size=72
        )
        c = words["$c$"]
        c.set_color(YELLOW)
...
class SameRotationRate(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(words), LaggedStartMap(GrowArrow, arrows))
        self.wait()
