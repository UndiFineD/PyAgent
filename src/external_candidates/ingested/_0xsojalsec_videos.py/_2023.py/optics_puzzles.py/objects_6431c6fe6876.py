# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\optics_puzzles\objects.py
...
class OscillatingWave(VMobject):
    ...
    def update_points(self, dt):
        if not self.clock_is_stopped:
            self.time += dt
        xs = np.arange(
            self.axes.x_axis.x_min, self.axes.x_axis.x_max, self.sample_resolution
        )
    ...
    def xt_to_yz(self, x, t):
        ...
        y_outs = self.y_amplitude * np.sin(
            TAU * x / self.wave_len - phase - self.y_phase
        )
        z_outs = self.z_amplitude * np.sin(
            TAU * x / self.wave_len - phase - self.z_phase
        )
...
class MeanWave(VMobject):
    ...
    def xt_to_yz(self, x, t):
        return tuple(
            np.array([wave.xt_to_yz(x, t)[i] for wave in self.waves]).mean(0)
            for i in (0, 1)
        )
...
class ChargeBasedVectorField(VectorField):
    ...
    def __init__(self, *charges, **kwargs):
        ...
        super().__init__(
            self.get_forces, color=kwargs.pop("color", self.default_color), **kwargs
        )
...
class CoulombField(ChargeBasedVectorField):
    ...
    def get_forces(self, points):
        return sum(coulomb_force(points, charge) for charge in self.charges)


class LorentzField(ChargeBasedVectorField):
    def __init__(self, *charges, radius_of_suppression=None, c=2.0, **kwargs):
    ...
    def get_forces(self, points):
        return sum(
            lorentz_force(points, charge, radius=self.radius_of_suppression, c=self.c)
...
class ColoumbPlusLorentzField(LorentzField):
    def get_forces(self, points):
        return sum(
            lorentz_force(points, charge, radius=self.radius_of_suppression, c=self.c)
            + sum(coulomb_force(points, charge) for charge in self.charges)
            for charge in self.charges
        )
...
class GraphAsVectorField(VectorField):
    def __init__(
        ...
        **kwargs,
        ...
        self.sample_xs = np.arange(
            axes.x_axis.x_min, axes.x_axis.x_max, 1.0 / x_density
        )
        self.axes = axes
...
class OscillatingFieldWave(GraphAsVectorField):
    def __init__(self, axes, wave, **kwargs):
        ...
        super().__init__(
            axes=axes, graph_func=lambda x: wave.xt_to_yz(x, wave.time), **kwargs
        )
...
class Molecule(Group):
    ...
    def __init__(self, height=2.0, **kwargs):
        ...
        rgbas = np.array(
            [color_to_rgba(self.atom_to_color[atom]) for atom in self.atoms]
        )
        ...
        for i, j in self.bonds:
            ...
            sticks.add(
                Line(c1 + r1 * unit_vect, c2 - r2 * unit_vect, **self.stick_config)
            )
...
class Carbonate(Molecule):
    ...
    coordinates = np.array(
        [
            [-6.9540e-01, -1.1061e00, 0.0000e00],
            [-6.9490e-01, 1.1064e00, 0.0000e00],
            [1.3055e00, -3.0000e-04, 1.0000e-04],
            [8.4700e-02, 0.0000e00, -1.0000e-04],
            [-1.6350e-01, -1.9304e00, 1.0000e-04],
            [-1.6270e-01, 1.9305e00, 1.0000e-04],
        ]
    )
...
class Calcite(Molecule):
    ...
    coordinates = np.array(
        [
            ...
            [-1.43769337, 2.49015797, 1.41822405],
            [-2.87538675, 0.00000000, 2.83644809],
            [-2.87538675, 0.00000000, 7.09112022],
            ...
            [2.87538675, 0.00000000, 1.41822405],
            [1.43769337, -2.49015797, 2.83644809],
            [1.43769337, -2.49015797, 7.09112022],
            [1.82730828, -0.60510839, 1.41822404],
            [2.87538675, 4.98031594, 1.41822405],
            [1.43769337, 2.49015797, 2.83644809],
            [1.43769337, 2.49015797, 7.09112022],
            [-0.38961490, 1.88504958, 1.41822404],
            [1.82730828, 4.37520755, 1.41822404],
            [2.87538675, 1.21021677, 1.41822404],
            [0.00000000, 0.00000000, 0.00000000],
            [0.00000000, 0.00000000, 8.50934427],
            [0.00000000, 0.00000000, 4.25467213],
            [-1.04807847, 0.60510839, 4.25467213],
            [1.04807847, 0.60510839, 4.25467213],
            [0.00000000, -1.21021677, 4.25467213],
            [-1.82730828, -0.60510839, 7.09112022],
            [0.38961490, 1.88504958, 7.09112022],
            ...
            [-0.38961490, -1.88504958, 9.92756831],
            [2.87538675, 9.96063187, 1.41822405],
            [1.43769337, 7.47047390, 2.83644809],
            [1.43769337, 7.47047390, 7.09112022],
            [-0.38961490, 6.86536552, 1.41822404],
            [1.82730828, 9.35552349, 1.41822404],
            [2.87538675, 6.19053271, 1.41822404],
            [0.00000000, 4.98031594, 0.00000000],
            [0.00000000, 4.98031594, 8.50934427],
            [0.00000000, 4.98031594, 4.25467213],
            [-1.04807847, 5.58542432, 4.25467213],
            [1.04807847, 5.58542432, 4.25467213],
            [0.00000000, 3.77009916, 4.25467213],
            [-1.82730828, 4.37520755, 7.09112022],
            [0.38961490, 6.86536552, 7.09112022],
            [1.43769337, 3.70037474, 7.09112022],
            [-1.43769337, 2.49015797, 5.67289618],
            [-1.43769337, 2.49015797, 9.92756831],
            [-2.48577184, 3.09526635, 9.92756831],
            [-0.38961490, 3.09526635, 9.92756831],
            [-1.43769337, 1.27994120, 9.92756831],
            [0.00000000, 9.96063187, 0.00000000],
            [0.00000000, 9.96063187, 8.50934427],
            [0.00000000, 9.96063187, 4.25467213],
            [-1.04807847, 10.56574026, 4.25467213],
            [1.04807847, 10.56574026, 4.25467213],
            [0.00000000, 8.75041510, 4.25467213],
            ...
            [-0.38961490, 8.07558229, 9.92756831],
            [-1.43769337, 6.26025713, 9.92756831],
            [7.18846687, -2.49015797, 1.41822405],
            [7.18846687, 2.49015797, 1.41822405],
            [5.75077349, 0.00000000, 2.83644809],
            [5.75077349, 0.00000000, 7.09112022],
            [3.92346522, -0.60510839, 1.41822404],
            [6.14038840, 1.88504958, 1.41822404],
            [7.18846687, -1.27994119, 1.41822404],
            [4.31308012, -2.49015797, 0.00000000],
            ...
            [4.70269502, -0.60510839, 7.09112022],
            ...
            [2.87538675, 0.00000000, 5.67289618],
            [2.87538675, 0.00000000, 9.92756831],
            [1.82730828, 0.60510839, 9.92756831],
            [3.92346522, 0.60510839, 9.92756831],
            ...
            [4.31308012, 7.47047390, 0.00000000],
            ...
            [10.45346852, -0.60510839, 1.41822404],
            [10.06385361, 2.49015797, 2.83644809],
            [10.06385361, 2.49015797, 7.09112022],
            [8.23654533, 1.88504958, 1.41822404],
            [10.45346852, 4.37520755, 1.41822404],
            [8.62616024, 0.00000000, 0.00000000],
            [8.62616024, 0.00000000, 8.50934427],
            [8.62616024, 0.00000000, 4.25467213],
            [7.57808177, 0.60510839, 4.25467213],
            [9.67423871, 0.60510839, 4.25467213],
            [8.62616024, -1.21021677, 4.25467213],
            [6.79885196, -0.60510839, 7.09112022],
            ...
            [8.62616024, 4.98031594, 0.00000000],
            ...
            [8.62616024, 9.96063187, 0.00000000],
            ...
            [10.45346852, 0.60510839, 9.92756831],
