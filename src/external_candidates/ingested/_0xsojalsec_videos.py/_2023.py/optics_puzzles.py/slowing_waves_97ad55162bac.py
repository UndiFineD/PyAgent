# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\optics_puzzles\slowing_waves.py
...
class SlicedWave(Group):
    ...
    def __init__(
        ...
        vwave_kw = merge_dicts_recursively(
        ...
        super().__init__(
            self.wave, self.vect_wave, self.layers, *self.phase_kick_trackers
        )
    ...
    def xt_to_yz(self, x, t):
        ...
        for layer_x, pkt, at in zip(
            self.layer_xs, self.phase_kick_trackers, self.absorbtion_trackers
        ):
...
class RevertToOneLayerAtATime(PhaseKickBacks):
    ...
    line_style = dict(stroke_width=1, stroke_opacity=0.25, stroke_color=BLUE_B)
    axes_config = dict(
        x_range=(-12, 12),
        y_range=(-4, 4),
    )
    wave_config = dict(color=YELLOW, sample_resolution=0.001)
    ...
    def construct(self):
        ...
        self.play(
            layers.animate.arrange(RIGHT, buff=0.3)
            .move_to(ORIGIN, LEFT)
        ...
        self.play(
            ...
            layers[0]
            .animate.set_stroke(width=2, opacity=1)
            .set_height(5)
            .set_anim_args(time_span=(4, 5)),
            FadeTransform(block_label, layer_label, time_span=(4, 5)),
        ...
        self.add(form2)
        self.play(
            pk_decimal.animate.match_height(pk_label).next_to(
                pk_label, RIGHT, 0.2, DOWN
            ),
        ...
        for layer, pkt in zip(layers[nls::nls], pkts[nls::nls]):
            ...
            self.play(FadeIn(kick_label, 0.5 * LEFT), pkt.animate.set_value(phase_kick))
        ...
        for value in [-0.01, self.exagerated_phase_kick]:
            ...
            self.play(
                ...
                *(pkt.animate.set_value(phase_kick) for pkt in pkts[::nls]),
                run_time=4,
            )
            self.wait(2)
        self.wait(2)
        ...
        while nls > 1:
            ...
            new_pk_decimal = DecimalNumber(
                -phase_kick, num_decimal_places=(2 if -phase_kick > 0.05 else 3)
            )
            ...
            self.play(
                LaggedStart(*(GrowFromCenter(layer) for layer in new_layers), **kw),
                old_layers.animate.set_stroke(width=stroke_width, opacity=opacity),
                LaggedStart(
                    *(pkt.animate.set_value(phase_kick) for pkt in pkts[::nls]), **kw
                ),
                LaggedStart(
            ...
            self.play(*(pkt.animate.set_value(phase_kick) for pkt in pkts[::nls]))
...
class SlowedAndAbsorbed(RevertToOneLayerAtATime):
    ...
    def construct(self):
        ...
        mu_indicator.add_updater(
            lambda m: m.move_to(mu_line.n2p(mu_tracker.get_value()), RIGHT)
        )
        ...
        self.play(
            ...
            FadeIn(mu_indicator),
            FadeIn(mu_label),
            LaggedStart(*(pkt.animate.set_value(self.kick_back_value) for pkt in pkts)),
            LaggedStart(*(at.animate.set_value(self.damping_per_layer) for at in ats)),
            LaggedStart(*(FadeIn(layer, scale=0.8) for layer in layers)),
            run_time=5,
        )
...
class DissolveLayers(PhaseKickBacks):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(*(pkt.animate.set_value(0) for pkt in pkts[:0:-1]), **kw),
            layers[0]
            .animate.set_stroke(WHITE, 2)
            .set_height(5)
            .set_anim_args(time_span=(7, 8)),
        )
        self.wait(3)
...
class PhaseKickBackAddInLayers(PhaseKickBacks):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(FadeIn(line, 0.5 * DOWN) for line in sliced_wave.layers), **lag_kw
            ),
            LaggedStart(
                *(
...
class FastWave(PhaseKickBacks):
    ...
    line_style = dict(stroke_width=1, stroke_opacity=0.35, stroke_color=BLUE_D)
    wave_config = dict(color=YELLOW, sample_resolution=0.001)
...
class KickForward(PhaseKickBacks):
    ...
    def construct(self):
        ...
        kick_forward = VGroup(
            ...
            Text("Kick forward\nthe phase", font_size=36, t2s={"forward": ITALIC}),
        )
        ...
        self.play(
            ...
            UpdateFromFunc(
                kick_forward,
                lambda m: m.align_to(remaining_layers.get_right(), LEFT).shift(
                    MED_SMALL_BUFF * RIGHT
                ),
            ),
            LaggedStart(
                *(pkt.animate.set_value(self.kick_back_value) for pkt in pkts[1:]),
                lag_ratio=1,
            ),
