# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\optics_puzzles\annotations.py
...
class AnnotateDemo(InteractiveScene):
    def construct(self):
        image = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/barber_pole/images/DemoStill.jpg"
        )
...
class DidntSteveCover(TeacherStudentsScene):
    def construct(self):
        ...
        morty = self.teacher
        image = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/barber_pole/chat with Steve/MouldThumbnail.jpg"
        )
        image.set_height(3)
        thumbnail = Group(
            SurroundingRectangle(image, buff=0).set_stroke(WHITE, 3), image
        )
...
class WhatWeNeedToUnderstand(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(3)
        self.play(self.change_students("erm", "sassy", "maybe", look_at=self.screen))
        self.wait(3)
...
class Questions(InteractiveScene):
    def construct(self):
        ...
        for question in last_questions:
            ...
            self.wait()
        self.wait()
        self.play(FadeIn(questions[0], shift=0.25 * DOWN), Restore(last_questions))
        self.wait()
            ...
            cross_lines.add(
                VGroup(
                    *(
                        Line(LEFT, RIGHT)
                        .set_stroke(YELLOW, 3)
                        .replace(full_question[phrase], dim_to_match=0)
        ...
        # Question 2
        self.play(arrow.animate.next_to(questions[2], RIGHT))
        self.wait()
...
class FromOnHigh(InteractiveScene):
    def construct(self):
        ...
        top_arrows = VGroup(
            *(Arrow(top_words.get_bottom(), rect.get_top()) for rect in rects)
        )
        low_arrows = VGroup(
            *(Arrow(low_words.get_top(), rect.get_bottom()) for rect in rects)
        )
...
class RSquaredVsR(InteractiveScene):
    def construct(self):
        ...
        # Add graphs
        graph1 = axes.get_graph(lambda x: 1 / x**2, x_range=(0.1, 10)).set_stroke(
            YELLOW, 5
        )
...
class MentionMaxwell(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            FadeTransform(
                stds[2].bubble.content["Maxwell's equations"].copy(), equations_title
            ),
            stds[2].debubble(),
...
class BasicallyZ(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeIn(words, 0.25 * UP), FadeIn(rect))
        self.wait()
...
class StrengthInDifferentDirectionsWithDecimal(InteractiveScene):
    def construct(self):
        ...
        angle_label.add_updater(
            lambda m: m.set_height(clip(arc.get_height(), 0.01, 0.4))
        )
        angle_label.add_updater(
            lambda m: m.next_to(
                arc.pfp(0.3), normalize(arc.pfp(0.3)), SMALL_BUFF, aligned_edge=DOWN
            )
        )
        ...
        def get_cos_tex():
            ...
            cos_tex.make_number_changeable("10", edge_to_fix=RIGHT).set_value(
                line.get_angle() / DEGREES
            )
        ...
        self.play(Rotate(rot_group, 30 * DEGREES, about_point=ORIGIN), run_time=3)
        self.wait(2)
        self.play(
...
class ContinuousWave(InteractiveScene):
    def construct(self):
        ...
        wave.add_updater(
            lambda m: m.set_points(points).stretch(
                (1 - math.sin(self.time)),
                1,
            )
        )
        self.add(wave)
...
class RedFilter(InteractiveScene):
    def construct(self):
        image = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/barber_pole/images/RedFilter.jpg"
        )
        image.set_height(FRAME_HEIGHT)
...
class LengthsOnDifferentColors(InteractiveScene):
    def construct(self):
        # Setup
        image = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/barber_pole/images/RainbowTubes.jpg"
        )
        image.set_height(FRAME_HEIGHT)
        # self.add(image)
        ...
        braces = VGroup(*(Brace(line, DOWN, buff=SMALL_BUFF) for line in lines))
        ...
        for brace, rect, number in zip(braces, rects, numbers):
            ...
            self.play(
                ...
                UpdateFromFunc(
                    VGroup(), lambda m: number.next_to(brace, DOWN, SMALL_BUFF)
                ),
...
class AskNoteVerticalVariation(RedFilter):
    def construct(self):
        image = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2023/barber_pole/images/GreenFilter.jpg"
        )
        image.set_height(FRAME_HEIGHT)
        # self.add(image)
...
class IndexOfRefraction(InteractiveScene):
    def construct(self):
        # Test
        equation = Tex(
            R"{\text{Speed in a vacuum} \over \text{Speed in water}} \approx 1.33"
        )
        equation.shift(UP)
...
class SugarIsChiral(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(self.frame.animate.reorient(-16, 75, 0).set_anim_args(run_time=6))
        self.add(sucrose)
        self.wait()
        ...
        # Chiral definition
        definition = TexText(
            R"Chiral $\rightarrow$ Cannot be superimposed onto its mirror image"
        )
...
class SucroseAction(InteractiveScene):
    ...
    def construct(self):
        ...
        for sign, word in zip([+1, -1], words):
            arrow = Line(
                ORIGIN, 5 * RIGHT + 1.5 * sign * UP, path_arc=-sign * 60 * DEGREES
            )
            ...
            self.play(ShowCreation(arrow), FadeIn(word, 0.25 * sign * UP))
            self.wait(2)
