# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\optics_puzzles\e_field.py
from _2023.optics_puzzles.objects import *
from manim_imports_ext import *


def get_influence_ring(
    center_point, color=WHITE, speed=2.0, max_width=3.0, width_decay_exp=0.5
):
    ring = Circle()
...
class IntroduceEField(InteractiveScene):
    def construct(self):
        ...
        force_words[0].add_updater(
            lambda w: place_force_word_on_arrow(w, coulomb_vects[0])
        )
        force_words[1].add_updater(
            lambda w: place_force_word_on_arrow(w, coulomb_vects[1])
        )
        ...
        d_line = always_redraw(
            lambda: DashedLine(
                charges[0].get_right(), charges[1].get_left(), dash_length=0.025
            )
        )
        ...
        for buff in (0.4, 8, 1.25):
            self.play(
                charges[1].animate.next_to(charges[0], RIGHT, buff=buff), run_time=4
            )
            self.wait()
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOut,
                Group(
                    axes, graph, graph_dot, d_line, d_label, force_words, coulomb_vects
                ),
            ),
        ...
        for vect in [2 * RIGHT, 4 * LEFT, 2 * RIGHT]:
            self.play(
                charges[0].animate.shift(vect).set_anim_args(path_arc=PI, run_time=3)
            )
        self.wait()
        ...
        self.wait()
        self.play(rect.animate.surround(e_coulombs_law[R"\frac{\vec{r}}{||\vec{r}||}"]))
        self.wait()
        ...
        tmp_charges = Group(
            *(
                ChargedParticle(track_position_history=True, charge=0.3)
                for x in range(2)
            )
        )
        ...
        influence_ring0 = self.get_influence_ring(charges[0].get_center()).set_stroke(
            opacity=0
        )
        influence_ring1 = self.get_influence_ring(charges[1].get_center()).set_stroke(
            opacity=0
        )
        ...
        self.wait_until(
            lambda: influence_ring0.get_radius() > dist, max_time=dist / 2.0
        )

        self.add(influence_ring1)
        self.play(charges[1].animate.shift(0.5 * DOWN).set_anim_args(**wiggle_kwargs))
        self.wait_until(
            lambda: influence_ring1.get_radius() > dist, max_time=dist / 2.0
        )
        self.play(charges[0].animate.shift(0.25 * UP).set_anim_args(**wiggle_kwargs))
        self.wait(6)
        self.play(
            FadeOut(influence_ring0), FadeOut(influence_ring1), FadeOut(lorentz_field)
        )
        ...
        charges[0].add_updater(
            lambda m, dt: m.shift(3 * dt * (target - m.get_center()))
        )
        ...
        self.play(
            ...
            UpdateFromFunc(time_decimal, lambda m: m.set_value(ring.get_radius() / 2)),
            charges[0]
            .animate.shift(2 * DOWN)
            .set_anim_args(
                time_span=(1, 4),
        ...
        for x in range(2):
            ...
            self.wait_until(lambda: ring.get_radius() > dist)
            self.play(
                charges[1].animate.shift(0.25 * DR).set_anim_args(**wiggle_kwargs)
            )
        ...
        plane = NumberPlane(
            background_line_style=dict(
                stroke_color=GREY_D, stroke_opacity=0.75, stroke_width=1
            ),
        ...
        for point in [2 * RIGHT, ORIGIN]:
            self.play(
                charge.animate.move_to(point).set_anim_args(
                    path_arc=PI, run_time=5, suspend_mobject_updating=False
                )
            )
        self.wait(5)
        ...
        charge.ignore_last_motion()
        charge.add_updater(
            lambda m: m.move_to(0.25 * np.sin(0.5 * TAU * m.get_internal_time()) * UP)
        )
        self.wait(30)

    def get_influence_ring(
        self, center_point, color=WHITE, speed=2.0, max_width=3.0, width_decay_exp=0.5
    ):
        return get_influence_ring(
            center_point, color, speed, max_width, width_decay_exp
        )
...
class AltEFieldIntroduction(IntroduceEField):
    def construct(self):
        ...
        c_field.add_updater(
            lambda m: m.set_stroke(opacity=c_field_opacity_tracker.get_value())
        )
        ...
        l_field = ColoumbPlusLorentzField(
            c1, c2, norm_to_opacity_func=lambda n: np.arctan(n), **field_config
        )
        self.play(
            FadeOut(points),
            ...
            FadeOut(hyp_words),
            FadeOut(vect),
            *(c.animate.move_to(10 * c.get_center()) for c in charges[1:]),
        )
        ...
        self.play(
            hyp_charge.animate.next_to(charges[0], LEFT, buff=0.5).set_anim_args(
                path_arc=PI
            ),
            run_time=8,
        )
        ...
        speed_words.add_updater(
            lambda m: m.next_to(m.ring.get_right(), LEFT, SMALL_BUFF)
        )
...
class ShowTheEffectsOfOscillatingCharge(InteractiveScene):
    ...
    def add_particles(self, axes):
        ...
        self.particles.add_updater(
            lambda m: m.move_to(axes.c2p(*self.oscillation_function(self.time)))
        )
    ...
    def add_acceleration_vectors(self, particles):
        self.acceleration_vectors = VGroup(
            *(AccelerationVector(particle) for particle in particles)
        )
...
class ThreeCharges(ChargeOnZAxis):
    def get_particles(self):
        return Group(
            *(ChargedParticle(**self.particle_config) for n in range(3))
        ).arrange(UP, buff=2)
...
class Introduce3dMovements(ChargeOnZAxis):
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.frame.animate.reorient(-17, 71, 0)
            .move_to([-0.12, 0.17, 0.27])
            .set_height(7.16),
            run_time=6,
        )
        self.play(
        ...
        # self.wait(4)
        self.play(
            charge.animate.shift(0.8 * (UP + OUT)).set_anim_args(
                rate_func=lambda t: wiggle(t, 6), **kw
            ),
            self.frame.animate.reorient(20, 71, 0)
            .move_to([0.31, 0.54, -0.3])
            .set_height(8.22),
            run_time=8,
        )
        self.play(
            charge.animate.shift(4 * DOWN).set_anim_args(
                rate_func=there_and_back, **kw
            ),
            self.frame.animate.reorient(-21, 64, 0)
            .move_to([0.31, 0.54, -0.3])
            .set_height(8.22),
            run_time=9,
        )
...
class RowOfCharges(ChargeOnZAxis):
    ...
    def get_particles(self):
        return Group(
            *(ChargedParticle(**self.particle_config) for n in range(self.n_charges))
...
class AltRowOfCharges(RowOfCharges):
    def construct(self):
        # Test
        self.play(
            self.frame.animate.reorient(-4, 82, 0)
            .move_to([3.12, -0.06, 1.0])
            .set_height(5.23),
            run_time=12,
        )
        self.play(self.frame.animate.reorient(-20, 69, 0).set_height(8.00), run_time=12)
        self.play(
            self.frame.animate.reorient(-13, 78, 0)
            .move_to([4.32, -0.91, 0.42])
            .set_height(5.27),
            run_time=12,
        )
...
class RowOfChargesXAxis(RowOfCharges):
    ...
    def construct(self):
        ...
        # Zoom in
        self.play(
            self.frame.animate.reorient(-15, 84, 0)
            .move_to([4.36, -1.83, 0.37])
            .set_height(5.59),
            run_time=3,
        )
        ...
        self.wait()
        self.play(
            FadeOut(words[0], 0.5 * UP), FadeIn(words[1], 0.5 * UP), Transform(*graphs)
        )
        self.wait(6)
...
class WiggleHereWiggleThere(IntroduceEField):
    def construct(self):
        # Setup
        charges = Group(
            *(ChargedParticle(track_position_history=True) for _ in range(2))
        )
...
class CircularPolarization1D(ShowTheEffectsOfOscillatingCharge):
    ...
    def construct(self):
        ...
        field.add_updater(
            lambda m: m.set_stroke(opacity=field_opacity_tracker.get_value())
        )
...
class PolarizedScattering(RandomRicochet):
    ...
    def construct(self):
        ...
        vects.add_updater(
            lambda m: m.set_stroke(opacity=vects_opacity_tracker.get_value())
        )
        ...
        charge = ChargedParticle(
            show_sign=False, radius=0.1, track_position_history=True
        )
        ...
        for dv in direction_vectors.target:
            ...
            dv.rotate(
                70 * DEGREES, axis=cross(dv.get_vector(), OUT), about_point=ORIGIN
            )
...
class OneOfManyCharges(InteractiveScene):
    ...
    def setup(self):
        ...
        def plane_wave_func(points, t):
            return self.plane_wave_amlpitude * np.outer(
                np.cos(k * np.dot(points, RIGHT) - omega * t), OUT
            )

        plane_wave = TimeVaryingVectorField(
            plane_wave_func, **self.plane_wave_field_config
        )

        plane_wave.opacity_multiplier = ValueTracker(1)
        plane_wave.add_updater(
            lambda m: m.set_stroke(
                opacity=m.opacity_multiplier.get_value() * m.get_stroke_opacities()
            )
        )
    ...
    def construct(self):
        ...
        if self.charge_index > 0:
            ...
            charge.add_updater(
                lambda m: m.move_to(self.dots.get_points()[self.charge_index])
            )
            charge.update()
            charge.ignore_last_motion()
...
class AlternateCompositeChargesInPlane(OneOfManyCharges):
    ...
    def construct(self):
        ...
        for charge, index in zip(charges, indices):
            charge.index = index
            charge.add_updater(
                lambda m: m.move_to(dots.get_points()[m.index] + 0.01 * DOWN)
            )
        ...
        charge_field = LorentzField(
            charges[0], radius_of_suppression=0.2, **self.charge_field_config
        )
        self.add(charges[0])
...
class FullCompositeEffect(OneOfManyCharges):
    ...
    def construct(self):
        ...
        new_wave.add_updater(
            lambda m: m.set_stroke(
                opacity=m.get_stroke_opacities() * m.opacity_multiplier.get_value()
            )
        )
        ...
        for _ in range(10):
            ...
            for x in np.linspace(min_x, max_x, 6):
                for z in np.linspace(min_z, max_z, 10):
                    ring = get_influence_ring(
                        [x, 0, z], speed=self.speed, max_width=1, width_decay_exp=2
                    )
