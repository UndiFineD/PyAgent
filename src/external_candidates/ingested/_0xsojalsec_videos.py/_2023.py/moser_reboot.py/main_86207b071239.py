# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\moser_reboot\main.py
...
class Introduction(InteractiveScene):
    def construct(self):
        ...
        paths = VGroup(
            *(VMobject().set_points(path) for path in tale[0].get_subpaths())
        )
        ...
        self.add(tale)
        self.play(
            rect.animate.next_to(tale, RIGHT, buff=LARGE_BUFF).set_anim_args(
                rate_func=linear
            ),
...
class ShowPattern(InteractiveScene):
    def construct(self):
        ...
        expression = Tex(R",".join(map(str, values)) + R"\dots")
        ...
        for value in values:
            ...
            self.play(
                FadeIn(expression[max(n - 1, 0) : new_n], 0.25 * UP, run_time=0.5)
            )
            self.wait(0.5)
            n = new_n + 1
...
class ExplainNChoose2(InteractiveScene):
    ...
    def get_circle_diagram(self):
        ...
        circle.to_edge(LEFT)
        points = [
            circle.pfp(a + self.fudge_factor * np.random.uniform(-0.0, 0.5))
...
class CountIntersections(ExplainNChoose2):
    ...
    def show_quadruplets(self, diagram, quad_words):
        ...
        for sub_indices in it.combinations(indices, 4):
            ...
            int_dot = Dot(
                find_intersection(
                    temp_lines[0].get_start(),
                    temp_lines[0].get_vector(),
                    temp_lines[1].get_start(),
                    temp_lines[1].get_vector(),
                ),
                radius=0.04,
            )
            ...
            self.play(
                LaggedStart(
                    *(TransformFromCopy(dots[i], int_dot) for i in sub_indices)
                ),
                lag_ratio=0.1,
            )
            self.add(int_dot)
...
class BalancedEquation(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            full_rect.animate.set_fill(BLUE_E, 0.5).set_anim_args(
            ...
            return LaggedStart(
                *(
                    AnimationGroup(
                        UpdateFromAlphaFunc(
                            increments[n],
                            lambda m, a: m.set_y(number_y - 0.75 * a).set_opacity(
                                there_and_back(a)
                            ),
                        ),
                        ChangeDecimalToValue(
                            numbers[n], numbers[n].get_value() + 1, run_time=0.25
                        ),
                    )
                    for n in ns
                ),
        ...
        self.play(
            ShowCreation(edges[0]), FadeIn(new_vert_word[R"New edge $\rightarrow$"][0])
        )
        self.wait()
        self.play(
...
class FinalRearrangment(InteractiveScene):
    def construct(self):
        ...
        # Substitute E
        E_eq.set_color(BLUE_B)
        top_eq3 = Tex(Rf"F = \left({E_rhs}\right) - \left({V_rhs}\right) + 1", t2c=t2c)
...
class ExamplesOfFormula(InteractiveScene):
    ...
    def get_example(self, n):
        result = VGroup(
            Tex(Rf"1 + {{{n} \choose 2 }} + {{{n} \choose 4 }}"),
            Tex(
                Rf"= 1 + {choose(n, 2)} + {choose(n, 4)} = {1 + choose(n, 2) + choose(n, 4)}"
            ),
        )
...
class ChallengeProblem(InteractiveScene):
    def construct(self):
        numbers = VGroup(
            *(Integer(1 + choose(n, 2) + choose(n, 4)) for n in range(100))
        )
