# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\outside_videos\short_links.py
...
class CommentFlurry(InteractiveScene):
    base_folder = (
        "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/shorts/Custom Audio/lower_link/"
    )
    sub_folder = ""
    random_seed = 4
    ...
    def construct(self):
        ...
        for file in files:
            ...
            image.move_to([np.random.uniform(-0.5, 0.5), np.random.uniform(-2, 2), 0])
            images.add(image)
        ...
        self.play(
            LaggedStartMap(FadeIn, groups, lag_ratio=0.5, shift=0.25 * UP, run_time=4)
        )
...
class LinkHighlightOverlay(InteractiveScene):
    def construct(self):
        ...
        rect.move_to([-2.125, -3.315, 0.0], LEFT).shift(0 * UP)
        rect.insert_n_curves(100)
