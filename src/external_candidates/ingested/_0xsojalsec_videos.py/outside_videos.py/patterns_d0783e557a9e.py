    ...
    def construct(self):
        ...
        for layer in layers[1:]:
            ...
            for i, dot in enumerate(layer):
                ...
                self.play(
                    *[
                        ReplacementTransform(pre_dot.copy(), dot, run_time=rt)
...
class TriominoGrid(Scene):
    ...
    def construct(self):
        ...
        grid = VGroup(
            *[
                VGroup(*[Square() for x in range(2**n)]).arrange(RIGHT, buff=0)
                for y in range(2**n)
            ]
        ).arrange(UP, buff=0)
        for row in grid:
    ...
    def cover_grid(self, grid):
        ...
        for q in quads:
            ...
            corner_index = np.argmin(
                [get_norm(s.get_center() - grid.get_center()) for s in squares]
            )
