# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\outside_videos\tweets\starry_night.py
...
class StarryStarryNight(Scene):
    def construct(self):
        ...
        dots.set_radii(0.2 * np.random.random(dots.get_num_points()) ** 3)
        ...
        self.wait()
        self.play(
            dots.animate.apply_function(lambda p: stereo_project_point(p, r=3, axis=2)),
            run_time=7,
        )
        self.wait()
        self.play(dots.animate.apply_complex_function(lambda z: z**2 / 5), run_time=7)
        self.play(Rotate(dots, PI / 2, UP, about_point=ORIGIN), run_time=5)
        self.play(dots.animate.apply_complex_function(np.exp), run_time=7)
        ...
        dots.target.apply_function(
            lambda p: (10 + 100 * random.random()) * normalize(p)
        )
