# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\outside_videos\name_animation.py
#!/usr/bin/env python2
# -*- coding: utf-8 -*-
...
class ComplexMorphingNames(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            self.get_post_transform_rotation(name_mob, name_mob_copy), run_time=10
        )
        self.wait(3)
    ...
    def get_rotation(self, name_mob):
        return UpdateFromAlphaFunc(
            ...
            lambda mob, alpha: mob.move_to(
                rotate_vector(self.start_vect, 2 * np.pi * alpha)
            ),
        )
...
class FlowNameAnimation(Scene):
    ...
    def construct(self):
        ...
        from _2017.eoc.div_curl import (
            VectorField,
        ...
        self.play(
            ...
            LaggedStartMap(
                ApplyMethod, name_strokes, lambda m: (m.restore,), lag_ratio=0.2
            ),
            run_time=5,
        )
        self.wait()
            ...
            for name in self.all_names:
                ...
                try:
                    self.file_writer.file_name = (
                        name.replace(" ", "") + self.__class__.__name__
                    )
    ...
    def construct(self):
        ...
        for n in range(2, nb_letters + 2):
            ...
            for j, letter_mob in enumerate(name_mob.submobjects):
                ...
                new_pos = (
                    radius * np.cos(new_angle) * RIGHT + radius * np.sin(new_angle) * UP
                )
        ...
        for k in range(2, N + 1):
            ...
            lines = VGroup(
                *[
                    ...
                    .add_tip(0.1)
                    .set_opacity(0 if (i * k) % N == i else 1)
                    for i in range(N)
                ]
            )
