# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\outside_videos\aeoud.py
...
class VennDiagram(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeIn(l1), c1.animate.set_fill(BLUE, 0.5))
        self.wait()
        self.play(FadeIn(l2), c2.animate.set_fill(YELLOW, 0.5))
        ...
        hexagon.add(
            *(
                Line(v1, v2)
                for v1, v2 in zip(
                    hexagon.get_vertices()[:3], hexagon.get_vertices()[3:]
                )
            )
        )
