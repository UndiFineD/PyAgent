...
class TripleRedundancy(Scene):
    def construct(self):
        ...
        dots.next_to(
            bits.target[last_shown_bit_index], RIGHT, aligned_edge=DOWN, buff=SMALL_BUFF
        )
        ...
        self.play(
            ...
            LaggedStart(
                toggle_bit_anim(bits[0]), toggle_bit_anim(bits[2]), lag_ratio=0.5
            ),
            run_time=1.5,
        )
        self.wait()
        ...
        f_always(
            frac_labels[1].set_width, lambda: min(0.95 * rects[1].get_width(), rl_width)
        )
        ...
        self.play(
            ReplacementTransform(
                SurroundingRectangle(bits, color=BLUE, stroke_opacity=0), ecc_rect
            ),
            Write(ecc_label),
        )
        self.wait()
...
class TimeLine(Scene):
    def construct(self):
        ...
        timeline = NumberLine(
            (decades[0], decades[-1], 2), big_tick_numbers=decades, width=13
        )
...
class AmbientErrorCorrection(Scene):
    ...
    def construct(self):
        ...
        for x in range(10):
            ...
            self.play(
                FadeIn(block, 6 * RIGHT), FadeOut(last_block, 6 * LEFT), run_time=2
            )
...
class MultiplePerspectives(Scene):
    def construct(self):
        # Background
        background = VGroup(
            *[Rectangle().set_fill(color, 1) for color in [GREY_E, BLACK, GREY_E]]
        )
        background.set_stroke(width=0)
...
class SetupSixteenBitExample(Scene):
    def construct(self):
        ...
        self.play(
            FadeIn(r_label, 0.2 * RIGHT), LaggedStartMap(FadeIn, r_boxes, run_time=2)
        )
        self.wait()
        ...
        self.play(
            MoveToTarget(d_bit_copies, lag_ratio=0.1, run_time=4, rate_func=linear)
        )
        self.wait()
        self.play(
            UpdateFromAlphaFunc(
        ...
        power_of_2_rects = VGroup(
            *[SurroundingRectangle(p_labels[2**n]) for n in range(4)]
        )
...
class SenderReceiverDynamic(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            scan_anim(receiver.get_corner(UL), block, lag_factor=1, show_robot=False)
        )
        self.wait()
        ...
        self.play(
            *it.chain(
                *[
                    [pi.change, "horrified", pi.shift, 2 * UP, pi.set_opacity, 0]
                    for pi in pis
                ]
            ),
        ...
        self.play(
            ...
            LaggedStartMap(
                ApplyMethod, words, lambda m: (m.set_opacity, 0.75), lag_ratio=0.7
            ),
            run_time=1,
        )
...
class ParityChecks(Scene):
    def construct(self):
        ...
        scanim = scan_anim(
            point, block, final_stroke_width=0.5, run_time=2, lag_factor=2
        )
        lines, robot = scanim.mobject
        ...
        self.play(
            ShowIncreasingSubsets(one_rects, run_time=1, rate_func=bezier([0, 0, 1, 1]))
        )
        self.wait()
        ...
        parity_rects = VGroup(
            SurroundingRectangle(
                VGroup(block, one_counter_pair[0]), buff=MED_SMALL_BUFF
            ),
            SurroundingRectangle(
                VGroup(r_block, one_counter_pair[1]), buff=MED_SMALL_BUFF
            ),
        )
        ...
        parity_labels = VGroup(
            *[
                OldTexText("Parity: ", word, tex_to_color_map={word: color}).next_to(
                    rect, UP
                )
        ...
        for bit, label, rect in zip([0, 1], parity_labels, parity_rects):
            ...
            x_shift = (
                rect.get_center() - VGroup(label[0], bit_mob).get_center()
            ) * RIGHT
...
class ArrayOfValidMessages(Scene):
    def construct(self):
        ...
        self.add(title)
        self.play(
            Write(shuffled_dots, remover=True, run_time=6, lag_ratio=5 / len(dots))
        )
        self.add(dots)
        self.wait()
...
class TwentyQuestions(Scene):
    def construct(self):
        ...
        pb_arrow = Arrow(
            pb_label.get_bottom() + MED_SMALL_BUFF * LEFT, block[1].get_top(), buff=0.1
        )
        ...
        q1_highlight, q2_highlight = [
            get_bit_n_subgroup(h_rects, n).copy().set_fill(BLUE, 0.5) for n in [0, 1]
        ]
        ...
        self.play(
            VFadeIn(morty), morty.change, "shruggie", questions[0], FadeIn(words, DOWN)
        )
        ...
        self.play(
            ...
            LaggedStart(
                *map(ShowCreationThenFadeAround, [block[2**n] for n in range(4)])
            ),
        )
        self.play(Blink(randy))
        ...
        for x in range(2):
            self.play(
                LaggedStartMap(toggle_bit_anim, ecc_bits, lag_ratio=0.25, run_time=1)
            )
        self.wait()
        ...
        scanim = scan_anim(
            boxes.get_corner(DR) + 2 * UR, block, lag_factor=0.5, run_time=2
        )
        robot = scanim.mobject[-1]
...
class BlockSize256(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn, q_labels, lambda m: (m, DOWN), lag_ratio=0.3, run_time=5
            ),
            LaggedStartMap(
                FadeIn, parity_groups, lambda m: (m, DOWN), lag_ratio=0.3, run_time=5
            ),
        )
        self.wait()
        ...
        # Message bits
        message_bits = VGroup(
            *[bit for i, bit in enumerate(block) if i not in [2**k for k in range(N)]]
        )
...
class ChecksSpellOutPositionInBinary(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                ApplyMethod, yes_no_words, lambda m: (m.scale, 0), remover=True
            ),
        )
        ...
        bit_parts = list(
            it.chain(*[bin_equation.get_parts_by_tex(f"{{{d}}}") for d in [0, 1]])
        )
        ...
        self.play(
            ...
            *[
            ...
            ApplyMethod(
                equation[0].copy().replace, bin_equation[0], remover=True, run_time=1.5
            ),
        )
        ...
        # Four parity checks
        for n, label, group, word in zip(
            it.count(), q_labels, parity_groups, yes_no_words
        ):
            boxes.target = boxes.saved_state.copy()
        ...
        self.play(
            ShowIncreasingSubsets(
                bin_labels, run_time=3, rate_func=bezier([0, 0, 1, 1])
            )
        )
        self.wait()
        ...
        self.play(
            LaggedStart(*map(ShowCreationThenFadeAround, bin_labels), lag_ratio=0)
        )
        self.play(randy.change, "maybe")
        self.play(
            LaggedStart(
                *[
                    ShowCreationThenFadeOut(SurroundingRectangle(b, color=GREEN))
                    for b in block
                ],
                lag_ratio=0,
            ),
        ...
        for n in range(1, N):
            ...
            self.play(
                Transform(bit_arrows, bit_arrow_groups[n]), FadeOut(questions[n - 1])
            )
            self.wait()
...
class FullExampleWithNewEnd(Scene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            ShowCreation(
                lines, lag_ratio=0.1, rate_func=squish_rate_func(smooth, 0.5, 1)
            ),
            run_time=8,
        )
        ...
        message_bits = VGroup(
            *[bit for i, bit in enumerate(bits) if i not in [0, 1, 2, 4, 8]]
        )
    ...
    def construct(self):
        ...
        for N in range(4, 13):
            ...
            grid.arrange_in_grid(
                2 ** int(math.floor(N / 2)), 2 ** int(math.ceil(N / 2)), buff=0
            )
            grid.set_width(5.5)
            if N > 8:
...
class BinaryCounting(Scene):
    def construct(self):
        def get_bit_grids(bit_values):
            ...
            right_bits = get_bit_grid(
                1, 4, buff=SMALL_BUFF, bits=bit_values, height=0.6
            )
...
class ReviewOfXOR(Scene):
    ...
    def construct(self):
        ...
        bit_strings.arrange_in_grid(
            3, row_len, h_buff=SMALL_BUFF, v_buff=MED_LARGE_BUFF, fill_rows_first=False
        )
        ...
        # Compute parities
        parity_words = OldTexText(
            "Computes\\\\parity\\\\of each\\\\column", alignment=""
        )
        parity_words.set_color(YELLOW)
...
class WhyPointToError(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            rect.become, SurroundingRectangle(bit).match_style(rect), FadeOut(arrow)
        )
        self.wait()
...
class HammingCodesWithXOR(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            ShowCreationThenFadeOut(SurroundingRectangle(result, stroke_color=BLUE))
        )
        self.wait()

        for n, rects, result_bit in zip(
            it.count(), reversed(rect_columns), reversed(result)
        ):
            faders = VGroup()
            ...
            new_rects = VGroup()
            for bit, pos in zip(
                get_bit_n_subgroup(bits, n), get_bit_n_subgroup(bin_pos_labels, n)
            ):
                if get_bit_mob_value(bit) == 1:
...
class TooManyErrorsTripUpHamming(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                *[toggle_bit_anim(block[pos], target_color=RED) for pos in errors], **kw
            ),
        )
...
class ReedSolomonPreview(Scene):
    def construct(self):
        ...
        input_arrows = VGroup(
            *[Arrow(input_words.get_bottom(), dot.get_center()) for dot in dots[:4]]
        )
