...
def get_monster_size_label():
    ...
    size_parts = VGroup(
        *[size_label[i : i + 12] for i in range(0, len(size_label), 12)]
    )
...
def get_glassy_cube(frame):
    ...
    faces.add_updater(
        lambda m, f=frame: m.sort(
            lambda p: np.dot(p, [np.sign(f.euler_angles[0]) * 0.2, -1, 0.2])
        )
    )
...
class IntroduceMonsterSize(Scene):
    def construct(self):
        ...
        self.play(
            ...
            FadeIn(
                question, UP, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)
            ),
        )
        self.wait()
...
class IntroduceGroupTheory(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(GrowArrow, arrows, run_time=1), Write(sym_amb, run_time=2)
        )
        ...
        self.play(
            ...
            ShowCreationThenFadeOut(
                snowflake.copy().set_stroke(WHITE, 2).set_fill(opacity=0)
            ),
        ...
        self.play(
            FadeIn(
                footnote, rate_func=there_and_back_with_pause, run_time=3, remover=True
            )
        )
        self.wait()
...
class ZooOfGroups(ThreeDScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *[FadeIn(group[0], -0.5 * group.get_center()) for group in groups]
            )
        )
...
class SymmetriesOfACube(ThreeDScene):
    def construct(self):
        ...
        frame.add_updater(
            lambda m, dt, sc=self: m.set_theta(-30 * DEGREES * np.cos(sc.time * 0.05))
        )
        self.add(frame)
    ...
    def construct(self):
        ...
        def get_permutation(self=self, dots=dots, arc=PI / 2):
            ...
            return Succession(
                ...
                LaggedStartMap(
                    MoveToTarget, dots, path_arc=arc, lag_ratio=0.15, run_time=2
                ),
        ...
        self.play(
            ChangeDecimalToValue(
                perm_label[2], new_perm_label[2].get_value(), run_time=3
            ),
            UpdateFromAlphaFunc(
                perm_label[2],
                lambda m, a: m.move_to(
                    interpolate(
                        old_full_count_center, new_perm_label[2].get_center(), a
                    )
...
class SolutionsToPolynomials(Scene):
    def construct(self):
        ...
        for line in s5_lines:
            line.add_updater(
                lambda m: m.put_start_and_end_on(m.start, m.dot.get_center())
            )
        self.add(*s5_lines)
        ...
        # Arithmetic symbols
        symbols = VGroup(
            *[OldTex(s) for s in ["+", "-", "\\times", "/", "\\sqrt[n]{\\qquad}"]]
        )
...
class MentionGroupsInPhysics(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(
            "pondering", "pondering", "thinking", look_at=nt_label
        )
...
class ElementsAsAbstractions(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                *[FadeIn(mob) for mob in [*ops[0][0::2], *ops[1][:3], ops[1][4]]]
            ),
...
class MultiplicationTable(Scene):
    def construct(self):
        ...
        turn_animation_into_updater(
            Restore(frame, run_time=20, rate_func=bezier([0, 0, 1, 1]))
        )
        self.add(frame)
...
class UsualMultiplicationTable(Scene):
    def construct(self):
        ...
        dots = VGroup(
            ...
            *[
                Tex("\\vdots").scale(2).next_to(square, DOWN)
                for square in grid[-1][:-1]
            ],
        ...
        diagrams = VGroup(
            *[
                ...
                for mob1, mob2 in [
                    ...
                    (
                        OldTex("9").scale(1.5),
                        VGroup(*[Dot() for x in range(9)]).arrange_in_grid(
                            buff=SMALL_BUFF
                        ),
                    ),
                ]
...
class CubeRotations(ThreeDScene):
    def construct(self):
        ...
        frame.add_updater(
            lambda m, sc=self: m.set_euler_angles(
                theta=-20 * DEGREES * np.cos(0.1 * sc.time)
            )
        )
        self.add(frame)
    ...
    def get_quaternions(self, n_rotations=30):
        ijk = [
            quaternion_from_angle_axis(90 * DEGREES, axis) for axis in [RIGHT, UP, OUT]
        ]
        result = []
...
class Isomorphism(Scene):
    def construct(self):
        ...
        icon_labels = VGroup(
            *[OldTexText(f"$180^\\circ$ about\\\\{axis} axis") for axis in "xyz"]
        )
            ...
            Group(
                *[
                    cds[
                        np.argmin(
                            [
                                get_norm(cd.get_center() - diag.get_points()[i])
                                for cd in cds
                            ]
                        )
                    ]
...
class S4WithMultipleChildren(Scene):
    def construct(self):
        ...
        lines = VGroup(
            *[Line(rect.get_top(), s4_label.get_bottom(), buff=0.2) for rect in s_rects]
        )
    ...
    def construct(self):
        ...
        self.teacher_says(
            ...
            added_anims=[
                ...
                self.change_students(
                    "erm", "pondering", "pondering", look_at=question.target
                ),
            ],
        )
        self.wait(2)
        ...
        self.play(
            *[
                ReplacementTransform(
                    sym_question[i], abs_question[j], path_arc=10 * DEGREES
                )
    ...
    def construct(self):
        ...
        for n in range(10):
            if random.choice([True, False]):
                ...
                angle = deg * DEGREES
                icon = get_flip_icon(
                    angle, snowflake, mini_mob_height=snowflake.get_height()
                )
...
class IntroduceSimpleGroups(Scene):
    def construct(self):
        ...
        child_lines = VGroup(
            *[Line(groups.get_bottom(), child.get_top(), buff=0) for child in children]
        )
        child_lines.set_stroke(WHITE, 2)
        s_rects = VGroup(
            *[ScreenRectangle(height=3).move_to(child) for child in children]
        )
        ...
        self.wait()
        self.play(
            ShowCreation(child_lines[0]), FadeIn(inf_groups, 2 * UR), FadeIn(s_rects[0])
        )
        self.wait(2)
        self.play(
        ...
        trees = Group(
            ...
            VGroup(
                ...
                VGroup(
                    *[
                        OldTex("C_" + str(n), fill_color=(RED_B if n == 2 else BLUE))
                        for n in [2, 2, 2, 3]
                    ]
                ),
            ),
        ...
        self.play(
            MoveToTarget(fin_groups), LaggedStartMap(Restore, bg_rects, lag_ratio=0.3)
        )
...
class SymmetriesOfCirleAndLine(Scene):
    def construct(self):
        ...
        circle_ticks = VGroup(
            *[Line(0.95 * vect, 1.05 * vect) for vect in compass_directions(12)]
        )
...
class QuinticImpliesCyclicDecomposition(Scene):
    def construct(self):
        ...
        for children, buff in (prime_children, MED_LARGE_BUFF), (
            real_children,
            LARGE_BUFF,
        ):
        ...
        self.play(
            ...
            *[
                ApplyMethod(
                    line.scale, 0, {"about_point": line.get_end()}, remover=True
                )
...
class TwoStepsToAllFiniteGroups(Scene):
    def construct(self):
        ...
        table = VGroup(
            *[
                VGroup(*[Square() for x in range(n)]).arrange(DOWN, buff=0)
...
class ClassificationOfSimpleGroups(Scene):
    def construct(self):
        ...
        families_grid = VGroup(
            *[VGroup(*[square_template.copy() for x in range(8)]) for y in range(18)]
        )
        ...
        baby_arrow = Arrow(
            baby_name.get_bottom(), sporadics[1].get_corner(UR) + SMALL_BUFF * DL
        )
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                baby_monster, lambda m, a: m.set_opacity(1 - a), remover=True
            ),
            ApplyMethod(
                baby_arrow.scale,
                0,
...
class AskWhatTheMonsterActsOn(Scene):
    def construct(self):
        ...
        dim_words = VGroup(
            *[
                OldTexText("Something in\\\\", "{:,}".format(n), " dimensions", "?")
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                Mobject(),
                lambda m, a, fw=final_words: fw.set_color(
                    interpolate_color(WHITE, YELLOW, a)
                ),
                remover=True,
            ),
        )
...
class MonstrousMoonshine(Scene):
    def construct(self):
        ...
        self.play(
            GrowFromPoint(
                theories[1], theories_line.get_start(), path_arc=-90 * DEGREES
            ),
            ShowCreationThenFadeOut(theories_line, run_time=2),
            FadeIn(
                theories_arrow, rate_func=squish_rate_func(smooth, 0.3, 1), run_time=2
            ),
        )
        self.wait()
