...
class IntroduceDFT(Scene):
    def construct(self):
        ...
        vectors = VGroup(
            *(Arrow(axes.c2p(x, 0), axes.c2p(x, signal[x]), buff=0) for x in range(N))
        )
        ...
        self.play(
            FadeOut(s_title, UP), Write(lhs), Write(plusses), MoveToTarget(s_labels)
        )
        self.wait()
        ...
        zeta_power_vectors = VGroup(
            *(Arrow(plane.n2p(0), plane.n2p(zeta ** (-n)), buff=0) for n in range(N))
        )
        ...
        for z_power, vect in zip(zeta_powers, zeta_power_vectors):
            ...
            z_power.target.next_to(
                vect, np.round(vect.get_vector(), 2), buff=SMALL_BUFF
            )
        ...
        side_axes = Axes(
            (0, 8), (0, 3), width=3, height=2, axis_config={"include_tip": False}
        )
        ...
        for omega in range(3, 8):
            ...
            self.play(
                *anims, FadeIn(new_count, 0.5 * UP), FadeOut(omega_label[1], 0.5 * UP)
            )
