# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\18S191\dynamic_prog.py
...
def get_value_grid(n_rows=6, n_cols=6):
    ...
    for box in boxes:
        value = DecimalNumber(np.round(random.random(), 1), num_decimal_places=1)
...
class GreedyAlgorithm(Scene):
    def construct(self):
        ...
        while i < n_rows:
            ...
            sum_term = VGroup(OldTex("+"), value.copy())
            if i == 0:
            ...
            self.play(FadeIn(box_highlight), FadeIn(sum_term), *map(FadeOut, to_fade))
...
class RecrusiveExhaustiveSearch(Scene):
    def construct(self):
        ...
        def get_seam_sum(seam):
            ...
            row.add(terms[-1])
            row.add(OldTex("="))
            sum_term = DecimalNumber(
                sum([t.get_value() for t in terms]), num_decimal_places=1
            )
    ...
    def construct(self):
        ...
        for i in range(n_rows - 2, -1, -1):
            for j in range(n_cols):
                ...
                new_box.value.set_value(
                    new_box.value.get_value() + low_box.value.get_value()
                )
                new_box.generate_target()
                new_box.target.set_fill(
                    TEAL, opacity=op_factor * new_box.value.get_value()
                )
                new_box.target[1].set_fill(WHITE, 1)
