# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\18S191\convolutions.py
...
class ConvolutionIntroduction(InteractiveScene):
    def construct(self):
        ...
        def step(pos=0):
            ...
            h, w = kernel.shape
            pixels = np.array(
                [square.data["fill_rgba"][0] for square in pixel_array]
        ...
        def zoom_to_kernel():
            self.play(
                frame.animate.set_height(1.5 * kernel_array.get_height()).move_to(
                    kernel_array
                ),
                run_time=2,
            )
