# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\18S191\diffusion.py
...
class Diffusion1D(Scene):
    ...
    def construct(self):
            ...
            for bar in bars:
                ...
                for dot in dots:
                    if dot.x == bar.x:
                        count += 1
                bar.set_height(
                    count * y_unit + epsilon, about_edge=bar.get_bottom(), stretch=True
                )
        ...
        def step(
            dots=dots, bars=bars, x_unit=x_unit, x_range=x_range, time_label=time_label
        ):
