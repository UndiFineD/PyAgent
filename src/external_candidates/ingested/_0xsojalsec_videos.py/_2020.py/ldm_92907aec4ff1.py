...
class SineWave(Scene):
    def construct(self):
        ...
        sin_label = OldTex(
            "\\sin\\left(\\theta\\right)", tex_to_color_map={"\\theta": RED}
        )
...
class CosWave(SineWave):
    ...
    def construct(self):
        ...
        cos_label = OldTex(
            "\\cos\\left(\\theta\\right)", tex_to_color_map={"\\theta": RED}
        )
...
class ExpMeaning(Scene):
    ...
    def construct(self):
        ...
        theta_tracker = ValueTracker(0)
        theta_label = VGroup(
            OldTex("\\theta = "), DecimalNumber(0, num_decimal_places=4)
        )
...
class ImaginaryInterestRates(Scene):
    def construct(self):
        ...
        def get_arc(tracker=theta_tracker):
            theta = tracker.get_value()
            arc = Arc(
                radius=1, stroke_width=3, stroke_color=RED, start_angle=0, angle=theta
            )
            return arc
...
class ComplexExponentialPlay(Scene):
    ...
    def construct(self):
        ...
        # Titles
        title = OldTex(
            "f(x) =", "\\text{exp}(r\\cdot x)", tex_to_color_map={"r": YELLOW}
        )
        title.to_corner(UL)
