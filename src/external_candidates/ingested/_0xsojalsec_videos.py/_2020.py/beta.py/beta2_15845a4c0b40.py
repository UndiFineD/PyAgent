...
class ShowInfiniteContinuum(Scene):
    def construct(self):
        ...
        self.play(
            ShowCreation(x_line),
            UpdateFromFunc(
                h_part, lambda m: m.next_to(m.tracked.get_end(), UR, SMALL_BUFF)
            ),
            UpdateFromFunc(
                value, lambda m: m.set_value(m.x_axis.p2n(m.tracked.get_end()))
            ),
            run_time=3,
        )
        self.wait()
...
class ShowLimitToPdf(Scene):
    def construct(self):
        ...
        brace = Brace(
            Line(arrows[0].get_start(), arrows[1].get_start()), UP, buff=SMALL_BUFF
        )
        ...
        prob_label.target.next_to(
            all_ineq_labels, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
        ...
        self.play(
            UpdateFromAlphaFunc(
                region,
                lambda m, a: m.become(
                    m.func(m.axes, m.graph, m.min_x, interpolate(m.min_x, m.max_x, a))
                ),
            ),
...
class SumToIntegral(Scene):
    def construct(self):
        ...
        def get_lebesgue_bars(
            dy, axes=l_axes, func=dist.pdf, mx=0.7, y_max=dist.pdf(0.7)
        ):
            bars = VGroup()
        ...
        self.wait()
        for rbg0, rbg1, lbg0, lbg1 in zip(
            r_bar_groups, r_bar_groups[1:], l_bar_groups, l_bar_groups[1:]
        ):
            self.play(
                ReplacementTransform(
                    rbg0,
