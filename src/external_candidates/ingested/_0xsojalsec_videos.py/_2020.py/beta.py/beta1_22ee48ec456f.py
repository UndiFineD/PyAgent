# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\beta\beta1.py
import scipy.stats
...
class ShowThreeCases(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeInFrom, titles, lambda m: (m, DOWN), lag_ratio=0.2)
        )
        self.play(
            LaggedStart(
                *[
                    LaggedStartMap(FadeInFromLarge, review, lag_ratio=0.1)
                    for review in reviews
                ],
                lag_ratio=0.1,
            )
    ...
    def get_titles(self):
        titles = VGroup(
            OldTexText("$100\\%$ \\\\", "10 reviews"),
            OldTexText("$96\\%$ \\\\", "50 reviews"),
            OldTexText("$93\\%$ \\\\", "200 reviews"),
        )
        colors = [PINK, BLUE, YELLOW]
...
class PreviewThreeVideos(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *[FadeIn(topic, lag_ratio=0.1) for topic in topics],
                run_time=3,
                lag_ratio=0.3,
            )
        )
        ...
        images = [
            ImageMobject(
                os.path.join(consts.VIDEO_DIR, OUTPUT_DIRECTORY, "images", name)
            )
...
class LaplacesRuleOfSuccession(Scene):
    def construct(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                review_label[0], lambda m, a: m.set_value(int(interpolate(10, 11, a)))
            ),
            UpdateFromAlphaFunc(
                review_label[2], lambda m, a: m.set_value(int(interpolate(10, 12, a)))
            ),
            FadeIn(pretend_label, LEFT),
            old_review_label.scale,
            0.5,
            old_review_label.set_opacity,
            0.5,
            old_review_label.to_edge,
            LEFT,
        )
        self.wait()
        ...
        self.play(
            ...
            real_data.match_width,
            old_review_label.target,
            real_data.next_to,
            old_review_label.target,
            DOWN,
        )
        ...
        self.play(
            ...
            MaintainPositionRelativeTo(review_label, review_center),
            UpdateFromAlphaFunc(
                review_label[0], lambda m, a: m.set_value(int(interpolate(48, 49, a)))
            ),
            UpdateFromAlphaFunc(
                review_label[2], lambda m, a: m.set_value(int(interpolate(50, 52, a)))
            ),
            old_review_label.scale,
            0.5,
            old_review_label.to_edge,
...
class WhatsTheModel(Scene):
    ...
    def introduce_buyer_and_seller(self):
        if hasattr(self, "questions"):
            ...
            added_anims = [
                questions[0].to_edge,
                UP,
                questions[1].set_opacity,
                0.5,
                questions[1].scale,
                0.25,
                questions[1].to_corner,
                UR,
            ]
        else:
            added_anims = []
        ...
        self.play(
            LaggedStartMap(
                FadeInFromDown, VGroup(seller, buyer, *labels), lag_ratio=0.2
            ),
            *added_anims,
        )
        self.wait()
    ...
    def experience_animations(self, seller, buyer, arc=-30 * DEGREES, p=0.75):
        ...
        words = OldTexText(
            "Positive\\\\experience" if positive else "Negative\\\\experience"
        )
        ...
        result = [
            ApplyMethod(words.move_to, buyer.get_corner(UL), path_arc=arc, run_time=2),
...
class IsSellerOne100(Scene):
    ...
    def add_review(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                tens[0],
                lambda m, a: m.set_color(interpolate_color(RED, GREEN, a)).set_value(
                    len(m.to_count)
                ),
            ),
            run_time=2,
    ...
    def show_random_numbers(self):
        ...
        rows = VGroup(
            *[
                VGroup(
                    *[
                        Integer(random.randint(0, 99)).move_to(0.85 * x * RIGHT)
                        for x in range(10)
                    ]
                )
                for y in range(10 * 2)
        ...
        self.play(
            ...
            prob_label_group.set_height,
            0.75,
            prob_label_group.to_corner,
            UL,
        )
        self.wait()
...
class LookAtAllPossibleSuccessRates(Scene):
    def construct(self):
        ...
        sellers = VGroup(
            *[self.get_example_seller(label.get_value()) for label in x_labels]
        )
...
class AskAboutUnknownProbabilities(Scene):
    ...
    def show_many_coins(self, n_rows, n_cols):
        ...
        coins = VGroup(
            *[random.choice(coin_choices).copy() for x in range(n_rows * n_cols)]
        )
...
class AskProbabilityOfCoins(Scene):
    def construct(self):
        ...
        self.play(
            ShowIncreasingSubsets(coins), run_time=8, rate_func=bezier([0, 0, 1, 1])
        )
        self.wait()
...
class ComplainAboutSimplisticModel(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            *[ApplyMethod(pi.look_at, new_graph) for pi in self.pi_creatures],
        )
        self.wait(3)
...
class AskInverseQuestion(WhatsTheModel):
    ...
    def ask_question(self):
        ...
        self.play(
            Restore(labels[0]),
            *[ApplyMethod(pi.change, "pondering", labels) for pi in pis],
        )
        ...
        short_label = OldTex(
            "P(\\text{data} | s)",
            tex_to_color_map={"\\text{data}": GREY_B, "s": YELLOW},
        )
        short_label.scale(2)
...
class SimulationsOf10Reviews(Scene):
    ...
    def construct(self):
        ...
        self.play(
            FadeOut(stacks),
            FadeOut(count),
            histogram.bars.become,
            histogram.get_bars(data),
            histogram.axes.y_labels.set_opacity,
            1,
            histogram.axes.h_lines.set_opacity,
            1,
            histogram.axes.y_axis.set_opacity,
            1,
        )
        self.remove(stacks)
...
class SimulationsOf50Reviews(Scene):
    ...
    def construct(self):
        ...
        self.add(histogram)
        self.play(
            ShowIncreasingSubsets(row), ChangeDecimalToValue(count, original_count)
        )
...
class ShowBinomialFormula(SimulationsOf50Reviews):
    ...
    def construct(self):
        ...
        self.play(
            long_equation.next_to,
            self.slots,
            DOWN,
            MED_LARGE_BUFF,
            long_equation.to_edge,
            RIGHT,
        ...
        for cf, pre in (
            (clean_form[6], pre_c),
            (clean_form[8:11], pre_s),
            (clean_form[11:], pre_1ms),
        ):
            self.play(
        ...
        self.play(
            top_fade_rect.set_opacity, 1, fade_rects.set_opacity, 1, FadeIn(new_formula)
        )
    ...
    def explain_n_choose_k(self, row, formula):
        ...
        self.play(
            ...
            UpdateFromFunc(formula[1], lambda m: m.set_value(len(m.counted))),
            run_time=2,
        )
        formula.restore()
        self.add(formula)
