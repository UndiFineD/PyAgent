# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\beta\beta3.py
import scipy.stats
...
class RemindOfWeightedCoin(Scene):
    def construct(self):
        ...
        def get_grid_swap_anims(g1, g2):
            return [
                LaggedStartMap(
                    MoveToTarget, g1, lag_ratio=0.02, run_time=1.5, remover=True
                ),
                LaggedStartMap(
                    MoveToTarget,
                    g2,
        ...
        def get_region(
            axes=axes, graph=graph, lh_tracker=lh_tracker, rh_tracker=rh_tracker
        ):
        ...
        self.add(region)
        self.play(
            lh_tracker.set_value,
            0.6,
            rh_tracker.set_value,
            0.8,
            UpdateFromAlphaFunc(
                region_area_label,
        ...
        # Move plot
        self.play(
            n_heads_tracker.set_value,
            7,
            n_tails_tracker.set_value,
            3,
...
class BayesianFrequentistDivide(Scene):
    def construct(self):
        ...
        freq_list = VGroup(
            ...
            OldTexText(
                "2. Choose a test statistic,\\\\", "$\\qquad$ compute its value", **kw
            ),
            OldTexText("3. Calculate a $p$-value", **kw),
            OldTexText("4. Choose a significance value $\\alpha$", **kw),
            OldTexText(
                "5. Reject $H_0$ if $p$-value\\\\",
                "$\\qquad$ is less than $\\alpha$",
                **kw,
            ),
        )
        ...
        for H_word, graph, vect in zip(H_words, graphs[::2], [RIGHT, LEFT]):
            ...
            arrow = Arrow(
                H_word.get_bottom(), graph.get_center() + 0.75 * vect, buff=SMALL_BUFF
            )
            H_word.add(arrow)
...
class ShowBayesianUpdating(Scene):
    ...
    def construct(self):
        ...
        def update_graph(graph):
            ...
            graph.label95.set_width(
                graph.label95.width_ratio * graph.region.get_width()
            )
...
class VisualizeBayesRule(Scene):
    ...
    def show_continuum(self):
        ...
        self.play(
            ShowCreation(x_line),
            UpdateFromFunc(
                s_part, lambda m: m.next_to(m.tracked.get_end(), UR, SMALL_BUFF)
            ),
            UpdateFromFunc(
    ...
    def show_arrows(self):
        ...
        self.play(
            LaggedStartMap(
                ApplyMethod, arrows, lambda m: (m.scale, 0, {"about_edge": DOWN}), **kw
            )
        )
        self.remove(arrows)
        self.wait()
    ...
    def show_discrete_probabilities(self):
        ...
        self.play(
            given_data.scale,
            0.5,
            given_data.set_opacity,
            0.5,
            given_data.to_corner,
            UR,
    ...
    def show_bayes_formula(self):
        ...
        over.set_width(
            prior_label.get_width() + likelihood_label.get_width() + MED_SMALL_BUFF
        )
        ...
        self.bayes = VGroup(
            p_label, eq, prior_label, likelihood_label, over, p_data_label
        )
        self.data = data
    ...
    def update_from_data(self):
        ...
        self.play(
            ReplacementTransform(bars.copy().set_opacity(0), new_bars),
            bars.set_fill,
            {"opacity": 0.1},
            bars.set_stroke,
            {"opacity": 0.1},
...
class BayesRuleWithPdf(ShowLimitToPdf):
    def construct(self):
        ...
        scaled_graph = axes.get_graph(lambda x: scipy.stats.binom(3, x).pmf(1) + 1e-6)
        ...
        self.play(
            ApplyMethod(
                scaled_region.set_height,
                new_graph.get_height(),
                {"about_edge": DOWN, "stretch": True},
                run_time=2,
            ),
            over.set_opacity,
...
class TalkThroughCoinExample(ShowBayesianUpdating):
    def construct(self):
        ...
        self.play(
            FadeIn(exp1, DOWN),
            FadeIn(exp2, DOWN),
            VGroup(func_label, dot, eq_1mx).shift,
            shift_vect,
            bayes.scale,
            0.5,
            bayes.next_to,
