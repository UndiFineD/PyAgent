...
def get_random_process(
    choices, shuffle_time=2, total_time=3, change_rate=0.05, h_buff=0.1, v_buff=0.1
):
    content = choices[0]
...
def get_prob_positive_experience_label(
    include_equals=False, include_decimal=False, include_q_mark=False
):
    label = OldTex(
        "P",
        "(",
...
def scaled_pdf_axes(scale_factor=3.5):
    ...
    axes.y_axis.numbers.set_submobjects(
        [*axes.y_axis.numbers[:5], *axes.y_axis.numbers[4::5]]
    )
    sf = scale_factor
