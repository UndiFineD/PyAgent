# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\covid.py
import scipy.stats
...
class IntroducePlot(Scene):
    def construct(self):
        ...
        for new_day in range(day + 1, len(dots)):
            ...
            line.rotate(PI)
            self.play(dot.set_color, YELLOW, Uncreate(line), run_time=0.5)
        ...
        for d1, d2 in zip(dots[25:], dots[26:]):
            ...
            label = VGroup(
                OldTex("\\times"),
                DecimalNumber(
                    axes.y_axis.p2n(d2.get_center()) / axes.y_axis.p2n(d1.get_center()),
                ),
            )
...
class ViralSpreadModel(Scene):
    ...
    def get_randys(self):
        randys = VGroup(*[Randolph() for x in range(150)])
        for randy in randys:
    ...
    def spread_infection(self, randys):
        ...
        for randy in randys:
            if randy.infected:
                distances = [
                    get_norm(r2.get_center() - randy.get_center()) for r2 in randys
                ]
...
class GrowthEquation(Scene):
    def construct(self):
        ...
        labels = VGroup(N_label, E_label, p_label)
        ...
        self.play(GrowFromCenter(brace), FadeIn(text, 0.5 * UP))
        self.wait()
        ...
        self.play(GrowArrow(arrow), FadeIn(exp_eq, 2 * LEFT))
        self.wait()
        ...
        self.play(ShowCreation(p_factors_rect), ShowCreation(p_arrow))
        self.wait()
        ...
        self.play(ShowCreation(ep_rect), FadeIn(ep_question, LEFT))
        self.wait()
...
class RescaleToLogarithmic(IntroducePlot):
    def construct(self):
        ...
        brace_label = brace.get_tex("\\times 10", buff=SMALL_BUFF)
        ...
        for i in range(2, 5):
            self.play(
                brace.next_to, log_y_axis.tick_marks[i : i + 2], {"buff": SMALL_BUFF}
            )
            self.wait(0.5)
        ...
        reg = scipy.stats.linregress(range(7, len(CASE_DATA)), np.log10(CASE_DATA[7:]))
        x_max = axes.x_max
        ...
        self.play(LaggedStart(*remove_anims), ShowCreation(reg_line))
        ...
        # R^2 label
        R2_label = VGroup(OldTex("R^2 = "), DecimalNumber(0, num_decimal_places=3))
        ...
        extended_x_axis = NumberLine(
            ...
            unit_size=get_norm(axes.x_axis.n2p(1) - axes.x_axis.n2p(0)),
            big_tick_numbers=[],
        )
        ...
        self.play(reg_line.scale, 3, {"about_point": reg_line.get_start()})
        self.wait()
        ...
        self.play(
            ...
            ApplyMethod(last_rect.stretch, 0, 0, {"about_edge": RIGHT}, remover=True),
        )
...
class SARSvs1918(Scene):
    def construct(self):
        ...
        for title, vect, color, image in zip(
            titles, [LEFT, RIGHT], [YELLOW, RED], images
        ):
            image.set_height(4)
...
class ViralSpreadModelWithShuffling(ViralSpreadModel):
    ...
    def shuffle_randys(self, randys):
        ...
        self.play(LaggedStart(*anims, lag_ratio=1 / len(randys), run_time=3))
...
class ViralSpreadModelWithClustersAndTravel(ViralSpreadModelWithClusters):
    ...
    def travel_between_clusters(self):
        reps = VGroup(*[random.choice(cluster) for cluster in self.clusters])
        targets = list(reps)
...
class ShowLogisticCurve(Scene):
    def construct(self):
        ...
        exp_curve = axes.get_graph(lambda x: 0.15 * np.exp(0.68 * x))
        ...
        dot.add_updater(
            lambda m: m.move_to(axes.input_to_graph_point(x_tracker.get_value(), curve))
        )
        ...
        for label, tex, vect in (numer_label, "N_d", UL), (
            denom_label,
            "N_{d - 1}",
            DL,
        ):
        ...
        self.play(FadeIn(gf_label[0], RIGHT), FadeIn(gf_label[1:], LEFT), FadeOut(ode))
        self.wait()
        ...
        def get_growth_factor():
            ...
            x0 = x1 - 0.2
            x2 = x1 + 0.2
            p0, p1, p2 = [axes.input_to_graph_point(x, curve) for x in [x0, x1, x2]]
