# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\chess.py
...
class IntroducePuzzle(Scene):
    ...
    def construct(self):
        ...
        grid = NumberPlane(x_range=(0, 8), y_range=(0, 8), faded_line_ratio=0)
        ...
        count_label = VGroup(Integer(0, edge_to_fix=RIGHT), OldTexText("Coins"))
        ...
        k = (
            boolian_linear_combo(head_bools) ^ 63
        )  # To make the flip below the actual solution
...
class PrisonerPuzzleSetting(PiCreatureScene):
    CONFIG = {"camera_config": {"background_color": GREY_E}}
    ...
    def construct(self):
        ...
        self.play(p1.change, "sad", p2.change, "pleading", warden.eyes)
        self.play(
            FadeIn(warden),
            FadeIn(names[2]),
        )
...
class FromCoinToSquareMaps(ThreeDScene):
    CONFIG = {
        "messages": ["Please, ", "go watch", "Stand-up", "Maths on", "YouTube."],
        "flip_lag_ratio": 0.05,
    }
    ...
    def construct(self):
        ...
        self.play(GrowArrow(arrow), FadeIn(board2, 2 * LEFT))
        square = get_special_square()
        rect = SurroundingRectangle(square, buff=0)
        rect.set_color(PINK)
        rect.next_to(square, OUT, buff=0.01)
        self.play(square.set_color, MAROON_C, ShowCreation(rect), FadeIn(title2))
...
class ImpossibleVariations(FromCoinToSquareMaps):
    ...
    def construct(self):
        ...
        for x in range(8):
            self.set_board_message(
                next(messages), left_board, coins, get_special_square
            )
            self.wait()
        ...
        self.play(
            title.shift, 2 * LEFT, FadeIn(cross, 2 * RIGHT), FadeIn(imp_words, LEFT)
        )
        self.wait()
        self.play(to_fade.set_opacity, 1)

        # Remove a square
        to_remove = Group(left_board[63], right_board[63], coins[63])
    ...
    def set_board_message(self, message, left_board, coins, get_special_square):
        ...
        self.play(
            new_colored_square.set_color,
            BLUE,
            self.colored_square.set_color,
            self.colored_square.original_color,
            self.moving_square.move_to,
            get_special_square(),
            OUT,
        )
...
class ErrorCorrectionMention(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(bits_rect), FadeIn(data_label, DOWN))
        self.play(
            FadeOut(right_board),
        ...
        self.play(
            ...
            error_bit.set_value,
            1 - error_bit.get_value(),
            error_bit.set_color,
            RED,
        )
        meaning_label.save_state()
        self.play(Transform(meaning_label, error_meaning_label))
        self.wait()
        ...
        self.play(
            LaggedStartMap(ShowCreationThenFadeOut, lines, lag_ratio=0.02, run_time=3)
        )
        ...
        self.play(error_bit.set_value, 0, Restore(meaning_label))
        self.play(
            FadeOut(lines),
            FadeOut(alert),
        ...
        self.play(Write(hamming_label), FadeOut(ecc_name, DOWN))
        self.wait()


class StandupMathsWrapper(Scene):
    CONFIG = {"title": "Solution on Stand-up Maths"}
...
class TitleCard(Scene):
    def construct(self):
        ...
        grid = NumberPlane(x_range=(0, n), y_range=(0, n), faded_line_ratio=0)
...
class TwoSquareCase(ThreeDScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOut, to_remove, lambda m: (m, DOWN), run_time=2, lag_ratio=0.01
            ),
            small_group.center,
            small_group.set_height,
            1.5,
            frame.set_phi,
            10 * DEGREES,
            run_time=2,
        ...
        self.play(FadeOut(two_square_words), FlipCoin(coins[1]), FadeIn(states[0]))
        self.play(FlipCoin(coins[0]), FadeIn(states[1]))
        self.play(FlipCoin(coins[0]), FlipCoin(coins[1]), FadeIn(states[2]))
        self.play(FlipCoin(coins[0]), FadeIn(states[3]))
        self.wait()
        ...
        self.play(
            ...
            state_rect.move_to,
            states[1],
            arrows[3].match_style,
            arrows[0],
            arrows[1].match_style,
            arrows[3],
        )
        self.wait()
        self.play(
        ...
        self.play(
            LaggedStartMap(
                GrowFromCenter, Group(*bin_states.family_members_with_points())
            ),
            LaggedStartMap(
                ApplyMethod,
                Group(*states.family_members_with_points()),
                lambda m: (m.scale, 0),
            ),
        )
        self.wait()
        ...
        # Add labels
        c_labels = VGroup(*[OldTex(name) for name in self.coin_names])
        arrow_kw = {
            "tip_config": {
                "width": 0.2,
                "length": 0.2,
        ...
        for label, coin in zip(c_labels, bin_coins):
            ...
            self.play(FadeIn(label), GrowArrow(arrow))
        self.wait()
        ...
        self.wait()
        self.play(FadeOut(board_rects[0]), FadeOut(zero_rects), FadeIn(board_rects[1]))
        self.play(board_rects[1].stretch, 0.5, 0, {"about_edge": RIGHT})
        self.play(
        ...
        self.play(Write(null_word), GrowArrow(null_arrow))
        self.wait()

        for i in (0, 1, 0):
            self.play(
                FlipCoin(bin_coins[0]),
                arrows[3 - i].match_style,
                arrows[0],
                arrows[2 + i].match_style,
                arrows[3 - i],
        ...
        self.play(MoveToTarget(frame), FadeIn(rule_words, 2 * DOWN))
        self.wait()
...
class IGotThis(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("thinking", "pondering", look_at=self.screen)
        self.wait(6)
...
class WalkingTheSquare(ThreeDScene):
    def construct(self):
        ...
        low_rect = SurroundingRectangle(
            VGroup(
                edges[3], coord_labels[0], coord_labels[2], plane.x_axis.numbers[-1]
            ),
            buff=0.25,
        )
        ...
        self.play(
            ...
            key.match_x,
            board[0],
            high_rect.match_style,
            low_rect,
            low_rect.match_style,
            high_rect,
        )
        self.wait()
...
class ThreeSquareCase(ThreeDScene):
    ...
    def construct(self):
        ...
        # Show values of S
        s_labels = VGroup(*[OldTex(f"K = {n}") for n in range(3)])
        ...
        self.wait()
        self.play(rhs[1].set_value, 0, FadeOut(rhs_rhs))
        ...
        self.play(FadeOut(VGroup(morty, bubble, bubble.content)))
...
class FailedMod3Addition(Scene):
    def construct(self):
        ...
        coins = csum[1:7:2]
        csum[-1].add_updater(
            lambda m, coins=coins: m.set_value(
                coins[1].is_heads() + 2 * coins[2].is_heads()
            )
        )
...
class TreeOfThreeFlips(ThreeDScene):
    def construct(self):
        ...
        # Initial state label
        is_label = OldTexText(
            "Initial state: 010", tex_to_color_map={"0": RED_D, "1": GOLD_D}
        )
        ...
        for indices in [[0], [1, 2]]:
            self.play(
                *[
                    TransformFromCopy(
                        csum, new_csums[i], path_arc=30 * DEGREES, run_time=2
                    )
                    for i in indices
                ]
            )
            ...
            for i in indices:
                ...
                ncs.coins[i].flip()
                self.play(FlipCoin(ncs.coins[i]), MoveToTarget(rhs))
        ...
        self.play(FadeIn(board, DOWN), FadeIn(key))
        self.play(
...
class SeventyFivePercentChance(Scene):
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), FadeIn(b_label, RIGHT))
        self.wait(2)
        self.play(FadeIn(sum_label, 0.25 * UP))
        self.wait(2)
        self.play(
            LaggedStart(FadeIn(want_label, UP), FadeIn(need_label, UP), lag_ratio=0.3)
        )
        self.wait()
...
class ModNStrategy(ThreeDScene):
    def construct(self):
        ...
        self.add(coins)
        self.wait()
        self.play(MoveToTarget(coins), FadeIn(nums, lag_ratio=0.1))
        self.wait()
        ...
        self.play(GrowFromCenter(brace), FadeIn(s_label, 0.25 * UP))
        self.wait()
...
class ShowCube(ThreeDScene):
    def construct(self):
        ...
        def highlight(n, spheres=spheres, coord_labels=coord_labels):
            ...
            for k, sphere, cl in zip(it.count(), spheres, coord_labels):
                if k == n:
                    ...
                    Group(cl.target, sphere.target).scale(
                        1.5, about_point=sphere.get_center()
                    )
                    anims += [
                        MoveToTarget(sphere),
                        MoveToTarget(cl),
        ...
        self.play(
            LaggedStart(
                *[Indicate(s, color=GREEN) for s in spheres], run_time=2, lag_ratio=0.1
            ),
            LaggedStart(
                *[Indicate(c, color=GREEN) for c in coord_labels],
                run_time=2,
                lag_ratio=0.1,
            ),
        )
        ...
        self.play(FadeIn(board), FadeIn(coins), highlight(7))
        ...
        for ci in [1, 2, 0, 1, 2, 1, 0, 1]:
            ...
            self.play(
                FlipCoin(coin), highlight(new_n), ShowCreationThenDestruction(line)
            )
            self.wait()
        ...
        def get_coloring_animation(
            ...
            for n, sphere, coord_label, old_color_label in zip(
                ns, spheres, coord_labels, coord_labels.color_labels
            ):
                color = colors[int(n)]
                sphere.generate_target()
        ...
        S0.shift(UP)
        S0.fix_in_frame()
        self.play(FadeIn(S0, DOWN), get_coloring_animation([0] * 8))
        self.wait(5)
        ...
        self.play(
            ...
            get_coloring_animation(
                [np.dot(coords, [0, 1, 2]) % 3 for coords in vert_coords]
            ),
        )
        self.wait(4)
        ...
        self.play(
            FlipCoin(coins),
            coord_labels[1:].set_opacity,
            0.2,
            coord_labels.color_labels[1:].set_opacity,
            0.2,
            spheres[1:].set_opacity,
            0.2,
        )
        self.wait(2)
        ...
        for n in [1, 2, 4]:
            ...
            self.play(
                ...
                spheres[n].set_opacity,
                1,
                coord_labels[n].set_opacity,
                1,
                coord_labels.color_labels[n].set_opacity,
                1,
                FlipCoin(coin),
            )
            line.reverse_points()
            self.add(line, coord_labels)
            self.play(FlipCoin(coin), ShowCreation(line))
            lines.add(line)
        self.wait(10)
        ...
        self.play(
            ...
            Group(
                spheres[0],
                coord_labels[0],
                coord_labels.color_labels[0],
                spheres[1],
                coord_labels[1],
                coord_labels.color_labels[1],
                spheres[4],
                coord_labels[4],
                coord_labels.color_labels[4],
            ).set_opacity,
            0.2,
        )
        self.wait(3)
        ...
        for n in [1, 2, 4]:
            ...
            self.play(
                ShowCreationThenDestruction(line),
                spheres[new_n].set_opacity,
                1,
                coord_labels[new_n].set_opacity,
                1,
                coord_labels.color_labels[new_n].set_opacity,
                1,
                FlipCoin(coin),
            )
            line.reverse_points()
        ...
        self.play(
            ...
            spheres.set_opacity,
            1,
            coord_labels.set_opacity,
            1,
            coord_labels.color_labels.set_opacity,
            1,
        ...
        for message in messages:
            ...
            for head, coin in zip(bools, full_coins):
                if head ^ coin.is_heads():
                    to_flip.add(coin)
            self.play(LaggedStartMap(FlipCoin, to_flip, run_time=1))
            self.wait(0.5)
...
class CubeEdgeDescription(Scene):
    ...
    def construct(self):
        bits = VGroup(
            *[
                VGroup(*[Integer(int(b)) for b in string_to_bools(char)]).arrange(
        ...
        top_group = VGroup(bf_group, Vector(RIGHT), cube_words)
        top_group.arrange(RIGHT)
        top_group.to_edge(UP)

        self.add(bf_group)
        self.play(TransformFromCopy(*bits), GrowArrow(arrow), FadeIn(words, 0.25 * UP))
        self.wait()
        self.play(GrowArrow(top_group[1]), FadeIn(cube_words, LEFT))
        self.wait()
...
class GrahamsConstantAlt(Scene):
    def construct(self):
        ...
        for ndots in [1, 3, 6, 7, 9, 12]:
            row = VGroup(
                *[
                    ...
                    VGroup(*[OldTex("\\cdot") for x in range(ndots)]).arrange(
                        RIGHT, buff=0.2
                    ),
                    OldTex("\\uparrow\\uparrow"),
                    OldTex("3"),
        ...
        self.play(
            TransformFromCopy(rhs[4], rhs[5]), GrowFromCenter(rbrace), Write(rbrace_tex)
        )
        self.wait()
...
class AttemptAColoring(ThreeDScene):
    def construct(self):
        ...
        def get_color_change_animations(
            values, verts=cube.verts, labels=color_labels, gcv=get_colored_vertices
        ):
            ...
            return [
                ...
                LaggedStartMap(
                    FadeOut, old_labels, lambda m: (m, 0.5 * UP), lag_ratio=0.03
                ),
                LaggedStartMap(
                    FadeIn, labels, lambda m: (m, 0.5 * DOWN), lag_ratio=0.03
                ),
            ]

        # Prepare a few colorings
        mod3_strategy = [np.dot(int_to_bit_coords(n), [0, 1, 2]) % 3 for n in range(8)]
        ...
        for n, vert in enumerate(cube.verts):
            ...
            for edge, leaf, label, y in zip(
                tree.target.edges, tree.target.leafs, tree.target.labels, [0.4, 0, -0.4]
            ):
        ...
        for tree in trees:
            ...
            colors = [RED, GREEN, BLUE_D]
            letters = ["R", "G", "B"]
            for color, letter, leaf, label in zip(
                colors, letters, tree.leafs, tree.labels
            ):
        ...
        def restore_vertex(n, verts=cube.verts, labels=color_labels):
            return AnimationGroup(
                Transform(verts[n], verts.saved_state[n]), FadeIn(labels[n], DOWN)
            )
        ...
        for tree in trees:
            t_rect = SurroundingRectangle(
                Group(tree.leafs, tree.labels), buff=SMALL_BUFF
            )
            t_rect.set_stroke(YELLOW, 2)
            rects.add(t_rect)

        self.play(
            LaggedStartMap(ShowCreationThenFadeOut, rects, lag_ratio=0.025, run_time=3)
        )
        self.wait()
        ...
        self.play(
            LaggedStartMap(
                FadeOut,
                VGroup(*number_labels, *vert_eqs, *rhss, *implies),
            ),
            ShowCreation(cube.edges, lag_ratio=0.1),
            LaggedStartMap(FadeInFromLarge, cube.verts, lambda m: (m, 0.2)),
        )
        ...
        self.play(
            ...
            UpdateFromFunc(red_total, lambda m, lr=label_rects: m.set_value(len(lr))),
        )
        self.wait()
...
class HighDimensionalCount(ThreeDScene):
    def construct(self):
        ...
        grid = NumberPlane(x_range=(0, 3), y_range=(0, 2), faded_line_ratio=0)
        grid.match_height(board)
        ...
        self.play(FadeIn(vert_label), FadeOut(cl_mover))
        ...
        self.play(
            LaggedStartMap(
                TransformFromCopy, neighbor_boards, lambda m, b=vert_board: (b, m)
            )
        )
        self.wait()
        ...
        self.play(
            LaggedStartMap(TransformFromCopy, neighbors, lambda m, v=vert: (v, m))
        )
        self.wait()
        ...
        self.play(GrowFromCenter(low_brace), Write(neighbors_label, run_time=1))
        self.wait()
        self.play(
            GrowFromCenter(dots),
            GrowFromCenter(top_brace),
            LaggedStartMap(TransformFromCopy, new_trees, lambda m, t=tree: (t, m)),
        ...
        for tree in [*new_trees[:2], middle_tree, *new_trees[2:]]:
            arrow = Vector([-1, -2, 0], tip_config={"width": 0.2, "length": 0.2})
        ...
        edge_anim = ShowCreationThenFadeOut(highlighted_edges, lag_ratio=0.3)
...
class FourDCubeColoringFromTrees(ThreeDScene):
    ...
    def get_hypercube(self, dim=4, width=4):
        ...
        for n in range(2**dim):
            for power in range(dim):
                k = n ^ (1 << power)
                edge = Line3D(
                    hc_points[n], hc_points[k], width=0.05, resolution=(31, 31)
                )
                cube.edges.add(edge)
    ...
    def get_hypercube_points(self, dim=4, width=4):
        all_coords = [int_to_bit_coords(n, dim).astype(float) for n in range(2**dim)]
...
class IntroduceHypercube(FourDCubeColoringFromTrees):
    def construct(self):
        ...
        # Setup cubes
        cubes = [self.get_hypercube(dim=d) for d in range(5)]
        ...
        def get_cube_intro_anim(
            n, cubes=cubes, reconnect_edges=reconnect_edges, label=label
        ):
            if n == 0:
        ...
        self.play(FadeIn(label, DOWN), get_cube_intro_anim(0))
        self.wait()
        for n in [1, 2]:
        ...
        self.play(
            ...
            ApplyMethod(
                frame.set_euler_angles, -20 * DEGREES, 75 * DEGREES, run_time=3
            ),
        )
        ...
        self.wait(7)
        self.play(Restore(cubes[3], run_time=2), FadeOut(plane))
...
class WantAdditionToBeSubtraction(ThreeDScene):
    def construct(self):
        ...
        arrows = VGroup(
            *[Arrow(some_label.get_top(), coef.get_bottom()) for coef in coefs]
        )
        ...
        self.play(LaggedStartMap(Restore, coins, lag_ratio=0.3), run_time=1)
        self.play(
            FadeIn(to_fade),
        ...
        for label in add_label, sub_label:
            ...
            self.play(FlipCoin(coins[2]), FadeIn(label, 0.5 * DOWN))
...
class BitVectorSum(ThreeDScene):
    def construct(self):
        ...
        def get_rhs(coins=coins, bit_vectors=bit_vectors, all_coords=all_coords, eq=eq):
            bit_coords = (
                sum([(b * coords) for coords, b in zip(all_coords, coins.get_bools())])
                % 2
            )
...
class ExampleSquareAsBinaryNumber(Scene):
    def construct(self):
        ...
        for n, square in enumerate(board):
            bin_num = VGroup(
                *[Integer(int(b)) for b in int_to_bit_coords(n, min_dim=6)]
            )
        ...
        self.wait()
        self.play(
            FadeOut(last), bin_nums[n].set_opacity, 1, bin_nums[n].set_fill, YELLOW
        )
        self.wait()
...
class ShowCurrAndTarget(Scene):
    ...
    def construct(self):
        ...
        bit_strings = VGroup(
            *[get_bit_aligned_bit_string(bs) for bs in self.bit_strings]
        )
    ...
    def construct(self):
        ...
        for one_group, highlight in zip(one_groups, highlights):
            ...
            if curr_highlight is None:
                ...
                self.play(
                    LaggedStartMap(
                        DrawBorderThenFill, highlight, lag_ratio=0.1, run_time=3
                    )
                )
                curr_highlight = highlight
            else:
                self.add(one_group, curr_highlight)
                self.play(MoveToTarget(one_group), Transform(curr_highlight, highlight))
            self.wait()
...
class ShowFinalStrategy(Scene):
    ...
    def construct(self):
        ...
        # Animate
        for lines, fades, movers, og, cm, bit in zip(
            line_groups, fade_groups, mover_groups, one_groups, count_mobs, bits
        ):
            self.play(
                FadeIn(add_words),
                fades.set_opacity,
