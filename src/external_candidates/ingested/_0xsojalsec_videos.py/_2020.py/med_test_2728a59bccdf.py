# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\med_test.py
...
def get_covid_clipboard(
    disease_name="SARS\\\\CoV-2", sign="+", report="Detected", color=GREEN
):
...
class MathAsDesign(Scene):
    def construct(self):
        ...
        alt_l_formula = VGroup(
            VGroup(
                OldTex("\\pm").set_height(0.8), OldTex("k \\text{ odd}", font_size=36)
...
class NewBayesRuleAndMedicalTests(Scene):
    def construct(self):
        ...
        self.play(Write(clipboard[2], run_time=1))
        self.wait()
        ...
        self.play(FadeIn(prob_expression), randy.change, "pondering", prob_expression)
        self.play(
        ...
        self.play(
            VGroup(paradox_group, arrow).next_to,
            odds_formula[0],
            LEFT,
        ...
        self.play(VFadeIn(pis[0]), pis[0].change, "hooray", formula)
        self.play(Blink(pis[0]))
        self.play(VFadeIn(pis[1]), pis[1].change, "maybe", formula)
...
class SamplePopulationBreastCancer(Scene):
    def construct(self):
        ...
        self.play(
            FadeIn(fpr_words, shift=0.2 * UP), ShowIncreasingSubsets(new_boxes[:num_fp])
        )
        self.wait()
        self.play(
        ...
        self.play(
            ...
            UP,
            FadeOut(signs),
            positive_words[0].set_opacity,
            0,
            negative_words[0].set_opacity,
            0,
            positive_words[1].match_x,
            rects[0],
            negative_words[1].match_x,
            rects[1],
            LaggedStart(
        ...
        self.play(
            ...
            LaggedStartMap(
                Indicate, fp_cases, color=GREEN_A, lag_ratio=0.05, run_time=3
            ),
        )
        self.wait()
        ...
        # Sensitivity
        sens_eq = OldTex("\\text{Sensitivity}", "= {9 \\over 10}", "= 90\\%")
        sens_eq.next_to(rects[0], LEFT, MED_LARGE_BUFF, aligned_edge=UP)
        sens_eq.shift(DOWN)

        fnr_eq = OldTex("\\text{False Negative Rate}", "= 10\\%")
        ...
        # Specificity
        spec_eq = OldTex("\\text{Specificity}", "= {901 \\over 990}", "\\approx 91\\%")
        spec_eq.next_to(rects[1], RIGHT, MED_LARGE_BUFF, aligned_edge=DOWN)
        spec_eq.shift(UP)

        fpr_eq = OldTex("\\text{False Positive Rate}", "= 9\\%")
...
class AskWhatTheParadoxIs(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says("Consider the\\\\", "test accuracy")
        self.wait(3)
...
class MedicalTestsMatter(Scene):
    def construct(self):
        ...
        self.add(randy)
        self.play(FadeIn(clipboard, shift=LEFT), randy.change, "guilty")
        self.play(
...
class GigerenzerSession(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(image), Write(name))
        self.wait()
        ...
        self.play(
            LaggedStart(
                *(
                    ShowCreationThenFadeOut(
                        SurroundingRectangle(stat[1], color=stat[0].get_color())
                    )
                    for stat in stats
                )
            )
        ...
        self.play(
            LaggedStart(
                *(FadeIn(choice, scale=1.2) for choice in choices), lag_ratio=0.3
            )
        )
        self.wait()
        ...
        self.play(ShowCreation(a_rect), Write(q_mark2))
        ...
        self.play(
            ReplacementTransform(a_rect, c_rect), FadeOut(q_mark), Write(checkmark)
        )
        self.wait()
...
class OldGigerenzerMaterial(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(MoveToTarget(women), FadeIn(signs, lag_ratio=0.01))
        self.wait()

        sens_group = VGroup(sensitivity_words, with_bc_label, women_rect, women, signs)
        ...
        self.play(MoveToTarget(spec_women), FadeIn(spec_signs, lag_ratio=0.01))
        self.wait()
...
class AskIfItsAParadox(TeacherStudentsScene):
    def construct(self):
        ...
        # Commentary
        self.student_says(
            "I'm sorry, is that\\\\a paradox?", target_mode="sassy", index=1
        )
        self.play_student_changes(
            "angry", "sassy", "angry", added_anims=[self.teacher.change, "guilty"]
        )
        self.wait(2)
...
class GoalsOfEstimation(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("pondering", "thinking", "confused", look_at=stats[1])
        self.wait()
...
class QuickEstimatesAndMisconceptions(Scene):
    def construct(self):
        ...
        for circle, title, u in zip(circles, titles, [-1, 1]):
            circle.set_x(u)
            title.set_color(interpolate_color(circle.get_color(), WHITE, 0.5))
        ...
        titles[1].save_state()
        self.play(
            FadeOut(titles[0]), FadeOut(circles[0]), titles[1].match_x, circles[1]
        )
        self.wait()
        ...
        self.play(
            bayes_factor.move_to,
            VGroup(circles[0].saved_state, circles[1]),
...
class SamplePopulation10PercentPrevalence(Scene):
    def construct(self):
        ...
        accuracy_figures = VGroup(
            OldTexText(
                "90\\% Sensitivity,", " 10\\% False negative rate", font_size=36
            ),
            OldTexText("91\\% Specificity,", " 9\\% False positive rate", font_size=36),
        )
        ...
        # Show test stats
        self.play(FadeOut(title, shift=UP), FadeIn(accuracy_figures[0], 0.5 * UP))

        # Show test results
        c_boxes = VGroup(*(SurroundingRectangle(icon, buff=0) for icon in cancer_cases))
        h_boxes = VGroup(
            *(SurroundingRectangle(icon, buff=0) for icon in healthy_cases)
        )
        ...
        self.wait()
        self.play(
            false_positives.move_to,
            healthy_cases,
            FadeOut(
                VGroup(*(case for case in healthy_cases if case not in false_positives))
            ),
        ...
        true_positives.target.arrange(
            RIGHT,
            buff=get_norm(
                false_positives[0].get_right() - false_positives[1].get_left()
            ),
        )
        ...
        self.play(
            ...
            new_wc_words.next_to,
            true_positives.target,
            DOWN,
            new_wo_words.next_to,
...
class SamplePopulationOneInThousandPrevalence(Scene):
    def construct(self):
        ...
        # Show population
        dots1k, dots10k = [
            VGroup(*(Dot() for x in range(n))).set_fill(GREY_D) for n in [1000, 10000]
        ]
        ...
        self.play(
            ...
            nc_count.set_opacity,
            1,
            nc_count.next_to,
            non_cancer_cases.target,
            UP,
            FadeOut(titles[1]),
        )
        self.wait()
        ...
        for case in it.chain(tp_cases, fp_cases):
            box = SurroundingRectangle(case, buff=0.1 * case.get_width())
            box.set_stroke(GREEN, 3)
            case.box = box

        tn_cases = VGroup(*(case for case in non_cancer_cases if case not in fp_cases))
        ...
        self.play(
            FadeOut(c_count),
            FadeIn(tp_label),
            LaggedStart(*(ShowCreation(case.box) for case in tp_cases)),
            cancer_cases[9].set_opacity,
            0.25,
        )
        self.wait()
        self.play(
            ...
            FadeIn(fp_label),
            LaggedStart(
                *(ShowCreation(case.box) for case in fp_cases), lag_ratio=0.001
            ),
            tn_cases.set_opacity,
            0.25,
            run_time=2,
        )
        self.wait()
...
class AltShowUpdatingPrior(Scene):
    def construct(self):
        ...
        before_arrow1 = Arrow(
            before_words[0][0].get_left(),
            prior_prob.get_right() + MED_SMALL_BUFF * RIGHT,
        )
        ...
        update_words = OldTexText(
            "See positive test", tex_to_color_map={"positive": GREEN}
        )
        ...
        self.play(GrowFromCenter(post_brace), FadeIn(post_words, shift=0.5 * UP))
        self.wait()
        ...
        pop100, pop11, pop10, pop2 = pops = [
            VGroup(*[woman.copy() for x in range(n)]) for n in [100, 11, 10, 2]
        ]
        for pop in pops:
            pop.arrange_in_grid()
            pop.set_fill(GREY)
            pop[0].set_fill(YELLOW)
            pop.scale(post_pop[0].get_height() / pop[0].get_height())
        ...
        self.play(*replace_population_anims(pop11, pop2, post_prob[2], post_brace))
        self.wait(2)
...
class NewContrastThreeContexts(Scene):
    def construct(self):
        ...
        # Scene templates
        screens = VGroup(*(ScreenRectangle() for x in range(3)))
        ...
        positive_results = VGroup(*(get_positive_result() for screen in screens))
        ...
        # Different results
        probs = VGroup(
            *(Integer(0, unit="\\%").next_to(result, UP) for result in positive_results)
        )
...
class BayesFactor(Scene):
    def construct(self):
        ...
        with_bc_label = OldTexText(
            "Patients with\\\\breast cancer", font_size=36, color=bc_rect.get_color()
        )
        ...
        without_bc_label = OldTexText(
            "Patients without\\\\breast cancer", font_size=36, color=nc_rect.get_color()
        )
...
class RuleOfThumb(Scene):
    def construct(self):
        ...
        ex_part = OldTex(
            "\\text{e.g. } {90\\% \\over 9\\%} = {10}", tex_to_color_map=t2c
        )
        ...
        condition = OldTexText(
            "If Prior $\\ll$ 1 \\dots", tex_to_color_map={"Prior": YELLOW}
        )
        ...
        self.play(
            ...
            FadeIn(equation.get_parts_by_tex(")")),
            TransformFromCopy(
                condition.get_part_by_tex("Prior"), equation.get_part_by_tex("Prior")
            ),
            TransformFromCopy(
                bf_label[0],
        ...
        self.play(randy.change, "sad", ex2_lhs, ShowCreation(strike, run_time=0.3))
        self.play(Blink(randy))
        self.wait()
        self.play(
            randy.change,
        ...
        self.play(
            PiCreatureSays(
                morty,
                "This equation\\\\is precisely true!",
...
class ProbabilityVsOdds(Scene):
    def construct(self):
        ...
        self.play(FadeIn(people, lag_ratio=0.3), Write(prob))
        self.wait()
        ...
        self.play(
            ...
            TransformFromCopy(odds.copy().replace(prob).set_opacity(0), odds),
            GrowArrow(arrow),
        )
        self.wait()
        self.play(
        ...
        def get_example(n_pos, n_all):
            new_prob = OldTex(f"{int(100 * n_pos / n_all)}\\%", font_size=72)
            new_odds = get_odds_label(n_pos, n_all - n_pos)
            new_odds.next_to(new_prob, RIGHT, buff=1.5)
            arrow = Arrow(new_prob.get_right(), new_odds.get_left(), thickness=0.05)
        ...
        examples = VGroup(
            *(get_example(n, k) for n, k in [(1, 10), (1, 5), (1, 2), (4, 5), (9, 10)])
        )
        examples.arrange(UP)
        ...
        for n, k, ei in (1, 2, 1), (1, 10, 2), (4, 5, 0), (9, 10, 3):
            kw = transition_to_new_example(n, k, examples[ei], **kw)
            self.wait(2)
        ...
        self.play(Write(left_range), Write(right_range), run_time=1)
        self.play(
        ...
        right_ticker.add_updater(
            lambda m: m.move_to(
                right_range.n2p(prob.get_value() / (1 - prob.get_value())), DOWN
            )
        )
...
class OddsComments(Scene):
    def construct(self):
        ...
        self.play(FadeIn(morty))
        self.play(
            PiCreatureSays(morty, "The chances are\\\\1 to 1", run_time=1),
            FadeIn(randy),
        )
        self.play(Blink(morty))
        self.play(
...
class NewSnazzyBayesRuleSteps(Scene):
    def construct(self):
        # Add title
        title = OldTexText("Bayes' rule, the snazzy way", font_size=72)
        title.to_edge(UP)
        ...
        self.wait()
        self.play(Indicate(sens_part), FadeIn(sens_word), GrowArrow(sens_arrow))
        self.wait()
        self.play(
            Indicate(fpr_part),
            FadeIn(fpr_word),
        ...
        alt_prior_odds = OldTex("10\\%", "\\rightarrow", "\\text{1:9}")
        ...
        self.wait()
        self.play(
            Write(alt_prior_odds[1]), FadeIn(alt_prior_odds[2], shift=0.5 * RIGHT)
        )
        self.wait()
        ...
        self.play(FadeIn(rects[:2], lag_ratio=0.6, run_time=1.5))
        self.play(
        ...
        self.play(FadeOut(rects[2]), Write(alt_rhs))
        self.wait()
        ...
        self.play(
            LaggedStartMap(Indicate, population[1:], color=GREY_A, lag_ratio=0.01)
        )
        self.wait()
        ...
        self.wait()
        self.play(FadeOut(bf_computation), FadeIn(new_bf_computation[:-2]))
        ...
        rects = VGroup(
            ...
            SurroundingRectangle(
                new_bf_computation.get_parts_by_tex("90")[1], color=GREEN
            ),
            SurroundingRectangle(
                final_multiplication.get_part_by_tex("90:99"), color=RED
            ),
        )
...
class WhyTheBayesFactorTrickWorks(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                ...
                VGroup(
                    *(
                        SurroundingRectangle(
                            icon, color=GREY, stroke_width=1, buff=0.05
                        )
                        for icon in population[10:]
                    )
                ),
        ...
        tn_cases = VGroup(*(case for case in pop_copy[10:] if case not in fp_cases))
        ...
        self.play(TransformFromCopy(population, pop_copy), path_arc=30 * DEGREES)
        ...
        self.play(
            *(
                TransformFromCopy(
                    frac.get_part_by_tex(tex), final_frac.get_part_by_tex(tex)
                )
                for tex in (w_tex, "\\over")
            )
        )
        self.wait()
        self.play(TransformFromCopy(big_left_brace, left_brace), Write(final_frac[1:7]))
        self.wait()
        self.play(
            *(
                TransformFromCopy(
        ...
        self.play(
            TransformFromCopy(big_right_brace, right_brace), Write(final_frac[9:])
        )
        self.wait()
        self.add(final_frac)

        # Circle likelihood ratio
        likelihood_ratio = VGroup(final_frac[2:7], final_frac[10:])
    ...
    def construct(self):
        ...
        arrows = VGroup(
            *(
                Arrow(
                    factor.get_right(),
                    prior.target.get_corner(LEFT + u * UP),
                    buff=0.1,
        ...
        self.play(
            ShowCreation(strike),
            FadeIn(
                not_words, shift=0.2 * DOWN, rate_func=squish_rate_func(smooth, 0.3, 1)
            ),
        )
        self.wait()
...
class WhatAboutANegativeResult(Scene):
    def construct(self):
        ...
        self.play(
            FadeIn(clipboards[1], LEFT), randys[1].change, "hooray", clipboards[1]
        )
        ...
        self.play(
            *(FadeIn(equation, lag_ratio=0.1, run_time=2) for equation in equations),
            randys[0].change,
            "pondering",
            equations[0],
            randys[1].change,
...
class BayesTheorem(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(TransformFromCopy(prob_formula[0:10], prob_formula[11:21]))
        self.wait()
        self.play(FadeIn(prob_formula[22:26]))
        self.wait()
        self.play(
            FadeIn(prob_formula[26:]),
            braces[3].set_fill,
            GREY_B,
            symbols[3].set_opacity,
            1,
        )
        self.wait()
        ...
        self.play(VFadeIn(randy), randy.change, "maybe", prob_formula)
        self.play(Blink(randy))
...
class ContrastTwoFormulas(Scene):
    def construct(self):
        ...
        prior_arrow = Arrow(
            prior_words.get_top(), prior_part[:2].get_bottom(), buff=0.2
        )
        ...
        bf_terms = OldTex("\\text{Sensitivity} \\over \\text{FPR}", **kw)
        ...
        self.play(
            VGroup(bf_terms, bf_rect, bf_words).shift,
            0.25 * UR,
            prior_part.shift,
            0.25 * UL,
            rate_func=there_and_back_with_pause,
        ...
        self.play(
            TransformFromCopy(
                pt_comments[1][:-1], pt_comments[0][:-1], path_arc=-20 * DEGREES
            ),
            GrowFromPoint(
                pt_comments[0][-1], pt_comments[1][-1].get_left(), path_arc=20 * DEGREES
            ),
            run_time=1,
        )
        ...
        self.play(
            LaggedStart(
                *(
                    ...
                    for part, vect in zip(
                        words_to_move, [UP, DOWN, DOWN, UP, DOWN, DOWN]
                    )
                ),
                lag_ratio=0.05,
            )
        ...
        new_bf_rects = VGroup(
            *(SurroundingRectangle(part, buff=0.05) for part in new_bf_parts)
        )
        ...
        self.wait()
        self.play(LaggedStartMap(FadeIn, clipboards, shift=0.5 * DOWN, lag_ratio=0.3))
        self.wait()
        self.play(
        ...
        to_fade = VGroup(
            mult_formula, new_bf_labels, new_bf_rects, clipboards[0], sick_randy
        )
...
class ConfusedByFPR(Scene):
    def construct(self):
        ...
        self.add(randy)
        self.add(doctor)
        self.play(FadeIn(clipboard, LEFT), randy.change, "guilty")
        self.play(DrawBorderThenFill(bubble), Write(fpr_words))
        self.play(Blink(randy))
        self.wait()

        # Misinterpret
        false_prob = OldTex(
            "P(\\text{Test is false}) = 9\\%", tex_to_color_map={"9\\%": WHITE}
        )
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOut,
                VGroup(thought_bubble, false_prob, cross),
                scale=0.5,
                run_time=1,
            ),
        )
...
class ShowContrastingMethods(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(h_line), FadeIn(titles), *anims)
        ...
        globals()["ans"] = ans
        ans_arrows = VGroup(
            *(Arrow(step.get_right(), ans.get_left(), buff=0.1) for step in steps)
        )
        ...
        self.play(FadeIn(ppv_formula, RIGHT))
        self.play(*map(GrowArrow, ans_arrows), FadeIn(ans, RIGHT))
        self.wait()
...
class MentionPart2(Scene):
    def construct(self):
        ...
        words = OldTexText(
            "Possible follow-up\\\\breaking this down", alignment="", font_size=60
        )
        words.to_edge(RIGHT)
        words.align_to(tweet, UP)
        arrow = Arrow(
            words.get_bottom() + SMALL_BUFF * DOWN,
            tweet.get_right(),
            path_arc=-45 * DEGREES,
        )
...
class OldBayesFactorCode(Scene):
    def construct(self):
        ...
        def get_prob_bars(p_positive):
            ...
            braces = VGroup(*[Brace(rect, LEFT, buff=SMALL_BUFF) for rect in rects])
        ...
        boxes = VGroup(Square(color=YELLOW), Square(color=GREY_B))
        boxes.set_height(3)
        labels = VGroup(
        ...
        self.play(
            ...
            post_group.scale,
            0.1,
            {"about_point": FRAME_HEIGHT * DOWN / 2},
        )
        self.wait()
        self.play(FadeIn(boxes[1], lag_ratio=0.1))
        self.wait()
...
class HowDoesUpdatingWork(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says(
            "Where are these\\\\numbers coming\\\\from?", target_mode="raise_right_hand"
        )
        self.play(
            teacher.change,
            "happy",
...
class HighlightBayesFactorOverlay(Scene):
    def construct(self):
        ...
        words.shift(2 * RIGHT)
        words.match_color(rect)
        arrow = Arrow(words.get_left(), rect.get_bottom(), path_arc=-60 * DEGREES)
...
class LookOverTweet(Scene):
    def construct(self):
        ...
        result = OldTexText("+\\\\", "SARS\\\\CoV-2\\\\", "Detected")
        ...
        self.play(FadeOut(lines[0]), FadeOut(lines[2]), DrawBorderThenFill(rect))
...
class DisambiguateFPR(Scene):
    def construct(self):
        # Add title
        title = OldTexText(
            "1\\% False Positive Rate", isolate=["F", "P", "R"], font_size=72
        )
        title.to_edge(UP)
        ...
        word_grid.arrange_in_grid(v_buff=1.0, h_buff=0.5, fill_rows_first=False)
        word_grid.move_to(2 * DOWN)

        globals()["word_box"] = word_box
        word_boxes = VGroup(
            *(word_box.copy().move_to(word).match_color(word) for word in word_grid)
        )
        ...
        tp, fn, fp, tn = (word for word, box in zip(word_grid, word_boxes))
        ...
        title.add(underline)
        fpr = OldTexText("1\\% ", "F", "", "P", "", "R", "", font_size=72)
        ...
        self.play(FadeOut(arrow), LaggedStart(*fraction_anims(fp, tn, frac2)))
        self.wait(2)

        self.play(ReplacementTransform(title, fpr), Restore(frac2))
        self.play(
            LaggedStartMap(Write, word_grid[:2], lag_ratio=0),
            LaggedStartMap(ShowCreation, word_boxes[:2], lag_ratio=0),
            with_pop.set_opacity,
            0.2,
            run_time=1,
        )
        self.play(
        ...
        full_pop = VGroup(OldTexText("Imagine"), Integer(10000), OldTexText("People"))
        ...
        def move_number_above_word(
            number, word, target_count, buff=MED_SMALL_BUFF, scene=self
        ):
            mover = number.copy()
            ...
            scene.play(
                UpdateFromAlphaFunc(
                    ...
                    .move_to(
                        interpolate(m.original_bottom, word.get_top() + buff * UP, a),
                        DOWN,
                    ),
                )
            )
...
class BayesFactorForCovidExample(Scene):
    def construct(self):
        ...
        # Prior odds
        prior_odds = OldTexText("1", "\\,:\\,", "999", tex_to_color_map={"1": YELLOW})
        ...
        self.wait()
        self.play(TransformFromCopy(bf_rhs[0], eq_90[0]), Restore(eq_90[1]))
        self.wait()
        ...
        fraction = OldTex("{90", "\\over", "90", "+", "999}", "\\approx", "8.3\\%")
        ...
        self.wait()
        self.play(GrowArrow(arrow), FadeIn(fraction, DOWN))
        ...
        self.wait()
        self.play(ReplacementTransform(approx1, approx2))
        self.wait()
        self.play(GrowArrow(arrow2), FadeIn(approx3, DOWN))
        self.wait()
...
class WhyIsThisWrong(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(FadeIn(percent), self.teacher.change, "tease")
...
class TotalPopulationVsSymptomatic(Scene):
    def construct(self):
        ...
        full_pop.arrange_in_grid(buff=full_pop[0].get_width())
        ...
        self.play(FadeIn(full_pop_words[0]), Write(covid_pop), run_time=1)
        self.play(
        ...
        self.play(GrowArrow(arrow), FadeIn(prop_ex, DOWN))
        self.wait()
        ...
        self.play(
            VGroup(sym_pop, sym_pop_label).to_edge,
            RIGHT,
        ...
        right_title = OldTexText(
            "Population ", "\\emph{with symptoms}\\\\", "and a positive test result"
        )
        ...
        odds_underlines = VGroup(*(Underline(label[1:]) for label in odds_labels))
        odds_arrows = VGroup(
            *(
        ...
        bf_labels = VGroup(
            OldTexText(
                "Bayes factor for symptoms", tex_to_color_map={"for symptoms": BLUE}
            ),
            OldTexText(
                "Bayes factor positive test", tex_to_color_map={"positive test": GREEN}
            ),
        )
        ...
        self.play(curr_odds.move_to, odds_labels[1][1:], UR, path_arc=-30 * DEGREES)
        ...
        self.play(curr_odds.move_to, odds_labels[2][1:], UR, path_arc=-30 * DEGREES)
...
class TwoMissteps(Scene):
    def construct(self):
        words = VGroup(
            OldTexText("Misstep 1: ", "Fail to consider the prevalence."),
            OldTexText("Misstep 2: ", "Assume prior = prevalence"),
        )
