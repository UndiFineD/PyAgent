# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\sir.py
    ...
    def update_position(self, dt):
        ...
        if self.social_distance_factor > 0:
            ...
            for point in self.repulsion_points:
                ...
                if dist > 0:
                    repulsion_force -= (
                        self.social_distance_factor * to_point / (dist**3)
                    )
...
class SIRSimulation(VGroup):
        ...
        for box in self.boxes:
            ...
            for x in range(self.city_population):
                person = self.person_type(
                    dl_bound=dl_bound, ur_bound=ur_bound, **self.person_config
                )
                person.move_to(
                    [
    ...
    def update_statusses(self, dt):
        for box in self.boxes:
            s_group, i_group = [
                list(filter(lambda m: m.status == status, box.people))
                for status in ["S", "I"]
            ]
            ...
            for s_person in s_group:
                for i_person in i_group:
                    ...
                    if (
                        dist < s_person.infection_radius
                        and random.random() < self.p_infection_per_day * dt
                    ):
                        s_person.set_status("I")
                        i_person.num_infected += 1
            for i_person in i_group:
                if (
                    i_person.time - i_person.infection_start_time
                ) > self.infection_time:
        ...
        if self.limit_social_distancing_to_infectious:
            repelled_centers = np.array(
                [person.get_center() for person in self.people if person.symptomatic]
            )
        else:
        ...
        if len(repelled_centers) > 0:
            for center, person in zip(centers, self.people):
                if person.social_distance_factor > 0:
                    ...
                    person.repulsion_points = repelled_centers[
                        np.argsort(diffs)[1 : person.n_repulsion_points + 1]
                    ]

    def get_status_counts(self):
        return np.array(
            [
                len(list(filter(lambda m: m.status == status, self.people)))
                for status in "SIR"
            ]
        )
...
class GraphBraces(VGroup):
    ...
    def __init__(self, graph, simulation, **kwargs):
        ...
        self.lines = VGroup(
            *[Line(axes.c2p(1, y1), axes.c2p(1, y2)) for y1, y2 in zip(ys, ys[1:])]
        )
    ...
    def update_braces(self):
        ...
        for i, y1, y2 in zip([1, 0, 2], ys, ys[1:]):
            ...
            braces[i].set_height(max(height, epsilon), stretch=True)
    ...
    def add_R_label(self):
        ...
        def update_label(label):
            ...
            for person in self.simulation.people:
                if person.status == "I":
                    prop = (
                        person.time - person.infection_start_time
                    ) / self.simulation.infection_time
                    if prop > 0.1:
    ...
    def add_graph(self):
        ...
        if self.include_graph_braces:
            self.graph_braces = GraphBraces(
                graph, self.simulation, update_frequency=graph.update_frequency
            )
...
class SimpleTravelDelayedSocialDistancing50pThreshold100(
    SimpleTravelDelayedSocialDistancing
):
    CONFIG = {
        "sd_probability": 0.50,
...
class SimpleTravelDelayedSocialDistancing70pThreshold100(
    SimpleTravelDelayedSocialDistancing
):
    CONFIG = {
        "sd_probability": 0.70,
...
class SimpleTravelSocialDistancePlusZeroTravel99p(
...
class SimpleTravelDelayedTravelReductionThreshold100(
...
class SimpleTravelDelayedTravelReductionThreshold100TargetHalfPercent(
    SimpleTravelDelayedTravelReduction
):
    CONFIG = {
        "random_seed": 2,
...
class SimpleTravelDelayedTravelReductionThreshold100TargetHalfPercent2(
...
class SocialDistanceAfterFiveDays(DelayedSocialDistancing):
    CONFIG = {
        "sd_probability": 0.7,
        "delay_time": 5,
        "simulation_config": {"travel_rate": 0},
    }
...
class QuarantineInfectious(RunSimpleSimulation):
    ...
    def add_quarantine_box(self):
        ...
        q_box.next_to(boxes, LEFT, aligned_edge=DOWN, buff=0.25 * q_box.get_width())
            ...
            for person in simulation.people:
                send_to_q_box = all(
                    [
                        ...
                        person.symptomatic,
                        (person.time - person.infection_start_time)
                        > self.infection_time_before_quarantine,
                    ]
                )
                if send_to_q_box:
                    ...
                    person.push_anim(
                        ApplyMethod(
                            point.move_to, self.q_box.get_center(), run_time=0.5
                        )
                    )
...
class CentralMarket(DelayedSocialDistancing):
    CONFIG = {
        "sd_probability": 0.7,
        "delay_time": 5,
        "simulation_config": {"person_type": DotPerson, "travel_rate": 0},
    ...
    def setup(self):
        ...
        self.simulation.add_updater(lambda m, dt: self.add_travel_anims(m, dt))
        ...
        for person in simulation.people:
            ...
            if time_since_trip > shopping_time:
                if random.random() < dt * self.shopping_frequency:
                    ...
                    anim1 = ApplyMethod(
                        point.move_to,
                        person.box.get_center(),
                        path_arc=45 * DEGREES,
                        run_time=shopping_time,
                        rate_func=there_and_back_with_pause,
                    )
                    anim2 = MaintainPositionRelativeTo(
                        person, point, run_time=shopping_time
                    )
...
class CentralMarketVeryFrequentLargePopulationDelayedSocialDistancing(
...
class CentralMarketLessFrequent(
    ...
    def construct(self):
        self.wait_until(
            lambda: self.simulation.get_status_counts()[1]
            > self.trigger_infection_count
        )
        for person in self.simulation.people:
...
class CentralMarketTransitionToLowerInfection(CentralMarketLessFrequent):
    ...
    def change_slider(self):
        self.play(
            self.infection_slider.get_change_anim(self.target_p_infection_per_day)
        )
...
class CentralMarketTransitionToLowerInfectionAndLowerFrequency(
...
class DescribeModel(Scene):
    def construct(self):
        ...
        pi.set_status("R")
        self.play(word_anims[2], Animation(pi, suspend_mobject_updating=False))
        words[2].pi = pi.copy()
        self.play(
            words[2].pi.set_height,
            1,
            words[2].pi.next_to,
            words[2],
            RIGHT,
...
class DoubleRadiusInGroup(Scene):
    def construct(self):
        ...
        for pi in pis:
            if pi.status == "S":
                anims.append(ApplyMethod(pi.body.change, "pleading", sicky.body.eyes))
...
class CutPInfectionInHalf(Scene):
    def construct(self):
        ...
        self.play(FadeIn(eq))
        self.play(TransformFromCopy(eq[1], new_rhs), GrowArrow(arrow))
        ...
        for title, takeaway in zip(titles, takeaways):
            ...
            self.play(
                ...
                FadeOut(
                    takeaway,
                    DOWN,
                    lag_ratio=0.25 / len(takeaway.family_members_with_points()),
                ),
            )
            self.wait()
...
class WeDontHaveThat(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_all_student_changes("angry", look_at=self.teacher.eyes)
        self.wait(5)
...
class IntroduceSocialDistancing(Scene):
    def construct(self):
        ...
        for pi in pis:
            ...
            pi.repulsion_points = [pi2.get_center() for pi2 in pis if pi2 is not pi]
...
class FastForwardBy2(Scene):
    ...
    def construct(self):
        n = self.n
        triangles = VGroup(*[ArrowTip(start_angle=0) for x in range(n)])
...
class ThatsNotHowIBehave(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.teacher.change, "guilty", self.change_students("erm", "erm", "sassy")
        )
...
class DarkerInterpretation(Scene):
    def construct(self):
        ...
        self.play(FadeOut(qz), FadeOut(line), FadeIn(new_words))
        self.wait()
...
class SARS2002(TeacherStudentsScene):
    def construct(self):
        ...
        n_cases = VGroup(Integer(0, edge_to_fix=UR), OldTexText("total cases"))
...
class IntroduceR0(Scene):
    def construct(self):
        ...
        count = VGroup(OldTexText("Infection count: "), Integer(0))
...
class R0Categories(Scene):
    def construct(self):
        ...
        for dot, line in zip(mid_rows[:-1], mid_lines):
            self.add(dot)
            self.play(TransformFromCopy(dot, dot.child), ShowCreation(line))
...
class NickyCaseMention(Scene):
    def construct(self):
        words = OldTexText("Artwork by Nicky Case\\\\", "...who is awesome.")
        words.scale(1)
        words.to_edge(LEFT)
        arrow = Arrow(
            words.get_top(),
...
class VirusSpreading(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                MoveToTarget, viruses, run_time=8, lag_ratio=3 / len(viruses)
            )
        )
