# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\inscribed_rect\loops.py
...
class StateThePuzzle(LoopScene):
    def construct(self):
        ...
        self.play(ShowCreation(loop), Write(curve_words, time_span=(3, 5)), run_time=9)
        ...
        dots = self.get_movable_quad(
            quad_tracker, loop_func, colors=color_gradient([RED, PINK], 4), radius=0.075
        )
        ...
        self.play(Write(inscribed_words), FadeOut(curve_words, LEFT))
        self.wait()
        ...
        self.play(FadeIn(question_group, UP), FadeOut(inscribed_words, LEFT))
        self.wait()
        ...
        def true_find_square(loop_func, trg_angle=90 * DEG, cost_tol=1e-2, max_tries=8):
            ...
            for x in range(max_tries):
                params, cost = find_rectangle(
                    loop_func, target_angle=trg_angle, n_refinements=3, return_cost=True
                )
                if cost < min_cost:
                    min_params = params
                    min_cost = cost
        ...
        for new_loop in new_loops:
            ...
            self.add(dots, polygon)
            for _ in range(5):
                quad_tracker.set_value(
                    find_rectangle(
                        loop_func, np.random.random(4), target_angle=90 * DEG
                    )
                )
                self.wait(0.5)
        ...
        self.play(FadeOut(title, UP), ShowCreation(red_line))
        self.play(
        ...
        for dot, p in zip(new_dots, rect_params):
            dot.move_to(loop_func(p))
        self.play(Transform(dots, new_dots), run_time=4)
        self.wait()
        dots.resume_updating()
    ...
    def construct(self):
        ...
        self.play(
            ...
            TransformMatchingStrings(
                question1[0],
                question2[0],
                key_map={"four": "two pairs of"},
                mismatch_animation=FadeTransformPieces,
            ),
...
class ShowTheSurface(LoopScene):
    def construct(self):
        ...
        # Show the distance
        brace = Brace(
            Line(LEFT, RIGHT).set_width(connecting_line.get_length()),
            DOWN,
            buff=SMALL_BUFF,
        )
        ...
        d_label.move_to(
            brace.get_center()
            + 0.4 * normalize(brace.get_center() - midpoint_dot.get_center())
        )
        ...
        self.play(
            frame.animate.reorient(-21, 84, 0, (0.4, 0.01, 1.41), 5.07), run_time=5
        )
        self.play(
            frame.animate.reorient(0, 18, 0, (0.41, -0.13, 1.34), 4.76), run_time=2
        )
        self.play(FlashAround(coords, run_time=2, time_width=1.5))
        self.play(
            TransformFromCopy(
                connecting_line, z_line, suspend_mobject_updating=True, time_span=(4, 6)
            ),
            frame.animate.reorient(-38, 88, 0, (1.19, -0.11, 1.48), 3.94),
            run_time=6,
        )
        self.play(
            frame.animate.reorient(-4, 88, 0, (1.19, -0.11, 1.48), 3.94), run_time=5
        )
        self.wait()
        ...
        for _ in range(3):
            self.play(
                ...
                uv_tracker2.animate.increment_value(
                    np.random.uniform(-nudge, nudge, 2)
                ),
                run_time=2,
        ...
        self.play(ShowCreation(rect), loop.animate.set_stroke(width=2))
        self.wait(2)
        self.play(
            FadeOut(rect),
            FadeIn(z_group),
        ...
        for dot in [top_dot, top_dot2]:
            tail = TracingTail(
                dot, time_traced=5, stroke_color=BLUE, stroke_width=(0, 5)
            )
        ...
        # Surface
        surface, mesh, surface_func = self.get_surface_info(
            loop_func, surface_resolution=(301, 301)
        )
        ...
        self.play(
            ...
            frame.animate.reorient(-29, 81, 0, (0.14, 0.39, 2.1), 6.47).set_anim_args(
                run_time=8
            ),
        )
        self.wait(15)
        ...
        self.play(
            FadeIn(dot_group1), FadeIn(dot_group2), surface.animate.set_opacity(0.75)
        )
        ...
        rect = Rectangle()
        rect.f_always.set_points_as_corners(
            lambda: list(map(loop_func, abcd_tracker.get_value()))
        )
        ...
        self.animate_to_rectangle_with_angle(
            abcd_tracker,
            loop_func,
            80 * DEGREES,
            n_samples=10,
            param_range_per_step=0.02,
        )
        z_tracker.clear_updaters()
        self.play(
            frame.animate.reorient(0, 44, 0, (0.02, 0.63, 0.47), 7.67), run_time=5
        )
        ...
        self.play(
            frame.animate.reorient(2, 70, 0, (0.05, 1.06, 0.05), 4.29), run_time=10
        )
        self.wait()
    ...
    def animate_to_rectangle_with_angle(
        ...
        curr_angle = abs(
            angle_between_vectors(points[2] - points[0], points[3] - points[1])
        )
        if curr_angle > PI / 2:
        ...
        self.play(
            UpdateFromAlphaFunc(
                abcd_tracker, lambda m, a: m.set_value(smooth_index(rectangle_range, a))
            ),
            run_time=run_time,
        )
...
class ChangeTheSurface(ShowTheSurface):
    def construct(self):
        ...
        self.play(
            Transform(loop, circle), UpdateFromFunc(surface, update_surface), run_time=5
        )
        loop.become(circle)
        ...
        self.play(
            frame.animate.reorient(-11, 67, 0, (-0.08, -0.56, 1.19), 8.04), run_time=2
        )
        self.play(
        ...
        self.play(
            frame.animate.reorient(156, 77, 0, (-0.08, -0.56, 1.19), 8.04), run_time=10
        )
        self.wait(4)

        # Move the coordinates again
        self.play(
            frame.animate.reorient(174, 34, 0, (-0.08, -0.56, 1.19), 8.04), run_time=3
        )
        for value in [0.24, 0.01, 0.125]:
            self.play(x_tracker.animate.set_value(value), run_time=6)
...
class ParameterizeTheLoop(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeOut(loop_x_group), FadeOut(int_x_group), FadeIn(scissors))
        ...
        loop_y_group = self.get_loop_coord_group(
            loop, get_y, color=PINK, label_direction=UR
        )
        loop_y_group.update()
        ...
        axes = Axes(
            (0, 1), (0, 1), width=x_axis.get_length(), height=y_axis.get_length()
        )
        ...
        self.play(
            ShowCreation(right_edge), xy_tracker.animate.set_value([1, 1]), run_time=8
        )
        self.wait()
        self.play(
            Write(left_tips),
        ...
        self.add(pieces)
        self.play(
            VGroup(piece1, piece2).animate.space_out_submobjects(1.5).move_to(square)
        )
        self.wait()
        ...
        def get_partial_strip(upper_theta=1.0):
            result = ParametricSurface(
                lambda u, v: mobius_strip_func(u, custom_squish(v) * upper_theta / TAU)
            )
        ...
        torus_fold_line = ParametricCurve(
            lambda t: torus.uv_to_point(t, t), t_range=(0, 1, 0.01)
        )
        ...
        squished_torus = TexturedSurface(
            ParametricSurface(
                lambda u, v: mobius_strip_func(*torus_uv_to_mobius_uv(u, v))
            ),
            "TorusTexture",
        )
        ...
        new_fold_line = ParametricCurve(
            lambda t: squished_torus.uv_to_point(t, t), t_range=(0, 0.99, 0.01)
        )
    ...
    def get_loop_coord_group(
        self,
        loop,
        get_x,
        color=YELLOW,
        font_size=36,
        dot_to_num_buff=0.075,
        label_direction=UL,
    ):
...
class DiscussOrderOfPoints(LoopScene):
    def construct(self):
        ...
        # Swap points
        for _ in range(2):
            self.play(Swap(*dots), Swap(*labels), run_time=2)
            self.wait()
        ...
        self.play(
            TransformMatchingStrings(
                midpoint_word, dist_word, key_map={"midpoint": "distance"}, run_time=1
            )
        )
        self.play(Swap(*dots))
        ...
        rect = self.get_dot_polygon(list(it.chain(*zip(dots, dots2))))
        rect.update()
        ...
        self.play(
            Rotate(pair_group2, -angle, about_point=mid_dot.get_center()), run_time=5
        )
        self.wait()
        self.play(
            Rotate(pair_group2, -angle, about_point=mid_dot.get_center()), run_time=5
...
class MapTheStripOntoTheSurface(ShowTheSurface):
    def construct(self):
        ...
        strip_point.f_always.move_to(lambda: uv_to_strip_point(*uv_tracker.get_value()))
        ...
        uv_tracker.clear_updaters()
        pre_surface = ParametricSurface(get_half_parametric_func(uv_to_strip_point))
        ...
        pre_collision_dots, post_collision_dots = coll_dot_groups = Group(
            get_special_dot(color, radius=0.075) for color in [YELLOW, GREEN_SCREEN]
        ).replicate(2)
...
class GenericLoopPair(ShowTheSurface):
    def construct(self):
        ...
        dots = self.get_movable_pair(
            uv_tracker, loop_func, radius=0.075, colors=[YELLOW, YELLOW]
        )
        connecting_line = self.get_connecting_line(dots)
...
class SudaneseBand(InteractiveScene):
    def construct(self):
        ...
        # Back to strip
        self.play(Transform(strip, og_strip), run_time=6)
...
class ShowSurfaceReflection(ShowTheSurface):
    def construct(self):
        ...
        self.play(
            frame.animate.reorient(36, 85, 0, (-0.68, 0.78, 2.39), 11.39), run_time=6
        )
...
class ConstructKleinBottle(InteractiveScene):
    def construct(self):
        ...
        mobius_diagram = VGroup(
            ...
            Line(square.get_corner(DL), square.get_corner(UR)).set_stroke(
                Color("red"), 3
            ),
        )
        ...
        self.play(
            LaggedStart(
                TransformMatchingStrings(
                    mobius_label.copy(), reflection_label, run_time=1
                ),
        ...
        self.play(
            LaggedStart(
                ...
                reflection_label.animate.scale(0.75).next_to(
                    square, LEFT, MED_SMALL_BUFF
                ),
        ...
        yellow_arrows = self.get_tri_arrow(
            square.get_corner(UL), square.get_corner(DR), color=YELLOW
        ).replicate(2)
        ...
        trg_yellow_arrows = VGroup(
            self.get_tri_arrow(
                square.get_corner(DR), square.get_corner(DL), color=YELLOW
            ).flip(RIGHT),
            self.get_tri_arrow(
                square.get_corner(UL), square.get_corner(UR), color=YELLOW
            ),
        )
        ...
        self.play(
            UpdateFromAlphaFunc(
                moving_surface,
                lambda m, a: m.match_points(
                    ParametricSurface(
                        lambda u, v: klein_func(
                            u, interpolate(0.5, 1, a) * near_smooth(v)
                        )
                    )
    ...
    def get_kelin_bottle_func(self, width=4, z=4):
        ...
        def pre_klein_func(u, v):
            ...
            tangent = interpolate(
                c_prime, interpolate(UP, DOWN, v_alpha_func(v)), tangent_alpha
            )

            perp = normalize(cross(tangent, OUT))
            radius = radius_func(v)

            return c_point + radius * (
                math.cos(TAU * u) * OUT - math.sin(TAU * u) * perp
            )
        ...
        def true_kelin_func(u, v):
            if v <= v_upper_bound:
                ...
                return interpolate(
                    pre_klein_func(u, v_upper_bound), pre_klein_func(1 - u, 0), alpha
                )
...
class PuzzleOverMobiusDiagram(ConstructKleinBottle):
    def construct(self):
        ...
        low_arrow = self.get_tri_arrow(
            triangle.get_corner(DL), triangle.get_corner(DR), stroke_width=4
        )
        right_arrow = self.get_tri_arrow(
            triangle.get_corner(DR), triangle.get_corner(UR), stroke_width=4
        )
...
class ShowAngleInformation(ShowTheSurface):
    def construct(self):
        ...
        d_label.move_to(
            brace.get_center()
            + 0.5 * normalize(brace.get_center() - midpoint_dot.get_center())
        )
        ...
        # Show the angle
        cross_point = connecting_line.pfp(
            inverse_interpolate(
        ...
        dots2 = self.get_movable_pair(
            uv_tracker2, loop_func, radius=0.05, colors=[RED, MAROON_B]
        )
        connecting_line2 = self.get_connecting_line(dots2)
        pair_group2 = Group(dots2, connecting_line2)
        ...
        self.play(
            ...
            d_label.animate.rotate(-90 * DEGREES, about_point=midpoint).rotate(
                90 * DEG
            ),
        ...
        cross_point2 = connecting_line2.pfp(
            inverse_interpolate(
        ...
        arc2 = Arc(
            0, PI + connecting_line2.get_angle(), radius=0.45, arc_center=cross_point2
        )
        ...
        # Show the square
        square = self.get_dot_polygon(list(it.chain(*zip(dots, dots2))))
...
class TrackTheAngle(ShowTheSurface):
    ...
    def construct(self):
        ...
        # Move points
        self.play(uv_tracker.animate.set_value(self.second_uv), run_time=4)
        self.play(
...
class TrackTheAngleForFractal(TrackTheAngle):
    def get_loop(self, n_iters=7):
        ...
        snowflake = VMobject().set_points_as_corners(
            np.vstack(
                [
...
class MusicalIntervalsAsPairs(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ShowCreation(circle),
            LaggedStart(
                (FadeTransform(Group(key), dot) for key, dot in zip(keys, dots)),
                lag_ratio=0.1,
                group_type=Group,
            ),
