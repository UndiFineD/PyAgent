# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\inscribed_rect\helpers.py
...
def get_special_dot(color=YELLOW, radius=0.05, glow_radius_multiple=3, glow_factor=1.5):
    return Group(
...
def find_rectangle(
    ...
    for _ in range(n_refinements):
        param_groups = [
            np.linspace(x - param_range / 2, x + param_range / 2, n_samples_per_range)
            % 1
            for x in params
        ]
        sample_groups = [
