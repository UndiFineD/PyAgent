# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\inscribed_rect\supplements.py
...
class WhatIsTopology(InteractiveScene):
    def construct(self):
        ...
        self.add(question)
        self.add(morty)
        self.play(Transform(morty, conf_morty), ShowCreation(underline))
        ...
        self.play(FadeIn(answer1, lag_ratio=0.1, run_time=2), FadeOut(morty, DOWN))
        self.wait()
        ...
        # Cross out both
        self.play(
            answer1.animate.set_opacity(1).next_to(answer2, DOWN, buff=MED_LARGE_BUFF),
            run_time=2,
        )
        ...
        self.play(FadeOut(answers), FadeOut(crosses), FadeIn(morty))
        self.play(
...
class LabelMapping(InteractiveScene):
    def construct(self):
        ...
        self.play(GrowArrow(body[1]), TransformFromCopy(*body[0::2]))
        self.wait()
    ...
    def construct(self):
        ...
        self.play(
            self.change_students(
                "pondering", "pondering", "thinking", look_at=self.screen
            )
        )
        self.wait(3)
...
class ChekhovsGun(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(name), FadeIn(Group(image, border), UP))
        self.wait()
...
class CommutativeDiagram(InteractiveScene):
    ...
    def construct(self):
        ...
        down_arrow = Arrow(
            objs[0].get_corner(DL) + UP, objs[1].get_left(), path_arc=PI, **arrow_kw
        )
        lr_arrow = VGroup(
        ...
        dr_arrow = Arrow(
            cloud.get_bottom(), objs[1].get_left() + (-1, 0.25, 0), buff=0.3, **arrow_kw
        )

        self.play(Write(down_arrow))
        self.wait()
        self.play(*map(GrowArrow, lr_arrow), FadeIn(cloud, LEFT))
        self.wait()
...
class ContinuousAssociation(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(words), *map(Write, arrows))
        self.wait()
...
class StripClaim(InteractiveScene):
    def construct(self):
        ...
        self.play(morty.bubble.content.animate.set_color(RED), Blink(morty))
        self.wait()
        ...
        self.wait()
        self.play(Write(new_claim["without intersecting itself."]))
        self.wait()

        # This one is true
        morty = Mortimer(height=1.5).to_corner(DR)

        self.play(VFadeIn(morty), morty.says("This one\nis true", mode="tease"))
        self.play(Blink(morty))
        self.wait()
...
class AskAbout4DEmbeddings(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(GrowArrow(arrow), FadeIn(cloud, DOWN))
        self.wait()
        self.play(
...
class GreeneAndLobb(InteractiveScene):
    def construct(self):
        ...
        for image, name in zip(images, names):
            name.next_to(image, DOWN)
            self.play(FadeIn(image, 0.5 * UP), Write(name))
        self.wait()
...
class ProblemSolvingToRecreation(InteractiveScene):
    def construct(self):
        ...
        arrows = VGroup(
            Arrow(w1, w2, thickness=4, buff=0.45) for w1, w2 in zip(words, words[1:])
        )
...
class PlaylistMention(InteractiveScene):
    def construct(self):
        ...
        thumbnail = ImageMobject(
            "https://img.youtube.com/vi/OkmNXy7er84/maxresdefault.jpg"
        )
        ...
        self.play(FadeIn(group, UP), Write(words), GrowArrow(arrow))
        self.wait()
