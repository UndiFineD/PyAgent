# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\holograms\supplements.py
...
class PhotographVsHologram(InteractiveScene):
    def construct(self):
        ...
        for part, x in zip([photo, holo], [-1, 1]):
            ...
            part.add_to_back(
                Arrow(
                    title.get_edge_center(x * RIGHT),
                    part.get_top(),
                    path_arc=-x * 60 * DEGREES,
                )
            )
...
class WriteWhiteLightReflectionHologram(InteractiveScene):
    def construct(self):
        ...
        title.to_edge(UP)
        self.play(
            Write(title, stroke_color=WHITE, stroke_width=3, lag_ratio=0.1, run_time=3)
        )
        self.wait()
...
class IndicationOnPhotograph(InteractiveScene):
    def construct(self):
        ...
        self.play(
            FadeIn(dot1), FlashAround(dot1, stroke_width=5, time_width=2, color=TEAL)
        )
        self.play(
        ...
        self.play(
            ...
            arrow.animate.match_points(
                StrokeArrow(p1, p3, buff=0.1, path_arc=-30 * DEGREES)
            ),
...
class ContrastPhotographyAndHolography(InteractiveScene):
    def construct(self):
        ...
        points = BulletedList("All but one viewing angle", "Phase", buff=LARGE_BUFF)
        ...
        self.play(FadeIn(title), ShowCreation(underline))
...
class Outline(InteractiveScene):
    def construct(self):
        ...
        for rect, title in zip(top_rects, top_titles):
            ...
            self.play(FadeIn(rect), FadeIn(title, 0.25 * UP))
            self.wait()
        ...
        self.wait()
        self.play(Restore(frame), FadeIn(VGroup(in_arrow, up_arrow)), run_time=2)
        self.wait()

        # Back to the middle
        self.play(
            LaggedStartMap(
                FadeOut, Group(low_title, low_rect, arrow), lag_ratio=0.1, run_time=1
            ),
...
class NameCraigAndSally(InteractiveScene):
    def construct(self):
        ...
        VGroup(craig_arrow, craig_name, sally_arrow, sally_name).set_fill(
            WHITE
...
class HowIsThisHelpful(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.play(
            self.change_students("maybe", "confused", "pondering", look_at=self.screen)
        )
        self.wait(5)
...
class AnnotateSetup(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(pol_words), GrowArrow(pol_arrow))
        self.wait()
        self.play(
            FadeOut(pol_words),
            FadeOut(pol_arrow),
        ...
        obj_beam_spread = VGroup(
            Line(
                obj_beam_points[-1],
                scene_circle.pfp(a) + np.random.uniform(-0.025, 0.025, 3),
            )
        ...
        ref_beam_spread = VGroup(
            Line(
                ref_beam_points[-1],
                ref_circle.pfp(a) + np.random.uniform(-0.025, 0.025, 3),
            )
            for a in np.arange(0, 1, 0.001)
        )
...
class ArrowWithQMark(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(arrow), Write(q_marks), run_time=2)
        self.wait()
...
class ProblemSolvingTipNumber1(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, parts, shift=0.25 * UP, lag_ratio=0.25, run_time=2)
        )
...
class HoldUpDiffractionEquationInAnticipation(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOut,
                VGroup(equation, arrow, name),
                shift=UR,
                lag_ratio=0.2,
                run_time=2,
            ),
            self.change_students(
                "pondering", "pondering", "thinking", look_at=morty.eyes
            ),
        )
        self.wait(3)
...
class HoldUpDiffractionEquation(TeacherStudentsScene):
    def construct(self):
        ...
        equation = Tex(
            R"d \cdot \sin(\theta) = \lambda", t2c={R"\lambda": TEAL}, font_size=60
        )
        equation.move_to(self.hold_up_spot, DOWN)
...
class AskAboutCenterBeam(InteractiveScene):
    def construct(self):
        ...
        randy = Randolph()
        randy.to_edge(DOWN)
        self.play(randy.thinks("Why is there a\ncentral beam?", mode="confused"))
        self.play(Blink(randy))
...
class DistanceApproximation(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(circle), GrowArrow(arrow), FadeIn(words))
        self.wait()
...
class DiffractionGratingGreenLaserExampleNumbers(InteractiveScene):
    def construct(self):
        ...
        lines = VGroup(
            ...
            Tex(
                R"\theta = \sin^{-1}(\lambda / d) = \sin^{-1}\left(5 \times 10^{-7} / 2 \times 10^{-6} \right) \approx 14.5^\circ"
            ),
        )
...
class AnnotateZerothOrderBeam2(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(names[0]), GrowArrow(arrow0), run_time=1)
        self.wait()
        self.remove(arrow0)
        self.play(
            TransformMatchingStrings(
                names[0], names[1], key_map={"Zeroth": "First", "m": "ms"}
            ),
...
class DejaVu(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            stds[1].change("raise_left_hand", look_at=diff_eq), FadeIn(diff_eq, UP)
        )
...
class WhatAHologramReconstructs(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (FadeIn(item[:2]) for item in items),
                lag_ratio=0.05,
            )
        )
        self.wait()
        for n, item in enumerate(items):
            self.play(
                items[:n].animate.set_opacity(0.25), FadeIn(item[2:], lag_ratio=0.1)
            )
            self.wait()
...
class BinaryVsSinusoidalDiffraction(InteractiveScene):
    def construct(self):
        ...
        top_axes, low_axes = two_axes = VGroup(
            Axes((0, 4, 0.5), (0, 1, 0.5), width=6, height=1.5) for _ in range(2)
        )
        ...
        top_graph = top_axes.get_graph(
            self.func1, x_range=(0, 3.99), discontinuities=discontinuities
        )
        ...
        for graph, grating in [(top_graph, top_grating), (low_graph, low_grating)]:
            ...
            self.play(
                ...
                VShowPassingFlash(
                    graph.copy().set_stroke(width=5), time_width=3, rate_func=linear
                ),
            ...
            self.play(
                grating.animate.next_to(grating, RIGHT, MED_LARGE_BUFF), run_time=2
            )
            self.wait()
        ...
        self.play(
            LaggedStart(
                FadeIn(high_beam_labels), Write(check), Write(ex), lag_ratio=0.5
            )
        )
        self.wait()
...
class AskWhyForSinusoidalGratingFact(TeacherStudentsScene):
    def construct(self):
        ...
        implication = VGroup(
            Tex(R"\Leftarrow").scale(2), Text("More formal\nholography explanation")
        )
...
class NeedsMoreRigor(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(GrowArrow(arrow), Write(words))
        self.wait(4)
...
class CompareFilmTypes(InteractiveScene):
    def construct(self):
        ...
        for text, res, arrow in zip(categories, resolutions, arrows):
            ...
            self.wait()
            self.play(Restore(text), GrowArrow(arrow), FadeIn(res, RIGHT))
            self.wait()
...
class TheresADeeperIssue(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "erm", "guilty", "hesitant", look_at=self.teacher.eyes
            ),
        )
        self.wait(2)
        self.play(
            self.change_students(
                "pondering", "pondering", "pondering", look_at=self.screen
            )
        )
        self.wait(2)
...
class WhatThisExplanationLacks(InteractiveScene):
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, points, shift=UP, lag_ratio=0.5))
        self.wait()
...
class SimplicityToGenerality(InteractiveScene):
    def construct(self):
        ...
        triangles = VGroup(
            Polygon(*tri) for tri in zip(verts[0::3], verts[1::3], verts[2::3])
        )
        ...
        self.play(
            ...
            frame.animate.reorient(
                58, 66, 0, (-0.27, -0.05, -0.23), 4.29
            ).set_anim_args(run_time=4),
        )
        self.play(
            frame.animate.reorient(50, 79, 0, (-0.27, -0.05, -0.23), 4.29), run_time=3
        )
...
class ModeledAs2D(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeIn(words, 0.25 * UP), DrawBorderThenFill(arrow, run_time=2))
        self.wait()
...
class ThinkingAboutRediscovery(InteractiveScene):
    def construct(self):
        ...
        self.play(
            randy.change("pondering"), Write(bubble, stroke_color=WHITE, lag_ratio=0.2)
        )
        self.play(Blink(randy))
        self.wait(2)
...
class IntroduceFormalSection(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(TransformMatchingShapes(VGroup(a_label, b_label).copy(), z_label))
...
class GaborQuote(InteractiveScene):
    def construct(self):
        ...
        self.play(
            quote["finding something else"].animate.set_color(TEAL), lag_ratio=0.1
        )
        self.wait()
...
class PrepareComplexAlgebra(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(TransformMatchingStrings(lines[0].copy(), lines[1]), run_time=2)
        self.wait()
        self.play(TransformMatchingStrings(lines[1].copy(), lines[2]), run_time=2)
        self.wait()
        self.play(
            ShowCreation(rect),
...
class ComplexAlgebra(InteractiveScene):
    def construct(self):
        ...
        lines = VGroup(
            ...
            Tex(
                R"R \cdot R \cdot R^* + R \cdot R^* \cdot O  + R \cdot R \cdot O^*+ R \cdot O \cdot O^*"
            ),
        ...
        self.add(lines[0])
        self.play(Write(label), GrowArrow(arrow))
        self.wait()
        self.play(
            LaggedStart(
                TransformFromCopy(lines[0][:-1], lines[1][R"R \cdot (R + O)"][0]),
                TransformFromCopy(
                    lines[0]["|R + O|"][0].copy(), lines[1]["(R^* + O^*)"][0]
                ),
                lag_ratio=0.1,
                run_time=2,
            )
        ...
        arrows = VGroup(Arrow(real_label, rect.get_bottom()) for rect in pair_rects)
        arrows.set_color(RED)

        self.play(ShowCreation(pair_rects, lag_ratio=0.1))
        self.play(FadeIn(real_label, lag_ratio=0.1), LaggedStartMap(GrowArrow, arrows))
        self.wait()
        self.play(
            FadeOut(real_label),
            FadeOut(arrows),
        ...
        self.wait()
        self.play(GrowFromCenter(real_brace1), FadeIn(real_label, shift=0.25 * DOWN))
        self.wait()
        self.play(
            box1_lines.animate.set_stroke(WHITE, 1, 0.5),
            VGroup(l2_rects[0], l2_rects[3], l3_rects[0]).animate.set_stroke(
                GREY, 1, 0.5
            ),
