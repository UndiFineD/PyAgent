...
class HighLevelNetworkFlow(InteractiveScene):
    ...
    def show_initial_text_embedding(self, word_scale_factor=0.6, bump_first=False):
        ...
        self.play(
            ...
            LaggedStart(
                *(token.animate.match_color(rect) for token, rect in zip(words, rects))
            ),
            FadeOut(arrow),
        )
        self.wait()
        ...
        self.play(
            ...
            MoveToTarget(blocks),
            q_group.animate.scale(word_scale_factor).next_to(
                blocks.target, RIGHT, aligned_edge=UP
            ),
    ...
    def progress_through_attention_block(
        ...
        self.play(
            ...
            LaggedStart(
                ...
                (
                    self.blocks[-1].title if len(self.blocks) > 0 else VGroup()
        ...
        self.play_simple_attention_animation(
            ...
            added_anims=[
                self.frame.animate.reorient(0, -15, 0, (-0.07, 0.45, 2.03), 9.25)
            ],
        )
            ...
            LaggedStart(
                *(
                    AnimationGroup(
                        ...
                        LaggedStartMap(
                            ShowCreationThenFadeOut, arc_group, lag_ratio=0.15
                        ),
                    )
    ...
    def progress_through_mlp_block(
        ...
        for embedding in layer.embeddings:
            ...
            l2_points = np.array(
                [[x0, y, block_z] for y in np.linspace(y_min, y_max, n_neurons)]
            )
        ...
        if show_one_by_one:
            networks = Group(
                *(
                    Group(cluster, lines.copy())
                    for cluster, lines in zip(neuron_clusters, connections)
                )
            )
...
class SimplifiedFlow(HighLevelNetworkFlow):
    ...
    def show_simple_flow(self, x_range, orientation=None):
        ...
        self.frame.add_updater(
            lambda f: f.set_height(
                curr_height + self.frame_height_growth_rate * (self.time - curr_time)
            )
        )
        for x in x_range:
...
class MentionContextSizeAndUnembedding(SimplifiedFlow):
    ...
    def show_context_size(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOut, VGroup(brace, label, rhs, dim_brace, dim_label), shift=DOWN
            ),
        )
    ...
    def show_unembedding_matrix(self, vector_index=-1):
        ...
        for rect in rects:
            question_arrows.add(
                Arrow(
                    question.get_top(), rect.get_bottom(), stroke_color=rect.get_color()
                )
            )
    ...
    def construct(self):
        ...
        for group in word_groups:
            for word in group:
                ...
                size = sum(
                    len(m.get_points()) for m in word.family_members_with_points()
                )
        ...
        mole_vect_rects = VGroup(
            SurroundingRectangle(embs[index]) for index in mole_indices
        )
        ...
        fade_groups = [
            fg for n, fg in enumerate(full_groups) if n != self.highlighted_group_index
        ]
