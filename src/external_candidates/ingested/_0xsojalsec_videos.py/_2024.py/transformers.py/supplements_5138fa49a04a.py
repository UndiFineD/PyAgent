# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\transformers\supplements.py
...
class GPTInitials(InteractiveScene):
    def construct(self):
        ...
        for i in [1, 2]:
            ...
            self.play(
                line.animate.become(Underline(words[i]))
                .set_stroke(YELLOW)
                .set_anim_args(run_time=0.75),
                FlashAround(words[i]),
                MoveToTarget(words),
            )
            self.wait()
...
class DifferentUsesOfModel(InteractiveScene):
    def construct(self):
        ...
        path_arc = -0.65 * PI
        left_arrows = VGroup(
            Arrow(s0[word].get_top(), s0["model"].get_top(), path_arc=path_arc)
            for word in ["machine", "learning"]
        )
        right_arrow = Arrow(
            s1["fashion"].get_top(), s1["model"].get_top(), path_arc=path_arc
        )
        ...
        self.play(
            LaggedStartMap(
                FadeIn, vectors, shift=0.25 * DOWN, lag_ratio=0.25, run_time=3
            )
        )
...
class LongListOFQuestions(InteractiveScene):
    def construct(self):
        ...
        connections = VGroup(
            VGroup(
                Line(
                    elem.get_right(), neuron.get_center(), buff=neuron.get_width() / 2
                ).set_stroke(
        ...
        self.play(
            LaggedStart(
                (ShowCreation(line_group, lag_ratio=0) for line_group in connections),
                lag_ratio=lag_ratio,
            ),
...
class GamePlan(InteractiveScene):
    ...
    def construct(self):
        ...
        prev_thumbnails = Group(
            ImageMobject(f"nn{k}_thumbnail.png") for k in range(1, 5)
        )
        ...
        new_thumbnails = Group(
            ImageMobject(os.path.join(tn_dir, f"Chapter{n}")) for n in range(5, 8)
        )
        ...
        tr_rect = SurroundingRectangle(
            Group(new_thumbnails, chapter_titles[4:]), buff=0.25
        )
        ...
        self.play(
            ...
            *(FadeTransform(vid, tn) for vid, tn in zip(tr_vids, new_thumbnails)),
        )
        self.play(
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "confused", "pondering", "pondering", look_at=self.screen
            ),
        )
        self.wait(2)
...
class SeaOfNumbersUnderlay(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.play(
            self.change_students("erm", "pondering", "confused", look_at=self.screen)
        )
        self.wait(2)
        ...
        self.play(
            self.change_students(
                "hesitant", "pondering", "hesitant", look_at=5 * RIGHT + 2 * UP
            )
        )
        self.wait(3)
        self.play(
        ...
        self.play(
            ...
            self.change_students(
                "tease", "thinking", "pondering", look_at=5 * RIGHT + 3 * UP
            ),
        )
        self.wait(8)
...
class ConfusionAtScreen(TeacherStudentsScene):
    def construct(self):
        self.play(
            ...
            self.change_students(
                "maybe", "confused", "concentrating", look_at=self.screen
            ),
        )
        self.wait(2)
        self.play(
...
class HighlightAttentionTitle(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            title.animate.set_fill(WHITE)
            .scale(2)
            .next_to(morty, UP, MED_LARGE_BUFF)
            .to_edge(RIGHT),
            LaggedStartMap(VFadeIn, self.pi_creatures),
            morty.change("raise_right_hand"),
            self.change_students(
                "pondering", "well", "thinking", look_at=self.hold_up_spot
            ),
        )
        self.wait()
...
class ThinkOfMoreExamples(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.change_students(
                "maybe", "confused", "dejected", look_at=morty.eyes, lag_ratio=0
            ),
            morty.change("well"),
        )
        self.wait(2)
...
class ThisIsMadeUp(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.change_students("well", "sassy", "guilty", look_at=self.teacher.eyes)
        )
        self.wait(4)
        ...
        self.play(
            LaggedStart(
                ...
                self.students[2].says(
                    "Softmax!",
                    mode="surprised",
                    bubble_config=dict(buff=0.5, direction=LEFT),
                ),
                self.teacher.change("well"),
            )
        )
...
class SeekingMatchedParameters(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.change_students(
                "pondering", "confused", "hesitant", look_at=self.screen
            )
        )
        self.wait(4)
...
class RefreshersNeverHurt(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "pondering", "hesitant", "erm", look_at=self.teacher.eyes
            ),
        )
        self.wait(3)
...
class EmbeddingLabel(InteractiveScene):
    def construct(self):
        # Background
        bg = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2024/transformers/images/EmbeddingStill.jpg"
        )
...
class ContrastBetweenSimpleComputationDifficultInterpretation(InteractiveScene):
    def construct(self):
        ...
        items = VGroup(
            ...
            VGroup(
                Text("Interpretation"),
                SVGMobject("warning").set_color(RED).set_height(0.5),
            ),
        )
...
class MovingToSecondToken(InteractiveScene):
    def construct(self):
        ...
        arrows = VGroup(
            Arrow(rect, emb, buff=0.1) for rect, emb in zip(rects, embeddings)
        )
...
class AskWhy(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.teacher.change("guilty"),
            self.students[2].says(
                Text("Um, Why?", font_size=72), mode="maybe", look_at=self.screen
            ),
...
class AndGate(InteractiveScene):
    def construct(self):
        ...
        self.play(
            Write(gate), FadeIn(name, lag_ratio=0.1, time_span=(0, 2)), run_time=3
        )
        self.wait()
...
class AskIfThisIsReal(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.change_students(
                "speaking", "pondering", "skeptical", look_at=self.screen
            )
        )
        self.wait(2)
        self.play(
...
class SingleNeuronVsMultiple(InteractiveScene):
    def construct(self):
        ...
        radius = 0.1
        layers = VGroup(
            Dot(radius=radius).get_grid(n, 1, buff=radius) for n in [8, 16, 8]
        )
        ...
        connections = VGroup(
            get_network_connections(layers[i], layers[i + 1]) for i in (0, 1)
        )
...
class WriteSuperposition(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                VShowPassingFlash, outline, time_width=2, run_time=5, lag_ratio=0.01
            ),
        )
        self.wait()
...
class ContrastGPTDimensionSizes(InteractiveScene):
    def construct(self):
        ...
        arrows = VGroup(
            Arrow(n1, n2, buff=0.25) for n1, n2 in zip(model_names, model_names[1:])
        )
        dim_counts = VGroup(
            Text(f"Model dim: {dim}", font_size=36) for dim in ["768", "12,288", "???"]
        )
...
class ReferenceSAP(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            morty.says(
                TexText(R"There's nice\\research using\\Sparse Autoencoders"),
                mode="hooray",
            )
        )
        self.play(
...
class ListOfFacts(InteractiveScene):
    def construct(self):
        ...
        lines = VGroup(
            ...
            .insert_n_curves(20)
            .set_stroke(width=(0, 5, 5, 0))
            for fact, a in zip(facts, np.linspace(0, 1, len(facts)))
        )
        self.add(lines)
