...
class LotsOfTextIntoTheMachine(PredictTheNextWord):
    ...
    def construct(self):
        ...
        dials = (
            Dial()
            .get_grid(8, 12)
            .set_width(0.9 * blocks[-1].get_width())
            .move_to(blocks[-1])
        )
        ...
        text_mobs = VGroup(
            get_paragraph(snippet.split(" "), line_len=25) for snippet in snippets
        )
...
class AdjustingTheMachine(InteractiveScene):
    def construct(self):
        ...
        frame.add_updater(
            lambda f: f.set_theta(-30 * DEGREES * math.cos(0.1 * self.time))
        )
        self.add(frame)
        for x in range(6):
...
class FirthQuote(InteractiveScene):
    def construct(self):
        # Show Quote
        quote = TexText(
            R"``You shall know a word\\by the company it keeps!''", font_size=60
        )
        image = ImageMobject(
            "JohnRFirth"
        ...
        self.play(
            LaggedStart(
                ...
                phrases.animate.set_opacity(1)
                .arrange(DOWN, buff=3.5, aligned_edge=LEFT)
                .move_to(0.5 * UP),
            )
        )
        self.wait()
...
class PartialScript(SimpleAutogregression):
    ...
    def construct(self):
        ...
        curled_script_templates = Group(
            ParametricSurface(lambda u, v: (*curve.pfp(v)[:2], u), **surface_kw)
            for curve in curves
        )
        ...
        curled_script1_torn = TexturedSurface(
            curled_script_templates[1], "HumanAIScriptTorn"
        )
        ...
        for n in range(6):
            ...
            self.play(
                mover.animate.set_height(1.8)
                .next_to(machine, UP, SMALL_BUFF)
                .set_anim_args(path_arc=-30 * DEGREES),
            )
            self.animate_text_input(
                mover, machine, position_text_over_machine=False, lag_ratio=1e-3
            )
...
class ShowMachineWithDials(PredictTheNextWord):
    ...
    def construct(self):
        ...
        new_dials = VGroup(
            all_dials.target.copy().shift(
                3 * 2 * x * (flat_dials.get_center() - last_dials.get_center())
            )
            for x in range(1, 9)
        )
...
class ThousandsOfWords(InteractiveScene):
    def construct(self):
        # Find passage
        file = Path(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2024/transformers/data/tale_of_two_cities.txt"
        )
...
class EnormousAmountOfTrainingText(PremiseOfMLWithText):
    def construct(self):
        ...
        frame.clear_updaters()
        frame.add_updater(
            lambda m: m.set_height(FRAME_HEIGHT * np.exp(0.2 * self.time))
        )
...
class BadChatBot(InteractiveScene):
    ...
    def get_bot(self):
        ...
        eyes = VGroup(
            Dot().replace(VMobject().set_points(subpath)) for subpath in subpaths[2:]
        )
        bot.eyes = eyes
        bot.add(eyes)
    ...
    def blink(self, bot):
        self.play(
            bot.eyes.animate.stretch(0, 1).set_anim_args(
...
class ManyComputationsPerUnitTimeV2(InteractiveScene):
    def construct(self):
        ...
        sec_brace = Brace(
            first_ticks, DOWN, buff=0, tex_string=R"\underbrace{\qquad\qquad}"
        )
        sec_label = Text("Second", font_size=30).next_to(sec_brace, DOWN, SMALL_BUFF)

        self.play(
            ShowCreation(minute_line, lag_ratio=0.01),
            box.animate.match_width(first_ticks)
            .move_to(first_ticks.get_center(), DOWN)
            .set_stroke(width=1),
        ...
        for new_box in new_boxes:
            ...
            new_box.move_to(box)
        computations = VGroup(
            self.get_computations(new_box, n_iterations=1) for new_box in new_boxes
        )
        ...
        self.play(
            LaggedStart(
                *(
                    AnimationGroup(*(ShowCreation(sl) for sl in pair))
                    for pair in scale_lines
                ),
                lag_ratio=lag_ratio,
            ),
            LaggedStart(
            ...
            .set_anim_args(
                rate_func=lambda t: interpolate(
                    smooth(t), linear(t), there_and_back_with_pause(t, pause_ratio=0.8)
                )
            ),
            run_time=30,
        )
    ...
    def fade_in_bigger_interval(
        self, new_interval, prev_interval, fader, scale_factor, added_anims=[]
    ):
        ...
        self.play(
            ...
            prev_interval.animate.scale(1.0 / scale_factor, about_point=pivot).set_fill(
                border_width=0
            ),
    ...
    def get_computations(
        self, box, n_lines=10, n_iterations=3, n_digits=4, cycle_time=0.5
    ):
        # Try adding lines
        lines = VGroup()
    ...
    def old(self):
        ...
        for new_int, prev_int, scale_factor in zip(
            number_lines[1:], number_lines[0:], scale_factors
        ):
        ...
        self.fade_in_bigger_interval(
            ...
            1000,
            added_anims=[
                self.frame.animate.reorient(0, 0, 0, (-3.51, -5.18, 0.0), 12.93)
            ],
        )
        ...
        side_brace, label100M = self.get_timeline_full_label(
            y1M_line, "100,000,000 Years"
        )
        ...
        self.play(
            LaggedStart(
                *(
                    ...
                    for letter in VGroup(
                        chm_phrases, mv_phrases
    ...
    def construct(self):
        ...
        thumbnails = Group(
            Group(
                ...
                ImageMobject(
                    f"https://img.youtube.com/vi/{slug}/maxresdefault.jpg", height=1
                ),
            )
            for slug in [
                "aircAruvnKk",
        ...
        self.play(
            LaggedStartMap(
                FadeIn, thumbnails, shift=0.3 * UP, lag_ratio=0.35, run_time=4
            )
        )
        self.wait()
        ...
        self.play(
            ...
            thumbnails.animate.arrange_in_grid(n_cols=2, buff=0.25)
            .set_height(6)
            .set_x(left_x)
            .to_edge(DOWN),
