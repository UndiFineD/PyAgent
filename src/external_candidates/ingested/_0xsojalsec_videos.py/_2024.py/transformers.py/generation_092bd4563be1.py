# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\transformers\generation.py
import openai
import tiktoken
import torch
...
class SimpleAutogregression(InteractiveScene):
    ...
    def animate_text_input(
        self,
        text_mob,
        machine,
        position_text_over_machine=True,
        added_anims=[],
        lag_ratio=0.02,
    ):
        blocks = machine[0]
    ...
    def animate_random_sample(self, bar_groups):
        ...
        self.play(
            UpdateFromAlphaFunc(
                highlight_rect, lambda rect, a: highlight_randomly(rect, dist, a)
            ),
    ...
    def animate_word_addition(
        self, bar_groups, text_mob, next_word_line, force_unskip=False
    ):
        ...
        new_text_mob[: len(self.seed_text.replace(" ", ""))].set_color(
            self.seed_text_color
        )
    ...
    def new_selection_cycle(
        self, text_mob, next_word_line, machine, quick=False, skip_anims=False
    ):
        if skip_anims:
            self.skip_animations = True
        ...
        new_text_mob = self.animate_word_addition(
            bar_groups, text_mob, next_word_line, force_unskip=skip_anims
        )
        return new_text_mob
...
class AnnotateNextWord(SimpleAutogregression):
    def construct(self):
        ...
        self.play(ShowCreation(highlight), GrowArrow(arrow))
        self.wait()
        self.play(
...
class ChatBotPrompt(SimpleAutogregression):
    ...
    def construct(self):
        ...
        all_strs = list(
            map(clean_text, [self.system_prompt, self.user_prompt, self.ai_seed])
        )

        system_prompt, user_prompt, ai_seed = all_text = VGroup(
            get_paragraph(
                s.split(" "), font_size=self.font_size, line_len=self.line_len
            )
            for s in all_strs
        )
...
class ChatBotPrompt2(ChatBotPrompt):
    user_prompt = (
        "User: Can you explain what temperature is, in the context of softmax?"
    )


class ChatBotPrompt3(ChatBotPrompt):
    user_prompt = (
        "User: Can you give me some ideas for what to do while visiting Munich?"
    )
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                ...
                LaggedStart(
                    (
                        block.animate.set_color(TEAL).set_anim_args(
...
class TextToImage(VoiceToTextExample):
    ...
    def generate_output(self):
        ...
        response = openai.Image.create_variation(
            image=open(
                "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/images/raster/PiCreatureDalle3_17.png",
                "rb",
            ),
            n=1,
            size="1024x1024",
...
class ManyParallelPredictions(SimpleAutogregression):
    ...
    def construct(self):
        ...
        last_layer = VGroup(NumericEmbedding(length=10) for n in range(12))
        ...
        substrings = [" ".join(words[: n + 1]) for n in range(len(words))]

        predictions = VGroup(
            self.get_prediction_group(substring) for substring in substrings
        )
        ...
        for rect, arrow, prediction in zip(rects, arrows, predictions):
            ...
            self.play(
                ...
                TransformMatchingStrings(
                    last_group[2][0].copy(), prediction[0], run_time=1
                ),
    ...
    def get_prediction_group(self, text):
        ...
        dist = self.get_distribution(words, probs, width_100p=2.0)
