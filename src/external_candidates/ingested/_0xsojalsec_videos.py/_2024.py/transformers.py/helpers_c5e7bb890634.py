...
def load_image_net_data(dataset_name="image_net_1k"):
    ...
    return [
        (Path(image_dir, f"{index}.jpeg"), label) for index, label in enumerate(labels)
    ]


def show_matrix_vector_product(
    scene, matrix, vector, buff=0.25, x_max=999, fix_in_frame=False
):
    # Show product
    eq = Tex("=")
    ...
    for n, row, entry in zip(it.count(), matrix.get_rows(), rhs[:-2]):
        if matrix.ellipses_row is not None and n == (matrix.ellipses_row % n_rows):
            ...
            last_rects = matrix_row_vector_product(
                scene, row, vector, entry, last_rects, fix_in_frame=fix_in_frame
            )
...
def matrix_row_vector_product(scene, row, vector, entry, to_fade, fix_in_frame=False):
    def get_rect(elem):
        return SurroundingRectangle(
            elem, buff=0.1, is_fixed_in_frame=fix_in_frame
        ).set_stroke(YELLOW, 2)
...
def show_symbolic_matrix_vector_product(
    scene, matrix, vector, rhs, run_time_per_row=0.75
):
    last_rects = VGroup()
...
def data_flying_animation(
    ...
    return UpdateFromAlphaFunc(
        word,
        lambda m, a: m.move_to(interpolate(point, point + vect, a)).set_opacity(
            there_and_back(a) * max_opacity
        ),
    )
        ...
        LaggedStart(
            (
                data_flying_animation(
                    p, vect=shift_vect, fix_in_frame=fix_in_frame, font_size=font_size
                )
                for p in points
            ),
...
def create_pixels(image_mob, pixel_width=0.1):
    ...
    pixels = VGroup(
        square.copy()
        .move_to(point, UL)
        .set_color(Color(rgb=image_mob.point_to_rgb(point)))
        for point in points
    )
    return pixels
...
class Dial(VGroup):
    def __init__(
        ...
        **kwargs,
        ...
        self.ticks = VGroup(
            *(Line((1.0 - relative_tick_size) * point, point) for point in tick_points)
        )
    ...
    def animate_set_value(self, value, **kwargs):
        ...
        angle1, angle2 = [
            (angle_of_vector(vect) + TAU / 4) % TAU - TAU / 4 for vect in vects
        ]
