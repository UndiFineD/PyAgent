# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\transformers\mlp.py
import torch
...
class LastTwoChapters(InteractiveScene):
        ...
        self.play(
            ...
            LaggedStart(
                (
                    Succession(
                        FadeIn(fact),
                        fact.animate.scale(0.5).set_opacity(0).move_to(mlp_blocks),
                    )
                    for fact in sport_facts
                ),
                lag_ratio=0.15,
    ...
    def get_thumbnails(self):
        ...
        thumbnails = Group(
            Group(SurroundingRectangle(image, buff=0).set_stroke(WHITE, 3), image)
...
class AltLastTwoChapters(LastTwoChapters):
    def construct(self):
        ...
        for thumbnail in thumbnails:
            ...
            thumbnail.target[-1].scale(1.0 / 1.5).next_to(
                thumbnail.target[0], DOWN, SMALL_BUFF
            )
...
class MLPStepsPreview(InteractiveScene):
    def construct(self):
        ...
        self.play(
            Restore(top_frame),
            Restore(titles[0]),
            structure.animate.set_width(
                0.9 * top_frame.saved_state.get_width()
            ).move_to(top_frame.saved_state),
...
class BasicMLPWalkThrough(InteractiveScene):
    ...
    def construct(self):
        ...
        full_block.target[3].rotate(90 * DEGREES, DOWN).next_to(
            full_block.target[0], UP, buff=0.5
        )
        ...
        simple_phase_copies = VGroup(
            simple_phases.copy().match_z(emb) for emb in embedding_array.embeddings
        )
        ...
        self.play(
            frame.animate.reorient(9, -15, 0, (0.55, -2.21, 0.18), 7.05), run_time=4
        )
        self.play(
            frame.animate.reorient(-24, -16, 0, (0.18, -2.13, 0.09), 7.63), run_time=12
        )
        block_title.next_to(block, UP)
        self.play(
        ...
        self.play(
            Write(mj_lines, stroke_width=2, stroke_color=YELLOW_B, lag_ratio=1e-2)
        )
        self.wait()
        ...
        row_labels = VGroup(
            VGroup(R_label, h_lines.copy().match_y(R_label)) for R_label in R_labels
        )
        ...
        for row_label, dot_prod in zip(row_labels, dot_prods):
            ...
            self.play(
                ...
                VShowPassingFlash(
                    Line(row_label.get_left(), row_label.get_right())
                    .set_stroke(YELLOW, 5)
                    .insert_n_curves(100),
                    time_width=1.5,
                ),
                VShowPassingFlash(
                    Line(E_col.get_top(), E_col.get_bottom())
                    .set_stroke(YELLOW, 5)
                    .insert_n_curves(100),
                    time_width=1.5,
                ),
        ...
        mde_rhs = VGroup(
            Tex(
                R"\approx 1 \quad \text{If } \vec{\textbf{E}} \text{ encodes ``First Name Michael''}"
            ),
        ...
        dist_rhs = VGroup(
            ...
            Tex("="),
            Tex(
                R"\vec{\textbf{M}} \cdot \vec{\textbf{E}} + \vec{\textbf{J}} \cdot \vec{\textbf{E}}"
            ),
        )
        dist_rhs.scale(0.75)
        ...
        two_condition = TexText(
            R"$\approx 2$ \; if $\vec{\textbf{E}}$ encodes ``Michael Jordan''"
        )
        ...
        self.play(
            TransformMatchingStrings(
                dist_rhs[0].copy(), dist_rhs[2], lag_ratio=0.01, path_arc=-45 * DEGREES
            ),
            Write(dist_rhs[1]),
        )
        self.wait()
        ...
        data_modifying_matrix(
            self, bias, word_shape=(5, 1), alpha_maxes=(0.4, 0.9), fix_in_frame=True
        )
        bias.get_entries()[0].set_value(-1).set_color(RED)

        # Pull up the MJ example again
        fe_rect = SurroundingRectangle(
            rhs.get_entries()[0], buff=0.1
        )  # fe = First entry
        ...
        self.play(
            FadeOut(bias_rect), GrowFromCenter(under_brace), FadeIn(conditions[0], DOWN)
        )
        self.wait()
        ...
        self.play(
            ...
            LaggedStart(
                *map(FadeOut, [fe_rect, fe_eq, fe_expr, under_brace, *conditions])
            ),
        )
        ...
        row_rects = VGroup(
            SurroundingRectangle(row, buff=0.1) for row in up_proj.get_rows()
        )
        ...
        VGroup(
            neg_left_rects, zero_right_rects, pos_left_rects, pos_right_rects
        ).set_stroke(width=2)
        ...
        self.play(
            FlashAround(equals, run_time=2, time_width=1.5), ineq.animate.become(equals)
        )
        self.wait()
        self.play(
        ...
        gelu_graph_label.next_to(
            relu_graph_label, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
        ...
        dots = VGroup(
            Dot(radius=0.15)
            .move_to(entry)
            .set_fill(WHITE, opacity=clip(entry.get_value(), 0, 1))
            for entry in phase1_copy[2].get_entries()
        )
        ...
        self.play(
            LaggedStart(
                *map(FadeOut, [up_emb, *lines, blocking_rect, *dots]), lag_ratio=0.01
            )
        )
        ...
        bias_as_col = self.get_col_matrix(
            bias, 1, dots_index=None, sym="B", top_index="", width_multiple=0.7
        )
        ...
        self.play(
            frame.animate.reorient(42, -23, 0, (0.55, -2.21, 0.18), 7.05), run_time=8
        )
        ...
        neuron_points = np.array(
            [entry.get_center() for vect in n_vects[1:] for entry in vect.get_entries()]
        )
        ...
        self.play(
            frame.animate.reorient(-11, -5, 0, (0.55, -2.21, 0.18), 7.05).set_anim_args(
                run_time=4
            ),
            FadeOut(n_vects),
        ...
        self.play(
            frame.animate.reorient(13, -7, 0, (0.55, -2.21, 0.18), 7.05), run_time=4
        )
        self.wait()

    def get_sum_circuit(
        self, in_vect, diff_vect, v_buff=0.15, h_buff=0.5, y_diff=0.65, color=YELLOW
    ):
        ...
        for e1, e2, e3 in zip(
            in_vect.get_entries(), diff_vect.get_entries(), output.get_entries()
        ):
    ...
    def get_col_matrix(
        ...
        col_labels = VGroup(
            VGroup(C_label, v_lines.copy().match_x(C_label)) for C_label in C_labels
        )
    ...
    def construct(self):
        ...
        plane = NumberPlane(
            ...
            background_line_style=dict(
                stroke_color=BLUE_D, stroke_width=1, stroke_opacity=0.75
            ),
            faded_line_ratio=1,
            unit_size=unit_size,
        )
        ...
        emb = Vector()
        emb.add_updater(
            lambda m: m.put_start_and_end_on(ORIGIN, emb_point.get_center())
        )
        ...
        shadow.add_updater(
            lambda m: m.put_start_and_end_on(ORIGIN, get_line_point())
        )  # This is a long line
        ...
        dashed_line = always_redraw(
            lambda: DashedLine(emb_point.get_center(), get_line_point()).set_stroke(
                PINK, 2
            )
        )
        ...
        dp_decimal.f_always.set_value(
            lambda: np.dot(normalized_sum, emb_point.get_center()) * 2.0 / 3.535534
        )
        ...
        self.play(
            ...
            Rotate(
                emb_point,
                TAU,
                about_point=emb_point.get_center() + 0.15 * DL,
                run_time=4,
            ),
        )
        self.wait()
        ...
        self.play(
            ...
            emb_point.animate.move_to(j_vect.get_end()).set_anim_args(
                path_arc=30 * DEGREES
            ),
            run_time=2,
        )
...
class Superposition(InteractiveScene):
    def construct(self):
        ...
        # Preview some ideas
        ideas = VGroup(
            Text("Latin"), Text("Microphone"), Text("Basketball"), Text("The 1920s")
        )
        ideas.scale(0.75)
        vectors = VGroup()
        idea_vects = VGroup()
        ...
        conditions = [R"$90^\circ$ apart", R"between $89^\circ$ and $91^\circ$ apart"]
        task1, task2 = tasks = VGroup(
            TexText(
                Rf"Choose multiple vectors,\\ each pair {phrase}",
                font_size=42,
                alignment="",
            )
        ...
        maximum1, maximum2 = maxima = VGroup(
            TexText(Rf"Maximum \# of vectors: {answer}", font_size=42)
        ...
        elbows = VGroup(
            Elbow(width=0.1)
            .rotate(angle, axis, about_point=ORIGIN)
            .set_stroke(WHITE, 2)
        ...
        self.play(
            FadeIn(task1), LaggedStartMap(GrowArrow, perp_vectors[:2], lag_ratio=0.5)
        )
        ...
        self.play(
            Rotate(
                perp_group,
                -50 * DEGREES,
                axis=perp_vectors[1].get_vector(),
                run_time=15,
            ),
        ...
        self.play(
            Rotate(
                perp_group, 50 * DEGREES, axis=perp_vectors[1].get_vector(), run_time=10
            )
        )
        ...
        for vect in alt_vects:
            vect.rotate(
                5 * DEGREES, axis=normalize(np.random.random(3)), about_point=origin
            )
        ...
        for v1, v2 in it.combinations(pv, 2):
            new_vects.add(
                Arrow(
                    ORIGIN,
                    v1.get_length() * normalize(v1.get_vector() + v2.get_vector()),
                    buff=0,
                ).shift(origin)
            )
...
class StackOfVectors(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (RandomizeMatrixEntries(row) for row in rows[:-2]),
                lag_ratio=0.05,
            )
        )
        self.wait()
...
class ShowAngleRange(InteractiveScene):
    def construct(self):
        ...
        vect_pair = always_redraw(
            lambda: get_vector_pair(
                angle_tracker.get_value(), length=3, colors=(RED, GREEN)
            )
        )
...
class MLPFeatures(InteractiveScene):
    def construct(self):
        ...
        layer1, layer2 = layers = VGroup(
            Dot(radius=radius).get_grid(n, 1, buff=radius / 2) for n in [8, 16]
        )
    ...
    def get_new_vector(self, coords, color=None, opacity=0.9):
        if color is None:
            color = random_bright_color(
                hue_range=(0.4, 0.6), luminance_range=(0.5, 0.9)
            )
...
class ShowBiasBakedIntoWeightMatrix(LastTwoChapters):
    def construct(self):
        ...
        att_block = self.get_block(
            att_icon, "Attention", "604M Parameters", color=YELLOW
        )
        mlp_block = self.get_block(mlp_icon, "MLP", "1.2B Parameters", color=BLUE)
        lnm_block = self.get_block(
            lnm_icon, "Layer Norm", "49K Parameters", color=GREY_B
        )
