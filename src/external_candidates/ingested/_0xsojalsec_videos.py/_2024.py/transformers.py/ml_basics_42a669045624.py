...
class MLWithinDeepL(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(
                    Succession(FadeIn(matrix, shift=mat_shift), MoveToTarget(matrix))
                    for matrix in matrices[:-1]
                ),
                lag_ratio=0.25,
                run_time=5,
    ...
    def get_titled_box(
        self, text, color, font_size=48, y_space=0.5, x_space=0.5, opacity=0.1
    ):
...
class PremiseOfML(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            FadeInFromPoint(
                out_data, machine.get_right(), lag_ratio=0.1, path_arc=60 * DEGREES
            ),
        ...
        down_group = Group(
            in_arrow, machine, param_label, out_arrow, out_data, out_word
        )
        self.play(
        ...
        self.play(
            ...
            out_data.animate.scale(0.75)
            .fade(0.5)
            .rotate(-PI / 2)
            .next_to(out_array, RIGHT, buff=0.25),
        )
        self.wait()
        ...
        self.play(
            ...
            LaggedStart(
                *(TransformFromCopy(column[n], data_parts[n]) for n in indices)
            ),
        ...
        self.play(
            ...
            LaggedStart(
                *(FadeTransform(top_dials[n].copy(), weight_parts[n]) for n in indices)
            ),
        ...
        for x in range(3):
            self.play(
                *(dial.animate_set_value(dial.get_random_value()) for dial in top_dials)
            )
        ...
        dial_lines.target = VGroup(
            *(
                Line(
                    weights_label.target, weight_parts[index].get_top(), buff=SMALL_BUFF
                )
        ...
        in_subset = VGroup(
            *(elem for row in in_array.get_rows()[:3] for elem in row[:3])
        )
        ...
        self.play(
            ...
            layer1.animate.rotate(70 * DEGREES, UP).next_to(
                in_arrow, RIGHT, buff=-0.25
            ),
        ...
        self.play(
            LaggedStart(
                ...
                weights_label.animate.set_height(0.5).next_to(
                    matrix, UP, buff=MED_SMALL_BUFF
                ),
        ...
        show_symbolic_matrix_vector_product(
            self, matrix, vector, rhs, run_time_per_row=1.5
        )
        ...
        self.play(
            ...
            frame.animate.set_height(FRAME_HEIGHT)
            .move_to(DOWN)
            .set_anim_args(time_span=(1, 2)),
            run_time=2,
        )
        self.wait()
        ...
        self.play(
            ...
            ShowIncreasingSubsets(
                small_mat_products, rate_func=linear, run_time=12, int_func=np.ceil
            ),
            ShowSubmobjectsOneByOne(
                mv_arrows, rate_func=linear, run_time=12, int_func=np.ceil
            ),
        )
        self.remove(mv_arrows)
...
class ShowGPT3Numbers(InteractiveScene):
    def construct(self):
        ...
        mat_text.shift(
            (weights_count.get_x(LEFT) - mat_text["27,938"].get_x(LEFT)) * RIGHT
        )

        mat_grid_shape = n, m = (3, 7)
        matrices = VGroup(
            WeightMatrix(shape=(5, 5)) for n in range(np.product(mat_grid_shape))
        )
        ...
        all_entries = VGroup(
            entry for mat in matrices for row in mat.get_rows() for entry in row
        )
        ...
        self.play(
            LaggedStart(
                ...
                title.animate.scale(title_scale_factor).next_to(
                    count_text.target, RIGHT, LARGE_BUFF
                ),
        ...
        self.play(
            FadeIn(
                per_head_exprs.sum_box, run_time=3, rate_func=there_and_back_with_pause
            )
        )
        self.wait()
        self.play(
        ...
        self.play(
            FadeIn(
                per_layer_exprs.sum_box, run_time=3, rate_func=there_and_back_with_pause
            )
        )
        self.wait()
        self.play(
        ...
        per_layer_exprs = VGroup(
            get_product_expression(
                category_names[i], const_lists[i][:2], suffix="per layer"
            )
        ...
        self.play(
            TransformMatchingStrings(
                per_layer_exprs[0][0].copy(), per_layer_exprs[1][0]
            ),
        ...
        all_rhss = VGroup(
            VGroup(expr[0]["="][0], expr[0].rhs) for expr in product_expressions
        )
        ...
        self.play(
            LaggedStart(
                ...
                weights_text.animate.arrange(DOWN)
                .scale(1.5)
                .next_to(all_rhss_rect, UP),
...
class DistinguishWeightsAndData(InteractiveScene):
    def construct(self):
        ...
        weights_title, data_title = titles = VGroup(
            Text(word, font_size=60) for word in ["Weights", "Data"]
        )
        ...
        vectors = VGroup(
            NumericEmbedding(length=8, ellipses_row=None) for n in range(8)
        )
        ...
        mat_braces = VGroup(
            brace for matrix in matrices for brace in matrix.get_brackets()
        )
        vec_nums = VGroup(elem for vector in vectors for elem in vector.get_entries())
        vec_braces = VGroup(
            brace for vector in vectors for brace in vector.get_brackets()
        )
...
class SoftmaxBreakdown(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(FadeTransform(pc.copy(), rhs) for pc in prob_copies), lag_ratio=0.07
            )
        )
        self.wait()
        ...
        gt0s = VGroup(
            Tex(R"> 0").next_to(exp_part, aligned_edge=DOWN) for exp_part in exp_parts
        )
        ...
        self.play(
            LaggedStartMap(FadeIn, gt0s, shift=0.5 * RIGHT, lag_ratio=0.25, run_time=1)
        )
        self.wait()
        ...
        lines = VGroup(
            *(
                Line(exp_part.get_right(), exp_sum.get_left(), buff=0.1)
        ...
        lines = VGroup(
            *(Line(exp_sum.get_left(), denom.get_center()) for denom in lil_denoms)
        )
        ...
        # Resize box
        sm_terms = VGroup(
            *(VGroup(exp_part, denom) for exp_part, denom in zip(exp_parts, lil_denoms))
        )
        ...
        VGroup(softmax_box.target, sm_terms.target).set_height(
            target_height + 0.5
        ).next_to(equals, LEFT)
...
class CostFunction(InteractiveScene):
    def construct(self):
        ...
        line = axes.get_line_from_axis_to_point(
            0, axes.i2gp(probs[index], graph), line_func=Line
        )
