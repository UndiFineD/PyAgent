# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\transformers\old_auto_regression.py
import openai
import tiktoken
import torch
...
class AthleteCompletion(SimpleAutogregression):
    ...
    def construct(self):
        ...
        fact_mobs = VGroup(
            get_paragraph(fact.split(" "), line_len=20) for fact in facts
        )
        ...
        self.play(
            ...
            Transform(
                full_input.copy(),
                full_input.copy().scale(0.5).set_opacity(0).move_to(machine.get_top()),
            ),
        )
        self.play(
        ...
        self.play(ShowCreation(out_arrow), FadeIn(bar_groups, lag_ratio=0.1))
        self.wait()
        self.play(ShowCreation(top_rect))

        # Reshow parameters
        self.play(FadeOut(llm_title), dials.animate.set_stroke(opacity=1))
        for _ in range(5):
            self.play(
                LaggedStart(
