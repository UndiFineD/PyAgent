# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\antp\main.py
import sympy
...
class TwinPrimScrolling(InteractiveScene):
    def construct(self):
        ...
        non_twin_primes = VGroup(
            *(pm for pm, prime in zip(prime_mobs, primes) if prime not in twin_primes)
        )
        ...
        def update_tp_groups(tp_groups):
            for tp_group in tp_groups:
                ...
                pre_alpha = inverse_interpolate(
                    highlight_point + 0.25, highlight_point - 0.25, x_coord
                )
...
class EuclidProof(InteractiveScene):
    def construct(self):
        ...
        finite_words.shift(
            RIGHT * (sequence_rect.get_x() - finite_words["All primes"].get_x())
        )

        last_arrow = Arrow(
            finite_words["prime"].get_corner(DL), last, path_arc=-PI / 3, buff=0.1
        )
        ...
        rect = SurroundingRectangle(
            Group(*(mob for mob in self.mobjects if isinstance(mob, StringMobject))),
            buff=0.5,
        )
...
class SieveWithMod(InteractiveScene):
    def construct(self):
        ...
        # Do the initial sift
        non_primes = VGroup(
            label for label in labels if not sympy.isprime(label.get_value())
        )
        ...
        self.play(
            non_primes.animate.set_fill(opacity=0.25), lag_ratio=0.025, run_time=5
        )
        self.wait()
        ...
        for _ in range(5):
            ...
            rows = VGroup(
                labeled_boxes[n : n + row_size] for n in range(0, len(grid), row_size)
            )
            ...
            self.play(
                *(
                    LaggedStartMap(
                        FadeOut,
                        VGroup(group[n] for n in killed_indices),
                        shift=0.1 * DOWN,
                        lag_ratio=0.1,
                    )
...
class DensityFormula(InteractiveScene):
    ...
    def old_mess(self):
        ...
        lhs_note = TexText(
            "(Some big number, e.g. $T = 1{,}000{,}000$)", font_size=36, **kw
        )
...
class NewGapsInPrimes(InteractiveScene):
    def construct(self):
        ...
        arrow = Arrow(
            prime_label[-1].get_top(),
            center_label.get_bottom(),
            buff=0.25,
        )
...
class CrankEmail(InteractiveScene):
    ...
    def create_table(
        ...
        # Numbers
        row1_mobs, row2_mobs = (
            VGroup(*(Integer(n, font_size=font_size) for n in content))
        ...
        v_lines = VGroup(
            *(
                Line(grid.get_height() * UP, ORIGIN)
                .set_x(
...
class SieveOfEratosthenes(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(self.n_iterations):
            ...
            to_remove = VGroup(
                *(mob for mob in number_mobs if mob.get_value() % modulus == 0)
            )
            rects = VGroup(
                *(
