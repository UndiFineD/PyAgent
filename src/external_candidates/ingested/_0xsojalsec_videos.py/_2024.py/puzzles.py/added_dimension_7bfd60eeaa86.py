# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\puzzles\added_dimension.py
...
class ShowLozenge(InteractiveScene):
    def construct(self):
        ...
        arc1 = Arc(
            -30 * DEGREES, 60 * DEGREES, arc_center=lozenge.get_left(), radius=0.75
        )
        arc2 = Arc(
            -150 * DEGREES, 120 * DEGREES, arc_center=lozenge.get_top(), radius=0.5
        )
        ...
        self.play(
            ...
            VShowPassingFlash(
                lozenge.copy().insert_n_curves(20).set_stroke(width=5), time_width=2
            ),
            run_time=3,
        )
        self.play(
        ...
        self.play(
            ...
            LaggedStart(
                (
                    TransformFromCopy(lozenge, tile, path_arc=30 * DEGREES)
                    for tile in row
                ),
...
class CubesAsHexagonTiling(InteractiveScene):
    ...
    def setup(self):
        ...
        self.base_cube = self.get_half_cube(
            side_length=n, shared_corner=[-1, -1, -1], grid=True
        )
...
class IntroduceHexagonFilling(InteractiveScene):
    ...
    def setup(self):
        ...
        lozenge.move_to(ORIGIN, DOWN)
        lozenges = VGroup(
            lozenge.copy().rotate(theta, about_point=ORIGIN)
    ...
    def fill_with_current_tiles(self):
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(starter, tile) for tile in random_order),
...
class DrawHexagon(IntroduceHexagonFilling):
    def construct(self):
        ...
        self.play(
            ...
            VShowPassingFlash(
                hexagon.copy().set_stroke(width=5).insert_n_curves(20), time_width=2
            ),
            run_time=2,
        )
        self.wait()
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.frame.animate.reorient(
                118, 79, 0, (-1.01, 0.75, 1.54), 12.00
            ).set_field_of_view(40 * DEGREES),
            run_time=3,
        )
        self.play(
        ...
        self.play(
            self.frame.animate.reorient(135, 55, 0, ORIGIN, 8).set_field_of_view(
                1 * DEGREES
            ),
            run_time=3,
        )
...
class Project3DCube(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            edges.animate.set_stroke(width=1, opacity=1), labels.animate.set_opacity(1)
        )
        ...
        self.play(
            frame.animate.reorient(135.795, 55.795, 0, (-0.02, -0.08, 0.05), 3.61),
            run_time=4,
        )
        self.wait(2, note="Take a moment to look down the corner")
        self.play(
            frame.animate.reorient(50, 68, 0, (-0.46, 0.29, 0.23), 3.45), run_time=4
        )
        ...
        self.play(
            frame.animate.reorient(
                134.75, 54.47, 0, (-0.46, 0.29, 0.23), 3.45
            ).set_field_of_view(1 * DEGREES),
            run_time=4,
        )
        self.wait()
        ...
        cube_grid = VGroup(
            cube_shell.copy().shift(vect) for vect in it.product(*3 * [[0, 1, 2]])
        )
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(ghost_cube, new_cube) for new_cube in cube_grid),
                lag_ratio=0.05,
            ),
        ...
        self.play(
            inner_faces.animate.space_out_submobjects(1.2),
        ...
        coord_labels = VGroup(
            Tex(str(tuple(basis)), font_size=16)
            .next_to(basis, UR, buff=0.05)
            .rotate(45 * DEGREES, RIGHT, about_point=basis)
            for basis in bases
        )
        ...
        proj_vects = VGroup(
            Vector(np.dot(basis, proj_mat.T), thickness=3) for basis in np.identity(3)
        )
        proj_coords = VGroup(
...
class Project4DCube(Project3DCube):
    def construct(self):
        ...
        coords3d = VGroup(
            Tex(str(tuple(coords))) for coords in it.product(*3 * [[0, 1]])
        )
        coords4d = VGroup(
            Tex(str(tuple(coords))) for coords in it.product(*4 * [[0, 1]])
        )
        ...
        self.play(
            LaggedStartMap(
                FadeIn, coords4d, shift=0.1 * DOWN, lag_ratio=0.1, run_time=3
            )
        )
        self.wait()
        ...
        self.play(
            ...
            LaggedStart(
                (FlashUnder(label[-3:], color=RED) for label in low_labels),
                lag_ratio=0.05,
            ),
        )
        self.wait()
        ...
        self.play(
            ...
            LaggedStart(
                (FlashUnder(label[-3:], color=RED) for label in high_labels),
                lag_ratio=0.05,
            ),
        )
        ...
        pre_proj_frame = VGroup(
            Line(pre_proj_points[i], pre_proj_points[j]) for i, j in edge_indices
        )
        ...
        pre_proj_frame.target.rotate(60 * DEGREES, LEFT).rotate(
            45 * DEGREES, UP
        ).rotate(15 * DEGREES, OUT)
        ...
        proj_frame = VGroup(
            Line(proj_points[i], proj_points[j]) for i, j in edge_indices
        )
        ...
        self.play(
            Transform(
                pre_proj_frame.copy(), proj_frame.copy(), run_time=3, remover=True
            )
        )
        ...
        self.play(
            ...
            LaggedStartMap(
                GrowArrow, proj_basis_vectors, suspend_mobject_updating=True
            ),
        ...
        self.play(
            LaggedStart(
                (cell.animate.move_to(-cell.get_center()) for cell in inner_cells),
                group=inner_cells,
                group_type=Group,
                run_time=3,
                lag_ratio=0.25,
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(inner_cells[2], cell) for cell in tiling),
                group_type=Group,
                lag_ratio=0.05,
            ),
        ...
        self.wait(20)
        self.play(
            frame.animate.reorient(36, 66, 0, (-1.32, 0.25, -0.7), 22.55), run_time=3
        )
...
class ShowRhombicDodecTesselation(Project4DCube):
    def construct(self):
        ...
        N = 6
        coords = [
            coords for coords in it.product(*4 * [list(range(N))]) if sum(coords) == 8
        ]
        pieces = Group(
            dodec.copy().shift(np.dot(coord, proj_bases)) for coord in coords
        )
        pieces.sort(lambda p: get_norm(p))
        for piece in pieces:
            piece.set_color(
                random_bright_color(hue_range=(0.5, 0.55), luminance_range=(0.25, 0.5))
            )
            piece.save_state()
        ...
        self.play(
            LaggedStart(
                (Restore(piece) for piece in pieces), lag_ratio=0.5, group_type=Group
            ),
...
class AskStripQuestion(InteractiveScene):
    def construct(self):
        ...
        strip1 = always_redraw(
            lambda: self.get_strip(
                circle,
        ...
        width_label.add_updater(
            lambda m: m.set_value(r1_tracker.get_value() - r0_tracker.get_value())
        )
        width_label.add_updater(
            lambda m: m.set_height(min(0.33, 0.5 * strip1.submobjects[0].get_height()))
        )
        ...
        strips = VGroup(
            self.get_strip(
                circle, r0, r1, theta=0, color=random_bright_color(hue_range=(0.3, 0.5))
            )
            for r0, r1 in zip(subdivision, subdivision[1:])
        )

        self.play(
            FadeOut(fat_strip.pre_rect), FadeIn(strips, lag_ratio=0.5, run_time=2)
        )
        self.wait()
...
class StruggleWithStrips(AskStripQuestion):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(strip.pre_rect, strip.rect) for strip in strips),
                lag_ratio=0.1,
            )
        )
        ...
        new_strips = VGroup(
            self.get_strip(circle, r0, r1, theta=0) for r0, r1 in r_pairs
        )
        ...
        area_dec.add_updater(
            lambda m: m.set_value(get_norm(strip.get_area_vector()) / radius**2)
        )
...
class SphereStrips(InteractiveScene):
    def construct(self):
        ...
        sphere = ParametricSurface(
            lambda u, v: [np.sin(u) * np.cos(v), np.sin(u) * np.sin(v), np.cos(u)],
            u_range=(0, PI),
            v_range=(0, 2 * PI),
        )
        ...
        strip.add_updater(
            lambda m: m.become(
                self.get_strip(
                    x0_tracker.get_value(), x0_tracker.get_value() + delta_x, theta=0
                )
            )
        )
        ...
        def clyinder_projection(points):
            ...
            return np.transpose(
                [points[:, 0] / radii, points[:, 1] / radii, points[:, 2]]
            )

        def get_proj_strip(strip):
            return (
                strip.copy().apply_points_function(clyinder_projection).set_opacity(0.8)
            )
...
class MongesTheorem(InteractiveScene):
    def construct(self):
        ...
        intersection_dots = always_redraw(
            lambda: self.get_all_intersection_dots(tangent_pairs)
        )

        dependents = Group(tangent_pairs, intersection_dots)
        dependents.suspend_updating()

        for tangents, dot, circle_pair in zip(
            tangent_pairs, intersection_dots, it.combinations(circles, 2)
        ):
        ...
        tangent_plane = self.get_plane_through_points(
            [inter_points[2], inter_points[0], blue_tip]
        )
        ...
        self.play(
            Rotating(
                partial_tangent_plane, PI / 2, axis=RIGHT, about_point=pivot_point
            ),
            run_time=8,
            rate_func=there_and_back,
        )
        ...
        self.play(
            self.frame.animate.reorient(-1, 83, 0, (-1.2, 0.14, -0.2)), run_time=3
        )
        self.wait()
        ...
        for x in range(2):
            self.play(
                cone_ghost.animate.scale(
                    1e-2, about_point=intersection_dots[2].get_center()
                ),
                run_time=8,
                rate_func=there_and_back,
            )
            self.wait()
            self.play(
                self.frame.animate.reorient(0, 7, 0, (-1.92, 0.22, 0.0)), run_time=3
            )
        self.play(
            FadeOut(cone_ghost),
        ...
        # Add plane
        plane = always_redraw(
            lambda: self.get_plane_through_points(
                [
                    intersection_dots[2].get_center(),
                    intersection_dots[0].get_center(),
                    cones[2].get_zenith(),
        ...
        dependents.suspend_updating()
        self.play(
            self.frame.animate.reorient(-173, 69, 0, (-1.36, 0.7, 1.01), 7.14),
            run_time=10,
        )
    ...
    def get_tangent_groups(self, circles, n_lines=24):
        ...
        for circ1, circ2 in it.combinations(circles, 2):
            ...
            for angle in np.arange(0, PI, PI / n_lines):
                group.add(
                    *tangent_pair.copy().rotate(angle, axis=axis, about_point=point)
                )
            for line in group:
...
class GeneralCentersOfSimilarity(MongesTheorem):
    def construct(self):
        ...
        self.add(circles)
        for lines, dot, pair in zip(
            similarity_lines, cos_dots, it.combinations(circles, 2)
        ):
        ...
        # Change shapes
        pis = VGroup(
            Tex(R"\pi")[0].match_style(circle).replace(circle) for circle in circles
        )
        ...
        self.play(
            LaggedStart(
                (FadeIn(cone, lag_ratio=0.05) for cone in cones),
                lag_ratio=0.5,
            ),
    ...
    def get_similarity_lines(self, shape1, shape2, n_lines=25):
        ...
        result = VGroup(
            Line(big.pfp(alpha), point) for alpha in np.linspace(0, 1, n_lines)
        )
...
class SimilarDiagrams(MongesTheorem):
    def construct(self):
        ...
        elbows = VGroup(
            ...
            Elbow(width=0.1)
            .rotate(-1.5 * PI + angle, about_point=ORIGIN)
            .shift(t_point2),
        )
...
class AskAboutVolumeOfParallelpiped(InteractiveScene):
    def construct(self):
        ...
        # Add vertex labels
        labels = VGroup(
            Tex(f"(x_{n}, y_{n}, z_{n})", font_size=36) for n in range(1, 5)
        )
        ...
        frame.add_updater(
            lambda m: m.set_theta(-math.cos(7 * self.time * DEGREES) * 35 * DEGREES)
        )

        self.play(
            LaggedStartMap(FadeIn, labels, shift=0.5 * OUT, lag_ratio=0.5, run_time=3)
        )
        self.wait(30)
...
class TriangleAreaFormula(InteractiveScene):
    def construct(self):
        ...
        # Add labels
        labels = VGroup(Tex(Rf"(x_{n}, y_{n})", font_size=36) for n in [1, 2, 3])
        ...
        # Set up 3D labels
        labels_3d = VGroup(Tex(Rf"(x_{n}, y_{n}, 1)", font_size=36) for n in [1, 2, 3])
        ...
        self.play(
            frame.animate.reorient(-13, 75, 0, (-0.14, 0.7, 1.48), 9.34).set_anim_args(
                run_time=2
            ),
            FadeIn(z_axis),
        )
        self.play(
            frame.animate.reorient(15, 81, 0, (-0.56, 0.95, 1.43), 9.34).set_anim_args(
                run_time=8
            ),
            TransformFromCopy(plane, ghost_plane),
            triangle.animate.shift(OUT),
            Transform(labels, labels_3d),
            *(dot.animate.shift(OUT).make_3d() for dot in dots),
        )
        self.wait()
        ...
        self.play(
            frame.animate.reorient(-5, 63, 0, (-0.04, 0.69, 0.39), 7.73).set_anim_args(
                run_time=5
            ),
            Write(cube),
        )
        ...
        self.play(
            frame.animate.reorient(-4, 77, 0, (0.28, 0.78, 0.41), 7.73).set_anim_args(
                run_time=4
            ),
            Write(tetrahedron),
        )
        self.wait()
        self.play(
            frame.animate.reorient(-5, 64, 0, (0.22, 0.72, -0.04), 7.14), run_time=8
        )
        self.wait()
...
class RandomVectorStatistics(InteractiveScene):
    def construct(self):
        ...
        for _ in range(0):
            vects = VGroup(
                Vector(
                    1.5 * rotate_vector(RIGHT, random.uniform(0, TAU)), thickness=4
    ...
    def get_random_angle_data_histogram(self, dim, n_vects=1000000, step_size=1):
        vects1, vects2 = all_vects = [
            np.random.normal(0, 1, (n_vects, dim)) for _ in range(2)
        ]
    ...
    def get_histogram(
        self, data, min_val=0, max_val=180, step_size=5, bar_color=BLUE_D
    ):
        ...
        axes.x_axis.add_numbers(
            np.arange(min_val, max_val + 1, 45), font_size=24, unit_tex=R"^\circ"
        )
        ...
        bars = VGroup(
            Rectangle(width=bar_width, height=density * y_unit).move_to(
                axes.c2p(x, 0), DL
            )
...
class ProbabilityQuestion(InteractiveScene):
    ...
    def get_uniform_random_indicator(self, value_tracker, n):
        ...
        line.add_numbers(
            np.arange(-1, 1.5, 0.5), font_size=12, num_decimal_places=1, buff=0.15
        )
        ...
        tip.add_updater(
            lambda m: m.set_fill(self.value_to_color(value_tracker.get_value()))
        )
    ...
    def construct(self):
        ...
        def update_circles(circles):
            self.place_circles_by_vectors(
                circles, [vt.get_center() for vt in vector_trackers]
            )
        ...
        # Animate about
        radial_lines.add_updater(
            lambda m: m.become(
                self.get_radial_lines(
                    circles, [vt.get_center() for vt in vector_trackers]
                )
            )
        )
...
class TriPod(MongesTheorem):
    def construct(self):
        ...
        proj_point = 3 * UP
        vects = [
            DOWN + 0.35 * LEFT + 0.5 * OUT,
            DOWN + 0.35 * RIGHT + 0.5 * OUT,
            DOWN + 0.5 * IN,
        ]
        ...
        self.play(
            ...
            VGroup(g[2] for g in tri_labels)
            .animate.shift(0.1 * RIGHT)
            .set_anim_args(time_span=(3, 4)),
        )
        self.wait()
        ...
        for tri in tris:
            ...
            mat = np.array(
                [normalize(b - a), normalize(c - a), normalize(cross(b - a, c - a))]
            ).T
        ...
        frame.clear_updaters()
        frame.add_updater(
            lambda m, dt: m.increment_theta(math.sin(self.time / 10) * dt * 2 * DEGREES)
        )
        self.wait(10)
