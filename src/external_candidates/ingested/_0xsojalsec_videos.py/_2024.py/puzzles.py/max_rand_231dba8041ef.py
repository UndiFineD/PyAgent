# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\puzzles\max_rand.py
...
def get_random_var_label_group(
    axis, label_name, color=GREY, initial_value=None, font_size=36, direction=None
):
    ...
    def construct(self):
        ...
        for interval in intervals:
            interval.add_numbers(
                np.arange(0, 1.1, 0.2), font_size=16, buff=0.1, direction=UP
            )
        ...
        max_tracker.add_updater(
            lambda m: m.set_value(max(x1_tracker.get_value(), x2_tracker.get_value()))
        )
...
class SqrtProcess(InteractiveScene):
    def construct(self):
        ...
        sqrt_tracker.add_updater(
            lambda m: m.set_value(math.sqrt(x_tracker.get_value()))
        )
...
class VisualizeMaxOfPairCDF(InteractiveScene):
    def construct(self):
        ...
        x1_tracker, x1_tip, x1_label = get_random_var_label_group(
            axes.x_axis, "x_1", BLUE, 0.7
        )
        x2_tracker, x2_tip, x2_label = get_random_var_label_group(
            axes.y_axis, "x_2", YELLOW, 0.3
        )
        tex_to_color = {"x_1": BLUE, "x_2": YELLOW}
        ...
        self.play(FadeIn(rect), FadeIn(eq, 0.25 * UP))
        self.wait(2)
        self.play(
            FadeOut(rect),
            FadeOut(eq),
        ...
        for tracker, line in zip([x2_tracker, x1_tracker], max_lines):
            ...
            self.play(tracker.animate.set_value(0), ShowCreation(line), run_time=3)
        ...
        self.wait()
        self.play(
            max_lines.animate.set_stroke(width=0.1).set_anim_args(
        ...
        inner_lines = VGroup(
            max_lines.copy()
            .scale(sf, about_point=axes.get_origin())
        ...
        self.play(
            TransformMatchingStrings(
                prob_ineq, gen_prob_ineq, key_map={"0.7": "R"}, run_time=1
            )
        )
        self.play(TransformMatchingStrings(gen_prob_ineq, cdf_expr, run_time=1))
        self.play(
            VGroup(inner_square, max_lines)
            .animate.scale(0.5, about_edge=DL)
        ...
        x3_tracker, x3_tip, x3_label = get_random_var_label_group(
            z_axis, "x_3", color=RED, direction=RIGHT
        )
...
class MaxOfThreeTex(InteractiveScene):
    def construct(self):
        # Test
        expr = TexText(
            R"max(rand(), rand(), rand()) $\leftrightarrow$ rand()$^{1 / 3}$"
        )
