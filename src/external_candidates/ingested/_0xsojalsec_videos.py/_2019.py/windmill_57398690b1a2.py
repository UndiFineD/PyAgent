    ...
    def get_flags(self):
        ...
        for country, count in countries_with_counts:
            ...
            try:
                ...
                image.set_width(1)
                label = VGroup(
                    *[OldTexText(l) for l in country_to_code3[country].upper()]
                )
...
class ShowTest(Scene):
    ...
    def introduce_test(self):
        ...
        for word in proof_words:
            ...
            scores.add(score)
            score[0].add_updater(
                lambda m: m.set_color(interpolate_color(RED, GREEN, m.get_value() / 7))
            )
...
class Describe2011IMO(IntroduceIMO):
    ...
    def comment_on_primality(self):
        ...
        words = VGroup(
            *[OldTexText("Prime").next_to(line, DOWN) for line in reversed(lines)]
        )
        ...
        self.play(
            ...
            "thinking",
            LaggedStart(
                *[FadeIn(word, UP) for word in words], run_time=3, lag_ratio=0.5
            ),
        )
        self.play(Blink(randy))
        self.play(
...
class SpiritOfIMO(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                LaggedStartMap, lights, lambda l: (VFadeInThenOut, l), run_time=3
            ),
            randy.change,
            "thinking",
        )
...
class FormalizeMiddle(WhereItStartsItEnds):
    ...
    def problem_solving_tip(self):
        ...
        self.play(
            FadeOut(pst), MoveToTarget(steps), Restore(mid_words), FadeOut(arrows)
        )
        self.wait()
...
class SecondColoringExample(WindmillScene):
    ...
    def construct(self):
        ...
        dots = self.get_dots(points)
        windmill = self.get_windmill(
            points, pivot=sorted_points[self.n_points // 2], angle=PI / 2
        )
        pivot_dot = self.get_pivot_dot(windmill)
    ...
    def ask_about_even_number(self):
        ...
        browns_counter.add_updater(
            lambda c: c.set_value(
                len(
                    list(
                        filter(lambda d: d.get_fill_color() == Color(GREY_BROWN), dots)
                    )
                )
            )
        )
...
class EasyToFoolYourself(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.pi_creature_says(
            "Isn't it\\\\obvious?", target_mode="maybe", added_anims=[FadeOut(bubble)]
        )
        self.wait(4)
...
class VastSpaceOfConsiderations(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *[FadeIn(mob, UP) for mob in considerations], run_time=3, lag_ratio=0.2
            )
        )
...
class TerryTaoQuote(Scene):
    def construct(self):
        ...
        #
        self.play(
            FadeIn(quote, lag_ratio=0.05, run_time=5, rate_func=bezier([0, 0, 1, 1]))
        )
        self.wait()
        story_line = Line()
