...
class ProbabilityBar(VGroup):
    ...
    def get_percentages(self):
        ...
        for label, bar in zip(labels, self.bars):
            ...
            label.move_to(bar)
            label.set_stroke(
                BLACK, self.percentage_background_stroke_width, background=True
            )
        return labels
...
class StateGoal(PiCreatureScene, Scene):
    ...
    def construct(self):
        ...
        equal_signs = VGroup(
            *[OldTexText("=").next_to(fp, RIGHT) for fp in formula_parts.target]
        )
        ...
        meanings = VGroup(
            OldTexText(
                "Probability a hypothesis is true\\\\(before any evidence)", **kw
            ),
            OldTexText(
        ...
        braces = VGroup(
            *[Brace(diagram.he_rect, vect, buff=SMALL_BUFF) for vect in [DOWN, LEFT]]
        )
        ...
        pe_picture = VGroup(
            diagram.he_rect.copy(), OldTex("+"), diagram.nhe_rect.copy()
        )
...
class HeartOfBayesTheorem(Scene):
    def construct(self):
        ...
        self.play(
            Restore(label2), ReplacementTransform(diagram_copy, restricted_diagram)
        )
        self.wait()
        ...
        self.play(
            *[ApplyMethod(d.set_antilikelihood, 0.6, run_time=2) for d in diagrams]
        )
        ...
        self.play(
            FadeIn(words, DOWN), GrowArrow(arrow), FadeOut(prob), title.to_edge, LEFT
        )
        self.wait()
...
class CreateFormulaFromDiagram(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            FadeOut(not_word), Transform(neg_sym, anti_label.get_part_by_tex("\\neg"))
        )
        self.remove(neg_sym)
        ...
        self.wait()
        self.play(
            Restore(parts), FadeOut(crosses), FadeOut(nps), answer.set_opacity, 0.2
        )
        self.wait()
...
class WhenDiscreteChunksArentSoClean(Scene):
    def construct(self):
        ...
        self.add(p_label[0])
        self.play(
            ChangeDecimalToValue(alt_percentage, 40), ShowIncreasingSubsets(icons[:40])
        )
...
class RandomnessVsProportions(Scene):
    def construct(self):
        ...
        example = VGroup(
            OldTexText("P(X = 5)", tex_to_color_map={"5": YELLOW}), Line(LEFT, RIGHT)
        )
...
class GlimpseOfNextVideo(GraphScene):
    ...
    def construct(self):
        ...
        self.play(
            FadeInFromDown(formula[:4]), Restore(rects1, lag_ratio=0.05, run_time=2)
        )
        self.wait()
...
class ReprogrammingThought(Scene):
    ...
    def construct(self):
        ...
        self.play(
            FadeIn(prior_outline), ApplyMethod(diagram.set_prior, 0.5, run_time=2)
        )
...
class Thumbnail2(Scene):
    def construct(self):
        ...
        right_arrow = Arrow(
            diagram.get_right(), frac[1].get_left(), thickness=0.1, fill_color=GREEN
        )
