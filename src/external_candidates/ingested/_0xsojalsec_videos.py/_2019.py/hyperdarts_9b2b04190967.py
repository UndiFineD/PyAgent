# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\hyperdarts.py
...
class HyperdartScene(MovingCameraScene):
    ...
    def get_square(self):
        return Square(side_length=self.square_width, **self.square_style)
    ...
    def get_hit_distance_line(self, point, circle=None):
        ...
        line = Line(circle.get_center(), point, **self.default_line_style)
        return line
    ...
    def get_radii_to_chord(self, chord, circle=None):
        ...
        center = circle.get_center()
        radii = VGroup(
            *[DashedLine(center, point) for point in chord.get_start_and_end()]
        )
    ...
    def show_full_hit_process(self, point, pace="slow", with_dart=True):
        ...
        self.play(FadeOut(to_fade), run_time=(1 if pace == "slow" else 0.5))

    def show_hits_with_darts(self, points, run_time=0.5, added_anims=None):
        if added_anims is None:
            added_anims = []

        darts = VGroup(*[self.get_dart().move_to(point, DR) for point in points])
        dots = VGroup(*[self.get_dot(point) for point in points])
        ...
        for n in range(n_points):
            self.add_dart_sound(time_offset=(-n / (2 * n_points)))
...
class ShowGiantBullseye(HyperdartScene):
    def construct(self):
        ...
        label.set_color(RED)
        arrow = Arrow(label.get_bottom(), bullseye.get_corner(DR))
        ...
        self.play(
            bullseye.match_width,
            board,
            ApplyMethod(arrow.scale, 0.4, {"about_point": arrow.get_start()}),
            run_time=2,
        )
...
class ShowExampleHit(HyperdartScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            ShowCreation(chord), ShowCreation(elbow), Write(chord_word, run_time=1)
        )
        self.wait()
        ...
        chord_copy.add_updater(lambda m: m.match_width(lines[1]).move_to(ORIGIN))
        ...
        chord_word.add_updater(
            lambda m: m.next_to(lines[1].get_center(), UR, SMALL_BUFF)
        )
        ndw_width = new_diam_word.get_width()
        new_diam_word.add_updater(
            lambda m: m.set_width(min(ndw_width, chord_copy.get_width())).next_to(
                chord_copy, DOWN, SMALL_BUFF
            )
        )

        self.add(
            new_circle, chord_copy, lines, h_label, dart, dot, chord_word, new_diam_word
        )
        self.play(FadeOut(outline), FadeIn(new_circle))
        self.wait()
...
class ContrastDistributions(HyperdartScene):
    def construct(self):
        ...
        right_points = np.random.multivariate_normal(
            mean=board_copy.get_center(), cov=0.6 * np.identity(3), size=n_points
        )

        left_dots, right_dots = [
            VGroup(*[Dot(p, radius=0.02) for p in points])
        ...
        self.play(LaggedStartMap(FadeInFromLarge, right_dots), run_time=5)
        self.wait()
        self.play(
            FadeOut(left_rect),
            FadeIn(right_rect),
        )
        self.play(LaggedStartMap(FadeInFromLarge, left_dots), run_time=5)
        self.wait()
...
class ChooseXThenYUniformly(Scene):
    def construct(self):
        ...
        points = [axes.c2p(*np.random.uniform(-1, 1, size=2)) for n in range(2000)]
...
class ShowDistributionOfScores(Scene):
    ...
    def construct(self):
        ...
        bars.add_updater(
            lambda b: self.set_histogram_bars(b, scores[: get_index()], axes)
        )
        self.add(bars)

        # Add score label
        score_label = VGroup(OldTexText("Last score: "), Integer(1))
        score_label.scale(1.5)
        score_label.arrange(RIGHT)
        score_label[1].align_to(score_label[0][0][-1], DOWN)

        score_label[1].add_updater(lambda m: m.set_value(scores[get_index() - 1]))
        ...
        n_trials_label[1].add_updater(lambda m: m.set_value(get_index()))
        ...
        mean_line.add_updater(lambda m: m.move_to(axes.c2p(get_mean(), 0), DOWN))
        mean_label = VGroup(
            OldTexText("Mean = "),
    ...
    def get_axes(self):
        ...
        axes.y_axis.add_numbers(*range(20, 120, 20), unit="\\%")
    ...
    def construct(self):
        ...
        self.play(
            ReplacementTransform(
                circle.copy().set_fill(opacity=0).set_stroke(WHITE, 1), square_frac[0]
            ),
        )
        self.play(
            ReplacementTransform(square.copy().set_fill(opacity=0), square_frac[2]),
        )
        self.wait(2)
...
class SamplingFourRandomNumbers(Scene):
    ...
    def construct(self):
        ...
        if self.include_title:
            title = OldTex(self.title_tex, tex_to_color_map=t2c)
            title.scale(1.5)
            title.to_edge(UP)
        ...
        tips = VGroup(
            *[
                ArrowTip(start_angle=-PI / 2, color=color).scale(self.tip_scale_factor)
                for color in colors
            ]
        )
        labels = VGroup(*[OldTex(tex) for tex in texs])

        for tip, tracker, line, label in zip(tips, trackers, number_lines, labels):
            tip.line = line
            tip.tracker = tracker
            tip.add_updater(
                lambda t: t.move_to(t.line.n2p(t.tracker.get_value()), DOWN)
            )
        ...
        total.add_updater(
            lambda d: d.set_value(np.sum([t.get_value() ** 2 for t in trackers]))
        )
        ...
        rects = VGroup(*[rect.copy().move_to(line.n2p(0)) for line in number_lines])
        ...
        self.play(
            LaggedStart(
                *[ShowCreationThenFadeAround(summand) for summand in summands[1:]]
            )
        )
        self.play(*[ApplyMethod(tracker.set_value, 0.1) for tracker in trackers[1:]])
        self.wait(10)
...
class SamplePointIn3d(SpecialThreeDScene):
    def construct(self):
        ...
        n_points = 3000
        points = [axes.c2p(*np.random.uniform(-1, 1, 3)) for x in range(n_points)]
        ...
        for x in range(6):
            self.play(point.move_to, axes.c2p(*np.random.uniform(-1, 1, size=3)))
            self.wait(2)
        self.wait(7)
...
class OverlayToPointIn3d(Scene):
    def construct(self):
        ...
        rhs = OldTexText("$({x}, {y}, {z})$", " lies within a\\\\sphere with radius 1")
...
class TwoDPlusTwoDEqualsFourD(HyperdartScene):
    def construct(self):
        ...
        title3 = VGroup(
            OldTexText("Point in 4d space"), OldTex("(x_0, y_0, x_1, y_1)", **kw)
        )
...
class ExpectedValueComputation(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                TransformFromCopy(
                    line1.get_part_by_tex("="), line2.get_part_by_tex("="), **kw
                ),
                TransformFromCopy(
                ...
                TransformFromCopy(
                    line1.get_parts_by_tex("+"), line2.get_parts_by_tex("+"), **kw
                ),
                TransformFromCopy(
                    line1.get_part_by_tex("1"), line2.get_part_by_tex("1"), **kw
                ),
                TransformFromCopy(
                    line1.get_part_by_tex("2"), line2.get_part_by_tex("2"), **kw
                ),
                TransformFromCopy(
                    line1.get_part_by_tex("3"), line2.get_part_by_tex("3"), **kw
                ),
                run_time=3,
                lag_ratio=0,
            ),
            LaggedStart(
                *[GrowFromCenter(bigs[i : i + 2]) for i in range(0, len(bigs), 2)]
            ),
        )
        self.wait()
...
class GameWithSpecifiedScore(HyperdartScene):
    ...
    def construct(self):
        ...
        score_label = VGroup(OldTexText("Score: "), Integer(1))
        score_label.scale(2)
...
class HistogramScene(ShowDistributionOfScores):
    ...
    def setup(self):
        self.scores = np.array([self.get_random_score() for x in range(self.n_scores)])
...
class ExpectedValueFromBars(HistogramScene):
    def construct(self):
        ...
        for i in range(3):
            equation.set_color_by_tex(str(i + 1), bars[i].get_fill_color())

        equation[4:].set_opacity(0.2)

        self.add(mean_label)
        self.play(mean_label[1:].shift, LEFT, FadeIn(equation, LEFT))
        ...
        for i in range(3):
            ...
            self.play(
                bars[: i + 1].set_opacity,
                1,
                bars[i + 1 :].set_opacity,
                0.2,
                equation[: 4 * (i + 1)].set_opacity,
                1,
        ...
        self.play(bars.set_opacity, 1, equation.set_opacity, 1, FadeOut(p_part_copies))

        braces = VGroup(*[Brace(p_part, UP) for p_part in p_parts])
        for brace in braces:
...
class ProbabilitySGtOne(HistogramScene):
    def construct(self):
        ...
        rhs = VGroup(OldTex("="), geo_frac, OldTex("= \\frac{\\pi}{4}"))
        rhs.arrange(RIGHT)
        rhs.next_to(label)
        ...
        self.play(label.shift, shift_val, FadeIn(rhs, LEFT))
        self.wait()
        ...
        self.play(MoveToTarget(new_label), FadeIn(new_rhs, LEFT))
        self.wait()
        ...
        self.play(
            bars[2].set_opacity,
            0.2,
            new_label[:-1].set_opacity,
            0.5,
            new_rhs.set_opacity,
            0.5,
            new_brace.set_opacity,
            0.5,
...
class VolumsOfNBalls(Scene):
    def construct(self):
        ...
        formulas = VGroup(
            *[
                OldTex(tex, tex_to_color_map={"R": WHITE})
                for tex in [
                    "2R",
                    "\\pi R^2",
                    "\\frac{4}{3} \\pi R^3",
        ...
        braces = VGroup(*[Brace(formula, DOWN) for formula in formulas[1::2]])
        ...
        self.play(
            formulas[::2].set_opacity, 0.25, ReplacementTransform(title, alt_title)
        )
...
class IntroduceGame(HyperdartScene):
    ...
    def show_flurry_of_points(self):
        ...
        darts, dots = self.show_hits_with_darts(
            points, added_anims=[FadeInFromDown(title)]
        )
        self.wait()
        ...
        self.play(
            ...
            *[MaintainPositionRelativeTo(dart, dot) for dart, dot in zip(darts, dots)],
        )
    ...
    def introduce_bullseye(self):
        ...
        label.next_to(square, LEFT, aligned_edge=UP)
        label.set_color(RED)
        arrow = Arrow(label.get_bottom(), circle.get_corner(DR))
        ...
        self.play(FadeOut(label), Uncreate(arrow), FadeOut(board))
        self.wait()
    ...
    def show_miss_example(self):
        ...
        self.wait()
        self.play(*map(FadeOut, [dart, dot, miss_word, game_over]))
...
class ShowScoring(HyperdartScene):
    ...
    def add_score_counter(self):
        score = Integer(0)
        score_label = VGroup(OldTexText("Score: "), score)
        score_label.arrange(RIGHT, aligned_edge=DOWN)
        score_label.scale(1.5)
    ...
    def show_single_round(self, pace="fast"):
        while True:
            ...
            if self.is_inside(point):
                ...
                self.wait(0.5)
                self.play(
                    ShowCreationThenFadeAround(self.score_label), FadeOut(to_fade)
                )
                return
