# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\clacks\solution1.py
...
class AskAboutFindingNewVelocities(Scene):
    ...
    def show_value_on_equations(self):
        ...
        sqrt_m_vect = np.array(
            [np.sqrt(self.block1.mass), np.sqrt(self.block2.mass), 0]
        )
...
class IntroduceVelocityPhaseSpace(AskAboutFindingNewVelocities):
    ...
    def show_momentum_line(self):
        ...
        foreground_mobs = VGroup(
            self.equations[1], self.momentum_xy_equation, self.blocks, self.vps_dot
        )
    ...
    def show_reflection_about_x(self):
        ...
        group = VGroup(
            self.ellipse, self.lines[-1], self.vps_dot.copy().clear_updaters()
        )
        for x in range(2):
            self.play(
                Rotate(group, PI, RIGHT, about_point=self.axes.coords_to_point(0, 0)),
            )
        self.remove(group[-1])
...
class HuntForPi(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "Hunt for $\\pi$!", bubble_config={"direction": LEFT}, target_mode="hooray"
        )
...
class AnalyzeCircleGeometry(CircleDiagramFromSlidingBlocks, MovingCameraScene):
    ...
    def show_equal_arc_lengths(self):
        ...
        lower_arcs = VGroup(
            *[
                Arc(start_angle=(PI + n * 2 * theta), angle=(2 * theta), radius=radius)
    ...
    def focus_on_three_points(self):
        ...
        two_theta_arc = Arc(
            radius=1, start_angle=(center_lines[0].get_angle() + PI), angle=2 * theta
        )
        ...
        self.play(
            LaggedStartMap(FadeOut, VGroup(*to_fade.family_members_with_points()))
        )
...
class AddTwoThetaManyTimes(Scene):
    def construct(self):
        ...
        for n in range(5):
            ...
            self.wait(0.25)
        self.play(
            FadeIn(expression[-2:], LEFT), GrowFromCenter(brace), FadeIn(question, UP)
        )
        self.wait(3)
        self.play(
...
class ThetaChart(Scene):
    ...
    def populate_columns(self):
        ...
        entries = [("$m_1$ : $m_2$", "$\\arctan(\\sqrt{m_2} / \\sqrt{m_1})$", "")] + [
            (
                "{:,} : 1".format(10 ** (2 * exp)),
...
class TangentTaylorSeries(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ShowCreation(series_error_rect), self.change_students(*3 * ["pondering"])
        )
...
class ConservationLawSummary(Scene):
    def construct(self):
        ...
        for group in energy_group, momentum_group:
            arrow = Arrow(
                LEFT, 2 * RIGHT, rectangular_stem_width=0.1, tip_length=0.5, color=WHITE
            )
        ...
        self.wait()
        self.play(
            LaggedStartMap(ShowCreationThenDestruction, red_energy_word),
        )
...
class FinalCommentsOnPhaseSpace(Scene):
    ...
    def state_to_point(self):
        ...
        arrow = Arrow(
            2 * LEFT, 2 * RIGHT, color=WHITE, rectangular_stem_width=0.1, tip_length=0.5
        )
