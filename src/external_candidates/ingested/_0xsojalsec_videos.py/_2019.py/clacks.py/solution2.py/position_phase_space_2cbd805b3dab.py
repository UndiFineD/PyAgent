# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\clacks\solution2\position_phase_space.py
from _2019.clacks.question import Block, ClackFlashes, Wall
...
class PositionPhaseSpaceScene(Scene):
    ...
    def get_clack_data(self):
        ...
        for k in range(1, int(PI / theta) + 1):
            ...
            reflected_point = rotate_vector(
                clack_ps_point, -2 * np.ceil((k - 1) / 2) * theta
            )
    ...
    def get_ps_point_change_anim(self, d1, d2, **added_kwargs):
        ...
        return ApplyMethod(self.ps_point.move_to, self.ds_to_point(d1, d2), **kwargs)
    ...
    def get_axes_labels(self, axes, with_sqrts=None):
        ...
        if with_sqrts:
            additions = map(Tex, ["\\sqrt{m_1}", "\\sqrt{m_2}"])
            for label, addition in zip(labels, additions):
                addition.move_to(label[2], DL)
                label[2].next_to(addition, RIGHT, SMALL_BUFF, aligned_edge=DOWN)
    ...
    def get_phase_space_point(self):
        ...
        ps_point.move_to(
            self.ds_to_point(
                self.block1.distance, self.block2.distance + self.block2.width
            )
        )
    ...
    def get_d2_label(self):
        self.d2_label = self.get_brace_d_label(2, self.get_d2, self.d2_brace, UP, 0)
...
class EqualMassCase(PositionPhaseSpaceScene):
    ...
    def show_first_point(self):
        ...
        self.play(
            ShowCreationThenFadeOut(
                Circle(color=RED).replace(ps_dot).scale(2), run_time=1
            )
        )
        self.wait()
        self.play(
    ...
    def up_to_first_collision(self):
        ...
        block_arrow.add_updater(lambda m: m.shift(m.block.get_center() - m.get_start()))
    ...
    def show_angles(self):
        ...
        for arc in arcs:
            ...
            self.play(
                ...
                UpdateFromAlphaFunc(
                    arc.line,
                    lambda m, a: m.set_stroke(opacity=(there_and_back(a) ** 0.5)),
                ),
            )
    ...
    def show_rescaling(self):
        ...
        def show_label(index, block, vect):
            ...
            self.play(
                Transform(
                    axes.labels[index],
                    VGroup(*new_axes_labels[index][:2], new_axes_labels[index][3]),
                ),
            ...
            self.play(
                MoveToTarget(group, rate_func=there_and_back_with_pause, run_time=3)
            )
            self.remove(group)
            self.wait()
...
class IntroduceVelocityVector(PositionPhaseSpaceScene, MovingCameraScene):
    ...
    def zoom_in_on_vector(self):
        ...
        ps_vect = self.ps_velocity_vector
        new_vect = Arrow(
            ps_vect.get_start(), ps_vect.get_end(), buff=0, **self.new_vect_config
        )
    ...
    def calculate_magnitude(self):
        ...
        rhs_scale_val = 0.8
        rhs = OldTex(
            "=\\sqrt{" "\\left( dx/dt \\right)^2 + " "\\left( dy/dt \\right)^2" "}"
        )
        ...
        final_rhs = OldTex("=", "\\sqrt{2(\\text{Kinetic energy})}")
...
class ShowMomentumConservation(IntroduceVelocityVector):
    ...
    def rearrange_for_slope(self):
        ...
        movers = VGroup(
            y_eq[0], equals.submobjects[0], y_eq[2], y_eq[1], x_eq[2], x_eq[0]
        ).copy()
        for mover, part in zip(movers, final_eq):
            mover.target = part
        self.play(
            LaggedStartMap(MoveToTarget, movers, path_arc=30 * DEGREES, lag_ratio=0.9),
    ...
    def up_to_first_collision(self):
        ...
        self.wait_until(lambda: abs(self.ps_velocity_vector.get_vector()[1]) > 0.01)
        self.end_sliding()
    ...
    def ask_what_next(self):
        ...
        for last_angle, angle in zip(np.cumsum([0] + angles), angles):
            ...
            self.play(
                Transform(ps_vect, target, path_arc=angle),
            )
    ...
    def show_rate_of_change_vector(self):
        ...
        d_array.add_updater(
            lambda m: m.next_to(
                ps_vect.get_end(), np.sign(ps_vect.get_vector()[0]) * RIGHT, SMALL_BUFF
            )
        )
    ...
    def show_sqrty_m_vector(self):
        ...
        self.play(MoveToTarget(sqrty_m_vector), MoveToTarget(sqrty_m_array), run_time=2)
    ...
    def show_same_angles(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                line_pair, lambda m, a: m.set_stroke(width=there_and_back(a) ** 0.5)
            ),
        )
    ...
    def show_trajectory_beam_bounce(self, n_times=2):
        ...
        for x in range(n_times):
            self.play(
                ShowPassingFlash(beam, run_time=2, rate_func=bezier([0, 0, 1, 1]))
            )
    ...
    def get_dot_product(
        self, m1="\\sqrt{m_1}", m2="\\sqrt{m_2}", d1="dx/dt", d2="dy/dt"
    ):
...
class JustTheProcessNew(PositionPhaseSpaceScene):
    CONFIG = {
        "block1_config": {"mass": 16, "velocity": -2},
        "wait_time": 10,
    }
