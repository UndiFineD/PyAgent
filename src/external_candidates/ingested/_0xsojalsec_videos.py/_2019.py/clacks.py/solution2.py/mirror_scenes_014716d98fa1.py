...
class MirrorScene(Scene):
    ...
    def setup(self):
        ...
        self.beam_point = VectorizedPoint(
            np.array([self.get_start_x_offset(), self.get_start_y_offset(), 0])
        )
    ...
    def get_special_flash(
        self, mobject, stroke_width, time_width, rate_func=linear, **kwargs
    ):
...
class ReflectWorldThroughMirrorNew(MirrorScene):
    ...
    def add_ghost_beam_point(self):
        self.ghost_beam_point.add_updater(
            lambda m: m.move_to(self.ghost_trajectory.get_points()[-1])
        )
        self.add(self.ghost_beam_point)

    def up_through_first_bounce(self):
        self.play(
            *self.get_both_partial_trajectory_anims(*self.partial_trajectory_values[:2])
        )
        self.wait()
    ...
    def focus_on_two_important_trajectories(self):
        ...
        self.ghost_beam_point.add_updater(
            lambda m: m.move_to(gt_beam_anims[0].mobject.get_points()[-1])
        )
    ...
    def get_partial_trajectory_anims(self, trajectory, a, b, **kwargs):
        ...
        return UpdateFromAlphaFunc(
            ...
            lambda m, alpha: m.pointwise_become_partial(
                m.full_self, 0, interpolate(a, b, alpha)
            ),
            **kwargs,
        )
...
class MirrorAndWiresOverlay(MirrorScene):
    ...
    def show_angles(self):
        ...
        self.play(
            ShowCreation(dl_arc),
            UpdateFromAlphaFunc(
                VMobject(), lambda m, a: incidence.set_fill(opacity=a), remover=True
            ),
        )
        self.wait()
        self.add(reflection)
        self.play(
            ShowCreation(dr_arc),
            UpdateFromAlphaFunc(
                VMobject(), lambda m, a: reflection.set_fill(opacity=a), remover=True
            ),
        )
        self.wait()
