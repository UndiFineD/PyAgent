# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\clacks\solution2\simple_scenes.py
...
class ShowDotProductMeaning(Scene):
    def construct(self):
        ...
        for angle in [135, 225, 270, 90, 150]:
            self.play(v_vect.angle_tracker.set_value, angle * DEGREES, run_time=2)
            self.wait()
...
class FinalComment(Scene):
    def construct(self):
        self.add(
            OldTexText(
                "Thoughts on what ending should go here?\\\\" "See the Patreon post."
            )
        )
...
class AskAboutAddingThetaToItself(Scene):
    ...
    def get_group(self, theta):
        ...
        wedge = VGroup(
            ...
            Line(ORIGIN, wedge_radius * RIGHT).rotate(theta, about_point=ORIGIN),
        )
    ...
    def get_inequality(self):
        ineq = OldTex("N", "\\cdot", "\\theta", "<", "\\pi", "=", "3.1415926\\dots")
    ...
    def get_dynamic_inequality(self, counter):
        ...
        def get_ineq():
            result = OldTex(">" if is_greater_than_pi() else "<")
...
class FinalFormula(Scene):
    def construct(self):
        ...
        formula = OldTex(
            "\\left\\lfloor", "{\\pi", "\\over", "\\theta}", "\\right\\rfloor"
        )
...
class TwoSolutionsWrapper(WrapperScene):
    def construct(self):
        big_rect = self.get_big_rect()
        screen_rects = VGroup(*[self.get_screen_rect(height=3) for x in range(2)])
        ...
        self.play(
            LaggedStartMap(
                ShowCreation, screen_rects.copy().set_fill(opacity=0), lag_ratio=0.8
            ),
            LaggedStartMap(FadeIn, screen_rects, lag_ratio=0.8),
            LaggedStartMap(
                ApplyMethod, pis, lambda pi: (pi.change, "pondering", screen_rects[0])
            ),
        )
        self.play(Blink(random.choice(pis)))
        self.play(
            LaggedStartMap(
                ApplyMethod, pis, lambda pi: (pi.change, "thinking", screen_rects[1])
            )
        )
        self.play(Blink(random.choice(pis)))
        self.wait()
...
class ClacksSolution2Thumbnail(Scene):
    ...
    def add_scene2(self):
        ...
        trajectories = VGroup(scene2.trajectory, *scene2.reflected_trajectories)
