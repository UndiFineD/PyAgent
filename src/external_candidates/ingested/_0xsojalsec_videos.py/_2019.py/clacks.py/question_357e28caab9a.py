# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\clacks\question.py
...
class SlidingBlocks(VGroup):
    ...
    def update_blocks_from_phase_space_point_tracker(self):
        ...
        reflected_point = rotate_vector(ps_point, -2 * np.ceil(n_clacks / 2) * theta)
    ...
    def get_clack_data(self):
        ...
        for k in range(1, int(PI / theta) + 1):
            ...
            s2 = reflected_point[1] / np.sqrt(block2.mass)
            location = np.array(
                [self.wall.get_right()[0] + s2, block2.get_center()[1], 0]
            )
            if k % 2 == 1:
...
class LightBouncing(MovingCameraScene):
    ...
    def get_beam_anim(self, beam):
        ...
        return AnimationGroup(
            ...
            UpdateFromFunc(dot, lambda m: m.move_to(beam.get_points()[-1])),
        )
...
class TwoBlocksLabel(Scene):
    def construct(self):
        ...
        label.to_edge(UP)
        arrows = VGroup(*[Arrow(label.get_bottom(), point) for point in [RIGHT, LEFT]])
        arrows.set_color(RED)
        self.play(
            Write(label), LaggedStartMap(GrowArrow, arrows, lag_ratio=0.7), run_time=1
        )
        self.wait()
...
class Mass1e1WithElasticLabel(BlocksAndWallExampleMass1e1):
    def add_flash_animations(self):
        ...
        label = OldTexText("Purely elastic collisions\\\\" "(no energy lost)")
        label.set_color(YELLOW)
        label.move_to(2 * LEFT + 2 * UP)
        self.add(label)
        self.add(*[self.get_arrow(label, flashes, flash) for flash in flashes.flashes])
...
class AskAboutSoundlessness(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says(
            ...
            added_anims=[self.change_students("pondering", "confused", "thinking")],
        )
...
class BlocksAndWallExampleMass1e4SlowMo(BlocksAndWallExample):
    CONFIG = {
        "sliding_blocks_config": {
            "block1_config": {"mass": 1e4, "velocity": -0.1, "distance": 4.1},
        },
        "wait_time": 50,
...
class BlocksAndWallExampleMass1e6SlowMo(BlocksAndWallExample):
    CONFIG = {
        "sliding_blocks_config": {
            "block1_config": {"mass": 1e6, "velocity": -0.1, "distance": 4.1},
        },
        "wait_time": 60,
        "collision_sound": "slow_clack.wav",
    }
...
class DigitsOfPi(Scene):
    ...
    def construct(self):
        ...
        equation = OldTex(("\\pi = {:." + str(nd) + "f}...").format(rounded_pi))
...
class PiComputingAlgorithmsAxes(Scene):
    ...
    def get_basel(self):
        formula = OldTex("\\frac{\\pi^2}{6} = " "\\sum_{n=1}^\\infty \\frac{1}{n^2}")
        formula.scale(0.5)
        return formula
    ...
    def get_blocks_image(self):
        scene = BlocksAndWallScene(
            ...
            n_wall_ticks=6,
            sliding_blocks_config={
                "block1_config": {"mass": 1e2, "velocity": -0.01, "distance": 3.5},
                "block2_config": {
                    "distance": 1,
                    "velocity": 0,
                },
...
class StepsOfTheAlgorithm(TeacherStudentsScene):
    def construct(self):
        ...
        for step in steps:
            self.play(FadeInFromDown(step[0]), self.teacher.change, "raise_right_hand")
            self.play(
    ...
    def get_steps(self):
        return VGroup(
            ...
            OldTexText(
                "Step 3:", "Set one mass to $100^{d - 1}$,\\\\" "the other to $1$"
            ),
...
class CompareToGalacticMass(Scene):
    ...
    def show_total_count(self):
        ...
        commas = VGroup(*[mob for c, mob in zip(tex_string, number) if c is ","])
        dots = VGroup(*[mob for c, mob in zip(digits.get_tex(), digits) if c is "."])
...
class BlocksAndWallExampleGalacticMass(BlocksAndWallExample):
    ...
    def setup(self):
        super().setup()
        words = OldTexText("Burst of $10^{38}$ clacks per second")
        words.scale(1.5)
        words.to_edge(UP)
        self.add(words)
...
class CompareAlgorithmToPhysics(PiCreatureScene):
    def construct(self):
        ...
        right_pic = ImageMobject(
            self.get_image_file_path().replace(str(self), "PiComputingAlgorithmsAxes")
        )
...
class AskAboutWhy(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_all_student_changes("pondering", look_at=circle)
        self.wait(2)
        self.play(
...
class NextVideo(Scene):
    def construct(self):
        ...
        mid_words = OldTexText("Patient\\\\", "problem\\\\", "solving")
        ...
        friends = VGroup(PiCreature(color=BLUE_E), PiCreature(color=BLUE_C), Mortimer())
...
class Thumbnail(BlocksAndWallExample, MovingCameraScene):
    ...
    def add_vector(self):
        ...
        arrow = self.arrow = Vector(
            2.5 * LEFT, color=RED, rectangular_stem_width=1.5, tip_length=0.5
        )
    ...
    def add_text(self):
        question = self.question = OldTexText("How many\\\\collisions?")
        question.scale(2.5)
        question.to_edge(UP)
