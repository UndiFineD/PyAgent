# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part3\staging.py
...
class FourierNameIntro(Scene):
    ...
    def show_two_titles(self):
        ...
        self.play(FadeIn(lt, RIGHT), ShowCreation(v_line))
        self.play(
            FadeIn(rt, LEFT),
        )
        ...
        for title, variants in [(rt, rt_variants)]:
            ...
            self.play(MoveToTarget(title), FadeIn(variants[0][0], LEFT))
    ...
    def transition_to_image(self):
        ...
        self.play(
            ...
            FadeIn(image),
            FadeIn(name[0]),
            *[ReplacementTransform(title[0].deepcopy(), name[1]) for title in titles],
            titles.scale,
            0.65,
            titles.arrange,
            DOWN,
    ...
    def show_paper(self):
        ...
        heat_rect = SurroundingRectangle(OldTexText("CHALEUR"))
...
class FourierSeriesIllustraiton(Scene):
    ...
    def construct(self):
        ...
        range_words = OldTexText("For $0 \\le x \\le 1$")
        range_words.next_to(
        ...
        rects = it.chain(
            [SurroundingRectangle(piece) for piece in self.get_sum_tex_pieces(sum_tex)],
            it.cycle([None]),
        )
    ...
    def get_axes_arrow_axes(self):
        ...
        axes1.x_axis.add_numbers(0.5, 1, num_decimal_places=1)
    ...
    def generate_nth_func(self, n):
        return lambda x: np.prod(
            [(4 / PI), (1 / n) * (-1) ** ((n - 1) / 2), np.cos(PI * n * x)]
        )
...
class ExponentialDecay(PiCreatureScene):
    def construct(self):
        ...
        arrow.add_updater(lambda m: m.scale(2.5 * abs(get_dydt()) / m.get_length()))
        ...
        ode = OldTex(
            ...
            mk_tex,
            "\\cdot {f}(t)",
            tex_to_color_map={"{f}": YELLOW, "=": WHITE, mk_tex: mk_tex_color},
        )
        ode.to_edge(UP)
        dfdt = ode[:3]
        ft = ode[-2:]
        ...
        self.play(Write(dot), TransformFromCopy(exp, exp_copy), rect.move_to, exp_copy)
...
class InvestmentGrowth(Scene):
    ...
    def construct(self):
        ...
        axes = Axes(
            ...
            x_axis_config={
                "unit_size": 0.3,
                "tick_size": 0.05,
                "big_tick_numbers": range(0, self.total_time, 5),
            },
        )
        ...
        # Graph
        graph = axes.get_graph(lambda x: self.initial_value * np.exp(self.k * x))
    ...
    def get_time_tracker(self):
        ...
        time_tracker.add_updater(lambda m, dt: m.increment_value(self.time_rate * dt))
        return time_tracker
...
class GrowingPileOfMoney(InvestmentGrowth):
    ...
    def construct(self):
        ...
        dollar_signs = VGroup(*[OldTex("\\$") for x in range(int(final_count))])
    ...
    def get_atom(self, body):
        ...
        while wn == 0:
            ...
            wn = get_winding_number(
                [body.point_from_proportion(a) - point for a in np.linspace(0, 1, 300)]
            )
            wn = int(np.round(wn))
...
class BoundaryConditionInterlude(Scene):
    def construct(self):
        background = FullScreenFadeRectangle(fill_color=GREY_D)
        ...
        bc_image = self.get_labeled_image(
            "Boundary\\\\conditions", "boundary_condition_thumbnail"
        )
        ...
        self.play(GrowFromCenter(brace), FadeIn(nv_text, RIGHT))
        self.wait()
