...
class SimpleCosExpGraph(TemperatureGraphScene):
    ...
    def get_cos_graph(self, axes, **config):
        return self.get_initial_state_graph(
            axes, lambda x: self.cos_exp(x, 0), **config
        )
...
class BreakDownAFunction(SimpleCosExpGraph):
        ...
        low_surfaces = VGroup(
            *[
                self.get_surface(
                    axes,
                    lambda x, t: np.prod(
                        [A, np.cos(n * x / 2), np.exp(-k * (n / 2) ** 2 * t)]
                    ),
                )
                for n, axes, A in zip(it.count(), low_axes_group, fourier_terms)
            ]
    ...
    def show_break_down(self):
        ...
        plusses = VGroup(
            *[
                OldTex("+").next_to(axes.x_axis.get_end(), RIGHT, MED_SMALL_BUFF)
    ...
    def get_fourier_cosine_terms(self, func, n_terms=40):
        result = [
            integrate.quad(lambda x: (1 / PI) * func(x) * np.cos(n * x / 2), 0, TAU)[0]
...
class OceanOfPossibilities(TemperatureGraphScene):
    ...
    def setup_surface(self):
        ...
        def func(x, t):
            return np.sum(
                [
                    np.prod([A * np.cos(omega * x), np.exp(-k * omega**2 * t)])
...
class AnalyzeSineCurve(TemperatureGraphScene):
    ...
    def reference_curvature(self):
        curve_segment, curve_x_tracker = self.get_curve_segment_with_x_tracker(
            self.graph
        )
...
class SlopeToHeatFlow(FlatEdgesContinuousEvolution):
    ...
    def show_heat_flow(self):
        ...
        flow_words = VGroup(
            *[
                OldTexText("Heat flow").next_to(point, DOWN).scale(0.8)
                for point in points
            ]
        )
        flow_mobjects = VGroup(
            *[self.get_flow(x, flow_rate) for x, flow_rate in zip(xs, flow_rates)]
        )
...
class ManipulateSinExpSurface(TemperatureGraphScene):
    ...
    def setup_axes(self):
        ...
        axes.z_axis.add_numbers(
            *np.arange(-1, 1.5, 0.5), direction=LEFT, num_decimal_places=1
        )
        for number in axes.z_axis.numbers:
            number.rotate(90 * DEGREES, RIGHT)
    ...
    def talk_through_omega(self):
        ...
        x_decimal = always_redraw(
            lambda: DecimalNumber(get_x(), color=x.get_color()).next_to(
                brace, DOWN, SMALL_BUFF
            )
        )
    ...
    def get_graph(self):
        return always_redraw(
            lambda: self.get_time_slice_graph(
                self.axes, self.func, t=self.t_tracker.get_value(), **self.graph_config
            )
        )
    ...
    def construct(self):
        ...
        for n in range(2, 16, 2):
            new_graph = self.get_time_slice_graph(
                axes, self.func, t=n, **self.graph_config
            )
