...
class EquationAboveSineAnalysis(WriteHeatEquationTemplate):
    def construct(self):
        ...
        new_rhs = OldTex(
            "=", "-\\alpha \\cdot {T}", "({x}, 0)", **self.tex_mobject_config
        )
...
class ShowSinExpDerivatives(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        equation1 = OldTex(
            "T({x}, {0}) = \\sin\\left({x}\\right)", **self.tex_mobject_config
        )
        equation2 = OldTex(
        ...
        dx = OldTex(
            "\\cos\\left({x}\\right)", "e^{-\\alpha {t}}", **self.tex_mobject_config
        )
        ddx = OldTex(
            "-\\sin\\left({x}\\right)", "e^{-\\alpha {t}}", **self.tex_mobject_config
        )
...
class DerivativesOfLinearFunction(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        for arrow, char, vect in zip(arrows, "xxt", [UP, UP, RIGHT]):
            label = OldTex(
                "\\partial \\over \\partial {%s}" % char, **self.tex_mobject_config
            )
            label.scale(0.7)
...
class WriteOutBoundaryCondition(
    FlatAtBoundaryWords, ThreeConstraints, MovingCameraScene
):
    def construct(self):
        self.force_skipping()
