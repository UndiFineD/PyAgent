...
class VisualizeStates(Scene):
    ...
    def initialize_plane(self):
        ...
        plane.axis_labels = VGroup(
            ...
            plane.get_y_axis_label("\\dot \\theta", UP, DR, buff=SMALL_BUFF).set_color(
                YELLOW
            ),
        )
    ...
    def show_grid_of_states_creation(self):
        ...
        self.play(
            ReplacementTransform(
                right_column_copy[-1], state_grid[-1][-1], remover=True
            )
        )
        self.wait()
    ...
    def show_evolution_from_a_start_state(self):
        ...
        self.play(
            ShowCreationThenFadeAround(td_label), ShowCreationThenFadeOut(y_axis_copy)
        )
    ...
    def tie_state_to_dot_position(self, state, dot):
        ...
        state.pendulum.get_arc_angle_theta = lambda: self.plane.x_axis.point_to_number(
            dot.get_center()
        )
        return self
...
class IntroduceVectorField(VisualizeStates):
    ...
    def write_vector_derivative(self):
        ...
        # Vector
        vect = Arrow(
            plane.coords_to_point(0, 0), dot.get_center(), buff=0, color=dot.get_color()
        )
        vect_sym, d_vect_sym = [
...
class ShowHighVelocityCase(ShowPendulumPhaseFlow, MovingCameraScene):
    ...
    def show_high_vector(self):
        field = self.vector_field
        top_vectors = VGroup(
            *filter(lambda a: np.all(a.get_center() > [-10, 1.5, -10]), field)
        ).copy()
...
class LovePhaseSpace(ShowHighVelocityCase):
    ...
    def show_vector_field(self):
        ...
        for arrow in axis_arrows:
            ...
            arrow.scale(
                0.5 * real_len / arrow.get_length(), about_point=arrow.get_start()
            )
    ...
    def show_example_trajectories(self):
        ...
        start_points = self.start_points = [
            2.5 * np.random.random() * rotate_vector(RIGHT, TAU * np.random.random())
...
class TakeManyTinySteps(IntroduceVectorField):
    ...
    def take_many_time_steps(self):
        ...
        count_label.add_updater(
            lambda m: m.next_to(
                count[-1], RIGHT, submobject_to_align=m[0][0], aligned_edge=DOWN
            )
        )
    ...
    def get_labels(self, get_t, get_delta_t):
        t_label, dt_label = labels = VGroup(
            *[
                VGroup(OldTex("{} = ".format(s)), DecimalNumber(0)).arrange(
        ...
        t_label[-1].add_updater(
            lambda d: d.set_value(int(np.ceil(get_t() / get_delta_t())) * get_delta_t())
        )
...
class SetupToTakingManyTinySteps(TakeManyTinySteps):
    ...
    def show_step(self):
        ...
        vector.scale(
            get_norm(vector_value) / vector.get_length(), about_point=vector.get_start()
        )
