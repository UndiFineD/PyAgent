# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part1\pendulum.py
...
class WherePendulumLeads(PiCreatureScene):
    def construct(self):
        ...
        linear_formula = OldTex(
            "\\frac{d \\vec{\\textbf{x}}}{dt}=" "A\\vec{\\textbf{x}}"
        )
        ...
        self.play(
            randy.change, "pondering", pendulum, FadeInFromDown(q_marks, lag_ratio=0.3)
        )
...
class AnalyzePendulumForce(MovingCameraScene):
    ...
    def show_arc_length(self):
        ...
        self.play(
            TransformFromCopy(line_L, rhs.get_part_by_tex("L")),
    ...
    def show_gsin_formula(self):
        ...
        g_term.add_updater(
            lambda m: m.next_to(
                g_vect, RIGHT if self.pendulum.get_theta() >= 0 else LEFT, SMALL_BUFF
            )
        )
        ...
        g_sin_label = always_redraw(
            lambda: create_vect_label(
                g_vect.tangent,
                "-g\\sin(\\theta)",
                UP,
            )
        )
        g_cos_label = always_redraw(
            lambda: create_vect_label(
                g_vect.perp,
                "-g\\cos(\\theta)",
                DOWN,
