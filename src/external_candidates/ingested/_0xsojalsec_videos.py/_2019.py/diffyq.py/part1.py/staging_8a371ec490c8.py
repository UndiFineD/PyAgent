    ...
    def construct(self):
        ...
        field = VectorField(
            lambda p: pendulum_vector_field_func(
                plane.point_to_coords(p), mu=mu_tracker.get_value()
            ),
            delta_x=0.5,
            delta_y=0.5,
            max_magnitude=6,
...
class TourOfDifferentialEquations(MovingCameraScene):
    ...
    def add_fourier_series(self, thumbnail):
        ...
        waves = VGroup(
            *[self.get_square_wave_approx(N, color) for N, color in enumerate(colors)]
        )
...
class ShowGravityAcceleration(Scene):
    ...
    def show_g_value(self):
        ...
        g_eq = self.g_eq = OldTex(
            "-9.8", "{\\text{m/s}", "\\over", "\\text{s}}", **Lg_formula_config
        )
        ...
        for t1, t2 in zip(times, times[1:]):
            ...
            kw = {
                "rate_func": lambda alpha: interpolate(
                    t1 / total_time, t2 / total_time, alpha
                )
            }
            anims = [
    ...
    def combine_v_vects(self):
        ...
        braces = VGroup(
            *[
                brace.copy()
                .match_height(arrow)
                .next_to(arrow, RIGHT, buff=0.2 * SMALL_BUFF)
                for arrow in delta_vects
            ]
        )
        amounts = VGroup(
            *[
                OldTexText("9.8 m/s").scale(0.5).next_to(brace, RIGHT, SMALL_BUFF)
                for brace in braces
            ]
        )
...
class SimpleProjectileEquation(ShowGravityAcceleration):
    ...
    def setup_trajectory(self):
        ...
        graph.add_updater(
            lambda g: g.pointwise_become_partial(
                graph_template, 0, get_t() / total_time
            )
        )
        traj.add_updater(
            lambda t: t.pointwise_become_partial(traj_template, 0, get_t() / total_time)
        )
    ...
    def solve_for_position(self):
        ...
        for deriv, terms in zip(derivs, all_terms):
            ...
            deriv.next_to(
                VGroup(equation, y_dot_equation), DOWN, MED_LARGE_BUFF + SMALL_BUFF
            )
...
class DefineODE(Scene):
    ...
    def show_value_slope_curvature(self):
        ...
        graph = axes.get_graph(
            lambda t: p.initial_theta
            * np.cos(np.sqrt(p.gravity / p.length) * t)
            * np.exp(-p.mu * t / 2)
        )
...
class SecondOrderEquationExample(DefineODE):
    ...
    def show_value_slope_curvature(self):
        ...
        x, d_x, dd_x = [
            OldTex("{" + s + "x}(t)", **tex_config) for s in ("", "\\dot ", "\\ddot ")
        ]
...
class BreakingSecondOrderIntoTwoFirstOrder(IntroduceVectorField):
    def construct(self):
        ...
        theta_dots = VGroup(
            *filter(
                lambda m: (
                    isinstance(m, SingleStringTex) and "{\\dot\\theta}" == m.get_tex()
                ),
                system1.get_family(),
            )
        )
...
class ThreeBodiesInSpace(SpecialThreeDScene):
    ...
    def add_bodies(self):
        ...
        center_of_mass = reduce(
            op.add, [body.mass * body.get_center() / total_mass for body in bodies]
        )
        average_momentum = reduce(
            op.add, [body.mass * body.velocity / total_mass for body in bodies]
        )
        for body in bodies:
    ...
    def get_initial_velocity(self, center, centers, mass):
        ...
        velocity = (
            0.2
            * mass
            * normalize(np.cross(*filter(lambda diff: get_norm(diff) > 0, to_others)))
        )
        return velocity
