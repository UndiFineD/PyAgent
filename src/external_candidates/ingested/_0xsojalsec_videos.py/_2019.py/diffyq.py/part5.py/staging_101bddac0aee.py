# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part5\staging.py
...
class IntroductionOfExp(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                ...
                *[FadeIn(questions[1][i]) for i in [1, 2, 4]],
            ),
        )
        self.wait()
        self.play(
        ...
        self.wait()
        self.play(Restore(derivative), FadeIn(ic, LEFT))
        self.wait()
        self.play(
            FadeOut(questions[0]),
            FadeOut(top_cross),
    ...
    def get_deriv_and_ic(self, const=None):
        ...
        derivative = OldTex(*args, **tex_config)
        if const:
...
class IntroducePhysicalModel(IntroductionOfExp):
    ...
    def setup_value_trackers(self):
        ...
        output_tracker.add_updater(lambda m: m.set_value(get_output()))
    ...
    def setup_tip(self):
        ...
        tip.add_updater(lambda m: m.move_to(self.get_output_point(), DOWN))
        # tip.set_color(WHITE)
        tip.set_fill(GREY, 1)
    ...
    def setup_tip_label(self):
        ...
        tip_label.add_updater(
            lambda m: m.next_to(
                tip, UP, buff=2 * SMALL_BUFF, index_of_submobject_to_align=0
            )
        )
        ...
        decimal.add_updater(lambda m: m.move_to(template_decimal, LEFT))
        tip_label.add(decimal)

        self.tip_label = tip_label

    def setup_output_label(self):
        label = VGroup(OldTex("="), DecimalNumber(1, **self.output_label_config))
        label.set_color(POSITION_COLOR)
        label.add_updater(lambda m: m[1].set_value(self.get_output()))
        label.add_updater(lambda m: m.arrange(RIGHT, buff=SMALL_BUFF))
        label.add_updater(
            lambda m: m.next_to(
                self.tip_label,
                RIGHT,
    ...
    def setup_vectors(self):
        ...
        def update_velocity_vect(vect):
            vect.put_start_and_end_on(
                number_line.cn2p(0), number_line.cn2p(self.const * self.get_output())
            )
            vect.shift(number_line.cn2p(self.get_output()) - number_line.n2p(0))
            return vect
    ...
    def let_time_pass(self):
        ...
        t_tracker.add_updater(lambda m, dt: m.increment_value(rate * dt))
        ...
        # For first zoom
        xs1 = range(25, 100, 25)
        nl.add(*[nl.get_tick(x, size=1.5) for x in xs1])
        nl.add_numbers(
            *xs1,
            buff=2,
            height=5,
        ...
        nl.add(*[nl.get_tick(x, size=15) for x in xs2])
        nl.add_numbers(
            *xs2,
            height=50,
            buff=20,
        ...
        nl.add(*[nl.get_tick(x, size=150) for x in xs3])
        nl.add_numbers(
            *xs3,
            height=300,
            buff=200,
...
class ConstantEquals2(IntroducePhysicalModel):
    ...
    def add_initial_objects(self):
        deriv, ic = self.get_deriv_and_ic(const=self.const)
        self.deriv = deriv
        self.ic = ic
        self.add(ic)
    ...
    def point_out_chain_rule(self):
        ...
        words = VGroup(*map(TexText, ["Velocity", self.const_text, "Position"]))
        ...
        self.play(
            ShowCreationThenFadeAround(
                v_part[-2],
                surrounding_rectangle_config={"color": CONST_COLOR},
            )
        )
        self.wait()
        self.play(
        ...
        self.play(Restore(c_part, path_arc=TAU / 6), FadeIn(dot))
        self.play(
            FadeIn(c_rect),
            FadeIn(c_word),
        )
    ...
    def let_time_pass(self):
        ...
        nl.add(*[nl.get_tick(x, size=1500) for x in xs3])
        nl.add_numbers(
            *xs3,
            buff=2000,
            height=3000,
        ...
        self.play(VFadeIn(self.output_label))
        for x in range(4):
            self.wait_until(lambda: self.position_vect.get_end()[0] > 0)
            self.play(
                nl.scale,
                0.1,
                {"about_point": zero_point},
...
class NegativeConstant(ConstantEquals2):
    ...
    def show_vector_manipulation(self):
        ...
        v_vect.add_updater(lambda m: m.shift(SMALL_BUFF * UP))
    ...
    def let_time_pass(self):
        ...
        new_ticks = VGroup(*[nl.get_tick(x) for x in np.arange(0.1, 1.5, 0.1)])
...
class ImaginaryConstant(ConstantEquals2):
    ...
    def add_initial_objects(self):
        ...
        colors = [VELOCITY_COLOR, CONST_COLOR, POSITION_COLOR]
        labels = VGroup(*map(TexText, ["Velocity", self.const_text, "Position"]))
        ...
        self.play(ShowCreation(p_rect), FadeIn(p_label, UP))
    ...
    def show_vector_manipulation(self):
        ...
        def update_rot_v_label(label):
            ...
            label.next_to(self.velocity_vect.get_end(), RIGHT, buff=SMALL_BUFF)
        ...
        self.play(
            ...
            self.tip_label.shift_vect.move_to,
            0.1 * UP + 0.2 * RIGHT,
        )
        self.play(
            FadeOut(
    ...
    def transition_to_complex_plane(self):
        ...
        background_rects = VGroup(
            *[BackgroundRectangle(rect) for rect in self.term_rects]
        )
        ...
        corner_group.target[-1].next_to(
            corner_group.target, DOWN, buff=0.75, aligned_edge=LEFT
        )
        ...
        nl.target.scale(plane.unit_size / nl.unit_size)
        ...
        self.play(Restore(plane, lag_ratio=0.01), FadeInFromDown(plane_title))
    ...
    def show_vector_field(self):
        ...
        # Initial examples
        n = 12
        zs = [np.exp(complex(0, k * TAU / n)) for k in range(n)]
        ...
        attached_vects = VGroup(
            *[vect.copy().shift(vect.get_vector()) for vect in vects]
        )
        attached_vects.set_color(VELOCITY_COLOR)
        v_vects = VGroup(
            *[
                av.copy().rotate(90 * DEGREES, about_point=av.get_start())
        ...
        for vf in [vf0, vf1, vf2]:
            vf.submobjects.sort(key=lambda m: get_norm(m.get_start()))
        ...
        self.play(Transform(vf0, vf1, path_arc=90 * DEGREES))
        self.play(
            Transform(
                vf0,
                vf2,
    ...
    def show_circle(self):
        ...
        self.play(
            Transform(
                ic[-1].copy(), output_label.deepcopy().clear_updaters(), remover=True
            ),
...
class PiMinuteMark(Scene):
    def construct(self):
        text = OldTex("\\pi \\text{ minutes} \\approx \\text{3:08}")
...
class ReferenceWhatItMeans(PiCreatureScene):
    def construct(self):
        ...
        epii = OldTex("e^{{i} \\pi} = -1", **tex_config)
        epii.scale(2)
        ...
        for mob in group[1:]:
            mob.shift((epii[0].get_bottom() - mob[0].get_bottom())[1] * UP)
        group.to_edge(UP)
        ...
        nonsense = OldTexText("\\dots because that's " "literal nonsense!")
...
class Thumbnail(Scene):
    def construct(self):
        ...
        p_vect = Arrow(
            plane.n2p(0), plane.n2p(1), buff=0, fill_color=GREY_A, thickness=0.1
        )
        v_vect = Arrow(
            plane.n2p(1),
