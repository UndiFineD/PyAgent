# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part2\heat_equation.py
...
class TwoDBodyWithManyTemperatures(ThreeDScene):
    ...
    def introduce_body(self):
        ...
        self.play(
            ...
            Write(plate_words, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)),
        )
        self.wait()
    ...
    def show_temperature_at_all_points(self):
        ...
        dot.add_updater(
            lambda p: p.move_to(body[-1] if (1 < len(body)) else start_corner)
        )
...
class BringTwoRodsTogether(Scene):
    ...
    def get_second_derivative(self, x, dx=0.001):
        ...
        ly, y, ry = [
            graph.point_from_proportion(inverse_interpolate(x_min, x_max, alt_x))[1]
...
class TalkThrough1DHeatGraph(ShowEvolvingTempGraphWithArrows, SpecialThreeDScene):
    ...
    def show_changes_over_time(self):
        ...
        self.play(
            self.get_graph_time_change_animation(graph, time),
        ...
        self.play(
            ShowCreationThenFadeAround(new_t_label.get_part_by_tex("t")),
        )
        self.wait()
        self.play(
            FadeOut(clock),
    ...
    def show_surface(self):
        ...
        slicing_plane.replace(
            Line(axes_copy.c2p(0, 0), axes_copy.c2p(10, 100)), stretch=True
        )
    ...
    def get_graph_time_change_animation(self, graph, new_time, **kwargs):
        ...
        return UpdateFromAlphaFunc(
            graph,
            lambda g, a: g.become(self.get_graph(interpolate(old_time, new_time, a))),
            **config,
        )
...
class DiscreteSetup(ShowEvolvingTempGraphWithArrows):
    ...
    def focus_on_three_points(self):
        ...
        x_labels, T_labels = [
            VGroup(*[OldTex("{}_{}".format(s, i)) for i in [1, 2, 3]]).scale(0.8)
            for s in ("x", "T")
        ]
        ...
        self.play(
            LaggedStart(
                *[TransformFromCopy(xl, Tl) for xl, Tl in zip(x_labels, T_labels)],
                lag_ratio=0.3,
                run_time=2,
            )
        )
        ...
        average_pointer.add_updater(
            lambda m: m.move_to(0.5 * (hl1.get_start() + hl3.get_start()), RIGHT)
        )
        average_arrows = always_redraw(
            lambda: VGroup(
                *[
                    Arrow(
                        hl.get_start(),
        ...
        self.play(
            ReplacementTransform(
                average_label.deepcopy(), formula[1 : 1 + len(average_label)]
            ),
...
class ShowFinitelyManyX(DiscreteSetup):
    def construct(self):
        ...
        x_labels = VGroup(
            *[OldTex("x_{}".format(i)).next_to(p, DOWN) for i, p in enumerate(points)]
        )
