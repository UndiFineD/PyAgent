# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part2\staging.py
...
class PartTwoOfTour(TourOfDifferentialEquations):
    ...
    def zoom_in_to_one_thumbnail(self):
        ...
        self.play(
            ApplyMethod(
                frame.replace, thumbnails[1], path_arc=(-30 * DEGREES), run_time=3
            ),
        )
        self.wait()
            ...
            for i in [0, 1]:
                if abs(c1[i]) + r1 > self.L:
                    ...
                    p1.velocity[i] *= -1 * op.mul(
                        np.sign(p1.velocity[i]), np.sign(c1[i])
                    )
...
class TwoBodyEquations(Scene):
    def construct(self):
        ...
        self.play(LaggedStart(*[FadeIn(v, RIGHT) for v in variables]))
        self.wait()
        self.play(
        ...
        self.wait()
        self.play(LaggedStart(*[ShowCreationThenFadeAround(lhs[:3]) for lhs in lhss]))
        self.wait()
        self.play(
            LaggedStartMap(
...
class LaplacianIntuition(SpecialThreeDScene):
    ...
    def construct(self):
        ...
        def func(x, y):
            return np.array(
                [x, y, 2.7 + 0.5 * (np.sin(x) + np.cos(y)) - 0.025 * (x**2 + y**2)]
            )
        ...
        surface_circle = always_redraw(
            lambda: circle.copy()
            .apply_function(lambda p: func(*p[:2]))
            .shift(0.02 * IN)
