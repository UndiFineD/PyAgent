...
class RelationToOtherVideos(Scene):
    ...
    def get_video_thumbnails(self):
        ...
        for thumbnail in thumbnails:
            ...
            thumbnail.add(
                SurroundingRectangle(thumbnail, color=WHITE, stroke_width=2, buff=0)
            )
    ...
    def show_cosine_waves(self):
        ...
        for k, axes, color in zip(it.count(1), axes_group, colors):
            ...
            graph = axes.get_graph(
                lambda x: self.trig_func(x * k * PI / L), color=color
            )
...
class ShowInfiniteSum(FourierSeriesIllustraiton):
    ...
    def walk_through_constants(self):
        ...
        rects = VGroup(
            *[
                SurroundingRectangle(term, color=term[-1].get_color(), stroke_width=2)
                for term in terms
            ]
        )
    ...
    def follow_single_inputs(self):
        ...
        def get_brace_value_label(brace, u, n):
            ...
            result.add_updater(
                lambda d: d.set_value((u / n) * np.cos(PI * n * get_input()))
            )
            return result
