...
class ComplexFourierSeriesExample(FourierOfTrebleClef):
    ...
    def transition_to_alt_path(self, new_path, morph_path=False):
        ...
        for vect, new_vect in zip(vectors, new_vectors):
            ...
            anims.append(
                ApplyMethod(line.put_start_and_end_on, *new_vect.get_start_and_end())
            )
    ...
    def get_top_row_labels(self, vector_copies):
        ...
        for vector_copy in vector_copies:
            ...
            label = Integer(freq)
            label.move_to(
                np.array([freq * self.top_row_x_spacing, self.top_row_label_y, 0])
            )
            labels.add(label)
        return labels
    ...
    def focus_on_vector_pair(self):
        ...
        def get_opacity_animation(i1, i2, alpha_func):
            ...
            return AnimationGroup(
                UpdateFromAlphaFunc(
                    VectorizedPoint(), lambda m, a: v_group.set_opacity(alpha_func(a))
                ),
                UpdateFromAlphaFunc(
                    VectorizedPoint(),
...
class LabelRotatingVectors(PiFourierSeries):
    ...
    def ask_about_labels(self):
        ...
        q_marks = VGroup(
            *[OldTex("??").scale(1.0).next_to(circle, DOWN) for circle in circles]
        )
    ...
    def show_complex_exponents(self):
        ...
        self.play(
            ShowCreation(circle_copy), arclen_tracker.set_value, 2 * TAU, run_time=5
        )
        self.wait(3)
        self.play(
            FadeOut(circle_copy),
    ...
    def get_vector_in_plane_group(self, top_vector, top_circle):
        ...
        vector.add_updater(
            lambda v: v.put_start_and_end_on(
                origin, plane.n2p(2 * top_vector.coefficient)
...
class IntegralTrick(LabelRotatingVectors, TRangingFrom0To1):
    ...
    def issolate_c0(self):
        ...
        self.play(
            opacity_tracker.set_value, 0.2, FadeOut(self.drawn_path), FadeIn(path)
        )
    ...
    def show_center_of_mass(self):
        ...
        self.wait()
        self.play(
            *[Transform(od.copy(), v0_dot.copy(), remover=True) for od in output_dots]
        )
        self.add(v0_dot)
        self.wait()
...
class IncreaseOrderOfApproximation(ComplexFourierSeriesExample):
    ...
    def construct(self):
        ...
        n_label.add_updater(
            lambda n: n[1].set_value(n_tracker.get_value()).align_to(n[2], DOWN)
        )
...
class ShowStepFunctionIn2dView(
    SimpleComplexExponentExample, ComplexFourierSeriesExample
):
    CONFIG = {
        "input_space_rect_config": {
            "width": 5,
    ...
    def show_sum(self):
        ...
        def update_vector(vector):
            vector.coefficient = interpolate(
                1, vector.original_coefficient, coef_tracker.get_value()
            )
