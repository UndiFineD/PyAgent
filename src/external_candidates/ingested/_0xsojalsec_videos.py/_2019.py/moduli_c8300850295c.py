# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\moduli.py
...
class TriangleModuliSpace(Scene):
    ...
    def show_meaning_of_similar(self):
        ...
        per_class_dots = VGroup(
            *[
                OldTex("\\vdots").move_to(tri_class).set_y(rects.get_bottom()[1] + 0.4)
                for tri_class in tri_classes.target
            ]
        )
        ...
        dots = VGroup(
            *[
                Dot(color=tri.get_color()).move_to(
                    self.get_triangle_x(tri) * RIGHT + self.get_triangle_y(tri) * UP,
                )
        ...
        self.play(
            LaggedStart(
                *[
                    ReplacementTransform(tri_class.copy().set_fill(opacity=0), dot)
    ...
    def show_xy_rule(self):
        ...
        self.wait()
        self.add(
            obtuse_region, coord_label, x_line, y_line, xpy1_line, x_eq_y_line, dot
        )
        self.play(
            tip_tracker.shift,
            1.5 * DOWN,
    ...
    def get_triangles_and_classes(self):
        original_triangles = VGroup(*[self.get_random_triangle() for x in range(5)])
