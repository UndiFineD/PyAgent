# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\spirals.py
import json
import numbers
...
class PrimesAndPi(Scene):
    ...
    def show_primes(self):
        ...
        matrix = IntegerMatrix(
            [[n_cols * x + y for y in range(n_cols)] for x in range(n_rows)]
        )
        ...
        non_primes = VGroup(*filter(lambda m: not is_prime(m.get_value()), numbers))
...
class RefresherOnPolarCoordinates(Scene):
    ...
    def show_polar_coordinates(self):
        ...
        theta_coord.add_updater(
            lambda d: d.set_value((angle_of_vector(dot.get_center()) % TAU))
        )
        coord_label[-1].add_updater(lambda m: m.next_to(theta_coord, RIGHT, SMALL_BUFF))
        ...
        r_coord_copy.add_updater(lambda m: m.next_to(r_line.get_center(), UL, buff=0))
        ...
        self.play(
            FadeOut(VGroup(degree_label, degree_cross), DOWN), FadeIn(theta_coord)
        )
        self.play(FadeIn(radians_word))
        self.wait()

        # Move point around
        r_line.add_updater(lambda l: l.put_start_and_end_on(ORIGIN, dot.get_center()))
        theta_tracker = ValueTracker(0)
        theta_tracker.add_updater(lambda m: m.set_value(r_line.get_angle() % TAU))
        self.add(theta_tracker)
        arc.add_updater(lambda m: m.become(self.get_arc(theta_tracker.get_value())))
    ...
    def show_all_nn_tuples(self):
        ...
        self.play(ReplacementTransform(one_rect, one_r_rect), ShowCreation(r_line))
        self.wait()
        # self.play(TransformFromCopy(r_line, pre_arc))
        # self.add(pre_arc, one)
        self.play(
            ReplacementTransform(Line(*r_line.get_start_and_end()), arc),
    ...
    def get_nn_point(self, n):
        ...
        coord_label = self.get_coord_label(
            n, n, include_background_rectangle=False, num_decimal_places=0
        )
    ...
    def get_coord_label(
        ...
        **decimal_kwargs,
        ...
        coord_label = VGroup(OldTex("("), x_coord, OldTex(","), y_coord, OldTex(")"))
...
class IntroducePrimePatterns(SpiralScene):
    ...
    def show_clumps_of_four(self):
        ...
        for n in range(71):
            ...
            for k in [-3, -1, 1, 3]:
                ...
                group.add(self.get_arithmetic_sequence_line(710, r, self.big_n_primes))
    ...
    def get_arithmetic_sequence_line(self, N, r, max_val, skip_factor=5):
        ...
        line.set_points_smoothly(
            [self.get_polar_point(x, x) for x in range(r, max_val, skip_factor * N)]
        )
        return line
...
class AskAboutRelationToPrimes(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeInFromDown, q_marks[0]), Blink(self.teacher))
        self.wait(3)
...
class HighlightGapsInSpirals(IntroducePrimePatterns):
    def construct(self):
        ...
        gaps = always_redraw(
            lambda: VGroup(
                *[self.get_highlighted_gap(n - 1, n + 1, get_max_n()) for n in [11, 33]]
            )
        )
...
class ShowSpiralsForWholeNumbers(CountSpirals):
    ...
    def count_44_spirals(self):
        ...
        new_spirals = PGroup(
            *[
                self.get_p_spiral(range((INV_7_MOD_44 * k) % 44, self.max_prime, 44))
                for k in range(44)
            ]
        )
...
class ExplainSixSpirals(ShowSpiralsForWholeNumbers):
    ...
    def comment_on_arms(self):
        ...
        formula = VGroup(*Tex("6k", "+"), Integer(1))
        ...
        self.play(
            ...
            *[ApplyMethod(group.set_opacity, 0.25) for group in six_groups[1:]],
            run_time=2,
        )
        self.play(
            LaggedStartMap(
    ...
    def talk_though_multiples_of_six(self):
        ...
        boxes = VGroup(*[VGroup(b.copy(), l.copy()) for b, l in zip(spiral, labels)])
        boxes.set_opacity(1)
        ...
        six = boxes[6][1]
        rhs = OldTex("\\text{radians}", "\\approx", "2\\pi", "\\text{ radians}")
...
class IntroduceResidueClassTerminology(Scene):
    ...
    def add_sequences(self):
        ...
        labels = VGroup(*[OldTex("6k + {}:".format(r)) for r in range(6)])
    ...
    def add_terms(self):
        ...
        terms = OldTexText("``", "Residue\\\\", "classes\\\\", "mod ", "6''")
        terms.scale(1.5)
        ...
        mod_def = OldTexText("``where the thing\\\\you divide by is''")
        mod_def.next_to(mod_brace, DOWN, SMALL_BUFF)

        arrows = VGroup(
            *[Arrow(terms.get_left(), sequence.get_right()) for sequence in sequences]
        )
        ...
        self.terminology = VGroup(
            terms, res_brace, remainder, mod_brace, mod_def, arrows
        )
    ...
    def highlight_example(self):
        ...
        label = labels[r]
        r_tex = label[0][3]
        n_rects = VGroup(*[SurroundingRectangle(num) for num in sequence.numbers])
    ...
    def simple_english(self):
        ...
        for x in range(2):
            self.play(Blink(randy))
            self.wait()
        self.play(FadeOut(new_phrase), FadeIn(terminology), FadeOut(randy, DOWN))
        self.wait()
        self.wait(6)
...
class Explain44Spirals(ExplainSixSpirals):
    ...
    def show_44_steps(self):
        ...
        def show_steps(start, stop, added_anims=None, run_time=2):
            ...
            self.add_count_clicks(N=(stop - start), time=run_time, rate_func=rate_func)
    ...
    def show_top_right_arithmetic(self):
        ...
        next_line = OldTex("=", "44", "/", "2\\pi", "\\text{ rotations}")
    ...
    def count_by_44(self):
        ...
        for label, point in zip(labels, points):
            label.point = point
            label.add_updater(
                lambda l: l.set_height(frame.get_height() * get_l_height_ratio())
            )
            label.add_updater(
                lambda l: l.move_to(
                    l.point + get_p2l() * UP,
...
class Label44Spirals(Explain44Spirals):
    ...
    def setup_spirals(self):
        ...
        self.set_scale(spiral=VGroup(*it.chain(*spirals)), scale=scale, run_time=0)
    ...
    def get_spiral_arm_labels(self, spirals, index=15):
        ...
        labels = VGroup(
            *[
                VGroup(*Tex("44k", "+"), Integer(n)).arrange(RIGHT, buff=SMALL_BUFF)
                for n in range(mod)
            ]
        )
...
class ResidueClassMod44Label(Scene):
    def construct(self):
        text = OldTexText("``Residue class mod 44''")
        text.scale(2)
        text.to_corner(UL)
...
class EliminateNonPrimativeResidueClassesOf44(Label44Spirals):
    ...
    def eliminate_classes(self):
        ...
        admissible_spirals = VGroup(
            *[spiral for n, spiral in enumerate(spirals) if n % 2 != 0 and n % 11 != 0]
        )
...
class IntroduceTotientJargon(TeacherStudentsScene):
    ...
    def add_title(self):
        ...
        self.play(
            ...
            LaggedStart(*[FadeOut(pi, 4 * DOWN) for pi in self.pi_creatures]),
    ...
    def eliminate_non_coprimes(self):
        number_grid = VGroup(
            *[
                VGroup(*[Integer(n) for n in range(11 * k, 11 * (k + 1))]).arrange(DOWN)
                for k in range(4)
            ]
        ...
        # Show coprimes
        self.play(ShowIncreasingSubsets(numbers, run_time=3), FadeIn(words, LEFT))
        self.wait()
        ...
        self.play(GrowFromCenter(brace), FadeIn(etf, UP))
        self.wait()
        self.play(
        ...
        arrows = VGroup(
            *[Arrow(totatives.get_top(), coprime.get_bottom()) for coprime in coprimes]
        )
...
class TwoUnrelatedFacts(Scene):
    ...
    def show_columns(self):
        ...
        approx = OldTex("{44 \\over 7} \\approx 2\\pi")
        approx.scale(1.5)
        approx.next_to(
...
class ExplainRays(Explain44Spirals):
    ...
    def show_arithmetic(self):
        ...
        self.play(FadeIn(equation[3:], LEFT))
        ...
        self.wait()
        self.play(
            FadeOut(VGroup(rect, equation, brace, values[-1], approx, approx2)),
    ...
    def zoom_and_count(self):
        ...
        def update_arrow(arrow):
            ...
            arrow.put_start_and_end_on(
                label.get_bottom() + SMALL_BUFF * DOWN, point + SMALL_BUFF * UP
            )
        ...
        new_label = OldTex("710", "k", "+", tex_to_color_map={"k": YELLOW})
...
class ShowClassesOfPrimeRays(SpiralScene):
    ...
    def setup_rays(self):
        ...
        self.add(spiral)
        self.set_scale(
            scale=self.scale, spiral=spiral, target_p_spiral_width=3, run_time=0
        )
    ...
    def show_classes(self):
        ...
        for i in range(40):
            ...
            label = VGroup(*Tex("710k", "+"), Integer(r))
...
class EliminateNonPrimative710Residues(ShowClassesOfPrimeRays):
    ...
    def eliminate_classes(self):
        ...
        pre_label, r_label = label = VGroup(OldTex("710k + "), Integer(100))
        label.scale(1.5)
        label.arrange(RIGHT, buff=SMALL_BUFF)
...
class DiscussPrimesMod10(Scene):
    def construct(self):
        labels = VGroup(*[OldTexText(str(n), " mod 10:") for n in range(10)])
        ...
        self.play(LaggedStart(*[FadeIn(label, UP) for label in labels]))
        self.wait()
        self.play(
            LaggedStart(
                *[
                    LaggedStart(*[FadeIn(item, LEFT) for item in sequence])
                    for sequence in sequences
                ]
            )
        )
        ...
        for sequence in sequences[:2]:
            ...
            rects = VGroup(*[SurroundingRectangle(ld, buff=0.05) for ld in lds])
        ...
        for prime, group in [(two, evens), (five, div5)]:
            ...
            self.play(
                LaggedStart(*[ApplyMethod(item.set_opacity, 0.2) for item in group])
            )
            self.wait()
        ...
        self.play(
            ...
            LaggedStart(
                *[
                    ApplyFunction(lambda m: m.scale(0.8).set_opacity(0.8), non_prime)
...
class PhraseDirichletsTheorem(DirichletIn1837):
    def construct(self):
        ...
        self.play(FadeIn(phi_N_label, DOWN))
        self.wait()
...
class MoreModestDirichlet(Scene):
    def construct(self):
        ed = OldTexText(
            "Each (coprime) residue class ", "is equally dense with ", "primes."
        )
        inf = OldTexText(
            "Each (coprime) residue class ", "has infinitely many ", "primes."
        )
        ed[1].set_color(BLUE)
...
class HighlightTwinPrimes(Scene):
    ...
    def show_twin_primes(self):
        ...
        prime_mobs = VGroup(*[Integer(p).next_to(line.n2p(p), UP) for p in primes])
...
class RandomToImportant(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(
            morty.change, "raise_left_hand", left_comment, FadeInFromDown(left_comment)
        )
        self.wait(2)
        self.play(
            morty.change,
...
class RandomWalkOfTopics(Scene):
    ...
    def setup_network(self):
        ...
        def add_edge(d1, d2):
            if d1 is d2:
                return
            edge = Line(d1.get_center(), d2.get_center(), buff=d1.get_width() / 2)
            d1.edges.add(edge)
            d2.edges.add(edge)
            d1.neighbors.add(d2)
            d2.neighbors.add(d1)
