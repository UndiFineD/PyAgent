...
class ClosedLoopScene(Scene):
    ...
    def get_line_anims(self):
        return [line.update_anim for line in self.connecting_lines] + [
            Animation(self.dots)
        ]
    ...
    def set_color_dots_by_pair(self):
        n_pairs = len(list(self.dots)) / 2
        for d1, d2, c in zip(
            self.dots[:n_pairs], self.dots[n_pairs:], self.pair_colors
        ):
...
class WhenIWasAKid(TeacherStudentsScene):
    ...
    def grow_up(self):
        ...
        self.play(
            ...
            *list(
                map(
                    FadeOut, [self.students, self.teacher, me.bubble, me.bubble.content]
                )
            ),
        )
        self.remove(me)
...
class RectangleProperties(Scene):
    def construct(self):
        ...
        vertex_dots = VGroup(
            *[Dot(anchor, color=YELLOW) for anchor in rect.get_anchors_and_handles()[0]]
        )
        dot_pairs = [
...
class PairOfPairBecomeRectangle(Scene):
    def construct(self):
        ...
        for dot, char in zip(dots, "acbd"):
            ...
            labels.add(label)
        lines = [
            Line(dots[i].get_center(), dots[j].get_center(), color=dots[i].get_color())
            for i, j in [(0, 1), (2, 3)]
        ]
        groups = [
        ...
        self.play(
            *[
                ApplyMethod(
                    group.shift, -group[-1].get_center() + midpoint.get_center()
                )
                for group in groups
            ]
        )
...
class DeclareFunction(ClosedLoopScene):
    def construct(self):
        ...
        self.set_color_dots_by_pair()
        self.add_connecting_lines()
        VGroup(self.loop, self.dots, self.connecting_lines).scale(0.7).to_edge(
            LEFT
        ).shift(DOWN)
...
class SeekSurfaceForPairs(ClosedLoopScene):
    def construct(self):
        ...
        for anim in anims:
            self.let_dots_wonder(
                random_seed=1, added_anims=[anim], run_time=anim.run_time
            )
...
class DefineUnorderedPair(ClosedLoopScene):
    def construct(self):
        ...
        for x in range(2):
            self.play(
                dots[0].move_to, dots[1], dots[1].move_to, dots[0], path_arc=np.pi / 2
            )
            self.wait()
...
class RepresentPairInUnitSquare(ClosedLoopScene):
    def construct(self):
        ...
        dashed_lines = VGroup(
            *[DashedLine(dot, inner_dot, color=dot.get_color()) for dot in self.dots]
        )
        self.play(ShowCreation(dashed_lines))
        self.play(ShowCreation(inner_dot))
        self.play(
            FadeIn(square), Animation(self.dots), *list(map(Write, right_words[1:]))
        )
        self.wait()
        ...
        for shift_val in shift_vals:
            ...
            for line, dot in zip(dashed_lines, self.dots):
                line.target.put_start_and_end_on(
                    dot.target.get_center(), inner_dot.target.get_center()
                )
...
class EdgesOfSquare(Scene):
    ...
    def get_coordinate_labels(self, square):
        ...
        label_groups = [
            VGroup(*[OldTex("(%s, %s)" % (a, b)).scale(0.7) for b in alpha_range])
            for a in (0, 1)
        ]
...
class EndpointsGluedTogether(ClosedLoopScene):
    def construct(self):
        ...
        self.transform_loop(
            original_loop,
            added_anims=[
                ApplyMethod(dot.move_to, original_loop.get_points()[0]) for dot in dots
            ],
            run_time=3,
        )
        self.wait()
...
class FoldUnitSquare(EdgesOfSquare):
    ...
    def get_point_from_coords(self, x, y):
        left, right, bottom, top = [
            self.triangles.get_edge_center(vect) for vect in (LEFT, RIGHT, DOWN, UP)
        ]
...
class CreditThree(Scene):
    def construct(self):
        ...
        self.play(Blink(randy))
        self.play(
            randy.change_mode, "hooray", randy.look_at, logo_dot, FadeOut(domains)
        )
        self.wait()
