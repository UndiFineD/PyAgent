# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\zeta.py
import mpmath
...
class ZetaTransformationScene(ComplexTransformationScene):
    ...
    def get_reflected_plane(self):
        ...
        reflected_plane.submobjects = list(
            reversed(reflected_plane.family_members_with_points())
        )
...
class IntroduceZeta(ZetaTransformationScene):
    ...
    def construct(self):
        ...
        func_mob = VGroup(
            OldTex("\\zeta(s) = "), OldTex("\\sum_{n=1}^\\infty \\frac{1}{n^s}")
        )
...
class WhyPeopleMayKnowIt(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.student_thinks(
            divergent_sum, index=1, added_anims=[student.change_mode, "plain"]
        )
    ...
    def plug_in_two(self, zeta_def):
        ...
        for frac, brace, line in zip(fracs, braces, lines):
            ...
            if frac is fracs[-1]:
                ...
                arrow = Arrow(
                    frac.get_bottom(), brace.get_top(), tip_length=0.1, buff=0.1
                )
        ...
        for frac, brace, line in zip(fracs, braces, lines):
            self.play(
                Write(frac), GrowFromCenter(brace), ShowCreation(line), run_time=0.7
            )
            self.wait(0.7)
        self.wait()
...
class IgnoreNegatives(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            GrowFromCenter(brace),
            Write(only_s_gt_1),
            *it.chain(*[[pi.look_at, definition] for pi in self.get_pi_creatures()]),
        )
        self.random_blink(3)
...
class RiemannFatherOfComplex(ComplexTransformationScene):
    def construct(self):
        ...
        output_dot = Dot(
            arc.get_points()[-1] + SMALL_BUFF * (2 * RIGHT + DOWN), color=MAROON_B
        )
...
class FromRealToComplex(ComplexTransformationScene):
    ...
    def show_real_to_real(self):
        ...
        number_line = NumberLine(
            unit_size=2, tick_frequency=0.5, big_tick_numbers=list(range(-2, 3))
        )
        number_line.add_numbers()
        ...
        self.wait(2)
        self.play(
            ShowCreation(self.background, run_time=2),
    ...
    def show_s_equals_two_lines(self):
        ...
        self.play(
            self.input_dot.shift,
            2 * DOWN,
            self.input_label.fade,
            0.7,
        )
    ...
    def ask_about_visualizing_all(self):
        ...
        self.play(
            morty.change_mode, "speaking", ShowCreation(bubble), Write(bubble.content)
        )
        ...
        self.play(
            ...
            morty.look_at,
            self.input_dot,
            *list(map(FadeOut, [bubble, bubble.content, self.domain_words])),
        )
    ...
    def get_sum_lines(self, exponent, line_thickness=6):
        powers = [0] + [
            x ** (-exponent) for x in range(1, self.num_lines_in_spiril_sum)
        ]
        power_sums = np.cumsum(powers)
        lines = VGroup(
            *[
...
class TerritoryOfExponents(ComplexTransformationScene):
    def construct(self):
        ...
        full_plane = Rectangle(
            width=FRAME_WIDTH, height=FRAME_HEIGHT, fill_color=YELLOW, fill_opacity=0.3
        )
...
class ComplexExponentiation(Scene):
    ...
    def extract_pure_imaginary_part(self):
        ...
        self.wait()
        self.play(
            GrowFromCenter(brace), FadeIn(we_understand), real_part.set_color, GREEN_B
        )
        self.wait()
        self.play(
            imag_part.move_to,
    ...
    def show_imaginary_powers(self):
        ...
        def output_dot_update(ouput_dot):
            ...
            output_dot.move_to(
                self.z_to_point(curr_base ** complex(0, y), is_input=False)
            )
            return output_dot
...
class ComplexFunctionsAsTransformations(ComplexTransformationScene):
    ...
    def get_dots(self):
        ...
        output_dots = VGroup(
            *[Dot(ip + on) for ip, on in zip(input_points, output_nudges)]
        )
...
class VisualizingSSquared(ComplexTransformationScene):
    ...
    def plug_in_specific_values(self):
        ...
        VGroup(
            *[
                ParametricCurve(lambda t: self.z_to_point(z ** (1.1 + 0.8 * t)))
                for z in inputs
            ]
        )
        for z, dot in zip(inputs, input_dots):
            ...
            stand_in_arrow = Arrow(
                arrow.get_points()[-2], arrow.get_points()[-1], tip_length=0.2
            )
    ...
    def show_transformation(self):
        ...
        self.apply_complex_homotopy(
            lambda z, t: z ** (1 + t),
            added_anims=[
                MoveAlongPath(dot, dot.path, run_time=5) for dot in self.input_dots
            ],
            run_time=5,
        )
        self.wait(2)
...
class ShowZetaOnHalfPlane(ZetaTransformationScene):
    ...
    def add_title(self):
        zeta = OldTex(
            ...
            "s",
            ")=",
            *["\\frac{1}{%d^s} + " % d for d in range(1, 5)] + ["\\cdots"],
        )
    ...
    def react_to_transformation(self):
        ...
        self.play(FadeIn(morty))
        self.play(
            morty.change_mode, "surprised", ShowCreation(bubble), Write(bubble.content)
        )
        self.play(Blink(morty))
    ...
    def show_cutoff(self):
        ...
        line = Line(
            *list(
                map(
                    self.z_to_point,
                    [complex(np.euler_gamma, u * FRAME_Y_RADIUS) for u in (1, -1)],
                )
            )
        )
    ...
    def set_color_i_line(self):
        right_i_lines, left_i_lines = [
            VGroup(
                *[
                    Line(
                        vert_vect + RIGHT, vert_vect + (FRAME_X_RADIUS + 1) * horiz_vect
                    )
    ...
    def show_continuation(self):
        ...
        self.play(
            ShowCreation(
                reflected_plane, run_time=6, rate_func=lambda t: 1 - there_and_back(t)
            )
        )
        self.wait(2)
    ...
    def restore_mobjects(self, *mobjects):
        self.play(
            *it.chain(*[[m.restore, m.make_smooth] for m in mobjects]), run_time=2
        )
        for m in mobjects:
            self.remove(m)
            m.restore()
...
class SquiggleOnExtensions(ZetaTransformationScene):
    ...
    def show_negative_one(self):
        ...
        self.apply_zeta_function(
            added_anims=[
                ApplyMethod(dot.move_to, self.z_to_point(-1.0 / 12), run_time=5)
            ]
        )
    ...
    def cycle_through_options(self):
        ...
        new_dots = [
            self.dot.copy().move_to(func(self.dot.get_center())) for func in funcs
        ]
        self.left_plane.save_state()
        for plane, dot in zip(new_left_planes, new_dots):
            self.play(
                Transform(self.left_plane, plane), Transform(self.dot, dot), run_time=3
            )
            self.wait()
...
class IntroduceAnglePreservation(VisualizingSSquared):
    ...
    def talk_about_derivative(self):
        ...
        for pi, words in (randy, "$f'(s) = 2s$"), (
            morty,
            "Here's some \\\\ related geometry...",
        ):
...
class NoteZetaFunctionAnalyticOnRightHalf(ZetaTransformationScene):
    ...
    def note_right_angles(self):
        intersection_inputs = [
            complex(x, y)
            for x in np.arange(1 + 2.0 / 16, 1.4, 1.0 / 16)
            for y in np.arange(-0.5, 0.5, 1.0 / 16)
            if abs(y) > 1.0 / 16
        ]
        brackets = VGroup(*list(map(self.get_right_angle_bracket, intersection_inputs)))
...
class MathematiciansLookingAtFunctionEquation(Scene):
    def construct(self):
        ...
        self.play(
            Transform(equation[0].copy(), equation[-1], path_arc=-np.pi / 3, run_time=2)
        )
        for mathy in mathys:
...
class DiscussZeros(ZetaTransformationScene):
    ...
    def ask_about_zeros(self):
        dots = VGroup(
            *[
                Dot((2 + np.sin(12 * alpha)) * rotate_vector(RIGHT, alpha + nudge))
    ...
    def show_trivial_zeros(self):
        ...
        for dot in self.dots.target:
            dot.move_to(ORIGIN)
        self.apply_zeta_function(
            added_anims=[MoveToTarget(self.dots, run_time=3)], run_time=3
        )
        self.wait(3)
        self.play(
            self.plane.restore, self.plane.make_smooth, self.dots.restore, run_time=2
        )
        self.remove(*self.get_mobjects_from_last_animation())
        self.plane.restore()
        self.dots.restore()
        ...
        self.play(FadeIn(randy))
        self.play(
            randy.change_mode, "confused", ShowCreation(bubble), Write(bubble.content)
        )
        self.play(Blink(randy))
        self.wait()
        self.play(Blink(randy))
        self.play(
            *list(map(FadeOut, [randy, bubble, bubble.content, trivial_zero_words]))
        )
    ...
    def show_critical_strip(self):
        ...
        new_dots = VGroup(
            *[Dot(0.5 * RIGHT + y * UP) for y in np.linspace(-2.5, 3.2, 5)]
        )
        ...
        self.wait(3)
        self.play(
            photo.shift, 7 * LEFT, *list(map(FadeOut, [primes, self.dots, strip]))
        )
        self.remove(photo)
    ...
    def give_dots_wandering_anims(self):
        ...
        for i, dot in enumerate(self.dots):
            ...
            dot.target.move_to(
                self.wandering_path.point_from_proportion(
    ...
    def get_dot_wandering_anims(self):
        ...
        return [
            MoveAlongPath(dot, self.wandering_path, rate_func=get_rate_func(i))
            for i, dot in enumerate(self.dots)
        ]
    ...
    def extend_transformed_critical_line(self):
        ...
        full_line = VGroup(
            *[ParametricCurve(func, t_min=t0, t_max=t0 + 1) for t0 in range(100)]
        )
...
class DiscussSumOfNaturals(Scene):
    def construct(self):
        ...
        neg_twelfth, eq, zeta_neg_1, sum_naturals = equation = OldTex(
            "-\\frac{1}{12}", "=", "\\zeta(-1)", "= 1 + 2 + 3 + 4 + \\cdots"
        )
        ...
        words = OldTexText(
            "If not equal, at least connected", "\\\\(see links in description)"
        )
...
class ZetaPartialSums(ZetaTransformationScene):
    ...
    def construct(self):
        ...
        interim_planes = [
            self.plane.copy().apply_complex_function(get_partial_sum_func(N))
            for N in N_list
        ]
