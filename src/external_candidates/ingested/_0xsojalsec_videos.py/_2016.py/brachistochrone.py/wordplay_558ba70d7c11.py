# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\brachistochrone\wordplay.py
...
class DisectBrachistochroneWord(Scene):
    def construct(self):
        ...
        chrono_arrow = Arrow(
            word.get_right(), chrono_example.get_bottom(), color=BLUE_D
        )
        ...
        brachy_arrow = Arrow(word.get_left(), brachy_example.get_top(), color=GREEN)
...
class OneSolutionTwoInsights(Scene):
    def construct(self):
        ...
        for word, man in [(two, johann), (insights, mark)]:
            self.play(Transform(word, Point(word.get_left())), GrowFromCenter(man))
            self.wait()
        self.clear()
...
class CircleOfIdeas(Scene):
    def construct(self):
        words = list(
            map(TexText, ["optics", "calculus", "mechanics", "geometry", "history"])
        )
        ...
        self.play(
            *[
                Transform(
                    word,
                    word.copy().set_color(BLACK).center().scale(0.1),
    ...
    def get_spinning_anims(self, words, angle=np.pi / 6):
        ...
        for word in words:
            ...
            anims.append(
                ApplyMethod(
                    word.shift, vect, path_func=path_along_arc(angle), rate_func=linear
                )
            )
        return anims
...
class FermatsPrincipleStatement(Scene):
    def construct(self):
        ...
        norms = np.apply_along_axis(get_norm, 1, everything.points)
        norms -= np.min(norms)
        norms /= np.max(norms)
        ...
        self.add(words)
        self.play(DelayByOrder(FadeIn(everything, run_time=3)), Animation(words))
        self.play(
...
class VideoProgression(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(ApplyMethod(brachy.shift, spacing), Transform(point, optics))
        optics = point
        arrow1 = Arrow(optics, brachy)
        self.play(ShowCreation(arrow1))
        self.wait()
        arrow2 = Arrow(light_in_two, optics)
        self.play(ShowCreation(arrow2), ShimmerIn(light_in_two))
        self.wait()
        self.play(
            FadeOut(light_in_two),
            GrowFromCenter(snells),
            DelayByOrder(ApplyMethod(arrow2.set_color, BLUE_D)),
        )
        self.wait()
        self.play(
            FadeOut(optics),
            GrowFromCenter(multi),
            DelayByOrder(ApplyMethod(arrow1.set_color, BLUE_D)),
        )
        self.wait()


class BalanceCompetingFactors(Scene):
    args_list = [("Short", "Steep"), ("Minimal time \\\\ in water", "Short path")]
    ...
    def construct(self, *words):
        ...
        self.rotate(-2)
        self.wait()
        self.play(Transform(factor1, real_factor1, path_func=path_along_arc(np.pi / 4)))
        self.rotate(2)
        self.wait()
        self.play(Transform(factor2, real_factor2, path_func=path_along_arc(np.pi / 4)))
        self.rotate(-2)
        self.wait()
        self.rotate(1)
...
class Section2(Scene):
    def construct(self):
        self.add(
            OldTexText(
                "Section 2: Mark Levi's insight, and a challenge", size="\\large"
            )
        )
        self.wait()
