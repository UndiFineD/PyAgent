# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\brachistochrone\curves.py
...
class Cycloid(ParametricCurve):
    ...
    def pos_func(self, t):
        T = t * self.end_theta
        return self.point_a + self.radius * np.array([T - np.sin(T), np.cos(T) - 1, 0])


class LoopTheLoop(ParametricCurve):
    CONFIG = {"color": YELLOW_D, "density": 10 * DEFAULT_POINT_DENSITY_1D}
...
class SlideWordDownCycloid(Animation):
    ...
    def __init__(self, word, **kwargs):
        ...
        for letter in word_mob.split():
            letter.center()
            letter.angle = 0
        unit_interval = np.arange(0, 1, 1.0 / len(word))
...
class BrachistochroneWordSliding(Scene):
    def construct(self):
        ...
        self.play(anim)
        self.wait()
        self.play(FadeOut(anim.path), ApplyMethod(anim.mobject.center))
...
class PathSlidingScene(Scene):
    ...
    def slide(self, mobject, path, roll=False, ceiling=None):
        ...
        while curr_index < len(points):
            ...
            self.adjust_mobject_to_index(self.slider, curr_index, points)
            if roll:
                distance = get_norm(points[curr_index] - points[last_index])
    ...
    def get_time_slices(self, points, ceiling=None):
        ...
        ds_list = np.apply_along_axis(get_norm, 1, points[1:] - points[:-1])
        if ceiling is None:
            ceiling = points[0, 1]
...
class TryManyPaths(PathSlidingScene):
    def construct(self):
        ...
        curr_path = None
        for path in paths:
            new_slider = self.adjust_mobject_to_index(randy.copy(), 1, path.points)
        ...
        for path in paths:
            self.play(
                Transform(
                    curr_path, path, path_func=path_along_arc(np.pi / 2), run_time=3
                )
            )
    ...
    def get_paths(self):
        ...
        paths = [
            Arc(angle=np.pi / 2, radius=3, start_angle=4),
            LoopTheLoop(),
            Line(7 * LEFT, 7 * RIGHT, color=RED_D),
            sharp_corner,
            FunctionGraph(lambda x: 0.05 * (x**2) + 0.1 * np.sin(2 * x)),
            FunctionGraph(
                lambda x: x**2, x_min=-3, x_max=2, density=3 * DEFAULT_POINT_DENSITY_1D
            ),
        ]
        cycloid = Cycloid()
...
class NotTheCircle(PathSlidingScene):
    def construct(self):
        ...
        self.play(
            ApplyMethod(
                path.put_start_and_end_on, start, end, path_func=path_along_arc(-angle)
            ),
            run_time=3,
        )
...
class MinimalPotentialEnergy(Scene):
    def construct(self):
        ...
        horiz_axis = NumberLine(numerical_radius=5, big_tick_numbers=[])
        ...
        self.play(ShimmerIn(nature_finds), ShowCreation(arrow), ShowCreation(min_point))
        self.wait()
        self.play(
            FadeOut(top_words[0]),
...
class WhatGovernsSpeed(PathSlidingScene):
    ...
    def show_geometry(self, slider, vector):
        ...
        vert_brace = Brace(
            Mobject(Point(ceil_point), Point(slider.get_center())), RIGHT, buff=0.5
        )
    ...
    def solve_energy(self):
        ...
        kinetic = OldTex(["\\dfrac{1}{2}", "m", "v", "^2", "="])
        ...
        self.wait()
        self.play(
            Transform(squared, sqrt, path_func=clockwise_path()),
...
class ThetaTInsteadOfXY(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(xy), ShowCreation(vect_mob))
        self.play(ShowCreation(arc), ShowCreation(vert_line), ShimmerIn(theta))
        self.wait()
