# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\brachistochrone\drawing_images.py
import inspect
...
def nearest_neighbor_align(mobject1, mobject2):
    ...
    for n in range(mobject1.get_num_points()):
        ...
        new_mob2.add_points(
            mobject2.get_points()[indices], rgbas=mobject2.rgbas[indices]
        )
...
class TracePicture(Scene):
    ...
    def get_edge_mobject(self, image_array):
        ...
        colored_edge_mobject_list = [
            MobjectFromPixelArray(colored_edge) for colored_edge in colored_edges
        ]
...
class JohannThinksHeIsBetter(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            Transform(johann, pensive_johann), Transform(point, bubble), run_time=2
        )
        self.remove(point)
        self.add(bubble)
        weakling = guys[1]
        self.play(
            FadeIn(comparitive_johann), ShowCreation(greater_than), FadeIn(weakling)
        )
        self.wait(2)
        for guy in guys[2:]:
