...
class MultilayeredScene(Scene):
    ...
    def get_layers(self, n_layers=None):
        ...
        rgb_range = [
            interpolate(*rgb_pair + [x]) for x in np.arange(0, 1, 1.0 / n_layers)
        ]
...
class TwoToMany(MultilayeredScene):
    ...
    def get_glass(self):
        return self.RectClass(
            height=FRAME_Y_RADIUS, width=FRAME_WIDTH, color=BLUE_E
...
class ShowDiscretePath(MultilayeredScene, PhotonScene):
    ...
    def construct(self):
        ...
        self.play(
            self.photon_run_along_path(
                self.discrete_path, rate_func=rush_into, run_time=3
            )
        )
        self.wait()
    ...
    def generate_discrete_path(self):
        ...
        self.discrete_path = Mobject1D(
            color=WHITE, density=3 * DEFAULT_POINT_DENSITY_1D
        )
        ...
        self.play(
            *it.chain(
                *[
                    [
                        GrowFromCenter(
                            mob, rate_func=squish_rate_func(smooth, start, 1)
                        )
...
class LimitingProcess(MultilayeredScene):
    ...
    def construct(self):
        num_iterations = 3
        layer_sets = [
            self.get_layers((2**x) * self.n_layers) for x in range(num_iterations)
        ]
        glass_sets = [
            Mobject(
                *[
...
class ShowLightAndSlidingObject(MultilayeredScene, TryManyPaths, PhotonScene):
    ...
    def construct(self):
        ...
        for path in paths:
            self.play(
                Transform(loop, path, path_func=path_along_arc(np.pi / 2), run_time=2)
            )
