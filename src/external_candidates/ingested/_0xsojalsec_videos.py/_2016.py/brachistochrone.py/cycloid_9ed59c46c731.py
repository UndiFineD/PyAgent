...
class CycloidScene(Scene):
    ...
    def generate_cycloid(self):
        self.cycloid = Cycloid(
            point_a=self.point_a, radius=self.radius, end_theta=self.end_theta
        )
...
class LeviSolution(CycloidScene):
    ...
    def show_similar_triangles(self):
        ...
        triangle1 = Polygon(
            self.p_point, self.c_point, self.bottom_point, color=MAROON, **kwargs
        )
    ...
    def rearrange(self):
        ...
        self.play(
            ...
            ShimmerIn(one_over),
            ApplyMethod(
                d_sqrt.next_to, one_over, DOWN, path_func=path_along_arc(-np.pi)
            ),
            ApplyMethod(
                y_sqrt.next_to, sin_over, DOWN, path_func=path_along_arc(-np.pi)
            ),
            run_time=2,
        )
        self.wait()
