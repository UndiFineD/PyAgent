...
class PhysicalIntuition(Scene):
    def construct(self):
        ...
        terms = OldTex(
            ["\\dfrac{1}{%d^2} + " % (x + 1) for x in range(n_terms)] + ["\\cdots"]
        )
        terms.shift(2 * UP)
...
class TimeLine(Scene):
    def construct(self):
        ...
        for point, event in zip(centers[1:], dated_events):
            self.play(
                ApplyMethod(
                    timeline.shift, -point.get_center(), run_time=next(run_times)
                )
            )
...
class StayedUpAllNight(Scene):
    def construct(self):
        ...
        ticks = Mobject(
            *[Line(1.8 * vect, 2 * vect, color=GREY) for vect in compass_directions(12)]
        )
        clock.add(ticks)
        ...
        lines = Mobject(
            *[
                Line(
                    lower_left,
                    lower_left
                    + 5 * r * np.cos(np.arctan(r)) * RIGHT
                    + 2 * r * np.sin(np.arctan(r)) * UP,
                )
                for r in r_range
            ]
        )
...
class VideoLayout(Scene):
    def construct(self):
        ...
        left_brace = Brace(
            Mobject(Point(left), Point(interpolate(left, right, 2.0 / 3))), DOWN
        )
        right_brace = Brace(
            Mobject(Point(interpolate(left, right, 2.0 / 3)), Point(right)), UP
        )
...
class MathBetterThanTalking(Scene):
    def construct(self):
        ...
        self.play(
            ApplyMethod(
                mathy.blink, rate_func=squish_rate_func(there_and_back, 0.4, 0.6)
            )
        )
...
class TalkedAboutSnellsLaw(Scene):
    def construct(self):
        ...
        for talker, phrase in zip(talkers, phrases):
            ...
            self.play(
                ApplyMethod(talker.blink, rate_func=squish_rate_func(there_and_back))
            )
            self.wait()
