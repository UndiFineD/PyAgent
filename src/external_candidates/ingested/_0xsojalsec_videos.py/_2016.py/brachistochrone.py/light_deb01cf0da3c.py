...
class PhotonThroughLens(MultipathPhotonScene):
    ...
    def get_paths(self):
        ...
        second_contact = [
            self.lens.point_from_proportion(0.3 * v + 0.1) for v in interval_values
        ]
        focal_point = 2 * RIGHT
        return [
            Mobject(
...
class PhotonOffMirror(MultipathPhotonScene):
    ...
    def get_paths(self):
        ...
        anchor_points = [
            self.mirror.point_from_proportion(0.6 * v + 0.3) for v in interval_values
        ]
        ...
        return [
            Mobject(
                Line(start_point, anchor_point), Line(anchor_point, end_point)
            ).ingest_submobjects()
...
class ShowMultiplePathsThroughLens(ShowMultiplePathsScene):
    ...
    def get_paths(self):
        ...
        lower_right, upper_right, upper_left, lower_left = list(
            map(self.lens.point_from_proportion, alphas)
        )
        return [
            Mobject(
                Line(self.start_point, a), Line(a, b), Line(b, self.end_point)
            ).set_color(color)
...
class ShowMultiplePathsOffMirror(ShowMultiplePathsScene):
    ...
    def get_paths(self):
        return [
            Mobject(
                Line(self.start_point, midpoint), Line(midpoint, self.end_point)
            ).set_color(color)
            for midpoint, color in zip(
                [2 * UP, 2 * DOWN], Color(YELLOW).range_to(WHITE, 2)
            )
        ]
...
class ShowMultiplePathsInWater(ShowMultiplePathsScene):
    ...
    def get_paths(self):
        self.leftmost, self.rightmost = result = [
            ...
            for midpoint, color in zip(
                [3 * LEFT, 3 * RIGHT], Color(YELLOW).range_to(WHITE, 2)
            )
        ]
        return result
...
class GeometryOfGlassSituation(ShowMultiplePathsInWater):
    def construct(self):
        ...
        pairs_for_end_transform = [
            (mob, mob.copy()) for mob in (top_line, bottom_line, left_brace, x_mob)
        ]
        ...
        self.clear()
        self.add(
            glass, point_a, point_b, A, B, top_line, bottom_line, left_brace, x_mob
        )
...
class SpringSetup(ShowMultiplePathsInWater):
    def construct(self):
        ...
        ring = Arc(
            angle=11 * np.pi / 6, start_angle=-11 * np.pi / 12, radius=0.2, color=YELLOW
        )
    ...
    def show_equation(self):
        ...
        v_air, v_water = [
            OldTex("v_{\\text{%s}}" % s, size="\\Large") for s in ("air", "water")
        ]
