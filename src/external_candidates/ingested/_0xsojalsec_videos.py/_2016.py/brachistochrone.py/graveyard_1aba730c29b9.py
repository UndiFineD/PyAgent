...
class MultilayeredGlass(PhotonScene, ZoomedScene):
    ...
    def construct(self):
        ...
        photon_run = self.photon_run_along_path(
            self.discrete_path, run_time=1, rate_func=rush_into
        )
    ...
    def generate_layers(self):
        ...
        def create_region(top, color):
            return Region(
                lambda x, y: (y < top) & (y > top - self.layer_thickness), color=color
            )
    ...
    def generate_discrete_path(self):
        ...
        self.discrete_path = Mobject1D(
            color=YELLOW, density=3 * DEFAULT_POINT_DENSITY_1D
        )
    ...
    def show_layer_variables(self):
        layer_top_pairs = list(
            zip(self.layer_tops[: self.num_variables], self.layer_tops[1:])
        )
        v_equations = []
        start_ys = []
        end_ys = []
        ...
        for (top1, top2), x in zip(layer_top_pairs, it.count(1)):
            ...
            center_paths.append(
                Line(
                    midpoint + FRAME_X_RADIUS * LEFT, midpoint + FRAME_X_RADIUS * RIGHT
                )
            )
            brace_endpoints = Mobject(
                Point(self.top * UP + x * RIGHT), Point(top2 * UP + x * RIGHT)
            )
        ...
        for v_eq, start_y, end_y, brace in quads:
            self.remove(brace)
            self.play(
                ShowCreation(v_eq[1]), ShowCreation(v_eq[2]), Transform(start_y, end_y)
            )
    ...
    def show_snells(self, index, frame):
        ...
        for x, numerator in [(index, left), (index + 1, right)]:
            v, sqrt_y = [
                OldTex(text, size="\\Large").next_to(numerator, DOWN)
                for text in ("v_%d" % x, "\\sqrt{y_%d}" % x)
            ]
            vs.append(v)
            sqrt_ys.append(sqrt_y)
        start, end = [
            Mobject(left.copy(), mobs[0], equals.copy(), right.copy(), mobs[1]).replace(
                frame
            )
