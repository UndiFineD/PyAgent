...
class StudentsFindThisConfusing(TeacherStudentsScene):
    def construct(self):
        ...
        question2.target.next_to(
            question1, DOWN, aligned_edge=LEFT, buff=MED_SMALL_BUFF
        )
        equation = OldTex(
            "\\det\\left( %s \\right)=0"
...
class EigenThingsArentAllThatBad(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            "Eigen-things aren't \\\\ actually so bad", target_mode="hooray"
        )
...
class IHatAsEigenVector(ExampleTranformationScene):
    ...
    def set_color_x_axis(self):
        ...
        targets = [
            self.i_hat.copy().scale(val) for val in (-FRAME_X_RADIUS, FRAME_X_RADIUS, 1)
        ]
        lines = [
...
class FullSneakyEigenspace(ExampleTranformationScene):
    def construct(self):
        ...
        self.wait()
        self.apply_transposed_matrix(
            self.t_matrix, added_anims=[Transform(words.start, words)], path_arc=0
        )
        self.wait()
...
class NameEigenvectorsAndEigenvalues(ExampleTranformationScene):
    ...
    def construct(self):
        ...
        self.play(
            ShowCreation(non_eigen_span), Write(non_eigen_words), Animation(non_eigen)
        )
...
class ContrastMatrixUnderstandingWithEigenvalue(TeacherStudentsScene):
    def construct(self):
        axis_and_rotation = OldTexText(
            "Rotate", "$30^\\circ$", "around", "$%s$" % matrix_to_tex_string([2, 3, 1])
        )
...
class SymbolicEigenvectors(Scene):
    ...
    def introduce_terms(self):
        self.expression = OldTex(
            "A", "\\vec{\\textbf{v}}", "=", "\\lambda", "\\vec{\\textbf{v}}"
        )
        ...
        self.play(
            *list(
                map(FadeOut, [A_brace, A_text, lamb_brace, lamb_text, v_text, v_arrows])
            )
        )
    ...
    def contrast_multiplication_types(self):
        ...
        solve_text = OldTexText(
            "Solve for \\\\", "$\\lambda$", "and", "$\\vec{\\textbf{v}}$"
        )
    ...
    def rewrite_righthand_side(self):
        ...
        q_marks = VGroup(
            *[OldTex("?").replace(entry) for entry in matrix.get_entries()]
        )
        ...
        for mob in lamb, id_text, v2:
            mob.target = mob.copy()
        VGroup(
            l_paren, lamb.target, id_text.target, r_paren, v2.target
        ...
        self.expression = VGroup(
            A, v1, equals, VGroup(l_paren, lamb, id_text, r_paren), v2
        )
    ...
    def reveal_as_linear_system(self):
        ...
        for mob in movers:
            mob.target = mob.copy()
        VGroup(
            A.target, v1.target, minus, lamb_group.target, v2.target
        ...
        matrix.scale(0.7)
        VGroup(matrix.get_brackets()[1], *matrix.get_mob_matrix()[:, 2]).shift(
            0.5 * RIGHT
        )
...
class NonZeroSolutionsVisually(LinearTransformationScene):
    ...
    def construct(self):
        equation = OldTex(
            "(A-", "\\lambda", "I)", "\\vec{\\textbf{v}}", "= \\vec{\\textbf{0}}"
        )
...
class IfYouDidntKnowDeterminants(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ShowCreation(bubble), Write(bubble.content), student.change_mode, "confused"
        )
        self.random_blink(4)
...
class RevisitExampleTransformation(ExampleTranformationScene):
    ...
    def introduce_matrix(self):
        ...
        for coords, vect in (i_coords, self.i_hat), (j_coords, self.j_hat):
            ...
            coords.add_to_back(coords.rect)
            coords.next_to(
                vect.get_end(), RIGHT + DOWN if coords is i_coords else RIGHT
            )
        self.play(
            Write(i_coords),
            Write(j_coords),
        ...
        self.play(
            ...
            Transform(
                j_coords.get_entries(), VGroup(*self.matrix.get_mob_matrix()[:, 1])
            ),
        )
        to_remove += self.get_mobjects_from_last_animation()
        self.wait()
    ...
    def get_lambda_to_diag_movements(self, lamb):
        ...
        result += list(
            map(Animation, [self.matrix.get_mob_matrix()[i, 1 - i] for i in range(2)])
        )
        self.diag_entries = [
...
class ShearExample(RevisitExampleTransformation):
    ...
    def show_determinant(self):
        ...
        self.wait()
        self.play(
            *list(map(FadeOut, [one, zero, minus, cdot, new_rect, brace, brace_text]))
        )
        self.wait()
        self.play(Write(equals_0))
        self.wait()
...
class RepeatedMultiplicationInAction(Scene):
    def construct(self):
        ...
        for new_exp in range(2, 6):
            ...
            movers = []
            to_remove = []
            for v_entry, scalar, new_scalar in zip(
                vector.get_entries(), scalars, new_scalars
            ):
...
class RepeatedMultilpicationOfMatrices(Scene):
    ...
    def construct(self):
        ...
        if self.diagonal:
            ...
            VGroup(last_matrix.target, hundred.target, hundred_copy.target).next_to(
                vector, LEFT
            )
...
class ChangeToEigenBasis(ExampleTranformationScene):
    ...
    def introduce_eigenvectors(self):
        ...
        self.apply_transposed_matrix(
            self.t_matrix, rate_func=there_and_back, path_arc=0
        )
