...
class NumericVsGeometric(Scene):
            ...
            " ".join(
                [
                    ...
                    matrix_to_tex_string(
                        [["1 \\cdot 1 + 0 \\cdot 2"], ["1 \\cdot 1 + (-1)\\cdot 2"]]
                    ),
                    "\\\\ &=",
    ...
    def list_geometric_benefits(self):
        ...
        # Ugly hack
        diff = (
            follow_words.submobjects[0].get_bottom()[1]
        ...
        for randy, thought in zip(randys, thoughts):
            ...
            if curr_randy:
                self.play(
                    Transform(curr_randy, randy), Transform(curr_thought, thought)
                )
            else:
...
class LinAlgPyramid(Scene):
    def construct(self):
        ...
        words = self.place_words_in_rects(
            ["Geometric understanding", "Computations", "Uses"], rects
        )
...
class IntimidatingProf(Scene):
    def construct(self):
        ...
        speech_bubble.write(
            "And of course $B^{-1}AB$ will \\\\ also have positive eigenvalues..."
        )
        ...
        self.wait()
        self.play(
            ApplyMethod(randy.change_mode, "confused"), Write(q_marks, run_time=1)
        )
...
class MatrixMultiplicationIs(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            Write(matrix2), GrowFromCenter(brace2), Write(apply_second), run_time=1
        )
        self.wait()
