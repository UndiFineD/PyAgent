# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter8p2.py
...
class BruteForceVerification(Scene):
    def construct(self):
        ...
        cross_product = VGroup(
            v.copy(), OldTex("\\times"), w.copy(), OldTex("="), cross.copy()
        )
        ...
        v_dot, w_dot = [
            OldTex(tex, "\\cdot", "(", v_tex, "\\times", w_tex, ")", "= 0")
...
class ThreeStepPlan(Scene):
    def construct(self):
        ...
        steps = [
            ...
            OldTexText("2. Find its", "dual vector"),
            OldTexText("3. Show that this dual is", cross_text),
        ]
        ...
        VGroup(*steps).arrange(DOWN, aligned_edge=LEFT, buff=LARGE_BUFF).next_to(
            h_line, DOWN, buff=MED_SMALL_BUFF
        )
...
class DefineDualTransform(Scene):
    ...
    def show_triple_cross_product(self):
        ...
        for mob in mobs_with_targets:
            mob.target = mob.copy()
        matrix = Matrix(
            np.array([[e.target for e in array.get_entries()] for array in arrays]).T
        )
        ...
        self.wait(2)
        self.play(
            *[Transform(mob.copy(), mob.target) for mob in tex_mobs]
            + [
                Write(times1),
                Write(times2),
            ]
    ...
    def expand_dot_product(self):
        ...
        for quint in quints:
            ...
            self.wait(0.5)
            self.play(
                *[MoveToTarget(mob) for mob in quint]
                + [Write(mob.sym) for mob in quint]
            )
            self.wait()
        self.play(
            ApplyFunction(
                lambda m: m.arrange(DOWN, buff=MED_SMALL_BUFF + SMALL_BUFF).next_to(
                    cross_components, LEFT
                ),
                new_ps,
            ),
...
class NowForTheCoolPart(TeacherStudentsScene):
    def construct(self):
        ...
        self.random_blink(2)
        self.teacher_says(
            "Let's answer the same question,\\\\", "but this time geometrically"
        )
