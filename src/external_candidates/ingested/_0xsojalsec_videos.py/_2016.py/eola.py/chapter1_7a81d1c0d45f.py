...
class DifferentConceptions(Scene):
    ...
    def physics_conception(self, creatures, original_vector):
        ...
        self.play(
            Rotate(vector, np.pi / 3, in_place=True), Write(direction), run_time=1
        )
    ...
    def house_example(self, starter_mobject, title):
        ...
        everything = VMobject(
            house, square_footage, price, brackets, brace, two_dimensional, title
        )
...
class HelpsToHaveOneThought(Scene):
    def construct(self):
        ...
        randys = VMobject(
            *[Randolph(color=color).scale(0.8) for color in (BLUE_D, BLUE_C, BLUE_E)]
        )
        ...
        self.play(
            ShowCreation(speech_bubble), Transform(morty, new_morty), Write(words)
        )
        self.wait(2)
        self.play(
...
class HowIWantYouToThinkAboutVectors(Scene):
    def construct(self):
        ...
        self.play(
            ApplyMethod(vector.shift, shift_val, path_arc=3 * np.pi / 2, run_time=3)
        )
        self.play(
            ApplyMethod(vector.shift, -shift_val, rate_func=rush_into, run_time=0.5)
        )
        self.wait(3)
...
class CoordinateSystemWalkthrough(VectorScene):
    ...
    def introduce_coordinate_plane(self):
        ...
        self.wait(2)
        self.play(
            Write(origin_words), GrowFromCenter(dot), ShowCreation(line), run_time=1
        )
        self.wait(2)
...
class VectorAdditionNumerically(VectorScene):
    def construct(self):
        ...
        self.play(
            ApplyFunction(lambda m: m.next_to(y_axis, RIGHT).to_edge(UP), coords1)
        )
        ...
        self.play(
            Write(brackets), Write(equals), Transform(sum_x_start, sum_x), run_time=1
        )
...
class ShowScalarMultiplication(VectorScene):
    ...
    def scale_vector(self, v, factor, v_label, v_name="v", factor_tex=None):
        ...
        label = self.label_vector(
            scaled_vector, label_tex, animate=False, add_to_vector=False
        )
        self.remove(label)
...
class ScalingNumerically(VectorScene):
    def construct(self):
        ...
        self.play(
            ApplyMethod(two_v_coords.next_to, equals, RIGHT), Write(equals, run_time=1)
        )
        self.wait(2)
        ...
        for sym, num in zip(syms, [x, y] + list(two_v_elems)):
            sym.move_to(num)
        self.play(
            Transform(x, x_sym), Transform(y, y_sym), FadeOut(VMobject(*two_v_elems))
        )
        self.wait()
        self.play(
...
class ItDoesntMatterWhich(Scene):
    def construct(self):
        ...
        back_and_forth = VMobject(
            new_arrow, OldTex("\\Leftrightarrow"), matrix_to_mobject([2, 1])
        )
...
class DataAnalyst(Scene):
    def construct(self):
        ...
        ellipse_points = [
            ellipse.point_from_proportion(x) for x in np.arange(0, 1, 1.0 / 20)
        ]
        string_vects = [
