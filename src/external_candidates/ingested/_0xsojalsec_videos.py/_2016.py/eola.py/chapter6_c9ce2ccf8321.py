...
class NoComputations(TeacherStudentsScene):
    def construct(self):
        self.setup()
        self.student_says(
            "Will you cover \\\\ computations?", target_mode="raise_left_hand"
        )
        self.random_blink()
        self.teacher_says(
            "Well...uh...no",
        ...
        self.play(
            *[
                ...
                for student, mode in zip(
                    self.get_students(), ["dejected", "confused", "angry"]
                )
            ]
        )
        self.random_blink()
...
class MachineLearningNetwork(Scene):
    def construct(self):
        ...
        for i, num_nodes in enumerate([3, 4, 4, 1]):
            layer = VMobject(
                *[Circle(radius=0.5, color=YELLOW) for x in range(num_nodes)]
            )
...
class SystemOfEquations(Scene):
    ...
    def show_linearity_rules(self, equations):
        ...
        scaled_vars = VMobject(
            *[VMobject(*top_equation.split()[3 * i : 3 * i + 2]) for i in range(3)]
        )
        ...
        self.play(
            *[ApplyMethod(p.scale, 1.5, rate_func=there_and_back) for p in plusses]
        )
        self.wait()
    ...
    def describe_organization(self, equations):
        ...
        xs, ys, zs = [
            VMobject(*[eq.split()[i] for eq in equations.split()]) for i in (1, 4, 7)
        ]
    ...
    def factor_into_matrix(self, equations):
        coefficients = np.array(
            [np.array(eq.split())[[0, 3, 6]] for eq in equations.split()]
        )
        variable_arrays = np.array(
            [np.array(eq.split())[[1, 4, 7]] for eq in equations.split()]
        )
            ...
            *it.chain(
                *[
                    [
                        ...
                        for m1, m2 in zip(
                            start_array.flatten(), matrix_mobject.get_entries().split()
                        )
                    ]
...
class DeclareNewTransformation(TeacherStudentsScene):
    def construct(self):
        words = OldTexText(
            "Playing a transformation in\\\\", "reverse gives a", "new transformation"
        )
...
class SymbolicInversion(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            ApplyFunction(lambda m: m.center().to_edge(UP), VMobject(x, eq, inv2, v))
        )
        self.wait()
...
class InvertNonInvertable(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.play(
            grid.restore, *list(map(Animation, self.foreground_mobjects)), run_time=3
        )
        self.wait()
...
class ColumnsRepresentBasisVectors(Scene):
    def construct(self):
        ...
        i_hat_words, j_hat_words = [
            OldTexText("Where $\\hat{\\%smath}$ lands" % char) for char in ("i", "j")
        ]
...
class TowDColumnsDontSpan(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            ApplyFunction(lambda m: m.scale(-1).shift(self.i_hat.get_end()), self.j_hat)
        )
        ...
        for x in range(8):
            ...
            self.j_hat.target.shift(
                -self.j_hat.target.get_start() + self.i_hat.target.get_end()
            )
            self.play(
                Transform(
                    VMobject(*bases), VMobject(*[m.target for m in bases]), run_time=2
                )
            )
...
class NameColumnSpace(Scene):
    ...
    def replace_number_matrix(self, matrix, new_numbers):
        ...
        self.play(
            *[Transform(*pair, path_arc=np.pi) for pair in zip(starters, targets)]
        )
...
class ZeroMatirx(LinearTransformationScene):
    def construct(self):
        origin = Dot(ORIGIN)
        self.play(
            Transform(VMobject(self.plane, self.i_hat, self.j_hat), origin, run_time=3)
        )
        self.wait()
...
class FullRankCase(LinearTransformationScene):
    ...
    def construct(self):
        ...
        vector = Matrix([0, 0]).set_color(YELLOW)
        title = VMobject(
            OldTexText("Only"), vector, OldTexText("lands on"), vector.copy()
        )
...
class NameNullSpace(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.apply_transposed_matrix(
            self.t_matrix, added_anims=[Transform(vectors, dot)], path_arc=0
        )
        self.wait()
        self.play(
            vectors.restore,
...
class OffsetNullSpace(NameNullSpace):
    def construct(self):
        ...
        dots = VMobject(
            *[Dot(v.get_end(), color=v.get_color()) for v in offset_vectors]
        )
        ...
        self.wait()
        self.apply_transposed_matrix(
            self.t_matrix, added_anims=[Transform(vectors, dot)]
        )
        self.wait()
...
class AddJustOneNullSpaceVector(NameNullSpace):
    def construct(self):
        ...
        self.apply_transposed_matrix(
            self.t_matrix, added_anims=[Transform(null_vector, dot), Write(transformer)]
        )
        self.play(Write(final_sum))
        self.wait()
...
class NullSpaceOffsetRule(Scene):
    def construct(self):
        ...
        v_text = OldTexText(
            "If", "$%s$" % vec("v"), "is in the", "column space", "of $A$"
        )
...
class WhatAboutNonsquareMatrices(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "What about \\\\ nonsquare matrices?", target_mode="raise_right_hand"
        )
