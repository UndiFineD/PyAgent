...
class RecapTime(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            ApplyFunction(lambda m: m.change_mode("pondering").look(LEFT), student)
        )
        self.play(Blink(student))
        self.wait()
...
class DeterminedByTwoBasisVectors(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.add_transformable_label(
            i_hat, "\\hat{\\imath}", "\\hat{\\imath}", color=X_COLOR
        )
        j_hat = self.add_vector([0, 1], color=Y_COLOR)
        self.add_transformable_label(
            j_hat, "\\hat{\\jmath}", "\\hat{\\jmath}", color=Y_COLOR
        )
    ...
    def record_basis_coordinates(self, vect_array, vect):
        ...
        self.play(
            *[ApplyMethod(m.fade) for m in everything]
            + [
                ShowCreation(
                    background, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)
                ),
...
class IntroduceIdeaOfComposition(RotationThenShear):
    ...
    def track_basis_vectors(self):
        ...
        words = OldTexText(
            ["Record where", "$\\hat{\\imath}$", "and", "$\\hat{\\jmath}$", "land:"]
        )
    ...
    def show_overall_effect(self, matrix):
        ...
        new_bases = VMobject(
            *[Vector(func(v.get_end()), color=v.get_color()) for v in bases.split()]
        )
        self.play(
...
class MoreComplicatedExampleVisually(LinearTransformationScene):
    ...
    def get_matrices(self):
        ...
        VMobject(m2_mob, m1_mob, comp_matrix).arrange(buff=0.1).to_corner(
            UP + LEFT
        ).shift(DOWN)
...
class MoreComplicatedExampleNumerically(MoreComplicatedExampleVisually):
    ...
    def construct(self):
        ...
        for i, (col, char, color) in enumerate(triplets):
            ...
            self.play(
                *[FadeOut(mob) for mob in self.get_mobjects() if mob not in start_state]
...
class AskAboutCommutativity(Scene):
    def construct(self):
        l_m1, l_m2, eq, r_m2, r_m1 = (
            OldTex(["M_1", "M_2", "=", "M_2", "M_1"]).scale(1.5).split()
        )
...
class AskAssociativityQuestion(Scene):
    ...
    def move_matrix_parentheses(self, morty, matrices):
        ...
        lp, rp = parens.split()
        state1 = VMobject(
            VectorizedPoint(m1.get_left()), m1, m2, VectorizedPoint(m2.get_right()), m3
        )
...
class ThreeSuccessiveTransformations(LinearTransformationScene):
    ...
    def construct(self):
        ...
        for t_matrix, sym in zip(self.t_matrices, [c, b, a]):
            self.play(
                brace.next_to, sym, DOWN, sym.set_color, YELLOW, last.set_color, WHITE
            )
