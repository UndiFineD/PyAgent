# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\footnote2.py
...
class ColumnsRepresentBasisVectors(Scene):
    ...
    def put_in_thought_bubble(self):
        ...
        self.play(
            ApplyFunction(
                lambda m: bubble.position_mobject_inside(m.set_height(2.5)), everything
            ),
            ShowCreation(bubble),
            randy.change_mode,
            "pondering",
...
class Symbolic2To3DTransform(Scene):
    def construct(self):
        ...
        self.play(
            Transform(mover, output_array), ShowCreation(out_arrow), rate_func=rush_from
        )
...
class DescribeColumnsInSpecificTransformation(Scene):
    def construct(self):
        ...
        for col, char, vect in zip([i_col, j_col], ["i", "j"], [UP, DOWN]):
            ...
            col.arrow = Arrow(
                col.words.get_edge_center(-vect), col.get_edge_center(vect), color=color
            )

        self.play(Write(matrix.get_brackets()))
        self.wait()
        for col in i_col, j_col:
            self.play(Write(col), ShowCreation(col.arrow), Write(col.words, run_time=1))
            self.wait()
...
class CountRowsAndColumns(Scene):
    def construct(self):
        ...
        self.add(matrix)
        self.play(GrowFromCenter(rows_brace), Write(rows_words, run_time=2))
        self.play(GrowFromCenter(cols_brace), Write(cols_words, run_time=2))
        self.wait()
        self.play(
            rows_words[0].copy().move_to,
            title[0],
            cols_words[0].copy().move_to,
            title[2],
...
class MatrixInTheWild(Scene):
    def construct(self):
        ...
        self.play(Blink(randy))
        self.play(ShowCreation(bubble), randy.change_mode, "pondering")
        ...
        self.play(Transform(matrix, new_matrix), FadeOut(bubble))
        self.remove(matrix)
        matrix = new_matrix
        self.add(matrix)
        ...
        self.play(GrowFromCenter(side_brace), Write(side_words, run_time=2))
        self.play(Blink(randy))
        self.wait()
...
class ThreeDToTwoDOutput(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.add(title)
        i, j, k = it.starmap(
            self.add_vector, [([3, 1], X_COLOR), ([1, 2], Y_COLOR), ([-2, -2], Z_COLOR)]
        )
        pairs = [(i, "L(\\hat\\imath)"), (j, "L(\\hat\\jmath)"), (k, "L(\\hat k)")]
        for v, tex in pairs:
...
class TwoDTo1DTransform(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.play(*list(map(FadeOut, mobjects)) + [ShowCreation(line)])
...
class TwoDTo1DTransformWithDots(TwoDTo1DTransform):
    def construct(self):
        ...
        words = OldTexText("Line of dots remains evenly spaced")
        words.next_to(line, UP, buff=MED_SMALL_BUFF)

        self.play(Write(dots))
        self.apply_transposed_matrix(
            self.t_matrix, added_anims=[Transform(dots, new_dots)]
        )
        self.play(Write(words))
        self.wait()
...
class DotProductPreview(VectorScene):
    ...
    def project_w(self):
        ...
        projected_w = Vector(
            self.v.get_end() * dot_product / (v_norm**2), color=self.w.get_color()
        )
        projection_line = Line(self.w.get_end(), projected_w.get_end(), color=GREY)
    ...
    def show_scaling(self):
        ...
        start_brace, interim_brace, final_brace = braces = [
            Brace(Line(ORIGIN, norm * RIGHT), UP)
        ...
        self.wait()
        self.play(Transform(start_brace, interim_brace), Transform(self.w, new_w))
        self.wait()
        self.play(Transform(start_brace, final_brace))
        self.wait()
