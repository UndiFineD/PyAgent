# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter11.py
from _2016.eola.chapter1 import plane_wave_homotopy
from _2016.eola.chapter3 import ColumnsToBasisVectors
from _2016.eola.chapter5 import Blob, NameDeterminant
...
class PhysicsStudent(Student):
    CONFIG = {"color": PINK, "name": "Physics student"}


class CSStudent(Student):
    CONFIG = {"color": PURPLE_E, "flip_at_start": True, "name": "CS Student"}
    ...
    def construct(self):
        ...
        self.teacher_says(
            ...
            added_anims=[
                ...
                for i, mode in enumerate(["pondering", "raise_right_hand", "erm"])
            ],
        )
        self.random_blink(2)
...
class WhatIsA2DVector(LinearTransformationScene):
    ...
    def introduce_vector_and_space(self):
        ...
        two_d_vector = OldTexText(
            "``Two-dimensional ", "vector", "''", arg_separator=""
        )
        two_d_vector.set_color_by_tex("vector", YELLOW)
        two_d_vector.add_background_rectangle()
        two_d_vector.to_edge(UP)

        self.play(Write(two_d_vector), ShowCreation(v), Write(coords), run_time=2)
        self.wait()
    ...
    def construct(self):
        ...
        vectors = VGroup(
            *list(
                map(
                    Matrix,
                    [[3, 1, 4, 1], [5, 9, 2, 6, 5], [3, 5, 8, "\\vdots", 0, 8, 6]],
                )
            )
        )
        ...
        for index, dim, direction in (0, 4, RIGHT), (2, 100, LEFT):
            ...
            self.play(MoveToTarget(v), GrowFromCenter(brace), Write(text))
...
class HyperCube(VMobject):
    ...
    def init_points(self):
        ...
        def project(four_d_array):
            ...
            scalar = interpolate(0.8, 1.2, (w + 1) / 2.0)
...
class AskAbout4DPhysicsStudent(Scene):
    def construct(self):
        ...
        for i, mob in enumerate([line, square, cube, hyper_cube]):
            ...
            group.shift(physy.bubble.get_top() - tex.get_top() + MED_SMALL_BUFF * DOWN)
        line.shift(DOWN)
...
class ManyCoordinateSystems(LinearTransformationScene):
    ...
    def animate_coordinates(self):
        ...
        cob_matrix = np.array([self.i_hat.get_end()[:2], self.j_hat.get_end()[:2]]).T
        ...
        for coord, vect in (coord1, self.i_hat), (coord2, self.j_hat):
            coord.target = coord.copy().next_to(
                vect.target.get_end() / 2, rotate_vector(vect.get_end(), -np.pi / 2)
            )
...
class DeterminantAndEigenvectorDontCare(LinearTransformationScene):
    ...
    def construct(self):
        ...
        words.to_edge(UP)
        dark_yellow = Color(
            rgb=interpolate(color_to_rgb(YELLOW), color_to_rgb(BLACK), 0.5)
        )
        ...
        det_label = OldTex("A")
        det_label = VGroup(
            VectorizedPoint(det_label.get_left()).set_color(WHITE), det_label
        )
        ...
        self.wait()
        self.play(FadeIn(blob), Write(det_label))
        self.play(
            ShowCreation(
                eigenvectors,
                run_time=2,
        ...
        for cob_matrix in cob_matrices:
            if cob_matrix is not None:
                self.play(FadeOut(self.plane), *list(map(Animation, non_plane_mobs)))
                transform = self.get_matrix_transformation(cob_matrix)
                self.plane.apply_function(transform)
                self.play(FadeIn(self.plane), *list(map(Animation, non_plane_mobs)))
                self.wait()
            self.apply_transposed_matrix(
                self.t_matrix, rate_func=special_rate_func, run_time=8
            )
    ...
    def get_eigenvectors(self):
        ...
        for v in v1, v2:
            vectors = VGroup(
                *[Vector(u * x * v) for x in range(7, 0, -1) for u in [-1, 1]]
            )
...
class OtherVectorishThings(TeacherStudentsScene):
    def construct(self):
        words = OldTexText("There are other\\\\", "vectorish", "things...")
...
class FunctionGraphScene(Scene):
    ...
    def get_function_graph(self, func=None, animate=True, add=True, **kwargs):
        index = len(self.graphs)
        if func is None:
            func = self.default_functions[index % len(self.default_functions)]
    ...
    def add_lines(self, output_lines):
        self.play(ShowCreation(output_lines, lag_ratio=0.5, run_time=2))
...
class AddTwoFunctions(FunctionGraphScene):
    def construct(self):
        ...
        self.play(Write(prefix, run_time=2), FadeIn(brace))
        self.wait()
        ...
        self.play(FadeOut(rect), Transform(sum_def, final_sum_def), FadeOut(arrow))
        self.show_line_addition(
            *it.starmap(VGroup, [f_lines[2:], g_lines[2:], sum_lines[2:]])
        )
    ...
    def show_line_addition(self, f_lines, g_lines, sum_lines):
        ...
        if len(list(g_lines)) == 1:
            ...
            kwargs = {"lag_ratio": 0.5, "run_time": 3}
        self.play(*[MoveToTarget(mob, **kwargs) for mob in (g_lines, dots)])
        # self.play(
...
class AddVectorsCoordinateByCoordinate(Scene):
    def construct(self):
        ...
        self.add(v1, plus, v2)
        self.wait()
        self.play(Write(equals), Write(v_sum.get_brackets()))
        self.play(
...
class ScaleFunction(FunctionGraphScene):
    def construct(self):
        graph = self.get_function_graph(
            lambda x: self.default_functions[0](x), animate=False
        )
        scaled_graph = self.get_function_graph(
            lambda x: graph.get_function()(x) * 2, animate=False, add=False
        )
...
class ScaleVectorByCoordinates(Scene):
    def construct(self):
        ...
        v2 = v1.copy()
        two_targets = VGroup(
            *[two.copy().next_to(entry, LEFT) for entry in v2.get_entries()]
        )
        ...
        self.add(two, dot, v1)
        self.play(Write(equals), Write(v2.get_brackets()))
        self.play(
...
class ShowSlopes(Animation):
    CONFIG = {"line_color": YELLOW, "dx": 0.01, "rate_func": None, "run_time": 5}
...
class FromVectorsToFunctions(VectorScene):
    ...
    def show_vector_addition_and_scaling(self):
        ...
        self.wait()
        self.play(Transform(words1, words2), FadeOut(vw_sum), w.restore)
        self.add(v.copy().fade(), w.copy().fade())
        self.play(v.scale, 2)
    ...
    def bring_in_functions(self):
        ...
        self.play(FadeIn(words, lag_ratio=0.5, run_time=3))
        self.wait()
        self.play(
            *[ApplyMethod(mob.shift, FRAME_WIDTH * RIGHT) for mob in (axes, everything)]
            + [Animation(words)]
        )
        ...
        self.play(FadeOut(words), top_word.shift, top_word.get_center()[0] * LEFT)
        self.wait()
    ...
    def show_derivative(self):
        ...
        new_graph = FunctionGraph(lambda x: (x**2) / 3.0 - 1)
        ...
        self.play(
            MoveToTarget(func_tex), *list(map(Write, [L, lp, rp, equals, new_func]))
        )
...
class ManyFunctions(FunctionGraphScene):
    def construct(self):
        ...
        for i in range(100):
            ...
            self.add_random_function(run_time=run_time, added_anims=added_anims)
    ...
    def add_random_function(self, run_time=1, added_anims=[]):
        ...
        self.play(ShowCreation(graph, run_time=run_time), *added_anims)
...
class FormalDefinitionOfLinear(LinearTransformationScene):
    ...
    def write_properties(self):
        title = OldTexText("Formal definition of linearity")
    ...
    def show_additive_property(self):
        ...
        self.play(MoveToTarget(v_label_copy), MoveToTarget(w_label_copy), Write(plus))
        ...
        self.apply_transposed_matrix(
            self.t_matrix, added_anims=[MoveToTarget(vw_label)]
        )
        self.wait()
        self.play(w_group.shift, v.get_end())
        v_label_copy, w_label_copy = v_label.copy(), w_label.copy()
        ...
        w_label_copy.generate_target()
        equals, plus = OldTex("=+")
        rhs = VGroup(equals, v_label_copy.target, plus, w_label_copy.target)
        rhs.arrange()
    ...
    def show_scaling_property(self):
        ...
        self.apply_transposed_matrix(
            self.t_matrix, added_anims=[MoveToTarget(scaled_v_label)]
        )
        self.wait()
        scaled_v = v.copy().scale(2)
        ...
        self.wait()
        faders = (
            [scaled_v_label, scaled_v, v_copy, v, rhs]
            + self.transformable_labels
            + self.moving_vectors
        )
...
class DerivativeIsLinear(Scene):
    ...
    def prepare_text(self):
        ...
        deriv_scaling = OldTex(
            deriv_tex, "(", "4", "x^3", ")", "=", "4", deriv_tex, "(", "x^3", ")"
        )
    ...
    def point_out(self, *terms):
        ...
        self.play(*anims, run_time=1, rate_func=there_and_back)
...
class PolynomialsHaveArbitrarilyLargeDegree(Scene):
    def construct(self):
        ...
        brace = Brace(
            Line(UP, DOWN).scale(FRAME_Y_RADIUS).shift(FRAME_X_RADIUS * RIGHT), LEFT
        )
        ...
        self.wait()
        self.play(FadeIn(arrow), Write(words), GrowFromCenter(brace))
        self.wait()
...
class GeneneralPolynomialCoordinates(Scene):
    def construct(self):
        ...
        array = Matrix(["a_0", "a_1", "\\vdots", "a_{n-1}", "a_n", "0", "\\vdots"])
        ...
        self.wait()
        self.play(Transform(pre_entries.copy(), array.get_entries()))
        self.wait()
...
class IntroducePolynomialSpace(Scene):
    ...
    def list_basis_functions(self):
        ...
        basis_group = VGroup(
            self.poly1[7][1], self.poly1[4], self.poly1[1], x_cubed
        ).copy()
        basis_group.generate_target()
        basis_group.target.arrange(DOWN, buff=0.75 * LARGE_BUFF, aligned_edge=LEFT)
        ...
        basis_functions = [
            OldTex("b_%d(x)" % i, "=") for i in range(len(list(basis_group)))
        ]
        ...
        self.play(
            FadeIn(title), ShowCreation(h_line), MoveToTarget(basis_group), Write(dots)
        )
    ...
    def show_example_coordinates(self):
        ...
        self.play(entries.restore, lag_ratio=0.5, run_time=3)
        self.wait()
        ...
        self.wait()
        self.play(FadeIn(VGroup(*more_terms), lag_ratio=0.5, run_time=2))
        self.wait()
        ...
        self.play(Write(self.poly2), Transform(coords, new_coords))
        self.wait()
        for i, mob in (2, VGroup(*self.poly2[3:5])), (7, self.poly2[0]):
            self.play(
                new_coords.get_entries()[i].scale,
                1.3,
                mob.scale,
                1.3,
    ...
    def derivative_as_matrix(self):
        matrix = Matrix(
            [
                [str(j) if j == i + 1 else "0" for j in range(4)] + ["\\cdots"]
                for i in range(4)
            ]
            + [["\\vdots"] * 4 + ["\\ddots"]]
        )
        matrix.shift(2 * LEFT)
        diag_entries = VGroup(*[matrix.get_mob_matrix()[i, i + 1] for i in range(3)])
        ##Horrible
        ...
        deriv.target.next_to(matrix.target, UP, buff=MED_SMALL_BUFF, aligned_edge=LEFT)
        deriv.target.shift(0.25 * RIGHT)
        self.play(FadeOut(equals), *list(map(MoveToTarget, [matrix, deriv])))

        poly = OldTex("(", "1", "x^3", "+", "5", "x^2", "+", "4", "x", "+", "5", ")")
        ...
        poly_result = OldTex("3", "x^2", "+", "10", "x", "+", "4")
        ...
        result_array = Matrix(result_entries + [OldTex("0"), OldTex("\\vdots")])
        result_array.next_to(eq2)

        rects = [
            Rectangle(color=YELLOW)
            .replace(VGroup(*matrix.get_mob_matrix()[i, :]), stretch=True)
            .stretch_in_place(1.1, 0)
            .stretch_in_place(1.3, 1)
            for i in range(3)
        ]
...
class MatrixVectorMultiplicationAndDerivative(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(teacher.change_mode, "speaking", ShowCreation(bubble), Write(group))
        self.random_blink()
...
class CompareTermsInLinearAlgebraToFunction(Scene):
    def construct(self):
        ...
        h_line = Line(LEFT, RIGHT).scale(FRAME_X_RADIUS)
        h_line.shift(VGroup(l_title, r_title).get_bottom()[1] * UP + SMALL_BUFF * DOWN)
...
class YouAsAMathematician(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(arrow), mathy.change_mode, "wave_1", mathy.look, OUT)
...
class ShowVectorSpaces(Scene):
    def construct(self):
        ...
        v_lines = [
            Line(h_line.get_center(), FRAME_Y_RADIUS * DOWN).shift(
                vect * FRAME_X_RADIUS / 3.0
            )
            for vect in (LEFT, RIGHT)
        ]
        vectors = self.get_vectors()
        vectors.shift(LEFT * FRAME_X_RADIUS * (2.0 / 3))
        arrays = self.get_arrays()
        functions = self.get_functions()
        functions.shift(RIGHT * FRAME_X_RADIUS * (2.0 / 3))
        ...
        vectors = VGroup(
            *[
                ...
                for scalar, angle in zip(
                    2 * np.random.random(n_vectors) + 0.5, np.linspace(0, 6, n_vectors)
                )
            ]
        )
    ...
    def get_arrays(self):
        arrays = VGroup(
            *[
                VGroup(*[Matrix(np.random.randint(-9, 9, 2)) for x in range(4)])
                for x in range(3)
            ]
        )
    ...
    def get_functions(self):
        ...
        functions.stretch_to_fit_width(FRAME_X_RADIUS / 2.0)
...
class ListAxioms(Scene):
    def construct(self):
        ...
        axioms = VGroup(
            *it.starmap(
                ...
                [
                    ...
                    ("2. \\,", v_tex, "+", w_tex, "=", w_tex, "+", v_tex),
                    (
                        "3. \\,",
                        "\\text{There is a vector }",
                        "\\textbf{0}",
                        "\\text{ such that }",
                        "\\textbf{0}+",
                        v_tex,
        ...
        axioms.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        axioms.set_width(FRAME_WIDTH - 1)
        axioms.next_to(h_line, DOWN, buff=MED_SMALL_BUFF)

        self.play(FadeIn(axioms, lag_ratio=0.5, run_time=5))
        self.wait()
...
class AxiomsAreInterface(Scene):
    def construct(self):
        ...
        red_line = Line(
            rules_of_nature.get_left(), rules_of_nature.get_right(), color=RED
        )

        self.play(Write(words))
        self.wait()
        self.play(ShowCreation(red_line))
        self.play(Transform(rules_of_nature.copy(), an_interface))
        self.wait()
        self.play(FadeIn(mathy))
        self.play(ShowCreation(double_arrow), FadeIn(others, lag_ratio=0.5, run_time=2))
...
class VectorSpaceOfPiCreatures(Scene):
    ...
    def add_creatures(self):
        creatures = VGroup(
            *[
                VGroup(*[PiCreature() for x in range(4)]).arrange(RIGHT, buff=1.5)
                for y in range(4)
            ]
        ...
        self.play(FadeIn(creatures, lag_ratio=0.5, run_time=3))
        self.wait()
        return creatures
    ...
    def show_sum(self, creatures):
        ...
        for pi in pis:
            pi.generate_target()
        plus, equals = OldTex("+=")
        sum_equation = VGroup(pi1.target, plus, pi2.target, equals, sum_pi)
        sum_equation.arrange().center()

        scaled_pi3 = pi3.copy().scale(2)
        equals2 = OldTex("=")
        two = OldTex("2 \\cdot")
        scale_equation = VGroup(two, pi3.target, equals2, scaled_pi3)
        scale_equation.arrange()

        VGroup(sum_equation, scale_equation).arrange(DOWN, buff=MED_SMALL_BUFF)
...
class MathematicianDoesntHaveToThinkAboutThat(Scene):
    def construct(self):
        ...
        self.play(mathy.change_mode, "hooray", ShowCreation(bubble), Write(words))
        self.play(Blink(mathy))
        self.wait()
        self.play(mathy.change_mode, "pondering", Transform(words, new_words))
        self.play(Blink(mathy))
        self.wait()
...
class TextbooksAreAbstract(TeacherStudentsScene):
    def construct(self):
        ...
        self.random_blink(2)
        self.teacher_says(
            "...then in some different\\\\", "context, like a function space"
        )
        self.play_student_changes(*["pondering"] * 2)
        self.random_blink()
        self.teacher_says(
            "Only then should you\\\\", "think from the axioms", target_mode="surprised"
        )
        self.play_student_changes(*["pondering"] * 3)
        self.random_blink()
...
class LastAskWhatAreVectors(TeacherStudentsScene):
    def construct(self):
        self.student_says("So...what are vectors?", target_mode="erm")
        self.random_blink()
        self.teacher_says("""
            The form they take
...
class WhatIsThree(Scene):
    def construct(self):
        what_is, three, q_mark = words = OldTexText(
            "What is ", "3", "?", arg_separator=""
        )
        words.scale(1.5)
        self.play(Write(words))
        self.wait()
        self.play(FadeOut(what_is), FadeOut(q_mark), three.center)
        ...
        self.wait()
        self.play(
            *[Transform(trip, three, lag_ratio=0.5, run_time=2) for trip in triplets]
        )
...
class AbstractionIsThePrice(Scene):
    def construct(self):
        words = OldTexText("Abstractness", "is the price\\\\" "of", "generality")
...
class GoodLuck(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            "Good luck with \\\\ your future learning!", target_mode="hooray"
        )
