...
class ShowVaryingLinearCombinations(VectorScene):
    ...
    def construct(self):
        ...
        v1_label = self.label_vector(
            v1, self.vector1_label, color=self.vector1_color, buff_factor=3
        )
        v2_label = self.label_vector(
            v2, self.vector2_label, color=self.vector2_color, buff_factor=3
        )
        label_anims = [
    ...
    def get_scalar_anims(self, v1, v2, v1_label, v2_label):
        def get_val_func(vect):
            ...
            return (
                lambda a: np.dot(original_vect, vect.get_end() - vect.get_start())
                / square_norm
            )
    ...
    def initial_scaling(self, v1, v2, label_anims, scalar_anims):
        ...
        anims = [
            ...
            for v, s, rf in zip([v1, v2], scalar_pair, self.get_rate_func_pair())
        ]
...
class VectorsOnALine(VectorsToDotsScene):
    def get_vectors(self):
        return [
            ...
            for a in np.linspace(-FRAME_Y_RADIUS, FRAME_Y_RADIUS, self.num_vectors)
        ]
...
class VectorsInThePlane(VectorsToDotsScene):
    ...
    def play_final_animation(self, vectors, rate_functions):
        ...
        v_line = Line(
            FRAME_Y_RADIUS * UP, FRAME_Y_RADIUS * DOWN, stroke_width=0.5, color=BLUE_E
        )
        line_pairs = [
...
class AlternateDefOfLinearlyDependent(Scene):
    def construct(self):
        ...
        scalar_specification.add(
            *[Arrow(scalar_specification, equations[0].split()[i]) for i in (2, 5)]
        )
        ...
        for i, new_eq in enumerate(equations):
            ...
            self.play(
                Transform(equation, new_eq, path_arc=(np.pi / 2 if i == 1 else 0))
            )
            self.wait(3)
        ...
        self.play(
            ApplyFunction(lambda m: m.scale(0.5).to_corner(UP + LEFT), everything)
        )
        self.wait()
