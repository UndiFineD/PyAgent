...
class SimpleDefine2dCrossProduct(LinearTransformationScene):
    ...
    def show_area(self):
        ...
        self.wait()
        self.play(
            FadeIn(self.square), *list(map(Animation, [self.v, self.w, v_copy, w_copy]))
        )
        self.wait()
    ...
    def write_area_words(self):
        ...
        self.play(
            ...
            *[
                ApplyMethod(
                    vect.label.target.restore, rate_func=lambda t: smooth(1 - t)
                )
                for vect in (self.v, self.w)
            ],
        )
        ...
        self.play(
            ApplyFunction(
                lambda m: m.scale(1.2).set_color(RED), times, rate_func=there_and_back
            )
        )
        self.wait()
    ...
    def show_sign(self):
        ...
        for word in positive, negative:
            ...
            word.arrow = Arrow(
                word.get_top(), word.get_top() + 1.5 * UP, color=word.get_color()
            )
            VGroup(word, word.arrow).next_to(
                self.area_words, DOWN, aligned_edge=LEFT, buff=SMALL_BUFF
            )
        minus_sign = OldTex("-")
...
class HowDoYouCompute(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "How do you \\\\ compute this?", target_mode="raise_left_hand"
        )
        self.random_blink(2)
...
class ContrastDotAndCross(Scene):
    ...
    def add_cross_product(self):
        ...
        arrays = [
            [random.randint(0, 9) for in_count in range(3)] for out_count in range(2)
        ]
...
class Define2dCrossProduct(LinearTransformationScene):
    ...
    def initial_definition(self):
        ...
        self.wait()
        self.play(
            ShowCreation(matrix_background), Write(matrix.get_brackets()), run_time=1
        )
    ...
    def transform_square(self):
        ...
        vect_stuffs = VGroup(
            *it.chain(*[[m, m.label, m.coord_array] for m in (self.v, self.w)])
        )
        ...
        self.wait()
        self.play(
            FadeOut(self.plane), Animation(self.matrix), *list(map(FadeIn, vect_stuffs))
        )
        ...
        area_arrow = Arrow(
            area_words.get_bottom(), self.square.get_center(), color=WHITE
        )
        self.play(
            GrowFromCenter(det_text_brace), Write(area_words), ShowCreation(area_arrow)
        )
        self.wait()
        ...
        self.wait()
        self.play(
            *list(map(FadeOut, [area_arrow, self.v.coord_array, self.w.coord_array]))
        )
...
class TwoDCrossProductExample(Define2dCrossProduct):
    ...
    def construct(self):
        ...
        equation_start = VGroup(
            cross_product, VGroup(matrix_background, det_text, matrix)
        )
        ...
        det_sym.scale(1.5)
        det_sym.next_to(
            v.get_end() + w.get_end(), DOWN + RIGHT, buff=MED_SMALL_BUFF / 2
        )
...
class BiggerWhenPerpendicular(LinearTransformationScene):
    ...
    def construct(self):
        ...
        group.remove(bigger)
        self.play(
            FadeIn(group), ShowCreation(start_square), *list(map(Animation, [v, w]))
        )
...
class WriteAreaOfParallelogram(Scene):
    def construct(self):
        words = OldTexText(
            "Area of ", "parallelogram", " $=$ ", "$2.5$", arg_separator=""
        )
...
class WriteCrossProductProperties(Scene):
    def construct(self):
        ...
        length_words = OldTexText(
            "Length of ", p_cash, "\\\\ = ", "(parallelogram's area)"
        )
        ...
        perpendicular = OldTexText(
            "\\centering Perpendicular to", v_cash, "and", w_cash
        )
...
class DeterminantTrick(Scene):
    def construct(self):
        ...
        for quint in quints:
            ...
            term = VGroup(
                basis.t, lp, quint[1].t, quint[2].t, minus, quint[3].t, quint[4].t, rp
            )
            term.arrange()
            if last_mob:
                plus = OldTex("+")
            ...
            self.wait()
            self.play(
                *[Transform(mob.copy(), mob.t) for mob in quint]
...
class CrossAndDualWords(Scene):
    def construct(self):
        ...
        dot_with_cross = OldTex(
            "%s \\cdot ( " % input_array_tex, v_tex, "\\times", w_tex, ")"
        )
        ...
        VGroup(vector_word, cross).arrange(RIGHT, buff=MED_SMALL_BUFF).center().shift(
            LEFT
        ).to_edge(UP)
