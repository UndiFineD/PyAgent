# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter7.py
...
def get_vect_mob_projection(vector_to_project, stable_vector):
    return Vector(
        get_projection(vector_to_project.get_end(), stable_vector.get_end()),
...
class TraditionalOrdering(RandolphScene):
    def construct(self):
        ...
        self.play(
            ...
            FadeIn(topics, run_time=3, lag_ratio=0.5),
        )
...
class ThisSeriesOrdering(RandolphScene):
    def construct(self):
        ...
        chapters.arrange(DOWN, buff=SMALL_BUFF, aligned_edge=LEFT)
        ...
        self.play(
            FadeIn(chapters, lag_ratio=0.5, run_time=3), self.randy.change_mode, "sassy"
        )
...
class OneMustViewThroughTransformations(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("pondering", "plain", "raise_right_hand")
        self.random_blink(2)
        words = OldTexText("First, the ", "standard view...", arg_separator="\n")
        self.teacher_says(words)
        self.random_blink(2)
...
class ShowNumericalDotProduct(Scene):
    ...
    def construct(self):
        ...
        products = VGroup(
            *[
                VGroup(p1.copy(), dot.copy(), p2.copy()).arrange(RIGHT, buff=SMALL_BUFF)
                for p1, p2 in pairs
            ]
        )
        ...
        final_sum = VGroup(*it.chain(*list(zip(symbols, products.target))))
        ...
        self.play(Write(v1), Write(v2), FadeIn(inter_array_dot), FadeIn(title))
        self.wait()
        if self.write_dot_product_words:
            self.play(
                inter_array_dot.set_color,
                YELLOW,
                ShowCreation(arrow),
        ...
        self.play(
            Write(symbols), Transform(products, products.target, path_arc=np.pi / 2)
        )
        self.wait()
...
class GeometricInterpretation(VectorScene):
    ...
    def project(self):
        ...
        projection_line = Line(
            self.proj_vect.get_end(), projected.get_end(), color=GREY
        )
    ...
    def show_lengths(self):
        ...
        self.play(GrowFromCenter(proj_brace), Write(proj_words.start, run_time=2))
        self.wait()
        self.play(Transform(proj_words.start, proj_words), FadeOut(proj_brace))
        self.play(
        ...
        self.wait()
        self.play(Transform(stable_words.start, stable_words), Write(product))
        self.wait()
    ...
    def handle_possible_negative(self):
        ...
        words.set_color(RED)
        words.next_to(
            VMobject(*self.product[2:]), DOWN, buff=LARGE_BUFF, aligned_edge=LEFT
        )
        ...
        self.play(
            Write(neg),
            ShowCreation(arrow),
            VMobject(*self.product[1:]).next_to,
            neg,
            Write(words),
        )
        self.wait()
...
class ShowQualitativeDotProductValues(VectorScene):
    def construct(self):
        ...
        words = list(
            map(TexText, ["Similar directions", "Perpendicular", "Opposing directions"])
        )
        ...
        for mob in v, w, line, proj_line:
            self.play(ShowCreation(mob), Animation(v))
        ...
        self.wait()
        self.play(Rotate(w, -np.pi / 3), shadow.scale, 0)
        self.play(Transform(comp, equals), Transform(word, words[1]))
        self.wait()
        self.play(Rotate(w, -np.pi / 3), Transform(shadow, shadow_opposite))
        self.play(Transform(comp, less_than), Transform(word, words[2]))
        self.wait()
...
class SymmetricVAndW(VectorScene):
    def construct(self):
        ...
        for v1, v2 in (v, w), (w, v):
            ...
            v1.proj_line = Line(v1.get_end(), v1.proj.get_end(), color=GREY)
        ...
        self.play(
            Rotate(w, angle), Rotate(v, -angle), rate_func=there_and_back, run_time=2
        )
        self.wait()
        self.play(
            ShowCreation(line_of_symmetry), Write(line_of_symmetry_words, run_time=1)
        )
        self.wait(0.5)
        ...
        for vect in w, v:
            ...
            self.add(vect.proj)
            self.wait()
        self.play(*list(map(FadeOut, [v.proj, v.proj_line, w.proj, w.proj_line])))
    ...
    def show_doubling(self, v, w):
        ...
        new_v.proj.fade()
        new_v.proj_line = Line(new_v.get_end(), new_v.proj.get_end(), color=GREY)
        ...
        self.play(
            Transform(*[VGroup(mob, mob.label) for mob in (v, new_v)]), run_time=2
        )
        last_mob = self.get_mobjects_from_last_animation()[0]
        self.remove(last_mob)
        self.add(*last_mob)
        Transform(
            VGroup(v.proj, v.proj_line), VGroup(new_v.proj, new_v.proj_line)
        ).update(
            1
        )  ##Hacky
        ...
        for full_v, projector, stable in zip([False, True], [w, v], [v, w]):
            ...
            self.wait()
            self.play(*list(map(FadeOut, [projector.proj, projector.proj_line])))
...
class LurkingQuestion(TeacherStudentsScene):
    def construct(self):
        ...
        #     width = 6,
        # )
        self.play_student_changes("raise_right_hand", "confused", "raise_left_hand")
        self.random_blink(5)
        answer = OldTexText(
            """
        ...
        self.play(
            ApplyPointwiseFunction(
                lambda p: 10 * (p + 2 * DOWN) / get_norm(p + 2 * DOWN), everything
            )
        )
...
class Introduce2Dto1DLinearTransformations(TwoDToOneDScene):
    def construct(self):
        ...
        self.play(
            ShowCreation(number_line), *[Animation(v) for v in (self.i_hat, self.j_hat)]
        )
...
class Symbolic2To1DTransform(Scene):
    def construct(self):
        ...
        VMobject(input_array, in_arrow, func, out_arrow, output_array).arrange(
        ...
        self.play(GrowFromCenter(input_brace), Write(input_words))
        ...
        self.play(GrowFromCenter(output_brace), Write(output_words))
        self.wait()
...
class AdditivityProperty(TwoDToOneDScene):
    CONFIG = {"show_basis_vectors": False, "sum_before": True}
    ...
    def play_sum(self, v, w):
        ...
        self.play(*[Transform(mob, mob.target) for mob in (v, w, sum_vect)])
    ...
    def get_symbols(self):
        v_tex, w_tex = ["\\vec{\\textbf{%s}}" % c for c in ("v", "w")]
        if self.sum_before:
            tex_mob = OldTex("L(", v_tex, "+", w_tex, ")")
            result = VGroup(tex_mob[0], VGroup(*tex_mob[1:4]), tex_mob[4])
        else:
            tex_mob = OldTex("L(", v_tex, ")", "+", "L(", w_tex, ")")
            result = VGroup(
...
class NonLinearFailsDotTest(TwoDTo1DTransformWithDots):
    def construct(self):
        ...
        vect = 2 * RIGHT + UP
        dots = VMobject(
            *[Dot(offset + a * vect, radius=0.075) for a in np.linspace(-2, 3, 18)]
        )
        dots.set_submobject_colors_by_gradient(YELLOW_B, YELLOW_C)
        func = lambda p: (p[0] ** 2 - p[1] ** 2) * RIGHT
        new_dots = VMobject(
            *[
                Dot(func(dot.get_center()), color=dot.get_color(), radius=dot.radius)
                for dot in dots
            ]
        )
        words = OldTexText("Line of dots", "do not", "remain evenly spaced")
        words.set_color_by_tex("do not", RED)
        words.next_to(line, UP, buff=MED_SMALL_BUFF)
        array_tex = matrix_to_tex_string(["x", "y"])
        equation = OldTex("f", "\\left(%s \\right)" % array_tex, " = x^2 - y^2")
        ...
        self.apply_nonlinear_transformation(
            func, added_anims=[Transform(dots, new_dots)]
        )
        self.play(Write(words))
        self.wait()
...
class AlwaysfollowIHatJHat(TeacherStudentsScene):
    def construct(self):
        ...
        words = OldTexText("Always follow", i_tex, "and", j_tex)
        words.set_color_by_tex(i_tex, X_COLOR)
        words.set_color_by_tex(j_tex, Y_COLOR)
        self.teacher_says(words)
        students = VMobject(*self.get_students())
        ponderers = VMobject(*[pi.copy().change_mode("pondering") for pi in students])
        self.play(Transform(students, ponderers, lag_ratio=0.5, run_time=2))
        self.random_blink(2)
    ...
    def show_matrix(self):
        for vect, char in zip([self.i_hat, self.j_hat], ["i", "j"]):
            vect.words = OldTexText(
                "$\\hat\\%smath$ lands on" % char, str(int(vect.get_end()[0]))
            )
        ...
        self.play(Write(matrix_words), Write(matrix.get_brackets()), run_time=1)
...
class FollowVectorViaCoordinates(TwoDToOneDScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(v), Write(array), Write(v_label))
        ...
        for i, anim_tuple in enumerate(scaling_anim_tuples):
            ...
            if i == 1:
                self.play(
                    *[ApplyMethod(m.shift, self.v_coords[0] * RIGHT) for m in movers]
                )
            self.wait()
        self.play(
            *list(map(FadeOut, to_fade))
            + [vect.restore for vect in (self.i_hat, self.j_hat)]
        )
        ...
        if self.concrete:
            new_labels = [OldTex("(%d)" % num) for num in self.t_matrix[:, 0]]
        else:
            new_labels = [OldTex("L(\\hat{\\%smath})" % char) for char in ("i", "j")]

        new_labels[0].set_color(X_COLOR)
        new_labels[1].set_color(Y_COLOR)

        new_labels.append(OldTex("L(\\vec{\\textbf{v}})").set_color(YELLOW))
        for label, vect, direction in zip(
            new_labels, list(bases) + [v], [UP, DOWN, UP]
        ):
    ...
    def get_scaling_anim_tuples(self, labels, array, directions):
        ...
        bases = self.i_hat, self.j_hat
        quints = list(
            zip(bases, self.v_coords, labels, array.get_entries(), directions)
        )
        for basis, scalar, label, entry, direction in quints:
            ...
            scaled_label.next_to(basis.scaled, direction)
...
class FollowVectorViaCoordinatesConcrete(FollowVectorViaCoordinates):
    CONFIG = {"v_coords": [4, 3], "concrete": True}
...
class TwoDOneDMatrixMultiplication(Scene):
    ...
    def construct(self):
        ...
        for m in vector, matrix:
            self.play(Write(m))
            self.play(GrowFromCenter(m.brace), Write(m.label), run_time=1)
            self.wait()
    ...
    def show_product(self, matrix, vector):
        ...
        pairs = [
            VMobject(e1.copy(), OldTex("\\cdot"), e2.copy()).arrange(
        ...
        for starter_pair, pair in zip(starter_pairs, pairs):
            self.play(
                Transform(VMobject(*starter_pair).copy(), pair, path_arc=-np.pi / 2)
            )
        self.wait()
...
class AssociationBetweenMatricesAndVectors(Scene):
    ...
    def construct(self):
        ...
        VGroup(matrices_words, arrow, vectors_words).arrange(buff=MED_SMALL_BUFF)
        ...
        self.wait()
        self.play(vectors.restore, path_arc=-np.pi / 2, lag_ratio=0.5, run_time=2)
        self.wait()
...
class SomeKindOfConnection(Scene):
    ...
    def construct(self):
        ...
        squish_plane.apply_function(lambda p: np.dot(p, [4, 1, 0]) * RIGHT)
        ...
        self.play(
            *list(map(ShowCreation, [plane, number_line, v_line]))
...
class ProjectOntoUnitVectorNumberline(VectorScene):
    ...
    def setup(self):
        ...
        if self.animate_setup:
            self.play(ShowCreation(number_line), Write(numbers), run_time=3)
            self.wait()
        ...
        if self.zoom_factor != 1:
            ...
            self.play(*[Transform(mob, mob.target) for mob in self.get_mobjects()])
            self.wait()
    ...
    def get_vectors(self, num_vectors=10, randomize=True):
        ...
        vectors = VGroup(*[Vector(x * RIGHT + y * UP) for x, y in zip(x_vals, y_vals)])
        vectors.set_color_by_gradient(PINK, MAROON_B)
        return vectors

    def get_dots(self, vectors):
        return VGroup(
            *[Dot(v.get_end(), color=v.get_color(), radius=0.075) for v in vectors]
        )
    ...
    def get_proj_dots(self, dots):
        return VGroup(
            *[
                dot.copy().move_to(
                    get_projection(dot.get_center(), self.u_hat.get_end())
                )
                for dot in dots
            ]
        )
...
class ProjectionFunctionSymbol(Scene):
    def construct(self):
        v_tex = "\\vec{\\textbf{v}}"
        equation = OldTex(
            "P(", v_tex, ")=", "\\text{number }", v_tex, "\\text{ lands on}"
        )
        equation.set_color_by_tex(v_tex, YELLOW)
        equation.shift(2 * UP)
        words = OldTexText("This projection function is", "linear")
        words.set_color_by_tex("linear", BLUE)
        arrow = Arrow(words.get_top(), equation[0].get_bottom(), color=BLUE)
...
class ProjectSingleVectorOnUHat(ProjectOntoUnitVectorNumberline):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(v.proj_line), Transform(v.copy(), v.proj))
        self.wait()
        self.play(
            FadeOut(v), FadeOut(v.proj_line), FadeOut(v.label), Write(dot_product)
        )
        self.wait()
...
class AskAboutProjectionMatrix(Scene):
    def construct(self):
        ...
        for b_words, q_mark, direction in zip(
            basis_words, matrix.get_entries(), [UP, DOWN]
        ):
        ...
        self.play(Write(words), Write(matrix))
        self.wait()
        for b_words in basis_words:
            self.play(Write(b_words), ShowCreation(b_words.arrow))
        self.wait()
...
class ProjectBasisVectors(ProjectOntoUnitVectorNumberline):
    ...
    def construct(self):
        ...
        for dot in proj_dots:
            dot.scale(0.1)
        proj_basis = VGroup(
            *[get_vect_mob_projection(vector, self.u_hat) for vector in basis_vectors]
        )

        i_tex, j_tex = ["$\\hat{\\%smath}$" % char for char in ("i", "j")]
        question = OldTexText("Where do", i_tex, "and", j_tex, "land?")
        ...
        u_x, u_y = [
            OldTex("u_%s" % c).set_color(self.u_hat.get_color()) for c in ("x", "y")
        ]
    ...
    def show_symmetry(self, vect, coord, coord_landing_spot):
        ...
        for v in vect, self.u_hat:
            v.proj_line = DashedLine(v.get_end(), v.proj.get_end(), color=v.get_color())
            v.proj_line.fade()
            ...
            self.wait()
        for v in vect, self.u_hat:
            self.play(ShowCreation(v.tick), Write(v.q_mark))
            self.wait()
...
class ShowSingleProjection(ProjectBasisVectors):
    ...
    def construct(self):
        ...
        proj_line = DashedLine(
            vector.get_end(), proj.get_end(), color=vector.get_color()
        )
        coords = Matrix(["x", "y"])
        ...
        self.play(Transform(vector.copy(), proj), Animation(self.u_hat))
        self.wait()
...
class GeneralTwoDOneDMatrixMultiplication(TwoDOneDMatrixMultiplication):
    ...
    def construct(self):
        ...
        to_fade = [
            m for m in everything if isinstance(m, Brace) or isinstance(m, TexText)
        ]
        ...
        self.play(everything.to_corner, UP + RIGHT, *list(map(FadeOut, to_fade)))
...
class AskAboutNonUnitVectors(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "What about \\\\ non-unit vectors", target_mode="raise_left_hand"
        )
        self.random_blink(2)
...
class ScaleUpUHat(ProjectOntoUnitVectorNumberline):
    ...
    def scale_u_hat(self):
        ...
        self.play(
            Transform(self.u_hat, new_u), Transform(self.u_hat.coords, new_u.coords)
        )
        self.wait()
    ...
    def show_matrix(self):
        ...
        self.play(Transform(self.u_hat.coords, matrix))
    ...
    def transform_basis_vectors(self):
        ...
        for b, char in zip(bases, ["x", "y"]):
            ...
            b.proj_line = DashedLine(b.get_end(), b.proj.get_end(), dash_length=0.05)
        ...
        self.play(
            *list(
                map(
                    FadeOut,
                    [mob for mob in self.get_mobjects() if mob not in start_state],
                )
            )
        )

    def transform_some_vector(self):
        words = OldTexText("\\centering Project\\\\", "then scale")
        ...
        proj_line = DashedLine(v.get_end(), proj.get_end(), color=v.get_color())
        mover = v.copy()

        self.play(ShowCreation(v))
        self.play(Transform(self.matrix_words, words))
        self.play(ShowCreation(proj_line))
        self.play(Transform(mover, proj), project.set_color, YELLOW)
        self.wait()
        self.play(mover.scale, self.scalar, then_scale.set_color, YELLOW)
        self.wait()
...
class TwoDOneDTransformationSeparateSpace(Scene):
    ...
    def construct(self):
        ...
        start_words = OldTexText(
            "\\centering Any time you have a \\\\", "2d-to-1d linear transform..."
        )
...
class IsntThisBeautiful(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says("Isn't this", "beautiful", target_mode="surprised")
        for student in self.get_students():
            self.play(student.change_mode, "happy")
        self.random_blink()
        duality_words = OldTexText("It's called", "duality")
...
class TranslateToTheWorldOfTransformations(TwoDOneDMatrixMultiplication):
    ...
    def construct(self):
        v1, v2 = [Matrix(["x_%d" % n, "y_%d" % n]) for n in (1, 2)]
        ...
        self.play(GrowFromCenter(brace), Write(word))
        self.wait()
...
class WhatTheVectorWantsToBe(Scene):
    ...
    def construct(self):
        ...
        words = OldTexText("What the vector", "\\\\ wants", "to be")
        ...
        self.wait()
        self.play(Transform(v_coords.copy(), matrix), Write(words))
        self.play(
            Transform(plane.copy(), squish_plane, run_time=3),
            *list(map(Animation, [words, matrix, plane, vect, v_coords])),
        )
        self.wait()
