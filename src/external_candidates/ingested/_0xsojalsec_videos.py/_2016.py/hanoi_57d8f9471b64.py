...
class CountingScene(Scene):
    ...
    def increment(self, run_time_per_anim=1, added_anims=[], total_run_time=None):
        ...
        while continue_rolling_over:
            ...
            self.play(
                MoveToTarget(moving_dot), *added_anims, run_time=run_time_per_anim
            )
        ...
        self.disks = VGroup(
            *[
                ...
                for width, color in zip(
                    np.linspace(
                        self.disk_min_width, self.disk_max_width, self.num_disks
                    ),
...
class IntroduceKeith(Scene):
    def construct(self):
        ...
        self.play(
            keith.change_mode, "hooray", Transform(original_content, bubble.content)
        )
        self.play(Blink(morty))
        self.wait()
...
class IntroduceTowersOfHanoi(TowersOfHanoiScene):
    ...
    def note_disk_labels(self):
        ...
        for label in labels:
            label.save_state()
            self.play(
                label.scale, 2, label.set_color, YELLOW, last.restore, run_time=0.5
            )
            last = label
    ...
    def cannot_move_disk_onto_smaller_disk(self):
        ...
        self.move_disks_to_peg(
            [2], 2, added_anims=[Transform(self.title, also_not_allowed, run_time=1)]
        )
        cross.replace(disk)
...
class ExampleFirstMoves(TowersOfHanoiScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            self.disks[1].set_fill, YELLOW_D, self.disks[1].label.set_color, BLACK
        )
...
class KeithShowingBinary(Scene):
    def construct(self):
        ...
        def get_bit_flip():
            return Transform(
                bits, bits_list.pop(0), rate_func=squish_rate_func(smooth, 0, 0.7)
            )
...
class RhythmOfDecimalCounting(CountingScene):
    ...
    def construct(self):
        ...
        self.wait()
        groups = [
            VGroup(*pair) for pair in zip(self.dot_templates, self.curr_configurations)
        ]
        self.play(
            groups[2].to_edge,
            RIGHT,
...
class BinaryCountingAtEveryScale(Scene):
    ...
    def construct(self):
        ...
        bit_mobs = [
            get_binary_tex_mob(n, self.num_bits) for n in range(2**self.num_bits)
        ]
        ...
        def get_run_through(mobs):
            return Succession(
                *[
                    Transform(
                        curr_bits, mob, rate_func=squish_rate_func(smooth, 0, 0.5)
                    )
                    for mob in mobs
                ],
                run_time=1,
        ...
        self.wait()
        self.play(
            get_run_through(bit_mobs[2 ** (self.num_bits - 1) + 1 :]), Write(again)
        )
        self.wait()
...
class IntroduceSolveByCounting(TowersOfHanoiScene):
    ...
    def construct(self):
        ...
        braces = [
            Brace(VGroup(*self.curr_bit_mob[:n])) for n in range(1, self.num_disks + 1)
        ]
        word_list = [
            brace.get_text(text)
        ...
        self.play(
            disk.set_fill, GREEN, disk[1].set_fill, BLACK, eights_bit.set_fill, GREEN
        )
...
class RecursiveSolution(TowersOfHanoiScene):
    ...
    def construct(self):
        ...
        steps = VGroup(
            title, sub_step_brace, sub_steps, sub_sub_steps_brace, sub_sub_steps
        )
        steps.arrange(DOWN)
        steps.scale(0.7)
        steps.to_edge(UP)
        ...
        self.wait()
        self.play(
            GrowFromCenter(sub_sub_steps_brace), *list(map(FadeOut, [arc, q_mark]))
        )
        ...
        self.play(
            FadeIn(self.eyes), big_disk.set_fill, RED, big_disk.label.set_fill, BLACK
        )
        big_disk.add(self.eyes)
        ...
        big_disk.add(self.eyes)
        self.move_subtower_to_peg(
            3, 1, run_time=2, added_anims=[self.eyes.look_at_anim(self.pegs[0])]
        )
...
class KeithSaysBigToSmall(Scene):
    def construct(self):
        ...
        self.play(Blink(keith))
        self.play(
            keith.change_mode, "speaking", ShowCreation(bubble), Write(bubble.content)
        )
        self.wait()
        self.play(Blink(keith))
        self.wait()
...
class BothAreSelfSimilar(Scene):
    def construct(self):
        ...
        self.add(morty)
        self.play(
            morty.change_mode, "hooray", ShowCreation(bubble), Write(bubble.content)
        )
        self.play(Blink(morty))
        self.wait()
...
class RecursiveSolutionToConstrained(RecursiveSolution):
    ...
    def construct(self):
        ...
        steps_to_fade = VGroup(
            title, sub_steps_brace, *list(sub_steps[:2]) + list(sub_steps[3:])
        )
        self.add(title)

        # Initially move big disk
        self.play(
            FadeIn(self.eyes), big_disk.set_fill, GREEN, big_disk.label.set_fill, BLACK
        )
        big_disk.add(self.eyes)
        big_disk.save_state()
        self.blink()
        ...
        self.blink()
        self.move_disk_to_peg(
            num_disks - 1, 1, added_anims=[Write(sub_sub_steps[1], run_time=1)]
        )
        self.wait()
        self.move_subtower_to_peg(
            num_disks - 1,
        ...
        self.blink()
        self.move_disk_to_peg(
            num_disks - 1, 2, added_anims=[Write(sub_sub_steps[3], run_time=1)]
        )
        self.wait()
        big_disk.remove(self.eyes)
        self.move_subtower_to_peg(
...
class SolveConstrainedByCounting(ConstrainedTowersOfHanoiScene):
    ...
    def construct(self):
        ...
        for index in get_ternary_ruler_sequence(self.num_disks - 1):
            self.move_disk(
                index, stay_on_peg=False, added_anims=[self.increment_animation()]
            )
...
class SolveConstrainedWithTernaryCounting(ConstrainedTowersOfHanoiScene):
    ...
    def move_next_disk(self, run_time=None, stay_on_peg=False):
        self.move_disk(
            next(self.disk_index_iter), run_time=run_time, stay_on_peg=stay_on_peg
        )
...
class DescribeTriforcePattern(SierpinskiGraphScene):
    ...
    def construct(self):
        ...
        islands = [
            VGroup(*[self.nodes[i] for i in index_set]) for index_set in index_sets
        ]
...
class ShowSierpinskiCurvesOfIncreasingOrder(Scene):
    ...
    def construct(self):
        ...
        for new_graph, new_path in zip(graphs[1:], paths[1:]):
            self.play(
                Transform(graph, new_graph), Transform(path, new_path), run_time=2
            )
            self.wait()
        self.path = path
