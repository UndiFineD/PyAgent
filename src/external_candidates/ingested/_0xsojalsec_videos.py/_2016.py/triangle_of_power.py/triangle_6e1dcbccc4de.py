# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\triangle_of_power\triangle.py
import numbers
...
class TOP(VMobject):
    ...
    def get_surrounding_circle(self, color=YELLOW):
        return Circle(radius=1.7 * self.radius, color=color).shift(
            self.triangle.get_center(), (self.triangle.get_height() / 6) * DOWN
        )
...
class SixDifferentInverses(Scene):
    def construct(self):
        ...
        vects = it.starmap(
            op.add, it.product([3 * UP, 0.5 * UP, 2 * DOWN], [2 * LEFT, 2 * RIGHT])
        )
...
class AdditiveProperty(Scene):
    def construct(self):
        ...
        self.play(
            ApplyMethod(exp_rule.to_edge, UP), ApplyMethod(log_rule.to_edge, DOWN, 1.5)
        )
        ...
        self.wait()
        all_tops = [
            m for m in t_exp_rule.split() + t_log_rule.split() if isinstance(m, TOP)
        ]
...
class MultiplyWithConstantTop(Scene):
    def construct(self):
        ...
        self.play(
            ...
            Transform(VMobject(*old_tops), VMobject(*new_tops), path_arc=np.pi / 2),
        )
        self.wait(3)
...
class RightStaysConstantQ(Scene):
    def construct(self):
        top1, top2, top3 = old_tops = [
            TOP(None, s, "8") for s in ("x", "y", OldTex("x?y"))
        ]
        ...
        symbols_at_lower_right = VMobject(
            *[top.put_on_vertex(0, top.values[1].copy()) for top in (top1, top2, top3)]
        )
        ...
        self.play(
            ...
            ApplyPointwiseFunction(
                lambda p: (p + 2 * DOWN) * 15 / get_norm(p + 2 * DOWN), bubble
            ),
        ...
        self.wait(4)
        self.play(
            Transform(symbols_at_top, symbols_at_lower_right, path_arc=np.pi / 2),
...
class AOplusB(Scene):
    def construct(self):
        self.add(
            OldTex("a \\oplus b = \\dfrac{1}{\\frac{1}{a} + \\frac{1}{b}}").scale(2)
        )
        self.wait()
...
class ConstantLowerRight(Scene):
    def construct(self):
        ...
        new_syms = VMobject(
            *[t.put_on_vertex(1, s) for t, s in zip(tops, ["x", "y", "x \\oplus y"])]
        )
