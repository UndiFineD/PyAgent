# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\hilbert\section2.py
...
def get_time_line():
    ...
    time_line.set_color_by_gradient(
        PeanoCurve.CONFIG["start_color"], PeanoCurve.CONFIG["end_color"]
    )
...
class HilbertCurveIsPerfect(Scene):
    def construct(self):
        ...
        closest_point_indices = np.apply_along_axis(np.argmin, 1, distance_matrix)
...
class AskMathematicianFriend(Scene):
    def construct(self):
        ...
        self.add(mathy)
        self.wait()
        self.play(ApplyMethod(mathy.blink, rate_func=squish_rate_func(there_and_back)))
        self.wait()
...
class TimeLineAboutSpaceFilling(Scene):
    def construct(self):
        ...
        self.add(time_line)
        self.play(
            ApplyMethod(time_line.shift, -time_line.number_to_point(1900), run_time=3)
        )
        brace = Brace(
            Mobject(
        ...
        self.wait()
        self.play(Transform(time_line, curve_start), FadeOut(brace), FadeOut(words))
...
class HistoryOfDiscover(Scene):
    def construct(self):
        ...
        for year, curve, vect, text in [
            ...
            self.play(ShowCreation(arrow), ShimmerIn(year_mob), ShimmerIn(words))
            self.play(ShowCreation(curve))
            self.wait()
        self.play(ShowCreation(squares))
        self.wait()
        self.play(ApplyMethod(Mobject(*self.mobjects).shift, 20 * (DOWN + RIGHT)))
...
class DefinitionOfCurve(Scene):
    def construct(self):
        ...
        curve_quote.shift(space_filling.get_left() - curve_quote.get_left())
        space_filling = Point(space_filling.get_center())
        end_words = (
            Mobject(*[quote, space_filling, curve_quote])
            .center()
            .to_edge(TOP, buff=0.25)
        )
        ...
        dots = Mobject(
            *[
                Dot(curve.get_points()[n * curve.get_num_points() / 15], color=YELLOW_C)
                for n in range(1, 15)
                if n not in [4, 11]
            ]
        )
...
class PseudoHilbertCurvesDontFillSpace(Scene):
    def construct(self):
        ...
        square.scale(2 ** (-5))
        square.corner.set_color(Color(rgb=curve.rgbas[curve.get_num_points() / 3]))
        square.shift(grid.get_corner(UP + LEFT) - square.get_corner(UP + LEFT))
...
class DefiningCurves(Scene):
    def construct(self):
        words = OldTexText(["One does not simply define the limit \\\\ \
            of a sequence of", "curves", "\\dots"])
        top_words = OldTexText(["curves", "are functions"]).to_edge(UP)
...
class PseudoHilbertCurveAsFunctionExample(Scene):
    ...
    def construct(self, order):
        ...
        phc, arg, result = (
            OldTex(["\\text{PHC}_%d" % order, "(0.3)", "= %s" % result_tex])
            .to_edge(UP)
            .split()
        )
        ...
        self.remove(arrow1)
        self.play(
            FadeIn(grid), Transform(line, curve), Transform(dot1, dot2), run_time=2
        )
        self.wait()
...
class FormalDefinitionOfContinuity(Scene):
    ...
    def setup(self):
        ...
        self.input_homotopy = lambda x_y_z_t: (
            x_y_z_t[0],
            x_y_z_t[1],
            x_y_z_t[3],
        ) + interpolate(left, right, x_y_z_t[3])
        output_size = self.output.get_num_points() - 1
        output_points = self.output.points
        self.output_homotopy = (
            lambda x_y_z_t1: (x_y_z_t1[0], x_y_z_t1[1], x_y_z_t1[2])
            + output_points[int(x_y_z_t1[3] * output_size)]
        )

    def get_circles_and_points(self, min_input, max_input):
        input_left, input_right = [
            self.interval.number_to_point(num) for num in (min_input, max_input)
        ]
        input_circle = Circle(
            radius=get_norm(input_left - input_right) / 2, color=WHITE
        )
        ...
        output_center = output_points.get_points()[
            int(0.5 * output_points.get_num_points())
        ]
    ...
    def label_jump(self):
        jump_points = Mobject(
            Point(self.spiril1.get_points()[-1]), Point(self.spiril2.get_points()[0])
        )
    ...
    def draw_circles(self):
        ...
        for dot in self.input_dot, self.output_dot:
            dot.center()
        kwargs = {"rate_func": lambda t: interpolate(1, input_value, smooth(t))}
        self.play(
            Homotopy(self.input_homotopy, self.input_dot, **kwargs),
            Homotopy(self.output_homotopy, self.output_dot, **kwargs),
        )
        ...
        tup = self.get_circles_and_points(
            input_value - input_radius, input_value + input_radius
        )
    ...
    def discontinuous_point(self):
        point_description = OldTexText("Point where the function jumps")
        point_description.shift(3 * RIGHT)
        discontinuous_at_A = OldTexText("``Discontinuous at A''", size="\\Large")
        ...
        for dot in self.input_dot, self.output_dot:
            dot.center()
        kwargs = {"rate_func": lambda t: interpolate(0.45, input_value, smooth(t))}
        self.play(
            Homotopy(self.input_homotopy, self.input_dot, **kwargs),
            Homotopy(self.output_homotopy, self.output_dot, **kwargs),
        )
        ...
        arrow = Arrow(
            point_description, self.output_dot, buff=0.05, color=self.output_color
        )
        ...
        self.play(vary_circles)
        self.wait()
        self.play(ShimmerIn(discontinuous_at_A), ShowCreation(discontinuous_arrow))
        self.wait(3)
...
class VaryCircles(Animation):
    ...
    def interpolate_mobject(self, alpha):
        ...
        self.mobject = Mobject(
            *self.scene.get_circles_and_points(
                self.input_value - radius, self.input_value + radius
            )
        ).ingest_submobjects()
...
class FormalDefinitionOfHilbertCurve(Scene):
    def construct(self):
        val = 0.7
        text = OldTex(
            ["\\text{HC}(", "x", ")", "=\\lim_{n \\to \\infty}\\text{PHC}_n(", "x", ")"]
        )
        text.to_edge(UP)
        x1 = text.split()[1]
        x2 = text.split()[-2]
        ...
        self.play(ShowCreation(curve))
        self.play(
            ApplyMethod(x1.set_color, BLUE), ShowCreation(x_arrow), ShowCreation(dot)
        )
        self.wait()
        self.remove(x_arrow)
...
class TilingSpace(Scene):
    def construct(self):
        ...
        squares = Mobject(
            *[square.copy().shift(coords) for coords in coords_set]
        ).ingest_submobjects()
        self.play(DelayByOrder(FadeIn(squares)), run_time=3)
        curve = HilbertCurve(order=6).scale(1.0 / 6)
        all_curves = Mobject(
            *[curve.copy().shift(coords) for coords in coords_set]
        ).ingest_submobjects()
