# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\hilbert\section3.py
...
class InfiniteResultsFiniteWorld(Scene):
    def construct(self):
        ...
        for words in left_words, right_words:
            words.scale(0.8)
        left_formula = OldTex("\\sum_{n = 0}^{\\infty} 2^n = -1")
        ...
        right_overwords = OldTexText("\\substack{\
                \\text{How computers} \\\\ \
                \\text{represent $-1$}\
            }").scale(1.5)
        ...
        self.wait()
        self.play(
            ShimmerIn(left_formula), ApplyMethod(left_words.next_to, left_formula, UP)
        )
        self.wait()
        self.play(ShimmerIn(right_formula), Transform(right_words, right_overwords))
        self.wait()
        self.finite_analog(
    ...
    def finite_analog(self, left_mob, arrow, right_mob):
        ...
        middle = OldTex("\\sum_{n=0}^N 2^n \\equiv -1 \\mod 2^{N+1}")
        ...
        left_arrow, right_arrow = [
            Arrow(mob1.get_right()[0] * RIGHT, mob2.get_left()[0] * RIGHT, buff=0)
        ...
        self.wait()
        self.equivalence(left_mob, left_arrow, Mobject(middle, brace, finite_analog))
    ...
    def equivalence(self, left_mob, arrow, right_mob):
        ...
        new_right.shift((words.get_right()[0] - right_mob.get_left()[0] + 0.5) * RIGHT)
...
class InfiniteObjectsEncapsulateFiniteObjects(Scene):
    def get_triangles(self):
        triangle = Polygon(LEFT / np.sqrt(3), UP, RIGHT / np.sqrt(3), color=GREEN)
        triangles = Mobject(
    ...
    def construct(self):
        ...
        infinite_objects = [
            OldTex("\\sum_{n=0}^\\infty", size="\\normal").set_color(RED_E),
            Sierpinski(order=8).scale(0.3),
            OldTexText("$\\exists$ something infinite $\\dots$").set_color(RED_B),
        ]
        finite_objects = [
            OldTex("\\sum_{n=0}^N", size="\\normal").set_color(GREEN_E),
            self.get_triangles(),
            OldTexText("$\\forall$ finite somethings $\\dots$").set_color(GREEN_B),
        ]
        for infinite, finite, n in zip(
            infinite_objects, finite_objects, it.count(1, 2)
        ):
...
class StatementRemovedFromReality(Scene):
    def construct(self):
        ...
        for bub, buddy in [(bubble, mathy), (morty_bubble, morty)]:
            ...
            self.play(
                ApplyMethod(buddy.blink, rate_func=squish_rate_func(there_and_back))
            )
