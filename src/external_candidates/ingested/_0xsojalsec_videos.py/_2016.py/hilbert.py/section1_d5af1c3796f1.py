...
class AboutSpaceFillingCurves(TransformOverIncreasingOrders):
    ...
    def show_infinite_objects(self):
        ...
        rationals, other_equals, naturals = (
            OldTex(["|\\mathds{Q}|", "=", "|\\mathds{N}|"]).scale(2).split()
        )
    ...
    def pose_question(self):
        infinity, rightarrow, N = (
            OldTex(["\\infty", "\\rightarrow", "N"]).scale(2).split()
        )
...
class PostponePhilosophizing(Scene):
    def construct(self):
        abstract, arrow, concrete = (
            OldTexText(["Abstract", " $\\rightarrow$ ", "Concrete"]).scale(2).split()
        )
...
class ImageToSound(Scene):
    def construct(self):
        ...
        self.play(
            Transform(string.mobject, picture_copy, run_time=5, rate_func=rush_from)
        )
...
class SoundDataIsOneDimensional(Scene):
    def construct(self):
        ...
        component_strings = [
            Vibrate(
                num_periods=k + 1, overtones=1, color=color, center=2 * DOWN + UP * k
            )
            for k, color in zip(
                list(range(overtones)), Color(BLUE_E).range_to(WHITE, overtones)
            )
        ]
        dots = [
    ...
    def construct(self):
        ...
        pixel = MobjectFromRegion(
            region_from_polygon_vertices(
                *0.2 * np.array([RIGHT + DOWN, RIGHT + UP, LEFT + UP, LEFT + DOWN])
            )
        )
...
class ListenToAllPixels(Scene):
    def construct(self):
        ...
        colors = [
            Color(rgb=interpolate(np.array(red.rgb), np.array(blue.rgb), alpha))
            for alpha in np.arange(4) / 3.0
        ]
        ...
        words = OldTex(
            "&\\vdots \\\\ \\text{thousands }& \\text{of frequencies} \\\\ &\\vdots"
        )
        ...
        self.play(
            *[TransformAnimations(sub_vib, vibration) for sub_vib in sub_vibrations]
            + [FadeOut(words)]
        )
        self.clear()
        self.add(freq_line)
    ...
    def construct(self, order):
        ...
        for square in squares.submobjects:
            ...
            distances = np.apply_along_axis(
                lambda p: get_norm(p - center), 1, curve.points
            )
...
class HilbertBetterThanSnakeQ(Scene):
    def construct(self):
        ...
        for new_hc, new_sc in zip(hilbert_curves[1:], snake_curves[1:]):
            self.play(
                *[Transform(hilbert_curve, new_hc), Transform(snake_curve, new_sc)]
            )
            self.wait()
...
class IncreaseResolution(Scene):
    def construct(self):
        grids = [
            Grid(2**order, 2**order, stroke_width=1).shift(0.3 * DOWN)
            for order in (6, 7)
        ]
        grid = grids[0]
    ...
    def construct(self):
        ...
        start_dots, end_dots = [
            Mobject(
                *[
                    Dot(
                        curve.get_points()[int(x * curve.get_num_points())], color=color
                    )
...
class TrackSpecificCurvePoint(Scene):
    ...
    def construct(self):
        ...
        dots = [
            Dot(curve.get_points()[alpha * curve.get_num_points()]) for curve in curves
        ]
