...
class TwoTypesOfVideos(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            morty.change_mode, "raise_right_hand", morty.look_at, box, ShowCreation(box)
        )
        for x in range(3):
            self.wait(2)
            self.play(Blink(morty))
...
class MakeALotOfPiCreaturesHappy(Scene):
    def construct(self):
        ...
        pis = VGroup(
            *[
                VGroup(*[Randolph() for x in range(7)]).arrange(
...
class IntegrationByParts(Scene):
    def construct(self):
        ...
        equation = OldTex(
            "\\int_0^1 g\\,df", "+\\int_0^1 f\\,dg", "= \\big(fg \\big)_0^1"
        )
        equation.to_edge(UP)
