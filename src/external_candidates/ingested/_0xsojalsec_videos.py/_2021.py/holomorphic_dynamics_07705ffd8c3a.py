...
class HolomorphicDynamics(Scene):
    ...
    def complex_functions(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn, examples, shift=0.25 * DOWN, run_time=3, lag_ratio=0.5
            )
        )
    ...
    def repeated_functions(self):
        ...
        self.wait()
        self.play(
            LaggedStartMap(FadeOut, VGroup(words, iterations, rule, self.title_rect))
        )
...
class ShowFatouAndJulia(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(Write(VGroup(*figure[2:])) for figure in figures), lag_ratio=0.7
            )
        )
        self.play(
...
class FixedPoints(Scene):
    def construct(self):
        ...
        def get_arrows(point, inward=False):
            arrows = VGroup(
                *(Arrow(ORIGIN, vect, buff=0.3) for vect in compass_directions(8))
            )
            arrows.set_height(1)
...
class DescribeDerivative(Scene):
    ...
    def construct(self):
        ...
        if self.zoom_in_frame:
            ...
            fade_anims = [
                ApplyMethod(
                    dots.set_opacity, 0, rate_func=squish_rate_func(smooth, 0.5, 1.0)
                ),
...
class Cycles(FixedPoints):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                morty, "How many solutions?", bubble_config={"height": 2, "width": 4}
            ),
        )
        self.play(Blink(morty))
        self.wait()
...
class TwoToMillionPoints(Scene):
    ...
    def get_julia_set_points(self, plane, n_points, n_steps):
        values = np.array(
            [complex(math.cos(x), math.sin(x)) for x in np.linspace(0, TAU, n_points)]
        )
...
class ShowCenterOfMassPoint(PlayWithRootsSeekingCycles):
    ...
    def construct(self):
        ...
        mean_dot.add_updater(
            lambda m: m.move_to(sum([rd.get_center() for rd in self.root_dots]) / 3)
        )
        self.add(mean_dot)
        ...
        self.play(
            MoveAlongPath(self.root_dots[2], circle, rate_func=linear, run_time=20)
        )
        self.play(
...
class GenerateCubicParameterPlot(Scene):
    ...
    def construct(self):
        ...
        left_fractal.add_updater(
            lambda m: m.set_roots([planes[0].p2n(rd.get_center()) for rd in root_dots])
        )
        ...
        com_dot.add_updater(
            lambda m: m.move_to(np.array([rd.get_center() for rd in root_dots]).mean(0))
        )
...
class SmallCircleProperty(Scene):
    def construct(self):
        ...
        semis = (
            circles[1].copy().pointwise_become_partial(circles[1], 0, 0.5).replicate(2)
        )
...
class AboutJuliaDisks(AboutFatouDisks):
    def construct(self):
        ...
        words4 = OldTexText(
            "``Stuff goes everywhere'' principle of Julia sets", font_size=60
        )
...
class SimulationOfTinyDisk(RepeatedNewton):
    ...
    def construct(self):
        ...
        julia_set.add_updater(
            lambda m: m.set_julia_highlight(get_height_ratio() * 1e-3)
        )
