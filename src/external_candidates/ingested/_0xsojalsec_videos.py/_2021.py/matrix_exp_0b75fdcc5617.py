...
class ZoomInOnProblem(Scene):
    def construct(self):
        ...
        for part in ["e^{At}", "matrix A"]:
            self.play(
                FlashAround(problem.get_part_by_tex(part), run_time=2, time_width=4)
            )
            self.wait()
        ...
        # Show inputs
        examples_label = OldTexText(
            "Various matrices to\\\\plug in for $A$", font_size=40
        )
...
class PlanForThisVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                s0, OldTexText("But what\\\\is $e^{M}$?"), target_mode="raise_left_hand"
            ),
            s1.change("erm", UL),
...
class IntroduceTheComputation(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                *(FadeIn(part) for part in real_equation[4:] if part not in xs)
            ),
        ...
        rects = VGroup(
            *(Rectangle(height=2**n / math.factorial(n), width=1) for n in range(11))
        )
        ...
        for n in range(11):
            ...
            self.play(
                ...
                UpdateFromAlphaFunc(
                    value, lambda m, a: m.next_to(rect, UP, SMALL_BUFF).set_opacity(a)
                ),
        ...
        self.play(
            ...
            *(
                FadeTransform(dec.copy().set_opacity(0), sum_value)
                for dec in term_values
            ),
        )
        self.play(Blink(randy))
        ...
        for n, rect, value_mob in zip(it.count(0), rects, term_values):
            ...
            rect.target.set_height(
                unit_height * new_value, stretch=True, about_edge=DOWN
            )
            value_mob.rect = rect
            anims += [
                MoveToTarget(rect),
        ...
        self.show_mat_mult(
            matrix, matrix, square_eq[0][2:11], square_eq[1][2:11], result.elements
        )
        ...
        sum_eq = VGroup(
            ...
            Matrix(
                np.array(
                    [f"{m1} + {m2}" for m1, m2 in zip(mat1.flatten(), mat2.flatten())]
                ).reshape((3, 3)),
                h_buff=1.8,
            ),
        )
        ...
        last_rects = VGroup()
        for e1, e2, e3 in zip(
            sum_eq[0].elements, sum_eq[2].elements, sum_eq[4].elements
        ):
        ...
        pi_mat_tex = ""
        pi_mat_tex = (
            "\\left[ \\begin{array}{cc} 0 & -\\pi \\\\ \\pi & 0 \\end{array} \\right]"
        )
        pi_mat_rhs = OldTex(
    ...
    def show_mat_mult(
        self, m1, m2, m1_terms, m2_terms, rhs_terms, per_term=0.1, between_terms=0.35
    ):
        dim = m1.shape[0]
        ...
        for n in range(dim * dim):
            ...
            for k in range(dim):
                ...
                right_elem.scale(
                    rhs_terms[0][0].get_height() / right_elem[-1].get_height()
                )
    ...
    def construct(self):
        ...
        self.play(
            self.change_students(
                "confused", "pondering", "raise_left_hand", look_at=self.screen
            ),
...
class DefinitionFirstVsLast(Scene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            versus.animate.move_to(
                midpoint(low_title.get_top(), top_rect.get_bottom())
            ),
        )
        self.wait()
...
class RomeoAndJuliet(Scene):
    def construct(self):
        ...
        self.play(
            juliet.love_tracker.animate.set_value(-5), rate_func=linear, run_time=5
        )
        ...
        self.play(
            romeo.love_tracker.animate.set_value(-3), run_time=4, rate_func=linear
        )
        ...
        juliet.love_tracker.add_updater(
            lambda m: m.set_value(ps_point.get_location()[0])
        )
        romeo.love_tracker.add_updater(
            lambda m: m.set_value(ps_point.get_location()[1])
        )
        self.add(ps_point)
    ...
    def get_romeo_juilet_name_labels(
        self, lovers, font_size=36, spacing=1.2, buff=MED_SMALL_BUFF
    ):
        name_labels = VGroup(
            *(Text(name, font_size=font_size) for name in ["Romeo", "Juliet"])
        )
        ...
        def update_func(pi):
            ...
            if love < cutoff_values[0]:
                ...
                alpha = inverse_interpolate(
                    cutoff_values[i - 1], cutoff_values[i], love
                )
    ...
    def get_love_scale(self, creature, direction, var_name, color):
        ...
        dot = Dot(color=color)
        dot.add_updater(
            lambda m: m.move_to(number_line.n2p(creature.love_tracker.get_value()))
        )

        label = VGroup(OldTex(var_name, "=", font_size=36), DecimalNumber(font_size=24))
        label.set_color(color)
        label[0].shift(
            label[1].get_left() + SMALL_BUFF * LEFT - label[0][1].get_right()
        )
        label.next_to(number_line, UP)
        label[1].add_updater(
            lambda m: m.set_value(creature.love_tracker.get_value()).set_color(color)
        )
...
class DiscussSystem(Scene):
    def construct(self):
        ...
        bubble[:3].rotate(
            30 * DEGREES, about_point=bubble[3].get_center() + 0.2 * RIGHT
        )
...
class MoreGeneralSystem(Scene):
    def construct(self):
        ...
        self.add(equations)
        self.play(
            LaggedStartMap(FadeIn, equations, shift=UP, lag_ratio=0.5, run_time=3)
        )
        self.wait()
...
class HowExampleLeadsToMatrixExponents(Scene):
    def construct(self):
        ...
        mat_exp = get_matrix_exponential(
            [["a", "b"], ["c", "d"]], height=2, h_buff=0.95, v_buff=0.75
        )
...
class RomeoJulietVectorSpace(RomeoAndJuliet):
    def construct(self):
        ...
        name_labels = self.get_romeo_juilet_name_labels(
            lovers, font_size=36, spacing=1.1
        )
        ...
        # Show equation
        rhs = VGroup(
            OldTex("="), Matrix([["-y(t)"], ["x(t)"]], bracket_h_buff=SMALL_BUFF)
        )
        ...
        # Fade all else out
        self.play(
            FadeOut(VGroup(gf_rect, general_form, new_eq, deriv_lhs, deriv_label))
        )
        self.wait(4 * TAU)
...
class From2DTo1D(Scene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                ...
                lambda m, a: m.put_start_and_end_on(
                    interpolate(
                        deriv_underline.get_start(), deriv_vector.get_start(), a
                    ),
        ...
        self.play(
            FadeOut(rhs_rect), LaggedStartMap(GrowArrow, vector_field, lag_ratio=0)
        )
        self.wait(14)
        ...
        vector.add_updater(
            lambda m: m.put_start_and_end_on(nl.n2p(0), nl.n2p(vt.get_value()))
        )
        deriv_vector.add_updater(
            lambda m: m.set_width(0.5 * vector.get_width()).move_to(
                vector.get_right(), LEFT
            )
        )
...
class SchroedingersEquationIntro(Scene):
    def construct(self):
        ...
        self.play(
            ...
            Write(equation[3]),
            ReplacementTransform(
                original_equation[0], equation[4], path_arc=90 * DEGREES
            ),
            ReplacementTransform(original_equation[4:], equation[5:]),
            state_arrows.animate.become(
                VGroup(
                    *(
                        Arrow(state_label, psi)
                        for psi in equation.get_parts_by_tex("\\psi")
                    )
                )
            ),
...
class GraphAndHistoryOfExponential(Scene):
    def construct(self):
        ...
        solution = OldTex(
            "x({t}) = e^{r{t} }", tex_to_color_map={"{t}": GREY_B, "r": BLUE}
        )
        ...
        words.add_updater(
            lambda m: m.become(original_words)
            .set_fill(opacity=fill_tracker.get_value())
            .apply_function(func)
        )
        ...
        self.play(
            ApplyMethod(
                original_words.next_to, axes.c2p(14, 0), UP, SMALL_BUFF, run_time=2
            ),
        ...
        for sec_dot in dot1, dot2:
            ...
            sec_dot.add_updater(
                lambda d: d.move_to(
                    axes.input_to_graph_point(d.x_tracker.get_value(), exp_graph)
                )
            )
        ...
        for name1, name2, letter, year in [
            ...
            for im, name, index in [(im1, name1, 0), (im2, name2, 4)]:
                ...
                rect = SurroundingRectangle(
                    im, buff=0, stroke_color=GREY_A, stroke_width=1
                )
        ...
        self.play(
            LaggedStartMap(FadeIn, b_group, shift=0.5 * UP, lag_ratio=0.2, run_time=1)
        )
        ...
        self.play(
            ...
            FadeIn(book),
            FadeIn(date),
            e_usage.animate.scale(1.4)
            .next_to(book, RIGHT, aligned_edge=UP)
            .shift(0.25 * DOWN),
        )
        self.wait()
...
class BernoullisThoughts(Scene):
    def construct(self):
        ...
        continuous_money = VGroup(
            *(
                dollar.copy().shift(
                    smooth(x) * RIGHT * 2.0 + np.exp(smooth(x) - 1) * UP
                )
...
class CompoundInterestPopulationAndEpidemic(Scene):
    def construct(self):
        ...
        points = np.array(
            list(sorted(points, key=lambda p: get_norm(p) + 0.5 * random.random()))
        )
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                dollars, update_height, rate_func=lambda t: smooth(1 - t), remover=True
            ),
        )
        self.wait(4)
        self.play(
            ...
            UpdateFromAlphaFunc(
                people, update_height, rate_func=lambda t: smooth(1 - t), remover=True
            ),
        )
        self.wait(4)
        ...
        self.play(
            ...
            LaggedStart(
                *(
                    ...
                    for m1, m2 in zip(
                        [eq1d[:2], eq1d[2:5], eq1d[5], eq1d[6], eq1d[7:]], eq2d
                    )
                ),
                lag_ratio=0.05,
            ),
...
class ManySolutionsDependingOnInitialCondition(VideoWrapper):
    def construct(self):
        ...
        step = 0.05
        graphs = VGroup(
            *(get_graph(x0) for x0 in np.arange(step, axes.y_range[1], step))
        )
        ...
        alt_rhs.next_to(
            solution.get_part_by_tex("="), DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
        ...
        self.play(
            FlashAround(
                solution.get_part_by_tex("e"), time_width=2, buff=0.05, run_time=2
            )
        )
        self.wait()
        self.play(
            LaggedStartMap(FadeIn, alt_rhs, shift=0.5 * DOWN, lag_ratio=0.6, run_time=5)
        )
        self.wait()
        self.play(
        ...
        group = VGroup(
            VGroup(one, sol_label.target).arrange(
                RIGHT, buff=MED_SMALL_BUFF, aligned_edge=DOWN
            ),
            many,
        )
        ...
        x0_label.add_updater(
            lambda m: m.move_to(axes.c2p(0, get_x0()), LEFT).shift(sv * LEFT)
        )
...
class ExoticExponentsWithEBase(Scene):
    def construct(self):
        ...
        equations = VGroup(
            ...
            OldTex(
                "{\\partial \\over \\partial t}f(x, t) = {\\partial \\over \\partial x}f(x, t)"
            ),
        )
        ...
        self.play(
            VGroup(
                labels[0], labels[2:], exps[0], exps[2:], equations[0], equations[2:]
...
class ExpRotMatrixComputation(Scene):
    def construct(self):
        ...
        final_result = Matrix(
            [["\\cos(t)", "-\\sin(t)"], ["\\sin(t)", "\\cos(t)"]], h_buff=2.0
        )
        final_result.next_to(new_lhs, RIGHT)

        anims = []
        colors = [BLUE_B, BLUE_D, BLUE_D, BLUE_B]
        for color, entry1, entry2 in zip(
            colors, rhs.get_entries(), final_result.get_entries()
        ):
...
class ThatsHorrifying(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.teacher_says(
            "Just wait", bubble_config={"height": 3, "width": 3.5}, target_mode="tease"
        )
        self.play_student_changes(
            "pondering",
            "thinking",
...
class ReadColumnsOfRotationMatrix(Scene):
    ...
    def construct(self):
        ...
        for label, color, rot_b, direction in zip(
            rot_basis_labels, [GREEN, RED], rot_basis_vectors, [UR, LEFT]
        ):
            label.set_color(color)
            label.scale(0.7)
        ...
        v_line = DashedLine(
            plane.c2p(math.cos(angle), 0), plane.c2p(math.cos(angle), math.sin(angle))
        )
...
class AnalyzeRomeoAndJulietSpace(RomeoJulietVectorSpace):
    ...
    def get_romeo_juliet_axes(self, height=5):
        ...
        name_labels = self.get_romeo_juilet_name_labels(
            lovers, font_size=12, spacing=1.0, buff=0.05
        )
...
class GeometricReasoningForRomeoJuliet(AnalyzeRomeoAndJulietSpace):
    def construct(self):
        ...
        self.play(
            deriv_vect.animate.put_start_and_end_on(
                ps_arrow.get_start(), ps_arrow.get_end()
            ).shift(0.2 * UP),
            run_time=3,
...
class ComplexEquation(Scene):
    def construct(self):
        ...
        braces = VGroup(
            *(Brace(equation[i:j], UP, buff=SMALL_BUFF) for i, j in [(2, 8), (11, 17)])
        )
...
class RotationIn3dPlane(Scene):
    def construct(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                v_vect, lambda m, a: m.set_fill(interpolate_color(BLUE, RED, a))
            ),
            ShowCreation(arc),
            run_time=2,
        )
...
class StoryForAnotherTime(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            ...
            added_anims=[
                self.change_students("confused", "erm", "hesitant", look_at=self.screen)
            ],
        )
        self.wait(5)
...
class BasicVectorFieldIdea(Scene):
    ...
    def construct(self):
        ...
        plane = NumberPlane()
        vector_field = VectorField(
            func, plane, magnitude_range=(0, 2), vector_config={"thickness": 0.025}
        )
        dots = VGroup(
            *(Dot(v.get_start(), radius=0.02, fill_color=YELLOW) for v in vector_field)
        )
        ...
        self.wait()
        self.play(
            FadeOut(dots), FadeOut(term_labels), vector_field.animate.set_opacity(0.25)
        )
        ...
        self.play(
            FadeIn(vector),
            ReplacementTransform(
                vector.copy().set_opacity(0), Mv, path_arc=-45 * DEGREES
            ),
...
class MoreShakesperianRomeoJuliet(RomeoAndJuliet):
    def construct(self):
        ...
        romeo.love_tracker.add_updater(
            lambda m: m.set_value(plane.p2c(ps_point.get_center())[0])
        )
        juliet.love_tracker.add_updater(
            lambda m: m.set_value(plane.p2c(ps_point.get_center())[1])
        )
...
class TransitionWrapper(Scene):
    def construct(self):
        ...
        vf_words = OldTexText(
            "Vector field defined by $\\vec{\\textbf{v} } \\rightarrow M\\vec{\\textbf{v} }$"
        )
...
class DerivativeOfExpMt(Scene):
    def construct(self):
        ...
        equation = OldTex(
            "{d \\over dt} \\vec{\\textbf{v} }({t}) = M \\vec{\\textbf{v} }({t})", **kw
        )
        ...
        self.add(solution)
        self.play(
            GrowArrow(arrow), FadeIn(equation, RIGHT), FadeIn(solves, lag_ratio=0.1)
        )
        self.wait()
        ...
        lines = VGroup(
            *(OldTex(tex.replace("v_0", v0_tex), **kw) for tex in tex_expressions)
        )
        ...
        l1_indices, l2_indices, l3_indices = [
            [
                ...
                for part in it.chain(
                    lines[i].get_parts_by_tex("="), lines[i].get_parts_by_tex("+")
                )
            ]
            for i in (1, 2, 3)
        ]
        ...
        last_rects = VMobject()
        for l1i, l1j, l2i, l2j in zip(
            l1_indices, l1_indices[1:], l2_indices, l2_indices[1:]
        ):
        ...
        last_rects = VMobject()
        for l2i, l2j, l3i, l3j in zip(
            l2_indices, l2_indices[1:], l3_indices, l3_indices[1:]
        ):
...
class TryIt(Scene):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(morty, "Brace yourself now...", target_mode="hesitant")
        )
        morty.look(ORIGIN)
        self.wait(2)
...
class TracePropertyAndComputation(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(
            "pondering", "confused", "pondering", look_at=trace_eq
        )
        self.wait(2)
...
class ExponentialPhaseFlow(Scene):
    ...
    def construct(self):
        ...
        basis_vectors[0].add_updater(
            lambda m: m.put_start_and_end_on(plane.get_origin(), box.pfp(7 / 8))
        )
        basis_vectors[1].add_updater(
            lambda m: m.put_start_and_end_on(plane.get_origin(), box.pfp(1 / 8))
        )
