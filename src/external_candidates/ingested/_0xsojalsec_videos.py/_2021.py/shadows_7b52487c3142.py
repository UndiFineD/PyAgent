...
def update_shadow(shadow, mobject, light_source):
    ...
    for sm, mm in zip(
        shadow.family_members_with_points(), mobject.family_members_with_points()
    ):
...
class ShadowScene(ThreeDScene):
    ...
    def add_plane(self):
        ...
        if self.limited_plane_extension > 0:
            plane.set_height(
                height // 2 + self.limited_plane_extension, about_edge=UP, stretch=True
            )
        self.plane = plane
    ...
    def begin_ambient_rotation(
        self, mobject, speed=0.2, about_point=None, initial_axis=[1, 1, 1]
    ):
    ...
    def get_shadow_outline(self, stroke_width=1):
        ...
        outline.add_updater(
            lambda m: m.set_points_as_corners(get_convex_hull(self.shadow)).close_path()
        )
        return outline
    ...
    def random_toss(
        self, mobject=None, angle=TAU, about_point=None, meta_speed=5, **kwargs
    ):
        if mobject is None:
            mobject = self.solid
        ...
        def update(mob, time):
            ...
            mob.rot_time = time
            mob.rot_axis = rotate_vector(
                mob.rot_axis, meta_speed * dt, normalize(np.random.random(3))
            )
    ...
    def init_frame_rotation(self, factor=0.0025, max_speed=0.01):
        ...
        def update_frame(frame, dt):
            ...
            frame.increment_theta(
                clip(factor * frame.d_theta, -max_speed * dt, max_speed * dt)
            )
...
class IntroduceShadow(ShadowScene):
    ...
    def construct(self):
        ...
        frame = self.camera.frame
        frame.add_updater(
            lambda f, dt: f.increment_theta(0.01 * dt)
        )  # Ambient rotation
        ...
        vert_proj_lines = VGroup(
            *(DashedLine(*pair) for pair in zip(verts, proj_dots.get_points()))
        )
        ...
        text = OldTexText(
            "Take the mean.", "\\\\What does that\\\\approach?", font_size=30
        )
...
class PauseAndPonder(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says(
            ...
            added_anims=[
                self.change_students("tease", "well", "pondering", look_at=self.screen)
            ],
        )
        self.wait(2)
        self.play(
...
class FocusOnOneFace(ShadowScene):
    ...
    def construct(self):
        ...
        light_lines = self.get_light_lines(
            n_lines=4, outline=shadow, only_vertices=True
        )
        ...
        self.play(
            ...
            TransformFromCopy(
                face, face.deepcopy().set_opacity(0).set_z(0), remover=True
            ),
            run_time=3,
        )
        self.wait(3)
        ...
        self.play(
            Rotate(face, -PI / 3, UP),
            UpdateFromAlphaFunc(
                light_lines, lambda m, a: m.set_opacity(0.5 * (1 - a)), remover=True
            ),
            run_time=2,
        )
        ...
        theta.add_updater(
            lambda m: m.move_to(get_fc() + 1.3 * (arc.pfp(0.5) - get_fc()))
        )
        ...
        line = Line(
            long_slice.get_corner(LEFT + OUT), long_slice.get_corner(RIGHT + IN)
        )
        line.scale(0.97)
        ...
        ul_arc = Arc(
            radius=0.4, start_angle=-get_theta(), angle=-(PI / 2 - get_theta())
        )
        ...
        lower_formula = OldTex(
            "\\text{shadow}", "=", "\\cos(\\theta)", "\\cdot", "\\text{slice}"
        )
...
class DiscussLinearity(Scene):
    def construct(self):
        ...
        arrows = VGroup(
            *(
                Arrow(p1.get_top(), p2.get_top(), path_arc=-0.6 * PI).scale(
                    0.75, about_edge=DOWN
                )
        ...
        self.play(
            LaggedStart(
                *map(FadeOut, (*det, rot_brace, details)), lag_ratio=0.3, run_time=2
            )
        )
        ...
        self.play(
            arrow.animate.match_points(
                Arrow(factor.get_corner(UL), ind_words.get_corner(DR))
            ),
    ...
    def construct(self):
        ...
        self.play(
            ReplacementTransform(
                planes[0].copy().fade(1), planes[0].output, run_time=2
            ),
        ...
        self.play(
            ...
            new_area_label[1]
            .animate.next_to(det, LEFT, SMALL_BUFF)
            .match_y(new_area_label[1]),
        )
        self.wait()
    ...
    def construct(self):
        ...
        if display_mode == "full_3d":
            ...
            proj_lines = always_redraw(
                lambda: VGroup(
                    *(
                        Line(rotated.pfp(a), flat_project(rotated.pfp(a))).set_stroke(
                            WHITE, 0.5, 0.2
                        )
        ...
        angle_anim = rotated.angle_tracker.animate.set_value(
            np.round(angle / TAU, 0) * TAU
        )
        ...
        if self.display_mode == "full_3d":
            ...
            anims += [
                UpdateFromAlphaFunc(
                    proj_lines, lambda m, a: m.set_stroke(opacity=pso * (1 - a))
                ),
                UpdateFromAlphaFunc(
                    light_lines, lambda m, a: m.set_stroke(opacity=lso * a)
                ),
                angle_anim,
        ...
        self.play(
            rotated.angle_tracker.animate.set_value(70 * DEGREES + TAU), run_time=2
        )
        self.play(
...
class AverageShadowAnnotation(Scene):
    def construct(self):
        ...
        formula = VGroup(
            ...
            OldTex("=").rotate(PI / 2),
            OldTexText(
                "$c$", " $\\cdot$", f"(Area({shape_name}))", tex_to_color_map=t2c
            ),
        )
...
class AlicesFaceAverage(Scene):
    def construct(self):
        ...
        for fra, area, s_label, panel in zip(
            fra_labels, area_shadow_labels, s_labels, panels
        ):
        ...
        new_pieces = VGroup(
            *(sm for sm in sample_average.target if sm.get_tex() not in ["A", "\\cdot"])
        )
        new_A = As[0].copy()
...
class AllPossibleOrientations(ShadowScene):
    ...
    def construct(self):
        ...
        sphere_lines = VGroup(
            *(Line(sphere.get_center(), p) for p in sphere_dots.get_points())
        )
        ...
        prob = Group(
            *Tex("P(", "0.", ")", "=", "{Num ", "\\over ", "Den}", font_size=60)
        )
        prob.fix_in_frame()
        prob.to_corner(UR)
        prob.shift(DOWN)
        ...
        self.play(
            LaggedStart(
                *map(FadeOut, (sphere, sphere_mesh, patch, *prob, uniform_words))
            )
        )
        self.play(
        ...
        def get_arc():
            ...
            result.rotate(
                angle_of_vector([*get_normal()[:2], 0]), OUT, about_point=ORIGIN
            )
            result.shift(fc)
        ...
        brace_text = OldTexText(
            "How do you average this\\\\over the sphere?", font_size=36
        )
        ...
        prob_words = OldTexText(
            "How likely is a given value of $\\theta$?", font_size=36
        )
        ...
        nonsense = nonsense_brace.get_text(
            "Not really a sensible expression", font_size=36
        )
        ...
        self.play(
            FadeTransformPieces(loose_sum[1:4], integral[1:4]), Write(integral[4])
        )
        self.wait(5)
        face.clear_updaters()
        self.wait(5)
        ...
        def set_theta(face, theta):
            face.apply_matrix(
                rotation_between_vectors(normal_vect.get_vector(), OUT), about_point=fc
            )
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                face,
                lambda f, a: set_theta(
                    face, theta_samples[int(a * (len(theta_samples) - 2))]
                ),
            ),
            run_time=4,
        )
        ...
        denom_label.scale(
            area_question.target[0].get_height() / denom_label[0][0].get_height()
        )
        ...
        self.play(
            ...
            *map(
                FadeOut, [prob[5], numer_rect, denom_rect, area_question, denom_label]
            ),
        )
        self.wait()
        self.play(
        ...
        self.play(
            FadeIn(average_eq[1]),
            UpdateFromAlphaFunc(
                face, lambda f, a: f[0].set_fill(opacity=0.5 * (1 - a))
            ),
            FadeIn(sphere),
            FadeIn(tick_marks),
            FadeIn(sphere_mesh),
        ...
        self.play(
            LaggedStart(
                ShowCreationThenFadeOut(
                    SurroundingRectangle(average_copy[1][-3]).fix_in_frame()
                ),
        ...
        brace_text = brace.get_text(
            "What does this approach for finer subdivisions?", font_size=30
        )
        ...
        for n in range(1, 4):
            new_ticks = get_tick_marks(
                np.linspace(0, PI, sphere_mesh.resolution[0] * 2**n), tl=0.05 / n
            )
            self.play(
        ...
        new_lines = VGroup(
            OldTex(
                "{1 \\over 2} s^2 \\cdot 2 \\int_0^{\\pi / 2} \\cos(\\theta)\\sin(\\theta)\\,d\\theta"
            ),
            OldTex(
                "{1 \\over 2} s^2 \\cdot \\int_0^{\\pi / 2} \\sin(2\\theta)\\,d\\theta"
            ),
            OldTex(
                "{1 \\over 2} s^2 \\cdot \\left[ -\\frac{1}{2} \\cos(2\\theta) \\right]_0^{\\pi / 2}"
            ),
            OldTex(
        ...
        annotations = VGroup(
            OldTexText(
                "To avoid the annoying absolute value, just\\\\cover the northern hemisphere and double it."
            ),
            OldTexText(
                "Trig identity: $\\sin(2\\theta) = 2\\cos(\\theta)\\sin(\\theta)$"
            ),
...
class AskAboutAverageCosValue(AllPossibleOrientations):
    def construct(self):
        ...
        theta.set_backstroke(width=2)
        theta.add_updater(
            lambda m: m.next_to(arc.pfp(0.5), arc.pfp(0.5) - fc, buff=0.05)
        )
...
class ThreeCamps(TeacherStudentsScene):
    def construct(self):
        ...
        for student, phrase, mode, height in zip(
            reversed(students), phrases, modes, heights
        ):
            self.play(
                PiCreatureSays(
                    student,
        ...
        self.play(
            ...
            LaggedStart(
                *(student.change("pondering", integral) for student in students)
            ),
            FadeIn(integral, UP),
...
class AlicesInsights(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(*(FadeIn(insight[:2], 0.25 * DOWN) for insight in insights))
        )
        self.wait()
...
class TwoToOneCover(ShadowScene):
    ...
    def construct(self):
        ...
        face_shadow_area = DecimalNumber(
            get_norm(face_shadow.get_area_vector()) / (self.unit_size**2)
        )
...
class DefineConvexity(Scene):
    def construct(self):
        ...
        self.play(
            title[
                definition.index("if") : definition.index("is", 16)
...
class ShowGridSum(TwoToOneCover):
    def construct(self):
        ...
        equation = self.get_top_expression(
            " = ", "$\\displaystyle \\frac{1}{2}$", n_faces=n_faces
        )
        self.add(equation)
        ...
        self.play(
            LaggedStart(
                *(ReplacementTransform(lhs.alt2.copy(), target) for target in lhss)
            ),
        ...
        self.play(
            ...
            LaggedStart(
                *(FadeTransform(summand.alt2.copy(), part) for part in grid[0][0:7:2])
            ),
        ...
        col_rects = VGroup(
            *(SurroundingRectangle(col, buff=SMALL_BUFF) for col in cols)
        )
        ...
        self.play(
            LaggedStart(
                VGroup(corner_rect, mean_sa)
                .animate.scale(1.25)
                .move_to(frame.target, UL)
                .shift(0.1 * DR),
                MoveToTarget(frame),
        ...
        rhss = VGroup(
            ...
            OldTex(
                "=", "\\frac{1}{2}", "{c}", "\\cdot ", "(\\text{Surface area})", **kw
            ),
        )
...
class FromRowsToColumns(Scene):
    def construct(self):
        ...
        col_rects = VGroup(
            *(SurroundingRectangle(grids[1][k::n], buff=buff) for k in range(n))
        )
...
class SupposedlyObviousProportionality(ShadowScene):
    ...
    def construct(self):
        ...
        # Ask about constant
        question = OldTexText(
            "What is $c$?!", font_size=72, tex_to_color_map={"$c$": RED}
        )
        question.fix_in_frame()
    ...
    def construct(self):
        ...
        area.add_updater(
            lambda m: m.set_value(
                get_norm(outline.get_area_vector()) / (self.unit_size**2)
            )
        )
...
class PopularizaitonVsDoing(Scene):
    def construct(self):
        ...
        def set_bar_alpha(alpha, **kwargs):
            self.play(
                left_bar.animate.set_width(
                    alpha * width, about_edge=LEFT, stretch=True
                ),
                right_bar.animate.set_width(
                    (1 - alpha) * width, about_edge=RIGHT, stretch=True
                ),
                **kwargs,
            )
...
class CleverProofExample(Scene):
    def construct(self):
        ...
        self.play(
            initial_sum.animate.set_width(8).to_edge(UP), FadeIn(tri, lag_ratio=0.1)
        )
        self.wait()
    ...
    def construct(self):
        ...
        for highlight, phrase in zip(highlights, phrases):
            hlines.add(
                VGroup(
                    DashedLine(
                        highlight.get_corner(UR), phrase.get_corner(UL), buff=0.1
                    ),
                    DashedLine(
                        highlight.get_corner(DR), phrase.get_corner(DL), buff=0.1
                    ),
                )
            )
...
class FamousMathematicians(Scene):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(*map(FadeIn, (*im_groups, dots)), lag_ratio=0.25), run_time=5
        )
        self.wait()
        ...
        self.play(
            ...
            LaggedStart(
                *(FadeOut(mob, DR) for mob in (*im_groups[1:], dots)), lag_ratio=0.25
            ),
            run_time=2,
        )
        self.wait()
...
class QuantifyConvexity(Scene):
    def construct(self):
        ...
        frac = VGroup(
            four_shadow, Line().match_width(four_shadow).set_stroke(width=2), sa
        )
        frac.arrange(DOWN)
        frac.set_width(3)
        frac.to_corner(UR)
