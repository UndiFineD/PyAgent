# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2021\quick_eigen.py
...
def det_path_anim(matrix, run_time=2):
    ...
    path.set_points_smoothly(
        [
            ...
            *[matrix.get_entries()[i].get_center() for i in [0, 3, 1, 2]],
            matrix.get_corner(DL),
        ]
    )
    path.set_stroke(BLUE, 3)

    return VShowPassingFlash(
        path, time_width=1, run_time=run_time, rate_function=linear
    )
...
class Assumptions(TeacherStudentsScene):
    def construct(self):
        self.play(
            PiCreatureSays(
                self.teacher,
                OldTexText("I'm assuming you know\\\\ what eigenvalues are."),
            ),
    ...
    def construct(self):
        ...
        self.play(
            RemovePiCreatureBubble(
                self.teacher, target_mode="raise_right_hand", look_at=recap_words
            ),
            self.change_students(
                "pondering", "hesitant", "pondering", look_at=recap_words
            ),
...
class EigenvalueEquationRearranging(Scene):
    def construct(self):
        ...
        scalar_prod_label.next_to(
            scalar_prod_brace, DOWN, SMALL_BUFF, aligned_edge=LEFT
        )
        ...
        self.play(
            ...
            VGroup(mat_prod_label, mat_prod_brace).animate.shift(
                lines[1].get_left() - lines[0].get_left()
            ),
        )
        self.wait()
        ...
        self.play(
            ...
            FadeOut(
                VGroup(mat_prod_label, mat_prod_brace, id_brace, id_label), 0.5 * DOWN
            ),
        )
        self.wait()
        self.play(
        ...
        self.play(Write(nz_label, run_time=2), GrowArrow(arrow))
        self.wait()
        self.play(
...
class TypicalComputation(Scene):
    def construct(self):
        ...
        rhs = VGroup(t0.copy(), t3.copy(), OldTex("-"), t1.copy(), t2.copy())
        rhs.arrange(RIGHT)
        ...
        self.play(
            TransformFromCopy(
                mat.get_brackets(), VGroup(*(det_expression[i] for i in [4, 13]))
            ),
            TransformFromCopy(
                mat.get_entries(), VGroup(*(det_expression[i] for i in [5, 8, 9, 10]))
            ),
            FadeTransform(
        ...
        self.play(FadeIn(rhs[2]), TransformFromCopy(VGroup(t1, t2), rhs[3:]))
        self.wait()
        ...
        for pair, bt in zip(it.product(*top_terms), bottom_terms):
            ...
            self.remove(rects)
        self.play(
            FadeIn(VGroup(rhs2[0], rhs2[8])), FadeTransform(rhs[2:].copy(), rhs2[9:])
        )
        self.wait()
...
class OutlineThreeFacts(Scene):
    def construct(self):
        ...
        new_tr_rhs = OldTex(
            "= {a + d \\over 2} = {\\lambda_1 + \\lambda_2 \\over 2}", **tex_kw
        )
        ...
        self.wait()
        self.play(TransformMatchingShapes(lambdas.copy(), det_rhs[6:]))
        self.wait()
        ...
        self.wait()
        self.play(randy.change("thinking", eq_m), Write(eq_m))
        self.play(
        ...
        tr_rect = SurroundingRectangle(VGroup(indices[0], tr, eq_m)).set_stroke(
            MEAN_COLOR
        )
        det_rect = SurroundingRectangle(VGroup(indices[1], det, eq_p)).set_stroke(
            PROD_COLOR
        )
        ...
        self.play(FadeIn(full_rect), randy.change("pondering", ORIGIN))
        for x in range(10):
        ...
        self.play(FadeOut(full_rect), FadeOut(randy), MoveToTarget(ex_group))

        self.play(
            FadeIn(form_lhs, 0.25 * UP), FadeOut(third_point_placeholder, 0.25 * UP)
        )
        self.play(
...
class ShowSquishingAndStretching(Scene):
    def construct(self):
        ...
        eigenvect_mobs[0].add_updater(
            lambda m: m.put_start_and_end_on(
                plane.c2p(0, 0), plane.c2p(*eigenvectors[0][:2])
            )
        )
        eigenvect_mobs[1].add_updater(
            lambda m: m.put_start_and_end_on(
                plane.c2p(0, 0), plane.c2p(*eigenvectors[1][:2])
            )
        )
        ...
        bases[0].add_updater(
            lambda m: m.put_start_and_end_on(plane.c2p(0, 0), plane.c2p(1, 0))
        )
        bases[1].add_updater(
            lambda m: m.put_start_and_end_on(plane.c2p(0, 0), plane.c2p(0, 1))
        )
        ...
        self.play(morpher.animate.apply_matrix(mat), run_time=4)
        self.wait()
        ...
        labels = VGroup(
            *(
                OldTex(
                    "\\text{Stretch by }", f"\\lambda_{i}", color=color, font_size=30
                )
...
class MeanProductExample(Scene):
    def construct(self):
        ...
        l1_arrow.add_updater(
            lambda m: m.set_points_by_ends(
                m_label.get_left() + SMALL_BUFF * LEFT,
                l1_label.get_right() + SMALL_BUFF * RIGHT,
            )
        )
        l2_arrow.add_updater(
            lambda m: m.set_points_by_ends(
                m_label.get_right() + SMALL_BUFF * RIGHT,
                l2_label.get_left() + SMALL_BUFF * LEFT,
            )
        )
        ...
        for t1, t2 in zip(texs[2:], texs[3:]):
            ...
            t2.shift(
                (t1.get_part_by_tex("=").get_x() - t2.get_part_by_tex("=").get_x())
                * RIGHT
            )
        ...
        self.play(
            TransformMatchingShapes(texs[0].copy(), texs[1], path_arc=45 * DEGREES)
        )
        self.wait(3)
        ...
        self.play(
            LaggedStart(
                FadeOut(minus_d, 0.25 * UP), FadeOut(plus_d, 0.25 * UP), lag_ratio=0.25
            ),
            LaggedStart(
                FadeIn(minus_3, 0.25 * UP), FadeIn(plus_3, 0.25 * UP), lag_ratio=0.25
            ),
        )
        self.wait()
        ...
        for tex in texs[:-2]:
            for fourty in tex.get_parts_by_tex("40"):
                fourties.add(fourty)
                ps.add(
                    OldTex("p").set_color(PROD_COLOR).move_to(fourty).shift(0.1 * DOWN)
                )
...
class AskIfThatsBetter(Scene):
    def construct(self):
        ...
        self.play(Blink(randy))
        self.wait()
        self.play(RemovePiCreatureBubble(randy, target_mode="thinking"))
        self.wait(2)
...
class OutstandingChannel(Scene):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                morty,
                OldTexText("Outstanding\\\\channel"),
                target_mode="hooray",
...
class JingleAnimation(Scene):
    def construct(self):
        ...
        self.play(
            TransformFromCopy(
                m, m2, path_arc=-120 * DEGREES, rate_func=smooth, run_time=0.6
            ),
        )
...
class Example1(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.teacher.change("raise_right_hand", mat), FadeIn(mat.get_brackets(), UP)
        )
        self.play(
        ...
        self.play(self.students[2].change("tease", bubble), Write(bubble))
        self.wait(3)
        ...
        # Show mean
        m_eq = OldTex(
            "m", "=", "2", tex_to_color_map={"m": MEAN_COLOR, "2": MEAN_COLOR}
        )
        ...
        diag_rects = VGroup(
            *(SurroundingRectangle(mat.get_entries()[i]) for i in [0, 3])
        )
        ...
        self.play(
            Write(m_eq[:2]), *(pi.animate.look_at(m_eq) for pi in self.pi_creatures)
        )
        self.play(
        ...
        prod_eq = OldTex(
            # "\\lambda_1 \\lambda_2 =",
            "{p} = {3} - {4} = -1",
            tex_to_color_map={"{3}": COL_COLORS[0], "{4}": COL_COLORS[0], **t2c},
        )
        ...
        self.play(
            ...
            VFadeInThenOut(
                VGroup(*(SurroundingRectangle(e) for e in mat.get_entries()[0::3]))
            ),
        ...
        self.play(
            ...
            VFadeInThenOut(
                VGroup(*(SurroundingRectangle(e) for e in mat.get_entries()[1:3]))
            ),
        )
        self.wait()
        self.play(
            Write(prod_eq[-2:]),
            self.change_students(
                "tease", "tease", "happy", look_at=prod_eq.get_right()
            ),
        )
        self.play(
            FadeOut(p_rect),
        ...
        self.wait()
        self.play(prod_eq.animate.scale(0.7).next_to(m_eq, UP).to_edge(LEFT))
        self.wait(2)
...
class GeneralExample(Scene):
    ...
    def construct(self):
        ...
        mean_eq = OldTex(
            f"m = {mean_str}", tex_to_color_map={"m": MEAN_COLOR, mean_str: MEAN_COLOR}
        )
        mean_eq.next_to(mat, UP, LARGE_BUFF)
        diag_rects = get_diag_rects(mat)

        self.play(FadeIn(mean_eq[:2], 0.25 * UP))
        ...
        self.play(
            FadeIn(prod_eq[3:5]),
            VFadeInThenOut(
                get_diag_rects(mat, color=YELLOW, off_diagonal=True), run_time=1.5
            ),
        )
        self.wait()
        ...
        # Simplify
        line2 = OldTex(
            mean_str, "\\pm", "\\sqrt{\\,", str(mean**2 - prod), "}", font_size=60
        )
        if mean == 0:
...
class PauliMatrices(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn, matrices, shift=0.25 * RIGHT, lag_ratio=0.3, run_time=2
            )
        )
        self.wait()
        ...
        # Axes
        axes = ThreeDAxes((-2, 2), (-2, 2), (-2, 2), axis_config={"include_tip": False})
        ...
        axes_copies = VGroup()
        for i, matrix, color, (v1, v2) in zip(
            it.count(), matrices, colors, [(UP, DOWN), (LEFT, RIGHT), (RIGHT, RIGHT)]
        ):
            ac = axes.deepcopy()
        ...
        self.play(
            LaggedStartMap(
                FadeOut, axes_copies, shift=0.25 * DOWN, run_time=1, lag_ratio=0.3
            )
        )

        # Compute means
        means_rects = VGroup(*(get_diag_rects(matrix) for matrix in matrices))
        mean_eqs = VGroup(
            *(
                OldTex("m = 0", tex_to_color_map={"m": MEAN_COLOR, "0": WHITE}).next_to(
        ...
        forms = VGroup(
            *(
                OldTex("0 \\pm \\sqrt{\\,0^2 - {p}}", **kw)
                .next_to(matrix, RIGHT)
                .to_edge(RIGHT)
                for matrix in matrices
            )
        )
        simple_forms = VGroup(
            *(OldTex("\\pm \\sqrt{-{p}}", **kw).move_to(form, LEFT) for form in forms)
        )
        ...
        for me, mr, form in zip(mean_eqs, means_rects, forms):
            ...
            self.play(TransformFromCopy(mr, me[2], run_time=0.7))
        self.wait()
        self.play(
            LaggedStart(
                *(FadeIn(VGroup(form[1], form[3], form[4])) for form in forms),
                lag_ratio=0.2,
            ),
            LaggedStart(
                *(
                    TransformFromCopy(
                        me.get_parts_by_tex("0"), form.get_parts_by_tex("0")
                    )
        ...
        prod_eqs = VGroup(
            *(
                OldTex("p = -1", tex_to_color_map={"p": PROD_COLOR, "-1": WHITE})
                .next_to(matrix, RIGHT, buff=MED_LARGE_BUFF)
                .shift(0.5 * DOWN)
                for matrix in matrices
            )
        )
        ...
        self.play(
            LaggedStart(*(det_path_anim(matrix) for matrix in matrices), lag_ratio=0.2)
        )
        ...
        final_forms = VGroup(
            *(OldTex("= \\pm 1").move_to(sf.get_center(), LEFT) for sf in simple_forms)
        )
...
class HeresTheThing(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.change_students(
                "maybe", "confused", "raise_right_hand", look_at=self.screen
            ),
            self.teacher.change("happy"),
        )
        self.wait(3)
...
class PauliMatricesWithCharacteristicPolynomial(PauliMatrices):
    def construct(self):
        ...
        self.play(
            VFadeIn(randy), randy.animate.set_opacity(1).change("hesitant", full_mat)
        )
        self.play(ShowCreation(diag_rects))
        self.play(Write(bubble), Write(m_eq), randy.change("pondering", bubble))
...
class MPIsSolvingCharPoly(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("happy", "thinking", "tease", look_at=formula)
        self.wait()
        self.play(
...
class QuadraticPolynomials(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(graph, run_time=3), FadeIn(graph_label, UP))
        self.play(
        ...
        self.play(Write(eq1), TransformMatchingShapes(root_labels.copy(), line3))
        self.wait()
        ...
        # Show final roots
        d_terms = VGroup(
            *(OldTex(u, "\\sqrt{25 - 9}", font_size=24) for u in ["+", "-"])
        )
        my = m_label.get_y()
        arrows = VGroup(
...
class SkipTheMiddleStep(Scene):
    def construct(self):
        ...
        char_poly = OldTex(
            "\\lambda^2 - 4\\lambda - 1", tex_to_color_map={"\\lambda": EIGEN_COLORS[0]}
        )
        mp_formula = OldTex(
            "{2} \\pm \\sqrt{\\,{2}^2 - (-1)}",
            tex_to_color_map={
        ...
        for label, mob, color in zip(
            labels, group, [COL_COLORS[0], EIGEN_COLORS[0], MEAN_COLOR]
        ):
            ...
            LaggedStart(
                *(
                    ShowCreationThenFadeOut(
                        SurroundingRectangle(sm, buff=0.1, color=WHITE), run_time=2
                    )
        ...
        self.wait()
        self.play(Write(words), ShowCreation(cross))
        self.wait()
...
class GeneralCharPoly(Scene):
    def construct(self):
        ...
        rhs2 = OldTex(
            "= \\lambda^2 - (a + d)\\lambda + (ad - bc)", tex_to_color_map=t2c
        )
        ...
        self.play(
            ShowCreation(SurroundingRectangle(rhs2[3:9], buff=0.05, color=MEAN_COLOR))
        )
        self.play(
            ShowCreation(SurroundingRectangle(rhs2[11:], buff=0.05, color=PROD_COLOR))
        )
        self.wait()
