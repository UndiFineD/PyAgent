    ...
    def construct(self):
        ...
        for pi, word, mode in zip(pis, words, modes):
            bubble = pi.get_bubble(
                word, bubble_type=SpeechBubble, height=3, width=3, direction=RIGHT
            )
...
class InterpolatingOrientations(ThreeDScene):
    ...
    def construct(self):
        ...
        # Tie logo to matrix
        logo = always_redraw(
            lambda: logo_template.copy()
            .apply_matrix(get_matrix())
            .sort(lambda p: -p[1])
        )
        ...
        if self.matrix:
            ...
            def update_quat_label(quat_label):
                ...
                for mob, value, color in zip(
                    quat_label, quat_tracker.get_value(), colors
                ):
        ...
        def move_to_quaternion(quaternion, run_time=5):
            self.play(
                ...
                UpdateFromFunc(
                    quat_tracker, lambda m: m.set_value(normalize(m.get_value()))
                ),
                run_time=run_time,
            )
            self.wait()
...
class NeverNeeded(Scene):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                morty, OldTexText("Who actually\\\\uses it!"), target_mode="angry"
            )
        )
