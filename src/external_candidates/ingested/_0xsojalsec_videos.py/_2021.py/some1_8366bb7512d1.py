# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2021\some1.py
...
class PartialFraction(Scene):
    def construct(self):
        frac = OldTex(r"\frac{x+9}{(x-3)(x+5)} = \frac{?}{x - 3} + \frac{?}{x + 5}")
        frac.set_width(10)
        self.add(frac)
...
class TrigIdentity(Scene):
    def construct(self):
        ...
        tan_line = always_redraw(
            lambda: Line(
                get_point(), plane.c2p(1 / math.cos(get_theta()), 0), stroke_color=RED
            )
        )
...
class ContentAdvice(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn, VGroup(*(point[:2] for point in points)), shift=0.25 * RIGHT
            )
        )
        self.wait()
...
class FractionsExample(Scene):
    def construct(self):
        ...
        self.play(
            *(FadeTransform(expr[i].copy(), pies[i]) for i in range(len(pies))),
...
class ConcreteToAbstract(Scene):
    def construct(self):
        ...
        # Algebra
        expr = OldTex(
            "x^2 - y^2 = (x + y)(x - y)", tex_to_color_map={"x": BLUE_D, "y": BLUE_B}
        )
        expr.scale(1.5)
...
class DifferenceOfSquares(Scene):
    def construct(self):
        ...
        squares = VGroup(
            *[
                VGroup(*[Square() for x in range(x)]).arrange(RIGHT, buff=0)
                for y in range(x)
            ]
        ).arrange(DOWN, buff=0)
        ...
        self.play(
            ReplacementTransform(squares[:-y].copy().set_fill(opacity=0), new_squares),
            ShowCreation(arrow),
            lag_ratio=0,
        )
...
class AbstractVectorSpace(Scene):
    def construct(self):
        ...
        funcs = OldTex(
            "(f + g)(x) = f(x) + g(x)", tex_to_color_map={"f": BLUE_B, "g": BLUE_D}
        )
...
class TransitionToProductionQuality(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says(
            OldTexText("What parts of\\\\production quality matter?"), index=1
        )
        self.play(
