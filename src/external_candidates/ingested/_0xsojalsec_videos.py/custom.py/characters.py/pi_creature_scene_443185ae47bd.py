# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\custom\characters\pi_creature_scene.py
...
from custom.characters.pi_creature import Mortimer, PiCreature, Randolph
...
from manimlib.animation.fading import FadeIn, FadeTransform
from manimlib.animation.transform import ApplyMethod, ReplacementTransform, Transform
from manimlib.constants import *
from manimlib.mobject.frame import FullScreenFadeRectangle, ScreenRectangle
from manimlib.mobject.mobject import Group
from manimlib.mobject.svg.drawings import SpeechBubble, ThoughtBubble
from manimlib.mobject.types.vectorized_mobject import VGroup
from manimlib.scene.interactive_scene import InteractiveScene
from manimlib.scene.scene import Scene
from manimlib.utils.rate_functions import squish_rate_func, there_and_back
...
class PiCreatureScene(InteractiveScene):
    ...
    def introduce_bubble(
        ...
        **kwargs,
        ...
        pi_creatures_with_bubbles = [
            pi for pi in self.get_pi_creatures() if pi.bubble in on_screen_mobjects
        ]
        if pi_creature in pi_creatures_with_bubbles:
            ...
            bubble = pi_creature.get_bubble(
                content, bubble_type=bubble_type, **bubble_config
            )
            anims += [
    ...
    def anims_from_play_args(self, *args, **kwargs):
        ...
        if (
            hasattr(first_anim, "target_mobject")
            and first_anim.target_mobject is not None
        ):
...
class TeacherStudentsScene(PiCreatureScene):
    ...
    def student_thinks(self, content, target_mode=None, index=2, **kwargs):
        return self.pi_creature_thinks(
            self.get_students()[index], content, target_mode=target_mode, **kwargs
        )
    ...
    def teacher_holds_up(
        self, mobject, target_mode="raise_right_hand", added_anims=None, **kwargs
    ):
