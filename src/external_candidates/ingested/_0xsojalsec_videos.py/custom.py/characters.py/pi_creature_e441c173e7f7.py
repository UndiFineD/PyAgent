# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\custom\characters\pi_creature.py
...
class PiCreature(SVGMobject):
    ...
    def replace_bubble(
        self, content, mode="pondering", look_at=None, **kwargs
    ) -> Animation | _AnimationBuilder:
        if self.bubble is None:
            return self.change(mode, look_at)
        old_bubble = self.bubble
        new_bubble = self.get_bubble(
            content, bubble_type=old_bubble.__class__, **kwargs
        )
        self.bubble = new_bubble
        return AnimationGroup(
            ReplacementTransform(old_bubble, new_bubble), self.change(mode, look_at)
        )
...
class Eyes(VGroup):
    def __init__(
        self, body: VMobject, height: float = 0.3, mode: str = "plain", **kwargs
    ):
