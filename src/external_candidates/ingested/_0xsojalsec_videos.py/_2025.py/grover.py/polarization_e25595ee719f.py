# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\grover\polarization.py
...
class BeamSplitter(InteractiveScene):
    def construct(self):
        ...
        self.play(
            frame.animate.reorient(-57, 74, 0, (-3.29, -0.18, -0.18), 4.80), run_time=8
        )
        ...
        corner_plane, h_plane, v_plane, plane_in_situ = planes = VGroup(
            NumberPlane((-1, 1), (-1, 1)) for _ in range(4)
        )
        for plane in planes:
        ...
        self.play(
            frame.animate.reorient(-80, 79, 0, (-3.36, 0.1, -0.46), 4.80), run_time=12
        )
        ...
        short_wave = self.get_wave(
            theta_tracker,
            pointer.get_right(),
            max_x=split_point_dist,
            stroke_opacity=0.5,
        )
        short_wave.time = wave.time
        corner_vector.wave = short_wave
        top_wave = self.get_wave(
            theta_tracker,
            split_point,
            refraction_angle=45 * DEG,
            project_horizontal=True,
            stroke_opacity=0.25,
        )
        low_wave = self.get_wave(
            theta_tracker,
            split_point,
            refraction_angle=-45 * DEG,
            project_vertical=True,
            stroke_opacity=0.25,
        )
        ...
        top_beam.f_always.set_stroke(
            opacity=lambda: math.cos(theta_tracker.get_value())
        )
        low_beam.f_always.set_stroke(
            opacity=lambda: math.sin(theta_tracker.get_value())
        )
        ...
        self.play(
            ...
            frame.animate.reorient(-76, 62, 0, (-2.7, 0.08, -0.8), 6.22).set_anim_args(
                run_time=4
            ),
        )
        self.wait(3)
        ...
        self.play(
            frame.animate.reorient(-90, 82, 0, (-1.73, 0.07, 1.0), 8.00), run_time=6
        )
        ...
        coords = VGroup(
            DecimalNumber(0, unit=R"\times"), DecimalNumber(0, unit=R"\times")
        )  # Stopped using these, maybe later?
        coords.fix_in_frame()
        coords.scale(0.75)
        for coord, plane in zip(coords, fixed_planes[1:]):
            coord.next_to(plane, LEFT, SMALL_BUFF)
        coords[0].add_updater(
            lambda m: m.set_value(math.sin(theta_tracker.get_value()))
        )
        coords[1].add_updater(
            lambda m: m.set_value(math.cos(theta_tracker.get_value()))
        )

        h_vect, soft_h_vect, v_vect, soft_v_vect = corner_vector.replicate(
            4
        ).clear_updaters()
        ...
        arc = always_redraw(
            lambda: Arc(
                ...
                radius=0.5,
                arc_center=corner_plane.get_center(),
            ).fix_in_frame()
        )
        ...
        self.play(
            ...
            FadeTransform(
                theta_label.copy().clear_updaters(),
                cos_term[R"\theta"][0],
                time_span=(0.25, 1.25),
            ),
            FadeTransform(
        ...
        amp_brace = LineBrace(
            polarization_line.copy().scale(0.5, about_edge=UR), buff=SMALL_BUFF
        )
    ...
    def get_wave(
        ...
        def field_func(points, time):
            theta = theta_tracker.get_value()
            magnitudes = amplitude * np.cos(
                wave_number * points[:, 0] - TAU * freq * time
            )
