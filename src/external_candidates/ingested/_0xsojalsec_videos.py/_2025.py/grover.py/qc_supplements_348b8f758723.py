# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\grover\qc_supplements.py
from _2025.blocks_and_grover.state_vectors import *
from manim_imports_ext import *


def get_quantum_computer_symbol(
    height=2, color=GREY_B, symbol_tex=R"|Q\rangle", symbol_color=TEAL
):
...
def get_classical_computer_symbol(
    height=2, color=GREY_B, symbol_tex=R"\mathcal{C}", symbol_color=YELLOW
):
    return get_quantum_computer_symbol(height, color, symbol_tex, symbol_color)


def get_blackbox_machine(
    height=2, color=GREY_D, label_tex="f(n)", label_height_ratio=0.25
):
...
class Superposition(Group):
    def __init__(
        ...
        self.pieces = pieces
        self.center_points = Group(Point(piece.get_center()) for piece in pieces)
    ...
    def update_piece_positions(self, dt):
        ...
        for piece in self.pieces:
            piece.offset_vect = rotate_vector(
                piece.offset_vect, dt * piece.offset_vect_rot_vel
            )
...
class WriteQuantumComputingTitle(InteractiveScene):
    def construct(self):
        ...
        title.move_to(UP)
        subtitle = Tex(
            R"\frac{1}{\sqrt{2}}\big(|0\rangle + |1\rangle \big)", font_size=90
        )
...
class StudentsCommentOnQuiz(TeacherStudentsScene):
    def construct(self):
        ...
        q_marks = VGroup(
            Text("?!?", font_size=72).next_to(std, UP) for std in self.students
        )
        ...
        self.play(
            self.change_students(
                "pondering", "pondering", "thinking", look_at=self.screen, lag_ratio=0.1
            ),
            FadeIn(bubble, lag_ratio=0.2),
            LaggedStartMap(
                FadeOut,
                VGroup(machine, arrow, q_marks[2], qc),
                shift=LEFT,
                lag_ratio=0.1,
                run_time=1,
            ),
...
class DotsAndArrow(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(FadeIn(dots, lag_ratio=0.5), GrowArrow(arrow), lag_ratio=0.7)
        )
        self.wait()
...
class QuestionsOnTheStateVector(TeacherStudentsScene):
    def construct(self):
        ...
        # Questions
        self.play(stds[0].says("But...\nwhat is it?", mode="maybe", look_at=morty.eyes))
        self.wait()
        self.add(Point())
        self.play(
            stds[1].says(
                "Why square things?", mode="raise_right_hand", look_at=morty.eyes
            )
        )
        self.wait(3)
...
class MagnifyingGlassOverComputer(InteractiveScene):
    def construct(self):
        ...
        self.add(comp, glass)
        self.play(
            glass.animate.shift(-glass[0].get_center()).set_anim_args(
                path_arc=-60 * DEG
            ),
...
class ClassicalGates(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn, inputs, shift=0.5 * RIGHT, lag_ratio=0.2, run_time=1.0
            )
        ),
        self.play(
            LaggedStart(
        ...
        self.play(
            gate_groups.animate.scale(0.35)
            .space_out_submobjects(0.9)
            .next_to(title, DOWN, MED_LARGE_BUFF),
...
class AskAreYouSure(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            FadeOut(answer[0]),
            number.animate.scale(1.5)
            .next_to(machine, LEFT)
        ...
        self.play(
            FadeOutToPoint(
                number, machine.get_center(), path_arc=-45 * DEG, lag_ratio=0.01
            )
        )
        self.play(
...
class WrapUpList(InteractiveScene):
    def construct(self):
        ...
        items = BulletedList(
            "The Lie", R"Why $\sqrt{\quad}$", "A suprising analogy", buff=0.75
        )
...
class PythagoreanIntuition(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(MoveAlongPath(dot, square, rate_func=lambda t: 1 - 0.5 * smooth(t)))
        ...
        self.play(
            ...
            *(Write(sqrts[0][tex], time_span=(1, 2)) for tex in [R"\sqrt", "+", "2"]),
            run_time=2,
        )
        self.wait()
        self.play(TransformMatchingTex(*sqrts, key_map={"1^2 + 1^2": "2"}, run_time=1))
        self.wait()
...
class GrainOfSalt(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "pondering", "pondering", "pondering", look_at=morty.eyes
            ),
        )
        self.wait(3)
        self.play(
...
class ConstructQRCode2(InteractiveScene):
    def construct(self):
        ...
        union_pieces = VGroup(
            VMobject().set_points(path) for path in union.get_subpaths()
        )
        ...
        self.play(
            LaggedStart(
                *(Rotate(square, 90 * DEG) for square in squares),
                lag_ratio=0.02,
                run_time=10,
            )
        )
