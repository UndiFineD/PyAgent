# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\grover\state_vectors.py
...
class ContrstClassicalAndQuantum(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(bit, num_mob) for bit in bits),
                lag_ratio=0.02,
            ),
            FadeIn(layer_names[2], UP),
        )
        self.wait()
        self.play(num_mob.animate.shift(0.75 * LEFT), FadeIn(character, 0.5 * RIGHT))
        self.wait()
        self.play(Write(circuitry), FadeIn(layer_names[0], DOWN))
        self.wait()
        ...
        self.wait()
        self.play(TransformFromCopy(qubits, qunit))
        self.wait()
        ...
        for value in [ord("C"), ord("Q")]:
            self.play(
                ...
                UpdateFromFunc(
                    qunit_num, lambda m: m.set_value(int(value_tracker.get_value()))
                ),
                UpdateFromFunc(
                    qubit_string, lambda m: m.set_value(int(value_tracker.get_value()))
                ),
                rate_func=linear,
                run_time=1.0,
            )
        ...
        for value in [*np.random.randint(0, 2**8, 4), ord("Q")]:
            ...
            self.play(
                LaggedStartMap(
                    VShowPassingFlash, lasers, lag_ratio=0.1, time_width=2.0, run_time=2
                ),
        ...
        morty.move_to(np.array([13.0, -6.0, 0.0]))
        ...
        self.play(
            ...
            VFadeIn(morty),
            *(TransformFromCopy(src, big_ket) for src in [qubit_ket, qunit_ket]),
        )
        self.play(
        ...
        self.play(FadeOut(layers[1]), FadeOut(layer_names[1]), run_time=3)
        ...
        self.play(
            *(
                LaggedStart(
                    (
                        bit.animate.set_stroke(YELLOW, 3).set_anim_args(
        ...
        self.play(
            value_tracker.animate.set_value(0).set_anim_args(
...
class AmbientStateVector(InteractiveScene):
    ...
    def get_plane_and_axes(self, scale_factor=2.0):
        ...
        plane = NumberPlane((-1, 1 - 1e-5), (-1, 1 - 1e-5), faded_line_ratio=5)
...
class DisectAQuantumComputer(InteractiveScene):
    def construct(self):
        ...
        wire_glows = Group(
            GlowDot(wire.get_start(), color=TEAL, radius=0.25) for wire in wires
        )
        ...
        for glow in gate_glows:
            glow.add_updater(
                lambda m: m.set_stroke(
                    width=4 * np.exp(-((0.5 * (m.get_x() - wire_glows.get_x())) ** 2))
                )
            )
        ...
        for n in range(n_repetitions):
            ...
            self.play(
                wire_glows.animate.match_x(mes_gates), rate_func=linear, run_time=1
            )
        ...
        all_dist_rects = VGroup(
            self.get_dist_bars(dist, all_qubits, width_ratio=6) for dist in dists
        )
        dist_rects = all_dist_rects[0]

        dist_rect_labels = VGroup(
            Integer(100 * prob, unit=R"\%", font_size=24, num_decimal_places=1).next_to(
                rect, RIGHT, SMALL_BUFF
            )
            for prob, rect in zip(dists[0], dist_rects)
        )

        self.play(
            FadeOut(machine_rect, DOWN), FadeOut(circuit, DOWN), MoveToTarget(qc_label)
        )
        self.play(
        ...
        self.play(
            LaggedStartMap(
                FadeIn, dist_rect_labels, shift=0.25 * RIGHT, lag_ratio=0.1, run_time=2
            )
        )
        self.wait(2)
        ...
        if False:
            ...
            wigglers = Superposition(
                VGroup(
                    VGroup(qb, bar) for qb, bar in zip(all_qubits, dist_rects)
                ).copy(),
                max_rot_vel=8,
            ...
            for _ in range(2):
                self.play(
                    glass.animate.shift(
                        qc_label.get_center() - glass[0].get_center()
                    ).set_anim_args(path_arc=-45 * DEG),
        ...
        for k in [5, 6, "k"]:
            ...
            multi_bitstrings = VGroup(
                BitString(n, length=n_bits) for n in range(2**n_bits)
            )
            new_qubits = VGroup(VGroup(bits, Ket(bits)) for bits in multi_bitstrings)
            new_qubits.arrange(DOWN)
            new_qubits.set_height(FRAME_HEIGHT - 1)
            new_qubits.move_to(all_qubits)
            big_dist_rects = self.get_dist_bars(
                np.random.random(2**n_bits) ** 2, new_qubits
            )
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOut,
                VGroup(
                    brace, brace_label, longer_example, down_brace, down_brace_label
                ),
            ),
            FadeOut(last_qubits),
            FadeOut(last_dist_rects),
        ...
        dist_rect = SurroundingRectangle(
            VGroup(comp_to_dist_arrow, all_qubits, dist_rects), buff=0.25
        )
        ...
        self.play(FadeIn(superposition), FadeOut(output), ShowCreation(big_cross))
        self.wait(6)
        self.play(
        ...
        for _ in range(n_samples):
            self.play(
                RandomSampling(selection, list(output_choices), weights=dists[-1])
            )
            self.wait()
        ...
        state_vector = DecimalMatrix(
            np.zeros((16, 1)), decimal_config=dict(include_sign=True, edge_to_fix=LEFT)
        )
        ...
        vect_lines = self.get_connecting_lines(
            qc_label, state_vector, from_buff=-0.1, to_buff=0.1
        )
        ...
        self.wait()
        self.play(
            dist_rect.animate.stretch(1.5, 0, about_edge=LEFT).set_anim_args(
                time_span=(1, 2)
            ),
            ...
            LaggedStart(
                (FadeIn(value[0]) for value in bar_values),
                lag_ratio=0.5,
                time_span=(2, 6),
            ),
            LaggedStart(
                (FadeIn(value[2:]) for value in bar_values),
                lag_ratio=0.5,
                time_span=(2, 6),
            ),
        ...
        label_line = (
            Line(UP, DOWN)
            .set_height(1.0)
            .next_to(qc_label.get_right(), LEFT, SMALL_BUFF)
        )
        ...
        for new_state in [*alt_states, state]:
            ...
            self.play(
                LaggedStartMap(
                    VShowPassingFlash, comp_lines, time_width=2.0, lag_ratio=0.02
                ),
                Transform(
                    dist_rects,
                    self.get_dist_bars(new_state**2, all_qubits),
                    time_span=(0.75, 1.75),
                ),
                *(
        ...
        for index in [point5_index, point1_index]:
            self.play(
                ...
                *(group[index].animate.set_opacity(1) for group in groups),
                run_time=2,
            )
            self.play(
            ...
            self.play(
                FlashAround(
                    bar_values[index][-1], buff=SMALL_BUFF, color=RED, time_width=1.5
                )
            )
            self.wait()
        ...
        self.play(
            ...
            FadeIn(vector_title),
            *(group.animate.set_opacity(1) for group in groups),
        ...
        if False:  # Only used for an insertion
            ...
            for alpha in [0.1, 0.1, 0.2, 0.25, 0.25, 0.25, 0.25, 1.0]:
                ...
                self.play(
                    Transform(
                        dist_rects,
                        self.get_dist_bars(new_state**2, all_qubits, width_ratio=4),
                        time_span=(0.5, 1.5),
                    ),
                    LaggedStartMap(
                        VShowPassingFlash,
        ...
        self.play(
            LaggedStart(
                (
                    FadeTransform(vector_entries[n].copy(), sum_expr[Rf"x_{{{n}}}^2"])
        ...
        self.wait()
        self.play(Write(top_eq[0]), Write(top_eq[2:]), FadeOut(value_rect))
        self.wait()
        self.play(
            entry.animate.set_value(0.5),
            top_value.animate.set_value(0.5),
        ...
        sign_choices = VGroup(sign_choice.copy().move_to(sign, RIGHT) for sign in signs)
        ...
        qubits = VGroup(VGroup(bit, Ket(bit)) for bit in bits)
        qubits.scale(1.5)
        ...
        small_vect_lines = self.get_connecting_lines(
            qc_label, small_state_vector, from_buff=-0.1, to_buff=0.1
        )
        ...
        self.play(
            LaggedStart(
                ...
                TransformFromCopy(
                    state_vector.get_brackets(), small_state_vector.get_brackets()
                ),
                TransformFromCopy(
                    state_vector.get_entries(), small_state_vector.get_entries()
                ),
        ...
        self.play(
            FadeOut(
                VGroup(small_vect_lines, small_state_vector, qubits, small_dist_bars)
            ),
        ...
        for n, state in enumerate(new_states):
            anims = [
                Transform(
                    dist_rects,
                    self.get_dist_bars(state**2, all_qubits, width_ratio=4.0),
                    time_span=(1, 2),
                )
            ]
            if n == 1:
                ...
                entry_rects = VGroup(
                    SurroundingRectangle(e, buff=0.05) for e in vector_entries
                )
                qubit_rects = VGroup(
                ...
                self.play(
                    Restore(bits),
                    key_icon.animate.scale(0.5).next_to(
                        bits.saved_state, LEFT, buff=SMALL_BUFF
                    ),
                )
            ...
            if n % 2 == 1:
                ...
                anims.append(
                    ChangeDecimalToValue(
                        vector_entries[key], state[key], time_span=(0.5, 1.5)
                    )
                )
            else:
                anims.append(
...
class Qubit(DisectAQuantumComputer):
    def construct(self):
        ...
        vector_label = DecimalMatrix(
            [[1.0], [0.0]], bracket_h_buff=0.1, decimal_config=dict(include_sign=True)
        )
        ...
        def position_label(vector_label):
            x, y = get_state()
            buff = SMALL_BUFF + 0.5 * interpolate(
                vector_label.get_width(), vector_label.get_height(), x**2
            )
        ...
        dist_bars = always_redraw(
            lambda: self.get_dist_bars(get_state() ** 2, qubits, width_ratio=1.5)
        )
        ...
        dist_group = VGroup(
            coord_vect, dist_arrow, dist_arrow_label, qubits, dist_bars, bar_labels
        )
        ...
        self.play(
            LaggedStart(
                Transform(
                    var_vect.copy(), coord_vect.copy().clear_updaters(), remover=True
                ),
                ...
                FadeTransformPieces(
                    VGroup(pr[5:] for pr in prob_rule).copy(), dist_bars
                ),
        ...
        self.play(one_label.animate.scale(2).next_to(one_vect, LEFT, buff=0))
        self.play(
        ...
        self.play(
            var_vect_copy.animate.scale(0.75).next_to(vector.get_end(), UR, buff=0)
        )
        ...
        faders = VGroup(pythag, var_group, dist_group)
        ...
        matrix_tex = Tex(
            R"\frac{1}{\sqrt{2}} \left[\begin{array}{cc} 1 & 1 \\ 1 & -1 \end{array}\right]",
            font_size=24,
        )
        ...
        movers_image.flip(
            axis=rotate_vector(RIGHT, PI / 8), about_point=plane.get_center()
        )
        ...
        labels_image = VGroup(
            Tex(R"\text{H}|0\rangle")
            .scale(0.75)
            .rotate(45 * DEG)
            .next_to(movers_image[1].get_center(), UL, buff=-0.05),
            Tex(R"\text{H}|1\rangle")
            .scale(0.75)
            .rotate(-45 * DEG)
            .next_to(movers_image[2].get_center(), DL, buff=-0.05),
            Tex(R"\text{H}\big(x|0\rangle + y|1\rangle\big)")
            .scale(0.75)
            .next_to(movers_image[3].get_end(), RIGHT, SMALL_BUFF),
        )
        ...
        for index in range(2):
            ...
            self.play(
                TransformFromCopy(
                    movers[index + 1], movers_image[index + 1], path_arc=-30 * DEG
                ),
                TransformFromCopy(
                    labels[index], labels_image[index], path_arc=-30 * DEG
                ),
                rect.animate.surround(labels_image[index]).set_anim_args(
                    path_arc=-30 * DEG
                ),
                run_time=2,
            )
    ...
    def get_qubit_labels(self, plane):
        ...
        zero_label, one_label = qubit_labels = VGroup(
            VGroup(Ket(bit), bit) for bit in bits
        )
...
class ShowAFewFlips(Qubit):
    def construct(self):
        ...
        for i in [1, 0, 2, 1, 2, 1, 0, 1]:
            self.play(
                LaggedStart(
                    AnimationGroup(FadeIn(lines[i]), FadeIn(gate_labels[i])),
...
class ExponentiallyGrowingState(InteractiveScene):
    ...
    def get_state_vector(self, n):
        ...
        array = DecimalMatrix(
            values.reshape((2**n, 1)), decimal_config=dict(include_sign=True)
        )
        array.set_max_height(7)

        bit_strings = VGroup(BitString(k, length=n) for k in range(2**n))
...
class InvisibleStateValues(InteractiveScene):
    def construct(self):
        ...
        vect = DecimalMatrix(
            values.reshape(-1, 1), decimal_config=dict(include_sign=True)
        )
...
class ThreeDSample(InteractiveScene):
    def construct(self):
        ...
        frame.add_updater(
            lambda m, dt: m.increment_theta(dt * rot_vel_tracker.get_value())
        )
        ...
        coord_array = DecimalMatrix(
            np.zeros((3, 1)), decimal_config=dict(include_sign=True)
        )
        ...
        for symbol, direction, trg_coords in zip(symbols, directions, np.identity(3)):
            ...
            self.play(
                self.set_vect_anim(vector, trg_coords), FadeIn(symbol, 0.25 * OUT)
            )
            self.play(
                symbol.animate.scale(0.5).next_to(trg_coords, direction, SMALL_BUFF)
            )
            self.wait(0.5)
        self.wait(2)
        ...
        coord_rects = VGroup(
            SurroundingRectangle(elem) for elem in coord_array.elements
        )
        ...
        self.wait()
        self.play(
            FadeTransformPieces(balance_name, balance_ket[1]), Write(balance_ket[0])
        )
        ...
        self.play(self.set_vect_anim(z_vect, OUT, run_time=2))
        self.wait(3)
        self.remove(tail)
        ...
        self.play(
            frame.animate.reorient(67, 77, 0, (-0.57, 0.12, 0.86), 6.62), run_time=2
        )
        self.wait(4)
        self.play(
    ...
    def flip_along_key_axis(self):
        ...
        self.add(key_vect)
        self.play(self.set_vect_anim(vector, normalize([1, 1, 1])), FadeIn(mesh))
        vector.clear_updaters()
        for _ in range(4):
            self.play(
                Group(mesh, vector, key_vect).animate.stretch(
                    -1, 2, about_point=ORIGIN
                ),
                run_time=2,
            )
            self.wait()
...
class GroversAlgorithm(InteractiveScene):
    def construct(self):
        ...
        b_vect = key_vect.copy().rotate(
            -np.arccos(1 / math.sqrt(3)), about_point=ORIGIN
        )
        b_vect.set_fill(TEAL)

        key_label, b_label = labels = VGroup(KetGroup(Tex(char)) for char in "kb")
        ...
        rhs = Tex(
            R"= \frac{1}{\sqrt{3}}\big(|0\rangle + |1\rangle + |2\rangle \big)",
            font_size=36,
        )
        ...
        self.play(
            ...
            UpdateFromFunc(
                dashed_line,
                lambda m: m.become(DashedLine(b_vect.get_end(), b_vect_proj.get_end())),
            ),
            UpdateFromFunc(
                b_label, lambda m: m.next_to(b_vect.get_end(), UR, SMALL_BUFF)
            ),
            run_time=12,
        )
        self.wait()
        ...
        self.play(
            FadeIn(
                VGroup(b_vect_proj, dashed_line),
                run_time=3,
                rate_func=there_and_back_with_pause,
                remover=True,
            )
        ...
        for vect in [b_vect, key_vect]:
            vect.target = Arrow(
                ORIGIN, vect.get_end(), thickness=target_thickness, buff=0
            )
        ...
        self.play(
            LaggedStartMap(
                FadeOut,
                VGroup(array_labels, arrays, dot, equals, rhs, cos_group, sin_expr),
                lag_ratio=0.25,
            ),
        ...
        vect_coords = DecimalMatrix(
            initial_coords.reshape(-1, 1),
        ...
        arc_labels = VGroup(
            Tex(R"2\theta", font_size=24).next_to(
                arc.pfp(0.5), normalize(arc.pfp(0.5)), SMALL_BUFF
            )
            for arc in arcs
        )
        ...
        rhs_terms.shift(
            lhs.get_right() + 0.2 * RIGHT + 0.05 * UP - rhs_terms[0].get_left()
        )
        ...
        self.wait()
        self.play(
            LaggedStartMap(FadeIn, rhs_terms[1:], shift=0.5 * DOWN, lag_ratio=0.5)
        )
        self.wait()
    ...
    def key_flip_insertion(self):
        ...
        self.add(key_ghost)
        self.play(key_vect.animate.flip(axis=RIGHT, about_edge=DOWN), run_time=2)
        self.wait()
...
class TwoFlipsEqualsRotation(InteractiveScene):
    def construct(self):
        ...
        for line in flip_lines:
            ...
            self.play(
                randy.animate.flip(
                    axis=line.get_vector(), about_point=plane.get_origin()
                ),
                plane.animate.flip(
                    axis=line.get_vector(), about_point=plane.get_origin()
                ),
        ...
        self.play(
            self.frame.animate.move_to(
                midpoint(plane.get_center(), plane2.get_center())
            ),
            FadeIn(ghost_plane2),
            FadeIn(plane2),
            FadeIn(randy2),
        ...
        self.play(
            Rotate(
                Group(plane2, Point(), randy2),
                2 * theta,
                about_point=plane2.get_center(),
            ),
            *map(FadeIn, arcs),
        )
        self.wait()
...
class ComplexComponents(InteractiveScene):
    def construct(self):
        ...
        dec_vect = DecimalMatrix(
            state.reshape(-1, 1), decimal_config=dict(include_sign=True)
        )
        ...
        def update_dots(dots):
            for dot, value, plane in zip(
                dots, state_tracker.get_value(), complex_planes
            ):
        ...
        def update_dot_lines(lines):
            for line, x, dot in zip(lines, x_vect.elements, dots):
                line.put_start_and_end_on(
                    x.get_right() + 0.05 * RIGHT, dot.get_center()
                )
        ...
        self.play(
            ...
            ReplacementTransform(
                dec_vect.get_brackets(), x_vect.get_brackets(), time_span=(1, 2)
            ),
            run_time=2,
        )
        ...
        arc = always_redraw(
            lambda: Arc(
                0,
                c_line.get_angle() % TAU,
                radius=0.1,
                arc_center=plane.c2p(0),
        ...
        # Zoom back out
        fader = Group(
            prob_eq, mag_label, brace, c_line, c_dot, arc, phi_label, ghost_line
        )
