# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\grover\runtime.py
...
class Quiz(InteractiveScene):
    def construct(self):
        ...
        covers = VGroup(
            SurroundingRectangle(choice[2:], buff=SMALL_BUFF) for choice in choices
        )
        ...
        for bar in prob_bars:
            ...
            label.bar = bar
            label.add_updater(
                lambda m: m.set_value(np.round(100 * m.bar.get_width() / max_bar_width))
            )
        ...
        self.play(
            LaggedStart(
                (GrowFromPoint(bar, bar.get_right()) for bar in prob_bars),
                lag_ratio=0.2,
            ),
            VFadeIn(prob_labels),
        )
        ...
        self.wait()
        self.play(FadeOut(covers[1]), FadeIn(covers[3]))
        self.wait()
        self.play(FadeOut(covers[0]), FadeIn(covers[1]))
        self.wait()
...
class ShowOptionGraphs(InteractiveScene):
    def construct(self):
        ...
        labels = VGroup(
            Tex(sym, font_size=30)
            .match_color(graph)
            .next_to(graph.get_end(), RIGHT, SMALL_BUFF)
            for graph, sym in zip(
                graphs,
                [
...
class NeedleInAHaystackProblem(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeIn(machine, scale=2), Write(q_marks))
        ...
        self.play(
            LaggedStart(
                (self.evaluation_animation(mob, machine) for mob in other_inputs),
                lag_ratio=0.2,
            )
        )
        self.wait()
        ...
        self.play(machine.animate.set_fill(opacity=0), FadeIn(innards))
        self.wait()
        self.play(
            FadeOut(innards),
        ...
        superposition.target[2].arrange(DOWN, buff=0.25).next_to(
            machine, LEFT, buff=1.5
        )
        ...
        answers = VGroup(
            (
                Text("True").set_color(GREEN)
                if n == key_number
                else Text("False").set_color(RED)
            )
        ...
        self.play(
            answer_superposition.animate.set_offset_multiple(0.025).set_glow_opacity(
                1e-2
            )
        )
        self.wait(10)
    ...
    def evaluation_animation(self, input_mob, machine, output=False, run_time=1.0):
        ...
        return AnimationGroup(
            ...
            FadeInFromPoint(
                out_mob, machine.get_left(), time_span=(0.25 * run_time, run_time)
            ),
        )
...
class LargeGuessAndCheck(InteractiveScene):
    ...
    def construct(self):
        ...
        for n, mob in enumerate(grid):
            ...
            if n != self.key_value:
                self.play(
                    grid[n].animate.set_opacity(0.5), run_time=self.wait_time_per_mob
                )
                continue
...
class SuperpositionAsParallelization(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(boxes), Write(bit_mobs))
        value_tracker = ValueTracker(0)
        self.play(
            value_tracker.animate.set_value(12),
            UpdateFromFunc(
                bit_mobs, lambda m: match_value(m, value_tracker.get_value())
            ),
            run_time=3,
            rate_func=linear,
        )
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(bit_mobs, bit_string) for bit_string in bit_strings),
                lag_ratio=0.05,
            )
        )
        self.play(
            superposition.animate.set_offset_multiple(0.1)
            .set_glow_opacity(0.05)
            .shift(1.5 * LEFT),
        ...
        self.play(
            ...
            LaggedStart(
                (TransformFromCopy(classical, mc) for mc in mini_classical),
                lag_ratio=0.05,
            ),
...
class SecondMisconception(InteractiveScene):
    def construct(self):
        ...
        lines = VGroup(
            Line(product.get_bottom(), factor.get_top(), buff=0.2) for factor in factors
        )
        ...
        self.play(
            ...
            LaggedStart(
                (TransformFromCopy(product, factor) for factor in factors),
                lag_ratio=0.25,
            ),
...
class GroverTimeline(InteractiveScene):
    def construct(self):
        # Test
        timeline = NumberLine(
            (1990, 2025, 1), big_tick_spacing=5, width=FRAME_WIDTH - 1
        )
        timeline.set_y(-2)
        ...
        self.play(FadeIn(bbbv_statement, 0.5 * DOWN))
        self.wait()
        ...
        self.wait()
        self.play(VGroup(rect, big_O).animate.shift(DOWN), FadeIn(runtime, scale=2))
        self.wait()
...
class ShowSha256(InteractiveScene):
    def construct(self):
        ...
        def update_hash(text_mob):
            ...
            new_text = "\n".join(
                "".join(row) for row in np.array(list(hash_hex)).reshape((4, 16))
            )
        ...
        self.add(equation)
        self.play(ChangeDecimalToValue(input_int, 2400, rate_func=linear, run_time=24))
...
class ContrastTwoAlgorithmsFrame(DisectAQuantumComputer):
    def construct(self):
        ...
        boxes = Square(0.1).get_grid(
            25, 4, fill_rows_first=False, v_buff=0.05, h_buff=1.1
        )
        ...
        for n in range(1, 10):
            ...
            self.play(
                LaggedStartMap(
                    VShowPassingFlash, lines, time_width=1.5, lag_ratio=2e-3
                ),
                Transform(
                    bars,
                    self.get_dist_bars(dist, boxes, width_ratio=width_ratio),
                    time_span=(0.5, 1),
                ),
            )
...
class QuantumCompilation(InteractiveScene):
    def construct(self):
        ...
        q_label = Text(
            "Quantum\nGates"
        )  # If I were ambitious, I'd show the proper quantum circuit here
        ...
        self.play(FadeOut(output_rect), Write(check[0], run_time=1))
        self.wait()
        self.play(
            input_rect.animate.surround(q_input),
            TransformFromCopy(
                VGroup(VectorizedPoint(bit_string.get_center()), bit_string), q_input
            ),
        )
        self.play(
            FadeOut(input_rect),
        ...
        self.play(
            ...
            UpdateFromFunc(
                bit_string, lambda m: m.set_value(int(input_value_tracker.get_value()))
            ),
            UpdateFromFunc(
                q_input[1], lambda m: m.set_value(int(input_value_tracker.get_value()))
            ),
            run_time=2,
        )
        self.wait()
