# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\grover\clarification.py
...
class Intro(InteractiveScene):
    ...
    def construct(self):
        ...
        # Comments
        folder = Path(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2025/blocks_and_grover/Comments"
        )
        comments = Group(
            Group(ImageMobject(folder / name)) for name in os.listdir(folder)
        )
        for comment in comments:
            comment.add_to_back(
                SurroundingRectangle(comment, buff=0).set_stroke(WHITE, 1)
            )
            comment.set_width(4)
            comment.move_to(4 * LEFT)
            comment.shift(
                np.random.uniform(-1, 1) * RIGHT + np.random.uniform(-3, 3) * UP
            )

        self.play(
            FadeOut(icon, time_span=(0, 2)),
            LaggedStartMap(
                FadeIn, comments, lag_ratio=0.6, shift=0.25 * UP, run_time=8
            ),
        )
        self.wait()
...
class HowDoYouKnowWhichAxis(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            morty.change("guilty"),
            stds[2].says(
                "Doesn't this assumes\nknowing the key value?",
                mode="angry",
...
class SudokuChecker(InteractiveScene):
    def construct(self):
        ...
        self.play(
            sudoku.unlocked_numbers.animate.set_opacity(1).set_anim_args(lag_ratio=0.1),
            run_time=3,
        )
        self.wait()
        ...
        row_rects, col_rects, square_rects = rect_groups = VGroup(
            VGroup(VGroup(SurroundingRectangle(num) for num in sg) for sg in group)
            for group in [
                sudoku.get_rows(),
                sudoku.get_columns(),
                sudoku.get_number_squares(),
            ]
        )
        ...
        for group in rect_groups:
            self.play(
                LaggedStart(
                    *(VFadeInThenOut(sg, lag_ratio=0.025) for sg in group),
                    lag_ratio=0.5,
                    run_time=5,
                )
            )
        ...
        self.play(Transform(unlocked_numbers, q_marks, lag_ratio=1e-2, run_time=2))
        self.wait()
        ...
        self.play(
            UpdateFromFunc(sudoku.unlocked_numbers, randomize_numbers, run_time=10)
        )
...
class CompiledSudokuVerifier(InteractiveScene):
    def construct(self):
        ...
        self.play(
            TransformFromCopy(bit_groups, target_bits, lag_ratio=1e-3, run_time=2)
        )
        self.wait()
...
class StateVectorsAsABasis(InteractiveScene):
    def construct(self):
        ...
        basis_labels = VGroup(
            ...
            .scale(0.3)
            .rotate(90 * DEG, RIGHT)
            .next_to(basis_vectors[0].get_end(), OUT, SMALL_BUFF),
            two_qubits[1]
            .copy()
            .scale(0.3)
            .rotate(90 * DEG, RIGHT)
            .next_to(basis_vectors[1].get_end(), OUT, SMALL_BUFF),
            two_qubits[2]
            .copy()
            .scale(0.3)
            .rotate(90 * DEG, RIGHT)
            .next_to(basis_vectors[2].get_end(), RIGHT, SMALL_BUFF),
        )
...
class TwoByTwoGrid(InteractiveScene):
    ...
    def construct(self):
        ...
        for sudoku, verifier, output, mark in zip(
            sudokus, verifiers, classical_outputs, marks
        ):
        ...
        for sudoku, verifier, output, mark in zip(
            ...
            self.play(
                LaggedStart(
                    FadeOutToPoint(
                        sudoku.numbers.copy(), verifier.get_center(), lag_ratio=1e-3
                    ),
        ...
        self.play(
            LaggedStart(
                FadeOutToPoint(
                    good_ket.copy(), verifiers[2].get_center(), lag_ratio=0.01
                ),
                FadeInFromPoint(
                    good_ket_out, verifiers[2].get_center(), lag_ratio=0.01
                ),
                lag_ratio=0.5,
            ),
        ...
        self.play(
            LaggedStart(
                FadeOutToPoint(
                    bad_ket.copy(), verifiers[3].get_center(), lag_ratio=0.01
                ),
    ...
    def turn_into_bits(self, sudoku):
        ...
        self.play(sudoku.animate.fade(0.9), FadeIn(bits, lag_ratio=1e-3))
        self.play(
            LaggedStart(
...
class AskWhyThatsTrue(TeacherStudentsScene):
    def construct(self):
        ...
        mapping = VGroup(
            VGroup(
                Text("True").set_color(GREEN), Vector(DOWN), Tex(R"\times -1")
            ).arrange(DOWN),
            VGroup(
                Text("False").set_color(RED), Vector(DOWN), Tex(R"\times +1")
            ).arrange(DOWN),
        )
        for part in mapping:
            tex = part[2]
...
class ListOfConfusions(InteractiveScene):
    def construct(self):
        # Test
        items = BulletedList(
            "Insufficient detail", "Bad framing", "Glossing over linearity", buff=1.0
        )
...
class ShowSuperposition(InteractiveScene):
    def construct(self):
        ...
        basis_vectors = VGroup(
            Vector(unit_size * vect, thickness=5) for vect in np.identity(3)
        )
        basis_vectors.set_submobject_colors_by_gradient(BLUE_D, BLUE_B)
        ...
        # Label bases
        basis_labels = VGroup(
            KetGroup(BitString(n, length=2)).set_height(0.35) for n in range(3)
        )
        basis_labels.rotate(90 * DEG, RIGHT)
        for label, vect, direction in zip(
            basis_labels, basis_vectors, [RIGHT, UP + OUT, RIGHT]
        ):
        ...
        # Show column vector
        col = DecimalMatrix(
            np.array([*vect_coords, 0]).reshape(-1, 1), decimal_config=dec_kw
        )
        col.scale(0.75)
        ...
        rects = VGroup(
            SurroundingRectangle(mob.copy().rotate(90 * DEG, LEFT)).rotate(
                90 * DEG, RIGHT
            )
...
class NorthEastTraveler(InteractiveScene):
    def construct(self):
        ...
        t_rot_marks = self.show_90_degree_rotation(
            travler, 45 * DEG, about_point=vel_vect.get_start()
        )
        self.wait()

        self.play(
            north_group.animate.set_fill(opacity=1).shift(UR), FadeOut(t_rot_marks)
        )
        n_rot_marks = self.show_90_degree_rotation(
            components[0], 90 * DEG, about_point=components[0].get_start()
        )
        self.wait()
        self.play(east_group.animate.set_fill(opacity=1).shift(DR + DOWN))
        e_rot_marks = self.show_90_degree_rotation(
            components[1], 0, about_point=components[1].get_start()
        )

        self.add(components.copy().set_opacity(0.5))
        self.play(
            components[1].animate.shift(
                vel_vect.get_start() - components[1].get_start()
            )
        )
        self.play(
            components[0].animate.shift(
                components[1].get_end() - components[0].get_start()
            )
        )
        self.wait()
        self.play(
            LaggedStart(
                *(
                    Rotate(
                        mob,
                        -90 * DEG,
                        about_point=vel_vect.get_start(),
                        run_time=6,
    ...
    def get_compass(self):
        ...
        spikes = VGroup(
            spike.copy().rotate(x * TAU / 4, about_point=ORIGIN) for x in range(4)
        )
        ...
        labels.scale(0.5)
        for label, spike, vect in zip(
            labels, spikes, compass_directions(4, start_vect=UP)
        ):
...
class ShowLinearityExample(InteractiveScene):
    def construct(self):
        ...
        self.play(
            FadeOutToPoint(
                in_group.copy(), machine.get_left() + RIGHT, lag_ratio=0.01, run_time=2
            )
        )
        ...
        self.play(
            FadeInFromPoint(
                out_group, machine.get_right() + LEFT, lag_ratio=0.01, run_time=2
            )
        )
        self.wait()
...
class ZGateExample(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                FadeOutToPoint(
                    gen_input.copy(), z_gates[2].get_center(), lag_ratio=0.01
                ),
...
class PassSuperpositionIntoVerifier(InteractiveScene):
    def construct(self):
        ...
        out_Vs = VGroup(
            Tex(R"V").set_color(TEAL).next_to(ket, LEFT, SMALL_BUFF) for ket in out_kets
        )
        ...
        self.play(
            LaggedStart(
                FadeOutToPoint(
                    in_group.copy(), machine.get_left() + RIGHT, lag_ratio=0.01
                ),
        ...
        new_machines = VGroup(
            machine.copy().scale(0.25).move_to(line) for line in lines
        )
        ...
        self.play(
            Transform(pre_machine, new_machines), LaggedStartMap(ShowCreation, lines)
        )
        self.wait()
        self.play(
            FadeOut(lines),
        ...
        in_col = TexMatrix(
            np.array(
                ["x_0", "x_1", R"\vdots", "x_k", R"\vdots", R"x_{n-2}", R"x_{n-1}"]
            ).reshape(-1, 1)
        )
        out_col = TexMatrix(
            np.array(
...
class SudokuBruteForce(InteractiveScene):
    def construct(self):
        ...
        grover_steps = TexText(
            R"$\displaystyle \left\lceil\frac{\pi}{4} 9^{30}\right\rceil$ Steps",
            isolate=["30"],
        )
        grover_steps.move_to(steps, LEFT)
        new_brace = Brace(grover_steps[:8], DOWN)
        new_number = Tex(
            "{:,d}".format(int(np.ceil(9**30 * PI / 4))).replace(",", "{,}")
        )
...
class ShaInversionCounts(InteractiveScene):
    def construct(self):
        ...
        quantum = VGroup(
            ...
            TexText(
                R"$\displaystyle \left\lceil \frac{\pi}{4} 2^{128} \right\rceil$ Steps"
            ),
        )
...
class FactoringNumbers(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                FadeOutToPoint(
                    product.copy(), icon.get_center(), lag_ratio=0.02, path_arc=45 * DEG
                ),
                FadeInFromPoint(
                    factor_group, icon.get_center(), lag_ratio=0.02, path_arc=45 * DEG
                ),
                lag_ratio=0.3,
            )
        )
