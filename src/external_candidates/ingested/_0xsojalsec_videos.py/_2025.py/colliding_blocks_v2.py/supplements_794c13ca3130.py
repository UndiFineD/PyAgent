...
class ShowPastVideos(InteractiveScene):
    def construct(self):
        ...
        explanation = VGroup(
            ...
            Text(
                "encourage them to pop out for a full explanation. Originally I didn't"
            ),
            Text(
                "want to trash up this channel with shorts, but then the only way to link to"
            ),
            Text(
                "link to a long-form videos was if the short lived on the same channel.  ¯\\_(ツ)_/¯"
            ),
        )
...
class RewindArrows(InteractiveScene):
    def construct(self):
        ...
        arrows.scale(2)
        self.play(
            LaggedStartMap(FadeIn, arrows, lag_ratio=0.1, shift=0.5 * LEFT, run_time=1)
        )
        self.wait()
...
class LoadSolutionIntoHead(InteractiveScene):
    def construct(self):
        # Test
        self.add(
            FullScreenFadeRectangle().set_fill(interpolate_color(GREY_E, BLACK, 0.5), 1)
        )
        randy = Randolph()
...
class ShowMassRatioToCountChart(InteractiveScene):
    def construct(self):
        ...
        counts = VGroup(
            Integer(int(PI * 10**n), font_size=42) for n in range(0, n_terms)
        )
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "pondering", "pondering", "pondering", look_at=underline
            ),
        ...
        for item, pair in zip(items, index_mode_pairs):
            ...
            self.play(
                ...
                FadeIn(
                    item, scale=3, shift=item.get_center() - stds[index].get_center()
                ),
            )
        self.wait(3)
...
class KeyStep(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            self.change_students("thinking", "well", "pondering", look_at=self.screen)
        )
        self.wait(3)
...
class StateSpaceLabel(InteractiveScene):
    def construct(self):
        ...
        title.to_corner(UL)
        arrow = Arrow(
            title.get_bottom() + 0.1 * DOWN + 1.0 * RIGHT, 1.0 * UP, thickness=5
        )
    ...
    def construct(self):
        ...
        radial_label.add_updater(
            lambda m: m.move_to(
                ...
                + 0.1 * rotate_vector(radial_line.get_vector(), -90 * DEG),
            )
        )
        ...
        arc = always_redraw(
            lambda: Arc(0, get_theta(), radius=radius).set_stroke(RED, 5)
        )
    ...
    def construct(self):
        ...
        self.play(
            self.students[2].says(
                "...why?", mode="confused", bubble_direction=LEFT, look_at=self.screen
            ),
...
class HighlightTheSlope(TeacherStudentsScene):
    def construct(self):
        # Test
        slope = Tex(
            R"-\sqrt{m_1 \over m_2}", t2c={"m_1": BLUE, "m_2": BLUE}, font_size=60
        )
...
class ExplainSmallAngleApprox(InteractiveScene):
    def construct(self):
        ...
        radial_label.add_updater(
            lambda m: m.move_to(
                radial_line.get_center()
                + 0.15 * rotate_vector(radial_line.get_vector(), -90 * DEG),
            )
        )
        ...
        get_theta = theta_tracker.get_value
        point.add_updater(
            lambda m: m.move_to(unit_size * rotate_vector(RIGHT, get_theta()))
        )
        ...
        self.play(
            TransformFromCopy(
                theta_label.copy().clear_updaters(), tan_eq[R"\theta"][0]
            ),
            Write(tan_eq[R"\tan("]),
            Write(tan_eq[R") ="]),
        )
        ...
        self.play(
            ...
            frame.animate.reorient(0, 0, 0, (1.46, -0.0, 0.0), 1.87).set_anim_args(
                run_time=3
            ),
        )
        self.play(
...
class DigitsOfPi(InteractiveScene):
    def construct(self):
        ...
        prefix_brace = Brace(
            VGroup(one_line_pi[2:index], nines[:3]), UP, MED_SMALL_BUFF
        )
        ...
        self.play(
            ...
            FadeOut(
                VGroup(prefix_brace, prefix_label, nines_brace, nines_label, nines)
            ),
...
class WhoCares(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(stds[1].animate.look_at(stds[0].eyes), stds[0].change("maybe"))
        )
...
class SimplifyingMessiness(InteractiveScene):
    ...
    def construct(self):
        ...
        rough_points = np.array(
            [
                np.random.uniform(0.95, 1.05) * circle.pfp(a)
        ...
        mid_arrow = Arrow(
            rough_circle, circle, buff=0.25, thickness=5, path_arc=1 * DEG
        )
...
class HiddenConnections(InteractiveScene):
    def construct(self):
        ...
        q_marks = VGroup(
            Tex(R"???", font_size=72).move_to(screen) for screen in screens
        )
        ...
        self.play(
            ...
            ReplacementTransform(
                circle.replicate(len(connections)), connections, lag_ratio=0.1
            ),
