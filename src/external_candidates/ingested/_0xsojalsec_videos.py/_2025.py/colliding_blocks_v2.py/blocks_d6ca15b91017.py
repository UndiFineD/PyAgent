# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\colliding_blocks_v2\blocks.py
...
class StateTracker(ValueTracker):
    ...
    def time_from_last_collision(self):
        ...
        collision_state = rot_state * (
            state[1] / rot_state[1]
        )  # Rescale to match y coordinate
...
class Blocks(InteractiveScene):
    ...
    def setup(self):
        ...
        self.floor_wall = (
            self.get_floor_and_wall_3d()
            if self.three_d
            else self.get_floor_and_wall_2d()
        )
        self.floor, self.wall = self.floor_wall
        self.blocks = self.get_block_pair(self.floor, three_d=self.three_d)

        self.state_tracker = StateTracker(
            self.blocks, self.initial_positions, self.initial_velocities
        )
    ...
    def construct(self):
        ...
        ke_equation = Tex(
            R"\frac{1}{2} m_1 (v_1)^2 + \frac{1}{2}m_2 (v_2)^2 = \text{E}", **kw
        )
        ...
        dec_equation = Tex(
            R"\frac{1}{2}(10)(+0.00)^2 + \frac{1}{2}(1)(+0.00)^2 = +0.00", font_size=42
        )
        dec_equation.next_to(ke_equation, DOWN, LARGE_BUFF)

        terms = dec_equation.make_number_changeable(
            "+0.00", replace_all=True, include_sign=True
        )
        ...
        label_targets = VGroup(
            dec_equation["(10)"], terms[0], dec_equation["(1)"], terms[1]
        ).copy()
        ...
        self.play(
            ...
            Transform(
                VGroup(*labels).copy().clear_updaters(),
                label_targets,
                remover=True,
                run_time=2,
                lag_ratio=1e-3,
        ...
        p_terms = p_dec_equation.make_number_changeable(
            "+0.00", replace_all=True, include_sign=True
        )
        ...
        label_targets = VGroup(
            p_dec_equation["(10)"], p_terms[0], p_dec_equation["(1)"], p_terms[1]
        ).copy()
        label_targets.clear_updaters()
        ...
        self.play(
            ...
            Transform(
                VGroup(*labels).copy().clear_updaters(), label_targets, remover=True
            ),
            run_time=2,
        )
        self.play(
        ...
        self.play(
            ...
            frame.animate.to_default_state(),
            equations.animate.set_width(4.5)
        ...
        self.play(
            LaggedStartMap(
                FlashAround, v_terms, lag_ratio=0.1, time_width=1.0, time_span=(0, 2)
            ),
            MoveToTarget(v_point, path_arc=PI),
            UpdateFromFunc(
                marked_velocity, lambda m: m.set_value(v_point.get_center()[:2])
            ),
            run_time=3,
        )
        self.wait()
        ...
        state_point.add_updater(
            lambda m: m.move_to(plane.c2p(*marked_velocity.get_value()))
        )
        ...
        marked_velocity.add_updater(
            lambda m: m.set_value(state_tracker.get_block_velocities())
        )
        ...
        terms = coord_label.make_number_changeable(
            "+0.00", replace_all=True, include_sign=True
        )
        ...
        self.play(
            LaggedStart(
                TransformFromCopy(
                    velocity_vectors[0][1].copy().clear_updaters(), terms[0]
                ),
                TransformFromCopy(
                    velocity_vectors[1][1].copy().clear_updaters(), terms[1]
                ),
        ...
        self.play(
            ...
            FadeOut(ke_rect),
            FadeOut(question),
            VGroup(ke_equation, xy_equation)
            .animate.arrange(DOWN, buff=0.5)
            .next_to(plane, LEFT, buff=LARGE_BUFF),
        )
        underlines = VGroup(
        ...
        self.play(
            Group(ellipse, traced_path, state_point).animate.stretch(
                math.sqrt(10), 0, about_point=plane.get_origin()
            ),
        ...
        state_point.add_updater(
            lambda m: m.move_to(plane.c2p(*state_tracker.get_scaled_block_velocities()))
        )
        ...
        if False:
            ...
            for _ in range(10):
                self.wait_until(
                    lambda: state_tracker.get_n_collisions() > last_n_collisions
                )
        ...
        self.play(
            x_rect.animate.shift(0.2 * RIGHT),
            run_time=4,
        ...
        marked_velocity.add_updater(
            lambda m: m.set_value(
                np.array(plane.p2c(state_point.get_center()))
                / state_tracker.sqrt_mass_vect
            )
        )
        arc = Arc(
            190 * DEG,
            50 * DEG,
            radius=0.5 * ellipse.get_width(),
        ...
        self.wait()
        self.play(
            p_line.animate.move_to(
                plane.c2p(state_tracker.get_momentum() / math.sqrt(self.masses[0]))
            ),
            run_time=2,
        )
        self.wait()
        ...
        VGroup(endzone_line_label, slope_label).rotate(
            state_tracker.theta, about_point=ORIGIN
        ).shift(endzone_line.pfp(0.45))
    ...
    def get_velocity_vector(
        ...
        label = DecimalNumber(
            0, num_decimal_places=num_decimal_places, font_size=font_size
        )
        label.set_fill(RED)
        label.set_backstroke(BLACK, label_backstroke_width)
        label.add_updater(
            lambda m: m.set_value(vel_function()).next_to(
...
class PreviewClip(BasicBlockCount):
    ...
    def construct(self):
        ...
        self.play(
            frame.animate.reorient(-46, -4, 0, (-0.78, -2.2, -0.17), 5.41), run_time=8
        )
        self.play(
        ...
        self.play(
            frame.animate.reorient(-3, -2, 0, (1.68, -0.59, -0.79), 9.86), run_time=20
        )
...
class SlowMoBlockCount(BasicBlockCount1e4):
    ...
    def setup(self):
        ...
        time_tracker.add_updater(
            lambda m, dt: m.increment_value(rate_tracker.get_value() * dt)
        )
        ...
        factor_label.add_updater(
            lambda m: m.set_value(int(1 / rate_tracker.get_value())).fix_in_frame()
        )
    ...
    def construct(self):
        ...
        self.play(
            self.frame.animate.reorient(
                -1, 0, 0, (-5.94, -2.36, -1.0), 3.80
            ).set_field_of_view(25 * DEG),
...
class SlowMoBlockCount1e6(SlowMoBlockCount):
    ...
    def construct(self):
        ...
        self.clack_sound = "clack"
        self.play(
            frame.animate.to_default_state().set_field_of_view(10 * DEG), run_time=5
        )
        self.wait(15)
...
class IntroduceSetup(BasicBlockCount):
    ...
    def construct(self):
        ...
        # Let it play out
        big_block_vect.add_updater(
            lambda m: m.shift(blocks[0][0].get_center() - m.get_start())
        )
        time_tracker.clear_updaters()
...
class CirclePuzzle(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            slope_label.animate.next_to(count_label, DOWN, aligned_edge=RIGHT).shift(
                right_shift
            ),
            VFadeIn(count_label),
        ...
        self.play(
            LaggedStart(
                (
                    line.animate.set_stroke(PINK, 5).set_anim_args(
                        rate_func=there_and_back
                    )
                    for line in lines
                ),
        ...
        slope_rect.add_updater(
            lambda m: m.set_width(
                slope_label.get_width() + 2 * SMALL_BUFF, stretch=True, about_edge=LEFT
            )
        )
        ...
        self.play(
            slope_tracker.animate.set_value(-10), rate_func=there_and_back, run_time=10
        )
        ...
        slope_tracker.add_updater(
            lambda m: m.set_value(-math.sqrt(int(get_mass_ratio())))
        )
        ...
        stacked_arcs = VGroup(
            arc.copy().rotate(PI - angle_of_vector(arc.pfp(0.5))) for arc in arcs
        )
        ...
        for n in range(6, len(lines), 2):
            self.play(
                ...
                two_theta_label.animate.next_to(
                    arcs[n].pfp(0.5), arcs[n].pfp(0.5), SMALL_BUFF
                ),
        ...
        self.play(
            theta_group.animate.rotate(PI, about_point=points[10])
            .shift(points[1] - points[10])
            .scale(0.75, about_point=points[1]),
            Rotate(new_lines, -12 * theta, about_point=ORIGIN),
            two_theta_label.animate.next_to(
                arcs[1].pfp(0.5), arcs[1].pfp(0.5), buff=0.05
            ),
        ...
        self.wait()
        self.play(
            slope_label_rect.animate.surround(mass_equations).stretch(
                1.2, 0, about_edge=LEFT
            ),
        )
        ...
        self.play(
            LaggedStart(
                TransformFromCopy(
                    new_slope_eq[R"\Delta y"], tan_eq[R"\Delta y"], path_arc=-90 * DEG
                ),
                TransformFromCopy(
                    new_slope_eq[R"\Delta x"], tan_eq[R"\Delta x"], path_arc=-90 * DEG
                ),
                run_time=2,
            )
        )
        ...
        self.play(
            LaggedStart(
                ...
                TransformFromCopy(
                    new_rhs[R"\sqrt{m_2 / m_1}"], arctan_eq[R"\sqrt{m_2 / m_1}"]
                ),
                lag_ratio=0.1,
                run_time=2,
            )
    ...
    def get_arcs(self, points, slope):
        ...
        for i in range(-1, len(points) - 2):
            arc = Line(
                points[max(i, 0)], points[i + 2], path_arc=-2 * theta * (-1) ** (i % 2)
            )
