...
class GroverPreview(InteractiveScene):
    def construct(self):
        ...
        for block, tex, color in zip(
            blocks, it.cycle(["U_k", "U_s"]), it.cycle([BLUE, GREEN])
        ):
            label = Tex(tex)
        ...
        amplitudes = VGroup(
            DecimalNumber(
                0.1, font_size=24, include_sign=True, num_decimal_places=3
            ).next_to(wire, LEFT, buff=0.1)
