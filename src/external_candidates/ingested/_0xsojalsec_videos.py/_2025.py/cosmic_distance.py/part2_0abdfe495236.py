# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\cosmic_distance\part2.py
import gzip
...
class StatsToVenus(InteractiveScene):
    def construct(self):
        # Test
        title = TexText(
            R"Nearest distance to Venus $\approx$ 39,000,000 km $\approx 6{,}200 \times R_E$"
        )
        title.to_edge(UP)
...
class AngleDeviationForVenusParallax(InteractiveScene):
    def construct(self):
        # Test
        title = TexText(
            R"Angle deviation $ = 2 \tan^{-1}(1 / 6200) \approx$ 1 arc-minute = $\displaystyle \frac{1}{60} \cdot 1^\circ$"
        )
        title.to_edge(UP)
...
class SunInTheSky(InteractiveScene):
    def construct(self):
        ...
        pure_sun = ImageMobject(
            str(
                Path(
                    base_path,
                    "Paul Animations/6. Transit Of Venus/New Transit of Venus scenes/JustSunBackgroundRemoved.png",
                )
            )
        )
        clean_sun = ImageMobject(
            str(
                Path(
                    base_path,
                    "Paul Animations/6. Transit Of Venus/New Transit of Venus scenes/JustSun.tif",
                )
            )
        )
        ...
        protractor.scale(
            sun_disk.get_height() / protractor.get_unit_size() / sun_arc_minutes
        )
        ...
        deg_labels = VGroup(
            Tex(Rf"{n}^\circ", font_size=2).next_to(
                protractor.n2p(60 * n), RIGHT, buff=0.01
            )
            for n in range(5)
        )
        ...
        self.play(
            frame.animate.set_height(1.75 * pure_sun.get_height())
            .move_to(pure_sun)
            .shift([0.1, 0.08, 0]),
        ...
        dashed_line = DashedLine(
            point, point + 2 * sun_disk.get_width() * LEFT, dash_length=dash_length
        )
        ...
        lines = VGroup(
            DashedLine(
                point + 0.675 * sun_disk.get_width() * RIGHT,
                [protractor.n2p(0)[0], point[1], 0],
                dash_length=0.5 * dash_length,
            ).set_stroke(TEAL, 1)
            for point in [p1, p2]
        )
...
class ShowDuration(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            time_tracker.animate.set_value(self.max_time),
            rate_func=linear,
...
class VenusTransitTimeline(InteractiveScene):
    def construct(self):
        ...
        numbers = timeline.add_numbers(
            range(1500, 2050, 50), group_with_commas=False, font_size=20, buff=0.15
        )
        ...
        dots = Group(GlowDot(timeline.n2p(year)).set_color(YELLOW) for year in years)
        ...
        self.play(
            ...
            .move_to(dots[0].get_center() + 0.5 * UP)
...
class LabelIo(InteractiveScene):
    def construct(self):
        # Background
        im = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2025/cosmic_distance/supplements2/JovianSystem.png"
        )
    ...
    def construct(self):
        ...
        deg10_labels = VGroup(
            ...
            for tick, number in zip(
                [*deg10_ticks[-2:], *deg10_ticks[1:3]], [340, 350, 10, 20]
            )
        )
        deg_labels = VGroup(
        ...
        arc_second_ticks = VGroup(
            self.get_tick_mark(
                circle, n * DEG / 60 / 60, 3.5e-4, max_aparent_length=0.5
            )
            for n in range(0, 60)
        )
        ...
        self.play(
            frame.animate.set_height(1.5 * deg_height).move_to(
                circle.pfp(0.5 * DEG / TAU)
            ),
            run_time=8,
        )
        self.wait(2)
        ...
        self.play(
            frame.animate.set_height(1.5 * deg_height / 60).move_to(
                circle.pfp(0.5 * DEG / TAU / 60)
            ),
            run_time=4,
        )
        self.wait()
        ...
        self.play(
            VGroup(circle, arc_minute_ticks, arc_second_ticks).animate.scale(
                20, about_point=zero_point
            ),
...
class ArcMinuteLabels(InteractiveScene):
    def construct(self):
        ...
        for label in [am_label, as_label]:
            self.clear()
            self.play(GrowFromCenter(brace), FadeIn(label, scale=3, shift=1 * LEFT))
            self.wait()
...
class CompareTwoStars(InteractiveScene):
    def construct(self):
        ...
        self.add(lines)
        self.wait()
        self.play(stars[0].animate.shift(lines[0].get_vector()).scale(4), run_time=2)
...
class InverseSquareLaw(InteractiveScene):
    def construct(self):
        ...
        radiation, op_tracker = self.beaming_effect(
            big_sphere, opacity_range=(0.15, 0), n_components=20, speed=0.5
        )
        ...
        self.wait(2)
        self.play(
            frame.animate.reorient(72, 75, 0, (-0.12, -0.13, 0.0), 8), run_time=10
        )
        ...
        division = VGroup(
            ParametricCurve(
                lambda t: 2 * sphere.uv_func(t, 94.5 * DEG),
                t_range=(0 * DEG, 9 * DEG, DEG),
            ),
            ParametricCurve(
                lambda t: 2 * sphere.uv_func(4.5 * DEG, t),
                t_range=(90 * DEG, 99 * DEG, DEG),
            ),
        )
    ...
    def beaming_effect(
        self, piece, n_components=20, speed=0.5, opacity_range=(0.5, 0.25)
    ):
...
class WriteInverseSquareLaw(InteractiveScene):
    def construct(self):
        # Test
        eq = Tex(
            R"\text{Apparent brightness} = \text{const} \cdot {\text{Absolute brightness} \over (\text{distance})^2}"
        )
        eq.to_edge(UP)
        eq.set_color(GREY_B)
        ...
        for text, color in zip(words, colors):
            part = eq[text][0]
            self.play(
                FlashAround(part, color=color, buff=0.1), part.animate.set_color(color)
            )
            self.wait()
...
class MeasuringNearbyStars(InteractiveScene):
    def construct(self):
        ...
        frame.add_updater(
            lambda m, dt: m.increment_theta(dtheta_tracker.get_value() * dt)
        )
        self.add(frame)
        ...
        for n in range(10):
            ...
            label = DecimalNumber(
                line.get_length() * 10, num_decimal_places=2, unit="L.Y.", font_size=24
            )
            ...
            vect = normalize(
                np.cross(line.get_end(), frame.get_implied_camera_location())
            )
        ...
        color_alphas = inverse_interpolate(
            color_index.min(), color_index.max(), color_index
        )
        ...
        self.play(
            FadeIn(axes),
            frame.animate.to_default_state(),
            stars.animate.set_points(random_points)
            .set_radii(new_radii)
            .set_glow_factor(0)
            .make_3d()
            .set_opacity(0.5),
        ...
        self.play(
            stars.animate.set_points(sorted_by_color).set_anim_args(
                run_time=5, path_arc=30 * DEG
            ),
            Write(x_axis_label, run_time=2),
        )
        self.wait()
        self.play(
            stars.animate.set_points(fully_sorted)
        ...
        self.play(
            frame.animate.reorient(0, 0, 0, (-0.75, -1.45, 0.0), 5.71).set_anim_args(
                run_time=3
            ),
    ...
    def color_index_to_rgb(self, bv_index):
        ...
        red = "#FF0000"
        cmap = get_colormap_from_colors(
            ["#0000FF", BLUE, WHITE, YELLOW, ORANGE, RED, *4 * [red]]
        )
        return cmap(alpha)
...
class LineToDistantStar(InteractiveScene):
    def construct(self):
        # Add Galaxy
        path = os.path.join(
            self.file_writer.get_output_file_rootname().parent, "GalaxyStill.png"
        )
...
class SolarSpectrum(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (Write(line, stroke_color=WHITE, stroke_width=0.1) for line in lines),
                lag_ratio=0.005,
                run_time=5,
            )
        )
        ...
        spectral_lines = self.get_spectral_lines(
            axes,
            spectral_wavelengths,
            np.ones_like(spectral_wavelengths),
            stroke_width=3,
        )
    ...
    def bucket_data(self, arr, bucket_size=10):  # TODO, change
        ...
        compressed = (
            arr[:full_size].reshape((full_size // bucket_size, bucket_size)).min(1)
        )
        return compressed
...
class GalacticSurveyData(InteractiveScene):
    def construct(self):
        ...
        dots.f_always.set_radii(
            lambda: max((rad_factor.get_value() * frame.get_height() ** 0.9), 0.1)
            * radii
        )
...
class TenPercentFrame(InteractiveScene):
    def construct(self):
        ...
        self.play(ShowCreation(lines, lag_ratio=0), Write(title))
        self.wait()
    ...
    def construct(self):
        ...
        plane = NumberPlane(
            ...
            faded_line_style=dict(
                stroke_color=BLUE, stroke_width=0.5, stroke_opacity=0.25
            ),
            faded_line_ratio=0,
        )
        ...
        self.add(plane)
        self.play(
            frame.animate.reorient(-90, 84, 0, (-0.04, 0.01, -0.01), 8.00), run_time=2
        )
