# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\cosmic_distance\paralax.py
...
class SimpleDotsParalax(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(VFadeIn(randy), randy.change("pondering", look_at=RIGHT))
        ...
        star_points = np.array(
            list(filter(lambda p: get_norm(p) < 1, stars.get_points()))
        )
        star_points[0] *= 10
        random.seed(1)
...
class ParalxInSolarSystem(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                sun,
                lambda m, a: frame.move_to(
                    interpolate(frame.get_center(), earth.get_center(), min(2 * a, 1))
                ),
            ),
            run_time=5,
        )
        ...
        self.play(frame.animate.reorient(-89, 74, 0), run_time=12)
...
class ParalaxMeasurmentFromEarth(InteractiveScene):
    def construct(self):
        ...
        for pi, label, angle in zip(pis, labels, angles):
            ...
            label.target.next_to(
                pi.target, UP * np.sign(angle), buff=SMALL_BUFF, aligned_edge=LEFT
            )
        ...
        for line, dot in zip(obs_lines, obs_dots):
            line.dot = dot
            line.add_updater(
                lambda m: m.put_start_and_end_on(m.dot.get_center(), obj.get_center())
            )
        ...
        celestial_sphere = get_celestial_sphere(
            radius=JUPITER_ORBIT_RADIUS * conversion_factor, constellation_opacity=0.0
        )
        ...
        # Add angle labels
        colors = [BLUE, RED]
        angle_labels = self.get_angle_labels(
            obs_lines, obs_points, line_between, arc_props=[0.75, 0.5], colors=colors
        )
        ...
        new_obs_lines = VGroup(Line(ol.get_start(), ol.get_end()) for ol in obs_lines)
        ...
        frame.add_updater(
            lambda m, dt: m.set_phi(interpolate(m.get_phi(), -90 * DEG, 0.025 * dt))
        )
        ...
        self.add(sun)
        self.play(
            frame.animate.set_height(0.4 * earth_orbit.get_width()).move_to(
                interpolate(venus_orbit.get_left(), sun.get_center(), 0.25)
            ),
        ...
        if False:
            ...
            self.play(
                frame.animate.reorient(0, 0, 0, (5.93, 0.25, 0.0), 15.86), run_time=5
            )
            self.wait()
        ...
        self.play(
            ...
            UpdateFromFunc(
                ...
                lambda m: m.put_start_and_end_on(
                    obs_dots[0].get_center(), obs_dots[1].get_center()
                ),
            ),
            run_time=3,
        )
        ...
        self.play(
            ...
            celestial_sphere.animate.set_width(
                20 * JUPITER_ORBIT_RADIUS * conversion_factor
            ),
            run_time=20,
        )
    ...
    def get_angle_labels(
        ...
        colors = [BLUE, RED]
        angle_labels = VGroup()
        for obs_line, obs_point, angle_sym, arc_prop, color in zip(
            obs_lines, obs_points, angle_syms, arc_props, colors
        ):
            obs_angle = obs_line.get_angle()
            line_angle = line_between.get_angle() + (PI if obs_angle > 0 else 0)
            arc = Arc(
                obs_angle,
                line_angle - obs_angle,
                arc_center=obs_point,
                radius=arc_radius,
            )
...
class NearbyStars(InteractiveScene):
    def construct(self):
        ...
        dist_label.f_always.move_to(
            lambda: dist_line.get_center()
            + 0.5 * normalize(rotate_vector(dist_line.get_vector(), 90 * DEG))
        )
        ...
        def update_au_label(label):
            ...
            step = 0.65 * interpolate(
                label.get_width(), label.get_height(), abs(direction[1])
            )
        ...
        self.play(
            LaggedStart(
                *(
                    ...
                    for t1, t2, i in zip(
                        "A.U.", ["A", "stronomical", "U", "nit"], [0, 0, 0, 1]
                    )
                ),
                lag_ratio=0.2,
            )
            ...
            UpdateFromAlphaFunc(
                ...
                lambda m, a: m.reorient(
                    *interpolate(np.array([-74, 79, 0]), np.zeros(3), a),
                    ...
                    np.exp(
                        interpolate(
                            np.log(2 * earth.get_height()), np.log(14), smooth(a)
                        )
                    ),
                ),
                run_time=5,
        ...
        star[1].add_updater(
            lambda m: m.set_width(0.4 * ((1 + math.sin(1.5 * self.time))))
        )
        ...
        obs_lines = VGroup(
            self.get_obs_line(obs_point, star) for obs_point in obs_points
        )
        obs_lines.set_stroke(WHITE, 2)
        for line, point in zip(obs_lines, obs_points):
            ...
            line.add_updater(
                lambda m: m.put_start_and_end_on(
                    m.start_point.get_center(), m.star.get_center()
                )
            )
        ...
        self.play(star.animate.move_to(15 * RIGHT), run_time=2)
        self.wait()

        obs_lines.suspend_updating()
        sample_obs_line = self.get_obs_line(earth_glow, star)
        self.play(FadeIn(sample_obs_line), obs_lines.animate.set_stroke(opacity=0.1))
        ...
        self.play(
            UpdateFromAlphaFunc(
                ...
                lambda m, a: m.move_to(
                    ...
                    * orbit_radius
                    / math.tan(
                        interpolate(curr_angle, 1e-5, there_and_back_with_pause(a)) / 2
                    )
                ),
            ),
            run_time=6,
        ...
        self.play(
            Transform(
                obs_lines[0].copy().clear_updaters(), obs_lines[1].copy(), remover=True
            ),
            run_time=2,
        )
        self.wait()
        ...
        kw = dict(t2c={R"\text{A.U.}": BLUE, "D": RED}, font_size=72)
        ...
        obs_lines.resume_updating()
        self.play(
            star.animate.move_to(1000 * RIGHT), rate_func=lambda t: t**4, run_time=5
        )

    def get_obs_line(
        self, obj1, obj2, dash_length=0.1, stroke_color=WHITE, stroke_width=2
    ):
