# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\cosmic_distance\supplements.py
...
class PilesOfResearchTopics(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, images, shift=0.2 * UP, lag_ratio=0.5, run_time=12)
        )
...
class AskingTaoForTopics(InteractiveScene):
    def construct(self):
        ...
        self.play(
            tau[:4].animate.stretch(0, 1, about_edge=DOWN),
        ...
        self.play(
            ...
            FadeIn(
                tau.get_bubble(
                    "How about the\nCosmic Distance Ladder?", bubble_type=SpeechBubble
                ),
                lag_ratio=0.1,
                run_time=2,
            ),
...
class TableOfContents(InteractiveScene):
    def construct(self):
        ...
        self.play(
            *(item.highlight_point.animate.match_y(item) for item in items), run_time=2
        )
        self.wait()
...
class MainCharacterTimeline(InteractiveScene):
    def construct(self):
        ...
        numbers = timeline.add_numbers(
            range(-500, 2100, 100), group_with_commas=False, font_size=20, buff=0.15
        )
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                frame, lambda m, a: m.set_height(interpolate(6, 12, there_and_back(a)))
            ),
            run_time=3,
        )
        self.play(
        ...
        self.play(
            ...
            cook_image.animate.scale(0.5, about_point=cook_label.get_corner(UR)).shift(
                0.1 * LEFT
            ),
        )
        self.wait()
        ...
        huygens_image.set_height(1.5).next_to(huygens_label, UP, SMALL_BUFF).shift(
            0 * LEFT
        )
        ...
        self.play(
            ...
            romer_image.animate.set_height(1.5)
            .next_to(romer_label, UR, 0)
            .shift(0.25 * LEFT),
        ...
        # Point out Venus
        venus_points = Group(
            TrueDot(timeline.n2p(year), color=YELLOW).make_3d() for year in [1761, 1769]
        )
        ...
        venus_arrows = VGroup(
            Arrow(
                venus_words["r"][0].get_top(), dot.get_center(), buff=0.1, thickness=1.5
            ).set_color(YELLOW)
        ...
        star_arrow = Arrow(star_words, star_point, buff=0.1, thickness=2).set_color(
            YELLOW
        )
        ...
        self.play(
            ...
            frame.animate.match_x(leavitt_label)
            .set_height(5.0)
...
class RouleauxTriangle(InteractiveScene):
    def construct(self):
        # Add triangle
        arcs = VGroup(
            Arc(30 * DEG + x * 120 * DEG, 60 * DEG, radius=1) for x in range(3)
        )
...
class WellOfSyene(InteractiveScene):
    def construct(self):
        # Test
        image_path = Path(
            self.file_writer.output_directory, "../KurtArtwork/TheWellOfSyene.jpg"
        ).resolve()
...
class NileLabels(InteractiveScene):
    def construct(self):
        ...
        alex_label = Text("Alexandria", font_size=60, font="CMU Serif").match_color(
            alex_point
        )
        alex_label.next_to(alex_point, DR, SMALL_BUFF)
        syene_label = Text("Syene", font_size=60, font="CMU Serif").match_color(
            syene_point
        )
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                ...
                FadeTransformPieces(
                    equation[R"\over"][0].copy(), equation[R"\over"][1]
                ),
                FadeTransformPieces(
                    equation[R"360^\circ"][0].copy(),
                    equation[R"\text{Circumference of Earth}"][0],
                ),
                lag_ratio=0.1,
            )
        )
...
class ShowFourLittleCircles(InteractiveScene):
    def construct(self):
        # Add image
        image_path = Path(
            self.file_writer.output_directory, "LunarEclipseComposite.jpg"
        ).resolve()
        ...
        lil_circle.move_to([-1.00, -0.167, 0.0])
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(lil_circle, circ) for circ in four_circles),
                lag_ratio=0.25,
                run_time=3,
            )
        )
...
class MoonSizeCalculation(InteractiveScene):
    def construct(self):
        ...
        # Annotations
        top_rect = SurroundingRectangle(
            SurroundingRectangle(eq["R_M"], buff=0.01).set_stroke(width=1)
        )
        low_rect = SurroundingRectangle(
            SurroundingRectangle(eq["D_M"], buff=0.01).set_stroke(width=1)
        )
...
class OtherGreeks(InteractiveScene):
    def construct(self):
        ...
        base_image = ImageMobject(
            Path(folder, "AristarchusProposingEarthGoesAroundTheSun.jpg")
        )
        sub_images = Group(
...
class EqualAreas1(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            UpdateFromAlphaFunc(
                arc, lambda m, a: m.become(self.get_slice(a * self.angle))
            ),
...
class EvenWithMathRight(InteractiveScene):
    def construct(self):
        # Test
        words = TexText(
            R"Even when you have the math right\\you don't necessarily get to the truth",
            font_size=36,
        )
        words.to_corner(UL)
...
class UniversalProblemSolvingTip(InteractiveScene):
    def construct(self):
        ...
        words = TexText(
            R"If you can't solve a problem, try \\ to solve a simpler problem instead",
            font_size=60,
            isolate="simpler problem ",
        )
...
class CountUpDates(InteractiveScene):
    def construct(self):
        ...
        label = always_redraw(
            lambda: self.day_to_date_label(int(day_tracker.get_value()))
        )
        self.add(label)
...
class Subscribe(InteractiveScene):
    def construct(self):
        ...
        self.play(morty.change("raise_right_hand"))
        self.play(
            LaggedStartMap(FadeIn, logos, shift=0.25 * DR, lag_ratio=0.25, run_time=2)
        )
        self.play(Blink(morty))
        self.wait()
        ...
        self.play(
            ...
            logos.animate.arrange_in_grid(2, 4, h_buff=0.25, v_buff=0.5)
            .set_height(1.0)
