# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\cosmic_distance\planets.py
...
def get_earth(
    radius=1.0, day_texture="EarthTextureMap", night_texture="NightEarthTextureMap"
):
...
def get_moon(radius=1.0, resolution=(101, 51)):
    moon = TexturedSurface(
        Sphere(radius=radius, resolution=resolution), "MoonTexture", "DarkMoonTexture"
    )
...
def get_sun(
    ...
    big_glow = GlowDot(
        radius=big_glow_ratio * radius,
        glow_factor=big_glow_factor,
        opacity=big_glow_opacity,
    )
...
def get_planet(name, radius=1.0):
    planet = TexturedSurface(
        Sphere(radius=radius), f"{name}Texture", f"Dark{name}Texture"
    )
...
def get_celestial_sphere(radius=1000, constellation_opacity=0.1):
    sphere = Group(
        ...
        TexturedSurface(
            Sphere(radius=0.99 * radius, clockwise=True), "constellation_figures"
        ),
    )
...
class PerspectivesOnEarth(InteractiveScene):
    def construct(self):
        ...
        dashed_lines = VGroup(
            DashedLine(
                brace.get_corner(OUT + RIGHT), earth.get_zenith(), dash_length=0.02
            ),
            DashedLine(
                brace.get_corner(IN + RIGHT), earth.get_center(), dash_length=0.02
            ),
        )
        ...
        self.wait(4)
        self.play(
            LaggedStartMap(FadeOut, VGroup(label, brace, *dashed_lines), shift=IN)
        )
        ...
        frame.add_updater(
            lambda m: m.move_to(
                interpolate(
                    m.get_center(), moon.get_center(), interp_factor.get_value()
                )
            )
        )
        self.play(
...
class SphericalEarthVsFlat(InteractiveScene):
    def construct(self):
        ...
        self.play(frame.animate.reorient(166, 63, 0, ORIGIN, FRAME_HEIGHT), run_time=2)
        self.wait()
        ...
        # Squish
        day_earth = get_earth(
            radius=earth.get_height() / 2, night_texture="EarthTextureMap"
        )
        ...
        self.play(Transform(day_earth, flat_earth), light.animate.set_z(10), run_time=2)
    ...
    def look_from_various_angles(self, run_time=1.5):
        frame = self.frame
        orientations = [(10, 80, 0), (-150, 129, 0), (97, 55, 0), (0, 0, 0)]
...
class SizeOfEarthRenewed(InteractiveScene):
    ...
    def construct(self):
        ...
        inner_shell.add_updater(
            lambda m: m.set_clip_plane(OUT, slice_tracker.get_value())
        )
        mesh.add_updater(lambda m: m.set_clip_plane(IN, -slice_tracker.get_value()))
        transparent_earth.add_updater(
            lambda m: m.set_clip_plane(IN, -slice_tracker.get_value())
        )
        ...
        self.play(FadeIn(rays, shift=0.5 * LEFT, lag_ratio=0.02), run_time=2)
        self.play(
        ...
        self.play(
            ...
            frame.animate.reorient(
                -130, -14, 0, (2.66, 0.14, -0.69), 8.18
            ).set_anim_args(time_span=(2, 7)),
            run_time=7,
        )
        ...
        self.play(
            ...
            frame.animate.reorient(-205, -11, 0, (3.0, -0.0, 0.0), 6.50).set_anim_args(
                run_time=10
            ),
        )
        ...
        self.play(
            Rotate(earth_group, 40 * DEG, axis=UP, run_time=3, rate_func=there_and_back)
        )
        self.play(Rotate(earth_group, -40 * DEG, axis=OUT, run_time=3))

        # Emphasize the tilt of the earth's axis
        axis_line = Line(
            -1.25 * self.radius * earth_axis, 1.25 * self.radius * earth_axis
        )
        ...
        self.play(
            ...
            FadeIn(
                ...
                remover=True,
                time_span=(4, 15),
                rate_func=lambda t: there_and_back_with_pause(t, 9 / 11),
            ),
            frame.animate.reorient(
                -91, -36, 0, (4.91, -1.44, 0.39), 11.01
            ).set_anim_args(
                time_span=(4, 15), rate_func=lambda t: there_and_back_with_pause(t, 0.2)
            ),
            run_time=15,
        )
        ...
        alex_arrow = Arrow(
            alex_name.get_bottom() + 0.35 * LEFT,
            alex_point,
            buff=SMALL_BUFF,
            thickness=2,
        )
        ...
        syene_arrow = Arrow(
            syene_name.get_top(), syene_point, buff=SMALL_BUFF, thickness=2
        )
        ...
        self.wait()
        self.play(
            arc.animate.shift(earth_point - arc.get_end())
            .shift(0.01 * LEFT)
            .set_stroke(RED),
        ...
        self.play(
            LaggedStartMap(
                FadeOut, VGroup(alex_name, alex_arrow, syene_name, syene_arrow)
            )
        )
...
class AlBiruniEarthMeasurement(InteractiveScene):
    def construct(self):
        ...
        mountain = Polyline(earth.pfp(0.02), mountain_tip, earth.pfp(0.98))
        ...
        line_of_sight = DashedLine(
            ORIGIN, line_length * RIGHT, dash_length=DEFAULT_DASH_LENGTH / 2
        )
        ...
        self.play(
            TransformMatchingTex(
                equation.copy(), eq2, path_arc=45 * DEG, lag_ratio=0.01
            )
        )
        self.wait()
        self.play(
            TransformMatchingTex(eq2.copy(), eq3, path_arc=45 * DEG, lag_ratio=0.01)
        )
        self.add(rect, eq3)
...
class LuarEclipse(InteractiveScene):
    def construct(self):
        ...
        dashed_lines = VGroup(
            ...
            DashedLine(
                brace.get_corner(DR), earth.get_bottom() + RIGHT, dash_length=0.03
            ),
        )
        ...
        radial_line.add_updater(
            lambda m: m.put_start_and_end_on(earth.get_center(), moon.get_center())
        )
        ...
        dist_question = TexText(
            R"Distance to the moon \\ $\approx$ 385,000km $\pm$ 21,000km", font_size=600
        )
        dist_question.add_updater(
            lambda m: m.next_to(radial_line.get_center(), DR, buff=1)
        )
        ...
        self.play(
            ...
            frame.animate.set_height(6 * EARTH_RADIUS * conversion_factor).move_to(
                MOON_ORBIT_RADIUS * conversion_factor * RIGHT
            ),
        ...
        self.play(GrowFromCenter(brace_copy), FadeIn(label_copy, lag_ratio=0.1))
        self.wait()
        ...
        month_arrow.add_tip()
        month_arrow.tip.set_height(
            4 * EARTH_RADIUS * conversion_factor, about_point=month_arrow.get_end()
        )
        ...
        self.play(
            frame.animate.move_to(MOON_ORBIT_RADIUS * conversion_factor * LEFT)
            .set_height(1.35 * orbit.get_height())
            .set_anim_args(run_time=4),
            orbit_rate.animate.set_value(15).set_anim_args(run_time=4),
            time_label.animate.set_height(
                large_text_height, about_point=brace_copy.get_right()
            ).set_anim_args(run_time=4),
...
class PenumbraAndUmbra(InteractiveScene):
    def construct(self):
        ...
        # Add shadows
        umbra, penumbra, umbral_lines, penumbral_lines = shadow_group = (
            self.get_umbral_lines(sun[0], earth)
        )
        ...
        self.play(
            ...
            UpdateFromFunc(
                shadow_group, lambda m: m.become(self.get_umbral_lines(sun[0], earth))
            ),
            run_time=8,
        )
    ...
    def get_umbral_lines(self, circle1, circle2):
        ...
        penumbra = Polygon(
            c2 + r2 * v3,
            penumbral_lines[0].get_end(),
            penumbral_lines[1].get_end(),
            c2 + r2 * v4,
        )
...
class LineOfSight(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            frame.animate.set_height(
                MOON_ORBIT_RADIUS * conversion_factor * 2.25
            ).center(),
        ...
        self.play(
            orbit.animate.stretch(0.9, 1)
            .stretch(1.1, 0)
            .shift(3 * earth.get_width() * LEFT),
        ...
        # Zoom in then out
        self.play(
            frame.animate.reorient(80, 82, 0, moon.get_center(), 0.80), run_time=5
        )
        self.wait()
        self.play(
            frame.animate.reorient(
                0, 0, 0, ORIGIN, MOON_ORBIT_RADIUS * conversion_factor * 2.25
            ),
            run_time=5,
        )
        self.wait()
        ...
        self.play(
            ...
            FadeOut(moons),
            frame.animate.set_height(1.5 * small_orbit.get_height())
            .center()
            .set_anim_args(time_span=(1, 5)),
            run_time=5,
        )
...
class DistanceToSun(InteractiveScene):
    def construct(self):
        ...
        scaled_moon_radius = (
            0.5
            * (sun[0].get_height() / get_dist(earth.get_center(), sun.get_center()))
            * orbit_radius
        )
        ...
        sun[0].add_updater(
            lambda m: m.move_to(sun_point).set_height(
                size_ratio * get_dist(sun_point.get_center(), earth.get_center())
            )
        )
        sun[1].add_updater(
            lambda m: m.move_to(sun[0]).set_height(1.2 * sun[0].get_height())
        )
        sun[2].add_updater(lambda m: m.move_to(sun[0]))

        VGroup(sun_brace, sun_radius_label).set_color(WHITE)
        sun_brace.f_always.set_height(
            lambda: 0.5 * sun[0].get_height(), stretch=lambda: True
        )
        ...
        self.play(
            frame.animate.reorient(
                -87.85, 89.126, 0, [-5.6279674, -0.0808167, 0.00590339], 0.03
            ),
        ...
        self.wait()
        self.play(frame.animate.to_default_state(), FadeIn(orbit), run_time=3)
        ...
        true_sun_center = earth.get_center() + to_moon * (
            EARTH_ORBIT_RADIUS / MOON_ORBIT_RADIUS
        )
        ...
        self.play(
            LaggedStartMap(
                FadeOut, VGroup(dist_brace, dist_brace2, side_brace, side_brace_pair)
            )
        )
        self.wait()
        ...
        VGroup(words1, words2, sub_words, arrow1, arrow2).scale(
            0.6 / FRAME_HEIGHT, about_point=ORIGIN
        ).shift(moon.get_center())
        ...
        self.play(frame.animate.reorient(0, 0, 0, moon.get_center(), 0.6), run_time=2)
        self.play(
            FadeIn(words1, lag_ratio=0.1),
            Write(arrow1),
        )
        self.wait()
        self.play(FadeIn(words2, lag_ratio=0.1), Write(arrow2), ShowCreation(our_half))
        self.wait()
        self.play(FadeIn(sub_words, 0.03 * DOWN))
        self.play(
            FadeOut(VGroup(words1, arrow1, words2, arrow2, sub_words)),
            frame.animate.reorient(0, 0, 0, (-6.23, 0.02, 0.0), 4.73).set_anim_args(
                run_time=2
            ),
        )
        ...
        self.play(
            Rotate(orbit_group, TAU / 4, about_point=earth.get_center(), run_time=5)
        )
        ...
        self.play(
            frame.animate.reorient(3, 63, 0, (-6.08, 1.44, -0.78), 2.88), run_time=3
        )
        self.play(
            Rotate(
                orbit_group,
        ...
        self.play(
            Rotate(orbit_group, TAU / 8, about_point=earth.get_center(), run_time=2)
        )
        full_moon = moon.copy()
        ...
        self.add(full_moon)
        self.play(
            Rotate(orbit_group, TAU / 2, about_point=earth.get_center(), run_time=2)
        )
        ...
        self.play(
            Rotate(
                orbit_group, 3 * TAU / 8, about_point=earth.get_center(), run_time=8
            ),
            VFadeIn(question, time_span=(1, 3)),
        )
        self.play(
            Rotate(orbit_group, -TAU / 8, about_point=earth.get_center(), run_time=8)
        )
        self.wait()
        ...
        def get_half_moon_point():
            ...
            return (
                earth.get_center() + rotate_vector(RIGHT, theta) * orbit.get_width() / 2
            )
        ...
        elbow2 = Elbow(width=0.15, angle=get_half_moon_angle() - PI).shift(
            half_moon_point
        )
        ...
        self.play(
            Transform(
                lit_half,
                lit_half.copy()
                .rotate(90 * DEG, about_point=half_moon_point)
                .set_color(YELLOW, 0.35),
                path_arc=90 * DEG,
            ),
            run_time=1,
        )
        ...
        def get_implied_sun_location():
            return earth.get_center() + RIGHT * (orbit.get_width() / 2) / math.cos(
                get_lunar_angle()
            )
        ...
        sun_line.add_updater(
            lambda m: m.put_start_and_end_on(moon.get_center(), sun.get_center())
        )

        self.remove(lines2)
        self.add(orbit_group, sun_line)
        self.play(
            Rotate(orbit_group, 25 * DEG, about_point=earth.get_center()), run_time=5
        )
        self.play(
            Rotate(orbit_group, -35 * DEG, about_point=earth.get_center()), run_time=5
        )
        self.play(
            Rotate(orbit_group, 25 * DEG, about_point=earth.get_center()), run_time=10
        )

        # Add angle label
        v_line = DashedLine(
            earth.get_center(), earth.get_center() + orbit.get_width() * UP
        )
        ...
        self.wait()
        self.play(
            Rotate(orbit_group, 10 * DEG, about_point=earth.get_center()), run_time=8
        )

        # Equation
        dist_eq = Tex(
            R"D_S = {D_M \over \sin(\theta)}", t2c={"D_S": YELLOW, "D_M": GREY_B}
        )
        dist_eq.to_edge(UP)
        ...
        self.play(FadeIn(question), FadeIn(arrow), ShowCreation(disc_arc))
        self.wait()
        self.play(
        ...
        v_line_copies = VGroup(
            v_line.copy().align_to(brace_copy, RIGHT) for brace_copy in brace_copies
        )
        ...
        self.play(
            ...
            FadeOut(dist_label),
            Rotate(
                orbit_group, 1 * DEG - math.asin(1 / 20), about_point=earth.get_center()
            ),
        )
        self.play(
        ...
        self.play(
            Rotate(
                orbit_group,
                math.asin(1 / 20) - math.asin(1 / 383),
                about_point=earth.get_center(),
            ),
...
class HowManyEarthsInsideSun(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(circle, circle2) for circle2 in stack),
                lag_ratio=0.05,
            )
        )
        self.wait()
...
class EarthAroundSun(InteractiveScene):
    def construct(self):
        ...
        # Orbits
        sun_orbit = TracingTail(
            sun, stroke_color=YELLOW, stroke_width=(0, 10), time_traced=5
        )
        earth_orbit = TracingTail(
            earth, stroke_color=BLUE, stroke_width=(0, 5), time_traced=5
        )
        ...
        self.wait(3)
        self.play(
            sun.animate.scale(109 / 7, about_edge=LEFT).move_to(10 * RIGHT), run_time=4
        )
        self.wait()
...
class NearestPlanets(InteractiveScene):
    ...
    def construct(self):
        ...
        for radius, period, color in zip(radii, periods, colors):
            ...
            orbit.period = period
            orbit.add_updater(
                lambda m, dt: m.rotate(0.5 * (seconds_per_day / m.period) * TAU * dt)
            )
            orbits.add(orbit)
        ...
        braces = VGroup()
        for orbit, symbol_tex, angle in zip(
            orbits, symbol_texs, np.linspace(90 * DEG, 0, len(symbol_texs))
        ):
        ...
        self.play(
            Succession(
                ...
                *(
                    Transform(
                        main_brace, b, rate_func=lambda t: smooth(clip(1.5 * t, 0, 1))
                    )
                    for b in braces[1:]
                ),
            ),
        ...
        spheres = Group(self.get_open_sphere(orbit.get_radius()) for orbit in orbits)
        ...
        for solid in solids:
            solid.add_updater(
                lambda m, dt: m.rotate(10 * DEG * dt * math.cos(self.time), axis=OUT)
            )
        ...
        self.play(
            LaggedStartMap(
                FadeIn, solids, shift=0.25 * OUT, run_time=2, lag_ratio=0.25
            ),
            FadeOut(symbols),
        )
        self.wait(3)
        ...
        def drop_sphere(index, run_time=1.5):
            ...
            self.play(FadeIn(spheres[index], scale=0.7), run_time=run_time)
        ...
        self.play(
            LaggedStart(
                *(sphere.clip_tracker.animate.set_value(0) for sphere in spheres[::-1]),
                lag_ratio=0.25,
                run_time=6,
            )
        )
        ...
        self.play(
            *(
                mob.animate.scale(1.1).set_anim_args(
                    rate_func=lambda t: wiggle(t, 5),
                    time_span=(random.random(), 4 + random.random()),
                )
        ...
        planets = Group(
            Sphere(
                radius=small_radius * math.sqrt(orbit.get_radius()),
                color=orbit.get_color(),
            ).set_shading(0.25, 0.25, 1)
            for orbit in orbits
        )
    ...
    def get_open_sphere(self, radius, color=GREY_C, opacity=0.5):
        ...
        sphere.add_updater(
            lambda m: m.set_clip_plane(IN, result.clip_tracker.get_value() * radius)
        )
        mesh.add_updater(
            lambda m: m.set_clip_plane(OUT, result.clip_tracker.get_value() * radius)
        )
    ...
    def wireframe_from_points(self, points, n_neighbors):
        ...
        for point in points:
            ...
            lines.add(
                *(Line(point, points[index]) for index in indices[1 : 1 + n_neighbors])
            )
...
class KeplersMethod(InteractiveScene):
    def construct(self):
        ...
        self.play(
            mars.animate.move_to(
                interpolate(mars.get_center(), earth.get_center(), 0.5)
            ),
            run_time=4,
            rate_func=wiggle,
        )
        ...
        self.play(
            ...
            Rotate(
                mars,
                (EARTH_ORBIT_PERIOD / MARS_ORBIT_PERIOD) * TAU,
        ...
        self.play(
            ...
            Rotate(
                mars,
                (EARTH_ORBIT_PERIOD / MARS_ORBIT_PERIOD) * angle,
                about_point=sun.get_center(),
            ),
        ...
        earth_orbit.move_to(
            0.5
            * math.sqrt(earth_orbit.get_width() ** 2 - earth_orbit.get_height() ** 2)
            * LEFT
        )
        earth_orbit.rotate(
            angle_of_vector(earth.get_center())
            - angle_of_vector(earth_orbit.get_start()),
            about_point=ORIGIN,
        )
        dens_func = bezier([0, 0.2, 0.8, 1])
        n_samples = 24
        sample_points = [
            earth_orbit.pfp(dens_func(a)) for a in np.arange(0, 1, 1 / n_samples)
        ]
        ...
        ghost_lines, ghost_earths = self.show_time_series_measurments(
            earth, mars, sun, earth_glow, 5, 4
        )
        self.bind_ghost_lines_to_mars(ghost_lines, ghost_earths, mars)

        self.play(*map(FadeOut, [earth, earth_glow, earth_symbol]))
        self.play(
            Rotate(mars, 5 * DEG, about_point=ORIGIN, run_time=5, rate_func=wiggle)
        )
        ...
        self.play(
            ...
            Rotate(
                earth,
                (MARS_ORBIT_PERIOD / EARTH_ORBIT_PERIOD) * 5 * DEG,
                about_point=sun.get_center(),
            ),
            FadeIn(earth_glow),
        )
        ...
        for n in range(30):
            ...
            mars_lines = VGroup(
                Line(mars.get_center(), dot.get_center(), buff=0) for dot in cluster
            )
            ...
            self.play(FadeOut(last_mars_lines), mars.animate.move_to(new_mars_spot))
            self.play(
        ...
        earth.add_updater(
            lambda m: m.move_to(
                earth_orbit.pfp(dens_func(earth_location_tracker.get_value() % 1))
            )
        )
        earth_glow.update()
        mars_lines = VGroup()
        ...
        for n in range(5):
            ...
            self.play(ShowCreation(mars_line), FadeIn(earth_copy))
            mars_lines.add(mars_line)
            earth_copies.add(earth_copy)
            if n == 0:
                self.play(
                    mars.animate.move_to(mars_line.pfp(0.75)).set_anim_args(
                        rate_func=wiggle
                    ),
            ...
            if n < 4:
                self.play(
                    ...
                    earth_location_tracker.animate.increment_value(
                        MARS_ORBIT_PERIOD / EARTH_ORBIT_PERIOD
                    ),
                    run_time=2,
                )
            else:
        ...
        for dot, mars_line in zip(earth_copies, mars_lines):
            dot.add_updater(
                lambda m: m.move_to(
                    earth_orbit.pfp(
                        dens_func(
                            (earth_location_tracker.get_value() + m.orbit_offset) % 1
                        )
                    )
                )
            )
        ...
        for n in range(100):
            ...
            mars.rotate(
                d_alpha * (EARTH_ORBIT_PERIOD / MARS_ORBIT_PERIOD) * TAU,
                about_point=0.5 * UR,
            )
            earth_location_tracker.increment_value(d_alpha)
            self.wait(0.2)

    def get_line_between_bodies(
    ...
    def get_mystery_orbit(self, orbit, opacity=0.25, n_q_marks=12):
        ...
        q_marks = VGroup(
            Tex("?").move_to(orbit.pfp(a)) for a in np.arange(0, 1, 1 / n_q_marks)
        )
        q_marks.set_fill(opacity=0.25)
        return VGroup(annulus, q_marks)

    def show_time_series_measurments(
        self, earth, mars, sun, earth_glow, n_measurements=5, rotation_time=2
    ):
        ghost_lines = VGroup()
        ghost_earths = Group()
        corner_counter = VGroup(
            Integer(687, include_sign=True, edge_to_fix=RIGHT), Text("Days")
        )
        ...
        for n in range(n_measurements):
            ...
            if n < n_measurements - 1:
                ...
                self.play(
                    ...
                    Rotate(
                        earth,
                        (MARS_ORBIT_PERIOD / EARTH_ORBIT_PERIOD) * TAU,
                        about_point=sun.get_center(),
                    ),
                    ChangeDecimalToValue(corner_counter[0], 687),
                    ghost_lines.animate.set_stroke(opacity=0.2).set_anim_args(
                        time_span=(0, 1)
                    ),
                ...
                self.play(
                    ghost_lines.animate.set_stroke(opacity=0.2), FadeOut(corner_counter)
                )
    ...
    def bind_ghost_lines_to_mars(self, ghost_lines, ghost_earths, mars):
        ...
        for pair, dot in zip(ghost_lines, ghost_earths):
            ...
            dot.add_updater(
                lambda m: m.move_to(
                    find_intersection(
                        m.sun_line.get_start(),
                        m.sun_line.get_vector(),
                        m.mars_line.get_start(),
                        m.mars_line.get_vector(),
                    )
                )
            )
    ...
    def get_rigid_lines_from_mars(self, mars, ghost_earths):
        mars_lines = VGroup(
            self.get_line_between_bodies(mars, dot, RED, 1, 1) for dot in ghost_earths
        )
...
class ShowCreationOfAllOrbits(KeplersMethod):
    def construct(self):
        ...
        eccentricities = [0.206, 0, 0.017, 0.093, 0.048, 0.056]
        colors = [GREY_C, TEAL, BLUE, RED, ORANGE, GREY_BROWN]
        ...
        for orbit, symbol in zip(orbits, symbols):
            ...
            orbit.symbol = symbol
            symbol.buff_factor = (
                orbit.get_radius() + 0.6 * symbol.get_height()
            ) / orbit.get_radius()
            symbol.dot = dot
        ...
        frame.add_updater(
            lambda m, dt: m.set_height(
                m.get_height() + growth_rate_tracker.get_value() * dt
            )
        )
    ...
    def draw_orbit(
        self, earth_orbit, orbit, period, n_samples=3, run_time=4, added_anims=[]
    ):
        ...
        for n in range(n_samples):
            ...
            line = self.get_line_between_bodies(
                dot, orbit.dot, orbit.get_color(), 1, 1.1
            )
            lines.add(line),
...
class LightFromEarthToMoon(InteractiveScene):
    ...
    def get_pulse(
        ...
        tail = TracingTail(
            pulse, stroke_width=(0, max_stroke_width), time_traced=tail_time
        )
...
class EarthAndVenus(InteractiveScene):
    def construct(self):
        ...
        venus, earth = glows
        angles = [
            (180 / period) * TAU for period in [VENUS_ORBIT_PERIOD, EARTH_ORBIT_PERIOD]
        ]
...
class RadarToVenus(LightFromEarthToMoon):
    def construct(self):
        ...
        glows = Group(
            GlowDot(orbit.get_start(), color=orbit.get_color()) for orbit in orbits
        )
        symbols = VGroup(*map(get_planet_symbols, ["venus", "earth"]))
