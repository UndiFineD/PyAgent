...
class IntroduceGraph(GraphScene):
    def construct(self):
        ...
        connected, planar, graph = (
            OldTexText(["Connected ", "Planar ", "Graph"]).to_edge(UP).split()
        )
        ...
        self.play(
            ShowCreation(
                Mobject(*edges_to_remove), rate_func=lambda t: 1 - t, run_time=1.0
            )
        )
        self.wait(2)
        self.remove(not_okay)
    ...
    def construct(self):
        ...
        for path, non_path in zip(paths, non_paths):
            path_lines = Mobject(
                *[
                    Line(
                        self.get_points()[path[i]], self.get_points()[path[i + 1]]
                    ).set_color("yellow")
    ...
    def construct(self):
        ...
        for pair in self.spanning_tree_index_pairs:
            self.play(
                ShowCreation(
                    Line(
                        self.get_points()[pair[0]], self.get_points()[pair[1]]
                    ).set_color("yellow"),
...
class FacebookGraph(GraphScene):
    def construct(self):
        ...
        friends = OldTex("\\leftarrow \\text{friends} \\rightarrow").scale(
...
class ExamplesOfGraphs(GraphScene):
    def construct(self):
        ...
        vertical_line = Line(
            (vert_line_x_val, FRAME_Y_RADIUS, 0), (vert_line_x_val, -FRAME_Y_RADIUS, 0)
        )
        objects_and_notions = [
        ...
        self.play(
            ShowCreation(
                Mobject(
                    *[
                        Line(
                            self.get_points()[last], self.get_points()[next]
                        ).set_color("green")
...
class MortimerCannotTraverseCycle(GraphScene):
    ...
    def construct(self):
        ...
        self.play(
            *[
                Transform(
                    v, Dot(v.get_center(), radius=3 * Dot.DEFAULT_RADIUS, color="green")
                )
