# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\matrix_as_transform_2d.py
#!/usr/bin/env python
...
class ShowMultiplication(NumberLineScene):
    ...
    def copy_original_line(self):
        ...
        self.play(
            ApplyFunction(lambda m: m.shift(DOWN).set_color("lightgreen"), copied_line),
...
class ExamplesOfNonlinearOneDimensionalTransforms(NumberLineScene):
    ...
    def run_function(self, function, explanation):
        ...
        self.play(
            ...
            *[
                ApplyMethod(mob.shift, function(mob.get_center()) - mob.get_center())
...
class TransformScene2D(Scene):
    ...
    def add_x_y_arrows(self):
        ...
        self.number_plane.filter_out(
            lambda x_y_z1: (0 < x_y_z1[1])
            and (x_y_z1[1] < 1)
            and (abs(x_y_z1[0]) < 0.1)
        )
        return self
...
class ShowMatrixTransform(TransformScene2D):
    ...
    def construct(self, matrix, with_background, show_matrix):
        ...
        def func(mobject):
            mobject.get_points()[:, :2] = np.dot(
                mobject.get_points()[:, :2], np.transpose(matrix)
            )
            return mobject
    ...
    def apply_function(self, function, explanation):
        ...
        self.play(
            ...
            *[
            ...
            + [Animation(self.blackness), Animation(words), Animation(self.nonlinear)],
            run_time=2.0,
        )
        self.wait(3)
...
class ShowMatrixTransformWithDot(TransformScene2D):
    ...
    def construct(self, matrix, with_background, show_matrix):
        ...
        anims = [
            ...
            ApplyMethod(
                dot.shift, func(deepcopy(dot)).get_center() - dot.get_center(), **kwargs
            ),
        ]
        if hasattr(self, "x_arrow") and hasattr(self, "y_arrow"):
            for arrow, index in (self.x_arrow, 0), (self.y_arrow, 1):
        ...
        x_arrow_copy = deepcopy(self.x_arrow)
        y_arrow_copy = Arrow(
            LEFT + 2 * UP, 5 * RIGHT + 2 * UP, color=LIGHT_RED, **ARROW_CONFIG
        )
        self.play(ApplyMethod(x_arrow_copy.rotate, np.pi))
        self.play(ShowCreation(y_arrow_copy))
        self.wait(3)
