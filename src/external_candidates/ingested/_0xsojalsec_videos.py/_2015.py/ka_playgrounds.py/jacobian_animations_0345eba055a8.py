# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\ka_playgrounds\jacobian_animations.py
#!/usr/bin/env python
...
class ExampleMultivariableFunctionWithZoom(ZoomedScene, ExampleMultivariableFunction):
    def construct(self):
        ...
        self.apply_nonlinear_transformation(
            ...
            added_anims=[
                ApplyMethod(
                    self.little_rectangle.move_to, example_function(point), run_time=3
                )
            ],
        )
        self.wait()
    ...
    def capture_mobjects_in_camera(self, mobjects, **kwargs):
        ...
        if self.zoom_activated:
            filter_mobjects = [
                m for m in mobjects if m not in self.background_plane.get_family()
            ]
            self.zoomed_camera.capture_mobjects(filter_mobjects, **kwargs)


class ExampleMultivariableFunctionWithMuchZoom(ExampleMultivariableFunctionWithZoom):
    CONFIG = {"zoom_factor": 20}
