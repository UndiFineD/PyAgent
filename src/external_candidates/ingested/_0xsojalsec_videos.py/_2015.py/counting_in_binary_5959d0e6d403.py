...
def read_reversed_binary(string):
    return sum(
        [2**count if char == "1" else 0 for count, char in zip(it.count(), string)]
    )
...
class CountTo1023(Scene):
    def construct(self):
        ...
        self.frames = [
            disp.paint_mobject(
                Mobject(rh_map[count % 32], lh_map[count // 32], get_num(count))
            )
...
class ShowReadingRule(Scene):
    ...
    def read_sample(self, num):
        ...
        if num in [6, 17]:
            hand.shift(0.8 * LEFT)
        sum_mobs = (
            OldTex(" + ".join([str(2**c) for c in counts]).split(" ") + ["=%d" % num])
            .to_corner(UP + RIGHT)
            .split()
        )
    ...
    def construct(self):
        ...
        for phrase, (start, end, pause) in zip(phrases, ranges):
            ...
            self.frames += [
                ...
                for frame in oh.frames[
                    COUNT_TO_FRAME_NUM[start] : COUNT_TO_FRAME_NUM[end]
                ]
            ]
            if pause:
...
class MindFindsShortcuts(Scene):
    def construct(self):
        ...
        self.clear()
        hands = dict(
            [(num, Hand(num).center().scale(0.5).shift(DOWN)) for num in [23, 16, 7]]
        )
...
class CountingExampleSentence(ShowCounting):
    def construct(self):
        ...
        self.words = (
            OldTexText(words.split(), size="\\Huge")
            .scale(0.7)
...
class FinishCountingExampleSentence(Scene):
    def construct(self):
        words = "As an example, this is me counting the number of words in this sentence on just one hand!"
        words = (
            OldTexText(words, size="\\Huge").scale(0.7).to_corner(UP + LEFT, buff=0.25)
        )
        hand = Hand(18)
...
class Question(Scene):
    def construct(self):
        self.add(
            OldTexText("Left to ponder: Why does this rule for incrementing work?")
        )
...
class TwoHandStatement(Scene):
    def construct(self):
        self.add(
            OldTexText("With 10 fingers, you can count up to $2^{10} - 1 = 1023$...")
        )
