...
class TauPoem(Scene):
    ...
    def add_line_and_number(self):
        ...
        first_word, rest_of_line = (
            OldTexText([line_str[:index], line_str[index:]])
            .to_edge(UP)
            .shift(BUFF * DOWN)
            .split()
        )
    ...
    def first_word_to_last_digit(self):
        ...
        self.play_over_time_range(
            ...
            4,
            Transform(
                self.number_word, self.new_digit, rate_func=squish_rate_func(smooth)
            ),
        )
    ...
    def line1(self):
        ...
        self.play(
            CounterclockwiseTransform(
                two_pi, sphere, rate_func=lambda t: 2 * smooth(t / 2)
            )
        )
        self.remove(two_pi)
        self.play(
            CounterclockwiseTransform(
                sphere, tau, rate_func=lambda t: 2 * (smooth(t / 2 + 0.5) - 0.5)
            )
        )
        self.remove(sphere)
        self.add(tau)
    ...
    def line8(self):
        ...
        self.play(
            ...
            Transform(
                pi.right_leg, Point(pi.right_leg.get_points()[0, :]).set_color("black")
            ),
    ...
    def line13(self):
        formula = form_start, two_pi, form_end = (
            OldTex(["\\hat{f^{(n)}}(\\xi) = (", "2\\pi", "i\\xi)^n \\hat{f}(\\xi)"])
            .shift(DOWN)
            .split()
        )
