...
class HardProblemsSimplerQuestions(Scene):
    def __init__(self, *args, **kwargs):
        ...
        circle_grid = Mobject(
            Circle(), Grid(radius=2), OldTex(r"\mathds{R}^2").shift((2, -2, 0))
        )
        ...
        self.play(
            ...
            Transform(
                Point((-FRAME_X_RADIUS, 0, 0)), Mobject(*CircleScene(RADIANS).mobjects)
            ),
        )
...
class CountLines(CircleScene):
    def __init__(self, radians, *args, **kwargs):
        ...
        formula, answer = OldTex(
            [r"{%d \choose 2} = \frac{%d(%d - 1)}{2} = " % (n, n, n), str(choose(n, 2))]
        )
...
class LineCorrespondsWithPair(CircleScene):
    ...
    def __init__(self, radians, dot0_index, dot1_index, *args, **kwargs):
        ...
        self.play(
            *[
                Transform(
                    dot, Dot(dot.get_center(), 3 * dot.radius), rate_func=there_and_back
                )
    ...
    def __init__(self, radians, indices, *args, **kwargs):
        ...
        self.play(
            *[
                Transform(
                    Line(
                        self.get_points()[indices[p0]], self.get_points()[indices[p1]]
                    ),
...
class GraphsAndEulersFormulaJoke(Scene):
    def __init__(self, *args, **kwargs):
        ...
        graph = ParametricCurve(
            lambda t: (10 * t, ((10 * t) ** 3 - 10 * t), 0), expected_measure=40.0
        )
        ...
        self.play(
            CounterclockwiseTransform(
                Mobject(axes, graph), Point((-FRAME_X_RADIUS, FRAME_Y_RADIUS, 0))
            )
        )
        self.play(
            CounterclockwiseTransform(
...
class ApplyEulerToMoser(CircleScene):
    def __init__(self, *args, **kwargs):
        ...
        V[1], minus[1], E[1], plus[1], F[1], equals[1], two[1] = OldTex(
            ["V", "-", "E", "+", "F", "=", "2"]
        ).split()
        F[2], equals[2], E[2], minus[2], V[2], plus[2], two[2] = OldTex(
            ["F", "=", "E", "-", "V", "+", "2"]
        ).split()
        ...
        self.play(
            Transform(
                E[3], Mobject(nc2[4], plus1[4], two1[4], nc41[4], plus2[4], n1[4])
            ),
            *[
                Transform(d[3], d[4])
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in [0, 1]:
            ...
            self.play(
                *[
                    CounterclockwiseTransform(
                        deepcopy(mob_dicts[i][n][k]), mob_dicts[1 - i][n][k]
                    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        for n in range(self.nrows):
            ...
            desired_center = np.array(
                (self.diagram_width / 2.0, self.coords_to_mobs[n][0].get_center()[1], 0)
            )
        ...
        for n in range(self.nrows):
            self.play(
                CounterclockwiseTransform(
                    powers_of_two[n], powers_of_two_symbols[n], run_time=run_time
                )
            )
...
class MoserSolutionInPascal(PascalsTriangleScene):
    ...
    def __init__(self, nrows, n, *args, **kwargs):
        ...
        self.play(
            *[
                CounterclockwiseTransform(
                    self.coords_to_n_choose_k[n0][k0], self.coords_to_mobs[n0][k0]
                )
                for n0, k0 in self.coords
...
class RotatingPolyhedra(Scene):
    ...
    def __init__(self, polyhedra_classes, *args, **kwargs):
        ...
        for mob in polyhedra:
            self.play(
                TransformAnimations(
                    Rotating(curr_mob, **rot_kwargs), Rotating(mob, **rot_kwargs)
                )
            )
            for m in polyhedra:
...
class ExplainNChoose2Formula(Scene):
    ...
    def __init__(self, n, a, b, *args, **kwargs):
        ...
        self.play(
            *[
                CounterclockwiseTransform(
                    mob, Point(mob.get_center()).set_color("black")
                )
                for mob in nums
            ]
        )
