...
def interesting_problems():
    ...
    fermat_copies, face_copies, tb_copies, sb_copies = (
        Mobject(*[deepcopy(mob).scale(0.5).shift(locale) for locale in locales])
...
def response_invitation():
    ...
    comments = Mobject(
        *[Line((-1.2, y, 0), (1.2, y, 0), color="white") for y in [-1.5, -1.75, -2]]
    )
...
def different_points(radians1, radians2):
    ...
    sc.add(circle)
    points1, points2 = (
        [(RADIUS * np.cos(angle), RADIUS * np.sin(angle), 0) for angle in radians]
        for radians in (radians1, radians2)
    )
    dots1, dots2 = (
        Mobject(*[Dot(point) for point in points]) for points in (points1, points2)
    )
    lines1, lines2 = (
