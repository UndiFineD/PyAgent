...
class Triangle(Polygon):
    def __init__(self, **kwargs):
        ...
        Polygon.__init__(
            self, *POINTS[[0, 1, 2]], edge_colors=[B_COLOR, C_COLOR, A_COLOR], **kwargs
        )
        nudge = 0.2
    ...
    def construct(self, *args):
        ...
        for triplet in triplets:
            self.set_color_region(
                region_from_polygon_vertices(*triplet), color="BLUE_D"
            )
...
class ZoomInOnTroublePoint(Scene):
    ...
    def construct(self, with_labels, rotate):
        ...
        angle1_arc.filter_out(
            lambda x_y_z2: not (
                x_y_z2[0] > 0 and x_y_z2[1] > 0 and x_y_z2[1] < x_y_z2[0] / 3
            )
        )
        angle2_arc.filter_out(
            lambda x_y_z3: not (
                x_y_z3[0] < 0 and x_y_z3[1] > 0 and x_y_z3[1] < -3 * x_y_z3[0]
            )
        )
...
class DrawTriangleWithAngles(Scene):
    def construct(self):
        ...
        angle1_arc = (
            ...
            .filter_out(
                lambda x_y_z: not (
                    x_y_z[0] > 0 and x_y_z[1] < 0 and x_y_z[1] < -3 * x_y_z[0]
                )
            )
            .shift(vertices[1])
        )
        angle2_arc = (
            ...
            .filter_out(
                lambda x_y_z1: not (
                    x_y_z1[0] < 0 and x_y_z1[1] > 0 and x_y_z1[1] < -3 * x_y_z1[0]
                )
            )
            .shift(vertices[2])
        )
        ...
        for triplet in [[0, 1, 7], [2, 3, 1], [4, 5, 3], [6, 7, 5]]:
            ...
            self.set_color_region(
                region_from_line_boundary(
                    *[[points[i], points[j]] for i, j in zip(triplet, triplet[1:])]
                ),
                color=BLUE_D,
            )
