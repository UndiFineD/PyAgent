...
def zero_to_one_interval():
    interval = NumberLine(
        radius=INTERVAL_RADIUS, interval_size=2.0 * INTERVAL_RADIUS / NUM_INTERVAL_TICKS
    )
...
class IntervalScene(NumberLineScene):
    ...
    def show_all_fractions(
        self, num_fractions=27, pause_time=1.0, remove_as_you_go=True
    ):
    ...
    def cover_fractions(self, epsilon=0.3, num_fractions=10, run_time_per_interval=0.5):
        ...
        while True:
            ...
            if num_intervals >= num_fractions:
                break
            if (
                fraction < self.number_line.left_num
                or fraction > self.number_line.right_num
            ):
                continue
            num_intervals += 1
...
class ChallengeOne(Scene):
    def construct(self):
        ...
        self.bottom_vibration = Vibrate(
            num_periods=1, run_time=3.0, center=DOWN, color=start_color
        )
        top_vibrations = [
...
class QuestionAndAnswer(Scene):
    def construct(self):
        ...
        question = OldTexText("For what values of $r$ will the frequencies 220~Hz and \
        ...
        correction1 = OldTexText(
            "with sufficiently low denominator", size="\\small"
        ).scale(1.5)
...
class PlaySimpleRatio(Scene):
    ...
    def construct(self, fraction, color):
        ...
        string2 = Vibrate(
            num_periods=fraction, run_time=5.0, center=2 * UP, color=color
        )
...
class PatternInFrequencies(Scene):
    ...
    def construct(self, num1, num2, color):
        ...
        bottom_lines = Mobject(
            *[deepcopy(line_template).shift(k * RIGHT) for k in range(num_bot_lines)]
        )
...
class PianoTuning(Scene):
    ...
    def show_interval(self, interval, half_steps):
        ...
        semicircles = self.semicircles[
            half_notes_to_base : half_notes_to_base + half_steps
        ]
        product = OldTex(
...
class AllValuesBetween1And2(NumberLineScene):
    def construct(self):
        ...
        approximations = [
            continued_fraction(cont_frac[:k]) for k in range(1, len(cont_frac))[:8]
        ]
        ...
        for approx in approximations:
            ...
            self.play(
                Transform(top_arrow, new_arrow), Transform(frac_mob, mob), run_time=0.5
            )
            self.wait(0.5)
...
class ShowAllFractions(IntervalScene):
    def construct(self):
        ...
        self.show_all_fractions(
            num_fractions=100, remove_as_you_go=False, pause_time=0.3
        )
        self.wait()
        self.play(
            *[
...
class StepsToSolution(IntervalScene):
    ...
    def multiply_by_epsilon(self):
        self.play(
            *[
                CounterclockwiseTransform(one, OldTex("\\epsilon").replace(one))
    ...
    def stretch_intervals(self):
        for interval, count in zip(self.intervals, it.count(1)):
            self.play(
                ApplyMethod(interval.scale, 1.0 / (count**2)), run_time=1.0 / count
            )
        self.wait()
...
class ShiftSetupByOne(IntervalScene):
    def construct(self):
        ...
        self.wait()
        words = OldTexText(
            "Almost all the covered numbers are harmonious!", size="\\small"
        ).shift(2 * UP)
        self.play(ShimmerIn(words))
        self.wait()
        for num in [7, 5]:
        ...
        answer2 = OldTexText(
            "real numbers \\emph{very very very} close to them", size="\\small"
        )
