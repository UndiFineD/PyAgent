...
class NVOmniVideoInference:
    ...
    def _print_model_info(self):
        ...
        if self.config:
            ...
            logger.info(
                f"Hidden size: {getattr(self.config, 'hidden_size', 'Unknown')}"
            )
    ...
    def generate_response(
        ...
        if not self.model or not self.processor:
            logger.error(
                "Model or processor not loaded. Please initialize the model first."
            )
            return None
        ...
        if True:
            ...
            logger.info(
                f"Model config - load_audio_in_video: {self.model.config.load_audio_in_video}, num_video_frames: {self.model.config.num_video_frames}, audio_chunk_length: {self.model.config.audio_chunk_length}"
            )
    ...
    def batch_generate(
        self, video_text_pairs: List[tuple], **generation_kwargs
    ) -> List[Optional[str]]:
        """
        ...
        for i, (video_path, text_prompt) in enumerate(video_text_pairs):
            ...
            response = self.generate_response(
                video_path, text_prompt, **generation_kwargs
            )
