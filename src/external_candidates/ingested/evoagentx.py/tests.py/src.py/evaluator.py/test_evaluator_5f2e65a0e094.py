# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\tests\src\evaluator\test_evaluator.py
import unittest
...
class TestEvaluator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.evaluator = Evaluator(
            llm=self.llm, num_workers=1, agent_manager=self.agent_manager
        )
    ...
    def test_single_thread_evaluation_workflow_graph(self, mock_execute):
        ...
        results = self.evaluator.evaluate(
            graph=self.workflow_graph, benchmark=self.benchmark, eval_mode="test"
        )
    ...
    def test_single_thread_evaluation_action_graph(self):
        ...
        results = self.evaluator.evaluate(
            graph=self.action_graph, benchmark=self.benchmark, eval_mode="test"
        )
    ...
    def test_evaluation_with_output_postprocess(self):
        ...
        evaluator = Evaluator(
            llm=self.llm, num_workers=1, output_postprocess_func=postprocess_func
        )
    ...
    def test_invalid_eval_mode(self):
        with self.assertRaises(AssertionError):
            self.evaluator.evaluate(
                graph=self.action_graph, benchmark=self.benchmark, eval_mode="invalid"
            )
