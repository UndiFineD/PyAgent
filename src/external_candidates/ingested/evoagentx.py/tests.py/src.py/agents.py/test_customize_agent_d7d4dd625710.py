...
class TestModule(unittest.TestCase):
    ...
    def test_simple_agent(self, mock_generate):
        ...
        self.assertEqual(
            simple_agent.prompt, "You are a simple agent that prints hello world."
        )
        ...
        new_agent: CustomizeAgent = CustomizeAgent.from_file(
            self.save_files[0], llm_config=llm_config
        )
    ...
    def test_agent_with_inputs_and_outputs(self, mock_generate):
        ...
        new_agent_with_inputs: CustomizeAgent = CustomizeAgent.from_file(
            self.save_files[1], llm_config=llm_config
        )
        self.assertEqual(len(new_agent_with_inputs.action.inputs_format.get_attrs()), 1)
        self.assertEqual(
            len(new_agent_with_inputs.action.outputs_format.get_attrs()), 0
        )
        ...
        new_agent_with_outputs: CustomizeAgent = CustomizeAgent.from_file(
            self.save_files[2], llm_config=llm_config
        )
        self.assertEqual(
            len(new_agent_with_outputs.action.inputs_format.get_attrs()), 0
        )
        self.assertEqual(
            len(new_agent_with_outputs.action.outputs_format.get_attrs()), 1
        )
        self.assertEqual(
            new_agent_with_outputs.parse_func.__name__, "customize_parse_func"
        )
        ...
        self.assertEqual(
            len(agent_with_inputs_outputs.action.inputs_format.get_attrs()), 1
        )
        self.assertEqual(
            len(agent_with_inputs_outputs.action.outputs_format.get_attrs()), 1
        )
        ...
        self.assertEqual(
            len(new_agent_with_inputs_outputs.action.inputs_format.get_attrs()), 1
        )
        self.assertEqual(
            len(new_agent_with_inputs_outputs.action.outputs_format.get_attrs()), 1
        )
        ...
        self.assertEqual(
            agent_with_parser.action.outputs_format.__name__, "CodeWriterActionOutput"
        )
        agent_with_parser.save_module(self.save_files[4])
        new_agent_with_parser: CustomizeAgent = CustomizeAgent.from_file(
            self.save_files[4], llm_config=llm_config
        )
        self.assertEqual(
