# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\tests\src\rag\test_rag.py
import json
import os
import tempfile
import unittest
...
from evoagentx.rag.schema import ChunkMetadata, Corpus, Query, RagResult
from evoagentx.storages.base import IndexStore, StorageHandler
...
class MockOpenAIEmbedding(BaseEmbedding):
    ...
    def __init__(
        self, model_name: str = "text-embedding-ada-002", dimensions: int = 1536
    ):
    ...
    def get_embedding_model(self) -> BaseEmbedding:
        ...
        if self._embedding_model is None:
            self._embedding_model = MockOpenAIEmbedding(
                model_name=self.model_name, dimensions=self._dimensions
            )
...
class TestSearchEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.store_config = StoreConfig(
            dbConfig=DBConfig(
                db_name="sqlite", path=os.path.join(self.temp_dir, "test_hotpotQA.sql")
            ),
        ...
        self.search_engine = RAGEngine(
            config=self.rag_config, storage_handler=self.storage_handler
        )

        # Prepare HotpotQA corpus
        self.corpus_id = HOTPOTQA_EXAMPLE["_id"]
        self.context_files = []
        self.supporting_titles = {
            fact[0] for fact in HOTPOTQA_EXAMPLE["supporting_facts"]
    ...
    def test_read(self):
        ...
        for chunk in corpus.chunks:
            self.assertIsInstance(
                chunk.metadata, ChunkMetadata, "Chunk should have metadata"
            )
            self.assertIn(
                "file_name",
                chunk.metadata.model_dump(),
                "Metadata should include file_name",
            )
    ...
    def test_add(self):
        ...
        self.search_engine.add(
            index_type=IndexType.VECTOR, nodes=corpus, corpus_id=self.corpus_id
        )
        self.assertIn(
            self.corpus_id, self.search_engine.indices, "Corpus should be indexed"
        )
        self.assertIn(
            IndexType.VECTOR,
            self.search_engine.indices[self.corpus_id],
        ...
        for node_id, node in index.id_to_node.items():
            self.assertEqual(
                node.metadata["corpus_id"],
                self.corpus_id,
                "Node metadata should include corpus_id",
            )
            self.assertEqual(
                node.metadata["index_type"],
                IndexType.VECTOR,
                "Node metadata should include index_type",
            )
        logger.info(
            f"Added {len(corpus.chunks)} nodes to vector index for corpus {self.corpus_id}"
        )
    ...
    def test_query(self):
        ...
        self.assertIsInstance(
            result, RagResult, "query should return a RagResult object"
        )
        self.assertLessEqual(
            len(result.corpus.chunks), 10, "Should return at most top_k chunks"
        )
        self.assertEqual(
            len(result.scores), len(result.corpus.chunks), "Scores should match chunks"
        )
        ...
        recall = len(retrieved_titles.intersection(self.supporting_titles)) / len(
            self.supporting_titles
        )
        ...
        logger.info(
            f"Query retrieved {len(result.corpus.chunks)} chunks with recall@10={recall}"
        )
    ...
    def test_delete_by_node_ids(self):
        ...
        self.search_engine.delete(
            corpus_id=self.corpus_id, index_type=IndexType.VECTOR, node_ids=node_ids
        )
        remaining_node_count = len(index.id_to_node)
        self.assertEqual(
            remaining_node_count,
            initial_node_count - len(node_ids),
            "Nodes should be deleted",
        )
        for node_id in node_ids:
            self.assertNotIn(
                node_id, index.id_to_node, f"Node {node_id} should be deleted"
            )
    ...
    def test_delete_by_metadata(self):
        ...
        index = self.search_engine.indices[self.corpus_id][IndexType.VECTOR]
        metadata_filters = {
            "file_name": str(self.context_files[0])
        }  # Delete nodes from first file
        initial_node_count = len(index.id_to_node)
        self.search_engine.delete(
            corpus_id=self.corpus_id,
        ...
        self.assertEqual(
            len(index.id_to_node),
            len(remaining_nodes),
            "Nodes matching metadata should be deleted",
        )
        logger.info(
            f"Deleted nodes with metadata {metadata_filters} from corpus {self.corpus_id}"
        )
    ...
    def test_clear(self):
        ...
        self.assertNotIn(
            self.corpus_id, self.search_engine.indices, "Corpus should be cleared"
        )
        self.assertNotIn(
            self.corpus_id,
            self.search_engine.retrievers,
            "Retrievers should be cleared",
        )
    ...
    def test_save_to_files(self):
        ...
        with open(metadata_files[0], "r", encoding="utf-8") as f:
            ...
            self.assertEqual(
                metadata["corpus_id"],
                self.corpus_id,
                "Metadata should include corpus_id",
            )
    ...
    def test_load_from_files(self):
        ...
        self.assertIn(
            self.corpus_id, self.search_engine.indices, "Corpus should be loaded"
        )
        index = self.search_engine.indices[self.corpus_id][IndexType.VECTOR]
        self.assertGreater(len(index.id_to_node), 0, "Index should contain nodes")
        query = Query(query_str=self.query_text, top_k=10)
    ...
    def test_save_to_database(self):
        ...
        self.search_engine.save(
            corpus_id=self.corpus_id, index_type=IndexType.VECTOR, table="indexing"
        )
        ...
        for record in records:
            ...
            self.assertEqual(
                parsed["corpus_id"], self.corpus_id, "Record should match corpus_id"
            )
    ...
    def test_edge_case_empty_corpus(self):
        ...
        result = self.search_engine.query(
            query=self.query_text, corpus_id="nonexistent"
        )
        self.assertEqual(
            len(result.corpus.chunks),
            0,
            "Query on nonexistent corpus should return empty result",
        )
        self.search_engine.delete(corpus_id="nonexistent")
        self.assertNotIn(
            "nonexistent",
            self.search_engine.indices,
            "Delete on nonexistent corpus should not fail",
        )
        self.search_engine.clear(corpus_id="nonexistent")
        self.assertNotIn(
            "nonexistent",
            self.search_engine.indices,
            "Clear on nonexistent corpus should not fail",
        )
