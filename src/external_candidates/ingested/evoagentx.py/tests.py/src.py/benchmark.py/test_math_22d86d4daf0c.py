# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\tests\src\benchmark\test_math.py
import os
import shutil
import tempfile
import unittest
...
class TestMath(unittest.TestCase):
    ...
    def create_test_files(self):
        ...
        train_file = os.path.join(
            self.temp_dir, "MATH", "train", "Counting & Probability", "sample1.json"
        )
        os.makedirs(os.path.dirname(train_file), exist_ok=True)
        save_json(self.sample_data1, train_file, type="json")

        test_file = os.path.join(
            self.temp_dir, "MATH", "test", "Counting & Probability", "sample1.json"
        )
    ...
    def test_extract_answer(self):
        ...
        self.assertEqual(
            benchmark.extract_answer(example["solution"]), "\\frac{625}{648}"
        )
    ...
    def test_evaluate(self):
        ...
        # test evaluate
        self.assertEqual(
            benchmark.evaluate(example["solution"], "\\frac{625}{648}"),
            {"solve_rate": 1.0},
        )
        self.assertEqual(
            benchmark.evaluate(example["solution"], "\\frac{625}{649}"),
            {"solve_rate": 0.0},
        )
