# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\tests\src\workflow\test_action_graph.py
import os
import unittest
...
class TestModule(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.qa_action_graph = QAActionGraph(
    ...
    async def test_async_execute(self, mock_sc_ensemble, mock_answer_generate):
        ...
        result = await self.qa_action_graph.async_execute(
            problem="This is a test async problem."
        )
    ...
    def test_get_graph_info(self):
        ...
        self.assertEqual(
            ...
            "This workflow aims to address multi-hop QA tasks.",
        )
        self.assertEqual(len(graph_info["operators"]), 2)
        self.assertEqual(
            graph_info["operators"]["answer_generate"]["name"], "AnswerGenerate"
        )
    ...
    def test_save_and_load(self):
        self.qa_action_graph.save_module(
            "tests/src/workflow/saved_qa_action_graph.json"
        )
        loaded_graph = ActionGraph.from_file(
            "tests/src/workflow/saved_qa_action_graph.json", llm_config=self.llm_config
        )
        self.assertEqual(loaded_graph.name, "QAActionGraph")
        self.assertEqual(
            loaded_graph.description,
