# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\tests\src\workflow\test_workflow_graph.py
import unittest
...
class TestWorkFlowGraph(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.task2 = WorkFlowNode(
            ...
            inputs=[
                Parameter(
                    name="output1", type="string", description="Output from Task1"
                )
            ],
        ...
        self.task3 = WorkFlowNode(
            ...
            inputs=[
                Parameter(
                    name="output2", type="string", description="Output from Task2"
                )
            ],
            outputs=[
                Parameter(
                    name="final_output", type="string", description="Final output"
                )
            ],
            agents=["TestAgent"],
            status=WorkFlowNodeState.PENDING,
        ...
        self.task4 = WorkFlowNode(
            ...
            inputs=[
                ...
                Parameter(
                    name="final_output", type="string", description="Output from Task3"
                ),
            ],
            outputs=[
                Parameter(name="result", type="string", description="Final result")
            ],
            agents=["TestAgent"],
            status=WorkFlowNodeState.PENDING,
        )
    ...
    def test_node_status_management(self):
        ...
        self.assertEqual(
            WorkFlowNodeState.PENDING, self.linear_graph.get_node_status("Task1")
        )

        # Set status to RUNNING
        self.linear_graph.set_node_status("Task1", WorkFlowNodeState.RUNNING)
        self.assertEqual(
            WorkFlowNodeState.RUNNING, self.linear_graph.get_node_status("Task1")
        )
        ...
        self.linear_graph.set_node_status("Task1", WorkFlowNodeState.COMPLETED)
        self.assertEqual(
            WorkFlowNodeState.COMPLETED, self.linear_graph.get_node_status("Task1")
        )
        ...
        self.assertEqual(
            WorkFlowNodeState.FAILED, self.linear_graph.get_node_status("Task1")
        )
