# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\tests\src\workflow\test_workflow_manager.py
import unittest
...
from evoagentx.workflow.workflow_manager import (
    NextAction,
    TaskSchedulerOutput,
    WorkFlowManager,
)
...
class TestWorkFlowManager(unittest.TestCase):
    ...
    def create_test_workflow(self):
        ...
        self.workflow = WorkFlowGraph(
            goal="Test Workflow", nodes=[task1, task2, task3], edges=[edge1, edge2]
        )
    ...
    async def test_sync_task_scheduling_with_single_task(
        self, mock_task_scheduler_execute
    ):
        ...
        task = await self.workflow_manager.schedule_next_task(
            graph=self.workflow, env=self.env
        )
    ...
    async def test_action_scheduling(self, mock_action_scheduler_execute):
        ...
        action = await self.workflow_manager.schedule_next_action(
            goal="Test Goal", task=task, agent_manager=mock_agent_manager, env=self.env
        )
