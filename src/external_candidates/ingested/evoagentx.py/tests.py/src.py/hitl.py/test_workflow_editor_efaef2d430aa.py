...
class TestWorkFlowEditor(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        os.environ["PYTEST_CURRENT_TEST"] = (
            "test_workflow_editor.py::TestWorkFlowEditor"
        )
        ...
        if self.expected_optimized_workflow["nodes"]:
            ...
            self.expected_optimized_workflow["nodes"] = (
                self.expected_optimized_workflow["nodes"][:-1]
            )
            ...
            self.expected_optimized_workflow["edges"] = [
                ...
                if edge["target"] != last_node["name"]
                and edge["source"] != last_node["name"]
            ]
    ...
    def test_workflow_editor_instantiation(self):
        ...
        editor_custom = WorkFlowEditor(
            save_dir=self.temp_dir, llm=custom_mock_llm, max_retries=5
        )
    ...
    async def test_edit_workflow_without_new_file_path(
        self, mock_workflow_graph, mock_workflow
    ):
        ...
        result = await editor.edit_workflow(
            file_path=self.test_workflow_file, instruction=self.test_instruction
        )
    ...
    @patch("evoagentx.workflow.workflow.WorkFlow")
    @patch("evoagentx.workflow.workflow_graph.WorkFlowGraph")
    async def test_edit_workflow_with_new_file_path(
        self, mock_workflow_graph, mock_workflow
    ):
    ...
    @patch("evoagentx.workflow.workflow.WorkFlow")
    @patch("evoagentx.workflow.workflow_graph.WorkFlowGraph")
    async def test_edit_workflow_invalid_json_structure(
        self, mock_workflow_graph, mock_workflow
    ):
...
class TestWorkFlowEditorIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        # Set environment variables to ensure testing mode
        os.environ["PYTEST_CURRENT_TEST"] = (
            "test_workflow_editor.py::TestWorkFlowEditorIntegration"
        )
    ...
    @patch("evoagentx.workflow.workflow.WorkFlow")
    @patch("evoagentx.workflow.workflow_graph.WorkFlowGraph")
    async def test_full_workflow_editing_process(
        self, mock_workflow_graph, mock_workflow
    ):
