...
class MessageBenchmark(Benchmark):
    ...
    def _get_label(self, example):
        ...
        return [m["content"] for m in example["messages"] if m["role"] == "assistant"][
            -1
        ]
...
def f1_graph(prediction: str, label: str) -> float:
    ...
    def build_graph_structure(nodes, edges):
        ...
        def get_topological_features():
            ...
            features = {
                ...
                "avg_branching": (
                    sum(len(graph[node]) for node in node_ids) / len(node_ids)
                    if node_ids
                    else 0
                ),
                "parallel_paths": sum(1 for node in node_ids if len(graph[node]) > 1),
                "sequential_paths": sum(
                    1 for node in node_ids if len(graph[node]) == 1
                ),
            }
    ...
    def structural_similarity(pred_nodes, pred_edges, label_nodes, label_edges):
        ...
        try:
            ...
            avg_feature_similarity = (
                feature_similarity / total_features if total_features > 0 else 0.0
            )
    ...
    structural_sim = structural_similarity(
        pred_nodes, pred_edges, label_nodes, label_edges
    )
        ...
        for idx, gold_item in enumerate(gold_50):
            ...
            if not item:
                ...
                try:
                    ...
                    nodes, edges = [], []
            pred_records.append(
                {"id": gold_item["id"], "workflow": {"nodes": nodes, "edges": edges}}
            )
