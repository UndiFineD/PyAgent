...
class CustomizeAction(Action):
    ...
    tools: Optional[List[Toolkit]] = Field(
        default=None, description="The tools that the action can use"
    )
    conversation: Optional[Message] = Field(
        default=None, description="Current conversation state"
    )

    max_tool_try: int = Field(
        default=2, description="Maximum number of tool calling attempts allowed"
    )
    ...
    def __init__(self, **kwargs):
        ...
        description = kwargs.pop(
            "description", "Customized action that can use tools to accomplish its task"
        )
        ...
        if not self.prompt and not self.prompt_template:
            raise ValueError(
                "`prompt` or `prompt_template` is required when creating CustomizeAction action"
            )
        ...
        **kwargs,
        ...
        for param in prompt_params_names:
            ...
            if isinstance(value, str):
                ...
                raise TypeError(
                    f"The input type {type(value)} is invalid! Valid types: [str, dict, list]."
                )

        if self.prompt:
            prompt = (
                self.prompt.format(**prompt_params_values)
                if prompt_params_values
                else self.prompt
            )
            if self.tools:
                tools_schemas = [
                    j["function"]
                    for i in [tool.get_tool_schemas() for tool in self.tools]
                    for j in i
                ]
                prompt += "\n\n" + TOOL_CALLING_TEMPLATE.format(
                    tools_description=tools_schemas
                )
            return prompt
        else:
    ...
    def prepare_extraction_prompt(self, llm_output_content: str) -> str:
        ...
        return OUTPUT_EXTRACTION_PROMPT.format(
            text=llm_output_content, output_description=output_description
        )
            ...
            try:
                ...
                for tool_caller in tool_callers:
                    ...
                    if not tool_caller_name or not callable(tool_caller):
                        logger.warning(
                            f"Invalid tool function in '{toolkit.name}': missing name or not callable."
                        )
                        continue
                ...
                if valid_tools_count == 0:
                    logger.info(
                        f"No valid tools found in toolkit '{toolkit.name}'. Skipping."
                    )
                    continue

                if valid_tools_count > 0 and all(
                    name in self.tools_caller for name in valid_tools_names
                ):
                    logger.info(
                        f"All tools from toolkit '{toolkit.name}' are already added. Skipping."
                    )
                    continue
    ...
    def _extract_tool_calls(
        self, llm_output: str, llm: Optional[BaseLLM] = None
    ) -> List[dict]:
            ...
            try:
                ...
                if llm is not None:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Retrying tool call parse with fixed output:\n{fixed_output}"
                        )
    ...
    def _extract_output(self, llm_output: Any, llm: BaseLLM = None, **kwargs):
        ...
        try:
            ...
            llm_extracted_output: LLMOutputParser = llm.generate(
                prompt=extraction_prompt
            )
            llm_extracted_data: dict = parse_json_from_llm_output(
                llm_extracted_output.content
            )
    ...
    async def _async_extract_output(
        self, llm_output: Any, llm: BaseLLM = None, **kwargs
    ):
    ...
    def _call_single_tool(self, function_param: dict) -> tuple:
        try:
            ...
            print(
                f"Executing function calling: {function_name} with parameters: {function_args}"
            )
    ...
    def _calling_tools(self, tool_call_args: List[dict]) -> dict:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_tool = {
                executor.submit(self._call_single_tool, param): param
                for param in tool_call_args
            }
        ...
        try:
            ...
            if inspect.iscoroutinefunction(callable_fn):
                ...
                result = await loop.run_in_executor(
                    None, lambda: callable_fn(**function_args)
                )
    ...
    async def _async_calling_tools(self, tool_call_args: List[dict]) -> dict:
        ...
        results = [
            res for res, err in results_with_errors if err is None and res is not None
        ]
    ...
    def execute(
        ...
        **kwargs,
        ...
        if not inputs and input_attributes:
            logger.error(
                "CustomizeAction action received invalid `inputs`: None or empty."
            )
        ...
        if self.prompt_template:
            ...
            if isinstance(self.prompt_template, ChatTemplate):
                ...
                conversation = self.prepare_action_prompt(
                    inputs=inputs, system_prompt=sys_msg
                )
            elif isinstance(self.prompt_template, StringTemplate):
                conversation = [
                    {
                        "role": "system",
                        "content": self.prepare_action_prompt(
                            inputs=inputs, system_prompt=sys_msg
                        ),
                    }
                ]
