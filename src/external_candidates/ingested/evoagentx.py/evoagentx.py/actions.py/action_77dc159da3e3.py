...
class Action(BaseModule):
    ...
    inputs_format: Optional[Type[ActionInput]] = (
        None  # specify the input format of the action
    )
    outputs_format: Optional[Type[Parser]] = (
        None  # specify the possible structured output format
    )
    ...
    def validate_data(cls, data: Any) -> Any:
        if (
            "inputs_format" in data
            and data["inputs_format"]
            and isinstance(data["inputs_format"], str)
        ):
            # only used when loading from a file
            data["inputs_format"] = MODULE_REGISTRY.get_module(data["inputs_format"])
        if (
            "outputs_format" in data
            and data["outputs_format"]
            and isinstance(data["outputs_format"], str)
        ):
            # only used when loading from a file
    ...
    def execute(
        ...
        **kwargs,
        ...
        raise NotImplementedError(
            f"`execute` function of {type(self).__name__} is not implemented!"
        )
    ...
    async def async_execute(
        ...
        **kwargs,
        ...
        """
        raise NotImplementedError(
            f"`async_execute` function of {type(self).__name__} is not implemented!"
        )
...
class ContextExtraction(Action):
    ...
    def __init__(self, **kwargs):
        ...
        description = (
            ...
            else CONTEXT_EXTRACTION["description"]
        )
