# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\actions\agent_generation.py
import re
...
class AgentGenerationInput(ActionInput):
    ...
    goal: str = Field(
        description="A detailed statement of the workflow's goal, explaining the objectives the entire workflow aims to achieve"
    )
    workflow: str = Field(
        description="An overview of the entire workflow, detailing all sub-tasks with their respective names, descriptions, inputs, and outputs"
    )
    task: str = Field(
        description="A detailed JSON representation of the sub-task requiring agent generation. It should include the task's name, description, inputs, and outputs."
    )

    history: Optional[str] = Field(
        default=None,
        description="Optional field containing previously selected or generated agents.",
    )
    suggestion: Optional[str] = Field(
        default=None, description="Optional suggestions to refine the generated agents."
    )
    existing_agents: Optional[str] = Field(
        default=None,
        description="Optional field containing the description of predefined agents, including each agent's name, role, and available actions.",
    )
    tools: Optional[str] = Field(
        default=None,
        description="Optional field containing the description of tools that agents can use, including each tool's name and functionality.",
    )
...
class GeneratedAgent(BaseModule):
    ...
    def validate_prompt(cls, agent: "GeneratedAgent"):
        ...
        if not all(prompt_has_inputs):
            missing_input_names = [
                name
                for name, has_input in zip(input_names, prompt_has_inputs)
                if not has_input
            ]
        ...
        def replace_with_braces(match):
            ...
            for name in input_names:
                instructions = re.sub(
                    rf"<input>{{*\b{re.escape(name)}\b}}*</input>",
                    rf"<input>{{{name}}}</input>",
                    instructions,
                )
        ...
        def fix_output_names(match):
            ...
            if len(generated_outputs) != len(outputs_names):
                raise ValueError(
                    f"The number of outputs in the prompt is different from that defined in the `outputs` field of the agent. The outputs in the prompt are: {generated_outputs}, while the outputs from the agent's `outputs` field are: {outputs_names}"
                )
            # check whether the generated output names are the same as agent outputs
            for generated_output in generated_outputs:
                if generated_output not in outputs_names:
                    most_similar_output_name = cls.find_output_name(
                        text=generated_output, outputs=outputs_names
                    )
                    output_format = output_format.replace(
                        generated_output, most_similar_output_name
                    )
                    logger.warning(
                        f"Couldn't find output name in prompt ('{generated_output}') in agent's outputs. Replace it with the most similar agent output: '{most_similar_output_name}'"
                    )
...
class AgentGenerationOutput(ActionOutput):
    ...
    generated_agents: List[GeneratedAgent] = Field(
        description="A list of generated agetns to address a sub-task"
    )
...
class AgentGeneration(Action):
    ...
    def __init__(self, **kwargs):
        name = (
            kwargs.pop("name") if "name" in kwargs else AGENT_GENERATION_ACTION["name"]
        )
        description = (
            kwargs.pop("description")
            if "description" in kwargs
            else AGENT_GENERATION_ACTION["description"]
        )
        prompt = (
            kwargs.pop("prompt")
            if "prompt" in kwargs
            else AGENT_GENERATION_ACTION["prompt"]
        )
    ...
    def execute(
        ...
        **kwargs,
        ...
        if not inputs:
            logger.error(
                "AgentGeneration action received invalid `inputs`: None or empty."
            )
        ...
        prompt_params_values = {
            param: inputs.get(param, "") for param in prompt_params_names
        }
        if self.tools:
            tool_description = [
                {
            ...
            prompt_params_values["tools"] = AGENT_GENERATION_TOOLS_PROMPT.format(
