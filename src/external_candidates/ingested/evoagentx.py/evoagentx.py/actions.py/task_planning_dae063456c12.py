# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\actions\task_planning.py
...
class TaskPlanningInput(ActionInput):
    ...
    goal: str = Field(
        description="A clear and detailed description of the user's goal, specifying what needs to be achieved."
    )
    history: Optional[str] = Field(
        default=None,
        description="Optional field containing previously generated task plan.",
    )
    suggestion: Optional[str] = Field(
        default=None,
        description="Optional suggestions or ideas to guide the planning process.",
    )
...
class TaskPlanningOutput(ActionOutput):
    ...
    sub_tasks: List[WorkFlowNode] = Field(
        description="A list of sub-tasks that collectively achieve user's goal."
    )
...
class TaskPlanning(Action):
    ...
    def __init__(self, **kwargs):
        ...
        description = (
            ...
            else TASK_PLANNING_ACTION["description"]
        )
        prompt = (
            kwargs.pop("prompt")
            if "prompt" in kwargs
            else TASK_PLANNING_ACTION["prompt"]
        )
    ...
    def execute(
        ...
        **kwargs,
        ...
        if not inputs:
            logger.error(
                "TaskPlanning action received invalid `inputs`: None or empty."
            )
