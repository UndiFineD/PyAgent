# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\actions\code_extraction.py
import os
...
class CodeExtractionInput(ActionInput):
    ...
    target_directory: str = Field(
        description="The directory path where extracted code files will be saved"
    )
    project_name: Optional[str] = Field(
        default=None, description="Optional name for the project folder"
    )
...
class CodeExtractionOutput(ActionOutput):
    ...
    extracted_files: Dict[str, str] = Field(
        description="Map of filename to file path of saved files"
    )
    main_file: Optional[str] = Field(
        default=None, description="Path to the main file if identified"
    )
...
class CodeExtraction(Action):
    ...
    def __init__(self, **kwargs):
        ...
        description = (
            ...
            else CODE_EXTRACTION["description"]
        )
        prompt = (
            kwargs.pop("prompt") if "prompt" in kwargs else CODE_EXTRACTION["prompt"]
        )
    ...
    def save_code_blocks(
        self, code_blocks: List[Dict], target_directory: str
    ) -> Dict[str, str]:
    ...
    def execute(
        ...
        **kwargs,
        ...
        if not inputs:
            error_msg = (
                "CodeExtraction action received invalid `inputs`: None or empty."
            )
            return CodeExtractionOutput(extracted_files={}, error=error_msg)
        ...
        try:
            ...
            system_message = (
                CODE_EXTRACTION["system_prompt"] if sys_msg is None else sys_msg
            )
            ...
            result = CodeExtractionOutput(
                extracted_files=saved_files, main_file=main_file
            )
