...
class Evaluator:
    ...
    def _get_eval_data(
        ...
        if eval_mode == "test":
            data = benchmark.get_test_data(
                indices=indices, sample_k=sample_k, seed=seed
            )
        elif eval_mode == "dev":
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if isinstance(graph, WorkFlowGraph) and update_agents:
            ...
            self.agent_manager.update_agents_from_workflow(
                workflow_graph=graph, llm_config=self.llm.config, **kwargs
            )
        ...
        results = self._evaluate_graph(
            graph=graph, data=data, benchmark=benchmark, verbose=verbose, **kwargs
        )
        return results
    ...
    def _execute_workflow_graph(
        ...
        **kwargs,
        ...
        workflow = WorkFlow(
            llm=self.llm, graph=graph_copy, agent_manager=self.agent_manager, **kwargs
        )
        ...
        **kwargs,
        ...
        try:
            ...
            if not isinstance(inputs, dict):
                raise ValueError(
                    f"The collate_func should return a dictionary. Got {type(inputs)}."
                )

            # execute the workflow or action graph
            if isinstance(graph, ActionGraph):
                output: dict = self._execute_action_graph(
                    graph=graph, inputs=inputs, **kwargs
                )
                ...
                raise ValueError(
                    f"Invalid workflow type: {type(graph)}. Must be WorkFlowGraph or ActionGraph."
                )
    ...
    def _calculate_average_score(self, scores: List[dict]) -> dict:
        ...
        return {
            k: sum(d[k] for d in scores if d is not None) / num_total_items
            for k in first_valid_score
        }
    ...
    def get_example_evaluation_record(
        self, benchmark: Benchmark, example: Any
    ) -> Optional[dict]:
        """
        ...
        **kwargs,
        ...
        async def process_with_semaphore(example):
            async with sem:
                try:
                    ...
                    logger.warning(
                        f"Async evaluation failed for example with semaphore: {str(e)}"
                    )
                    return None
        ...
        if verbose:
            results = await tqdm_asyncio.gather(
                *tasks, desc=f"Evaluating {benchmark.name}", total=len(data)
            )
        else:
    ...
    async def _async_evaluate_single_example(
        ...
        **kwargs,
        ...
        try:
            ...
            # execute the workflow or action graph
            if isinstance(graph, ActionGraph):
                output: dict = await self._async_execute_action_graph(
                    graph=graph, inputs=inputs, **kwargs
                )
            elif isinstance(graph, WorkFlowGraph):
            ...
            if hasattr(benchmark, "async_evaluate") and callable(
                getattr(benchmark, "async_evaluate")
            ):
    ...
    async def _async_execute_action_graph(
        self, graph: ActionGraph, inputs: dict, **kwargs
    ) -> dict:
        """
    ...
    async def _async_execute_workflow_graph(
        ...
        **kwargs,
        ...
        workflow = WorkFlow(
            llm=self.llm, graph=graph_copy, agent_manager=local_agent_manager, **kwargs
        )
