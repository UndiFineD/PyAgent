# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\app\api.py
"""
...
from evoagentx.app.schemas import (  # UserLogin,
    AgentCreate,
    AgentResponse,
    AgentUpdate,
    ExecutionCreate,
...
from evoagentx.app.services import (
    AgentService,
...
@auth_router.post(
    "/auth/register", response_model=UserResponse, tags=["Authentication"]
)
...
async def create_agent(
    agent: AgentCreate, current_user: Dict[str, Any] = Depends(get_current_active_user)
):
    """Create a new agent."""
    try:
        created_agent = await AgentService.create_agent(
            agent, user_id=str(current_user["_id"])
        )
...
async def get_agent(
    agent_id: str, current_user: Dict[str, Any] = Depends(get_current_active_user)
):
...
async def delete_agent(
    agent_id: str, current_user: Dict[str, Any] = Depends(get_current_admin_user)
):
...
@workflows_router.post(
    "/workflows", response_model=WorkflowResponse, status_code=201, tags=["Workflows"]
)
async def create_workflow(
    ...
    try:
        created_workflow = await WorkflowService.create_workflow(
            workflow, user_id=str(current_user["_id"])
        )
...
@workflows_router.get(
    "/workflows/{workflow_id}", response_model=WorkflowResponse, tags=["Workflows"]
)
async def get_workflow(
    workflow_id: str, current_user: Dict[str, Any] = Depends(get_current_active_user)
):
...
async def update_workflow(
    ...
    try:
        updated_workflow = await WorkflowService.update_workflow(
...
@workflows_router.delete(
    "/workflows/{workflow_id}", status_code=204, tags=["Workflows"]
)
async def delete_workflow(
    workflow_id: str, current_user: Dict[str, Any] = Depends(get_current_admin_user)
):
...
@workflows_router.get(
    "/workflows", response_model=List[WorkflowResponse], tags=["Workflows"]
)
async def list_workflows(
    ...
    converted_workflows = [
        {**workflow, "_id": str(workflow["_id"])} for workflow in workflows
    ]
...
@executions_router.post(
    "/executions", response_model=ExecutionResponse, status_code=202
)
async def create_execution(
    ...
    try:
        execution_result = await WorkflowExecutionService.create_execution(
            execution_data=execution, user_id=str(current_user["_id"])
        )
...
async def get_execution(
    execution_id: str, current_user: Dict[str, Any] = Depends(get_current_active_user)
):
...
@executions_router.post(
    "/executions/{execution_id}/stop", response_model=ExecutionResponse
)
async def stop_execution(
    ...
    try:
        updated_execution = await WorkflowExecutionService.update_execution_status(
            execution_id=execution_id, status=ExecutionStatus.CANCELLED
        )
...
@executions_router.get(
    "/executions/{execution_id}/logs", response_model=List[Dict[str, Any]]
)
async def get_execution_logs(
    ...
    logs, total = await WorkflowExecutionService.get_execution_logs(
        execution_id, params=pagination
    )
