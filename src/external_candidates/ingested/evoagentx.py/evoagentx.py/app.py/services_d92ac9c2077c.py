...
class AgentService:
    ...
    async def create_agent(
        agent_data: AgentCreate, user_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """Create a new agent."""
    ...
    async def update_agent(
        ...
        if "name" in update_data:
            ...
            existing = await Database.agents.find_one(
                {"name": update_data["name"], "_id": {"$ne": ObjectId(agent_id)}}
            )
            if existing:
                raise ValueError(
    ...
    async def delete_agent(agent_id: str) -> bool:
        ...
        workflow_count = await Database.workflows.count_documents(
            {"agent_ids": agent_id}
        )
        if workflow_count > 0:
            raise ValueError(
                f"Cannot delete agent {agent_id} as it is used in {workflow_count} workflows"
            )
...
class WorkflowService:
    ...
    async def create_workflow(
        workflow_data: WorkflowCreate, user_id: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if existing:
            raise ValueError(
                f"Workflow with name '{workflow_dict['name']}' already exists"
            )
        ...
        logger.info(
            f"Created workflow {workflow_dict['name']} with ID {result.inserted_id}"
        )
    ...
    async def update_workflow(
        workflow_id: str, workflow_data: WorkflowUpdate
    ) -> Optional[Dict[str, Any]]:
        """Update a workflow."""
        if not ObjectId.is_valid(workflow_id):
            raise ValueError(f"Invalid workflow ID: {workflow_id}")
        ...
        if "name" in update_data:
            ...
            if existing:
                raise ValueError(
                    f"Workflow with name '{update_data['name']}' already exists"
                )
        ...
        updated_workflow = await Database.workflows.find_one(
    ...
    async def delete_workflow(workflow_id: str) -> bool:
        ...
        if recent_executions > 0:
            raise ValueError(
                f"Cannot delete workflow {workflow_id} with {recent_executions} active executions"
            )
...
class WorkflowExecutionService:
    ...
    async def create_execution(
        execution_data: ExecutionCreate, user_id: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def list_executions(
        ...
        cursor = (
            Database.executions.find(query)
            .sort("created_at", -1)
            .skip(params.skip)
            .limit(params.limit)
        )
