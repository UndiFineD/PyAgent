# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\app\main.py
"""
...
async def root():
    ...
    """
    return {"app_name": settings.APP_NAME, "status": "healthy", "version": "0.1.0"}
...
async def get_metrics():
    ...
    try:
        ...
        running_workflows = await Database.workflows.count_documents(
            {"status": "running"}
        )

        # Collect execution metrics
        total_executions = await Database.executions.count_documents({})
        failed_executions = await Database.executions.count_documents(
            {"status": "failed"}
        )

        return {
            "agents": {"total": total_agents, "active": active_agents},
            "workflows": {"total": total_workflows, "running": running_workflows},
            "executions": {"total": total_executions, "failed": failed_executions},
        }
    except Exception as e:
        logger.error(f"Error retrieving metrics: {e}")
        return {"status": "error", "message": "Unable to retrieve metrics"}
