# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\benchmark\hotpotqa.py
import os
...
VALIDE_RAW_HOTPOTQA_FILES = [
    file for file in list(HOTPOTQA_FILES_MAP.values()) if file is not None
]
...
def download_raw_hotpotqa_data(name: str, save_folder: str):
    ...
    assert (
        name in VALIDE_RAW_HOTPOTQA_FILES
    ), f"'{name}' is an invalid hotpotqa file name. Available file names: {VALIDE_RAW_HOTPOTQA_FILES}"
...
class AFlowHotPotQA(HotPotQA):
    ...
    def _load_data(self):
        if self.mode == "train" or self.mode == "all":
            self._train_data = self._load_data_from_file(
                file_name=AFLOW_DATASET_FILES_MAP["hotpotqa"]["train"]
            )
        if self.mode == "dev" or self.mode == "all":
            self._dev_data = self._load_data_from_file(
                file_name=AFLOW_DATASET_FILES_MAP["hotpotqa"]["dev"]
            )
        if self.mode == "test" or self.mode == "all":
            self._test_data = self._load_data_from_file(
                file_name=AFLOW_DATASET_FILES_MAP["hotpotqa"]["test"]
            )
    ...
    async def async_evaluate(self, graph: Callable, example: Any) -> float:
        ...
        paragraphs = [
            item[1] for item in example["context"] if isinstance(item[1], list)
        ]
