# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\benchmark\WorfBench.py
import json
import os
import random
...
WORFBENCH_FILES_MAP = {"train": "worfbench_train.json", "test": "worfbench_test.json"}
...
def download_worfbench_data(dataset: str, save_folder: str) -> None:
    ...
    datasets_map = {
        "train": {
            "repo_id": "zjunlp/WorFBench_train",
            "filename": "worfbench_train.json",
            "split": "train",
        },
        "test": {
            "repo_id": "zjunlp/WorFBench_test",
            "filename": "worfbench_test.json",
            "split": "test",
        },
    }
    ...
    for split, info in datasets_map.items():
        ...
        if not os.path.exists(save_path):
            ...
            try:
                ...
                logger.info(
                    f"Successfully downloaded and saved {filename} to {save_path}"
                )
...
class WorfBench(Benchmark):
    ...
    def _load_data_from_file(self, file_name: str) -> Dict:
        ...
        if not os.path.exists(file_path):
            logger.error(
                f"File {file_path} still does not exist after download attempt!"
            )
            return None
    ...
    def _load_data(self) -> None:
        if self.mode in ["train", "dev"]:
            self._train_data = self._load_data_from_file(
                file_name=WORFBENCH_FILES_MAP["train"]
            )
            if self.mode == "dev":
                if self._train_data:
                    random.seed(42)
