# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\benchmark\measures.py
import copy
import string
import unicodedata
...
def exact_match_score(prediction: str, ground_truth: str) -> float:
    assert isinstance(
        ground_truth, str
    ), f"ground_truth must be a string, but got {type(ground_truth)}"
...
def ems(prediction: str, ground_truths: List[str]) -> float:
    assert isinstance(
        ground_truths, list
    ), f"ground_truths must be a list, but got {type(ground_truths)}"
...
def regex_match(text, pattern):
    ...
    try:
        pattern = regex.compile(
            pattern, flags=regex.IGNORECASE + regex.UNICODE + regex.MULTILINE
        )
...
def acc_score(prediction: str, ground_truths: List[str]) -> float:
    ...
    return float(
        has_answer(answers=ground_truths, text=prediction, match_type="string")
    )
