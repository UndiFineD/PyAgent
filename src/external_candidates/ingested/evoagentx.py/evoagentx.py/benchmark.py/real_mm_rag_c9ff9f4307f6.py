# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\benchmark\real_mm_rag.py
import json
import os
    ...
    try:
        ...
        if os.path.exists(dataset_path) and os.path.exists(images_dir):
            ...
            if len(image_files) > 0:
                logger.info(
                    f"Dataset already exists at {save_dir} with {len(image_files)} images"
                )
                return save_dir
...
class RealMMRAG(Benchmark):
    ...
    def _load_data(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self._test_data)} samples from REAL-MM-RAG dataset"
            )
    ...
    def get_sample(self, index: int) -> Dict[str, Any]:
        ...
        if index >= len(self._test_data):
            raise IndexError(
                f"Index {index} out of range for dataset size {len(self._test_data)}"
            )
    ...
    def get_random_samples(self, n: int, seed: int = 42) -> List[Dict[str, Any]]:
        ...
        indices = random.sample(
            range(len(self._test_data)), min(n, len(self._test_data))
        )
