...
class PromptTemplate(BaseModule):
    ...
    tools: Optional[List[Toolkit]] = Field(
        default=None, description="Tools that the LLM can use."
    )
    demonstrations: Optional[List[dict]] = Field(
        default=None, description="Examples of how to use the instruction."
    )
    ...
    def get_field_names(self) -> List[str]:
        return [
            name for name, _ in type(self).model_fields.items() if name != "class_name"
        ]
    ...
    def get(self, key: str) -> Any:
        ...
        if key not in fields:
            raise ValueError(
                f"Invalid key `{key}` for `{self.__class__.__name__}`. Valid keys are: {fields}"
            )
    ...
    def get_required_inputs_or_outputs(
        self, format: Type[LLMOutputParser]
    ) -> List[str]:
        """
    ...
    def check_required_inputs(self, inputs_format: Type[LLMOutputParser], values: dict):
        ...
        missing_required_inputs = [
            field for field in required_inputs if field not in values
        ]
    ...
    def render_input_example(
        ...
        if inputs_format is not None:
            ...
            field_values = {
                field: values.get(field, missing_field_value) for field in fields
            }
        else:
            field_values = values
        return "\n".join(
    ...
    def render_outputs(
        ...
        if (
            outputs_format is None
            or parse_mode in [None, "str", "custom"]
            or len(outputs_format.get_attrs()) == 0
        ):
    ...
    def format(
        ...
        **kwargs,
        ...
        raise NotImplementedError(
            f"`format` method is not implemented for `{self.__class__.__name__}`."
        )
...
class StringTemplate(PromptTemplate):
    ...
    def render_demonstrations(
        ...
        **kwargs,
        ...
        if inputs_format is None or outputs_format is None:
            raise ValueError(
                "`inputs_format` and `outputs_format` are required in `render_demonstrations`."
            )
        if len(inputs_format.get_attrs()) == 0 or len(outputs_format.get_attrs()) == 0:
            raise ValueError(
                "`inputs_format` and `outputs_format` must have at least one attribute."
            )
        ...
        for i, demo in enumerate(self.demonstrations):
            ...
            input_values = {
                field: demo.get(field, "Not provided") for field in input_fields
            }
            demo_str += self.render_input_example(
                inputs_format, input_values, missing_field_value="Not provided"
            )
            demo_str += "\n\n"
            ...
            output_values = {
                field: demo.get(field, "Not provided") for field in output_fields
            }
    ...
    def render_inputs(self, inputs_format: Type[LLMOutputParser], values: dict) -> str:
        ...
        input_str += self.render_input_example(
            inputs_format, values, missing_field_value="Not provided"
        )
        input_str += "\n"
        return input_str
...
class ChatTemplate(StringTemplate):
    ...
    def render_demonstrations(
        ...
        for demo in self.demonstrations:
            ...
            user_content = self.render_input_example(
                inputs_format, input_values, missing_field_value="Not provided"
            )
    ...
    def render_inputs(
        self, inputs_format: Optional[Type[LLMOutputParser]], values: Optional[dict]
    ) -> str:
