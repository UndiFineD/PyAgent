# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\prompts\workflow\multi_agent_debate.py
"""
...
def build_judge_prompt(problem: str, transcript_text: str, roles: list) -> str:
    ...
    return JUDGE_AGENT_PROMPT.format(
        problem=problem, roles_text=roles_text, transcript_text=transcript_text
    )
...
def collect_last_round_candidates(
    ...
    for agent_id in range(num_agents):
        ...
        records = [
            t
            for t in transcript
            if t.get("agent_id") == agent_id and t.get("round") == last_round_index
        ]
        if not records:
            continue
        rec = records[-1]
...
def collect_round_candidates(
    ...
    for agent_id in range(num_agents):
        records = [
            t
            for t in transcript
            if t.get("agent_id") == agent_id and t.get("round") == round_index
        ]
        if not records:
            continue
        rec = records[-1]
