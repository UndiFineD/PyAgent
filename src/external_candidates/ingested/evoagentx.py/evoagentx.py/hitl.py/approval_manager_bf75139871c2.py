# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\hitl\approval_manager.py
import asyncio
import json
import sys
...
class HITLManager(BaseModule):
    ...
    async def _handle_cli_interaction(
        self, request: HITLRequest, timeout: float
    ) -> HITLResponse:
        ...
        try:
            if request.interaction_type == HITLInteractionType.APPROVE_REJECT:
                ...
                request.interaction_type == HITLInteractionType.MULTI_TURN_CONVERSATION
            ):
    ...
    async def _handle_approve_reject(self, request: HITLRequest) -> HITLResponse:
        ...
        if decision == HITLDecision.REJECT:
            ...
            def get_feedback():
                return input(
                    "Please provide the reason for rejection (optional): "
                ).strip()
    ...
    async def _handle_review_edit(self, request: HITLRequest) -> HITLResponse:
        ...
        raise NotImplementedError(
            "Not implemented HITL type: HITLInteractionType.REVIEW_EDIT_STATE"
        )
        # def get_user_input():
    ...
    async def _handle_tool_calls(self, request: HITLRequest) -> HITLResponse:
        ...
        raise NotImplementedError(
            "Not implemented HITL type: HITLInteractionType.REVIEW_TOOL_CALLS"
        )
        # def get_user_input():
    ...
    async def _handle_conversation(self, request: HITLRequest) -> HITLResponse:
        ...
        raise NotImplementedError(
            "Not implemented HITL type: HITLInteractionType.MULTI_TURN_CONVERSATION"
        )
        # def get_user_input():
...
"""
    ...
    async def request_user_input(
        ...
        # display request and wait for response in CLI
        try:
            response = await self._handle_user_input_collection(
                request, input_fields, timeout
            )
            future.set_result(response)
            return response
        except asyncio.TimeoutError:
    ...
    async def _handle_user_input_collection(
        self, request: HITLRequest, input_fields: dict, timeout: float
    ) -> HITLResponse:
        ...
        try:
            ...
            def get_user_inputs():
                    ...
                    while True:
                        ...
                        if not user_input:
                            if not required and default_value is not None:
                                ...
                                print(
                                    f"Field '{field_name}' is required, please provide input."
                                )
                                continue
                        ...
                        try:
                            if field_type == "int":
                                ...
                                collected_inputs[field_name] = (
                                    str(user_input) if user_input else None
                                )  # prepare_action_prompt do not accept int
                                ...
                                collected_inputs[field_name] = (
                                    user_input.lower() in ["true", "1", "yes", "y"]
                                    if user_input
                                    else None
                                )
                            ...
                            print(
                                f"Input format error, field '{field_name}' needs {field_type} type value."
                            )
                            continue
                ...
                while True:
                    confirm = (
                        input("\nConfirm these inputs? [y]es / [n]o / [r]etry: ")
                        .lower()
                        .strip()
                    )
                    if confirm in ["y", "yes"]:
    ...
    def _generate_user_input_prompt_message(
        self, context: HITLContext, input_fields: dict
    ) -> str:
        """Generate prompt message for user input collection"""
