    ...
    def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            raise RuntimeError(
                "Cannot use asyncio.run() in async context. Use async_execute directly."
            )
        except RuntimeError:
            # if not in the asynchronous context, use asyncio.run()
            return asyncio.run(
                self.async_execute(llm, inputs, hitl_manager, sys_msg=sys_msg, **kwargs)
            )
    ...
    async def async_execute(
        ...
        **kwargs,
        ...
        if workflow_json_path:
            ...
            raise ValueError(
                "must provide the workflow_json_path or existing_workflow parameter"
            )

        # start the conversation loop
        workflow_json = await self._conversation_loop(
            llm, workflow_json, hitl_manager, **kwargs
        )
    ...
    async def _conversation_loop(
        ...
        **kwargs,
        ...
        if not hitl_manager.is_active:
            raise ValueError(
                "HITLManager is not active, please activate the HITLManager first"
            )
        ...
        while True:
            try:
                ...
                if choice == "1":
                    ...
                    # LLM optimization
                    user_advice = input(
                        "please input the optimization advice (type q to cancel): "
                    ).strip()
                    if user_advice == "q":
                        continue
...
class HITLOutsideConversationAgent(HITLBaseAgent):
        ...
        if hasattr(self, "actions") and len(self.actions) > 0:
            if isinstance(self.actions[0], HITLOutsideConversationAction):
                return self.actions[0].execute(
                    llm, inputs, hitl_manager, sys_msg, **kwargs
                )
            else:
                raise ValueError(
    ...
    async def async_execute(
        ...
        **kwargs,
        ...
        if hasattr(self, "actions") and len(self.actions) > 0:
            if isinstance(self.actions[0], HITLOutsideConversationAction):
                return await self.actions[0].async_execute(
                    llm, inputs, hitl_manager, sys_msg, **kwargs
                )
            else:
                raise ValueError(
