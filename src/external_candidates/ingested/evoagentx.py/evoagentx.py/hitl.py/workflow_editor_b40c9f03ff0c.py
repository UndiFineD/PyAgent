# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\hitl\workflow_editor.py
import json
import os
import time
    ...
    def __init__(self, config: MockLLMConfig = None, **kwargs):
        if config is None:
            config = MockLLMConfig(
                llm_type="MockLLM", model="mock-model", output_response=True
            )
...
def default_llm_config():
    ...
    is_testing = (
        ...
        or os.getenv("CI") is not None  # CI environment
        or OPENAI_API_KEY is None  # no API key
        or OPENAI_API_KEY.strip() == ""  # empty API key
    )
    ...
    if is_testing:
        ...
        llm_config = OpenAILLMConfig(
            model="gpt-4o", openai_key=OPENAI_API_KEY, stream=True, output_response=True
        )
...
class WorkFlowEditor(BaseModule):
    ...
    async def edit_workflow(
        self, file_path: str, instruction: str, new_file_path: Optional[str] = None
    ):
        """
            ...
            if not path_split[0]:
                ...
                if os.path.exists(path_split[0]) and path_split[1][:-5] == ".json":
                    ...
                    raise FileNotFoundError(
                        f"The directory {path_split[0]} does not exist or the file name is not a json file name."
                    )
        ...
        optimization_prompt = WORKFLOW_EDITOR_PROMPT.format(
            current_workflow_json=json.dumps(
                workflow_json, indent=2, ensure_ascii=False
            ),
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a helpful assistant that can optimize the workflow json structure.",
            },
            {"role": "user", "content": optimization_prompt},
        ]
        try:
            response = await self.llm.single_generate_async(
