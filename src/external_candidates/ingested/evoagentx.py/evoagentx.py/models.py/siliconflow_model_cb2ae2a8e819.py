# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\models\siliconflow_model.py
...
class SiliconFlowLLM(OpenAILLM):
    ...
    def init_model(self):
        ...
        self._client = self._init_client(
            config
        )  # OpenAI(api_key=config.siliconflow_key, base_url="https://api.siliconflow.cn/v1")
        self._default_ignore_fields = [
            "llm_type",
            "siliconflow_key",
            "output_response",
        ]  # parameters in SiliconFlowConfig that are not OpenAI models' input parameters

    def _init_client(self, config: SiliconFlowConfig):
        client = OpenAI(
            api_key=config.siliconflow_key, base_url="https://api.siliconflow.cn/v1"
        )
        return client
    ...
    def single_generate(self, messages: List[dict], **kwargs) -> str:
        ...
        try:
            ...
            if "account balance is insufficient" in str(e):
                print(
                    "Warning: Account balance insufficient. Please recharge your account."
                )
                return ""
