# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\models\openrouter_model.py
import asyncio
...
class OpenRouterLLM(BaseLLM):
    ...
    def get_completion_params(self, **kwargs):
        completion_params = self.config.get_set_params(
            ignore=self._default_ignore_fields
        )
    ...
    async def get_stream_output_async(
        self, response, output_response: bool = False
    ) -> str:
        output = ""
    ...
    def get_completion_output(
        self, response: ChatCompletion, output_response: bool = True
    ) -> str:
    ...
    def single_generate(self, messages: List[dict], **kwargs) -> str:
        ...
        try:
            ...
            if stream:
                output = self.get_stream_output(
                    response, output_response=output_response
                )
                cost = self._stream_cost(messages=messages, output=output)
            else:
                output: str = self.get_completion_output(
                    response=response, output_response=output_response
                )
                cost = self._completion_cost(response)
            self._update_cost(cost=cost)
        except Exception as e:
            raise RuntimeError(
                f"Error during single_generate of OpenRouterLLM: {str(e)}"
            )
    ...
    def batch_generate(self, batch_messages: List[List[dict]], **kwargs) -> List[str]:
        return [
            self.single_generate(messages=one_messages, **kwargs)
            for one_messages in batch_messages
        ]
        ...
        try:
            ...
            if stream:
                if hasattr(response, "__aiter__"):
                    output = await self.get_stream_output_async(
                        response, output_response=output_response
                    )
                else:
                    output = self.get_stream_output(
            ...
            raise RuntimeError(
                f"Error during single_generate_async of OpenRouterLLM: {str(e)}"
            )
