...
class CostManager:
    ...
    def update_cost(self, cost: Cost, model: str):
        ...
        self.total_input_tokens[model] = (
            self.total_input_tokens.get(model, 0) + cost.input_tokens
        )
        self.total_output_tokens[model] = (
            self.total_output_tokens.get(model, 0) + cost.output_tokens
        )
        current_total_tokens = cost.input_tokens + cost.output_tokens
        self.total_tokens[model] = (
            self.total_tokens.get(model, 0) + current_total_tokens
        )

        self.total_input_cost[model] = (
            self.total_input_cost.get(model, 0.0) + cost.input_cost
        )
        self.total_output_cost[model] = (
            self.total_output_cost.get(model, 0.0) + cost.output_cost
        )
    ...
    def display_cost(self):
        ...
        for model in self.total_tokens.keys():
            ...
            data["Total Input Cost (USD)"].append(
                round(self.total_input_cost[model], 4)
            )
            data["Total Output Cost (USD)"].append(
                round(self.total_output_cost[model], 4)
            )
