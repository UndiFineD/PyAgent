# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\models\base_model.py
import asyncio
import base64
import inspect
import io
...
class LLMOutputParser(Parser):
    ...
    content: str = Field(
        default=None, exclude=True, description="the text generated by LLM"
    )
    ...
    def get_attr_descriptions(cls) -> dict:
        ...
        for field_name, field_info in cls.model_fields.items():
            if field_name not in attrs:
                continue
            field_desc = (
                field_info.description if field_info.description is not None else "None"
            )
        ...
        **kwargs,
        ...
        if parse_mode == "str":
            ...
            if parse_func is None:
                raise ValueError(
                    "`parse_func` must be provided when `parse_mode` is 'custom'."
                )
            ...
            if not isinstance(data, dict):
                raise ValueError(
                    f"The output of `parse_func` must be a dictionary, but found {type(data)}."
                )
            return data
        else:
            raise ValueError(
    ...
    def _parse_json_content(cls, content: str, **kwargs) -> dict:
        ...
        if len(extracted_json_list) > 0:
            ...
            try:
                ...
                if not isinstance(data, dict):
                    if isinstance(data, list):
                        ...
                        raise ValueError(
                            f"The generated content is not a valid JSON string:\n{json_str}"
                        )
            except Exception:
                raise ValueError(
                    f"The generated content is not a valid JSON string:\n{json_str}"
            ...
            raise ValueError(
                f"The following generated content does not contain JSON string!\n{content}"
            )
    ...
    def _parse_xml_content(cls, content: str, **kwargs) -> dict:
        ...
        for attr, attr_type in attrs_with_types:
            ...
            if len(attr_raw_value_list) > 0:
                ...
                try:
                    attr_value = parse_data_from_text(
                        text=attr_raw_value, datatype=attr_type
                    )
                except Exception:
                    raise ValueError(
                        f"Cannot parse text: {attr_raw_value} into {attr_type} data!"
                    )
            else:
                raise ValueError(
    ...
    @classmethod
    def _parse_title_content(
        cls, content: str, title_format: str = "## {title}", **kwargs
    ) -> dict:
        ...
        for line in content.split("\n"):
            ...
            if is_title:
                if (
                    current_output_name is not None
                    and current_output_content is not None
                ):
    ...
    def parse(
        ...
        **kwargs,
        ...
        if not isinstance(content, str):
            raise ValueError(
                f"The input to {cls.__name__}.parse should be a str, but found {type(content)}."
            )
        data = cls.get_content_data(
            content=content, parse_mode=parse_mode, parse_func=parse_func, **kwargs
        )
...
def _is_multimodal_content(content: Any) -> bool:
    ...
    try:
        from ..rag.schema import ImageChunk, TextChunk
...
def _process_multimodal_content(
    content: List[Any], model_type: str = "openai"
) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise ImportError(
            "Cannot import TextChunk/ImageChunk from rag.schema for multimodal processing"
        )
...
def _get_image_data_url(image_chunk) -> str:
    ...
    try:
        ...
        if image is None:
            raise ValueError(
                f"Could not load image from path: {image_chunk.image_path}"
            )
...
class BaseLLM(ABC):
    ...
    async def single_generate_async(self, messages: List[dict], **kwargs) -> str:
        ...
        result = await loop.run_in_executor(
            None, self.single_generate, messages, **kwargs
        )
        return result

    async def batch_generate_async(
        self, batch_messages: List[List[dict]], **kwargs
    ) -> List[str]:
    ...
    def _prepare_messages(
        ...
        if prompt and messages:
            raise ValueError(
                "Both 'prompt' and 'messages' are provided. Please provide only one of them."
            )
        ...
        if isinstance(prompt, str):
            ...
            if not prompt:  # empty prompt
                return [], False
            if system_message:
                if not isinstance(system_message, list) or len(prompt) != len(
                    system_message
                ):
                    raise ValueError(
            ...
            raise ValueError(
                f"'prompt' must be a str or List[str], but found {type(prompt)}."
            )

        prepared_messages = self.formulate_messages(
            prompts=prompt, system_messages=system_message
        )
        return prepared_messages, single_generate

    def _process_messages_for_multimodal(
        ...
        for message_list in messages:
            ...
            for message in message_list:
                ...
                if _is_multimodal_content(content):
                    ...
                    logger.debug(
                        f"Processing multimodal content: llm_type={llm_type}, model_type={model_type}"
                    )

                    # Convert multimodal content to appropriate format
                    if isinstance(content, list):
                        processed_message["content"] = _process_multimodal_content(
                            content, model_type
                        )
                    else:
    ...
    def generate(
        ...
        **kwargs,
        ...
        """
        prepared_messages, single_generate = self._prepare_messages(
            prompt, system_message, messages
        )
        if not prepared_messages:  # Handle empty messages case
            return []

        generated_texts = self.batch_generate(
            batch_messages=prepared_messages, **kwargs
        )
        parsed_outputs = self.parse_generated_texts(
    ...
    async def async_generate(
        ...
        **kwargs,
        ...
        generated_texts = await self.batch_generate_async(
            batch_messages=prepared_messages, **kwargs
        )
        parsed_outputs = self.parse_generated_texts(
            texts=generated_texts,
