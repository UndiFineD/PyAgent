...
class LongTermMemory(BaseMemory):
    ...
    storage_handler: StorageHandler = Field(
        ..., description="Handler for persistent storage"
    )
    rag_config: RAGConfig = Field(..., description="Configuration for RAG engine")
    rag_engine: RAGEngine = Field(
        default=None, description="RAG engine for indexing and retrieval"
    )
    memory_table: str = Field(
        default="memory", description="Database table for storing memories"
    )
    default_corpus_id: Optional[str] = Field(
        default=None, description="Default corpus ID for memory indexing"
    )
    ...
    def init_module(self):
        ...
        logger.info(
            f"Initialized LongTermMemory with corpus_id {self.default_corpus_id}"
        )
    ...
    def add(
        self, messages: Union[Message, str, List[Union[Message, str]]]
    ) -> List[str]:
        """Store messages in memory and index them in RAGEngine, returning memory_ids."""
        if not isinstance(messages, list):
            messages = [messages]
        messages = [
            Message(content=msg) if isinstance(msg, str) else msg for msg in messages
        ]
        ...
        existing_hashes = {
            record["content_hash"]
            for record in self.storage_handler.load(tables=[self.memory_table]).get(
                self.memory_table, []
            )
        ...
        for msg, memory_id in zip(messages, memory_ids):
            ...
            if content_hash in existing_hashes:
                ...
                existing_id = next(
                    (
                        r["memory_id"]
                        for r in self.storage_handler.load(
                            tables=[self.memory_table]
                        ).get(self.memory_table, [])
    ...
    async def get(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(results)} memories for memory_ids: {memory_ids}"
            )
            return results
    ...
    def update(
        self,
        updates: Union[
            Tuple[str, Union[Message, str]], List[Tuple[str, Union[Message, str]]]
        ],
    ) -> List[bool]:
        ...
        if not isinstance(updates, list):
            updates = [updates]
        updates = [
            (mid, Message(content=msg) if isinstance(msg, str) else msg)
    ...
    async def search_async(
        ...
        try:
            result: RagResult = await self.rag_engine.query_async(
                query_obj, corpus_id=self.default_corpus_id
            )
            if return_chunk:
                return [
                    (chunk, chunk.metadata.memory_id) for chunk in result.corpus.chunks
                ]
            else:
                messages = [
                    (self._chunk_to_message(chunk), chunk.metadata.memory_id)
            ...
            logger.info(
                f"Retrieved {len(messages)} memories for query: {query_obj.query_str}"
            )
    ...
    def load(self, save_path: Optional[str] = None) -> List[str]:
        ...
        return self.rag_engine.load(
            source=save_path, corpus_id=self.default_corpus_id, table=self.memory_table
        )
