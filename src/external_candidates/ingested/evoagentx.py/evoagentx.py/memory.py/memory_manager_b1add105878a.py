# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\memory\memory_manager.py
import json
...
class MemoryManager(BaseModule):
    ...
    llm: Optional[BaseLLM] = Field(
        default=None, description="LLM for deciding memory operations"
    )
    use_llm_management: bool = Field(
        default=True, description="Toggle LLM-based memory management"
    )
    ...
    async def handle_memory(
        ...
        data: Optional[
            Union[
                ...
                Dict,
                List[Tuple[str, Union[Message, str]]],
            ]
        ] = None,
        ...
        if action == "add":
            ...
            messages = [
                Message(
                    ...
                    msg_type=(
                        MessageType.REQUEST if isinstance(msg, str) else msg.msg_type
                    ),
                    timestamp=(
                    ...
                    message_id=(
                        str(uuid4())
                        if isinstance(msg, str) or not msg.message_id
                        else msg.message_id
                    ),
                )
                for msg in data
            ...
            input_data = [
                {"action": "add", "memory_id": str(uuid4()), "message": msg.to_dict()}
                for msg in messages
            ]
            ...
            input_data = [
                {"action": "update", "memory_id": mid, "message": msg.to_dict()}
                for mid, msg in updates
            ]
            if self.use_llm_management and self.llm:
                existing_memories = await self.memory.get([mid for mid, _ in updates])
                llm_decisions = await self._prompt_llm_for_memory_operation(
                    input_data, relevant_data=existing_memories
                )
                final_updates = []
                ...
                return (
                    self.memory.update(final_updates)
                    if final_updates
                    else [False] * len(updates)
                )
            ...
            if self.use_llm_management and self.llm:
                input_data = [
                    {"action": "delete", "memory_id": mid} for mid in memory_ids
                ]
                ...
                valid_memory_ids = [
                    decision.get("memory_id")
                    for decision in llm_decisions
                    if decision.get("action") == "delete"
                ]
                return (
                    self.memory.delete(valid_memory_ids)
                    if valid_memory_ids
                    else [False] * len(memory_ids)
                )
            ...
            memories = await self.memory.search_async(
                user_prompt, top_k, metadata_filters
            )
