# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\frameworks\multi_agent_debate\debate.py
import json
import os
import random
...
class DebateAgentOutput(LLMOutputParser):
    ...
    answer: Optional[str] = Field(
        default=None, description="Current answer for this round (optional)"
    )
...
class DebateJudgeOutput(LLMOutputParser):
    ...
    winning_agent_id: int = Field(
        default=0, description="Winning debater ID (starting from 0)"
    )
...
class MultiAgentDebateActionGraph(ActionGraph):
    ...
    llm_config: LLMConfig = Field(
        default_factory=lambda: OpenAILLMConfig(
            model="gpt-4o-mini", openai_key=os.getenv("OPENAI_API_KEY")
        ),
    ...
    debater_agents: Optional[List[CustomizeAgent]] = Field(
        default=None,
        description="Optional: multiple debater CustomizeAgents, randomly selected during execution",
    )
    judge_agent: Optional[CustomizeAgent] = Field(
        default=None,
        description="Optional: judge CustomizeAgent, used for judging phase if provided",
    )
    # Optional model pool: for random selection of different models
    llm_config_pool: Optional[List[LLMConfig]] = Field(
        default=None,
        description="Optional: LLM configuration pool for random selection, provides choices for agents without specified models",
    )
    # Group workflow mode (each position occupied by an independent workflow graph instead of agent)
    group_graphs_enabled: bool = Field(
        default=False,
        description="Enable group graph mode: replace individual debaters with workflow graphs",
    )
    group_graphs: Optional[List[ActionGraph]] = Field(
        default=None,
        description="When group graph mode is enabled, provide workflow graph list (length >= 1)",
    )
    ...
    def init_module(self):
        ...
        if self.group_graphs_enabled and (
            not self.group_graphs or len(self.group_graphs) == 0
        ):
            raise ValueError(
    ...
    def _create_default_debater_agent(self) -> CustomizeAgent:
        ...
        llm_config = (
            random.choice(self.llm_config_pool)
            if self.llm_config_pool
        ...
        return CustomizeAgent(
            ...
            inputs=[
                {"name": "problem", "type": "str", "description": "Problem statement"},
                {
                    ...
                    "type": "str",
                    "description": "Formatted debate transcript so far",
                },
                {"name": "role", "type": "str", "description": "Debater role/persona"},
                {
                    "name": "agent_id",
                    "type": "str",
                    "description": "Debater id (string)",
                },
                {
                    "name": "round_index",
                    "type": "str",
                    "description": "1-based round index",
                },
            ...
            outputs=[
                ...
                },
                {
                    "name": "argument",
                    "type": "str",
                    "description": "Argument or rebuttal",
                    "required": True,
                },
                {
                    "name": "answer",
                    "type": "str",
                    "description": "Optional current answer",
                    "required": False,
                },
            ],
            parse_mode="xml",
        ...
        return CustomizeAgent(
            ...
            inputs=[
                {"name": "problem", "type": "str", "description": "Problem statement"},
                {
                    "name": "transcript_text",
                    "type": "str",
                    "description": "Formatted debate transcript",
                },
                {
                    "name": "roles_text",
                    "type": "str",
                    "description": "Roles listing text",
                },
            ],
            outputs=[
                {
                    "name": "rationale",
                    "type": "str",
                    "description": "Judging rationale",
                    "required": True,
                },
                {
                    "name": "winning_agent_id",
                    "type": "str",
                    "description": "Winning agent id (integer as string)",
                    "required": True,
                },
                {
                    "name": "final_answer",
    ...
    def execute(
        ...
        **kwargs,
        ...
        state = self._setup_debate(
            problem, num_agents, num_rounds, personas, agent_llm_configs
        )
        ...
        if enable_pruning:
            ...
            candidates = collect_last_round_candidates(
                transcript=transcript,
                num_agents=state["num_agents"],
                last_round_index=state["num_rounds"] - 1,
            )
        ...
        consensus = self._generate_consensus(
            problem, state, transcript, judge_mode, pruning_info
        )
        result: Dict[str, Any] = {
        ...
        if (
            enable_pruning
            and pruning_snapshot_mode
            and pruning_rounds_debug is not None
        ):
    ...
    async def async_execute(
        ...
        **kwargs,
        ...
        transcript = await self._run_debate_rounds_async(
            problem, state, transcript_mode
        )
        pruning_info = None
        pruning_debug = None
        pruning_rounds_debug: Optional[List[Dict[str, Any]]] = None
        if enable_pruning:
            ...
            try:
                ...
                pruning_debug = None
        consensus = await self._generate_consensus_async(
            problem, state, transcript, judge_mode, pruning_info
        )
        result: Dict[str, Any] = {
            "final_answer": consensus["final_answer"],
    ...
    def _setup_debate(
        ...
        agents_for_ids: List[CustomizeAgent] = self._prepare_runtime_debaters(
            num_agents, agent_llm_configs
        )
        state: Dict[str, Any] = {
            "problem": problem,
    ...
    def _prepare_runtime_debaters(
        self, num_agents: int, agent_llm_configs: Optional[List[LLMConfig]]
    ) -> List[CustomizeAgent]:
        ...
        if agent_llm_configs and len(agent_llm_configs) > 0:
            return [
                self._create_debater_agent_with_llm(
                    agent_llm_configs[i % len(agent_llm_configs)]
                )
                for i in range(num_agents)
            ]
        ...
        if self.llm_config_pool and len(self.llm_config_pool) > 0:
            return [
                self._create_debater_agent_with_llm(random.choice(self.llm_config_pool))
    ...
    def _run_debate_rounds(
        self, problem: str, state: dict, transcript_mode: str = "prev"
    ) -> List[dict]:
    ...
    async def _run_debate_rounds_async(
        self, problem: str, state: dict, transcript_mode: str = "prev"
    ) -> List[dict]:
        ...
        for round_index in range(num_rounds):
            ...
            # 若有外部注入的Agent，则并发调用Agent；否则走LLM批量生成
                ...
                state.get("agents")
                or self.debater_agents
                or self.debater_agent is not None
            ):
                import asyncio
                ...
                results = await self._llm.batch_generate_async(
                    batch_messages=[
                        [{"role": "user", "content": p}] for _, p in prompts
                    ]
                )
    ...
    def _generate_consensus(
        ...
        if judge_mode == "self_consistency":
            ...
            if len(agent_final_answers) == 0:
                # 无人给出显式答案，则合并最后一轮的论据
                agent_final_answers = [
                    t["argument"] for t in transcript if t.get("argument")
                ]
        ...
        # 获取获胜者的答案
        winner_answer = self._get_winner_answer(
            transcript, winner_id, state["num_rounds"]
        )
    ...
    def _collect_agent_final_answers(
        self, state: dict, transcript: List[dict]
    ) -> List[str]:
        """收集每位辩手的最终答案（若有）。"""
        num_agents = state["num_agents"]
        ...
        for agent_id in range(num_agents):
            ...
            records = [
                t
                for t in transcript
                if t["agent_id"] == agent_id and t["round"] == num_rounds - 1
            ]
            if len(records) == 0:
                continue
    ...
    def _get_winner_answer(
        self, transcript: List[dict], winner_id: int, num_rounds: int
    ) -> Optional[str]:
        """获取获胜者在最后一轮的答案。"""
        # 查找获胜者在最后一轮的记录
        records = [
            t
            for t in transcript
            if t["agent_id"] == winner_id and t["round"] == num_rounds - 1
        ]
        if len(records) == 0:
            return None
        ...
        if data.get("debater_agents"):
            ...
            for agent_info in data["debater_agents"]:
                try:
                    ...
                    llm_config = instance._deserialize_llm_config(
                    ...
                    print(
                        f"警告: 重建agent {agent_info.get('name', 'unknown')}失败: {e}"
                    )
                    continue
    ...
    @classmethod
    def load_module(
        cls, path: str, llm_config: LLMConfig = None, **kwargs
        ...
        if config.get("llm_config"):
            try:
                instance.llm_config = instance._deserialize_llm_config(
                    config["llm_config"]
                )
            except Exception as e:
            ...
            try:
                ...
                for agent_info in agent_data:
                    try:
                        ...
                        print(
                            f"警告: 加载agent {agent_info.get('name', 'unknown')}失败: {e}"
                        )
                        continue
        ...
        if judge_agent_file and os.path.exists(judge_agent_file):
            try:
                ...
                instance.judge_agent = CustomizeAgent.from_file(
                    path=judge_agent_file, llm_config=instance.llm_config or llm_config
                )
    ...
    def _serialize_llm_config(self, llm_config) -> Optional[Dict[str, Any]]:
        ...
        config_info = {
            ...
            "temperature": (
                llm_config.temperature if hasattr(llm_config, "temperature") else None
            ),
    ...
    def _deserialize_llm_config(
        self, config_info: Optional[Dict[str, Any]]
    ) -> Optional[LLMConfig]:
        """反序列化LLM配置（从环境变量重建）"""
        if not config_info:
            return None
