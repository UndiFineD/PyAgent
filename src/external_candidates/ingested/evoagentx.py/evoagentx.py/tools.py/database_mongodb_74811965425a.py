# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\database_mongodb.py
import json
import time
...
from .database_base import DatabaseBase, DatabaseConnection, DatabaseType, QueryType
...
class MongoDBConnection(DatabaseConnection):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if (
                "mongodb://" in self.connection_string
                or "mongodb+srv://" in self.connection_string
            ):
...
class MongoDBDatabase(DatabaseBase):
    ...
    def _is_remote_connection(self) -> bool:
        ...
        return self.connection_string and (
            self.connection_string.startswith(("mongodb://", "mongodb+srv://"))
            or "localhost" in self.connection_string
    ...
    def _load_local_collections(self):
        ...
        json_files = [
            f for f in self.local_path.glob("*.json") if f.name != "db_info.json"
        ]
            ...
            try:
                ...
                if documents:
                    ...
                    if cleaned_documents:
                        ...
                        logger.info(
                            f"Loaded {len(cleaned_documents)} documents into '{collection_name}'"
                        )
        ...
        if isinstance(doc, dict):
            ...
            for key, value in doc.items():
                if key == "_id" and isinstance(value, dict) and "$oid" in value:
                    ...
                    cleaned[key] = [
                        (
                            self._clean_document_for_insert(item)
    ...
    def execute_query(
        ...
        **kwargs,
        ...
        try:
            ...
            if self.read_only and query_type in [
                QueryType.INSERT,
                QueryType.UPDATE,
                QueryType.DELETE,
                QueryType.CREATE,
            ...
            return self.format_error_result(
                str(e), query_type, execution_time=execution_time
            )
    ...
    def _execute_insert(
        self, collection, query: Union[Dict, List], **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            if isinstance(query, dict):
                ...
                return self.format_error_result(
                    "Invalid insert query format", QueryType.INSERT
                )
        ...
        try:
            ...
            if multi:
                result = collection.update_many(
                    filter_query, update_query, upsert=upsert
                )
            else:
    ...
    def _execute_aggregate(
        self, collection, pipeline: List, **kwargs
    ) -> Dict[str, Any]:
    ...
    def get_collection_info(self, collection_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            info = {
                ...
                "indexes": [
                    {"name": idx["name"], "keys": idx["key"]} for idx in indexes
                ],
            ...
            if collection_name:
                ...
                for coll_name in collections[:10]:  # Limit to first 10 collections
                    ...
                    if coll_schema.get("success"):
                        schemas[coll_name] = coll_schema.get("data", {}).get(
                            "schema", {}
                        )
    ...
    def _update_schema_from_document(
        self, schema: Dict, document: Dict, path: str = ""
    ):
        ...
        for key, value in document.items():
            ...
            if isinstance(value, dict):
                ...
                self._update_schema_from_document(
                    schema[current_path]["fields"], value, current_path
                )
            elif isinstance(value, list):
                if current_path not in schema:
                ...
                schema[current_path]["element_types"] = list(
                    schema[current_path]["element_types"]
                )
            else:
                if current_path not in schema:
...
class MongoDBExecuteQueryTool(Tool):
    ...
    description: str = (
        "Execute MongoDB queries including find and aggregation pipelines (read-only operations)"
    )
    ...
    def __call__(
        self, query: str, query_type: str = None, collection_name: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            if not self.database:
                return {
                    "success": False,
                    "error": "MongoDB database not initialized",
                    "data": None,
                }
            ...
            if result["success"]:
                logger.info(
                    f"Successfully executed MongoDB query on collection {collection_name}"
                )
            else:
                logger.error(
                    f"Failed to execute MongoDB query: {result.get('error', 'Unknown error')}"
                )
...
class MongoDBFindTool(Tool):
    ...
    description: str = (
        "Find documents in a MongoDB collection with filtering, projection, sorting, and pagination"
    )
    inputs: Dict[str, Dict[str, str]] = {
        "collection_name": {
            "type": "string",
    ...
    def __call__(
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Successfully found documents in collection {collection_name}"
                )
            else:
                logger.error(
                    f"Failed to find documents: {result.get('error', 'Unknown error')}"
                )
...
class MongoDBUpdateTool(Tool):
    ...
    def __call__(
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Successfully updated documents in collection {collection_name}"
                )
            else:
                logger.error(
                    f"Failed to update documents: {result.get('error', 'Unknown error')}"
                )
...
class MongoDBDeleteTool(Tool):
    ...
    def __call__(
        self, collection_name: str, filter: str, multi: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Successfully deleted documents from collection {collection_name}"
                )
            else:
                logger.error(
                    f"Failed to delete documents: {result.get('error', 'Unknown error')}"
                )
...
class MongoDBInfoTool(Tool):
    ...
    def __call__(
        self, info_type: str = "database", collection_name: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.error(
                    f"Failed to retrieve {info_type} information: {result.get('error', 'Unknown error')}"
                )
            ...
            capabilities.update(
                {
                    ...
                    "local_path": (
                        str(self.database.local_path)
                        if self.database.local_path
                        else None
                    ),
        ...
        return (
            {
                "is_local_database": self.database.is_local_database,
                "local_path": (
                    str(self.database.local_path) if self.database.local_path else None
                ),
                "auto_save": self.database.auto_save,
                "read_only": self.database.read_only,
                "database_name": self.database_name,
