...
class GmailBase(BaseModule):
    ...
    def __init__(
        self, client_secret_file: str = None, token_file: str = None, **kwargs
    ):
        """
        ...
        self.client_secret_file = client_secret_file or os.getenv(
            "GMAIL_CLIENT_SECRET_FILE", CLIENT_SECRET_FILE
        )
    ...
    def _modify_message_labels(
        ...
        try:
            ...
            logger.error(
                f"Unexpected error modifying labels for {message_id}: {str(e)}"
            )
            return {"success": False, "error": str(e)}
    ...
    def _download_attachments(
        self, message_id: str, download_dir: str = None
    ) -> Dict[str, Any]:
        """
            ...
            for part in parts:
                if part.get("filename"):
                    try:
                        ...
                        logger.warning(
                            f"Error downloading attachment {part.get('filename')}: {str(e)}"
                        )
                        continue
            ...
            for msg in messages:
                try:
                    ...
                    formatted_msg = self._format_message(
                        message, include_attachments=include_attachments
                    )

                    # Download attachments if requested
                    if (
                        download_attachments
                        and formatted_msg.get("attachments_count", 0) > 0
                    ):
                        attach_result = self._download_attachments(
                            msg["id"], download_dir
                        )
                        if attach_result.get("success"):
                            formatted_msg["downloaded_attachments"] = attach_result[
                                "attachments"
                            ]
    ...
    def _fetch_threads(
        self, query: str = None, label_ids: list = None, max_results: int = 10
    ) -> Dict[str, Any]:
        """
...
class TrashMessagesTool(Tool):
    ...
    def __call__(
        self, message_ids: list = None, query: str = None, max_results: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for msg_id in target_ids:
                try:
                    ...
                    logger.warning(
                        f"Unexpected error trashing message {msg_id}: {str(e)}"
                    )
                    failed_ids.append({"id": msg_id, "error": str(e)})
...
class ModifyLabelsTool(Tool):
    ...
    def __call__(
        ...
        try:
            ...
            failed = []
            for mid in targets:
                res = self.gmail_base._modify_message_labels(
                    mid, add_labels=add_labels, remove_labels=remove_labels
                )
                if res.get("success"):
    ...
    def __call__(
        ...
        try:
            ...
            message = self.gmail_base._create_message(
                to, subject, body, cc, bcc, is_html, attachments
            )
...
class SearchDraftTool(Tool):
    ...
    def __call__(
        self, draft_id: str = None, keyword: str = None, max_results: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if draft_id:
                ...
                formatted = self.gmail_base._format_message(
                    message, include_attachments=True
                )
            ...
            for d in drafts:
                try:
                    ...
                    formatted = self.gmail_base._format_message(
                        msg, include_attachments=True
                    )

                    # simple case-insensitive search in subject, from, to, snippet, body
                    k = keyword.lower()
                    if any(
                        k in (str(formatted.get(k2, "")).lower())
                        for k2 in ("subject", "from", "to", "snippet")
                    ) or (
                        formatted.get("body") and k in formatted.get("body", "").lower()
                    ):
                ...
                if "parts" in existing_payload:
                    for part in existing_payload["parts"]:
                        if part.get("mimeType") in ["text/plain", "text/html"]:
                            data = part.get("body", {}).get("data", "")
                            if data:
                                final_body = base64.urlsafe_b64decode(data).decode(
                                    "utf-8"
                                )
...
class GmailToolkit(Toolkit):
    ...
    def __init__(
        ...
        gmail_base = GmailBase(
            client_secret_file=client_secret_file, token_file=token_file
        )
