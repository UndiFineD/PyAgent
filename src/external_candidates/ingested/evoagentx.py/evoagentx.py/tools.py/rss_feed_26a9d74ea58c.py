...
class RSSBase(RequestBase):
    ...
    def fetch_rss_feed(
        ...
        try:
            ...
            if feed.bozo:
                logger.warning(
                    f"RSS feed parsing warnings for {feed_url}: {feed.bozo_exception}"
                )
            ...
            # Process entries
            entries = (
                feed.entries[:max_entries] if max_entries is not None else feed.entries
            )

            for entry in entries:
                processed_entry = self._process_entry(
                    entry, feed_url, fetch_webpage_content
                )
    ...
    def _process_entry(
        self, entry, base_url: str, fetch_webpage_content: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if processed_entry["link"] and not processed_entry["link"].startswith(
            ("http://", "https://")
        ):
        ...
        if hasattr(entry, "category"):
            processed_entry["categories"] = (
                [entry.category] if isinstance(entry.category, str) else entry.category
            )
        ...
        processed_entry["description"] = self._clean_text(
...
class RSSFetchTool(Tool):
    ...
    name: str = "rss_fetch"
    description: str = (
        "Fetch and parse RSS feeds from URLs to get latest articles and updates. Use reasonable limits (10-20 entries) unless you specifically need more for comprehensive analysis."
    )
    ...
    def __call__(
        self, feed_url: str, max_entries: int = 10, fetch_webpage_content: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        """
        return self.rss_base.fetch_rss_feed(
            feed_url, max_entries, fetch_webpage_content
        )
...
class RSSValidateTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        "url": {"type": "string", "description": "URL to validate as an RSS feed"}
    }
