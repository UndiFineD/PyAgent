# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\database_postgresql.py
import json
import re
import time
...
class PostgreSQLDatabase(DatabaseBase):
    ...
    def _is_remote_connection(self) -> bool:
        return self.connection_string and (
            "@" in self.connection_string or "postgresql://" in self.connection_string
        )
    ...
    def _init_existing_local_database(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded existing local file-based database from: {self.local_path}"
            )
        except Exception as e:
            ...
            for json_file in self.local_path.glob("*.json"):
                ...
                with open(json_file, "r", encoding="utf-8") as f:
                    ...
                    if not isinstance(loaded_data, list):
                        logger.warning(
                            f"Table {table_name} file contains non-list data: {type(loaded_data)}, converting to empty list"
                        )
    ...
    def _parse_sql_query(self, sql: str) -> Dict[str, Any]:
        ...
        # INSERT
            ...
            match = re.search(
                r"INSERT INTO (\w+) *\((.*?)\) *VALUES", sql, re.IGNORECASE | re.DOTALL
            )
            if match:
                table = match.group(1).lower()
        ...
        # SELECT - Enhanced to support JOINs
            ...
            if "JOIN" in upper_sql:
                # Parse JOIN queries
                match = re.search(
                    r"SELECT (.*?) FROM (\w+)(?:\s+(\w+))?\s+(?:(\w+)\s+)?JOIN\s+(\w+)(?:\s+(\w+))?\s+ON\s+(.*?)(?: WHERE (.*?))?(?: ORDER BY (.*?))?(?: LIMIT (\d+))?",
                    sql,
                    re.IGNORECASE | re.DOTALL,
                )
                if match:
                ...
                # CROSS JOIN support
                    ...
                    match = re.search(
                        r"SELECT (.*?) FROM (\w+)(?:\s+(\w+))?\s+CROSS\s+JOIN\s+(\w+)(?:\s+(\w+))?(?: WHERE (.*?))?(?: ORDER BY (.*?))?(?: LIMIT (\d+))?",
                        sql,
                        re.IGNORECASE | re.DOTALL,
                    )
                    if match:
            ...
            # Simple SELECT (existing logic)
                ...
                match = re.search(
                    r"SELECT (.*?) FROM (\w+)(?: WHERE (.*?))?(?: GROUP BY (.*?))?(?: ORDER BY (.*?))?(?: LIMIT (\d+))?",
                    sql,
                    re.IGNORECASE | re.DOTALL,
                )
                if match:
        ...
        # DELETE
            ...
            match = re.search(
                r"DELETE FROM (\w+)(?: WHERE (.*?))?", sql, re.IGNORECASE | re.DOTALL
            )
            if match:
                table = match.group(1).lower()
                where = match.group(2)
    ...
    def _apply_where_filter(self, rows: List[Dict], where: str) -> List[Dict]:
        ...
        if len(valid_rows) != len(rows):
            logger.warning(
                f"_apply_where_filter: filtered out {len(rows) - len(valid_rows)} non-dict rows"
            )
    ...
    def _apply_column_selection(
        ...
        if len(valid_rows) != len(rows):
            logger.warning(
                f"_apply_column_selection: filtered out {len(rows) - len(valid_rows)} non-dict rows"
            )
    ...
    def _apply_group_by(self, rows: List[Dict], group_by: str) -> List[Dict]:
        ...
        if len(valid_rows) != len(rows):
            logger.warning(
                f"_apply_group_by: filtered out {len(rows) - len(valid_rows)} non-dict rows"
            )
        ...
        for group_val, group_rows in groups.items():
            ...
            salaries = [
                float(r.get("salary", 0))
                for r in group_rows
                if r.get("salary") is not None
            ]
            group_result["avg_salary"] = (
                    ...
                    if str(row1.get(col1, "")) == str(row2.get(col2, "")):
                        ...
                        for col in columns:
                            if "." in col:
                                ...
                                if (
                                    table_alias == parsed.get("alias1")
                                    or table_alias == table1
                                ):
                                    combined_row[col] = row1.get(col_name, "")
                                elif (
                                    table_alias == parsed.get("alias2")
                                    or table_alias == table2
                                ):
    ...
    def _execute_cross_join_query(self, parsed: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            for row1 in rows1:
                ...
                if not isinstance(row1, dict):
                    logger.warning(
                        f"Skipping non-dict row1 in CROSS JOIN: {type(row1)}"
                    )
                    continue
                for row2 in rows2:
                    # Ensure row2 is a dictionary
                    if not isinstance(row2, dict):
                        logger.warning(
                            f"Skipping non-dict row2 in CROSS JOIN: {type(row2)}"
                        )
                        continue
                    # Combine rows
                    combined_row = {}
            ...
            with self.conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                ...
                if isinstance(query, str):
                    ...
                    return self.format_error_result(
                        "Unsupported query format", query_type
                    )
    ...
    def _execute_file_based_query(
        self, query: Union[str, Dict, List], query_type: QueryType = None
    ) -> Dict[str, Any]:
        ...
        try:
            if isinstance(query, str):
                ...
                if not isinstance(parsed, dict) or "type" not in parsed:
                    logger.error(
                        f"_execute_file_based_query: parsed is not a valid dict: {parsed}"
                    )
                    return self.format_error_result(
                        f"Failed to parse SQL query: {query}", query_type
                    )
                ...
                if parsed["type"] == "CREATE":
                    ...
                    if not isinstance(self.tables[table_name], list):
                        logger.warning(
                            f"Reinitializing table {table_name} as list (was {type(self.tables[table_name])})"
                        )
                        self.tables[table_name] = []
                    ...
                    for values in all_values:
                        ...
                        if len(values) != len(columns):
                            logger.warning(
                                f"Skipping invalid row: {values} (expected {len(columns)} values, got {len(values)})"
                            )
                            continue
                    ...
                    if not isinstance(rows, list):
                        logger.warning(
                            f"Table {table_name} contains non-list data: {type(rows)}"
                        )
                        rows = []

                    # Debug logging
                    logger.debug(
                        f"SELECT query: table={table_name}, columns={columns}, where={where}, group_by={group_by}"
                    )
                    logger.debug(
                        f"Rows from table: {type(rows)}, length={len(rows) if isinstance(rows, list) else 'N/A'}"
                    )
                    if isinstance(rows, list) and rows:
                        logger.debug(
                            f"First row type: {type(rows[0])}, content: {rows[0]}"
                        )
                    ...
                    if (
                        isinstance(cross_join_result, dict)
                        and "error" in cross_join_result
                    ):
                    ...
                    for r in rows:
                        ...
                        if not isinstance(r, dict):
                            logger.warning(
                                f"Skipping non-dict row in UPDATE: {type(r)}"
                            )
                            continue
                        match = True
                        if where:
                    ...
                    return self.format_error_result(
                        "Unsupported query type in file-based mode", query_type
                    )
                execution_time = time.time() - start_time
                return self.format_query_result(
                ...
                return self.format_error_result(
                    "Unsupported query format in file-based mode", query_type
                )
        except Exception as e:
            execution_time = time.time() - start_time
        ...
        try:
            ...
            if self.file_based_mode:
                if collection_name in self.tables:
                    ...
                    return self.format_error_result(
                        f"Table {collection_name} not found"
                    )
            else:
                if self.conn is None:
                    return self.format_error_result("PostgreSQL server not available")
            ...
            if self.file_based_mode:
                if collection_name:
                    if collection_name in self.tables:
                        schema = {"id": "integer"}
                        return self.format_query_result(
                            {"table_name": collection_name, "schema": schema},
                            QueryType.SELECT,
                        )
                    else:
                        return self.format_error_result(
                ...
                with self.conn.cursor(
                    ...
                    if collection_name:
                        ...
                        columns = cur.fetchall()
                        schema = {
                            col["column_name"]: col["data_type"] for col in columns
                        }
                        return self.format_query_result(
                            {"table_name": collection_name, "schema": schema},
                            QueryType.SELECT,
...
class PostgreSQLExecuteTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        ...
        "query_type": {
            "type": "string",
            "description": "Type of query (select, insert, update, delete, create, drop, alter, index) - auto-detected if not provided",
        },
    }
    ...
    def __call__(self, query: str, query_type: str = None) -> Dict[str, Any]:
        try:
            if not self.database:
                return {
                    "success": False,
                    "error": "PostgreSQL database not initialized",
                    "data": None,
                }
            ...
            result = self.database.execute_query(
                query=query, query_type=query_type_enum
            )
            return result
        except Exception as e:
...
class PostgreSQLFindTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        ...
        "where": {
            "type": "string",
            "description": "WHERE clause (optional, e.g., 'age > 18')",
        },
        "columns": {
            "type": "string",
        ...
        "sort": {
            "type": "string",
            "description": "ORDER BY clause (optional, e.g., 'age ASC')",
        },
    }
...
class PostgreSQLUpdateTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        ...
        "set": {
            "type": "string",
            "description": "SET clause (e.g., 'status = 'active'')",
        },
...
class PostgreSQLCreateTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        "query": {
            "type": "string",
            "description": "CREATE statement (e.g., CREATE TABLE ...)",
        }
    }
    required: Optional[List[str]] = ["query"]
...
class PostgreSQLInfoTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        "info_type": {
            "type": "string",
            "description": "Type of information (database, tables, table, schema, capabilities)",
        },
        "table_name": {
            "type": "string",
            "description": "Table name for table-specific info (optional)",
        },
    }
    ...
    def __call__(
        self, info_type: str = "database", table_name: str = None
    ) -> Dict[str, Any]:
        try:
            if not self.database:
                return {
