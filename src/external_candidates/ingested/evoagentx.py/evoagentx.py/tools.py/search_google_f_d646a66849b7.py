...
class SearchGoogleFree(SearchBase):
    ...
    def search(
        self, query: str, num_search_pages: int = None, max_content_words: int = None
    ) -> Dict[str, Any]:
        """
...
class GoogleFreeSearchTool(Tool):
    ...
    description: str = (
        "Search Google without requiring an API key and retrieve content from search results"
    )
    ...
    def __call__(
        ...
        try:
            return self.search_google_free.search(
                query, num_search_pages, max_content_words
            )
        except Exception as e:
            return {
                "results": [],
