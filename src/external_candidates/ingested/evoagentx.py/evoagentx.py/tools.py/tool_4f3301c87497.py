    ...
    def validate_attributes(cls):
        ...
        for input_name, input_content in cls.inputs.items():
            ...
            if "type" not in input_content or "description" not in input_content:
                raise ValueError(
                    f"Input '{input_name}' must have 'type' and 'description'"
                )
            if input_content["type"] not in ALLOWED_TYPES:
                raise ValueError(
                    f"Input '{input_name}' must have a valid type, should be one of {ALLOWED_TYPES}"
                )
            ...
            if (
                call_signature.parameters[input_name].annotation
                != json_to_python[input_content["type"]]
            ):
                raise ValueError(
                    f"Input '{input_name}' has a type mismatch in __call__"
                )
