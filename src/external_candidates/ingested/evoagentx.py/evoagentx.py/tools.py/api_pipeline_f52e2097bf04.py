# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\api_pipeline.py
import glob
import json
import os
...
from evoagentx.rag.schema import ChunkMetadata, Corpus, Document, Query, TextChunk
...
store_config = StoreConfig(
    dbConfig=DBConfig(
        db_name="sqlite", path="./evoagentx/tools/api_pool/data/api_cache.db"
    ),
...
rag_config = RAGConfig(
    embedding=EmbeddingConfig(
        provider="openai", model_name="text-embedding-3-small", api_key=OPENAI_API_KEY
    ),
...
for i, doc in enumerate(documents):
    chunk = TextChunk(text=doc.text, metadata=doc.metadata, chunk_id=doc.doc_id)
    chunks.append(chunk)
    if (i + 1) % 10 == 0:
...
# 将索引保存到磁盘
rag_engine.save(
    output_path="./evoagentx/tools/api_pool/data/indexing",
    corpus_id="api_pool_corpus",
    index_type="vector",
)
...
def get_api_spec_by_title(
    api_title: str, api_pool_dir: str = "./evoagentx/tools/api_pool"
) -> Dict[str, Any]:
    json_files = glob.glob(os.path.join(api_pool_dir, "*.json"))
    ...
    for path, methods in paths.items():
        for method, operation in methods.items():
            if (
                ...
                and operation.get("operationId") == function_name
            ):
                return {
                    "api_title": api_title,
        ...
        if "error" not in details:
            ...
            if details["parameters"]:
                ...
                for param in details["parameters"]:
                    required = (
                        "(必需)"
                        if param.get("required") in ["true", True]
                        else "(可选)"
                    )
                    print(
                        f"    - {param.get('name')} {required}: {param.get('description', '')}"
                    )
        print()
