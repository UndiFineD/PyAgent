...
class DDGSSearchTool(Tool):
    ...
    description: str = (
        "Search using DDGS (Dux Distributed Global Search) which aggregates results from multiple search engines including DuckDuckGo, Google, Bing, and others"
    )
    ...
    def __call__(
        ...
        try:
            return self.search_ddgs.search(
                query, num_search_pages, max_content_words, backend, region
            )
        except Exception as e:
