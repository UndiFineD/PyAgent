# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\search_wiki.py
...
class SearchWiki(SearchBase):
    ...
    max_summary_sentences: Optional[int] = Field(
        default=None,
        description="Maximum number of sentences in the summary. Default None means return all available content.",
    )
    ...
    def search(
        ...
        try:
            logger.info(
                f"Searching wikipedia: {query}, num_results={num_search_pages}, max_content_words={max_content_words}, max_summary_sentences={max_summary_sentences}"
            )
                ...
                try:
                    ...
                    if max_summary_sentences is not None and max_summary_sentences > 0:
                        summary = wikipedia.summary(
                            title, sentences=max_summary_sentences
                        )
                    else:
                    ...
                    display_content = self._truncate_content(
                        page.content, max_content_words
                    )
...
class WikipediaSearchTool(Tool):
    ...
    def __call__(
        ...
        try:
            return self.search_wiki.search(
                query, num_search_pages, max_content_words, max_summary_sentences
            )
        except Exception as e:
            return {
                "results": [],
...
class WikipediaSearchToolkit(Toolkit):
    def __init__(
        ...
        **kwargs,
        ...
        tools = [WikipediaSearchTool(search_wiki=search_wiki)]
