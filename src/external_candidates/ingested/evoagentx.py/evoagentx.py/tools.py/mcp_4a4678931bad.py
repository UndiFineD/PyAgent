# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\mcp.py
"""
...
class MCPClient:
    ...
    def create_tool(
        ...
        tool_collection = Toolkit(
            name=next(iter(config.get("mcpServers").keys())), tools=all_tools
        )
    ...
    def get_toolkits(self) -> List[Toolkit]:
        ...
        return [
            ...
            for session, tools, config in zip(
                self.sessions, self.mcp_tools, self.server_configs
            )
        ]
    ...
    def __init__(
        ...
        for server in self.servers:
            try:
                ...
                logger.warning(
                    f"Timeout connecting to MCP servers: {str(e)}. Some tools may not be available."
                )
    ...
    def get_toolkits(self) -> List[Toolkit]:
        ...
        for server in self.servers:
            try:
                ...
                if thread.is_alive():
                    logger.warning(
                        "Timeout getting tools from MCP server after 30 seconds"
                    )
                    continue
