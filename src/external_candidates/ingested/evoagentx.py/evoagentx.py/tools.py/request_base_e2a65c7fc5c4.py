# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\request_base.py
import json
import os
import time
...
class RequestBase(BaseModule):
    ...
    def save_content(
        ...
        try:
            ...
            elif content_type.lower() in ["pdf", "binary"] or isinstance(
                content, bytes
            ):
    ...
    def request_and_process(
        ...
        try:
            ...
            if return_raw:
                ...
                if "json" in content_type:
                    try:
                        ...
                        result["content"] = response.text
                        result["warning"] = (
                            "Content-Type indicates JSON but parsing failed"
                        )
                else:
            ...
            if save_file_path:
                save_success = self._save_response_content(
                    response, save_file_path, content_type
                )
                result["saved_to_file"] = save_file_path if save_success else None
                if not save_success:
                    result["save_warning"] = (
                        f"Failed to save content to {save_file_path}"
                    )
    ...
    def _save_response_content(
        self, response: requests.Response, file_path: str, content_type: str
    ) -> bool:
        """
