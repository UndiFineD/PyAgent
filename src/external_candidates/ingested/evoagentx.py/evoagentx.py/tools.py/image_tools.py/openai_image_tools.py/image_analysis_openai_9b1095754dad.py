# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\image_tools\openai_image_tools\image_analysis_openai.py
...
class OpenAIImageAnalysisTool(Tool):
    ...
    description: str = (
        "Simple image analysis via OpenAI Responses API (input_text + input_image)."
    )

    inputs: Dict[str, Dict[str, str]] = {
        "prompt": {
            "type": "string",
            "description": "User question/instruction. Required.",
        },
        "image_url": {
            "type": "string",
            "description": "HTTP(S) image URL. Optional if image_path provided.",
        },
        "image_path": {
            "type": "string",
            "description": "Local image path; converted to data URL internally.",
        },
        "model": {
            "type": "string",
            "description": "OpenAI model for responses.create (e.g., gpt-4o-mini, gpt-4.1, gpt-5). Optional.",
        },
    }
    ...
    def __call__(
        ...
        try:
            ...
            if text is None:
                ...
                try:
                    choices = getattr(response, "output", None) or getattr(
                        response, "choices", None
                    )
                    if choices and isinstance(choices, list):
                        first = choices[0]
                        text = (
                            getattr(first, "message", {}).get("content", "")
