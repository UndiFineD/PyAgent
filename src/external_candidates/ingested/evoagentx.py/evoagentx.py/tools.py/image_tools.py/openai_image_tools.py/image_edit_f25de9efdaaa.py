...
class OpenAIImageEditTool(Tool):
    ...
    description: str = (
        "Edit images using OpenAI gpt-image-1 (direct, minimal validation)."
    )
        ...
        try:
            ...
            try:
                ...
                api_kwargs = {
                    ...
                    "prompt": prompt,
                    "image": (
                        opened_images if len(opened_images) > 1 else opened_images[0]
                    ),
                }
                if size is not None:
        ...
        try:
            ...
            if not result["success"]:
                raise FileNotFoundError(
                    f"Could not read image {image_path}: {result.get('error', 'Unknown error')}"
                )
            ...
            with Image.open(BytesIO(content)) as img:
                ...
                if result["success"]:
                    ...
                    temp_path = os.path.join(
                        "workplace", "images", "temp_rgba_image.png"
                    )
