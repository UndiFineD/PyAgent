# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\image_tools\openai_image_tools\openai_utils.py
"""
...
def validate_parameter(
    model: str, param: str, value: any, operation: str = "generation"
) -> Tuple[bool, str]:
    ...
    if param == "size" and value not in config.get("size_options", []):
        return (
            False,
            f"Invalid size '{value}' for {model}. Supported: {config['size_options']}",
        )
    if param == "quality" and value not in config.get("quality_options", []):
        return (
            False,
            f"Invalid quality '{value}' for {model}. Supported: {config['quality_options']}",
        )
    if param == "style" and value not in config.get("style_options", []):
        return (
            False,
            f"Invalid style '{value}' for {model}. Supported: {config['style_options']}",
        )
    if param == "background" and value not in config.get("background_options", []):
        return (
            False,
            f"Invalid background '{value}' for {model}. Supported: {config['background_options']}",
        )
    if param == "moderation" and value not in config.get("moderation_options", []):
        return (
            False,
            f"Invalid moderation '{value}' for {model}. Supported: {config['moderation_options']}",
        )
    if param == "input_fidelity" and value not in config.get(
        "input_fidelity_options", []
    ):
        return (
            False,
            f"Invalid input_fidelity '{value}' for {model}. Supported: {config['input_fidelity_options']}",
        )
    if param == "output_format" and value not in config.get(
        "output_format_options", []
    ):
        return (
            False,
            f"Invalid output_format '{value}'. Supported: {config['output_format_options']}",
        )
...
def validate_parameters(
    model: str, params: Dict, operation: str = "generation"
) -> Dict:
    validated_params = {}
    warnings = []
    errors = []
