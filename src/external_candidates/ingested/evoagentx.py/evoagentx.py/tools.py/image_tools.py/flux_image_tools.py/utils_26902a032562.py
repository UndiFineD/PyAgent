# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\image_tools\flux_image_tools\utils.py
import base64
...
def file_to_base64(
    path: str, storage_handler: Optional[FileStorageHandler] = None
) -> str:
    ...
    if result["success"]:
        if isinstance(result["content"], bytes):
            ...
            return base64.b64encode(str(result["content"]).encode("utf-8")).decode(
                "utf-8"
            )
    else:
        raise FileNotFoundError(
            f"Could not read file {path}: {result.get('error', 'Unknown error')}"
        )
