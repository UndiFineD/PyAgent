# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\finance_tool.py
import json
import os
...
class FinanceTool(Tool):
    ...
    description: str = (
        f"Retrieve financial price data for stocks, indexes, and crypto. Current date: {datetime.now().strftime('%Y-%m-%d')}."
    )
    ...
    def __init__(
        ...
        super().__init__()
        self.financial_data_api_key = financial_data_api_key or os.getenv(
            "FINANCIAL_DATA_KEY"
        )
        self.openrouter_api_key = openrouter_api_key or os.getenv("OPENROUTER_API_KEY")
        self.stock_symbols = self._load_stock_symbols()
        self.openrouter_client = (
            self._init_openrouter_client() if self.openrouter_api_key else None
        )
    ...
    def __call__(
        ...
        if unit == "minute":
            if (start_dt.year, start_dt.month, start_dt.day) != (
                end_dt.year,
                end_dt.month,
                end_dt.day,
            ):
        ...
        if unit == "daily":
            ...
            if not supports_minute:
                return {
                    "error": "minute data not supported for this symbol, please use daily data"
                }
        ...
        if fmt == "json":
            ...
            while True:
                ...
                if len(data) < step:
                    break
                offset += step
            filtered = self._filter_json(
                items, unit="daily", start_dt=start_dt, end_dt=end_dt
            )
            ...
            while True:
                ...
                body = (
                    chunk_lines[1:]
                    if chunk_lines and chunk_lines[0] == header
                    else chunk_lines
                )
                lines.extend(body)
            ...
            if header is None:
                return {"data": ""}
            filtered_lines = self._filter_csv(
                header, lines, unit="daily", start_dt=start_dt, end_dt=end_dt
            )
        ...
        if fmt == "json":
            ...
            for d in days:
                ...
                while True:
                    ...
                    offset += step
            filtered = self._filter_json(
                all_items, unit="minute", start_dt=start_dt, end_dt=end_dt
            )
            return {"data": filtered}
        else:
            header: Optional[str] = None
            ...
            if header is None:
                return {"data": ""}
            filtered_lines = self._filter_csv(
                header, out_lines, unit="minute", start_dt=start_dt, end_dt=end_dt
            )
            csv_text = "\n".join([header] + filtered_lines)
            return {"data": csv_text}
    ...
    def _load_stock_symbols(self) -> Dict[str, str]:
        ...
        try:
            ...
            json_path = os.path.join(
                current_dir, "financial_materials", "stock_symbols.json"
            )
            ...
            for item in symbols_data:
                if isinstance(item, dict) and "trading_symbol" in item:
                    symbols_dict[item["trading_symbol"].upper()] = item.get(
                        "registrant_name", ""
                    )
    ...
    def _extract_symbol_with_llm(
        self, user_query: str, available_symbols: List[str]
    ) -> Optional[str]:
        """
        ...
        if not self.openrouter_client:
            logger.warning(
                "OpenRouter client not initialized, cannot use LLM to extract stock code"
            )
            return None
...
Please return the stock code directly (e.g., AAPL) or UNKNOWN:"""
            ...
            response = self.openrouter_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional stock code recognition assistant, only return the stock code or UNKNOWN, do not return any other content.",
                    },
            ...
            extracted_symbol = (
                extracted_symbol.replace(".", "").replace(",", "").strip()
            )
            ...
            logger.info(
                f"LLM extracted stock code from query '{user_query}': {extracted_symbol}"
            )
    ...
    def _validate_stock_symbol(self, trading_symbol: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Stock code '{trading_symbol}' not in database, try using LLM to extract..."
        )

        # 2. Use OpenRouter LLM to extract stock code
        extracted_symbol = self._extract_symbol_with_llm(
            user_query=trading_symbol,
            available_symbols=list(self.stock_symbols.keys())[
                :100
        ...
        if extracted_symbol and extracted_symbol in self.stock_symbols:
            ...
            logger.info(
                f"✓ LLM extracted valid stock code: {extracted_symbol} - {company_name}"
            )
        ...
        if matched_symbol:
            ...
            logger.info(
                f"✓ Fuzzy matching successful: {matched_symbol} - {company_name}"
            )
