# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\database_faiss.py
"""
...
from ..rag.schema import Corpus, Document, DocumentMetadata, Query
...
def _ensure_database_path(db_path: str) -> str:
    ...
    if path.exists():
        ...
        try:
            ...
            logger.warning(
                f"Database validation failed: {e}. Will create new database."
            )
            ...
            try:
                ...
                logger.error(
                    f"Failed to remove corrupted database file: {unlink_error}"
                )
                raise ValueError(
                    f"Cannot remove corrupted database file: {unlink_error}"
                )
    else:
...
def _create_default_storage_config(db_path: Optional[str] = None) -> StoreConfig:
    ...
    storage_config = StoreConfig(
        ...
        vectorConfig=VectorStoreConfig(
            vector_name="faiss", dimensions=1536, index_type="flat_l2"
        ),
...
def _create_default_rag_config() -> RAGConfig:
    ...
    """
    from ..rag.rag_config import ChunkerConfig, EmbeddingConfig, RAGConfig

    return RAGConfig(
        embedding=EmbeddingConfig(
            provider="openai", model_name="text-embedding-ada-002"
        ),
        chunker=ChunkerConfig(chunk_size=500, chunk_overlap=50),
    )
...
class FaissDatabase(BaseModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rag_engine = RAGEngine(
            config=rag_config, storage_handler=self.storage_handler
        )
    ...
    def query(
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "Detected running event loop, using thread executor for query"
                )
                ...
                return self._query_sync(
                    query, corpus_id, top_k, similarity_threshold, metadata_filters
                )
    ...
    def _query_sync(
        ...
        try:
            ...
            if corpus_id not in self.rag_engine.indices:
                logger.warning(
                    f"Corpus {corpus_id} not found. Returning empty results."
                )
                return {
                    "success": True,
                    "data": {
            ...
            for i, chunk in enumerate(chunks):
                score = (
                    results.scores[i]
                    if results.scores and i < len(results.scores)
                    else 0.0
                )
                formatted_results["results"].append(
                    {
                        ...
                        "score": score,
                        "metadata": (
                            chunk.metadata.model_dump() if chunk.metadata else {}
                        ),
            ...
            logger.info(
                f"Query executed successfully. Found {len(formatted_results['results'])} results."
            )
    ...
    def _is_file_path(self, text: str) -> bool:
        ...
        return any(
            indicator in text for indicator in path_indicators
        ) and os.path.exists(text)

    def _process_file_path(
        self, file_path: str, doc_index: int, metadata: Optional[Dict[str, Any]] = None
    ) -> List[Document]:
        """
        ...
        try:
            ...
            try:
                ...
                # We're in an event loop, use thread executor to avoid asyncio.run() conflict
                logger.info(
                    f"Detected running event loop, using thread executor for {file_path}"
                )
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        self._process_file_path_sync, file_path, doc_index, metadata
                    )
                    return future.result()
            except RuntimeError:
                # No event loop running, safe to use asyncio.run() in the RAG engine
                logger.info(
                    f"No event loop detected, using direct processing for {file_path}"
                )
    ...
    def _process_file_path_sync(
        ...
        """
        try:
            # Use StorageHandler to read the file if available
            ...
            corpus = self.rag_engine.process_documents(
                [temp_doc], corpus_id=temp_corpus_id
            )
        ...
        try:
            ...
            for i, doc in enumerate(documents):
                if isinstance(doc, str):
                    ...
                    doc_metadata.update(
                        {"doc_index": i, "insertion_time": datetime.now().isoformat()}
                    )
                    # Create DocumentMetadata object
                    document_metadata = DocumentMetadata(**doc_metadata)
                    # Create Document object
            ...
            for i in range(0, len(processed_docs), batch_size):
                ...
                # Add to index
                self.rag_engine.add(
                    self.default_index_type, batch_corpus, corpus_id=corpus_id
                )
                ...
                logger.info(
                    f"Processed batch {i//batch_size + 1}, total processed: {total_processed}"
                )

            # Save index
            self.rag_engine.save(
                corpus_id=corpus_id, index_type=self.default_index_type
            )
            ...
            logger.info(
                f"Successfully inserted {len(documents)} documents into corpus {corpus_id}"
            )
            if file_paths_processed:
                logger.info(
                    f"Processed {len(file_paths_processed)} file paths: {file_paths_processed}"
                )
            ...
            if corpus_id not in self.rag_engine.indices:
                ...
                return {
                    ...
                    "data": {
                        ...
                        "corpus_id": corpus_id,
                        "message": "Corpus not found, nothing to delete",
                    },
                }
        ...
        try:
            ...
            for corpus_id, indices in self.rag_engine.indices.items():
                corpus_info = {
                    ...
                    "retrievers": list(
                        self.rag_engine.retrievers.get(corpus_id, {}).keys()
                    ),
                }
        ...
        try:
            if corpus_id:
                ...
                stats = {
                    ...
                    "index_types": list(
                        self.rag_engine.indices.get(corpus_id, {}).keys()
                    ),
                    "retrievers": list(
                        self.rag_engine.retrievers.get(corpus_id, {}).keys()
                    ),
                ...
                if corpus_id in self.rag_engine.indices:
                    vector_index = self.rag_engine.indices[corpus_id].get(
                        ...
                        try:
                            ...
                            if hasattr(index, "vector_store"):
                                ...
                                if hasattr(vector_store, "faiss_index"):
                                    stats["vector_count"] = (
                                        vector_store.faiss_index.ntotal
                                    )
                ...
                stats = {
                    ...
                    "vector_store_type": (
                        self.rag_engine.storage_handler.storageConfig.vectorConfig.vector_name
                        if self.rag_engine.storage_handler.storageConfig.vectorConfig
                        else None
                    ),
                }
...
class FaissQueryTool(Tool):
    ...
    description: str = (
        "Query the FAISS vector database using semantic search to find relevant documents and chunks"
    )
...
class FaissInsertTool(Tool):
    ...
    description: str = (
        "Insert documents into the FAISS vector database with automatic chunking and embedding. Supports both text content and file paths - if a string looks like a file path and exists, it will automatically read and process the file content."
    )
    inputs: Dict[str, Dict[str, Any]] = {
        "documents": {
            "type": "array",
...
class FaissDeleteTool(Tool):
    ...
    description: str = (
        "Delete documents or chunks from the FAISS vector database. You can delete specific documents by ID, filter by metadata, or clear the entire corpus."
    )
    inputs: Dict[str, Dict[str, Any]] = {
        "corpus_id": {
            "type": "string",
...
class FaissListTool(Tool):
    ...
    name: str = "faiss_list"
    description: str = (
        "List all available corpora and their metadata in the FAISS vector database. This tool takes no parameters."
    )
    inputs: Dict[str, Dict[str, Any]] = {}
    required: Optional[List[str]] = []
...
class FaissStatsTool(Tool):
    ...
    description: str = (
        "Get statistics about the FAISS vector database or a specific corpus. Optionally provide a corpus_id to get stats for a specific corpus."
    )
    inputs: Dict[str, Dict[str, Any]] = {
        "corpus_id": {
            "type": "string",
