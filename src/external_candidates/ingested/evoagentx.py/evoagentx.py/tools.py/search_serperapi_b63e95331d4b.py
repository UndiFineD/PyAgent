# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\search_serperapi.py
import os
...
class SearchSerperAPI(SearchBase):
    ...
    api_key: Optional[str] = Field(
        default=None, description="SerperAPI authentication key"
    )
    default_location: Optional[str] = Field(
        default=None, description="Default geographic location"
    )
    default_language: Optional[str] = Field(
        default="en", description="Default interface language"
    )
    default_country: Optional[str] = Field(
        default="us", description="Default country code"
    )
    enable_content_scraping: Optional[bool] = Field(
        default=True, description="Enable full content scraping"
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.api_key:
            logger.warning(
                "SerperAPI key not found. Set SERPERAPI_KEY environment variable or pass api_key parameter."
            )

    def _build_serperapi_payload(
        self,
        query: str,
        location: str = None,
        language: str = None,
    ...
    def _process_serperapi_results(
        self, serperapi_data: Dict[str, Any], max_content_words: int = None
    ) -> Dict[str, Any]:
        """
                ...
                processed_results.append(
                    {
                        ...
                        "content": self._truncate_content(
                            content, max_content_words or 200
                        ),
            ...
            if (
                ...
                try:
                    ...
                    if scraped_content and scraped_content.strip():
                        ...
                        result["site_content"] = self._truncate_content(
                            scraped_content, max_content_words or 400
                        )
                    else:
        ...
        processed_results.sort(
            key=lambda x: (x.get("priority", 999), x.get("position", 0))
        )
    ...
    def search(
        ...
        try:
            ...
            # Process results
            response_data = self._process_serperapi_results(
                serperapi_data, max_content_words
            )

            logger.info(
                f"Successfully retrieved {len(response_data['results'])} processed results"
            )
...
class SerperAPITool(Tool):
    ...
    description: str = (
        "Search Google using SerperAPI with comprehensive result processing and content scraping"
    )
    ...
    def __call__(
        ...
        try:
            ...
            return {
                "results": [],
                "error": f"Error executing SerperAPI search: {str(e)}",
            }
...
class SerperAPIToolkit(Toolkit):
    def __init__(
        ...
        **kwargs,
        ...
        tools = [SerperAPITool(search_serperapi=search_serperapi)]
