# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\interpreter_docker.py
import io
import shlex
import tarfile
import uuid
...
class DockerInterpreter(BaseInterpreter):
    ...
    require_confirm: bool = Field(
        default=False,
        description="Whether to require confirmation before executing code",
    )
    ...
    host_directory: str = Field(
        default="",
        description="The path to the host directory to use for the container",
    )
    container_directory: str = Field(
        default="/home/app/", description="The directory to use for the container"
    )
    container_command: str = Field(
        default="tail -f /dev/null", description="The command to use for the container"
    )
    tmp_directory: str = Field(
        default="/tmp", description="The directory to use for the container"
    )
    image_tag: Optional[str] = Field(
        default=None, description="The Docker image tag to use"
    )
    dockerfile_path: Optional[str] = Field(
        default=None, description="Path to the Dockerfile to build"
    )
    auto_cleanup: bool = Field(
        default=True,
        description="Whether to automatically cleanup container on cleanup() call",
    )
    auto_destroy: bool = Field(
        default=True,
        description="Whether to automatically cleanup container on object destruction",
    )
    ...
    def __del__(self):
        try:
            if (
                hasattr(self, "auto_destroy")
                and self.auto_destroy
    ...
    def _initialize_if_needed(self):
        ...
        if image_tag:
            ...
            if not dockerfile_path:
                raise ValueError(
                    "dockerfile_path or image_tag must be provided to build the image"
                )

            dockerfile_path = Path(dockerfile_path)
            if not dockerfile_path.exists():
                raise FileNotFoundError(
                    f"Dockerfile not found at provided path: {dockerfile_path}"
                )

            dockerfile_dir = dockerfile_path.parent
            self.client.images.build(
                path=str(dockerfile_dir), tag=image_tag, rm=True, buildargs={}
            )
            ...
            for file_path in host_directory.rglob("*"):
                if file_path.is_file():
                    ...
                    tarinfo = tarfile.TarInfo(
                        name=str(target_path.relative_to(self.container_directory))
                    )
    ...
    def _run_file_in_container(self, file: Path, language: str) -> str:
        ...
        command = shlex.split(
            self.CODE_EXECUTE_CMD_MAPPING[language].format(file_name=file.as_posix())
        )
        if self.container is None:
            raise RuntimeError("Container is not initialized.")
    ...
    def execute(self, code: str, language: str) -> str:
        ...
        if self.host_directory:
            code = (
                f"import sys; sys.path.insert(0, '{self.container_directory}');" + code
            )
    ...
    def execute_script(self, file_path: str, language: str = None) -> str:
        ...
        if result["success"]:
            ...
            raise RuntimeError(
                f"Could not read file '{file_path}': {result.get('error', 'Unknown error')}"
            )
...
class DockerExecuteScriptTool(Tool):
    ...
    description: str = (
        "Execute code from a script file in a secure Docker container environment"
    )
    inputs: Dict[str, Dict[str, str]] = {
        "file_path": {
            "type": "string",
