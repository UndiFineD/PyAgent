...
class ArxivBase(RequestBase):
    ...
    def _parse_atom_response(self, xml_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                "success": True,
                "total_results": (
                    int(total_results.text) if total_results is not None else 0
                ),
                "start_index": int(start_index.text) if start_index is not None else 0,
                "items_per_page": (
                    int(items_per_page.text) if items_per_page is not None else 0
                ),
                "papers": [],
            }
    ...
    def download_pdf(
        self, pdf_url: str, save_path: str, storage_handler: FileStorageHandler = None
    ) -> Dict[str, Any]:
        """
...
class ArxivDownloadTool(Tool):
    ...
    def __init__(
        self, arxiv_base: ArxivBase = None, storage_handler: FileStorageHandler = None
    ):
        super().__init__()
        self.arxiv_base = arxiv_base
...
class ArxivToolkit(Toolkit):
    def __init__(
        self, name: str = "ArxivToolkit", storage_handler: FileStorageHandler = None
    ):
