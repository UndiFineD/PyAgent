# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\telegram_tools.py
"""
...
class TelegramBase(BaseModule):
    ...
    def __init__(
        self, api_id: str = None, api_hash: str = None, phone: str = None, **kwargs
    ):
        """
...
class FetchLatestMessagesTool(Tool):
    ...
    description: str = (
        "Retrieve the most recent messages from a specific Telegram contact by their name. If multiple contacts match the name, it will ask for clarification."
    )
        ...
        async def _fetch_messages():
            ...
            try:
                ...
                if len(matches) > 1:
                    ...
                    clarification_list = [
                        f"{m['name']} (ID: {m['id']})" for m in matches
                    ]
                    return {
                        "success": False,
...
class SearchMessagesByKeywordTool(Tool):
    ...
    description: str = (
        "Find specific information by searching for a keyword within a contact's chat history. If multiple contacts match the name, it will ask for clarification."
    )
    inputs: Dict[str, Dict[str, str]] = {
        "contact_name": {
            "type": "string",
    ...
    def __call__(
        self, contact_name: str, keyword: str, limit: int = 10
    ) -> Dict[str, Any]:
        """
        ...
        async def _search_messages():
            ...
            try:
                ...
                async for message in client.iter_messages(
                    chat, search=keyword, limit=limit
                ):
...
class SendMessageTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        ...
        "message_text": {"type": "string", "description": "The text message to send"},
    }
        ...
        async def _send_message_by_name():
            ...
            try:
                ...
                return {
                    ...
                    "sent_at": (
                        sent_message.date.isoformat() if sent_message.date else None
                    ),
                }
...
class ListRecentChatsTool(Tool):
    ...
    description: str = (
        "Get a list of recent conversations, allowing the agent to ask for clarification if a user's request is ambiguous (e.g., 'Summarize my last chat')."
    )
    inputs: Dict[str, Dict[str, str]] = {
        "limit": {
            "type": "integer",
    ...
    def __call__(self, limit: int = 10) -> Dict[str, Any]:
        ...
        async def _list_chats():
            ...
            try:
                ...
                return {"success": True, "chats_count": len(dialogs), "chats": dialogs}

            except Exception as e:
                logger.error(f"Error listing chats: {str(e)}")
                return {"success": False, "error": f"Failed to list chats: {str(e)}"}
            finally:
                if client:
                    await client.disconnect()
...
class FindAndRetrieveFileTool(Tool):
    ...
    description: str = (
        "Locate a specific file within a contact's chat based on a search query. This tool should return metadata about the file (name, size, type), not download its contents. If multiple contacts match the name, it will ask for clarification."
    )
    inputs: Dict[str, Dict[str, str]] = {
        "contact_name": {
            "type": "string",
            ...
            try:
                ...
                async for message in client.iter_messages(chat):
                    ...
                    if message.document:
                        ...
                        if (
                            not filename_query
                            or filename_query.lower() in filename.lower()
                        ):
                            files.append(
                                {
...
class SummarizeContactMessagesTool(Tool):
    ...
    description: str = (
        "Summarize recent messages from a specific Telegram contact by their name. Provides a summary of the conversation history."
    )
    inputs: Dict[str, Dict[str, str]] = {
        "contact_name": {
            "type": "string",
...
• Recent activity: {'Active' if total_messages > 0 else 'No recent messages'}"""
                ...
                return {
                    ...
                    "recent_messages": messages[
                        :5
                    ],  # Show first 5 messages as examples
                }
...
class DownloadFileTool(Tool):
    ...
    description: str = (
        "Download a file from a Telegram contact by their name. Downloads the file to a local directory."
    )
    inputs: Dict[str, Dict[str, str]] = {
        "contact_name": {
            "type": "string",
    ...
    def __call__(
        self, contact_name: str, filename_query: str, download_dir: str = "downloads"
    ) -> Dict[str, Any]:
        """
        ...
        async def _download_file():
            ...
            try:
                ...
                downloaded_path = await client.download_media(
                    found_message, file=os.path.join(download_dir, filename)
                )
...
class ReadFileContentTool(Tool):
    ...
    description: str = (
        "Read the content of a file from a Telegram contact by their name. Downloads the file and extracts its text content."
    )
    inputs: Dict[str, Dict[str, str]] = {
        "contact_name": {
            "type": "string",
    ...
    def __call__(
        ...
        async def _read_file_content():
            ...
            try:
                ...
                downloaded_path = await client.download_media(
                    found_message, file=os.path.join(temp_dir, unique_filename)
                )
...
class TelegramToolkit(Toolkit):
    ...
    def __init__(
        ...
        tools = [
            ...
            ),  # This is now send_message_by_name
