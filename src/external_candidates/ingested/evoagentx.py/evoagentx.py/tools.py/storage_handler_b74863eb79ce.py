# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\storage_handler.py
import os
import shutil
                    ...
                    for item in current_path.iterdir():
                        ...
                        try:
                            ...
                            item_info = {
                                ...
                                "extension": (
                                    item.suffix.lower() if item.is_file() else ""
                                ),
...
class SupabaseStorageHandler(FileStorageHandler):
    ...
    def __init__(self, bucket_name: str = None, base_path: str = "/", **kwargs):
        ...
        self.bucket_name = (
            bucket_name or os.getenv("SUPABASE_BUCKET_STORAGE") or "default"
        )
        ...
        try:
            ...
            logger.info(
                f"Creating Supabase client with URL: {self.supabase_url[:30]}..."
            )
            self.supabase: Client = create_client(self.supabase_url, self.supabase_key)
            logger.info(
                f"Successfully initialized Supabase client for bucket: {bucket_name}"
            )
        except ImportError:
            raise ImportError(
    ...
    def _initialize_storage(self):
        ...
        try:
            ...
            logger.info(
                f"Successfully connected to Supabase bucket: {self.bucket_name}"
            )
        except Exception as e:
    ...
    def _write_raw(self, path: str, content: bytes, **kwargs) -> bool:
        ...
        try:
            ...
            if response and (
                not isinstance(response, dict) or response.get("error") is None
            ):
        ...
        try:
            ...
            if response and isinstance(response, list):
                for item in response:
                    ...
                    # Calculate full path
                    full_path = (
                        f"{list_path}/{item['name']}" if list_path else item["name"]
                    )
                    ...
                    items.append(
                        {
                            ...
                            "type": (
                                "directory"
                                if item.get("metadata", {}).get("mimetype")
                                == "application/x-directory"
                                else "file"
                            ),
                            "size_bytes": item.get("metadata", {}).get("size", 0),
                            "size_mb": round(
                                item.get("metadata", {}).get("size", 0) / (1024 * 1024),
                                2,
                            ),
