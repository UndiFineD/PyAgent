# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\search_serpapi.py
import os
...
class SearchSerpAPI(SearchBase):
    ...
    api_key: Optional[str] = Field(
        default=None, description="SerpAPI authentication key"
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.api_key:
            logger.warning(
                "SerpAPI key not found. Set SERPAPI_KEY environment variable or pass api_key parameter."
            )
    ...
    def _process_serpapi_results(
        self, serpapi_data: Dict[str, Any], max_content_words: int = None
    ) -> Dict[str, Any]:
        """
        ...
        if knowledge_graph := serpapi_data.get("knowledge_graph", {}):
            if description := knowledge_graph.get("description"):
                ...
                if kg_list := knowledge_graph.get("list", {}):
                    content += "\n\n**Key Information:**"
                    for key, value in list(kg_list.items())[
                        :5
    ...
    def search(
        ...
        try:
            ...
            logger.info(
                f"Searching {search_engine} via SerpAPI: {query}, "
            ...
            # Process results
            response_data = self._process_serpapi_results(
                serpapi_data, max_content_words
            )
...
class SerpAPITool(Tool):
    ...
    description: str = (
        "Search multiple search engines using SerpAPI with comprehensive result processing and content scraping"
    )
...
class SerpAPIToolkit(Toolkit):
    def __init__(
        ...
        **kwargs,
        ...
        tools = [SerpAPITool(search_serpapi=search_serpapi)]
