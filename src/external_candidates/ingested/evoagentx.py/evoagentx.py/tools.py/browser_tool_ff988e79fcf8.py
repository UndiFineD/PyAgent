...
class BrowserBase(BaseModule):
    ...
    timeout: int = Field(
        default=10, description="Default timeout in seconds for browser operations"
    )
    ...
    headless: bool = Field(
        default=False, description="Whether to run the browser in headless mode"
    )
    ...
    def _handle_function_params(
        ...
        for param in function_params:
            ...
            if fn_name == function_name or fn_name in param_mapping.get(
                "alt_names", []
            ):
    ...
    def navigate_to_url(
        self, url: str = None, timeout: int = None, function_params: list = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not page_loaded:
                logger.warning(
                    f"Page load timeout for URL: {url}, but continuing with snapshot"
                )
            ...
            if snapshot_result["status"] == "success":
                return {
                    ...
                    "snapshot": {
                        "interactive_elements": snapshot_result.get(
                ...
                return {
                    ...
                    "title": self.driver.title,
                    "current_url": self.driver.current_url,
                    "snapshot_error": snapshot_result.get(
                        "message", "Unknown error capturing snapshot"
                    ),
                }
    ...
    def find_element(
        self, selector: str, selector_type: str = "css", timeout: int = None
    ) -> Dict[str, Any]:
        """
    ...
    def _extract_element_properties(self, element, selector: str) -> Dict[str, Any]:
        ...
        for attr in ["href", "id", "class"]:
            try:
                ...
                logger.warning(
                    f"Element became stale when trying to get {attr} attribute for {selector}"
                )
            except Exception as e:
                logger.warning(
                    f"Could not get {attr} attribute for {selector}: {str(e)}"
                )
    ...
    def find_multiple_elements(
        ...
        try:
            ...
            for idx, element in enumerate(elements):
                try:
                    element_properties = self._extract_element_properties(
                        element, f"{selector}[{idx}]"
                    )
                    element_properties["index"] = idx
                    elements_properties.append(element_properties)
                except StaleElementReferenceException:
                    logger.warning(
                        f"Element {idx} became stale while extracting properties"
                    )
                except Exception as e:
                    logger.warning(
                        f"Error extracting properties for element {idx}: {str(e)}"
                    )
    ...
    def switch_to_frame(
        self, frame_reference: str, reference_type: str = "index"
    ) -> Dict[str, Any]:
        """
    ...
    def switch_to_window(
        self, window_reference: str, reference_type: str = "index"
    ) -> Dict[str, Any]:
        """
    ...
    def browser_click(
        self, element: str = None, ref: str = None, function_params: list = None
    ) -> Dict[str, Any]:
        """
    ...
    def _classify_element_interactivity(
        ...
        if not is_disabled and is_visible:
            ...
            element_data["editable"] = (
                ...
                (
                    tag_name == "INPUT"
                    and element_data.get("attributes", {}).get("type", "text").lower()
                    in editable_input_types
                )
    ...
    def _process_accessibility_tree(self, accessibility_tree):
        ...
        def extract_elements(node, path="", index=0):
            ...
            current_path = (
                path + "/" + (node.get("name") or node.get("role") or "element")
            )
        ...
        try:
            ...
            return {
                ...
                "interactive_elements": [
                    ...
                    if e.get("interactable") or e.get("editable")
                ],
            }
...
class NavigateToUrlTool(Tool):
    ...
    def __call__(
        self, url: str, timeout: int = None, function_params: list = None
    ) -> Dict[str, Any]:
...
class InputTextTool(Tool):
    ...
    def __call__(
        ...
        try:
            return self.browser_tool.input_text(
                element, ref, text, submit, slowly, function_params
            )
        except Exception as e:
...
class BrowserClickTool(Tool):
    ...
    def __call__(
        self, element: str, ref: str, function_params: list = None
    ) -> Dict[str, Any]:
...
class BrowserSnapshotTool(Tool):
    ...
    description: str = (
        "Capture a fresh snapshot of the current page, including all elements"
    )
    inputs: Dict[str, Dict[str, str]] = {}
    required: Optional[List[str]] = []
