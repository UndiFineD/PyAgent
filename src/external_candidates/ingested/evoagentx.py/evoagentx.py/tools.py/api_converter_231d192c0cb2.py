# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\api_converter.py
import json
...
class APITool(Tool):
    ...
    def __init__(
        ...
        super().__init__(
            name=name, description=description, inputs=inputs, required=required
        )
        ...
        if hasattr(cls, "required") and cls.required:
            for required_input in cls.required:
                if required_input not in cls.inputs:
                    raise ValueError(
                        f"Required input '{required_input}' is not found in inputs"
                    )
...
class BaseAPIConverter(BaseModule, ABC):
    ...
    def _extract_parameters(self, endpoint_config: Dict[str, Any]) -> tuple:
        ...
        for param in parameters:
            ...
            inputs[param_name] = {"type": param_type, "description": param_desc}
...
class OpenAPIConverter(BaseAPIConverter):
    ...
    def _create_tool_from_operation(
        self, path: str, method: str, operation: Dict[str, Any], base_url: str
    ) -> Optional[APITool]:
    ...
    def _create_api_function(self, endpoint_config: Dict[str, Any]) -> Callable:
        ...
        def api_call(**kwargs):
            ...
            parameters = operation.get("parameters", [])
            param_locations = {
                param["name"]: param.get("in", "query") for param in parameters
            }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not rapidapi_key:
            ...
            if not rapidapi_key:
                raise ValueError(
                    "rapidapi_key not provided or RAPIDAPI_KEY environment variable not set"
                )
        if not rapidapi_host:
            raise ValueError(
                "rapidapi_host not provided or RAPIDAPI_HOST environment variable not set"
            )
