# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\storage_base.py
import csv
import json
import os
import pickle
...
class StorageBase(BaseModule, ABC):
    ...
    def list(
        self, path: str = None, max_depth: int = 3, include_hidden: bool = False
    ) -> Dict[str, Any]:
        """List files and directories"""
        try:
            target_path = self.translate_in(path) if path else str(self.base_path)
            items = self._list_raw(
                target_path, max_depth=max_depth, include_hidden=include_hidden
            )
    ...
    def append(self, file_path: str, content: Any, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_extension in self.appendable_formats:
                return self.appendable_formats[file_extension](
                    target_file_path, content, **kwargs
                )
            else:
                return {
                    "success": False,
    ...
    # Text file handlers
    def _save_text(
        self, file_path: str, content: Any, encoding: str = "utf-8", **kwargs
    ) -> Dict[str, Any]:
    ...
    def _read_text(
        self, file_path: str, encoding: str = "utf-8", **kwargs
    ) -> Dict[str, Any]:
    ...
    def _append_text(
        self, file_path: str, content: str, encoding: str = "utf-8", **kwargs
    ) -> Dict[str, Any]:
    ...
    # JSON file handlers
    def _save_json(
        self, file_path: str, content: Any, indent: int = 2, **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(existing_content, list):
                ...
                if isinstance(content, dict):
                    ...
                    return {
                        "success": False,
                        "error": "Cannot append non-dict to JSON dict",
                    }
            else:
    ...
    def _save_csv(self, file_path: str, content: Any, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # If content is a list of lists, use CSV writer
                ...
                return {
                    "success": False,
                    "error": "CSV content must be a string, list of dictionaries, or list of lists",
                }
    ...
    def _append_csv(
        self, file_path: str, content: List[Dict[str, Any]], **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(existing_content, list):
                ...
                if isinstance(content, dict):
                    ...
                    return {
                        "success": False,
                        "error": "Cannot append non-dict to YAML dict",
                    }
            else:
                existing_content = [existing_content]
                if isinstance(content, list):
            ...
            yaml_content = yaml.dump(
                existing_content, default_flow_style=False, allow_unicode=True
            )
            content_bytes = yaml_content.encode("utf-8")
    ...
    # XML file handlers
    def _save_xml(
        self, file_path: str, content: Any, root_tag: str = "root", **kwargs
    ) -> Dict[str, Any]:
    ...
    def _read_excel(
        self, file_path: str, sheet_name: str = None, **kwargs
    ) -> Dict[str, Any]:
    ...
    def _append_excel(
        self, file_path: str, content: List[List[Any]], sheet_name: str = None, **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._exists_raw(file_path):
                return self._save_excel(
                    file_path, content, sheet_name or "Sheet1", **kwargs
                )
    ...
    def _save_pdf(self, file_path: str, content: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": "reportlab library not available for PDF creation",
            }
        except Exception as e:
    ...
    def _save_image(self, file_path: str, content: Any, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(content, "save") and callable(getattr(content, "save", None)):
                ...
                return {
                    "success": False,
                    "error": "Content must be a PIL Image object, binary data, or valid file path",
                }
