# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\optimizers\aflow_optimizer.py
...
from ..utils.aflow_utils.graph_utils import OPERATOR_MAP, GraphUtils
...
class AFlowOptimizer(BaseModule):
    ...
    graph_path: str = Field(
        description="The folder of the workflow graph. This folder must contain a `graph.py` file that defines the workflow structure, and a `prompt.py` file that defines the prompt for the workflow."
    )
    optimized_path: str = Field(
        default=None,
        description="The path to save the optimized workflow. If not provided, the optimized path will be the same as the graph path.",
    )
    initial_round: int = Field(
        default=0,
        description="The round number to start or continue optimization from. If not provided, will start from round 0 using the `graph.py` file in `graph_path`.",
    )
    optimizer_llm: BaseLLM = Field(
        default=None, description="The LLM to use for optimization."
    )
    executor_llm: BaseLLM = Field(
        default=None, description="The LLM to use for execution."
    )

    operators: List[str] = Field(
        default_factory=lambda: list(OPERATOR_MAP.keys()),
        description="The operators to use for optimization. If not provided, will use all operators in OPERATOR_MAP.",
    )
    sample: int = Field(
        default=4, description="The number of rounds to sample from the top scores."
    )
    max_rounds: int = Field(
        default=20, description="The maximum number of rounds to optimize the workflow."
    )
    validation_rounds: int = Field(
        default=5,
        description="Run the workflow for `validation_rounds` times to evaluate the performance on the validation set.",
    )
    eval_rounds: int = Field(
        default=3,
        description="Run the workflow for `eval_rounds` times to evaluate the performance on the test set.",
    )
    check_convergence: bool = Field(
        default=True, description="Whether to check for convergence."
    )
    ...
    def init_module(self, **kwargs):
        ...
        if self.round == 0:
            ...
            shutil.copy2(
                os.path.join(self.graph_path, "graph.py"),
                os.path.join(round_zero_path, "graph.py"),
            )
            shutil.copy2(
                os.path.join(self.graph_path, "prompt.py"),
                os.path.join(round_zero_path, "prompt.py"),
            )
            self.graph_utils.update_prompt_import(
                os.path.join(round_zero_path, "graph.py"), round_zero_path
            )
    ...
    def optimize(self, benchmark: Benchmark):
        ...
        for _ in range(self.max_rounds):
            ...
            if self.round >= self.max_rounds:
                logger.info(
                    f"Max rounds reached: {self.max_rounds}, stopping optimization."
                )
                break
    ...
    async def _optimize_graph(self) -> float:
        ...
        if self.round == 0:
            self.avg_score = await self._handle_initial_round(
                graph_path, validation_n, data
            )

        return await self._handle_optimization_round(graph_path, validation_n, data)

    async def _handle_initial_round(
        self, graph_path: str, validation_n: int, data: list
    ) -> float:
        ...
        return await self.evaluation_utils.evaluate_graph_async(
            self, validation_n, data, initial=True
        )

    async def _handle_optimization_round(
        self, graph_path: str, validation_n: int, data: list
    ) -> float:
        ...
        while True:
            ...
            operator_description = self.graph_utils.load_operators_description(
                self.operators, self.optimizer_llm
            )
            ...
            response = await self.optimizer_llm.async_generate(
                prompt=graph_optimize_prompt, parse_mode="str"
            )
            print(response.content)
            try:
                parsed_response = GraphOptimizeOutput.parse(
                    response.content, parse_mode="xml"
                )
                response = parsed_response.get_structured_data()
            except Exception:
                response = self._parse_optimizer_llm_output(
                    response.content, orig_graph=graph[0], orig_prompt=prompt
                )

            if self.experience_utils.check_modification(
                processed_experience, response["modification"], sample["round"]
            ):
                break

        # Save and evaluate results
        avg_score = await self._evaluate_and_save_optimization_results(
            directory, response, sample, data, validation_n
        )
        return avg_score
    ...
    def _parse_optimizer_llm_output(
        self, content: str, orig_graph: str, orig_prompt: str
    ) -> dict:
    ...
    async def _evaluate_and_save_optimization_results(
        self,
        directory: str,
        response: dict,
        sample: dict,
        ...
        avg_score = await self.evaluation_utils.evaluate_graph_async(
            self, validation_n, data, initial=False
        )
    ...
    async def _run_test(self, test_rounds: List[int]):
        ...
        for round in test_rounds:
            ...
            score, avg_cost, total_cost = (
                await self.evaluation_utils.evaluate_graph_test_async(self)
            )
            scores.append(score)
            ...
            logger.info(
                f"Test round {round} score: {score}, avg_cost: {avg_cost}, total_cost: {total_cost}"
            )
