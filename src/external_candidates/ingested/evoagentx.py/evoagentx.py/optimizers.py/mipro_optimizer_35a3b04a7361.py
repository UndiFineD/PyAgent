# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\optimizers\mipro_optimizer.py
# -----------------------------------------------------------------------------
...
from ..prompts.template import MiproPromptTemplate, PromptTemplate
...
class MiproEvaluator:
        ...
        if len(score_dict) == 1:
            ...
            with self._log_lock:
                if self._log_counter == 0:
                    logger.info(
                        f"`{type(self.benchmark)}.evaluate` returned a dictionary of scores, but no metric name was provided. Will return the average score across all metrics."
                    )
                    self._log_counter += 1
            return avg_score
    ...
    def metric(self, example: dspy.Example, prediction: Any, *args, **kwargs):
        ...
        if isinstance(self.benchmark.get_train_data()[0], dspy.Example):
            ...
            score = self.benchmark.evaluate(
                prediction=prediction, label=self.benchmark.get_label(example)
            )
            ...
            score = self.benchmark.evaluate(
                ...
                label=self.benchmark.get_label(
                    example.toDict()
                ),  # convert the dspy.Example to a dict
            )
        else:
            raise ValueError(
                f"Unsupported example type in `{type(self.benchmark)}`! Expected `dspy.Example` or `dict`, got {type(self.benchmark.get_train_data()[0])}"
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        return_all_scores = (
            kwargs.get("return_all_scores", None) or self.return_all_scores
        )
        ...
        def process_item(example):
            ...
            try:
                if not isinstance(example, dspy.Example):
                    raise ValueError(
                        f"Example from benchmark must be a dspy.Example object, got {type(example)}"
                    )
        ...
        results = [
            ...
            for example, (prediction, score) in zip(evalset, results)
        ]
...
class MiproOptimizer(BaseOptimizer, MIPROv2):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        BaseOptimizer.__init__(
            self, registry=registry, program=program, evaluator=evaluator
        )
    ...
    def _validate_program(self, program: Callable):
        ...
        if not hasattr(program, "save"):
            ...
            logger.warning(
                "program does not have a `save(path=...)` method, will use the default save method in dspy.Module"
            )
        else:
        ...
        if not hasattr(program, "load"):
            ...
            logger.warning(
                "program does not have a `load(path=...)` method, will use the default load method in dspy.Module"
            )
        else:
    ...
    def _validate_evaluator(
        ...
        if evaluator is None:
            if not hasattr(benchmark, "evaluate"):
                raise ValueError(
                    "`evaluator` is not provided and the benchmark does not have a `evaluate` method."
                )
            logger.info(
                "`evaluator` is not provided. Will construct a default evaluator using the `evaluate` method in the benchmark."
            )
        ...
        if not callable(evaluator):
            raise TypeError(
                "evaluator must be callable, i.e., a function or a class with interface `__call__(program, evalset, *kwargs) -> float`"
            )

        # Check if evaluator has __call__ method with correct signature

        sig = inspect.signature(
            evaluator.__call__ if hasattr(evaluator, "__call__") else evaluator
        )
        params = list(sig.parameters.keys())

        if len(params) < 2:
            raise ValueError(
                "evaluator must accept at least two parameters (program and evalset)"
            )
        ...
        if not hasattr(evaluator, "metric"):
            raise ValueError(
                "evaluator must have a `metric(example: dspy.Example, prediction: Any) -> float/int/bool` method"
            )
        ...
        if len(metric_params) < 2:
            raise ValueError(
                "evaluator.metric must accept at least two parameters (example and prediction)"
            )

        if metric_params[0] != "example" or metric_params[1] != "prediction":
            raise ValueError(
                "evaluator.metric must have parameters in order: example, prediction"
            )
        ...
        def wrapped_evaluator(*args, **kwargs):
            ...
            if not isinstance(result, (float, int, bool)):
                raise TypeError(
                    f"evaluator must return a float, int, or bool, got {type(result)}"
                )
    ...
    def optimize(self, dataset: Benchmark, metric_name: Optional[str] = None, **kwargs):
        ...
        if self.auto is None and (
            ...
            raise ValueError(
                f"If auto is None, max_steps must also be provided. Given num_candidates={self.num_candidates}, we'd recommend setting max_steps to ~{self._set_num_trials_from_num_candidates(self.model, zeroshot_opt, self.num_candidates)}."
            )
        ...
        if self.auto is not None and (
            ...
            raise ValueError(
                "If auto is not None, num_candidates and max_steps cannot be set, since they would be overrided by the auto settings. Please either set auto to None, or do not specify num_candidates and max_steps."
            )
        ...
        evaluator = self._validate_evaluator(
            evaluator=self.evaluator, benchmark=dataset, metric_name=metric_name
        )
        self.metric = evaluator.metric

        # Step 1: Bootstrap few-shot examples
        demo_candidates = self._bootstrap_fewshot_examples(
            program, trainset, seed, teacher=None
        )
    ...
    def _get_input_keys(self, dataset: Benchmark) -> Optional[List[str]]:
        ...
        if hasattr(dataset, "get_input_keys"):
            ...
            if isinstance(candidate_input_keys, (list, tuple)) and all(
                isinstance(key, str) for key in candidate_input_keys
            ):
    ...
    def _set_and_validate_datasets(self, dataset: Benchmark):
        ...
        if not trainset:
            raise ValueError(
                "No training data found in the dataset. Please set `_train_data` in the benchmark."
            )
        if trainset and not isinstance(trainset[0], (dict, dspy.Example)):
            raise ValueError(
                "Training set in the benchmark must be a list of dictionaries or dspy.Example objects."
            )
        ...
        if not valset:
            if len(trainset) < 2:
                raise ValueError(
                    "Training set in the benchmark must have at least 2 examples if no validation set is provided."
                )
            ...
            if len(valset) < 1:
                raise ValueError(
                    "Validation set in the benchmark must have at least 1 example."
                )
        ...
        if input_keys is None:
            logger.warning(
                "`get_input_keys` is not implemented in the benchmark. Will use all keys as input keys. This may cause unexpected behavior if the program does not use all the keys."
            )
            input_keys = trainset[0].keys()

        dspy_trainset = self._convert_benchmark_data_to_dspy_examples(
            trainset, input_keys
        )
    ...
    def _convert_benchmark_data_to_dspy_examples(
        self, data: List[dict], input_keys: List[str]
    ) -> List[dspy.Example]:
        """
        ...
        dspy_examples = [
            (
                ...
                if isinstance(example, dspy.Example)
                else dspy.Example(**example).with_inputs(*input_keys)
            )
            for example in data
        ]
    ...
    def _optimize_prompt_parameters(
        ...
        default_score = self.evaluate(
            evalset=valset, program=program, evaluator=evaluator, batch_size=len(valset)
        )
        ...
        default_params = {
            f"{i}_predictor_instruction": 0 for i in range(len(program.predicts))
        }
        if demo_candidates:
            default_params.update(
                {f"{i}_predictor_demos": 0 for i in range(len(program.predicts))}
            )
    ...
    def _log_minibatch_eval(
        ...
        trial_logs[trial_num]["mb_program_path"] = save_candidate_program(
            candidate_program, self.save_path, trial_num=trial_num, note="mb"
        )
    ...
    def evaluate(
        ...
        batch_size: Optional[
            int
        ] = None,  # if provided, sample `batch_size` examples from the evalset
        **kwargs,
    ):
        ...
        if evalset is None:
            assert (
                dataset is not None
            ), "Either `evalset` or `dataset` must be provided."
            data_map = {
            ...
            evalset = self._convert_benchmark_data_to_dspy_examples(
                evaldata, input_keys
            )
...
def eval_candidate_program(
    ...
    try:
        ...
        logger.error(
            f"An exception occurred during evaluation: {str(e)}", exc_info=True
        )
        if return_all_scores:
...
class WorkFlowGraphProgram:
    ...
    def __call__(self, **input_data):
        ...
        if use_teacher:
            ...
            new_graph, new_agent_manager = self.inject_teacher_settings(
                new_graph, self.agent_manager
            )
            workflow = WorkFlow(
                llm=self.executor_llm, graph=new_graph, agent_manager=new_agent_manager
            )
        else:
            # use the original executor llm to execute the graph
            workflow = WorkFlow(
                llm=self.executor_llm, graph=new_graph, agent_manager=self.agent_manager
            )
        ...
        execution_data = {
            k: v for k, v in all_execution_data.items() if k in all_input_output_keys
        }

        return output, execution_data

    def inject_teacher_settings(
        self, graph: WorkFlowGraph, agent_manager: AgentManager
    ):
        """
        ...
        new_agent_manager.add_agents_from_workflow(
            graph, llm_config=optimizer_llm_config
        )
...
class MiproEvaluatorWrapper(MiproEvaluator):
    ...
    def __call__(
        self, program: PromptTuningModule, evalset: List[dspy.Example], **kwargs
    ) -> float:
        ...
        if isinstance(program, PromptTuningModule):
            ...
            raise ValueError(
                f"Invalid program type: {type(program)}. Must be PromptTuningModule or WorkFlowGraphProgram."
            )

        self.evaluator._evaluation_records.clear()

        # update agents
        self.evaluator.agent_manager.update_agents_from_workflow(
            workflow_graph=graph, llm_config=self.evaluator.llm.config, **kwargs
        )
        ...
        for example in data:
            ...
            if evaluation_record is None:
                ...
                example_score = (
                    self._extract_score_from_dict(example_metrics)
                    if isinstance(example_metrics, dict)
                    else example_metrics
                )
...
class WorkFlowMiproOptimizer(MiproOptimizer):
    ...
    def _validate_graph_compatibility(self, graph: WorkFlowGraph):
        ...
        for node in graph.nodes:
            if len(node.agents) > 1:
                raise ValueError(
                    "WorkFlowMiproOptimizer only supports workflows where every node only has a single agent."
                )
            else:
                agent = node.agents[0]
                if not isinstance(agent, dict):
                    raise ValueError(
                        f"Unsupported agent type {type(agent)}. Expected 'dict'."
                    )
                else:
                    if "actions" in agent:
                        ...
                        if len(non_ContextExtraction_actions) > 1:
                            raise ValueError(
                                f"WorkFlowMiproOptimizer only supports workflows where every agent only has a single action. {agent['name']} has {len(non_ContextExtraction_actions)} actions."
                            )
                        # if "prompt_template" not in non_ContextExtraction_actions[0]:
                        if (
                            non_ContextExtraction_actions[0].get(
                            ...
                            logger.warning(
                                f"{agent['name']} does not have a MiproPromptTemplate, its prompt will not be optimized."
                            )
                        else:
                            prompt_template = non_ContextExtraction_actions[0][
                                "prompt_template"
                            ]
                            if isinstance(prompt_template, dict):
                                prompt_template = PromptTemplate.from_dict(
                                    prompt_template
                                )
                            if isinstance(prompt_template, MiproPromptTemplate):
                                ...
                                non_ContextExtraction_actions[0][
                                    "prompt_template"
                                ] = prompt_template
                            else:
                                logger.warning(
                                    f"{agent['name']} has a non-MiproPromptTemplate, its prompt will not be optimized. You should use `MiproPromptTemplate` to define the optimizable prompt."
                                )
                    else:
    ...
    def _validate_evaluator(
        ...
        if evaluator and isinstance(evaluator, Evaluator):
            ...
            evaluator = MiproEvaluatorWrapper(
                evaluator=evaluator, benchmark=benchmark, metric_name=metric_name
            )
        ...
        for i, node in enumerate(workflow_graph.nodes):
            ...
            if "actions" in agent:
                ...
                for j, action in enumerate(agent["actions"]):
                    ...
                    if action_prompt_template and isinstance(
                        action_prompt_template, MiproPromptTemplate
                    ):
                        registry.track(
                            root_or_obj=program,
