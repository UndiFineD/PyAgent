# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\optimizers\engine\registry.py
import copy
import re
import warnings
...
def safe_deepcopy(obj):
    ...
    try:
        ...
        warnings.warn(
            f"Failed to deepcopy {obj.__class__.__name__}. Falling back to advanced handling."
        )
    ...
    try:
        ...
        warnings.warn(
            f"Failed to create a blank instance of {obj.__class__.__name__}. Falling back to reference."
        )
    ...
    for attr, value in getattr(obj, "__dict__", {}).items():
        try:
            ...
            try:
                warnings.warn(
                    f"Failed to copy {attr} of {obj.__class__.__name__}. Falling back to shallow copy."
                )
                setattr(new_instance, attr, copy.copy(value))  # shallow copy
            except Exception:
                warnings.warn(
                    f"Failed to copy {attr} of {obj.__class__.__name__}. Falling back to reference."
                )
