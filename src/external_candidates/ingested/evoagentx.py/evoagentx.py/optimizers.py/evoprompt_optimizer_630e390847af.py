# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\optimizers\evoprompt_optimizer.py
# -----------------------------------------------------------------------------
...
[Your paraphrased version here]""",
            ...
            inputs=[
                {
                    "name": "instruction",
                    "type": "string",
                    "description": "The instruction to paraphrase.",
                },
            ],
            outputs=[
                {
                    ...
                    "type": "string",
                    "description": "The paraphrased instruction.",
                }
            ],
            parse_mode="title",
        )
        ...
        with open(filepath, "w", newline="", encoding="utf-8") as f:
            ...
            for node_name in self.node_populations.keys():
                ...
                sorted_indices = sorted(
                    range(len(node_scores)), key=lambda i: node_scores[i], reverse=True
                )
                ...
                for rank, idx in enumerate(sorted_indices, 1):
                    ...
                    status = (
                        ...
                        if rank == 1
                            ...
                            "Survivor" if rank <= self.population_size else "Eliminated"
                        )
                    )
    ...
    def _log_detailed_evaluation(
        ...
        with open(filepath, "w", newline="", encoding="utf-8") as f:
            ...
            for combo_id, (combination, avg_score) in enumerate(
                zip(combinations, combination_scores)
            ):
                try:
                    ...
                    logger.error(
                        f"Error logging evaluation for combination {combo_id}: {e}"
                    )
    ...
    def _plot_and_save_performance_graph(self, algorithm_name: str):
        ...
        all_gen_keys = set(self.best_scores_per_gen.keys()) | set(
            self.best_combo_scores_per_gen.keys()
        )
        generations = sorted(
            [
                int(re.search(r"\d+", gen).group())
                for gen in all_gen_keys
        ...
        if self.best_combo_scores_per_gen:
            combo_best = [
                self.best_combo_scores_per_gen.get(f"Gen_{g}") for g in generations
            ]
            combo_avg = [
                self.avg_combo_scores_per_gen.get(f"Gen_{g}") for g in generations
            ]
            ax_combined.plot(
                generations,
                combo_best,
                ...
                linewidth=2.5,
                markersize=10,
                label="Best Combination Score (Overall)",
            )
            ax_combined.plot(
                generations,
                combo_avg,
                ...
                linewidth=2.5,
                markersize=8,
                label="Average Combination Score (Overall)",
            )
        ...
        for metric in sorted(list(all_node_metrics)):
            best_scores = [
                self.best_scores_per_gen.get(f"Gen_{g}", {}).get(metric)
                for g in generations
            ]
            avg_scores = [
                self.avg_scores_per_gen.get(f"Gen_{g}", {}).get(metric)
                for g in generations
            ]
            ax_combined.plot(
                generations,
            ...
            ax_combined.plot(
                ...
                linestyle="--",
                alpha=0.7,
                label=f"Average Score ({metric})",
            )

        ax_combined.set_title(
            f"Overall Performance Evolution ({algorithm_name})",
            fontsize=18,
            weight="bold",
        )
        ...
        ax_combined.set_xticklabels(
            [f"Gen {g}" for g in generations], rotation=45, ha="right"
        )
        ...
        node_indices = [
            i for i, label in enumerate(labels) if "Combination" not in label
        ]
        ax_combined.legend(
            [handles[i] for i in combo_indices + node_indices],
        ...
        combined_filepath = os.path.join(
            self.log_dir, "performance_summary_OVERALL.png"
        )
        try:
    ...
    def _log_optimization_summary(
        ...
        with open(filepath, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(
                ["Combination_Sample_Size", self.combination_sample_size, timestamp]
            )
            writer.writerow(
                ["Early_Stopping_Enabled", self.enable_early_stopping, timestamp]
            )
            if self.enable_early_stopping:
                writer.writerow(
                    ["Early_Stopping_Patience", self.early_stopping_patience, timestamp]
                )

            if test_accuracy is not None:
                writer.writerow(
                    ["Final_Test_Accuracy", f"{test_accuracy:.6f}", timestamp]
                )
            ...
            for gen_name in self.best_scores_per_gen.keys():
                for metric_name, best_score in self.best_scores_per_gen[
                    gen_name
                ].items():
                    writer.writerow(
                        [
                            f"{gen_name}_{metric_name}_Best",
                            f"{best_score:.6f}",
                            timestamp,
                        ]
                    )
                ...
                if gen_name in self.avg_scores_per_gen:
                    for metric_name, avg_score in self.avg_scores_per_gen[
                        gen_name
                    ].items():
                        writer.writerow(
                            [
                                f"{gen_name}_{metric_name}_Avg",
                                f"{avg_score:.6f}",
                                timestamp,
                            ]
                        )
    ...
    def _save_best_config_json(
        self, best_config: Dict[str, str], filename: str = "best_config.json"
    ) -> None:
        """
    ...
    async def _log_evaluation_details(
        self,
        ...
        with open(filepath, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(
                ["example_id", "input_text", "prediction", "ground_truth", "score"]
            )
    ...
    async def _evaluate_combination_list(
        self, combinations: List[Dict], benchmark: BIGBenchHard, dev_set: list
    ) -> List[float]:
        if not combinations:
            return []
        ...
        for combo in combinations:
            tasks = [
                self._evaluate_combination_on_example(combo, benchmark, ex)
                for ex in eval_dev_set
            ]
            example_scores = await asyncio.gather(*tasks)
            avg_score = (
                sum(example_scores) / len(example_scores) if example_scores else 0.0
            )
    ...
    def _generate_combinations(
        ...
        total_possible = (
            np.prod([len(p) for p in node_prompts if p])
            if all(p for p in node_prompts)
            else 0
        )

        if total_possible == 0:
            logger.warning(
                "Cannot generate combinations, one or more node populations are empty."
            )
            return []
        ...
        logger.info(
            f"Total possible combinations: {total_possible}, sampling: {target_size}"
        )
        ...
        if target_size >= total_possible:
            ...
            for combination in itertools.product(*node_prompts):
                combo_dict = {
                    node_names[i]: combination[i] for i in range(len(node_names))
                }
        ...
        while len(sampled_combinations) < target_size and attempts < max_attempts:
            combination = {
                ...
                for name, prompts in node_populations.items()
            }
    ...
    async def _evaluate_combination_on_example(
        self, combination: Dict[str, str], benchmark: BIGBenchHard, example: Dict
    ) -> float:
        combo_key = tuple(sorted(combination.items()))
        ...
        async with self.semaphore:
            try:
                ...
                inputs = {
                    k: v for k, v in example.items() if k in benchmark.get_input_keys()
                }
    ...
    async def _evaluate_combinations_and_update_node_scores(
        self, combinations: List[Dict[str, str]], benchmark: BIGBenchHard, dev_set: list
    ) -> List[float]:
        eval_dev_set = dev_set[:50] if len(dev_set) > 50 else dev_set
        combination_scores = []
        print(
            f"Evaluating {len(combinations)} combinations on {len(eval_dev_set)} examples..."
        )
        combo_pbar = aio_tqdm(total=len(combinations), desc="Evaluating Combinations")
        for combination in combinations:
            tasks = [
                self._evaluate_combination_on_example(combination, benchmark, ex)
                for ex in eval_dev_set
            ]
            example_scores = await asyncio.gather(*tasks)
            avg_score = (
        ...
        for node_name in self.node_populations.keys():
            ...
            for prompt_idx, prompt in enumerate(self.node_populations[node_name]):
                participating_scores = [
                    combo_score
                    for combo_idx, combo_score in enumerate(combination_scores)
                    if combinations[combo_idx].get(node_name) == prompt
                ]
                if participating_scores:
                    self.node_scores[node_name][prompt_idx] = sum(
                        participating_scores
                    ) / len(participating_scores)
                else:
    ...
    async def _perform_paraphrase(self, prompt: str) -> str:
        async with self.semaphore:
            output = await asyncio.to_thread(
                self.paraphrase_agent, inputs={"instruction": prompt}
            )
        ...
        for node_name, initial_value in initial_config.items():
            ...
            if isinstance(initial_value, list):
                ...
                if self.population_size < provided_size:
                    logger.info(
                        f"Node '{node_name}': Provided population ({provided_size}) is larger than target size ({self.population_size}). Randomly sampling."
                    )
                    node_population = random.sample(initial_value, self.population_size)
                elif self.population_size == provided_size:
                    logger.info(
                        f"Node '{node_name}': Provided population size ({provided_size}) matches target size. Using directly."
                    )
                    node_population = list(initial_value)
                else:
                    logger.info(
                        f"Node '{node_name}': Target population size ({self.population_size}) is larger than provided ({provided_size}). Expanding."
                    )
                    node_population = list(initial_value)
                    num_to_generate = self.population_size - provided_size
                    source_prompts_for_generation = random.choices(
                        initial_value, k=num_to_generate
                    )
                    paraphrase_tasks = [
                        self._perform_paraphrase(prompt)
                        for prompt in source_prompts_for_generation
                    ]
                ...
                logger.info(
                    f"Node '{node_name}': Generating population from a single initial prompt."
                )
                node_population = [initial_value]
                if self.population_size > 1:
                    num_to_generate = self.population_size - 1
                    paraphrase_tasks = [
                        self._perform_paraphrase(initial_value)
                        for _ in range(num_to_generate)
                    ]
                    new_prompts = await aio_tqdm.gather(
                        *paraphrase_tasks,
                        desc=f"Generating initial population for {node_name}",
                    )
                    node_population.extend(new_prompts)
            else:
                raise TypeError(
                    f"Unsupported type for tracked parameter '{node_name}': {type(initial_value)}. Must be str or list."
                )
            self.node_populations[node_name] = node_population
            self.node_scores[node_name] = [0.0] * self.population_size

    async def evaluate(
        self, benchmark: BIGBenchHard, eval_mode: str = "test"
    ) -> Dict[str, float]:
        """
        ...
        dataset = (
            benchmark.get_test_data()
            if eval_mode == "test"
            else benchmark.get_dev_data()
        )
        if not dataset:
            logger.warning(
                f"No data found for '{eval_mode}' set. Returning empty results."
            )
            return {}

        async def evaluate_example(example: Dict) -> tuple[float, str]:
            prediction, _ = await asyncio.to_thread(
                self.program, input=example["input"]
            )
        ...
        results = await aio_tqdm.gather(
            *tasks, desc=f"Evaluating on {eval_mode.capitalize()} Set"
        )
        ...
        logger.info(
            f"{eval_mode.capitalize()} Set Accuracy: {accuracy:.4f} ({int(correct_count)}/{total_count})"
        )
            ...
            inputs=[
                {
                    "name": "parent1",
                    "type": "string",
                    "description": "The first parent prompt.",
                },
                {
                    "name": "parent2",
                    "type": "string",
                    "description": "The second parent prompt.",
                },
            ],
            outputs=[
                {
                    "name": "evolved_prompt",
                    "type": "string",
                    "description": "The evolved prompt with XML tags preserved.",
                }
            ],
            parse_mode="title",
        )

    async def _perform_node_evolution(
        self,
        node_name: str,
        node_population: List[str],
        node_scores: List[float] = None,
        evolution_agent: Callable = None,
    ) -> List[str]:
        probabilities = None
        if node_scores:
        ...
        for _ in range(num_children_to_create):
            parents = (
                random.choices(node_population, weights=probabilities, k=2)
                if probabilities
                else random.choices(node_population, k=2)
            )
            task = self._perform_evolution(
                agent=agent, inputs={"parent1": parents[0], "parent2": parents[1]}
            )
            evolution_tasks.append(task)

        new_children = await aio_tqdm.gather(
            *evolution_tasks, desc=f"Evolving {node_name}"
        )
        return new_children

    async def optimize(
        self, benchmark: BIGBenchHard
        ...
        if self.full_evaluation:
            logger.info(
                "--- Starting Node-Based Evolution with Makeup Evaluation (full_evaluation=True) ---"
            )

            print("--- Step 1: Initial evaluation of node combinations... ---")
            combinations = self._generate_combinations(self.node_populations)
            combination_scores = (
                await self._evaluate_combinations_and_update_node_scores(
                    combinations, benchmark, dev_set
                )
            )
            ...
            self.best_scores_per_gen["Gen_0"] = {
                name: max(scores) if scores else 0
                for name, scores in self.node_scores.items()
            }
            self.avg_scores_per_gen["Gen_0"] = {
            ...
            if combination_scores:
                ...
                logger.info(
                    f"Early stopping baseline set to initial best combination score: {self._best_score_so_far:.4f}"
                )
            ...
            for t in range(self.iterations):
                ...
                for node_name in self.node_populations.keys():
                    children = await self._perform_node_evolution(
                        node_name,
                        self.node_populations[node_name],
                        self.node_scores[node_name],
                        self.ga_agent,
                    )
                ...
                print(
                    f"Performing main evaluation for {len(list(current_populations.values())[0])} individuals in each node..."
                )
                combinations = self._generate_combinations(self.node_populations)
                combination_scores = (
                    await self._evaluate_combinations_and_update_node_scores(
                ...
                for node_name, scores in self.node_scores.items():
                    for idx, score in enumerate(scores):
                        if score == 0.0:
                            ...
                            is_in_combos = any(
                                c.get(node_name) == prompt_to_check
                                for c in combinations
                            )
                            if not is_in_combos:
                                prompts_needing_makeup.append(
                                    (node_name, idx, prompt_to_check)
                                )

                if prompts_needing_makeup:
                    print(
                        f"--- Performing makeup evaluation for {len(prompts_needing_makeup)} unsampled individuals... ---"
                    )
                    makeup_combinations = []
                    for node_name, idx, prompt in prompts_needing_makeup:
                        makeup_combo = {
                            name: random.choice(pop)
                            for name, pop in self.node_populations.items()
                        }
                        makeup_combo[node_name] = prompt
                        makeup_combinations.append(makeup_combo)

                    makeup_scores = await self._evaluate_combination_list(
                        makeup_combinations, benchmark, dev_set
                    )

                    for i, (node_name, idx, prompt) in enumerate(
                        prompts_needing_makeup
                    ):
                        self.node_scores[node_name][idx] = makeup_scores[i]
                        logger.info(
                            f"Updated score for '{prompt[:30]}...' to {makeup_scores[i]:.4f} after makeup eval."
                        )
                ...
                for node_name in self.node_populations.keys():
                    ...
                    print(
                        f"Node {node_name}: Selected top {len(survivor_populations[node_name])} from {len(population)} individuals"
                    )
                ...
                if combination_scores:
                    self._log_detailed_evaluation(
                        t + 1, combinations, combination_scores
                    )
                ...
                best_combo_score_this_gen = (
                    max(combination_scores) if combination_scores else -float("inf")
                )
                self.best_combo_scores_per_gen[gen_name] = best_combo_score_this_gen
                self.avg_combo_scores_per_gen[gen_name] = (
                    np.mean(combination_scores) if combination_scores else 0.0
                )
                ...
                if self.enable_early_stopping:
                    if best_combo_score_this_gen > self._best_score_so_far + 1e-6:
                        ...
                        self._generations_without_improvement = 0
                        logger.info(
                            f"Early stopping: New best combination score found: {self._best_score_so_far:.4f}."
                        )
                    else:
                        self._generations_without_improvement += 1
                        logger.info(
                            f"Early stopping: No improvement for {self._generations_without_improvement} generation(s)."
                        )
            ...
            logger.info(
                "--- Starting Combo-Based Evolution (full_evaluation=False) ---"
            )

            print(
                "--- Step 1: Creating and evaluating initial combination population... ---"
            )
            initial_combinations = self._generate_combinations(self.node_populations)
            initial_scores = await self._evaluate_combination_list(
                initial_combinations, benchmark, dev_set
            )

            combo_population_with_scores = sorted(
                zip(initial_combinations, initial_scores),
                key=lambda x: x[1],
                reverse=True,
            )
            combo_population_with_scores = combo_population_with_scores[
            ...
            if gen_0_scores:
                ...
                print(
                    f"Generation 0 complete. Best score: {best_gen_score:.4f}, Avg score: {avg_gen_score:.4f}"
                )
                logger.info(
                    f"Early stopping baseline set to: {self._best_score_so_far:.4f}"
                )
            ...
            for t in range(self.iterations):
                ...
                for node_name, prompts in parent_prompts_for_node.items():
                    children_populations[node_name] = (
                        await self._perform_node_evolution(node_name, prompts)
                    )

                print("Evaluating new child combinations...")
                child_combinations = self._generate_combinations(children_populations)
                child_scores = await self._evaluate_combination_list(
                    child_combinations, benchmark, dev_set
                )
                ...
                combined_population = (
                    combo_population_with_scores + child_combos_with_scores
                )

                sorted_combos = sorted(
                    combined_population, key=lambda x: x[1], reverse=True
                )
                ...
                print(
                    f"Generation {t + 1} complete. Best score: {best_gen_score:.4f}, Avg score: {avg_gen_score:.4f}"
                )
                ...
                if self.enable_early_stopping:
                    if best_gen_score > self._best_score_so_far + 1e-6:
                        ...
                        self._generations_without_improvement = 0
                        logger.info(
                            f"Early stopping: New best combination score found: {self._best_score_so_far:.4f}. Patience counter reset."
                        )
                    else:
                        self._generations_without_improvement += 1
                        logger.info(
                            f"Early stopping: No improvement for {self._generations_without_improvement} generation(s). Patience: {self.early_stopping_patience}."
                        )
        ...
        if self.full_evaluation:
            best_config = {
                ...
                for name in self.node_populations.keys()
                if self.node_populations.get(name) and self.node_scores.get(name)
            }
        else:
            best_config, _ = (
                max(combo_population_with_scores, key=lambda x: x[1])
                if combo_population_with_scores
                else ({}, 0)
            )
        ...
        logger.info(
            "Optimization finished! The best configuration has been applied to the program."
        )
...
[Your evolved prompt here]""",
            ...
            inputs=[
                {
                    "name": "current_prompt",
                    "type": "string",
                    "description": "The base prompt to be mutated, p_i.",
                },
                {
                    "name": "donor1",
                    "type": "string",
                    "description": "The first donor prompt, p_r1.",
                },
                {
                    "name": "donor2",
                    "type": "string",
                    "description": "The second donor prompt, p_r2.",
                },
                {
                    "name": "best_prompt",
                    "type": "string",
                    "description": "The best prompt found so far in the population, p_best.",
                },
            ],
            outputs=[
                {
    ...
    async def _evolve_and_select_one(
        ...
        trial_scores = await self._evaluate_combination_list(
            [trial_combo], benchmark, dev_set
        )
        trial_score = trial_scores[0]

        # Selection: return the better combination
        return (
            (trial_combo, trial_score)
            if trial_score > target_score
            else (target_combo, target_score)
        )
    ...
    async def optimize(
        ...
        print(
            f"Initial population - Best score: {initial_best:.4f}, Avg score: {initial_avg:.4f}"
        )
        ...
        for t in range(self.iterations):
            ...
            evolution_pipeline_tasks = [
                self._evolve_and_select_one(
                    combo_with_score, combo_pop_with_scores, benchmark, dev_set
                )
                for combo_with_score in combo_pop_with_scores
            ]
            pbar = aio_tqdm(
                total=len(evolution_pipeline_tasks),
                desc=f"Pipelined Evolution Gen {t+1}",
            )
            ...
            if self.enable_early_stopping:
                if best_gen_score > self._best_score_so_far + 1e-6:
                    ...
                    self._generations_without_improvement = 0
                    logger.info(
                        f"Early stopping: New best score found: {self._best_score_so_far:.4f}. Patience counter reset."
                    )
                else:
                    self._generations_without_improvement += 1
                    logger.info(
                ...
                if (
                    ...
                    logger.warning(
                        f"No improvement in best score for {self.early_stopping_patience} consecutive generations."
                    )
                    break

        print("\n--- Combination-Level Evolution complete ---")
        best_combination, best_score = (
            max(combo_pop_with_scores, key=lambda x: x[1])
            if combo_pop_with_scores
            else ({}, 0)
        )
        logger.info(
            f"Optimization finished! Best combination found with score {best_score:.4f}."
        )
        ...
        return (
            best_combination,
            self.best_combo_scores_per_gen,
            self.avg_combo_scores_per_gen,
        )
