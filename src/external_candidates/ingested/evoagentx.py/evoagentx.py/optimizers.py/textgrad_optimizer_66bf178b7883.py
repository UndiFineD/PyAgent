# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\optimizers\textgrad_optimizer.py
# -----------------------------------------------------------------------------
...
class CustomAgentCall:
    ...
    def __call__(
        self, instruction: Variable, system_prompt: Variable, **inputs: Variable
    ) -> Variable:
        ...
        for key, input_variable in inputs.items():
            if key in input_names:
                ...
                parsed_inputs: dict[str, str] = {
                    k: v
                    for k, v in input_variable.parsed_outputs.items()
                    if k in input_names
                }
                agent_inputs.update(parsed_inputs)

        with suppress_logger_info():
            outputs = self.agent.execute(
                action_name=action.name, action_input_data=agent_inputs
            ).content
...
class TextGradAgent:
    ...
    def __init__(
        ...
        self.output_description = " and ".join(
            agent.actions[0].outputs_format.get_attr_descriptions().values()
        )
...
class TextGradOptimizer(BaseModule):
    ...
    optimize_mode: Literal["all", "system_prompt", "instruction"] = Field(
        default="all",
        description="The mode to optimize the workflow. 'all' optimizes both system prompts and instructions, 'system_prompt' only optimizes system prompts, and 'instruction' only optimizes instructions.",
    )
    ...
    batch_size: PositiveInt = Field(
        default=1, description="The batch size for optimization."
    )
    max_steps: PositiveInt = Field(
        default=10, description="The maximum number of optimization steps."
    )
    evaluator: Evaluator = Field(
        default=None,
        description="The evaluator to perform evaluation during optimization.",
    )
    eval_every_n_steps: Optional[PositiveInt] = Field(
        default=None,
        description="Evaluate the workflow every `eval_every_n_steps` steps.",
    )
    eval_rounds: PositiveInt = Field(
        default=1, description="The number of times to evaluate the performance."
    )
    eval_config: dict = Field(
        default={},
        description="The configuration for evaluation. The keys are the arguments of `TextGradOptimizer.evaluate`.",
    )
    save_interval: Optional[PositiveInt] = Field(
        default=None, description="Save the workflow every `save_interval` steps."
    )
    save_path: str = Field(
        default="./", description="The path to save the optimized workflow."
    )
    rollback: bool = Field(
        default=True,
        description="Whether to rollback to the best graph after each evaluation during optimization.",
    )
    constraints: List[str] = Field(
        default=[],
        description="The constraints for optimization. e.g. ['They system prompt must not exceed 100 words.']",
    )
    ...
    def _init_textgrad(self, dataset: Benchmark, use_answers: bool = True):
        ...
        if use_answers:
            ...
            self.loss_fn = MultiFieldEvaluation(
                evaluation_instruction, role_descriptions, self.optimizer_engine
            )
        else:
        ...
        if self.optimize_mode == "all":
            optimize_variables = (
                self._get_all_system_prompts() + self._get_all_instructions()
            )
            ...
            raise ValueError(
                "Unsupported `optimize_mode`, should be one of 'all', 'system_prompt', 'instruction'."
            )
    ...
    def optimize(
        self, dataset: Benchmark, use_answers: bool = True, seed: Optional[int] = None
    ) -> None:
        ...
        def iterator() -> (
            Iterator[
                Tuple[List[dict[str, str]], Optional[List[Union[str, dict[str, str]]]]]
            ]
        ):
            epoch = 0
            while True:
                ...
                train_data = dataset.get_train_data(
                    sample_k=len(dataset._train_data), seed=effective_seed
                )
        ...
        for step in tqdm(range(self.max_steps)):
                ...
                if self.rollback:
                    if len(self._snapshot) == 1:
                        ...
                        if current_average_score >= best_average_score:
                            ...
                            logger.info(
                                f"Metrics are worse than the best snapshot which has {best_snapshot['metrics']}. Rolling back to the best snapshot."
                            )
            ...
            if self.save_interval is not None and (step + 1) % self.save_interval == 0:
                ...
                self.save(
                    os.path.join(
                        self.save_path, f"{dataset.name}_textgrad_step_{step+1}.json"
                    )
                )

        logger.info(
            f"Reached the maximum number of steps {self.max_steps}. Optimization has finished."
        )
        ...
        if len(self._snapshot) > 0:
            ...
            self.save(
                os.path.join(self.save_path, f"{dataset.name}_textgrad_best.json"),
                graph=best_graph,
            )
            ...
            for input, label in zip(inputs, labels, strict=True):
                ...
                if isinstance(label, str):
                    ...
                    if not isinstance(dataset, CodingBenchmark):
                        raise ValueError(
                            "Label must be a string for non-coding benchmarks."
                        )
    ...
    def save(
        self, path: str, graph: Optional[WorkFlowGraph] = None, ignore: List[str] = []
    ) -> None:
    ...
    def restore_best_graph(self) -> None:
        ...
        best_graph, best_metrics = self._select_graph_with_highest_score(
            return_metrics=True
        )
        self.graph = best_graph
        logger.info(
            f"Restored the best graph from snapshot with metrics {best_metrics}"
        )

    def _format_code_label(
        self, code: str, label: dict[str, str], dataset: CodingBenchmark
    ) -> str:
    ...
    def _initial_inputs_to_variables(
        self, initial_inputs: dict[str, str]
    ) -> dict[str, Variable]:
        ...
        raise ValueError(
            f"Initial inputs do not match the inputs of the initial nodes. Missing inputs: {missing_inputs}"
        )

    def _compute_node(
        self, node: Union[str, WorkFlowNode], initial_inputs: dict[str, Variable]
    ) -> Variable:
        ...
        input_node_names: set[str] = (
            set()
        )  # which nodes we need to compute the output of
    ...
    def _create_textgrad_agent(self, node: Union[str, WorkFlowNode]) -> TextGradAgent:
        ...
        if isinstance(node.agents[0], dict):
            ...
            agent: Union[CustomizeAgent, Agent] = CustomizeAgent.from_dict(
                node.agents[0]
            )
        else:
            raise ValueError(
                f"Unsupported agent type {type(node.agents[0])}. Expected 'dict'."
            )
    ...
    def _update_agent_prompts(
        self, agent_dict: dict[str, Any], system_prompt: str, instruction: str
    ) -> dict[str, Any]:
        ...
        if "actions" in agent_dict:
            ...
            agent_dict["actions"][0]["prompt_template"] = (
                self._update_agent_instructions(
                    agent_dict["actions"][0]["prompt_template"], instruction
                )
            )
        else:
            # CustomizeAgent does not have actions in its dict
            agent_dict["prompt_template"] = self._update_agent_instructions(
                agent_dict["prompt_template"], instruction
            )
        return agent_dict

    def _update_agent_instructions(
        self, prompt_template: Union[PromptTemplate, dict[str, str]], instruction: str
        ...
        if isinstance(prompt_template, PromptTemplate):
            ...
            raise ValueError(
                f"Unsupported prompt template type {type(prompt_template)}. Expected 'PromptTemplate' or 'dict'."
            )
    ...
    def _select_graph_with_highest_score(
        self, return_metrics: bool = False
    ) -> Union[WorkFlowGraph, tuple[WorkFlowGraph, Optional[dict]]]:
        ...
        snapshot_scores = [
            np.mean(list(snapshot["metrics"].values())) for snapshot in self._snapshot
        ]
    ...
    def _validate_graph_compatibility(self, graph: WorkFlowGraph) -> None:
        ...
        for node in graph.nodes:
            if len(node.agents) > 1:
                raise ValueError(
                    "TextGrad optimizer only supports workflows where every node only has a single agent."
                )
            else:
                agent = node.agents[0]
                if not isinstance(agent, dict):
                    ...
                    if "actions" in agent:
                        ...
                        if len(non_ContextExtraction_actions) > 1:
                            raise ValueError(
                                f"TextGrad optimizer only supports workflows where every agent only has a single action. {agent['name']} has {len(non_ContextExtraction_actions)} actions."
                            )
                        if "prompt_template" not in non_ContextExtraction_actions[0]:
                            raise ValueError(
                                f"Please provide a PromptTemplate for {agent['name']}."
                            )
                    else:
                        # CustomizeAgent does not have actions in its dict
