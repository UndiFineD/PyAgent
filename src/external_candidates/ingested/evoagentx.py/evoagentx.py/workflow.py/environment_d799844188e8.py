# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\workflow\environment.py
...
class Environment(BaseModule):
    ...
    def update_task_execution_history(self, message: Message):
        if message.wf_task is not None and message.msg_type in [MessageType.RESPONSE]:
            ...
            if (
                not self.task_execution_history
                or message.wf_task != self.task_execution_history[-1]
            ):
    ...
    def get_task_messages(
        ...
        **kwargs,
        ...
        for step in self.trajectory:
            ...
            if (
                include_inputs
                and message.msg_type == MessageType.INPUT
                and message not in message_list
            ):
                message_list.append(message)
    ...
    def get_execution_data(self, params: Union[str, List[str]]) -> dict:
        ...
        for param in params:
            if param not in self.execution_data:
                raise KeyError(
                    f"Couldn't find execution data with key '{param}'. Available execution data: {list(self.execution_data.keys())}"
                )
