# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\workflow\workflow_generator.py
import json
import time
...
class WorkFlowGenerator(BaseModule):
    ...
    task_planner: Optional[TaskPlanner] = Field(
        default=None,
        description="Responsible for breaking down the high-level task into manageable sub-tasks.",
    )
    agent_generator: Optional[AgentGenerator] = Field(
        default=None,
        description="Assigns or generates the appropriate agent(s) to handle each sub-task.",
    )
    workflow_reviewer: Optional[WorkFlowReviewer] = Field(
        default=None,
        description="Provides feedback and reflections to improve the generated workflow.",
    )
    num_turns: Optional[PositiveInt] = Field(
        default=0,
        description="Specifies the number of refinement iterations for the generated workflow.",
    )
    tools: Optional[List[Toolkit]] = Field(
        default=None, description="A list of tools that can be used in the workflow."
    )
    ...
    def _execute_with_retry(
        self, operation_name: str, operation, retries_left: int = 1, **kwargs
    ):
        ...
        while cur_retries <= retries_left:  # Changed < to <= to include the initial try
            try:
                logger.info(
                    f"{operation_name} (attempt {cur_retries + 1}/{retries_left + 1}) ..."
                )
                ...
                if cur_retries == retries_left:
                    raise ValueError(
                        f"Failed to {operation_name} after {cur_retries + 1} attempts.\nError: {e}"
                    )
                sleep_time = 2**cur_retries
                logger.error(
                    f"Failed to {operation_name} in {cur_retries + 1} attempts. Retry after {sleep_time} seconds.\nError: {e}"
                )
    ...
    def generate_workflow(
        ...
        **kwargs,
        ...
        logger.info(
            f"Successfully generate the following workflow:\n{workflow.get_workflow_description()}"
        )
        ...
        for node in workflow.nodes:
            if not node.agents:
                raise ValueError(
                    f"Node {node.name} has no agents assigned after agent generation"
                )

        return workflow

    def generate_plan(
        self, goal: str, history: Optional[str] = None, suggestion: Optional[str] = None
    ) -> TaskPlanningOutput:
    ...
    def generate_agents(
        ...
        for subtask in workflow.nodes:
            ...
            subtask_data = {
                key: value
                for key, value in subtask.to_dict(ignore=["class_name"]).items()
    ...
    def build_workflow_from_plan(
        self, goal: str, plan: TaskPlanningOutput
    ) -> WorkFlowGraph:
        ...
        for node in nodes:
            for another_node in nodes:
                ...
                another_node_input_params = [
                    param.name for param in another_node.inputs
                ]
                if any(
                    [param in another_node_input_params for param in node_output_params]
                ):
                    edges.append(
                        WorkFlowEdge(edge_tuple=(node.name, another_node.name))
                    )
