...
class TaskScheduler(Action):
    ...
    def __init__(self, **kwargs):
        name = (
            ...
            if "name" in kwargs
            else DEFAULT_TASK_SCHEDULER["name"]
        )
        description = (
            kwargs.pop("description", None)
            if "description" in kwargs
            else DEFAULT_TASK_SCHEDULER["description"]
        )
        prompt = (
            kwargs.pop("prompt", None)
            if "prompt" in kwargs
            else DEFAULT_TASK_SCHEDULER["prompt"]
        )
        super().__init__(
            name=name,
        ...
        self.max_num_turns = kwargs.get(
            "max_num_turns", DEFAULT_TASK_SCHEDULER["max_num_turns"]
        )

    def get_predecessor_tasks(
        self, graph: WorkFlowGraph, tasks: List[WorkFlowNode]
    ) -> List[str]:
        predecessors = []
        for task in tasks:
    ...
    def _handle_edge_cases(
        self, candidate_tasks: List[WorkFlowNode]
    ) -> Union[TaskSchedulerOutput, None]:
        """
    ...
    def _prepare_execution(
        ...
        predecessor_tasks = self.get_predecessor_tasks(
            graph=graph, tasks=candidate_tasks
        )
        execution_outputs = "\n\n".join(
            [str(msg) for msg in env.get_task_messages(tasks=predecessor_tasks)]
        )
        candidate_tasks_info = "\n\n".join(
            [task.get_task_info() for task in candidate_tasks]
        )
        prompt_inputs = {
    ...
    def execute(
        ...
        **kwargs,
        ...
        """
        assert (
            graph is not None and env is not None
        ), "must provide 'graph' and 'env' when executing TaskScheduler"
        ...
        scheduled_task = llm.generate(
            prompt=prompt, system_message=sys_msg, parser=self.outputs_format
        )
    ...
    async def async_execute(
        ...
        **kwargs,
        ...
        _, prompt = self._prepare_execution(graph, env, candidate_tasks)
        scheduled_task = await llm.async_generate(
            prompt=prompt, system_message=sys_msg, parser=self.outputs_format
        )
...
class NextAction(LLMOutputParser):
    ...
    action_graph: Optional[ActionGraph] = Field(
        default=None, description="The predefined action graph to be executed."
    )
    ...
    def to_str(self, **kwargs) -> str:
        if self.agent is not None and self.action is not None:
            ...
            raise ValueError(
                "must provide either both agent (str) and action (str), or action_graph (ActionGraph)."
            )
...
class ActionScheduler(Action):
    ...
    def __init__(self, **kwargs):
        name = (
            kwargs.pop("name", None)
            if "name" in kwargs
            else DEFAULT_ACTION_SCHEDULER["name"]
        )
        description = (
            kwargs.pop("description", None)
            if "description" in kwargs
            else DEFAULT_ACTION_SCHEDULER["description"]
        )
        prompt = (
            kwargs.pop("prompt", None)
            if "prompt" in kwargs
            else DEFAULT_ACTION_SCHEDULER["prompt"]
        )
        super().__init__(
            name=name,
            description=description,
    ...
    def get_agent_action_pairs(
        self, action: str, agent_actions_map: Dict[str, List[str]]
    ) -> List[Tuple[str, str]]:
        pairs = []
    ...
    def _prepare_action_execution(
        ...
        if not task_agent_names:
            raise ValueError(
                f"The task '{task.name}' does not provide any agents for execution!"
            )

        task_agents = [agent_manager.get_agent(name) for name in task_agent_names]
        task_agent_actions_map = {
            agent.name: [action.name for action in agent.get_all_actions()]
        ...
        if task_execution_messages and task_execution_messages[-1].next_actions:
            ...
            self.check_candidate_action(
                task.name, predefined_next_actions, task_agent_actions_map
            )
            if len(predefined_next_actions) == 1:
                predefined_next_action = predefined_next_actions[0]
                agent_action_pairs = self.get_agent_action_pairs(
                    predefined_next_action, task_agent_actions_map
                )
        ...
        if (
            not next_action
            and len(task_agent_names) == 1
            and len(task_agent_actions_map[task_agent_names[0]]) == 1
        ):
        ...
        agent_actions_info = "\n\n".join(
            [
                agent.get_agent_profile(
                    action_names=candidate_agent_actions[agent.name]
                )
                for agent in task_agents
        ...
        task_execution_history = "\n\n".join(
            [str(msg) for msg in task_execution_messages]
        )
    ...
    def execute(
        ...
        **kwargs,
        ...
        """
        result = self._prepare_action_execution(
            task=task, agent_manager=agent_manager, env=env
        )
    ...
    async def async_execute(
        ...
        **kwargs,
        ...
        # Handle async LLM generation case
        _, _, prompt = result
        next_action = await llm.async_generate(
            prompt=prompt, system_message=sys_msg, parser=self.outputs_format
        )
...
class WorkFlowManager(BaseModule):
    ...
    async def schedule_next_task(
        self, graph: WorkFlowGraph, env: Environment = None, **kwargs
    ) -> WorkFlowNode:
        """
    ...
    async def extract_output(
        self, graph: WorkFlowGraph, env: Environment, **kwargs
    ) -> str:
        """
        ...
        end_task_predecesssors = sum(
            [graph.get_node_predecessors(node=end_task) for end_task in end_tasks], []
        )
        ...
        candidate_msgs_with_output = Message.sort_by_timestamp(
            messages=candidate_msgs_with_output
        )
        ...
        prompt = OUTPUT_EXTRACTION_PROMPT.format(
            ...
            workflow_execution_results="\n\n".join(
                [str(msg) for msg in candidate_msgs_with_output]
            ),
        )
