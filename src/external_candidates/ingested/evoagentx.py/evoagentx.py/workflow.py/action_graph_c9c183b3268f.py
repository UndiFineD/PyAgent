# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\workflow\action_graph.py
import json
...
from .operators import AnswerGenerate, Operator, QAScEnsemble
...
class ActionGraph(BaseModule):
    ...
    llm_config: LLMConfig = Field(
        description="The config of LLM used to execute the ActionGraph."
    )
    ...
    def execute(self, *args, **kwargs) -> dict:
        raise NotImplementedError(
            f"The execute function for {type(self).__name__} is not implemented!"
        )

    def async_execute(self, *args, **kwargs) -> dict:
        raise NotImplementedError(
            f"The async_execute function for {type(self).__name__} is not implemented!"
        )
    ...
    def load_module(cls, path: str, llm_config: LLMConfig = None, **kwargs) -> Dict:
        ...
        ), "must provide `llm_config` when using `load_module` or `from_file` to load the ActionGraph from local storage"
...
class QAActionGraph(ActionGraph):
    ...
    def __init__(self, llm_config: LLMConfig, **kwargs):
        ...
        description = (
            ...
            else "This is a simple QA workflow that use self-consistency to make predictions."
        )
        super().__init__(
            name=name, description=description, llm_config=llm_config, **kwargs
        )
