...
class WorkFlow(BaseModule):
    ...
    agent_manager: AgentManager = Field(
        default=None, description="Responsible for managing agents"
    )
    ...
    hitl_manager: HITLManager = Field(
        default=None, description="Responsible for HITL work management"
    )
    ...
    def init_module(self):
        ...
        if self.agent_manager is None:
            logger.warning(
                "agent_manager is NoneType when initializing a WorkFlow instance"
            )
    ...
    async def async_execute(self, inputs: dict = {}, **kwargs) -> str:
        ...
        while not self.graph.is_complete and not failed:
            try:
                ...
                self.environment.update(
                    message=error_message, state=TrajectoryState.FAILED, error=str(e)
                )
        ...
        output: str = await self.workflow_manager.extract_output(
            graph=self.graph, env=self.environment
        )
        return output
    ...
    def _prepare_inputs(self, inputs: dict) -> dict:
        ...
        for initial_node_name in initial_node_names:
            ...
            if initial_node.inputs:
                initial_node_required_inputs.update(
                    [inp.name for inp in initial_node.inputs if inp.required]
                )
    ...
    async def get_next_task(self) -> WorkFlowNode:
        ...
        logger.info(
            f"Task Execution Trajectory: {task_execution_history}. Scheduling next subtask ..."
        )
        task: WorkFlowNode = await self.workflow_manager.schedule_next_task(
            graph=self.graph, env=self.environment
        )
    ...
    async def execute_task(self, task: WorkFlowNode):
        ...
        if next_action.action_graph is not None:
            await self._async_execute_task_by_action_graph(
                task=task, next_action=next_action
            )
        else:
            await self._async_execute_task_by_agents(task=task, next_action=next_action)
        self.graph.completed(node=task)

    async def _async_execute_task_by_action_graph(
        self, task: WorkFlowNode, next_action: NextAction
    ):
        """
        ...
        if async_execute:
            action_graph_output: dict = await action_graph.async_execute(
                **execute_params
            )
        else:
    ...
    async def _async_execute_task_by_agents(
        self, task: WorkFlowNode, next_action: NextAction
    ):
        """
        ...
        while next_action:
            ...
            if not self.agent_manager.wait_for_agent_available(
                agent_name=agent.name, timeout=300
            ):
                raise TimeoutError(
                    f"Timeout waiting for agent {agent.name} to become available"
                )
            self.agent_manager.set_agent_state(
                agent_name=next_action.agent, new_state=AgentState.RUNNING
            )
            try:
                ...
                #     wf_task_desc=task.description
                # )
                message = await self._async_execute_action(
                    task=task, agent=agent, next_action=next_action
                )
                self.environment.update(
                    message=message, state=TrajectoryState.COMPLETED
                )
            finally:
                self.agent_manager.set_agent_state(
                    agent_name=next_action.agent, new_state=AgentState.AVAILABLE
                )
    ...
    async def _async_execute_action(
        self, task: WorkFlowNode, agent: Agent, next_action: NextAction
    ) -> Message:
        """
        ...
        if action_inputs_format:
            ...
            action_required_input_names = (
                action_inputs_format.get_required_input_names()
            )
            if not all(inp in action_input_data for inp in action_required_input_names):
                ...
                self.environment.update_execution_data_from_context_extraction(
                    extracted_data
                )
                return message
    ...
    def _validate_workflow_structure(self, inputs: dict, **kwargs):
        ...
        for node in self.graph.nodes:
            ...
            if not all(
                input_name in node_input_names
                for input_name in node_required_input_names
            ):
        ...
        for node in self.graph.nodes:
            for agent in node.agents:
                if hasattr(agent, "forbidden_in_workflow") and (
                    agent.forbidden_in_workflow
                ):
                    raise ValueError(
                        f"The Agent of class {agent.__class__} is forbidden to be used in the workflow."
                    )
    ...
    def _prepare_single_hitl_agent(self, agent: Agent, node: WorkFlowNode):
        ...
        if successors == []:
            ...
            raise ValueError(
                "WorkFlowNode with a HITL Agent can not be set as the ending node."
            )
        ...
        if self.hitl_manager.hitl_input_output_mapping is None:
            raise ValueError(
                "hitl_input_output_mapping attribute missing in HITLManager instance."
            )
        return
            ...
            for agent in agents:
                ...
                if isinstance(agent, dict):
                    agent = self.agent_manager.get_agent(
                        self.agent_manager.get_agent_name(agent)
                    )
