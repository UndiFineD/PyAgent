# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\workflow\workflow_graph.py
import inspect
import json
import threading
...
class WorkFlowNode(BaseModule):
    ...
    def check_agent_format(cls, agents: List[Union[str, dict, Agent]]):
        ...
        for agent in agents:
            if isinstance(agent, str):
                ...
                assert (
                    "name" in agent and "description" in agent
    ...
    def check_action_graph(cls, instance: "WorkFlowNode"):
        ...
        output_param_names = {
            param.name for param in instance.outputs if param.required
        }
        ...
        def check_method_signature(method, method_name):
            ...
            if missing_inputs:
                raise ValueError(
                    f"`{method_name}` method requires parameters that are not in `inputs`: {missing_inputs}"
                )
        ...
        def check_method_return(method_name, result):
            if not isinstance(result, dict):
                raise TypeError(
                    f"{method_name} must return a dictionary, got {type(result)}"
                )
            ...
            if missing_outputs:
                raise ValueError(
                    f"{method_name} return value is missing required outputs: {missing_outputs}"
                )
    ...
    def to_dict(
        ...
        for agent in data.get("agents", []):
            ...
            if (
                ...
                and "parse_func" in agent
                and isinstance(agent["parse_func"], Callable)
            ):
...
class WorkFlowEdge(BaseModule):
    ...
    def __eq__(self, other: "WorkFlowEdge"):
        ...
        return all(
            ...
            for self_attr, other_attr in zip(self_compare_attrs, other_compare_attrs)
        )
...
class WorkFlowGraph(BaseModule):
    ...
    graph: Optional[Union[MultiDiGraph, "WorkFlowGraph"]] = Field(
    ...
    def _init_from_nodes_and_edges(
    ...
    def _init_from_multidigraph(
        ...
        graph_nodes = [
            deepcopy(node_attrs["ref"]) for _, node_attrs in graph.nodes(data=True)
        ]
        graph_edges = [
            deepcopy(edge_attrs["ref"]) for *_, edge_attrs in graph.edges(data=True)
        ]
    ...
    def add_node(self, node: WorkFlowNode, update_graph: bool = True, **kwargs):
        ...
        if self.node_exists(node.name):
            raise ValueError(
                f"Duplicate node names are not allowed! Found duplicate node name: {node.name}"
            )
    ...
    def add_edge(self, edge: WorkFlowEdge, update_graph: bool = True, **kwargs):
        ...
        if self.edge_exists(edge):
            raise ValueError(
                f"Duplicate edges are not allowed! Found duplicate edges: {edge}"
            )
        ...
        if len(source_output_names & target_input_names) == 0:
            logger.warning(
                f"The edge ({edge.source}, {edge.target}) has no matching inputs and outputs! You may need to check the inputs and outputs of the nodes to ensure that at least one input of the target node is the output of the source node."
            )
    ...
    def _edge_exists(self, source: str, target: str, **attr_filters) -> bool:
        ...
        if attr_filters:
            for key, value in attr_filters.items():
                if (
                    key not in self.graph[source][target]
                    or self.graph[source][target][key] != value
                ):
                    return False
        return True

    def edge_exists(
        self, edge: Union[Tuple[str, str], WorkFlowEdge], **attr_filters
    ) -> bool:
        """
        ...
        if isinstance(edge, tuple):
            assert (
                len(edge) == 2
            ), "edge must be a tuple (source, target) or WorkFlowEdge instance"
            source, target = edge
    ...
    def find_loops_with_start_and_end(
        self, start_node: Union[str, WorkFlowNode], end_node: Union[str, WorkFlowNode]
    ) -> List[List[str]]:
        if len(self._loops) == 0:
            return []
    ...
    def get_node(self, node_name: str) -> WorkFlowNode:
        ...
        if not self.node_exists(node=node_name):
            raise KeyError(
                f"{node_name} is an invalid node name. Currently available node names: {self.list_nodes()}"
            )
    ...
    def set_node_status(
        self, node: Union[str, WorkFlowNode], new_state: WorkFlowNodeState
    ) -> bool:
        """
    ...
    def get_uncomplete_initial_nodes(self) -> List[str]:
        initial_nodes = self.find_initial_nodes()
        are_initial_nodes_complete = [
            self.get_node(node_name).is_complete for node_name in initial_nodes
        ]
    ...
    def get_all_paths_from_node(
        self, start_node: Union[str, WorkFlowNode]
    ) -> List[List[str]]:
    ...
    def find_completed_leaf_nodes(
        ...
        paths_starting_from_node = self.get_all_paths_from_node(
    ...
    def find_completed_leaf_nodes_start_from_initial_nodes(self) -> List[str]:
        ...
        for initial_node in initial_nodes:
            for complete_node in self.find_completed_leaf_nodes(
    ...
    def get_all_children_nodes(
        self, nodes: List[Union[str, WorkFlowNode]]
    ) -> List[str]:
    ...
    def get_candidate_children_nodes(
        self, completed_nodes: List[Union[str, WorkFlowNode]]
    ) -> List[str]:
        """
        ...
        """
        node_names = [
            node if isinstance(node, str) else node.name for node in completed_nodes
        ]
        ...
        if has_loop:
                ...
                if self.is_loop_end(node=node_name):
                    ...
                    for child in children_nodes:
                        if self.is_loop_start(node=child):
                            ...
                            current_uncompleted_children_nodes.extend(
                                self.filter_completed_nodes(nodes=[child])
                            )
                else:
                    current_uncompleted_children_nodes = self.filter_completed_nodes(
            ...
            uncompleted_children_nodes = self.filter_completed_nodes(
                nodes=children_nodes
            )
    ...
    def filter_nodes_with_uncompleted_predecessors(
        self, nodes: List[Union[str, WorkFlowNode]]
    ) -> List[str]:
        node_names = [node if isinstance(node, str) else node.name for node in nodes]
    ...
    def get_next_candidate_nodes(self) -> List[str]:
        ...
        candidate_children_nodes = self.get_candidate_children_nodes(
            completed_nodes=completed_leaf_nodes
        )
        ...
        children_nodes_with_complete_predecessors = (
            self.filter_nodes_with_uncompleted_predecessors(candidate_children_nodes)
        )
    ...
    def next(self) -> List[WorkFlowNode]:
        ...
        candidate_tasks = [
            self.get_node(node_name=node_name) for node_name in candidate_node_names
        ]
        return candidate_tasks

    def step(
        self,
        source_node: Union[str, WorkFlowNode],
        target_node: Union[str, WorkFlowNode],
    ):
        ...
        source_node_name = (
            source_node if isinstance(source_node, str) else source_node.name
        )
        target_node_name = (
            target_node if isinstance(target_node, str) else target_node.name
        )
        source_node_status = self.get_node_status(source_node_name)
        if source_node_status != WorkFlowNodeState.COMPLETED:
            raise ValueError(
                f"The state of `source_node` should be WorkFlowNodeState.COMPLETED, but found {source_node_status}"
            )
    ...
    def display(self):
        ...
        node_colors = [
            status_colors.get(self.get_node_status(node), "lightgray")
            for node in self.graph.nodes
        ]

        # Prepare node labels with additional information
        node_labels = {
            node: self.get_node_description(data["ref"])
        ...
        if len(self.graph.nodes) == 1:
            ...
            for node, (x, y) in pos.items():
                if (
                    y < lower_third_boundary
                ):  # If in the lower third, display label above the node
...
class SequentialWorkFlowGraph(WorkFlowGraph):
    ...
    def _infer_node_from_task(self, task: dict) -> WorkFlowNode:
        ...
        if not node_name:
            raise ValueError(
                "The `name` for the following task is required: {}".format(task)
            )
        node_description = task.get("description", None)
        if not node_description:
            raise ValueError(
                "The `description` for the following task is required: {}".format(task)
            )
        ...
        if not agent_prompt and not agent_prompt_template:
            raise ValueError(
                "The `prompt` or `prompt_template` for the following task is required: {}".format(
                    task
                )
            )
            ...
            "tasks": [
                {
                    ...
                    "inputs": [
                        param.to_dict(ignore=["class_name"]) for param in node.inputs
                    ],
                    "outputs": [
                        param.to_dict(ignore=["class_name"]) for param in node.outputs
                    ],
                    "prompt": node.agents[0].get("prompt", None),
                    "prompt_template": (
                        node.agents[0].get("prompt_template", None).to_dict()
                        if node.agents[0].get("prompt_template", None)
                        else None
                    ),
                    "system_prompt": node.agents[0].get("system_prompt", None),
                    "parse_mode": node.agents[0].get("parse_mode", "str"),
                    "parse_func": (
                        node.agents[0].get("parse_func", None).__name__
                        if node.agents[0].get("parse_func", None)
                        else None
                    ),
