# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\utils\aflow_utils\data_utils.py
# Acknowledgement:
...
AFLOW_DATASET_FILES_MAP = {
    "hotpotqa": {
        "train": None,
        "dev": "hotpotqa_validate.jsonl",
        "test": "hotpotqa_test.jsonl",
    },
    "humaneval": {
        "train": None,
        "dev": "humaneval_validate.jsonl",
        "test": "humaneval_test.jsonl",
        "test_cases": "humaneval_public_test.jsonl",
    },
    "mbpp": {
        "train": None,
        "dev": "mbpp_validate.jsonl",
        "test": "mbpp_test.jsonl",
        "test_cases": "mbpp_public_test.jsonl",
    },
...
def download_aflow_benchmark_data(dataset: str, save_folder: str):
    ...
    if dataset.lower() not in lower_candidate_datasets:
        raise ValueError(
            f"Invalid value for dataset: {dataset}. Available choices: {candidate_datasets}"
        )
    ...
    logger.info(
        f"Extracting data for {dataset} dataset(s) from {aflow_data_save_file} ..."
    )
    ...
    if dataset != "all":
        dataset_files = [
            file
            for file in list(AFLOW_DATASET_FILES_MAP[dataset].values())
            if file is not None
        ]
...
class DataUtils:
    ...
    def get_top_rounds(self, sample: int, path=None, mode="Graph"):
        ...
        first_round = next(
            (item for item in self.top_scores if item["round"] == 0), None
        )
        if first_round:
