...
class GraphUtils:
    ...
    def _load_operator_description(
        self, id: int, operator_name: str, llm: BaseLLM
    ) -> str:
    ...
    def update_prompt_import(self, graph_file: str, prompt_folder: str):
        ...
        if not prompt_folder_path.is_absolute():
            ...
            if not prompt_folder_full_path.exists():
                raise ValueError(
                    f"Prompt folder {prompt_folder_full_path} does not exist!"
                )
        ...
        try:
            ...
            raise ValueError(
                f"Prompt folder {prompt_folder} must be within the project directory"
            )
