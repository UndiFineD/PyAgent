# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\utils\mipro_utils\module_utils.py
import asyncio
import copy
...
class PromptTuningModule(dspy.Module):
    ...
    def from_registry(
        ...
        return cls(
            ...
            registry=registry,
            signature_name2register_name=signature_name2register_name,
        )
    ...
    def reset(self):
        ...
        for predict in self.predicts:
            ...
            if isinstance(register_element, PromptTemplate):
                ...
                logger.warning(
                    f"Unsupported register element type: {type(register_element)}"
                )
    ...
    def _validate_prompt(
        self, prompt: str, input_names: List[str], verbose: bool = True
    ) -> str:
        """
        ...
        missing_required_inputs = [
            name for name in required_inputs if f"{{{name}}}" not in prompt
        ]
        if missing_required_inputs:
            input_values = "\n\n".join(
                [f"{name}: {{{name}}}" for name in missing_required_inputs]
            )
            prompt += (
                f"\n\nThe followings are some required input values: \n{input_values}"
            )
            modified_messages.append(
                f"added missing inputs: {', '.join(missing_required_inputs)}"
            )
    ...
    def get_field_type(self, field: Field) -> str:
        ...
        return (
            ...
            if field.json_schema_extra.get("__dspy_field_type")
            else None
        )
    ...
    def is_prompt_template(self, register_name: str) -> bool:
        ...
        """
        return self.registry.get(register_name) is not None and isinstance(
            self.registry.get(register_name), PromptTemplate
        )
    ...
    def sync_predict_inputs_to_program(self):
        ...
        for predict in self.predicts:
            ...
            input_names = [
                name
                for name, field in predict.signature.fields.items()
                if self.get_field_type(field) == "input"
            ]
            output_names = [
                name
                for name, field in predict.signature.fields.items()
                if self.get_field_type(field) == "output"
            ]
            ...
            if self.is_prompt_template(register_name):
                ...
                prompt = self._inject_demos_to_string(
                    instruction, self.get_demos(demos), input_names, output_names
                )
        ...
        **kwargs : dict
        ...
        if asyncio.iscoroutinefunction(self.program):
            output, execution_data = (
                asyncio.run(self.program(**kwargs))
                if kwargs
                else asyncio.run(self.program())
            )
        else:
            output, execution_data = (
                self.program(**kwargs) if kwargs else self.program()
            )
