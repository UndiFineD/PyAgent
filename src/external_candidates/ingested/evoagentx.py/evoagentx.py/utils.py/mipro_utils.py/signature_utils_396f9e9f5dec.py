# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\utils\mipro_utils\signature_utils.py
import ast
import importlib
...
def signature_from_registry(
    ...
    for key in registry.names():
        ...
        if is_valid_identifier(key):
            ...
            # 打印warning
            print(
                f"Warning: The key `{key}` is not a valid identifier, so we will add an underscore to it."
            )
            signature_name = f"DefaultSignature_{len(signature_dict)}"

        signature_class = make_signature(
            signature=sig,
