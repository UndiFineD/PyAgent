# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\utils\utils.py
import os
import re
import time
    ...
    for attempt in range(max_retries):
        try:
            ...
            response = requests.get(
                url=url, stream=True, headers=headers, timeout=timeout
            )
            response.raise_for_status()
            # total_size = int(response.headers.get("content-length", 0))
            mode = "ab" if resume_byte_pos else "wb"
            progress_bar = tqdm(
                total=total_size, unit="iB", unit_scale=True, initial=resume_byte_pos
            )
            ...
            logger.warning(
                f"Download error: {e}. Retrying ({attempt+1}/{max_retries})..."
            )
            time.sleep(5)
