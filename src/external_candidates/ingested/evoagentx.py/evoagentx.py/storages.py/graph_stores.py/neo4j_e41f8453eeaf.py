# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\storages\graph_stores\neo4j.py
import json
...
class Neo4jGraphStoreWrapper(GraphStoreBase):
    ...
    def __init__(
        self, uri: str, username: str, password: str, database: str = "neo4j", **kwargs
    ):
        try:
    ...
    def supports_vector_queries(self):
        return (
            self.graph_store.supports_vector_queries
            and self.graph_store._supports_vector_index
        )
    ...
    def verify_version(self):
        ...
        if version_tuple >= target_version:
            ...
            logger.warning(
                f"The version of Neo4j server is {version_tuple}, which is less than {target_version}. Disable the vector indexing."
            )
    ...
    async def aload(self, node: Union[LabelledNode, Relation, BaseNode]) -> None:
        ...
        try:
            ...
            if not isinstance(node, (BaseNode, EntityNode, ChunkNode, Relation)):
                raise ValueError(
                    f"Unsupported node type: {type(node)}. Must be BaseNode, EntityNode, ChunkNode, Relation."
                )
    ...
    def build_kv_store(
        self,
    ) -> Sequence[Union[LabelledNode, EntityNode, ChunkNode, Relation]]:
        """
        ...
        try:
                ...
                if "Chunk" in labels:
                    ...
                    node_dict["label"] = (
                        [
                            label
                            for label in labels
                            if label not in [BASE_NODE_LABEL, BASE_ENTITY_LABEL]
                        ][0]
                        if any(
                            label not in [BASE_NODE_LABEL, BASE_ENTITY_LABEL]
                            for label in labels
                        )
                        else "entity"
                    )
                    node = EntityNode(
                        ...
                        properties={
                            "triplet_source_id": node_dict["properties"][
                                "triplet_source_id"
                            ]
                        },
                    )
                    ...
                    logger.warning(
                        f"Skipping node with id {record['id']} due to unsupported labels: {labels}"
                    )
                    continue
            ...
            relations = [
                Relation(
                    ...
                    properties=(
                        json.loads(record["properties"].get("metadata", {}))
                        if isinstance(record["properties"].get("metadata", {}), str)
                        else record["properties"].get("metadata", {})
                    ),
                )
            ...
            logger.info(
                f"Exported {len(nodes)} nodes and {len(relations)} relations from Neo4j graph store"
            )
