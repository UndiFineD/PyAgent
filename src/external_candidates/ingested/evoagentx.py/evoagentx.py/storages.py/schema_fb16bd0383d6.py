...
class MemoryStore(BaseModel):
    ...
    key_words: Optional[List] = Field(
        [], description="Optional list of keywords associated with the memory"
    )
    entity_content: Optional[Dict[str, Any]] = Field(
        {}, description="Optional dictionary of entity-related content"
    )
    embedding: Optional[List] = Field(
        [], description="Optional list of embedding vectors"
    )
...
class WorkflowStore(BaseModel):
    ...
    content: Dict[str, Any] = Field(
        ..., description="Dictionary containing workflow details"
    )
...
class AgentStore(BaseModel):
    ...
    content: Dict[str, Any] = Field(
        ..., description="Dictionary containing agent details"
    )
...
class HistoryStore(BaseModel):
    ...
    updated_at: Optional[str] = Field(
        "", description="Optional timestamp for last update"
    )
...
class IndexStore(BaseModel):
    ...
    content: Dict[str, Any] = Field(
        ..., description="Serialized index content (e.g., LlamaIndex JSON)"
    )
