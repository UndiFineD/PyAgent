# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\storages\vectore_stores\faiss.py
...
class FaissVectorStoreWrapper(VectorStoreBase):
    ...
    def __init__(
        self,
        dimensions: int = 1536,
        metrics: Union[Literal["flat_l2", "ivf_flat"]] = "flat_l2",
        **kwargs,
    ):
        self.dimensions = dimensions
        self.metrics = metrics
        self.faiss_index = self._create_index()
        self.vector_store = FaissMapVectorStore(
            faiss_index=faiss.IndexIDMap2(self.faiss_index)
        )
    ...
    async def aload(self, node: BaseNode) -> None:
        ...
        try:
            if not isinstance(node, BaseNode):
                raise ValueError(
                    f"Unsupported node type: {type(node)}. Must be Chunk or BaseNode."
                )
            ...
            if node_id in existing_ids:
                logger.info(
                    f"Node with ID {node_id} already exists in FAISS vector store, skipping insertion."
                )
                return
            ...
            logger.error(
                f"Failed to load node with ID {node_id} into FAISS vector store: {str(e)}"
            )
            raise
