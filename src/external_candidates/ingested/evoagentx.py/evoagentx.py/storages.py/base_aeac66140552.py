...
class StorageHandler(BaseModule):
    ...
    storageConfig: StoreConfig = Field(
        ..., description="Configuration for all storage backends"
    )
    storageDB: Optional[Union[DBStoreBase, Any]] = Field(
        None, description="Database storage backend"
    )
    vector_store: Optional[Union[VectorStoreBase, Any]] = Field(
        None, description="Single vector storage backend"
    )
    graph_store: Optional[Union[GraphStoreBase, Any]] = Field(
        None, description="Optional graph storage backend"
    )
    ...
    def save(self, data: Dict[str, Any], *args, **kwargs):
        ...
        for table_name, records in data.items():
            ...
            for record in records:
                self.storageDB.insert(
                    metadata=record, store_type=store_type, table=table_name
                )
    ...
    def load_memory(
        self, memory_id: str, table: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def save_memory(
        self, memory_data: Dict[str, Any], table: Optional[str] = None, **kwargs
    ):
        """
        ...
        if existing:
            self.storageDB.update(
                memory_id, new_metadata=memory_data, store_type="memory", table=table
            )
        else:
            self.storageDB.insert(
                metadata=memory_data, store_type="memory", table=table
            )

    def load_agent(
        self, agent_name: str, table: Optional[str] = None, *args, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def save_agent(
        self, agent_data: Dict[str, Any], table: Optional[str] = None, *args, **kwargs
    ):
        """
        ...
        if existing:
            self.storageDB.update(
                agent_name, new_metadata=agent_data, store_type="agent", table=table
            )
        else:
            self.storageDB.insert(metadata=agent_data, store_type="agent", table=table)

    def load_workflow(
        self, workflow_id: str, table: Optional[str] = None, *args, **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        result = self.storageDB.get_by_id(
            workflow_id, store_type="workflow", table=table
        )
        # Parse the result to convert JSON strings to Python objects
        if result is not None:
    ...
    def save_workflow(
        ...
        **kwargs,
        ...
        existing = self.storageDB.get_by_id(
            workflow_id, store_type="workflow", table=table
        )
        if existing:
            ...
            self.storageDB.insert(
                metadata=workflow_data, store_type="workflow", table=table
            )

    def load_history(
        self, memory_id: str, table: Optional[str] = None, *args, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def save_history(
        self, history_data: Dict[str, Any], table: Optional[str] = None, *args, **kwargs
    ):
        """
        ...
        existing = self.storageDB.get_by_id(
            memory_id, store_type="history", table=table
        )
        if existing:
            # parse the history, then change the old_hisotry
            result = HistoryStore.model_validate(
                self.parse_result(existing, HistoryStore)
            )
            history_data["old_memory"] = result.old_memory
            self.storageDB.update(
                memory_id, new_metadata=history_data, store_type="history", table=table
            )
        else:
            self.storageDB.insert(
                metadata=history_data, store_type="history", table=table
            )

    def load_index(
        self, corpus_id: str, table: Optional[str] = None
    ...
    def save_index(self, index_data: Dict[str, Any], table: Optional[str] = None):
        ...
        existing = self.storageDB.get_by_id(
            corpus_id, store_type="indexing", table=table
        )
        if existing:
            self.storageDB.update(
                corpus_id, new_metadata=index_data, store_type="indexing", table=table
            )
        else:
            self.storageDB.insert(
                metadata=index_data, store_type="indexing", table=table
            )
