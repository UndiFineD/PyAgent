...
def _create_table(table: str, column: List[str]) -> str:
    ...
    column_defs = [f'"{column[0]}" TEXT PRIMARY KEY'] + [
        f'"{col}" TEXT' for col in column[1:]
    ]
...
class SQLite(DBStoreBase):
    ...
    def insert_memory(
        ...
        store_type: Optional[
            Literal["memory", "agent", "workflow", "history", "indexing"]
        ],
        ...
        **kwargs,
        ...
        with self._lock:
            with self.connection:
                ...
                insert_string = _insert_meta(
                    table, list(MemoryStore.model_fields.keys())
                )
        ...
        **kwargs,
        ...
        with self._lock:
            with self.connection:
                ...
                insert_string = _insert_meta(
                    table, list(AgentStore.model_fields.keys())
                )
                self.connection.execute(
                    insert_string,
                    tuple(
        ...
        **kwargs,
        ...
        with self._lock:
            with self.connection:
                ...
                insert_string = _insert_meta(
                    table, list(WorkflowStore.model_fields.keys())
                )
                self.connection.execute(
                    insert_string,
                    tuple(
        ...
        **kwargs,
        ...
        with self._lock:
            with self.connection:
                ...
                insert_string = _insert_meta(
                    table, list(HistoryStore.model_fields.keys())
                )
                self.connection.execute(
                    insert_string,
                    tuple(
        ...
        **kwargs,
        ...
        with self._lock:
            with self.connection:
                ...
                insert_string = _insert_meta(
                    table, list(IndexStore.model_fields.keys())
                )
                self.connection.execute(
                    insert_string,
                    tuple(
    ...
    def insert(
        ...
        **kwargs,
        ...
        if store_type == TableType.store_memory:
            self.insert_memory(
                metadata, store_type=store_type, table=table, *args, **kwargs
            )
        elif store_type == TableType.store_agent:
            self.insert_agent(
                metadata, store_type=store_type, table=table, *args, **kwargs
        ...
        **kwargs,
        ...
        with self._lock:
            with self.connection:
                ...
                update_query = (
                    f'UPDATE {table} SET {set_clause} WHERE "{columns[0]}" = ?'
                )

                values = list(
                    [
                        json.dumps(v) if not isinstance(v, str) else v
                        for v in new_metadata.model_dump().values()
                    ]
                )[1:] + [metadata_id]
