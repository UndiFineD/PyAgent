# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\agents\customize_agent.py
import inspect
import json
...
class CustomizeAgent(Agent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if tools is not None:
            ...
            tools = [
                (
                    ...
                    else Toolkit(name=tool.name, tools=[tool])
                )
                for tool in tools
            ]
        else:
        ...
        if prompt is not None and prompt_template is not None:
            logger.warning(
                "Both `prompt` and `prompt_template` are provided in `CustomizeAgent`. `prompt_template` will be used."
            )
            prompt = None
        ...
        if isinstance(parse_func, str):
            if not PARSE_FUNCTION_REGISTRY.has_function(parse_func):
                raise ValueError(
                    f"parse function `{parse_func}` is not registered! To instantiate a CustomizeAgent from a file, you should use decorator `@register_parse_function` to register the parse function."
                )
    ...
    def validate_data(
        ...
        if prompt is None and prompt_template is None:
            raise ValueError(
                "`prompt` or `prompt_template` is required when creating a CustomizeAgent."
            )
        ...
        if prompt_template is None and inputs:
            ...
            inputs_names_not_in_prompt = [
                name for name in all_input_names if f"{{{name}}}" not in prompt
            ]
            if inputs_names_not_in_prompt:
                raise KeyError(
                    f"The following inputs are not found in the prompt: {inputs_names_not_in_prompt}."
                )
        ...
        if parse_mode == "custom":
            if parse_func is None:
                raise ValueError(
                    "`parse_func` (a callable function with an input argument `content`) must be provided when `parse_mode` is 'custom'."
                )

        if parse_func is not None:
            if not callable(parse_func):
                raise ValueError(
                    "`parse_func` must be a callable function with an input argument `content`."
                )
        ...
        if title_format is not None:
            if parse_mode != "title":
                logger.warning(
                    f"`title_format` will not be used because `parse_mode` is '{parse_mode}', not 'title'. Set `parse_mode='title'` to use title formatting."
                )
            if r"{title}" not in title_format:
                raise ValueError(
                    r"`title_format` must contain the placeholder `{title}`."
                )
    ...
    def create_customize_action(
        ...
        """
        assert (
            prompt is not None or prompt_template is not None
        ), "must provide `prompt` or `prompt_template` when creating CustomizeAgent"
    ...
    def _check_output_parser(
        self, outputs: List[dict], output_parser: Type[ActionOutput]
    ):

        if output_parser is not None:
            if not isinstance(output_parser, type):
                raise TypeError(
                    f"output_parser must be a class, but got {type(output_parser).__name__}"
                )
            if not issubclass(output_parser, ActionOutput):
                raise ValueError(
                    f"`output_parser` must be a class and a subclass of `ActionOutput`, but got `{output_parser.__name__}`."
                )
    ...
    def _store_inputs_outputs_info(
        ...
        inputs: List[dict],
        outputs: List[dict],
        tool_map: Dict[str, Union[Toolkit, Tool]],
    ):
    ...
    def get_customize_agent_info(self) -> dict:
        ...
        config = {
            ...
            "prompt_template": (
                customize_action.prompt_template.to_dict()
            ...
            "output_parser": (
                self.output_parser.__name__ if self.output_parser is not None else None
            ),
            "parse_mode": self.parse_mode,
            "parse_func": (
                self.parse_func.__name__ if self.parse_func is not None else None
            ),
            "title_format": self.title_format,
            "tool_names": (
                [tool.name for tool in customize_action.tools]
                if customize_action.tools
                else []
            ),
    ...
    def load_module(
        ...
        tools: List[Union[Toolkit, Tool]] = None,
        **kwargs,
    ) -> "CustomizeAgent":
        """
        ...
        if agent.get("tool_names", None):
            assert (
                tools is not None
            ), "must provide `tools: List[Union[Toolkit, Tool]]` when using `load_module` or `from_file` to load the agent from local storage and `tool_names` is not None or empty"
