# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\agents\agent.py
import asyncio
import inspect
...
class Agent(BaseModule):
    ...
    short_term_memory: Optional[ShortTermMemory] = Field(
        default_factory=ShortTermMemory
    )  # store short term memory for a single workflow.
    ...
    n: int = Field(
        default=None,
        description="number of latest messages used to provide context for action execution. It uses all the messages in short term memory by default.",
    )
    ...
    def init_module(self):
        ...
        self._action_map = (
            {action.name: action for action in self.actions} if self.actions else dict()
        )
    ...
    def _prepare_execution(
        ...
        **kwargs,
        ...
        """
        assert (
            msgs is not None or action_input_data is not None
        ), "must provide either `msgs` or `action_input_data`"
    ...
    def init_llm(self):
        ...
        if not self.is_human and (not self.llm_config and not self.llm):
            raise ValueError(
                "must provide `llm_config` or `llm` when `is_human` is False"
            )
    ...
    def init_long_term_memory(self):
        ...
        """
        assert (
            self.storage_handler is not None
        ), "must provide ``storage_handler`` when use_long_term_memory=True"
        ...
        if not self.long_term_memory_manager:
            self.long_term_memory_manager = MemoryManager(
                storage_handler=self.storage_handler, memory=self.long_term_memory
            )
    ...
    def check_action_name(self, action_name: str):
        ...
        if action_name not in self._action_map:
            raise KeyError(
                f"'{action_name}' is an invalid action for {self.name}! Available action names: {list(self._action_map.keys())}"
            )
    ...
    def get_action_name(self, action_cls: Type[Action]) -> str:
        ...
        for name, action in self._action_map.items():
            if isinstance(action, action_cls):
                return name
        raise ValueError(
            f"Couldn't find an action that matches Type '{action_cls.__name__}'"
        )
    ...
    def get_action_inputs(self, action: Action) -> Union[dict, None]:
        ...
        action_inputs = cext_action.execute(
            llm=self.llm, action=action, context=context
        )
    ...
    def get_all_actions(self) -> List[Action]:
        ...
        """
        actions = [
            action for action in self.actions if action.name != self.cext_action_name
        ]
        return actions
    ...
    def get_agent_profile(self, action_names: List[str] = None) -> str:
        ...
        if action_names is None:
            ...
            action_descriptions = "\n".join(
                [f"  - {action.name}: {action.description}" for action in all_actions]
            )
        else:
    ...
    def set_prompt(
        self, action_name: str, prompt: str, system_prompt: Optional[str] = None
    ) -> bool:
        """
    ...
    def set_prompts(self, prompts: dict) -> bool:
        ...
        for action_name, prompt_data in prompts.items():
            ...
            if not isinstance(prompt_data, dict):
                raise ValueError(
                    f"Invalid prompt data for action '{action_name}'. Expected a dictionary with 'prompt' and 'system_prompt' (optional) keys."
                )
            if "prompt" not in prompt_data:
                raise ValueError(
                    f"Missing 'prompt' key in prompt data for action '{action_name}'."
                )
            self.set_prompt(
                action_name,
                prompt_data["prompt"],
                prompt_data.get("system_prompt", None),
            )
        return True
