# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\agents\long_term_memory_agent.py
import asyncio
import json
...
class MemoryActionInput(ActionInput):
    ...
    conversation_id: Optional[str] = Field(
        default=None, description="ID for tracking conversation"
    )
    top_k: Optional[int] = Field(
        default=5, description="Number of memory results to retrieve"
    )
    metadata_filters: Optional[Dict] = Field(
        default=None, description="Filters for memory retrieval"
    )
...
class MemoryAction(Action):
    ...
    def execute(
        ...
        **kwargs,
        ...
        return asyncio.run(
            self.async_execute(
                llm, inputs, sys_msg, return_prompt, memory_manager, **kwargs
            )
        )
    ...
    async def async_execute(
        ...
        **kwargs,
        ...
        if not conversation_id:
            ...
            logger.warning(
                "No conversation_id provided; generated a new UUID4 for this session"
            )
        ...
        action_input_data = {
            attr: getattr(action_input, attr, "undefined")
            for attr in action_input_attrs
        }
        ...
        final_output = self.outputs_format(
            response=output.content, memory_ids=memory_ids
        )
...
class MemoryAgent(Agent):
    memory_manager: Optional[MemoryManager] = Field(
        default=None, description="Manager for long-term memory operations"
    )
    inputs: List[Dict] = Field(
        default_factory=list, description="Input specifications for the memory action"
    )
    outputs: List[Dict] = Field(
        default_factory=list, description="Output specifications for the memory action"
    )
    ...
    def _create_output_message(
        ...
        **kwargs,
        ...
        if action_input_data and "user_prompt" in action_input_data:
            ...
            asyncio.create_task(
                self.memory_manager.handle_memory(action="add", data=user_msg)
            )

        response_msg = Message(
            content=(
                action_output.response
                if hasattr(action_output, "response")
        ...
        asyncio.create_task(
            self.memory_manager.handle_memory(action="add", data=response_msg)
        )
    ...
    def _default_conversation_id(self) -> str:
        ...
        if scope == "session":
            return str(uuid4())
        return (
            getattr(getattr(self, "long_term_memory", None), "default_corpus_id", None)
            or "global_corpus"
        )
    ...
    def save_module(
        self,
        path: str,
        ignore: List[str] = ["llm", "llm_config", "memory_manager"],
        **kwargs,
    ) -> str:
        """
        ...
        **kwargs,
        ...
        return cls(
            ...
            description=config.get(
                "description", "An agent that uses long-term memory"
            ),
            llm_config=llm_config,
            storage_handler=storage_handler,
            rag_config=rag_config,
