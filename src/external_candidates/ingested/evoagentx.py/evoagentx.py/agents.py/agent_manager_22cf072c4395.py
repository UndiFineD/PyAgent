# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\agents\agent_manager.py
import threading
...
class AgentManager(BaseModule):
    ...
    )  # agent_name to AgentState mapping
    storage_handler: Optional[StorageHandler] = (
        None  # used to load and save agent from storage.
    )
    ...
    def init_module(self):
        ...
        if self.agents:
            for agent in self.agents:
                self.agent_states[agent.name] = self.agent_states.get(
                    agent.name, AgentState.AVAILABLE
                )
    ...
    def check_agents(self):
        ...
        if duplicate_agent_names:
            raise ValueError(
                f"The agents should be unique. Found duplicate agent names: {duplicate_agent_names}!"
            )
        # check agent states
        if len(self.agents) != len(self.agent_states):
            raise ValueError(
                f"The lengths of self.agents ({len(self.agents)}) and self.agent_states ({len(self.agent_states)}) are different!"
            )
        missing_agents = self.find_missing_agent_states()
        if missing_agents:
            raise ValueError(
                f"The following agents' states were not found: {missing_agents}"
            )
    ...
    def find_missing_agent_states(self):
        missing_agents = [
            agent.name for agent in self.agents if agent.name not in self.agent_states
        ]
    ...
    def load_agent(self, agent_name: str, **kwargs) -> Agent:
        ...
        if not self.storage_handler:
            raise ValueError(
                "must provide ``self.storage_handler`` to use ``load_agent``"
            )
    ...
    def create_customize_agent(
        ...
        **kwargs,
        ...
        if not agent_data.get("is_human", False) and not agent_llm_config:
            raise ValueError(
                "`agent_data` should contain a `llm_config` key or `llm_config` should be provided."
            )
    ...
    def get_agent_name(self, agent: Union[str, dict, Agent]) -> str:
        ...
        if isinstance(agent, str):
            ...
            raise ValueError(
                f"{type(agent)} is not a supported type for ``get_agent_name``. Supported types: [str, dict, Agent]."
            )
        return agent_name
        ...
        if isinstance(agent, str):
            if self.storage_handler is None:
                ...
                if not self.has_agent(agent_name=agent):
                    raise ValueError(
                        f"Agent ``{agent}`` does not exist! You should provide a dictionary or an Agent instance when ``self.storage_handler`` is not provided."
                    )
            ...
            if not agent.get("is_human", False) and (
                llm_config is None and "llm_config" not in agent
            ):
                raise ValueError(
                    "When providing an agent as a dictionary, you must either include 'llm_config' in the dictionary or provide it as a parameter."
                )
            agent_instance = self.create_customize_agent(
                agent_data=agent, llm_config=llm_config, **kwargs
            )
        elif isinstance(agent, Agent):
            agent_instance = agent
        else:
            raise ValueError(
                f"{type(agent)} is not a supported input type of ``create_agent``. Supported types: [str, dict, Agent]."
            )
    ...
    def add_agents(
        self,
        agents: List[Union[str, dict, Agent]],
        llm_config: Optional[LLMConfig] = None,
        **kwargs,
    ):
        """
    ...
    def add_agents_from_workflow(
        self, workflow_graph, llm_config: Optional[LLMConfig] = None, **kwargs
    ):
        """
    ...
    def update_agents_from_workflow(
        self, workflow_graph, llm_config: Optional[LLMConfig] = None, **kwargs
    ):
        """
            ...
            if node.agents:
                for agent in node.agents:
                    ...
                    if self.has_agent(agent_name=agent_name):
                        ...
                        self.update_agent(
                            agent=agent, llm_config=agent_llm_config, **kwargs
                        )
                    else:
                        self.add_agent(agent=agent, llm_config=llm_config, **kwargs)
    ...
    @atomic_method
    def remove_agent(
        self, agent_name: str, remove_from_storage: bool = False, **kwargs
    ):
        """
