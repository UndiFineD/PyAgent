# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\agents\action_agent.py
import asyncio
import json
...
from ..core.registry import ACTION_FUNCTION_REGISTRY, MODULE_REGISTRY
...
class ActionAgent(Agent):
    ...
    def _validate_inputs_outputs(self, inputs: List[dict], outputs: List[dict]):
        ...
        for i, input_field in enumerate(inputs):
            if not isinstance(input_field, dict):
                raise ValueError(
                    f"Input field {i} must be a dictionary, got {type(input_field)}"
                )
            ...
            if not isinstance(input_field["name"], str):
                raise ValueError(
                    f"Input field {i} 'name' must be a string, got {type(input_field['name'])}"
                )

            if not isinstance(input_field["type"], str):
                raise ValueError(
                    f"Input field {i} 'type' must be a string, got {type(input_field['type'])}"
                )

            if not isinstance(input_field["description"], str):
                raise ValueError(
                    f"Input field {i} 'description' must be a string, got {type(input_field['description'])}"
                )
            ...
            if len(input_names) != len(set(input_names)):
                raise ValueError(
                    f"Duplicate input names found: {[name for name in input_names if input_names.count(name) > 1]}"
                )
        ...
        for i, output_field in enumerate(outputs):
            if not isinstance(output_field, dict):
                raise ValueError(
                    f"Output field {i} must be a dictionary, got {type(output_field)}"
                )
            ...
            if not isinstance(output_field["name"], str):
                raise ValueError(
                    f"Output field {i} 'name' must be a string, got {type(output_field['name'])}"
                )

            if not isinstance(output_field["type"], str):
                raise ValueError(
                    f"Output field {i} 'type' must be a string, got {type(output_field['type'])}"
                )

            if not isinstance(output_field["description"], str):
                raise ValueError(
                    f"Output field {i} 'description' must be a string, got {type(output_field['description'])}"
                )
            ...
            if len(output_names) != len(set(output_names)):
                raise ValueError(
                    f"Duplicate output names found: {[name for name in output_names if output_names.count(name) > 1]}"
                )

    def _create_function_action_input_type(
        self, name: str, inputs: List[dict]
    ) -> Type[ActionInput]:
    ...
    def _create_function_action_output_type(
        self, name: str, outputs: List[dict]
    ) -> Type[ActionOutput]:
    ...
    def _create_execute_method(self, execute_func: Callable):
        ...
        def execute_method(
            ...
            **kwargs,
            ...
            missing_inputs = [
                input_name for input_name in required_inputs if input_name not in inputs
            ]
            if missing_inputs:
    ...
    def _create_async_execute_method(
        self, async_execute_func: Callable, execute_func: Callable
    ):
        """Create the async execute method for the action."""

        async def async_execute_method(
            action_self,
            llm=None,
            inputs=None,
            sys_msg=None,
            ...
            try:
                if async_execute_func is not None:
                    ...
                    result = await loop.run_in_executor(
                        None, lambda: execute_func(**filtered_inputs)
                    )
            except Exception as e:
                # Create error output - try to use error field if it exists, otherwise use first available field
                try:
    ...
    def _create_function_action_with_params(
        self,
        name: str,
        execute_func: Callable,
        async_execute_func: Callable,
        inputs: List[dict],
        outputs: List[dict],
    ) -> Action:
        ...
        async_execute_method = self._create_async_execute_method(
            async_execute_func, execute_func
        )

        # Bind the methods to the action instance
        function_action.execute = execute_method.__get__(
            function_action, type(function_action)
        )
        function_action.async_execute = async_execute_method.__get__(
            function_action, type(function_action)
        )

        return function_action

    def _create_function_action(
        self,
        name: str,
        execute_func: Callable,
        async_execute_func: Callable,
        ...
        config.update(
            {
                ...
                "execute_func_name": (
                    self.execute_func.__name__ if self.execute_func else None
                ),
                "async_execute_func_name": (
                    self.async_execute_func.__name__
                    if self.async_execute_func
                    else None
                ),
                "inputs": self.inputs,
    ...
    def load_module(
        ...
        if execute_func_name:
            if not ACTION_FUNCTION_REGISTRY.has_function(execute_func_name):
                raise KeyError(
                    f"Function '{execute_func_name}' not found in registry. Please register it first."
                )
        ...
        if async_execute_func_name:
            if not ACTION_FUNCTION_REGISTRY.has_function(async_execute_func_name):
                raise KeyError(
                    f"Function '{async_execute_func_name}' not found in registry. Please register it first."
                )
            async_execute_func = ACTION_FUNCTION_REGISTRY.get_function(
                async_execute_func_name
            )
