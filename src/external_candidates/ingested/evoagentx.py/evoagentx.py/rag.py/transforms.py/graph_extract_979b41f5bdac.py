# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\transforms\graph_extract.py
import asyncio
from typing import Any, Optional, Sequence

from evoagentx.core.logging import logger
from evoagentx.models.base_model import BaseLLM, LLMOutputParser
from evoagentx.prompts.rag.graph_extract import (
    ...
    async def _aextract(self, node: BaseNode) -> BaseNode:
        ...
        try:
            ...
            entity_label_mapping = {
                ...
                for entity_dict in LLMOutputParser._parse_json_content(json_string)[
                    "entities"
                ]
            }

            # Step 2: Extract relations between entities
            relation_extract_prompt = self.relation_extract_prompt.replace(
                "{text}", text
            ).replace("{entities_json}", json_string)
            ...
            triples = LLMOutputParser._parse_json_content(llm_response.content.strip())[
                "graph"
            ]

        except ValueError as e:
            logger.warning(
                f"Failed to parse LLM output for node {node.node_id}: {str(e)}. Returning empty triples."
            )
            entity_label_mapping = {}
            triples = []
