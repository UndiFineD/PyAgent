...
class SimpleReranker(BasePostprocessor):
    ...
    def __init__(
        ...
        if keyword_filters:
            self.postprocessors.append(
                KeywordNodePostprocessor(required_keywords=keyword_filters)
            )
        ...
        try:
            ...
            for score_node in nodes:
                ...
                if original_chunk:
                    ...
                    chunk_class = (
                        type(results[0].corpus.chunks[0])
                        if results and results[0].corpus.chunks
                        else Chunk
                    )
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to reconstruct chunk from node: {e}"
                        )
                        continue
