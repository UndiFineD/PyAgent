...
class DocumentMetadata(BaseModule):
    ...
    file_name: Optional[str] = Field(
        default=None, description="The name of the document file, excluding the path."
    )
    file_path: Optional[str] = Field(
        default=None, description="The file path or URL where the document is stored."
    )
    ...
    file_size: Optional[int] = Field(
        default=None, description="The size of the document."
    )
    ...
    creation_date: Optional[str] = Field(
        default=None, description="The creation date and time of the document."
    )
    ...
    hash_doc: Optional[str] = Field(
        default=None, description="The hash code of this Document for deduplication"
    )
...
class GraphNodeData(BaseModule):
    ...
    # Relation
    node_class_name: Optional[str] = Field(
        default=None, description="The class name of the source llama_index node."
    )
    properties: Optional[Dict] = Field(
        default_factory=dict, description="Represents all information from the Node."
    )

    # Entity Node
    node_name: Optional[str] = Field(
        default=None, description="Entity name of each node."
    )
    ...
    text: Optional[str] = Field(
        default=None, description="The text stored in the ChunkNode."
    )
...
class ChunkMetadata(DocumentMetadata):
    ...
    doc_id: Optional[str] = Field(
        default=None, description="The unique identifier of the parent document."
    )
    corpus_id: Optional[str] = Field(
        default=None, description="The unique identifier of the Corpus(Indexing)."
    )
    chunk_size: Optional[int] = Field(
        default=None, description="The size of the chunk in characters, if applicable."
    )
    ...
    chunk_index: Optional[int] = Field(
        default=None, description="The index of the chunk within the parent document."
    )
    ...
    similarity_score: Optional[float] = Field(
        default=None, description="Similarity score from retrieval."
    )
    # graph support
    graph_node: Optional[GraphNodeData] = Field(
        default=None, description="The properties of all types of graph nodes."
    )
    # LongTermMemory
    ...
    memory_id: Optional[str] = Field(
        default=None, description="Unique identifier for memory entries."
    )
    ...
    next_actions: Optional[List[str]] = Field(
        default=None, description="The following actions after the message."
    )
    wf_task: Optional[str] = Field(
        default=None, description="The name of a task in the workflow."
    )
    wf_task_desc: Optional[str] = Field(
        default=None, description="The description of a task in the workflow."
    )
    ...
    wf_goal: Optional[str] = Field(
        default=None, description="the goal of the whole workflow."
    )
    timestamp: Optional[str] = Field(
        default=None, description="the timestame of the message. "
    )
...
class IndexMetadata(BaseModule):
    ...
    index_type: str = Field(
        ..., description="Type of index (e.g., 'vector', 'graph', 'summary', 'tree')"
    )
    ...
    graph_db_type: Optional[str] = Field(
        default=None, description="Graph database type (e.g., 'neo4j')"
    )
    embedding_model_name: Optional[str] = Field(default=None, description="")
    date: Optional[str] = Field(
        default=None, description="Creation or last update date"
    )
...
class Document(BaseModule):
    ...
    def __init__(
        ...
        metadata = (
            ...
            else metadata or DocumentMetadata()
        )
        ...
        super().__init__(
            ...
            excluded_embed_metadata_keys=list(
                set(DEAFULT_EXCLUDED + excluded_embed_metadata_keys)
            ),
            excluded_llm_metadata_keys=list(
                set(DEAFULT_EXCLUDED + excluded_llm_metadata_keys)
            ),
...
class TextChunk(BaseModule):
    ...
    def __init__(
        ...
        metadata = (
            ...
            if isinstance(metadata, dict)
            else metadata or ChunkMetadata()
        )
        super().__init__(
            text=text.strip(),
    ...
    def to_llama_node(self) -> Union[TextNode, Relation, EntityNode, ChunkNode]:
        ...
        for k, v in self.relationships.items():
            relatiuonships[k] = (
                v if isinstance(v, RelatedNodeInfo) else RelatedNodeInfo.from_dict(v)
            )
        ...
        if self.metadata.graph_node is not None:
            ...
            if class_name == "relation":
                cls = Relation(
                    ...
                    properties={
                        "metadata": json.dumps(
                            self.metadata.graph_node.properties["metadata"]
                        )
                    },
                )
                ...
                cls = EntityNode(
                    ...
                    properties={
                        "triplet_source_id": self.metadata.graph_node.properties[
                            "triplet_source_id"
                        ]
                    },
                )
    ...
    @classmethod
    def from_llama_node(
        cls, node: Union[TextNode, Relation, EntityNode, ChunkNode]
    ) -> "Chunk":
        ...
        if isinstance(node, TextNode):
            ...
            properties = (
                ...
                else node.properties.model_dump()
            )
...
class ImageChunk(BaseModule):
    ...
    def from_llama_node(cls, node: ImageNode) -> "ImageChunk":
        ...
        logger.debug(
            f"Creating ImageChunk from ImageNode - image_path: {node.image_path}"
        )
...
class Corpus(BaseModule):
    ...
    def __init__(
        ...
        chunk_index = (
            {} if chunks is None else {chunk.chunk_id: chunk for chunk in chunks}
        )
    ...
    def filter_by_doc_id(self, doc_id: str) -> List[Union[TextChunk, ImageChunk]]:
        ...
        return [
            ...
            if hasattr(chunk.metadata, "doc_id") and chunk.metadata.doc_id == doc_id
        ]
    ...
    def filter_by_similarity(
        ...
        return [
            chunk
            for chunk in self.chunks
            if chunk.metadata.similarity_score
            and chunk.metadata.similarity_score >= threshold
        ]
    ...
    def sort_by_similarity(
        ...
        return sorted(
            [
                chunk
                for chunk in self.chunks
                if chunk.metadata.similarity_score is not None
            ],
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            for chunk in self.chunks:
                ...
                if "\n" in json_str:
                    ...
                    print(
                        f"Chunk {chunk.chunk_id} contains newlines in JSON, which may break JSONL format."
                    )
                f.write(json_str + "\n")
        ...
        with open(input_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                chunk = Chunk(
                    ...
                    excluded_embed_metadata_keys=chunk_dict[
                        "excluded_embed_metadata_keys"
                    ],
                    excluded_llm_metadata_keys=chunk_dict["excluded_llm_metadata_keys"],
                    relationships={
                        k: RelatedNodeInfo(**v)
                        for k, v in chunk_dict["relationships"].items()
                    },
                )
                chunks.append(chunk)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for chunk in self.chunks:
            ...
            if (
                hasattr(chunk.metadata, "chunking_strategy")
                and chunk.metadata.chunking_strategy
            ):
...
class Query(BaseModule):
    ...
    top_k: Optional[int] = Field(
        default=None, description="Number of top results to retrieve."
    )
    ...
    keyword_filters: Optional[List[str]] = Field(
        default=None, description="Keywords to filter results."
    )
    ...
    def to_QueryBundle(self):
        return QueryBundle(
            query_str=self.query_str, custom_embedding_strs=self.custom_embedding_strs
        )
