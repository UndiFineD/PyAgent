# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\embeddings\huggingface_embedding.py
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        if not EmbeddingProvider.validate_model(
            ...
            raise ValueError(
                f"Unsupported HuggingFace model: {model_name}. Supported models: {SUPPORTED_MODELS['huggingface']}"
            )
        try:
    ...
    def get_embedding_model(self) -> BaseEmbedding:
        ...
        if self._embedding_model is None:
            try:
                ...
                logger.debug(
                    f"Initialized HuggingFace embedding wrapper for model: {self.model_name}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to initialize HuggingFace embedding wrapper: {str(e)}"
                )
                raise
