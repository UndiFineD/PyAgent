# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\embeddings\azure_openai_embedding.py
import os
...
class AzureOpenAIEmbedding(BaseEmbedding):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__(
            api_key=api_key, model_name=model_name, embed_batch_size=embed_batch_size
        )
        ...
        self.deployment_name = (
            deployment_name or os.getenv("AZURE_EMBED_DEPLOYMENT") or model_name
        )
        self.dimensions = dimensions
        self.embed_batch_size = embed_batch_size
        self.kwargs = kwargs or {}

        if not EmbeddingProvider.validate_model(
            EmbeddingProvider.AZURE_OPENAI, model_name
        ):
            raise ValueError(
    ...
    def get_embedding_model(self) -> BaseEmbedding:
        if self._embedding_model is None:
            try:
                ...
                logger.error(
                    "Failed to initialize Azure OpenAI embedding wrapper: %s", exc
                )
                raise
