# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\embeddings\openai_embedding.py
import os
import warnings
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not EmbeddingProvider.validate_model(EmbeddingProvider.OPENAI, model_name):
            raise ValueError(
                f"Unsupported OpenAI model: {model_name}. Supported models: {SUPPORTED_MODELS['openai']}"
            )
    ...
    def get_embedding_model(self) -> BaseEmbedding:
        ...
        if getattr(self, "_embedding_model", None) is None:
            try:
                ...
                logger.debug(
                    f"Initialized OpenAI embedding wrapper for model: {self.model_name}"
                )
            except Exception as e:
