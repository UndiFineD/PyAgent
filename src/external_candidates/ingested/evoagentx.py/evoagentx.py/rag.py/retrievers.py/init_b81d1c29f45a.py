# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\retrievers\__init__.py
    ...
    def create(
        ...
        if retriever_type == RetrieverType.VECTOR.value:
            ...
            retriever = VectorRetriever(
                index=index, top_k=query.top_k if query else 5, chunk_class=chunk_class
            )
        elif retriever_type == RetrieverType.GRAPH.value:
            if not (graph_store and embed_model and llm):
                raise ValueError(
                    "Graph store, embed model and llm model required for graph retriever"
                )
