...
class VectorRetriever(BaseRetrieverWrapper):
    ...
    def __init__(self, index: BaseIndex, top_k: int = 5, chunk_class=None):
        ...
        self.top_k = top_k
        self.chunk_class = (
            chunk_class  # Use chunk class passed from RAGEngine based on config
        )
        self.retriever = VectorIndexRetriever(
            index=self.index, similarity_top_k=self.top_k
        )
        ...
        try:
            ...
            for score_node in nodes:
                if self.chunk_class is None:
                    raise ValueError(
                        "chunk_class not set - RAGEngine must pass chunk class based on config"
                    )
