# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\rag.py
import asyncio
import json
import os
...
from llama_index.core.schema import ImageNode, NodeWithScore, RelatedNodeInfo, TextNode

from .chunkers import ChunkFactory
from .embeddings import EmbeddingFactory, EmbeddingProvider
from .indexings import BaseIndexWrapper, IndexFactory
...
from .retrievers import BaseRetrieverWrapper, RetrieverFactory
...
class RAGEngine:
    def __init__(
        ...
        self.storage_handler = (
            storage_handler  # Maybe reinit the vector_store by the load funcion.
        )
        ...
        if (self.storage_handler.vector_store is not None) and (
            self.embed_model.dimensions is not None
        ):
            if (
                self.storage_handler.storageConfig.vectorConfig.dimensions
                != self.embed_model.dimensions
            ):
                logger.warning(
                    "The dimensions in vector_store is not equal with embed_model. Reiniliaze vector_store."
                )
                self.storage_handler.storageConfig.vectorConfig.dimensions = (
                    self.embed_model.dimensions
                )
        ...
        self.indices: Dict[str, Dict[str, BaseIndexWrapper]] = (
            {}
        )  # Nested: {corpus_id: {index_type: index}}
        self.retrievers: Dict[str, Dict[str, BaseRetrieverWrapper]] = (
            {}
        )  # Nested: {corpus_id: {index_type: retriever}}

    def read(
        self,
        file_paths: Union[Sequence[str], str],
        ...
        try:
            ...
            if self.config.modality == "multimodal":
                ...
                for doc in documents:
                    ...
                    image_path = getattr(doc, "image_path", None) or doc.metadata.get(
                        "file_path"
                    )
                    ...
                    image_chunk = self.chunk_class(
                        ...
                        chunk_id=doc.metadata.get(
                            "file_name", f"img_{len(image_chunks)}"
                        ),
                        metadata=ChunkMetadata(
                            doc_id=doc.metadata.get(
                                "file_name", f"doc_{len(image_chunks)}"
                            ),
                            corpus_id=corpus_id,
                            **doc.metadata,
                        ),
                ...
                logger.info(
                    f"Read {len(documents)} multimodal documents (no chunking) for corpus {corpus_id}"
                )
            else:
                corpus = self.chunker.chunk(documents)
                corpus.corpus_id = corpus_id
                logger.info(
                    f"Read {len(documents)} documents and created {len(corpus.chunks)} chunks for corpus {corpus_id}"
                )
            return corpus
    ...
    def add(
        self,
        index_type: str,
        nodes: Union[Corpus, List[NodeWithScore], List[TextNode], List[ImageNode]],
        corpus_id: str = None,
    ) -> None:
        ...
        try:
            ...
            if index_type not in self.indices[corpus_id]:
                index = self.index_factory.create(
                    ...
                    index_config=(
                        self.config.index.model_dump(exclude_unset=True)
                ...
                self.retrievers[corpus_id][index_type] = self.retriever_factory.create(
                    ...
                    query=Query(
                        query_str="",
                        top_k=(
                            self.config.retrieval.top_k if self.config.retrieval else 5
                        ),
                    ),
                    storage_handler=self.storage_handler,
            ...
            nodes_to_insert = (
                nodes.to_llama_nodes() if isinstance(nodes, Corpus) else nodes
            )
            for node in nodes_to_insert:
                node.metadata.update({"corpus_id": corpus_id, "index_type": index_type})
            nodes_ids = self.indices[corpus_id][index_type].insert_nodes(
                nodes_to_insert
            )
            logger.info(
                f"Added {len(nodes_to_insert)} nodes to {index_type} index for corpus {corpus_id}"
            )
            return nodes_ids
        except Exception as e:
            logger.error(
                f"Failed to add nodes to {index_type} index for corpus {corpus_id}: {str(e)}"
            )
            return []
    ...
    def delete(
        ...
        try:
            ...
            target_indices = (
                [index_type] if index_type else self.indices[corpus_id].keys()
            )
            for idx_type in list(
                target_indices
            ):  # Use list to avoid runtime modification issues
                if idx_type not in self.indices[corpus_id]:
                    logger.warning(
                        f"Index type {idx_type} not found for corpus {corpus_id}"
                    )
                    continue
                ...
                if node_ids or metadata_filters:
                    ...
                    node_ids_list = (
                        [node_ids] if isinstance(node_ids, str) else node_ids
                    )
                    index.delete_nodes(
                        node_ids=node_ids_list, metadata_filters=metadata_filters
                    )
                    logger.info(
                        f"Deleted nodes from {idx_type} index for corpus {corpus_id}"
                    )
                else:
                    # Delete entire index
                    index.clear()
                    ...
                    logger.info(
                        f"Deleted entire {idx_type} index for corpus {corpus_id}"
                    )
            ...
            logger.error(
                f"Failed to delete from corpus {corpus_id}, index {index_type}: {str(e)}"
            )
            raise
    ...
    def clear(self, corpus_id: Optional[str] = None) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to clear indices for corpus {corpus_id or 'all'}: {str(e)}"
            )
            raise
    ...
    def save(
        ...
        try:
            ...
            for cid in target_corpora:
                ...
                target_indices = (
                    [index_type]
                    if index_type and index_type in self.indices[cid]
                    else self.indices[cid].keys()
                )
                for idx_type in target_indices:
                    ...
                    if idx_type == IndexType.GRAPH and not graph_exported:
                        logger.warning(
                            f"Skipping save for graph index {idx_type} in corpus {cid} as graph_exported is False"
                        )
                        continue
                    ...
                    # Prepare metadata
                    vector_config = (
                        self.storage_handler.storageConfig.vectorConfig.model_dump()
                        if self.storage_handler.storageConfig.vectorConfig
                        else {}
                    )
                    graph_config = (
                        self.storage_handler.storageConfig.graphConfig.model_dump()
                        if self.storage_handler.storageConfig.graphConfig
                        else {}
                    )
                    ...
                    if output_path:
                        ...
                        safe_cid = "".join(
                            c if c.isalnum() or c in ["-", "_"] else "_" for c in cid
                        )
                        safe_idx_type = "".join(
                        ...
                        nodes_file = os.path.join(
                            output_path, f"{safe_cid}_{safe_idx_type}_nodes.jsonl"
                        )
                        metadata_file = os.path.join(
                            output_path, f"{safe_cid}_{safe_idx_type}_metadata.json"
                        )

                        # Save corpus as JSONL
                        corpus.to_jsonl(nodes_file, indent=0)
                        logger.info(
                            f"Saved {len(corpus.chunks)} chunks to {nodes_file}"
                        )

                        # Save metadata as JSON
                        with open(metadata_file, "w", encoding="utf-8") as f:
                            json.dump(
                                metadata.model_dump(), f, indent=2, ensure_ascii=False
                            )
                        ...
                        logger.info(
                            f"Saved {idx_type} index with {len(corpus.chunks)} chunks for corpus {cid} to database table {table}"
                        )

        except Exception as e:
            logger.error(
                f"Failed to save indices for corpus {corpus_id or 'all'}: {str(e)}"
            )
            raise
        ...
        try:
            ...
            if source:
                ...
                for file_name in os.listdir(source):
                    ...
                    if (corpus_id and corpus_id != cid) or (
                        index_type and index_type != idx_type
                    ):
                        continue
                    ...
                    if not self.embed_model.validate_model(
                        self.config.embedding.provider, metadata.embedding_model_name
                    ):
                        raise ValueError(
                    ...
                    if not os.path.exists(nodes_file):
                        logger.warning(
                            f"Nodes file {nodes_file} not found for metadata {metadata_file}"
                        )
                        continue
                    ...
                    if (
                        ...
                        logger.info(
                            f"Reinitializing embedding model to {metadata.embedding_model_name}"
                        )
                        self.embed_model = self.embedding_factory.create(
                            provider=self.config.embedding.provider,
                            model_config=self.config.embedding.model_dump(
                                exclude_unset=True
                            ),
                        )
                    ...
                    logger.info(
                        f"Loaded {idx_type} index with {len(corpus.chunks)} chunks for corpus {cid} from {nodes_file}"
                    )
            else:
                # Database loading
                ...
                for record in records:
                    ...
                    for chunk_data in parsed["content"]["chunks"]:
                        ...
                        if self.config.modality == "multimodal":
                            ...
                            chunk = ImageChunk(
                                ...
                                excluded_embed_metadata_keys=chunk_data[
                                    "excluded_embed_metadata_keys"
                                ],
                                excluded_llm_metadata_keys=chunk_data[
                                    "excluded_llm_metadata_keys"
                                ],
                                relationships={
                                    k: RelatedNodeInfo(**v)
                                    for k, v in chunk_data["relationships"].items()
                                },
                            )
                        else:
                    ...
                    loaded_chunk_ids.extend(chunk_ids)
                    logger.info(
                        f"Loaded {idx_type} index with {len(corpus.chunks)} chunks for corpus {cid} from database table {table}"
                    )
    ...
    def _load_index(
        self, corpus: Corpus, corpus_id: str, index_type: str
    ) -> Sequence[str]:
        ...
        try:
            ...
            if index_type not in self.indices[corpus_id]:
                ...
                retriever_type = (
                    RetrieverType.GRAPH
                    if index_type == IndexType.GRAPH
                    else RetrieverType.VECTOR
                )
                self.retrievers[corpus_id][index_type] = self.retriever_factory.create(
            ...
            logger.info(
                f"Inserted {len(nodes)} nodes into {index_type} index for corpus {corpus_id}"
            )
            return chunk_ids
        except Exception as e:
            logger.error(
                f"Failed to load index for corpus {corpus_id}, index_type {index_type}: {str(e)}"
            )
            raise

    async def aget(
        self, corpus_id: str, index_type: str, node_ids: List[str]
    ...
    async def query_async(
        ...
        corpus_id: Optional[str] = None,
        query_transforms: Optional[List] = None,
    ) -> RagResult:
        ...
        try:
            ...
            for cid in target_corpora:
                for idx_type, retriever in self.retrievers[cid].items():
                    if (
                        ...
                        and query.metadata_filters.get("index_type")
                        and query.metadata_filters["index_type"] != idx_type
                    ):
                        continue
            ...
            for (_, cid, idx_type), result in zip(tasks, retrieval_results):
                if isinstance(result, Exception):
                    logger.error(
                        f"Retrieval failed for {idx_type} in corpus {cid}: {str(result)}"
                    )
                else:
                    results.append(result)
                    logger.info(
                        f"Retrieved {len(result.corpus.chunks)} chunks from {idx_type} retriever for corpus {cid}"
                    )
            ...
            query.similarity_cutoff = (
                self.config.retrieval.similarity_cutoff
                if query.similarity_cutoff is None
                else query.similarity_cutoff
            )
            query.keyword_filters = (
                self.config.retrieval.keyword_filters
                if query.keyword_filters is None
                else query.keyword_filters
            )

            postprocessor = self.postprocessor_factory.create(
                self.config.retrieval.postprocessor_type, query=query
            )
            ...
            if query.metadata_filters:
                final_result.corpus.chunks = [
                    ...
                    if all(
                        chunk.metadata.model_dump().get(k) == v
                ...
                logger.info(
                    f"Applied metadata filters, retained {len(final_result.corpus.chunks)} chunks"
                )

            logger.info(
                f"Query returned {len(final_result.corpus.chunks)} chunks after post-processing"
            )
            return final_result
        except Exception as e:
