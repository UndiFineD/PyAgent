...
class GraphIndexing(BaseIndexWrapper):
    ...
    def __init__(
        ...
        try:
            ...
            self.index = PropertyGraphIndex(
                ...
                vector_store=(
                    vector_store
                    if (not self.storage_handler.graph_store.supports_vector_queries)
                    else None
    ...
    def _create_storage_context(self):
        ...
        assert (
            self.storage_handler.graph_store is not None
    ...
    def insert_nodes(self, nodes: List[Union[Chunk, BaseNode]]):
        ...
        try:
            filtered_nodes = [
                node.to_llama_node() if isinstance(node, Chunk) else node
                for node in nodes
            ]
    ...
    async def aload(
        self,
        nodes: List[
            Union[Chunk, BaseNode, LabelledNode, Relation, EntityNode, ChunkNode]
        ],
    ) -> Sequence[str]:
        """
