...
class VectorIndexing(BaseIndexWrapper):
    ...
    def _create_storage_context(
        ...
        assert (
            self.storage_handler.vector_store is not None
        ...
        try:
            ...
            for node in nodes:
                ...
                if node_id in self.id_to_node:
                    ...
                    logger.info(
                        f"Find the same node in vector database: {node_id}. Update it."
                    )
            ...
            # Caching the node
            nodes_with_embedding = self.index._get_node_with_embedding(
            ...
            logger.info(
                f"Inserted {len(nodes_with_embedding)} nodes into VectorStoreIndex"
            )
            ...
            if node_ids:
                for node_id in node_ids:
                    if node_id in self.id_to_node:
                        ...
                        if (
                            self.index.storage_context.docstore._kvstore._collections_mappings.get(
                                node_id, None
                            )
                            is not None
                        ):
                            self.index.storage_context.docstore._kvstore._collections_mappings.pop(
                                node_id
                            )
                ...
                if nodes_to_delete:
                    ...
                    logger.info(
                        f"Deleted {len(nodes_to_delete)} nodes matching metadata filters from VectorStoreIndex"
                    )
            else:
