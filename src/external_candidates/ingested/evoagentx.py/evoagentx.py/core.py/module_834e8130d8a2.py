# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\core\module.py
import copy
import json
import logging
import os
...
class BaseModule(BaseModel, metaclass=MetaModule):
        ...
        try:
            ...
            if exception_handler is None:
                error_message = get_base_module_init_error_message(
                    cls=self.__class__, data=kwargs, errors=e
                )
    ...
    @classmethod
    def from_file(
        cls, path: str, load_function: Callable = None, **kwargs
    ) -> "BaseModule":
        """
    ...
    def to_dict(
        self, exclude_none: bool = True, ignore: List[str] = [], **kwargs
    ) -> dict:
        """
        ...
        for field_name, _ in type(self).model_fields.items():
            ...
            if isinstance(field_value, BaseModule):
                data[field_name] = field_value.to_dict(
                    exclude_none=exclude_none, ignore=ignore
                )
            elif isinstance(field_value, list):
                data[field_name] = [
                    (
                        item.to_dict(exclude_none=exclude_none, ignore=ignore)
                ...
                data[field_name] = {
                    key: (
                        value.to_dict(exclude_none=exclude_none, ignore=ignore)
    ...
    def to_json(
        self, use_indent: bool = False, ignore: List[str] = [], **kwargs
    ) -> str:
        """
    ...
    def save_module(self, path: str, ignore: List[str] = [], **kwargs) -> str:
        ...
        return save_json(
            self.to_json(use_indent=True, default=lambda x: None, ignore=ignore),
            path=path,
        )
