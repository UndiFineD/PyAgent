# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\hitl\hitl_example.py
import asyncio
import os
...
class EmailSendActionInput(ActionInput):
    human_verified_data: str = Field(
        description="The extracted data that will be sent to the user"
    )
...
class DummyEmailSendAction(Action):
    ...
    def execute(
        ...
        **kwargs,
        ...
        action_input_data = {
            attr: inputs.get(attr, "undefined") for attr in action_input_attrs
        }
        prompt = self.prompt.format(
            **action_input_data
        )  # format the prompt with the action input data
...
async def main():
    ...
    # create workflow
    graph = WorkFlowGraph(
        goal="send the extracted data to the user by mail", nodes=nodes, edges=edges
    )
