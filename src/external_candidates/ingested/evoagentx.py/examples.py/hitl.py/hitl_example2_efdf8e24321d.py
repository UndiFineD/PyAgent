...
class UserProfileInput(ActionInput):
    ...
    user_preferences: Optional[str] = Field(
        default=None, description="User's preferences"
    )
...
async def main():
    ...
    edges = [
        WorkFlowEdge(
            source="user_input_collection_node", target="profile_processing_node"
        )
    ]
    ...
    workflow = WorkFlow(
        graph=graph, llm=llm, agent_manager=agent_manager, hitl_manager=hitl_manager
    )
