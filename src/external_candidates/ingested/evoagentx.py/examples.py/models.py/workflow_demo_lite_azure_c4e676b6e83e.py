...
def save_intermediate_result(data, stage: str, output_dir: str):
    ...
    try:
        ...
        print(
            f"Warning: Cannot fully serialize {stage} results ({e}), saving string representation"
        )
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(str(data))
...
def generate_agents_for_workflow(
    llm: LiteLLM, goal: str, workflow: WorkFlowGraph, output_dir: str
):
...
def generate_workflow_step_by_step(
    llm: LiteLLM, goal: str, output_dir: str
) -> WorkFlowGraph:
...
def verify_and_extract_code(llm: LiteLLM, goal: str, output: str, target_dir: str):
    ...
    verified = verifier.execute(
        llm=llm, inputs={"requirements": goal, "code": output}
    ).verified_code
    ...
    res = extractor.execute(
        llm=llm, inputs={"code_string": verified, "target_directory": target_dir}
    )
