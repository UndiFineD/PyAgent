# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\optimization\evoprompt\evoprompt_singleagent.py
"""
...
class SinglePromptProgram:
    ...
    def __init__(self, model: OpenAILLM, task_name: str):
        ...
        lib_path = os.path.join(
            os.path.dirname(__file__), "lib_prompt", f"{task_name}.txt"
        )
        try:
    ...
    def __call__(self, input: str) -> tuple[str, dict]:
        ...
        prefix = (
            self.chain_of_thought_prefix[0]
...
async def main():
    ...
    eval_llm_config = OpenAILLMConfig(
        model="gpt-4.1-nano", openai_key=OPENAI_API_KEY, stream=False, temperature=0
    )
    llm = OpenAILLM(config=eval_llm_config)

    # Tasks to optimize
    tasks = ["geometric_shapes", "multistep_arithmetic_two"]
