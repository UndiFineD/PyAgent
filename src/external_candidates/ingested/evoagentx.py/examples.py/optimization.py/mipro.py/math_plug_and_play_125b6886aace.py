# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\optimization\mipro\math_plug_and_play.py
import json
import os
...
def main():
    ...
    optimizer_config = OpenAILLMConfig(
        model="gpt-4o", openai_key=OPENAI_API_KEY, stream=True, output_response=False
    )
    ...
    registry.track(
        program, "prompt", input_names=["problem"], output_names=["solution"]
    )
