# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\tools\google_maps_example.py
"""
...
def main():
    ...
    result = places_search_tool(
        query="restaurants near Mountain View, CA", location=f"{lat},{lng}", radius=2000
    )
    ...
    if result["success"]:
        ...
        for origin_data in result["matrix"]:
            ...
            for dest in origin_data["destinations"]:
                if dest["status"] == "OK":
                    print(
                        f"  To {dest['destination_address']}: {dest['distance'].get('text', 'N/A')} - {dest['duration'].get('text', 'N/A')}"
                    )
                else:
