...
def run_search_examples():
    ...
    ddgs_toolkit = DDGSSearchToolkit(
        num_search_pages=3, max_content_words=200, backend="auto", region="us-en"
    )

    # Initialize SerpAPI toolkit (will check for API key)
    serpapi_toolkit = SerpAPIToolkit(
        num_search_pages=3, max_content_words=300, enable_content_scraping=True
    )
    ...
    if serpapi_api_key:
        ...
        print(
            "✓ SerpAPI toolkit initialized successfully (API key required for search)"
        )
    ...
    if serperapi_api_key:
        ...
        print(
            "✓ SerperAPI toolkit initialized successfully (API key required for search)"
        )
    ...
    try:
        ...
        for feed_url in test_feeds:
            ...
            if validate_result.get("success") and validate_result.get("is_valid"):
                print(
                    f"✓ Valid {validate_result.get('feed_type')} feed: {validate_result.get('title', 'Unknown')}"
                )
                ...
                if fetch_result.get("success"):
                    ...
                    print(
                        f"✓ Fetched {len(entries)} entries from '{fetch_result.get('title')}'"
                    )
                ...
                print(
                    f"❌ Invalid or inaccessible feed: {validate_result.get('error', 'Unknown error')}"
                )
        ...
        print(f"Error: {str(e)}")
        print(
            "Note: RSS feed availability may vary. Some feeds may be temporarily unavailable."
        )
...
def run_request_tool_example():
    ...
    try:
        ...
        if get_result.get("success"):
            ...
            print(
                f"Response size: {len(str(get_result.get('content', '')))} characters"
            )
        else:
        ...
        if post_result.get("success"):
            ...
            print(
                f"❌ POST request failed: {post_result.get('error', 'Unknown error')}"
            )
        ...
        if delete_result.get("success"):
            ...
            print(
                f"❌ DELETE request failed: {delete_result.get('error', 'Unknown error')}"
            )

        # Test error handling with invalid URL
        print("\n5. Testing error handling...")
        error_result = http_tool(
            url="https://invalid-domain-that-does-not-exist-12345.com", method="GET"
        )
