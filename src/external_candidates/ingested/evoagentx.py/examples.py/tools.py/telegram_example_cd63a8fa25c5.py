# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\tools\telegram_example.py
"""
...
def main():
    ...
    if (
        not telegram_toolkit.telegram_base.api_id
        or not telegram_toolkit.telegram_base.api_hash
    ):
    ...
    if result["success"]:
        ...
        for file_info in result["files"]:
            print(
                f"  - {file_info['filename']} ({file_info['file_size']} bytes, {file_info['mime_type']})"
            )
    else:
    ...
    download_result = download_tool(
        contact_name=test_contact_name, filename_query="Kafka", download_dir="downloads"
    )
    ...
    for content_type, description in content_tests:
        ...
        if read_result["success"]:
            ...
            content_preview = (
                ...
                if len(read_result["content"]) > 200
                else read_result["content"]
            )
    ...
    print(
        "✅ Core Tools (6): fetch_latest_messages, search_messages_by_keyword, send_message_by_name, list_recent_chats, find_and_retrieve_file, summarize_contact_messages"
    )
