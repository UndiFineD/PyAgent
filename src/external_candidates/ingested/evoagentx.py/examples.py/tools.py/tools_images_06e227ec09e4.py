# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\tools\tools_images.py
#!/usr/bin/env python3
...
def run_image_analysis_example():
    ...
    try:
        ortk = OpenRouterImageToolkit(
            name="DemoORImageToolkit", api_key=openrouter_api_key
        )
        ...
        result = analyze_tool(
            prompt="Describe this image in detail.", image_url=test_image_url
        )
        if "error" in result:
...
def run_flux_image_toolkit_pipeline():
    ...
    analyze = (
        flux.get_tool("image_analysis") if flux.get_tool("image_analysis") else None
    )
