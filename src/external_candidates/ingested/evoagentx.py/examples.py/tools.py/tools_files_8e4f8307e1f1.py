# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\tools\tools_files.py
#!/usr/bin/env python3
...
def run_storage_handler_examples():
    ...
    try:
        ...
        custom_list_result = custom_list_tool(
            path=".", max_depth=1, include_hidden=False
        )
        ...
        for test_file in test_files:
            if os.path.exists(test_file):
                ...
                if delete_result.get("success"):
                    ...
                    print(
                        f"❌ Failed to delete {test_file}: {delete_result.get('error')}"
                    )
