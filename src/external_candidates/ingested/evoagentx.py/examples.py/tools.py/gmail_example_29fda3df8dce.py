# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\tools\gmail_example.py
"""
...
def main():
    ...
    if attachment_path:
        try:
            ...
            result = create_draft_tool(to=test_to, subject=test_subject, body=test_body)
    else:
        result = create_draft_tool(to=test_to, subject=test_subject, body=test_body)
    ...
    if created_draft_id:
        ...
        if update_attachments:
            ...
            result = update_draft_tool(
                draft_id=created_draft_id, subject=updated_subject, body=updated_body
            )
    ...
    create_result = create_draft_tool(to=send_to, subject=send_subject, body=send_body)
    ...
    try:
        ...
        if trash_res.get("success"):
            ...
            print(
                f"   ❌ Trash operation failed: {trash_res.get('error', 'Unknown error')}"
            )
    except Exception as e:
    ...
    try:
        search_res = search_messages_tool(
            keyword="from:noreply@medium.com", max_results=2
        )
        if search_res.get("success") and search_res.get("messages"):
            msg_ids = [
                m["id"] for m in search_res["messages"][:1]
            ]  # Take just first one for safety
            ...
            if msg_ids:
                ...
                if trash_res.get("success"):
                    print(
                        f"   ✅ Trashed {len(trash_res.get('trashed_ids', []))} message(s)"
                    )
                else:
                    print(
                        f"   ❌ Trash failed: {trash_res.get('error', 'Unknown error')}"
                    )
        else:
    ...
    print(
        "   Step 1: Searching for messages from noreply@gmail.com (including attachments)..."
    )
    try:
        search_res = search_messages_tool(
            keyword="from:noreply@gmail.com", max_results=5, include_attachments=True
        )
        if search_res.get("success") and search_res.get("messages"):
            ...
            for i, msg in enumerate(search_res["messages"], 1):
                ...
                if attach_count > 0:
                    ...
                    for att in msg.get("attachments", []):
                        print(
                            f"            - {att['filename']} ({att.get('size', 0)} bytes)"
                        )
            ...
            if mod_res.get("success"):
                ...
                print(
                    f"   ❌ Label modification failed: {mod_res.get('error', 'Unknown error')}"
                )
        else:
    ...
    try:
        search_res = search_messages_tool(
            keyword='subject:"Test Draft from EvoAgentX"', max_results=5
        )
        if search_res.get("success") and search_res.get("messages"):
            ids = [m["id"] for m in search_res["messages"]]
            ...
            if mod_res2.get("success"):
                ...
                print(
                    f"   ❌ Custom label application failed: {mod_res2.get('error', 'Unknown error')}"
                )
        else:
    ...
    user_subject_query = input(
        "   Enter a Gmail subject to search for (or press Enter for default 'has:attachment'): "
    ).strip()
    if not user_subject_query:
        user_subject_query = "has:attachment"

    # User can specify custom download directory
    custom_download_dir = input(
        "   Enter download directory (or press Enter for default './downloads'): "
    ).strip()
    ...
    try:
        ...
        if result["success"]:
            ...
            for idx, msg in enumerate(result["messages"], 1):
                ...
                print(f"      Date: {msg['date']}")
                print(
                    f"      Attachments in message: {msg.get('attachments_count', 0)}"
                )
