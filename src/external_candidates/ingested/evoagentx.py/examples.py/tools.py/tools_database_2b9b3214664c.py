...
def run_mongodb_examples():
    ...
    try:
        ...
        toolkit = MongoDBToolkit(
            name="DemoMongoDBToolkit", database_name="demo_db", auto_save=True
        )
        ...
        insert_result = execute_tool(
            query=json.dumps(products), query_type="insert", collection_name="products"
        )

        if insert_result.get("success"):
            print(f"✓ Successfully inserted {len(products)} products")
            print(
                f"  Documents inserted: {insert_result.get('data', {}).get('inserted_count', 'Unknown')}"
            )
        else:
        ...
        if agg_result.get("success"):
            ...
            for category in categories:
                ...
                print(
                    f"  - {cat_name}: Avg price ${avg_price:.2f}, Total stock: {total_stock}"
                )
        else:
        ...
        delete_result = delete_tool(
            collection_name="products", filter='{"category": "Furniture"}', multi=True
        )
...
def run_postgresql_examples():
    ...
    try:
        ...
        toolkit = PostgreSQLToolkit(
            name="DemoPostgreSQLToolkit", database_name="demo_db", auto_save=True
        )
...
def run_faiss_examples():
    ...
    try:
        ...
        if filtered_search_result.get("success"):
            ...
            print(
                f"❌ Filtered search failed: {filtered_search_result.get('error', 'Unknown error')}"
            )
        ...
        if delete_result.get("success"):
            ...
            verify_result = query_tool(
                query="artificial intelligence", top_k=5, similarity_threshold=0.1
            )
        ...
        if "DocumentMetadata" in str(e):
            print(
                "Note: This appears to be a dependency issue with the RAG engine components"
            )
