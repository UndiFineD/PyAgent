# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\memory\example_short_term_memory.py
import asyncio
import os
...
class DummyAction(Action):
    def __init__(self, **kwargs):
        super().__init__(
            name="DummyAction", description="A test action that echoes input", **kwargs
        )
...
async def test_short_term_memory_via_agent():
    ...
    assert (
        len(responses) <= agent.short_term_memory.max_size
    ), "Short-term memory exceeded max_size"
    assert (
        responses[0].content == "[Echo] Msg 5"
    ), "Oldest response message should be overwritten"
    assert (
        responses[-1].content == "[Echo] Msg 7"
    ), "Latest response message should be at the end"
