# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\rag\rag.py
import json
import os
...
from evoagentx.rag.schema import Chunk, ChunkMetadata, Corpus, Query
    ...
    for title, sentences in context:
        for idx, sentence in enumerate(sentences):
            chunk = Chunk(
                ...
                text=sentence,
                metadata=ChunkMetadata(doc_id=str(idx), corpus_id=corpus_id),
                start_char_idx=0,
...
def evaluate_retrieval(
    retrieved_chunks: List[Chunk], supporting_facts: List[List], top_k: int
) -> Dict[str, float]:
...
def run_evaluation(samples: List[Dict], top_k: int = 5) -> Dict[str, float]:
    ...
    for sample in samples:
        ...
        if CHECK_SAVE:
            ...
            search_engine1 = RAGEngine(
                config=rag_config, storage_handler=storage_handler, llm=llm
            )
            ...
            # Evaluate
            sample_metrics = evaluate_retrieval(
                retrieved_chunks, supporting_facts, top_k
            )
            logger.info(f"Metrics for sample {corpus_id}: {sample_metrics}")
