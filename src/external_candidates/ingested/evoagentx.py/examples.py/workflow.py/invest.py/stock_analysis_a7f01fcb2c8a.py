# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\workflow\invest\stock_analysis.py
...
# EvoAgentX imports
from evoagentx.models import OpenAILLM, OpenAILLMConfig, OpenRouterConfig, OpenRouterLLM
...
OPEN_ROUTER_API_KEY = read_api_key("openrouter_api_key.txt") or os.getenv(
    "OPENROUTER_API_KEY"
)
...
def generate_workflow():
    ...
    workflow_graph: WorkFlowGraph = wf_generator.generate_workflow(
        goal=WORKFLOW_GOAL, retry=5
    )
...
def generate_html_report(stock_code, base_dir, report_dir, graphs_dir, timestamp):
    ...
    try:
        ...
        html_output = (
            ...
            / "html_report"
            / f"report_{stock_code}_{timestamp}.html"
        )
...
def main():
    ...
    html_success = generate_html_report(
        stock_code, base_dir, report_dir, graphs_dir, timestamp
    )
