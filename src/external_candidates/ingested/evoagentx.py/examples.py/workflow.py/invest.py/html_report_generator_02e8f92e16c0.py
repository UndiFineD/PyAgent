# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\workflow\invest\html_report_generator.py
#!/usr/bin/env python3
"""
...
class MarkdownParser:
    ...
    def _has_content(self, subsection_data: Dict[str, Any]) -> bool:
        ...
        for table in tables:
            rows = table.get("rows", [])
            if rows and not all(
                all(cell in ["", "-", "N/A", "无", "0"] for cell in row) for row in rows
            ):
                meaningful_tables.append(table)

        # Check for meaningful lists
        meaningful_lists = [
            lst for lst in lists if lst and any(item.strip() for item in lst)
        ]
    ...
    def _extract_table(
        self, content: List[str], start_idx: int
...
class HTMLGenerator:
    ...
    def _get_latest_close_price(self, stock_code: str, timestamp: str) -> str:
        ...
        try:
            # 构建CSV文件路径
            csv_path = Path(
                f"{stock_code}/{timestamp}/data/stock_daily_catl_{timestamp}_{stock_code}.csv"
            )
    ...
    def generate_report(
        self, md_file_path: str, technical_chart_path: str, price_volume_chart_path: str
    ) -> str:
        ...
        html_content = self._generate_html_structure(
            parser, metadata, technical_chart_base64, price_volume_chart_base64
        )
    ...
    def _read_news_from_csv(
        self, stock_code: str, timestamp: str
    ) -> List[Dict[str, str]]:
        """Read news data from CSV file and return the latest 10 entries."""
        try:
            # Construct the CSV file path
            csv_path = Path(
                f"{stock_code}/{timestamp}/data/stock_news_catl_{timestamp}_{stock_code}.csv"
            )
    ...
    def _read_ratings_from_csv(
        ...
        try:
            # Construct the CSV file path
            csv_path = Path(
                f"{stock_code}/{timestamp}/data/institution_recommendation_catl_{timestamp}_{stock_code}.csv"
            )
    ...
    def _generate_html_structure(
        ...
        technical_chart_base64: str,
        price_volume_chart_base64: str,
    ) -> str:
        ...
        charts_html = self._generate_charts_section(
            technical_chart_base64, price_volume_chart_base64
        )
    ...
    def _generate_neomorphism_header(
        self, metadata: Dict[str, str], sections: Dict[str, Any]
    ) -> str:
    ...
    def _generate_dashboard_overview(
        self, sections: Dict[str, Any], metadata: Dict[str, str]
    ) -> str:
            ...
            if price_targets:
                ...
                if tables and len(tables) > 0:
                    ...
                    if rows and len(rows) > 0:
                        ...
                        if len(row) >= 4:
                            target_price = (
                                str(row[1]).replace("RMB", "").replace(" ", "")
                                if row[1]
                                else target_price
                            )
                            stop_price = (
                                str(row[2]).replace("RMB", "").replace(" ", "")
                                if row[2]
                                else stop_price
                            )
    ...
    def _generate_charts_section(
        self, technical_chart_base64: str, price_volume_chart_base64: str
    ) -> str:
    ...
    def _generate_detailed_sections(
        self, sections, metadata: Dict[str, str] = None
    ) -> str:
        ...
        for section_key in section_order:
            if section_key in sections:
                ...
                if "基本面分析" in section_name:
                    section_content = self._generate_fundamentals_section_from_csv(
                        metadata
                    )
                else:
    ...
    def _generate_subsection(
        self, subsection_name: str, subsection_data: Dict[str, Any]
    ) -> str:
    ...
    def _generate_table(self, table_data: Dict[str, Any]) -> str:
        ...
        is_news_table = any(
            ...
            for keyword in ["新闻", "news", "标题", "title"]
        )
        has_link_column = any(
            keyword in " ".join(headers).lower() for keyword in ["链接", "url", "link"]
        )
        ...
        for row in rows:
            ...
            for i, cell in enumerate(row):
                ...
                if any(
                    ...
                    for keyword in ["决策", "操作建议", "决策类型"]
                ):
                    # Clean up cell content for CSS class
                    cell_class = (
                        cell.replace(" ", "").replace("-", "").replace("股", "")
                    )
                    cells_html.append(
                        f'<td><span class="status-badge status-{cell_class}">{cell}</span></td>'
                    )
                elif any(
                    keyword in header_name
                    for keyword in ["风险等级", "等级", "风险级别"]
                ):
                    cells_html.append(
                        f'<td><span class="status-badge risk-{cell}">{cell}</span></td>'
                    )
                # Handle news title links
                    ...
                    if link_index is not None and link_index < len(row):
                        ...
                        if (
                            link_url
                            and link_url.lower() not in ["n/a", "-", "na", ""]
                            and (
                                "http://" in link_url.lower()
                                or "https://" in link_url.lower()
                            )
                        ):
                            cells_html.append(
                                f'<td><a href="{link_url}" target="_blank" class="news-title-link">{cell}</a></td>'
                            )
                        else:
                ...
                # Handle link columns
                    ...
                    if (
                        cell
                        and cell.lower() not in ["n/a", "-", "na", ""]
                        and ("http://" in cell.lower() or "https://" in cell.lower())
                    ):
                        cells_html.append(
                            f'<td><a href="{cell}" target="_blank" class="news-link">{cell}</a></td>'
                        )
                    else:
                        cells_html.append(f"<td>{cell}</td>")
                else:
    ...
    def _generate_section_content(self, section_data: Dict[str, Any]) -> str:
        ...
        for subsection_name, subsection_data in subsections.items():
            content_html.append(
                self._generate_subsection(subsection_name, subsection_data)
            )
...
def main():
    ...
    output_file = generator.generate_report(
        args.md_file, args.technical_chart, args.price_volume_chart
    )
