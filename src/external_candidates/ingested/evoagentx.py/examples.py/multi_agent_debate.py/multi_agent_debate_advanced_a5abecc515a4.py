# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\multi_agent_debate\multi_agent_debate_advanced.py
#!/usr/bin/env python3
"""
...
def create_optimized_agent(
    role_name, role_description, model_config, temperature_adjustment=0.0
):
...
"""
    ...
    if hasattr(adjusted_config, "temperature"):
        adjusted_config.temperature = max(
            0.0, min(1.0, adjusted_config.temperature + temperature_adjustment)
        )
...
def create_role_model_mapping():
    ...
    models = {
        "gpt4o_mini": OpenAILLMConfig(
            model="gpt-4o-mini", openai_key=openai_key, temperature=0.3
        ),
        "gpt4o": OpenAILLMConfig(
            model="gpt-4o", openai_key=openai_key, temperature=0.2
        ),
        "llama": OpenRouterConfig(
    ...
    role_model_mapping = {
        ...
        "Mediator": (
            "gpt4o_mini",
            0.1,
        ),  # Slightly increase temperature for flexibility
    }
...
def run_optimized_debate():
    ...
    for i, agent in enumerate(agents):
        model_name = (
            agent.llm_config.model if hasattr(agent.llm_config, "model") else "Unknown"
        )
        temp = (
            agent.llm_config.temperature
            if hasattr(agent.llm_config, "temperature")
            else "Unknown"
        )
        print(
            f"  {agent.name}: {model_name} (Temperature: {temp}) - {roles[agent.name]}"
        )
