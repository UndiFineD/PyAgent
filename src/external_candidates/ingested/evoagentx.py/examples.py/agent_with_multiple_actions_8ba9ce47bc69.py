# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\agent_with_multiple_actions.py
import os
...
class AddMemoriesInput(ActionInput):
    messages: List[Dict] = Field(
        description="List of messages to add, each with content and optional attributes"
    )
...
class UpdateMemoriesInput(ActionInput):
    updates: List[Dict] = Field(
        description="List of updates, each with memory_id and message attributes"
    )
        ...
        **kwargs,
        ...
        if return_prompt:
            prompt = self.prompt.format(
                updates=[
                    {"memory_id": mid, "message": msg.model_dump()}
...
class SearchMemoriesInput(ActionInput):
    ...
    metadata_filters: Optional[Dict] = Field(
        default=None, description="Metadata filters (e.g., agent, msg_type)"
    )


class SearchMemoriesOutput(ActionOutput):
    results: List[Dict] = Field(
        description="List of retrieved messages with their memory IDs"
    )
...
class SearchMemories(Action):
    ...
    def execute(
        ...
        **kwargs,
        ...
        action_input_attrs = self.inputs_format.get_attrs()
        action_input_data = {
            attr: inputs.get(attr, None) for attr in action_input_attrs
        }
        ...
        output = SearchMemoriesOutput(
            results=[
                {"message": msg.model_dump(), "memory_id": mid} for msg, mid in results
            ]
        )
        if return_prompt:
            prompt = self.prompt.format(
...
def main():
    ...
    store_config = StoreConfig(
        dbConfig=DBConfig(
            db_name="sqlite", path="./debug/data/hotpotqa/cache/test_hotpotqa.sql"
        ),
    ...
    rag_config = RAGConfig(
        ...
        chunker=ChunkerConfig(
            strategy="simple", chunk_size=512, chunk_overlap=0, max_chunks=None
        ),
        embedding=embedding,
    ...
    for result in search_result.content.results:
        print(
            f"- Memory ID: {result['memory_id']}, Content: {result['message'].content}"
        )
    ...
    updates = [
        {
            "memory_id": (
                add_result.content.memory_ids[0]
                if add_result.content.memory_ids
                else ""
            ),
