# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\Wonderful_workflow_corpus\invest\catl_data_functions.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
股票数据抓取函数库
...
class StockDataFetcher:
    ...
    def save_data(self, data, filename_prefix, description=""):
        ...
        try:
            ...
            if isinstance(data, pd.DataFrame):
                ...
                self.logger.info(
                    f"✅ {description} 已保存: {filepath} (共{len(data)}条记录)"
                )
            else:
    ...
    def fetch_stock_daily(self, days=30):
        ...
        try:
            ...
            self.save_data(
                recent_data, "stock_daily_catl", f"{self.stock_code}日线数据"
            )
            return recent_data
        ...
        try:
            ...
            if not cpi_df.empty:
                ...
                if "月份" in cpi_df.columns:
                    ...
                    if not cpi_df.empty:
                        two_years_ago = datetime.datetime.now() - datetime.timedelta(
                            days=2 * 365
                        )
                        cpi_df = cpi_df[cpi_df["月份"] >= two_years_ago]
                        self.logger.info(
                            f"✅ CPI数据已限制为过去2年: {len(cpi_df)} 条记录"
                        )
    ...
    def fetch_stock_news(self):
        ...
        try:
            self.logger.info(
                f"📰 开始抓取{self.stock_name}({self.stock_code})新闻数据..."
            )
            # 尝试使用akshare的新闻接口
        ...
        try:
            ...
            if not vol50.empty:
                if "date" in vol50.columns:
                    ...
                    one_month_ago = datetime.datetime.now() - datetime.timedelta(
                        days=30
                    )
                    vol50 = vol50[vol50["date"] >= one_month_ago]
                    self.logger.info(
                        f"✅ 期权波动率数据已限制为过去1个月: {len(vol50)} 条记录"
                    )
    ...
    def fetch_institution_recommendation(self):
        ...
        try:
            self.logger.info(
                f"🏦 开始抓取{self.stock_name}({self.stock_code})机构评级..."
            )
            ...
            if not inst_rec.empty:
                ...
                if date_col:
                    ...
                    six_months_ago = datetime.datetime.now() - datetime.timedelta(
                        days=180
                    )
                    inst_rec = inst_rec[inst_rec[date_col] >= six_months_ago]
                    self.logger.info(
                        f"✅ 机构评级数据已限制为过去半年: {len(inst_rec)} 条记录"
                    )
    ...
    def fetch_all_data(self):
        ...
        tasks = [
            ...
            (
                "industry_fund_flow",
                lambda: self.fetch_industry_fund_flow(),
                "行业资金流数据",
            ),
            ...
            (
                "option_volatility",
                lambda: self.fetch_option_volatility(),
                "期权波动率指数",
            ),
            (
                "institution_recommendation",
                lambda: self.fetch_institution_recommendation(),
                "机构评级数据",
            ),
        ]
...
def fetch_stock_data(stock_code, output_dir=None):
    ...
    fetcher = StockDataFetcher(
        stock_code=stock_code, auto_create_output_dir=False
    )  # 设置auto_create_output_dir为False，由output_dir参数控制
    if output_dir:
