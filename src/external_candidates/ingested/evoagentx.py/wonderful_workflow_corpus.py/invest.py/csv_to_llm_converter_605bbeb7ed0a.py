# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\Wonderful_workflow_corpus\invest\csv_to_llm_converter.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
CSV数据转LLM JSON格式转换器
...
class CSVToLLMConverter:
        ...
        for file_path in self.data_dir.glob("*.csv"):
            ...
            if file_type:
                csv_files[file_type] = {
                    "file_path": file_path,
                    "filename": filename,
                    "config": self.file_priority.get(
                        file_type, {"weight": "normal", "max_rows": 10}
                    ),
                }
    ...
    def read_and_process_csv(
        self, file_path: Path, max_rows: int, weight: str
    ) -> List[Dict]:
        """读取并处理CSV文件"""
        try:
    ...
    def generate_llm_analysis_prompt(self) -> str:
        ...
        for i, (file_type, file_info) in enumerate(sorted_files, 1):
            ...
            prompt_parts.append(
                f"{i}. {emoji} {self._get_chinese_name(file_type)} ({file_info['filename']})"
            )
        ...
        for i, (file_type, file_info) in enumerate(sorted_files, 1):
            ...
            data = self.read_and_process_csv(
                file_path, config["max_rows"], config["weight"]
            )
    ...
    def get_json_data(self) -> Dict[str, List[Dict]]:
        ...
        for file_type, file_info in csv_files.items():
            config = file_info["config"]
            data = self.read_and_process_csv(
                file_info["file_path"], config["max_rows"], config["weight"]
            )
            if data:
                chinese_name = self._get_chinese_name(file_type)
