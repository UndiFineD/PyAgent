...
def find_leaks_proxynova(email, proxy, number):
    ...
    if response.status_code == 200:
        ...
        print(
            colored(
                f"[*] Found {total_results} different records in database", "magenta"
            ),
            end="",
        )
    ...
    if database.endswith(".json"):
        with open(database, "r") as json_file:
            try:
                ...
                print(
                    colored(
                        "[!] Error: Failed to parse local database as JSON\n", "red"
                    )
                )
                exit(-1)
    else:
        ...
        try:
            with open(database, "r") as file:
                while True:
                    ...
                    print(
                        colored(
                            f"\r[*] Reading {line_count} lines in database..", "magenta"
                        ),
                        end="",
                        flush=True,
                    )
