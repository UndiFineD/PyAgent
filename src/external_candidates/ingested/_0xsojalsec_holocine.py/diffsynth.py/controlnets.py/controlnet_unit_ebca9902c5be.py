# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\controlnets\controlnet_unit.py
import numpy as np
import torch
...
class ControlNetConfigUnit:
    def __init__(
        self, processor_id: Processor_id, model_path, scale=1.0, skip_processor=False
    ):
...
class MultiControlNetManager:
    ...
    def process_image(self, image, processor_id=None):
        ...
        processed_image = torch.concat(
            [
                torch.Tensor(np.array(image_, dtype=np.float32) / 255)
                .permute(2, 0, 1)
                .unsqueeze(0)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        res_stack = None
        for processor, conditioning, model, scale in zip(
            self.processors, conditionings, self.models, self.scales
        ):
            res_stack_ = model(
                sample,
                timestep,
...
class FluxMultiControlNetManager(MultiControlNetManager):
    ...
    def __call__(self, conditionings, **kwargs):
        ...
        for processor, conditioning, model, scale in zip(
            ...
            res_stack_, single_res_stack_ = model(
