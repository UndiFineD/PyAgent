    ...
    def __init__(
        ...
        if not skip_processor:
            if processor_id == "canny":
                ...
                self.processor = LineartAnimeDetector.from_pretrained(model_path).to(
                    device
                )
                ...
                self.processor = NormalBaeDetector.from_pretrained(model_path).to(
                    device
                )
            elif (
                processor_id == "tile"
                or processor_id == "none"
                or processor_id == "inpaint"
            ):
                self.processor = None
            else:
    ...
    def __call__(self, image, mask=None):
        ...
        if self.processor is not None:
            detect_resolution = (
                ...
                if self.detect_resolution is not None
