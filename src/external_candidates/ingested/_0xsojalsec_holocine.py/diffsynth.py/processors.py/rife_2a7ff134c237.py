# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\processors\RIFE.py
import numpy as np
import torch
...
class RIFESmoother(VideoProcessor):
    ...
    def process_image(self, image):
        ...
        image = torch.Tensor(
            np.array(image, dtype=np.float32)[:, :, [2, 1, 0]] / 255
        ).permute(2, 0, 1)
        return image
    ...
    def process_tensors(self, input_tensor, scale=1.0, batch_size=4):
        ...
        for batch_id in range(0, input_tensor.shape[0], batch_size):
            ...
            flow, mask, merged = self.model(
                batch_input_tensor, [4 / scale, 2 / scale, 1 / scale]
            )
