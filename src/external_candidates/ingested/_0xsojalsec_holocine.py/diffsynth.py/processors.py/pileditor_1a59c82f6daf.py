# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\processors\PILEditor.py
...
class ContrastEditor(VideoProcessor):
    ...
    def __call__(self, rendered_frames, **kwargs):
        rendered_frames = [
            ImageEnhance.Contrast(i).enhance(self.rate) for i in rendered_frames
        ]
...
class SharpnessEditor(VideoProcessor):
    ...
    def __call__(self, rendered_frames, **kwargs):
        rendered_frames = [
            ImageEnhance.Sharpness(i).enhance(self.rate) for i in rendered_frames
        ]
        return rendered_frames
