# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\processors\sequencial_processor.py
...
class SequencialProcessor(VideoProcessor):
    ...
    def from_model_manager(model_manager, configs):
        processors = [
            AutoVideoProcessor.from_model_manager(
                model_manager, config["processor_type"], **config["config"]
            )
