# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\processors\FastBlend.py
import cupy as cp
import numpy as np
...
class FastBlendSmoother(VideoProcessor):
    ...
    def inference_fast(self, frames_guide, frames_style):
        ...
        # merge
        frames = []
        for (frame_l, weight_l), frame_m, (frame_r, weight_r) in zip(
            table_l, frames_style, table_r
        ):
            weight_m = -1
            weight = weight_l + weight_m + weight_r
            frame = (
                frame_l * (weight_l / weight)
                + frame_m * (weight_m / weight)
                + frame_r * (weight_r / weight)
            )
            frames.append(frame)
    ...
    def inference_balanced(self, frames_guide, frames_style):
        ...
        for target in range(n):
            for source in range(
                target - self.window_size, target + self.window_size + 1
            ):
        ...
        for batch_id in tqdm(
            range(0, len(tasks), self.batch_size), desc="Balanced Mode"
        ):
            tasks_batch = tasks[batch_id : min(batch_id + self.batch_size, len(tasks))]
            source_guide = np.stack(
                [frames_guide[source] for source, target in tasks_batch]
            )
            target_guide = np.stack(
                [frames_guide[target] for source, target in tasks_batch]
            )
            source_style = np.stack(
                [frames_style[source] for source, target in tasks_batch]
            )
            ...
            for (source, target), result in zip(tasks_batch, target_style):
                ...
                if weight + 1 == min(n, target + self.window_size + 1) - max(
                    0, target - self.window_size
                ):
    ...
    def inference_accurate(self, frames_guide, frames_style):
        ...
        for target in tqdm(range(n), desc="Accurate Mode"):
            l, r = max(target - self.window_size, 0), min(
                target + self.window_size + 1, n
            )
            remapped_frames = []
            for i in range(l, r, self.batch_size):
                j = min(i + self.batch_size, r)
                source_guide = np.stack(
                    [frames_guide[source] for source in range(i, j)]
                )
                target_guide = np.stack([frames_guide[target]] * (j - i))
                source_style = np.stack(
                    [frames_style[source] for source in range(i, j)]
                )
                _, target_style = patch_match_engine.estimate_nnf(
                    source_guide, target_guide, source_style
                )
