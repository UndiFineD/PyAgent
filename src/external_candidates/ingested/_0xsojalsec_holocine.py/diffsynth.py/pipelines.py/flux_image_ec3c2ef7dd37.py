# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\flux_image.py
...
class FluxImagePipeline(BasePipeline):
        ...
        for config in controlnet_config_units:
            controlnet_unit = ControlNetUnit(
                Annotator(
                    config.processor_id,
                    device=self.device,
                    skip_processor=config.skip_processor,
                ),
    ...
    def from_model_manager(
        ...
        pipe = FluxImagePipeline(
            ...
            torch_dtype=(
                model_manager.torch_dtype if torch_dtype is None else torch_dtype
            ),
        )
        pipe.fetch_models(
            model_manager,
    ...
    def prepare_extra_input(self, latents=None, guidance=1.0):
        ...
        guidance = torch.Tensor([guidance] * latents.shape[0]).to(
    ...
    def prepare_controlnet_input(
        ...
        for i in range(len(self.controlnet.processors)):
            # image annotator
            image = self.controlnet.process_image(controlnet_image[i], processor_id=i)[
                0
            ]
            if (
                controlnet_inpaint_mask is not None
                and self.controlnet.processors[i].processor_id == "inpaint"
            ):
                image = self.apply_controlnet_mask_on_image(
                    image, controlnet_inpaint_mask
                )
    ...
    def prepare_ipadapter_inputs(self, images, height=384, width=384):
        images = [
            image.convert("RGB").resize((width, height), resample=3) for image in images
        ]
        images = [
            self.preprocess_image(image).to(device=self.device, dtype=self.torch_dtype)
            for image in images
        ]
    ...
    def prepare_entity_inputs(
        ...
        if enable_eligen_inpaint:
            ...
            fg_masks = torch.cat(
                [
                    self.preprocess_image(mask.resize((width // 8, height // 8))).mean(
                        dim=1, keepdim=True
                    )
                    for mask in masks_
                ]
        ...
        entity_prompts = self.encode_prompt(
            entity_prompts, t5_sequence_length=t5_sequence_length
        )["prompt_emb"].unsqueeze(0)
        return entity_prompts, entity_masks, fg_mask, bg_mask

    def prepare_latents(
        self, input_image, height, width, seed, tiled, tile_size, tile_stride
    ):
        if input_image is not None:
            ...
            input_latents = self.encode_image(
                image, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
            )
            ...
            latents = self.scheduler.add_noise(
                input_latents, noise, timestep=self.scheduler.timesteps[0]
            )
        else:
    ...
    def prepare_ipadapter(self, ipadapter_images, ipadapter_scale):
        if ipadapter_images is not None:
            ...
            ipadapter_image_encoding = self.ipadapter_image_encoder(
                ipadapter_images
            ).pooler_output
    ...
    def prepare_controlnet(
        ...
        if controlnet_image is not None:
            ...
            controlnet_kwargs_posi = {
                "controlnet_frames": self.prepare_controlnet_input(
                    controlnet_image, controlnet_inpaint_mask, tiler_kwargs
                )
            }
            if len(masks) > 0 and controlnet_inpaint_mask is not None:
                ...
                local_controlnet_kwargs = [
                    {
                        "controlnet_frames": self.prepare_controlnet_input(
                            controlnet_image, mask, tiler_kwargs
                        )
                    }
                    for mask in masks
                ]
            ...
            controlnet_kwargs_posi, local_controlnet_kwargs = {
                "controlnet_frames": None
            }, [{}] * len(masks)
        controlnet_kwargs_nega = (
            controlnet_kwargs_posi if enable_controlnet_on_negative else {}
        )
    ...
    def prepare_eligen(
        ...
        if eligen_entity_masks is not None:
            entity_prompt_emb_posi, entity_masks_posi, fg_mask, bg_mask = (
    ...
    def prepare_prompts(
        ...
        prompt, local_prompts, masks, mask_scales = self.extend_prompt(
            prompt, local_prompts, masks, mask_scales
        )

        # Encode prompts
        prompt_emb_posi = self.encode_prompt(
            prompt, t5_sequence_length=t5_sequence_length
        )
        prompt_emb_nega = (
            self.encode_prompt(
    ...
    def prepare_infinite_you(
        self, id_image, controlnet_image, infinityou_guidance, height, width
    ):
        if self.infinityou_processor is not None and id_image is not None:
            return self.infinityou_processor.prepare_infinite_you(
    ...
    def prepare_flex_kwargs(
        ...
        if self.dit.input_dim == 196:
            if flex_inpaint_image is None:
                ...
                flex_inpaint_image = self.preprocess_image(flex_inpaint_image).to(
                    device=self.device, dtype=self.torch_dtype
                )
                flex_inpaint_image = self.encode_image(
                    flex_inpaint_image,
                    tiled=tiled,
                    tile_size=tile_size,
                    tile_stride=tile_stride,
            ...
            if flex_inpaint_mask is None:
                ...
                flex_inpaint_mask = self.preprocess_image(flex_inpaint_mask).to(
                    device=self.device, dtype=self.torch_dtype
                )
            ...
            if flex_control_image is None:
                ...
                flex_control_image = self.preprocess_image(flex_control_image).to(
                    device=self.device, dtype=self.torch_dtype
                )
                flex_control_image = (
                    self.encode_image(
    ...
    def __call__(
        ...
        latents, input_latents = self.prepare_latents(
            input_image, height, width, seed, tiled, tile_size, tile_stride
        )

        # Prompt
        prompt_emb_posi, prompt_emb_nega, prompt_emb_locals = self.prepare_prompts(
            prompt,
            local_prompts,
            masks,
            mask_scales,
        ...
        # InfiniteYou
        infiniteyou_kwargs, controlnet_image = self.prepare_infinite_you(
            infinityou_id_image, controlnet_image, infinityou_guidance, height, width
        )

        # Entity control
        eligen_kwargs_posi, eligen_kwargs_nega, fg_mask, bg_mask = self.prepare_eligen(
            prompt_emb_nega,
            eligen_entity_prompts,
            eligen_entity_masks,
            width,
        ...
        # IP-Adapter
        ipadapter_kwargs_list_posi, ipadapter_kwargs_list_nega = self.prepare_ipadapter(
            ipadapter_images, ipadapter_scale
        )

        # ControlNets
        controlnet_kwargs_posi, controlnet_kwargs_nega, local_controlnet_kwargs = (
        ...
        # Flex
        flex_kwargs = self.prepare_flex_kwargs(
            latents,
            flex_inpaint_image,
            flex_inpaint_mask,
            flex_control_image,
        ...
        # Step1x
        step1x_kwargs_posi, step1x_kwargs_nega = self.prepare_step1x_kwargs(
            prompt, negative_prompt, image=step1x_reference_image
        )
        ...
        for progress_id, timestep in enumerate(
            ...
            # Positive side
            inference_callback = (
                lambda prompt_emb_posi, controlnet_kwargs: lets_dance_flux(
                    dit=self.dit,
                    controlnet=self.controlnet,
                    step1x_connector=self.step1x_connector,
            ...
            noise_pred_posi = self.control_noise_via_local_prompts(
                ...
                mask_scales,
                inference_callback,
                special_kwargs=controlnet_kwargs_posi,
                special_local_kwargs_list=local_controlnet_kwargs,
            )

            # Inpaint
            if enable_eligen_inpaint:
                noise_pred_posi = self.inpaint_fusion(
                    latents,
                    input_latents,
                    noise_pred_posi,
                    fg_mask,
...
class InfinitYou:
    ...
    def prepare_infinite_you(
        self, model, id_image, controlnet_image, infinityou_guidance, height, width
    ):
        import cv2
        ...
        if controlnet_image is None:
            controlnet_image = Image.fromarray(
    ...
    **kwargs,
    ...
    if tiled:
        ...
        def flux_forward_fn(hl, hr, wl, wr):
            tiled_controlnet_frames = (
                [f[:, :, hl:hr, wl:wr] for f in controlnet_frames]
