# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\hunyuan_image.py
import numpy as np
import torch
...
class ImageSizeManager:
    ...
    def get_meshgrid(self, start, *args):
        ...
        grid_h = np.linspace(
            start[0], stop[0], num[0], endpoint=False, dtype=np.float32
        )  # 12-20 中间差值32份   0-32 中间差值124份
        grid_w = np.linspace(
            start[1], stop[1], num[1], endpoint=False, dtype=np.float32
        )
    ...
    def get_2d_rotary_pos_embed(self, embed_dim, start, *args, use_real=True):
        ...
        grid = grid.reshape(
            [2, 1, *grid.shape[1:]]
        )  # 返回一个采样矩阵  分辨率与目标分辨率一致
        pos_embed = self.get_2d_rotary_pos_embed_from_grid(
    ...
    def get_2d_rotary_pos_embed_from_grid(self, embed_dim, grid, use_real=False):
        ...
        emb_h = self.get_1d_rotary_pos_embed(
            embed_dim // 2, grid[0].reshape(-1), use_real=use_real
        )  # (H*W, D/4)
        emb_w = self.get_1d_rotary_pos_embed(
...
class HunyuanDiTImagePipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.float16):
        ...
        self.scheduler = EnhancedDDIMScheduler(
            prediction_type="v_prediction", beta_start=0.00085, beta_end=0.03
        )
    ...
    def from_model_manager(
        model_manager: ModelManager, prompt_refiner_classes=[], device=None
    ):
    ...
    def encode_prompt(self, prompt, clip_skip=1, clip_skip_2=1, positive=True):
        text_emb, text_emb_mask, text_emb_t5, text_emb_mask_t5 = (
    ...
    def prepare_extra_input(
        self, latents=None, tiled=False, tile_size=64, tile_stride=32
    ):
        ...
        image_meta_size = torch.as_tensor([width, height, width, height, 0, 0]).to(
            device=self.device
        )
        ...
        return {
            ...
            "freq_cis_img": (
                freqs_cis_img[0].to(dtype=self.torch_dtype, device=self.device),
                freqs_cis_img[1].to(dtype=self.torch_dtype, device=self.device),
            ),
            "tiled": tiled,
            "tile_size": tile_size,
            "tile_stride": tile_stride,
    ...
    def __call__(
        ...
        prompt_emb_posi = self.encode_prompt(
            prompt, clip_skip=clip_skip, clip_skip_2=clip_skip_2, positive=True
        )
        if cfg_scale != 1.0:
        ...
        prompt_emb_locals = [
            ...
            for prompt_local in local_prompts
        ]
        ...
        for progress_id, timestep in enumerate(
            ...
            timestep = torch.tensor([timestep]).to(
