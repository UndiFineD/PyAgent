# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\wan_video.py
import os
import types
...
class WanVideoPipeline(BasePipeline):
    ...
    def encode_image(
        ...
        if end_image is not None:
            end_image = self.preprocess_image(end_image.resize((width, height))).to(
                self.device
            )
    ...
    def encode_control_video(
        self, control_video, tiled=True, tile_size=(34, 34), tile_stride=(18, 16)
    ):
        control_video = self.preprocess_images(control_video)
        control_video = torch.stack(control_video, dim=2).to(
    ...
    def prepare_reference_image(self, reference_image, height, width):
        if reference_image is not None:
            ...
            reference_image = torch.stack(reference_image, dim=2).to(
                dtype=self.torch_dtype, device=self.device
            )
    ...
    def prepare_vace_kwargs(
        ...
        if (
            ...
            if vace_reference_image is None:
                ...
                vace_reference_image = torch.stack(vace_reference_image, dim=2).to(
                    dtype=self.torch_dtype, device=self.device
                )
                ...
                noise = self.generate_noise(
                    (1, 16, 1, latents.shape[3], latents.shape[4]),
                    seed=seed,
                    device=rand_device,
                    dtype=torch.float32,
                )
    ...
    def __call__(
        ...
        if input_image is not None and self.image_encoder is not None:
            ...
            image_emb = self.encode_image(
                input_image, end_image, num_frames, height, width, **tiler_kwargs
            )
        else:
            image_emb = {}
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if reference_latents is not None:
        reference_latents = (
            dit.ref_conv(reference_latents[:, :, 0]).flatten(2).transpose(1, 2)
        )
