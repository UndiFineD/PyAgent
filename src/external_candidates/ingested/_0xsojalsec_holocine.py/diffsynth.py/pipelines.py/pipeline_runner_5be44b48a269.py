# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\pipeline_runner.py
import json
import os
...
from .sd_video import ControlNetConfigUnit, ModelManager, SDVideoPipeline
...
class SDVideoPipelineRunner:
    ...
    def load_pipeline(
        ...
        for file_name in os.listdir(textual_inversion_folder):
            if (
                ...
                textual_inversion_paths.append(
                    os.path.join(textual_inversion_folder, file_name)
                )
        pipe.prompter.load_textual_inversions(textual_inversion_paths)
        return model_manager, pipe

    def load_smoother(self, model_manager, smoother_configs):
        smoother = SequencialProcessor.from_model_manager(
            model_manager, smoother_configs
        )
        return smoother
    ...
    def synthesize_video(self, model_manager, pipe, seed, smoother, **pipeline_inputs):
        ...
        if self.in_streamlit:
            ...
            output_video = pipe(
                **pipeline_inputs, smoother=smoother, progress_bar_st=progress_bar_st
            )
    ...
    def load_video(
        ...
        video = VideoData(
            video_file=video_file, image_folder=image_folder, height=height, width=width
        )
    ...
    def add_data_to_pipeline_inputs(self, data, pipeline_inputs):
        ...
        pipeline_inputs["width"], pipeline_inputs["height"] = pipeline_inputs[
            "input_frames"
        ][0].size
        if len(data["controlnet_frames"]) > 0:
            pipeline_inputs["controlnet_frames"] = [
                self.load_video(**unit) for unit in data["controlnet_frames"]
            ]
    ...
    def run(self, config):
        ...
        config["pipeline"]["pipeline_inputs"] = self.add_data_to_pipeline_inputs(
            config["data"], config["pipeline"]["pipeline_inputs"]
        )
        if self.in_streamlit:
        ...
        self.save_output(
            output_video, config["data"]["output_folder"], config["data"]["fps"], config
        )
        if self.in_streamlit:
        ...
        video_file = open(
            os.path.join(os.path.join(config["data"]["output_folder"], "video.mp4")),
            "rb",
        )
        if self.in_streamlit:
