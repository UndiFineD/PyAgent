# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\omnigen_image.py
import os
...
class OmniGenCache(DynamicCache):
    def __init__(self, num_tokens_for_img: int, offload_kv_cache: bool = False) -> None:
        if not torch.cuda.is_available():
            print(
                "No available GPU, offload_kv_cache will be set to False, which will result in large memory usage and time cost when input multiple images!!!"
            )
...
class OmnigenImagePipeline(BasePipeline):
    ...
    def fetch_models(self, model_manager: ModelManager, prompt_refiner_classes=[]):
        # Main models
        self.transformer, model_path = model_manager.fetch_model(
            "omnigen_transformer", require_model_path=True
        )
    ...
    def encode_images(self, images, tiled=False, tile_size=64, tile_stride=32):
        latents = [
            self.encode_image(
                image.to(device=self.device), tiled, tile_size, tile_stride
            ).to(self.torch_dtype)
            for image in images
        ]
        return latents
    ...
    def encode_prompt(self, prompt, clip_skip=1, positive=True):
        prompt_emb = self.prompter.encode_prompt(
            prompt, clip_skip=clip_skip, device=self.device, positive=positive
        )
    ...
    def __call__(
        ...
        # Encode images
        reference_latents = [
            self.encode_images(images, **tiler_kwargs)
            for images in input_data["input_pixel_values"]
        ]

        # Pack all parameters
        model_kwargs = dict(
            input_ids=[
                input_ids.to(self.device) for input_ids in input_data["input_ids"]
            ],
            input_img_latents=reference_latents,
            input_image_sizes=input_data["input_image_sizes"],
            attention_mask=[
                attention_mask.to(self.device)
                for attention_mask in input_data["attention_mask"]
            ],
            position_ids=[
                position_ids.to(self.device)
                for position_ids in input_data["position_ids"]
            ],
            cfg_scale=cfg_scale,
        ...
        cache = (
            [
                OmniGenCache(latents.size(-1) * latents.size(-2) // 4, offload_kv_cache)
        ...
        for progress_id, timestep in enumerate(
            ...
            # Forward
            noise_pred, cache = self.transformer.forward_with_separate_cfg(
                latents, timestep, past_key_values=cache, **model_kwargs
            )
        ...
        image = self.decode_image(
            latents, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
        )
