...
class HunyuanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.float16):
        ...
        self.scheduler = FlowMatchScheduler(
            shift=7.0, sigma_min=0.0, extra_one_step=True
        )
    ...
    def enable_vram_management(self):
        ...
        self.text_encoder_2.enable_auto_offload(
    ...
    def prepare_vae_images_inputs(self, semantic_images, i2v_resolution="720p"):
        ...
        semantic_image_pixel_values = [
            ref_image_transform(semantic_image) for semantic_image in semantic_images
        ]
        semantic_image_pixel_values = (
    ...
    def encode_video(self, frames, tile_size=(17, 30, 30), tile_stride=(12, 20, 20)):
        ...
        latents = self.vae_encoder.encode_video(
            frames, tile_size=tile_size, tile_stride=tile_stride
        )
        return latents
    ...
    def __call__(
        ...
        if input_images is not None:
            ...
            with torch.autocast(
                device_type=self.device, dtype=torch.float16, enabled=True
            ):
        ...
        if input_video is not None:
            ...
            latents = noise * t + image_latents.repeat(
                1, 1, (num_frames - 1) // 4 + 1, 1, 1
            ) * (1 - t)
        ...
        prompt_emb_posi = self.encode_prompt(
            prompt, positive=True, input_images=input_images
        )
        if cfg_scale != 1.0:
        ...
        for progress_id, timestep in enumerate(
            ...
            with torch.autocast(device_type=self.device, dtype=self.torch_dtype):
                ...
                if cfg_scale != 1.0:
                    noise_pred_nega = forward_func(
                        self.dit, latents, timestep, **prompt_emb_nega, **extra_input
                    )
            ...
            if step_processor is not None:
                ...
                rendered_frames = self.vae_decoder.decode_video(
                    rendered_frames, **tiler_kwargs
                )
                rendered_frames = self.tensor2video(rendered_frames[0])
                rendered_frames = step_processor(
                    rendered_frames, original_frames=input_video
                )
                self.load_models_to_device(["vae_encoder"])
                rendered_frames = self.preprocess_images(rendered_frames)
                rendered_frames = torch.stack(rendered_frames, dim=2)
                target_latents = self.encode_video(rendered_frames).to(
                    dtype=self.torch_dtype, device=self.device
                )
...
class TeaCache:
    ...
    def check(self, dit: HunyuanVideoDiT, img, vec):
        img_ = img.clone()
        vec_ = vec.clone()
        img_mod1_shift, img_mod1_scale, _, _, _, _ = (
            dit.double_blocks[0].component_a.mod(vec_).chunk(6, dim=-1)
        )
    ...
    **kwargs,
    ...
    if tea_cache_update:
        ...
        for block in tqdm(dit.double_blocks, desc="Double stream blocks"):
            img, txt = block(
                img, txt, vec, (freqs_cos, freqs_sin), split_token=split_token
            )

        x = torch.concat([img, txt], dim=1)
        for block in tqdm(dit.single_blocks, desc="Single stream blocks"):
            x = block(
                x, vec, (freqs_cos, freqs_sin), txt_len=txt_len, split_token=split_token
            )
        img = x[:, :-txt_len]
