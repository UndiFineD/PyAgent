# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\step_video.py
import numpy as np
import torch
...
from ..models.stepvideo_dit import RMSNorm, StepVideoModel
from ..models.stepvideo_text_encoder import STEP1TextEncoder
from ..models.stepvideo_vae import (
    BaseGroupNorm,
    CausalConv,
    CausalConvAfterNorm,
    StepVideoVAE,
...
class StepVideoPipeline(BasePipeline):
    ...
    def encode_prompt(self, prompt, positive=True):
        clip_embeds, llm_embeds, llm_mask = self.prompter.encode_prompt(
        ...
        return {
            "encoder_hidden_states_2": clip_embeds,
            "encoder_hidden_states": llm_embeds,
    ...
    def __call__(
        ...
        latents = self.generate_noise(
            (1, max(num_frames // 17 * 3, 1), 64, height // 16, width // 16),
            seed=seed,
            device=rand_device,
        ...
        for progress_id, timestep in enumerate(
            ...
            if cfg_scale != 1.0:
                noise_pred_nega = self.dit(
                    latents, timestep=timestep, **prompt_emb_nega
                )
        ...
        frames = self.vae.decode(
            latents, device=self.device, smooth_scale=smooth_scale, **tiler_kwargs
        )
