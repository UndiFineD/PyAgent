# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\flux_image_new.py
import glob
import os
import types
import warnings
    ...
    def forward(
        ...
        **kwargs,
        ...
        for controlnet_input, conditioning in zip(controlnet_inputs, conditionings):
            progress = (num_inference_steps - 1 - progress_id) / max(
            ...
            res_stack_, single_res_stack_ = self.process_single_controlnet(
                controlnet_input, conditioning, **kwargs
            )
            if res_stack is None:
                ...
                single_res_stack = [
                    i + j for i, j in zip(single_res_stack, single_res_stack_)
                ]
        return res_stack, single_res_stack
...
class FluxImagePipeline(BasePipeline):
    ...
    def enable_lora_magic(self):
        if self.dit is not None:
            if not (
                hasattr(self.dit, "vram_management_enabled")
                and self.dit.vram_management_enabled
            ):
    ...
    def enable_vram_management(
        ...
        # Default config
        default_vram_management_models = [
            "text_encoder_1",
            "vae_decoder",
            "vae_encoder",
            "controlnet",
        ...
        for model_name in default_vram_management_models:
            self._enable_vram_management_with_default_config(
                getattr(self, model_name), vram_limit
            )
...
class FluxImageUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: FluxImagePipeline, height, width, seed, rand_device):
        noise = pipe.generate_noise(
            (1, 16, height // 8, width // 8), seed=seed, rand_device=rand_device
        )
        return {"noise": noise}
...
class FluxImageUnit_InputImageEmbedder(PipelineUnit):
    ...
    def process(
        self, pipe: FluxImagePipeline, input_image, noise, tiled, tile_size, tile_stride
    ):
        ...
        input_latents = pipe.vae_encoder(
            image, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
        )
...
class FluxImageUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(
        self, pipe: FluxImagePipeline, prompt, t5_sequence_length, positive
    ) -> dict:
...
class FluxImageUnit_EmbeddedGuidanceEmbedder(PipelineUnit):
    ...
    def process(self, pipe: FluxImagePipeline, embedded_guidance, latents):
        guidance = torch.Tensor([embedded_guidance] * latents.shape[0]).to(
...
class FluxImageUnit_Kontext(PipelineUnit):
    ...
    def process(
        self, pipe: FluxImagePipeline, kontext_images, tiled, tile_size, tile_stride
    ):
        ...
        for kontext_image in kontext_images:
            ...
            kontext_latent = pipe.vae_encoder(
                kontext_image, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
            )
        ...
        return {
            "kontext_latents": kontext_latents,
    ...
    def process(
        ...
        for controlnet_input in controlnet_inputs:
            ...
            if controlnet_input.inpaint_mask is not None:
                image = self.apply_controlnet_mask_on_image(
            ...
            image = pipe.vae_encoder(
                image, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
            )

            if controlnet_input.inpaint_mask is not None:
                image = self.apply_controlnet_mask_on_latents(
                    pipe, image, controlnet_input.inpaint_mask
                )
...
class FluxImageUnit_IPAdapter(PipelineUnit):
    def __init__(self):
        super().__init__(
            take_over=True, onload_model_names=("ipadapter_image_encoder", "ipadapter")
        )

    def process(self, pipe: FluxImagePipeline, inputs_shared, inputs_posi, inputs_nega):
        ipadapter_images, ipadapter_scale = inputs_shared.get(
        ...
        images = [
            image.convert("RGB").resize((384, 384), resample=3)
            for image in ipadapter_images
        ]
        images = [
            pipe.preprocess_image(image).to(device=pipe.device, dtype=pipe.torch_dtype)
            for image in images
        ]
        ipadapter_images = torch.cat(images, dim=0)
        ipadapter_image_encoding = pipe.ipadapter_image_encoder(
            ipadapter_images
        ).pooler_output

        inputs_posi.update(
            {
                "ipadapter_kwargs_list": pipe.ipadapter(
                    ipadapter_image_encoding, scale=ipadapter_scale
                )
            }
        )
        if inputs_shared.get("cfg_scale", 1.0) != 1.0:
            inputs_nega.update(
                {
                    "ipadapter_kwargs_list": pipe.ipadapter(
                        torch.zeros_like(ipadapter_image_encoding)
                    )
                }
            )
        return inputs_shared, inputs_posi, inputs_nega
...
class FluxImageUnit_EntityControl(PipelineUnit):
    def __init__(self):
        super().__init__(
            take_over=True, onload_model_names=("text_encoder_1", "text_encoder_2")
        )
        ...
        for mask in masks:
            mask = (
                pipe.preprocess_image(
                    mask.resize((width, height), resample=Image.NEAREST)
                ).mean(dim=1, keepdim=True)
                > 0
            )
            mask = mask.repeat(1, dim, 1, 1).to(
                device=pipe.device, dtype=pipe.torch_dtype
            )
    ...
    def prepare_entity_inputs(
        ...
        entity_masks = self.preprocess_masks(
            pipe, entity_masks, height // 8, width // 8, 1
        )
    ...
    def prepare_eligen(
        ...
        if enable_eligen_on_negative and cfg_scale != 1.0:
            entity_prompt_emb_nega = (
                prompt_emb_nega["prompt_emb"]
                .unsqueeze(1)
                .repeat(1, entity_masks_posi.shape[1], 1, 1)
            )
    ...
    def process(self, pipe: FluxImagePipeline, inputs_shared, inputs_posi, inputs_nega):
        eligen_entity_prompts, eligen_entity_masks = inputs_shared.get(
...
class FluxImageUnit_TeaCache(PipelineUnit):
    ...
    def process(
        self, pipe: FluxImagePipeline, num_inference_steps, tea_cache_l1_thresh
    ):
    ...
    def process(
        ...
        if pipe.dit.input_dim == 196:
            ...
            if flex_inpaint_mask is None:
                ...
                flex_inpaint_mask = flex_inpaint_mask.resize(
                    (latents.shape[3], latents.shape[2])
                )
            ...
            flex_condition = torch.concat(
                [flex_inpaint_image, flex_inpaint_mask, flex_control_image], dim=1
            )
            flex_uncondition = torch.concat(
                [
                    flex_inpaint_image,
                    flex_inpaint_mask,
                    torch.zeros_like(flex_control_image),
                ],
                dim=1,
            )
...
class FluxImageUnit_InfiniteYou(PipelineUnit):
    ...
    def process(
        self, pipe: FluxImagePipeline, infinityou_id_image, infinityou_guidance
    ):
...
class FluxImageUnit_ValueControl(PipelineUnit):
    ...
    def process(
        self, pipe: FluxImagePipeline, prompt_emb, text_ids, value_controller_inputs
    ):
        if value_controller_inputs is None:
            return {}
        if not isinstance(value_controller_inputs, list):
            value_controller_inputs = [value_controller_inputs]
        value_controller_inputs = torch.tensor(value_controller_inputs).to(
        ...
        prompt_emb, text_ids = self.add_to_text_embedding(
            prompt_emb, text_ids, value_emb
        )
...
class InfinitYou(torch.nn.Module):
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        self.arcface_model = init_recognition_model("arcface", device=self.device).to(
            torch_dtype
        )
    ...
    def extract_arcface_bgr_embedding(self, in_image, landmark, device):
        ...
        arc_face_image = face_align.norm_crop(
            in_image, landmark=np.array(landmark), image_size=112
        )
        arc_face_image = (
            torch.from_numpy(arc_face_image).unsqueeze(0).permute(0, 3, 1, 2) / 255.0
        )
        arc_face_image = 2 * arc_face_image - 1
        arc_face_image = arc_face_image.contiguous().to(
    ...
    def prepare_infinite_you(self, model, id_image, infinityou_guidance, device):
        ...
        id_emb = model(
            id_emb.unsqueeze(0).reshape([1, -1, 512]).to(dtype=self.torch_dtype)
        )
        infinityou_guidance = torch.Tensor([infinityou_guidance]).to(
            device=device, dtype=self.torch_dtype
        )
...
class FluxImageUnit_LoRAEncode(PipelineUnit):
    def __init__(self):
        super().__init__(take_over=True, onload_model_names=("lora_encoder",))
...
def model_fn_flux_image(
    ...
    **kwargs,
    ...
    if controlnet is not None and controlnet_conditionings is not None:
        ...
        if id_emb is not None:
            ...
            controlnet_extra_kwargs.update(
                {
                    "prompt_emb": id_emb,
                    "text_ids": controlnet_text_ids,
                    "guidance": infinityou_guidance,
                }
            )
    ...
    if step1x_llm_embedding is not None:
        prompt_emb, pooled_prompt_emb = step1x_connector(
            step1x_llm_embedding, timestep / 1000, step1x_mask
        )
        text_ids = torch.zeros(
            (1, prompt_emb.shape[1], 3),
    ...
    conditioning = dit.time_embedder(
        timestep, hidden_states.dtype
    ) + dit.pooled_text_embedder(pooled_prompt_emb)
    if dit.guidance_embedder is not None:
        guidance = guidance * 1000
        conditioning = conditioning + dit.guidance_embedder(
            guidance, hidden_states.dtype
        )
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if (
                ...
                and controlnet_res_stack is not None
            ):
        ...
        for block_id, block in enumerate(dit.single_blocks):
            ...
            if (
                ...
                if kontext_latents is None:
                    ...
                    hidden_states[
                        :, prompt_emb.shape[1] : -kontext_latents.shape[1]
                    ] = (
                        hidden_states[
                            :, prompt_emb.shape[1] : -kontext_latents.shape[1]
                        ]
