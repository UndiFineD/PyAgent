# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\sd_image.py
...
class SDImagePipeline(BasePipeline):
    ...
    def __call__(
        ...
        prompt_emb_locals = [
            self.encode_prompt(prompt_local, clip_skip=clip_skip, positive=True)
        ...
        if controlnet_image is not None:
            ...
            controlnet_image = self.controlnet.process_image(controlnet_image).to(
                device=self.device, dtype=self.torch_dtype
            )
