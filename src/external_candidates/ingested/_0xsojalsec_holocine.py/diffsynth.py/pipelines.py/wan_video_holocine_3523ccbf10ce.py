# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\wan_video_holocine.py
import glob
import os
import types
import warnings
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoHoloCinePipeline, prompt, positive) -> dict:
        ...
        prompt_emb, positions = pipe.prompter.encode_prompt(
...
class WanVideoUnit_Prompt_separatelyEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoHoloCinePipeline, prompt, positive) -> dict:
        pipe.load_models_to_device(self.onload_model_names)
        prompt_emb, positions = pipe.prompter.encode_prompt_separately(
            prompt, positive=positive, device=pipe.device
        )
        return {"context": prompt_emb, "text_cut_positions": positions}
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    ...
    def process(
        self, pipe: WanVideoHoloCinePipeline, input_image, end_image, height, width
    ):
        if (
            input_image is None
...
class WanVideoUnit_CfgMerger(PipelineUnit):
    ...
    def process(
        self, pipe: WanVideoHoloCinePipeline, inputs_shared, inputs_posi, inputs_nega
    ):
