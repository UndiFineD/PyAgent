# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\svd_video.py
import numpy as np
import torch
...
class SVDVideoPipeline(BasePipeline):
    ...
    def from_model_manager(model_manager: ModelManager, **kwargs):
        pipe = SVDVideoPipeline(
            device=model_manager.device, torch_dtype=model_manager.torch_dtype
        )
    ...
    def encode_image_with_vae(self, image, noise_aug_strength, seed=None):
        ...
        noise = self.generate_noise(
            image.shape, seed=seed, device=self.device, dtype=self.torch_dtype
        )
    ...
    def post_process_latents(
        self, latents, post_normalize=True, contrast_enhance_scale=1.0
    ):
        if post_normalize:
            mean, std = latents.mean(), latents.std()
            latents = (
                latents - latents.mean(dim=[1, 2, 3], keepdim=True)
            ) / latents.std(dim=[1, 2, 3], keepdim=True) * std + mean
    ...
    def __call__(
        ...
        if denoising_strength == 1.0:
            ...
            latents = self.scheduler.add_noise(
                latents, noise, self.scheduler.timesteps[0]
            )
        ...
        image_emb_vae_posi = repeat(
            self.encode_image_with_vae(input_image, noise_aug_strength, seed=seed),
            "B C H W -> (B T) C H W",
            T=num_frames,
        )
        ...
        cfg_scales = cfg_scales.reshape(num_frames, 1, 1, 1).to(
            device=self.device, dtype=self.torch_dtype
        )
        ...
        for progress_id, timestep in enumerate(
            ...
            for frame_id, mask_frame_id in enumerate(mask_frame_ids):
                latents[mask_frame_id] = self.scheduler.add_noise(
                    mask_latents[frame_id], noise[mask_frame_id], timestep
                )
        ...
        # Decode image
        latents = self.post_process_latents(
            latents,
...
class SVDCLIPImageProcessor:
    ...
    def resize_with_antialiasing(
        self, input, size, interpolation="bicubic", align_corners=True
    ):
        h, w = input.shape[-2:]
