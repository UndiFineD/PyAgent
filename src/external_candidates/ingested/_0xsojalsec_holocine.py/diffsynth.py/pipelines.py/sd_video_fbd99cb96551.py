# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\sd_video.py
...
def lets_dance_with_long_video(
    ...
    hidden_states_output = [
        (torch.zeros(sample[0].shape, dtype=sample[0].dtype), 0)
    ...
    for batch_id in range(0, num_frames, animatediff_stride):
        ...
        hidden_states_batch = lets_dance(
            ...
            controlnet_frames=(
                controlnet_frames[:, batch_id:batch_id_].to(device)
        ...
        for i, hidden_states_updated in zip(
            range(batch_id, batch_id_), hidden_states_batch
        ):
            bias = max(
                1
                - abs(i - (batch_id + batch_id_ - 1) / 2)
                / ((batch_id_ - batch_id - 1 + 1e-2) / 2),
                1e-2,
            )
            hidden_states, num = hidden_states_output[i]
            hidden_states = hidden_states * (
                num / (num + bias)
            ) + hidden_states_updated * (bias / (num + bias))
...
class SDVideoPipeline(SDImagePipeline):
    ...
    def __init__(
        self, device="cuda", torch_dtype=torch.float16, use_original_animatediff=True
    ):
        super().__init__(device=device, torch_dtype=torch_dtype)
        self.scheduler = EnhancedDDIMScheduler(
            beta_schedule="linear" if use_original_animatediff else "scaled_linear"
        )
    ...
    def fetch_models(
        ...
        self.ipadapter_image_encoder = model_manager.fetch_model(
    ...
    def from_model_manager(
        ...
        pipe.fetch_models(
            model_manager, controlnet_config_units, prompt_refiner_classes
        )
        return pipe
    ...
    def encode_video(self, processed_images, tiled=False, tile_size=64, tile_stride=32):
        ...
        for image in processed_images:
            image = self.preprocess_image(image).to(
    ...
    def __call__(
        ...
        prompt_emb_nega = self.encode_prompt(
            negative_prompt, clip_skip=clip_skip, positive=False
        )
        ...
        if ipadapter_images is not None:
            ...
            ipadapter_kwargs_list_posi = {
                "ipadapter_kwargs_list": self.ipadapter(
            ...
            if isinstance(controlnet_frames[0], list):
                ...
                for processor_id in range(len(controlnet_frames)):
                    controlnet_frames_.append(
                        torch.stack(
                            [
                                self.controlnet.process_image(
                                    controlnet_frame, processor_id=processor_id
                                ).to(self.torch_dtype)
                                for controlnet_frame in progress_bar_cmd(
                                    controlnet_frames[processor_id]
                                )
                            ],
                            dim=1,
                        )
                ...
                controlnet_frames = torch.stack(
                    [
                        self.controlnet.process_image(controlnet_frame).to(
                            self.torch_dtype
                        )
        ...
        for progress_id, timestep in enumerate(
            ...
            noise_pred_posi = lets_dance_with_long_video(
                self.unet,
                motion_modules=self.motion_modules,
            ...
            if smoother is not None and progress_id in smoother_progress_ids:
                rendered_frames = self.scheduler.step(
                    noise_pred, timestep, latents, to_final=True
                )
                rendered_frames = self.decode_video(rendered_frames)
                rendered_frames = smoother(
                    rendered_frames, original_frames=input_frames
                )
                target_latents = self.encode_video(rendered_frames)
                noise_pred = self.scheduler.return_to_timestep(
                    timestep, latents, target_latents
                )
        ...
        # Post-process
        if smoother is not None and (
            num_inference_steps in smoother_progress_ids or -1 in smoother_progress_ids
        ):
