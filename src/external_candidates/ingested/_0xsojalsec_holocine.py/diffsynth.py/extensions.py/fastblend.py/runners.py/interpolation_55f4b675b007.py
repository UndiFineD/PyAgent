...
class InterpolationModeRunner:
    ...
    def get_weight(self, l, m, r):
        ...
        if weight_l + weight_r == 0:
            ...
            weight_l, weight_r = weight_l / (weight_l + weight_r), weight_r / (
                weight_l + weight_r
            )
    ...
    def run(
        ...
        for tasks in task_group:
            index_start, index_end = min([i[1] for i in tasks]), max(
                [i[1] for i in tasks]
            )
            for batch_id in tqdm(
                ...
                if save_path is not None:
                    for frame_l, frame_r, (l, m, r) in zip(
                        target_style[0::2], target_style[1::2], tasks_batch
                    ):
                        weight_l, weight_r = self.get_weight(l, m, r)
                        frame = frame_l * weight_l + frame_r * weight_r
                        frame = frame.clip(0, 255).astype("uint8")
                        Image.fromarray(frame).save(
                            os.path.join(save_path, "%05d.png" % m)
                        )
        ...
        for i in tqdm(
            ...
            for i, frame in zip(range(l, r), target_style):
                if i == frame_id:
                    frame = frame.clip(0, 255).astype("uint8")
                    Image.fromarray(frame).save(
                        os.path.join(save_path, "%05d.png" % frame_id)
                    )
                    frame_id += 1
