...
class TableManager:
    ...
    def build_remapping_table(
        ...
        for batch_id in tqdm(range(0, len(tasks), batch_size), desc=desc):
            ...
            source_guide = np.stack(
                [frames_guide[task["source"]] for task in tasks_batch]
            )
            target_guide = np.stack(
                [frames_guide[task["target"]] for task in tasks_batch]
            )
            source_style = np.stack(
                [frames_style[task["source"]] for task in tasks_batch]
            )
    ...
    def tree_query(self, leftbound, rightbound):
        ...
        while node_index >= leftbound:
            node_level = 0
            while (1 << node_level) & node_index and node_index - (
                1 << node_level + 1
            ) + 1 >= leftbound:
                node_level += 1
    ...
    def process_window_sum(
        ...
        for batch_id in tqdm(range(0, len(tasks), batch_size), desc=desc):
            ...
            source_style = np.stack(
                [
                    blending_table[task["source"]][task["level"]][0]
        ...
        if save_path is not None:
            for target, frame in enumerate(frames):
                Image.fromarray(frame).save(
                    os.path.join(save_path, "%05d.png" % target)
                )
