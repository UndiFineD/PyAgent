# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\FastBlend\patch_match.py
import cupy as cp
import cv2
import numpy as np
...
class PatchMatcher:
    ...
    def random_step(self, nnf, r):
        ...
        step = cp.random.randint(
            -r, r + 1, size=(batch_size, self.height, self.width, 2), dtype=cp.int32
        )
...
class PyramidPatchMatcher:
    ...
    def initialize_nnf(self, batch_size):
        if self.initialize == "random":
            ...
            nnf = cp.stack(
                [
                    cp.random.randint(
                        0, height, (batch_size, height, width), dtype=cp.int32
                    ),
                    cp.random.randint(
                        0, width, (batch_size, height, width), dtype=cp.int32
                    ),
                ],
                axis=3,
            )
    ...
    def update_nnf(self, nnf, level):
        ...
        if height != nnf.shape[0] * 2 or width != nnf.shape[1] * 2:
            ...
            nnf = [
                cv2.resize(n, (width, height), interpolation=cv2.INTER_LINEAR)
                for n in nnf
            ]
    ...
    def estimate_nnf(self, source_guide, target_guide, source_style):
        with cp.cuda.Device(self.gpu_id):
            ...
            for level in range(self.pyramid_level):
                nnf = (
                    self.initialize_nnf(source_guide.shape[0])
                    if level == 0
                    else self.update_nnf(nnf, level)
                )
