# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\ImageQualityMetric\clip.py
    ...
    def score(
        ...
        if isinstance(images, (str, Image.Image)):
            ...
            if isinstance(images, str):
                image = (
                    self.preprocess_val(Image.open(images))
                    .unsqueeze(0)
                    .to(device=self.device, non_blocking=True)
                )
            else:
                image = (
                    self.preprocess_val(images)
                    .unsqueeze(0)
                    .to(device=self.device, non_blocking=True)
                )
            ...
            for one_images in images:
                if isinstance(one_images, str):
                    image = (
                        self.preprocess_val(Image.open(one_images))
                        .unsqueeze(0)
                        .to(device=self.device, non_blocking=True)
                    )
                elif isinstance(one_images, Image.Image):
                    image = (
                        self.preprocess_val(one_images)
                        .unsqueeze(0)
                        .to(device=self.device, non_blocking=True)
                    )
                else:
