# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\ImageQualityMetric\pickscore.py
import os
...
class PickScore(torch.nn.Module):
    def __init__(self, device: Union[str, torch.device], path: str = MODEL_PATHS):
        ...
        self.model = (
            AutoModel.from_pretrained(model_pretrained_name_or_path)
            .eval()
            .to(self.device)
        )

    def _calculate_score(
        self, image: torch.Tensor, prompt: str, softmax: bool = False
    ) -> float:
    ...
    def score(
        ...
        try:
            if isinstance(images, (str, Image.Image)):
                ...
                return [
                    self._calculate_score(image_inputs["pixel_values"], prompt, softmax)
                ]
                ...
                for one_image in images:
                    ...
                    scores.append(
                        self._calculate_score(
                            image_inputs["pixel_values"], prompt, softmax
                        )
                    )
                return scores
            else:
