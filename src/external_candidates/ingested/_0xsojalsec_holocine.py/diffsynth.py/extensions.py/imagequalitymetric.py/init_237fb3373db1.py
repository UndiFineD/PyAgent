# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\ImageQualityMetric\__init__.py
import os
...
def download_preference_model(model_name: preference_model_id, cache_dir="models"):
    ...
    load_path = {
        key: os.path.join(cache_dir, metadata["model_id"], path)
        for key, path in load_path.items()
    }
    return load_path
