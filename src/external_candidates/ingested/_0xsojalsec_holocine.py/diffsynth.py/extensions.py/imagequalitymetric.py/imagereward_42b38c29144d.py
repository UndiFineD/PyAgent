# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\ImageQualityMetric\imagereward.py
import os
...
class ImageReward(torch.nn.Module):
    ...
    def inference_rank(
        self, prompt: str, generations_list: List[Union[str, Image.Image]]
    ) -> tuple:
...
class ImageRewardScore(torch.nn.Module):
    def __init__(self, device: Union[str, torch.device], path: str = MODEL_PATHS):
        ...
        self.model = ImageReward(
            device=self.device,
            med_config=med_config,
            bert_model_path=path.get("bert_model_path"),
        ).to(self.device)
