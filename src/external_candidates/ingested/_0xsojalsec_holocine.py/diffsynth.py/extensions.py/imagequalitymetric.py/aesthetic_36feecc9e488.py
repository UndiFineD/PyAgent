# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\ImageQualityMetric\aesthetic.py
import os
...
class AestheticScore(torch.nn.Module):
    def __init__(self, device: torch.device, path: str = MODEL_PATHS):
        ...
        try:
            ...
            raise ValueError(
                f"Error loading model weights from {self.aes_model_path}: {e}"
            )
