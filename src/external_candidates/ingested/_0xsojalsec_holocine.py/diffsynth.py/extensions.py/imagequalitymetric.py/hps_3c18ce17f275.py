...
class HPScore_v2(torch.nn.Module):
    def __init__(
        self, device: torch.device, path: str = MODEL_PATHS, model_version: str = "v2"
    ):
        super().__init__()
        ...
        if model_version == "v2":
            ...
            raise ValueError(
                f"Unsupported model version: {model_version}. Choose 'v2' or 'v21'."
            )
        ...
        try:
            ...
            raise ValueError(
                f"Error loading model weights from {safetensors_path}: {e}"
            )
