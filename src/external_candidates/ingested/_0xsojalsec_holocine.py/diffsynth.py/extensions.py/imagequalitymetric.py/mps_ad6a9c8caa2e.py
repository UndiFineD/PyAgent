# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\ImageQualityMetric\mps.py
import gc
import json
...
class MPScore(torch.nn.Module):
    ...
    def _calculate_score(self, image: torch.Tensor, prompt: str) -> float:
        ...
        if self.condition == "overall":
            ...
            condition_prompt = (
                "shape, face, hair, hands, limbs, structure, instance, texture"
            )
        ...
        condition_batch = (
            _tokenize(condition_prompt).repeat(text_input.shape[0], 1).to(self.device)
        )
        ...
        with torch.no_grad():
            ...
            image_features = self.model.cross_model(image_f, text_f, mask.half())[
                :, 0, :
            ]
    ...
    @torch.no_grad()
    def score(
        self, images: Union[str, List[str], Image.Image, List[Image.Image]], prompt: str
    ) -> List[float]:
        ...
        if isinstance(images, (str, Image.Image)):
            ...
            if isinstance(images, str):
                image = self.image_processor(Image.open(images), return_tensors="pt")[
                    "pixel_values"
                ].to(self.device)
            else:
            ...
            for one_images in images:
                if isinstance(one_images, str):
                    image = self.image_processor(
                        Image.open(one_images), return_tensors="pt"
                    )["pixel_values"].to(self.device)
                elif isinstance(one_images, Image.Image):
                    image = self.image_processor(one_images, return_tensors="pt")[
                        "pixel_values"
                    ].to(self.device)
                else:
