...
class RIFEInterpolater:
    ...
    def from_model_manager(model_manager):
        return RIFEInterpolater(
            model_manager.fetch_model("rife"), device=model_manager.device
        )
    ...
    def interpolate(
        ...
        for iter in range(num_iter):
            ...
            processed_images = self.add_interpolated_images(
                processed_images, output_tensor
            )
    ...
    def __call__(self, rendered_frames, scale=1.0, batch_size=4, num_iter=1, **kwargs):
        ...
        for iter in range(num_iter):
            ...
            output_tensor = self.process_tensors(
                input_tensor, scale=scale, batch_size=batch_size
            )
