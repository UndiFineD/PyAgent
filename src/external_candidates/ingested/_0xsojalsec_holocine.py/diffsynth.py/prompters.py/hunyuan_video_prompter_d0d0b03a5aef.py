# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\prompters\hunyuan_video_prompter.py
import os
...
from ..models.hunyuan_video_text_encoder import (
...
class HunyuanVideoPrompter(BasePrompter):
    ...
    def __init__(
        ...
        self.tokenizer_2 = LlamaTokenizerFast.from_pretrained(
            tokenizer_2_path, padding_side="right"
        )
    ...
    def encode_prompt_using_llm(
        ...
        last_hidden_state = self.text_encoder_2(
            input_ids, attention_mask, hidden_state_skip_layer
        )
    ...
    def encode_prompt_using_mllm(
        ...
        image_outputs = self.processor(images, return_tensors="pt")["pixel_values"].to(
            device
        )
        max_length += crop_start
        inputs = self.tokenizer_2(
        ...
        text_crop_start = (
            crop_start - 1 + self.prompt_template_video.get("image_emb_len", 576)
        )
    ...
    def encode_prompt(
        ...
        # CLIP
        pooled_prompt_emb = self.encode_prompt_using_clip(
            prompt, clip_sequence_length, device
        )
        ...
        if images is None:
            ...
            prompt_emb, attention_mask = self.encode_prompt_using_mllm(
                prompt_formated,
                images,
                llm_sequence_length,
                device,
