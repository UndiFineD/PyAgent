# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\prompters\kolors_prompter.py
import json
import os
import re
        ...
        if encode_special_tokens:
            ...
            for match in re.finditer(self.role_special_token_expression, s):
                if last_index < match.start():
                    t.extend(
                        self.sp_model.EncodeAsPieces(s[last_index : match.start()])
                    )
...
class ChatGLMTokenizer(PreTrainedTokenizer):
    ...
    def _tokenize(self, text, **kwargs):
        return self.tokenizer.tokenize(
            text, encode_special_tokens=self.encode_special_tokens
        )
    ...
    def build_single_message(self, role, metadata, message):
        ...
        role_tokens = [self.get_command(f"<|{role}|>")] + self.tokenizer.encode(
            f"{metadata}\n"
        )
...
class KolorsPrompter(BasePrompter):
    def __init__(self, tokenizer_path=None):
        if tokenizer_path is None:
            ...
            tokenizer_path = os.path.join(
                base_path, "tokenizer_configs/kolors/tokenizer"
            )
        super().__init__()
    ...
    def encode_prompt(
        self, prompt, clip_skip=1, clip_skip_2=2, positive=True, device="cuda"
    ):
        prompt = self.process_prompt(prompt, positive=positive)
        prompt_emb, pooled_prompt_emb = self.encode_prompt_using_ChatGLM(
            prompt, self.text_encoder, self.tokenizer, 256, clip_skip_2, device
        )
