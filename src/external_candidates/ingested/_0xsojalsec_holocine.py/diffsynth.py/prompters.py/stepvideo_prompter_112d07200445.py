# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\prompters\stepvideo_prompter.py
import os
...
class StepVideoPrompter(BasePrompter):
    ...
    def fetch_models(
        self,
        text_encoder_1: HunyuanDiTCLIPTextEncoder = None,
        text_encoder_2: STEP1TextEncoder = None,
    ):
    ...
    def encode_prompt(self, prompt, positive=True, device="cuda"):
        ...
        clip_embeds = self.encode_prompt_using_clip(
            prompt, max_length=77, device=device
        )
        llm_embeds, llm_mask = self.encode_prompt_using_llm(
            prompt, max_length=320, device=device
        )
