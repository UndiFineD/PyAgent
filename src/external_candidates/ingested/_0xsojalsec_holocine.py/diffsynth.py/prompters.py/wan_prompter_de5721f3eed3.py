# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\prompters\wan_prompter.py
import html
import os
import string
    ...
    def encode_prompt(self, prompt, positive=True, device="cuda"):
        ...
        if per_shot_match:
            ...
            for shot_cut_match in shot_cut_matches:
                ...
                char_spans.append(
                    {"id": shot_id, "start": current_start_pos, "end": end_char}
                )
        ...
        if len(global_indices) > 0:
            positions["global"] = [
                global_indices.min().item(),
                global_indices.max().item() + 1,
            ]
        ...
        for i in range(max_shot_id + 1):
            ...
            if len(shot_indices) > 0:
                positions["shots"].append(
                    [shot_indices.min().item(), shot_indices.max().item() + 1]
                )
        ...
        if self.text_encoder is None:
            raise ValueError(
                "Text encoder has not been fetched. Call fetch_models() first."
            )
    ...
    def encode_prompt_separately(self, prompt, positive=True, device="cuda"):
        ...
        if global_match is None:
            output = self.tokenizer(
                cleaned_prompt, return_mask=True, add_special_tokens=True
            )
        ...
        for part in prompt_parts:
            ...
            enc_output = self.tokenizer(
                text, return_mask=True, add_special_tokens=True, return_tensors="pt"
            )
        ...
        if not embeddings_list:
            ...
            return torch.zeros(
                1, self.text_len, self.text_encoder.config.hidden_size, device=device
            ), {"global": None, "shots": []}
        ...
        if total_len > self.text_len:
            ...
            print(
                f"Warning: Concatenated prompt length ({total_len}) exceeds max length ({self.text_len}). Truncating."
            )
