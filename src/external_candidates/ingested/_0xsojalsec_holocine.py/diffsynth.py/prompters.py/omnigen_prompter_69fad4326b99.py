# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\prompters\omnigen_prompter.py
import os
import re
        ...
        for i in range(len(instructions)):
            ...
            if use_img_cfg:
                if cur_input_images is not None and len(cur_input_images) >= 1:
                    img_cfg_prompt = [
                        f"<img><|image_{i+1}|></img>"
                        for i in range(len(cur_input_images))
                    ]
                    img_cfg_mllm_input = self.process_multi_modal_prompt(
                        " ".join(img_cfg_prompt), cur_input_images
                    )
                else:
            ...
            if use_input_image_size_as_output:
                input_data.append(
                    (
                        ...
                        img_cfg_mllm_input,
                        [
                            mllm_input["pixel_values"][0].size(-2),
                            mllm_input["pixel_values"][0].size(-1),
                        ],
                    )
                )
            else:
                input_data.append(
                    (mllm_input, neg_mllm_input, img_cfg_mllm_input, [height, width])
                )
...
class OmniGenCollator:
    ...
    def create_position(self, attention_mask, num_tokens_for_output_images):
        ...
        for mask in attention_mask:
            ...
            temp_position = [0] * (text_length - temp_l) + [
                i for i in range(temp_l + img_length + 1)
    ...
    def create_mask(self, attention_mask, num_tokens_for_output_images):
        ...
        for mask in attention_mask:
            ...
            if pad_img_length > 0:
                ...
                temp_padding_imgs = torch.zeros(
                    size=(1, pad_img_length, self.hidden_size)
                )
            else:
    ...
    def process_mllm_input(self, mllm_inputs, target_img_size):
        ...
        padded_input_ids, attention_mask, image_sizes = self.pad_input_ids(
            input_ids, image_sizes
        )
        position_ids = self.create_position(
            attention_mask, num_tokens_for_output_images
        )
        attention_mask, padding_images = self.create_mask(
            attention_mask, num_tokens_for_output_images
        )
