...
class SDPrompter(BasePrompter):
    def __init__(self, tokenizer_path=None):
        if tokenizer_path is None:
            ...
            tokenizer_path = os.path.join(
                base_path, "tokenizer_configs/stable_diffusion/tokenizer"
            )
        super().__init__()
    ...
    def add_textual_inversions_to_model(self, textual_inversion_dict, text_encoder):
        ...
        for keyword in textual_inversion_dict:
            ...
            token_embeddings.append(
                embeddings.to(dtype=dtype, device=token_embeddings[0].device)
            )
        token_embeddings = torch.concat(token_embeddings, dim=0)
        state_dict["weight"] = token_embeddings
        text_encoder.token_embedding = torch.nn.Embedding(
            token_embeddings.shape[0], token_embeddings.shape[1]
        )
        text_encoder.token_embedding = text_encoder.token_embedding.to(
            dtype=dtype, device=token_embeddings[0].device
        )
    ...
    def load_textual_inversions(self, model_paths):
        ...
        self.add_textual_inversions_to_model(
            self.textual_inversion_dict, self.text_encoder
        )
        self.add_textual_inversions_to_tokenizer(
            self.textual_inversion_dict, self.tokenizer
        )
