# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\lora\flux_lora.py
import math
                ...
                for i in names:
                    if i.isdigit():
                        return i, name.replace(
                            f"transformer_blocks.{i}.", "transformer_blocks.blockid."
                        )
            return None, None
            ...
            for name in list(state_dict_.keys()):
                if "single_blocks." in name and ".a_to_q." in name:
                    ...
                    if mlp is None:
                        ...
                        state_dict_.pop(
                            name.replace(".a_to_q.", ".proj_in_besides_attn.")
                        )
                    if "lora_A" in name:
                        param = torch.concat(
                            [
                        ...
                        param[d : 2 * d, r : 2 * r] = state_dict_.pop(
                            name.replace(".a_to_q.", ".a_to_k.")
                        )
                        param[2 * d : 3 * d, 2 * r : 3 * r] = state_dict_.pop(
                            name.replace(".a_to_q.", ".a_to_v.")
                        )
                        param[3 * d :, 3 * r :] = mlp
                    else:
                        param = torch.concat(
            ...
            for name in list(state_dict_.keys()):
                for component in ["a", "b"]:
                    if f".{component}_to_q." in name:
                        name_ = name.replace(
                            f".{component}_to_q.", f".{component}_to_qkv."
                        )
                        concat_dim = 0
                        if "lora_A" in name:
                            param = torch.concat(
                            ...
                            # print(d, r)
                            param = torch.zeros(
                                (3 * d, 3 * r), dtype=origin.dtype, device=origin.device
                            )
                            param[:d, :r] = state_dict_[
                                name.replace(
                                    f".{component}_to_q.", f".{component}_to_q."
                                )
                            ]
        ...
        for i in range(19):
            for suffix in lora_dict:
                lora_patterns.append(
                    {"name": f"blocks.{i}.{suffix}", "dim": lora_dict[suffix]}
                )
        lora_dict = {"to_qkv_mlp": 21504, "proj_out": 3072, "norm.linear": 9216}
        for i in range(38):
            for suffix in lora_dict:
                lora_patterns.append(
                    {"name": f"single_blocks.{i}.{suffix}", "dim": lora_dict[suffix]}
                )
