# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\hunyuan_dit.py
import math
...
class HunyuanDiTRotaryEmbedding(torch.nn.Module):
    ...
    def __init__(self, q_norm_shape=88, k_norm_shape=88, rotary_emb_on_k=True):
        super().__init__()
        self.q_norm = torch.nn.LayerNorm(
            (q_norm_shape,), elementwise_affine=True, eps=1e-06
        )
        self.k_norm = torch.nn.LayerNorm(
            (k_norm_shape,), elementwise_affine=True, eps=1e-06
        )
...
class HunyuanDiTFinalLayer(torch.nn.Module):
    def __init__(
        self, final_hidden_size=1408, condition_dim=1408, patch_size=2, out_channels=8
    ):
        super().__init__()
...
class HunyuanDiTBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.rota1 = HunyuanDiTRotaryEmbedding(
        ...
        self.rota2 = HunyuanDiTRotaryEmbedding(
            hidden_dim // num_heads, hidden_dim // num_heads, rotary_emb_on_k=False
        )
        ...
        self.modulation = torch.nn.Sequential(
            FP32_SiLU(), torch.nn.Linear(condition_dim, hidden_dim, bias=True)
        )
        ...
        if skip_connection:
            self.skip_norm = FP32_Layernorm(
                (hidden_dim * 2,), eps=1e-6, elementwise_affine=True
            )
    ...
    def forward(
        ...
        hidden_states = hidden_states + self.attn1(
            attn_input,
            qkv_preprocessor=lambda q, k, v: self.rota1(
                q, k, v, freq_cis_img, to_cache=to_cache
            ),
        )
        ...
        hidden_states = hidden_states + self.attn2(
            attn_input,
            text_emb,
            qkv_preprocessor=lambda q, k, v: self.rota2(q, k, v, freq_cis_img),
        )
...
class HunyuanDiT(torch.nn.Module):
    def __init__(
        ...
        # Embedders
        self.text_emb_padding = torch.nn.Parameter(
            torch.randn(text_length + t5_length, text_dim, dtype=torch.float32)
        )
    ...
    def prepare_text_emb(self, text_emb, text_emb_t5, text_emb_mask, text_emb_mask_t5):
        ...
        text_emb = torch.where(
            text_emb_mask.unsqueeze(2), text_emb, self.text_emb_padding.to(text_emb)
        )
        return text_emb
    ...
    def tiled_block_forward(
        ...
        for hl, hr, wl, wr in tasks:
            hidden_states_batch = hidden_states[:, :, hl:hr, wl:wr].to(
                computation_device
            )
            ...
            hidden_states_batch = rearrange(
                hidden_states_batch, "B (H W) C -> B C H W", H=hr - hl
            )
    ...
    def forward(
        ...
        # Embeddings
        text_emb = self.prepare_text_emb(
            text_emb, text_emb_t5, text_emb_mask, text_emb_mask_t5
        )
        condition_emb = self.prepare_extra_emb(
            text_emb_t5, timestep, size_emb, hidden_states.dtype, hidden_states.shape[0]
        )
        ...
        if tiled:
            hidden_states = rearrange(
                hidden_states, "B (H W) C -> B C H W", H=height // 2
            )
            residuals = []
            for block_id, block in enumerate(self.blocks):
                ...
                hidden_states = self.tiled_block_forward(
                    ...
                    computation_device=hidden_states.device,
                    tile_size=tile_size,
