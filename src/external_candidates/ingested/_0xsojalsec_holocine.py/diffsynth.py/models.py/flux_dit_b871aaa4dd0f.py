# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\flux_dit.py
import torch
from einops import rearrange

from .sd3_dit import AdaLayerNorm, RMSNorm, TimestepEmbeddings
...
class FluxJointAttention(torch.nn.Module):
    ...
    def forward(
        ...
        qkv_a = qkv_a.view(batch_size, -1, 3 * self.num_heads, self.head_dim).transpose(
            1, 2
        )
        ...
        qkv_b = qkv_b.view(batch_size, -1, 3 * self.num_heads, self.head_dim).transpose(
            1, 2
        )
        ...
        if ipadapter_kwargs_list is not None:
            hidden_states_a = interact_with_ipadapter(
                hidden_states_a, q_a, **ipadapter_kwargs_list
            )
...
class FluxJointTransformerBlock(torch.nn.Module):
    def __init__(self, dim, num_attention_heads):
        ...
        self.attn = FluxJointAttention(
            dim, dim, num_attention_heads, dim // num_attention_heads
        )
    ...
    def forward(
        ...
        norm_hidden_states_a, gate_msa_a, shift_mlp_a, scale_mlp_a, gate_mlp_a = (
            self.norm1_a(hidden_states_a, emb=temb)
        )
        norm_hidden_states_b, gate_msa_b, shift_mlp_b, scale_mlp_b, gate_mlp_b = (
            self.norm1_b(hidden_states_b, emb=temb)
        )
        ...
        norm_hidden_states_a = (
            self.norm2_a(hidden_states_a) * (1 + scale_mlp_a) + shift_mlp_a
        )
        ...
        norm_hidden_states_b = (
            self.norm2_b(hidden_states_b) * (1 + scale_mlp_b) + shift_mlp_b
        )
...
class FluxSingleTransformerBlock(torch.nn.Module):
    ...
    def process_attention(
        ...
        hidden_states = hidden_states.to(q.dtype)
        if ipadapter_kwargs_list is not None:
            hidden_states = interact_with_ipadapter(
                hidden_states, q, **ipadapter_kwargs_list
            )
        return hidden_states
    ...
    def forward(
        ...
        attn_output, mlp_hidden_states = (
            hidden_states_a[:, :, : self.dim * 3],
            hidden_states_a[:, :, self.dim * 3 :],
        )

        attn_output = self.process_attention(
            attn_output, image_rotary_emb, attn_mask, ipadapter_kwargs_list
        )
        mlp_hidden_states = torch.nn.functional.gelu(
            mlp_hidden_states, approximate="tanh"
        )
...
class FluxDiT(torch.nn.Module):
    def __init__(self, disable_guidance_embedder=False, input_dim=64, num_blocks=19):
        ...
        self.guidance_embedder = (
            None if disable_guidance_embedder else TimestepEmbeddings(256, 3072)
        )
        ...
        self.blocks = torch.nn.ModuleList(
            [FluxJointTransformerBlock(3072, 24) for _ in range(num_blocks)]
        )
        self.single_blocks = torch.nn.ModuleList(
            [FluxSingleTransformerBlock(3072, 24) for _ in range(38)]
        )
    ...
    def patchify(self, hidden_states):
        hidden_states = rearrange(
            hidden_states, "B C (H P) (W Q) -> B (H W) (C P Q)", P=2, Q=2
        )
        return hidden_states
    ...
    def prepare_image_ids(self, latents):
        ...
        latent_image_ids = latent_image_ids.to(
    ...
    def construct_mask(self, entity_masks, prompt_seq_len, image_seq_len):
        ...
        attention_mask = torch.ones(
            (batch_size, total_seq_len, total_seq_len), dtype=torch.bool
        ...
        for i in range(N):
            ...
            attention_mask[:, prompt_start:prompt_end, image_start:image_end] = (
                image_mask
            )
            # image update with prompt
            attention_mask[:, image_start:image_end, prompt_start:prompt_end] = (
                image_mask.transpose(1, 2)
            )
        # prompt-prompt mask
        for i in range(N):
            for j in range(N):
                if i != j:
                    ...
                    attention_mask[
                        :, prompt_start_i:prompt_end_i, prompt_start_j:prompt_end_j
                    ] = False
    ...
    def process_entity_masks(
        ...
        if entity_masks is not None:
            ...
            entity_masks = [
                entity_masks[:, i, None].squeeze(1) for i in range(max_masks)
            ]
            # global mask
            global_mask = torch.ones_like(entity_masks[0]).to(
                device=hidden_states.device, dtype=hidden_states.dtype
            )
            entity_masks = entity_masks + [global_mask]  # append global to last
            # attention mask
            attention_mask = self.construct_mask(
                entity_masks, prompt_emb.shape[1], hidden_states.shape[1]
            )
            ...
            local_embs = [
                entity_prompt_emb[:, i, None].squeeze(1) for i in range(max_masks)
            ]
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.guidance_embedder is not None:
            ...
            conditioning = conditioning + self.guidance_embedder(
        ...
        if entity_prompt_emb is not None and entity_masks is not None:
            ...
            image_rotary_emb = self.pos_embedder(
        ...
        class quantized_layer:
            ...
            class RMSNorm(torch.nn.Module):
                ...
                def forward(self, hidden_states, **kwargs):
                    ...
                    variance = (
                        hidden_states.to(torch.float32).square().mean(-1, keepdim=True)
                    )
                    hidden_states = hidden_states * torch.rsqrt(
                        variance + self.module.eps
                    )
    ...
    def from_diffusers(self, state_dict):
            ...
            if name.endswith(".weight") or name.endswith(".bias"):
                ...
                if prefix in global_rename_dict:
                    ...
                    if middle in rename_dict:
                        name_ = ".".join(
                            names[:2] + [rename_dict[middle]] + [suffix[1:]]
                        )
                    ...
                    if middle in rename_dict_single:
                        name_ = ".".join(
                            names[:2] + [rename_dict_single[middle]] + [suffix[1:]]
                        )
                        state_dict_[name_] = param
                    else:
                        pass
        ...
        for name in list(state_dict_.keys()):
            if "single_blocks." in name and ".a_to_q." in name:
                mlp = state_dict_.get(
                    name.replace(".a_to_q.", ".proj_in_besides_attn."), None
                )
                if mlp is None:
                    mlp = torch.zeros(
                ...
                if f".{component}_to_q." in name:
                    ...
                    param = torch.concat(
                        [
                            state_dict_[
                                name.replace(
                                    f".{component}_to_q.", f".{component}_to_q."
                                )
                            ],
                            state_dict_[
                                name.replace(
                                    f".{component}_to_q.", f".{component}_to_k."
                                )
                            ],
                            state_dict_[
                                name.replace(
                                    f".{component}_to_q.", f".{component}_to_v."
                                )
                            ],
                        ],
                        dim=0,
                    ...
                    state_dict_[name_] = param
                    state_dict_.pop(
                        name.replace(f".{component}_to_q.", f".{component}_to_q.")
                    )
                    state_dict_.pop(
        ...
        for name, param in state_dict.items():
            ...
            if name in rename_dict:
                ...
                if ".".join(names[2:]) in suffix_rename_dict:
                    rename = (
                        f"single_blocks.{names[1]}."
                        + suffix_rename_dict[".".join(names[2:])]
                    )
                    state_dict_[rename] = param
            else:
                pass
