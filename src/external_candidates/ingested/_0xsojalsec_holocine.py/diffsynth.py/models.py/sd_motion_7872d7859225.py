# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\sd_motion.py
import torch
from einops import rearrange, repeat

from .sd_unet import GEGLU, Attention, SDUNet


class TemporalTransformerBlock(torch.nn.Module):

    def __init__(
        self, dim, num_attention_heads, attention_head_dim, max_position_embeddings=32
    ):
        super().__init__()
    ...
    def forward(self, hidden_states, batch_size=1):
        ...
        norm_hidden_states = rearrange(
            norm_hidden_states, "(b f) h c -> (b h) f c", b=batch_size
        )
        attn_output = self.attn1(
            norm_hidden_states + self.pe1[:, : norm_hidden_states.shape[1]]
        )
        ...
        attn_output = self.attn2(
            norm_hidden_states + self.pe2[:, : norm_hidden_states.shape[1]]
        )
        attn_output = rearrange(attn_output, "(b h) f c -> (b f) h c", b=batch_size)
        hidden_states = attn_output + hidden_states
...
class TemporalBlock(torch.nn.Module):
    ...
    def forward(self, hidden_states, time_emb, text_emb, res_stack, batch_size=1):
        ...
        for block in self.transformer_blocks:
            hidden_states = block(hidden_states, batch_size=batch_size)
    ...
    def from_diffusers(self, state_dict):
        ...
        for name in name_list:
            ...
            if "pos_encoder" in names:
                rename = ".".join(
                    ["motion_modules", str(module_id), rename_dict[middle_name]]
                )
            else:
                rename = ".".join(
                    ["motion_modules", str(module_id), rename_dict[middle_name], suffix]
                )
