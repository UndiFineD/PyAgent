# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\hunyuan_dit_text_encoder.py
import torch
from transformers import BertConfig, BertModel, T5Config, T5EncoderModel
...
class HunyuanDiTCLIPTextEncoder(BertModel):
    ...
    def forward(self, input_ids, attention_mask, clip_skip=1):
        ...
        if clip_skip > 1:
            ...
            prompt_emb = (
                prompt_emb - prompt_emb.mean()
            ) / prompt_emb.std() * std + mean
        return prompt_emb
...
class HunyuanDiTT5TextEncoder(T5EncoderModel):
    ...
    def forward(self, input_ids, attention_mask, clip_skip=1):
        ...
        if clip_skip > 1:
            mean, std = (
                outputs.hidden_states[-1].mean(),
                outputs.hidden_states[-1].std(),
            )
            prompt_emb = (
                prompt_emb - prompt_emb.mean()
            ) / prompt_emb.std() * std + mean
