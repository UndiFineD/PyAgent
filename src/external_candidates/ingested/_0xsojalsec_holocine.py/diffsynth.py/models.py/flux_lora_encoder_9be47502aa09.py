# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\flux_lora_encoder.py
import torch
...
class LoRAEmbedder(torch.nn.Module):
    def __init__(self, lora_patterns=None, L=1, out_dim=2048):
        ...
        for lora_pattern in lora_patterns:
            ...
            if layer_type not in proj_dict:
                proj_dict[layer_type.replace(".", "___")] = torch.nn.Linear(
                    dim[1], out_dim
                )
    ...
    def default_lora_patterns(self):
        ...
        lora_dict = {
            "to_qkv_mlp": (3072, 21504),
            "proj_out": (15360, 3072),
            "norm.linear": (3072, 9216),
        }
        for i in range(38):
            for suffix in lora_dict:
                lora_patterns.append(
...
class FluxLoRAEncoder(torch.nn.Module):
    def __init__(
        ...
        self.encoders = torch.nn.ModuleList(
            [
                CLIPEncoderLayer(
                    embed_dim, encoder_intermediate_size, num_heads=32, head_dim=128
                )
        ...
        # special embedding
        self.special_embeds = torch.nn.Parameter(
            torch.randn(1, num_special_embeds, embed_dim)
        )
    ...
    def forward(self, lora):
        ...
        special_embeds = self.special_embeds.to(
            dtype=lora_embeds.dtype, device=lora_embeds.device
        )
