# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\omnigen.py
...
class OmniGenOriginalModel(nn.Module):
        ...
        if input_ids is not None:
            ...
            for b_inx in input_image_sizes.keys():
                for start_inx, end_inx in input_image_sizes[b_inx]:
                    condition_embeds[b_inx, start_inx:end_inx] = input_latents[
                        input_img_inx
                    ]
                    input_img_inx += 1
            if input_img_latents is not None:
    ...
    def forward_with_cfg(
        ...
        if use_img_cfg:
            ...
            cond = (
                uncond
                + img_cfg_scale * (img_cond - uncond)
                + cfg_scale * (cond - img_cond)
            )
