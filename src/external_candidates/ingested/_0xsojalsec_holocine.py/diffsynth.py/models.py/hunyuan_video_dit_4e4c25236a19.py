...
class IndividualTokenRefinerBlock(torch.nn.Module):
    def __init__(self, hidden_size=3072, num_heads=24):
        ...
        self.adaLN_modulation = torch.nn.Sequential(
            torch.nn.SiLU(),
            torch.nn.Linear(
                hidden_size, hidden_size * 2, device="cuda", dtype=torch.bfloat16
            ),
        )
...
class SingleTokenRefiner(torch.nn.Module):
    def __init__(self, in_channels=4096, hidden_size=3072, depth=2):
        ...
        self.blocks = torch.nn.ModuleList(
            [IndividualTokenRefinerBlock(hidden_size=hidden_size) for _ in range(depth)]
        )
    ...
    def forward(self, x, t, mask=None):
        ...
        context_aware_representations = (x * mask_float).sum(dim=1) / mask_float.sum(
            dim=1
        )
...
class MMDoubleStreamBlockComponent(torch.nn.Module):
    ...
    def forward(
        ...
        mod1_shift, mod1_scale, mod1_gate, mod2_shift, mod2_scale, mod2_gate = self.mod(
            conditioning
        ).chunk(6, dim=-1)
    ...
    def process_ff(self, hidden_states, attn_output, mod, mod_tr=None, tr_token=None):
        ...
        hidden_states = hidden_states + apply_gate(
            self.to_out(attn_output), mod1_gate, tr_mod1_gate, tr_token
        )
        x = self.ff(
            modulate(
...
class MMDoubleStreamBlock(torch.nn.Module):
    def __init__(self, hidden_size=3072, heads_num=24, mlp_width_ratio=4):
        super().__init__()
        self.component_a = MMDoubleStreamBlockComponent(
            hidden_size, heads_num, mlp_width_ratio
        )
        self.component_b = MMDoubleStreamBlockComponent(
            hidden_size, heads_num, mlp_width_ratio
        )
    ...
    def forward(
        ...
        (q_b, k_b, v_b), mod_b, _ = self.component_b(
            hidden_states_b, conditioning, freqs_cis=None
        )
        ...
        attn_output_a, attn_output_b = attn_output_a[
            :, :-split_token
        ].contiguous(), torch.concat(
            [attn_output_a[:, -split_token:], attn_output_b], dim=1
        )

        hidden_states_a = self.component_a.process_ff(
            hidden_states_a, attn_output_a, mod_a, mod_tr, tr_token
        )
        hidden_states_b = self.component_b.process_ff(
            hidden_states_b, attn_output_b, mod_b
        )
...
class MMSingleStreamBlockOriginal(torch.nn.Module):
    ...
    def forward(self, x, vec, freqs_cis=None, txt_len=256):
        ...
        attn_output_a = attention(
            q[:, :-185].contiguous(), k[:, :-185].contiguous(), v[:, :-185].contiguous()
        )
        attn_output_b = attention(
            q[:, -185:].contiguous(), k[:, -185:].contiguous(), v[:, -185:].contiguous()
        )
...
class MMSingleStreamBlock(torch.nn.Module):
    ...
    def forward(
        ...
        if token_replace_vec is not None:
            assert tr_token is not None
            tr_mod_shift, tr_mod_scale, tr_mod_gate = self.mod(token_replace_vec).chunk(
                3, dim=-1
            )
        else:
        ...
        hidden_states = hidden_states + apply_gate(
            self.to_out(attn_output), mod_gate, tr_mod_gate, tr_token
        )
        hidden_states = hidden_states + apply_gate(
            self.ff(norm_hidden_states), mod_gate, tr_mod_gate, tr_token
        )
        return hidden_states
...
class FinalLayer(torch.nn.Module):
    def __init__(self, hidden_size=3072, patch_size=(1, 2, 2), out_channels=16):
        ...
        self.linear = torch.nn.Linear(
            hidden_size, patch_size[0] * patch_size[1] * patch_size[2] * out_channels
        )

        self.adaLN_modulation = torch.nn.Sequential(
            torch.nn.SiLU(), torch.nn.Linear(hidden_size, 2 * hidden_size)
        )
...
class HunyuanVideoDiT(torch.nn.Module):
    def __init__(
        ...
        self.guidance_in = (
            TimestepEmbeddings(256, hidden_size, computation_device="cpu")
            if guidance_embed
            else None
        )
        self.double_blocks = torch.nn.ModuleList(
            [MMDoubleStreamBlock(hidden_size) for _ in range(num_double_blocks)]
        )
        self.single_blocks = torch.nn.ModuleList(
            [MMSingleStreamBlock(hidden_size) for _ in range(num_single_blocks)]
        )
    ...
    def enable_auto_offload(self, dtype=torch.bfloat16, device="cuda"):
        ...
        class quantized_layer:
            class Linear(torch.nn.Linear):
                def __init__(
                    self, *args, dtype=torch.bfloat16, device="cuda", **kwargs
                ):
            ...
            if direct_name in direct_dict:
                ...
                if isinstance(single_suffix_dict[suffix], list):
                    if suffix == "linear1":
                        ...
                        param_a, param_b = torch.split(
                            param, (3072 * 3, 3072 * 4), dim=0
                        )
                        ...
                        if names[-1] == "weight":
                            name_a, name_b = single_suffix_dict[suffix]
                            param_a, param_b = torch.split(
                                param, (3072 * 1, 3072 * 4), dim=-1
                            )
                            state_dict_[prefix + "." + name_a + "." + names[-1]] = (
                                param_a
                            )
                            state_dict_[prefix + "." + name_b + "." + names[-1]] = (
                                param_b
                            )
                        else:
                            name_a, name_b = single_suffix_dict[suffix]
