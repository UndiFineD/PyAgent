# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\cog_dit.py
import numpy as np
import torch
...
class CogAdaLayerNorm(torch.nn.Module):
    ...
    def forward(self, hidden_states, prompt_emb, emb):
        ...
        if self.single:
            ...
            shift_a, scale_a, gate_a, shift_b, scale_b, gate_b = emb.unsqueeze(1).chunk(
                6, dim=2
            )
...
class CogDiTBlock(torch.nn.Module):
    def __init__(self, dim, dim_cond, num_heads):
        ...
        self.norm_q = torch.nn.LayerNorm(
            (dim // num_heads,), eps=1e-06, elementwise_affine=True
        )
        self.norm_k = torch.nn.LayerNorm(
            (dim // num_heads,), eps=1e-06, elementwise_affine=True
        )
    ...
    def process_qkv(self, q, k, v, image_rotary_emb, text_seq_length):
        q = self.norm_q(q)
        k = self.norm_k(k)
        q[:, :, text_seq_length:] = self.apply_rotary_emb(
            q[:, :, text_seq_length:], image_rotary_emb
        )
        k[:, :, text_seq_length:] = self.apply_rotary_emb(
            k[:, :, text_seq_length:], image_rotary_emb
        )
        return q, k, v
    ...
    def forward(self, hidden_states, prompt_emb, time_emb, image_rotary_emb):
        ...
        attention_io = torch.cat(
            [norm_encoder_hidden_states, norm_hidden_states], dim=1
        )
        attention_io = self.attn1(
            attention_io,
            qkv_preprocessor=lambda q, k, v: self.process_qkv(
                q, k, v, image_rotary_emb, prompt_emb.shape[1]
            ),
        )
...
class CogDiT(torch.nn.Module):
    def __init__(self):
        ...
        self.blocks = torch.nn.ModuleList(
            [CogDiTBlock(3072, 512, 48) for _ in range(42)]
        )
        self.norm_final = torch.nn.LayerNorm(
            (3072,), eps=1e-05, elementwise_affine=True
        )
    ...
    def tiled_forward(
        ...
        value = torch.zeros(
            (B, C, T, H, W), dtype=hidden_states.dtype, device=hidden_states.device
        )
        weight = torch.zeros(
            (B, C, T, H, W), dtype=hidden_states.dtype, device=hidden_states.device
        )
        ...
        for h in range(0, H, tile_stride):
            for w in range(0, W, tile_stride):
                ...
                if h_ > H:
                    h, h_ = max(H - tile_size, 0), H
                if w_ > W:
                    w, w_ = max(W - tile_size, 0), W
        ...
        for hl, hr, wl, wr in tasks:
            ...
            model_output = self.forward(
                hidden_states[:, :, :, hl:hr, wl:wr], timestep, prompt_emb
            )
    ...
    def forward(
        ...
        if tiled:
            return TileWorker2Dto3D().tiled_forward(
                ...
                computation_device=self.context_embedder.weight.device,
                computation_dtype=self.context_embedder.weight.dtype,
            )
        num_frames, height, width = hidden_states.shape[-3:]
        if image_rotary_emb is None:
            image_rotary_emb = self.prepare_rotary_positional_embeddings(
                height, width, num_frames, device=self.context_embedder.weight.device
            )
