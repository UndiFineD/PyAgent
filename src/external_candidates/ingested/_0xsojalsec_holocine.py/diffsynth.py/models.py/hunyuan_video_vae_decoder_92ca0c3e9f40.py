...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.group_norm = nn.GroupNorm(
            num_groups=num_groups, num_channels=in_channels, eps=eps, affine=True
        )
        ...
        self.to_out = nn.Sequential(
            nn.Linear(dim_inner, in_channels, bias=bias), nn.Dropout(dropout)
        )
    ...
    def forward(self, input_tensor, attn_mask=None):
        ...
        if attn_mask is not None:
            attn_mask = attn_mask.view(
                batch_size, self.num_heads, -1, attn_mask.shape[-1]
            )
...
class UNetMidBlockCausal3D(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            ...
            attn_mask = prepare_causal_attention_mask(
...
class DecoderCausal3D(nn.Module):
    ...
    def __init__(
        ...
        self.conv_norm_out = nn.GroupNorm(
            num_channels=block_out_channels[0], num_groups=num_groups, eps=eps
        )
...
class HunyuanVideoVAEDecoder(nn.Module):
    ...
    def tile_forward(self, hidden_states, tile_size, tile_stride):
        ...
        weight = torch.zeros(
            (1, 1, (T - 1) * 4 + 1, H * 8, W * 8), dtype=torch_dtype, device=data_device
        )
        values = torch.zeros(
            (B, 3, (T - 1) * 4 + 1, H * 8, W * 8), dtype=torch_dtype, device=data_device
        )
