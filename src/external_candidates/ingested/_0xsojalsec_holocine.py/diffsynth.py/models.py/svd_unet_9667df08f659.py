...
class TrainableTemporalTimesteps(torch.nn.Module):
    def __init__(
        ...
        embeddings = get_timestep_embedding(
            timesteps, num_channels, flip_sin_to_cos, downscale_freq_shift
        )
...
class PositionalID(torch.nn.Module):
    ...
    def forward(self, num_frames, pivot_frame_id=0):
        position_ids = [
            self.frame_id_to_position_id(abs(i - pivot_frame_id))
...
class TemporalAttentionBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        if add_positional_conv is not None:
            self.positional_embedding = TrainableTemporalTimesteps(
                in_channels, True, 0, add_positional_conv
            )
    ...
    def forward(self, hidden_states, time_emb, text_emb, res_stack, **kwargs):
        ...
        pos_emb = self.positional_embedding(pos_emb).to(
            dtype=hidden_states.dtype, device=hidden_states.device
        )
        pos_emb = self.positional_embedding_proj(pos_emb)

        hidden_states = rearrange(hidden_states, "T C H W -> 1 C T H W") + rearrange(
            pos_emb, "T C -> 1 C T 1 1"
        )
        ...
        attn_output = self.attn2(
            norm_hidden_states, encoder_hidden_states=text_emb.repeat(height * width, 1)
        )
        hidden_states = attn_output + hidden_states
        ...
        hidden_states = hidden_states.reshape(height, width, batch, inner_dim).permute(
            2, 3, 0, 1
        )
            ...
            for h in range(0, H, stride_height):
                for w in range(0, W, stride_width):
                    if (
                        ...
                        or (
                            h - stride_height >= 0
                            and h - stride_height + batch_height >= H
                        )
                        or (
                            w - stride_width >= 0
                            and w - stride_width + batch_width >= W
                        )
                    ):
                        continue
                    tasks.append(
                        (t, t + batch_time, h, h + batch_height, w, w + batch_width)
                    )

        # Run
        for tl, tr, hl, hr, wl, wr in progress_bar(tasks):
            sample_batch = sample[tl:tr, :, hl:hr, wl:wr].to(computation_device)
            sample_batch = self.forward(
                sample_batch, timestep, encoder_hidden_states, add_time_id
            ).to(data_device)
        ...
        **kwargs,
        ...
        for i, block in enumerate(self.blocks):
            if (
                ...
                and not (
                    isinstance(block, PushBlock)
                    or isinstance(block, PopBlock)
                    or isinstance(block, PopMixBlock)
                )
            ):
...
class SVDUNetStateDictConverter:
    ...
    def from_diffusers(self, state_dict):
        ...
        blocks_rename_dict = {
            i: j for j, i in enumerate(blocks_rename_dict) if i is not None
        }
        state_dict_ = {}
            ...
            if names[-1] in ["weight", "bias"]:
                ...
                if name_prefix in rename_dict:
                    ...
                    if "resnets" in block_name and block_name in blocks_rename_dict:
                        rename = ".".join(
                            ["blocks", str(blocks_rename_dict[block_name])] + names[5:]
                        )
                        ...
                        rename = ".".join(
                            ["blocks", str(blocks_rename_dict[block_name])] + names[-2:]
                        )
                        state_dict_[rename] = param
                    elif (
                        "attentions" in block_name and block_name in blocks_rename_dict
                    ):
                ...
                if len(block_name) > 0 and block_name in blocks_rename_dict:
                    rename = ".".join(
                        ["blocks", str(blocks_rename_dict[block_name]), names[-1]]
                    )
                    state_dict_[rename] = param
        return state_dict_
