# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\hunyuan_video_vae_encoder.py
import numpy as np
import torch
...
from .hunyuan_video_vae_decoder import (
    CausalConv3d,
    ResnetBlockCausal3D,
...
class DownsampleCausal3D(nn.Module):
    ...
    def __init__(self, channels, out_channels, kernel_size=3, bias=True, stride=2):
        ...
        self.conv = CausalConv3d(
            channels, out_channels, kernel_size, stride=stride, bias=bias
        )
...
class EncoderCausal3D(nn.Module):
    ...
    def __init__(
        ...
        self.conv_norm_out = nn.GroupNorm(
            num_channels=block_out_channels[-1], num_groups=num_groups, eps=eps
        )
        self.conv_act = nn.SiLU()
        self.conv_out = CausalConv3d(
            block_out_channels[-1], 2 * out_channels, kernel_size=3
        )
...
class HunyuanVideoVAEEncoder(nn.Module):
    ...
    def tile_forward(self, hidden_states, tile_size, tile_stride):
        ...
        for t in range(0, T, stride_t):
            if t - stride_t >= 0 and t - stride_t + size_t >= T:
                continue
    ...
    def encode_video(
        self, latents, tile_size=(65, 256, 256), tile_stride=(48, 192, 192)
    ):
