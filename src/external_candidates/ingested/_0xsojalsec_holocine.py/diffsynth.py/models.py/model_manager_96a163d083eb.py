# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\model_manager.py
import importlib
import json
import os
...
from .cog_vae import CogVAEDecoder, CogVAEEncoder
...
from .flux_vae import FluxVAEDecoder, FluxVAEEncoder
...
from .sd_ipadapter import IpAdapterCLIPImageEmbedder, SDIpAdapter
...
from .sdxl_ipadapter import IpAdapterXLCLIPImageEmbedder, SDXLIpAdapter
...
def load_model_from_single_file(
    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        if missing_keys:
            print(
                f"        The following parameters were not in the checkpoint and will be initialized: {sorted(list(missing_keys))}"
            )
            ...
            for key in missing_keys:
                ...
                print(
                    f"            Initializing '{key}' with zeros. Shape: {meta_param.shape}, Dtype: {meta_param.dtype}"
                )
                model_state_dict[key] = torch.zeros(
                    meta_param.shape, dtype=meta_param.dtype, device="cpu"
                )
