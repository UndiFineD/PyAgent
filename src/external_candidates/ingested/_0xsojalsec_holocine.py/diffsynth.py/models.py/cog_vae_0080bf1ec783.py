...
class Resnet3DBlock(torch.nn.Module):
    def __init__(
        ...
        self.conv1 = CachedConv3d(
            in_channels, out_channels, kernel_size=3, padding=(0, 1, 1)
        )

        self.conv2 = CachedConv3d(
            out_channels, out_channels, kernel_size=3, padding=(0, 1, 1)
        )

        if in_channels != out_channels:
            if use_conv_shortcut:
                self.conv_shortcut = CachedConv3d(
                    in_channels, out_channels, kernel_size=3, padding=(0, 1, 1)
                )
            else:
...
class CogVAEDecoder(torch.nn.Module):
    ...
    def decode_small_video(self, sample):
        ...
        for i in range(T // 2):
            ...
            tr = i * 2 + 2 + T % 2
            model_input = sample[:, :, tl:tr, :, :].to(
                dtype=computation_dtype, device=computation_device
            )
            model_output = self.forward(model_input).to(
...
class CogVAEEncoder(torch.nn.Module):
    def __init__(self):
        ...
        self.conv_act = torch.nn.SiLU()
        self.conv_out = CachedConv3d(
            512, 32, kernel_size=3, stride=1, padding=(0, 1, 1)
        )
