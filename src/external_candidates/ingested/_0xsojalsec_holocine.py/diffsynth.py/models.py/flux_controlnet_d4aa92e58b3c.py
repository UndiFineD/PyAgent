# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\flux_controlnet.py
import torch
from einops import rearrange, repeat

from .flux_dit import (
...
class FluxControlNet(torch.nn.Module):
    def __init__(
        ...
        self.blocks = torch.nn.ModuleList(
            [FluxJointTransformerBlock(3072, 24) for _ in range(num_joint_blocks)]
        )
        ...
        self.controlnet_blocks = torch.nn.ModuleList(
            [torch.nn.Linear(3072, 3072) for _ in range(num_joint_blocks)]
        )
        ...
        self.controlnet_mode_embedder = (
            torch.nn.Embedding(num_mode, 3072) if len(mode_dict) > 0 else None
        )
    ...
    def align_res_stack_to_original_blocks(self, res_stack, num_blocks, hidden_states):
        ...
        aligned_res_stack = [
            res_stack[block_id // interval] for block_id in range(num_blocks)
        ]
        ...
        **kwargs,
        ...
        if self.controlnet_mode_embedder is not None:  # Different from FluxDiT
            ...
            prompt_emb = torch.concat(
                [self.controlnet_mode_embedder(processor_id), prompt_emb], dim=1
            )
        ...
        controlnet_conditioning = self.patchify(
            controlnet_conditioning
        )  # Different from FluxDiT
        ...
        for block, controlnet_block in zip(
            self.single_blocks, self.controlnet_single_blocks
        ):
            hidden_states, prompt_emb = block(
                hidden_states, prompt_emb, conditioning, image_rotary_emb
            )
            controlnet_single_res_stack.append(
                controlnet_block(hidden_states[:, prompt_emb.shape[1] :])
            )
            ...
            for name, module in model.named_children():
                ...
                if isinstance(module, torch.nn.Linear):
                    with init_weights_on_device():
                        new_layer = quantized_layer.QLinear(
        ...
        for name in list(state_dict_.keys()):
            if ".proj_in_besides_attn." in name:
                ...
                param = torch.concat(
                    [
                        state_dict_[
                            name.replace(".proj_in_besides_attn.", f".a_to_q.")
                        ],
                        state_dict_[
                            name.replace(".proj_in_besides_attn.", f".a_to_k.")
                        ],
                        state_dict_[
                            name.replace(".proj_in_besides_attn.", f".a_to_v.")
                        ],
                        state_dict_[name],
                    ],
                    dim=0,
