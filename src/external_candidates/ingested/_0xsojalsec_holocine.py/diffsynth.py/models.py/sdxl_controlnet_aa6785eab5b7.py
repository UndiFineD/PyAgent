# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\sdxl_controlnet.py
from collections import OrderedDict

import torch

from .sd_controlnet import ControlNetConditioningLayer
from .sd_unet import AttentionBlock, DownSampler, PushBlock, ResnetBlock, Timesteps
...
class SDXLControlNetUnion(torch.nn.Module):
    def __init__(self, global_pool=False):
        ...
        self.time_embedding = torch.nn.Sequential(
            torch.nn.Linear(320, 1280), torch.nn.SiLU(), torch.nn.Linear(1280, 1280)
        )
        self.add_time_proj = Timesteps(256)
        self.add_time_embedding = torch.nn.Sequential(
            torch.nn.Linear(2816, 1280), torch.nn.SiLU(), torch.nn.Linear(1280, 1280)
        )
        self.control_type_proj = Timesteps(256)
        self.control_type_embedding = torch.nn.Sequential(
            torch.nn.Linear(256 * 8, 1280), torch.nn.SiLU(), torch.nn.Linear(1280, 1280)
        )
        self.conv_in = torch.nn.Conv2d(4, 320, kernel_size=3, padding=1)

        self.controlnet_conv_in = ControlNetConditioningLayer(
            channels=(3, 16, 32, 96, 256, 320)
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        control_type = torch.zeros(
            (sample.shape[0], 8), dtype=sample.dtype, device=sample.device
        )
        ...
        hidden_states = self.fuse_condition_to_input(
            hidden_states, task_id, conditioning
        )
        ...
        controlnet_res_stack = [
            block(res) for block, res in zip(self.controlnet_blocks, res_stack)
        ]

        # pool
        if self.global_pool:
            controlnet_res_stack = [
                res.mean(dim=(2, 3), keepdim=True) for res in controlnet_res_stack
            ]
    ...
    def from_diffusers(self, state_dict):
        ...
        for name in name_list:
            ...
            if names[0] in [
                ...
                while (
                    block_id[block_type] < len(block_types)
                    and block_types[block_id[block_type]] != block_type
                ):
                    block_id[block_type] += 1
                block_type_with_id = ".".join(names[:4])
