# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\sd3_text_encoder.py
import torch
...
class SD3TextEncoder1(SDTextEncoder):
    ...
    def forward(self, input_ids, clip_skip=2, extra_mask=None):
        ...
        embeds = embeds + self.position_embeds.to(
            dtype=embeds.dtype, device=input_ids.device
        )
    ...
    def from_civitai(self, state_dict):
        ...
        for name in state_dict:
            if name in rename_dict:
                ...
                if name == "text_model.embeddings.position_embedding.weight":
                    param = param.reshape((1, param.shape[0], param.shape[1]))
                state_dict_[rename_dict["text_encoders.clip_l.transformer." + name]] = (
                    param
                )
        return state_dict_
    ...
    def from_civitai(self, state_dict):
        ...
        for name in state_dict:
            if name in rename_dict:
                ...
                if name == "text_model.embeddings.position_embedding.weight":
                    param = param.reshape((1, param.shape[0], param.shape[1]))
                state_dict_[rename_dict["text_encoders.clip_g.transformer." + name]] = (
                    param
                )
        return state_dict_
