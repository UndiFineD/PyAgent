# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\lora.py
import torch
...
class LoRAFromCivitai:
    ...
    def convert_state_dict_up_down(
        ...
        for key in state_dict:
            ...
            weight_down = state_dict[key.replace(".lora_up", ".lora_down")].to(
                device="cuda", dtype=torch.float16
            )
            if len(weight_up.shape) == 4:
                weight_up = weight_up.squeeze(3).squeeze(2).to(torch.float32)
                weight_down = weight_down.squeeze(3).squeeze(2).to(torch.float32)
                lora_weight = alpha * torch.mm(weight_up, weight_down).unsqueeze(
                    2
                ).unsqueeze(3)
            else:
            ...
            target_name = (
                key.split(".")[0]
                .replace(lora_prefix, renamed_lora_prefix)
                .replace("_", ".")
                + ".weight"
            )
            for special_key in self.special_keys:
                target_name = target_name.replace(
                    special_key, self.special_keys[special_key]
                )
            state_dict_[target_name] = lora_weight.cpu()
        return state_dict_

    def convert_state_dict_AB(
        self,
        state_dict,
        lora_prefix="",
        alpha=1.0,
        ...
        for key in state_dict:
            ...
            weight_down = state_dict[key.replace(".lora_B.", ".lora_A.")].to(
    ...
    def load(self, model, state_dict_lora, lora_prefix, alpha=1.0, model_resource=None):
        ...
        state_dict_lora = self.convert_state_dict(
            state_dict_lora, lora_prefix=lora_prefix, alpha=alpha
        )
        if model_resource == "diffusers":
            state_dict_lora = model.__class__.state_dict_converter().from_diffusers(
                state_dict_lora
            )
        elif model_resource == "civitai":
            state_dict_lora = model.__class__.state_dict_converter().from_civitai(
                state_dict_lora
            )
        ...
        if len(state_dict_lora) > 0:
            ...
            for name in state_dict_lora:
                ...
                if state_dict_model[name].dtype == torch.float8_e4m3fn:
                    state_dict_model[name] = state_dict_model[name].to(
                        state_dict_lora[name].dtype
                    )
                    fp8 = True
    ...
    def match(self, model, state_dict_lora):
        for lora_prefix, model_class in zip(
            self.lora_prefix, self.supported_model_classes
        ):
            ...
            for model_resource in ["diffusers", "civitai"]:
                try:
                    state_dict_lora_ = self.convert_state_dict(
                        state_dict_lora, lora_prefix=lora_prefix, alpha=1.0
                    )
                    converter_fn = (
                        model.__class__.state_dict_converter().from_diffusers
                        if model_resource == "diffusers"
...
class GeneralLoRAFromPeft:
    ...
    def load(
        self, model, state_dict_lora, lora_prefix="", alpha=1.0, model_resource=""
    ):
        state_dict_model = model.state_dict()
        device, dtype, computation_device, computation_dtype = (
            self.fetch_device_and_dtype(state_dict_model)
        )
        lora_name_dict = self.get_name_dict(state_dict_lora)
        for name in lora_name_dict:
            ...
            weight_model = state_dict_model[name].to(
                device=computation_device, dtype=computation_dtype
            )
    ...
    def align_to_opensource_format(state_dict, alpha=None):
        ...
        for name, param in state_dict.items():
            ...
            if rename.endswith("lora_up.weight"):
                ...
                state_dict_[rename.replace("lora_up.weight", "alpha")] = torch.tensor(
                    (lora_alpha,)
                )[0]
        return state_dict_
...
class WanLoRAConverter:
    ...
    def align_to_opensource_format(state_dict, **kwargs):
        state_dict = {
            "diffusion_model." + name.replace(".default.", "."): param
            for name, param in state_dict.items()
        }
        return state_dict
    ...
    def align_to_diffsynth_format(state_dict, **kwargs):
        state_dict = {
            ...
            .replace(".lora_B.weight", ".lora_B.default.weight"): param
            for name, param in state_dict.items()
        }
        return state_dict
