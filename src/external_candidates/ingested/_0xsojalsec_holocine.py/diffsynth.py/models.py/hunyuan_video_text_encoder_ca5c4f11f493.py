# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\hunyuan_video_text_encoder.py
...
class HunyuanVideoLLMEncoder(LlamaModel):
    ...
    def forward(self, input_ids, attention_mask, hidden_state_skip_layer=2):
        embed_tokens = (
            deepcopy(self.embed_tokens).to(input_ids.device)
        ...
        rotary_emb = (
            deepcopy(self.rotary_emb).to(input_ids.device)
            if self.auto_offload
...
class HunyuanVideoMLLMEncoder(LlavaForConditionalGeneration):
    ...
    def forward(
        self, input_ids, pixel_values, attention_mask, hidden_state_skip_layer=2
    ):
