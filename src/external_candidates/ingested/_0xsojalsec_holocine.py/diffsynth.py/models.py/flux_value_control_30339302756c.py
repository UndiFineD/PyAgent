# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\flux_value_control.py
import torch
...
class SingleValueEncoder(torch.nn.Module):
    def __init__(
        self, dim_in=256, dim_out=4096, prefer_len=32, computation_device=None
    ):
        super().__init__()
    ...
    def forward(self, value, dtype):
        ...
        positional_embedding = self.positional_embedding.to(
            dtype=base_embeddings.dtype, device=base_embeddings.device
        )
