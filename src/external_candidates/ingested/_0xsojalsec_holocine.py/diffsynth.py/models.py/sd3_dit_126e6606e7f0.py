# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\sd3_dit.py
import torch
...
class PatchEmbed(torch.nn.Module):
    def __init__(
        self, patch_size=2, in_channels=16, embed_dim=1536, pos_embed_max_size=192
    ):
        super().__init__()
        ...
        self.pos_embed = torch.nn.Parameter(
            torch.zeros(1, self.pos_embed_max_size, self.pos_embed_max_size, embed_dim)
        )
    ...
    def cropped_pos_embed(self, height, width):
        ...
        spatial_pos_embed = self.pos_embed[
...
class JointAttention(torch.nn.Module):
    def __init__(
        self, dim_a, dim_b, num_heads, head_dim, only_out_a=False, use_rms_norm=False
    ):
        super().__init__()
    ...
    def process_qkv(self, hidden_states, to_qkv, norm_q, norm_k):
        ...
        qkv = qkv.view(batch_size, -1, 3 * self.num_heads, self.head_dim).transpose(
            1, 2
        )
    ...
    def forward(self, hidden_states_a, hidden_states_b):
        ...
        qa, ka, va = self.process_qkv(
            hidden_states_a, self.a_to_qkv, self.norm_q_a, self.norm_k_a
        )
        qb, kb, vb = self.process_qkv(
            hidden_states_b, self.b_to_qkv, self.norm_q_b, self.norm_k_b
        )
...
class DualTransformerBlock(torch.nn.Module):
    ...
    def forward(self, hidden_states_a, hidden_states_b, temb):
        ...
        attn_output_a, attn_output_b = self.attn(
            norm_hidden_states_a, norm_hidden_states_b
        )

        # Part A
        hidden_states_a = hidden_states_a + gate_msa_a * attn_output_a
        hidden_states_a = hidden_states_a + gate_msa_a_2 * self.attn2(
            norm_hidden_states_a_2
        )
        norm_hidden_states_a = (
        ...
        self.blocks = torch.nn.ModuleList(
            [
                JointTransformerBlock(
                    embed_dim, embed_dim // 64, use_rms_norm=use_rms_norm, dual=True
                )
                for _ in range(num_dual_blocks)
            ]
            + [
                JointTransformerBlock(
                    embed_dim, embed_dim // 64, use_rms_norm=use_rms_norm
                )
                for _ in range(num_layers - 1 - num_dual_blocks)
            ]
            + [
...
class SD3DiTStateDictConverter:
    ...
    def from_diffusers(self, state_dict):
        ...
        merged_keys = [
            name for name in state_dict_ if ".a_to_q." in name or ".b_to_q." in name
        ]
            ...
            rename_dict.update(
                {
                    ...
                    f"model.diffusion_model.joint_blocks.{i}.context_block.attn.qkv.bias": [
                        f"blocks.{i}.attn.b_to_q.bias",
                        f"blocks.{i}.attn.b_to_k.bias",
                        f"blocks.{i}.attn.b_to_v.bias",
                    ],
                    f"model.diffusion_model.joint_blocks.{i}.context_block.attn.qkv.weight": [
                        f"blocks.{i}.attn.b_to_q.weight",
                        f"blocks.{i}.attn.b_to_k.weight",
                        f"blocks.{i}.attn.b_to_v.weight",
                    ],
                    ...
                    f"model.diffusion_model.joint_blocks.{i}.x_block.attn.qkv.bias": [
                        f"blocks.{i}.attn.a_to_q.bias",
                        f"blocks.{i}.attn.a_to_k.bias",
                        f"blocks.{i}.attn.a_to_v.bias",
                    ],
                    f"model.diffusion_model.joint_blocks.{i}.x_block.attn.qkv.weight": [
                        f"blocks.{i}.attn.a_to_q.weight",
                        f"blocks.{i}.attn.a_to_k.weight",
                        f"blocks.{i}.attn.a_to_v.weight",
                    ],
        ...
        for name in state_dict:
            if name in rename_dict:
                ...
                if name == "model.diffusion_model.pos_embed":
                    ...
                    param = param.reshape(
                        (1, pos_embed_max_size, pos_embed_max_size, param.shape[-1])
                    )
                if isinstance(rename_dict[name], str):
                    ...
                    name_ = (
                        rename_dict[name][0]
                        .replace(".a_to_q.", ".a_to_qkv.")
                        .replace(".b_to_q.", ".b_to_qkv.")
                    )
                    state_dict_[name_] = param
        extra_kwargs = self.infer_architecture(state_dict_)
        num_layers = extra_kwargs["num_layers"]
        for name in [
            f"blocks.{num_layers-1}.norm1_b.linear.weight",
            f"blocks.{num_layers-1}.norm1_b.linear.bias",
