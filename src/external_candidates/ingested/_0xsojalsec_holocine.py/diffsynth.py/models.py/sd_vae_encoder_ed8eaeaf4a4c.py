# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\sd_vae_encoder.py
import torch
from einops import rearrange

from .sd_unet import DownSampler, ResnetBlock
...
class SDVAEEncoder(torch.nn.Module):
    def __init__(self):
        ...
        self.conv_norm_out = torch.nn.GroupNorm(
            num_channels=512, num_groups=32, eps=1e-6
        )
    ...
    def forward(self, sample, tiled=False, tile_size=64, tile_stride=32, **kwargs):
        ...
        if tiled:
            return self.tiled_forward(
                sample, tile_size=tile_size, tile_stride=tile_stride
            )
    ...
    def encode_video(self, sample, batch_size=8):
        ...
        for i in range(0, sample.shape[2], batch_size):
            ...
            hidden_states_batch = rearrange(
                hidden_states_batch, "(B T) C H W -> B C T H W", B=B
            )
