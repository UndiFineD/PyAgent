...
class SVDVAEDecoder(torch.nn.Module):
    def __init__(self):
        ...
        self.conv_norm_out = torch.nn.GroupNorm(
            num_channels=128, num_groups=32, eps=1e-5
        )
        self.conv_act = torch.nn.SiLU()
        self.conv_out = torch.nn.Conv2d(128, 3, kernel_size=3, padding=1)
        self.time_conv_out = torch.nn.Conv3d(
            3, 3, kernel_size=(3, 1, 1), padding=(1, 0, 0)
        )
    ...
    def decode_video(
        ...
        weight = torch.zeros(
            (1, T, H * 8, W * 8), dtype=torch_dtype, device=data_device
        )
        values = torch.zeros(
            (3, T, H * 8, W * 8), dtype=torch_dtype, device=data_device
        )
...
class SVDVAEDecoderStateDictConverter:
    ...
    def from_diffusers(self, state_dict):
        ...
        for name in static_rename_dict:
            state_dict_[static_rename_dict[name] + ".weight"] = state_dict[
                name + ".weight"
            ]
        ...
        for prefix_name in prefix_rename_dict:
            for suffix_name in suffix_rename_dict:
                ...
                name_ = (
                    prefix_rename_dict[prefix_name]
                    + "."
                    + suffix_rename_dict[suffix_name]
                )
