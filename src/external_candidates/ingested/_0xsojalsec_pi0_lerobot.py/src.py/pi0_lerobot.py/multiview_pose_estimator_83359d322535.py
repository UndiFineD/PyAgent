# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pi0-lerobot\src\pi0_lerobot\multiview_pose_estimator.py
...
def project_multiview(
    ...
    kp3d_h = np.hstack(
        (xyzc[:, :3], np.ones((n_kpts, 1), dtype=xyzc.dtype))
    )  # Shape: (K, 4)
    ...
    uv_normalized = np.where(
        valid_division_mask, uv_raw_division, np.nan
    )  # Shape: (V, K, 2)
    ...
    confidence_mask = (original_conf[:, None] > 0.0).astype(
        kp2ds.dtype
    )  # Shape: (K, 1)
...
class MultiviewBodyTracker:
    ...
    def __call__(
        ...
        if self.xyzc_t1 is not None and self.xyzc_t2 is not None:
            ...
            uv_max: Float[ndarray, "n_views 2"] = np.nanmax(
                uvc_extrap[:, :, 0:2], axis=1
            )
            uv_min: Float[ndarray, "n_views 2"] = np.nanmin(
                uvc_extrap[:, :, 0:2], axis=1
            )
            bboxes_extrap: Float[ndarray, "n_views 4"] = np.concatenate(
                [uv_min, uv_max], axis=1
            ).astype(np.float32)

        uvc_list = []
        for image_idx, bgr in enumerate(bgr_list):
            if xyzc_extrap is not None:
                bboxes: Float32[ndarray, "n_dets 4"] = rearrange(
                    bboxes_extrap[image_idx], "B -> 1 B"
                )
            else:
    ...
    def extrapolate_3d_keypoints(
        ...
        xyzc_t2: Float32[ndarray, "n_kpts 4"],
