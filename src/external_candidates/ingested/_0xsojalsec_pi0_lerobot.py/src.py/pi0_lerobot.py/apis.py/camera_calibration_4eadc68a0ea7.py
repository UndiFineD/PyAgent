# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pi0-lerobot\src\pi0_lerobot\apis\camera_calibration.py
...
from pi0_lerobot.vggt_load_utils import VGGTPredictions
from pi0_lerobot.vggt_load_utils import create_blueprint as cb_vggt
from pi0_lerobot.vggt_load_utils import preprocess_images
...
def calibrate_with_dust3r(sequence: HOCapSequence):
    ...
    # load dust3r model
    dust3r_model = AsymmetricCroCo3DStereo.from_pretrained("pablovela5620/dust3r").to(
        "cuda"
    )
    exo_data: ExoData = next(iter(sequence))
    # Convert BGR images to RGB format
    rgb_list: list[UInt8[ndarray, "H W 3"]] = [
        cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB) for bgr in exo_data.bgr_list
    ]
    ...
    log_optimized_result(
        optimized_results, parent_log_path=parent_log_path, log_depth=True
    )


def calibrate_with_vggt(
    sequence: HOCapSequence, confidence_threshold: float = 50.0
) -> None:
    parent_log_path = Path("world")
    ...
    img_tensors: Float32[Tensor, "num_img 3 H W"] = preprocess_images(rgb_list).to(
        device
    )
    dtype = (
    ...
    blueprint = cb_vggt(
        parent_log_path,
        image_paths=[Path(exo_cam.name) for exo_cam in sequence.exo_cam_list],
    )
    rr.send_blueprint(blueprint)
    ...
    extrinsic, intrinsic = pose_encoding_to_extri_intri(
        predictions["pose_enc"], img_tensors.shape[-2:]
    )
    ...
    original_images: Float32[ndarray, "num_cams H W 3"] = rearrange(
        original_images, "num_cams C H W -> num_cams H W C"
    )
    # Flatten both points and colors
    flattened_points: Float32[ndarray, "num_points 3"] = rearrange(
        world_points, "num_cams H W C -> (num_cams H W) C"
    )
    ...
    conf_threshold = (
        ...
        else np.percentile(conf, confidence_threshold)
    )
    ...
    for idx, (intri, extri, image, depth_map, depth_conf) in enumerate(
        ...
        conf_threshold = (
            0.0
            if confidence_threshold == 0.0
            else np.percentile(depth_conf, confidence_threshold)
        )
        ...
        rr.log(
            f"{cam_log_path}/pinhole/confidence", rr.Image(conf_mask.astype(np.float32))
        )
