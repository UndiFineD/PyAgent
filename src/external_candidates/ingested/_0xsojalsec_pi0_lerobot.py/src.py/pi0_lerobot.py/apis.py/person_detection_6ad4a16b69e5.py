...
def run_person_detection(config: VisualzeConfig):
    ...
    shortest_timestamp: Int[ndarray, "num_frames"] = min(
        all_timestamps, key=len
    )  # noqa: UP037
    assert len(shortest_timestamp) == len(
        sequence
    ...
    Pall: Float32[np.ndarray, "n_views 3 4"] = np.array(
        [P for P in projection_all_list]
    )
    ...
    for ts_idx, timestamp in enumerate(tqdm(shortest_timestamp[:min_num_frames])):
        ...
        for cam_idx, (exo_pinhole, (xyxy, uv, uv_scores)) in enumerate(
            ...
            if mv_output.uvc_extrap is not None and config.log_extrapolated_keypoints:
                ...
                uv_max: Float64[ndarray, "1 2"] = np.nanmax(
                    uvc_extrap[:, 0:2], axis=0, keepdims=True
                )
                uv_min: Float64[ndarray, "1 2"] = np.nanmin(
                    uvc_extrap[:, 0:2], axis=0, keepdims=True
                )
                bbox_extrap: Float64[ndarray, "1 4"] = np.concatenate(
        ...
        vis_scores_3d: Float32[ndarray, "num_kpts"] = (
            mv_output.scores_3d.copy()
        )  # noqa: UP037
        # filter to only include the desired keypoints
