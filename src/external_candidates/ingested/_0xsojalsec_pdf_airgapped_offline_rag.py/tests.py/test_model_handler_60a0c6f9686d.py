# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdf-airgapped-offline-Rag\tests\test_model_handler.py
...
def test_generate_stream(mock_llama, model_handler):
    ...
    with patch.object(model_handler, "get_model", return_value=mock_llama_instance):
        result = list(
            model_handler.generate_stream("Test prompt", model_choice="Llama 3")
        )
...
def test_check_available_models(model_handler, mock_config):
    with patch("os.path.exists", return_value=True):
        ...
        assert set(model_handler.available_models) == set(
...
def test_log_performance_metrics(mock_logger, model_handler):
    ...
    mock_logger.info.assert_called_once_with(
        "Generated 100 tokens in 10.00 seconds (10.00 tokens/sec)"
    )
