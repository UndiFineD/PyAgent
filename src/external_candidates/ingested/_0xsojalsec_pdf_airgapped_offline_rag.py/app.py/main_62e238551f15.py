# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdf-airgapped-offline-Rag\app\main.py
...
def load_models():
    with st.spinner("Loading models... This may take a few minutes."):
        ...
        if not model_handler.available_models:
            st.error(
                "No models are available. Please check your configuration and model files."
            )
        else:
            # st.write(f"Debug: Available models: {model_handler.available_models}")  # More debug info
            alert = st.success(
                f"Models loaded successfully! Available models: {', '.join(model_handler.available_models)}"
            )
            time.sleep(2)
            alert.empty()
...
def main():
    ...
    with header:
        ...
        """.format(get_base64_of_image("assets/airgapped_offline_rag_icon.png")),
            unsafe_allow_html=True,
        )
    ...
    with st.container():
        ...
        if model_handler.available_models:
            model_choice = st.selectbox(
                "Choose a model", model_handler.available_models
            )
            st.session_state.model_choice = model_choice
        else:
            st.error(
                "No models available. Please check your configuration and model files."
            )
            return
        ...
        with col3:
            if st.button("Process"):
                if uploaded_files or existing_docs:
                    ...
                    st.warning(
                        "No documents found. Please upload documents to use RAG or disable RAG."
                    )
                else:
        ...
        if "processing_result" in st.session_state:
            ...
            if st.session_state.debug_mode and "processing_logs" in st.session_state:
                with st.expander("Processing Logs"):
                    st.markdown(
                        f'<div class="processing-logs">{st.session_state.processing_logs}</div>',
                        unsafe_allow_html=True,
                    )
    ...
    with st.spinner("Processing documents..."):
        try:
            ...
            st.session_state.processing_result = (
                f'<div class="stAlert error">Error processing documents: {str(e)}</div>'
            )
            st.session_state.chat_enabled = False
    ...
    if st.session_state.chat_enabled or not st.session_state.use_rag:
        ...
        if prompt := st.chat_input("What is your question?"):
            ...
            with st.chat_message("assistant"):
                ...
                try:
                    for response in model_handler.generate_stream(
                        full_prompt, model_choice=model_choice
                    ):
