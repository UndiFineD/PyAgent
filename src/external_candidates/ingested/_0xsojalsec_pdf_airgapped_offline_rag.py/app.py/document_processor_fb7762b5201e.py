# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdf-airgapped-offline-Rag\app\document_processor.py
...
def remove_document(document_name):
    try:
        ...
        if results and results["ids"]:
            ...
            logging.info(
                f"Removed {len(results['ids'])} embeddings for document: {document_name}"
            )
            ...
            logging.warning(
                f"No embeddings found in vectorstore for document: {document_name}"
            )
            return False
    except Exception as e:
