# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdf-airgapped-offline-Rag\app\model_handler.py
...
class ModelHandler:
    ...
    def get_model(self, model_choice):
        if model_choice not in self.loaded_models:
            ...
            if model_choice not in model_paths:
                raise ValueError(
                    f"Model {model_choice} is not available. Available models: {', '.join(self.available_models)}"
                )
            self.loaded_models[model_choice] = self.load_model(
    ...
    def _log_performance_metrics(self, start_time, end_time, tokens_generated):
        ...
        logger.info(
            f"Generated {tokens_generated} tokens in {total_time:.2f} seconds ({tokens_per_second:.2f} tokens/sec)"
        )
