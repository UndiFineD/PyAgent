# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Neton\NetonWeb\core\libs\execution_helpers.py
import logging
import os
import time
...
def check_module(json_obj, ip):
    ...
    try:
        ...
        if execution_obj is None:
            execution_obj = insert_execution(
                executionId, sandboxId, wave, ip, version, md5_sample, sha256_sample
            )
...
def insert_execution(
    ...
    try:
        ...
        log_error.error(
            "insert_execution - analyze_ip <IP: " + str(ip) + "> - " + str(e)
        )
    ...
    try:
        ...
        log_error.error(
            "insert_execution <executionId: " + executionId + "> - " + str(e)
        )
        return None
...
def get_executions_helper():
    ...
    try:
        ...
        for elem in execution_q:
            if (
                elem["m_hook"] == 0
                and elem["m_systeminfo"] == 0
                and elem["m_filecrawler"] == 0
                and elem["m_checksandbox"] == 0
                and elem["m_sharpedrchecker"] == 0
                and elem["m_pafish"] == 0
                and elem["m_alkhaser"] == 0
            ):
                pass
            else:
                elem["creation_date"] = elem["creation_date"].strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                l_res.append(elem)
    except Exception as e:
