    ...
    try:
        ...
        if not exists:
            h_obj = Checksandbox(
                ...
                checkWindowTitleTags=helpers.b64decode(
                    json_obj["checkWindowTitleTags"]
                ),
        ...
        log_error.error(
            "insert_checksandbox <executionId: " + executionId + "> - " + str(e)
        )
...
def get_checksandbox_helper(execution_id=None):
    ...
    try:
        if execution_id == None:
            ...
            checksandbox_obj = Checksandbox.objects.filter(
                execution_id=execution_id
            ).values()

        for elem in checksandbox_obj:
            execution_info = execution_helpers.get_execution_id(elem["execution_id"])
            checkFilesTags_l = helpers.remove_empty_lists(
                elem["checkFilesTags"].split("\n")
            )
            ...
            checkExeRootTags_l = helpers.remove_empty_lists(
                elem["checkExeRootTags"].split("\n")
            )
            ...
            checkPathTags_l = helpers.remove_empty_lists(
                elem["checkPathTags"].split("\n")
            )
            checkPathTags_l_aux = []
            ...
            checkKeyValueTags_l = helpers.remove_empty_lists(
                elem["checkKeyValueTags"].split("\n")
            )
            ...
            checkWindowTitleTags_l = helpers.remove_empty_lists(
                elem["checkWindowTitleTags"].split("\n")
            )
