# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Neton\NetonWeb\core\libs\export_helper.py
import json
import logging
import os
...
def export_json_helper():
    ...
    for e in l_execution:
        if e.wave != 0:
            ...
            checksandbox_obj = (
                Checksandbox.objects.filter(execution_id=e.id).values().first()
            )
            for field in checksandbox_fields:
                if checksandbox_obj is not None:
                    d_checksandbox[field] = checksandbox_obj[field]
            filecrawler_obj = (
                Filecrawler.objects.filter(execution_id=e.id).values().first()
            )
    ...
    output_file_path = os.path.join(settings.TEMP_FOLDER, output_file)
