# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Neton\NetonWeb\core\libs\systeminfo_helpers.py
import logging
    ...
    try:
        ...
        if not exists:
            ...
            h_obj = Systeminfo(
                ...
                processor_information=helpers.b64decode(
                    json_obj["processorInformation"]
                ),
                ...
                environment_variables=helpers.b64decode(
                    json_obj["environmentVariables"]
                ),
            ...
            if "processorInformation" in json_obj:
                exists.processor_information = helpers.b64decode(
                    json_obj["processorInformation"]
                )
            ...
            if "environmentVariables" in json_obj:
                exists.environment_variables = helpers.b64decode(
                    json_obj["environmentVariables"]
                )
        ...
        log_error.error(
            "insert_systeminfo <executionId: " + executionId + "> - " + str(e)
        )
...
def get_systeminfo_helper(execution_id=None):
    ...
    try:
        if execution_id == None:
            ...
            systeminfo_obj = Systeminfo.objects.filter(
        ...
        for elem in systeminfo_obj:
            ...
            environment_variables_l = helpers.remove_empty_elements(
                environment_variables_l
            )
            ...
            for s in networkInfo_l:
                ...
                try:
                    ...
                    pass
            dir_drivers_l = helpers.remove_empty_elements(
                elem["dir_drivers"].split("*********")
            )
            dir_drivers_l_aux = []
            for aux in dir_drivers_l:
                dir_drivers_l_aux.append(helpers.remove_empty_elements(aux.split("\n")))
            systeminfo_l.append(
                {
                    ...
                    "sysinfo_output": helpers.remove_empty_elements(
                        elem["sysinfo_output"].split("\n")
                    ),
                    "os_version": os_version_aux,
                    "processor_information": helpers.remove_empty_elements(
                        elem["processor_information"].split("\n")
                    ),
                    "physical_memory": helpers.remove_empty_elements(
                        elem["physical_memory"].split("\n")
                    ),
                    "computer_ram": helpers.remove_empty_elements(
                        elem["computer_ram"].split("\n")
                    ),
                    "bios_ser_no": helpers.remove_empty_elements(
                        elem["bios_ser_no"].split("\n")
                    ),
                    "networkInfo": networkInfo_l_aux,
                    "tasklist": task_l_aux,
                    "users": helpers.remove_empty_elements(elem["users"].split("\n")),
                    "localgroup": helpers.remove_empty_elements(
                        elem["localgroup"].split("\n")
                    ),
                    ...
                    "system_pipes": helpers.remove_empty_elements(
                        elem["system_pipes"].split("\n")
                    ),
                    ...
                    "recent_files": helpers.remove_empty_elements(
                        elem["recent_files"].split("\n")
                    ),
                    "installed_apps": installed_apps_l_aux,
                    "mouse_position": mouse_position_l_aux,
                    "modules": modules_l_aux,
                    "creation_date": elem["creation_date"].strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                }
            )
