# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Neton\NetonWeb\core\libs\filecrawler_helpers.py
import logging
...
def insert(json_obj, execution_obj, executionId):
    try:
        ...
        log_error.error(
            "insert_filecrawler <executionId: " + executionId + "> - " + str(e)
        )
...
def get_filecrawler_helper(execution_id=None):
    ...
    try:
        if execution_id == None:
            ...
            filecrawler_obj = Filecrawler.objects.filter(
            ...
            filecrawler_l.append(
                {
                    ...
                    "files_in_drives": helpers.remove_empty_elements(
                        elem["files_in_drives"].split("\n")
                    ),
                    "file_paths_64": helpers.remove_empty_elements(
                        elem["file_paths_64"].split("\n")
                    ),
                    "file_paths_32": helpers.remove_empty_elements(
                        elem["file_paths_32"].split("\n")
                    ),
                    "dir_c": helpers.remove_empty_elements(elem["dir_c"].split("\n")),
                    "drivers": helpers.remove_empty_elements(
                        elem["drivers"].split("\n")
                    ),
