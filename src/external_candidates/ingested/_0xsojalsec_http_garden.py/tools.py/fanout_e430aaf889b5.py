# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\tools\fanout.py
...
from http1 import HTTPRequest, HTTPResponse, parse_response
from targets import Service
from util import eager_pmap, stream_t
...
def transducer_roundtrip(data: stream_t, transducer: Service) -> stream_t:
    ...
    with socket.create_connection((transducer.address, transducer.port)) as sock:
        ...
        for datum in data:
            try:
                ...
                raise ValueError(
                    f"{transducer.name} broke the pipe in response to {data!r}!"
                )
    ...
    while len(remaining) > 0:
        ...
        if parsed_response.code != b"200":  # It parsed, but the status is bad
            raise ValueError(
                f"{transducer.name} rejected the payload with status {parsed_response.code!r}"
            )
...
def fanout(
    ...
    return eager_pmap(
        functools.partial(parsed_server_roundtrip, data, traced=traced), servers
    )
