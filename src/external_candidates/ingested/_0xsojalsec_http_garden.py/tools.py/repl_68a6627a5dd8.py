# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\tools\repl.py
"""The Garden repl"""
...
from diff_fuzz import SEEDS, DiscrepancyType, categorize_discrepancy, run_one_generation
from fanout import adjust_host_header, fanout, server_roundtrip, transducer_roundtrip
from http1 import HTTPRequest, HTTPResponse
from mutations import mutate
from targets import Service  # This won't change across reloads
from util import eager_pmap, fingerprint_t, stream_t

INTERESTING_DISCREPANCY_TYPES = [
    DiscrepancyType.StreamDiscrepancy,
    DiscrepancyType.SubtleDiscrepancy,
]
    ...
    for s, (pts, _) in zip(servers, fanout(payload, servers, traced=False)):
        ...
        for r in pts:
            if isinstance(r, HTTPRequest):
                ...
                print(
                    f"        method={r.method!r}, uri={r.uri!r}, version={r.version!r},"
                )
                if len(r.headers) == 0:
                    print("        headers=[],")
                else:
...
def compute_grid(
    payload: stream_t, servers: list[Service]
) -> tuple[tuple[bool | None, ...], ...]:
    pts: list[list[HTTPRequest | HTTPResponse]] = [
        pt for pt, _ in fanout(payload, servers, traced=False)
    ]
    result = []
    for i, (s1, pt1) in enumerate(zip(servers, pts)):
        ...
        for j, (s2, pt2) in enumerate(zip(servers, pts)):
            if j <= i:
                ...
                row.append(
                    categorize_discrepancy([pt1, pt2], [s1, s2])
                    in INTERESTING_DISCREPANCY_TYPES
                )
        ...
        for entry in row:
            print(
                (
                    " " * column_width
                    if entry is None
                    else ("❌" if entry else "✅").ljust(column_width - 1)
                ),
                end="",
            )
        print()
...
def print_help_message() -> None:
    ...
    print("mutate")
    print(
        "    Mutates the current payload using a random choice of the mutation operations."
    )
        ...
        for command in commands:
            ...
            if command == ["help"]:
                ...
                for symbol in command[1:]:
                    ...
                    for symbol in command[1:]:
                        pprint.pp(
                            (targets.SERVER_DICT | targets.TRANSDUCER_DICT)[symbol]
                        )
                ...
                if len(command) == 1:
                    ...
                    if command[1] == "name":
                        print(
                            f"Name pattern:  {name_pattern and name_pattern.pattern!r}"
                        )
                    elif command[1] == "value":
                        print(
                            f"Value pattern:  {value_pattern and value_pattern.pattern!r}"
                        )
                    elif command[1] == "body":
                        print(
                            f"Body pattern:  {body_pattern and body_pattern.pattern!r}"
                        )
                    else:
                        invalid_syntax()
                        continue
                ...
                if len(command) == 1:
                    ...
                    len(command) == 2 and command[1].isascii() and command[1].isdigit()
                ):
                ...
                if len(command) != 1:
                    ...
                    continue
                print_fanout(
                    payload, servers, name_pattern, value_pattern, body_pattern
                )
                ...
                if len(command) == 1:
                    ...
                    try:
                        transducers = [
                            targets.TRANSDUCER_DICT[t_name] for t_name in command[1:]
                        ]
                    except KeyError:
                        t_name = next(
                            name
                            for name in command[1:]
                            if name not in targets.TRANSDUCER_DICT
                        )
                ...
                for server in servers:
                    if server.name not in targets.SERVER_DICT:
                        print(
                            f"{server.name} no longer available. Removing it from selection."
                        )
                    else:
                ...
                for i in range(
                    num_generations + 1
                ):  # +1 because there's a zeroth generation: the seeds
                    try:
                        ...
                        print(
                            "    https://github.com/narfindustries/http-garden/issues/8"
                        )
                        break
                    except KeyboardInterrupt:
                        break
                ...
                for result in results:
                    for transduced in transducer_fanout(
                        result, list(targets.TRANSDUCER_DICT.values())
                    ):
                        pts: list[list[HTTPRequest | HTTPResponse]] = [
                            pt for pt, _ in fanout(transduced, servers, traced=False)
                        ]
                        if (
                            categorize_discrepancy(pts, servers)
                            in INTERESTING_DISCREPANCY_TYPES
                        ):
                ...
                categorized_results: dict[
                    tuple[tuple[bool | None, ...], ...], list[stream_t]
                ] = {}
                for result in durable_results:
                    grid: tuple[tuple[bool | None, ...], ...] = compute_grid(
                        result, servers
                    )
