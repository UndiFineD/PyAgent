# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\tools\targets.py
...
_EXTERNAL_YML_PATH: PosixPath = PosixPath(
    f"{sys.path[0] or '.'}/../external-services.yml"
)
...
class Service:
    ...
    timeout: (
        float  # The fastest timeout that can be used in connections with this server
    )
...
def _get_container_ip(
    container: docker.models.containers.Container, network_name: str
) -> str | None:
...
def _extract_services(role: str) -> list[Service]:
    ...
    for svc_name in services:
        ...
        container: docker.models.containers.Container | None = _CONTAINER_DICT.get(
            svc_name
        )
        address: str | None = x_props.get("address") or _get_container_ip(
            container, _NETWORK_NAME
        )
        ...
        result.append(
            Service(
                ...
                translates_chunked_to_cl=anomalies.get("translates-chunked-to-cl")
                or False,
                requires_length_in_post=anomalies.get("requires-length-in-post")
                or False,
                allows_missing_host_header=anomalies.get("allows-missing-host-header")
                or False,
                ...
                method_whitelist=[
                    s.encode("latin1") for s in anomalies.get("method-whitelist") or []
                ]
                or None,
                removed_headers=[
                    (k.encode("latin1"), v.encode("latin1"))
...
SERVER_DICT: dict[str, Service] = {
    server.name: server for server in _extract_services("server")
}

TRANSDUCER_DICT: dict[str, Service] = {
    t.name: t for t in _extract_services("transducer")
}
