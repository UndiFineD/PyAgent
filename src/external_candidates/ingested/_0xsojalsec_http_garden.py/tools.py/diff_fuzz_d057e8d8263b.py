# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\tools\diff_fuzz.py
...
from util import fingerprint_t, stream_t, translate, unzip
...
def stream_is_invalid(parse_trees: list[HTTPRequest | HTTPResponse]) -> bool:
    return any(
        isinstance(r, HTTPResponse) and r.code == b"400" for r in parse_trees[:-1]
    )


def normalize_messages(
    r1: HTTPRequest | HTTPResponse | None,
    s1: Service,
    r2: HTTPRequest | HTTPResponse | None,
    s2: Service,
):
    ...
    if (
        s1.translates_chunked_to_cl
        and not s2.translates_chunked_to_cl
    ...
    if (
        len(s1.header_name_translation) > 0
        and isinstance(r1, HTTPRequest)
        and isinstance(r2, HTTPRequest)
    ):
        r2 = translate_request_header_names(r2, s1.header_name_translation)
    if (
        len(s2.header_name_translation) > 0
        and isinstance(r2, HTTPRequest)
        and isinstance(r1, HTTPRequest)
    ):
    ...
    for (pt1, s1), (pt2, s2) in itertools.combinations(zip(parse_trees, servers), 2):
        ...
        for r1, r2 in itertools.zip_longest(pt1, pt2):
            ...
            if (isinstance(r1, HTTPRequest) and not isinstance(r2, HTTPRequest)) or (
                ...
                if (
                    r1 is None and isinstance(r2, HTTPResponse) and r2.code == b"400"
                ) or (
...
def run_one_generation(
    ...
    for current_input in tqdm.tqdm(inputs):
        ...
        if (
            categorize_discrepancy(parse_trees, servers)
            != DiscrepancyType.NoDiscrepancy
        ):
