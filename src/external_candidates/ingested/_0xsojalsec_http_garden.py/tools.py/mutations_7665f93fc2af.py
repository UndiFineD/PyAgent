# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\tools\mutations.py
...
def _replace_random_byte(s: stream_t) -> stream_t:
    ...
    for req_idx, req in enumerate(s):
        if len(req) > idx:
            result[req_idx] = (
                req[:idx] + bytes([random.randint(0, 255)]) + req[idx + 1 :]
            )
            return result
        idx -= len(req)
    assert False
...
def _unparse_request(request: HTTPRequest) -> bytes:
    is_chunked: bool = b"transfer-encoding" in map(
        lambda p: p[0].lower(), request.headers
    )
    return (
        request.method
        + b" "
...
def _replace_random_header(s: stream_t) -> stream_t:
    ...
    request_idx, request = random.choice(list(enumerate(parsed_substream)))
    assert len(request.headers) >= 1
    idx: int = random.randint(
        0, len(request.headers) - 1
    )  # You can replace in n places
    new_request: HTTPRequest = copy.deepcopy(request)
    new_request.headers = (
