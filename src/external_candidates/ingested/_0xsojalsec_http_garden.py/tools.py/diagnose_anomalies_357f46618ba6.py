# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\tools\diagnose_anomalies.py
"""This is for automatically detecting certain simple parsing quirks in servers, so that they can later be ignored."""

import argparse
from typing import Any

from fanout import parsed_server_roundtrip, server_roundtrip
from http1 import HTTPRequest, HTTPResponse, remove_request_header
...
def doesnt_support_version(server: Service) -> bool:
    pts1, _ = parsed_server_roundtrip(
        [b"GET / HTTP/1.1\r\nHost: a\r\n\r\n"], server, traced=False
    )
    ...
    pts2, _ = parsed_server_roundtrip(
        [b"GET / HTTP/1.0\r\nHost: a\r\n\r\n"], server, traced=False
    )
