# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\tools\http1.py
...
def parse_headers(
    ...
    for line in header_lines:
        header_match: re.Match[bytes] | None = re.match(
            rb"\A(?P<name>.+):\s+(?P<value>.*?)\s*\Z", line
        )
...
def remove_request_header(
    req: HTTPRequest, key: bytes, value: bytes | None = None
) -> HTTPRequest:
    result: HTTPRequest = copy.deepcopy(req)
    result.headers = [
        h for h in req.headers if h[0].lower() != key.lower() and h[1] != value
    ]
    return result
