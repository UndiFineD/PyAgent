# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\images\uwsgi\server.py
...
def application(environ, start_response) -> list[bytes]:
    response_body: bytes = (
        ...
        + b",".join(
            b'["'
            + b64encode(
                k.encode("latin1")[len("HTTP_") if k not in RESERVED_HEADERS else 0 :]
            )
            + b'","'
        ...
        + b64encode(
            (
                environ["PATH_INFO"]
                + (("?" + environ["QUERY_STRING"]) if environ["QUERY_STRING"] else "")
            ).encode("latin1")
        )
        + b'","method":"'
