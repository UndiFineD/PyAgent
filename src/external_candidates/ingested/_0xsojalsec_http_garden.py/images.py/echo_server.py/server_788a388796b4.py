# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\images\echo_server\server.py
...
def handle_connection(
    client_sock: socket.socket, _client_address: tuple[str, int]
) -> None:
...
def main() -> None:
    ...
    while True:
        t: threading.Thread = threading.Thread(
            target=handle_connection, args=(*server_sock.accept(),)
        )
        t.start()
