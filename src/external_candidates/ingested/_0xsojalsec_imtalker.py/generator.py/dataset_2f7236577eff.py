# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMTalker\generator\dataset.py
import random
    ...
    def __getitem__(self, index):
        try:
            (
                ...
                cam_clip,
                cam_prev,
            ) = self._get_full_clip(index)
        except Exception as e:
            print(
                f"[Error] Failed to get clip for index {index}: {e}. Trying a random sample."
            )
