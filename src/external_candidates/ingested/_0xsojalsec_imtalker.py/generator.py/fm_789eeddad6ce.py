# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMTalker\generator\FM.py
import math
    ...
    def sample(self, data, a_cfg_scale=1.0, nfe=10, seed=None):
        ...
        for t in range(num_chunks):
            ...
            if self.opt.fix_noise_seed:
                ...
                x0 = torch.randn(
                    B, self.num_frames_for_clip, self.opt.dim_w, device=device
                )

            # Setup Previous Context
            if t == 0:
                prev_x_t = torch.zeros(
                    B, self.num_prev_frames, self.opt.dim_c, device=device
                )
                prev_a_t = torch.zeros(
                    B, self.num_prev_frames, self.opt.dim_w, device=device
                )
                prev_gaze_t = torch.zeros(
                    B, self.num_prev_frames, gaze.shape[-1], device=device
                )
                prev_pose_t = torch.zeros(
                    B, self.num_prev_frames, pose.shape[-1], device=device
                )
                prev_cam_t = torch.zeros(
                    B, self.num_prev_frames, cam.shape[-1], device=device
                )
            else:
