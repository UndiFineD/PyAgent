    ...
    def update(self):
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                new_average = (
                    1.0 - self.decay
                ) * param.data + self.decay * self.shadow[name].to(param.device)
...
class System(pl.LightningModule):
    ...
    def load_ckpt(self, ckpt_path):
        ...
        if "ema_state_dict" in ckpt:
            print(
                "[INFO] Found EMA weights in checkpoint. Loading EMA weights for better stability."
            )
    ...
    def configure_optimizers(self):
        ...
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            opt, T_max=self.opt.iter, eta_min=1e-5
        )
...
class DataModule(pl.LightningDataModule):
    ...
    def val_dataloader(self):
        return data.DataLoader(
            self.val_dataset, num_workers=0, batch_size=8, shuffle=False
        )
