...
class FlowMatchingTransformer(nn.Module):
    ...
    def sequence_embedder(
        self, sequence: torch.Tensor, dropout_prob: float, train: bool = False
    ) -> torch.Tensor:
        if train:
    ...
    def forward(
        ...
        **kwargs,
        ...
        a = self.sequence_embedder(
            a, dropout_prob=self.opt.audio_dropout_prob, train=train
        )
        pose = self.sequence_embedder(
            pose, dropout_prob=self.opt.audio_dropout_prob, train=train
        )
        cam = self.sequence_embedder(
            cam, dropout_prob=self.opt.audio_dropout_prob, train=train
        )
        gaze = self.sequence_embedder(
            gaze, dropout_prob=self.opt.audio_dropout_prob, train=train
        )
