# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMTalker\renderer\inference.py
import argparse
import os
...
class Demo(nn.Module):
    ...
    def process_single(self, source_path, driving_path):
        ...
        source_tensor = (
            self.processor.transform(source_img).unsqueeze(0).to(self.device)
        )
        ...
        while True:
            ...
            frame_tensor = (
                self.processor.transform(frame_pil).unsqueeze(0).to(self.device)
            )
    ...
    def run(self):
        if self.args.source_path and self.args.driving_path:
            ...
            raise ValueError(
                "Usage: Provide either --source_path & --driving_path OR --data_dir"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir", type=str, help="Batch directory containing subfolders"
    )
    parser.add_argument(
        "--save_path", type=str, default="./results", help="Output directory"
    )
    ...
    # Options
    parser.add_argument(
        "--fps", type=int, default=None, help="Output FPS (default: same as input)"
    )
    parser.add_argument(
        "--crop", action="store_true", help="Crop face from source image"
    )
