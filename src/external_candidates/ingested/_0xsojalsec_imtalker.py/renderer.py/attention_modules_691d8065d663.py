# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMTalker\renderer\attention_modules.py
import torch
...
class GuidedResampler(nn.Module):
    ...
    def forward(self, v_high_feat, coarse_attn_map):
        ...
        topk_values, topk_indices_low = torch.topk(
            coarse_attn_map, k=self.k_samples, dim=-1
        )
        ...
        topleft = torch.stack(
            [topk_indices_high_topleft_row, topk_indices_high_topleft_col], dim=-1
        )
        ...
        low_res_grid_indices = (high_res_q_coords[:, 0] // self.ratio) * W_low + (
            high_res_q_coords[:, 1] // self.ratio
        )

        K_sparse_len = sparse_indices_1d.shape[-1]
        low_res_grid_indices_expanded = low_res_grid_indices.view(1, N_high, 1).expand(
            B, -1, K_sparse_len
        )

        final_sparse_indices = torch.gather(
            sparse_indices_1d, 1, low_res_grid_indices_expanded
        )
        ...
        low_res_grid_indices_weights_expanded = low_res_grid_indices.view(
            1, N_high, 1
        ).expand(B, -1, self.k_samples)
        weights_high = torch.gather(
            normalized_weights_low, 1, low_res_grid_indices_weights_expanded
        )

        v_reshaped = v_sparse_seq.view(B, N_high, self.k_samples, self.ratio**2, C)

        weights_for_broadcast = (weights_high / (self.ratio**2)).view(
            B, N_high, self.k_samples, 1, 1
        )
...
class SwinUnifiedAttention(nn.Module):
    def __init__(
        self, dim, num_heads, window_size, qkv_bias=True, attn_drop=0.0, proj_drop=0.0
    ):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
    ...
    def __init__(
        ...
        if self.shift_size > 0:
            ...
            mask_windows = window_partition(
                img_mask.view(1, H, W, 1), self.window_size
            ).view(-1, self.window_size * self.window_size)
    ...
    def forward(self, query, key=None, value=None):
        ...
        if self.shift_size > 0:
            shifted_q, shifted_k, shifted_v = [
                torch.roll(t, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
                for t in (q, k, v)
            ]
        else:
...
class SelfAttention(nn.Module):
    def __init__(self, args, dim, resolution):
        ...
        if resolution[0] >= args.swin_res_threshold:
            self.blocks.append(
                UnifiedSwinBlock(
                    window_size=args.window_size, shift_size=0, **common_kwargs
                )
            )
            self.blocks.append(
                UnifiedSwinBlock(
