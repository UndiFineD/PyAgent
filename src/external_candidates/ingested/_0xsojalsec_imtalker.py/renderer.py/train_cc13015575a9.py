        ...
        if self.trainer.global_rank == 0 and batch_idx == 0:
            ...
            for name, img in zip(name_list, img_list):
                self.logger.experiment.add_images(
                    tag=name, img_tensor=img.clamp(0, 1), global_step=self.global_step
                )
        ...
        for k, v in ckpt_dict_noprefix.items():
            if k in my_state_dict:
                if v.shape == my_state_dict[k].shape:
                    ...
                    print(
                        f"[WARN] Shape mismatch for {k}: ckpt {v.shape} vs model {my_state_dict[k].shape}"
                    )
...
if __name__ == "__main__":
    ...
    # Dataset
    parser.add_argument(
        "--dataset_path", type=str, required=True, help="Path to the dataset root"
    )
