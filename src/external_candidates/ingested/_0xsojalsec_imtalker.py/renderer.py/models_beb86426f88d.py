# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMTalker\renderer\models.py
import torch
import torch.nn as nn
from renderer.attention_modules import CrossAttention, SelfAttention
from renderer.lia_resblocks import EqualConv2d, EqualLinear, StyledConv
from renderer.modules import ConvResBlock, DownConvResBlock, ResBlock, UpConvResBlock
...
class IdentityEncoder(nn.Module):
    def __init__(
        self,
        in_channels=3,
        output_channels=[64, 128, 256, 512, 512, 512],
        initial_channels=32,
        dm=512,
    ):
        ...
        self.initial_conv = nn.Sequential(
            nn.Conv2d(
                in_channels, initial_channels, kernel_size=7, stride=1, padding=3
            ),
        ...
        self.equalconv = EqualConv2d(
            output_channels[-1], output_channels[-1], kernel_size=3, stride=1, padding=1
        )
        self.linear_layers = nn.ModuleList(
            [EqualLinear(output_channels[-1], output_channels[-1]) for _ in range(4)]
        )
...
class MotionEncoder(nn.Module):
    def __init__(
        self, initial_channels=64, output_channels=[64, 128, 256, 512, 512, 512], dm=32
    ):
...
class IMTRenderer(nn.Module):
    def __init__(self, args):
        ...
        self.latent_token_encoder = MotionEncoder(
            initial_channels=64, output_channels=[128, 256, 512, 512, 512]
        )
        self.latent_token_decoder = MotionDecoder()
        self.frame_decoder = SynthesisNetwork(
            args, self.feature_dims, self.spatial_dims
        )
        ...
        for dim, s_dim in zip(self.feature_dims[::-1], self.spatial_dims[::-1]):
            self.imt.append(
                CrossAttention(args=args, dim=dim, resolution=(s_dim, s_dim))
            )
