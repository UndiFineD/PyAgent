# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\chat\sglang_engine.py
...
class SGLangEngine(BaseEngine):
    ...
    def __init__(
        ...
        try:
            ...
            logger.info_rank0(
                f"Waiting for SGLang server to be ready at {self.base_url}"
            )
            wait_for_server(self.base_url, timeout=300)
            logger.info_rank0(
                f"SGLang server initialized successfully at {self.base_url}"
            )
            try:
    ...
    async def _generate(
        ...
        **input_kwargs,
        ...
        if images is not None and not any(
            IMAGE_PLACEHOLDER in message["content"] for message in messages
        ):
            messages[0]["content"] = (
                IMAGE_PLACEHOLDER * len(images) + messages[0]["content"]
            )

        if videos is not None and not any(
            VIDEO_PLACEHOLDER in message["content"] for message in messages
        ):
            messages[0]["content"] = (
                VIDEO_PLACEHOLDER * len(videos) + messages[0]["content"]
            )

        if audios is not None and not any(
            AUDIO_PLACEHOLDER in message["content"] for message in messages
        ):
            messages[0]["content"] = (
                AUDIO_PLACEHOLDER * len(audios) + messages[0]["content"]
            )
        ...
        skip_special_tokens: Optional[bool] = input_kwargs.pop(
        ...
        def stream_request():
            ...
            response = requests.post(
                f"{self.base_url}/generate", json=json_data, stream=True
            )
            if response.status_code != 200:
                raise RuntimeError(
        ...
        **input_kwargs,
        ...
        results = [
            Response(
                ...
                finish_reason=(
                    "stop"
                    if final_output["meta_info"]["finish_reason"] == "stop"
                    else "length"
                ),
            )
        ]
