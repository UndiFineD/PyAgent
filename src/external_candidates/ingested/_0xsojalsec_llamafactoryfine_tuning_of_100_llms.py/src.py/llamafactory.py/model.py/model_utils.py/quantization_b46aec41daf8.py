# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\model\model_utils\quantization.py
...
def configure_quantization(
    ...
    if getattr(config, "quantization_config", None):  # ptq
        ...
        logger.info_rank0(
            f"Loading {quant_bits}-bit {quant_method.upper()}-quantized model."
        )
        ...
        logger.info_rank0(
            f"Quantizing model to {model_args.export_quantization_bit} bit with GPTQModel."
        )
