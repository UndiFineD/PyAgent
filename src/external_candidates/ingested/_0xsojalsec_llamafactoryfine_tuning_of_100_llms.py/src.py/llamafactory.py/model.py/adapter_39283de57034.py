# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\model\adapter.py
...
def _setup_freeze_tuning(
    ...
    if (
        not finetuning_args.freeze_multi_modal_projector
        and model_type in COMPOSITE_MODELS
    ):
...
def init_adapter(
    ...
    if not is_trainable:
        ...
        logger.info_rank0(
            "DeepSpeed ZeRO3 detected, remaining trainable params in float32."
        )
    else:
