# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\model\patcher.py
...
def patch_qwen3_omni_moe_thinker_text_sparse_moe_block():
    if is_transformers_version_greater_than(
        "4.57.0"
    ) and not is_transformers_version_greater_than("4.58.0"):
        ...
        modeling_qwen3_omni_moe.Qwen3OmniMoeThinkerTextSparseMoeBlock = (
...
def patch_config(
    ...
    if getattr(
        config, "model_type", None
    ) == "lfm2_vl" and not is_transformers_version_greater_than("4.58.0"):
        raise RuntimeError(
    ...
    if (
        not (is_deepspeed_zero3_enabled() or is_fsdp_enabled())
        and init_kwargs["low_cpu_mem_usage"]
    ):
...
def patch_model(
    ...
    if model_args.resize_vocab:
        resize_embedding_layer(
            model,
            tokenizer,
            new_special_tokens_config=getattr(
                model_args, "_special_token_descriptions", None
            ),
...
def patch_valuehead_model(model: "AutoModelForCausalLMWithValueHead") -> None:
    ...
    def get_rope_index_func(self: "AutoModelForCausalLMWithValueHead"):
        ...
        if base_model and hasattr(base_model, "get_rope_index"):
            ...
            and hasattr(base_model.model, "get_rope_index")
        ):
