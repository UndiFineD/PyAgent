...
def load_model(
    ...
    if model is None and not lazy_load:
        ...
        if model_args.mixture_of_depths == "load":
            ...
            if (
                type(config) in AutoModelForImageTextToText._model_mapping.keys()
            ):  # image-text
                ...
                type(config) in AutoModelForSeq2SeqLM._model_mapping.keys()
            ):  # audio-text
    ...
    if is_torch_version_greater_than("2.9.0") and not is_torch_version_greater_than(
        "2.10.0"
    ):
