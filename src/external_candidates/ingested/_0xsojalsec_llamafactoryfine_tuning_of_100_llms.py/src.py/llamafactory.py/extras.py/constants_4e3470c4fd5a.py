# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\extras\constants.py
...
def register_model_group(
    ...
    for name, path in models.items():
        ...
        if template is not None and (
            any(
                suffix in name
                for suffix in ("-Chat", "-Distill", "-Instruct", "-Thinking")
            )
            or multimodal
        ):
