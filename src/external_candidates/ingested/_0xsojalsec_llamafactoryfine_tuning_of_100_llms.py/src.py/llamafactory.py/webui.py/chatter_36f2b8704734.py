...
def _format_response(
    text: str, lang: str, escape_html: bool, thought_words: tuple[str, str]
) -> str:
...
class WebChatModel(ChatModel):
    def __init__(
        ...
        if (
            demo_mode and os.getenv("DEMO_MODEL") and os.getenv("DEMO_TEMPLATE")
        ):  # load demo model
    ...
    def load_model(self, data) -> Generator[str, None, None]:
        ...
        args = dict(
            ...
            rope_scaling=(
                get("top.rope_scaling") if get("top.rope_scaling") != "none" else None
            ),
        ...
        with update_attr(self.engine.template, "enable_thinking", enable_thinking):
            ...
            for new_text in self.stream_chat(
                ...
                if isinstance(result, list):
                    tool_calls = [
                        {"name": tool.name, "arguments": json.loads(tool.arguments)}
                        for tool in result
                    ]
                    ...
                    bot_text = _format_response(
                        result, lang, escape_html, self.engine.template.thought_words
                    )
