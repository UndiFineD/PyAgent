# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\webui\components\top.py
...
def create_top() -> dict[str, "Component"]:
    with gr.Row():
        ...
        default_hub = (
            "modelscope"
            if use_modelscope()
            else "openmind" if use_openmind() else "huggingface"
        )
        hub_name = gr.Dropdown(
            choices=["huggingface", "modelscope", "openmind"],
            value=default_hub,
            scale=2,
        )
    ...
    model_path.input(
        save_config, inputs=[lang, hub_name, model_name, model_path], queue=False
    )
