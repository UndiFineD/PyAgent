# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\data\processor\feedback.py
...
class FeedbackDatasetProcessor(DatasetProcessor):
    def _encode_data_example(
        ...
        kl_messages = self.template.mm_plugin.process_messages(
            kl_messages, images, videos, audios, self.processor
        )
        prompt_ids, response_ids = self.template.encode_oneturn(
            self.tokenizer, messages, system, tools
        )
        kl_prompt_ids, kl_response_ids = self.template.encode_oneturn(
            self.tokenizer, kl_messages, system, tools
        )
        ...
        source_len, target_len = infer_seqlen(
            len(prompt_ids), len(response_ids), self.data_args.cutoff_len
        )
    ...
    def preprocess_dataset(
        ...
        for i in range(len(examples["_prompt"])):
            if (
                len(examples["_prompt"][i]) % 2 != 1
                or len(examples["_response"][i]) < 2
            ):
            ...
            input_ids, labels, kl_input_ids, kl_labels, kto_tag = (
