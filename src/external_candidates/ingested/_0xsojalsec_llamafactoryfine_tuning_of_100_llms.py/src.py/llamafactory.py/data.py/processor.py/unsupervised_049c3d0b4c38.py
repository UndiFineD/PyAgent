# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\data\processor\unsupervised.py
...
class UnsupervisedDatasetProcessor(DatasetProcessor):
    def _encode_data_example(
        ...
        source_len, target_len = infer_seqlen(
            len(input_ids), len(labels), self.data_args.cutoff_len
        )
    ...
    def print_data_example(self, example: dict[str, list[int]]) -> None:
        ...
        print(
            "labels:\n{}".format(
                self.tokenizer.decode(example["labels"], skip_special_tokens=False)
            )
        )
