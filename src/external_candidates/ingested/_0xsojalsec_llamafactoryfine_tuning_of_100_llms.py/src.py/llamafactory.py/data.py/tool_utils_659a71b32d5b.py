# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\data\tool_utils.py
    ...
    def tool_extractor(content: str) -> Union[str, list["FunctionCall"]]:
        ...
        for match in action_match:
            ...
            try:
                ...
                results.append(
                    FunctionCall(tool_name, json.dumps(arguments, ensure_ascii=False))
                )
...
class Llama3ToolUtils(ToolUtils):
    ...
    def function_formatter(functions: list["FunctionCall"]) -> str:
        ...
        return json.dumps(
            function_objects[0] if len(function_objects) == 1 else function_objects,
            ensure_ascii=False,
        )
    ...
    def tool_extractor(content: str) -> Union[str, list["FunctionCall"]]:
        ...
        try:
            return [
                FunctionCall(
                    tool["name"], json.dumps(tool["parameters"], ensure_ascii=False)
                )
                for tool in tools
            ]
        except KeyError:
...
class MiniMaxM2ToolUtils(ToolUtils):
    ...
    def tool_extractor(content: str) -> Union[str, list["FunctionCall"]]:
        ...
        for func_name, params_block in re.findall(invoke_regex, tool_calls_content):
            ...
            results.append(
                FunctionCall(
                    func_name.strip(), json.dumps(args_dict, ensure_ascii=False)
                )
            )
...
class MistralToolUtils(ToolUtils):
    ...
    def tool_formatter(tools: list[dict[str, Any]]) -> str:
        ...
        return (
            "[AVAILABLE_TOOLS] "
            + json.dumps(wrapped_tools, ensure_ascii=False)
            + "[/AVAILABLE_TOOLS]"
        )
    ...
    def tool_extractor(content: str) -> Union[str, list["FunctionCall"]]:
        ...
        try:
            return [
                FunctionCall(
                    tool["name"], json.dumps(tool["arguments"], ensure_ascii=False)
                )
                for tool in tools
            ]
        except KeyError:
...
class QwenToolUtils(ToolUtils):
    ...
    def function_formatter(functions: list["FunctionCall"]) -> str:
        function_texts = [
            json.dumps(
                {"name": name, "arguments": json.loads(arguments)}, ensure_ascii=False
            )
            for name, arguments in functions
        ]
        return "\n".join(
...
class SeedToolUtils(ToolUtils):
    ...
    @staticmethod
    def tool_formatter(tools: list[dict[str, Any]]) -> str:
        return SEED_TOOL_PROMPT.format(
...
class LingToolUtils(QwenToolUtils):
    ...
    def tool_formatter(tools: list[dict[str, Any]]) -> str:
        ...
        return (
            LING_TOOL_PROMPT.format(tool_text=tool_text)
            + "\n"
...
class LFM2ToolUtils(ToolUtils):
    ...
    def tool_formatter(tools: list[dict[str, Any]]) -> str:
        ...
        for tool in tools:
            tool = (
                tool.get("function", tool) if tool.get("type") == "function" else tool
            )
            tool_list.append(tool)

        return LFM2_TOOL_PROMPT.format(
            tool_text=json.dumps(tool_list, ensure_ascii=False)
        )
    ...
    def tool_extractor(content: str) -> Union[str, list["FunctionCall"]]:
        ...
        for node in call_nodes:
            ...
            results.append(
                FunctionCall(func_name, json.dumps(args_dict, ensure_ascii=False))
            )
