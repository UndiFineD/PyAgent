    ...
    def process_messages(
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                image_seqlen = (
                    image_grid_thw[image_idx].prod() // merge_length
                    if self.expand_mm_tokens
                    else 1
                )
            ...
            while VIDEO_PLACEHOLDER in content:
                video_seqlen = (
                    video_grid_thw[video_idx].prod() // merge_length
                    if self.expand_mm_tokens
                    else 1
                )
                content = content.replace(
...
class PixtralPlugin(BasePlugin):
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                if self.expand_mm_tokens:
                    patch_size = processor.patch_size * getattr(
...
class Qwen2VLPlugin(BasePlugin):
    ...
    def _get_mm_inputs(
        ...
        video_processor: BaseVideoProcessor = getattr(
        ...
        if len(videos) != 0:
            ...
            mm_inputs.update(
                video_processor(videos=video_data["videos"], return_tensors="pt")
            )
...
class Qwen3VLPlugin(Qwen2VLPlugin):
    ...
    def process_messages(
        ...
        for idx, message in enumerate(messages):
            ...
            while IMAGE_PLACEHOLDER in content:
                image_seqlen = (
                    image_grid_thw[num_image_tokens].prod() // image_merge_length
                    if self.expand_mm_tokens
                    else 1
                )
                content = content.replace(
            ...
            while VIDEO_PLACEHOLDER in content:
                if self.expand_mm_tokens:
                    ...
                    for frame_index in range(num_frames):
                        video_seqlen = (
                            video_grid_thw[num_video_tokens][1:].prod()
                            // video_merge_length
                            if self.expand_mm_tokens
                            else 1
                        )
    ...
    def _get_mm_inputs(
        ...
        if len(videos) != 0:
            ...
            video_metadata = [
                ...
                for video, duration in zip(
                    video_data["videos"], video_data["durations"]
                )
            ]
            mm_inputs.update(
                video_processor(
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            while VIDEO_PLACEHOLDER in content:
                ...
                for frame_index in range(num_frames):
                    video_seqlen = (
                        video_grid_thw[num_video_tokens][1:].prod() // merge_length
                        if self.expand_mm_tokens
                        else 1
                    )
...
class Qwen2OmniPlugin(Qwen2VLPlugin):
    ...
    def _get_mm_inputs(
        ...
        if len(videos) != 0:
            ...
            mm_inputs.update(
                video_processor(videos=video_dict["videos"], return_tensors="pt")
            )
            temporal_patch_size: int = getattr(
                image_processor, "temporal_patch_size", 2
            )
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            if (
                ...
                while AUDIO_PLACEHOLDER in content:
                    audio_seqlen = (
                        audio_lengths[num_audio_tokens] if self.expand_mm_tokens else 1
                    )
                    content = content.replace(
                        AUDIO_PLACEHOLDER,
