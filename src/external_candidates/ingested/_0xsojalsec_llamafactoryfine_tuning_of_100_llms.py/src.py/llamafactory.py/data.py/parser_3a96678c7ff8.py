# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\data\parser.py
...
class DatasetAttr:
    ...
    def set_attr(
        self, key: str, obj: dict[str, Any], default: Any | None = None
    ) -> None:
...
def get_dataset_list(
    dataset_names: list[str] | None, dataset_dir: str | dict
) -> list["DatasetAttr"]:
    ...
    for name in dataset_names:
        if dataset_info is None:  # dataset_dir is ONLINE
            load_from = (
                "ms_hub"
                if use_modelscope()
                else "om_hub" if use_openmind() else "hf_hub"
            )
        ...
        if has_hf_url or has_ms_url or has_om_url:
            ...
            dataset_attr = DatasetAttr(
                "cloud_file", dataset_name=dataset_info[name]["cloud_file_name"]
            )
        else:
            dataset_attr = DatasetAttr(
