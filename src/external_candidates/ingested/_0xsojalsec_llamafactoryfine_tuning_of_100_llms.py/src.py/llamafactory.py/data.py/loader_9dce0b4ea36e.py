# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\data\loader.py
...
def _load_single_dataset(
    ...
    if dataset_attr.load_from == "ms_hub":
        ...
        dataset = Dataset.from_list(
            read_cloud_json(data_path), split=dataset_attr.split
        )
    else:
...
def _get_merged_dataset(
    ...
    if return_dict:
        ...
        return merge_dataset(
            list(datasets.values()), data_args, seed=training_args.seed
        )
...
def get_dataset(
    ...
    if data_args.tokenized_path is not None:
        if has_tokenized_data(data_args.tokenized_path):
            ...
            if data_args.streaming:
                dataset_module["train_dataset"] = dataset_module[
                    "train_dataset"
                ].to_iterable_dataset()

            logger.info_rank0(
                f"Loaded tokenized dataset from {data_args.tokenized_path}."
            )
    ...
    with training_args.main_process_first(
        ...
        train_dict, eval_dict = split_dataset(
        ...
        if data_args.tokenized_path is not None:  # save tokenized dataset to disk
            if training_args.should_save:
                ...
                logger.info_rank0(
                    f"Tokenized dataset is saved at {data_args.tokenized_path}."
                )
                logger.info_rank0(
