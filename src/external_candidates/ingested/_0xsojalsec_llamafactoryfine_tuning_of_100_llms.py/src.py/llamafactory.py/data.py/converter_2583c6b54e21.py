# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\data\converter.py
...
class OpenAIDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            ...
            system = (
                example.get(self.dataset_attr.system, "")
                if self.dataset_attr.system
                else ""
            )
        ...
        for turn_idx, message in enumerate(messages):
            ...
            if role in [
                ...
                if "tool_calls" in message and len(message["tool_calls"]) > 0:
                    tool_calls_list = [
                        tool["function"] for tool in message["tool_calls"]
                    ]
        ...
        tools = (
            example.get(self.dataset_attr.tools, "") if self.dataset_attr.tools else ""
        )
        ...
        if not system:
            ...
            if not tools:
                if (
                    "detailed thinking on" in system
                    or "detailed thinking off" in system
                ):
                    pass
                else:
                    system += "\n" + short_system_prompt
