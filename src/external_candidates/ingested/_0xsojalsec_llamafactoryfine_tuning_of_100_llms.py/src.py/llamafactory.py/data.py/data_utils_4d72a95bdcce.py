# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\data\data_utils.py
        ...
        if isinstance(eval_dataset, dict):
            ...
            if data_args.streaming:
                eval_dataset = eval_dataset.shuffle(
                    buffer_size=data_args.buffer_size, seed=seed
                )
...
def setup_fs(path: str, anon: bool = False) -> "fsspec.AbstractFileSystem":
    ...
    if path.startswith("s3://"):
        ...
        raise ValueError(
            f"Unsupported protocol in path: {path}. Use 's3://' or 'gs://'."
        )
...
def read_cloud_json(cloud_path: str) -> list[Any]:
    ...
    files = filter(
        lambda file: file.endswith(".json") or file.endswith(".jsonl"), files
    )
    if not files:
        raise ValueError(
            f"No JSON/JSONL files found in the specified path: {cloud_path}."
        )
