...
def parse_chatml_message(generated_text: str) -> Message:
    ...
    """
    return Message(
        role="assistant", content=[{"type": "text", "value": generated_text}]
    )
    ...
    def render_messages(
        ...
        if self.template == "chatml":
            ...
            return RenderingPlugin(self.template).render_messages(
                self.processor, messages, tools, is_generate
            )
    ...
    def process_samples(self, samples: list[Sample]) -> list[ModelInput]:
        ...
        for sample in samples:
            if "messages" in sample:
                model_input = self.render_messages(
                    sample["messages"], sample.get("tools")
                )
            elif "chosen_messages" in sample and "rejected_messages" in sample:
                chosen_input = self.render_messages(
                    sample["chosen_messages"], sample.get("tools")
                )
                rejected_input = self.render_messages(
                    sample["rejected_messages"], sample.get("tools")
                )
                chosen_input["token_type_ids"] = [1] * len(chosen_input["input_ids"])
                rejected_input["token_type_ids"] = [2] * len(
                ...
                model_input = ModelInput(
                    ...
                    + rejected_input["attention_mask"],
                    labels=chosen_input["labels"] + rejected_input["labels"],
                    loss_weights=chosen_input["loss_weights"]
                    + rejected_input["loss_weights"],
                    token_type_ids=chosen_input["token_type_ids"]
                    + rejected_input["token_type_ids"],
                )
