# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\v1\core\data_engine.py
...
class DataEngine(Dataset):
    ...
    def _load_dataset(self) -> None:
        ...
        is_streaming = [
            dataset_info.get("streaming", False)
            for dataset_info in self.dataset_infos.values()
        ]
        ...
        for dataset_name, dataset_info in self.dataset_infos.items():
            ...
            if dataset_info.get("source", "hf_hub") == "hf_hub":
                ...
                self.datasets[dataset_name] = load_dataset(
                    dataset_info["path"], split=split, streaming=self.streaming
                )
            else:  # data loader plugin
                from ..plugins.data_plugins.loader import DataLoaderPlugin

                self.datasets[dataset_name] = DataLoaderPlugin(
                    dataset_info["source"]
                ).load(dataset_info)
    ...
    def _build_data_index(self) -> None:
        ...
        for dataset_name, dataset in self.datasets.items():
            if self.streaming:
                ...
                data_index = [
                    (dataset_name, sample_index) for sample_index in range(len(dataset))
                ]
    ...
    def _convert_data_sample(
        self, raw_sample: dict[str, Any], dataset_name: str
    ) -> Sample:
    ...
    def __getitem__(self, index: int | Any) -> Sample | list[Sample]:
        ...
        if isinstance(index, int):
            ...
            return self._convert_data_sample(
                self.datasets[dataset_name][sample_index], dataset_name
            )
