# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\v1\core\utils\batching.py
...
def default_collate_fn(
    buffer: StatefulBuffer, batch_info: BatchInfo
...
class BatchGenerator(Iterator):
    ...
    def _fill_buffer(self) -> None:
        if self.batching_strategy == BatchingStrategy.NORMAL:
            ...
            BatchingPlugin(self.batching_strategy).fill_buffer(
                self._buffer, self._batch_info
            )
    ...
    def _generate_batch(self) -> list[BatchInput] | None:
        if self.batching_strategy == BatchingStrategy.NORMAL:
            ...
            return BatchingPlugin(self.batching_strategy).generate_batch(
                self._buffer, self._batch_info
            )
