# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\v1\core\base_trainer.py
...
class BaseTrainer:
    def __init__(
        ...
        self.num_training_steps = self.args.num_train_epochs * len(
    ...
    def compute_log_probs(self, model: HFModel, batch: BatchInput) -> Tensor:
        ...
        model_inputs = {
            ...
            if k in self.model_input_names
        }
        ...
        return -F.cross_entropy(shift_logits, shift_labels, reduction="none").view(
            batch_size, -1
        )
    ...
    def fit(self) -> None:
        ...
        for epoch in range(self.args.num_train_epochs):
            ...
            for micro_batches in self.train_batch_generator:
                ...
                step_valid_tokens = DistributedInterface().all_reduce(
                    step_valid_tokens, op=ReduceOp.SUM
                )
                for micro_batch in micro_batches:
                    ...
                    loss = (
                        loss
                        * mini_step_valid_tokens
                        * self.dp_size
                        / (step_valid_tokens + 1e-6)
                    )
                ...
                step_loss, grad_norm = DistributedInterface().all_reduce(
                    [step_loss, grad_norm]
                )
                DistributedInterface().sync()
                print(
                    f"Epoch {epoch}, Step {self.global_step}, Loss: {step_loss:.4f}, Grad Norm: {grad_norm:.4f}"
                )
