# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\v1\plugins\model_plugins\rendering.py
...
def render_qwen3_nothink_messages(
    ...
    temp_str = _update_model_input(
        processor, input_ids, labels, loss_weights, temp_str, temp_weight
    )
        ...
        if message["role"] == "user" or (message["role"] == "system" and turn_idx != 0):
            ...
            for val_idx, content in enumerate(message["content"]):
                if content["type"] == "text":
                    ...
                    if val_idx != 0 and message["content"][val_idx - 1]["type"] in [
                        "text",
                        "tool_call",
                    ]:
                        temp_str += "\n"
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Invalid tool call format: {content['value']}."
                        )
            ...
            if (
                ...
                or messages[turn_idx + 1]["role"] != "tool"
            ):
                temp_str += "<|im_end|>\n"
