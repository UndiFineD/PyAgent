# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\v1\plugins\model_plugins\kernels\ops\mlp\npu_swiglu.py
...
def _npu_swiglu_glm4_forward(self, hidden_states):
    ...
    return self.down_proj(
        torch_npu.npu_swiglu(torch.cat((gate, up_states), dim=-1), dim=-1)
    )
...
def _npu_swiglu_gemma3ntext_forward(self, hidden_states):
    ...
    down_proj = self.down_proj(
        torch_npu.npu_swiglu(
            torch.cat((gate_proj, self.up_proj(hidden_states)), dim=-1), dim=-1
        )
    )
    return down_proj
    ...
    def apply(cls, **kwargs) -> "HFModel":
        ...
        for name, module in model.named_modules():
            ...
            if (
                ...
                kernel_func = kernel_mapping.get(
                    module.__class__.__name__, npu_swiglu_forward
                )
