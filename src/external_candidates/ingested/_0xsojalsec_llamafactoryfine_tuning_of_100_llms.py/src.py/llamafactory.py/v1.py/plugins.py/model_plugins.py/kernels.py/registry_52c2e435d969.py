# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\v1\plugins\model_plugins\kernels\registry.py
...
class Registry:
    ...
    def register(cls, kernel_cls: type[BaseKernel]) -> type[BaseKernel] | None:
        ...
        if not kernel_id:
            raise ValueError(
                f"Kernel ID (_kernel_id) is needed for {kernel_cls} to register"
            )

        if kernel_id in cls._kernels:
            raise ValueError(
                f"{kernel_id} already registered! The registered kernel is {cls._kernels[kernel_id]}"
            )
