...
class HybridGmmFunction(torch.autograd.Function):
    ...
    def backward(ctx, *grad_outputs):
        ...
        group_list = torch.tensor(
            split_sizes, device=x_concat.device, dtype=torch.int64
        )
...
class NpuMoeFused:
    ...
    def npu_moe_experts_forward(
        ...
        intermediate_hidden_states = GmmFunction.apply(
            permuted_hidden_states, self.gate_up_proj, tokens_per_expert
        )
        intermediate_activations = torch_npu.npu_swiglu(
            intermediate_hidden_states, dim=-1
        )
        output = GmmFunction.apply(
...
class Qwen3NpuMoeFused:
    ...
    def qwen3moe_sparse_moe_block_forward(self, hidden_states: torch.Tensor):
        ...
        gate_out_tuple = HybridGmmFunction.apply(
            len(input_list), *input_list, *gate_weights
        )
        up_out_tuple = HybridGmmFunction.apply(
            len(input_list), *input_list, *up_weights
        )

        inter_list = [F.silu(g) * u for g, u in zip(gate_out_tuple, up_out_tuple)]

        down_out_tuple = HybridGmmFunction.apply(
            len(inter_list), *inter_list, *down_weights
        )
