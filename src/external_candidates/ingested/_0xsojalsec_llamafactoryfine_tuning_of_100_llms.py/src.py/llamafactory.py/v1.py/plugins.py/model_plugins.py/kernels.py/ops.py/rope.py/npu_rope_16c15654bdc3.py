...
def _apply_multimodal_rotary_pos_emb_qwen25_vl(
...
class NpuRoPEKernel(BaseKernel):
    ...
    def apply(cls, **kwargs) -> "HFModel":
        ...
        if not cls.check_deps():
            raise RuntimeError(
                f"torch_npu is not available but {cls.__name__} was called."
            )
            ...
            if "Attention" in module.__class__.__name__:
                ...
                try:
                    ...
                    if hasattr(target_module, "apply_rotary_pos_emb"):
                        if (
                            getattr(target_module, "apply_rotary_pos_emb")
                            is not _apply_rotary_pos_emb
                        ):
                            setattr(
                                target_module,
                    ...
                    logger.warning_rank0_once(
                        f"Failed to apply RoPE kernel to module {module_name}: {e}"
                    )
