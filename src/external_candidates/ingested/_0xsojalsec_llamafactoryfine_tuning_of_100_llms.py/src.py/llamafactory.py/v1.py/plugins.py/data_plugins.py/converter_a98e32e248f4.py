# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\v1\plugins\data_plugins\converter.py
...
SharegptMessage = TypedDict(
    "SharegptMessage",
    {
        "from": Literal["human", "gpt", "system", "function_call", "observation"],
        "value": str,
    },
)
        ...
        messages.append(
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "value": raw_sample.get("instruction", "")
        ...
        if tag not in tag_mapping:
            ...
            try:
                ...
                logger.warning_rank0(
                    f"Invalid tool call format: {str(message['value'])}"
                )
                continue
            ...
            messages.append(
                {
                    "role": "assistant",
                    "content": [
                        {"type": "tool_call", "value": json.dumps(tool_call)}
    ...
    def process_message(raw_messages: list[OpenaiMessage]):
        ...
        for message in raw_messages:
            if message["role"] == "tool":
                try:
                    tool_calls: ToolCall | list[ToolCall] = json.loads(
                        message["content"]
                    )
                except json.JSONDecodeError:
                    logger.warning_rank0(
                        f"Invalid tool call format: {str(message['content'])}"
                    )
                    continue
