# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\v1\utils\plugin.py
...
class BasePlugin:
    ...
    def register(self, method_name: str = "__call__") -> Callable:
        ...
        if method_name in self._registry[self.name]:
            logger.warning_rank0_once(
                f"Method {method_name} of plugin {self.name} is already registered."
            )
    ...
    def __getitem__(self, method_name: str) -> Callable:
        ...
        if method_name not in self._registry[self.name]:
            raise ValueError(
                f"Method {method_name} of plugin {self.name} is not registered."
            )
