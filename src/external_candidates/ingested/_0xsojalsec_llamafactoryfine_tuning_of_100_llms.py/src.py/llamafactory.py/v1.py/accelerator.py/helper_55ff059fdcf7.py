# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\v1\accelerator\helper.py
...
def get_current_device() -> torch.device:
    ...
    if get_current_accelerator().type == DeviceType.CPU:
        ...
        return torch.device(
            type=get_current_accelerator().type,
...
def operate_tensorlike(
    fn: Callable[[...], Tensor], data: TensorLike, **kwargs
) -> TensorLike:
...
def all_gather(tensor: Tensor, group: Optional[ProcessGroup] = None) -> Tensor:
    ...
    output_tensor = torch.empty(
        world_size * tensor.numel(), dtype=tensor.dtype, device=tensor.device
    )
...
def all_reduce(
    tensor: Tensor, op: ReduceOp = ReduceOp.MEAN, group: Optional[ProcessGroup] = None
) -> Tensor:
...
def broadcast(
    tensor: Tensor, src: int = 0, group: Optional[ProcessGroup] = None
) -> Tensor:
