# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\train\tuner.py
...
def _training_function(config: dict[str, Any]) -> None:
    ...
    if finetuning_args.stage in ["pt", "sft", "dpo"] and finetuning_args.use_mca:
        if not is_mcore_adapter_available():
            raise ImportError(
                "mcore_adapter is not installed. Please install it with `pip install mcore-adapter`."
            )
