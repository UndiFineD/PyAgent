# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\train\mca\workflow.py
...
if TYPE_CHECKING:
    from mcore_adapter.training_args import (
        Seq2SeqTrainingArguments as McaSeq2SeqTrainingArguments,
    )
...
def run_sft(
    ...
    data_args.neat_packing = training_args.sequence_packing = (
        data_args.neat_packing or training_args.sequence_packing
    )
    ...
    if getattr(model.config, "hf_model_type", None) in [
        "qwen2_vl",
        "qwen2_5_vl",
        "qwen3_vl",
    ]:
...
def run_dpo(
    ...
    if finetuning_args.use_ref_model:
        ref_config = AutoConfig.from_pretrained(
            model_args.model_name_or_path, training_args
        )
