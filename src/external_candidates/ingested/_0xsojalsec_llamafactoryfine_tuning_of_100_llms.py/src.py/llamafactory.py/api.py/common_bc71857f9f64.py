# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\api\common.py
...
SAFE_MEDIA_PATH = os.environ.get(
    "SAFE_MEDIA_PATH", os.path.join(os.path.dirname(__file__), "safe_media")
)
...
def check_lfi_path(path: str) -> None:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Invalid or inaccessible file path.",
        )
...
def check_ssrf_url(url: str) -> None:
    ...
    try:
        ...
        if not hostname:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid URL hostname."
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid URL: {e}"
        )
