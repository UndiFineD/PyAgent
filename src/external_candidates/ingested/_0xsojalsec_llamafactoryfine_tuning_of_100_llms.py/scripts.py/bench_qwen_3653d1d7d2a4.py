# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\scripts\bench_qwen.py
...
class DummyDataset(Dataset):
    def __init__(
        ...
        size: int = 1000,
        seq_length: int = 1024,
        processor: Qwen2_5_VLProcessor = None,
    ):
        self.size = size
        ...
        video_seq_length = (
            self.seq_length - self.text_seqlen - image_t * image_token_num
        )
    ...
    def __getitem__(self, index: int):
        ...
        input_ids[self.image_seqlen : self.image_seqlen + self.video_seqlen] = (
...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    def __post_init__(self):
        ...
        if self.model is not None and hasattr(
            ...
            and hasattr(self.model.model, "get_rope_index")
        ):
            self.get_rope_func = (
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        batch_pixel_values_videos = [
            feature.pop("pixel_values_videos") for feature in features
        ]
        ...
        if self.get_rope_func is not None:
            ...
            batch["position_ids"], batch["rope_deltas"] = self.get_rope_func(
                **rope_index_kwargs
            )
...
def bench_qwen(
    ...
    trainset = DummyDataset(
        size=100000, seq_length=seq_length, processor=tokenizer_module["processor"]
    )
