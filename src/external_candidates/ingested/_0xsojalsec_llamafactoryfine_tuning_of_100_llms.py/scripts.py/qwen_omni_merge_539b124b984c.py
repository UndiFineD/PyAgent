# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\scripts\qwen_omni_merge.py
...
def merge_lora(
    ...
    if not hasattr(model, submodule_name):
        raise AttributeError(
            f"The model does not have a submodule named '{submodule_name}'."
        )
...
def save_full_model(
    ...
    if getattr(config, "model_type") == "qwen2_5_omni":
        ...
        from transformers.models.qwen3_omni_moe import (
            Qwen3OmniMoeThinkerForConditionalGeneration,  # type: ignore
        )
    ...
    thinker = ThinkerClass.from_pretrained(
        thinker_path, torch_dtype="auto", device_map="cpu"
    )
