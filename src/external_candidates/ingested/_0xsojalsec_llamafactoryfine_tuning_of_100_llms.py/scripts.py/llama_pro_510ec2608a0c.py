# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\scripts\llama_pro.py
...
def block_expansion(
    ...
    if num_layers % num_expand != 0:
        raise ValueError(
            f"`num_layers` {num_layers} should be divisible by `num_expand` {num_expand}."
        )
    ...
    print(
        f"Expanding model of {num_layers} layers to {num_layers + num_expand} layers."
    )
            ...
            for key, value in state_dict.items():
                if f".{i:d}." in key:
                    if "down_proj" in key or "o_proj" in key:
                        output_state_dict[change_name(key, i, layer_cnt)] = (
                            torch.zeros_like(value)
                        )
                    else:
                        output_state_dict[change_name(key, i, layer_cnt)] = torch.clone(
                            value
                        )
    ...
    for shard_file, tensors in tqdm(
        state_dict_split.filename_to_tensors.items(), desc="Save weights"
    ):
