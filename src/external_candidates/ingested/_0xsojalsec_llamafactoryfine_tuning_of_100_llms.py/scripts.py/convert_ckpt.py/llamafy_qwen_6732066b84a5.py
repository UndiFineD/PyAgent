# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\scripts\convert_ckpt\llamafy_qwen.py
    ...
    for key, value in tqdm(qwen_state_dict.items(), desc="Convert format"):
        ...
        if "wte" in key:
            ...
            if "attn.c_attn" in key:
                ...
                llama_state_dict[key.replace("attn.c_attn", "self_attn.q_proj")] = (
                    value[:proj_size, ...]
                )
                llama_state_dict[key.replace("attn.c_attn", "self_attn.k_proj")] = (
                    value[proj_size : 2 * proj_size, ...]
                )
                llama_state_dict[key.replace("attn.c_attn", "self_attn.v_proj")] = (
                    value[2 * proj_size :, ...]
                )
                ...
                llama_state_dict[key.replace("ln_2", "post_attention_layernorm")] = (
                    value
                )
            elif "mlp.w1" in key:
