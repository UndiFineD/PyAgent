# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\scripts\stat_utils\length_cdf.py
...
def length_cdf(
    ...
    trainset = get_dataset(
        template, model_args, data_args, training_args, "sft", **tokenizer_module
    )["train_dataset"]
    ...
    for length, count in length_tuples:
        ...
        print(
            f"{count_accu:d} ({prob_accu:.2f}%) samples have length < {length + interval}."
        )
