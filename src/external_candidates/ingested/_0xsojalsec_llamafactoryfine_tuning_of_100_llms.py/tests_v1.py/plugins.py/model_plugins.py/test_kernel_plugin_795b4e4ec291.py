# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\tests_v1\plugins\model_plugins\test_kernel_plugin.py
...
def _apply_kernel(rank) -> None:
    with patch("torch.accelerator.current_accelerator") as mock_get_accelerator:
        ...
        assert (
            model.model.layers[0].input_layernorm.forward.__func__
            is not original_rmsnorm_forward.__func__
        )
        assert (
            model.model.layers[0].mlp.forward.__func__
            is original_swiglu_forward.__func__
        )
...
def _apply_all_kernels(rank) -> None:
    with patch("torch.accelerator.current_accelerator") as mock_get_accelerator:
        ...
        assert (
            model.model.layers[0].mlp.forward.__func__
            is not original_swiglu_forward.__func__
        )
