# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\tests_v1\core\utils\test_rendering.py
...
HF_MESSAGES_WITH_TOOLS = [
    ...
    {"role": "user", "content": "What is 6*8?"},
    {
        ...
        "tool_calls": [
            {
                "type": "function",
                "function": {"name": "multiply", "arguments": {"a": 6, "b": 8}},
            }
        ],
    },
...
V1_MESSAGES_WITH_TOOLS = [
    ...
    {
        "role": "assistant",
        "content": [{"type": "text", "value": "The result of 6*8 is 48."}],
    },
]
...
def test_chatml_rendering():
    tokenizer: Processor = AutoTokenizer.from_pretrained(
    ...
    hf_inputs = tokenizer.apply_chat_template(
        HF_MESSAGES[:-1], add_generation_prompt=True
    )
    ...
    hf_inputs_part = tokenizer.apply_chat_template(
        HF_MESSAGES[:-1], add_generation_prompt=False
    )
    hf_inputs_full = tokenizer.apply_chat_template(
        HF_MESSAGES, add_generation_prompt=False
    )
    ...
    assert (
        v1_inputs_full["labels"]
        == [-100] * len(hf_inputs_part) + hf_inputs_full[len(hf_inputs_part) :]
    )
...
def test_chatml_rendering_remote(num_samples: int):
    ...
    for index in range(num_samples):
        v1_inputs = renderer.render_messages(
            data_engine[index]["messages"], is_generate=True
        )
...
def test_qwen3_nothink_rendering():
    ...
    hf_inputs = tokenizer.apply_chat_template(
        HF_MESSAGES_WITH_TOOLS[:-1], tools=V1_TOOLS, add_generation_prompt=True
    )
    v1_inputs = renderer.render_messages(
        V1_MESSAGES_WITH_TOOLS[:-1], tools=json.dumps(V1_TOOLS), is_generate=True
    )
    assert v1_inputs["input_ids"] == hf_inputs
    assert v1_inputs["attention_mask"] == [1] * len(hf_inputs)
    assert v1_inputs["labels"] == [-100] * len(hf_inputs)
    ...
    hf_inputs_full = tokenizer.apply_chat_template(
        HF_MESSAGES_WITH_TOOLS, tools=V1_TOOLS, add_generation_prompt=False
    )
    v1_inputs_full = renderer.render_messages(
        V1_MESSAGES_WITH_TOOLS, tools=json.dumps(V1_TOOLS), is_generate=False
    )
    assert v1_inputs_full["input_ids"] == hf_inputs_full
    assert v1_inputs_full["attention_mask"] == [1] * len(hf_inputs_full)
    assert (
...
def test_qwen3_nothink_parse():
    ...
    assert parsed_message == {
        ...
        "content": [
            {
                "type": "reasoning",
                "value": "I need to use the multiply function to calculate 6*8.",
            },
...
def test_qwen3_nothink_rendering_remote(num_samples: int):
    ...
    for index in range(num_samples):
        v1_inputs = renderer.render_messages(
            data_engine[index]["messages"], tools=data_engine[index]["tools"]
        )
        prefix_text = (
...
def test_process_sft_samples():
    ...
    samples = [
        {"messages": V1_MESSAGES, "extra_info": "test", "_dataset_name": "default"}
    ]
...
def test_process_dpo_samples():
    ...
    assert model_inputs[0]["token_type_ids"] == [1] * len(hf_inputs) + [2] * len(
        hf_inputs
    )
    assert model_inputs[0]["extra_info"] == "test"
    assert model_inputs[0]["_dataset_name"] == "default"
