# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\tests_v1\core\test_model_loader.py
...
def test_tiny_qwen_with_kernel_plugin():
    from llamafactory.v1.plugins.model_plugins.kernels.ops.rms_norm.npu_rms_norm import (
        npu_rms_norm_forward,
    )

    model_args = ModelArguments(
        model="llamafactory/tiny-random-qwen3",
        kernel_config={"name": "auto", "include_kernels": "auto"},
    )
    model_engine = ModelEngine(model_args)
    # test enable apply kernel plugin
    if hasattr(torch, "npu"):
        assert (
            model_engine.model.model.layers[0].input_layernorm.forward.__code__
