# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\tests\conftest.py
...
def pytest_collection_modifyitems(config: Config, items: list[Item]):
    ...
    for item in items:
        if "tests_v1" in str(item.fspath) and not is_transformers_version_greater_than(
            "4.57.0"
        ):
