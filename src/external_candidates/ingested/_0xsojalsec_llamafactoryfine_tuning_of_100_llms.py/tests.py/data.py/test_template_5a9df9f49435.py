# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\tests\data\test_template.py
...
def _check_template(
    ...
    template = get_template_and_fix_tokenizer(
        tokenizer, DataArguments(template=template_name)
    )
...
def test_encode_oneturn(use_fast: bool):
    ...
    template = get_template_and_fix_tokenizer(
        tokenizer, DataArguments(template="llama3")
    )
    prompt_ids, answer_ids = template.encode_oneturn(tokenizer, MESSAGES)
    prompt_str = (
...
def test_encode_multiturn(use_fast: bool):
    ...
    prompt_str_2 = "<|start_header_id|>user<|end_header_id|>\n\n你好<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
...
def test_reasoning_encode_oneturn(
    use_fast: bool, cot_messages: bool, enable_thinking: bool
):
    ...
    prompt_ids, answer_ids = template.encode_oneturn(
        tokenizer, MESSAGES_WITH_THOUGHT if cot_messages else MESSAGES
    )
...
@pytest.mark.parametrize("cot_messages", [True, False])
@pytest.mark.parametrize("enable_thinking", [True, False, None])
def test_reasoning_encode_multiturn(
    use_fast: bool, cot_messages: bool, enable_thinking: bool
):
    tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-8B", use_fast=use_fast)
    data_args = DataArguments(template="qwen3", enable_thinking=enable_thinking)
...
def test_jinja_template(use_fast: bool):
    ...
    tokenizer.chat_template = template._get_jinja_template(
        tokenizer
    )  # llama3 template no replace
    assert tokenizer.chat_template != ref_tokenizer.chat_template
    assert tokenizer.apply_chat_template(MESSAGES) == ref_tokenizer.apply_chat_template(
        MESSAGES
    )
...
@pytest.mark.parametrize(
    ...
    [
        True,
        pytest.param(
            False, marks=pytest.mark.xfail(reason="Llama 4 has no slow tokenizer.")
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            True, marks=pytest.mark.xfail(not HF_TOKEN, reason="Authorization.")
        ),
        pytest.param(
            False, marks=pytest.mark.xfail(reason="Phi-4 slow tokenizer is broken.")
        ),
    ],
)
@pytest.mark.runs_on(["cpu", "mps"])
...
def test_qwen2_5_template(use_fast: bool):
    ...
    answer_str = f"{MESSAGES[3]['content']}<|im_end|>\n"
    _check_template(
        "Qwen/Qwen2.5-7B-Instruct", "qwen", prompt_str, answer_str, use_fast
    )
...
def test_qwen3_template(use_fast: bool, cot_messages: bool):
    ...
    _check_template(
        "Qwen/Qwen3-8B", "qwen3", prompt_str, answer_str, use_fast, messages=messages
    )
...
def test_parse_llama3_template():
    ...
    assert template.format_system.slots == [
