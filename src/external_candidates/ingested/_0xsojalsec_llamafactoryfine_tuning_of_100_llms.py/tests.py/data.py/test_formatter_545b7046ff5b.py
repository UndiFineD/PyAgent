# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\tests\data\test_formatter.py
...
def test_mistral_function_formatter():
    formatter = FunctionFormatter(
        slots=["[TOOL_CALLS] {{content}}", "</s>"], tool_format="mistral"
    )
    tool_calls = json.dumps(FUNCTION)
    assert formatter.apply(content=tool_calls) == [
        "[TOOL_CALLS] "
...
def test_lfm2_tool_round_trip():
    ...
    original = {
        "name": "my_func",
        "arguments": {"arg1": "hello", "arg2": 42, "arg3": True},
    }
