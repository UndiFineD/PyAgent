# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\tests\data\test_mm_plugin.py
...
def _is_close(batch_a: dict[str, Any], batch_b: dict[str, Any]) -> None:
    ...
    for key in batch_a.keys():
        ...
        elif isinstance(batch_a[key], list) and all(
            isinstance(item, torch.Tensor) for item in batch_a[key]
        ):
...
def _load_tokenizer_module(model_name_or_path: str) -> "TokenizerModule":
    model_args, *_ = get_infer_args(
        {"model_name_or_path": model_name_or_path, "template": "default"}
    )
...
def _check_plugin(
    ...
    if plugin.__class__.__name__ == "Qwen2OmniPlugin":  # test omni_messages
        assert (
            plugin.process_messages(OMNI_MESSAGES, IMAGES, NO_VIDEOS, AUDIOS, processor)
            == expected_mm_messages
        )
        assert plugin.process_token_ids(
            INPUT_IDS, LABELS, IMAGES, NO_VIDEOS, AUDIOS, tokenizer, processor
        ) == (
            expected_input_ids,
            expected_labels,
        )
        ...
        assert (
            plugin.process_messages(
                VIDEO_MESSAGES, NO_IMAGES, VIDEOS, NO_AUDIOS, processor
            )
            == expected_mm_messages
        )
        ...
        assert (
            plugin.process_messages(
                MM_MESSAGES, IMAGES, NO_VIDEOS, NO_AUDIOS, processor
            )
            == expected_mm_messages
        )
        assert plugin.process_token_ids(
            INPUT_IDS, LABELS, IMAGES, NO_VIDEOS, NO_AUDIOS, tokenizer, processor
        ) == (
            expected_input_ids,
            expected_labels,
        )
    ...
    assert (
        plugin.process_messages(
            TEXT_MESSAGES, NO_IMAGES, NO_VIDEOS, NO_AUDIOS, processor
        )
        == TEXT_MESSAGES
    )
    assert plugin.process_token_ids(
        INPUT_IDS, LABELS, NO_IMAGES, NO_VIDEOS, NO_AUDIOS, tokenizer, processor
    ) == (
        INPUT_IDS,
        LABELS,
    )
...
@pytest.mark.skipif(
    not is_transformers_version_greater_than("4.50.0"),
    reason="Requires transformers>=4.50.0",
)
...
@pytest.mark.runs_on(["cpu", "mps"])
@pytest.mark.skipif(
    not is_transformers_version_greater_than("4.52.0"),
    reason="Requires transformers>=4.52.0",
)
def test_internvl_plugin():
    ...
    internvl_plugin = get_mm_plugin(
        "intern_vl", image_token="<image>", video_token="<video>"
    )
    check_inputs = {"plugin": internvl_plugin, **tokenizer_module}
    check_inputs["expected_mm_messages"] = [
        {
            key: value.replace(
                "<image>", f"<img>{'<IMG_CONTEXT>' * image_seqlen * 1}</img>"
            )
            for key, value in message.items()
        }
        for message in MM_MESSAGES
...
@pytest.mark.runs_on(["cpu", "mps"])
@pytest.mark.skipif(
    not is_transformers_version_greater_than("4.51.0"),
    reason="Requires transformers>=4.51.0",
)
def test_llama4_plugin():
    ...
    tokens_for_this_image = processor._prompt_split_image(
        aspect_ratios[0], num_patches_per_chunk
    )
    check_inputs["expected_mm_messages"] = [
        {
            key: value.replace("<image>", tokens_for_this_image)
            for key, value in message.items()
        }
        for message in MM_MESSAGES
    ]
...
def test_llava_next_video_plugin():
    ...
    llava_next_video_plugin = get_mm_plugin(
...
def test_paligemma_plugin():
    ...
    check_inputs["expected_mm_messages"] = [
        {key: value.replace("<image>", "") for key, value in message.items()}
        for message in MM_MESSAGES
    ]
    ...
    check_inputs["expected_mm_inputs"] = _get_mm_inputs(tokenizer_module["processor"])
    check_inputs["expected_mm_inputs"]["token_type_ids"] = [
        [0] * image_seqlen + [1] * (1024 - image_seqlen)
    ]
...
def test_pixtral_plugin():
    ...
    check_inputs["expected_mm_messages"] = [
        {
            key: value.replace(
                "<image>",
                (
                    "{}[IMG_BREAK]".format("[IMG]" * image_slice_width)
                    * image_slice_height
                ).rsplit("[IMG_BREAK]", 1)[0]
                + "[IMG_END]",
            )
            for key, value in message.items()
        }
    ...
    check_inputs["expected_mm_inputs"] = _get_mm_inputs(tokenizer_module["processor"])
    check_inputs["expected_mm_inputs"]["pixel_values"] = check_inputs[
        "expected_mm_inputs"
    ]["pixel_values"][0]
    _check_plugin(**check_inputs)
    ...
    check_inputs["expected_mm_messages"] = [
        {
            key: (
                value.replace(
                    "<image>",
                    f"<|vision_bos|>{'<|IMAGE|>' * image_seqlen}<|vision_eos|>",
                ).replace(
...
@pytest.mark.runs_on(["cpu", "mps"])
@pytest.mark.skipif(
    not is_transformers_version_greater_than("4.57.0"),
...
@pytest.mark.runs_on(["cpu", "mps"])
@pytest.mark.skipif(
    not is_transformers_version_greater_than("4.47.0"),
    reason="Requires transformers>=4.47.0",
)
def test_video_llava_plugin():
    ...
    video_llava_plugin = get_mm_plugin(
