# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IC9600\ICNet.py
import torch
...
class up_conv_bn_relu(nn.Module):
    def __init__(
        self, up_size, in_channels, out_channels=64, kernal_size=1, padding=0, stride=1
    ):
        super(up_conv_bn_relu, self).__init__()
        self.upSample = nn.Upsample(
            size=(up_size, up_size), mode="bilinear", align_corners=True
        )
        self.conv = nn.Conv2d(
            in_channels=in_channels,
            out_channels=out_channels,
...
class ICNet(nn.Module):
    def __init__(self, is_pretrain=True, size1=512, size2=256):
        ...
        ## upsample
        self.upsize = size1 // 8
        self.up1 = up_conv_bn_relu(
            up_size=self.upsize, in_channels=128, out_channels=256
        )
        self.up2 = up_conv_bn_relu(
            up_size=self.upsize, in_channels=512, out_channels=256
        )
    ...
    def forward(self, x1):
        ...
        x2 = F.interpolate(
            x1, size=(self.size2, self.size2), mode="bilinear", align_corners=True
        )
