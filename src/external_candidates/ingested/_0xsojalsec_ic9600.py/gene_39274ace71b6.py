# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IC9600\gene.py
import argparse
import os
...
def infer_one_image(img_path):
    with torch.no_grad():
        ...
        out_ic_map_name = (
            os.path.basename(img_path).split(".")[0] + "_" + str(ic_score)[:7] + ".npy"
        )
        ...
        ## gene blend map
        ic_map_img = (
            (ic_map * 255).round().squeeze().detach().cpu().numpy().astype("uint8")
        )
...
if __name__ == "__main__":
    ...
    model.load_state_dict(
        torch.load("./checkpoint/ck.pth", map_location=torch.device("cpu"))
    )
    model.eval()
