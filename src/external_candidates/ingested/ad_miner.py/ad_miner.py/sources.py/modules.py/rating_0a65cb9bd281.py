...
def rating(users, domains, computers, objects, arguments):
    d[presence_of(["1"] * domains.max_da_per_domain, criticity=2, threshold=10)].append(
        "nb_domain_admins"
    )
    ...
    d[presence_of(users.can_read_gmsapassword_of_adm)].append(
        "can_read_gmsapassword_of_adm"
    )
    d[presence_of(users.objects_to_operators_member)].append(
    ...
    d[presence_of(domains.unpriv_users_to_GPO_parsed.items())].append(
        "users_GPO_access"
    )

    d[1 if domains.total_dangerous_paths > 0 else 5].append("dangerous_paths")
    d[presence_of(users.users_password_not_required, 3)].append(
    ...
    d[presence_of([ude for ude in users.guest_accounts if ude[-1]])].append(
        "guest_accounts"
    )
    d[
        rate_admincount(
