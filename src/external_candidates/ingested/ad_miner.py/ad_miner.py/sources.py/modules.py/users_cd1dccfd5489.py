...
class Users:
    def __init__(self, arguments, neo4j, domain):
        ...
        self.users_password_not_required = neo4j.all_requests[
        ...
        self.unpriviledged_users_with_admincount = neo4j.all_requests[
            "unpriviledged_users_with_admincount"
        ]["result"]
        self.users_nb_domain_admins = neo4j.all_requests["nb_domain_admins"]["result"]

        self.primaryGroupID_lower_than_1000 = neo4j.all_requests[
            "primaryGroupID_lower_than_1000"
        ]["result"]

        self.pre_windows_2000_compatible_access_group = neo4j.all_requests[
    ...
    def genGroupAnomalyAcl(self, domain):
        ...
        for k in range(len(self.anomaly_acl)):
            name_label_instance = (
                f"{self.anomaly_acl[k]['g.name']}{self.anomaly_acl[k]['LABELS(g)[0]']}"
            )
            if (
                formated_data.get(name_label_instance)
                and formated_data[name_label_instance]["type"]
                == self.anomaly_acl[k]["type(r2)"]
                and formated_data[name_label_instance]["label"]
                == self.anomaly_acl[k]["LABELS(g)[0]"]
            ):
                        ...
                        if "No data to show" not in u["List of computers"]:
                            count = int(
                                u["List of computers"][
                                    ...
                                    + 2 : u["List of computers"].find("Computer")
                                ].strip()
                            )
                            tmp_dict["Computers admin"] = grid_data_stringify(
                                {
                                    "link": u["Path to computers"]
                                    .split("href='", 1)[-1]
                                    .split("'", 1)[0],
            ...
            anomaly_acl_extract.append(
                {
                    ...
                    "interest": f"<span class='{interest}'></span><i class='bi bi-star-fill'></i>"
                    * interest
    ...
    def genHasSIDHistory(self):
        ...
        for row in self.has_sid_history:
            ...
            if row["Type_a"] == "User":
                ...
                row["Has SID History"] = (
                    "<i class='bi bi-people-fill' title='Group'></i> "
                    + row["Has SID History"]
                )
            else:
                row["Has SID History"] = (
                    "<i class='bi bi-question-circle-fill' title='Unknown'></i> "
                    + row["Has SID History"]
                )
            if row["Type_b"] == "User":
                row["Target"] = (
                    "<i class='bi bi-person-fill' title='User'></i> " + row["Target"]
                )
            elif row["Type_b"] == "Group":
                row["Target"] = (
                    "<i class='bi bi-people-fill' title='Group'></i> " + row["Target"]
                )
            else:
                row["Target"] = (
                    "<i class='bi bi-question-circle-fill' title='Unknown'></i> "
