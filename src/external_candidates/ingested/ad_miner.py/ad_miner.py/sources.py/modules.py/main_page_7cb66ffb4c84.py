...
def getData(arguments, neo4j, domains, computers, users, objects, extract_date):
    ...
    data["nb_domain_collected"] = len(
        neo4j.all_requests["nb_domain_collected"]["result"]
    )
...
def create_dico_data(
    ...
    dico_data["value"] = {
        ...
        "dom_admin_on_non_dc": (
            ...
            if users.users_domain_admin_on_nondc
            else 0
        ),
        "nb_domain_admins": (
        ...
        "computers_admin_of_computers": (
            computers.count_computers_admins if computers.count_computers_admins else 0
        ),
            ...
            len(
                list(
                    set(
                        [
                            item
                            for sublist in domains.users_to_domain_admin.values()
                            for item in sublist
                        ]
                    )
                )
        ...
        "vuln_permissions_adminsdholder": (
            ...
            if users.vuln_permissions_adminsdholder
            else 0
        ),
        "objects_to_adcs": (
            len(computers.ADCS_path_sorted.keys()) if computers.ADCS_path_sorted else 0
        ),
        ...
        "unpriviledged_users_with_admincount": len(
            users.unpriviledged_users_with_admincount
        ),
...
def render(
    ...
    for risk_control in global_risk_controls:
        ...
        for category in categories:
            if categories[category] > 0 and f"{category}_letter_grade" not in data:
                data[f"{category}_letter_grade"] = global_risk_controls[risk_control][
                    "code"
                ]
                data[f"{category}_letter_color"] = global_risk_controls[risk_control][
                    "colors"
                ]
    ...
    data["vuln_text_major_risk"] = manage_plural(
        data["potential_risk"], ("vulnerability", "vulnerabilities")
    )
    data["alert_or_alerts"] = manage_plural(data["potential_risk"], ("Alert", "Alerts"))
    data["minor_alert_or_alerts"] = manage_plural(
        data["minor_risk"], ("Minor issue", "Minor issues")
    )
