# Extracted from: C:\DEV\PyAgent\.external\absadiki-whatsapp-msgstore-viewer\decryption\dbs\WhatsAppCrypt14Crypt15Decrypter\utils\hex_string_to_encrypted_backup_key.py
#!/usr/bin/env python
"""
...
def from_hex(string: str) -> bytes:
    ...
    if len(string) != 64:
        Log.f(
            "Wrong string length: It's {} but should be 64 characters long".format(
                len(string)
            )
        )
...
def parsecmdline() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Creates an encrypted_backup.key from a hex string."
    )
...
def create_encrypted_backup_key_file(ikey: str, output):
    ...
    j_key: javaobj.JavaByteArray = javaobj.JavaByteArray(
        key, create_class_description()
    )
