# Extracted from: C:\DEV\PyAgent\.external\absadiki-whatsapp-msgstore-viewer\decryption\dbs\WhatsAppCrypt14Crypt15Decrypter\utils\protobuf_bruteforce.py
"""
...
def parsecmdline() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--keep-going",
        action="store_true",
        help="Don't stop after the first result.",
    )
    parser.add_argument(
        "-r",
        "--range",
        type=int,
        default=512,
        help="The number of bytes to search. Ignored if -w is " "set.",
    )
    parser.add_argument(
        "-w",
        "--whole-file",
        action="store_true",
        help="Search the whole file. Not advised for large " "files",
    )
...
def load_file(file_name: str, byte_range=0, reverse=False) -> bytes:
    ...
    try:
        ...
        if byte_range > size / 2:
            raise ValueError(
                "Range provided is bigger than half of file. Use -w or lower the range."
            )
...
def search(whole_file: bytes, keep_going: bool, offset=0):
    ...
    for i in range(len(whole_file)):
        ...
        if message.last_good_j and message.last_good_i == i:
            print(
                "\n Message from byte {} to {}".format(
                    message.last_good_i + offset, message.last_good_j + offset
                )
            )
