# Extracted from: C:\DEV\PyAgent\.external\absadiki-whatsapp-msgstore-viewer\decryption\dbs\WhatsAppCrypt14Crypt15Decrypter\decrypt14_15.py
#!/usr/bin/env python
"""
...
def parsecmdline() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Decrypts WhatsApp backup files" " encrypted with crypt12, 14 or 15"
    )
    parser.add_argument(
        "keyfile",
        nargs="?",
    ...
    parser.add_argument(
        ...
        "--buffer-size",
        type=int,
        help="How many bytes of data to process at a time. "
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Prints all offsets and messages"
    )
...
class Key:
    ...
    def load_crypt14(self, logger, keyfile: bytes):
        ...
        if not version_supported:
            logger.e(
                "Invalid keyfile: Unsupported key version {}".format(
                    keyfile[index : index + len(self.SUPPORTED_KEY_VERSIONS[0])].hex()
                )
            )
        ...
        if expected_digest != actual_digest:
            logger.e(
                "Invalid keyfile: Invalid SHA-256 of salt.\n    "
        ...
        for byte in padding:
            if byte:
                logger.e(
                    "Invalid keyfile: IV is not zeroed out but is: {}".format(
                        padding.hex()
                    )
                )
                break
...
def test_decompression(logger, test_data: bytes) -> bool:
    ...
    try:
        ...
        if zlib_obj[:15].decode("ascii") != "SQLite format 3":
            logger.e(
                "Test decompression: Decryption and decompression ok but not a valid SQLite database"
            )
            return logger.force
        else:
            return True
...
def find_data_offset(
    logger, header: bytes, iv_offset: int, key: bytes, starting_data_offset: int
) -> int:
    ...
    for i in oscillate(
        n=starting_data_offset, n_min=iv_offset + len(iv), n_max=HEADER_SIZE - 128
    ):
...
def guess_offsets(
    ...
    for iv_offset in oscillate(n=def_iv_offset, n_min=0, n_max=HEADER_SIZE - 128):
        data_offset = find_data_offset(
            logger, db_header, iv_offset, key, def_data_offset
        )
        if data_offset != -1:
            logger.i(
                "Offsets guessed (IV: {}, data: {}).".format(iv_offset, data_offset)
            )
            if iv_offset != def_iv_offset or data_offset != def_data_offset:
                logger.i(
                    "Next time, use -ivo {} -do {} for guess-free decryption".format(
                        iv_offset, data_offset
                    )
                )
            break
    if data_offset == -1:
...
def parse_protobuf(logger, file_hash: _Hash, key: Key, encrypted):
    ...
    try:
        ...
        logger.e(
            "Could not import the proto classes: {}\n    ".format(e)
    ...
    try:
        ...
        try:
            ...
            if p.ParseFromString(protobuf_raw) != protobuf_size:
                ...
                logger.v(
                    "Your phone number ends with {}".format(p.info.substringedUserJid)
                )
                ...
                if len(p.c15_iv.IV) != 0:
                    ...
                    if not key.is_crypt15():
                        logger.e(
                            "You are using a crypt14 key file with a crypt15 backup."
                        )
                    if len(p.c15_iv.IV) != 16:
                        logger.e(
                            "IV is not 16 bytes long but is {} bytes long".format(
                                len(p.c15_iv.IV)
                            )
                        )
                    iv = p.c15_iv.IV
                    ...
                    if key.is_crypt15():
                        logger.f(
                            "You are using a crypt15 key file with a crypt14 backup."
                        )
                    ...
                    key.key_version = (key.key_version[0] + 48).to_bytes(
                        1, byteorder="big"
                    )
                        ...
                        if key.key_version > p.c14_cipher.key_version:
                            logger.e(
                                "Key version mismatch: {} != {} .\n    ".format(
                                    key.key_version, p.c14_cipher.key_version
                                )
                            ...
                            logger.e(
                                "Key version mismatch: {} != {} (?)".format(
                                    key.key_version, p.c14_cipher.key_version
                                )
                            )
                    if key.serversalt != p.c14_cipher.server_salt:
                        logger.e(
                            "Server salt mismatch: {} != {}".format(
                                key.serversalt, p.c14_cipher.server_salt
                            )
                        )
                    if key.googleid != p.c14_cipher.google_id:
                        logger.e(
                            "Google ID mismatch: {} != {}".format(
                                key.googleid, p.c14_cipher.google_id
                            )
                        )
                    ...
                    logger.e(
                        "Could not parse the IV from the protobuf message. Please report a bug."
                    )
                    return None
...
def decrypt(
    logger, file_hash: _Hash, cipher, encrypted, decrypted, buffer_size: int = 0
):
        ...
        if buffer_size == 0:
            ...
            try:
                ...
                if file_hash.digest() != checksum:
                    ...
                    logger.v(
                        "Checksum OK ({}). Decrypting...".format(file_hash.hexdigest())
                    )
                ...
                try:
                    ...
                    if test_decompression(
                        logger, output_file[: io.DEFAULT_BUFFER_SIZE]
                    ):
                        logger.i(
                            "Decrypted data is a ZIP file that I will not decompress automatically."
                        )
                    else:
                        logger.e(
            ...
            if buffer_size < 17:
                logger.i(
                    "Invalid buffer size, will use default of {}".format(
            ...
            while True:
                ...
                if chunk is None:
                    ...
                    if len(chunk) < buffer_size:
                        ...
                        logger.f(
                            "Buffer size too large, use a smaller buffer size or don't use a buffer."
                        )
                    continue
                ...
                if checksum is not None:
                    ...
                    try:
                        ...
                        logger.e(
                            "Authentication tag mismatch: {}."
                            "\n    This probably means your backup is corrupted.".format(
                                e
                            )
                        )
                    break
...
def main():
    ...
    if not args.no_protobuf:
        ...
        try:
            ...
            cipher = parse_protobuf(
                logger=logger, file_hash=file_hash, key=key, encrypted=args.encrypted
            )
    ...
    if args.buffer_size is not None:
        decrypt(
            logger, file_hash, cipher, args.encrypted, args.decrypted, args.buffer_size
        )
    elif args.no_mem:
        decrypt(
            logger,
