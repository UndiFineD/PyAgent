# Extracted from: C:\DEV\PyAgent\.external\absadiki-whatsapp-msgstore-viewer\decryption\dbs\decrypt_db.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
    ...
    with open(encrypted, "rb") as encrypted:
        with open(decrypted, "wb") as decrypted:
            ...
            if not (0 < data_offset < HEADER_SIZE - 128):
                logger.f(
                    "The data offset must be between 1 and {}".format(HEADER_SIZE - 129)
                )
            if not (0 < iv_offset < HEADER_SIZE - 128):
                logger.f(
                    "The IV offset must be between 1 and {}".format(HEADER_SIZE - 129)
                )
            ...
            if not no_protobuf:
                ...
                try:
                    ...
                    cipher = parse_protobuf(
                        logger=logger, file_hash=file_hash, key=key, encrypted=encrypted
                    )
            ...
            if cipher is None and not no_guess:
                ...
                cipher = guess_offsets(
                    ...
                    key=key.key,
                    encrypted=encrypted,
                    def_iv_offset=iv_offset,
                    def_data_offset=data_offset,
                )
