# Extracted from: C:\DEV\PyAgent\.external\absadiki-whatsapp-msgstore-viewer\View\ChatScreen\chat_screen.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
from Utility.Utils import check_path, fix_emojis
...
class Attachment(
    ButtonBehavior, RectangularRippleBehavior, CommonElevationBehavior, MLabel
):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
...
class ChatMessage(RecycleDataViewBehavior, MDBoxLayout):
    ...
    def open_media(self, file_path):
        ...
        if MDApp.get_running_app().wp_dir is None:
            self.show_dialog(
                msg="No Whatsapp directory has been provided.\n"
                "To View this file, provide the `Whatsapp directory path` in the login screen",
                title="Error",
            )
            return
    ...
    def refresh_view_attrs(self, rv, index, data):
        self.index = index
        self.txt_data = fix_emojis(
            data["text_data"], MDApp.get_running_app().emojis_font
        )
        ...
        if data["message_quoted_text_data"] is not None:
            self.msg_quoted_text_data = fix_emojis(
                data["message_quoted_text_data"], MDApp.get_running_app().emojis_font
            )
        else:
...
class ChatScreenView(BaseScreenView):
    ...
    def __init__(self, **kw):
        ...
        self.dialog.bind(
            on_dismiss=lambda x: print("run something on dialog dismissed")
        )
        self.image = "image"
