# Extracted from: C:\DEV\PyAgent\.external\absadiki-whatsapp-msgstore-viewer\View\LoginScreen\login_screen.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
class About(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        with open("about", "r") as file:
            content = file.read()
            self.about = content.replace(
                "$$version$$", MDApp.get_running_app().version
...
class LoginScreenView(BaseScreenView):
    ...
    def __init__(self, **kw):
        ...
        self.key_file_widget = TextFieldFileManager(
            id="key_file",
            hint_text="Key",
    ...
    def settings_save(self, *args):
        ...
        if general_font_path == "":
            ...
            if not check_path(general_font_path):
                self.show_dialog(
                    msg=f"The path `{general_font_path}` does not exist",
                    title="General Font error",
                    auto_dismiss=True,
                )
                return
        ...
        if emojis_font_path == "":
            ...
            if not check_path(general_font_path):
                self.show_dialog(
                    msg=f"The path `{emojis_font_path}` does not exist",
                    title="Emojis Font error",
                    auto_dismiss=True,
                )
                return
        ...
        if call_log_size == "":
            ...
            try:
                ...
                self.show_dialog(
                    msg=f"Call log size`{call_log_size}` is not a number",
                    title="Calls log size error",
                    auto_dismiss=True,
                )
                return
