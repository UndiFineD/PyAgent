# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\tests\deps\test_db.py
import pytest
from acontext_core.infra.db import DatabaseClient
from acontext_core.schema.orm import Block, Project, Session, Space
...
async def test_db():
    ...
    async with db_client.get_session_context() as session:
        ...
        space_with_blocks_query = await session.execute(
            select(Space).options(selectinload(Space.blocks)).where(Space.id == sid)
        )
