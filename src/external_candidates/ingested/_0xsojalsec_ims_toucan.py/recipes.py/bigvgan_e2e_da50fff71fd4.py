# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Recipes\BigVGAN_e2e.py
import time
...
def run(
    ...
    if model_dir is not None:
        ...
        model_save_dir = os.path.join(MODEL_DIR, "BigVGAN_e2e")
    ...
    for i, f in enumerate(fl):
        if os.path.exists(f.replace(".wav", "_synthetic_spec.pt")):
            ...
            file_lists_for_this_run_combined_synthetic.append(
                f.replace(".wav", "_synthetic_spec.pt")
            )
    print("filepaths collected")

    train_set = HiFiGANDataset(
        list_of_original_paths=file_lists_for_this_run_combined,
