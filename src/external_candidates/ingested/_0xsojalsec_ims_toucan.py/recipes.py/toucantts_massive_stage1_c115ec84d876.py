# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Recipes\ToucanTTS_Massive_stage1.py
"""
...
def run(
    ...
    base_dir = os.path.join(
        MODEL_DIR, "ToucanTTS_MassiveDataBigModel_stage1_reworked_v10"
    )
    ...
    chunks = split_dictionary_into_chunks(
        build_path_to_transcript_mls_english(), split_n=chunk_count
    )
    ...
    chunk_count = 20
    chunks = split_dictionary_into_chunks(
        build_path_to_transcript_mls_german(), split_n=chunk_count
    )
    for index in range(chunk_count):
        if index > 3:
            break
