# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Utility\Scorer.py
"""
...
class TTSScorer:
    ...
    def __init__(
        ...
        self.spec_extractor = AudioPreprocessor(
    ...
    def score(self, path_to_toucantts_dataset, lang_id):
        ...
        for index in tqdm(range(len(dataset.datapoints))):
            ...
            gold_pitch = (
                datapoint[6].to(self.device).unsqueeze(0)
            )  # mind the switched order
            gold_energy = (
                datapoint[5].to(self.device).unsqueeze(0)
            )  # mind the switched order
            lang_ids = dataset.language_id.to(self.device)
            filepath = datapoint[8]
            with torch.inference_mode():
                wave = self.ap.indexes_to_audio(
                    speech_indexes.int().to(self.device)
                ).detach()
                mel = (
                    self.spec_extractor.audio_to_mel_spec_tensor(
            ...
            try:
                regression_loss, _, duration_loss, pitch_loss, energy_loss = self.tts(
    ...
    def show_samples_with_highest_loss(self, n=-1):
        ...
        for index, path in enumerate(
            sorted(self.path_to_score, key=self.path_to_score.get, reverse=True)
        ):
            if index < n or n == -1:
    ...
    def remove_samples_with_highest_loss(self, n=10):
        if self.current_dset is None:
            ...
            if self.nans_removed:
                print(
                    "Indexes are no longer accurate. Please re-run the scoring. \n\n"
    ...
    def remove_samples_with_loss_three_std_devs_higher_than_avg(self):
        if self.current_dset is None:
            ...
            if self.nans_removed:
                ...
                scores_without_nans = [
                    value
                    for value in list(self.path_to_score.values())
    ...
    def remove_nans(self):
        if self.nans_removed:
            ...
            if self.current_dset is None:
                ...
                if len(self.nans) > 0:
                    print(
                        "The following filepaths had an infinite loss and are being removed from the dataset cache:"
                    )
                    for path in self.nans:
                        print(path)
