# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Utility\utils.py
"""
...
from Preprocessing.TextFrontend import ArticulatoryCombinedTextFrontend, get_language_id
...
def pad_to_multiple_of_n(x, n=4, seq_dim=1, pad_value=0):
    ...
    return torch.nn.functional.pad(
        x, [0, 0, 0, diff, 0, 0], mode="constant", value=pad_value
    )


@torch.inference_mode()
def plot_progress_spec_toucantts(
    net, device, save_dir, step, lang, default_emb, run_stochastic
):
    ...
    plot_code_spec(
        ...
        durations,
        mel,
        os.path.join(save_dir, "visualization"),
        tf,
        step,
    )
...
def plot_code_spec(pitch, energy, sentence, durations, mel, save_path, tf, step):
    ...
    for p, e, d in zip(
        pitch.cpu().squeeze().numpy(),
        energy.cpu().squeeze().numpy(),
        durations.cpu().numpy(),
    ):
        for _ in range(d):
    ...
    plt.subplots_adjust(
        left=0.05, bottom=0.1, right=0.95, top=0.95, wspace=0.0, hspace=0.0
    )
...
def plot_spec_tensor(spec, save_path, name, title=None):
    ...
    plt.subplots_adjust(
        ...
        bottom=0.1,
        right=0.95,
        top=0.95 if title is None else 0.85,
        wspace=0.0,
        hspace=0.0,
    )
    os.makedirs(save_path, exist_ok=True)
...
def delete_old_checkpoints(checkpoint_dir, keep=5):
        ...
        if el.endswith(".pt"):
            try:
                checkpoint_list.append(
                    int(el.replace("checkpoint_", "").replace(".pt", ""))
                )
            except ValueError:
                pass
    if len(checkpoint_list) <= keep:
        ...
        checkpoints_to_delete = [
            os.path.join(checkpoint_dir, "checkpoint_{}.pt".format(step))
            for step in checkpoint_list[:-keep]
        ]
