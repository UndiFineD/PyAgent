...
def build_path_to_transcript_mls_english():
    ...
    path_to_transcript = build_path_to_transcript_multi_ling_librispeech_template(
        root=root
    )
    return path_to_transcript
...
def build_path_to_transcript_gigaspeech():
    ...
    path_to_transcript = dict()
    with open(
        os.path.join(root, "transcripts_only_clean_samples.txt"), "r", encoding="utf8"
    ) as file:
        lookup = file.read()
    for line in lookup.split("\n"):
        if line.strip() != "":
    ...
    for speaker_dir in os.listdir(root):
        for audio_file in os.listdir(os.path.join(root, speaker_dir)):
            if audio_file.split("-")[4] == "01":
                path_to_transcript[os.path.join(root, speaker_dir, audio_file)] = (
...
def build_path_to_transcript_thorsten_emotional():
    ...
    for line in trans_lines:
        if line.strip() != "":
            ...
            path_to_transcript[root + "/disgusted/" + line.split("|")[0] + ".wav"] = (
                line.split("|")[1]
            )
            ...
            path_to_transcript[root + "/surprised/" + line.split("|")[0] + ".wav"] = (
                line.split("|")[1]
            )
    return path_to_transcript
...
def build_path_to_transcript_captain_japanese():
    ...
    for line in transcriptions.split("\n"):
        if line.strip() != "":
            ...
            audio_path = os.path.join(
                root, "male", "wav", "train_parallel", audio_path + ".wav"
            )
    ...
    for line in transcriptions.split("\n"):
        if line.strip() != "":
            ...
            transcript = parsed_line[1]
            audio_path = os.path.join(
                root, "female", "wav", "train_parallel", audio_path + ".wav"
            )
            if os.path.exists(audio_path):
                path_to_transcript[audio_path] = transcript.strip()
            else:
...
def build_path_to_transcript_african_voices_hausa_cmv():
    ...
    path_to_transcript = build_path_to_transcript_african_voices_template(
        f"{main_root}/hau_cmv_f"
    )
    path_to_transcript.update(
        build_path_to_transcript_african_voices_template(f"{main_root}/hau_cmv_m")
    )
    return path_to_transcript
...
def build_path_to_transcript_mms_template(
