# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\InferenceInterfaces\ToucanTTSInterface.py
import itertools
import os
...
from Utility.utils import cumsum_durations, float2pcm
...
class ToucanTTSInterface(torch.nn.Module):
    ...
    def __init__(
        ...
        if tts_model_path is None:
            ...
            tts_model_path = os.path.join(
                MODEL_DIR, f"ToucanTTS_{tts_model_path}", "best.pt"
            )
        if vocoder_model_path is None:
            vocoder_model_path = hf_hub_download(
                cache_dir=MODEL_DIR, repo_id="Flux9665/ToucanTTS", filename="Vocoder.pt"
            )
        ...
        self.text2phone = ArticulatoryCombinedTextFrontend(
            language=language, add_silence_to_end=True, device=device
        )
        ...
        self.speaker_embedding_func_ecapa = EncoderClassifier.from_hparams(
            ...
            savedir=os.path.join(
                MODEL_DIR, "Embedding", "speechbrain_speaker_embedding_ecapa"
            ),
        )
    ...
    def set_phonemizer_language(self, lang_id):
        self.text2phone = ArticulatoryCombinedTextFrontend(
            language=lang_id, add_silence_to_end=True, device=self.device
        )
