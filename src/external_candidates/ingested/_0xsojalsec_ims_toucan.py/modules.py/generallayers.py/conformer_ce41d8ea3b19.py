# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\GeneralLayers\Conformer.py
"""
...
from Modules.GeneralLayers.ConditionalLayerNorm import AdaIN1d, ConditionalLayerNorm
...
class Conformer(torch.nn.Module):
    ...
    def __init__(
        ...
        if utt_embed is not None:
            ...
            ):  # the encoder gets an additional conditioning signal added to its output
                if embedding_integration == "AdaIN":
                    self.encoder_embedding_projection = AdaIN1d(
                        style_dim=utt_embed, num_features=attention_dim
                    )
                elif embedding_integration == "ConditionalLayerNorm":
                    self.encoder_embedding_projection = ConditionalLayerNorm(
                        speaker_embedding_dim=utt_embed, hidden_dim=attention_dim
                    )
                else:
                    self.encoder_embedding_projection = torch.nn.Linear(
                        attention_dim + utt_embed, attention_dim
                    )
            else:
                if embedding_integration == "AdaIN":
                    self.decoder_embedding_projections = repeat(
                        num_blocks,
                        lambda lnum: AdaIN1d(
                            style_dim=utt_embed, num_features=attention_dim
                        ),
                    ...
                    self.decoder_embedding_projections = repeat(
                        num_blocks,
                        lambda lnum: ConditionalLayerNorm(
                            speaker_embedding_dim=utt_embed, hidden_dim=attention_dim
                        ),
                    )
                else:
                    self.decoder_embedding_projections = repeat(
                        num_blocks,
                        lambda lnum: torch.nn.Linear(
                            attention_dim + utt_embed, attention_dim
                        ),
                    )
        if lang_embs is not None:
            self.language_embedding = torch.nn.Embedding(
                num_embeddings=lang_embs, embedding_dim=lang_emb_size
            )
            if lang_emb_size == attention_dim:
                self.language_embedding_projection = lambda x: x
            else:
                self.language_embedding_projection = torch.nn.Linear(
                    lang_emb_size, attention_dim
                )
    ...
    def forward(self, xs, masks, utterance_embedding=None, lang_ids=None):
        ...
        if lang_ids is not None:
            ...
            projected_lang_embs = (
                self.language_embedding_projection(lang_embs)
                .unsqueeze(-1)
                .transpose(1, 2)
            )
            projected_lang_embs = self.language_emb_norm(projected_lang_embs)
            xs = (
                xs + projected_lang_embs
            ...
            if self.utt_embed:
                if isinstance(xs, tuple):
                    ...
                    if self.conformer_type != "encoder":
                        x = integrate_with_utt_embed(
                            hs=x,
                            utt_embeddings=utterance_embedding,
                            projection=self.decoder_embedding_projections[
                                encoder_index
                            ],
