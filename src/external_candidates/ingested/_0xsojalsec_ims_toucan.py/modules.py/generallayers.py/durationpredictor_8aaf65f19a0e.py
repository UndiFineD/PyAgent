# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\GeneralLayers\DurationPredictor.py
...
class DurationPredictor(torch.nn.Module):
            ...
            if utt_embed_dim is not None:
                if embedding_integration == "AdaIN":
                    self.embedding_projections += [
                        AdaIN1d(style_dim=utt_embed_dim, num_features=idim)
                    ]
                elif embedding_integration == "ConditionalLayerNorm":
                    self.embedding_projections += [
                        ConditionalLayerNorm(
                            speaker_embedding_dim=utt_embed_dim, hidden_dim=idim
                        )
                    ]
                else:
                    self.embedding_projections += [
                        torch.nn.Linear(utt_embed_dim + idim, idim)
                    ]
            else:
    ...
    def _forward(self, xs, x_masks=None, is_inference=False, utt_embed=None):
        ...
        for f, c, d, p in zip(
            self.conv, self.norms, self.dropouts, self.embedding_projections
        ):
            xs = f(xs)  # (B, C, Tmax)
            if self.utt_embed_dim is not None:
                xs = integrate_with_utt_embed(
                    hs=xs.transpose(1, 2),
                    utt_embeddings=utt_embed,
                    projection=p,
