# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\GeneralLayers\STFT.py
"""
Taken from ESPNet
"""
...
class STFT(torch.nn.Module):
    ...
    def forward(self, input_wave, ilens=None):
        ...
        if self.window is not None:
            ...
            window = window_func(
                self.win_length, dtype=input_wave.dtype, device=input_wave.device
            )
        else:
            window = None

        complex_output = torch_stft(
            input=input_wave,
            n_fft=self.n_fft,
