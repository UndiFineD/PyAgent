# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\EmbeddingModel\StyleEmbedding.py
import torch
...
if __name__ == "__main__":
    ...
    print(
        f"GST parameter count: {sum(p.numel() for p in style_emb.style_encoder.parameters() if p.requires_grad)}"
    )

    seq_length = 398
    print(
        style_emb(
            torch.randn(5, seq_length, 512),
    ...
    print(
        f"StyleTTS encoder parameter count: {sum(p.numel() for p in style_emb.style_encoder.parameters() if p.requires_grad)}"
    )
