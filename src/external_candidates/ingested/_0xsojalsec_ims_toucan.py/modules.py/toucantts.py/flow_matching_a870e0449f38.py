# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ToucanTTS\flow_matching.py
"""
...
class CFMDecoder(torch.nn.Module):
    ...
    def compute_loss(self, x1, mask, mu, c):
        ...
        loss = F.mse_loss(
            self.estimator(y, mask, mu, t.squeeze(), c), u, reduction="sum"
...
def create_plot_of_all_solutions(sol, fps=8):
    ...
    for step_index, solution in enumerate(sol):
        unbatched_solution = solution[
            0
        ]  # remove the batch axis (if there are more than one element in the batch, we only take the first)
    ...
    for _ in range(fps * 2):
        gif_collector.append(
            gif_collector[-1]
        )  # freeze-frame on the final one for two seconds
