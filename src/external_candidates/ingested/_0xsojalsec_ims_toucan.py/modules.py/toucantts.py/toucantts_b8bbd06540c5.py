...
class ToucanTTS(torch.nn.Module):
    ...
    def _forward(
        ...
        if is_inference:
            ...
            reduced_pitch_space = torchfunc.dropout(
                self.pitch_latent_reduction(encoded_texts), p=0.1
            ).transpose(1, 2)
            ...
            embedded_energy_curve = self.energy_embed(energy_predictions).transpose(
                1, 2
            )

            reduced_duration_space = torchfunc.dropout(
                self.duration_latent_reduction(
            ...
            predicted_durations = (
                torch.clamp(torch.ceil(predicted_durations), min=0.0).long().squeeze(1)
            )
