# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ToucanTTS\InferenceToucanTTS.py
import dotwiz
import torch
...
class ToucanTTS(torch.nn.Module):
    ...
    def __init__(self, weights, config):
        ...
        transformer_enc_positional_dropout_rate = (
            config.transformer_enc_positional_dropout_rate
        )
        ...
        transformer_dec_positional_dropout_rate = (
            config.transformer_dec_positional_dropout_rate
        )
        ...
        integrate_language_embedding_into_encoder_out = (
            config.integrate_language_embedding_into_encoder_out
        )
        ...
        self.integrate_language_embedding_into_encoder_out = (
            integrate_language_embedding_into_encoder_out
        )
        self.use_conditional_layernorm_embedding_integration = (
            embedding_integration in ["AdaIN", "ConditionalLayerNorm"]
        )

        articulatory_feature_embedding = Sequential(
            Linear(input_feature_dimensions, 100),
            Tanh(),
            Linear(100, attention_dimension),
        )
        self.encoder = Conformer(
            conformer_type="encoder",
        ...
        self.duration_predictor = CFMDecoder(
            hidden_channels=prosody_channels,
            out_channels=1,
        ...
        self.pitch_predictor = CFMDecoder(
            hidden_channels=prosody_channels,
            out_channels=1,
            filter_channels=prosody_channels,
            n_heads=1,
        ...
        self.energy_predictor = CFMDecoder(
            hidden_channels=prosody_channels,
            out_channels=1,
            filter_channels=prosody_channels,
            n_heads=1,
        ...
        self.decoder = Conformer(
            ...
            use_output_norm=not embedding_integration
            in ["AdaIN", "ConditionalLayerNorm"],
            utt_embed=utt_embed_dim,
            embedding_integration=embedding_integration,
        )

        self.output_projection = torch.nn.Linear(attention_dimension, spec_channels)
        self.pitch_latent_reduction = torch.nn.Linear(
            attention_dimension, prosody_channels
        )
        self.energy_latent_reduction = torch.nn.Linear(
            attention_dimension, prosody_channels
        )
        self.duration_latent_reduction = torch.nn.Linear(
            attention_dimension, prosody_channels
        )

        self.flow_matching_decoder = CFMDecoder(
            hidden_channels=spec_channels,
    ...
    def _forward(
        ...
        # encoding the texts
        text_masks = make_non_pad_mask(
            text_lengths, device=text_lengths.device
        ).unsqueeze(-2)
        ...
        reduced_energy_space = self.energy_latent_reduction(
        ...
        reduced_duration_space = self.duration_latent_reduction(
        ...
        predicted_durations = (
            ...
            .long()
            .squeeze(1)
            if gold_durations is None
            else gold_durations.squeeze(1)
        )

        # modifying the predictions with control parameters
        predicted_durations[0][
            0
        ] = 1  # if the initial pause is too long, we get artifacts. This is once more a dirty hack.
        for phoneme_index, phoneme_vector in enumerate(text_tensors.squeeze(0)):
            if phoneme_vector[get_feature_to_index_lookup()["word-boundary"]] == 1:
                predicted_durations[0][phoneme_index] = 0
            if (
                phoneme_vector[get_feature_to_index_lookup()["silence"]] == 1
                and pause_duration_scaling_factor != 1.0
            ):
                predicted_durations[0][phoneme_index] = torch.round(
                    predicted_durations[0][phoneme_index].float()
                    * pause_duration_scaling_factor
                ).long()
        if duration_scaling_factor != 1.0:
            assert duration_scaling_factor > 0.0
            predicted_durations = torch.round(
                predicted_durations.float() * duration_scaling_factor
            ).long()
        ...
        upsampled_enriched_encoded_texts = self.length_regulator(
            enriched_encoded_texts, predicted_durations
        )

        # decoding spectrogram
        decoded_speech, _ = self.decoder(
        ...
        refined_codec_frames = self.flow_matching_decoder(
            mu=preliminary_spectrogram.transpose(1, 2),
            mask=make_non_pad_mask(
                [len(decoded_speech[0])], device=decoded_speech.device
            ).unsqueeze(-2),
            n_timesteps=30,
    ...
    def forward(
        ...
        text_length = torch.tensor(
            [text.shape[0]], dtype=torch.long, device=text.device
        )
        ...
        outs, predicted_durations, pitch_predictions, energy_predictions = (
            self._forward(
                text.unsqueeze(0),
                text_length,
