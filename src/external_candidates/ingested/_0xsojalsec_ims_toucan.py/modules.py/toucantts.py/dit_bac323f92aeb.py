# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ToucanTTS\dit.py
"""
...
class DiTConVBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = nn.LayerNorm(
            hidden_channels + out_channels, elementwise_affine=False, eps=1e-6
        )
        ...
        self.adaLN_modulation = nn.Sequential(
            (
                ...
                if gin_channels != hidden_channels + out_channels
                else nn.Identity()
            ),
            nn.SiLU(),
            nn.Linear(
    ...
    def forward(self, x, c, x_mask):
        ...
        """
        x = x * x_mask
        attn_mask = x_mask.unsqueeze(1) * x_mask.unsqueeze(
            -1
        )  # shape: [batch_size, 1, time, time]
        ...
        if c is not None:
            ...
            x = (
                ...
                * self.mlp(
                    self.modulate(
                        self.norm2(x.transpose(1, 2)).transpose(1, 2),
                        shift_mlp,
                        scale_mlp,
                    ),
                    x_mask,
            ...
            x = x + self.mlp(
                self.norm1(x.transpose(1, 2)).transpose(1, 2), x_mask
            )  # TODO this is technically a bug, but it seems to work ok with the bug. Before the next major model is trained for a release, norm1 should be changed to norm2 in this line.
        return x
