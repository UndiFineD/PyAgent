# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ToucanTTS\toucantts_meta_train_loop.py
...
from Utility.weight_averaging import (
    average_checkpoints,
    get_n_recent_checkpoints_paths,
    load_net_toucan,
    save_model_for_use,
)
...
def collate_and_pad(batch):
    ...
    return (
        pad_sequence([datapoint[0] for datapoint in batch], batch_first=True).float(),
...
def train_loop(
    ...
    if steps % steps_per_checkpoint == 0:
        ...
        steps = steps + (
            (steps_per_checkpoint + 1) - (steps % steps_per_checkpoint)
        )  # making sure to stop at the closest point that makes sense to the specified stopping point
    if steps < warmup_steps * 5:
        print(
            f"too much warmup given the amount of steps, reducing warmup to {warmup_steps} steps"
        )
    ...
    scheduler = WarmupScheduler(
        optimizer, peak_lr=lr, warmup_steps=warmup_steps, max_steps=steps
    )
    ...
    if path_to_checkpoint is not None:
        ...
        if freeze_lang_embs:
            ...
            model.encoder.language_embedding.weight.requires_grad = (
                False  # and we never reset that.
            )
        else:
    ...
    if not fine_tune and not resume and use_less_loss and not freeze_lang_embs:
        ...
        for i in tqdm(range(warmup_steps * 8)):
            ...
            language_embeddings = model.encoder.language_embedding(
                torch.LongTensor(language_ids).to(device)
            )
    ...
    for step_counter in tqdm(range(steps_run_previously, steps)):
        ...
        speech_batch = (
            list()
        )  # I wish this could be done in the collate function or in the getitem, but using DL models in multiprocessing on very large datasets causes just way too many issues.
        for index, speech_sample in enumerate(speech_indexes):
            with torch.inference_mode():
                wave = ap.indexes_to_audio(speech_sample.int().to(device)).detach()
                mel = (
                    spec_extractor.audio_to_mel_spec_tensor(
                        wave, explicit_sampling_rate=16000
                    )
                    .transpose(0, 1)
                    .detach()
                    .cpu()
        ...
        if (
            torch.isnan(regression_loss)
            or torch.isnan(duration_loss)
            or torch.isnan(pitch_loss)
            or torch.isnan(energy_loss)
        ):
        ...
        if step_counter % steps_per_checkpoint == 0 and step_counter != 0:
            ...
            if rank == 0:
                ...
                print(
                    "Reconstruction Loss:    {}".format(
                        round(
                            sum(regression_losses_total) / len(regression_losses_total),
                            3,
                        )
                    )
                )
                ...
                torch.save(
                    ...
                    os.path.join(
                        save_directory, "checkpoint_{}.pt".format(step_counter)
                    ),
                )
                ...
                if use_wandb:
                    wandb.log(
                        {
                            "regression_loss": round(
                                sum(regression_losses_total)
                                / len(regression_losses_total),
                                5,
                            ),
                            "stochastic_loss": round(
                                sum(stochastic_losses_total)
                                / len(stochastic_losses_total),
                                5,
                            ),
                            "duration_loss": round(
                                sum(duration_losses_total) / len(duration_losses_total),
                                5,
                            ),
                            "pitch_loss": round(
                                sum(pitch_losses_total) / len(pitch_losses_total), 5
                            ),
                            "energy_loss": round(
                                sum(energy_losses_total) / len(energy_losses_total), 5
                            ),
                ...
                try:
                    path_to_most_recent_plot = plot_progress_spec_toucantts(
                        model,
                        device,
                ...
                checkpoint_paths = get_n_recent_checkpoints_paths(
                    checkpoint_dir=save_directory, n=1
                )
                averaged_model, default_embed = average_checkpoints(
                    checkpoint_paths, load_func=load_net_toucan
                )
                save_model_for_use(
                    model=averaged_model,
                    default_embed=default_embed,
                    name=os.path.join(save_directory, "best.pt"),
                )
