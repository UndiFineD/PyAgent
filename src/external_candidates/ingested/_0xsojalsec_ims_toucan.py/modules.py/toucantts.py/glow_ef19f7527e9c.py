# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ToucanTTS\Glow.py
import numpy as np
import scipy
import torch
    ...
    def __init__(
        ...
        if text_condition_channels != 0 and share_cond_layers:
            cond_layer = torch.nn.Conv1d(
                text_condition_channels * n_sqz, 2 * hidden_channels * n_layers, 1
            )
            if use_weightnorm:
    ...
    def forward(
        ...
        if not infer:
            ...
            try:
                ...
                print(
                    "log probability of postflow could not be calculated for this step"
                )
                postflow_loss = None
            return postflow_loss
        else:
            nonpadding = (
                torch.ones_like(x_recon[:, :1, :])
                if tgt_nonpadding is None
                else tgt_nonpadding
            )
