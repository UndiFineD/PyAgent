# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\Aligner\autoaligner_train_loop.py
import os
import time
...
def train_loop(
    ...
    ap = CodecAudioPreprocessor(
        input_sr=-1, device=device
    )  # only used to transform features into continuous matrices
    spectrogram_extractor = AudioPreprocessor(
        ...
        for batch in tqdm(train_loader):
            ...
            for datapoint in batch[2]:
                with torch.inference_mode():
                    ...
                    mel = (
                        spectrogram_extractor.audio_to_mel_spec_tensor(
                            speech, explicit_sampling_rate=16000
                        )
                        .transpose(0, 1)
                        .cpu()
                    )
            ...
            ctc_loss = asr_model.ctc_loss(
                pred.transpose(0, 1).log_softmax(2), tokens, mel_len, tokens_len
            )

            if use_reconstruction:
                speaker_embeddings_expanded = (
                    torch.nn.functional.normalize(speaker_embeddings)
                    .unsqueeze(1)
                    .expand(-1, pred.size(1), -1)
                )
                tts_lambda = min([0.1, step_counter / 10000])  # super simple schedule
                reconstruction_loss = (
                    tiny_tts(
                        x=torch.cat([pred, speaker_embeddings_expanded], dim=-1),
                        # combine ASR prediction with speaker embeddings to allow for reconstruction loss on multiple speakers
                        lens=mel_len,
                        ys=mel,
                    )
                    * tts_lambda
                )  # reconstruction loss to make the states more distinct
            ...
            if step_counter % steps_per_checkpoint == 0 and rank == 0:
                ...
                print(
                    "Total Loss:   {}".format(round(sum(loss_sum) / len(loss_sum), 3))
                )
                print(
