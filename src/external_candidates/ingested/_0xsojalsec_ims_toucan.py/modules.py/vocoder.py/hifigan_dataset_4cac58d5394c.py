# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\Vocoder\HiFiGAN_Dataset.py
import os
import random
...
def random_pitch_shifter(x):
    n_steps = random.choice(
        [-12, -9, -6, 3, 12]
    )  # when using 12 steps per octave, these are the only ones that are pretty fast. I benchmarked it and the variance is many orders of magnitude.
...
class HiFiGANDataset(Dataset):
    ...
    def __init__(
        ...
        self.melspec_ap = AudioPreprocessor(
            input_sr=self.desired_samplingrate, output_sr=16000, cut_silence=False
        )
        ...
        if loading_processes == 1:
            ...
            for path_split in path_splits:
                process_list.append(
                    Process(
                        target=self.cache_builder_process,
                        args=(path_split,),
                        daemon=True,
                    )
                )
        ...
        self.wave_augs = [
            random_pitch_shifter,
            polarity_inverter,
            lambda x: x,
            lambda x: x,
            lambda x: x,
            lambda x: x,
        ]  # just some data augmentation
        self.wave_distortions = [
            CodecSimulator(),
            lambda x: x,
            lambda x: x,
            lambda x: x,
            lambda x: x,
        ]  # simulating the fact, that we train the TTS on codec-compressed waves
            ...
            try:
                ...
                if sr != self.desired_samplingrate:
                    wave = librosa.resample(
                        y=wave, orig_sr=sr, target_sr=self.desired_samplingrate
                    )
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            while (
                len(wave) < self.samples_per_segment + 50
            ):  # + 50 is just to be extra sure
            ...
            if self.use_random_corruption:
                ...
                wave = random.choice(self.wave_augs)(wave.unsqueeze(0)).squeeze(
                    0
                )  # it is intentional that this affects the target as well. This is not a distortion, but an augmentation.
            ...
            resampled_segment = self.melspec_ap.resample(
                segment
            ).float()  # 16kHz spectrogram as input, 24kHz wave as output, see Blizzard 2021 DelightfulTTS
            if self.use_random_corruption:
                # augmentations for the wave
                resampled_segment = random.choice(self.wave_distortions)(
                    resampled_segment.unsqueeze(0)
                ).squeeze(0)
            melspec = (
                self.melspec_ap.audio_to_mel_spec_tensor(
                    resampled_segment, explicit_sampling_rate=16000, normalize=False
                )
                .transpose(0, 1)[:-1]
                .transpose(0, 1)
            )
...
if __name__ == "__main__":
    ...
    spec = (
        melspec_ap.audio_to_mel_spec_tensor(
            melspec_ap.resample(resampled_wave).float(),
    ...
    cs = CodecSimulator()
    masker = torchaudio.transforms.FrequencyMasking(
        freq_mask_param=16, iid_masks=True
    )  # up to 8 consecutive bands can be masked
    ...
    for _ in range(5):
        ...
        shifted_spec = (
            melspec_ap.audio_to_mel_spec_tensor(
                melspec_ap.resample(shifted_wave).float(),
                explicit_sampling_rate=16000,
                normalize=False,
            )
            .transpose(0, 1)[:-1]
