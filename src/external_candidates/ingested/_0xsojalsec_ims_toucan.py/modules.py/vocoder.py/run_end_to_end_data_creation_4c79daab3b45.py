# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\Vocoder\run_end_to_end_data_creation.py
"""
...
class UtteranceCloner:
    ...
    def __init__(self, model_id, device, language="eng"):
        ...
        self.device = device
        acoustic_checkpoint_path = os.path.join(
            PREPROCESSING_DIR, "libri_all_clean", "Aligner", "aligner.pt"
        )
    ...
    def extract_prosody(
        self, transcript, ref_audio_path, lang="eng", on_line_fine_tune=False
    ):
        wave, sr = sf.read(ref_audio_path)
        if self.tf.language != lang:
            self.tf = ArticulatoryCombinedTextFrontend(
                language=lang, device=self.device
            )
    ...
    def clone_utterance(
        ...
        duration, pitch, energy = self.extract_prosody(
            transcription_of_intonation_reference,
            path_to_reference_audio_for_intonation,
            lang=lang,
        )
        self.tts.set_language(lang)
        cloned_speech = self.tts(
            transcription_of_intonation_reference,
            view=False,
            durations=duration,
...
if __name__ == "__main__":
    ...
    for path in tqdm(all_dict):
        ...
        reader.read_texts(
            sentence=all_dict[path], filename=filename, speaker_reference=path
        )
