# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\Vocoder\BigVGAN.py
...
class BigVGAN(torch.nn.Module):
    ...
    def __init__(
        ...
        upsample_rates=(
            ...
            2,
            2,
        ),  # CAREFUL: Avocodo discriminator assumes that there are always 4 upsample scales, because it takes intermediate results.
...
if __name__ == "__main__":
    vgan = BigVGAN()
    print(
        f"BigVGAN parameter count: {sum(p.numel() for p in vgan.parameters() if p.requires_grad)}"
    )
