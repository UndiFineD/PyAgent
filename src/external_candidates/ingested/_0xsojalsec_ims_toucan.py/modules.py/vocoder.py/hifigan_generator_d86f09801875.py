# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\Vocoder\HiFiGAN_Generator.py
...
class HiFiGAN(torch.nn.Module):
    ...
    def __init__(
        ...
        upsample_scales=(
            ...
            2,
            2,
        ),  # CAREFUL: Avocodo assumes that there are always 4 upsample scales, because it takes intermediate results.
...
if __name__ == "__main__":
    hifi = HiFiGAN()
    print(
        f"HiFiGAN parameter count: {sum(p.numel() for p in hifi.parameters() if p.requires_grad)}"
    )
