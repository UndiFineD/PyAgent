# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\Vocoder\HiFiGAN_train_loop.py
import os
import time
...
import wandb
from Modules.Vocoder.AdversarialLoss import discriminator_adv_loss, generator_adv_loss
...
def train_loop(
    ...
    optimizer_g = torch.optim.RAdam(
        g.parameters(), betas=(0.5, 0.9), lr=0.0005, weight_decay=0.0
    )
    scheduler_g = MultiStepLR(
        optimizer_g, gamma=0.5, milestones=[400000, 8000000, 000000, 1000000]
    )
    optimizer_d = torch.optim.RAdam(
        d.parameters(), betas=(0.5, 0.9), lr=0.00025, weight_decay=0.0
    )
    scheduler_d = MultiStepLR(
        optimizer_d, gamma=0.5, milestones=[400000, 8000000, 000000, 1000000]
    )
    ...
    for _ in range(epochs):
        ...
        for datapoint in tqdm(train_loader):
            ...
            if (
                ...
                d_outs, d_fmaps = d(
                    wave=pred_wave.detach(), discriminator_train_flag=True
                )
        ...
        if epoch % epochs_per_save == 0:
            ...
            checkpoint_paths = get_n_recent_checkpoints_paths(
                checkpoint_dir=model_save_dir, n=1
            )
