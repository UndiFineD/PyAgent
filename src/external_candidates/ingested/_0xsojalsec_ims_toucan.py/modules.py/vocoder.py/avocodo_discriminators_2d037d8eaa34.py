# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\Vocoder\Avocodo_Discriminators.py
"""
...
class MultiCoMBDiscriminator(torch.nn.Module):
    ...
    def forward(
        self,
        wave_final,
        intermediate_wave_upsampled_twice=None,
        intermediate_wave_upsampled_once=None,
        discriminator_train_flag=False,
    ):

        if (
            intermediate_wave_upsampled_twice is not None
            and intermediate_wave_upsampled_once is not None
        ):
            ...
            out2, p2_fmap_hat_ = self.combd_2(
                intermediate_wave_upsampled_twice, discriminator_train_flag
            )
            features_of_predicted = features_of_predicted + p2_fmap_hat_

            out1, p1_fmap_hat_ = self.combd_1(
                intermediate_wave_upsampled_once, discriminator_train_flag
            )
...
class MultiSubBandDiscriminator(torch.nn.Module):
    ...
    def forward(self, wave, discriminator_train_flag):
        ...
        q3_hat, feat_q3_hat = self.tsbd3(
            xn_hat[:, : self.tsubband3, :], discriminator_train_flag
        )
        fmap_hat = fmap_hat + feat_q3_hat

        q2_hat, feat_q2_hat = self.tsbd2(
            xn_hat[:, : self.tsubband2, :], discriminator_train_flag
        )
        fmap_hat = fmap_hat + feat_q2_hat

        q1_hat, feat_q1_hat = self.tsbd1(
            xn_hat[:, : self.tsubband1, :], discriminator_train_flag
        )
...
class CoMBD(torch.nn.Module):
    ...
    def __init__(self, filters, kernels, groups, strides, use_spectral_norm=False):
        ...
        self.conv_post = norm_f(
            SANConv1d(filters[-1], 1, 3, 1, padding=get_padding(3, 1))
        )
...
class MDC(torch.nn.Module):
    ...
    def __init__(
        ...
        self.conv_out = norm_f(
            SANConv1d(channel, channel, 3, stride=stride, padding=get_padding(3, 1))
        )
...
class SubBandDiscriminator(torch.nn.Module):
    ...
    def __init__(
        ...
        self.conv_post = norm_f(
            SANConv1d(init_channel, 1, 3, padding=get_padding(3, 1))
        )
