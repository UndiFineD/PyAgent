# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ControllabilityGAN\GAN.py
import torch
...
class GanWrapper:
    ...
    def load_model(self, path):
        ...
        self.wgan = create_wgan(
            parameters=gan_checkpoint["model_parameters"], device=self.device
        )
    ...
    def compute_controllability(self, n_samples=200000):
        _, intermediate, z = self.wgan.sample_generator(
            num_samples=n_samples, nograd=True, return_intermediate=True
        )
