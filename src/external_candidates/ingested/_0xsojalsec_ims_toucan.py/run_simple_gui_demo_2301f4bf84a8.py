# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\run_simple_GUI_demo.py
import gradio as gr
import torch.cuda
...
class TTSWebUI:
    ...
    def __init__(
        ...
        text_selection = [
            f"{iso_to_name[iso_code]} ({iso_code})" for iso_code in iso_to_name
        ]
        ...
        self.iface = gr.Interface(
            ...
            inputs=[
                ...
                gr.Slider(
                    ...
                    step=0.1,
                    value=0.0,
                    label="Gender of artificial Voice",
                ),
                gr.Audio(
                    type="filepath",
                    show_label=True,
                    container=True,
                    label="[OPTIONAL] Voice to Clone (if left empty, will use an artificial voice instead)",
                ),
