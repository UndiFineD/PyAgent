# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Preprocessing\multilinguality\generate_zero_shot_lang_embs.py
import argparse
import json
import os
...
def approximate_and_inject_language_embeddings(
    ...
    for row in tqdm(
        ...
        avg_emb = torch.zeros(
            [32]
        )  # If you change the size of the language embedding in the model, you need to change the size here as well.
        dists = [
            getattr(row, d)
...
if __name__ == "__main__":
    default_model_path = os.path.join(
        MODEL_DIR, "ToucanTTS_Meta", "best.pt"
    )  # MODEL_DIR must be absolute path, the relative path will fail at this location
