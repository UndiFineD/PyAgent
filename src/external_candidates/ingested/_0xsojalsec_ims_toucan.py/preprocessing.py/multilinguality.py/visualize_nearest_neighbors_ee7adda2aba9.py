...
for lang_1 in distance_measure:
    ...
    for lang_2 in distance_measure[lang_1]:
        try:
            if (
                lang_2 not in supervised_iso_codes
                and iso_codes_to_names[lang_2] != neighbor
            ):
                continue
        except KeyError:
            continue
...
edges_connected_to_specific_node = [
    (u, v) for u, v in G.edges() if u == neighbor or v == neighbor
]
...
for u, v, d in edges:
    if u == neighbor or v == neighbor:
        nx.draw_networkx_edge_labels(
            ...
            pos,
            edge_labels={
                (u, v): round((thresh - (d["weight"] / 20000) ** (1 / 2)) * 10, 2)
            },
            font_color="red",
            alpha=0.3,
...
nx.draw_networkx_labels(
    G, pos, font_size=14, font_family="sans-serif", font_color="green"
)
nx.draw_networkx_labels(
    G,
    pos,
