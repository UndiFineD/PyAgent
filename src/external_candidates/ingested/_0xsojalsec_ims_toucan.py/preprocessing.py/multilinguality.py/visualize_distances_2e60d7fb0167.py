# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Preprocessing\multilinguality\visualize_distances.py
import pickle
...
if distance_type == "l1":
    ...
    model_path = hf_hub_download(
        cache_dir=MODEL_DIR, repo_id="Flux9665/ToucanTTS", filename="ToucanTTS.pt"
    )
...
if distance_type == "learned":
    ...
    for key1 in learned_dist:
        for key2 in learned_dist[key1]:
            learned_dist[key1][key2] = (
                learned_dist[key1][key2] / largest_value_learned_dist
            )
    distance_measure = learned_dist

if distance_type == "asp":
    asp_dict_path = hf_hub_download(
        cache_dir=MODEL_DIR, repo_id="Flux9665/ToucanTTS", filename="asp_dict.pkl"
    )
...
normalized_distances = [
    (entity1, entity2, (d - min_dist) / (max_dist - min_dist))
    for entity1, entity2, d in distances
]
