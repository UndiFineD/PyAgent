...
class MetricsCombiner(torch.nn.Module):
    def __init__(self, m):
        super().__init__()
        self.scoring_function = kan.KAN(
            width=[3, 5, 1], grid=5, k=5, seed=m, auto_save=False, device=DEVICE
        )
...
def create_learned_cache(model_path, cache_root="."):
    ...
    for m in range(n_models):
        ...
        running_loss = list()
        for epoch in tqdm(
            range(25), desc=f"MetricsCombiner {m + 1}/{n_models} - Epoch"
        ):
            for i in range(2000):
                ...
                scores = model_list[-1](
                    torch.stack(metric_distance_batch).squeeze().to(DEVICE)
                )
                ...
                target_batch = (
                    torch.stack(embedding_distance_batch).squeeze().to(DEVICE)
                )
                loss = torch.nn.functional.mse_loss(
                    scores.squeeze(), target_batch, reduction="none"
                )
                loss = loss / (
                    target_batch + 0.0001
    ...
    for lang_1 in tqdm(tree_dist):
        ...
        if len(batch_accumulator) != 0:
            ...
            for predicted_distance, pair in zip(predicted_distances, pair_accumulator):
                language_to_language_to_learned_distance[pair[0]][
                    pair[1]
