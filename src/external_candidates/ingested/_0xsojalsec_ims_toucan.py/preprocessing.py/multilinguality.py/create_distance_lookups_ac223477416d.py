# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Preprocessing\multilinguality\create_distance_lookups.py
import argparse
import json
import os.path
...
class CacheCreator:
    ...
    def create_tree_cache(self, cache_root="."):
        iso_to_family_memberships = load_json_from_path(
            hf_hub_download(
                cache_dir=MODEL_DIR,
                repo_id="Flux9665/ToucanTTS",
                filename="iso_to_memberships.json",
            )
        )
        ...
        for pair in tqdm(self.pairs, desc="Generating tree pairs"):
            ...
            self.pair_to_tree_distance[pair] = (depth_of_l1 + depth_of_l2) - (
                2 * (depth_of_lca + 1) if depth_of_lca > 1 else depth_of_lca
            )  # with discounting for added importance of earlier nodes
        ...
        for pair in self.pair_to_tree_distance:
            if pair[0] == pair[1]:
                ...
                self.pair_to_tree_distance[pair] = (
                    self.pair_to_tree_distance[pair] + abs(min_dist)
                ) / (max_dist + abs(min_dist))
    ...
    def create_required_files(self, model_path, create_oracle=False):
        ...
        if not os.path.exists(
            os.path.join(self.cache_root, "asp_dict.pkl")
        ) or os.path.exists(
            hf_hub_download(
                cache_dir=MODEL_DIR,
                repo_id="Flux9665/ToucanTTS",
                filename="asp_dict.pkl",
            )
        ):
