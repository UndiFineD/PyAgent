# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Preprocessing\articulatory_features.py
# -*- coding: utf-8 -*-
...
def get_phone_to_id():
    ...
    for index, phone in enumerate(
        "~#?!ǃ.ɜəaðɛɪŋɔɒɾʃθʊʌʒæbʔdefghijklmnɳopɡɹrstuvwxzʀøçɐœyʏɑcɲɣʎβʝɟqɕɭɵʑʋʁɨʂɓʙɗɖχʛʟɽɢɠǂɦǁĩʍʕɻʄũɤɶõʡʈʜɱɯǀɸʘʐɰɘħɞʉɴʢѵ"
    ):
...
def generate_feature_table():
    ...
    for ipa in ipa_to_phonemefeats:
        if len(ipa) == 1:
            phone_to_vector[ipa] = [0] * (
                15
                + sum(
                    [
                        len(values)
                        for values in [
                            feat_to_val_set[feat] for feat in feat_to_val_set
                        ]
                    ]
                )
            )
            ...
            for feat in ipa_to_phonemefeats[ipa]:
                if ipa_to_phonemefeats[ipa][feat] in value_to_index:
                    phone_to_vector[ipa][
                        value_to_index[ipa_to_phonemefeats[ipa][feat]]
                    ] = 1
