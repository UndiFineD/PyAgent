# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Preprocessing\TextFrontend.py
# -*- coding: utf-8 -*-
...
from Preprocessing.articulatory_features import (
...
class ArticulatoryCombinedTextFrontend:
    ...
    def __init__(
        ...
        if language == "eng" or language == "en-us":
            ...
            self.kakasi = (
                pykakasi.Kakasi()
            )  # this is not a satisfactory solution, but it is the best one I could come up with so far.
            self.expand_abbreviations = lambda x: " ".join(
                [chunk["hepburn"] for chunk in self.kakasi.convert(x)]
            )
    ...
    def string_to_tensor(
        ...
        for char in phones:
            ...
            # affects previous phoneme -----------------
                ...
                char.strip() == "̃"
                and phones_vector[-1][get_feature_to_index_lookup()["nasal"]] != 1
            ):
                # nasalized (vowel)
                ...
                char.strip()
                == "̧"
                != phones_vector[-1][get_feature_to_index_lookup()["palatal"]]
                != 1
            ):
                # palatalized
                ...
                char.strip() == "ʷ"
                and phones_vector[-1][get_feature_to_index_lookup()["labial-velar"]]
                != 1
            ):
                # labialized
                ...
                char.strip() == "ʰ"
                and phones_vector[-1][get_feature_to_index_lookup()["aspirated"]] != 1
            ):
                # aspirated
                phones_vector[-1][get_feature_to_index_lookup()["aspirated"]] = 2
            elif (
                char.strip() == "ˠ"
                and phones_vector[-1][get_feature_to_index_lookup()["velar"]] != 1
            ):
                # velarized
                phones_vector[-1][get_feature_to_index_lookup()["velar"]] = 2
            elif (
                char.strip() == "ˁ"
                and phones_vector[-1][get_feature_to_index_lookup()["pharyngal"]] != 1
            ):
                # pharyngealized
                phones_vector[-1][get_feature_to_index_lookup()["pharyngal"]] = 2
            elif (
                char.strip() == "ˀ"
                and phones_vector[-1][get_feature_to_index_lookup()["glottal"]] != 1
            ):
                # glottalized
                phones_vector[-1][get_feature_to_index_lookup()["glottal"]] = 2
            elif (
                char.strip() == "ʼ"
                and phones_vector[-1][get_feature_to_index_lookup()["ejective"]] != 1
            ):
                # ejective
                phones_vector[-1][get_feature_to_index_lookup()["ejective"]] = 2
            elif (
                char.strip() == "̹"
                and phones_vector[-1][get_feature_to_index_lookup()["rounded"]] != 1
            ):
                # rounding
                phones_vector[-1][get_feature_to_index_lookup()["rounded"]] = 2
            elif (
                char.strip() == "̞"
                and phones_vector[-1][get_feature_to_index_lookup()["open"]] != 1
            ):
                # open
                phones_vector[-1][get_feature_to_index_lookup()["open"]] = 2
            elif (
                char.strip() == "̪"
                and phones_vector[-1][get_feature_to_index_lookup()["dental"]] != 1
            ):
                # dental
                phones_vector[-1][get_feature_to_index_lookup()["dental"]] = 2
            elif (
                char.strip() == "̬"
                and phones_vector[-1][get_feature_to_index_lookup()["voiced"]] != 1
            ):
                # voiced
                phones_vector[-1][get_feature_to_index_lookup()["voiced"]] = 2
            elif (
                char.strip() == "̝"
                and phones_vector[-1][get_feature_to_index_lookup()["close"]] != 1
            ):
                # closed
                ...
                char.strip() == "̰"
                and phones_vector[-1][get_feature_to_index_lookup()["glottal"]] != 1
                and phones_vector[-1][get_feature_to_index_lookup()["epiglottal"]] != 1
            ):
                # laryngalization
                phones_vector[-1][get_feature_to_index_lookup()["glottal"]] = 2
                ...
                char.strip() == "̈"
                and phones_vector[-1][get_feature_to_index_lookup()["central"]] != 1
            ):
                # centralization
                phones_vector[-1][get_feature_to_index_lookup()["central"]] = 2
            elif (
                char.strip() == "̜"
                and phones_vector[-1][get_feature_to_index_lookup()["unrounded"]] != 1
            ):
                # unrounded
                phones_vector[-1][get_feature_to_index_lookup()["unrounded"]] = 2
            elif (
                char.strip() == "̥"
                and phones_vector[-1][get_feature_to_index_lookup()["unvoiced"]] != 1
            ):
                # voiceless
    ...
    def get_phone_string(
        ...
        text = text.replace(
            "|", "."
        )  # Hindi uses this symbol to indicate the end of a sentence.
        ...
        if (
            self.g2p_lang == "vi"
            or self.g2p_lang == "vi-vn-x-central"
            or self.g2p_lang == "vi-vn-x-south"
        ):
...
def chinese_number_conversion(text):
    ...
    def _zhnum(n):
        if n < 10:
            return zhdigits[n]
        named_place_len = zhplace_keys[
            bisect.bisect_right(zhplace_keys, numdigits(n) - 1) - 1
        ]
...
def get_language_id(language):
    try:
        ...
        try:
            iso_codes_to_ids = load_json_from_path(
                str(Path(__file__).parent / "multilinguality/iso_lookup.json")
            )[-1]
        except FileNotFoundError:
...
if __name__ == "__main__":
    ...
    tf.string_to_tensor(
        "医師会がなくても、近隣の病院なら紹介してくれると思います。", view=True
    )
    print(
        tf.get_phone_string(
            "医師会がなくても、近隣の病院なら紹介してくれると思います。"
        )
    )
    ...
    tf.string_to_tensor(
        "I don't know this language, but this is just a dummy text anyway.", view=True
    )
    print(
        tf.get_phone_string(
            "I don't know this language, but this is just a dummy text anyway."
        )
    )
