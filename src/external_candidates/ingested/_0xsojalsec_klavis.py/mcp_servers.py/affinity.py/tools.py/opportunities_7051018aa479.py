# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\affinity\tools\opportunities.py
import logging
...
async def get_single_opportunity(opportunity_id: int) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"Executing tool: get_single_opportunity with opportunity_id: {opportunity_id}"
    )
    try:
