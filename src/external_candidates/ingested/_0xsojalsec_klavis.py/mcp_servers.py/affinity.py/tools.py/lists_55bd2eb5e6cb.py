# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\affinity\tools\lists.py
import logging
...
async def get_all_list_entries_on_a_list(
    ...
    """
    logger.info(
        f"Executing tool: get_all_list_entries_on_a_list with list_id: {list_id}"
    )
    try:
        params = {}
        if cursor:
        ...
        return await make_v2_request(
            "GET", f"/lists/{list_id}/list-entries", params=params
        )
...
async def get_metadata_on_a_single_list(list_id: int) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"Executing tool: get_metadata_on_a_single_list with list_id: {list_id}"
    )
    try:
...
async def get_metadata_on_a_single_list_fields(
    list_id: int, cursor: Optional[str] = None, limit: Optional[int] = None
) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"Executing tool: get_metadata_on_a_single_list_fields with list_id: {list_id}"
    )
    try:
        params = {}
        if cursor:
        ...
        logger.exception(
            f"Error executing tool get_metadata_on_a_single_list_fields: {e}"
        )
        raise e
...
async def get_a_single_list_entry_on_a_list(
    ...
    """
    logger.info(
        f"Executing tool: get_a_single_list_entry_on_a_list with list_id: {list_id}, list_entry_id: {list_entry_id}"
    )
    try:
        params = {}
        if field_ids:
        ...
        return await make_v2_request(
            "GET", f"/lists/{list_id}/list-entries/{list_entry_id}", params=params
        )
    except Exception as e:
