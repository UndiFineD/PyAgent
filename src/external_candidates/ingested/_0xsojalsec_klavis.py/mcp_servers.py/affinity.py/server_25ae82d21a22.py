# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\affinity\server.py
import base64
import contextlib
import json
import logging
...
@click.command()
@click.option(
    "--port", default=AFFINITY_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
...
def main(
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "AFFINITY_USER"}),
            ),
            # Lists
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "AFFINITY_LIST"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "AFFINITY_PERSON"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "AFFINITY_COMPANY"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "AFFINITY_NOTE"}),
            ),
            types.Tool(
    ...
    async def call_tool(
        ...
        # Lists
            ...
            try:
                result = await get_all_list_entries_on_a_list(
                    list_id, cursor, limit, field_ids, field_types
                )
                return [
                    types.TextContent(
                        type="text",
            ...
            try:
                result = await get_metadata_on_a_single_list_fields(
                    list_id, cursor, limit
                )
                return [
                    types.TextContent(
                        type="text",
            ...
            try:
                result = await get_a_single_list_entry_on_a_list(
                    list_id, list_entry_id, field_ids, field_types
                )
                return [
                    types.TextContent(
                        type="text",
        ...
        # Persons
            ...
            try:
                result = await get_all_persons(
                    cursor, limit, ids, field_ids, field_types
                )
                return [
                    types.TextContent(
                        type="text",
        ...
        # Notes
            ...
            try:
                result = await get_all_notes(
                    person_id, organization_id, opportunity_id, page_size, page_token
                )
