# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\quickbooks\tools\http_client.py
import logging
import os
...
class QuickBooksHTTPClient:
    ...
    def __init__(
        ...
        self.environment = (
            environment or os.getenv("QB_ENVIRONMENT", "production")
        ).lower()
    ...
    async def _make_request(
        ...
        logger.debug(f"Sending {method} to {url} with data: {kwargs.get('json')}")
