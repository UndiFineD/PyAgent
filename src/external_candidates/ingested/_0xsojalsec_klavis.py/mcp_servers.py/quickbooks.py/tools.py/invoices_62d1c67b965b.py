...
void_invoice_tool = Tool(
    ...
    inputSchema={
        "type": "object",
        "properties": {
            "Id": {"type": "string", "description": "The QuickBooks invoice ID to void"}
        },
        "required": ["Id"],
    },
    annotations=types.ToolAnnotations(**{"category": "QUICKBOOKS_INVOICE"}),
            ...
            if detail_type == "SalesItemLineDetail":
                ...
                if item.get("GroupLines") and isinstance(item["GroupLines"], list):
                    ...
                    for group_item in item["GroupLines"]:
                        ...
                        if group_item.get("DiscountRate"):
                            group_sales_detail["DiscountRate"] = group_item[
                                "DiscountRate"
                            ]
                ...
                if item.get("DiscountAccountId"):
                    ...
                    if item.get("DiscountAccountName"):
                        discount_detail["DiscountAccountRef"]["name"] = item[
                            "DiscountAccountName"
                        ]
                if item.get("TaxCodeId"):
        ...
        for line in invoice_data["Line"]:
            if isinstance(line, dict) and "Amount" in line:
                ...
                if (
                    ...
                    and "SalesItemLineDetail" in line
                ):
                    ...
                    detail_type == "DescriptionOnlyLine"
                    and "DescriptionLineDetail" in line
                ):
                    ...
                    detail_type == "DiscountLineDetail" and "DiscountLineDetail" in line
                ):
                    ...
                    detail_type == "SubTotalLineDetail" and "SubTotalLineDetail" in line
                ):
...
class InvoiceManager:
    ...
    async def list_invoices(
        self, MaxResults: int = 100, StartPosition: int = 1
    ) -> List[Dict[str, Any]]:
    ...
    async def delete_invoice(self, Id: str) -> Dict[str, Any]:
        ...
        return await self.client._post(
            "invoice", delete_data, params={"operation": "delete"}
        )
    ...
    async def void_invoice(self, Id: str) -> Dict[str, Any]:
        ...
        response = await self.client._post(
            "invoice", void_data, params={"operation": "void"}
        )
