...
async def add_slide(
    presentation_id: str, title: Optional[str] = None, content: Optional[str] = None
) -> str:
    """
...
async def batch_update_presentation(presentation_id: str, requests: List[Dict]) -> str:
    ...
    try:
        ...
        created_slides = [
            r.get("createSlide", {}).get("objectId")
            for r in replies
            if "createSlide" in r
        ]
        if created_slides:
...
async def summarize_presentation(
    presentation_id: str, include_notes: bool = False
) -> str:
    """
                    ...
                    for element in notes_page["pageElements"]:
                        if "shape" in element and "text" in element["shape"]:
                            ...
                            for text_run in text_runs:
                                if (
                                    "textRun" in text_run
                                    and "content" in text_run["textRun"]
                                ):
...
@click.command()
@click.option(
    "--port", default=GOOGLE_SLIDES_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
        ...
        return [
            types.Tool(
                ...
                annotations=types.ToolAnnotations(
                    **{"category": "GOOGLE_SLIDES_PRESENTATION"}
                ),
            ),
            types.Tool(
                name="add_slide",
    ...
    async def call_tool(
        ...
        if name == "create_presentation":
            ...
            if not title:
                return [
                    types.TextContent(
                        type="text", text="Error: 'title' parameter is required"
                    )
                ]
            ...
            if not requests:
                return [
                    types.TextContent(
                        type="text", text="Error: 'requests' parameter is required"
                    )
                ]
