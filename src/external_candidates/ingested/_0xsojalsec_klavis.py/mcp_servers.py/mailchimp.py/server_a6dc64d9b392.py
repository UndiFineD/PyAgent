# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\mailchimp\server.py
import asyncio
import json
import logging
import os
        ...
        try:
            ...
            # Audience tools
                ...
                result = await get_all_audiences(
                    count=arguments.get("count", 10), offset=arguments.get("offset", 0)
                )
            ...
            # Member tools
                ...
                if not arguments.get("list_id") or not arguments.get("email_address"):
                    raise ValueError(
                        "Missing required arguments: list_id and email_address"
                    )
                ...
                result = await get_member_info(
                    arguments["list_id"], arguments["email_address"]
                )
                ...
                if not all(
                    [
                        ...
                        arguments.get("email_address"),
                        arguments.get("tags"),
                    ]
                ):
                    raise ValueError(
                        "Missing required arguments: list_id, email_address, and tags"
                    )
                result = await add_member_tags(
                    arguments["list_id"], arguments["email_address"], arguments["tags"]
                )
                ...
                result = await remove_member_tags(
                    arguments["list_id"], arguments["email_address"], arguments["tags"]
                )
                ...
                result = await get_member_activity(
                    arguments["list_id"],
            ...
            # Campaign tools
                ...
                if not arguments.get("campaign_id") or not arguments.get(
                    "schedule_time"
                ):
                    raise ValueError(
                        "Missing required arguments: campaign_id and schedule_time"
                    )
