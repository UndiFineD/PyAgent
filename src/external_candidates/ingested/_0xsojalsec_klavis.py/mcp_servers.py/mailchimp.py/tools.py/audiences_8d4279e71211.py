...
async def get_all_audiences(count: int = 10, offset: int = 0) -> Dict[str, Any]:
    ...
    logger.info(
        f"Executing tool: get_all_audiences with count: {count}, offset: {offset}"
    )
    try:
        endpoint = "/lists"
        ...
        for audience in audiences_data.get("lists", []):
            audience_info = {
                ...
                "stats": {
                    ...
                    "unsubscribe_count": audience.get("stats", {}).get(
                        "unsubscribe_count"
                    ),
                    "cleaned_count": audience.get("stats", {}).get("cleaned_count"),
                    "member_count_since_send": audience.get("stats", {}).get(
                        "member_count_since_send"
                    ),
                    "unsubscribe_count_since_send": audience.get("stats", {}).get(
                        "unsubscribe_count_since_send"
                    ),
                    "cleaned_count_since_send": audience.get("stats", {}).get(
                        "cleaned_count_since_send"
                    ),
                    "campaign_count": audience.get("stats", {}).get("campaign_count"),
                    "campaign_last_sent": audience.get("stats", {}).get(
                        "campaign_last_sent"
                    ),
                    "merge_field_count": audience.get("stats", {}).get(
                        "merge_field_count"
                    ),
...
async def create_audience(
    ...
    try:
        ...
        audience_data = await make_mailchimp_request(
...
async def update_audience(
    ...
    try:
        ...
        audience_data = await make_mailchimp_request(
            "PATCH", endpoint, json_data=payload
        )
