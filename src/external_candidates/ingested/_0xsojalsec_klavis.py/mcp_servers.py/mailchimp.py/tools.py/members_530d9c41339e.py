# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\mailchimp\tools\members.py
import hashlib
import logging
...
async def get_audience_members(
    ...
    logger.info(
        f"Executing tool: get_audience_members with list_id: {list_id}, count: {count}, offset: {offset}"
    )
    try:
        ...
        for member in members_data.get("members", []):
            member_info = {
                ...
                "consents_to_one_to_one_messaging": member.get(
...
async def add_member_to_audience(
    ...
    logger.info(
        f"Executing tool: add_member_to_audience with list_id: {list_id}, email: {email_address}"
    )
    try:
        endpoint = f"/lists/{list_id}/members"

        payload = {"email_address": email_address, "status": status}
...
async def get_member_info(list_id: str, email_address: str) -> Dict[str, Any]:
    ...
    logger.info(
        f"Executing tool: get_member_info with list_id: {list_id}, email: {email_address}"
    )
    try:
        ...
        result = {
            ...
            "consents_to_one_to_one_messaging": member_data.get(
                "consents_to_one_to_one_messaging"
            ),
            "merge_fields": member_data.get("merge_fields", {}),
            "interests": member_data.get("interests", {}),
...
async def update_member(
    ...
    logger.info(
        f"Executing tool: update_member with list_id: {list_id}, email: {email_address}"
    )
    try:
        subscriber_hash = _get_subscriber_hash(email_address)
        endpoint = f"/lists/{list_id}/members/{subscriber_hash}"
...
async def delete_member(list_id: str, email_address: str) -> Dict[str, Any]:
    ...
    logger.info(
        f"Executing tool: delete_member with list_id: {list_id}, email: {email_address}"
    )
    try:
        subscriber_hash = _get_subscriber_hash(email_address)
        endpoint = f"/lists/{list_id}/members/{subscriber_hash}"
...
async def add_member_tags(
    list_id: str, email_address: str, tags: List[str]
) -> Dict[str, Any]:
    """Add tags to a specific audience member."""
    logger.info(
        f"Executing tool: add_member_tags with list_id: {list_id}, email: {email_address}, tags: {tags}"
    )
    try:
        subscriber_hash = _get_subscriber_hash(email_address)
        ...
        await make_mailchimp_request(
            "POST", endpoint, json_data=payload, expect_empty_response=True
        )
...
async def remove_member_tags(
    ...
    logger.info(
        f"Executing tool: remove_member_tags with list_id: {list_id}, email: {email_address}, tags: {tags}"
    )
    try:
        subscriber_hash = _get_subscriber_hash(email_address)
        endpoint = f"/lists/{list_id}/members/{subscriber_hash}/tags"
...
async def get_member_activity(
    list_id: str, email_address: str, count: int = 10
) -> Dict[str, Any]:
    """Get the last 50 events of a member's activity on a specific list."""
    logger.info(
        f"Executing tool: get_member_activity with list_id: {list_id}, email: {email_address}"
    )
    try:
        subscriber_hash = _get_subscriber_hash(email_address)
