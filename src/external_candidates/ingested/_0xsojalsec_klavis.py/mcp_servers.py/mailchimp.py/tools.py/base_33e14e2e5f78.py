# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\mailchimp\tools\base.py
import logging
import os
import ssl
...
def get_mailchimp_api_key() -> str:
    ...
    try:
        ...
        if not api_key:
            raise RuntimeError(
                "Mailchimp API key not found in request context or environment"
            )
        return api_key
...
def _extract_datacenter_from_api_key(api_key: str) -> str:
    ...
    if "-" in api_key:
        ...
        raise ValueError(
            "Invalid Mailchimp API key format. Expected format: key-datacenter"
        )
...
async def make_mailchimp_request(
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            logger.error(
                f"Mailchimp API request failed: {e.status} {e.message} for {method} {url}"
            )
            ...
            try:
                ...
                pass
            raise RuntimeError(
                f"Mailchimp API Error ({e.status}): {error_details}"
            ) from e
        except Exception as e:
            logger.error(
                f"An unexpected error occurred during Mailchimp API request: {e}"
            )
            raise RuntimeError(
