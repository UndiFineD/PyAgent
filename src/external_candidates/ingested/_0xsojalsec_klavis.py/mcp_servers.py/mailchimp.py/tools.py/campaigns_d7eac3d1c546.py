# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\mailchimp\tools\campaigns.py
import logging
...
async def get_all_campaigns(
    ...
    logger.info(
        f"Executing tool: get_all_campaigns with count: {count}, offset: {offset}"
    )
    try:
        endpoint = "/campaigns"
        ...
        result = {"total_items": campaigns_data.get("total_items"), "campaigns": []}
        ...
        return {"error": "Failed to retrieve campaigns", "exception": str(e)}
...
async def create_campaign(
    ...
    logger.info(
        f"Executing tool: create_campaign with type: {type}, list_id: {list_id}, subject: {subject_line}"
    )
    try:
        endpoint = "/campaigns"
        ...
        campaign_data = await make_mailchimp_request(
...
async def schedule_campaign(
    ...
    logger.info(
        f"Executing tool: schedule_campaign with campaign_id: {campaign_id}, schedule_time: {schedule_time}"
    )
    try:
        endpoint = f"/campaigns/{campaign_id}/actions/schedule"

        payload = {"schedule_time": schedule_time, "timewarp": timewarp}
