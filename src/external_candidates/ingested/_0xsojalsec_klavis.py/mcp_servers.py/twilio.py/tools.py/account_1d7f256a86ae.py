# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\twilio\tools\account.py
import logging
...
async def twilio_get_usage_records(
    ...
    try:
        ...
        if granularity.lower() == "daily":
            ...
            raise ValueError(
                "Granularity must be one of: daily, monthly, yearly, all-time"
            )
...
async def twilio_get_usage_triggers(limit: int = 50) -> dict:
    ...
    try:
            ...
            trigger_list.append(
                {
                    ...
                    "date_created": (
                        trigger.date_created.isoformat()
                        if trigger.date_created
                        else None
                    ),
                    "date_fired": (
                        trigger.date_fired.isoformat() if trigger.date_fired else None
                    ),
                    "date_updated": (
                        trigger.date_updated.isoformat()
                        if trigger.date_updated
                        else None
                    ),
                }
            )
        ...
        return {"usage_triggers": trigger_list, "count": len(trigger_list)}
    except Exception as e:
