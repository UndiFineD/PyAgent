# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\twilio\tools\phone_numbers.py
import logging
...
async def twilio_search_available_numbers(
    ...
    try:
        ...
        if country_code.upper() == "US":
            ...
            numbers = client.available_phone_numbers(country_code.upper()).local.list(
                **search_params
            )
...
async def twilio_purchase_phone_number(
    ...
    try:
        ...
        return {
            ...
            "date_created": (
                incoming_number.date_created.isoformat()
                if incoming_number.date_created
                else None
            ),
        }
    except Exception as e:
...
async def twilio_list_phone_numbers(limit: int = 20) -> dict:
    ...
    try:
            ...
            number_list.append(
                {
                    ...
                    "date_updated": (
                        number.date_updated.isoformat() if number.date_updated else None
                    ),
                }
            )
        ...
        return {"phone_numbers": number_list, "count": len(number_list)}
    except Exception as e:
...
async def twilio_update_phone_number(
    ...
    try:
        ...
        incoming_number = client.incoming_phone_numbers(phone_number_sid).update(
            **update_params
        )
        ...
        return {
            ...
            "date_updated": (
                incoming_number.date_updated.isoformat()
                if incoming_number.date_updated
                else None
            ),
        }
    except Exception as e:
