# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\twilio\server.py
import asyncio
import contextlib
import json
import logging
...
from tools import (  # Messaging; Voice; Phone Numbers; Account
    auth_token_context,
...
@click.command()
@click.option(
    "--port", default=TWILIO_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
...
def run_http_mode(port: int, json_response: bool) -> int:
    ...
    async def handle_sse(request):
        ...
        auth_token = request.headers.get("x-auth-token") or os.getenv(
            "TWILIO_AUTH_TOKEN"
        )
