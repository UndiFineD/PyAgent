# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\linkedin\server.py
import base64
import contextlib
import json
import logging
...
@click.command()
@click.option(
    "--port", default=LINKEDIN_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
...
def main(
    ...
    async def call_tool(
        ...
        if name == "linkedin_create_post":
            ...
            try:
                result = await create_url_share(
                    url, text, title, description, visibility
                )
                return [
                    types.TextContent(
                        type="text",
