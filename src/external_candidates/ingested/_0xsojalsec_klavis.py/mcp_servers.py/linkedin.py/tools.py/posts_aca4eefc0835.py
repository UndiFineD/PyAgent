# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\linkedin\tools\posts.py
import logging
import urllib.parse
...
def format_rich_post(
    ...
    try:
        ...
        if numbered_list:
            numbered_section = "\n\n" + "\n".join(
                [f"{i+1}. {item}" for i, item in enumerate(numbered_list)]
            )
...
async def create_post(
    ...
    try:
        ...
        if title:
            result["title"] = title
            result["note"] = (
                "Created as text post with article format (title + content)"
            )
        ...
        if title:
            ...
            error_result["error"] = (
                "Article creation failed - trying alternative approach"
            )
            ...
            "specificContent": {
                "com.linkedin.ugc.ShareContent": {
                    ...
                    "media": [
                        {
                            "status": "READY",
                            "description": {"text": description or "Shared link"},
                            "originalUrl": url,
                            "title": {"text": title or "Shared Content"},
                        }
                    ],
                }
            },
