...
def get_linkedin_access_token() -> str:
    ...
    try:
        ...
        if not token:
            raise RuntimeError(
                "LinkedIn access token not found in request context or environment"
            )
        return token
...
async def make_linkedin_request(
    ...
    async with aiohttp.ClientSession(headers=headers, connector=connector) as session:
        try:
            ...
            logger.error(
                f"LinkedIn API request failed: {e.status} {e.message} for {method} {url}"
            )
            ...
            try:
                ...
                pass
            raise RuntimeError(
                f"LinkedIn API Error ({e.status}): {error_details}"
            ) from e
        except Exception as e:
            logger.error(
                f"An unexpected error occurred during LinkedIn API request: {e}"
            )
            raise RuntimeError(
