# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\asana\tools\tasks.py
import base64
import json
import logging
...
def validate_date_format(name: str, date_str: str | None) -> None:
    ...
    try:
        ...
        raise AsanaToolExecutionError(
            f"Invalid {name} date format. Use the format YYYY-MM-DD."
        )
...
async def handle_new_task_associations(
    ...
    if not workspace_id and parent_task_id:
        ...
        response = await client.get(
            f"/tasks/{parent_task_id}", params={"opt_fields": "workspace"}
        )
...
async def get_project_by_name_or_raise_error(
    ...
    if not response["matches"]["projects"]:
        ...
        projects = [
            {"name": project["name"], "id": project["id"]} for project in projects
        ]
        message = (
        ...
        additional_prompt = (
            f"Projects matching the name '{project_name}': {json.dumps(projects)}"
        )
        raise RetryableToolError(
            message=message,
...
async def find_projects_by_name(
    ...
    result = {"matches": {"projects": matches, "count": len(matches)}}

    if return_projects_not_matched:
        result["not_matched"] = {"projects": all_projects, "tags": not_matched}
...
async def find_tags_by_name(
    ...
    result = {
        "matches": {"tags": matches, "count": len(matches)},
        "not_found": {"tags": not_found},
    }
...
async def get_task_by_id(
    ...
    try:
        ...
        if max_subtasks > 0:
            ...
            subtasks = await get_subtasks_from_a_task(
                task_id=task_id, limit=max_subtasks
            )
...
async def update_task(
    ...
    try:
        ...
        response = await client.put(
            f"/tasks/{task_id}", json_data={"data": update_data}
        )
        return {"task": response["data"]}
...
async def attach_file_to_task(
    ...
    try:
        ...
        if not any([file_content_str, file_content_base64, file_content_url]):
            raise ValueError(
                "One of file_content_str, file_content_base64, or file_content_url must be provided"
            )
        ...
        if file_content_url:
            ...
            response = await client.post(
                "/attachments", json_data={"data": attachment_data}
            )
        else:
            # File upload
            if file_content_str:
