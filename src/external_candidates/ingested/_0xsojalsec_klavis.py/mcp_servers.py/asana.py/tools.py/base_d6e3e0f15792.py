# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\asana\tools\base.py
import asyncio
import json
import logging
...
def get_next_page(response: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"next_page_token": next_page.get("uri") if next_page else None}
...
class AsanaClient:
    ...
    def _build_error_messages(self, response: httpx.Response) -> tuple[str, str]:
        try:
            ...
            developer_message = (
                f"Failed to parse Asana error response: {type(e).__name__}: {e!s}"
            )
