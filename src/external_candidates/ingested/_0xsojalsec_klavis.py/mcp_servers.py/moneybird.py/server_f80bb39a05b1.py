# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\moneybird\server.py
import base64
import contextlib
import json
import logging
...
@click.command()
@click.option(
    "--port", default=MONEYBIRD_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
...
def main(
        ...
        return [
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(
                    **{"category": "MONEYBIRD_ADMINISTRATION"}
                ),
            ),
            # Contacts
            types.Tool(
    ...
    async def call_tool(
        ...
        if name == "moneybird_list_contacts":
            ...
            try:
                result = await moneybird_create_contact_person(
        ...
        # Sales Invoices
            ...
            try:
                result = await moneybird_list_sales_invoices(
                    administration_id,
                    state,
                    period,
                    contact_id,
            ...
            try:
                result = await moneybird_get_sales_invoice(
                    administration_id, invoice_id
                )
                return [
                    types.TextContent(
                        type="text",
            ...
            try:
                result = await moneybird_create_sales_invoice(
                    administration_id, invoice_data
                )
                return [
                    types.TextContent(
                        type="text",
        ...
        # Projects & Time
            ...
            try:
                result = await moneybird_list_time_entries(
                    administration_id, period, contact_id, project_id, user_id, page
                )
                return [
                    types.TextContent(
                        type="text",
