# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\moneybird\tools\contacts.py
import logging
...
async def moneybird_list_contacts(
    administration_id: str, query: Optional[str] = None, page: Optional[int] = None
) -> Dict[str, Any]:
...
async def moneybird_get_contact(
    administration_id: str, contact_id: str
) -> Dict[str, Any]:
...
async def moneybird_create_contact(
    administration_id: str, contact_data: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        return await make_request(
            "POST", administration_id, "/contacts", data=contact_data
        )
    except Exception as e:
...
async def moneybird_create_contact_person(
    administration_id: str, contact_id: str, contact_person_data: Dict[str, Any]
) -> Dict[str, Any]:
    """Create a new contact person within an existing contact in Moneybird."""
    logger.info(
        f"Executing tool: moneybird_create_contact_person for contact_id: {contact_id}"
    )
    ...
    try:
        return await make_request(
            "POST",
            administration_id,
            f"/contacts/{contact_id}/contact_people",
