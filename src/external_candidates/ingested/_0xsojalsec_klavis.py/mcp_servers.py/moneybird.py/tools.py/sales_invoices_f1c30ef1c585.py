# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\moneybird\tools\sales_invoices.py
import logging
...
async def moneybird_list_sales_invoices(
    ...
    try:
        return await make_request(
            "GET", administration_id, "/sales_invoices", params=params
        )
...
async def moneybird_get_sales_invoice(
    administration_id: str, invoice_id: str
) -> Dict[str, Any]:
    """Get details for a specific sales invoice by ID."""
    logger.info(
        f"Executing tool: moneybird_get_sales_invoice for invoice_id: {invoice_id}"
    )

    try:
        return await make_request(
            "GET", administration_id, f"/sales_invoices/{invoice_id}"
        )
    except Exception as e:
...
async def moneybird_create_sales_invoice(
    administration_id: str, invoice_data: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        return await make_request(
            "POST", administration_id, "/sales_invoices", data=invoice_data
        )
    except Exception as e:
