# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\moneybird\tools\base.py
import logging
import os
...
def get_auth_token() -> str:
    ...
    try:
        ...
        if not token:
            ...
            if not token:
                raise RuntimeError(
                    "No Moneybird auth token found in context or environment"
                )
        return token
    except LookupError:
...
class MoneybirdClient:
    ...
    async def make_request(
        ...
        if not api_key:
            raise RuntimeError(
                "No API key provided. Please set the x-auth-token header."
            )
...
async def make_request(
    ...
    """Make an HTTP request to Moneybird API."""
    return await MoneybirdClient.make_request(
        method, administration_id, endpoint, data, params
    )
