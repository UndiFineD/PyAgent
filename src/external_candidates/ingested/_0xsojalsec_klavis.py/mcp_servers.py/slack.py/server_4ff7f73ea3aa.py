# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\slack\server.py
import base64
import contextlib
import json
import logging
...
from user_tools import invite_users_to_channel
from user_tools import list_channels as user_list_channels
from user_tools import user_token_context
...
def extract_access_tokens(request_or_scope) -> tuple[str, str]:
    ...
    try:
        ...
        user_token = auth_json.get("authed_user", {}).get(
            "access_token", ""
        )  # User token in authed_user
        return bot_token, user_token
...
@click.command()
@click.option(
    "--port", default=SLACK_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "SLACK_CHANNEL"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "SLACK_USER"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "SLACK_MESSAGE"}),
            ),
            # User Messages
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "SLACK_REACTION"}),
            ),
        ...
        # User Search
            ...
            try:
                result = await user_search_messages(
                    query, channel_ids, sort, sort_dir, count, cursor, highlight
                )
                return [
                    types.TextContent(
                        type="text",
