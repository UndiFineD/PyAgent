...
async def bot_reply_to_thread(
    ...
    logger.info(
        f"Executing tool: bot_reply_to_thread in channel {channel_id}, thread {thread_ts}"
    )
...
async def bot_add_reaction(
    ...
    logger.info(
        f"Executing tool: bot_add_reaction to message {timestamp} in channel {channel_id}"
    )
