# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\slack\user_tools\user_messages.py
import logging
...
async def user_post_message(channel_id: str, text: str) -> Dict[str, Any]:
...
async def user_reply_to_thread(
    ...
    logger.info(
        f"Executing tool: user_reply_to_thread in channel {channel_id}, thread {thread_ts}"
    )
...
async def user_add_reaction(
    channel_id: str, timestamp: str, reaction: str
) -> Dict[str, Any]:
    """Add a reaction emoji to a message using user token."""
    logger.info(
        f"Executing tool: user_add_reaction to message {timestamp} in channel {channel_id}"
    )
