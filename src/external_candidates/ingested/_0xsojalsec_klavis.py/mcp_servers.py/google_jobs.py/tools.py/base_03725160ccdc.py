# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\google_jobs\tools\base.py
import logging
import os
import ssl
...
def get_serpapi_access_token() -> str:
    ...
    try:
        ...
        if not token:
            raise RuntimeError(
                "SerpApi API key not found in request context or environment"
            )
        return token
...
def _get_serpapi_headers() -> Dict[str, str]:
    ...
    return {"User-Agent": "MCP Google Jobs Server", "Accept": "application/json"}
...
async def make_serpapi_request(
    params: Dict[str, Any], expect_empty_response: bool = False
) -> Any:
    """
            ...
            async with session.get(url, params=params) as response:
                ...
                if expect_empty_response:
                    if response.status in [200, 201, 204]:
                        ...
                        logger.warning(
                            f"Expected empty response for SerpApi request, but got status {response.status}"
                        )
                        try:
                    ...
                    if "application/json" in response.headers.get("Content-Type", ""):
                        ...
                        logger.warning(
                            f"Received non-JSON response from SerpApi: {text_content[:100]}..."
                        )
