# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\google_jobs\server.py
import base64
import contextlib
import json
import logging
...
@click.command()
@click.option(
    "--port", default=GOOGLE_JOBS_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
...
def main(
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(
                    **{"category": "GOOGLE_JOBS_SUGGESTION"}
                ),
            ),
        ]
    ...
    async def handle_streamable_http(
        ...
        logger.info(
            f"Handling StreamableHTTP request: {scope['method']} {scope['path']}"
        )
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Could not send error response, connection may be closed"
                )
        finally:
            serpapi_token_context.reset(token)
