    ...
    try:
        ...
        if company_contains:
            ...
            company_like_conditions = " OR ".join(
                [
                    f"Company LIKE '%{variation}%'"
                    for variation in set(company_variations)
                ]
            )
...
async def get_lead_by_id(
    lead_id: str, fields: Optional[List[str]] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.get("success"):
            return format_success_response(
                result.get("id"), "created", "Lead", lead_data
            )
        else:
            return {
                "success": False,
...
async def convert_lead(
    lead_id: str, conversion_data: Dict[str, Any] = None
) -> Dict[str, Any]:
