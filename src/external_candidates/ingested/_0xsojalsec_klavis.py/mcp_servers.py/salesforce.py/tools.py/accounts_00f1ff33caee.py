# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\salesforce\tools\accounts.py
import logging
...
async def get_accounts(
    ...
    logger.info(
        f"Executing tool: get_accounts with limit: {limit}, name_contains: {name_contains}, industry: {industry}, account_type: {account_type}"
    )
    try:
        ...
        if name_contains:
            ...
            name_like_conditions = " OR ".join(
                [f"Name LIKE '%{variation}%'" for variation in set(name_variations)]
            )
...
async def get_account_by_id(
    account_id: str, fields: Optional[List[str]] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.get("success"):
            return format_success_response(
                result.get("id"), "created", "Account", account_data
            )
        else:
            return {
                "success": False,
...
async def update_account(
    ...
    try:
        ...
        if result == 204:  # HTTP 204 No Content indicates successful update
            return format_success_response(
                account_id, "updated", "Account", account_data
            )
        else:
            return {
                "success": False,
