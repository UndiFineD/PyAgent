# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\salesforce\tools\cases.py
import logging
...
async def get_cases(
    ...
    logger.info(
        f"Executing tool: get_cases with account_id: {account_id}, status: {status}, priority: {priority}, limit: {limit}, subject_contains: {subject_contains}, case_type: {case_type}"
    )
    try:
        ...
        if subject_contains:
            ...
            subject_like_conditions = " OR ".join(
                [
                    f"Subject LIKE '%{variation}%'"
                    for variation in set(subject_variations)
                ]
            )
...
async def get_case_by_id(
    case_id: str, fields: Optional[List[str]] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.get("success"):
            return format_success_response(
                result.get("id"), "created", "Case", case_data
            )
        else:
            return {
                "success": False,
