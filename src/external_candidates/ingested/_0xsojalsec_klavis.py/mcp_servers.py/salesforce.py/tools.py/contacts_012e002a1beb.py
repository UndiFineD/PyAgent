...
async def get_contacts(
    ...
    try:
        ...
        if email_contains:
            ...
            email_like_conditions = " OR ".join(
                [f"Email LIKE '%{variation}%'" for variation in set(email_variations)]
            )
        ...
        if title_contains:
            ...
            title_like_conditions = " OR ".join(
                [f"Title LIKE '%{variation}%'" for variation in set(title_variations)]
            )
...
async def get_contact_by_id(
    contact_id: str, fields: Optional[List[str]] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.get("success"):
            return format_success_response(
                result.get("id"), "created", "Contact", contact_data
            )
        else:
            return {
                "success": False,
...
async def update_contact(
    ...
    try:
        ...
        if result == 204:  # HTTP 204 No Content indicates successful update
            return format_success_response(
                contact_id, "updated", "Contact", contact_data
            )
        else:
            return {
                "success": False,
