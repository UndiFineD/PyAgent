# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\salesforce\tools\base.py
import logging
...
def get_salesforce_conn() -> Salesforce:
    ...
    try:
        ...
        if not access_token or not instance_url:
            raise RuntimeError(
                "Salesforce access token and instance URL are required. Provide them via x-auth-token and x-instance-url headers."
            )
...
def format_success_response(
    record_id: str, operation: str, object_type: str, data: Dict[str, Any] = None
) -> Dict[str, Any]:
...
def create_case_insensitive_like_conditions(search_term: str, *field_names: str) -> str:
    ...
    for field_name in field_names:
        field_conditions = [
            f"{field_name} LIKE '%{variation}%'" for variation in set(variations)
        ]
