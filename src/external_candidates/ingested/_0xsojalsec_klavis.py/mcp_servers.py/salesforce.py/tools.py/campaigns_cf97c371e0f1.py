# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\salesforce\tools\campaigns.py
import logging
...
async def get_campaigns(
    ...
    logger.info(
        f"Executing tool: get_campaigns with status: {status}, type: {type_filter}, limit: {limit}"
    )
    try:
...
async def get_campaign_by_id(
    campaign_id: str, fields: Optional[List[str]] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.get("success"):
            return format_success_response(
                result.get("id"), "created", "Campaign", campaign_data
            )
        else:
            return {
                "success": False,
...
async def update_campaign(
    campaign_id: str, campaign_data: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        if result == 204:  # HTTP 204 No Content indicates successful update
            return format_success_response(
                campaign_id, "updated", "Campaign", campaign_data
            )
        else:
            return {
                "success": False,
