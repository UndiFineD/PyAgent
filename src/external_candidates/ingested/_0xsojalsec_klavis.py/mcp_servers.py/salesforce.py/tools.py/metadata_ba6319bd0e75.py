# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\salesforce\tools\metadata.py
import logging
...
async def describe_object(object_name: str, detailed: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        if detailed and object_name.endswith("__c"):
            ...
            return {"describe": dict(result), "metadata": metadata_result}
...
async def get_component_source(
    metadata_type: str, component_names: List[str]
) -> Dict[str, Any]:
        ...
        for name in component_names:
            try:
                if metadata_type == "ApexClass":
                    query = (
                        f"SELECT Id, Name, Body FROM ApexClass WHERE Name = '{name}'"
                    )
                elif metadata_type == "ApexTrigger":
                    query = (
                        f"SELECT Id, Name, Body FROM ApexTrigger WHERE Name = '{name}'"
                    )
                ...
                result = sf.toolingexecute(f"query/?q={query}")
                results.append(
                    {"name": name, "type": metadata_type, "data": dict(result)}
                )
            except Exception as e:
                results.append({"name": name, "type": metadata_type, "error": str(e)})
