# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\salesforce\tools\opportunities.py
import logging
...
async def get_opportunities(
    ...
    logger.info(
        f"Executing tool: get_opportunities with account_id: {account_id}, stage: {stage}, name_contains: {name_contains}, account_name_contains: {account_name_contains}, limit: {limit}"
    )
    try:
        ...
        if account_name_contains:
            ...
            # Create OR conditions for different case variations
            account_like_conditions = " OR ".join(
                [
                    f"Account.Name LIKE '%{variation}%'"
                    for variation in set(account_variations)
                ]
            )
...
async def get_opportunity_by_id(
    opportunity_id: str, fields: Optional[List[str]] = None
) -> Dict[str, Any]:
    """Get a specific opportunity by ID."""
    logger.info(
        f"Executing tool: get_opportunity_by_id with opportunity_id: {opportunity_id}"
    )
    try:
        sf = get_salesforce_conn()
...
async def create_opportunity(opportunity_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        missing_fields = [
            field for field in required_fields if field not in opportunity_data
        ]
        ...
        if result.get("success"):
            return format_success_response(
                result.get("id"), "created", "Opportunity", opportunity_data
            )
        else:
            return {
                "success": False,
...
async def update_opportunity(
    ...
    logger.info(
        f"Executing tool: update_opportunity with opportunity_id: {opportunity_id}"
    )
    try:
        sf = get_salesforce_conn()
        ...
        if result == 204:  # HTTP 204 No Content indicates successful update
            return format_success_response(
                opportunity_id, "updated", "Opportunity", opportunity_data
            )
        else:
            return {
                "success": False,
...
async def delete_opportunity(opportunity_id: str) -> Dict[str, Any]:
    ...
    logger.info(
        f"Executing tool: delete_opportunity with opportunity_id: {opportunity_id}"
    )
    try:
        sf = get_salesforce_conn()
