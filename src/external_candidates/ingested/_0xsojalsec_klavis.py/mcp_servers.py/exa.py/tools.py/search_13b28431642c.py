# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\exa\tools\search.py
import logging
...
async def exa_search(
    ...
    try:
        ...
        response = {
            ...
            "autoprompt_string": (
                result.autoprompt_string
                if hasattr(result, "autoprompt_string")
                else None
            ),
        }
        ...
        return {
            "error": f"Could not complete Exa search for query: {query}. Error: {str(e)}"
        }
...
async def exa_find_similar(
    ...
    try:
        ...
        return {
            "error": f"Could not find similar pages for URL: {url}. Error: {str(e)}"
        }
...
async def exa_answer(
    ...
    try:
            ...
            "sources": [
                {
                    ...
                    "text": (
                        r.text[:500] + "..."
                        if hasattr(r, "text") and r.text and len(r.text) > 500
                        else r.text if hasattr(r, "text") else None
                    ),
                }
                for r in result.results
            ],
        ...
        return {
            "error": f"Could not get Exa answer for query: {query}. Error: {str(e)}"
        }
...
async def exa_research(
    ...
    try:
        ...
        return {
            "error": f"Could not complete Exa research for query: {query}. Error: {str(e)}"
        }
