# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\youtube\server.py
import asyncio
import contextlib
import logging
import os
...
TRANSCRIPT_LANGUAGES = [
    lang.strip() for lang in os.getenv("TRANSCRIPT_LANGUAGE", "en").split(",")
]
...
async def _make_youtube_request(
    endpoint: str, params: Dict[str, Any], headers: Dict[str, Any] = None
) -> Any:
    """
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            logger.error(
                f"YouTube API request failed: {e.status} {e.message} for GET {url}"
            )
            ...
            try:
                ...
                pass
            raise RuntimeError(
                f"YouTube API Error ({e.status}): {error_details}"
            ) from e
        except Exception as e:
            logger.error(
                f"An unexpected error occurred during YouTube API request: {e}"
            )
...
@click.command()
@click.option(
    "--port", default=YOUTUBE_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
...
def main(
    ...
    async def get_youtube_video_transcript(url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Executing tool: get_video_transcript with video_id: {video_id}"
            )

            try:
                # Use the initialized API with or without proxy
                raw_transcript = youtube_transcript_api.fetch(
                ...
                logger.warning(
                    f"Error fetching transcript: {transcript_error}. Falling back to video details."
                )
            ...
            return {"error": f"Failed to process request: {error_message}"}
