# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\google_calendar\server.py
import base64
import contextlib
import json
import logging
...
GOOGLE_CALENDAR_MCP_SERVER_PORT = int(
    os.getenv("GOOGLE_CALENDAR_MCP_SERVER_PORT", "5000")
)
...
async def list_events(
    ...
    logger.info(
        f"Executing tool: list_events from {min_end_datetime} to {max_start_datetime}"
    )
    try:
        access_token = get_auth_token()
        service = get_calendar_service(access_token)
...
async def update_event(
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"Event with ID {event_id} not found. Available events: {valid_events_with_id}"
            )
        ...
        if updated_start_datetime:
            update_fields["start"] = {
                "dateTime": updated_start_datetime,
                "timeZone": time_zone,
            }

        if updated_end_datetime:
            update_fields["end"] = {
                "dateTime": updated_end_datetime,
                "timeZone": time_zone,
            }
...
@click.command()
@click.option(
    "--port", default=GOOGLE_CALENDAR_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
        ...
        if name == "google_calendar_list_calendars":
            try:
                ...
                result = await list_calendars(
                    max_results, show_deleted, show_hidden, next_page_token
                )
                return [
                    types.TextContent(
                        type="text",
            ...
            try:
                ...
                result = await list_events(
                    min_end_datetime, max_start_datetime, calendar_id, max_results
                )
                return [
                    types.TextContent(
                        type="text",
            ...
            try:
                ...
                result = await add_attendees_to_event(
                    event_id, attendees, calendar_id, send_updates
                )
                return [
                    types.TextContent(
                        type="text",
