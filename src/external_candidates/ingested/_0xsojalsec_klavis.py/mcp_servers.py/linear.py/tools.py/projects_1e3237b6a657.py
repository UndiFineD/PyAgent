# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\linear\tools\projects.py
import logging
...
async def get_projects(
    team_id: str = None, limit: int = 50, filter: Dict[str, Any] = None
) -> Dict[str, Any]:
    """Get projects with optional filtering by team and timestamps."""
    logger.info(
        f"Executing tool: get_projects with team_id: {team_id}, limit: {limit}, filter: {filter}"
    )
    try:
