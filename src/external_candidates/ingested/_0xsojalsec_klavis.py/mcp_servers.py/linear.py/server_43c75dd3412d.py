...
@click.command()
@click.option(
    "--port", default=LINEAR_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
        ...
        if name == "linear_get_teams":
            ...
            try:
                result = await create_project(
                    name, description, team_ids, lead_id, target_date
                )
                return [
                    types.TextContent(
                        type="text",
            ...
            try:
                result = await update_project(
                    project_id, name, description, state, target_date, lead_id
                )
                return [
                    types.TextContent(
                        type="text",
