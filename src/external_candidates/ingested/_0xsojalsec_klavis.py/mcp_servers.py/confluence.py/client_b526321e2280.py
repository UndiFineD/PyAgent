...
class ConfluenceClientV2(ConfluenceClient):
    ...
    def transform_get_spaces_response(
        ...
        to absolute URLs using the base URL from the response.
        """
        pagination_token = parse_qs(
            urlparse(response.get("_links", {}).get("next", "")).query
        ).get(
            "cursor",
    ...
    async def get_root_pages_in_space(self, space_id: str) -> dict[str, Any]:
        ...
        return {
            "pages": [
                self._transform_links(page, base_url) for page in pages["results"]
            ]
        }
    ...
    async def get_page_id(self, page_identifier: str) -> str:
        ...
        if page_identifier.isdigit():
            ...
            if not page_data:
                raise ToolExecutionError(
                    message=f"No page found with title: '{page_identifier}'"
                )
            return page_data["id"]
    ...
    async def process_page_descendants(
        self, root_children: list, base_url: str
    ) -> None:
            ...
            if root_child["type"] == "page":
                try:
                    ...
                    response = await self.get(
                        f"content/{root_child['id']}/descendant", params=params
                    )
                    ...
                    if descendants:
                        ...
                        for desc in descendants:
                            ...
                            # Build URL
                            child_node["url"] = (
                                build_child_url(base_url, child_node) or ""
                            )
                        ...
                        # Build hierarchy
                        build_hierarchy(
                            transformed_children, root_child["id"], root_child
                        )
