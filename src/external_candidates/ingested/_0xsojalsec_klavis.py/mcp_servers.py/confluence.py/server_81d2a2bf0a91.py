# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\confluence\server.py
import base64
import contextlib
import json
import logging
...
from enums import (
    ...
    convert_sort_order_to_enum,
    convert_update_mode_to_enum,
)
from errors import (
    AuthenticationError,
    InvalidTokenError,
...
# Import tools
from tools import (  # Page tools; Space tools; Search tools; Attachment tools
    create_page,
    create_space,
    get_attachment,
...
@click.command()
@click.option(
    "--port", default=CONFLUENCE_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "CONFLUENCE_PAGE"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "CONFLUENCE_SPACE"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "CONFLUENCE_SEARCH"}),
            ),
            # Attachment tools
            types.Tool(
        ...
        try:
            ...
            if name == "confluence_create_page":
                ...
                result = await update_page_content(
                    page_identifier=arguments["page_identifier"],
                    content=arguments["content"],
                    update_mode=convert_update_mode_to_enum(
