# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\google_sheets\models.py
import json
...
class SheetDataInput(BaseModel):
    ...
    def _validate_inner_cells(cls, cells, row_int: int) -> dict:  # type: ignore[no-untyped-def]
        ...
        for col_key, cell_value in cells.items():
            ...
            if not col_string.isalpha():
                raise TypeError(
                    f"Column key '{col_key}' is invalid. Must be alphabetic."
                )
    ...
    def validate_and_convert_keys(cls, value):  # type: ignore[no-untyped-def]
        ...
        raise TypeError(
            "data must be a dict or a valid JSON string representing a dict"
        )
