...
async def get_spreadsheet_tool(spreadsheet_id: str) -> Dict[str, Any]:
    ...
    logger.info(
        f"Executing tool: get_spreadsheet with spreadsheet_id: {spreadsheet_id}"
    )
    try:
        access_token = get_auth_token()
        service = get_sheets_service(access_token)
...
async def write_to_cell_tool(
    ...
    logger.info(
        f"Executing tool: write_to_cell with spreadsheet_id: {spreadsheet_id}, cell: {column}{row}"
    )
    try:
        access_token = get_auth_token()
        service = get_sheets_service(access_token)
...
@click.command()
@click.option(
    "--port", default=GOOGLE_SHEETS_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
...
def main(
    ...
    async def call_tool(
        ...
        if name == "google_sheets_create_spreadsheet":
            ...
            try:
                result = await write_to_cell_tool(
                    spreadsheet_id, column, row, value, sheet_name
                )
                return [
                    types.TextContent(
                        type="text",
