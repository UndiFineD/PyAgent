# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\google_sheets\utils.py
...
def create_sheet(sheet_data_input: SheetDataInput) -> Sheet:
    ...
    """
    (_, max_row), (min_col_index, max_col_index) = compute_sheet_data_dimensions(
        sheet_data_input
    )
        ...
        try:
            ...
            cell_format = CellFormat(
                numberFormat=NumberFormat(
                    type=NumberFormatType.CURRENCY, pattern="$#,##0.00"
                )
            )
            cell_val = CellExtendedValue(numberValue=num_value)
            return CellData(userEnteredValue=cell_val, userEnteredFormat=cell_format)
...
def _create_int_cell(cell_value: int) -> CellData:
    ...
    return CellData(
        userEnteredValue=CellExtendedValue(numberValue=cell_value),
...
def validate_write_to_cell_params(  # type: ignore[no-any-unimported]
    ...
    if row < 1:
        raise ToolExecutionError(
            message=(
                f"Invalid row number {row}. It must be a positive integer greater than 0."
            ),
        )
    ...
    sheet_row_count = sheet_properties["sheets"][0]["properties"]["gridProperties"][
        "rowCount"
    ]
