...
async def _make_discord_request(
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        try:
            ...
            logger.error(
                f"Discord API request failed: {e.status} {e.message} for {method} {url}"
            )
            ...
            try:
                ...
                pass
            raise RuntimeError(
                f"Discord API Error ({e.status}): {error_details}"
            ) from e
        except Exception as e:
            logger.error(
                f"An unexpected error occurred during Discord API request: {e}"
            )
            raise RuntimeError(
...
async def list_members(server_id: str, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    logger.info(
        f"Executing tool: list_members with server_id: {server_id}, limit: {limit}"
    )
    try:
        ...
        if not isinstance(members_data, list):
            logger.error(
                f"Unexpected response type for list_members: {type(members_data)}"
            )
...
async def add_reaction(channel_id: str, message_id: str, emoji: str) -> str:
    ...
    logger.info(
        f"Executing tool: add_reaction '{emoji}' to message {message_id} in channel {channel_id}"
    )
    try:
        ...
        await _make_discord_request(
            "PUT", endpoint, expect_empty_response=True
...
async def add_multiple_reactions(
    ...
    logger.info(
        f"Executing tool: add_multiple_reactions {emojis} to message {message_id} in channel {channel_id}"
    )
    added_emojis = []
    errors = []
...
async def remove_reaction(channel_id: str, message_id: str, emoji: str) -> str:
    ...
    logger.info(
        f"Executing tool: remove_reaction '{emoji}' from message {message_id} in channel {channel_id}"
    )
    try:
        ...
        endpoint = f"/channels/{channel_id}/messages/{message_id}/reactions/{encoded_emoji}/@me"
        await _make_discord_request(
            "DELETE", endpoint, expect_empty_response=True
        )  # Expects 204 No Content
...
async def read_messages(channel_id: str, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    logger.info(
        f"Executing tool: read_messages from channel {channel_id}, limit: {limit}"
    )
    try:
        ...
        if not isinstance(messages_data, list):
            logger.error(
                f"Unexpected response type for read_messages: {type(messages_data)}"
            )
...
@click.command()
@click.option(
    "--port", default=DISCORD_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
