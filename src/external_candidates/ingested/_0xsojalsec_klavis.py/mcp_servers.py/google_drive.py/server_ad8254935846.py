...
async def create_shared_drive(
    ...
    logger.info(
        f"Executing tool: create_shared_drive with name: {name}, request_id: {request_id}"
    )
    try:
        access_token = get_auth_token()
        service = get_drive_service(access_token)
        ...
        result = (
            service.drives().create(body=drive_metadata, requestId=request_id).execute()
        )
        ...
        for drive_id, drive_files in file_tree.items():
            if drive_id == "My Drive":
                ...
                try:
                    ...
                    drive_name = drive_details.get(
                        "name", "Shared Drive (name unavailable)"
                    )
                except HttpError as e:
...
@click.command()
@click.option(
    "--port", default=GOOGLE_DRIVE_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
        ...
        if name == "google_drive_search_documents":
            try:
                result = await search_documents(
                    ...
                    search_only_in_shared_drive_id=arguments.get(
                        "search_only_in_shared_drive_id"
                    ),
                    include_shared_drives=arguments.get("include_shared_drives", False),
                    include_organization_domain_documents=arguments.get(
            ...
            try:
                result = await get_file_tree_structure(
                    include_shared_drives=arguments.get("include_shared_drives", False),
                    restrict_to_shared_drive_id=arguments.get(
                        "restrict_to_shared_drive_id"
                    ),
                    include_organization_domain_documents=arguments.get(
                        "include_organization_domain_documents", False
