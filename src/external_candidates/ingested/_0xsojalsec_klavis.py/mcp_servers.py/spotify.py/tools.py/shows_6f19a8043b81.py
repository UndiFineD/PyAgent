...
def get_multiple_shows(
    show_ids: List[str], sp: Optional[Spotify] = None, market: str = "US"
) -> List[Dict]:
    """
...
def get_show_episodes(
    ...
    try:
        ...
        response = sp.show_episodes(
            show_id=show_id, limit=limit, offset=offset, market=market
        )
...
def get_current_user_saved_shows(
    sp: Optional[Spotify] = None, limit: int = 20, offset: int = 0
) -> List[Dict]:
    """
...
def save_shows_to_user_library(
    show_ids: List[str], sp: Optional[Spotify] = None
) -> str:
    """
