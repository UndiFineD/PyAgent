# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\spotify\tools\tracks.py
...
def get_tracks_info(
    track_ids: List[str], sp: Optional[Spotify] = None
    ...
    try:
        ...
        return [
            t
            for t in (
                process_track_info(track) for track in tracks if track is not None
            )
            if t is not None
        ]
...
def remove_user_saved_tracks(track_ids: List[str], sp: Optional[Spotify] = None) -> str:
    """
