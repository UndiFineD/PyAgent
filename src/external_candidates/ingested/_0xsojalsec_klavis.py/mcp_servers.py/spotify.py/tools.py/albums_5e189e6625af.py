...
def get_albums_info(album_ids: List[str], sp: Spotify = None) -> List[Dict]:
    ...
    try:
        ...
        cleaned_results = [
            process_album_info(album) for album in albums if album is not None
        ]
...
def get_album_tracks(
    album_id: str, sp: Spotify = None, limit: int = 50, offset: int = 0
) -> List[Dict]:
    """
...
def get_user_saved_albums(
    ...
    try:
        ...
        items = results.get("items", [])
        return [
            process_album_info(item.get("album")) for item in items if item is not None
        ]
    except Exception as e:
...
def check_user_saved_albums(
    album_ids: List[str], sp: Spotify = None
) -> List[bool] | Dict:
    """
