...
def main(
    ...
    async def call_tool(
        ...
        if name == "spotify_search_tracks":
            ...
            logger.info(
                f"Searching tracks with query: {query}, type: {search_type}, limit: {limit}"
            )
            ...
            logger.info(
                f"Getting user saved tracks with limit: {limit}, offset: {offset}"
            )
            ...
            offset = arguments.get("offset", 0)
            logger.info(
                f"Getting user saved albums with limit: {limit}, offset: {offset}"
            )
            ...
            logger.info(
                f"Getting artist top tracks for artist_id: {artist_id}, country: {country}"
            )
            if not artist_id:
                return [
                    types.TextContent(
            ...
            logger.info(
                f"Getting episodes info for IDs: {episode_ids}, market: {market}"
            )
            if not episode_ids:
                return [
                    types.TextContent(
            ...
            offset = arguments.get("offset", 0)
            logger.info(
                f"Getting user saved episodes with limit: {limit}, offset: {offset}"
            )
            ...
            offset = arguments.get("offset", 0)
            logger.info(
                f"Getting user owned playlists for user_id: {user_id}, limit: {limit}, offset: {offset}"
            )
            ...
            result = update_playlist_details(
                playlist_id, name, public, description, sp_oauth
            )
            return [
                types.TextContent(
                    type="text",
            ...
            result = get_current_user_top_items(
                sp_oauth, item_type, time_range, limit, offset
            )
            return [
                types.TextContent(
                    type="text",
            ...
            offset = arguments.get("offset", 0)
            logger.info(
                f"Getting current user playlists with limit: {limit}, offset: {offset}"
            )
            ...
            offset = arguments.get("offset", 0)
            logger.info(
                f"Getting current user saved shows with limit: {limit}, offset: {offset}"
            )
