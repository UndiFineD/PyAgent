# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\onedrive\tools\files.py
import logging
import os
import uuid
    ...
    try:
        ...
        async with httpx.AsyncClient() as httpx_client:
            response = await httpx_client.get(
                url, headers=client["headers"], follow_redirects=True
            )
...
async def onedrive_create_file(
    parent_folder: str, new_file_name: str, data: str = None, if_exists: str = "error"
    ...
    try:
        logger.info(
            f"Creating file '{new_file_name}' in folder {parent_folder} with if_exists={if_exists}"
        )
        ...
        if new_file_name in existing_names:
            if if_exists == "error":
                logger.warning(
                    f"File '{new_file_name}' already exists in folder {parent_folder}"
                )
        ...
        async with httpx.AsyncClient() as httpx_client:
            put_response = await httpx_client.put(
                url, headers=client["headers"], data=data or ""
            )
