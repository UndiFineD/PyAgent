# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\onedrive\tools\folders.py
import logging
...
async def onedrive_create_folder(
    parent_folder: str, new_folder_name: str, behavior: str = "fail"
    ...
    try:
        logger.info(
            f"Creating folder '{new_folder_name}' in parent {parent_folder} with behavior={behavior}"
        )
