# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\onedrive\tools\combined_files_and_folder.py
import logging
...
async def onedrive_rename_item(
    file_id: str, new_name: str
...
async def onedrive_move_item(
    ...
    try:
        ...
        async with httpx.AsyncClient() as httpx_client:
            response = await httpx_client.patch(
                url, headers=client["headers"], json=body
            )
    ...
    try:
        ...
        async with httpx.AsyncClient() as httpx_client:
            ...
            if response.status_code == 204:
                ...
                logger.error(
                    f"Error deleting item {item_id}: {response.status_code} - {response.text}"
                )
