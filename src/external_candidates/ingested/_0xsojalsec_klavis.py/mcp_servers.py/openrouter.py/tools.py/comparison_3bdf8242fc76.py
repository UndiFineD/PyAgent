# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\openrouter\tools\comparison.py
"""
...
async def compare_models(
    ...
    try:
        validate_required_params(
            {"models": models, "test_prompt": test_prompt}, ["models", "test_prompt"]
        )
        ...
        if successful_results:
            ...
            comparison_summary["token_usage"] = {
                ...
                "avg_completion_tokens": sum(completion_tokens)
...
async def analyze_model_performance(
    ...
    try:
        validate_required_params(
            {"model": model, "test_prompts": test_prompts}, ["model", "test_prompts"]
        )
        ...
        for i, prompt in enumerate(test_prompts):
            try:
                ...
                logger.info(
                    f"Successfully tested prompt {i+1}/{len(test_prompts)} with model: {model}"
                )
        ...
        performance_summary = {
            ...
            "failed_tests": len(results) - len(successful_results),
            "success_rate": (
                len(successful_results) / len(test_prompts) if test_prompts else 0
            ),
            "parameters": {
                "max_tokens": max_tokens or 100,
                "temperature": temperature or 0.7,
        ...
        logger.info(
            f"Successfully analyzed model {model} performance across {len(successful_results)} prompts"
        )
...
async def get_model_recommendations(
    ...
    try:
        ...
        logger.info(
            f"Generated {len(recommendations)} model recommendations for use case: {use_case}"
        )
