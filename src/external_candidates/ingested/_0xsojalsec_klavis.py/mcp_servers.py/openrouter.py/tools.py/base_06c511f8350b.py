# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\openrouter\tools\base.py
"""
                                ...
                                for line in lines:
                                    if line.startswith("data: "):
                                        ...
                                        if data_line.strip() == "[DONE]":
                                            logger.info(
                                                "Received [DONE] marker - stream complete"
                                            )
                                            yield {
                                                "chunk": None,
                                                "is_complete": True,
                                        ...
                                        try:
                                            ...
                                            if (
                                                ...
                                                if "content" in delta:
                                                    ...
                                                    logger.info(
                                                        f"Yielding chunk {chunk_count}: '{content}'"
                                                    )
                                                    yield {
                                                        "chunk": content,
                                                        "is_complete": False,
...
class ChatCompletionRequest(BaseModel):
    ...
    n: Optional[int] = Field(
        None, ge=1, le=10, description="Number of completions to generate"
    )
