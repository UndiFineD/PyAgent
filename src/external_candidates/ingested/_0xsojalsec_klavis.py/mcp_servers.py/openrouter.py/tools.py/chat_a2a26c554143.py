# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\openrouter\tools\chat.py
"""
...
async def create_chat_completion(
    ...
    try:
        validate_required_params(
            {"model": model, "messages": messages}, ["model", "messages"]
        )
...
async def create_chat_completion_stream(
    ...
    try:
        ...
        logger.info(
            f"Successfully created streaming chat completion with model: {model}"
        )
...
async def create_completion(
    ...
    try:
        validate_required_params(
            {"model": model, "prompt": prompt}, ["model", "prompt"]
        )
        validate_model_id(model)
