# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\mixpanel\tools\frequency.py
import logging
...
async def run_frequency_query(
    ...
    if addiction_unit not in allowed_addiction_units:
        raise ValueError(
            f"addiction_unit must be one of {sorted(allowed_addiction_units)}"
        )
