# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\mixpanel\tools\projects.py
import logging
...
async def get_projects() -> Dict[str, Any]:
    ...
    try:
        ...
        result = await MixpanelAppAPIClient.make_request("GET", "/me")
        ...
        if isinstance(result, dict):
            ...
            if "results" in result and "projects" in result.get("results", {}):
                ...
                for item in result.get("results", []):
                    pid = str(item.get("id"))
                    projects[pid] = {"id": item.get("id"), "name": item.get("name", "")}
