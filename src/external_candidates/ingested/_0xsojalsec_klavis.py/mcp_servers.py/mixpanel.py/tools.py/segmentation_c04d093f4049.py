# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\mixpanel\tools\segmentation.py
import logging
...
async def run_segmentation_query(
    ...
    if (
        numerical_aggregation is not None
        and numerical_aggregation not in allowed_num_aggs
    ):
        raise ValueError(
            f"numerical_aggregation must be one of {sorted(allowed_num_aggs)}"
        )
