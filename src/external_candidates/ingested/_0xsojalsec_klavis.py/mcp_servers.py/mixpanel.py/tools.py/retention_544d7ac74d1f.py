# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\mixpanel\tools\retention.py
import logging
...
async def run_retention_query(
    ...
    if retention_type not in allowed_retention_types:
        raise ValueError(
            f"retention_type must be one of {sorted(allowed_retention_types)}"
        )
