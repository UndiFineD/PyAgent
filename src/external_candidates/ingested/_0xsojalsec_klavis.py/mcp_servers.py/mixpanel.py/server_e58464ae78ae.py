# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\mixpanel\server.py
import base64
import contextlib
import json
import logging
...
@click.command()
@click.option(
    "--port", default=MIXPANEL_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
        ...
        return [
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(
                    **{"category": "MIXPANEL_EVENT_METADATA"}
                ),
            ),
            types.Tool(
    ...
    async def call_tool(
        ...
        if name == "mixpanel_send_events":
            ...
            if not event:
                return [
                    types.TextContent(
                        type="text", text="Error: event parameter is required"
                    )
                ]
            ...
            if not property_name:
                return [
                    types.TextContent(
                        type="text", text="Error: property parameter is required"
                    )
                ]

            try:
                result = await get_event_property_values(
                    str(project_id), event, property_name
                )
                return [
                    types.TextContent(
                        type="text",
            ...
            if not events:
                return [
                    types.TextContent(
                        type="text", text="Error: events parameter is required"
                    )
                ]
            if not from_date or not to_date:
                return [
                    types.TextContent(
                        type="text", text="Error: from_date and to_date are required"
                    )
                ]
