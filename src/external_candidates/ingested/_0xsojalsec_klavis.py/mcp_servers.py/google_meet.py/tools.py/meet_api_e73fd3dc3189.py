# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\google_meet\tools\meet_api.py
...
def _capability_probe() -> Dict[str, Any]:
    if os.getenv(MEET_API_DISABLED_ENV, "").lower() in {"1", "true", "yes"}:
        return {
            "enabled": False,
            "code": "disabled_env",
            "error": "Meet API v2 disabled by env",
        }
    token = get_auth_token()
    if not token:
        return {
    ...
    try:
        ...
        if not (has_list or has_create):
            return {
                ...
                "error": "Meet API v2 methods not present in discovery",
            }
...
def _ensure_enabled() -> Optional[Dict[str, Any]]:
    ...
    if not probe.get("enabled"):
        return failure(
            ...
            details={
                k: v for k, v in probe.items() if k != "enabled" and v is not None
            },
        )
    return None
...
async def create_instant_meeting() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            "tool=google_meet_v2_create_instant action=success space_id=%s",
            shaped.get("space_id"),
        )
        ...
        return failure(
            "Meet API error", code=str(status or "http_error"), details=detail
        )
    except Exception as e:
...
async def get_meeting(space_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        space = (
            ...
            .get(
                name=(
                    space_id if space_id.startswith("spaces/") else f"spaces/{space_id}"
                )
            )
            .execute()
        )
        ...
        try:
            ...
            detail = {}
        return failure(
            "Failed to fetch meeting", code=str(status or "http_error"), details=detail
        )
    except Exception as e:
...
async def list_meetings(max_results: int = 10) -> Dict[str, Any]:
    logger.info(
        "tool=google_meet_v2_list_meetings action=start max_results=%s", max_results
    )
    ...
    try:
        ...
        while len(spaces) < max_results:
            ...
            if not use_http_fallback:
                resp = (
                    spaces_res.list(pageSize=page_size, pageToken=page_token).execute()
                    if page_token
                    else spaces_res.list(pageSize=page_size).execute()
                )
            else:
        ...
        try:
            ...
            detail = {}
        return failure(
            "Failed to list meetings", code=str(status or "http_error"), details=detail
        )
    except Exception as e:
...
async def get_participants(space_id: str, max_results: int = 50) -> Dict[str, Any]:
    logger.info(
        "tool=google_meet_v2_get_participants action=start space_id=%s", space_id
    )
    if not space_id:
        return failure("space_id is required")
