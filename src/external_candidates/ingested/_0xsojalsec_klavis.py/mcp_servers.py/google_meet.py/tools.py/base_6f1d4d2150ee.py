# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\google_meet\tools\base.py
...
async def list_meetings(
    max_results: int = 10, start_after: str | None = None, end_before: str | None = None
) -> Dict[str, Any]:
        ...
        for event in events:
            ...
            if hangout_link and "meet.google.com" in hangout_link:
                ...
                for ep in conference_data.get("entryPoints", []) or []:
                    if ep.get("entryPointType") == "video" and "meet.google.com" in (
                        ep.get("uri") or ""
                    ):
                        is_meet = True
                        break
...
async def get_meeting_details(event_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            error_detail = {}
        logger.error(
            f"tool=get_meeting_details http_error status={status} event_id={event_id}"
        )
        return failure(detail, code=str(status or "http_error"), details=error_detail)
    except Exception as e:  # pragma: no cover
...
async def update_meeting(
    ...
    try:
        ...
        if not changed and attendees is not None:
            orig_emails = sorted(
                [a.get("email") for a in original.get("attendees", []) or []]
            )
            new_emails = sorted(
        ...
        try:
            ...
            error_detail = {}
        logger.error(
            f"tool=update_meeting http_error status={status} event_id={event_id}"
        )
        return failure(detail, code=str(status or "http_error"), details=error_detail)
    except Exception as e:  # pragma: no cover
...
async def delete_meeting(event_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            error_detail = {}
        logger.error(
            f"tool=delete_meeting http_error status={status} event_id={event_id}"
        )
        return failure(detail, code=str(status or "http_error"), details=error_detail)
    except Exception as e:  # pragma: no cover
...
async def list_past_meetings(
    max_results: int = 10, since: str | None = None
) -> Dict[str, Any]:
    logger.info(
        f"tool=list_past_meetings action=start max_results={max_results} since={since}"
    )
    try:
        if max_results <= 0 or max_results > 100:
            return failure("max_results must be between 1 and 100")
        ...
        meet_events.sort(
            key=lambda e: (e.get("start", {}) or {}).get("dateTime") or "", reverse=True
        )
...
async def get_past_meeting_attendees(event_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if len(end_dt) == 10:
            return failure(
                "Event is all-day or missing precise end time", code="not_supported"
            )
        now = datetime.datetime.utcnow().replace(microsecond=0).isoformat() + "Z"
        if end_dt > now:
        ...
        logger.info(
            f"tool=get_past_meeting_attendees action=success event_id={event_id} count={len(shaped_attendees)}"
        )
        meet_url = next(
            (
                ep.get("uri")
                ...
                and "meet.google.com" in (ep.get("uri") or "")
            ),
        ...
        try:
            ...
            error_detail = {}
        logger.error(
            f"tool=get_past_meeting_attendees http_error status={status} event_id={event_id}"
        )
        return failure(detail, code=str(status or "http_error"), details=error_detail)
    except Exception as e:  # pragma: no cover
