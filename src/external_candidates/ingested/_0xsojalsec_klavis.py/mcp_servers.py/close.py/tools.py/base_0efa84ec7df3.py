...
def format_currency_from_cents(
    amount_cents: Optional[int], currency: str = "USD"
) -> Optional[str]:
...
def format_opportunity_values(opportunity: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in money_fields:
        if field in formatted_opp and formatted_opp[field] is not None:
            ...
            formatted_opp[field] = format_currency_from_cents(
                formatted_opp[field], currency
            )
    ...
    def response_cleaner(data: dict[str, Any]) -> dict[str, Any]:
        ...
        for k, v in data.items():
            if isinstance(v, dict):
                ...
                data[k] = [
                    item if not isinstance(item, dict) else response_cleaner(item)
                    for item in v
                ]
...
class CloseClient:
    ...
    def __post_init__(self) -> None:
        self._semaphore = self._semaphore or asyncio.Semaphore(
    ...
    def _build_error_messages(self, response: httpx.Response) -> tuple[str, str]:
        try:
            ...
            if "error" in data:
                ...
                developer_message = (
                    f"{error_message} (HTTP status code: {response.status_code})"
                )
            elif "errors" in data:
                errors = data["errors"]
                if len(errors) == 1:
            ...
            developer_message = (
                f"Failed to parse Close error response: {type(e).__name__}: {e!s}"
            )
    ...
    def _set_request_body(
        self, kwargs: dict, data: dict | None, json_data: dict | None
    ) -> dict:
