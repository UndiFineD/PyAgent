...
@click.command()
@click.option(
    "--port", default=CLICKUP_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
        ...
        try:
            if name == "clickup_get_teams":
                ...
                if not team_id:
                    return [
                        types.TextContent(
                            type="text", text="Error: team_id parameter is required"
                        )
                    ]
                ...
                if not space_id:
                    return [
                        types.TextContent(
                            type="text", text="Error: space_id parameter is required"
                        )
                    ]
                name = arguments.get("name")
                color = arguments.get("color")
                ...
                if not list_id:
                    return [
                        types.TextContent(
                            type="text", text="Error: list_id parameter is required"
                        )
                    ]
                name = arguments.get("name")
                content = arguments.get("content")
                ...
                result = await update_list(
                    list_id, name, content, due_date, priority, assignee, unset_status
                )
                return [
                    types.TextContent(type="text", text=json.dumps(result, indent=2))
                ]
                ...
                result = await get_tasks(
                    list_id, archived, include_closed, page, subtasks=subtasks
                )
                return [
                    types.TextContent(type="text", text=json.dumps(result, indent=2))
                ]
                ...
                if not task_id:
                    return [
                        types.TextContent(
                            type="text", text="Error: task_id parameter is required"
                        )
                    ]
                include_subtasks = arguments.get("include_subtasks", False)
                result = await get_task_by_id(
                    task_id, include_subtasks=include_subtasks
                )
                return [
                    types.TextContent(type="text", text=json.dumps(result, indent=2))
                ]
                ...
                result = await update_task(
                    task_id, name, description, status, priority, due_date
                )
                return [
                    types.TextContent(type="text", text=json.dumps(result, indent=2))
                ]
                ...
                result = await create_comment(
                    task_id, comment_text, notify_all=notify_all
                )
                return [
                    types.TextContent(type="text", text=json.dumps(result, indent=2))
                ]
