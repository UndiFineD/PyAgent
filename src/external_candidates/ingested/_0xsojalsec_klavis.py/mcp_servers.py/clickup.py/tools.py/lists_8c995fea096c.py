...
async def get_lists(
    folder_id: Optional[str] = None, space_id: Optional[str] = None
) -> Dict[str, Any]:
    """Get all lists in a folder or space."""
    logger.info(
        f"Executing tool: get_lists with folder_id: {folder_id}, space_id: {space_id}"
    )
    try:
        if folder_id:
    ...
    try:
        ...
        if folder_id:
            result = await make_clickup_request(
                f"folder/{folder_id}/list", "POST", data
            )
        elif space_id:
