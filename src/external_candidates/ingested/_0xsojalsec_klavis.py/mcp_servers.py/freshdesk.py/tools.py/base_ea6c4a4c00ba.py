# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\freshdesk\tools\base.py
import base64
import os
import random
import string
...
def get_auth_token() -> str:
    return get_config_value(
        context_var=auth_token_context, env_name="FRESHDESK_API_KEY"
    )


def get_domain() -> str:
    return get_config_value(context_var=domain_context, env_name="FRESHDESK_DOMAIN")
...
async def make_freshdesk_request(
    ...
    if not use_pwd:
        api_key = base64.b64encode(f"{FRESHDESK_API_KEY}:{random_password}").decode(
            "utf-8"
        )
    ...
    try:
        ...
        print(
            f"Freshdesk API {method.upper()} {endpoint} - Status: {response.status_code}"
        )
        ...
        raise requests.exceptions.Timeout(
            f"Request to Freshdesk API timed out after {timeout} seconds"
        ) from e

    except requests.exceptions.TooManyRedirects as e:
        raise requests.exceptions.TooManyRedirects(
            "Too many redirects while connecting to Freshdesk API"
        ) from e

    except ValueError as e:
        raise ValueError(
            f"Failed to parse JSON response from Freshdesk API: {str(e)}"
        ) from e


def handle_freshdesk_error(
    e: Exception, operation: str, object_type: str = ""
) -> Dict[str, Any]:
        ...
        if hasattr(e, "response") and e.response is not None:
            ...
            try:
                if e.response.content:
                    ...
                    error_response["error"]["message"] = error_data.get(
                        "description", f"Freshdesk API returned status {status_code}"
                    )
                    ...
                    # If no field errors but there's a message, use it
                        ...
                        not error_response["error"]["details"]
                        and "message" in error_data
                    ):
                        error_response["error"]["details"].append(
                            {"message": error_data["message"], "code": error_code}
                        )
                else:
                ...
                error_response["error"][
                    "message"
                ] = f"Freshdesk API error: {e.response.text}"
        else:
            ...
            if isinstance(e, requests.exceptions.Timeout):
                ...
                error_response["error"].update(
                    {"code": "request_failed", "message": f"{str(e)}"}
                )
    ...
    if object_type:
        error_response["error"][
            "message"
        ] = f"Failed to {operation} {object_type} due to: {error_response['error']['message']}"
    else:
        error_response["error"][
            "message"
        ] = f"Failed to {operation} due to: {error_response['error']['message']}"

    return error_response


def handle_freshdesk_attachments(
    field_name: str, attachments: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
            ...
            resolved_attachments.append(
                (
                    ...
                    (
                        file_name,
                        file_content,
                        attachment.get("media_type", "application/octet-stream"),
                    ),
                )
            )
