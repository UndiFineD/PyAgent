# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\freshdesk\tools\companies.py
import logging
...
async def list_companies(
    updated_since: Optional[str] = None, page: int = 1, per_page: int = 30
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        response = await make_freshdesk_request(
            "GET", "/companies", options={"query_params": params}
        )
        return response
...
async def filter_companies(
    query: str, page: int = 1, per_page: int = 30
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        response = await make_freshdesk_request(
            "GET", "/search/companies", options={"query_params": params}
        )
        return response
...
async def update_company(
    ...
    try:
        ...
        response = await make_freshdesk_request(
            "PUT", f"/companies/{company_id}", data=update_data
        )
        return response
...
async def search_companies_by_name(name: str) -> Dict[str, Any]:
    ...
    try:
        response = await make_freshdesk_request(
            "GET", "/companies/autocomplete", options={"query_params": {"name": name}}
        )
        return response
    except Exception as e:
        return handle_freshdesk_error(e, "search", "companies")
