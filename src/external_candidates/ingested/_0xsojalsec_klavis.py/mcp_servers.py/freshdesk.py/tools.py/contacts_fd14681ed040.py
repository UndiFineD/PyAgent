# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\freshdesk\tools\contacts.py
import logging
import mimetypes
import os
...
async def create_contact(
    ...
    **kwargs,
    ...
    try:
        if avatar_path:
            options["files"] = handle_freshdesk_attachments(
                "avatar",
                [
                    {
                        "type": "local",
                        ...
                        "media_type": mimetypes.guess_type(avatar_path)[0],
                    }
                ],
            )

        return await make_freshdesk_request(
            "POST", "/contacts", data=contact_data, options=options
        )
...
async def list_contacts(
    ...
    try:
        return await make_freshdesk_request(
            "GET", "/contacts", options={"query_params": params}
        )
    except Exception as e:
...
async def update_contact(
    ...
    **kwargs,
    ...
    try:
        ...
        return await make_freshdesk_request(
            "PUT", f"/contacts/{contact_id}", data=contact_data, options=options
        )
...
async def delete_contact(
    contact_id: int, hard_delete: bool = False, force: bool = False
) -> Dict[str, Any]:
    """
    Delete a contact.
...
async def filter_contacts(
    query: str, page: int = 1, updated_since: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    """
    params = {"query": f"'{query}'", "page": page, "updated_since": updated_since}

    params = remove_none_values(params)

    try:
        return await make_freshdesk_request(
            "GET", "/search/contacts", options={"query_params": params}
        )
    except Exception as e:
...
async def search_contacts_by_name(name: str) -> Dict[str, Any]:
    ...
    try:
        return await make_freshdesk_request(
            "GET", "/contacts/autocomplete", options={"query_params": {"term": name}}
        )
    except Exception as e:
...
async def make_contact_agent(
    ...
    try:
        return await make_freshdesk_request(
            "PUT", f"/contacts/{contact_id}/make_agent", data=agent_data
        )
    except Exception as e:
...
async def restore_contact(contact_id: int) -> Dict[str, Any]:
    ...
    """
    try:
        return await make_freshdesk_request("PUT", f"/contacts/{contact_id}/restore")
    except Exception as e:
...
async def send_contact_invite(contact_id: int) -> Dict[str, Any]:
    ...
    try:
        return await make_freshdesk_request(
            "POST", f"/contacts/{contact_id}/send_invite"
        )
    except Exception as e:
...
async def merge_contacts(
    ...
    try:
        return await make_freshdesk_request("PUT", f"/contacts/merge", data=merge_data)
    except Exception as e:
