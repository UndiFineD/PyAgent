# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\freshdesk\server.py
import base64
import contextlib
import json
import logging
...
from tools import (  # Context variables; Ticket tools; Contact tools; Company tools; Account tools; Agent tools; Thread tools
    add_note_to_ticket,
    auth_token_context,
    bulk_create_agents,
    create_agent,
...
@click.command()
@click.option(
    "--port", default=FRESHDESK_MCP_SERVER_PORT, help="Port to listen on for HTTP"
)
@click.option(
    "--log-level",
    default="INFO",
...
def main(port: int, log_level: str, json_response: bool) -> int:
            ...
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "contact_id": {
                            "type": "integer",
                            "description": "ID of the contact to make an agent",
                        },
                        "occasional": {
                            "type": "boolean",
                            "description": "Whether agent is occasional",
                        },
                        "signature": {
                            "type": "string",
                        ...
                        "agent_type": {
                            "type": "string",
                            "description": "Agent type (support_agent or business_agent)",
                        },
                        "focus_mode": {
                            "type": "boolean",
                            "description": "Whether agent is in focus mode",
                        },
                    },
                    "required": ["contact_id"],
                },
            ...
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "contact_id": {
                            "type": "integer",
                            "description": "ID of the contact to send an invite to",
                        }
                    },
                    "required": ["contact_id"],
                },
            ...
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "time_zone": {"type": "string", "description": "New time zone"},
                        "focus_mode": {
                            "type": "boolean",
                            "description": "Whether focus mode is enabled",
                        },
    ...
    async def call_tool(
        ...
        try:
            ...
            logger.info(
                f"Tool {name} executed successfully with arguments: {arguments}"
            )
