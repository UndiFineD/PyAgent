...
class RateLimiter:
    def __init__(self, max_requests_per_second: Optional[int] = None):
        self.max_requests_per_second = (
            max_requests_per_second or config.default_max_requests_per_second
        )
    ...
    async def with_retry(
        self, operation: Callable, context: str = "API request"
    ) -> Any:
        """
        ...
        while attempt <= config.max_retry_attempts:
            try:
                ...
                if (
                    ...
                    and attempt < config.max_retry_attempts
                ):
...
def get_rate_limiter(
    api_type: str = "default", max_requests_per_second: Optional[int] = None
) -> RateLimiter:
    """
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            rate_limiter = get_rate_limiter(
                api_type=api_type, max_requests_per_second=max_requests_per_second
            )
