# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\hubspot\tools\properties.py
import ast
import json
import logging
...
async def hubspot_list_properties(object_type: str) -> list[dict]:
    ...
    try:
        ...
        logger.info(
            f"Successfully Executed hubspot_list_properties for object_type: {object_type}"
        )
        return [
            {
                "name": p.name,
...
async def hubspot_search_by_property(
    ...
    logger.info(
        f"Executing hubspot_search_by_property on {object_type}: {property_name} {operator} {value}"
    )
    ...
    try:
        ...
        # Operators that require a list of values
            ...
            if not values_list:
                raise ValueError(
                    "Operator IN/NOT_IN requires a non-empty list of values"
                )
        ...
        # Between expects two endpoints: low and high
            ...
            if low is None or high is None:
                raise ValueError(
                    "Operator BETWEEN requires a list with two values [low, high]"
                )
        ...
        search_request = PublicObjectSearchRequest(
            filter_groups=[FilterGroup(filters=[Filter(**filter_kwargs)])],
        ...
        logger.info(
            f"hubspot_search_by_property: Found {len(results.results)} result(s)"
        )
...
async def hubspot_create_property(
    name: str, label: str, description: str, object_type: str
) -> str:
    """
    ...
    try:
        logger.info(
            f"Creating property with name: {name}, label: {label}, object_type: {object_type}"
        )
        ...
        client.crm.properties.core_api.create(
            object_type=object_type, property_create=property
        )
