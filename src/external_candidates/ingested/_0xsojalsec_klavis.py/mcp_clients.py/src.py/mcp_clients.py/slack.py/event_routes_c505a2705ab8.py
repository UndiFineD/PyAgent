# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\slack\event_routes.py
import logging
import time
...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            process_time = time.time() - start_time
            logger.error(
                f"Error: {method} {path} - Error: {str(e)} - Time: {process_time:.3f}s"
            )
            raise
