# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\slack\interactive_handlers.py
"""
...
def register_interactive_handlers(app):
    ...
    async def handle_connect_mcp_server_button(ack, body, logger):
        ...
        await ack()
        logger.info(
            f"Connect to MCP server button clicked: {body.get('user', {}).get('id')}"
        )
