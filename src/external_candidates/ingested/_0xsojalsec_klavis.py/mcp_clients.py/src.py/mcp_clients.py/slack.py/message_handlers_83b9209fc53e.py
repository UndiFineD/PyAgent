# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\slack\message_handlers.py
"""
...
def create_login_blocks(
    platform_username: str, platform_user_id: str, team_id: str = None
) -> List[Dict[str, Any]]:
    """
    ...
    return [
        ...
        },
        {
            ...
            "elements": [
                {
                    "type": "button",
                    "text": {"type": "plain_text", "text": "Login", "emoji": True},
...
def create_connect_mcp_server_blocks(mcp_client_id: str = None) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        {
            "type": "context",
            "elements": [{"type": "mrkdwn", "text": "⏳ _Working on your request..._"}],
        },
    ]
...
def register_message_handlers(app, bot):
    ...
    async def add_loading_reaction(client, channel_id, message_ts):
        ...
        try:
            await client.reactions_add(
                channel=channel_id, timestamp=message_ts, name="thinking_face"
            )
    ...
    async def handle_app_mention(event, context, client, ack, logger):
        ...
        logger.info(
            f"--- Received app mention from user {user_id} in channel {channel_id}: {clean_text}"
        )
        ...
        try:
            async with asyncio.timeout(200), bot.user_locks[user_id]:
                ...
                if not server_urls:
                    team_id = context.get("team_id")
                    mcp_client_id = await get_mcp_client_id_by_slack_info(
                        team_id, user_id
                    )
                ...
                if clean_text:
                    usage_under_limit = await bot.check_and_update_usage_limit(
                        slack_context
                    )
                    ...
                    try:
                        ...
                        messages_history = await bot.get_messages_history(
                            mcp_client.conversation, slack_context
                        )
                        ...
                        await bot.send_message(
                            slack_context, f"Error processing query: {str(e)}"
                        )
                    finally:
                        await remove_loading_reaction(client, channel_id, message_ts)
                        logger.info(
    ...
    async def handle_direct_message(event, context, client, ack, logger):
        ...
        if event.get("channel_type") == "im" and not event.get("bot_id"):
            ...
            )  # since we reply DM in thread, we set thread_ts to event ts
            message_ts = event.get("ts")
            ...
            logger.info(
                f"---Received DM from user {user_id} in channel {channel_id}: {text}"
            )
            try:
                # Process the message with the lock acquired
                async with asyncio.timeout(200), bot.user_locks[user_id]:
                    ...
                    if not server_urls:
                        ...
                        await client.chat_postMessage(
                            ...
                            blocks=create_connect_mcp_server_blocks(
                                mcp_client_id
                                or verification_result.get("mcp_client_id")
                            ),
                        )
                        # Don't return here to allow processing with 0 MCP server
