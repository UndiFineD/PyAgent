# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\slack\oauth_routes.py
import base64
import json
import logging
import urllib.parse
...
if USE_PRODUCTION_DB:
    from mcp_clients.database.database import (
        create_mcp_client,
        insert_slack_client_auth,
    )
...
def setup_oauth_routes(router: APIRouter):
    ...
    async def install_slack_app(account_id: str = None):
        ...
        if account_id:
            logger.info(
                f"Installing Klavis powered Slack MCP Client with account_id: {account_id}"
            )
    ...
    async def oauth_callback(code: str = None, error: str = None, state: str = None):
        ...
        if state:
            try:
                ...
                if account_id:
                    logger.info(
                        f"OAuth callback received with account_id: {account_id} (from state)"
                    )
        ...
        user_id = (
            oauth_response.authed_user.get("id") if oauth_response.authed_user else None
        )
        username = None

        if user_id:
            username, _ = await get_user_profile(
                user_id=user_id, bot_token=oauth_response.access_token
            )
        ...
        if account_id and USE_PRODUCTION_DB:
            ...
            if auth_result["error"]:
                logger.error(
                    f"Failed to save Slack auth metadata: {auth_result['error']}"
                )
        ...
        try:
            user_id = (
                oauth_response.authed_user.get("id")
                if oauth_response.authed_user
                else None
            )
            if user_id:
                ...
                logger.warning(
                    "Could not identify installing user to send welcome message"
                )
        except Exception as e:
        ...
        if (
            ...
            and mcp_client_result["mcp_client"].get("id")
        ):
...
async def send_welcome_message(
    user_id: str, username: str, bot_token: str, mcp_client_result: Dict[str, Any]
):
    """
...
def create_welcome_blocks(dashboard_url: str):
    return [
        {
            "type": "section",
            "text": {"type": "mrkdwn", "text": "*🎉 Welcome to Klavis AI*"},
        },
        {
            "type": "section",
            "text": {
