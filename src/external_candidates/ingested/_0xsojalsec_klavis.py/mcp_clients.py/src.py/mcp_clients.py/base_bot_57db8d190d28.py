...
class BaseBot(ABC):
    ...
    async def verify_user(self, context: BotContext) -> Dict[str, Any]:
        ...
        if USE_PRODUCTION_DB:
            ...
            logger.info(
                f"Verify user took {time.time() - start_time} seconds to complete"
            )
    ...
    async def check_and_update_usage_limit(self, context: BotContext) -> bool:
        ...
        if USE_PRODUCTION_DB:
            ...
            logger.info(
                f"Check and update usage limit took {time.time() - start_time} seconds to complete"
            )
            if result["error"]:
                await self.send_message(
                    context,
    ...
    async def get_server_urls(self, context: BotContext) -> List[str]:
        ...
        if USE_PRODUCTION_DB:
            ...
            logger.info(
                f"Get server urls took {time.time() - start_time} seconds to complete"
            )
            ...
            json_path = os.path.join(
                os.path.dirname(__file__), "local_mcp_servers.json"
            )
    ...
    async def store_new_messages(
        ...
        if USE_PRODUCTION_DB:
            ...
            # Skip database operation
            logger.info(
                f"Database operations skipped: store_new_messages for {len(messages)} messages"
            )
            return None
