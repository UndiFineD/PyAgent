...
----------------------------"""
    ...
    async def send_special_message(
        self, context: WhatsAppBotContext, special_content: str
    ) -> Any:
        """
        ...
        try:
            ...
            logger.info(
                f"Sent special message to {context.user_id}: {special_content[:50]}..."
            )
            return response
        except Exception as e:
        ...
        try:
            ...
            if response:
                ...
                return await self.send_message(
                    context, "I couldn't generate a response. Please try again."
                )
        ...
        try:
            ...
            async for chunk in mcp_client.process_query_stream(
                ...
                if chunk.startswith("<special>") and chunk.endswith("\n"):
                    ...
                    special_content = (
                        chunk.replace("<special>", "")
                        .split(mcp_client.get_message_split_token())[0]
                        .strip()
                    )
                ...
                should_send_update = (
                    current_time - last_send_time >= update_interval
    ...
    def run(self, port: Optional[int] = None) -> None:
        ...
        """
        logger.info(
            f"WhatsApp bot running within FastAPI server on port {port if port else 'default'}"
        )
        pass
...
async def handle_message(client: WhatsApp, message: Message):
    ...
    try:
        ...
        context = WhatsAppBotContext(
            platform_name="whatsapp", user_id=message.from_user.wa_id, message=message
        )
        ...
        # Get message history
        messages_history = await whatsapp_bot.get_messages_history(
        ...
        try:
            await client.send_message(
                to=message.from_user.wa_id, text=f"Sorry, an error occurred: {str(e)}"
            )
        except:
            pass
