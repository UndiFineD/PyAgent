# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\mcp_client.py
import asyncio
import base64
import logging
import os
...
class MCPClient:
            ...
            while not tool_call_task.done():
                ...
                try:
                    result = await asyncio.wait_for(
                        asyncio.shield(tool_call_task), timeout=30.0
                    )
                    yield str(result)
                    return
                    ...
                    logger.info(
                        f"Tool call {tool_name} still running after {elapsed} seconds"
                    )
    ...
    async def process_query_stream(
        ...
        for resource in resources:
            resource_content = await self.read_resource(
                resource["server_id"], resource["uri"]
            )
            text_resource_contents.extend(
                [content["text"] for content in resource_content if content["text"]]
            )
            ...
            for message in last_chat_messages:
                for content in message.content:
                    if content.type == ContentType.TOOL_CALL:
                        ...
                        display_args = (
                            self._redact_sensitive_args(tool_args)
                            if tool_args
                            else None
                        )
                        ...
                        result_text = ""
                        start_time = time.time()
                        async for update in self._process_tool_call(
                            tool_name, tool_args
                        ):
                            if "[Tool" in update and "still running" in update:
                                ...
                                result_text = update
                        logger.info(
                            f"Tool {tool_name} took {time.time() - start_time} seconds to complete"
                        )
        ...
        if store_new_messages_callback:
            ...
            logger.info(
                f"Store new messages took {time.time() - start_time} seconds to complete"
            )
        ...
        try:
            ...
            for content in response.contents:
                content_data = {
                    "uri": content.uri,
                    "mimeType": (
                        content.mimeType if hasattr(content, "mimeType") else None
                    ),
                }
                ...
                if hasattr(content, "text") and content.text is not None:
                    ...
                    if (
                        content.mimeType
                        and "application/pdf" in content.mimeType.lower()
                    ):
            ...
            logger.error(
                f"Error reading resource {uri} from server {server_id}: {str(e)}"
            )
            return []
