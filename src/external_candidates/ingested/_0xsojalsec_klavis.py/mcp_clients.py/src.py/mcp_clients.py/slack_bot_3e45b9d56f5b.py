...
if USE_PRODUCTION_DB:
    ...
    async def get_slack_auth_metadata(team_id):
        logging.getLogger("slack_bot").info(
            f"Database operations skipped: get_slack_auth_metadata for {team_id}"
        )
        return {
...
class SlackBot(BaseBot):
    ...
    def __init__(self):
        ...
        self.app: AsyncApp = AsyncApp(
            signing_secret=settings.SLACK_SIGNING_SECRET, authorize=self.authorize
        )
    ...
    async def authorize(
        ...
        """
        logger.info(
            f"Authorizing Slack request for team_id={team_id}, enterprise_id={enterprise_id}"
        )
    ...
    async def send_message(
        self, context: SlackBotContext, message: str
        ...
        try:
            ...
            if hasattr(context, "bot_token") and context.bot_token:
                ...
                logger.warning(
                    "No bot token in context, using default client (likely to fail)"
                )
    ...
    async def _create_special_message_blocks(self, special_content: str) -> tuple:
        ...
        if special_content.startswith("[Calling tool"):
            ...
            if any(tool in tool_name for tool in LONG_RUNNING_TOOLS):
                waiting_message = (
                    f"⏳ _This tool may take several minutes to complete..._"
                )
    ...
    async def _send_special_message(
        self, context: SlackBotContext, special_content: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        blocks, fallback_text = await self._create_special_message_blocks(
            special_content
        )
    ...
    async def process_query_with_streaming(
        ...
        try:
            ...
            async with asyncio.timeout(settings.STREAMING_TIMEOUT):
                ...
                if buffer.strip():
                    ...
                    if "<special>" in buffer:
                        ...
                        return await self._send_special_message(
                            context, special_content
                        )
            ...
            return await self.send_message(
                context, "The response took too long. Please try again."
            )
        except Exception as e:
            ...
            if response["ok"]:
                ...
                for message in messages:
                    ...
                    if is_bot_message:
                        ...
                        content.append(
                            TextContent(
                                text=f"User {username} says: {message.get('text', '')}"
                            )
                        )
                ...
                logger.error(
                    f"Error fetching Slack conversation history: {response.get('error', 'Unknown error')}"
                )
