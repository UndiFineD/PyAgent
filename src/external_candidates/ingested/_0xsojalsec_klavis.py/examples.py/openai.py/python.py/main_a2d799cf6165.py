# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\examples\openai\python\main.py
import json
import os
import webbrowser
...
def stream_chat_completion(
    ...
    try:
        ...
        tools_info = klavis_client.mcp_server.list_tools(
            server_url=server_url, format=ToolFormat.OPENAI
        )
...
def main():
    ...
    mcp_instance = klavis_client.mcp_server.create_server_instance(
        server_name=McpServerName.YOUTUBE, user_id="1234"  # Close CRM as an example
    )
    ...
    if hasattr(mcp_instance, "oauth_url") and mcp_instance.oauth_url:
        ...
        print(
            "Please complete the OAuth authorization in your browser before continuing..."
        )
    ...
    while True:
        try:
            ...
            stream_chat_completion(
                openai_client, messages, klavis_client, mcp_instance.server_url
            )
