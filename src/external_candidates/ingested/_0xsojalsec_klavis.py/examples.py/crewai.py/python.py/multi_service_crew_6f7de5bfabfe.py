# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\examples\crewai\python\multi_service_crew.py
"""
...
VIDEO_URL = (
    "https://www.youtube.com/watch?v=LCEmiRjPEtQ"  # Change to your desired video
)
...
def main():
    ...
    try:
        ...
        print(
            f"📱 If you are not redirected to the Gmail OAuth page, please open it manually: {gmail_mcp_instance.oauth_url}"
        )

        # Step 3: Configure multiple MCP servers
        multiple_server_params = [
            {"url": youtube_mcp_instance.server_url, "transport": "streamable-http"},
            {"url": gmail_mcp_instance.server_url, "transport": "streamable-http"},
        ]
