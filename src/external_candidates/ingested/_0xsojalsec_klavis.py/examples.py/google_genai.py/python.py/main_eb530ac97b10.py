# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\examples\google-genai\python\main.py
import os
import webbrowser
...
def main():
    ...
    mcp_instance = klavis_client.mcp_server.create_server_instance(
        server_name=McpServerName.NOTION, user_id="1234"
    )
    print("--- mcp_instance --- \n", mcp_instance)
    ...
    mcp_tools = klavis_client.mcp_server.list_tools(
        server_url=mcp_instance.server_url, format=ToolFormat.GEMINI
    )
                ...
                for part in response.candidates[0].content.parts:
                    if hasattr(part, "function_call") and part.function_call:
                        ...
                        function_response_part = types.Part.from_function_response(
                            name=part.function_call.name, response=function_response
                        )
