...
class NaviVLLM:
    ...
    def create_message_for_image(
        self, image: str, problem: str
    ) -> List[Dict[str, Any]]:
        return [
    ...
    def _process_batch(
        ...
        messages_list = [
            self.create_message_for_image(img, prob)
            for img, prob in zip(images, problems)
        ]
    ...
    def __call__(self, data, print_log=False):
        ...
        outputs = self.model.generate(
            llm_input_list, sampling_params=self.sampling_params
        )
        responses = []
