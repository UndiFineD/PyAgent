# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-UI-Venus\models\navigation\ui_venus_navi_agent.py
import logging
...
class VenusNaviAgent:
    def __init__(
        self, model_config, logger: logging.Logger, history_length: int = 0
    ) -> None:
    ...
    def _convert_coordinate(self, action_json: dict, size_params: dict):
        ...
        try:
            if action_type == "click" or action_type == "longpress":
                ...
                action_json["params"]["box"] = self._rescale_coordinate(
                    x, y, orig_size, resized_size
                )
            elif action_type == "drag":
                x1, y1 = action_json["params"]["start"]
                x2, y2 = action_json["params"]["end"]
                action_json["params"]["start"] = self._rescale_coordinate(
                    x1, y1, orig_size, resized_size
                )
                action_json["params"]["end"] = self._rescale_coordinate(
                    x2, y2, orig_size, resized_size
                )
            elif action_type == "scroll":
                if (
                    "start" in action_json["params"]
                    and len(action_json["params"]["start"]) > 0
                ):
                ...
                if (
                    "end" in action_json["params"]
                    and len(action_json["params"]["end"]) > 0
                ):
    ...
    def step(self, goal: str, image_path: str):
        ...
        answer_text = (
            generated_text.split("<action>")[1].split("</action>")[0].strip("\n")
        )
        conclusion_text = (
            generated_text.split("<conclusion>")[1]
            .split("</conclusion>")[0]
            .strip("\n")
        )
