...
def collect_results_to_eval(
    ...
    for sample in results:
        ...
        if (
            ...
            and (
                instruction_style is None
                or sample.get("instruction_style") == instruction_style
            )
...
def make_combinations(
    ...
    filtered_values = {
        key: list(value) for key, value in unique_values.items() if value
    }
...
def evaluate_fine_grained(results):
    ...
    combinations = make_combinations(
        results, platform=True, application=True, instruction_style=True, gt_type=True
    )
...
def evaluate_seeclick_paper_style(results):
    # Generate all combinations of platform, instruction_style, and gt_type
    combinations = make_combinations(
        results, platform=True, instruction_style=True, gt_type=True
    )
...
def evaluate(results):
    ...
    result_report["metrics"]["leaderboard_simple_style"] = (
        evaluate_leaderboard_simple_style(results)
    )
    result_report["metrics"]["leaderboard_detailed_style"] = (
        evaluate_leaderboard_detailed_style(results)
    )
...
def main(args):
                ...
                for lang in languages:
                    for task_instance in task_data:
                        ...
                        if lang == "cn":
                            ...
                            task_instance["prompt_to_evaluate"] = task_instance[
                                "instruction_cn"
                            ]
                        elif lang == "en":
                            task_instance["prompt_to_evaluate"] = task_instance[
    ...
    for sample in tqdm(tasks_to_run):
        ...
        response = model.inference(
            instruction=sample["prompt_to_evaluate"], image_path=img_path
        )
        ...
        point_in_pixel = (
            [point[0] * img_size[0], point[1] * img_size[1]] if point else None
        )
        ...
        sample_result = {
            ...
            "instruction_style": (
                sample["instruction_style"] if "instruction_style" in sample else None
            ),
            "prompt_to_evaluate": (
                sample["prompt_to_evaluate"] if "prompt_to_evaluate" in sample else None
            ),
