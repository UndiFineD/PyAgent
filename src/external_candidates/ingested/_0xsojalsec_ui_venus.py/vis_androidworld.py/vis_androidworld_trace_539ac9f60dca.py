# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-UI-Venus\vis_androidworld\vis_androidworld_trace.py
import argparse
import os
...
parser.add_argument("--path", type=str, default="", help="path of visualization files")
...
def index():
    ...
    selected_original_name = request.args.get(
        "task", tasks[0]["original_name"] if tasks else None
    )
...
def prepare_task_data(task_name):
    ...
    for prefix, files in sorted(step_groups.items()):
        step = {
            ...
            "conclusion": next(
                (f for f in files if f.endswith("_conclusion.txt")), None
            ),
        }
