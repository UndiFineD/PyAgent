# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MobileAgent\Mobile-Agent-E\run.py
    ...
    if args.instruction is not None:
        ...
        for i, task in enumerate(tasks):
            ...
            if "task_id" in task:
                ...
                task_id = (
                    args.tasks_json.split("/")[-1].split(".")[0]
                    + f"_{args.setting}"
                    + f"_{i}"
                )
            try:
                ...
                print(
                    "IMPORTANT: Please reset the device as needed before running the next task!"
                )
