# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MobileAgent\Mobile-Agent-E\MobileAgentE\agents.py
import copy
import json
import os
import re
...
class InfoPool:
    ...
    width: int = 1080
    height: int = 2340
    perception_infos_pre: list = field(
        default_factory=list
    )  # List of clickable elements pre action
    keyboard_pre: bool = False  # keyboard status pre action
    perception_infos_post: list = field(
        default_factory=list
    )  # List of clickable elements post action
...
ATOMIC_ACTION_SIGNITURES = {
    ...
    "Home": {"arguments": [], "description": lambda info: "Return to home page."},
    "Wait": {
        "arguments": [],
        ...
        if action in ATOMIC_ACTION_SIGNITURES:
            ...
            if screenshot_log_dir is not None:
                ...
                screenshot_file = os.path.join(
                    screenshot_log_dir, f"{iter}__{action.replace(' ', '')}.png"
                )
        ...
        # execute shortcut
            ...
            for i, atomic_action in enumerate(shortcut["atomic_action_sequence"]):
                try:
                    ...
                    if (
                        atomic_action["arguments_map"] is None
                        or len(atomic_action["arguments_map"]) == 0
                    ):
                        atomic_action_args = None
                    else:
                        atomic_action_args = {}
                        for atomic_arg_key, value in atomic_action[
                            "arguments_map"
                        ].items():
                            if (
                                value in arguments
                            ):  # if the mapped key is in the shortcut arguments
...
class Notetaker(BaseAgent):
    ...
    def parse_response(self, response: str) -> dict:
        important_notes = (
            response.split("### Important Notes ###")[-1]
            .replace("\n", " ")
            .replace("  ", " ")
            .strip()
        )
...
class ExperienceReflectorShortCut(BaseAgent):
    ...
    def get_prompt(self, info_pool: InfoPool) -> str:
        ...
        if info_pool.action_history != []:
            ...
            for act, summ, outcome, err_des, progress in zip(
                latest_actions,
                latest_summary,
                action_outcomes,
                error_descriptions,
...
class ExperienceRetrieverShortCut(BaseAgent):
    ...
    def parse_response(self, response: str) -> dict:
        selected_shortcuts_str = (
            response.split("### Selected Shortcuts ###")[-1]
            .replace("\n", " ")
            .replace("  ", " ")
            .strip()
        )
        try:
            selected_shortcut_names = extract_json_object(
                selected_shortcuts_str, json_type="list"
            )
...
class ExperienceRetrieverTips(BaseAgent):
    ...
    def parse_response(self, response: str) -> dict:
        selected_tips = (
            response.split("### Selected Tips ###")[-1]
            .replace("\n", " ")
            .replace("  ", " ")
            .strip()
        )
