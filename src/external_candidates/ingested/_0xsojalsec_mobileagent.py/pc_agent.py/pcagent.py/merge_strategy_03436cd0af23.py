# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MobileAgent\PC-Agent\PCAgent\merge_strategy.py
import numpy as np
...
def merge_boxes_and_texts(texts, boxes, iou_threshold=0):
    ...
    while len(boxes) > 0:
        ...
        if len(to_merge_boxes) > 1:
            ...
            merged_text = " ".join(
                to_merge_texts
            )  # You can change the merging strategy here
...
def is_contained(bbox1, bbox2):
    ...
    if x1_min >= x2_min and y1_min >= y2_min and x1_max <= x2_max and y1_max <= y2_max:
        ...
        x2_min >= x1_min and y2_min >= y1_min and x2_max <= x1_max and y2_max <= y1_max
    ):
        return True
    return False
...
def merge_all_icon_boxes_new(elements):
    ...
    while elements:
        ...
        for idx, existing_ele in enumerate(result_elements):
            existing_bbox = [
                existing_ele["position"][0],
                existing_ele["position"][1],
                existing_ele["position"][0] + existing_ele["size"][0],
                existing_ele["position"][1] + existing_ele["size"][1],
            ]
            if is_contained(bbox, existing_bbox):
                if get_area(bbox) > get_area(existing_bbox):
...
def merge_boxes_and_texts_new(texts, bounding_boxes, iou_threshold=0):
    ...
    for i, boxA in enumerate(bounding_boxes):
        ...
        overlapping_indices.sort(
            key=lambda idx: (bounding_boxes[idx][1] + bounding_boxes[idx][3]) / 2
        )  # TODO
