# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MobileAgent\PC-Agent\PCAgent\text_localization.py
import logging
import os
import re
...
class Sample:
    ...
    def main(image) -> None:
        ...
        output = client.recognize_all_text_with_options(
...
def ocr(image_path):
    ...
    for output in outputs:
        ...
        bbox = [
            int(output.block_points[0].x),
            int(output.block_points[0].y),
            int(output.block_points[2].x),
            int(output.block_points[2].y),
        ]
