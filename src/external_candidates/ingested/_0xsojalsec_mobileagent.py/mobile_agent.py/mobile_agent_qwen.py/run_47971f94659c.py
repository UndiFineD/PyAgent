# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MobileAgent\Mobile-Agent\Mobile-Agent-qwen\run.py
import argparse
import os
import random
import shutil
...
from MobileAgent.chat import add_response, init_chat, print_status
...
from MobileAgent.crop import clip_for_icon, crop_for_clip
from MobileAgent.icon_localization import det
from MobileAgent.prompt import action_prompt, format_prompt, thought_prompt
...
def run(args):
    device = "cpu"
    groundingdino_model = snapshot_download("AI-ModelScope/GroundingDINO")
    bert = snapshot_download("AI-ModelScope/bert-base-uncased", cache_dir="./")
    ...
    while True:
        ...
        while True:
            if error_flag == 0:
                ...
                operation_history = add_response(
                    "assistant", thought, operation_history
                )
                operation_history = add_response(
                    "user", action_prompt, operation_history
                )
            ...
            while True:
                ...
                if (
                    ...
                    try:
                        ...
                        operation_history = add_response(
                            "assistant", action, operation_history
                        )
                        operation_history = add_response(
                            "user", format_prompt, operation_history
                        )
                    else:
                        break
                else:
            ...
            if "stop" in action:
                ...
                if len(out_coordinate) == 0:
                    ...
                    tap_coordinate = [
                        (in_coordinate[random_id][0] + in_coordinate[random_id][2]) / 2,
                        (in_coordinate[random_id][1] + in_coordinate[random_id][3]) / 2,
                    ]
                    tap_coordinate = [
                        round(tap_coordinate[0] / iw, 2),
                        round(tap_coordinate[1] / ih, 2),
                ...
                parameter1, parameter2 = (
                    parameter.split(",")[0].replace('"', "").strip(),
                    parameter.split(",")[1].replace('"', "").strip(),
                )
            ...
            if error_flag == 0:
                ...
                operation_history = add_response(
                    "user", error_prompt, operation_history, image
                )
