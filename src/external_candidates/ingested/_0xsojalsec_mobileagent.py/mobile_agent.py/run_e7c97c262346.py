# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MobileAgent\Mobile-Agent\run.py
import argparse
import copy
import os
import re
...
from MobileAgent.chat import add_multiimage_response, add_response, init_chat
...
from MobileAgent.crop import clip_for_icon, crop, crop_for_clip
from MobileAgent.icon_localization import det
from MobileAgent.prompt import choose_opreation_prompt, opreation_prompt
...
def run(args):
    ...
    while True:
        ...
        operation_history = add_response(
            "user", opreation_prompt, operation_history, image
        )
        ...
        if "stop" in action:
            ...
            in_coordinate, out_coordinate = ocr(
                image_ori, parameter, ocr_detection, ocr_recognition, iw, ih
            )
            ...
            if len(in_coordinate) == 0:
                ...
                tap_coordinate = [
                    round(tap_coordinate[0] / iw, 2),
                    round(tap_coordinate[1] / ih, 2),
                ]
                tap(
                    args.adb_path,
                    tap_coordinate[0],
                    tap_coordinate[1] - round(50 / y, 2),
                    x,
                    y,
                )
            ...
            choose_chat = add_response(
                "user", choose_opreation_prompt, choose_chat, image
            )
            ...
            if len(out_coordinate) == 0:
                ...
                choose_chat = add_multiimage_response(
                    "user", ocr_prompt, choose_chat, images
                )
            ...
            if len(out_coordinate) == 1:
                ...
                clip_filter = clip_for_icon(
                    clip_model, clip_preprocess, clip_filter, parameter1
                )
        ...
        struct_operation_history = add_response(
            "user",
            "This is the current screenshot. Please give me your action.",
            struct_operation_history,
            image,
        )
        struct_operation_history = add_response(
            "assistant", action, struct_operation_history
        )

        if error_flag == 0:
            if choose_flag == 1:
                struct_operation_history = add_multiimage_response(
                    "user", choose_user, struct_operation_history, choose_images
                )
                struct_operation_history = add_response(
                    "assistant", choose_response, struct_operation_history
                )
        else:
            struct_operation_history = add_response(
                "user", error_prompt, struct_operation_history, image
            )
            struct_operation_history = add_response(
                "assistant",
                "I will try again with another action or parameter.",
                struct_operation_history,
            )
