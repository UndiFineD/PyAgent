# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-XSS-Automation-Tool\xss_scanner.py
import asyncio
import base64
import html
import sys
...
async def scan(url, payloads, vuln_type, injection_point, http_method, encoding):
    ...
    for payload in payloads:
        task = asyncio.ensure_future(
            test_xss(url, injection_point, payload, vuln_type, http_method, encoding)
        )
        tasks.append(task)
...
def main(url, payloads_file, vuln_type, injection_point, http_method, encoding):
    ...
    responses = loop.run_until_complete(
        scan(url, payloads, vuln_type, injection_point, http_method, encoding)
    )
    return responses
