# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Lumina-DiMOO\utils\prompt_utils.py
# -*- coding: utf-8 -*-
"""
...
def generate_text_to_image_prompt(
    prompt_text: str, templates: Optional[Dict] = None
) -> Tuple[str, str]:
    """
    ...
    input_prompt = (
        "<system>" + system_prompt + "</system>" + "<user>" + prompt_text + "</user>"
    )
    uncon_prompt = (
        "<system>"
        + system_prompt
...
def generate_image_to_image_prompt(
    prompt_text: str, edit_type: str, templates: Optional[Dict] = None, **kwargs
) -> Tuple[str, str, str]:
    """
    ...
    if "dense" in edit_type:
        ...
        prompt_text_used = (
            "Transform the current image into the style of the provided image."
        )
...
def generate_multimodal_understanding_prompt(
    question: str, templates: Optional[Dict] = None
) -> str:
    """
    ...
    input_prompt = (
        "<system>" + system_prompt + "</system>" + "<user>" + question + "</user>"
    )

    return input_prompt


def get_edit_type_specific_prompt(
    edit_type: str, prompt_text: str, templates: Optional[Dict] = None
) -> str:
    """
    ...
    if "dense" in edit_type:
        ...
        if "multiturn" in edit_type:
            ...
            if ids == 0:
                ...
                return (
                    prompt_text[ids][0]
                    if isinstance(prompt_text[ids], list)
                    else prompt_text[ids]
                )
        else:
            return prompt_text
...
def get_system_prompt_for_edit_type(
    edit_type: str, templates: Optional[Dict] = None
) -> str:
    """
