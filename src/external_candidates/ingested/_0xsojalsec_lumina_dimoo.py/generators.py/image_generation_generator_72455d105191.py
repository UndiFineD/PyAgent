# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Lumina-DiMOO\generators\image_generation_generator.py
# -*- coding: utf-8 -*-
"""
    ...
    *,
    ...
    for step in range(timesteps):
        if (
            not use_cache
            or step <= warmup_step
            or (step - warmup_step) % refresh_interval == 0
        ):
    ...
    for step in range(timesteps):
        ...
        if cfg_scale > 0:
            ...
            uncond_vq_mask = torch.cat(
                (
                    torch.zeros((1, uncon_ids.size()[1]), dtype=torch.bool).to(
                        x.device
                    ),
            ...
            cond_logits = model(
                ...
                use_cache=use_cache,
                to_compute_mask=(
                    cond_to_compute_mask if not refresh_steps[step] else None
                ),
            ...
            uncond_logits = model(
                ...
                use_cache=use_cache,
                to_compute_mask=(
                    uncond_to_compute_mask if not refresh_steps[step] else None
                ),
            ).logits[..., vocab_offset : vocab_offset + codebook_size]
            uncond_mask_logits = uncond_logits[uncond_vq_mask].view(
                B, -1, codebook_size
            )
        ...
        if use_cache and step < timesteps - 1 and not refresh_steps[step + 1]:
            ...
            cond_conf_threshold = torch.quantile(
                cond_conf.to(torch.float), compute_ratio, dim=-1, keepdim=True
            )
            cond_to_compute_mask = cond_conf <= cond_conf_threshold

            uncond_conf = uncond_logits.max(dim=-1)[0]
            uncond_conf_threshold = torch.quantile(
                uncond_conf.to(torch.float), compute_ratio, dim=-1, keepdim=True
            )
