# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Lumina-DiMOO\model\modeling_xllmx_dimoo.py
import functools
import logging
import math
...
class LLaDAForMultiModalGeneration(LLaDAModelLM):
    ...
    def forward(
        ...
        **kwargs,
        ...
        ]  # every sample len --> record for attention mask
        input_ids = [
            example + [0] * (max_tokens - len(example)) for example in input_ids
        ]  # padding 0 to right --> max length
        input_ids = torch.tensor(input_ids, dtype=torch.int64, device=self.device)
        # attn mask
        attention_mask = create_attention_mask(
            original_lengths, max_tokens, self.device
        )
