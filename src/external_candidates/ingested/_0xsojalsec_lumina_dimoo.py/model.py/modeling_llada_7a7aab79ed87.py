# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Lumina-DiMOO\model\modeling_llada.py
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(
        self, q: torch.Tensor, k: torch.Tensor, q_mask=None
    ) -> Tuple[torch.Tensor, torch.Tensor]:
...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        if self.config.rope:
            to_compute_index = (
                to_compute_mask.nonzero(as_tuple=True)[1]
                if self.use_cache and to_compute_mask is not None
                else None
            )
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        if use_cache:
            ...
            if to_compute_mask is not None:
                self.logit_cache[cat][to_compute_mask] = logits.view(
                    -1, logits.shape[-1]
                )
