# Extracted from: C:\DEV\PyAgent\.external\action\2_engineering_team\engineering_team\example_output_4o\app.py
import gradio as gr
...
def list_transactions():
    ...
    for idx, tx in enumerate(transactions, 1):
        if tx["type"] == "deposit":
            result += (
                f"{idx}. Deposit: ${tx['amount']:.2f}, Balance: ${tx['balance']:.2f}\n"
            )
        elif tx["type"] == "withdraw":
            result += (
                f"{idx}. Withdraw: ${tx['amount']:.2f}, Balance: ${tx['balance']:.2f}\n"
            )
        elif tx["type"] == "buy":
...
with gr.Blocks(title="Trading Simulation Platform") as demo:
    ...
    with gr.Tab("Create Account"):
        ...
        create_btn.click(
            create_account, inputs=[deposit_input], outputs=[create_output]
        )
    ...
    with gr.Tab("Deposit/Withdraw"):
        ...
        withdraw_btn.click(
            withdraw_funds, inputs=[withdraw_amount], outputs=[fund_output]
        )
    ...
    with gr.Tab("Trade Stocks"):
        ...
        buy_btn.click(
            buy_stock, inputs=[buy_symbol, buy_quantity], outputs=[trade_output]
        )
        sell_btn.click(
            sell_stock, inputs=[sell_symbol, sell_quantity], outputs=[trade_output]
        )
    ...
    with gr.Tab("Transaction History"):
        ...
        transaction_btn.click(
            list_transactions, inputs=[], outputs=[transaction_output]
        )
