# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IDOR-Forge\core\IDORChecker.py
import argparse
import base64
...
from tkinter import END, Button, Entry, Label, Scrollbar, Text, Tk, messagebox, ttk
...
class IDORChecker:
    ...
    def login(self, login_url: str, credentials: Dict, method: str = "POST") -> bool:
        ...
        try:
            ...
            if response.status_code == 200 or response.status_code == 302:
                ...
                print(
                    f"{Fore.RED}Login failed. Status Code: {response.status_code}{Style.RESET_ALL}"
                )
                return False
    ...
    def check_idor(
        self, param: str, test_values: List[str], method: str = "GET"
    ) -> List[Dict]:
        """
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = [
                executor.submit(self._test_payload, payload, method)
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    self.logger(
                        f"{Fore.RED}Error testing payload: {e}{Style.RESET_ALL}"
                    )
    ...
    def _load_payloads_from_file(self, filename: str) -> List[str]:
        ...
        try:
            ...
            print(
                f"{Fore.RED}Warning: Payload file {filename} not found. Using default values.{Style.RESET_ALL}"
            )
            return []
    ...
    def _send_request(
        self, params: Dict, method: str = "GET"
            ...
            try:
                ...
                if method.upper() == "GET":
                    response = self.session.get(
                        self.base_url, params=params, **request_args
                    )
                elif method.upper() == "POST":
                    response = self.session.post(
                        self.base_url, data=params, **request_args
                    )
                ...
                print(
                    f"{Fore.YELLOW}Request failed (Attempt {retries}/{self.max_retries}): {e}{Style.RESET_ALL}"
                )
                time.sleep(delay)
        return None
    ...
    def _test_payload(self, payload: Dict, method: str = "GET") -> Optional[Dict]:
        ...
        try:
            ...
            self.logger(
                f"{Fore.RED}Request failed for payload {payload}: {e}{Style.RESET_ALL}"
            )
            return None
    ...
    def _save_results_txt(self, results: List[Dict], output_file: str):
        ...
        with open(output_file, "w") as f:
            for result in results:
                ...
                f.write(
                    f"Sensitive Data Detected: {result['sensitive_data_detected']}\n"
                )
                f.write("-" * 40 + "\n")
    ...
    def generate_report(
        self, results: List[Dict], output_file: str, format_type: str = "txt"
    ):
        """
        ...
        if format_type == "json":
            ...
            print(
                f"{Fore.RED}Unsupported format: {format_type}. Using default (txt).{Style.RESET_ALL}"
            )
            self._save_results_txt(results, output_file)
    ...
    def visualize_results(self, results: List[Dict]):
        ...
        vulnerable_count = sum(
            1 for result in results if result["sensitive_data_detected"]
        )
    ...
    def _display_summary(self, results: List[Dict]):
        ...
        """
        vulnerabilities = [
            result for result in results if result["sensitive_data_detected"]
        ]
        print(f"\n{Fore.CYAN}Scan Summary:{Style.RESET_ALL}")
        print(f"{Fore.GREEN}Total Payloads Tested: {len(results)}{Style.RESET_ALL}")
        print(
            f"{Fore.RED}Vulnerabilities Found: {len(vulnerabilities)}{Style.RESET_ALL}"
        )
