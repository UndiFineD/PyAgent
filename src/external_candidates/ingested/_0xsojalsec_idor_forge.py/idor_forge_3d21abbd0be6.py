# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IDOR-Forge\IDOR-Forge.py
import argparse
import json
...
def main():
    ...
    parser.add_argument(
        "-u", "--url", help="Target URL to test for IDOR vulnerabilities"
    )
    parser.add_argument(
        "-p", "--parameters", action="store_true", help="Scan all parameters in the URL"
    )
    parser.add_argument(
        "-m",
        "--method",
    ...
    parser.add_argument(
        "--sensitive-keywords", help="Custom sensitive keywords in JSON format"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="Launch interactive GUI mode"
    )
    parser.add_argument("--login-url", help="URL for the login page")
    parser.add_argument(
        "--credentials",
        help='Login credentials in JSON format (e.g., \'{"username": "admin", "password": "password"}\')',
    )
    parser.add_argument(
        "--login-method",
        default="POST",
        help="HTTP method to use for login (default: POST)",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
        default=5,
        help="Number of threads for multi-threaded scanning",
    )
    parser.add_argument(
        "--num-range",
        help="Range of numbers to test as payloads, format: start-end (e.g., 1-100)",
    )
    ...
    if hasattr(checker, "visualize_results"):
        visualize = (
            input("Do you want to visualize the results? (y/n): ").strip().lower()
        )
        if visualize == "y":
