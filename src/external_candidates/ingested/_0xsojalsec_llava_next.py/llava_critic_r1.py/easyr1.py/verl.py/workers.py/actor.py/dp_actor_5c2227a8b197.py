# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLaVA-NeXT\llava-critic-r1\EasyR1\verl\workers\actor\dp_actor.py
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
                ...
                for key in (
                    ...
                    if not torch.is_tensor(x):
                        try:
                            ...
                            x = torch.tensor(x)
                    if key == "pixel_values":
                        if (not x.dtype.is_floating_point) or (
        ...
        if self.config.padding_free:
            ...
            full_logits = pad_input(
                hidden_states=logits_rmpad.unsqueeze(-1),
                indices=indices,
                batch=batch_size,
                seqlen=seqlen,
            )
            logits = full_logits.squeeze(-1)[
                :, -response_length - 1 : -1, :
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            ...
            log_probs, _ = self._forward_micro_batch(
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    log_probs, logits = self._forward_micro_batch(
                        model_inputs, temperature=temperature
                    )
                    ...
                    if self.config.is_entropy_reg:
                        loss = (
                            pg_loss + self.config.entropy_coef * new_entropy_loss
                        ) / gradient_accumulation
                    else:
