# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLaVA-NeXT\scripts\archived\data_info.py
import json
import os
...
def calculate_image_dimensions_multiprocess(
    ...
    with Pool(num_processes) as p:
        dimensions = list(
            tqdm(
                p.imap(
                    functools.partial(
                        calculate_image_dimension, images_folder=images_folder
                    ),
                    filtered_data,
                ),
...
def main():
    ...
    parser.add_argument(
        "--json_path", type=str, help="Path to the JSON file containing data."
    )
    parser.add_argument(
        "--images_folder",
        type=str,
    ...
    if len(filtered_data) != 0:
        print(
            f"Total data items: {len(data)}, Filtered data items: {len(filtered_data)}"
        )
        widths, heights = calculate_image_dimensions_multiprocess(
            filtered_data, images_folder
        )
    ...
    hist, bin_edges = np.histogram(
        tokenized_lengths, bins=np.arange(0, max(tokenized_lengths) + 10, 100)
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"/mnt/bn/vl-research/workspace/boli01/projects/LLaVA_Next/notebooks/sft_data/dist_{llava_instruct_name}_combined.png"
    )
    print(
        f"Plots saved to /mnt/bn/vl-research/workspace/boli01/projects/LLaVA_Next/notebooks/sft_data/dist_{llava_instruct_name}_combined.png"
    )
