# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLaVA-NeXT\playground\demo\video_demo.py
import argparse
import base64
import json
import math
...
def parse_args():
    ...
    parser.add_argument(
        "--load_8bit", type=lambda x: (str(x).lower() == "true"), default=False
    )
...
def run_inference(args):
        ...
        if args.overwrite == True:
            ...
            if "qwen" not in args.model_path.lower():
                if "224" in cfg_pretrained.mm_vision_tower:
                    ...
                    least_token_number = (
                        args.for_get_frames_num
                        * (16 // args.mm_spatial_pool_stride) ** 2
                        + 1000
                    )
                else:
                    least_token_number = (
                        args.for_get_frames_num
                        * (24 // args.mm_spatial_pool_stride) ** 2
                        + 1000
                    )
    ...
    for video_path in all_video_pathes:
        ...
        if "gpt4v" != args.model_path:
            ...
            if not sucess_flag:
                print(
                    f"Calling OpenAI failed after retrying for {max_num_retries} times. Check the logs for details."
                )
                exit()
