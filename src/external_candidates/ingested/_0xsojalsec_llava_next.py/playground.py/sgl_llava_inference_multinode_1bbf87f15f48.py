...
def main(args):
    ...
    end_index = (
        start_index + shard_size if dist_rank < dist_size - 1 else len(image_files)
    )
    shard_files = image_files[start_index:end_index]

    print(
        f"Querying {len(shard_files)} images from index {start_index} to {end_index - 1}"
    )
    ...
    for batch_start in tqdm.tqdm(range(0, len(shard_files), batch_size)):
        ...
        batch_arguments = [
            ...
            for image_file in shard_files[batch_start:batch_end]
        ]
        try:
            ...
            for i, ret in enumerate(batch_states):
                ...
                progress_data["annotations"].append(
                    {"image_file": image_file, "caption": caption}
                )
                progress_data["last_index"] = (
                    batch_start + i
                )  # Update last_index relative to this rank's shard
    ...
    result_file = args.result_file.replace(
        ".json", f"_shard_{dist_rank}_or_{dist_size}.json"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--images_root", type=str, default="/mnt/bn/vl-research/data/llava_data/cc3m"
    )
    parser.add_argument(
        "--json_path",
        type=str,
    ...
    parser.add_argument(
        "--dist", type=int, default=0, help="The rank of the distributed machine"
    )
    parser.add_argument(
        "--total_dist", type=int, default=6, help="Total number of distributed machines"
    )
