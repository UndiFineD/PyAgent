# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLaVA-NeXT\playground\data_checker.py
import json
import os
...
class DataProcessor:
    ...
    def check_item_structure(self, item):
        if not all(key in item for key in ["conversations"]):
            print(
                f"WARNING!!! Item {item.get('id', 'unknown')} is missing required fields!"
            )
            return False
    ...
    def process_images(self):
        if isinstance(self.data, list):
            ...
            with Pool(processes=cpu_count()) as pool:
                list(
                    tqdm(
                        pool.imap(self.check_image_and_structure, args),
                        total=len(self.data),
                    )
                )
                ...
                with Pool(processes=cpu_count()) as pool:
                    list(
                        tqdm(
                            pool.imap(self.check_image_and_structure, args),
                            total=len(data),
                            desc=f"Processing {dd_json_path}",
                        )
                    )
            ...
            for d in self.data["datasets"]:
                ...
                try:
                    if sampling_strategy != "all":
                        percentage = (
                            float(sampling_strategy.split(":")[-1].replace("%", ""))
                            / 100.0
                        )
                    else:
                        percentage = 1.0
    ...
    def stat_data(self):
        if isinstance(self.data, dict):
            ...
            print(
                f"Max length: {max(cur_lens_list)}, Min length: {min(cur_lens_list)}, Average length: {sum(cur_lens_list) / len(cur_lens_list)}"
            )
            print(f"Total items: {total_count}")
            print(f"Text items: {text_count} ({text_count/total_count*100:.2f}%)")
            print(
                f"Single image items: {single_image_count} ({single_image_count/total_count*100:.2f}%)"
            )
            print(
                f"Multiple image items: {multiple_image_count} ({multiple_image_count/total_count*100:.2f}%)"
            )
            ...
            for d in self.data["datasets"]:
                ...
                for item in data:
                    try:
                        ...
                        if (
                            num_img_token_appearance == num_visuals
                            or num_img_token_appearance < num_visuals
                            and len(conv_text) > 0
                        ):
                ...
                if len(filtered_data) < len(data):
                    saving_dd_json_path = dd_json_path.replace(
                        ".jsonl", f"fltd_{len(filtered_data)}.json"
                    ).replace(".json", f"fltd_{len(filtered_data)}.json")
            ...
            for d in self.data["datasets"]:
                ...
                for item in data:
                    ...
                    for i in range(0, len(conversations), 2):
                        if i + 1 < len(conversations):
                            ...
                            pair_tokens = len(human_conv["value"].split()) + len(
                                gpt_conv["value"].split()
                            )
                ...
                if filtered_data and save_flag:
                    if dd_json_path.endswith(".jsonl"):
                        output_file = dd_json_path.replace(
                            ".jsonl",
                            f"_filtered_{threshold}tokens_{len(filtered_data)}.jsonl",
                        )
                        ...
                        output_file = dd_json_path.replace(
                            ".json",
                            f"_filtered_{threshold}tokens_{len(filtered_data)}.json",
                        )
                        with open(output_file, "w") as f:
                            json.dump(filtered_data, f, indent=2)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file_path",
        type=str,
        default="/mnt/bn/vl-research/workspace/boli01/projects/LLaVA_Next/scripts/i18n/scale_llms/next_continual.yaml",
    )
    parser.add_argument(
        "--image_root", type=str, default="/mnt/bn/vl-research/data/llava_data"
    )
    parser.add_argument(
        "--video_root", type=str, default="/mnt/bn/vl-research/data/llava_video"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Threshold for stat_and_filter operation",
    )
    args = parser.parse_args()
    main(
        args.file_path, args.image_root, args.operation, args.video_root, args.threshold
    )
