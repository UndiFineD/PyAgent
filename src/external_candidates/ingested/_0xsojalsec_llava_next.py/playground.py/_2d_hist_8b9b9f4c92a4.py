# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLaVA-NeXT\playground\2d_hist.py
import argparse
import functools
import json
import os
...
def main():
    ...
    # Plot 2D histogram
    widths_bins = (
        [min(widths), max(widths) + 1]
        if min(widths) == max(widths)
        else np.arange(min(widths), max(widths) + 100, 100)
    )
    heights_bins = (
        [min(heights), max(heights) + 1]
        if min(heights) == max(heights)
        else np.arange(min(heights), max(heights) + 100, 100)
    )

    h, xedges, yedges, image = ax1.hist2d(
        widths, heights, bins=[widths_bins, heights_bins], cmap=plt.cm.jet, density=True
    )
    ...
    hist, bin_edges = np.histogram(
        tokenized_lengths, bins=np.arange(0, max(tokenized_lengths) + 10, 10)
    )
