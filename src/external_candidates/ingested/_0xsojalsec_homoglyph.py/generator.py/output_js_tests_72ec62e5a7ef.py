# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-homoglyph\generator\output_js_tests.py
    ...
    def create(self, char_manager, chars):
        ...
        for char in chars:
            ...
            for char_homoglyph in char_homoglyphs:
                char_homoglyphs_as_unicode.append(
                    '"\\u{'
                    + "{:0>4}".format(self._hex_code_for_char(char_homoglyph))
                    + '}"'
                )

            check_statements.append(
                '    check("{}", [{}]);'.format(
                    char, " ,".join(char_homoglyphs_as_unicode)
                )
            )

        text = self._get_template_text().replace(
