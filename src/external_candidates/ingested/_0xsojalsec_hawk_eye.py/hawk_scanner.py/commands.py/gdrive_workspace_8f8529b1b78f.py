# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hawk-eye\hawk_scanner\commands\gdrive_workspace.py
import json
import os
...
def download_file(args, drive, file_obj, base_path):
    ...
    try:
        ...
        if (
            "mimeType" in file_obj
            and file_obj["mimeType"] == "application/vnd.google-apps.folder"
        ):
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            folder_files = (
                drive.files()
                .list(q=f"'{file_id}' in parents")
                .execute()
                .get("files", [])
            )
            ...
            for impersonate_user in impersonate_users or [None]:
                ...
                if drive:
                    ...
                    for file_obj in files:
                        ...
                        if system.should_exclude_file(
                            args, file_name, exclude_patterns
                        ):
                            continue
                        ...
                        if config.get("cache") and os.path.exists(file_path):
                            is_cache_enabled = False
                            system.print_debug(
                                args, f"File already exists in cache, using it."
                            )
                        else:
