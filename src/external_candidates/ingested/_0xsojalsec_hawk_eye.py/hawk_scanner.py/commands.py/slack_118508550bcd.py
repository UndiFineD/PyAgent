# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hawk-eye\hawk_scanner\commands\slack.py
import time
...
def check_slack_messages(
    args, client, patterns, profile_name, channel_types, channel_ids=None, limit_mins=60
):
    results = []
    try:
        ...
        current_time_readable = datetime.fromtimestamp(current_time).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        oldest_time_readable = datetime.fromtimestamp(oldest_time).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        system.print_info(
            args,
            f"Fetching messages from the last {limit_mins} minutes (Oldest Time: {oldest_time_readable}, Unix: {int(oldest_time)})",
        )
        ...
        if not channel_ids:
            ...
            for channel_id in channel_ids:
                try:
                    ...
                    system.print_error(
                        args,
                        f"Failed to fetch channel with id {channel_id} with error: {e.response['error']}",
                    )
        ...
        system.print_info(
            args,
            f"Checking messages in channels: {', '.join([channel['name'] for channel in channels])}",
        )
        ...
        for channel in channels:
            ...
            messages = client.conversations_history(
                channel=channel_id, oldest=oldest_time
            )["messages"]
            ...
            for message in messages:
                ...
                if "thread_ts" in message:
                    ...
                    replies = client.conversations_replies(
                        channel=channel_id, ts=thread_ts, oldest=oldest_time
                    )["messages"]
        ...
        system.print_error(
            args,
            f"Failed to fetch messages from Slack with error: {e.response['error']}",
        )
        return results
