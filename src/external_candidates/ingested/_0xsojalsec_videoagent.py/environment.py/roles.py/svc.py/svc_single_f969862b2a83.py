...
class SVCSingle(BaseTool):
    ...
    def execute(self, **kwargs):
        ...
        name = params.name
        source_vocal_song = self._generate_audio_segments_batch(
            name, text_list, notes_list, notes_duration_list
        )
    ...
    def _generate_audio_segments(
        self, name, text_list, notes_list, notes_duration_list
    ):
        ...
        for i in range(len(text_list)):
            ...
            if segment_type == "AP":
                ...
                segment = self._create_segment(
                    i, text_list, notes_list, notes_duration_list
                )
                segment_name = f"{new_name}_part_{i}"
                inp_path = os.path.join(
                    "dataset/mad_svc/lyrics_annotation", f"{segment_name}.json"
                )
                ...
                try:
                    ...
                    if not np.isclose(current_duration, target_duration_sec, atol=0.01):
                        ...
                        audio_array = librosa.effects.time_stretch(
                            audio_array, rate=stretch_factor
                        )

                    # 转换为AudioSegment（原始长度，不做截断）
                    audio_segment = AudioSegment(
                        data=(np.clip(audio_array, -1.0, 1.0) * 32767)
                        .astype(np.int16)
                        .tobytes(),
                        sample_width=2,
                        frame_rate=sr,
                    ...
                    if actual_ms != target_ms:
                        ...
                        if actual_ms < target_ms:
                            ...
                            silence = AudioSegment.silent(
                                duration=target_ms - actual_ms, frame_rate=sr
                            )
                        ...
                        print(
                            f"After adjustment: {len(audio_segment)}ms "
                    ...
                    if abs(final_error_ms) > 1:
                        print(
                            f"Warning: Segment {i} still has error: {final_error_ms}ms"
                        )
    ...
    def _create_segment_with_min_duration(
        ...
        if duration < threshold_duration:
            ...
            while (
                ...
                and idx < len(text_list)
                and text_list[idx] != "AP"
            ):
    ...
    def _generate_audio_segments_batch(
        self, name, text_list, notes_list, notes_duration_list
    ):
        """生成完整音频并返回最终路径（精确时长控制版）"""
        new_name = f"{name}_cover"
        ...
        while start_idx < len(text_list):
            ...
            if segment["text"] != "AP":
                with open(
                    os.path.join(
                        tmp_dir, f"{new_name}_part_{start_idx_str}-{end_idx_str}.json"
                    ),
                    "w",
                    encoding="utf-8",
                ) as f:
