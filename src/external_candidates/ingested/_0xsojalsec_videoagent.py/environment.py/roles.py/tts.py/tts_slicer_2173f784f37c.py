    ...
    class _Slicer:
        def __init__(
            ...
            if not min_length >= min_interval >= hop_size:
                raise ValueError(
                    "The condition to be satisfied is：min_length >= min_interval >= hop_size"
                )

            if not max_sil_kept >= hop_size:
                raise ValueError(
                    "The condition to be satisfied is: max_sil_kept >= hop_size"
                )
        ...
        def slice(self, waveform):
            ...
            if total_samples <= self.min_length * self.hop_size:
                return [
                    {"audio": waveform, "start": 0.0, "end": total_samples / self.sr}
                ]
            ...
            for i, rms in enumerate(rms_list):
                ...
                if i - silence_start <= self.max_sil_kept:
                    ...
                    sil_tags.append(
                        (pos_l, pos_r) if silence_start != 0 else (0, pos_r)
                    )
                    clip_start = pos_r
