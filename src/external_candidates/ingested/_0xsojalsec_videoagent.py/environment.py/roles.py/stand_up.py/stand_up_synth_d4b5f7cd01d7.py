    ...
    def execute(self, **kwargs):
        ...
        for line in script.split("\n"):
            ...
            try:
                ...
                with open(
                    os.path.join(target_vocal_dir, f"{tone}.lab"), "r", encoding="utf-8"
                ) as f:
                    prompt_text = f.read().strip()

                prompt_speech_16k = load_wav(
                    os.path.join(target_vocal_dir, f"{tone}.wav"), 16000
                )
                ...
                if "reaction" in result:
                    ...
                    try:
                        ...
                        combined_audio.export(
                            os.path.join(seg_dir, f"{cnt}.wav"), format="wav"
                        )
                        print(f"Successfully combined reaction audio for line {cnt}.")
                    except Exception as e:
                        print(
                            f"Error combining reaction audio for line {cnt}: {str(e)}"
                        )
