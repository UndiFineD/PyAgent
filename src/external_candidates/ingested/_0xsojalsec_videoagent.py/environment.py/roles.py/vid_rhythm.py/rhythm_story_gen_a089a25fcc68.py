...
class RhythmContentGenerator(BaseTool):
    ...
    def _extract_video_content(
        self, video_segments_path, summary_output_path, chunks=4
    ):
        ...
        print(
            f"Split content into {len(content_chunks)} parts with approximately {chunk_size} segments each"
        )
        ...
        for i, chunk_content in enumerate(content_chunks):
            print(
                f"Creating narrative summary for chunk {i+1}/{len(content_chunks)}..."
            )
            chunk_summary = self._get_chunk_summary(
                chunk_content, i + 1, len(content_chunks)
            )
        ...
        if len(chunk_summaries) > 2:
            ...
            for i in range(0, len(chunk_summaries), 2):
                if i + 1 < len(chunk_summaries):
                    ...
                    combined = self._combine_pair(
                        chunk_summaries[i], chunk_summaries[i + 1], i // 2 + 1
                    )
    ...
    def _get_chunk_summary(
        self, content: str, chunk_number: int, total_chunks: int
    ) -> str:
    ...
    def _combine_pair(
        self, first_summary: str, second_summary: str, pair_number: int
    ) -> str:
    ...
    def _create_storyboard(
        self, user_idea, audio_json_path, rhythm_plot_path, video_summary=None
    ):
        ...
        try:
            if os.path.exists(audio_json_path):
                ...
                print(
                    f"Audio JSON file not found at {audio_json_path}, using default of {sections_num} sections"
                )
        except Exception as e:
            print(
                f"Error reading audio JSON file: {e}. Using default of {sections_num} sections"
            )
        ...
        try:
            response = self._call_llm_api(
                system=system_prompt, prompt=user_prompt, temperature=0.7
            )
            return response.choices[0].message.content
        except Exception as e:
    ...
    def execute(self, **kwargs):
        ...
        rhythm_detection_path = os.path.join(
            rhythm_analysis_dir, "rhythm_detection.png"
        )
