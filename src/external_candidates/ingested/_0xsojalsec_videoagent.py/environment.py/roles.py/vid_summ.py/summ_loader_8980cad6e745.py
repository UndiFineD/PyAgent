...
class VideoSummarizationGenerator(BaseTool):
    ...
    def _transcribe_video(self, video_path: str) -> str:
        ...
        try:
            ...
            result = self.transcript_pipe(
                video_path, generate_kwargs={"language": "en", "task": "transcribe"}
            )
        ...
        try:
            ...
            for i, video_file in enumerate(video_files, 1):
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to transcribe {video_file} after retries: {e}"
                    )
                    # Add error message to transcript
                    video_name = os.path.basename(video_file)
                    combined_transcript += f"\n\n=== VIDEO: {video_name} ===\n"
                    combined_transcript += (
                        f"[Error transcribing {video_name}: {str(e)}]"
                    )
            ...
            print(
                f"Successfully processed {len(processed_videos)}/{len(video_files)} videos"
            )
            logger.info(
    ...
    def _make_api_call(
        self, system_message, user_message, temperature=0.7, timeout=None
    ):
    ...
    def _get_content_from_video_dir(
        self, video_dir: str
        ...
        if os.path.isfile(video_dir):
            ...
            if video_files:
                ...
                combined_transcript, processed_videos = self._transcribe_all_videos(
                    video_files
                )
                return (
                    combined_transcript,
                ...
                if os.path.exists(default_transcript):
                    print(
                        f"No videos found, loading default transcript: {default_transcript}"
                    )
                    content = self._load_text(default_transcript)
                    return content, None, "loaded_default_transcript"
                else:
                    raise ValueError(
                        f"No video files or transcript found in directory: {video_dir}"
                    )
        else:
    ...
    def _process_pipeline(
        ...
        content, processed_videos, source_type = self._get_content_from_video_dir(
            video_dir
        )
        ...
        try:
            ...
            presenter_output = self._presenter_agent(
                user_idea, content, present_content
            )
    ...
    def execute(self, **kwargs):
        ...
        if not os.path.isabs(present_style) and os.path.exists(
            os.path.join(parent_root, present_style)
        ):
        ...
        try:
            ...
            if "error" in content_result:
                raise RuntimeError(
                    f"Failed to create content: {content_result['error']}"
                )
            ...
            return {
                ...
                "processed_videos": (
                    [os.path.basename(v) for v in processed_videos]
                    if processed_videos
                    else None
                ),
