...
class CommentaryContentGenerator(BaseTool):
    ...
    def _make_api_call(self, messages, temperature=0.7, max_tokens=None, timeout=None):
        ...
        try:
            logger.info(
                f"Making API call with {len(messages)} messages, timeout={timeout}s"
            )
    ...
    def _judger_agent(self, user_idea: str, presenter_output: str) -> str:
        ...
        for attempt in range(2):
            try:
                ...
                result = self._make_api_call(
                    messages, temperature=0.4, timeout=60, max_tokens=16384
                )
    ...
    def _process_pipeline(
        self, user_idea: str, txt_path: str, present_style: str
    ) -> str:
        ...
        try:
            ...
            presenter_output = self._presenter_agent(
                user_idea, book_content, present_style
            )
