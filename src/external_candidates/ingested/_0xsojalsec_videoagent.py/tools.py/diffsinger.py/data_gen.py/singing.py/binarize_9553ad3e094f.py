# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\DiffSinger\data_gen\singing\binarize.py
import glob
import json
import logging
import os
...
class SingingBinarizer(BaseBinarizer):
    ...
    def split_train_test_set(self, item_names):
        ...
        test_item_names = [
            x for x in item_names if any([ts in x for ts in hparams["test_prefixes"]])
        ]
    ...
    def load_meta_data(self):
        for ds_id, processed_data_dir in enumerate(self.processed_data_dirs):
            ...
            for piece_path in all_wav_pieces:
                item_name = raw_item_name = piece_path[
                    len(processed_data_dir) + 1 :
                ].replace("/", "-")[: -len(wav_suffix)]
                if len(self.processed_data_dirs) > 1:
                    item_name = f"ds{ds_id}_{item_name}"
                self.item2txt[item_name] = open(
                    f"{piece_path.replace(wav_suffix, txt_suffix)}"
                ).readline()
                self.item2ph[item_name] = open(
                    f"{piece_path.replace(wav_suffix, ph_suffix)}"
                ).readline()
...
class MidiSingingBinarizer(SingingBinarizer):
    ...
    def load_meta_data(self):
        for ds_id, processed_data_dir in enumerate(self.processed_data_dirs):
            meta_midi = json.load(
                open(os.path.join(processed_data_dir, "meta.json"))
            )  # [list of dict]
            ...
            for song_item in meta_midi:
                ...
                self.item2wdb[item_name] = [
                    1 if x in ALL_YUNMU + ["AP", "SP", "<SIL>"] else 0
                    for x in song_item["phs"]
                ]
    ...
    def get_pitch(wav_fn, wav, spec, ph, res):
        ...
        item_name = "/".join(os.path.splitext(wav_fn)[0].split("/")[-2:]).replace(
            "_wf0", ""
        )
    ...
    def process_item(
        ...
        try:
            ...
            if binarization_args["with_txt"]:
                ...
                if binarization_args["with_align"]:
                    cls.get_align(
                        MidiSingingBinarizer.item2ph_durs[item_name],
                        mel,
                        phone_encoded,
                        res,
                    )
    ...
    def process_item(
        ...
        try:
            ...
            if binarization_args["with_txt"]:
                ...
                if binarization_args["with_align"]:
                    cls.get_align(
                        OpencpopBinarizer.item2ph_durs[item_name],
                        mel,
                        phone_encoded,
                        res,
                    )
