# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\DiffSinger\data_gen\tts\base_binarizer.py
import os
...
import numpy as np
import pandas as pd
from data_gen.tts.data_gen_utils import build_phone_encoder, get_mel2ph, get_pitch
...
class BaseBinarizer:
    def __init__(self, processed_data_dir=None):
        ...
        for ds_id, processed_data_dir in enumerate(self.processed_data_dirs):
            self.meta_df = pd.read_csv(
                f"{processed_data_dir}/metadata_phone.csv", dtype=str
            )
            for r_idx, r in self.meta_df.iterrows():
                ...
                self.item2wavfn[item_name] = os.path.join(
                    hparams["raw_data_dir"],
                    "wavs",
                    os.path.basename(r["wav_fn"]).split("_")[1],
                )
                self.item2spk[item_name] = r.get("spk", "SPK1")
                if len(self.processed_data_dirs) > 1:
                    self.item2spk[item_name] = f"ds{ds_id}_{self.item2spk[item_name]}"
                if tg_dir is not None:
                    self.item2tgfn[item_name] = (
                        f"{processed_data_dir}/{tg_dir}/{raw_item_name}.TextGrid"
                    )
    ...
    def test_item_names(self):
        return self.item_names[
            0 : hparams["test_num"]
        ]  # Audios for MOS testing are in 'test_ids'
        ...
        for f_id, (_, item) in enumerate(
            zip(
                ...
                chunked_multiprocess_run(
                    self.process_item, args, num_workers=num_workers
                ),
            )
        ):
            if item is None:
                continue
            item["spk_embed"] = (
                voice_encoder.embed_utterance(item["wav"])
                if self.binarization_args["with_spk_embed"]
                else None
            )
