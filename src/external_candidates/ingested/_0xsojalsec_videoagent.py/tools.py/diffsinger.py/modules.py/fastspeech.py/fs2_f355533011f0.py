# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\DiffSinger\modules\fastspeech\fs2.py
...
class FastSpeech2(nn.Module):
    def __init__(self, dictionary, out_dims=None):
        ...
        self.encoder_embed_tokens = self.build_embedding(
            self.dictionary, self.hidden_size
        )
        self.encoder = FS_ENCODERS[hparams["encoder_type"]](
            hparams, self.encoder_embed_tokens, self.dictionary
        )
    ...
    def add_pitch(self, decoder_inp, f0, uv, mel2ph, ret, encoder_out=None):
        ...
        if hparams["pitch_type"] == "cwt":
            ...
            ret["pitch_pred"] = pitch_pred = self.pitch_predictor(
                decoder_inp, f0 if self.training else None
            )
            if f0 is None:
                f0 = pitch_pred[:, :, 0]
        else:
