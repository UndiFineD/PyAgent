# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\seed-vc\baselines\openvoice.py
import os
import sys
...
def convert(source_path, reference_path, output_path):
    target_se, audio_name = se_extractor.get_se(
        reference_path, tone_color_converter, vad=False
    )
    source_se, audio_name = se_extractor.get_se(
        source_path, tone_color_converter, vad=False
    )
