# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\seed-vc\modules\quantize.py
import numpy as np
import torch
import torchaudio
...
class FAquantizer(nn.Module):
    ...
    def forward(self, x, wave_segments):
        ...
        z_p, codes_p, latents_p, commitment_loss_p, codebook_loss_p = (
            self.prosody_quantizer(f0_input, 1)
        )
        outs += z_p.detach()

        z_c, codes_c, latents_c, commitment_loss_c, codebook_loss_c = (
            self.content_quantizer(x, 2)
        )
        outs += z_c.detach()

        residual_feature = x - z_p.detach() - z_c.detach()

        z_r, codes_r, latents_r, commitment_loss_r, codebook_loss_r = (
