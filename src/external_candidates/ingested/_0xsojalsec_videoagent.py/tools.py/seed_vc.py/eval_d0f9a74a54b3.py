# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\seed-vc\eval.py
import argparse
import os
...
def load_models(args):
    ...
    if speech_tokenizer_type == "whisper":
        ...
        raise ValueError(
            f"Unsupported speech tokenizer type: {model_params.speech_tokenizer.type}"
        )
...
def main(args):
    ...
    asr_model = HubertForCTC.from_pretrained("facebook/hubert-large-ls960-ft").to(
        device
    )
    ...
    for source_i, source_line in enumerate(tqdm(source_audio_list)):
        ...
        for target_i, target_name in enumerate(target_audio_list):
            ...
            if os.path.exists(
                ...
                if baseline == "openvoice":
                    ...
                    ref_waves_16k, vc_wave_16k = openvoice_convert(
                        source_path, target_path, "temp.wav"
                    )
                elif baseline == "cosyvoice":
                    from baselines.cosyvoice import convert as cosyvoice_convert

                    ref_waves_16k, vc_wave_16k = cosyvoice_convert(
                        source_path, target_path, "temp.wav"
                    )
                else:
                ...
                os.makedirs(
                    osp.join(conversion_result_dir, source_index), exist_ok=True
                )
                torchaudio.save(
                    osp.join(conversion_result_dir, source_index, f"{target_name}"),
                    vc_wave_16k.cpu(),
            ...
            if args.xvector_extractor == "wavlm":
                ...
                ref_embeddings = torch.nn.functional.normalize(
                    ref_embeddings, dim=-1
                ).cpu()
                ...
                vc_embeddings = torch.nn.functional.normalize(
                    vc_embeddings, dim=-1
                ).cpu()
            ...
            # calculate dnsmos
            sig, bak, ovr = calc_mos(
                mos_computer, vc_wave_16k.squeeze(0).cpu().numpy(), 16000
            )
        ...
        print(
            f"Average DNS MOS SIG: {sum([x[0] for x in dnsmos_list]) / len(dnsmos_list)}"
        )
        print(
            f"Average DNS MOS BAK: {sum([x[1] for x in dnsmos_list]) / len(dnsmos_list)}"
        )
        print(
            f"Average DNS MOS OVR: {sum([x[2] for x in dnsmos_list]) / len(dnsmos_list)}"
        )

        # save wer and cer result into this directory as a txt
        with open(
            osp.join(conversion_result_dir, source_index, "result.txt"), "w"
        ) as f:
            f.write(
            ...
            f.write(
                f"Average similarity: {sum(similarity_list[-len(target_audio_list):]) / len(target_audio_list)}\n"
            )
    ...
    with open(
        osp.join(conversion_result_dir, f"{args.xvector_extractor}_similarity.tsv"), "w"
    ) as f:
...
def convert(
    ...
    if source_audio.size(1) + ref_audio.size(1) > 30 * sr:
        print(
            f"reference audio clipped from {ref_audio.size(1)/sr} seconds to {30 * sr - source_audio.size(1)} seconds"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source", type=str, default="./examples/libritts-test-clean/")
