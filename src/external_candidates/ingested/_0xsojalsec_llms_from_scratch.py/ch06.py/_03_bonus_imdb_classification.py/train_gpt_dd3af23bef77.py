# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMs-from-scratch\ch06\03_bonus_imdb-classification\train-gpt.py
...
def calc_accuracy_loader(
    data_loader, model, device, num_batches=None, trainable_token=-1
):
    model.eval()
...
def evaluate_model(
    model, train_loader, val_loader, device, eval_iter, trainable_token=-1
):
    model.eval()
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    train_losses, val_losses, train_accs, val_accs, examples_seen = (
        train_classifier_simple(
            ...
            tokenizer=tokenizer,
            max_steps=None,
            trainable_token=args.trainable_token,
        )
    )
    ...
    train_accuracy = calc_accuracy_loader(
        train_loader, model, device, trainable_token=args.trainable_token
    )
    val_accuracy = calc_accuracy_loader(
        val_loader, model, device, trainable_token=args.trainable_token
    )
    test_accuracy = calc_accuracy_loader(
        test_loader, model, device, trainable_token=args.trainable_token
    )
