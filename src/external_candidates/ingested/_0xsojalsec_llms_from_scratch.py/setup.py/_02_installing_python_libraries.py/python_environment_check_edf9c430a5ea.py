# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMs-from-scratch\setup\02_installing-python-libraries\python_environment_check.py
...
def check_packages(d):
    ...
    for (pkg_name, suggested_ver), actual_ver in zip(d.items(), versions):
        if actual_ver == "N/A":
            continue
        actual_ver, suggested_ver = version_parse(actual_ver), version_parse(
            suggested_ver
        )
