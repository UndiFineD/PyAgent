# Extracted from: C:\DEV\PyAgent\.external\Stream-Omni\stream_omni\conversation.py
import base64
import dataclasses
import re
...
class ChatItem:
    def __init__(
        self, msg="", text="", image=None, image_process_mode=None, audio=None
    ):
        self.msg = msg
        self.text = text
        self.image = image
    ...
    def __repr__(self):
        return (
            f"Chat(msg={self.msg}, text={self.text}, "
            f"image={self.image}, image_process_mode={self.image_process_mode}, audio={self.audio})"
        )
...
class Conversation:
    ...
    def to_gradio_chatbot_from_chat(self):
        ...
        for i, item in enumerate(self.chatbot[self.offset :]):
            # print(item)
            if i % 2 == 0:
                msg, text, image, image_process_mode, audio = (
                    item.msg,
                    item.text,
                    item.image,
                    item.image_process_mode,
                    item.audio,
                )
                if type(image) != list:
                    image = [image]
