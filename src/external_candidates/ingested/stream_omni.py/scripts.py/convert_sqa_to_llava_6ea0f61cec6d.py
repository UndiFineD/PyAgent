# Extracted from: C:\DEV\PyAgent\.external\Stream-Omni\scripts\convert_sqa_to_llava.py
import json
import os
import re
...
def convert_to_llava(base_dir, split, prompt_format="QCM-LEA"):
    ...
    split_problems = build_prompt_chatbot(
        problems, split_indices, prompt_format, use_caption=False, is_test=False
    )
...
def convert_to_jsonl(base_dir, split, prompt_format="QCM-LEPA"):
    ...
    writer = open(
        os.path.join(base_dir, f"scienceqa_{split}_{prompt_format}.jsonl"), "w"
    )
    for prob_id, (input, output) in split_problems.items():
        if input.startswith("Question: "):
            input = input.replace("Question: ", "")
