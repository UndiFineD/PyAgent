# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mahilo\mahilo\integrations\pydanticai\agent.py
...
class PydanticAIAgent(BaseAgent):
    ...
    async def process_chat_message(
        ...
        return {"response": response_text, "activated_agents": activated_agents}
        ...
        for envelope in pending_messages:
            try:
                ...
                result = await self._pydantic_agent.run(
                    message_full, deps=self._dependencies
                )
                response_text = str(result.data)
                ...
                print(
                    f"In process_queue_message: Response for {self.name}: {response_text}"
                )
