# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mahilo\mahilo\server.py
import asyncio
import os
import uuid
...
class ServerManager:
    ...
    def _setup_routes(self):
        ...
        async def get_traces(
            ...
            return self.agent_manager.telemetry.get_traces(
        ...
        async def get_messages(
            ...
            start_time: Optional[float] = Query(
                None, description="Start timestamp in Unix epoch format"
            ),
            end_time: Optional[float] = Query(
                None, description="End timestamp in Unix epoch format"
            ),
        ...
        async def voice_stream_endpoint(websocket: WebSocket, agent_name: str):
            ...
            if not agent:
                self.console.print(
                    f"[bold red]⛔  Agent not found:[/bold red] [green]{agent_name}[/green]"
                )
                await websocket.send_text(
                    f"Error: Agent '{agent_name}' is not registered with the server"
                )
                await websocket.close(1008)  # Using 1008 (Policy Violation) status code
                return

            self.console.print(
                f"[bold blue]🎙️ New voice stream connection[/bold blue] for agent: [green]{agent_name}[/green]"
            )

            if not self.key:
                await websocket.send_text(
                    "OpenAI credentials not configured. Voice streaming is unavailable."
                )
                await websocket.close(1008)  # Using 1008 (Policy Violation) status code
                return
            ...
            try:
                ...
                try:
                    ...
                    self.console.print(
                        f"[bold red]Error with OpenAI WS connection: {e}[/bold red]"
                    )
                    await asyncio.sleep(1)

            except WebSocketDisconnect:
                self.console.print(
                    f"[bold yellow]⚠️  WebSocket disconnected[/bold yellow] for agent: [green]{agent_name}[/green]"
                )
                ...
                self.console.print(
                    f"[bold red]⛔  Error in voice stream:[/bold red] {str(e)}",
                    style="red",
                )
        ...
        async def websocket_endpoint(websocket: WebSocket, agent_name: str):
            ...
            self.console.print(
                f"[bold blue]🔌 New WebSocket connection[/bold blue] for agent: [green]{agent_name}[/green]"
            )
            ...
            try:
                ...
                while True:
                    ...
                    self.console.print(
                        f"[dim blue]📨 Received message for agent:[/dim blue] [green]{agent_name}[/green]: [dim]{data}[/dim]"
                    )
                    # if the agent is not active, ignore the message
                    if not agent.is_active():
                        self.console.print(
                            f"[bold yellow]⚠️  Agent[/bold yellow] [green]{agent_name}[/green] [bold yellow]is not active[/bold yellow]"
                        )
                        await websocket.send_text(f"Agent {agent_name} is not active.")
                        continue
                    response = await agent.process_chat_message(
                        data, websockets=[websocket]
                    )
                ...
                self.console.print(
                    f"[bold red]⛔  Error in websocket:[/bold red] {str(e)}",
                    style="red",
                )
        ...
        while True:
            for agent in self.agent_manager.get_all_agents():
                if agent.is_active():
                    ...
                    pending_messages = (
                        self.agent_manager.message_broker.get_pending_messages(
                            agent.name
                        )
                    )
                    if pending_messages:
