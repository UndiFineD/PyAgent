# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mahilo\mahilo\monitoring.py
import json
import logging
import time
...
class MahiloTelemetry:
    ...
    def __init__(self, service_name: str = "mahilo"):
        ...
        trace_provider.add_span_processor(
            SimpleSpanProcessor(InMemorySpanExporter(self.traces))
        )
        ...
        # Log initialization
        self.logger.info(
            f"MahiloTelemetry initialized with in-memory storage for service: {service_name}"
        )
    ...
    def _setup_metrics(self):
        ...
        self.message_counter = self.meter.create_counter(
            "mahilo.messages", description="Number of messages processed", unit="1"
        )
        ...
        self.message_retry_counter = self.meter.create_counter(
            "mahilo.message.retries", description="Number of message retries", unit="1"
        )
        ...
        self.active_agents = self.meter.create_up_down_counter(
            "mahilo.agents.active", description="Number of active agents", unit="1"
        )
    ...
    def record_event(
        ...
        with self.tracer.start_as_current_span(
            f"mahilo.event.{event_type.value}", attributes=attributes
        ) as span:
    ...
    def get_metrics(self, agent_id: Optional[str] = None) -> Dict:
        ...
        if agent_id:
            ...
            agent_traces = [
                t for t in self.traces if t["attributes"].get("agent_id") == agent_id
            ]
    ...
    def get_traces(
        ...
        return sorted(filtered_traces, key=lambda x: x["start_time"], reverse=True)[
            :limit
        ]
    ...
    def start_processing_span(self, message_id: str, agent_id: str) -> trace.Span:
        ...
        return self.tracer.start_span(
            ...
            attributes={"message_id": message_id, "agent_id": agent_id},
        )
