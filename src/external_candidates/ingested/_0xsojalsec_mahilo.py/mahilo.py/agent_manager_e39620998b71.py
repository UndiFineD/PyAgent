...
class AgentManager(AgentRegistry):
    ...
    def unregister_agent(self, agent_name: str) -> None:
        ...
        if agent_name in self.agents:
            self.telemetry.record_event(
                event_type=EventType.AGENT_UNREGISTERED, agent_id=agent_name
            )
    ...
    def get_policy_violations(
        self, limit: int = 100, policy_name: Optional[str] = None
    ...
    def enable_standard_policies(self, policy_names: List[str] = None) -> None:
        ...
        for name in policy_names:
            if name in standard_policies:
                ...
                print(
                    f"Warning: '{name}' is not a standard policy. Standard policies are: {standard_policies}"
                )
