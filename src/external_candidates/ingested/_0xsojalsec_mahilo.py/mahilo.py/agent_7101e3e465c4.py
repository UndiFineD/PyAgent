# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mahilo\mahilo\agent.py
import asyncio
import base64
import inspect
import json
...
class BaseAgent:
    ...
    def tools_for_realtime(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            console.print(
                "[bold red] ⚠️  Agent not registered with AgentManager:[/bold red]"
            )
            available_agents = {}
        TOOLS = [
            {
    ...
    def remove_tool(self, tool_name: str) -> Dict[str, Any]:
        ...
        return {"tool": removed_tool, "function": removed_function}
    ...
    def prompt_message(self) -> str:
        ...
        for agent_type, desc in available_agents.items():
            console.print(
                f"  [green]▪[/green] [cyan]{agent_type}:[/cyan] [dim]{desc}[/dim]"
            )
    ...
    async def process_chat_message(
        self, message: str = None, websockets: List[WebSocket] = []
    ) -> Dict[str, Any]:
        ...
        if self._telemetry:
            with self._telemetry.start_processing_span(
                ...
                try:
                    result = await self._process_chat_message_internal(
                        message, websockets
                    )
    ...
    async def _process_chat_message_internal(
        self, message: str = None, websockets: List[WebSocket] = []
    ) -> Dict[str, Any]:
        ...
        other_agent_messages = self._agent_manager.get_agent_messages(
            self.name, num_messages=7
        )
        ...
        response = await llm_config.chat_completion(
            ...
            tools=[
                ...
                if tool["function"]["name"] != "contact_human"
            ],
            tool_choice="auto",
        )
        ...
        while tool_calls:
            ...
            for tool_call in tool_calls:
                ...
                console.print(
                    f"[bold green] 🛠️  Function called:[/bold green] {function_name}"
                )
    ...
    async def process_queue_message(self, websockets: List[WebSocket] = []) -> None:
        ...
        if self._telemetry:
            with self._telemetry.start_processing_span(
                message_id=str(id(websockets)), agent_id=self.name
            ) as span:
                try:
    ...
    async def _process_queue_message_internal(
        self, websockets: List[WebSocket] = []
    ) -> None:
        """Internal method for processing queue messages."""
        # Get pending messages from broker
        pending_messages = self._agent_manager.message_broker.get_pending_messages(
            self.name
        )
            ...
            try:
                ...
                if self._agent_manager.message_broker.secret_key:
                    if not envelope.verify(
                        self._agent_manager.message_broker.secret_key
                    ):
                        print(
                            f"Warning: Message {envelope.message_id} failed signature verification"
                        )
                        continue
                ...
                current_messages.append(
                    {"content": self.prompt_message(), "role": "system"}
                )
                ...
                while tool_calls:
                    ...
                    for tool_call in tool_calls:
                        ...
                        console.print(
                            f"[bold blue] Function response:[/bold blue] {func_resp}"
                        )
    ...
    async def _send_session_update(self, openai_ws: WebSocketClientProtocol) -> None:
        ...
        session_update = {
            ...
            "session": {
                ...
                "tools": [
                    tool
                    for tool in self.tools_for_realtime
    ...
    async def _receive_from_client(
        self, websocket: WebSocket, openai_ws: WebSocketClientProtocol
    ) -> None:
        ...
        try:
            async for openai_message in openai_ws:
                ...
                if (
                    ...
                    console.print(
                        f"[bold blue] User input transcription:[/bold blue] {transcription}"
                    )
                ...
                if response["type"] == "response.audio_transcript.done":
                    ...
                    console.print(
                        f"[bold green] Assistant audio transcript:[/bold green] {transcript}"
                    )
    ...
    def deactivate(self) -> None:
        ...
        if self._telemetry:
            self._telemetry.record_event(
                event_type=EventType.AGENT_DEACTIVATED, agent_id=self.name
            )
        self._session = None

    async def contact_human(
        self, message: str, websockets: List[WebSocket] = []
    ) -> str:
        ...
        if self._voice_connections:
            for openai_ws in self._voice_connections:
                response_create = {
                    ...
                    "response": {
                        "modalities": ["audio"],
                        "instructions": "Please say the following message to the human: "
                        + message,
                        "voice": "ash",
                        "output_audio_format": "pcm16",
                        "tools": self.tools_for_realtime,
        ...
        if return_type not in valid_return_types and not (
            ...
            and (
                (
                    ...
                    and return_type.__args__[0] in (str, Dict, dict)
                )
                or (return_type.__origin__ is dict)
            )
        ):
            raise ToolFunctionError(
