    ...
    async def evaluate(
        ...
        if self.policy_type == PolicyType.HEURISTIC:
            if callable(self.policy_content):
                ...
                raise ValueError(
                    f"Heuristic policy {self.name} has non-callable content"
                )
            ...
            if model_name is None:
                ...
                model_name = os.getenv(
                    "MAHILO_POLICY_MODEL", os.getenv("MAHILO_LLM_MODEL", None)
                )
...
class PolicyManager:
    ...
    def __init__(
        self, validator_model_name: Optional[str] = None, verbose: bool = False
    ):
    ...
    async def evaluate_message(
        self, message: MessageEnvelope, context: Dict
        ...
        for policy in self.policies:
            ...
            try:
                passed, reason = await policy.evaluate(
                    message, context, self.model_name
                )
                if not passed:
...
def create_default_policies(
    validator_model_name: Optional[str] = None, verbose: bool = False
) -> List[Policy]:
    ...
    # Anti-Loop Policy
    def anti_loop_policy(
        message: MessageEnvelope, context: Dict
    ) -> Tuple[bool, Optional[str]]:
        ...
        if sender_messages:
            ...
            similarity = SequenceMatcher(
                None, current_message, last_sender_message
            ).ratio()
