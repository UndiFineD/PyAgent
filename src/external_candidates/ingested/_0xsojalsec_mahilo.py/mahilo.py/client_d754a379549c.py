...
class Client:
    def __init__(self, url: str, agent_name: Optional[str] = None, voice: bool = False):
        ...
        if self.voice:
            ...
            try:
                ...
                raise RuntimeError(
                    "No audio input device found. Check your microphone configuration."
                )
    ...
    async def connect(self):
        if self.voice:
            ...
            websocket_url = (
                f"ws://{self.base_url.split('://')[-1]}/ws/{self.agent_name or 'main'}"
            )
    ...
    async def _listen(self):
        try:
            while True:
                ...
                if self.voice:
                    ...
                    if data["event"] == "media":
                        ...
                        rich.print(
                            f"[bold magenta]{self.agent_name or 'Agent'}:[/bold magenta] {message}"
                        )
                else:
                    rich.print(
                        f"[bold magenta]{self.agent_name or 'Agent'}:[/bold magenta] {message}"
            ...
            rich.print(
                f"[bold red]⚠️  Connection to {self.agent_name} closed[/bold red]"
            )
