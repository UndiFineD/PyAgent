...
from socket import AF_UNSPEC, AI_CANONNAME, IPPROTO_IP, SOCK_DGRAM, getaddrinfo
    ...
    try:
        ...
        if not (dns_server or dns_tcp):
            for res in getaddrinfo(
                target, None, AF_UNSPEC, SOCK_DGRAM, IPPROTO_IP, AI_CANONNAME
            ):
                af, _, _, canonname, sa = res
                address_info[af.name] = sa[0]

            if (
                address_info["AF_INET6"]
                and ip_address(address_info["AF_INET6"]).is_link_local
            ):
            ...
            try:
                answers_ipv4 = dnsresolver.resolve(
                    target, rdatatype.A, raise_on_no_answer=False, tcp=dns_tcp
                )
            ...
            try:
                answers_ipv6 = dnsresolver.resolve(
                    target, rdatatype.AAAA, raise_on_no_answer=False, tcp=dns_tcp
                )
        ...
        if strBinding["wTowerId"] == 7:
            ...
            if binding.upper().find(iInterface.get_target().upper()) >= 0:
                ...
                and binding.upper().find(
                    iInterface.get_target().upper().partition(".")[0]
                )
                >= 0
            ):
...
class connection:
    def __init__(self, args, db, target):
        ...
        self.kerberos = bool(
            self.args.kerberos
            or self.args.use_kcache
            or self.args.aesKey
            or (hasattr(self.args, "delegate") and self.args.delegate)
        )
        ...
        if dns_result:
            self.host, self.is_ipv6, self.is_link_local_ipv6 = (
                dns_result["host"],
                dns_result["is_ipv6"],
                dns_result["is_link_local_ipv6"],
            )
        else:
            return
        ...
        self.logger.info(
            f"Socket info: host={self.host}, hostname={self.hostname}, kerberos={self.kerberos}, ipv6={self.is_ipv6}, link-local ipv6={self.is_link_local_ipv6}"
        )
        ...
        try:
            ...
            if "ERROR_DEPENDENT_SERVICES_RUNNING" in str(e):
                self.logger.error(
                    f"Exception while calling proto_flow() on target {target}: {e}"
                )
    ...
    def proto_flow(self):
        ...
        if not self.create_conn_obj():
            self.logger.info(
                f"Failed to create connection object for target {self.host}, exiting..."
            )
        else:
    ...
    def over_fail_limit(self, username):
        ...
        if (
            ...
            and self.args.ufail_limit == user_failed_logins[username]
        ):
            return True
    ...
    def query_db_creds(self):
        ...
        creds = (
            []
        )  # list of tuples (cred_id, domain, username, secret, cred_type, pillaged_from) coming from the database
        ...
        for cred in creds:
            ...
            owned.append(
                False
            )  # As these are likely valid we still want to test them if they are specified in the command line
        ...
        for password in self.args.password:
            if isfile(password):
                try:
                    with open(
                        password,
                        errors=("ignore" if self.args.ignore_pw_decoding else "strict"),
                    ) as password_file:
                    ...
                    self.logger.error(
                        f"{type(e).__name__}: Could not decode password file. Make sure the file only contains UTF-8 characters."
                    )
                    self.logger.error(
                        "You can ignore non UTF-8 characters with the option '--ignore-pw-decoding'"
                    )
                    sys.exit(1)
            else:
            ...
            for ntlm_hash in self.args.hash:
                if isfile(ntlm_hash):
                    with open(ntlm_hash) as ntlm_hash_file:
                        for i, line in enumerate(ntlm_hash_file):
                            ...
                            if len(line) != 32 and len(line) != 65:
                                self.logger.fail(
                                    f"Invalid NTLM hash length on line {(i + 1)} (len {len(line)}): {line}"
                                )
                                continue
                            else:
                                secret.append(line)
                    ...
                    if len(ntlm_hash) != 32 and len(ntlm_hash) != 65:
                        self.logger.fail(
                            f"Invalid NTLM hash length {len(ntlm_hash)}, authentication not sent"
                        )
                        exit(1)
                    else:
    ...
    def try_credentials(self, domain, username, owned, secret, cred_type, data=None):
        ...
        if self.args.jitter:
            ...
            value = (
                ...
                else random.choice(range(jitter[0], jitter[1]))
            )
        ...
        with sem:
            if cred_type == "plaintext":
                if self.kerberos:
                    ...
                    return self.kerberos_login(
                        domain, username, secret, "", "", self.kdcHost, False
                    )
                elif hasattr(
                    self.args, "domain"
                ):  # Some protocols don't use domain for login
                    self.logger.debug(
                        "Trying to authenticate using plaintext with domain"
                    )
    ...
    def login(self):
        ...
        owned = (
            []
        )  # Determines whether we have found a valid credential for this user. Default: False
        ...
        if self.args.cred_id:
            db_domain, db_username, db_owned, db_secret, db_cred_type, db_data = (
                self.query_db_creds()
            )
        ...
        if self.args.pfx_cert or self.args.pfx_base64 or self.args.pem_cert:
            ...
            if not self.args.username:
                self.logger.fail(
                    "You must specify a username when using certificate authentication"
                )
                return False
            with sem:
        ...
        if hasattr(self.args, "laps") and self.args.laps:
            ...
            username[0], secret[0], domain[0] = laps_search(
                self, username, secret, cred_type, domain, self.dns_server
            )
            cred_type = ["plaintext"]
        ...
        if not self.args.no_bruteforce:
            for secr_index, secr in enumerate(secret):
                for user_index, user in enumerate(username):
                    if self.try_credentials(
                        domain[user_index],
                        user,
                        owned[user_index],
                        secr,
                        cred_type[secr_index],
                        data[secr_index],
                    ):
            ...
            if len(username) != len(secret):
                self.logger.error(
                    "Number provided of usernames and passwords/hashes do not match!"
                )
                return False
            for user_index, user in enumerate(username):
                if (
                    self.try_credentials(
                        domain[user_index],
                        user,
                        owned[user_index],
                        secret[user_index],
                        cred_type[user_index],
                        data[user_index],
                    )
                    and not self.args.continue_on_success
                ):
                    owned[user_index] = True
                    if not self.args.continue_on_success:
                        return True
