...
class NXCModule:
    ...
    def on_admin_login(self, context, connection):
        ...
        if "success" in p:
            context.log.success(
                f"NTDS.dit dumped to {self.tmp_dir}{self.dump_location}"
            )
        else:
        ...
        try:
            ...
            p = connection.execute(command, True)
            context.log.success(
                f"Deleted {self.tmp_dir}{self.dump_location} remote dump directory"
            )
        except Exception as e:
            context.log.fail(
                f"Error deleting {self.dump_location} remote directory on share {self.share}: {e}"
            )
        ...
        try:
            ...
            context.log.display(
                f"grep -iv disabled {connection.output_filename}.ntds | cut -d ':' -f1"
            )
        except Exception as e:
            context.log.fail(e)
        ...
        if self.no_delete:
            context.log.display(
                f"Raw NTDS dump copied to {self.dir_result}, parse it with:"
            )
            context.log.display(
