...
class NXCModule:
    ...
    def on_login(self, context, connection):
        ...
        if search_result is None:
            context.log.success(
                'Unable to find any members of the "' + self.GROUP + '" group'
            )
            return True
        ...
        distinguished_name = (
            do_search(self, context, connection, search_filter, attribute)
        ).decode("utf-8")
        ...
        if len(self.answers) > 0:
            context.log.success(
                "Found the following members of the " + self.GROUP + " group:"
            )
...
def do_search(self, context, connection, searchFilter, attributeName):
    try:
        ...
        resp = connection.ldap_connection.search(
            searchFilter=searchFilter, attributes=[attributeName], sizeLimit=0
        )
