    ...
    def on_admin_login(self, context, connection):
        ...
        try:
            ...
            if "SCHED_S_TASK_HAS_NOT_RUN" in str(e):
                self.logger.fail(
                    "Task was not run, seems like the specified user has no active session on the target"
                )
...
"""
        if self.__retOutput:
            fileLocation = (
                "\\Windows\\Temp\\" if self.location is None else self.location
            )
            if self.file is None:
                self.__output_filename = os.path.join(
                    fileLocation, gen_random_string(6)
                )
            else:
    ...
    def execute_handler(self, command, fileless=False):
        ...
        try:
            ...
            tsch.hSchRpcRegisterTask(
                dce, f"\\{self.task}", xml, tsch.TASK_CREATE, NULL, tsch.TASK_LOGON_NONE
            )
        except Exception as e:
            if "ERROR_NONE_MAPPED" in str(e):
                self.logger.fail(
                    f"User {self.user} is not connected on the target, cannot run the task"
                )
                with contextlib.suppress(Exception):
                    tsch.hSchRpcDelete(dce, f"\\{self.task}")
        ...
        if self.__retOutput:
            if fileless:
                while True:
                    try:
                        with open(
                            os.path.join(TMP_PATH, self.__output_filename)
                        ) as output:
                ...
                while True:
                    try:
                        ...
                        if "STATUS_BAD_NETWORK_NAME" in str(e):
                            ...
                            break
                        if "SHARING" in str(e) or "STATUS_OBJECT_NAME_NOT_FOUND" in str(
                            e
                        ):
                            sleep(3)
                            tries += 1
