...
class SMBSpiderPlus:
    ...
    def reconnect(self):
        ...
        for i in range(1, self.max_connection_attempts + 1):
            self.logger.display(
                f"Reconnection attempt #{i}/{self.max_connection_attempts} to server."
            )
    ...
    def list_path(self, share, subfolder):
        ...
        try:
            ...
            self.logger.debug(
                f'Failed listing files on share "{share}" in folder "{subfolder}": {e!s}'
            )
        ...
        try:
            ...
            for share in shares:
                ...
                try:
                    ...
                    self.logger.exception(e)
                    self.logger.fail(
                        f"Got a session or NetBIOSTimeout error while spidering share: {share_name}"
                    )
                    self.reconnect()
    ...
    def spider_folder(self, share_name, folder):
        ...
        for result in filelist:
            ...
            if any(d in next_filedir.lower() for d in self.exclude_filter):
                self.logger.info(
                    f'The {result_type} "{next_filedir}" has been excluded'
                )
            ...
            if result_type == "folder":
                self.logger.info(
                    f'Current folder in share "{share_name}": "{next_fullpath}"'
                )
                self.spider_folder(share_name, next_fullpath + "/")
            else:
                self.logger.info(
                    f'Current file in share "{share_name}": "{next_fullpath}"'
                )
    ...
    def parse_file(self, share_name, file_path, file_info):
        ...
        if exists(download_path):
            if (
                file_modified_time <= stat(download_path).st_mtime
                and getsize(download_path) == file_size
            ):
                self.logger.info(
                    f'File already downloaded "{file_path}" => "{download_path}".'
                )
    ...
    def save_file(self, remote_file, share_name):
        ...
        try:
            ...
            self.logger.fail(
                f'Error writing file "{download_path}" from share "{share_name}": {e}'
            )
    ...
    def print_stats(self):
        ...
        if shares_readable:
            ...
            shares_readable_str = (
                ", ".join(shares_readable[:10]) + "..."
                if len(shares_readable) > 10
                else ", ".join(shares_readable)
            )
            self.logger.display(
                f"SMB Readable Shares:  {num_readable_shares} ({shares_readable_str})"
            )
        shares_writable = self.stats.get("shares_writable", [])
        if shares_writable:
            num_writable_shares = len(shares_writable)
            shares_writable_str = (
                ", ".join(shares_writable[:10]) + "..."
                if len(shares_writable) > 10
                else ", ".join(shares_writable)
            )
            self.logger.display(
                f"SMB Writable Shares:  {num_writable_shares} ({shares_writable_str})"
            )
        ...
        if file_sizes:
            ...
            self.logger.display(
                f"File size average:    {human_size(average_file_size)}"
            )
        ...
        if file_exts:
            ...
            self.logger.display(
                f"File unique exts:     {num_unique_file_exts} ({unique_exts_str})"
            )
...
class NXCModule:
    ...
    def options(self, context, module_options):
        ...
        self.exclude_exts = [
            d.lower() for d in self.exclude_exts
        ]  # force case-insensitive
        self.exclude_filter = get_list_from_option(
            module_options.get("EXCLUDE_FILTER", "print$,ipc$")
        )
        self.exclude_filter = [
            d.lower() for d in self.exclude_filter
        ]  # force case-insensitive
        self.max_file_size = int(module_options.get("MAX_FILE_SIZE", 50 * 1024))
        self.output_folder = module_options.get(
            "OUTPUT_FOLDER", abspath(join(NXC_PATH, "modules/nxc_spider_plus"))
        )
