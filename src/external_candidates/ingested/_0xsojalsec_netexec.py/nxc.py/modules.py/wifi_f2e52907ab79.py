...
class NXCModule:
    ...
    def on_admin_login(self, context, connection):
        ...
        masterkeys = collect_masterkeys_from_target(
            connection, target, conn, user=False
        )
        ...
        context.log.success(
            f"Got {highlight(len(masterkeys))} decrypted masterkeys. Looting Wifi interfaces"
        )
            ...
            if wifi_cred.auth.upper() == "OPEN":
                ...
                try:
                    context.log.highlight(
                        f"[{wifi_cred.auth.upper()}] {wifi_cred.ssid} - Passphrase: {wifi_cred.password.decode('latin-1')}"
                    )
                except Exception:
                    context.log.highlight(
                ...
                try:
                    if self.eap_username is not None and self.eap_password is not None:
                        context.log.highlight(
                            f"[{wifi_cred.auth.upper()}] {wifi_cred.ssid} - {wifi_cred.eap_type} - Identifier: {wifi_cred.eap_username}:{wifi_cred.eap_password}"
                        )
                    else:
                        context.log.highlight(
                            f"[{wifi_cred.auth.upper()}] {wifi_cred.ssid} - {wifi_cred.eap_type}"
                        )
                except Exception:
                    context.log.highlight(
                        f"[{wifi_cred.auth.upper()}] {wifi_cred.ssid} - Passphrase: {wifi_cred.password}"
                ...
                context.log.highlight(
                    f"[WPA-EAP] {wifi_cred.ssid} - {wifi_cred.eap_type}"
                )
