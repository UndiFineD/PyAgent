...
from nxc.helpers.msada_guids import EXTENDED_RIGHTS, SCHEMA_OBJECTS
...
class NXCModule:
    ...
    def options(self, context, module_options):
        ...
        if module_options and "TARGET" in module_options:
            ...
            if re.search(r"^(.+)\/([^\/]+)$", module_options["TARGET"]) is not None:
                ...
                context.log.debug(
                    f"Setting target_sAMAccountName to {module_options['TARGET']}"
                )
    ...
    def on_login(self, context, connection):
        ...
        if self.principal_sAMAccountName is not None:
            ...
            try:
                ...
                context.log.highlight(
                    f"Found principal SID to filter on: {self.principal_sid}"
                )
            except Exception:
                context.log.fail(
                    f"Principal SID not found in LDAP ({_lookedup_principal})"
                )
                sys.exit(1)
        ...
        if (self.target_sAMAccountName or self.target_DN) and self.target_file is None:
            ...
            try:
                ...
                context.log.highlight(
                    f"Target principal found in LDAP ({self.target_principal[0]})"
                )
            except Exception:
                context.log.fail(
                    f"Target SID not found in LDAP ({self.target_sAMAccountName})"
                )
                sys.exit(1)
        ...
        # If there are multiple targets
            ...
            for target in targets:
                try:
                    ...
                    context.log.highlight(
                        f"Target principal found in LDAP ({self.target_sAMAccountName})"
                    )
                except Exception:
                    context.log.fail(
                        f"Target SID not found in LDAP ({self.target_sAMAccountName})"
    ...
    def search_target_principal_security_descriptor(self, context, connection):
        ...
        try:
            ...
            context.log.fail(
                f"Principal not found in LDAP ({_lookedup_principal}), probably an LDAP session issue."
            )
            sys.exit(0)
    ...
    def parse_perms(self, access_mask):
        perms = [
            ...
            if (access_mask & PERM.value) == PERM.value
        ]
        ...
        if ace["TypeName"] in [
            ...
            if ace["TypeName"] in ["ACCESS_ALLOWED_ACE", "ACCESS_DENIED_ACE"]:
                ...
                if ace["Ace"]["ObjectTypeLen"] != 0:
                    ...
                    try:
                        parsed_ace["Object type (GUID)"] = (
                            f"{OBJECT_TYPES_GUID[obj_type]} ({obj_type})"
                        )
                    except KeyError:
                ...
                if ace["Ace"]["InheritedObjectTypeLen"] != 0:
                    ...
                    try:
                        parsed_ace["Inherited type (GUID)"] = (
                            f"{OBJECT_TYPES_GUID[inh_obj_type]} ({inh_obj_type})"
                        )
                    except KeyError:
                        parsed_ace["Inherited type (GUID)"] = (
                            f"UNKNOWN ({inh_obj_type})"
                        )
            ...
            context.log.debug(
                f"ACE Type ({ace['TypeName']}) unsupported for parsing yet, feel free to contribute"
            )
            _ace_flags = [FLAG.name for FLAG in ACE_FLAGS if ace.hasFlag(FLAG.value)]
            parsed_ace = {
                "ACE type": ace["TypeName"],
    ...
    def print_parsed_dacl(self, context, parsed_dacl):
        ...
        for parsed_ace in parsed_dacl:
            print_ace = True
            context.log.debug(
                f"{parsed_ace=}, {self.rights=}, {self.rights_guid=}, {self.ace_type=}, {self.principal_sid=}"
            )
            ...
            if self.rights is not None:
                try:
                    ...
                    context.log.debug(
                        f"Error filtering with {parsed_ace=} and {self.rights=}, probably because of ACE type unsupported for parsing yet ({e})"
                    )
            ...
            if self.rights_guid is not None:
                try:
                    ...
                    context.log.debug(
                        f"Error filtering with {parsed_ace=} and {self.rights_guid=}, probably because of ACE type unsupported for parsing yet ({e})"
                    )
            ...
            if self.ace_type == "allowed":
                try:
                    ...
                    context.log.debug(
                        f"Error filtering with {parsed_ace=} and {self.ace_type=}, probably because of ACE type unsupported for parsing yet ({e})"
                    )
            else:
                try:
            ...
            if self.principal_sid is not None:
                try:
                    ...
                    context.log.debug(
                        f"Error filtering with {parsed_ace=} and {self.principal_sid=}, probably because of ACE type unsupported for parsing yet ({e})"
                    )
            if print_ace:
