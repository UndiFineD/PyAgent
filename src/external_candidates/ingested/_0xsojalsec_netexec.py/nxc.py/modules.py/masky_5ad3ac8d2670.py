...
class NXCModule:
    ...
    def on_admin_login(self, context, connection):
        if not self.ca:
            context.log.fail(
                r"Please provide a valid CA server and CA name (CA_SERVER\CA_NAME)"
            )
            return False
    ...
    def process_errors(self, context, tracker):
        ...
        if not tracker.files_cleaning_success:
            ...
            context.log.fail(
                f"Please remove the files named '{tracker.agent_filename}', '{tracker.error_filename}', '{tracker.output_filename}' & '{tracker.args_filename}' within the folder '\\Windows\\Temp\\'"
            )
            ret = False
