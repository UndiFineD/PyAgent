...
class NXCModule:
    ...
    def on_login(self, context, connection):
        ...
        searchFilter = (
            ...
            else "(&(objectCategory=computer)(|(msLAPS-EncryptedPassword=*)(ms-MCS-AdmPwd=*)(msLAPS-Password=*)))"
        )
        attributes = [
        ...
        if len(results) != 0:
            ...
            for computer in results:
                ...
                if "mslaps-encryptedpassword" in values:
                    ...
                    laps_computers.append(
                        (str(values["samaccountname"]), r["n"], str(r["p"]))
                    )
                    ...
                    laps_computers.append(
                        (
                            str(values["samaccountname"]),
                            "",
                            str(values["ms-mcs-admpwd"]),
                        )
                    )
                else:
                    context.log.fail(
            ...
            for sAMAccountName, user, password in laps_computers:
                context.log.highlight(
                    f"Computer:{sAMAccountName} User:{user:<15} Password:{password}"
                )
        else:
            context.log.fail(
                "No result found with attribute ms-MCS-AdmPwd or msLAPS-Password !"
