...
class NXCModule:
    ...
    def options(self, context, module_options):
        ...
        if "CHANGEPW" in module_options and (
            "NAME" not in module_options or "PASSWORD" not in module_options
        ):
    ...
    def do_samr_add(self, context):
        ...
        string_binding = epm.hept_map(
            self.__host, samr.MSRPC_UUID_SAMR, protocol="ncacn_np"
        )
        string_binding = (
            string_binding.replace(self.__host, self.__kdcHost)
        ...
        if len(domains_without_builtin) > 1:
            domain = list(
                filter(lambda x: x["Name"].lower() == self.__domainNetbios, domains)
            )
            if len(domain) != 1:
        ...
        if self.__noAdd or self.__delete:
            ...
            try:
                ...
                if e.error_code == 0xC0000022:
                    context.log.highlight(
                        f"{self.__username + ' does not have the right to ' + message + ' ' + self.__computerName}"
                    )
                    self.noLDAPRequired = True
                context.log.exception(e)
        else:
            if self.__computerName is not None:
                try:
                    ...
                    context.log.highlight(
                        "{}".format(
                            'Computer account already exists with the name: "'
                            + self.__computerName
                            + '"'
                        )
                    )
            ...
            try:
                ...
                self.noLDAPRequired = True
                context.log.highlight(
                    'Successfully added the machine account: "'
                    + self.__computerName
                    + '" with Password: "'
                ...
                if e.error_code == 0xC0000022:
                    context.log.highlight(
                        "{}".format(
                            'The following user does not have the right to create a computer account: "'
                            + self.__username
                            + '"'
                        )
                    )
                    ...
                    context.log.highlight(
                        "{}".format(
                            'The following user exceeded their machine account quota: "'
                            + self.__username
                            + '"'
                        )
                    )
        ...
        if self.__delete:
            ...
            if self.__noAdd:
                context.log.highlight(
                    "{}".format(
                        'Successfully set the password of machine "'
                        + self.__computerName
                        + '" with password "'
                        + self.__computerPassword
                        + '"'
    ...
    def do_ldaps_add(self, connection, context):
        ...
        ldap_server = ldap3.Server(
            connection.host, use_ssl=True, port=636, get_info=ldap3.ALL, tls=tls
        )
        c = ldap3.Connection(
            ldap_server,
            f"{connection.username}@{connection.domain}",
            connection.password,
        )
        c.bind()
        ...
        if self.__delete:
            ...
            if result:
                context.log.highlight(
                    f'Successfully deleted the "{self.__computerName}" Computer account'
                )
            elif result is False and c.last_error == "noSuchObject":
                context.log.highlight(
                    f'Computer named "{self.__computerName}" was not found'
                )
            elif result is False and c.last_error == "insufficientAccessRights":
                context.log.highlight(
                    f'Insufficient Access Rights to delete the Computer "{self.__computerName}"'
                )
            else:
                context.log.highlight(
            ...
            if result:
                ...
                context.log.highlight(
                    "You can try to verify this with the nxc command:"
                )
                context.log.highlight(
                    f"nxc ldap {connection.host} -u {connection.username} -p {connection.password} -M group-mem -o GROUP='Domain Computers'"
                )
            elif result is False and c.last_error == "entryAlreadyExists":
                context.log.highlight(
                    f"The Computer account '{self.__computerName}' already exists"
                )
            elif not result:
                context.log.highlight(
                    f"Unable to add the '{self.__computerName}' Computer account. The error was: {c.last_error}"
                )
            c.unbind()
