    ...
    def getSAMRResetInfo(self, context):
        ...
        try:
            ...
            resp = samr.hSamrLookupDomainInSamServer(
                dce, server_handle, domains[0]["Name"]
            )

            # obtain domain handle for samr connection
            resp = samr.hSamrOpenDomain(
                dce, serverHandle=server_handle, domainId=resp["DomainId"]
            )
            ...
            while status == STATUS_MORE_ENTRIES:
                ...
                for user in resp["Buffer"]["Buffer"]:
                    try:
                        context.log.info(
                            f"Querying security questions for User: {user['Name']}"
                        )
                        # request SAMR ID 30
                        ...
                        r = samr.hSamrOpenUser(
                            dce, domain_handle, samr.MAXIMUM_ALLOWED, user["RelativeId"]
                        )
                        info = samr.hSamrQueryInformationUser2(
                            dce,
                            r["UserHandle"],
                            samr.USER_INFORMATION_CLASS.UserResetInformation,
                        )
                        ...
                        if len(questions) == 0:
                            context.log.highlight(
                                f"User {user['Name']} has no security questions"
                            )
                        else:
                        ...
                        if "STATUS_INVALID_INFO_CLASS" in str(e):
                            context.log.debug(
                                f"Failed to query security questions for User: {user['Name']}: {e!s}"
                            )
                            continue
                        else:
                            context.log.fail(
