...
class BitLockerSMB:
    ...
    def check_bitlocker_status(self):
        ...
        try:
            ...
            check_bitlocker_command_str_output = self.connection.execute(
                check_bitlocker_command_str, True
            )

            if (
                "'Get-BitLockerVolume' is not recognized"
                in check_bitlocker_command_str_output
            ):
            ...
            for line in data_lines:
                ...
                if line[1] == ":":
                    parts = line.split()
                    MountPoint, EncryptionMethod, protection_status = (
                        parts[0],
                        parts[1],
                        parts[2],
                    )
                    ...
                    if protection_status == "On":
                        self.context.log.highlight(
                            f"BitLocker is enabled on drive {MountPoint} (Encryption Method: {EncryptionMethod})"
                        )
                    else:
                        self.context.log.highlight(
                            f"BitLocker is disabled on drive {MountPoint}"
                        )
    ...
    def check_bitlocker_status(self):
        try:
            ...
            try:
                ...
                i_interface = dcom_conn.CoCreateInstanceEx(
                ...
                try:
                    while True:
                        ...
                        if iWbemClassObject[0].ProtectionStatus == 1:
                            self.context.log.highlight(
                                f"BitLocker is enabled on drive {iWbemClassObject[0].DriveLetter} (Encryption Method: {encryptionTypeMapping.get(encryptionMethod, 'Unknown')})"
                            )
                        else:
                            if encryptionMethod == 0:  # Should be 0 if disabled
                                self.context.log.highlight(
                                    f"BitLocker is disabled on drive {iWbemClassObject[0].DriveLetter}"
                                )
                except Exception:
