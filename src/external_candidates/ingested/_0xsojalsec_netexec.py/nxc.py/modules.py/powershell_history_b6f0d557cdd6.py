...
class NXCModule:
    ...
    def on_admin_login(self, context, connection):
        for directory in connection.conn.listPath("C$", "Users\\*"):
            if (
                directory.get_longname() not in self.false_positive
                ...
                try:
                    ...
                    for file in connection.conn.listPath(
                        "C$", f"{powershell_history_dir}\\*"
                    ):
                        if file.get_longname() not in self.false_positive:
                            ...
                            if self.export:
                                ...
                                export_path = join(
                                    NXC_PATH, "modules", "powershell_history"
                                )
                                ...
                                try:
                                    ...
                                    context.log.highlight(
                                        f"PowerShell history written to: {path}"
                                    )
                                except Exception as e:
                                    context.log.fail(
                                        f"Failed to write history to {filename}: {e}"
                                    )
                except Exception:
                    pass
