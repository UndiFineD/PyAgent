    ...
    def wdigest_enable(self, context, smbconnection):
        ...
        if remote_ops._RemoteOperations__rrp:
            ...
            rtype, data = rrp.hBaseRegQueryValue(
                remote_ops._RemoteOperations__rrp, key_handle, "UseLogonCredential\x00"
            )
        ...
        if remote_ops._RemoteOperations__rrp:
            ...
            try:
                ...
                if int(data) == 1:
                    ...
                    context.log.fail(
                        f"Unexpected registry value for UseLogonCredential: {data}"
                    )
            except DCERPCException as d:
                if (
                    "winreg.HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\SecurityProviders\\WDigest"
                    in str(d)
                ):
                    context.log.fail(
                        "UseLogonCredential registry key is disabled (registry key not found)"
                    )
                else:
