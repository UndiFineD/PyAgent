    ...
    def on_login(self, context, connection):
        ...
        try:
            ...
            if e.getErrorCode() == nt_errors.STATUS_INVALID_PARAMETER:
                context.log.info(
                    "Target is not vulnerable to CVE-2019-1040 (authentication was rejected)"
                )
            else:
                context.log.info("Unexpected Exception while authentication")
        else:
            context.log.highlight(
                "Potentially vulnerable to CVE-2019-1040, next step: https://dirkjanm.io/exploiting-CVE-2019-1040-relay-vulnerabilities-for-rce-and-domain-admin/"
            )


class Modify_Func:
    # Slightly modified version of impackets computeResponseNTLMv2
    def mod_computeResponseNTLMv2(
        flags,
        serverChallenge,
        clientChallenge,
        serverName,
        ...
        av_pairs[ntlm.NTLMSSP_AV_TARGET_NAME] = (
            "cifs/".encode("utf-16le") + av_pairs[ntlm.NTLMSSP_AV_HOSTNAME][1]
        )
        ...
        lmChallengeResponse = (
            ntlm.hmac_md5(responseKeyNT, serverChallenge + clientChallenge)
            + clientChallenge
        )
    ...
    def mod_getNTLMSSPType3(
        ...
        if (
            ntlmChallenge["flags"] & ntlm.NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY
        ) == 0:
            # No extended session security, taking it out
            responseFlags &= (
                0xFFFFFFFF ^ ntlm.NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY
            )
