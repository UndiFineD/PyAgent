...
class NXCModule:
    ...
    def __init__(self):
        ...
        self.sccm_sites = (
            {}
        )  # List of SCCM sites with their management points (Sorted by site code)
        self.base_dn = ""
    ...
    def on_login(self, context, connection):
        ...
        self.base_dn = (
            connection.ldap_connection._baseDN if not self.base_dn else self.base_dn
        )
        ...
        try:
            ...
            search_filter = (
                f"(distinguishedName=CN=System Management,CN=System,{self.base_dn})"
            )
            controls = security_descriptor_control(sdflags=0x04)
            context.log.display(
                f"Looking for the SCCM container with filter: '{search_filter}'"
            )
            ...
            for item in result:
                if isinstance(item, ldapasn1_impacket.SearchResultEntry):
                    ...
                    self.context.log.success(
                        f"Found {len(self.sccm_site_servers)} Site Servers:"
                    )
                    for site in self.sccm_site_servers:
                        ip = self.connection.resolver(site)
                        self.context.log.highlight(
                            f"{site} - {ip['host'] if ip else 'unknown'}"
                        )
                    self.context.log.success(
                        f"Found {len(self.sccm_sites)} SCCM Sites:"
                    )
                    for site in self.sccm_sites:
                        ...
                        self.context.log.highlight(
                            f"  Assignment Site Code: {self.sccm_sites[site]['AssignmentSiteCode'].rjust(3)}"
                        )
                        ...
                        if self.sccm_sites[site]["ManagementPoints"]:
                            ...
                            for mp in self.sccm_sites[site]["ManagementPoints"]:
                                ...
                                self.context.log.highlight(
                                    f"    DNS Hostname:{' ':<2}{mp['dNSHostName']}"
                                )
                                self.context.log.highlight(
                                    f"    IP Address:{' ':<4}{mp['IPAddress']}"
                                )
                                self.context.log.highlight(
                                    f"    Default MP:{' ':<4}{mp['mSSMSDefaultMP']}"
                                )
                        else:
        ...
        if self.user_objects:
            context.log.success(
                f"Found {len(self.user_objects)} SCCM related user objects:"
            )
            for user in self.user_objects:
                context.log.highlight(user)
        if self.computer_objects:
            context.log.success(
                f"Found {len(self.computer_objects)} SCCM related computer objects:"
            )
            for computer in self.computer_objects:
                context.log.highlight(computer)
        if self.group_objects:
            context.log.success(
                f"Found {len(self.group_objects)} SCCM related group objects:"
            )
                ...
                for child in self.group_objects[group]["children"]:
                    if int(child["sAMAccountType"]) == SAM_USER_OBJECT:
                        ...
                        context.log.highlight(
                            f"  {child['sAMAccountName']} -> Computer"
                        )
    ...
    def get_sccm_named_objects(self, context, connection):
        ...
        try:
            ...
            for res in result:
                if (
                    "sAMAccountType" in res
                    and int(res["sAMAccountType"]) == SAM_USER_OBJECT
                ):
                    self.user_objects.append(res["sAMAccountName"])
                elif (
                    "sAMAccountType" in res
                    and int(res["sAMAccountType"]) == SAM_MACHINE_ACCOUNT
                ):
                    self.computer_objects.append(res["sAMAccountName"])
                elif (
                    "sAMAccountType" in res
                    and int(res["sAMAccountType"]) == SAM_GROUP_OBJECT
                ):
    ...
    def get_site_servers(self, item):
        ...
        principal_security_descriptor = ldaptypes.SR_SECURITY_DESCRIPTOR(
            data=raw_sec_descriptor
        )
        ...
        try:
            ...
            if not parsed_result:
                ...
                parsed_result = parsed_result[
                    0
                ]  # We only have one result as we always query a single SID

            if int(parsed_result["sAMAccountType"]) == SAM_MACHINE_ACCOUNT:
                self.context.log.debug(
                    f"Found object with full control over SCCM object. SID: {sid}, dns_hostname: {parsed_result['dNSHostName']}"
                )
