...
class NXCModule:
    ...
    name = "ldap-checker"
    description = (
        "Checks whether LDAP signing and channel binding are required and / or enforced"
    )
            ...
            try:
                ...
                with (
                    ...
                    ssl_context.wrap_socket(
                        sock, server_hostname=connection.host
                    ) as ssl_sock,
                ):
                ...
                if cert:
                    ...
                    context.log.debug(
                        f"Tampered certificate hash: {bytes(tampered_bytes).hex()}"
                    )
    ...
    async def run_ldap(self, context, target, credential):
        try:
            client = MSLDAPClientConnection(target, credential)
            client._disable_signing = (
                True  # deliberately disable LDAP signing on client connection
            )
            _, err = await client.connect()
            if err:
    ...
    def on_login(self, context, connection):
        ...
        anon_credential = NTLMCredential(
            secret="", username="", domain=connection.domain, stype=asyauthSecret.PASS
        )

        if not connection.username and not secret:
            context.log.highlight(
                "No credentials provided, skipping LDAP signing check"
            )
        ...
        if connection.username or secret:
            ...
            ldap_signing_status = asyncio.run(
                self.run_ldap(context, target, credential)
            )
        ...
        if self.does_ldaps_complete_handshake(context, connection.host):
            ...
            ldaps_noEPA = asyncio.run(
                self.run_ldaps_noEPA(context, connection, target, anon_credential)
            )
            ldaps_withEPA = asyncio.run(
                self.run_ldaps_withEPA(context, connection, target, anon_credential)
            )
            ...
            context.log.fail(
                f"{connection.domain} - TLS handshake failed; certificate likely not configured"
            )
