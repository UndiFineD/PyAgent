...
class NXCModule:
    ...
    def handle_confCons_file(self, file_content):
        ...
        for node_attribute in self.parse_xml_nodes(main):
            password = self.extract_remoteng_passwords(
                node_attribute["Password"], encryption_attributes
            )
            if password == b"":
                continue
            ...
            domain = (
                node_attribute["Domain"]
                if node_attribute["Domain"] != ""
                else node_attribute["Hostname"]
            )
            ...
            host = (
                ...
                if node_attribute["Hostname"] != ""
                else " "
            )
            self.context.log.highlight(
                f"{name}:{host} - {domain}\\{username}:{password}"
            )
    ...
    def dig_confCons_in_files(
        self, conn, directory_path, recurse_level=0, recurse_max=10
    ):
                    ...
                    if item.is_directory() > 0:
                        ...
                        if "confCons.xml" in item.get_longname():
                            self.context.log.info(
                                f"Found confCons.xml file: {new_path}"
                            )
                            content = conn.readFile(self.context.share, new_path)
                            self.handle_confCons_file(content)

    def extract_remoteng_passwords(
        self, encrypted_password, encryption_attributes: MRemoteNgEncryptionAttributes
    ):
