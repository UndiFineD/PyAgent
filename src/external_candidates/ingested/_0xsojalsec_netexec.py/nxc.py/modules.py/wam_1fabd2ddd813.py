...
class NXCModule:
    ...
    def on_admin_login(self, context, connection):
        ...
        context.log.success(
            f"Got {highlight(len(self.masterkeys))} decrypted masterkeys. Looting Token Broker Cache access tokens"
        )
            ...
            for attrib in token.attribs:
                if attrib["Key"].decode() == "WTRes_Token":
                    ...
                    for access_token in re.findall(
                        r"e[yw][A-Za-z0-9-_]+\.(?:e[yw][A-Za-z0-9-_]+)?\.[A-Za-z0-9-_]{2,}(?:(?:\.[A-Za-z0-9-_]{2,}){2})?",
                        attrib.__str__(),
                    ):
                        ...
                        if "preferred_username" in decoded_token:
                            ...
                            context.log.highlight(
                                f"[{token.winuser}] {decoded_token['preferred_username']}: {access_token}"
                            )
