...
class NXCModule:
    ...
    def options(self, context, module_options):
        ...
        if module_options["ACTION"].lower() not in [
            "enable",
            "disable",
            "enable-ram",
            "disable-ram",
                ...
                if "ram" in self.action:
                    ...
                    try:
                        ...
                        if "WBEM_E_NOT_FOUND" in str(e):
                            context.log.fail(
                                "System version under NT6 not support restricted admin mode"
                            )
                        else:
                    ...
                    try:
                        ...
                        if "WBEM_E_INVALID_NAMESPACE" in str(e):
                            context.log.fail(
                                "Looks like target system version is under NT6, please add 'OLD=true' in module options."
                            )
                        else:
                            context.log.fail(str(e))
...
class RdpSmb:
    ...
    def rdp_wrapper(self, action):
        ...
        if remote_ops._RemoteOperations__rrp:
            ...
            rtype, data = rrp.hBaseRegQueryValue(
                remote_ops._RemoteOperations__rrp, key_handle, "fDenyTSConnections"
            )
    ...
    def rdp_ram_wrapper(self, action):
        ...
        if remote_ops._RemoteOperations__rrp:
            ...
            rtype, data = rrp.hBaseRegQueryValue(
                remote_ops._RemoteOperations__rrp, key_handle, "DisableRestrictedAdmin"
            )

            if int(data) == 0:
                self.logger.success(
                    "Enable RDP Restricted Admin Mode via SMB(ncacn_np) succeed"
                )
            elif int(data) == 1:
                self.logger.success(
                    "Disable RDP Restricted Admin Mode via SMB(ncacn_np) succeed"
                )
    ...
    def query_rdp_port(self, remoteOps, regHandle):
        if remoteOps:
            ...
            rtype, data = rrp.hBaseRegQueryValue(
                remoteOps._RemoteOperations__rrp, key_handle, "PortNumber"
            )
    ...
    def firewall_cmd(self, action):
        ...
        if output:
            self.logger.success(
                f"{action.capitalize()} RDP firewall rules via cmd succeed"
            )
        else:
            self.logger.fail(
                f"{action.capitalize()} RDP firewall rules via cmd failed, maybe got detected by AV software."
            )
...
class RdpWmi:
    def __init__(self, context, connection, timeout):
        ...
        try:
            ...
            i_interface = self.__dcom.CoCreateInstanceEx(
                wmi.CLSID_WbemLevel1Login, wmi.IID_IWbemLevel1Login
            )
            if self.__currentprotocol == "smb":
                flag, self.__stringBinding = dcom_FirewallChecker(
                    i_interface, self.__remoteHost, self.__timeout
                )
            ...
            self.logger.fail(
                f'Unexpected wmi error: {e}, please try to use "-o" with "METHOD=smb"'
            )
    ...
    def rdp_wrapper(self, action, old=False):
        if old is False:
            ...
            if action == "enable":
                self.logger.info(
                    "Enabling RDP services (old system not support setting up firewall)"
                )
                i_wbem_class_object.SetAllowTSConnections(1)
            elif action == "disable":
                self.logger.info(
                    "Disabling RDP services (old system not support setting up firewall)"
                )
    ...
    def query_rdp_result(self, old=False):
        if old is False:
            ...
            if result == 0:
                self.logger.success(
                    "Disable RDP via WMI(ncacn_ip_tcp) successfully (old system)"
                )
            else:
                self.logger.success(
                    "Enable RDP via WMI(ncacn_ip_tcp) successfully (old system)"
                )

    def query_rdp_port(self):
        i_wbem_services = self.__iWbemLevel1Login.NTLMLogin(
            "//./root/DEFAULT", NULL, NULL
        )
        self.__iWbemLevel1Login.RemRelease()
    ...
    def rdp_ram_wrapper(self, action):
        ...
        if out.uValue == 0:
            self.logger.success(
                "Enable RDP Restricted Admin Mode via WMI(ncacn_ip_tcp) successfully"
            )
        elif out.uValue is None:
            self.logger.success(
                "Disable RDP Restricted Admin Mode via WMI(ncacn_ip_tcp) successfully"
            )
