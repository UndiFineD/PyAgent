...
class NXCModule:
        ...
        if self.action == "privesc":
            ...
            if self.is_admin_user(self.current_username):
                self.context.log.success(
                    ...
                    + highlight(f"({self.context.conf.get('nxc', 'pwn3d_label')})")
                )
    ...
    def browse_path(self, context, initial_user: User, user: User) -> User:
        ...
        for grantor in user.grantors:
            if grantor.is_sysadmin:
                self.context.log.success(
                    f"{user.username} can impersonate: {grantor.username} (sysadmin)"
                )
                return grantor
    ...
    def is_admin(self, exec_as="") -> bool:
        ...
        try:
            ...
            self.logger.fail(
                f"Error checking if user is admin, got {is_admin} as response. Expected 0 or 1."
            )
            return False
    ...
    def find_dbowner_priv(self, databases, exec_as="") -> list:
        ...
        """
        return [
            database for database in databases if self.is_db_owner(database, exec_as)
        ]
