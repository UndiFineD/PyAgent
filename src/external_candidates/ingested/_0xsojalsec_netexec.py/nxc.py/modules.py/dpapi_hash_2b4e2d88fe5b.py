...
class NXCModule:
    ...
    def on_admin_login(self, context, connection):
        ...
        try:
            context.log.display(
                "Collecting DPAPI masterkeys, grab a coffee and be patient..."
            )
            ...
            if self.outputfile is not None:
                with open(self.outputfile, "a+") as fd:
                    for mkhash in [
                        mkhash
                        for masterkey in masterkeys_triage.all_looted_masterkeys
                        for mkhash in masterkey.generate_hash()
                    ]:
