...
class NXCModule:
            ...
            try:
                ...
                for computer in results:
                    ...
                    if int(computer["userAccountControl"]) == 4128:  # 4096 | 32
                        ...
                        context.log.debug(
                            f"Added computer: {computer['sAMAccountName']}"
                        )

                # Save computers to file
                domain_dir = os.path.join(
                    f"{NXC_PATH}/modules/pre2k", connection.domain
                )
                ...
                if computers:
                    for computer in computers:
                        context.log.highlight(
                            f"Pre-created computer account: {computer}"
                        )
                    context.log.success(
                        f"Found {len(computers)} pre-created computer accounts. Saved to {output_file}"
                    )
                else:
                ...
                for computer in computers:
                    machine_name = computer[
                        :-1
                    ].lower()  # Remove trailing '$' and convert to lowercase
                    if self.get_tgt(
                        context,
                        machine_name,
                        connection.domain,
    ...
    def get_tgt(self, context, username, domain, kdcHost, ccache_base_dir):
        try:
            ...
            password = username[
                :14
            ]  # Password is the first 14 characters of the machine name in lowercase
