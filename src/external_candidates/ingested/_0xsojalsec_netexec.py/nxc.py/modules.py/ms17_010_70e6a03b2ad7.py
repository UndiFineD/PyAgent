...
class NXCModule:
    ...
    def on_login(self, context, connection):
        try:
            ...
            context.log.debug(
                f"Error connecting to host when checking for MS17-010: {e!s}"
            )
        except ValueError as e:
            if str(e) == "Buffer size too small (0 instead of at least 32 bytes)":
                context.log.debug(
                    "Buffer size too small, which means the response was not the expected size"
                )
    ...
    def negotiate_proto_request(self):
        ...
        return self.generate_smb_proto_payload(
            netbios, smb_header, negotiate_proto_request
        )
    ...
    def session_setup_andx_request(self):
        ...
        return self.generate_smb_proto_payload(
            netbios, smb_header, session_setup_andx_request
        )
    ...
    def tree_connect_andx_request(self, ip, userid):
        ...
        return self.generate_smb_proto_payload(
            netbios, smb_header, tree_connect_andx_request
        )
    ...
    def check(self, ip, port=445):
        ...
        raw_proto = self.peeknamedpipe_request(
        ...
        if nt_status == b"\x05\x02\x00\xc0":
            self.logger.highlight(
                f"[+] {ip} is likely VULNERABLE to MS17-010! ({native_os.decode()})"
            )
            ...
            if smb.multiplex_id == 0x0051:
                ...
                self.logger.highlight(
                    f"Host is likely INFECTED with DoublePulsar! - XOR Key: {key.decode()}"
                )
