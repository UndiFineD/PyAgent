...
from oscrypto.keys import parse_certificate, parse_pkcs12, parse_private
...
class myPKINIT(PKINIT):
    ...
    def build_asreq(self, domain=None, cname=None, kdcopts=None):
        ...
        kdc_req_body_data["cname"] = PrincipalName(
            {"name-type": NAME_TYPE.PRINCIPAL.value, "name-string": cname}
        )
    ...
    def sign_authpack_native(self, data, wrap_signed=False):
        ...
        si["signed_attrs"] = [
            ...
            cms.CMSAttribute(
                {"type": "message_digest", "values": [hashlib.sha1(data).digest()]}
        ...
        si["signature"] = rsa_pkcs1v15_sign(
            self.privkey, cms.CMSAttributes(si["signed_attrs"]).dump(), "sha1"
        )
    ...
    def decrypt_asrep(self, as_rep):
        ...
        pubkey = int.from_bytes(
            core.BitString(authdata["subjectPublicKey"]).dump()[7:], "big", signed=False
        )
...
class GETPAC:
    ...
    def printPac(self, data, key=None):
        ...
        adIfRelevant = decoder.decode(
            encTicketPart["authorization-data"][0]["ad-data"], asn1Spec=AD_IF_RELEVANT()
        )[0]
        ...
        for _bufferN in range(pacType["cBuffers"]):
            ...
            if infoBuffer["ulType"] == 2:
                ...
                for cred in pcc["Credentials"]:
                    credstruct = NTLM_SUPPLEMENTAL_CREDENTIAL(
...
def pfx_auth(self):
    ...
    if self.args.pfx_cert or self.args.pfx_base64:
        ...
        ini = myPKINIT.from_pfx(
            pfx, self.args.pfx_pass, dhparams, bool(self.args.pfx_base64)
        )
    elif self.args.pem_cert and self.args.pem_key:
        ini = myPKINIT.from_pem(self.args.pem_cert, self.args.pem_key, dhparams)
    else:
        self.logger.fail(
            "You must either specify a PFX file + optional password or a combination of Cert PEM file and Private key PEM file"
        )
        return None

    username = self.args.username[0]
    log_ccache = os.path.expanduser(
        f"{NXC_PATH}/logs/{self.hostname}_{self.host}_{datetime.datetime.now().strftime('%Y-%m-%d_%H%M%S')}-{username}.ccache".replace(
            ":", "-"
        )
    )
    ...
    if creds is not None:
        ...
        if not self.kerberos:
            ...
            self.kerberos_login(
                self.domain, username, "", nthash, "", self.kdcHost, False
            )
