...
from nxc.paths import DATA_PATH, NXC_PATH
...
for section in nxc_default_config.sections():
    for option in nxc_default_config.options(section):
        if not nxc_config.has_option(section, option):
            nxc_logger.display(
                f"Adding missing option '{option}' in config section '{section}' to nxc.conf"
            )
...
host_info_colors = literal_eval(
    nxc_config.get(
        "nxc", "host_info_colors", fallback=["green", "red", "yellow", "cyan"]
    )
)
...
def process_secret(text):
    ...
    return (
        text
        if not audit_mode
        else reveal + (audit_mode if len(audit_mode) > 1 else audit_mode * 8)
    )
