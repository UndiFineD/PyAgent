...
class ftp(connection):
    ...
    def enum_host_info(self):
        ...
        self.remote_version = welcome.split("220", 1)[
            1
        ].strip()  # strip out the extra space in the front
    ...
    def plaintext_login(self, username, password):
        ...
        if "230" in resp:
            ...
            if username in ["anonymous", ""]:
                self.logger.success(
                    f"{username}:{process_secret(password)} {highlight('- Anonymous Login!')}"
                )
            else:
        ...
        if self.args.ls:
            ...
            if self.args.ls == ".":
                ...
                try:
                    ...
                    self.logger.fail(
                        f"Failed to change directory. Response: ({error_message})"
                    )
                    self.conn.close()
                    return False
    ...
    def get_file(self, filename):
        ...
        try:
            ...
            self.conn.retrbinary(
                f"RETR {filename}", open(downloaded_file, "wb").write
            )  # noqa: SIM115
        except error_perm as error_message:
            self.logger.fail(
                f"Failed to download the file. Response: ({error_message})"
            )
            self.conn.close()
            return False
        except FileNotFoundError:
            self.logger.fail(
                "Failed to download the file. Response: (No such file or directory.)"
            )
            self.conn.close()
            return False
    ...
    def put_file(self, local_file, remote_file):
        try:
            ...
            self.conn.storbinary(
                f"STOR {remote_file}", open(local_file, "rb")
            )  # noqa: SIM115
        except error_perm as error_message:
            ...
            self.logger.fail(
                f"Failed to upload file. {local_file} does not exist locally."
            )
            return False
    ...
    def supported_commands(self):
        ...
        supported_commands = [
            item
            for sublist in (x.split() for x in raw_supported_commands.split("\n")[1:-1])
            for item in sublist
        ]
