...
class WMIEXEC:
    ...
    def execute_WithOutput(self, command):
        ...
        self.execute_remote(command)
        self.logger.info(
            f"Waiting {self.__exec_timeout}s for command completely executed."
        )
    ...
    def queryRegistry(self, keyName):
        try:
            ...
            retVal = descriptor.GetStringValue(
                2147483650, self.__registry_Path, keyName
            )
            self.__outputBuffer = (
                base64.b64decode(retVal.sValue)
                .decode(self.__codec, errors="replace")
                .rstrip("\r\n")
            )
        except Exception:
            self.logger.fail(
                "WMIEXEC: Could not retrieve output file, it may have been detected by AV. Please try increasing the timeout with the '--exec-timeout' option. If it is still failing, try the 'smb' protocol or another exec method"
            )

        try:
            self.logger.debug(
                f"Removing temporary registry path: HKLM\\{self.__registry_Path}"
            )
            retVal = descriptor.DeleteKey(2147483650, self.__registry_Path)
        except Exception as e:
            self.logger.debug(
                f"Target: {self.__target} removing temporary registry path error: {e!s}"
            )
