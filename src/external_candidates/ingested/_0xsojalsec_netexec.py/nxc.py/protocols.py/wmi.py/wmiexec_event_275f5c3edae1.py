...
class WMIEXEC_EVENT:
    def __init__(
        ...
        self.__iWbemServices = iWbemLevel1Login.NTLMLogin(
            "//./root/subscription", NULL, NULL
        )
    ...
    def process_vbs(self, command):
        ...
        if self.__retOutput:
            ...
            with open(
                get_ps_script("wmiexec_event_vbscripts/Exec_Command_WithOutput.vbs")
            ) as vbs_file:
                vbs = vbs_file.read()
            vbs = vbs.replace(
                "REPLACE_ME_BASE64_COMMAND", base64.b64encode(command.encode()).decode()
            )
            ...
            with open(
                get_ps_script("wmiexec_event_vbscripts/Exec_Command_Silent.vbs")
            ) as vbs_file:
                vbs = vbs_file.read()
            vbs = vbs.replace(
                "REPLACE_ME_BASE64_COMMAND", base64.b64encode(command.encode()).decode()
    ...
    def execute_vbs(self, vbs_content):
        ...
        event_filter.Query = (
            f'select * from __TimerEvent where TimerID = "{self.__instanceID}" '
        )
        ...
        filter_binding.Consumer = (
            f'ActiveScriptEventConsumer.Name="{self.__instanceID}"'
        )
    ...
    def get_command_result(self):
        try:
            ...
            self.__outputBuffer = base64.b64decode(
                record["ScriptText"]["value"]
    ...
    def remove_instance(self):
        ...
        resp = self.__iWbemServices.DeleteInstance(
            f'__IntervalTimerInstruction.TimerId="{self.__instanceID}"'
        )
        self.check_error(
        ...
        resp = self.__iWbemServices.DeleteInstance(
            f'__EventFilter.Name="{self.__instanceID}"'
        )
        self.check_error(
