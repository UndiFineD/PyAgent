...
"""
    ...
    def qwinsta(self):
        ...
        maxUsernameLen = (
            maxUsernameLen if len("Username") < maxUsernameLen else len("Username") + 1
        )
        ...
        maxRemoteIp = (
            maxRemoteIp
            if len("RemoteAddress") < maxRemoteIp
            else len("RemoteAddress") + 1
        )
        maxClientName = max([len(sessions[i]["ClientName"]) + 1 for i in sessions])
        maxClientName = (
            maxClientName
            if len("ClientName") < maxClientName
            else len("ClientName") + 1
        )
        template = (
            "{SESSIONNAME: <%d} "
            "{USERNAME: <%d} "
        ...
        for i in sessions:
            ...
            userName = (
                sessions[i]["Domain"] + "\\" + sessions[i]["Username"]
                if len(sessions[i]["Username"])
                else ""
            )
    ...
    def shares(self):
        ...
        for share in permissions:
            ...
            remark = share["remark"]
            perms = share["access"]
            if self.args.filter_shares and not any(
                x in perms for x in self.args.filter_shares
            ):
                continue
    ...
    def interfaces(self):
        ...
        try:
            ...
            if remoteOps._RemoteOperations__rrp:
                reg_handle = rrp.hOpenLocalMachine(remoteOps._RemoteOperations__rrp)[
                    "phKey"
                ]
                ...
                sub_key_list = rrp.hBaseRegQueryInfoKey(
                    remoteOps._RemoteOperations__rrp, key_handle
                )["lpcSubKeys"]
                sub_keys = [
                    rrp.hBaseRegEnumKey(
                        remoteOps._RemoteOperations__rrp, key_handle, i
                    )["lpNameOut"][:-1]
                ...
                for sub_key in sub_keys:
                    ...
                    try:
                        ...
                        interface["DefaultGateway"] = (
                            default_gateway if default_gateway else None
                        )
                        ...
                        self.logger.info(
                            f"Failed to retrieve the network interface info for {sub_key}: {e!s}"
                        )
        ...
        try:
            ...
            if not flag or not stringBinding:
                ...
                (
                    ...
                    if not flag
                    else self.logger.debug(error_msg)
                )
