...
class wmi(connection):
    ...
    def enum_host_info(self):
        ...
        self.remoteName = (
            self.host if not self.kerberos else f"{self.hostname}.{self.domain}"
        )
    ...
    def check_if_admin(self):
        try:
            ...
            flag, self.stringBinding = dcom_FirewallChecker(
                iInterface, self.host, self.args.rpc_timeout
            )
        except Exception as e:
    ...
    def kerberos_login(
        ...
        kerb_pass = (
            next(s for s in [nthash, password, aesKey] if s)
        ...
        try:
            ...
            try:
                ...
                self.logger.fail(
                    out, color=("red" if "access_denied" in error_msg else "magenta")
                )
                return False
            else:
    ...
    def plaintext_login(self, domain, username, password):
        ...
        try:
            ...
            try:
                ...
                self.logger.fail(
                    (
                        f"{self.domain}\\{self.username}:{process_secret(self.password)} ({error_msg.upper()})"
                    ),
                    color=("red" if "access_denied" in error_msg else "magenta"),
                )
                return False
            else:
                self.check_if_admin()
    ...
    def hash_login(self, domain, username, ntlm_hash):
        ...
        try:
            ...
            try:
                ...
                self.logger.fail(
                    (
                        f"{self.domain}\\{self.username}:{process_secret(self.nthash)} ({error_msg.upper()})"
                    ),
                    color=("red" if "access_denied" in error_msg else "magenta"),
                )
                return False
