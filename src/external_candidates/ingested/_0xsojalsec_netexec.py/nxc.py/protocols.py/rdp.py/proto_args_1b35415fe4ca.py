...
def proto_args(parser, parents):
    ...
    rdp_parser.add_argument(
        "--rdp-timeout", type=int, default=5, help="RDP timeout on socket connection"
    )
    rdp_parser.add_argument(
        "--nla-screenshot",
        action="store_true",
    ...
    egroup.add_argument(
        "--screenshot", action="store_true", help="Screenshot RDP if connection success"
    )
    egroup.add_argument(
        "--screentime", type=int, default=10, help="Time to wait for desktop image"
    )
    egroup.add_argument(
        "--res", default="1024x768", help="Resolution in WIDTHxHEIGHT format"
    )
