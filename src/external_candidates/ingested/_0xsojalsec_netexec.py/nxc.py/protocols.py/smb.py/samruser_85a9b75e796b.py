...
class UserSamrDump:
    ...
    def fetch_users(self, requested_users, dump_path):
        ...
        if requested_users:
            self.logger.debug(
                f"Looping through users requested and looking up their information: {requested_users}"
            )
            try:
                names_lookup_resp = samr.hSamrLookupNamesInDomain(
                    self.dce, domain_handle, requested_users
                )
                ...
                if "STATUS_SOME_NOT_MAPPED" in str(e):
                    ...
                    self.logger.fail(
                        "One of the users requested does not exist in the domain, causing a critical failure during translation, re-check the users and try again"
                    )
                else:
                    self.logger.fail(
                        f"Error occurred when looking up users in domain: {e}"
                    )
        else:
            ...
            while status == STATUS_MORE_ENTRIES:
                try:
                    enumerate_users_resp = samr.hSamrEnumerateUsersInDomain(
                ...
                rids = [
                    r["RelativeId"] for r in enumerate_users_resp["Buffer"]["Buffer"]
                ]
    ...
    def get_user_info(self, domain_handle, user_ids):
        ...
        self.logger.highlight(
            f"{'-Username-':<30}{'-Last PW Set-':<20}{'-BadPW-':<8}{'-Description-':<60}"
        )
        users = []
        ...
        for user in user_ids:
            ...
            open_user_resp = samr.hSamrOpenUser(
                self.dce, domain_handle, samr.MAXIMUM_ALLOWED, user
            )
