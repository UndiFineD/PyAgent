...
class FirefoxTriage:
                ...
                try:
                    if gather_cookies:
                        ...
                        if cookies_data is not None:
                            firefox_cookies += self.parse_cookie_data(
                                user, cookies_data
                            )
                    logins_path = (
                        self.firefox_generic_path.format(user)
    ...
    def parse_cookie_data(self, windows_user, cookies_data):
        ...
        cursor = db.cursor()
        cursor.execute(
            "SELECT name, value, host, path, expiry, lastAccessed, creationTime FROM moz_cookies;"
        )
    ...
    def get_users(self):
        ...
        for d in directories:
            if d.get_longname() not in self.false_positive and d.is_directory() > 0:
                users.append(
                    d.get_longname()
                )  # noqa: PERF401, ignoring for readability
        return users
