...
class database(BaseDB):
    ...
    def db_schema(db_conn):
        ...
        db_conn.execute("""CREATE TABLE "conf_checks" (
            "id" integer PRIMARY KEY,
            "name" text,
            "description" text
            )""")

        db_conn.execute("""CREATE TABLE "conf_checks_results" (
            "id" integer PRIMARY KEY,
            "host_id" integer,
            "check_id" integer,
            "secure" boolean,
    ...
    def reflect_tables(self):
        with self.db_engine.connect():
            try:
                ...
                self.ConfChecksTable = Table(
                    "conf_checks", self.metadata, autoload_with=self.db_engine
                )
                self.ConfChecksResultsTable = Table(
                    "conf_checks_results", self.metadata, autoload_with=self.db_engine
                )
    ...
    def add_admin_user(self, credtype, domain, username, password, host, user_id=None):
        ...
        creds_q = (
            creds_q.filter(self.UsersTable.c.id == user_id)
            if user_id
            else creds_q.filter(
                func.lower(self.UsersTable.c.credtype) == func.lower(credtype),
                func.lower(self.UsersTable.c.domain) == func.lower(domain),
    ...
    def insert_data(self, table, select_results=None, **new_row):
        ...
        q = q.on_conflict_do_update(
            index_elements=table.primary_key, set_=update_column
        )
    ...
    def add_check_result(self, host_id, check_id, secure, reasons):
        ...
        q = select(self.ConfChecksResultsTable).filter(
            self.ConfChecksResultsTable.c.host_id == host_id,
        ...
        updated_ids = self.insert_data(
            self.ConfChecksResultsTable, select_results, **new_row
        )

        if updated_ids:
            nxc_logger.debug(
                f"add_check_result() - Check Results IDs Updated: {updated_ids}"
            )
            return updated_ids
