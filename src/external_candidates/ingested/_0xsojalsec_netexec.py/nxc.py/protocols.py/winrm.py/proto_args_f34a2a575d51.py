...
def proto_args(parser, parents):
    ...
    winrm_parser.add_argument(
        ...
        nargs="+",
        default=["5985", "5986"],
        help="WinRM port - format: 'http-port https-port'(with space separated) or 'single-port' (http & https will use same port when given single port)",
    )
    winrm_parser.add_argument(
        "--check-proto",
        nargs="+",
        default=["http", "https"],
        help="Choose what prorocol you want to check - format: 'http https'(with space separated) or 'single-protocol'",
    )
    winrm_parser.add_argument(
        "--laps",
        dest="laps",
    ...
    cgroup.add_argument(
        ...
        help="Select shell type in hashes dump",
    )
    cgroup.add_argument(
    ...
    cgroup.add_argument(
        "-x", metavar="COMMAND", dest="execute", help="execute the specified command"
    )
    cgroup.add_argument(
        "-X",
        metavar="PS_COMMAND",
