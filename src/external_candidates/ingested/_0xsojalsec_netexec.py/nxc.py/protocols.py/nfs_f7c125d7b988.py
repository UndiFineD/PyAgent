...
class nfs(connection):
    ...
    def print_host_info(self):
        root_escape_str = colored(
            f"root escape:{self.root_escape}",
            host_info_colors[1 if self.root_escape else 0],
            attrs=["bold"],
        )
        self.logger.display(
            f"Supported NFS versions: ({', '.join(str(x) for x in self.nfs_versions)}) ({root_escape_str})"
        )
                ...
                for entry in entries:
                    if "name" in entry and entry["name"] not in [b".", b".."]:
                        ...
                        if entry.get("name_attributes", {}).get("present", False):
                            if (
                                entry["name_attributes"]["attributes"]["type"] == 2
                                and recurse > 0
                            ):  # Recursive directory listing. Entry type shows file format. 1 is file, 2 is folder.
                                dir_handle = entry["name_handle"]["handle"]["data"]
                                contents += self.list_dir(
                                    dir_handle, item_path, recurse=recurse - 1
                                )
                            else:
                                ...
                                read_perm, write_perm, exec_perm = self.get_permissions(
                                    entry["name_handle"]["handle"]["data"]
                                )
                                contents.append(
                                    {
                                        "path": item_path,
                    ...
                    if entry["nextentry"]:
                        ...
                        contents += process_entries(
                            entry["nextentry"], path, uid, recurse
                        )

                return contents
            except Exception as e:
                self.logger.debug(
                    f"Error on Listing Entries for NFS Shares: {self.host}:{self.port} {e}"
                )
    ...
    def shares(self):
        ...
        try:
            ...
            self.logger.highlight(
                f"{'UID':<11}{'Perms':<9}{'Storage Usage':<17}{'Share':<30} {'Access List':<15}"
            )
            self.logger.highlight(
                f"{'---':<11}{'-----':<9}{'-------------':<17}{'-----':<30} {'-----------':<15}"
            )
            for share, network in zip(shares, networks, strict=True):
                try:
                    ...
                    if mnt_info["status"] != 0:
                        self.logger.debug(
                            f"Error mounting share {share}: {NFSSTAT3[mnt_info['status']]}"
                        )
                        self.logger.highlight(
                            f"{'-':<11}{'---':<9}{'---'}/{'---':<12} {share:<30} {', '.join(network) if network else 'No network':<15}"
                        )
                    else:
                        ...
                        self.logger.highlight(
                            f"{self.auth['uid']:<11}{'r' if read_perm else '-'}{'w' if write_perm else '-'}{('x' if exec_perm else '-'):<7}{convert_size(used_space) + "/" + convert_size(total_space):<16} {share:<30} {', '.join(network) if network else 'No network':<15}"
                        )
                except Exception as e:
            ...
            self.logger.fail(
                f"Error on Enumeration NFS Shares: {self.host}:{self.port} {e}"
            )
        finally:
            self.nfs3.disconnect()
    ...
    def get_permissions(self, file_handle):
        ...
        try:
            read_perm = (
                self.nfs3.access(file_handle, ACCESS3_READ, self.auth)
                .get("resok", {})
                .get("access", 0)
                is ACCESS3_READ
            )
        ...
        try:
            write_perm = (
                self.nfs3.access(file_handle, ACCESS3_MODIFY, self.auth)
                .get("resok", {})
                .get("access", 0)
                is ACCESS3_MODIFY
            )
        ...
        try:
            exec_perm = (
                self.nfs3.access(file_handle, ACCESS3_EXECUTE, self.auth)
                .get("resok", {})
                .get("access", 0)
                is ACCESS3_EXECUTE
            )
            ...
            for share, network in zip(shares, networks, strict=True):
                try:
                    ...
                    if mount_info["status"] != 0:
                        self.logger.fail(
                            f"Error mounting share {share}: {NFSSTAT3[mount_info['status']]}"
                        )
                        continue
                    ...
                    if contents:
                        self.logger.highlight(
                            f"{'UID':<11}{'Perms':<9}{'File Size':<15}{'File Path':<45} {'Access List':<15}"
                        )
                        self.logger.highlight(
                            f"{'---':<11}{'-----':<9}{'---------':<15}{'---------':<45} {'-----------':<15}"
                        )
                    for content in contents:
                        self.logger.highlight(
                            f"{content['uid']:<11}{'r' if content['read'] else '-'}{'w' if content['write'] else '-'}{'x' if content['execute'] else '-':<7}{content['filesize']:<14} {content['path']:<45} {', '.join(network) if network else 'No network':<15}"
                        )
                except Exception as e:
                    if "RPC_AUTH_ERROR: AUTH_REJECTEDCRED" in str(e):
                        ...
                        self.logger.fail(
                            f"{share} - Insufficient Permissions for share listing"
                        )
                    else:
                        self.logger.exception(f"{share} - {e}")
        except Exception as e:
            self.logger.debug(
                f"Error on Listing NFS Shares Directories: {self.host}:{self.port} {e}"
            )
            self.logger.debug(
                "It is probably unknown format or can not access as anonymously."
            )
        finally:
            self.nfs3.disconnect()
    ...
    def get_file(self):
        ...
        try:
            ...
            if self.root_escape and not self.args.share:
                ...
                if mnt_info["status"] != 0:
                    self.logger.fail(
                        f"Error mounting share {self.args.share}: {NFSSTAT3[mnt_info['status']]}"
                    )
                    return
            ...
            self.logger.highlight(
                f"File successfully downloaded from {remote_file_path} to {local_file_path}"
            )
            ...
            self.logger.fail(
                f'Error retrieving file "{file_name}" from "{remote_dir_path}": {e}'
            )
            if (
        ...
        try:
            ...
            for sub_path in list(filter(None, remote_dir_path.lstrip("/").split("/"))):
                ...
                if "resfail" in res and res["status"] == NFS3ERR_NOENT:
                    ...
                    if res["status"] != 0:
                        self.logger.fail(
                            f"Error creating directory '/{sub_path}/': {NFSSTAT3[res['status']]}"
                        )
                        return
                    else:
            ...
            if lookup_response["resok"] is None:
                ...
                res = self.nfs3.create(
                    curr_fh, file_name, create_mode=1, mode=0o777, auth=self.auth
                )
                if res["status"] != 0:
                ...
                ans = input(
                    highlight(
                        f"[!] {file_name} already exists on {remote_file_path}. Do you want to overwrite it? [Y/n] ",
                        "red",
                    )
                )
                if ans.lower() in ["y", "yes", ""]:
                    self.logger.display(
                        f"{file_name} already exists on {remote_file_path}. Trying to overwrite it..."
                    )
            ...
            try:
                ...
                self.logger.info(
                    f"Trying to write data from {local_file_path} to {remote_file_path}"
                )
                res = self.nfs3.write(
                    file_handle, 0, len(file_data), file_data, 1, auth=self.auth
                )
                if res["status"] != 0:
                    self.logger.fail(
                        f"Error writing to {remote_file_path}: {NFSSTAT3[res['status']]}"
                    )
                    return
                else:
                    self.logger.success(
                        f"Data from {local_file_path} successfully written to {remote_file_path} with permissions 777"
                    )
            except Exception as e:
            ...
            self.logger.highlight(
                f"File {local_file_path} successfully uploaded to {remote_file_path}"
            )
    ...
    def get_root_handles(self, mount_fh):
        ...
        dir_data = self.format_directory(
            self.nfs3.readdirplus(mount_fh, auth=self.auth)
        )
    ...
    def try_root_escape(self) -> bool:
        ...
        for share in shares:
            ...
            if mount_info["status"] != 0:
                self.logger.debug(
                    f"Root escape: can't list directory {share}: {NFSSTAT3[mount_info['status']]}"
                )
                self.mount.umnt(self.auth)
                continue
            ...
            try:
                ...
                for fh in possible_root_fhs:
                    if "resfail" not in self.nfs3.readdir(fh, auth=self.auth):
                        self.logger.info(
                            f"Root escape successful on share '{share}' with handle: {fh.hex()}"
                        )
    ...
    def ls(self):
        ...
        if dir_listing["status"] != 0:
            self.logger.fail(
                f"Error on listing directory: {NFSSTAT3[dir_listing['status']]}"
            )
            return
        ...
        for item in content:
            if not item["name_attributes"]["present"]:
                try:
                    res = self.nfs3.lookup(
                        curr_fh, item["name"].decode(), auth=self.auth
                    )
                    item["name_attributes"]["attributes"] = res["resok"][
                        "obj_attributes"
                    ]["attributes"]
                    ...
                    self.logger.debug(
                        f"Error on getting attributes for {item['name'].decode()}: {e}"
                    )
    ...
    def print_directory(self, content, path):
        ...
        for item in content:
            if (
                not item["name_attributes"]["present"]
                or not item["name_handle"]["present"]
            ):
                uid = "-"
                perms = "----"
                file_size = "-"
                ...
                is_dir = (
                    "d" if item["name_attributes"]["attributes"]["type"] == 2 else "-"
                )
                read_perm, write_perm, exec_perm = self.get_permissions(
                    item["name_handle"]["handle"]["data"]
                )
            ...
            self.logger.highlight(
                f"{uid:<11}{perms:<7}{file_size:<14}{path.rstrip('/') + '/' + item['name'].decode()}"
            )
    ...
    def update_auth(self, file_handle):
        ...
        attrs = self.nfs3.getattr(file_handle, auth=self.auth)
        self.logger.debug(
            f"Updating auth with UID: {attrs['attributes']['uid']} and GID: {attrs['attributes']['gid']}"
        )
        self.auth["uid"] = attrs["attributes"]["uid"]
