...
class MSSQLEXEC:
    ...
    def execute(self, command):
        ...
        try:
            ...
            if result:
                result = "\n".join(
                    line["output"] for line in result if line["output"] != "NULL"
                )
                self.logger.debug(
                    f"Concatenated result together for easier parsing: {result}"
                )
                # if you prepend SilentlyContinue it will still output the error, but it will still continue on (so it's not silent...)
                if (
                    "Preparing modules for first use" in result
                    and "Completed" not in result
                ):
                    self.logger.error(
                        "Error when executing PowerShell (received 'preparing modules for first use'), try prepending $ProgressPreference = 'SilentlyContinue'; to your command"
                    )
    ...
    def restore(self, option):
        try:
            if not self.backuped_options[option]:
                self.logger.debug(
                    f"Option '{option}' was not enabled originally, attempting to disable it."
                )
                ...
                self.logger.debug(
                    f"Option '{option}' was originally enabled, leaving it enabled."
                )
        except Exception as e:
            self.logger.error(
                f"[OPSEC] Error when attempting to restore option '{option}': {e}"
            )
    ...
    def backup_and_enable(self, option):
        try:
            ...
            if not self.backuped_options[option]:
                self.logger.debug(
                    f"Option '{option}' is disabled, attempting to enable it."
                )
