...
def proto_args(parser, parents):
    ...
    mssql_parser.add_argument(
        "--port", default=1433, type=int, metavar="PORT", help="MSSQL port"
    )
    mssql_parser.add_argument(
        "--mssql-timeout", help="SQL server connection timeout", type=int, default=5
    )
    mssql_parser.add_argument(
        "-q",
        "--query",
    ...
    psgroup = mssql_parser.add_argument_group(
        "Powershell Options", "Options for PowerShell execution"
    )
    ...
    mapping_enum_group = mssql_parser.add_argument_group(
