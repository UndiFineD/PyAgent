...
class ldap(connection):
    ...
    def enum_host_info(self):
        ...
        self.output_filename = os.path.expanduser(
            f"~/.nxc/logs/{self.hostname}_{self.host}".replace(":", "-")
        )
    ...
    def kerberos_login(
        ...
        try:
            # Connect to LDAP
            self.logger.extra["protocol"] = (
                "LDAPS" if (self.args.gmsa or self.port == 636) else "LDAP"
            )
            self.logger.extra["port"] = (
                "636" if (self.args.gmsa or self.port == 636) else "389"
            )
            proto = "ldaps" if (self.args.gmsa or self.port == 636) else "ldap"
        ...
        if self.args.groups:
            ...
            for item in resp_parsed:
                try:
                    ...
                    self.logger.highlight(
                        f"{item['cn']:<40} membercount: {len(item.get('member', []))}"
                    )
                except Exception as e:
    ...
    def dc_list(self):
        ...
        for item in resp_parsed:
            name = item.get(
                "dNSHostName", ""
            )  # Get dNSHostName attribute or empty string
            try:
                        ...
                        try:
                            ...
                            for rdata in answers:
                                if record_type in ["A", "AAAA"]:
                                    ...
                                    self.logger.highlight(
                                        f"{name} = {colored(ip_address, host_info_colors[0])}"
                                    )
                                    found_record = (
                                        True  # Set flag to true since a record is found
                                    )
                                elif record_type == "CNAME":
                                    self.logger.highlight(
                                        f"{name} CNAME = {colored(rdata.to_text(), host_info_colors[0])}"
                                    )
                                    found_record = True
                                elif record_type == "PTR":
                                    self.logger.highlight(
                                        f"{name} PTR = {colored(rdata.to_text(), host_info_colors[0])}"
                                    )
                                    found_record = True
                                elif record_type == "NS":
                                    self.logger.highlight(
                                        f"{name} NS = {colored(rdata.to_text(), host_info_colors[0])}"
                                    )
                                    found_record = True
                            ...
                            self.logger.fail(
                                f"{name} encountered an unexpected error: {e}"
                            )
                else:
        ...
        if not resp_parsed:
            ...
            for user in enabled:
                ...
                if TGT:
                    ...
                    self.logger.fail(
                        f"Error retrieving TGT for {self.username}\\{self.domain} from {self.kdcHost}"
                    )
