...
class LDAPConnect:
    ...
    def kerberos_login(
        ...
        if kdcHost is None or domain not in kdcHost:
            self.logger.fail(
                "Please provide the FQDN of the domain controller with --kdcHost"
            )
            exit(1)
        ...
        try:
            self.logger.info(
                f"Connecting to ldap://{kdcHost} - {baseDN} - {domain} [1]"
            )
...
class LAPSv2Extract:
    ...
    def run(self):
        ...
        try:
            ...
            parsed_cms_data, remaining = decoder.decode(
                encrypted_laps_blob["Blob"], asn1Spec=rfc5652.ContentInfo()
            )
            enveloped_data_blob = parsed_cms_data["content"]
            parsed_enveloped_data, _ = decoder.decode(
                enveloped_data_blob, asn1Spec=rfc5652.EnvelopedData()
            )
            ...
            self.logger.error(
                f"Cannot unpack msLAPS-EncryptedPassword blob due to error {e}"
            )
            return None
        ...
        enc_content_parameter = bytes(
            parsed_enveloped_data["encryptedContentInfo"]["contentEncryptionAlgorithm"][
                "parameters"
            ]
        )
...
def laps_search(self, username, password, cred_type, domain, dns_server):
    ...
    if len(results) != 0:
        for host in results:
            values = {
                str(attr["type"]).lower(): attr["vals"][0]
