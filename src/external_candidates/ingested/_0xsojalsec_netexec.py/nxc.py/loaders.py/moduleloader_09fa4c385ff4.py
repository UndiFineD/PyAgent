...
class ModuleLoader:
    ...
    def module_is_sane(self, module, module_path):
        ...
        if not hasattr(module, "name"):
            ...
            hasattr(module, "name")
            and module.name != module_path.split("/")[-1].split("\\")[-1][:-3]
        ):
            self.logger.fail(
                f"{module_path} filename must match the module name {module.name}"
            )
            module_error = True
        ...
        if module:
            ...
            if self.args.protocol in module.supported_protocols:
                try:
                    ...
                    self.logger.fail(
                        f"Error loading NXCAdaptor for module {module.name.upper()}: {e}"
                    )
        ...
        try:
            ...
            module = {
                f"{module_spec.name}": {
                    ...
                    "requires_admin": bool(
                        hasattr(module_spec, "on_admin_login")
                        and callable(module_spec.on_admin_login)
                    ),
                }
            }
