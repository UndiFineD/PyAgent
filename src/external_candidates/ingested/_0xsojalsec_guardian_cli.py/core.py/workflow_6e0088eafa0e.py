# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-guardian-cli\core\workflow.py
"""
...
class WorkflowEngine:
    ...
    async def run_workflow(self, workflow_name: str) -> Dict[str, Any]:
        ...
        """
        self.logger.info(
            f"Starting workflow: {workflow_name} for target: {self.target}"
        )
    ...
    async def run_autonomous(self) -> Dict[str, Any]:
        ...
        try:
            while self.is_running and self.current_step < self.max_steps:
                ...
                if decision.get("next_action", "").lower() in [
                    "done",
                    "complete",
                    "finish",
                ]:
    ...
    async def _execute_step(self, step: Dict[str, Any]):
        ...
        if step_type == "tool":
            ...
            if result.get("success"):
                ...
                self.logger.info(
                    f"Found {len(analysis['findings'])} findings from {tool_name}"
                )
            else:
            ...
            # Save report
            output_dir = Path(
                self.config.get("output", {}).get("save_path", "./reports")
            )
            output_dir.mkdir(parents=True, exist_ok=True)

            report_file = (
                output_dir
                / f"report_{self.memory.session_id}.{step.get('format', 'md')}"
            )
    ...
    async def _execute_ai_decision(self, decision: Dict[str, Any]):
        ...
        try:
            tool_selection = await self.tool_agent.execute(
                objective=action, target=self.target
            )
    ...
    def _maybe_advance_phase(self):
        ...
        current_idx = (
            phases.index(self.memory.current_phase)
            if self.memory.current_phase in phases
            else 0
        )
