# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-guardian-cli\core\analyst_agent.py
"""
Analyst Agent
...
class AnalystAgent(BaseAgent):
    ...
    async def interpret_output(
        self, tool: str, target: str, command: str, output: str
    ) -> Dict[str, Any]:
        """
        ...
        prompt = ANALYST_INTERPRET_PROMPT.format(
            tool=tool, target=target, command=command, output=output
        )
    ...
    async def correlate_findings(self) -> Dict[str, Any]:
        ...
        prompt = ANALYST_CORRELATION_PROMPT.format(
            target=self.memory.target, tool_results=tool_results
        )
    ...
    def _parse_findings(
        self, ai_response: str, tool: str, target: str
    ) -> List[Finding]:
