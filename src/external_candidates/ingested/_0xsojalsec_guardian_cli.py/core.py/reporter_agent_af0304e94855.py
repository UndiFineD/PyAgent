# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-guardian-cli\core\reporter_agent.py
"""
Reporter Agent
...
class ReporterAgent(BaseAgent):
    ...
    async def generate_ai_trace(self) -> str:
        ...
        prompt = REPORTER_AI_TRACE_PROMPT.format(
            ai_decisions=ai_decisions or "No AI decisions recorded", workflow=workflow
        )
...
""")
    ...
    def _format_tool_executions(self) -> str:
        ...
        for t in self.memory.tool_executions:
            formatted.append(
                f"- **{t.tool}**: {t.command} (Duration: {t.duration:.2f}s)"
            )
