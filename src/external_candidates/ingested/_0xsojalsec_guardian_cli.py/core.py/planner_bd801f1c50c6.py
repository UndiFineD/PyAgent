# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-guardian-cli\core\planner.py
"""
...
class PlannerAgent(BaseAgent):
    ...
    async def decide_next_action(self) -> Dict[str, Any]:
        ...
        prompt = PLANNER_DECISION_PROMPT.format(
            ...
            completed_actions="\n".join(f"- {a}" for a in self.memory.completed_actions)
            or "None",
    ...
    def _parse_decision(self, response: str) -> Dict[str, Any]:
        ...
        if "PARAMETERS:" in response:
            start = response.find("PARAMETERS:") + len("PARAMETERS:")
                ...
                response.find("EXPECTED_OUTCOME:", start)
