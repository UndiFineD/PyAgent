# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HuMo\humo\models\text\encoder.py
import os
        ...
        for model in config.models:
            ...
            if model.type == "llm14b":
                ...
                model = model_cls.from_pretrained(
                    path, trust_remote_code=True, bf16=True
                )
            else:
                tokenizer = tokenizer_cls.from_pretrained(
    ...
    def forward(self, text: Union[str, List[str]]) -> TextEncoderOutput:
        ...
        for encoder_config, tokenizer, model in zip(
            ...
            if encoder_config.type == "llm14b":
                ...
                range_ids = torch.arange(
                    len(token_ids), device=token_ids.device, dtype=torch.long
                )
                token_ids[range_ids, num_tokens.clamp(max=token_ids.size(1) - 1)] = (
                    tokenizer.pad_token_id
                )
                attention_mask[
                    range_ids, num_tokens.clamp(max=token_ids.size(1) - 1)
                ] = 1
                ...
                if layer == "last":
                    ...
                    embeddings.append(
                        model.text_model.final_layer_norm(output.hidden_states[-2])
                    )
            ...
            if embedding_output_type == "undefined" and len(self.models) == 1:
                ...
                raise NotImplementedError(
                    f"output.embedding_and_mask: {embedding_output_type}"
                )
