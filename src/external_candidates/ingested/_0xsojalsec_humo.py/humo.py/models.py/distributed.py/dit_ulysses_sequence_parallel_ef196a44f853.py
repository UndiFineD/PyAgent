# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HuMo\humo\models\distributed\dit_ulysses_sequence_parallel.py
...
def ulysses_dit_forward(self, x, t, context, seq_len, audio=None, y=None):
    """
    ...
    if self.insert_audio:
        ...
        audio_seq_len = torch.tensor(
            max([au.shape[2] for au in audio]) * audio[0].shape[3], device=get_device()
        )
        audio = [au.flatten(2).transpose(1, 2) for au in audio]  # [1, t*32, 1536]
        audio_seq_lens = torch.tensor(
            [au.size(1) for au in audio], dtype=torch.long, device=device
        )
        audio = torch.cat(
            [
                torch.cat(
                    [au, au.new_zeros(1, audio_seq_len - au.size(1), au.size(2))], dim=1
                )
                for au in audio
            ]
        )
    ...
    # ulysses support
    x = gather_outputs(
        x, gather_dim=1, padding_dim=1, unpad_dim_size=seq_len, scale_grad=True
    )
...
def ulysses_attn_forward(self, x, seq_lens, grid_sizes, freqs, dtype=torch.bfloat16):
    ...
    x = flash_attention(
        q=half(q), k=half(k), v=half(v), k_lens=seq_lens, window_size=self.window_size
    )
...
def ulysses_audio_cross_attn_forward(
    self, x, audio, seq_lens, grid_sizes, freqs, audio_seq_len, dtype=torch.bfloat16
):
    b, s, n, d = *x.shape[:2], self.num_heads, self.head_dim
    seq_len = seq_lens.max()
