# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HuMo\humo\generate.py
...
from humo.models.utils.utils import prepare_json_dataset, tensor_to_video
...
class Generator:
    ...
    def get_fsdp_sharding_config(self, sharding_strategy, device_mesh_config):
        ...
        if (
            fsdp_strategy
            in [ShardingStrategy._HYBRID_SHARD_ZERO2, ShardingStrategy.HYBRID_SHARD]
    ...
    def configure_vae_model(self, device=get_device()):
        ...
        self.vae = WanVAE(vae_pth=self.config.vae.checkpoint, device=device)
        ...
        if self.config.generation.height == 480:
            ...
            raise ValueError(
                f"Unsupported height {self.config.generation.height} for zero-vae."
            )
    ...
    def forward_tia(
        self, latents, timestep, t, step_change, arg_tia, arg_ti, arg_i, arg_null
    ):
        pos_tia, _ = self.parse_output(self.dit(latents, t=timestep, **arg_tia))
        torch.cuda.empty_cache()

        pos_ti, _ = self.parse_output(self.dit(latents, t=timestep, **arg_ti))
        torch.cuda.empty_cache()

        if t > step_change:
            neg, _ = self.parse_output(
                self.dit(latents, t=timestep, **arg_i)
            )  # img included in null, same with official Wan-2.1
            torch.cuda.empty_cache()

            noise_pred = (
                self.config.generation.scale_a * (pos_tia - pos_ti)
                + self.config.generation.scale_t * (pos_ti - neg)
                + neg
            )
        else:
            neg, _ = self.parse_output(
                self.dit(latents, t=timestep, **arg_null)
    ...
    def forward_ta(self, latents, timestep, arg_ta, arg_t, arg_null):
        pos_ta, _ = self.parse_output(self.dit(latents, t=timestep, **arg_ta))
        torch.cuda.empty_cache()

        pos_t, _ = self.parse_output(self.dit(latents, t=timestep, **arg_t))
        torch.cuda.empty_cache()

        neg, _ = self.parse_output(self.dit(latents, t=timestep, **arg_null))
        torch.cuda.empty_cache()
    ...
    def inference(
        ...
        target_shape = (
            ...
            (F - 1) // self.vae_stride[0] + 1 + latents_ref[0].shape[1],
        ...
        with amp.autocast(dtype=torch.bfloat16), torch.no_grad(), no_sync():
            ...
            zero_vae = self.zero_vae[
                :, : (target_shape[1] - latents_ref[0].shape[1])
            ].to(device=get_device(), dtype=latents_ref[0].dtype)
            y_c = torch.cat([zero_vae, latents_ref[0]], dim=1)
            ...
            for _, t in enumerate(tqdm(timesteps)):
                ...
                if self.config.generation.mode == "TIA":
                    ...
                    noise_pred = self.forward_ta(
                        latents, timestep, arg_ta, arg_t, arg_null
                    )
                else:
                    raise ValueError(
                        f"Unsupported generation mode: {self.config.generation.mode}"
                    )
    ...
    def save_sample(self, *, sample: torch.Tensor, audio_path: str, itemname: str):
        ...
        if sample.ndim == 4:
            if audio_path is not None:
                tensor_to_video(
                    sample.numpy(), pathname, audio_path, fps=gen_config.fps
                )
            else:
                mediapy.write_video(
                    path=pathname,
