# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HuMo\humo\common\distributed\advanced.py
...
def init_unified_parallel(unified_parallel_size):
    ...
    for i in range(data_parallel_size):
        ...
        unified_parallel_cpu_group = dist.new_group(
            unified_parallel_ranks, backend="gloo"
        )
...
def gather_seq_scatter_heads_qkv(
    ...
    if async_op:
        ...
        qkv_tensor = SeqAllToAll.apply(
            group, qkv_tensor, scatter_dim, seq_dim, async_op
        )
...
def _all_to_all_single(
    ...
    comm = dist.all_to_all_single(
        output, x.contiguous(), group=group, async_op=async_op
    )
    ...
    if async_op:
        ...
        def wait():
            ...
            if scatter_dim == 0:
                return torch.cat(
                    output.split(x.size(0) // sp_world_size), dim=gather_dim
                )
            else:
                return output
...
def init_model_shard_cpu_group(
    sharding_strategy: str, device_mesh: Optional[Tuple] = None
):
    """
