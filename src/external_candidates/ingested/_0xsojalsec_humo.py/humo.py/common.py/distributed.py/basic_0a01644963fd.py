# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HuMo\humo\common\distributed\basic.py
            ...
            if hasattr(submodule, "freqs"):
                ...
                if isinstance(freqs, torch.Tensor) and freqs.is_meta:
                    ...
                    freqs_tensor = torch.cat(
                        ...
                        dim=1,
                    ).to(dtype=torch.cfloat, device="cpu")

                    setattr(submodule, "freqs", freqs_tensor)
                    print(
                        f"Successfully materialized freqs for {submodule.__class__.__name__}"
                    )
