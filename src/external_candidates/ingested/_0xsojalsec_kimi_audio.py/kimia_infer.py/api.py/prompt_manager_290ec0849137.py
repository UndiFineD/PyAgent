# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Kimi-Audio\kimia_infer\api\prompt_manager.py
import os
...
class KimiAPromptManager:
    def __init__(
        ...
        if os.path.exists(model_path) and os.path.exists(
            ...
            logger.info(
                f"Can not find text tokenizer in {model_path}, Loading default text tokenizer from moonshotai/Kimi-Audio-7B-Instruct"
            )
            self.text_tokenizer = AutoTokenizer.from_pretrained(
    ...
    def tokenize_message(
        ...
        if message["message_type"] == "text":
            ...
            if role == "assistant":
                ...
                kimia_content_msg.audio_append(
                    self.extra_tokens.kimia_text_blank, audio_token_loss_mask=False
                )
            ...
            kimia_content_msg.audio_extend(
                speech_tokens, is_continuous=True, audio_token_loss_mask=has_loss
            )
            kimia_content_msg.audio_append(
