# Extracted from: C:\DEV\PyAgent\.external\AskVideos-VideoCLIP\video_llama\datasets\datasets\webvid_datasets.py
"""
...
class WebvidDataset(BaseDataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_root):
        ...
        for file_name in os.listdir(ann_root):
            if file_name.endswith(".csv"):
                ...
                print(
                    df.apply(
                        lambda x: self._get_video_path(x) in self.existing_videos,
                        axis=1,
                    ).sum()
                )
                df = df[
    ...
    def _get_video_path(self, sample):
        rel_video_fp = os.path.join(
            str(sample["page_dir"]), str(sample["videoid"]) + ".mp4"
        )
