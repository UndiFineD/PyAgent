# Extracted from: C:\DEV\PyAgent\.external\AskVideos-VideoCLIP\video_llama\datasets\datasets\askyoutube_datasets.py
"""
...
from video_llama.processors import AlproVideoTrainProcessor, transforms_video
...
class AskYoutubeDataset(BaseDataset):
    ...
    def _load_video_paths(self, vis_root):
        return dict(
            [
                (path.split("/")[-1], path)
                for path in glob.glob(os.path.join(vis_root, "*"))
            ]
        )

    def _load_annotations(self, ann_root):
        captions_files = glob.glob(
            os.path.join(ann_root, "*", "chunked_captions_30s.json")
        )
        all_captions = []
        for captions_file in captions_files:
            ...
            for i, chunk in enumerate(captions):
                ...
                cap = ""
                for seg in chunk:
                    if "dDurationMs" not in seg or (
                        "aAppend" in seg and seg["aAppend"]
                    ):
                        continue
                ...
                if not self._check_video_chunk_exists(video_id, i):
                    continue
                all_captions.append(
                    {"video_id": video_id, "seq_num": i, "caption": cap}
                )
...
class AskYoutubeInstructDataset(BaseDataset):
    def __init__(
        ...
        captions_files = glob.glob(
            os.path.join(
                ann_root, "*", "qa_captions_30s_vicuna1.5_8bit_13b_instruct.json"
            )
        )
        for captions_file in captions_files:
            video_id = os.path.dirname(captions_file).split("/")[-1]
    ...
    def create_conversation_list(self, sample_dict, use_transcripts=True):
        ...
        if use_transcripts:
            prompt = (
                f"Answer the questions given the following transcript: {transcript}\n\n"
            )
        else:
        ...
        for _ in range(num_retries):
            ...
            try:
                ...
                if video_path is None:
                    ...
                    continue
                conversation_list = self.create_conversation_list(
                    sample_dict, self.use_transcripts
                )
                ...
                print(e)
                print(
                    f"Failed to load examples with video: {video_path}. Exception"
                    f"Will randomly sample an example as a replacement."
                )
                index = random.randint(0, len(self) - 1)
                continue

            if video is None or video.size() != torch.Size(
                [3, self.vis_processor.n_frms, 224, 224]
            ):
                print(
                    f"Failed to load examples with video: {video_path}. Video is None. "
                    f"Will randomly sample an example as a replacement."
                )
