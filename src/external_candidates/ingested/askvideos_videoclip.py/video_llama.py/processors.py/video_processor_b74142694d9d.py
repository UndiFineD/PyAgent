# Extracted from: C:\DEV\PyAgent\.external\AskVideos-VideoCLIP\video_llama\processors\video_processor.py
"""
...
def split_clip(vlen, frame_rate, clip_len, n_frms):
    ...
    for i in range(num_clips):
        ...
        if len(frames_in_clip) == 0:
            ...
            # Sample n_frms.
            sampled_frames_per_clip = (
                np.arange(start_frame, end_frame, len(frames_in_clip) / n_frms)
                .astype(int)
                .tolist()
            )
...
def load_video_start_end(
    ...
    if sampling == "uniform":
        ...
        indices_h = sorted(
            np.random.choice(
                range(start, start + (end - start) // 2), half_n_frms, replace=False
            )
        )
        indices_t = sorted(
            np.random.choice(
                range(start + (end - start) // 2, end), half_n_frms, replace=False
            )
        )
        indices = indices_h + indices_t
    else:
...
def load_video_all_frames(
    video_path, frame_sep=4, height=-1, width=-1, return_msg=False
):
    decord.bridge.set_bridge("torch")
    vr = VideoReader(uri=video_path, height=height, width=width)
