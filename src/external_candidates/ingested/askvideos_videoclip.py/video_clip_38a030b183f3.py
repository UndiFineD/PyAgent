# Extracted from: C:\DEV\PyAgent\.external\AskVideos-VideoCLIP\video_clip.py
import glob
import os
import time
...
from video_llama.processors.video_processor import (
    load_video,
    load_video_long,
    load_video_long_subset,
    load_video_start_end,
)
...
def upload_video_long_itg(model, prompt, video_path, vis_processor, clip_len=10):
    ...
    for clip in clips:
        ...
        video_query_tokens, past_key_values, frame_atts = (
            model.encode_videoQformer_visual(clip, output_for_itg=True)
        )
        ...
        print(
            ...
            frame_atts.size(),
            video_query_tokens.size(),
        )
        clip = embed_text_itg(
            model,
            prompt,
            past_key_values,
            query_tokens=video_query_tokens,
            video_atts=frame_atts,
        )
        print(clip.size())
...
def upload_video_itm(model, prompt, video_path, vis_processor):
    ...
    video_query_tokens, frame_hidden_state, frame_atts = (
        model.encode_videoQformer_visual(video, output_for_itm=True)
    )
    video_emb = embed_text_itm(
        model,
        prompt,
        frame_hidden_state,
        query_tokens=video_query_tokens,
...
def upload_video_long_itm(model, prompt, video_path, vis_processor, clip_len=10):
    ...
    for clip in clips:
        ...
        # TODO: ITM Here.
        video_query_tokens, frame_hidden_state, frame_atts = (
            model.encode_videoQformer_visual(clip, output_for_itm=True)
        )
        clip = embed_text_itm(
            model,
            prompt,
...
def upload_video_long_subsets(
    model, video_path, vis_processor, subsets, orig_clip_len, clip_len=10
):
    # if not isinstance(video_path, str) and not isinstance(video_path, bytes):
    #    raise NotImplementedError("Non-string or non-bytes video paths are not implemented.")
...
def upload_image_itg(model, prompt, image_path, vis_processor):
    ...
    video_query_tokens, past_key_values, frame_atts = model.encode_videoQformer_visual(
        image, output_for_itg=True
    )
    image_emb = embed_text_itg(
        model,
        prompt,
        past_key_values,
        query_tokens=video_query_tokens,
...
def upload_image_itm(model, prompt, image_path, vis_processor):
    ...
    image = vis_processor(raw_image).unsqueeze(0).unsqueeze(2).to(DEVICE)
    video_query_tokens, frame_hidden_state, frame_atts = (
        model.encode_videoQformer_visual(image, output_for_itm=True)
    )
    image_emb = embed_text_itm(
        model,
        prompt,
        frame_hidden_state,
        query_tokens=video_query_tokens,
...
def get_all_video_embeddings_start_end(
    video_path, model, vis_processor, start_seconds, end_seconds
):
...
def get_all_video_embeddings_long_video_itg(
    video_path, prompt, model, vis_processor, clip_len=10
):
...
def get_all_video_embeddings_long_video_itm(
    video_path, prompt, model, vis_processor, clip_len=10
):
...
def get_all_video_embeddings_long_video_subsets(
    video_path, model, vis_processor, subsets, orig_clip_len=30, clip_len=10
):
...
def embed_text_itc(model, prompt):
    ...
    embds = model.video_Qformer.bert(
        inputs.input_ids, inputs.attention_mask, return_dict=True
    ).last_hidden_state
...
def embed_text_itm(model, prompt, video_emb, query_tokens=None, video_atts=None):
    ...
    if video_atts is None:
        video_atts = torch.ones(video_emb.size()[:-1], dtype=torch.long).to(
            video_emb.device
        )
...
def rank_matches_videoq(query_video_path, video_paths, model, vis_processor):
    query_video_embs = get_all_video_embeddings(
        [query_video_path], model, vis_processor
    )
    video_embs = get_all_video_embeddings(video_paths, model, vis_processor)
