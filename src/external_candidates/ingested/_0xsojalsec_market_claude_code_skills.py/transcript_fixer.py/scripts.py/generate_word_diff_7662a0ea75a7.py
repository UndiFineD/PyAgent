# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\generate_word_diff.py
#!/usr/bin/env python3
"""
...
def get_word_diff(old, new):
    ...
    for tag, i1, i2, j1, j2 in s.get_opcodes():
        ...
        if tag == "equal":
            ...
            result.append(
                f'<del class="word-del" title="删除: {html.escape(old_part)}">{html.escape(old_part)}</del>'
            )
        elif tag == "insert":
            result.append(
                f'<ins class="word-ins" title="添加: {html.escape(new_part)}">{html.escape(new_part)}</ins>'
            )
        elif tag == "replace":
            result.append(
                f'<span class="word-change"><del class="word-del" title="原文">{html.escape(old_part)}</del> → <ins class="word-ins" title="修正后">{html.escape(new_part)}</ins></span>'
            )
        ...
        if old_line.strip() != new_line.strip():
            ...
            if i - last_change_idx > context_lines + 1:
                if last_change_idx > 0:
                    html_parts.append(
                        '<div style="text-align: center; color: #999; margin: 20px 0; font-size: 18px;">⋯ ⋯ ⋯</div>\n'
                    )

            # Show changed line
            diff_html = get_word_diff(old_line, new_line)
            html_parts.append(
                f'<div class="diff-line changed"><span class="line-number">第 {i} 行</span>{diff_html}</div>\n'
            )
            last_change_idx = i
            ...
            html_parts.append(
                f'<div class="diff-line unchanged"><span class="line-number">第 {i} 行</span>{escaped}</div>\n'
            )
...
def main():
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python generate_word_diff.py <original_file> <corrected_file> [output_file]"
        )
        print("\nExample:")
    ...
    total_changes = sum(
        1
        for old, new in zip(original_lines, corrected_lines)
    ...
    try:
        ...
        print(
            f"📊 共修改了 {total_changes} 行，占总行数的 {total_changes/len(original_lines)*100:.1f}%"
        )
