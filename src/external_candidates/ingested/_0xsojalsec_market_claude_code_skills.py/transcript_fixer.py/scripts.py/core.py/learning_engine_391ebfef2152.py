# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\core\learning_engine.py
#!/usr/bin/env python3
"""
...
class LearningEngine:
    ...
    def analyze_and_suggest(self) -> List[Suggestion]:
        ...
        patterns = {k: v for k, v in patterns.items() if k not in rejected}
    ...
    def reject_suggestion(self, from_text: str, to_text: str) -> None:
        ...
        with self._file_lock(self.pending_lock, "reject suggestion (pending)"):
            ...
            if removed > 0:
                logger.info(
                    f"Removed {removed} suggestions from pending: {from_text} → {to_text}"
                )
    ...
    def _calculate_confidence(self, occurrences: List[Dict]) -> float:
        ...
        # Weighted average
        confidence = (
            0.5 * frequency_score + 0.3 * consistency_score + 0.2 * recency_score
        )
    ...
    def _save_suggestions_unlocked(
        self, suggestions: List[Dict], filepath: Path
    ) -> None:
        """
    ...
    def _save_rejected_unlocked(self, rejected: set) -> None:
        ...
        data = {
            "rejected": [
                {"from": from_text, "to": to_text} for from_text, to_text in rejected
            ]
        }
    ...
    def analyze_and_auto_approve(self, changes: List, domain: str = "general") -> Dict:
        ...
        for (from_text, to_text), occurrences in patterns.items():
            ...
            if (
                frequency >= self.AUTO_APPROVE_FREQUENCY
                and avg_confidence >= self.AUTO_APPROVE_CONFIDENCE
            ):

                if self.correction_service:
                    try:
                        self.correction_service.add_correction(
                            from_text, to_text, domain
                        )
        ...
        if total_dict_covered > 0:
            ...
            stats["savings_potential"] = (
                f"{savings_pct}% of current errors now handled by dictionary (free)"
            )
        ...
        with self._file_lock(self.auto_approved_lock, "save auto-approved"):
            ...
            if self.auto_approved_file.exists():
                with open(self.auto_approved_file, "r", encoding="utf-8") as f:
                    ...
                    if content:
                        data = json.load(
                            json.loads(content) if isinstance(content, str) else f
                        )
            ...
            logger.info(
                f"Saved {len(patterns)} auto-approved patterns (total: {len(all_patterns)})"
            )
