...
class CorrectionService:
    ...
    def __init__(
        self, repository: CorrectionRepository, rules: Optional[ValidationRules] = None
    ):
        """
        ...
        if validate_all:
            ...
            for from_text, to_text in corrections.items():
                try:
                    ...
                    logger.error(
                        f"Validation failed for '{from_text}' → '{to_text}': {e}"
                    )
                    invalid_count += 1
    ...
    def get_statistics(self, domain: Optional[str] = None) -> Dict[str, any]:
        ...
        if domain:
            ...
            corrections = self.repository.get_all_corrections(
                domain=domain, active_only=True
            )
        else:
    ...
    def _detect_conflicts(
        self, incoming: Dict[str, str], existing: Dict[str, str]
    ...
    def save_history(
        ...
        try:
            with self.repository._transaction() as conn:
                ...
                logger.info(
                    f"Saved correction history for {filename}: {stage1_changes + stage2_changes} total changes"
                )
