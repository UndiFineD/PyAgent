# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\core\connection_pool.py
#!/usr/bin/env python3
"""
    ...
    def __init__(
        ...
        if not self.db_path.parent.exists():
            raise FileNotFoundError(
                f"Database directory doesn't exist: {self.db_path.parent}"
            )
        ...
        with self._init_lock:
            ...
            for i in range(self.max_connections):
                try:
                    ...
                    logger.error(
                        f"Failed to create connection {i+1}: {e}", exc_info=True
                    )
            ...
            logger.info(
                f"Connection pool ready with {self.max_connections} connections"
            )
        ...
        try:
            ...
            if conn is not None:
                try:
                    ...
                    logger.debug(
                        f"Connection returned to pool (held for {duration_ms:.1f}ms)"
                    )

                except Exception as e:
                    # This should never happen, but if it does, log and close connection
                    logger.error(
                        f"Failed to return connection to pool: {e}", exc_info=True
                    )
                    try:
                        conn.close()
                    except Exception:
