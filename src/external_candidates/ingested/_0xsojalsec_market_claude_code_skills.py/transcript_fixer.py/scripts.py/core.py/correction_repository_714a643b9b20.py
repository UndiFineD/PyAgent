# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\core\correction_repository.py
#!/usr/bin/env python3
"""
...
from utils.domain_validator import ValidationError as DomainValidationError
...
class CorrectionRepository:
    ...
    def __init__(self, db_path: Path, max_connections: int = 5):
        ...
        self._pool = ConnectionPool(
            db_path=self.db_path, max_connections=max_connections
        )
    ...
    def add_correction(
        ...
        with self._transaction() as conn:
            try:
                ...
                logger.info(
                    f"Added correction ID {correction_id}: {from_text} → {to_text}"
                )
                ...
                if "UNIQUE constraint failed" in str(e):
                    ...
                    if cursor.rowcount > 0:
                        ...
                        logger.info(
                            f"Updated correction ID {correction_id}: {from_text} → {to_text}"
                        )
                        return correction_id
                    else:
                        raise ValidationError(
                            f"Correction not found: {from_text} in domain {domain}"
                        )
                raise ValidationError(f"Integrity constraint violated: {e}") from e

    def get_correction(
        self, from_text: str, domain: str = "general"
    ) -> Optional[Correction]:
    ...
    def delete_correction(
        self, from_text: str, domain: str = "general", deleted_by: Optional[str] = None
    ) -> bool:
    ...
    def bulk_import_corrections(
        ...
        logger.info(
            f"Bulk import: {inserted} inserted, {updated} updated, {skipped} skipped"
        )
