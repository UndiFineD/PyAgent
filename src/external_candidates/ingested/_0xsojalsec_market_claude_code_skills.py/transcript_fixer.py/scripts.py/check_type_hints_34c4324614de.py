# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\check_type_hints.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    for stats in all_stats:
        if stats.overall_coverage < 100.0:
            ...
            print(
                f"   Functions: {stats.functions_with_return_type}/{stats.total_functions} "
                f"({stats.function_coverage:.1f}%)"
            )
            print(
                f"   Parameters: {stats.parameters_with_type}/{stats.total_parameters} "
    ...
    overall_function_coverage = (
        (total_functions_typed / total_functions * 100)
        if total_functions > 0
        else 100.0
    )
    overall_parameter_coverage = (
        (total_parameters_typed / total_parameters * 100)
        if total_parameters > 0
        else 100.0
    )
    overall_coverage = (
        (
            (total_functions_typed + total_parameters_typed)
            / (total_functions + total_parameters)
            * 100
        )
        if (total_functions + total_parameters) > 0
        else 100.0
    )
    ...
    print(
        f"Functions with Return Type: {total_functions_typed} ({overall_function_coverage:.1f}%)"
    )
    print(f"Total Parameters: {total_parameters}")
    print(
        f"Parameters with Type: {total_parameters_typed} ({overall_parameter_coverage:.1f}%)"
    )
