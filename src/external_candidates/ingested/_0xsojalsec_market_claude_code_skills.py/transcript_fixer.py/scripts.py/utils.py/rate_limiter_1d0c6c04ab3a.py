...
class RateLimitPresets:
    ...
    API_CONSERVATIVE = RateLimitConfig(
        max_requests=10, window_seconds=60.0, strategy=RateLimitStrategy.TOKEN_BUCKET
    )

    API_MODERATE = RateLimitConfig(
        max_requests=60, window_seconds=60.0, strategy=RateLimitStrategy.TOKEN_BUCKET
    )

    API_AGGRESSIVE = RateLimitConfig(
        max_requests=100, window_seconds=60.0, strategy=RateLimitStrategy.TOKEN_BUCKET
    )
    ...
    STRICT_LIMIT = RateLimitConfig(
        max_requests=100, window_seconds=60.0, strategy=RateLimitStrategy.SLIDING_WINDOW
    )
...
def get_rate_limiter(
    ...
    with _limiters_lock:
        if name not in _global_limiters:
            if config is None:
                raise ValueError(
                    f"Rate limiter '{name}' not found and no config provided"
                )
