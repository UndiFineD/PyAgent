...
class DatabaseMigrationManager:
    ...
    def _validate_migration(self, migration: Migration) -> Tuple[bool, List[str]]:
        ...
        if migration.check_function:
            try:
                with self._get_connection() as conn:
                    is_valid, validation_error = migration.check_function(
                        conn, migration
                    )
                    if not is_valid:
    ...
    def _run_migration(
        self, migration: Migration, direction: MigrationDirection, dry_run: bool = False
    ) -> None:
        """
        ...
        if dry_run:
            logger.info(
                f"[DRY RUN] Would apply {direction.value} migration {migration.version}"
            )
            return
        ...
        with self._transaction() as cursor:
            ...
            try:
                ...
                logger.info(
                    f"Successfully applied {direction.value} migration {migration.version} "
    ...
    def get_pending_migrations(self) -> List[Migration]:
        ...
        all_versions = sorted(
            self.migrations.keys(), key=lambda v: tuple(map(int, v.split(".")))
        )
    ...
    def migrate_to_version(
        self, target_version: str, dry_run: bool = False, force: bool = False
    ) -> None:
        """
        ...
        current_version = self.get_current_version()
        logger.info(
            f"Current version: {current_version}, Target version: {target_version}"
        )
    ...
    def _migrate_backward(
        ...
        for version in all_versions:
            if version <= from_version and version > to_version:
                ...
                if not migration.backward_sql:
                    raise RuntimeError(
                        f"Migration {migration.version} cannot be rolled back"
                    )
    ...
    def rollback_migration(
        self, version: str, dry_run: bool = False, force: bool = False
    ) -> None:
        """
        ...
        applied_versions = [
            m.version for m in history if m.status == MigrationStatus.COMPLETED
        ]
    ...
    def get_migration_plan(
        self, target_version: str = "latest"
    ) -> List[Dict[str, Any]]:
        """
