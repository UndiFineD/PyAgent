...
class Config:
    ...
    def from_env(cls) -> Config:
        ...
        try:
            ...
            logger.warning(
                f"Invalid environment '{env_str}', defaulting to development"
            )
        ...
        config_dir = Path(
            os.getenv(
                "TRANSCRIPT_FIXER_CONFIG_DIR", str(Path.home() / ".transcript-fixer")
            )
        )

        # Database config
        db_path = Path(
            os.getenv("TRANSCRIPT_FIXER_DB_PATH", str(config_dir / "corrections.db"))
        )
        ...
        resources = ResourceLimits(
            max_concurrent_tasks=int(
                os.getenv("TRANSCRIPT_FIXER_MAX_CONCURRENT", "10")
            ),
    ...
    def from_file(cls, config_path: Path) -> Config:
        ...
        database = DatabaseConfig(
            path=Path(
                db_data.get(
                    "path", str(Path.home() / ".transcript-fixer" / "corrections.db")
                )
            ),
        ...
        config_dir = Path(
            paths_data.get("config_dir", str(Path.home() / ".transcript-fixer"))
        )
        paths = PathConfig(
            config_dir=config_dir,
    ...
    def validate(self) -> tuple[list[str], list[str]]:
        ...
        if not self.database.path.parent.exists():
            errors.append(
                f"Database directory doesn't exist: {self.database.path.parent}"
            )
        ...
        if self.resources.max_concurrent_tasks > 50:
            warnings.append(
                f"max_concurrent_tasks is very high: {self.resources.max_concurrent_tasks}"
            )
