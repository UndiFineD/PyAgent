# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\utils\path_validator.py
#!/usr/bin/env python3
"""
...
class PathValidator:
    ...
    def __init__(
        ...
        self.allowed_read_extensions = (
            allowed_read_extensions or ALLOWED_READ_EXTENSIONS
        )
        self.allowed_write_extensions = (
            allowed_write_extensions or ALLOWED_WRITE_EXTENSIONS
        )
    ...
    def _check_symlink(self, path: Path) -> None:
        ...
        if not self.allow_symlinks and path.is_symlink():
            raise PathValidationError(f"Symlink detected and not allowed: {path}")
    ...
    def _validate_extension(
        self, path: Path, allowed_extensions: Set[str], operation: str
    ) -> None:
        """
    ...
    def validate_output_path(self, path_str: str, create_parent: bool = True) -> Path:
        ...
        if not self.allow_symlinks:
            ...
            while current != current.parent:
                if current.is_symlink():
                    raise PathValidationError(f"Symlink in path hierarchy: {current}")
                current = current.parent
...
if __name__ == "__main__":
    ...
    try:
        result = validator.validate_output_path(
            str(Path.home() / "Documents" / "output.html")
        )
        print(f"✓ Valid: {result}\n")
    except PathValidationError as e:
        print(f"✗ Failed: {e}\n")
