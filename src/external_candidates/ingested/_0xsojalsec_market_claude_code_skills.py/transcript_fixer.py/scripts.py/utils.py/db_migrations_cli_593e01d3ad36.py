...
class DatabaseMigrationCLI:
    ...
    def cmd_status(self, args) -> None:
        ...
        try:
            ...
            print(
                f"Total Migrations Applied: {len([h for h in history if h.status == MigrationStatus.COMPLETED])}"
            )
            ...
            if history:
                ...
                for i, record in enumerate(history[:5]):
                    ...
                    print(
                        f"  {status_icon} {record.version}: {record.name} ({record.status.value})"
                    )
    ...
    def cmd_rollback(self, args) -> None:
        ...
        try:
            ...
            if not args.yes and not dry_run:
                response = input(
                    "⚠️  WARNING: Rollback may cause data loss. Continue? (y/N): "
                )
                if response.lower() != "y":
    ...
    def cmd_plan(self, args) -> None:
        ...
        try:
            ...
            # Safety validation
            is_safe, issues = self.migration_manager.validate_migration_safety(
                target_version
            )
            if is_safe:
