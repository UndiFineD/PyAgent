...
class ConcurrencyManager:
    ...
    def __init__(self, config: ConcurrencyConfig = None):
        ...
        logger.info(
            f"ConcurrencyManager initialized: max_concurrent={self.config.max_concurrent}"
        )
    ...
    def _adjust_concurrency(self) -> None:
        ...
        with self._response_times_lock:
            ...
            if avg_time > target_time * 1.5:
                ...
                new_concurrency = max(
                    self.config.min_concurrent, current_concurrency - 1
                )
    ...
    async def acquire(self, timeout: Optional[float] = None):
        ...
        try:
            ...
            raise asyncio.TimeoutError(
                f"Operation timed out after {elapsed:.1f}s " f"(timeout: {timeout}s)"
            )
