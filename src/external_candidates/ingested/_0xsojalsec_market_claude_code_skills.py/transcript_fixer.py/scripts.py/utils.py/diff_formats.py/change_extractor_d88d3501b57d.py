# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\utils\diff_formats\change_extractor.py
#!/usr/bin/env python3
"""
            ...
            changes.append(
                {
                    ...
                    "context_after": "".join(
                        original_words[i2 : min(len(original_words), i2 + 5)]
                    ),
                }
            )
        elif tag == "delete":
            ...
            changes.append(
                {
                    ...
                    "fixed": fixed_text,
                    "context_before": (
                        "".join(fixed_words[max(0, j1 - 5) : j1]) if j1 > 0 else ""
                    ),
                    "context_after": "".join(
                        fixed_words[j2 : min(len(fixed_words), j2 + 5)]
                    ),
                }
            )
...
def generate_change_summary(changes: list[dict]) -> str:
    ...
    for i, change in enumerate(changes, 1):
        change_type = {"replace": "替换", "delete": "删除", "insert": "添加"}[
            change["type"]
        ]
