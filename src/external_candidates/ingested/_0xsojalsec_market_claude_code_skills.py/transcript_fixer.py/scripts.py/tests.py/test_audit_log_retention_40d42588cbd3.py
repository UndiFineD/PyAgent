...
def test_default_retention_policies(retention_manager):
    ...
    assert (
        policies["correction"].critical_action_retention_days
        == RetentionPeriod.COMPLIANCE_SOX.value
    )
...
def test_retention_policy_validation():
    ...
    policy = RetentionPolicy(
        entity_type="test", retention_days=30, strategy=CleanupStrategy.ARCHIVE
    )
    ...
    with pytest.raises(ValueError, match="retention_days must be -1"):
        RetentionPolicy(
            entity_type="test", retention_days=-5, strategy=CleanupStrategy.DELETE
        )
...
def test_cleanup_strategy_archive(test_db, retention_manager):
    ...
    archive_files = list(
        retention_manager.archive_dir.glob("audit_log_suggestion_*.json.gz")
    )
...
def test_cleanup_strategy_anonymize(test_db, retention_manager):
    ...
    # Override policy
    retention_manager.default_policies["correction"].strategy = (
...
def test_critical_action_extended_retention(test_db, retention_manager):
    ...
    insert_audit_log(
        test_db, "delete_correction", "correction", days_ago=400
    )  # Critical
    ...
    retention_manager.default_policies["correction"].critical_action_retention_days = (
        2555  # 7 years (SOX)
    )
    retention_manager.default_policies["correction"].strategy = CleanupStrategy.DELETE
...
def test_archive_creation_and_compression(test_db, retention_manager):
    ...
    # Check archive file
    archive_files = list(
        retention_manager.archive_dir.glob("audit_log_correction_*.json.gz")
    )
    assert len(archive_files) == 1
...
def test_restore_from_archive(test_db, retention_manager):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id FROM audit_log WHERE entity_type = 'correction' ORDER BY id"
    )
...
def test_dry_run_mode_no_changes(test_db, retention_manager):
    ...
    results = retention_manager.cleanup_expired_logs(
        entity_type="correction", dry_run=True
    )
...
def test_dry_run_mode_archive_strategy(test_db, retention_manager):
    ...
    # Run cleanup in dry-run mode
    results = retention_manager.cleanup_expired_logs(
        entity_type="suggestion", dry_run=True
    )
...
def test_transaction_rollback_on_archive_failure(
    test_db, retention_manager, monkeypatch
):
...
def test_permanent_retention_skipped(test_db, retention_manager):
    ...
    for i in range(3):
        insert_audit_log(
            test_db, "migration_applied", "migration", days_ago=3000
        )  # 8+ years old
