# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\tests\test_domain_validator.py
#!/usr/bin/env python3
"""
...
class TestCorrectionInputsValidation:
    ...
    def test_text_too_long(self):
        ...
        with pytest.raises(ValidationError, match="too long"):
            validate_correction_inputs(
                from_text=long_text, to_text="test", domain="general", source="manual"
            )
...
class TestSecurityScenarios:
    ...
    def test_sql_injection_via_from_text(self):
        ...
        result = validate_correction_inputs(
            from_text=malicious_from, to_text="safe", domain="general", source="manual"
        )
    ...
    def test_dos_via_long_input(self):
        ...
        with pytest.raises(ValidationError, match="too long"):
            validate_correction_inputs(
                from_text=dos_text, to_text="test", domain="general", source="manual"
            )
