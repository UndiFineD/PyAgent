...
def check_ssl_configuration(
    ...
    try:
        ...
        if ssl_mode == "off":
            issues.append(
                "⚠️  SSL is disabled - visitors will see 'Not Secure' warnings"
            )
...
def check_dns_records(
    ...
    try:
        ...
        if proxied_records:
            issues.append(
                f"✅ Found {len(proxied_records)} proxied record(s) (Cloudflare CDN enabled)"
            )
...
def main():
    ...
    if len(sys.argv) < 4:
        ...
        print("\nExample:")
        print(
            "  python check_cloudflare_config.py typeof.tech user@example.com abc123..."
        )
