# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\cloudflare-troubleshooting\scripts\fix_ssl_mode.py
#!/usr/bin/env python3
"""
...
def fix_ssl_mode(zone_id: str, target_mode: str, email: str, api_key: str) -> bool:
    ...
    try:
        ...
        print(
            f"💡 Recommendation: Clear your browser cache or use incognito mode to test"
        )
...
def main():
    ...
    if len(sys.argv) < 5:
        print(
            "Usage: python fix_ssl_mode.py <domain> <email> <api_key> <mode> [--purge-cache]"
        )
        print("\nSSL Modes:")
        print(
            "  flexible - Cloudflare → Origin uses HTTP (can cause loops with HTTPS origins)"
        )
        print(
            "  full     - Cloudflare → Origin uses HTTPS (recommended for most origins)"
        )
        ...
        print(
            "  python fix_ssl_mode.py typeof.tech user@example.com abc123... full --purge-cache"
        )
