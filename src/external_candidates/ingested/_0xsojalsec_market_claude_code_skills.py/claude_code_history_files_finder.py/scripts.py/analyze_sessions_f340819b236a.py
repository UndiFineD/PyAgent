# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\claude-code-history-files-finder\scripts\analyze_sessions.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    if args.command == "list":
        ...
        matches = analyzer.search_sessions(sessions, args.keywords, args.case_sensitive)
        ...
        if args.show_files and stats["file_operations"]:
            ...
            for file_path, ops in list(files.items())[:20]:
                ...
                op_summary = ", ".join(f"{op}({ops.count(op)})" for op in set(ops))
                print(f"  {filename}")
