# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-YuE\inference\codecmanipulator.py
import json
...
class CodecManipulator(object):
    ...
    **mm tokenizer v0.1**
    ...
    def __init__(
        ...
        self.fps = (
            self.mm_v0_2_cfg[self.codec_type]["fps"]
            if "fps" in self.mm_v0_2_cfg[self.codec_type]
            else None
        )

        self.quantizer_begin = quantizer_begin if quantizer_begin is not None else 0
        self.n_quantizer = (
            n_quantizer if n_quantizer is not None else self.num_codebooks
        )
    ...
    def offset_tok_ids(self, x, global_offset=0, codebook_size=2048, num_codebooks=4):
        ...
        if isinstance(codebook_size, int):
            ...
            for i, cs in enumerate(codebook_size):
                assert (
                    x[i].max() < cs
                ), f"max(x)={x[i].max()}, codebook_size={cs}, layer_id={i}"
        else:
            raise ValueError(f"codebook_size={codebook_size}")
        assert x.min() >= 0, f"min(x)={x.min()}"
        assert (
            x.shape[0] == num_codebooks or x.shape[0] == self.n_quantizer
        ...
        for k in range(
            self.quantizer_begin, quantizer_end
        ):  # k: quantizer_begin to quantizer_end - 1
            if isinstance(codebook_size, int):
    ...
    def unoffset_tok_ids(self, x, global_offset=0, codebook_size=2048, num_codebooks=4):
        ...
        if isinstance(codebook_size, int):
            assert (
                x.max() < global_offset + codebook_size * num_codebooks
            ), f"max(x)={x.max()}, codebook_size={codebook_size}"
        elif isinstance(codebook_size, list):
            assert x.max() < global_offset + sum(
                codebook_size
            ), f"max(x)={x.max()}, codebook_size={codebook_size}"
        assert (
            x.min() >= global_offset
        ), f"min(x)={x.min()}, global_offset={global_offset}"
        assert (
            x.shape[0] == num_codebooks or x.shape[0] == self.n_quantizer
        ), f"x.shape[0]={x.shape[0]}, num_codebooks={num_codebooks}, n_quantizer={self.n_quantizer}"
    ...
    def flatten(self, x):
        if len(x.shape) > 2:
            x = x.squeeze()
        assert (
            x.shape[0] == self.num_codebooks or x.shape[0] == self.n_quantizer
    ...
    def unflatten(self, x, n_quantizer=None):
        ...
        assert (
            x.shape[0] % self.num_codebooks == 0 or x.shape[0] % self.n_quantizer == 0
        ), f"x.shape[0]={x.shape[0]}, num_codebooks={self.num_codebooks}, n_quantizer={self.n_quantizer}"
    ...
    def npy2ids(self, npy):
        ...
        assert (
            len(data.shape) == 2
        ), f"data shape: {data.shape} is not (n_codebook, seq_len)"
    ...
    def ids2npy(self, token_ids):
        ...
        if isinstance(self.codebook_size, int):
            ...
            codebook_0_range = (
                self.global_offset,
                self.global_offset + self.codebook_size[0],
            )
        assert (
            token_ids[0] >= codebook_0_range[0] and token_ids[0] < codebook_0_range[1]
