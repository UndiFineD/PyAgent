...
""""Attempts to fix a detected environment issue."""
        if issue["type"] == "missing_package":
            ...
            logging.info(fEnvironment: Attempting to install {package}...")
#             cmd_str = fpip install {package}
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install", package])
                self._record(cmd_str, "Success", provider="Shell", model="pip")
#                 return fSuccessfully installed {package}.
            except (subprocess.CalledProcessError, OSError) as e:
                self._record(cmd_str, fFailed: {str(e)}", provider="Shell", model="pip")
#                 return fFailed to install {package}: {e}
