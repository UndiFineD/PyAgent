...
class DummyRecorder:
    """A simple recorder mock for capturing agent interactions during compliance audit tests."""
    def __init__(self):
        self.interactions = []
...
class ComplianceCheck:
    """Encapsulates a single compliance check for extensibility and testability."""
    def __init__(self, name: str, check_fn: Callable[[], bool] = None, recommendation: str = ""):
        self.name = name
...
class ComplianceStandard:
    """Encapsulates a compliance standard and its checks for modularity and TDD."""
    def __init__(self, name: str, checks: List[ComplianceCheck]):
        self.name = name
        self.checks = checks
    ...
    def run(self) -> Dict[str, Any]:
        ...
        score = 100 * (len(results) - len(failed)) / len(results) if results else 0
        return {
            "standard": self.name,
            "score": score,
