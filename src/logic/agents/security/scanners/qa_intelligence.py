...
class QAIntelligence:
    ...
    def get_e2e_system_prompt() -> str:
        """Returns a high-quality system prompt for a testing agent (ported from BrowserUse/qa-use)."""
        return """
...
{ "status": "pass" | "failing", "steps": [ { "id": string, "description": string } ] | null, "error": string | null }
"""

    @staticmethod
    def get_test_case_template() -> str:
        """Returns a template for defining E2E test cases."""
        return """
<test>
  <steps>
    <step id="id1" label="1">Go to {url}</step>
    <step id="id2" label="2">Interact with {element}</step>
  </steps>
