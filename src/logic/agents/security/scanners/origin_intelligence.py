...
class OriginRecon:
    ...
    async def check_ip_origin(self, domain: str, ip: str, session: aiohttp.ClientSession) -> Dict[str, Any]:
        ...
        """
        results = {"ip": ip, "reasons": [], "is_origin": False}
        ...
        try:
            ...
            # Direct IP request
            headers = {"Host": domain}
            async with session.get(f"http://{ip}", headers=headers, timeout=5, ssl=False, allow_redirects=False) as resp:
...
async def find_origin(domain: str):
    async with aiohttp.ClientSession() as session:
        recon = OriginRecon()
        subs = await recon.get_subdomains_from_crt(domain, session)
