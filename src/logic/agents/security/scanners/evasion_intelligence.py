...
class EvasionIntelligence:
    ...
    def get_self_deletion_techniques() -> Dict[str, str]:
        ...
        return {
            ...
            "schtasks": (
                "schtasks /create /tn Melt /tr \"cmd /c del %s\" "
                "/sc once /st 00:00 /f && schtasks /run /tn Melt"
            )
        }
    ...
    def get_anti_forensics_techniques() -> Dict[str, str]:
        ...
        return {
            ...
            "gutmann_shred": (
                "35-pass overwrite with specific patterns (0x55, 0xAA, 0x92, etc.) "
                "to defeat magnetic force microscopy"
            ),
            "file_melt_nt": (
                "Deleting the currently executing binary from disk via NTFS stream tricks "
                "or pending move rename"
            ),
            "clear_shellbags": (
    ...
    def get_payload_delivery_methods() -> Dict[str, str]:
        ...
        return {
            ...
            "file_tunneling_relay": (
                "Synchronizing remote streams via shared file locks/write-offsets (File-Tunnel)"
            )
        }
