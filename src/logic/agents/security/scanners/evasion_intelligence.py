...
class EvasionIntelligence:
    ...
    def get_anti_forensics_techniques() -> Dict[str, str]:
        ...
        return {
            ...
            "gutmann_shred": (
                "35-pass overwrite with specific patterns (0x55, 0xAA, 0x92, etc.) to defeat magnetic force microscopy"
            ),
            "file_melt_nt": (
                "Deleting the currently executing binary from disk via NTFS stream tricks or pending move rename"
            ),
            "clear_shellbags": (
                r"reg delete \"HKCU\Software\Classes\Local Settings\Software\Microsoft\Windows"
                r"\Shell\BagMRU\" /f"
            ),
            "clear_shimcache": (
                r"reg delete \"HKLM\SYSTEM\CurrentControlSet\Control\Session Manager"
    ...
    def get_waf_bypass_techniques() -> Dict[str, str]:
        ...
        return {
            ...
            "challenge_solving_relay": (
                "Delegating Cloudflare Turnstile/HCaptcha solving to an automated browser instance"
            ),
    ...
    def get_payload_delivery_methods() -> Dict[str, str]:
        ...
        return {
            ...
            "active_directory_attribute": (
                "Executing shellcode staged in user/computer attributes (e.g., thumbnailPhoto)"
            ),
    ...
    def get_mfa_bypass_techniques() -> Dict[str, str]:
        ...
        return {
            "adversary_in_the_middle_proxy": (
                "Proxying legitimate login traffic to capture session cookies after 2FA completion"
            ),
            "push_notification_fatigue": (
                "Bombarding the user with push notifications until they approve the request (MFA Bombing)"
            ),
