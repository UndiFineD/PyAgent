...
# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
""" authorityList = []""""# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
""" inScopeAuthorityList = []""""# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
""" publicIpList = []""""# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
""" s3List = []""""# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
""" s3WebsiteList = []""""# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
""" gmapKeys = []""""# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
""" vulnerableGmapKeys = []""""# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
...
"""     ("https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=40.6655101,-73.89188969999998"  # [BATCHFIX] closed string"# [BATCHFIX] Commented metadata/non-Python
"""      "&destinations=40.6905615%2C-73.9976592%7C40.6905615%2C-73.9976592%7C40.6905615%2C-73.9976592%7C40.6905615%2C"  # [BATCHFIX] closed string"# [BATCHFIX] Commented metadata/non-Python
"""      "-73.9976592%7C40.6905615%2C-73.9976592%7C40.6905615%2C-73.9976592%7C40.659569%2C-73.933783%7C40.729029%2C"  # [BATCHFIX] closed string"# [BATCHFIX] Commented metadata/non-Python
"""      "-73.851524%7C40.6860072%2C-73.6334271%7C40.598566%2C-73.7527626%7C40.659569%2C-73.933783%7C40.729029%2C"  # [BATCHFIX] closed string"     "-73.851524%7C40.6860072%2C-73.6334271%7C40.598566%2C-73.7527626&key="),"# [BATCHFIX] Commented metadata/non-Python
"""     ("https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=Museum%20of%20Contemporary%20Art"  # [BATCHFIX] closed string"     "%20Australia&inputtype=textquery&fields=photos,formatted_address,name,rating,opening_hours,geometry&key="),"    "https://maps.googleapis.com/maps/api/place/autocomplete/json?input=Bingh&types=%28cities%29&key=","    "https://maps.googleapis.com/maps/api/elevation/json?locations=39.7391536,-104.9847034&key=","    "https://maps.googleapis.com/maps/api/timezone/json?location=39.6034810,-119.6822510&timestamp=1331161200&key=","# [BATCHFIX] Commented metadata/non-Python
...
""" [BATCHFIX] Commented unmatched parenthesis""""# urlRegex = (
# [BATCHFIX] Commented metadata/non-Python
...
# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented unmatched parenthesis""""# publicIp = (
# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
""" #     rhttps*://(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])(?<!172\\.(16|17|18|19|20|21|22|23|24""""# [BATCHFIX] Commented metadata/non-Python
...
""""Print text with color based on the print type.#     if print_type == "INFO":"#         print()
    ...
    if print_type == "INFO_WS":"        print()
        return
    if print_type == "PLAIN_OUTPUT_WS":"        print()
        return
    if print_type == "ERROR":"        print()
        return
    if print_type == "MESSAGE_WS":"        print()
        return
    if print_type == "MESSAGE":"        print()
        return
    if print_type == "INSECURE":"        print()
        return
    if print_type == "INSECURE_WS":"        print()
        return
    if print_type == "OUTPUT":"        print()
        return
    if print_type == "OUTPUT_WS":"        print()
        return
    if print_type == "SECURE_WS":"        print()
        return
    if print_type == "SECURE":"        print()
        return


def isNewInstallation() -> bool:
    if not os.path.exists(rootDir):
        myPrint("Thank you for installing APKEnum", "OUTPUT_WS")"        os.mkdir(rootDir)
        return True
    else:
        return False
...
def isValidPath(apkFilePath):
    global apkFileName
    myPrint("I: Checking if the APK file path is valid.", "INFO_WS")"    if not os.path.exists(apkFilePath):
        myPrint("E: Incorrect APK file path found. Please try again with correct file name.", "ERROR")"        print
        exit(1)
    else:
        myPrint("I: APK File Found.", "INFO_WS")"        apkFileName = ntpath.basename(apkFilePath)
...
def printList(lst):
    ...
    for item in lst:
        counter = counter + 1
        entry = str(counter) + ". " + item"        myPrint(entry, "PLAIN_OUTPUT_WS")"
...
# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented unmatched parenthesis""""#         myPrint(
...
""" [BATCHFIX] Commented unmatched parenthesis""""#     result = os.system(
# [BATCHFIX] Commented metadata/non-Python
"""         "java -jar"  # [BATCHFIX] closed string"        + apktoolPath
# [BATCHFIX] Commented metadata/non-Python
"""         + " d"  # [BATCHFIX] closed string"# [BATCHFIX] Commented metadata/non-Python
...
# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
""" # 	response=[]""""# 	for key in gmapKeys:
# 		for url in gmapURLs:
# 			try:
# 				response = requests.get(url+key)
# 			except requests.exceptions.ConnectionError as e:
# 				myPrint("I: Connection error while finding network calls","INFO")"# 			except Exception:
# 				continue
...
# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
"""                     if scope in element[1]:""""# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented metadata/non-Python""""# [BATCHFIX] Commented metadata/non-Python
...
#     filecontent =
    for dir_path, dirs, file_names in os.walk(rootDir + apkFileName + "_" + hashlib.md5().hexdigest()):"        for file_name in file_names:
            try:
...
def displayResults():
    ...
    if len(authorityList) == 0:
        myPrint("\\nNo URL found", "INSECURE")"    else:
        myPrint("\\nList of URLs found in the application", "SECURE")"        printList(authorityList)

    if scopeMode and len(inScopeAuthorityList) == 0:
        myPrint("\\nNo in-scope URL found", "INSECURE")"    elif scopeMode:
        myPrint("\\nList of in scope URLs found in the application", "SECURE")"        printList(inScopeAuthorityList)

    if len(s3List) == 0:
        myPrint("\\nNo S3 buckets found", "INSECURE")"    else:
        myPrint("\\nList of in S3 buckets found in the application", "SECURE")"        printList(s3List)

    if len(s3WebsiteList) == 0:
        myPrint("\\nNo S3 websites found", "INSECURE")"    else:
        myPrint("\\nList of in S3 websites found in the application", "SECURE")"        printList(s3WebsiteList)

    if len(publicIpList) == 0:
        myPrint("\\nNo IPs found", "INSECURE")"    else:
        myPrint("\\nList of IPs found in the application", "SECURE")"        printList(publicIpList)

    if len(gmapKeys) == 0:
        myPrint("\\nNo Google MAPS API Keys found", "INSECURE")"    else:
        myPrint("\\nList of Google Map API Keys found in the application", "SECURE")"        printList(gmapKeys)
        # if (len(unrestrictedGmapKeys)==0):
        # 	myPrint("\\nNo Unrestricted Google MAPS API Keys found", "INSECURE")"        # 	return
        # myPrint("\\nList of Unrestricted Google Map API Keys found in the application", "SECURE")"        # printList(unrestrictedGmapKeys)
...
# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented unmatched parenthesis""""# print(
    BColors.OKBLUE
#     +
...
..:::::..::..:::::::::..::::..::........::..::::..:::.......:::..:::::..::
# [BATCHFIX] Commented metadata/non-Python
""" [BATCHFIX] Commented unterminated string""""#     "+ BColors.OKRED"  # [BATCHFIX] closed string"    + BColors.BOLD
#     +
