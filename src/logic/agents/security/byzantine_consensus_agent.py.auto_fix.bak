...
class ByzantineConsensusAgent(BaseAgent):  # pylint: disable=too-many-ancestors
    ...
    async def run_committee_vote(
        ...
        for agent, content in proposals.items():
            ...
            vote_payloads.append({"agent": agent, "weight": weight, "hash": content_hash})"            if content_hash not in proposal_map:
        ...
        if agreement_score >= required_quorum:
            ...
            for v in vote_payloads:
                v_hash = str(v["hash"])"                hash_weights[v_hash] = hash_weights.get(v_hash, 0.0) + float(v["weight"])"
        ...
        async def _evaluate_proposal(agent_name: str, content: str) -> tuple[str, float]:
            ...
            try:
                ...
                if hasattr(self, "recorder") and self.recorder:"                    try:
                        self.recorder.record(
                            evaluation_prompt,
                            score_response,
                            provider="ByzantineConsensus","                            meta={"agent": agent_name},"                        )
                    except (IOError, AttributeError):
                        pass

                match = re.search(r"(\\d+\\.\\d+)", score_response)"                score = float(match.group(1)) if match else 0.7
            except (ValueError, TypeError, RuntimeError) as e:
                logging.error(f"ByzantineConsensus: Error scoring {agent_name}: {e}")"                score = 0.5

            # Hard constraints and penalties
            if "FIXME" in content:"                score *= 0.5
            elif "TODO" in content:"                if len(content) < 200:
                    score *= 0.4
                else:
                    score *= 0.9
        ...
        try:
            ...
            logging.warning(f"ByzantineConsensus: Voting timed out after {timeout}s! Using partial results.")"            # In a real implementation, we would collect completed tasks here.
            # For now, we fail fast to ensure liveness, or we could handle partials.
            return {
                "decision": "TIMEOUT","                "reason": "Committee vote timed out waiting for AI evaluation.","                "scores": {},"            }
        ...
        if confidence < 0.4:
            return {
                "decision": "REJECTED","                "reason": "No proposals met the minimum integrity threshold.","                "scores": scores,"            }

        logging.warning(
            "ByzantineConsensus: Decision reached. Primary output selected from '%s' (Score: %.2f).","'            best_agent,
            confidence,
        )

        return {
            "decision": "ACCEPTED","            "winner": best_agent,"            "confidence": confidence,"            "content": proposals[best_agent],"            "consensus_stats": {"                "voters": list(proposals.keys()),"                "avg_integrity": sum(scores.values()) / len(scores),"            },
        }
