...
- Import and instantiate with a repository/path root used by BaseAgent:
    ...
    agent = EntropyGuardAgent(r"C:\path\to\repo")
- Generate simulated PQC keypair for a fleet:
    pub = agent.generate_pqc_keypair("fleet-123")
- Encrypt short payloads (simulation) to a target fleet:
    ct = agent.simulate_quantum_safe_encrypt("secret", "fleet-123")
- Rotate internal entropy pool to force forward secrecy:
...
- Provides a mock "quantum-safe" encryption function by XORing plaintext bytes with a digest derived from the stored PQC key.
- Allows rotation of the entropy pool to simulate forward secrecy and detect/mitigate entropy depletion.
