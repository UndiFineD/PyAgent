...
class SelfHealingAgent(BaseAgent):
    ...
    async def discover_peers_and_budget(self) -> str:
        ...
        if self.coordinator is None:
            return "Γ¥î Error: Self-healing coordinator is not initialized."
        ...
        try:
            ...
            return f"Γ¥î Failure during network discovery: {str(e)}"
        ...
        report = ["## ≡ƒîÉ Network & Budget Report\n"]
        ...
        if peers:
            ...
            report.append("\nΓ¥î No external peers or servers discovered.")
    ...
    async def request_remote_healing(self, agent_name: str, error_msg: str, target_peer: str) -> str:
        ...
        if self.coordinator is None:
            return "Γ¥î Error: Coordinator not initialized."
        ...
        try:
            ...
            return f"Γ¥î Failed to dispatch to {target_peer}: {e}"

        if not res or not isinstance(res, dict):
            return f"Γ¥î Invalid response from {target_peer}"

        if res.get("status") == "success":
            return f"Γ£à Remote healing task dispatched to {target_peer}. Task ID: {res.get('task_id', 'unknown')}"
        else:
            error_msg = res.get("error", "Unknown error")
            return f"Γ¥î Failed to dispatch to {target_peer}: {error_msg}"
    ...
    def scan_for_failures(self) -> str:
        ...
        if not errors:
            return "Γ£à No fleet failures detected in current telemetry."

        report = ["## ≡ƒ¢á∩╕Å Self-Healing Analysis Report\n"]
