...
"""
        ...
        for name, path_str in current_adapters:
            path = Path(path_str)
            if not path.exists():
                logging.warning(fModelRegistry: Adapter '{name}' path '{path_str}' is missing. Healing...")
                del self.adapter_registry[name]
                self.unhealthy_entries.add(name)
                healed_count += 1

        if healed_count > 0:
            logging.info(fModelRegistry: Self-healing complete. {healed_count} entries removed.")
        return healed_count
