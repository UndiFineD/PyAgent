...
- Intended to be run from the repository root so resource and telemetry paths resolve to the workspace

WHAT IT DOES:
- Scans a project's "improvements.txt", categorizes and prioritizes items into stability, features, and maintenance buckets using simple keyword heuristics, and emits a structured Markdown "Strategic Optimization Roadmap".
- Gathers local resource metrics via ResourceMonitor and telemetry summary via ObservabilityEngine and combines them into a concise system-health report appended to optimization analyses.
