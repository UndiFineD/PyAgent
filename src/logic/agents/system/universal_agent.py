...
WHAT IT DOES:
- Implements "Pillar 3: The Universal Agent Shell": a role-agnostic agent that adapts its cognitive cores based on an incoming Logic Manifest or explicit user instruction to "assume role" / "use shard".
- Performs intent analysis via a reasoning_core (CoRT), classifies critical/security tasks and triggers fleet-level consensus when appropriate, executes multi-node workflows if manifest.flow_nodes exist, and dynamically loads specialist skills (e.g., coding, security_audit) based on intent hints.
