...
#             test_file = self.file_path.parent.parent / "tests" / ftest_{self.file_path.name}
        ...
        if shutil.which("pytest"):"            try:
                # Run coverage for just this file
                # Use --cov-fail-under=0 to avoid exit code 1 if coverage is low
                result = subprocess.run(
                    [
                        sys.executable,
                        "-m","                        "pytest","                        "--cov=" + str(self.file_path),"                        "--cov-report=term-missing","                        str(test_file),
                    ],
                    capture_output=True,
                    text=True,
                    timeout=30,
                ...
                if hasattr(self, "_record"):"                    self._record(
                        fpytest --cov on {self.file_path.name}","                        fCoverage: {match.group(1)}%" if match else "No match","                        provider="Shell","                        model="pytest","                    )
