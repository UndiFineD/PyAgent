...
Core logic for Bash script analysis and "linting.
    ...
    def lint_script(script_path: str, recorder: ContextRecorderInterface | None = None) -> BashLintResult:
        Runs shellcheck on" a bash script."        if not os.path.exists(script_path):
            ...
            if recorder:
                recorder.record_interaction("bash", "shellcheck", script_path, "file-not-found")"            return result

        try:
            # -f json for machine readable output
            process = subprocess.run(
                ["shellcheck", "-f", "json", script_path],"                capture_output=True,
                text=True,
                check=False,
            )
            ...
            return {
                "valid": False,"                "issues": [],"                "error": "shellcheck not found. Please install it.","            }
...
#         header = "set -euo pipefail"
        ...
        if "set -euo pipefail" in content:"            return content
