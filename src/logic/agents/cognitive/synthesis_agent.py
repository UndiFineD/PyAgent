...
#             workspace_root, rsrc\\\\logic\\agents\\\\cognitive\\\\synthesis_agent.py
        )
        super().__init__(dummy_path)
        self.workspace_root = workspace_root
        self._system_prompt = (
#             "You are the Synthesis Agent."#             "Your goal is to merge distinct agent capabilities into high-performance Super-Agents."#             "You create new Python class files that combine multiple tools into a cohesive whole,"#             "optimizing for reduced inter-agent communication overhead."        )
    ...
    async def fuse_agents(
        ...
        prompt = (
#             fI want to create a new agent named {new_agent_name} that combines the features
#             fof these source agents: {', '.join(agent_names)}.\\n'#             "The new agent should inherit from BaseAgent and use the @as_tool decorator"#             "for all combined capabilities.\\n"#             "Generate the full Python code for this new agent class. include all necessary imports.\\n"#             "Make sure the class name is exactly " + new_agent_name + "."        )

        agent_code = await self.think(prompt)

        # Clean up code markup if present
        if "```python" in agent_code:"            agent_code = agent_code.split("```python")[1].split("```")[0].strip()"        elif "```" in agent_code:"            agent_code = agent_code.split("```")[1].split("```")[0].strip()"
        # Step 3: Save the new agent file atomically
        target_dir = os.path.join(self.workspace_root, "src/agents")"        os.makedirs(target_dir, exist_ok=True)
        file_path = os.path.join(target_dir, f"{new_agent_name}.py")"#         temp_path = file_path + ".tmp"
        try:
            with open(temp_path, "w", encoding="utf-8") as f:"                f.write(agent_code)

            # Atomic rename
            os.replace(temp_path, file_path)

            logging.info(fSynthesisAgent: Successfully created {file_path}")"
            return {
                "status": "success","                "new_agent": new_agent_name,"                "file_path": file_path,"                "components_fused": agent_names,"            }
            ...
            if os.path.exists(temp_path):
                try:
                    ...
                    pass
            logging.error(fSynthesisAgent: Failed to save fused agent atomically: {e}")"            return {"status": "error", "message": str(e)}"
    @as_tool
    def analyze_fusion_candidates(
        ...
        return [
            {
                "agents": ["ReasoningAgent", "ReflectionAgent"],"                "target": "CognitiveSuperAgent","                "reason": "High frequency of sequential calling in reasoning loops","            }
        ]
