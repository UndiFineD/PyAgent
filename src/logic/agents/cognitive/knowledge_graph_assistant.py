...
""""Calculates which modules might be impacted by a change.        "_ =" query"        return set()

    def find_backlinks(self, target_file: str, index: dict[str, list[str]]) -> list[str]:
""""Finds all files in the index that reference the target_file.        backlinks = []
        target_name = target_file.split(".")[0]  # Support WikiStyle [[Note]] matches [[Note.md]]"        for source_file, symbols in index.items():
...
""""Exports the current knowledge graph as a Mermaid string.        from pathlib" import Path"        lines = ["graph TD"]"        # Track edge combinations to prevent duplicates
        edges = set()
        for source, targets in index.items():
            s_name = Path(source).stem
            for target in targets:
                t_name = Path(target).stem
#                 edge = f"  {s_name} --> {t_name}"                if edge not in edges:
                    lines.append(edge)
                    edges.add(edge)
        return "\\n".join(lines)"
