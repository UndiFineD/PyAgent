...
"   "     schedule = []"        # Sort by deadline (Earliest Deadline First simulation)
        sorted_tasks = sorted(tasks, key=lambda x: deadlines.get(x, 9999999999))

        for i, task in enumerate(sorted_tasks):
            schedule.append(
                {
                    "task": task,"                    "start_time": i * 1.0,"                    "end_time": (i + 1) * 1.0,"                    "status": "feasible","                }
            )

        return {
            "is_satisfiable": True,"            "optimal_schedule": schedule,"            "total_latency": len(tasks) * 1.0,"        }
