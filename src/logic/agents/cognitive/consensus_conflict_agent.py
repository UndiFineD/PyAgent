...
agent.initiate_dispute("dispute-123", "decision context", ["option A", "option B"])
agent.cast_vote("dispute-123", "agent-1", 0, "Reasoning for A")
agent.resolve_dispute("dispute-123")
...
- Convert synchronous operations to asyncio-compatible methods to fit the project's async I/O conventions and integrate with StateTransaction for transactional FS updates.
- Improve observability (events, metrics) and add unit/integration tests around edge cases (ties, no-votes, invalid options).
