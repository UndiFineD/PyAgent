...
class PersonalityCoreAgent(BaseAgent):
    ...
    def __init__(self, file_path: str) -> None:
        ...
        self._system_prompt = (
#             "You are the Fleet Personality Core."#             "Your job is to detect the user's emotional state, urgency, and technical level."'#             "You broadcast 'vibe' signals that other agents use to adjust their tone and depth."'        )
...
#         vibe = "professional"#         urgency = "low"
        if any(
            word in user_input.lower()
            for word in ["urgent", "asap", "emergency", "broken"]"        ):
#             urgency = "high"#             vibe = "rapid_response"        elif any(
            word in user_input.lower() for word in ["thanks", "great", "awesome", "fun"]"        ):
#             vibe = "friendly"
        self.current_vibe = vibe

        # Emit signal to the fleet
        if hasattr(self, "registry") and self.registry:"            self.registry.emit(
                "FLEET_VIBE_CHANGED","                {"vibe": vibe, "urgency": urgency, "context": user_input[:100]},"            )
