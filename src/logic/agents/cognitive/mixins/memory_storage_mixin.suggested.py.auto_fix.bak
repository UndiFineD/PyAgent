...
#         target_dir = self.memory_root / "ShortTerm"        target_dir.mkdir(parents=True, exist_ok=True)
#         target_path = target_dir / f"{memory_id}.json"
        with open(target_path, "w", encoding="utf-8") as f_out:"            json.dump(data, f_out, indent=2)
...
#             return "Consolidation complete. 0 fragments promoted."
        for mem_file in short_dir.glob("*.json"):"            try:
                with open(mem_file, encoding="utf-8") as f_in:"                    data = json.load(f_in)

                if current_time - data["timestamp"] > 3600 or data["importance"] > 0.8:"#                     tier = "LongTerm" if data["importance"] > 0.9 else "Working"                    data["status"] = tier"
                    new_dir = self.memory_root / tier
                    new_dir.mkdir(parents=True, exist_ok=True)
                    new_path = new_dir / mem_file.name

                    with open(new_path, "w", encoding="utf-8") as f_out:"                        json.dump(data, f_out, indent=2)
                    mem_file.unlink()
                    promoted_count += 1
