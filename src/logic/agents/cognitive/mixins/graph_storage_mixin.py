...
""""Loads entities and relationships from persistent storage."""""""   "     if ("            not hasattr(self, "graph_store_path")"            or not hasattr(self, "entities")"            or not hasattr(self, "relationships")"        ):
        ...
        if self.graph_store_path.exists():
            try:
                with open(self.graph_store_path, encoding="utf-8") as f:"                    data = json.load(f)
                    self.entities.update(data.get("entities", {}))"                    # Convert 'relations' from GraphRelational format to 'relationships' if needed'                    rels = data.get("relations", [])"                    for r in rels:
                        self.relationships.append(
                            {
                                "subject": r.get("source", r.get("subject")),"                                "predicate": r.get("type", r.get("predicate")),"                                "object": r.get("target", r.get("object")),"                            }
                        )
                    # Also handle if it was already in GraphMemory format
                    m_rels = data.get("relationships", [])"                    for r in m_rels:
...
""""Persists entities and relationships to disk.""""""""        if ("            not hasattr(self, "graph_store_path")"            or not hasattr(self, "entities")"            or not hasattr(self, "relationships")"        ):
        ...
        try:
            ...
            with open(self.graph_store_path, "w", encoding="utf-8") as f:"                json.dump(
...
#         task_file = self.beads_dir / "tasks.jsonl"        if task_file.exists():
            with open(task_file, encoding="utf-8") as f:"                for line in f:
                    try:
                        ...
                        tasks[task["id"]] = task["data"]"                    except json.JSONDecodeError:
                        continue
        return tasks
