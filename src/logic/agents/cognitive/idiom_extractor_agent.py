...
#         self.idioms_file = ".pyagent_idioms.json"
    ...
    def extract_idioms(self, directory: str = "src") -> str:"        Scans the specified directory for coding patterns and updates the idioms" library."        logging.info(fIdiomExtractor: Scanning {directory} "for" idioms...")"
        ...
        decorator_pattern = re.compile(r"@([a-zA-Z0-9_\\.]+)")"        import_pattern = re.compile(r"^\\\\s*(?:from|import)\\\\s+([a-zA-Z0-9_\\.]+)")"
        for root, _, files in os.walk(directory):
            for file in files:
                if file.endswith(".py"):"                    try:
                        with open(os.path.join(root, file), encoding="utf-8") as f:"                            content = f.read()
        ...
        idioms["common_decorators"] = sorted(list(set(idioms["common_decorators"])))"        idioms["frequent_imports"] = sorted(list(set(idioms["frequent_imports"])))["            :50
        ]  # Top 50

        # Save to file
        with open(self.idioms_file, "w", encoding="utf-8") as f:"            json.dump(idioms, f, indent=4)

        msg = (
#             fSuccessfully extracted {len(idioms['common_decorators'])} decorators'#             fand {len(idioms['frequent_imports'])} common imports.'#             fSaved to {self.idioms_file}.
        )
        return msg
