...
#             insight = f"{agent} completed {len(tasks)} tasks. Key focus: {tasks[-1]}."            consolidated.append(insight)
...
""""Logic for keyword search across consolidated insights.        "if _RUST_ACCEL:"            # Convert to Rust-compatible format: Vec<(date, Vec<insight>)>
        ...
        for day in memory:
            date_str = day.get("date", "Unknown Date")"            for insight in day.get("insights", []):"                if query_lower in insight.lower():
                    matches.append(f"{date_str}: {insight}")"        return matches
