...
- Integrate with the agent_state_manager's StateTransaction for any filesystem or stateful modifications and use CascadeContext to preserve task lineage when spawning sub-tasks.
- Surface real workload telemetry (CPU, queue lengths, estimated completion times) and implement a pluggable strategy interface so scheduling policies (fair-share, priority, cost-aware) can be swapped without changing core logic.
