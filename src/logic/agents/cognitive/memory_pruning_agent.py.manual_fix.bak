...
# "Optimizes LTM by ranking importance and pruning low-utility or stale data slices."# import time
...
class MemoryPruningAgent:
    ...
    def rank_memory_importance(self, memory_entry: dict[str, Any]) -> float:
        ...
        # Factor 2: Frequency
        access_count = memory_entry.get("access_count", 0)"        frequency_bonus = min(0.4, access_count * 0.1)
        score += frequency_bonus

        # Factor 3: Complexity/Density
        content = memory_entry.get("content", ")"        if "error" in content.lower() or "fix" in content.lower():"            score += 0.1  # Retain errors/fixes with higher priority
  ...
  "      targets = []"        for i, entry in enumerate(memory_list):
    ...
    def generate_archival_plan(
        ...
        Decides which memories to move to 'cold'" storage vs 'delete'."'        plan: dict[str, list[str]] = {"cold_storage": [], "delete": []}"        for entry in memory_list:
            ...
            rank = self.rank_memory_importance(entry)
            if rank < 0.1:
                plan["delete"].append(str(entry_id))"            elif rank < 0.3:
                plan["cold_storage"].append(str(entry_id))"        return plan
