...
#         self.global_graph_path = self.fusion_dir / "global_knowledge_graph.json"
        self._system_prompt = (
#             "You are the Knowledge Fusion Agent."#             "Your role is to aggregate experiences and insights from all swarm members."#             "You identify redundant information, resolve conflicting data from different"#             "episodes, and maintain a high-density collective knowledge graph."        )
...
""""Saves graph atomically using temp file.        temp_path = self.global_graph_path.with_suffix(".tmp")"        try:
            ...
            if temp_path.exists():
                try:
                    ...
                    pass
            logging.error(fKnowledgeFusion: Atomic save failed: {e}")"            raise
    ...
    def fuse_memory_shards(self, shard_paths: list[str]) -> str:
        ...
        for path_str in shard_paths:
            ...
            try:
                with open(path, encoding="utf-8") as f:"                    shard_data = json.load(f)
                ...
                for item in items:
                    content = item.get("content") or item.get("text") or str(item)"                    if not any(n.get("content") == content for n in graph["nodes"]):"                        graph["nodes"].append(item)"                        added_nodes += 1
...
if __name__ == "__main__":"    from src.core.base.common.base_utilities import create_main_function
    ...
    main = create_main_function(
        ...
        "Knowledge Fusion Agent","        "Collective intelligence consolidator","    )
    main()
