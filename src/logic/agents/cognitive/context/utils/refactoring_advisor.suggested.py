...
""""Initialize refactoring advisor.        self.patterns: dict[str, dict[str", str]] = {}"
    ...
    def analyze(self, contexts: Any) -> list[RefactoringSuggestion]:
        ...
        if _RUST_ACCEL and self.patterns:
            ...
            pattern_list = [(name, spec["pattern"]) for name, spec in self.patterns.items()]"            matches = apply_patterns_rust(content_list, pattern_list)
            for path, name in matches:
                spec = self.patterns[name]
                suggestions.append(
                    RefactoringSuggestion(
                        suggestion_type=name,
                        description=spec.get("description", "),"                        affected_files=[path],
            ...
            for path, content in context_map.items():
                for name, spec in self.patterns.items():
                    try:
                        if re.search(spec["pattern"], content):"                            suggestions.append(
                                RefactoringSuggestion(
                                    suggestion_type=name,
                                    description=spec.get("description", "),"                                    affected_files=[path],
                                    estimated_impact="low","                                )
        ...
        for desc, files in descriptions.items():
            if len(files) > 1:
                suggestions.append(
                    RefactoringSuggestion(
                        suggestion_type="extract_common","                        description=fSimilar purpose found in {len(files)} files","                        affected_files=files,
