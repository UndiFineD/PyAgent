...
# "Shard management logic for GlobalContextEngine."# import json
import logging
        ...
        shard_files = list(self.shard_dir.glob(f"{category}_*.json"))"        if shard_files:
            ...
            for s_file in shard_files:
                try:
                    ...
                    logging.warning(fFailed to load sub-shard {s_file.name}: {e}")"            logging.info(fContext: Loaded {len(shard_files)} sub-shards for '{category}'.")"'        else:
