...
class TestHybridMemoryCore:
    ...
    async def test_delete_memory(self):
        ...
        # Verify it's gone
        assert await memory_core.recall_memory(memory_id) is None
    ...
    async def test_vector_similarity_search(self):
        ...
        memories = [
            ("Machine learning is powerful", [1.0, 0.0, 0.0]),
            ("AI algorithms are complex", [0.9, 0.1, 0.0]),
            ("Data science involves statistics", [0.0, 1.0, 0.0]),
            ("Neural networks learn patterns", [0.8, 0.2, 0.0]),
        ]
    ...
    async def test_tag_based_search(self):
        ...
        await memory_core.store_memory("Python programming", tags=["programming", "python"])
        await memory_core.store_memory("Java development", tags=["programming", "java"])
        await memory_core.store_memory("Machine learning", tags=["ml", "ai"])
        await memory_core.store_memory("Data structures", tags=["programming", "algorithms"])
    ...
    async def test_memory_associations(self):
        ...
        memory1_id = await memory_core.store_memory("Python basics")
        memory2_id = await memory_core.store_memory("Object-oriented programming")
        memory3_id = await memory_core.store_memory("Inheritance concepts")
        ...
        assert 'central_node' in graph
        assert 'related_nodes' in graph
        assert len(graph['related_nodes']) > 0

    @pytest.mark.asyncio
    async def test_hybrid_search_scoring(self):
        """Test hybrid search with multiple scoring components"""
        memory_core = HybridMemoryCore()
    ...
    async def test_memory_consolidation_simulation(self):
        """Test memory consolidation patterns (gist extraction simulation)"""
        memory_core = HybridMemoryCore()
        # Store related memories that could be consolidated
        memories = [
            "Learned about variables in Python",
            "Variables store data values",
            "Python has different variable types",
            "Variable naming conventions are important",
        ]

        stored_ids = []
        for content in memories:
            memory_id = await memory_core.store_memory(content, tags=["python", "basics"])
            stored_ids.append(memory_id)
...
class TestGraphMemoryStore:
    ...
    async def test_store_and_retrieve(self):
        ...
        # Retrieve
        retrieved = await graph_store.get_memory("test-1")
    ...
    async def test_relationships(self):
        ...
        graph_store = GraphMemoryStore()
        # Store nodes
        await graph_store.store_memory(MemoryNode("node1", "Content 1"))
        await graph_store.store_memory(MemoryNode("node2", "Content 2"))

        # Add relationship
        relation = MemoryRelation("node1", "node2", "RELATES_TO", strength=0.8)
        await graph_store.add_relation(relation)

        # Get relationships
        relations = await graph_store.get_relations("node1")
        assert len(relations) == 1
        assert relations[0].relation_type == "RELATES_TO"
        assert relations[0].target_id == "node2"
    ...
    async def test_multi_hop_traversal(self):
        ...
        await graph_store.add_relation(MemoryRelation("node1", "node2", "LEADS_TO"))
        await graph_store.add_relation(MemoryRelation("node2", "node3", "LEADS_TO"))

        # Find related memories (should find node2 and node3)
        related = await graph_store.find_related_memories("node1", max_depth=2)
        assert len(related) == 2

        # Check that we have both related nodes
        related_ids = {node.id for node, _ in related}
        assert "node2" in related_ids
        assert "node3" in related_ids
...
class TestVectorMemoryStore:
    ...
    async def test_vector_similarity(self):
        ...
        for i, embedding in enumerate(embeddings):
            node = MemoryNode(f"vec-{i}", f"Content {i}", embedding=embedding)
            memory_id = await vector_store.store_memory(node)
            stored_ids.append(memory_id)
