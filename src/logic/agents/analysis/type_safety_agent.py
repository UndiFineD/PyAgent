...
class TypeSafetyAgent(BaseAgent):  # pylint: disable=too-many-ancestors
    ...
    def run_audit(self, directory: str = "src") -> str:
        ...
        if not root.exists():
            return f"Γ¥î Directory or file not found: {directory}"
        ...
        if not all_issues:
            return "Γ£à No type safety issues detected in the analyzed scope."

        report = ["## Type Safety Analysis Report\n"]
        for filename, issues in all_issues:
            report.append(f"### {filename}")
            for issue in issues:
                icon = "≡ƒÜ¿" if issue["severity"] == "HIGH" else "ΓÜá∩╕Å"
