...
WHAT IT DOES:
- Walks the workspace tree (default "src") and parses every .py file to extract import statements into a dependency_map keyed by relative module path.
- Offers a Rust-accelerated path (rust_core.find_dependents_rust) for faster dependent lookup when the optional native extension is installed; otherwise falls back to a Python scan.
