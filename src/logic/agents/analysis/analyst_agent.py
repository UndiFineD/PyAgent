...
- Run an analysis: results = await agent.run_analysis(target_path="path/to/project", context=my_cascade_context)
- The returned `results` is expected to be a dict[str, Any] describing analysis findings.

WHAT IT DOES:
- Provides a specialized BaseAgent subclass configured as an "analyst" specialist.
- Loads persona-specific setup via the inherited asynchronous setup flow (initialize_persona).
...
class AnalystAgent(BaseAgent):
    ...
    async def _process_task(self, task_data: Any) -> Any:
        ...
        if isinstance(task_data, dict) and "target_path" in task_data:
            ...
            return await self.run_analysis(target_path, context)
