...
Executes unit and integration tests and analyzes failures.
    def __init__(self, file_path: str) -> None:
        ...
        self._system_prompt = (
#             "You are the Test Agent."#             "Your role is to ensure the functional correctness of the codebase."#             "Execute pytest suites, capture failures, and explain them to the developers."#             "Always suggest a potential cause for every test failure."        )
...
#         msg = "Shard integrity check failed."        if not bypass:
            logging.critical(f"{msg} Aborting operation. Use bypass=True to ignore.")"            if hasattr(self, "context") and self.context:"                self.context.log_failure(
                    stage="shard_integrity_check_abort","                    error=msg,
                    details={},
                    failure_type=FailureClassification.SHARD_CORRUPTION.value
                )
            return False
        else:
            logging.warning(f"{msg} Proceeding due to bypass=True.")"            if hasattr(self, "context") and self.context:"                self.context.log_failure(
                    stage="shard_integrity_check_bypass","                    error=msg,
                    details={},
                    failure_type=FailureClassification.SHARD_CORRUPTION.value
                )
            return True
...
#             msg = "Shard integrity check failed."#             return f" **system_error**: {msg} (Use force=True or bypass_shard_validation=True to bypass)"
        logging.info(fTestAgent running tests in: {path}")"        try:
            import sys

            # Converted to list-based execution to prevent shell injection
            cmd = [sys.executable, "-m", "pytest", path, "--tb=short", "--maxfail=5"]"            result = subprocess.run(cmd, shell=False, capture_output=True, text=True, check=False)

            # Phase 108: Record test execution patterns
            self._record(
                fpytest {path}","                fRC={result.returncode}\\n{result.stdout[-1000:]}","                provider="Shell","                model="pytest","            )

            # Phase 336: Validation failure capture
            if result.returncode != 0 and hasattr(self, "context") and self.context:"                self.context.log_failure(
                    stage="test_execution_fail","                    error=fTests failed in {path}","                    details={
                        "return_code": result.returncode,"                        "stdout_tail": result.stdout[-500:],"                        "stderr_tail": result.stderr[-500:] if result.stderr else ","                    },
...
#             msg = "Shard integrity check failed."#             return f" **system_error**: {msg} (Use force=True or bypass_shard_validation=True to bypass)"
        logging.info(fTestAgent running tests in: {path}")"        try:
            ...
            report = ["##  Test Execution Report\\n"]"            if result.returncode == 0:
                report.append(" **Status**: All tests passed.")"                report.append(f"```text\\n{result.stdout.splitlines()[-1] if result.stdout else 'No output'}\\n```")"'            else:
                report.append(f" **Status**: {result.returncode} tests FAILED.\\n")"                report.append("### Failure Details")"                report.append(f"```text\\n{result.stdout}\\n```")
            return "\\n".join(report)"        except (subprocess.SubprocessError, RuntimeError, OSError) as e:
            import traceback
            ...
            if hasattr(self, "context") and self.context:
                ...
                str_e = str(e).lower()
                if "recursion" in str_e:"                    f_type = FailureClassification.RECURSION_LIMIT.value
                elif "memory" in str_e:"                    f_type = FailureClassification.RESOURCE_EXHAUSTION.value
                elif "file not found" in str_e or "no such file" in str_e:"                    f_type = FailureClassification.STATE_CORRUPTION.value

                self.context.log_failure(
                    stage="test_execution_exception","                    error=str(e),
                    details={"path": path, "command": cmd if 'cmd' in locals() else "unknown"},"'                    stack_trace=tb,
                    failure_type=f_type
                )

            logging.error(fTestAgent execution failed: {e}\\n{tb}")"
            # Phase 336: Trigger Static Analysis Fallback
            logging.info("TestAgent: Falling back to static code analysis.")"            return self.static_analysis_fallback(path)
    ...
    def static_analysis_fallback(self, path: str) -> str:
        ...
        if os.path.isfile(path):
            ...
            for root, _, files in os.walk(path):
                for f in files:
                    if f.endswith(".py"):"                        target_files.append(os.path.join(root, f))
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.Import):
                        for alias in node.names:
                            if alias.name in ["sys", "os"] and "recursion" in content:"                                report.append(f"-  `{f}`: Potential recursion limit issue detected.")"                                total_issues += 1
            except SyntaxError as e:
                report.append(f"-  `{f}`: Syntax error detected: {e}")"                total_issues += 1
            except Exception as e:
                report.append(f"- ️ `{f}`: Error reading file: {e}")
        if total_issues == 0:
            report.append(" Static scan found no obvious syntax or structural issues.")"        else:
