...
- Produce a UI manifest for rendering: manifest = agent.generate_ui_manifest(task_context="show sql chart")
- Intended to be called from orchestration code that composes Fleet Dashboard views and rendering pipelines.
...
- Separate orchestration from domain logic: move layout generation and manifest rules into a UiArchitectCore class so the Agent only handles prompts/state per "Core/Agent Separation".
- Make layouts data-typed (TypedDict/dataclasses) and validate plugin names against a registry; add schema validation and unit tests for layout and manifest outputs.
