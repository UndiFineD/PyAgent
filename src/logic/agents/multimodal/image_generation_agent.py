...
- Instantiate and use asynchronously:
  ...
  agent = ImageGenerationAgent(model_name='black-forest-labs/FLUX.1-dev', device='cuda', output_dir='generated_images')
  job_id = await agent.generate_image("A serene forest in Studio Ghibli style")
  image = await agent.get_generated_image(job_id)
...
class ImageGenerationAgent(BaseAgent, TaskQueueMixin):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.device = kwargs.get('device', 'cuda' if (torch and torch.cuda.is_available()) else 'cpu')
