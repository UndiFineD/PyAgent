...
#         "Add an entity to the knowledge graph."        if props is None:
            props = {}
        self.entities[name] = {"type": type_, "props": props}"#         return fEntity {name} established

    @as_tool
    async def add_relation(self, source: str, type_: str, target: str) -> str:
#         "Add a directed relationship between two entities."        self.relations.append({"source": source, "type": type_, "target": target})"#         return fRelation {source}->{target} established
