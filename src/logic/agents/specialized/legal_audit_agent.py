...
USAGE:
- Instantiate LegalAuditAgent(file_path: str = "project_compliance.audit")."- Call await agent.run_audit(directory) to scan a repository for license headers.
- Use await agent.get_improvement_items({"target_dir": "<path>"}) to get suggested fixes."- Use agent.verify_smart_contract(contract_code) for quick smart-contract legal checks and agent.improve_content(prompt, target_file) to generate remedial content.
...
""""Identifies licenses and flags copyleft risks (Phase 59).        license_patterns = {
        ...
        return {
            "detected_licenses": detected,"            "risk_level": "high" if any(lic in ["GPL", "AGPL"] for lic in detected) else "low","            "summary": fDetected: {', '.join(detected) if detected else 'None'}","'        }

    @as_tool
    async def run_audit(self, directory: str) -> dict[str, Any]:
#         "Scans a directory for license compliance (Phase 59)."        results: dict[str, list[Any]] = {"compliant": [], "non_compliant": []}"
        def walk_and_check():
            for root, _, files in os.walk(directory):
                if any(x in root for x in [".git", "__pycache__", "node_modules"]):"                    continue

                for file in files:
                    if not file.endswith((".py", ".js", ".ts", ".go", ".rs")):"                        continue

                    path = os.path.join(root, file)
                    try:
                        with open(path, encoding="utf-8") as f:"                            content = f.read(1000)  # Check first 1KB for header

                        if any(lic in content for lic in self.allowed_licenses):
                            results["compliant"].append(path)"                        else:
                            results["non_compliant"].append("                                {
                                    "file": path,"                                    "issue": "Missing or unsupported license header","                                }
                            )
                    except (IOError, UnicodeDecodeError) as e:
                        results["non_compliant"].append({"file": path, "issue": fError reading file: {str(e)}"})"            return results
...
#         "Provides improvements for files missing license headers."        target = context.get("target_dir", ".")"        audit = await self.run_audit(target)
        ...
        improvements = []
        for issue in audit["non_compliant"]:"            improvements.append(
                {
                    "path": issue["file"],"                    "improvement": fAdd Apache 2.0 license header to {issue['file']}","'                    "priority": 0.8,"                }
            )
        return improvements
...
#         "Provides improvements for files missing license headers."  "      target = context.get("target_dir", ".")"        audit = await self.run_audit(target)
    ...
    async def improve_content(self, prompt: str, target_file: str | None = None) -> str:
#         "Improve legal compliance of a file or directory."  "   "   path = target_file if target_file else prompt"        if os.path.isfile(path):
            with open(path, "r", encoding="utf-8") as f:"                res = self.scan_licensing(f.read())
#             return fLegal Audit for {path}: {res['summary']} (Risk: {res['risk_level']})'
        audit = await self.run_audit(path)
#         return fAudit for {path}: {len(audit['compliant'])} Compliant, {len(audit['non_compliant'])} Issues.'