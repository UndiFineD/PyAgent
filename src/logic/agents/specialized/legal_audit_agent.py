...
""""Identifies licenses and flags copyleft risks (Phase 59)."""
        license_patterns = {
            "GPL": rGPL|General Public License",
            "AGPL": rAGPL|Affero General Public License",
            "MIT": rMIT License",
            "Apache": rApache License 2\.0",
        }
        detected = []
        ...
        return {
            ...
            "summary": fDetected: {', '.join(detected) if detected else 'None'}",
        }
            ...
            for root, _, files in os.walk(directory):
                ...
                for file in files:
                    ...
                    try:
                        ...
                        results["non_compliant"].append({"file": path, "issue": fError reading file: {str(e)}"})
            return results
...
#         "Provides improvements for files missing license headers.
        ...
        for issue in audit["non_compliant"]:
            improvements.append(
                {
                    "path": issue["file"],
                    "improvement": fAdd Apache 2.0 license header to {issue['file']}",
                    "priority": 0.8,
                }
            )
        return improvements
