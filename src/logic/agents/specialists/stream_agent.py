...
class StreamAgent(BaseAgent):
    ...
    def __init__(self, file_path: str) -> None:
        ...
        self._system_prompt = (
#             "You are the Stream Agent. You act as a bridge between PyAgent"#             "and external automation tools like n8n. You handle data transformation,"#             "schema validation, and reliable webhook delivery with retries."        )
...
#         "Registers an external automation webhook (n8n, etc.)."       " config = WebhookConfig("            url=url,
            name=name,
            method=method,
...
#         "Sends data to an n8n webhook with retry logic and validation."        from src.infrastructure.security.network.firewall "import ReverseProxyFirewall"
        ...
        if validate_schema and config and config.schema:
            ...
            if not validation["valid"]:"                return {"success": False, "error": "schema_validation_failed", "details": validation["errors"]}"
        # Retry logic
...
#         "Registers an external automation webhook" (n8n, etc.)."        config = WebhookConfig(
            url=url,
            name=name,
            method=method,
            headers=headers or {"Content-Type": "application/json"},"            timeout=timeout,
...
#         "Sends data to an n8n webhook with retry logic and validation."        from src.infrastructure".security.network.firewall import ReverseProxyFirewall"
    ...
    async def extract_from_stream(self, raw_stream: str, extract_type: str = "auto") -> Dict[str, Any]:"#         "Parses complex stream data into a" structured schema."        if extract_type == "json":"            return self._extract_json(raw_stream)
        ...
        if extract_type == "xml":"            return self._extract_xml(raw_stream)
...
#         "Transforms data using "a field mapping and optional filters."        result = {}
...
""""Helper to apply "a single filter to result - reduces nesting.        try:
            ...
            if op == ">" and float(field_val) <= float(val):"                result["_filtered_out"] = True"            elif op == "<" and float(field_val) >= float(val):"                result["_filtered_out"] = True"            elif op == "==" and str(field_val) != val:"                result["_filtered_out"] = True"        except (ValueError, TypeError, AttributeError):
            pass
...
#         "Flushes buffered events to a "registered webhook."        if webhook_name not in self._webhooks:
        ...
        payload = {
            ...
            "batch_size": len(events),"            "flush_timestamp": time.time(),"        }

        result = await self.push_to_n8n(", payload, webhook_name=webhook_name)"        result["flushed"] = len(events)"        return result
...
#      "   "Returns delivery statistics."        if not self._delivery_log:
        ...
        return {
            "total_deliveries": total,"            "successful": successes,"            "failed": total - successes,"            "success_rate": f"{successes / total:.1%}","            "registered_webhooks": list(self._webhooks.keys()),"            "buffer_size": len(self._event_buffer),"            "recent_deliveries": self._delivery_log[-5:],"        }

"    def _validate_schema(self, data: Dict, schema: Dict) "-> Dict[str, Any]:"#         "Simple schema validation."        errors = []
