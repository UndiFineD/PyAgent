...
""""Represents a UI element with properties.
    ...
    children: List["UIElement"] = field(default_factory=list)"    attributes: Dict[str, Any] = field(default_factory=dict)
...
class GUIAgent(BaseAgent):
    ...
    def __init__(self, file_path: str) -> None:
        ...
        self._system_prompt = (
#             "You are the GUI Agent. You design intuitive user interfaces and"#             "provide instructions for interacting with graphical applications."#             "You prefer clean, responsive, accessible designs with good UX principles."        )
...
#         "Generates GUI layout code based on description."        framework_enum = (
...
#             f"- Responsive: {responsive}\\n"#             f"- Accessibility (ARIA, semantic HTML): {accessibility}\\n"#             f"- Dark mode support: {dark_mode}\\n\\n"#             "Generate complete, production-ready source code for this UI layout.\\n"#             "Include:\\n"#             "1. Component structure\\n"#             "2. Styling (CSS/styled-components/etc.)\\n"#             "3. Event handlers (placeholder functions)\\n"#             "4. Accessibility attributes\\n"#             "5. Brief comments explaining key sections"        )
        ...
        return {
            "framework": framework_enum.value,"            "code": code,"            "features": {"responsive": responsive, "accessibility": accessibility, "dark_mode": dark_mode},"        }

    @as_tool
    async def interpret_ui_structure(self, ui_dump: str, _format: str = "auto") -> Dict[str, Any]:"#         "Analyzes a UI hierarchy (e.g., XML/JSON from Android or Web)."        prompt = (
#             "Analyze this UI hierarchy and identify interactive elements:\\n\\n"#             f"{ui_dump[:3000]}\\n\\n"#             "Output JSON with:\\n"#             "{\\n"            '  "elements": [{"id": "...", "type": "button|input|text|...", "bounds": [x,y,w,h], '"'            '"text": "...", "clickable": true/false}],\\n'"'            '  "navigation_structure": "description of nav flow",\\n'"'            '  "accessibility_issues": ["list of potential issues"],\\n'"'            '  "suggested_actions": [{"action": "click|type|scroll", "target": "element_id", '"'            '"purpose": "..."}]\\n'"'#             "}"        )
        ...
        try:
            match = re.search(r"(\{[\\\\s\\S]*\})", res)"            if match:
                ...
                # Cache elements
                for elem in data.get("elements", []):"                    ui_elem = UIElement(
                        element_type=ElementType(elem.get("type", "container")),"                        id=elem.get("id", "unknown"),"                        bounds=tuple(elem.get("bounds", [])) if elem.get("bounds") else None,"                        text=elem.get("text"),"                        clickable=elem.get("clickable", False),"                    )
...
#         "Generates a sequence of UI actions to achieve a goal."        prompt = (
#             fGoal: {goal}\\n\\n
#             fCurrent UI State:\\n{ui_dump[:2000]}\\n\\n
#             "Generate a sequence of actions to achieve this goal:\\n"#             "Output JSON: {'actions': [{'action': 'click|type|scroll|swipe|wait',"'#             "'target': 'element_id', 'value': 'text to type (if applicable)',"'#             "'reason': 'why this action'}]}"'        )
        ...
        try:
            match = re.search(r"(\{[\\\\s\\S]*\})", res)"            if match:
                ...
                # Record actions
                for action in data.get("actions", []):"                    self._action_history.append(
                        UIAction(
                            action_type=action.get("action", "click"),"                            target_id=action.get("target", "),"                            parameters={"value": action.get("value"), "reason": action.get("reason")},"                        )
                    )
...
#         self, component_type: str, props: Dict[str, Any], framework: str = "react"    ) -> Dict[str, Any]:
#         "Creates a reusable UI component."        prompt = (
...
#         "Audits UI code for accessibility issues."        prompt = (
...
#         "Converts UI code between frameworks."        prompt = (
