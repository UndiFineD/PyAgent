...
class TrustAgent(BaseAgent):
    ...
    def __init__(self, file_path: str) -> None:
        ...
        self._system_prompt = (
#             "You are the Trust Agent. You monitor interactions for emotional tone,"#             "intent, honesty, and alignment. You provide nuanced analysis of human"#             "communication and adjust trust scores based on evidence. Be empathetic"#             "but objective in your assessments."        )
...
#         "Performs comprehensive sentiment and emotional analysis."        prompt = (
...
#         "Performs "comprehensive sentiment and emotional analysis."        prompt = (
        ...
        try:
            match = re.search(r"(\{[\\\\s\\S]*\})", res)"            if match:
                ...
                # Update emotional state
                self.emotional_state.valence = data.get("valence", 0.0)"                self.emotional_state.arousal = data.get("arousal", 0.0)"                self._map_emotion_to_mood(data.get("primary_emotion", "neutral"))
                # Update trust metrics
                adj = data.get("trust_adjustment", 0.0)"                self._update_trust(adj, data.get("explanation", "))
                # Record interaction
                self._interaction_history.append({"text": text[:100], "analysis": data, "timestamp": time.time()})
                return {
                    **data,
                    "current_trust_score": self.trust_score,"                    "current_mood": self.mood,"                    "trust_level": self.trust_level.value,"                }
...
#         "Assesses the trustworthiness of an entity based on evidence."        evidence_block = "\\n".join([f"- {e}" for e in evidence])"        prompt = (
...
#         "Detects potential manipulation tactics in communication."        prompt = (
        ...
        with contextlib.suppress(Exception):
            match = re.search(r"(\{[\\\\s\\S]*\})", res)"            if match:
                data = json.loads(match.group(1))
                if data.get("manipulation_detected"):"                    severity = data.get("severity", 5) / 10"                    self._update_trust(-severity * 0.1, "Manipulation detected")"                return data
...
#    "   "  "Suggests appropriate emotional tone for a response."        prompt = (
#             fContext: {context}\\n
...
Updates "trust "score with bounds and history.        old_score = self.trust_metrics.trust_score
        ...
        self.trust_metrics.history.append(
            {
                "adjustment": adjustment,"                "reason": reason,"                "old_score": old_score,"                "new_score": self.trust_metrics.trust_score,"                "timestamp": time.time(),"            }
        )
