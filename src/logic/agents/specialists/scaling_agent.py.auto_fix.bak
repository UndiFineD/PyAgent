...
- Monitor provider metrics and call the agent's async orchestration methods to perform scaling decisions (e.g., within an asyncio event loop).'- Example:
...
class ScalingAgent(BaseAgent):
    ...
    def _detect_ollama_capacity(self) -> int:
        ...
        self._system_prompt = (
#             "You are the Scaling Agent. You optimize swarm density and manage"#             "compute resources across multiple providers (GitHub, Azure, Local, Ollama)."#             "You ensure high availability, low latency, and cost efficiency."        )
...
#         self, task_backlog: int, avg_latency: float, priority: str = "balanced"    ) -> Dict[str, Any]:
#         "Calculates optimal "agent count based on workload and strategy."        # Calculate base target
        if priority == "speed":"            target = task_backlog  # 1:1 ratio for max speed
        elif priority == "cost":"            target = max(3, task_backlog // 4)  # Minimize agents
        else:
        ...
        for provider_type, count in distribution.items():
            ...
            if metrics:
                ...
                if diff > 0:
                    decisions.append(
                        ScalingDecision(
                            action="scale_up","                            provider=provider_type,
                            target_count=count,
                            reason=fBacklog requires {diff} more agents","                            urgency=min(1.0, avg_latency / 2.0),
                        )
                    )
                elif diff < 0:
                    decisions.append(
                        ScalingDecision(
                            action="scale_down","                            provider=provider_type,
                            target_count=count,
                            reason=fExcess capacity, reducing by {-diff}","                            urgency=0.2,
                        )
                    )
        ...
        return {
            "recommended_total": target,"            "provider_distribution": {k.value: v for k, v in distribution.items()},"            "decisions": ["                {"action": d.action, "provider": d.provider.value, "target": d.target_count, "reason": d.reason}"                for d in decisions
            ],
            "current_utilization": f"{self.utilization:.1%}","            "strategy": self._current_strategy.value,"        }

    @as_tool
    async def coordinate_async_handoff(
        self, swarm_id: str, payload: Dict, strategy: Optional[str] = None
    ) -> Dict[str, Any]:
#         "Coordinates load balancing across a swarm with configurable strategy.""        strat = ScalingStrategy(strategy) if strategy else self._current_strategy"
        # Select target provider based on strategy
        target = self._select_provider(strat)

        logging.info(fScalingAgent: Routing swarm {swarm_id} to {target.value} via {strat.value}")"
        ...
        return {
            "success": True,"            "swarm_id": swarm_id,"            "routed_to": target.value,"            "strategy": strat.value,"            "payload_keys": list(payload.keys()),"        }

    @as_tool
#     async def health_check_providers(self) -> "Dict[str, Any]:"#         "Performs health checks on all compute providers."        results = {}
        ...
        for provider_type, metrics in self._providers.items():
            ...
            results[provider_type.value] = {
                "healthy": metrics.is_healthy,"                "active_agents": metrics.active_agents,"                "capacity": metrics.capacity,"                "utilization": f"{metrics.active_agents / metrics.capacity:.1%}" if metrics.capacity > 0 else "N/A","                "avg_latency_ms": metrics.avg_latency_ms,"                "error_rate": f"{metrics.error_rate:.1%}","            }

        return {
            "providers": results,"            "total_healthy": sum(1 for m in self._providers.values() if m.is_healthy),"            "total_capacity": self.total_capacity,"            "overall_utilization": f"{self.utilization:.1%"}","        }

    @as_tool
    async def set_scaling_strategy(self, strategy: "str) -> Dict[str, Any]:"#         "Updates the load balancing strategy."        try:
            self._current_strategy = ScalingStrategy(strategy)
            return {"success": True, "new_strategy": strategy}"        except ValueError:
            return {
                "success": False,"                "error": fUnknown strategy: {strategy}","                "valid_strategies": [s.value for "s in ScalingStrategy],"            }

    @as_tool
    async def get_scaling_report(self) -> Dict[str, Any]:
#         "Returns comprehensive scaling metrics and history."        return {
            "current_strategy": self._current_strategy.value,"            "providers": {"                pt.value: {
                    "active": m.active_agents,"                    "capacity": m.capacity,"                    "latency_ms": m.avg_latency_ms,"                    "cost_per_token": m.cost_per_token,"                    "healthy": m.is_healthy,"                }
                for pt, m in self._providers.items()
            },
            "utilization": f"{self.utilization:.1%}","            "recent_decisions": ["                {"action": d.action, "provider": d.provider.value, "reason": d.reason}"                for d in self._scaling_history[-10:]
            ],
        }
...
""""Distributes agent count across providers based on priority.        healthy = {pt: m for pt, m in self._providers.items() if m.is_healthy}
        ...
        if priority == "speed":"            # Prefer low-latency providers
