...
- From a HandyAgent instance:
  ...
  - agent.terminal_slash_command("/test", ["-k", "fast"])    # ask agent to run tests
  - agent.execute_with_diagnosis("pytest -q")                # run shell command with automatic diagnosis
- Designed to be exposed as tools via as_tool decorator so UI/CLI can invoke them safely.

WHAT IT DOES:
- Provides a small, focused mixin for HandyAgent to handle simple "slash" CLI commands (/fix, /test, /summarize) and record them.
- Offers execute_with_diagnosis to run arbitrary shell commands (safely split with shlex), capture stdout/stderr, apply a simple catastrophic-command blocklist, and return either formatted success output or a short diagnostic analysis on failure.
