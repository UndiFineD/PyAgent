...
# #
        ...
        if self.recorder:
            self.recorder.record_interaction(
                ...
                prompt=fComprehensive validation of {file_path}",
                result=str(results),
            )
...
""""Validate Python "syntax and return critical issues."""
        ...
        try:
            ...
            issues.append(fSyntax Error: {e.msg} at line {e.lineno}")
        except (Exception,):  # pylint: disable=broad-exception-caught
...
""""Strictly validate indentation - only 4 spaces allowed, no tabs."""
        ...
        for i, line in enumerate(lines, 1):
            if line.strip():  # Skip empty lines
                # Check for tabs
                if '\t' in line:
                    issues["critical"].append(fLine {i}: Contains tabs - only spaces allowed")
                ...
                if isinstance(node, ast.Name):
                    ...
                    if isinstance(node.ctx, ast.Store):
                        if not re.match(r'^[a-z][a-z0-9_]*$', name):
                            issues["critical"].append(fVariable '{name}' should be snake_case")

                    # Check if this is a function definition
                    elif isinstance(node, ast.FunctionDef):
                        if not re.match(r'^[a-z][a-z0-9_]*$', name):
                            issues["critical"].append(fFunction '{name}' should be snake_case")

                elif isinstance(node, ast.ClassDef):
                    if not re.match(r'^[A-Z][a-zA-Z0-9]*$', node.name):
                        issues["critical"].append(fClass '{node.name}' should be PascalCase")
        ...
        for i, char in enumerate(content):
            if char in opening_brackets:
                ...
                if not stack:
                    ...
                    issues.append(fLine {line_num}: Unmatched closing bracket '{char}'")
                else:
                    opening, _open_pos = stack.pop()
                    if bracket_pairs[char] != opening:
                        line_num = content[:i].count('\n') + 1
                        issues.append(fLine {line_num}: Mismatched brackets '{opening}' and '{char}'")

        if stack:
            for opening, pos in stack:
                line_num = content[:pos].count('\n') + 1
                issues.append(fLine {line_num}: Unmatched opening bracket '{opening}'")
      ...
      '"'      while j < len(line):
           ...
           '"'     # Check for unescaped quote
             ...
             "'"           elif string_char == "'" and len(string_content) > 3 and '"' not in string_content:
                            issues.append(fLine {i}: Prefer double quotes for string literals")
        ...
        try:
            ...
            for issue in analysis.get("issues", []):
                if issue.get("severity") == "critical":
                    issues["critical"].append(fDocstring: {issue['message']}")
                else:
...
#             return fFormatting partially successful, but black reported: {result.stderr}
                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    for arg in node.args.args:
                        if arg.annotation is None and arg.arg != 'self':
                            issues.append(fFunction '{node.name}': Parameter '{arg.arg}' missing type hint")
                    ...
                    for arg in node.args.args:
                        if arg.annotation is None and arg.arg != 'self':
                            issues.append(fAsync function '{node.name}': Parameter '{arg.arg}' missing type hint")
...
# #
        ...
        for i, line in enumerate(lines, 1):
            if re.search(sleep_pattern, line):
              ...
              "      issues.append(fLine {i}: sleep usage forbidden")  # nosec
