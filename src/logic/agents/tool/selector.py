...
class ToolSelector:
    ...
    def _initialize_default_tools(self):
        ...
        self._tools.extend([
            ...
            Tool("nosql_scanner", "database", "Scan NoSQL databases", ["scan", "nosql", "mongodb"]),
            Tool("data_migrator", "database", "Migrate data between databases", ["migrate", "etl", "transfer"]),
        ])

        # API tools
        self._tools.extend([
            Tool("rest_client", "api", "Make REST API calls", ["rest", "http", "api"]),
            Tool("graphql_client", "api", "Execute GraphQL queries", ["graphql", "api"]),
            Tool("webhook_handler", "api", "Handle webhook events", ["webhook", "event", "callback"]),
        ])

        # Cloud tools
        self._tools.extend([
            Tool("s3_manager", "cloud", "Manage AWS S3 buckets", ["aws", "s3", "storage"], "python"),
            Tool("gcp_storage", "cloud", "Google Cloud Storage operations", ["gcp", "storage"], "python"),
            Tool("azure_blob", "cloud", "Azure Blob Storage operations", ["azure", "storage"], "python"),
        ])

        # Development tools
        self._tools.extend([
            Tool("code_formatter", "development", "Format and lint code", ["format", "lint", "code"], "python"),
            Tool("test_runner", "development", "Run automated tests", ["test", "ci", "automation"], "python"),
            Tool("dependency_scanner", "development", "Scan for security vulnerabilities", ["security", "scan"], "python"),
        ])
    ...
    def _calculate_relevance_score(self, tool: Tool, task_words: set, task_lower: str) -> float:
        ...
        # Category relevance (30%)
        category_keywords = {
            "database": ["database", "sql", "query", "data", "table"],
            "api": ["api", "rest", "http", "endpoint", "web"],
            "cloud": ["cloud", "aws", "azure", "gcp", "storage", "bucket"],
            "development": ["code", "test", "build", "deploy", "ci"],
            "language": ["compile", "execute", "run", "script"]
        }
        ...
        desc_overlap = len(desc_words.intersection(task_words))
    ...
    def recommend_tools_for_task(self, task_description: str) -> Dict[str, Any]:
        ...
        return {
            "primary_tools": selected_tools,
            "alternative_tools": alternative_tools,
            "task_analysis": self._analyze_task_requirements(task_description),
            "capability_coverage": self._assess_capability_coverage(selected_tools, task_description)
        }
    ...
    def _analyze_task_requirements(self, task_description: str) -> Dict[str, Any]:
        ...
        requirements = {
            ...
            "api": any(word in task_lower for word in ["api", "rest", "http", "endpoint"]),
            "cloud": any(word in task_lower for word in ["cloud", "aws", "azure", "storage"]),
            "development": any(word in task_lower for word in ["code", "test", "build", "deploy"]),
            "language": any(word in task_lower for word in ["python", "javascript", "java", "go", "rust"])
        }
    ...
    def _assess_capability_coverage(self, tools: List[Tool], task_description: str) -> Dict[str, Any]:
        ...
        if "database" in task_lower:
            key_capabilities.extend(["query", "database", "sql"])
        if "api" in task_lower:
            key_capabilities.extend(["rest", "http", "api"])
        if "cloud" in task_lower:
            key_capabilities.extend(["storage", "cloud"])
