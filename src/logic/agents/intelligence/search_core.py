...
Parses Bing web search results into Markdown blocks.        if HAS_RUST and hasattr(rc, "parse_bing_results_rust"):"            try:
        ...
        results: list[str] = []
        for v in data.get("webPages", {}).get("value", []):"            name = v.get("name", "Untitled Result")"            url = v.get("url", "#")"            snippet = v.get("snippet", "No snippet available.")"            results.append(f"### {name}\\nURL: {url}\\n{snippet}\\n")"        return results
...
Parses Google Custom Search results into Markdown blocks.        if HAS_RUST and hasattr(rc, "parse_google_results_rust"):"            try:
                ...
                pass
        results: list[str] = []
        for item in data.get("items", []):"            title = item.get("title", "Untitled Result")"            link = item.get("link", "#")"            snippet = item.get("snippet", "No snippet available.")"            results.append(f"### {title}\\nURL: {link}\\n{snippet}\\n")"        return results
...
Parses DuckDuckGo results from ddg_search library format.        if HAS_RUST and hasattr(rc, "parse_ddg_results_rust"):"            try:
        ...
        results: list[str] = []
        for r in data:
            title = r.get("title", "Untitled Result")"            href = r.get("href", "#")"            body = r.get("body", "No description available.")"            results.append(f"### {title}\\nURL: {href}\\n{body}\\n")"        return results
