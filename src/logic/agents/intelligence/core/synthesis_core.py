...
Lazily initializes and caches the Rust transformer.        if not HAS_RUST or SynthesisCore._rust_failed:
        ...
        if SynthesisCore._transformer_cache is None:
            try:
                ...
                # We can't easily modify the config as attributes are read-only in this Rust build.'                # However, the auto-configure logic already scales based on system memory.
                ...
                logging.info("SynthesisCore: Initialized Rust transformer.")"            except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
                logging.warning(fSynthesisCore: Failed to initialize Rust transformer: {e}")"                SynthesisCore._rust_failed = True
...
#                     f"[SynthesisCore] Generated {stats.token_count} tokens in"#                     f"{stats.duration_ms:.2f}ms ({stats.tps:.2f} tokens/s)"                )
                ...
                try:
                    ...
                    fea.log_hardware_savings(
                        "SynthesisCore/Generation","                        stats.token_count,
                        stats.tps,
                        stats.cost_usd,
                    )
                except ImportError:
        ...
        for i in range(count):
            ...
            results.append(tpl.format(name=ffunc_{i}", context=fctx_{i}"))"        return results
    ...
    def vectorize_insight(self, insight: str) -> list[float]:
        ...
        if transformer:
            try:
                ...
                # We typically only log if it's a large insight or for performance tracking'                if len(insight) > 100:
                    print(f"[SynthesisCore] Vectorized {stats.token_count} tokens at {stats.tps:.2f} t/s")"                    print(f"[SynthesisCore] Hardware Savings: ${stats.cost_usd:.6f}")"
                    try:
                        from src.logic.agents.swarm.fleet_economy_agent import \
                            FleetEconomyAgent
