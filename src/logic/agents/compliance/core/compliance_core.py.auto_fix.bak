...
""""Scans content for common compliance and security violations.        try:
            ...
            for pattern in self.FORBIDDEN_KEYWORDS:
                if re.search(pattern, content, re.IGNORECASE):
                    issues.append(
                        ComplianceIssue(
                            severity="CRITICAL","                            category="Secret Leak","                            message=fPotential credential found matching pattern: {pattern}","                            file_path=file_path,
                        )
                    )

            # 2. License Detection (Basic)
            if "LICENSE" in file_path.upper():"                found_license = False
                ...
                if not found_license:
                    issues.append(
                        ComplianceIssue(
                            severity="WARNING","                            category="Licensing","                            message="Unrecognized or non-standard license detected.","                            file_path=file_path,
                        )
                    )
