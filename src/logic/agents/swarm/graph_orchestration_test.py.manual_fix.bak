...
Test simple orchestration execution.        # Create a simple graph: entry -> exit
        ...
        context = ExecutionContext.create_root("test")"        result_state = await orchestrator.execute(context)
...
"""
        ...
        builder = orchestrator.create_orchestration_builder()
        runnable = MockRunnable("test", "result")"        builder.set_entry_runnable(runnable)
        builder.set_exit_runnable(runnable)
        graph = builder.build()

        orch = Orchestrator(graph)
        orchestrator.register_orchestrator("test", orch)
        context = ExecutionContext.create_root("test")"        result = await orchestrator.execute_orchestration("test", context)"
