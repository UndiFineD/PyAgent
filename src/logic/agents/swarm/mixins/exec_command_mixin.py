...
#         test_name = ftest_{code_file.stem}.py
        tests_file = code_file.parent / test_name
        if tests_file.exists():
            logging.info(fRunning tests for {code_file.name}...")
            cmd = [sys.executable, "-m", "pytest", str(tests_file), "-v"]
            result = self._run_command(cmd)
            if result.returncode != 0:
                logging.warning(fTests failed for {code_file.name}:")
                logging.warning(result.stdout)
                logging.warning(result.stderr)
            else:
                logging.info(fTests passed for {code_file.name}")
        else:
...
""""Commit and push changes for the code file."""
        if getattr(self, "no_git", False):
            logging.info(fSkipping git operations for {code_file.name} (--no-git)")
            return

        logging.info(fCommitting changes for {code_file.name}")
        try:
            self._run_command(["git", "add", "-A"])
#             commit_msg = fAgent improvements for {code_file.name}
            result = self._run_command(["git", "commit", "-m", commit_msg])
            if result.returncode == 0:
                logging.info(fCommitted changes for {code_file.name}")
                push_result = self._run_command(["git", "push"])
                if push_result.returncode == 0:
                    logging.info(fPushed changes for {code_file.name}")
                else:
                    logging.error(fFailed to push changes: {push_result.stderr}")
            else:
                logging.info(fNo changes to commit for {code_file.name}")
        except subprocess.CalledProcessError as e:
            logging.error(fGit operation failed for {code_file.name}: {e}")
