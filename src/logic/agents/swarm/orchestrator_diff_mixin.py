...
"""
Preview changes to a file without applying them.        if not hasattr(self, "diff_generator"):"            setattr(self, "diff_generator", DiffGenerator())"        original = file_path.read_text(encoding="utf-8") if file_path.exists() else ""
diff_gen = getattr(self, "diff_generator")"        return diff_gen.generate_diff(original, new_content, str(file_path))
