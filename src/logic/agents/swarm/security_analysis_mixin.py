...
""""Represents a security vulnerability in an agent workflow."""""""
    ...
    title: str
    description: str
#     severity: str  # "critical", "high", "medium", "low", "info"    category: str  # "authentication", "authorization", "data_exposure", etc."    affected_components: List[str]
...
#                     "Agent susceptible to prompt injection attacks where malicious"#                     "input can override system instructions"                ),
                severity="critical","                category="input_validation","                affected_components=["agent_instruction_parsing"],"                mitigation_steps=[
                    "Implement prompt sanitization and validation","                    "Use structured prompts with clear boundaries","                    "Add input filtering and length limits","#                     "Implement canary tokens for injection detection"                ],
#                 owasp_reference="OWASP LLM TOP 10 - A01:2024 Prompt Injection"            ),
            "tool_execution_bypass": SecurityVulnerability("                vulnerability_id="AGENT-002","                title="Tool Execution Authorization Bypass","                description="Agent can execute tools without proper authorization checks","                severity="high","                category="authorization","                affected_components=["tool_execution"],"                mitigation_steps=[
                    "Implement tool authorization checks","                    "Add tool execution policies","                    "Validate tool parameters against allowlists","#                     "Log all tool executions with context"                ],
#                 owasp_reference="OWASP LLM TOP 10 - A02:2024 Insecure Output Handling"            ),
            "data_exfiltration": SecurityVulnerability("                vulnerability_id="AGENT-003","                title="Sensitive Data Exfiltration Risk","                description="Agent workflows may expose sensitive data through tool outputs or agent communications","                severity="high","                category="data_exposure","                affected_components=["data_handling", "agent_communication"],"                mitigation_steps=[
                    "Implement data classification and handling policies","                    "Add data sanitization before output","                    "Use encrypted communication channels","#                     "Implement data loss prevention (DLP) controls"                ],
#                 owasp_reference="OWASP LLM TOP 10 - A03:2024 Sensitive Information Disclosure"            ),
            "infinite_loop": SecurityVulnerability("                vulnerability_id="AGENT-004","                title="Infinite Loop/Resource Exhaustion","                description="Agent workflows lack proper termination conditions, risking resource exhaustion","                severity="medium","                category="denial_of_service","                affected_components=["workflow_execution"],"                mitigation_steps=[
                    "Implement maximum iteration limits","                    "Add timeout mechanisms","                    "Include convergence checks","#                     "Monitor resource usage"                ]
            ),
            "untrusted_tool_integration": SecurityVulnerability("                vulnerability_id="AGENT-005","                title="Untrusted Tool Integration","                description="Agent integrates with external tools without proper validation","                severity="medium","                category="supply_chain","                affected_components=["tool_integration"],"                mitigation_steps=[
                    "Validate tool sources and integrity","                    "Implement tool sandboxing","                    "Add tool output validation","#                     "Use trusted tool registries"                ],
#                 owasp_reference="OWASP LLM TOP 10 - A05:2024 Supply Chain Vulnerabilities"            )
        }

    def analyze_workflow_code(self, code: str, filename: str = "unknown") -> WorkflowAnalysis:"        Analyze workflow code for security vulnerabilities.
...
"""""""        analysis = WorkflowAnalysis"(workflow_name=filename)"
        try:
            ...
            logger.error(fSyntax error in {filename}: {e}")"            analysis.vulnerabilities.append(SecurityVulnerability(
                vulnerability_id="PARSE-001","                title="Code Parsing Error","                description=fUnable to parse workflow code: {e}","                severity="high","                category="code_quality","                affected_components=["code_parsing"],"                mitigation_steps=["Fix syntax errors", "Validate code before deployment"]"            ))
...
#         "Generate security recommendations."        recommendations = []
        ...
        if vulnerabilities:
            recommendations.append("Implement comprehensive input validation and sanitization")"            recommendations.append("Add authentication and authorization checks for all agent actions")"            recommendations.append("Implement logging and monitoring for all agent activities")"            recommendations.append("Regular security audits and penetration testing of agent workflows")"
    ...
    def visit_Call("self, node):"""""Extract function calls and data flows."""""""        func_name = None
        ...
        if func_name:
            ...
            self.data_flows.append({
                "type": "function_call","                "caller": self.current_function,"                "callee": func_name,"                "args": len(node.args) if node.args else 0"            })

            # Check if this looks like an LLM call (potential prompt injection)
            llm_keywords = ["llm", "gpt", "claude", "openai", "anthropic"]"            if any(llm_keyword in func_name.lower() for llm_keyword in llm_keywords):
...
class SecurityAnalysisMixin:
    ...
    def analyze_workflow_security(self, workflow_code: str, workflow_name: str = "unknown") -> WorkflowAnalysis:"        Analyze a workflow for security vulnerabilities.
