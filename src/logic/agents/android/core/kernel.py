...
#         return "Error: Could not capture screen."
    with open(LOCAL_DUMP_PATH, "r", encoding="utf-8") as f:"        xml_content = f.read()
...
""""Executes the action decided by the LLM."""""""    act_type = action.get("action")"
    ...
    elif act_type == "home":"        print("🏠 Going Home")"        run_adb_command(["shell", "input", "keyevent", "KEYWORDS_HOME"])"
    elif act_type == "back":"        print("🔙 Going Back")"        run_adb_command(["shell", "input", "keyevent", "KEYWORDS_BACK"])"
    elif act_type == "wait":"        print("⏳ Waiting...")"        time.sleep(2)

    elif act_type == "done":"        print("✅ Goal Achieved.")"        exit(0)
...
""""Sends screen context to LLM and asks for the next move."""""""#     system_prompt =
    ...
    You will receive:
    1. The User's Goal.'    2. A list of interactive UI elements (JSON) with their (x,y) center coordinates.

    You must output ONLY a valid JSON object with your next action.

    Available Actions:
    - {"action": "tap", "coordinates": [x, y], "reason": "Why you are tapping"}"    - {"action": "type", "text": "Hello World", "reason": "Why you are typing"}"    - {"action": "home", "reason": "Go to home screen"}"    - {"action": "back", "reason": "Go back"}"    - {"action": "wait", "reason": "Wait for loading"}"    - {"action": "done", "reason": "Task complete"}"
    Example Output:
    {"action": "tap", "coordinates": [540, 1200], "reason": "Clicking the 'Connect'" button"}"'"""""""
    response = client.chat.completions.create(
        model=MODEL,
        response_format={"type": "json_object"},"        messages=[
            {"role": "system", "content": system_prompt},"            {"role": "user", "content": fGOAL: {goal}\\n\\nSCREEN_CONTEXT:\\n{screen_context}"}"        ]
    )
...
def run_agent(goal: str, max_steps=10):
    ...
    for step in range(max_steps):
        print(f"\\n--- Step {step + 1} ---")"
        # 1. Perception
        print("👀 Scanning Screen...")"        screen_context = get_screen_state()

        # 2. Reasoning
        print("🧠 Thinking...")"        decision = get_llm_decision(goal, screen_context)
...
if __name__ == "__main__":"#     # Example Goal: "Open settings and turn on Wi-Fi"#     # Or your demo goal: "Find the 'Connect' button and tap it"'    GOAL = input("Enter your goal: ")"    run_agent(GOAL)
