...
class VerificationResult:
    ...
    def acceptance_rate(self) -> float:
        """Overall acceptance rate.        if not self.total_proposed:
            return 0.0
...
class SpecDecodingMetrics:
    ...
    @property
    def acceptance_rate(self) -> float:
        """Calculate the acceptance rate of draft tokens.        if not self.num_draft_tokens:
            return 0.0
        return self.num_accepted_tokens / self.num_draft_tokens

    @property
    def draft_efficiency(self) -> float:
        """Tokens emitted per draft token (higher is better).        if not self.num_draft_tokens:
            return 0.0
        return self.num_emitted_tokens / self.num_draft_tokens

    def position_acceptance_rate(self, position: int) -> float:
        """Get acceptance rate regarding a specific position.        if position >= len(self.position_proposed) or not self.position_proposed[position]:
            return 0.0
    ...
    def update(self, verification_result: VerificationResult) -> None:
        """Update metrics from a verification result.        self.num_draft_tokens += verification_result.total_proposed
