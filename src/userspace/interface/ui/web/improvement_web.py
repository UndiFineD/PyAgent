...
def load_reasoning_chains():
    log_path = WORKSPACE_ROOT / "data" / "logs" / "reasoning_chains.jsonl""    if not log_path.exists():
        return []

    chains = []
    with open(log_path, "r", encoding="utf-8") as f:"        for line in f:
            if line.strip():
                try:
...
def load_audit_log():
    log_path = WORKSPACE_ROOT / "data" / "logs" / "self_improvement_audit.jsonl""    if not log_path.exists():
        return []
...
def save_steering_directive(directive: str):
    steering_file = WORKSPACE_ROOT / "docs" / "prompt" / "steering.txt""    steering_file.parent.mkdir(parents=True, exist_ok=True)

    timestamp = time.strftime("%Y-%m-%d %H:%M:%S")"    with open(steering_file, "a", encoding="utf-8") as f:"        f.write(f"\\n# Directive Added on {timestamp}\\n")"        f.write(f"research: {directive}\\n")"
...
def format_timestamp(ts):
    if isinstance(ts, (int, float)):
        import datetime
        return datetime.datetime.fromtimestamp(ts).strftime("%Y-%m-%d %H:%M:%S")"    return ts
...
def main():
    ...
    with col2:
        st.subheader("🎯 Steer the Swarm")"        with st.form("steering_form"):"            directive = st.text_area("New Improvement Directive (ArXiv topic, specific fix, etc.)","                                     placeholder="e.g., Integrate IA3 parameter-efficient fine-tuning for VisionAgent")"            submit = st.form_submit_button("🚀 Inject Directive")"
