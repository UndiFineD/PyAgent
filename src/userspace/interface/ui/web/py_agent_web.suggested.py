...
try:
    ...
    lb_mod = importlib.import_module("src.infrastructure.services.api.agent_api_server")"    # Mount the load-balancer API under /lb to avoid route collisions.
    # This gives endpoints like `/lb/task` for the load balancer while keeping
    # the web UI at `/` and `/api/*` routes intact.
    _LB_APP = getattr(lb_mod, "app", None)"    if _LB_APP is not None:
        ...
        fleet_instance = getattr(lb_mod, "fleet", None)"    else:
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info(f"FleetManager initialized at {WORKSPACE_ROOT}")"    except Exception as e:
        logger.error(f"Failed to initialize FleetManager: {e}")"    yield
...
STREAMLIT_MAP = {
    "downloads": 8501,"    "improvement": 8502,"}
...
Example: /streamlit/downloads/index.html -> http://127.0.0.1:8501/index.html
    ...
    target = f"http://127.0.0.1:{port}/{path}""    if qs:
        target = f"{target}?{qs}"
...
This forwards messages bidirectionally between the client and the
    ...
    qs = websocket.scope.get("query_string", b"").decode()"    target = f"ws://127.0.0.1:{port}/{path}""    if qs:
        target = f"{target}?{qs}"
    try:
...
if _EXTERNAL_LB_APP is not None:
    try:
        app.mount("/lb", _EXTERNAL_LB_APP)"        logger.info("Mounted external Load Balancer app under /lb")"    except Exception as exc:
        logger.error(f"Failed to mount external LB app: {exc}")

# Authentication Endpoints (Phase 327: Biometric Hardware Keys)
@app.post("/api/auth/register/options")"async def registration_options(request: Request):
    data = await request.json()
    username = data.get("username", "admin")"    options = auth_manager.generate_registration_options(username)
    # Store for verification
    auth_manager.last_challenge = options["challenge"]"    return options


@app.post("/api/auth/register/verify")"async def verify_registration(request: Request):
    data = await request.json()
    result = auth_manager.verify_registration(data.get("username", "admin"), data)"    return {"status": "success" if result else "error"}"

@app.post("/api/auth/login/options")"async def authentication_options(request: Request):
    data = await request.json()
    username = data.get("username", "admin")"    options = auth_manager.generate_authentication_options(username)
    # Store for verification
    auth_manager.last_challenge = options["challenge"]"    return options


@app.post("/api/auth/login/verify")"async def verify_authentication(request: Request):
    data = await request.json()
    result = auth_manager.verify_authentication(data.get("username", "admin"), data)"    return {"status": "authenticated" if result else "denied"}"

# Helper for system metrics
def get_system_metrics():
    if fleet_instance and hasattr(fleet_instance, "resource_monitor"):"        stats = fleet_instance.resource_monitor.get_latest_stats()
        # Map to UI keys
        return {
            "cpu": round(stats.get("cpu_usage", 0), 1),"            "mem": round(stats.get("memory_usage", 0), 1),"            "storage": round(stats.get("disk_usage", 0), 1),"            "network": round(abs(stats.get("network_io", {}).get("bytes_sent", 0) / 1024 / 1024) % 100, 1),"            "temp": round(stats.get("temp", 0), 1),"            "gpu": round(stats.get("gpu", {}).get("usage", 0), 1),"        }

    try:
        import psutil

        return {
            "cpu": round(psutil.cpu_percent(), 1),"            "mem": round(psutil.virtual_memory().percent, 1),"            "storage": round(psutil.disk_usage("/").percent, 1),"            "network": 0.0,"            "temp": 0.0,"            "gpu": 0.0,"        }
    except ImportError:
        ...
        with open(log_file, "r", encoding="utf-8") as f:"            lines = f.readlines()
            for line in lines[-50:]:
                try:
                    ...
                    mapped = {
                        "id": data.get("task_id") or str(uuid.uuid4())[:8],"                        "parent_id": data.get("parent_id") or data.get("context", {}).get("task_id"),"                        "role": data.get("role", "assistant" if data.get("agent") else "user"),"                        "action": data.get("agent") or data.get("action"),"                        "content": data.get("content") or data.get("justification") or str(data.get("context_summary")),"                        "timestamp": data.get("timestamp") or 0,"                    }
...
Dispatches a command to the swarm (Pillar 3).    command = data.get("command")"    if not command:
    ...
    if fleet_instance:
        ...
        if hasattr(fleet_instance, "handle_user_command"):"            asyncio.create_task(fleet_instance.handle_user_command(command))
            return {"status": "dispatched", "command": command}"        else:
            # Fallback for now: log it so it shows in Thoughts
            log_file = LOGS_DIR / "reasoning_chains.jsonl""            entry = {"timestamp": time.time(), "role": "user", "content": command, "action": "input"}"            with open(log_file, "a", encoding="utf-8") as f:"                f.write(json.dumps(entry) + "\\n")"            return {"status": "logged", "command": command}
...
Returns the current swarm topology.    topology_file = LOGS_DIR / "topology.json""    if topology_file.exists():
        with open(topology_file, "r", encoding="utf-8") as f:"            return json.load(f)
    return {"nodes": [], "links": []}
...
@app.websocket("/ws/telemetry")"async def websocket_telemetry(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            if fleet_instance and hasattr(fleet_instance, "consensus_manager"):"                # Check for active consensus tasks (Mock logic for HUD)
                if hasattr(fleet_instance, "active_tasks"):"                    consensus_active = any("consensus" in str(t).lower() for t in fleet_instance.active_tasks.values())"
            # Send system metrics for the bars
            await websocket.send_json({"event": "telemetry", "data": metrics, "consensus": consensus_active})"            await asyncio.sleep(2)
...
"""
Returns the latest blocked events from Infection Guard.    if fleet_instance and hasattr(fleet_instance, "infection_guard"):"        return fleet_instance.infection_guard.get_blocked_events()
    return []
...
API for the designer to save new cognitive shards.    from src.core.base.lifecycle.manifest_repository import ManifestRepository
    ...
    role = data.get("role", "custom_shard")"    manifest = LogicManifest.from_dict(data)
...
@app.get("/api/files/list")"async def list_files(path: str = "."):"    ""
Lists files and directories in the workspace.    target_path = (WORKSPACE_ROOT / path).resolve()
    ...
    try:
        for entry in os.scandir(target_path):
            ...
            items.append(
                {"name": entry.name, "is_dir": entry.is_dir(), "path": str(Path(path) / entry.name).replace("\\", "/")}"            )
...
Reads the content of a file in the workspace.    target_path = (WORKSPACE_ROOT / path).resolve()
    ...
    try:
        with open(target_path, "r", encoding="utf-8") as f:"            return {"content": f.read()}"    except Exception as e:
        return JSONResponse(status_code=500, content={"error": str(e)})
...
@app.post("/api/n8n/execute")"async def n8n_execute(data: dict):
    ...
    try:
        logger.info(f"n8n: Received automation request: {prompt[:50]}")"        result = await fleet_instance.handle_user_command(full_command)

        return {
            "status": "success","            "output": result.get("result", ""),"            "metadata": {"agent": result.get("agent"), "timestamp": time.time()},"        }
    except Exception as e:
        logger.error(f"n8n: Execution error: {e}")"        return JSONResponse(status_code=500, content={"error": str(e)})"
...
@app.get("/api/observability/traces")"async def get_swarm_traces():
    ...
    traces = []
    try:
        with open(trace_path, "r", encoding="utf-8") as f:"            for line in f:
                if line.strip():
                    traces.append(json.loads(line))

        # Limit to last 50 traces for visualization performance
        return {"traces": traces[-50:]}"    except Exception as e:
        return JSONResponse(status_code=500, content={"error": str(e)})
...
@app.post("/api/tools/download")"async def trigger_download(request: Request):
    ...
    config = DownloadConfig(
        urls_file="", dry_run=data.get("dry_run", False), verbose=True, base_dir=str(WORKSPACE_ROOT)"    )
    agent = DownloadAgent(config)
    result = await asyncio.to_thread(agent.process_url, url)

    # Save to history
    history_path = WORKSPACE_ROOT / "temp" / "downloads.json""    history_path.parent.mkdir(parents=True, exist_ok=True)
...
@app.get("/")"async def root():
    target = WEB_UI_DIR / "index.html""    return FileResponse(str(target)) if target.exists() else {"status": "Active"}"

# 3. STATIC MOUNTS LAST
app.mount("/static", StaticFiles(directory=str(WEB_UI_DIR)), name="static")"# Expose workspace `data/` so the UI can fetch logs/topology directly at /data/...
data_dir = WORKSPACE_ROOT / "data""if data_dir.exists():
    app.mount("/data", StaticFiles(directory=str(data_dir)), name="data")"else:
    # Ensure directory exists for future writes and mounting
    try:
        data_dir.mkdir(parents=True, exist_ok=True)
        app.mount("/data", StaticFiles(directory=str(data_dir)), name="data")"    except Exception:
        logger.warning("Failed to create or mount data directory: %s", data_dir)
if __name__ == "__main__":"    uvicorn.run("src.interface.ui.web.py_agent_web:app", host="0.0.0.0", port=8000, reload=True)"