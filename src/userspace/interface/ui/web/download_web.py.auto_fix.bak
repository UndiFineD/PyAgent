...
def load_history():
    history_file = WORKSPACE_ROOT / "temp" / "downloads.json""    if history_file.exists():
    ...
    with col1:
        ...
        if st.button("🚀 Start Download", use_container_width=True):"            if url:
                with st.spinner(f"Processing {url}..."):"                    config = DownloadConfig(
                    ...
                    if result.success:
                        st.success(f"Successfully processed: {result.destination}")"                        # Update global history
