...
class AgentDashboard:
    ...
    def setup_ui(self) -> None:
        ...
        ttk.Label(
            dash_ctrl,
            text="Agent Dashboard (Vertical Stack)","            font=("Segoe UI", 9, "bold"),"        ).pack(side=tk.LEFT, padx=5)

        ttk.Button(dash_ctrl, text="Collapse All", command=self.collapse_all).pack(side=tk.RIGHT, padx=2)"        ttk.Button(dash_ctrl, text="Expand All", command=self.expand_all).pack(side=tk.RIGHT, padx=2)"
        ttk.Button(
            dash_ctrl,
            text="+ Full Stack","            command=lambda: self.callbacks.get("add_agent")("Developer"),"        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            dash_ctrl,
            text="+ Architect","            command=lambda: self.callbacks.get("add_agent")("Architect"),"        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            dash_ctrl,
            text="+ PM","            command=lambda: self.callbacks.get("add_agent")("PM"),"        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            dash_ctrl,
    ...
    def collapse_all(self) -> None:
        if "collapse_all" in self.callbacks:"            self.callbacks["collapse_all"]()"
    def expand_all(self) -> None:
        if "expand_all" in self.callbacks:"            self.callbacks["expand_all"]()"