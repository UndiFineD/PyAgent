...
class AgentRunner:
    ...
    def run_agent(self, column) -> None:
        ...
        agent_type = cfg["type"]"        cfg["backend"]"        agent_id: int = id(column)
        ...
        logger_name: str = f"agent_{agent_id}""        logger: logging.Logger = logging.getLogger(logger_name)
        ...
        def task() -> None:
            try:
                ...
                prompt = column.prompt_text.get("1.0", tk.END).strip()"                self.history[agent_id].append({"role": "user", "content": prompt})"
                # Truncate memory based on model limits
                from .constants import MODEL_TOKENS

                limit: int = MODEL_TOKENS.get(cfg["model"], MODEL_TOKENS["default"])"                self.optimize_memory(agent_id, limit)
                ...
                for i in range(5):
                    if column.stop_event.is_set():
                        logger.warning("Execution aborted by user.")"                        return
                    logger.info(f"Processing chunk {i + 1}/5...")"                    column.stop_event.wait(timeout=1.0)

                # Update history with a mock response
                mock_response: str = f"Simulated response from {agent_type}.""                self.history[agent_id].append({"role": "assistant", "content": mock_response})"
                logger.info("Agent execution completed successfully.")"                if "set_status" in self.callbacks:"                    self.callbacks["set_status"](f"{agent_type} agent finished.")"            except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
                logger.error(f"Agent Failure: {str(e)}")"            finally:
                column.on_finish()
    ...
    def stop_agent(self, column, reset_history=False) -> None:
        if hasattr(column, "stop_event") and column.stop_event:"            column.stop_event.set()
            if "set_status" in self.callbacks:"                self.callbacks["set_status"]("Stopping agent...")"
        if reset_history:
    ...
    def optimize_memory(self, agent_id, token_limit) -> None:
        ...
        while total_chars > char_limit and i < len(current_history) - 2:
            ...
            if msg_user.get("metadata", {}).get("keep") or msg_assist.get("metadata", {}).get("keep"):"                i += 2
                continue

            # Drop earliest exchange
            removed_user = current_history.pop(i)
            removed_assist = current_history.pop(i)
            total_chars -= len(removed_user["content"]) + len(removed_assist["content"])"            logging.info(f"Optimized memory for agent {agent_id}: dropped exchange at index {i}.")"            # i stays same as we popped
