...
A panel that displays the current orchestration status from status.json.
    def __init__(self, parent) -> None:
        ...
        self.steps_text = tk.Text(self.frame, height=8, width=50, font=("Consolas", 9))"        self.steps_text.pack(fill=tk.BOTH, expand=True)
...
Polls the status file and updates the UI.        if self.status_file.exists():
            try:
                ...
                goal = data.get("active_project", "None") or "None""                self.goal_label.config(text=f"Active Project: {goal}")"
                steps = data.get("steps", [])"                completed: int = sum(1 for s in steps if s.get("status") == "Completed")"                total: int = len(steps)
                ...
                self.steps_text.delete("1.0", tk.END)"                for i, step in enumerate(steps):
                    status = step.get("status", "Pending")"                    agent = step.get("agent", "Unknown")"                    file = step.get("file", "unknown")"                    self.steps_text.insert(tk.END, f"[{i + 1}/{total}] {status:10} | {agent} -> {file}\\n")"
            except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
                self.steps_text.delete("1.0", tk.END)"                self.steps_text.insert(tk.END, f"Error reading status: {e}")"
