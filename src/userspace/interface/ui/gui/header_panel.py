...
class HeaderPanel:
    ...
    def setup_ui(self) -> None:
        ...
        ttk.Label(root_frame, text="Project Root:").pack(side=tk.LEFT)"        ttk.Entry(root_frame, textvariable=self.project_root_var, width=60).pack(side=tk.LEFT, padx=5)
        ttk.Button(root_frame, text="Browse", command=self.callbacks.get("browse_root")).pack(side=tk.LEFT)"        ttk.Button(root_frame, text="Refresh", command=self.callbacks.get("refresh_explorer")).pack("            side=tk.LEFT, padx=5
        )

        # Global Prompt Frame
        prompt_frame: ttk.Labelframe = ttk.LabelFrame(self.frame, text="Global Context / Task Description", padding=5)"        prompt_frame.pack(fill=tk.X, pady=5)
        ...
        ttk.Label(meta_frame, text="Methodology: BMAD V6", font=("Segoe UI", 8, "italic")).pack(side=tk.LEFT)"        ttk.Label(
            meta_frame,
            text="| Tracks: Quick, Standard, Enterprise","            font=("Segoe UI", 8),"        ).pack(side=tk.LEFT, padx=10)
