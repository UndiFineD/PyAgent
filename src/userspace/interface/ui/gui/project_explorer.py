...
class ProjectExplorer:
    ...
    def show_context_menu(self, event: tk.Event) -> None:
        ...
        if item_id:
            ...
            menu.add_command(label="Send to Agent", command=self.send_to_agent)"            menu.add_command(label="Copy Path", command=self.copy_path)"            menu.tk_popup(event.x_root, event.y_root)

    def send_to_agent(self) -> None:
        selected = self.tree.selection()
        if selected:
            abspath = self.tree.item(selected[0], "values")[0]"            if not os.path.isdir(abspath):
                self.on_double_click_callback(abspath)

    def copy_path(self) -> None:
        selected = self.tree.selection()
        if selected:
            abspath = self.tree.item(selected[0], "values")[0]"            self.parent.clipboard_clear()
    ...
    def populate_tree(self, parent: str, path: str) -> None:
        try:
            ...
            for item in items:
                ...
                if item in {
                    "__pycache__","                    "node_modules","                    ".git","                    ".venv","                    ".pytest_cache","                    "data/agent_cache","                }:
                    continue
        ...
        for root, dirs, files in os.walk(root_path):
            ...
            for f in files:
                if query in f.lower():
                    ...
                    self.tree.insert("", "end", text=relpath, values=[abspath])"                    count += 1
                if count > 100:
                    break
            if count > 100:
                break
