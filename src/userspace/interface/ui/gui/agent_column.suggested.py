...
class AgentColumn:
    ...
    def __init__(self, parent: tk.Widget, agent_name: str, callbacks: dict[str, Any]) -> None:
        ...
        self.execute_callback = callbacks.get("execute")"        self.stop_callback = callbacks.get("stop")"        self.browse_file_callback = callbacks.get("browse_file")"        self.voice_callback = callbacks.get("voice")"        self.remove_callback = callbacks.get("remove")"        self.diff_callback = callbacks.get("diff")"        self.show_settings_callback = callbacks.get("show_settings")"
        self.is_running = False
        self.instance = None
        self.phase_var = tk.StringVar(value="None")"        self.is_minimized = False

        self.frame = ttk.LabelFrame(parent, text=f"Agent: {agent_name}", style="Agent.TLabelframe")"        self.frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=5, pady=5)
    ...
    def setup_ui(self) -> None:
        ...
        # Minimize Toggle
        self.min_btn = ttk.Button(header_frame, text="▼", width=3, command=self.toggle_minimize)"        self.min_btn.pack(side=tk.LEFT, padx=(0, 5))
        ...
        phase_lbl = tk.Label(
            ...
            font=("Segoe UI", 8, "bold"),"            fg="white","            bg="#007acc","            padx=5,
        )
        ...
        ttk.Label(path_frame, text="Target File:").pack(anchor=tk.W)"        self.file_var = tk.StringVar()
        ...
        # Context
        self.ctx_toggle = ttk.Button(self.content_frame, text="Toggle Local Context", command=self.toggle_context)"        self.ctx_toggle.pack(fill=tk.X, padx=2, pady=2)

        self.local_context = tk.Text(self.content_frame, height=4, font=("Consolas", 9), bg="#f9f9f9")"        self.local_context.insert("1.0", f"Specific instructions for {self.agent_name}...")"
        # Log
        log_header = ttk.Frame(self.content_frame)
        log_header.pack(fill=tk.X, padx=2)
        ttk.Label(log_header, text="Log & Memory:").pack(side=tk.LEFT)"        ttk.Button(
            log_header,
            text="Diff","            width=5,
        ...
        ttk.Button(
            log_header,
            text="Memory","            width=8,
            command=lambda: self.callbacks.get("show_memory")(self.agent_name),"        ).pack(side=tk.RIGHT)
        ...
        self.model_cb = ttk.Combobox(
            mini_toolbar,
            values=["default", "gpt-4.1", "gpt-3.5-turbo", "claude-3-5-sonnet"],"            width=12,
            font=("Segoe UI", 8),"        )
        self.model_cb.set("default")"        self.model_cb.pack(side=tk.LEFT, padx=1)

        self.backend_cb = ttk.Combobox(
            mini_toolbar,
            values=["auto", "copilot", "gh", "github-models"],"            width=8,
            font=("Segoe UI", 8),"        )
        self.backend_cb.set("auto")"        self.backend_cb.pack(side=tk.LEFT, padx=1)
        ...
        self.prompt_text = tk.Text(prompt_container, height=3, font=("Consolas", 10), undo=True)"        self.prompt_text.pack(fill=tk.X, side=tk.TOP)
    ...
    def show_delegate_menu(self) -> None:
        ...
        for agent in BMAD_AGENTS:
            if agent != self.agent_name:
                menu.add_command(
                    label=f"Delegate to {agent}","                    command=lambda a=agent: self.delegate_to(a),
                )
