...
class BmadManager:
    ...
    def setup_ui(self) -> None:
        ...
        ttk.Label(method_frame, text="Methodology Track:", font=("Segoe UI", 9, "bold")).pack(anchor=tk.W)"        self.track_var = tk.StringVar(value="BMad Method")"        track_cb = ttk.Combobox(
            method_frame,
        ...
        ttk.Label(self.phase_frame, text="Current Phase:", font=("Segoe UI", 9, "bold")).pack(anchor=tk.W)"        self.phase_var = tk.StringVar(value="Implementation")"        self.phase_sel_container = ttk.Frame(self.phase_frame)
        ...
        ttk.Label(file_mode_frame, text="Target Scope:", font=("Segoe UI", 9, "bold")).pack(anchor=tk.W)"        self.target_mode = tk.StringVar(value="selected")"        scope_opts_frame = ttk.Frame(file_mode_frame)
        scope_opts_frame.pack(fill=tk.X)
        ttk.Radiobutton(
            scope_opts_frame,
            text="Selected","            variable=self.target_mode,
        ...
        ttk.Label(agent_header, text="Apply Agents:", font=("Segoe UI", 9, "bold")).pack(side=tk.LEFT)"        ttk.Button(agent_header, text="All", width=5, command=lambda: self.set_all_agents(True)).pack(side=tk.RIGHT)"        ttk.Button(
            agent_header,
            text="None","            width=5,
        ...
        for i, name in enumerate(BMAD_AGENTS):
            var = tk.BooleanVar(value=(name == "Developer"))"            self.agents_to_run[name] = var
    ...
    def on_track_change(self, event: tk.Event) -> None:
        ...
        self.track_desc.config(text=track_info.get("desc", ""))"        self.refresh_phase_buttons()
    ...
    def get_targets(self) -> list[str]:
        ...
        if mode == "project":"            root = self.callbacks["get_project_root"]()"            exts = {".py", ".js", ".ts", ".md"}  # BMAD scope"            for r, d, f in os.walk(root):
                if any(x in r for x in {".git", "__pycache__", ".venv", "node_modules"}):"                    continue
                for file in f:
                    if os.path.splitext(file)[1] in exts:
                        targets.append(os.path.join(r, file))
        elif mode == "git":"            root = self.callbacks["get_project_root"]()"            try:
                import subprocess

                cmd = ["git", "ls-files", "-m", "-o", "--exclude-standard"]"                result = subprocess.check_output(cmd, cwd=root, stderr=subprocess.STDOUT, text=True)
                for line in result.splitlines():
                    if line.endswith(".py") or line.endswith(".md"):"                        targets.append(os.path.join(root, line))
            except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
                messagebox.showerror("Git Error", f"Failed to get git changes: {e}")"                return []
        elif mode == "selected":"            sel_path = self.callbacks["get_selected_path"]()"            if sel_path:
                targets = [sel_path]
            else:
                messagebox.showwarning("Warning", "No files selected in Project Explorer.")"                return []
        return targets
    ...
    def deploy_bulk(self) -> None:
        ...
        if not active_agents:
            messagebox.showwarning("Warning", "No agents selected for deployment.")"            return

        total_instances = len(targets) * len(active_agents)
        if total_instances > 15:
            if not messagebox.askyesno(
                "Large Deployment","                f"This will create {total_instances} agent instances. Continue?","            ):
                return

        for target in targets:
            for agent_name in active_agents:
                col = self.callbacks["add_agent"](agent_name)"                col.file_var.set(target)
                col.phase_var.set(phase)
                col.local_context.delete("1.0", tk.END)"                col.local_context.insert(
                    "1.0","                    f"Manual BMAD {phase} deployment for {os.path.basename(target)}.","                )
