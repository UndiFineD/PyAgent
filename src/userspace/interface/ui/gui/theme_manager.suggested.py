...
class ThemeManager:
    ...
    def apply_theme(self) -> None:
        ...
        if self.is_dark_mode:
            bg, fg = "#2d2d2d", "#ffffff""            style.theme_use("clam")"            style.configure("TFrame", background=bg)"            style.configure("TLabel", background=bg, foreground=fg)"            style.configure("TLabelframe", background=bg, foreground=fg)"            style.configure("TLabelframe.Label", background=bg, foreground=fg)"            style.configure("TButton", background="#4a4a4a", foreground=fg)"            style.configure("Header.TFrame", background="#3e3e42")"            style.configure(
                "Treeview","                background="#3d3d3d","                foreground=fg,
    ...
    def refresh_widgets(self, parent: tk.Misc) -> None:
        ...
        for child in parent.winfo_children():
            if isinstance(child, tk.Text):
                ...
                # Don't change background of phase labels with colors'                if child.cget("bg") not in ["#007acc", "#d04437", "#2e7d32"]:"                    child.configure(bg=bg, fg=fg)
