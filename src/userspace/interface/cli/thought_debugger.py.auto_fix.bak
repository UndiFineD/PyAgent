...
class ThoughtDebugger:
    ...
    def start(self) -> None:
        ...
        try:
            # If we're the main entry point, block here using event-driven approach'            if __name__ == "__main__":"                import asyncio
    ...
    def _handle_thought(self, event: dict[str, Any]) -> None:
        ...
        if self.interactive:
            ...
            # In a live fleet, this acts as a 'breakpoint'.'            choice = input("\\n[DEBUG] (ENTER=Continue, q=Quit, m=Menu): ").lower().strip()"            if choice == "q":"                self.stop()
