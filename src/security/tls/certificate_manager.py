...
class CertificateManager:
    ...
    def __init__(
        self,
        cert_dir: str = "src/security/certificates",
        validity_days: int = 90,  # 3 months
    ...
    def generate_certificate(
        self,
        common_name: str,
        organization: str = "PyAgent Swarm",
        country: str = "US",
        state: str = "CA",
        ...
        try:
            ...
            cert = x509.CertificateBuilder().subject_name(
                ...
                x509.SubjectAlternativeName([
                    ...
                    x509.IPAddress(x509.IPAddress("127.0.0.1")),  # localhost
                    x509.IPAddress(x509.IPAddress("::1")),  # localhost IPv6
                ]),
                critical=False,
            ...
            self.certificates[common_name] = {
                ...
                "expires_at": datetime.now(timezone.utc) + timedelta(days=self.validity_days),
                "common_name": common_name,
            }

            self.logger.info(f"Generated certificate for {common_name}, expires: {self.certificates[common_name]['expires_at']}")
            return str(cert_path), str(key_path), cert_content

        except Exception as e:
            self.logger.error(f"Failed to generate certificate for {common_name}: {e}")
            raise
    ...
    def rotate_certificate(self, common_name: str) -> bool:
        ...
        """
        try:
            self.logger.info(f"Rotating certificate for {common_name}")
            self.generate_certificate(common_name)
            return True
        except Exception as e:
            self.logger.error(f"Failed to rotate certificate for {common_name}: {e}")
            return False

    def _certificate_monitor_loop(self):
        """Background thread to monitor certificate expiration and rotate as needed."""
        while True:
            try:
    ...
    def _check_and_rotate_certificates(self):
        """Check all certificates and rotate expired or expiring ones."""
        for common_name in list(self.certificates.keys()):
            if self.should_rotate_certificate(common_name):
                self.logger.info(f"Certificate for {common_name} needs rotation")
                self.rotate_certificate(common_name)
    ...
    def cleanup_expired_certificates(self, max_age_days: int = 365):
        ...
        try:
            ...
            for cert_file in self.cert_dir.glob("*.pem"):
                if cert_file.stat().st_mtime < cutoff_date.timestamp():
                    cert_file.unlink()
                    self.logger.info(f"Cleaned up old certificate: {cert_file}")

            for key_file in self.cert_dir.glob("*.key"):
                if key_file.stat().st_mtime < cutoff_date.timestamp():
                    key_file.unlink()
