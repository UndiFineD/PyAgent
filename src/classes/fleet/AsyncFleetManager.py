...
class AsyncFleetManager(FleetManager):
    ...
    def execute_workflow_async(
        self, task: str, workflow_steps: List[Dict[str, Any]]
    ) -> str:
...
if __name__ == "__main__":
    ...
    afleet.register_agent(
        "K1", KnowledgeAgent, root + "/src/classes/context/KnowledgeAgent.py"
    )
    afleet.register_agent(
        "S1", SecurityGuardAgent, root + "/src/classes/coder/SecurityGuardAgent.py"
    )
