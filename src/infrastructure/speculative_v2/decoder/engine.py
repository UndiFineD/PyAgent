...
class SpeculativeDecoder:
    ...
    def step(
        ...
        verify_logits = target_logits[
            len(input_ids) - 1 : len(input_ids) + len(proposed)
        ]

        draft_probs = np.array(
            [
                (
                    tree.tokens[i].probability
                    if i < len(tree.tokens)
                    else 1.0 / self.vocab_size
                )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_rate": (
                self._accepted_count / self._proposed_count
...
def create_ngram_decoder(
    vocab_size: int, max_depth: int = 5, ngram_order: int = 4
) -> SpeculativeDecoder:
...
def create_medusa_decoder(
    vocab_size: int, num_heads: int = 4, max_depth: int = 5
) -> SpeculativeDecoder:
