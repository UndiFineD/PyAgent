...
class SpeculativeVerifier:
    ...
    def verify_greedy(
        self, proposed_tokens: List[int], target_logits: np.ndarray
    ) -> VerificationResult:
    ...
    def verify_speculative(
        ...
        for i, proposed in enumerate(proposed_tokens):
            ...
            if np.random.random() < min(1.0, p_target / max(p_draft, 1e-10)):
                ...
                bonus = int(
                    np.random.choice(
                        self.vocab_size, p=residual / max(np.sum(residual), 1e-10)
                    )
                )
                break
    ...
    def acceptance_rate(self) -> float:
        ...
        return (
            self._accept_count / self._verify_count if self._verify_count > 0 else 0.0
        )
