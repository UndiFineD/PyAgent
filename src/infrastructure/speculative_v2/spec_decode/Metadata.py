...
class SpecDecodeMetadataV2:
    ...
    def build_logits_indices(self) -> None:
        ...
        self.bonus_logits_indices = [
            self.cu_num_draft_tokens[i] - 1 for i in range(batch_size)
        ]
...
class TreeVerificationMetadata:
    ...
    @classmethod
    def from_tree(
        cls, tree_tokens: list[list[int]], tree_parents: list[list[int]]
...
class SpecDecodeMetadataFactory:
    ...
    def create_simple(
        ...
        return SpecDecodeMetadataV2(
            draft_token_ids=draft_tokens, num_draft_tokens=num_draft
        )
