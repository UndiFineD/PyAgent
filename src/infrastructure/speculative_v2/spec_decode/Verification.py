    ...
    def verify_tree(
        ...
        for path_idx in range(tree_metadata.num_paths):
            ...
            if path_idx >= len(draft_logprobs) or path_idx >= len(target_logprobs):
                continue
            path_draft_lp, path_target_lp = (
                draft_logprobs[path_idx],
                target_logprobs[path_idx],
            )
            path_metadata = SpecDecodeMetadataV2(
                draft_token_ids=path_tokens, num_draft_tokens=[len(path_tokens)]
            )
            result = self._verify_rejection_sampling(
...
class StreamingVerifier:
    ...
    def add_token(
        self, token: int, draft_logprob: float, target_logprob: float
    ) -> bool | None:
        import random
