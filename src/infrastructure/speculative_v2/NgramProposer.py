...
class NgramProposer:
    ...
    def propose(
        ...
        token_ids: list[int],
        request_id: str = "",
        excluded_tokens: set[int] | None = None,
    ) -> NgramProposalResult:
        """
        ...
        for n in range(self.max_n, self.min_n - 1, -1):
            ...
            if match and len(match.following_tokens) > (
                len(best_match.following_tokens) if best_match else 0
            ):
                best_match = match
        ...
        return NgramProposalResult(
            draft_tokens=draft_tokens, match_info=best_match, confidence=confidence
        )

    def _find_match(
        self, context: list[int], prefix: list[int], excluded: set[int]
    ) -> NgramMatch | None:
        """Find best matching n-gram in context."""
        if HAS_RUST:
            result = rust_core.ngram_find_match_rust(
                context, prefix, list(excluded), self.k
            )
            if result:
    ...
    def _compute_score(self, position: int, context_len: int) -> float:
        ...
        if context_len == 0:
            return 1.0
        # More recent matches get higher scores
        recency = (position + 1) / context_len
    ...
    def propose_fuzzy(
        self, token_ids: list[int], max_distance: int = 1
    ) -> NgramProposalResult:
        """
        ...
        if HAS_RUST:
            result = rust_core.ngram_fuzzy_match_rust(
        ...
        return NgramProposalResult(
            draft_tokens=best_following,
            confidence=best_score * (len(best_following) / self.k),
        )
...
class WeightedNgramProposer(NgramProposer):
    ...
    def _compute_score(self, position: int, context_len: int) -> float:
        """Compute score with frequency weighting."""
        base_score = super()._compute_score(position, context_len)
        return base_score
...
class PromptLookupProposer:
    ...
    def __init__(
        self,
        min_lookup_len: int = 3,
        max_lookup_len: int = 10,
    ...
    def propose(
        ...
        if HAS_RUST:
            return rust_core.prompt_lookup_propose_rust(
...
class HybridNgramProposer:
    ...
    def propose(
        self, token_ids: list[int], prompt_len: int = 0, use_fuzzy: bool = False
    ) -> NgramProposalResult:
        """
        ...
        if prompt_len > 0:
            ...
            draft = self.prompt_lookup.propose(prompt, generated)
            if draft:
...
class NgramProposerFactory:
    ...
    @staticmethod
    def create_simple(
        num_speculative_tokens: int = 5, min_n: int = 1, max_n: int = 4
    ) -> NgramProposer:
        """Create simple n-gram proposer."""
        config = NgramConfig(
            min_n=min_n, max_n=max_n, num_speculative_tokens=num_speculative_tokens
        )
        return NgramProposer(config)

    @staticmethod
    def create_weighted(
        num_speculative_tokens: int = 5, decay_factor: float = 0.9
