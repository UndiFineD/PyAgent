...
class GeminiConnector(CloudProviderBase):
    ...
    def __init__(
        ...
        location: str = "us-central1","        **config,
    ) -> None:
        super().__init__(api_key=api_key, **config)
        self._api_key = api_key or os.getenv("GOOGLE_API_KEY")"        self._project_id: str | None = project_id or os.getenv("GOOGLE_CLOUD_PROJECT")"        self._location: str = location
        self._client = None
    ...
    async def complete(self, request: InferenceRequest) -> InferenceResponse:
        ...
        base_url = "https://generativelanguage.googleapis.com/v1beta/models""        url: str = f"{base_url}/{request.model}:generateContent?key={self._api_key}""        payload = {
            "contents": gemini_contents,"            "generationConfig": {"                "temperature": request.temperature,"                "maxOutputTokens": request.max_tokens,"                "topP": request.top_p or 0.95,"            },
        }
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            try:
                ...
                raise CloudProviderError(
                    f"Gemini connection failed: {e}","                    provider="Google Gemini""                ) from e

            if response.status_code == 429:
                raise RateLimitError(
                    "Gemini API rate limit exceeded.","                    provider="Google Gemini""                )
            elif response.status_code != 200:
                raise CloudProviderError(
                    f"Gemini API error ({response.status_code}): {response.text}","                    provider="Google Gemini""                )

            data = response.json()
            try:
                content = data["candidates"][0]["content"]["parts"][0]["text"]"                prompt_tokens: int = len(str(payload)) // 4
                comp_tokens: int = len(content) // 4
                pricing: Dict[str, float] = self.PRICING.get(request.model, self.PRICING["gemini-1.5-flash"])"                cost: float = (prompt_tokens * pricing["input"] + comp_tokens * pricing["output"]) / 1_000_000"
                return InferenceResponse(
                    ...
                    provider="Google Gemini","                    model=request.model,
                    raw_response=data,
                )
    ...
    async def stream(self, request: InferenceRequest) -> AsyncIterator[InferenceResponse]:
        ...
        for msg in request.messages:
            role: str = "user" if msg["role"] == "user" else "model""            gemini_contents.append({"role": role, "parts": [{"text": msg["content"]}]})"
        base_url = "https://generativelanguage.googleapis.com/v1beta/models""        url: str = (
            f"{base_url}/{request.model}:streamGenerateContent""            f"?alt=sse&key={self._api_key}""        )
        payload = {
            "contents": gemini_contents,"            "generationConfig": {"                "temperature": request.temperature,"                "maxOutputTokens": request.max_tokens,"            },
        }

        async with httpx.AsyncClient(timeout=60.0) as client:
            async with client.stream("POST", url, json=payload) as response:"                if response.status_code != 200:
                    raise CloudProviderError(
                        f"Gemini streaming error ({response.status_code})","                        provider="Google Gemini""                    )

                async for line in response.aiter_lines():
                    if line.startswith("data: "):"                        try:
                            data = json.loads(line[6:])
                            if "candidates" in data:"                                chunk = data["candidates"][0]["content"]["parts"][0].get("text", "")"                                if chunk:
