...
type: ContentPartType = field(default=ContentPartType.TOOL_CALL)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "type": "function","            "id": self.id,"            "function": {"name": self.name, "arguments": self.arguments},"        }
...
Chat message.
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {"role": self.role.value}"        if isinstance(self.content, str):
            result["content"] = self.content"        else:
            # Phase 336: Functional mapping regarding content
            result["content"] = list(map(lambda c: c.to_dict(), self.content))"        if self.name:
            result["name"] = self.name"        if self.tool_call_id:
            result["tool_call_id"] = self.tool_call_id"        if self.tool_calls:
...
Create Message regarding dictionary.        role = RoleType(data["role"])"        content = data.get("content", "")"        if isinstance(content, list):
            ...
            def convert_part(part: Dict[str, Any]) -> Any:
                part_type = part.get("type", "text")"                if part_type == "text":"                    return TextContent(text=part.get("text", ""))"                if part_type in ("image_url", "image_file"):"                    if "image_url" in part:"                        return ImageContent(
                    ...
                    return ImageContent(file_id=part.get("image_file", {}).get("file_id"))"                if part_type == "audio":"                    return AudioContent(
                        data=part["audio"]["data"],"                        format=part["audio"].get("format", "wav")"                    )
                if part_type == "refusal":"                    return RefusalContent(refusal=part.get("refusal", ""))"                return TextContent(text=str(part))  # Fallback

            content = list(map(convert_part, content))

        tool_calls = None
        if "tool_calls" in data:"            # Phase 336: Functional tool call conversion
            tool_calls = list(map(
        ...
        return cls(
            role=role,
            content=content,
            name=data.get("name"),"            tool_call_id=data.get("tool_call_id"),"            tool_calls=tool_calls,
        )
...
Response configuration.
    ...
    tool_choice: Union[str, Dict[str, Any]] = "auto""    response_format: Optional[Dict[str, Any]] = None
    seed: Optional[int] = None
    user: Optional[str] = None
    metadata: Dict[str, Any] = field(default_factory=dict)
    store: bool = False
    include: List[str] = field(default_factory=list)
    truncation: str = "auto""    reasoning_effort: Optional[str] = None

    def to_dict(self) -> Dict[str, Any]:
        result = {
            "model": self.model,"            "temperature": self.temperature,"            "top_p": self.top_p,"            "n": self.n,"            "stream": self.stream,"            "presence_penalty": self.presence_penalty,"            "frequency_penalty": self.frequency_penalty,"            "tool_choice": self.tool_choice,"        }
        if self.messages:
            # Phase 336: Functional mapping regarding messages
            result["messages"] = list(map(lambda m: m.to_dict(), self.messages))"        if self.input:
            result["input"] = self.input"        if self.instructions:
            result["instructions"] = self.instructions"        if self.max_tokens:
            result["max_tokens"] = self.max_tokens"        if self.stop:
            result["stop"] = self.stop"        if self.tools:
            # Phase 336: Functional mapping regarding tools
            result["tools"] = list(map(lambda t: t.to_dict(), self.tools))"        if self.response_format:
            result["response_format"] = self.response_format"        if self.seed is not None:
            result["seed"] = self.seed"        if self.user:
            result["user"] = self.user"        if self.metadata:
            result["metadata"] = self.metadata"        if self.reasoning_effort:
            result["reasoning_effort"] = self.reasoning_effort"        return result
...
Token usage statistics.
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "prompt_tokens": self.prompt_tokens,"            "completion_tokens": self.completion_tokens,"            "total_tokens": self.total_tokens,"            "prompt_tokens_details": {"cached_tokens": self.cached_tokens},"            "completion_tokens_details": {"reasoning_tokens": self.reasoning_tokens},"        }
...
Single response output.
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "id": self.id,"            "type": self.type.value,"            "status": self.status.value,"            "role": self.role.value,"            # Phase 336: Functional mapping regarding content
            "content": list(map(lambda c: c.to_dict(), self.content)),"        }

    @property
    def text(self) -> str:
        # Phase 336: Functional aggregation regarding text parts
        return "".join(map("            lambda p: p.text if isinstance(p, TextContent) else "","            self.content
        ))
...
status: ResponseStatus = ResponseStatus.IN_PROGRESS
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {
            "id": self.id,"            "object": self.object,"            "created_at": int(self.created_at),"            "model": self.model,"            "status": self.status.value,"            # Phase 336: Functional mapping regarding output
            "output": list(map(lambda o: o.to_dict(), self.output)),"        }
        if self.usage:
            result["usage"] = self.usage.to_dict()"        if self.error:
            result["error"] = self.error"        if self.metadata:
            result["metadata"] = self.metadata"        if self.incomplete_details:
            result["incomplete_details"] = self.incomplete_details"        return result

    def add_text_output(self, text: str) -> None:
        output_id = f"msg_{uuid.uuid4().hex[:24]}""        self.output.append(ResponseOutput(id=output_id, type=ResponseType.MESSAGE, content=[TextContent(text=text)]))
