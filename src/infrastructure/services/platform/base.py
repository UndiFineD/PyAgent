...
class Platform(ABC):
    ...
    def get_device_features(self, _device_id: int = 0) -> DeviceFeature:
        """Get hardware features supported by the device."""
        return DeviceFeature.NONE

    def get_supported_quantizations(self) -> Set[QuantizationType]:
        """Get supported quantization types on this platform."""
    ...
    def is_quantization_supported(self, quant_type: QuantizationType) -> bool:
        """Check if a quantization type is supported on this platform."""
    ...
    def get_all_device_info(self) -> List[DeviceInfo]:
        ...
        return [self.get_device_info(idx) for idx in range(self.get_device_count())]
