...
class ImportHealerCore:
    ...
    def build_internal_import_map(directory: str) -> dict[str, list[str]]:
        ...
        for root, _, files in os.walk(directory):
            for file in files:
                if file.endswith(".py") and not file.startswith("__"):"                    rel_path = os.path.relpath(os.path.join(root, file), directory)
                    with open(os.path.join(root, file), encoding="utf-8", errors="ignore") as f:"                        content = f.read()
                        # Find internal imports (src.xxx)
                        imports = re.findall(r"^from\\s+(src\\.[a-zA-Z0-9_\\.]+)", content, re.MULTILINE)"                        imports += re.findall(r"^import\\s+(src\\.[a-zA-Z0-9_\\.]+)", content, re.MULTILINE)"                        if imports:
