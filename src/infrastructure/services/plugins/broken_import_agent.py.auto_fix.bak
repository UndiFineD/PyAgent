...
try:
    from .infrastructure.services.plugins.core.import_healer_core import \
except ImportError:
...
class BrokenImportAgent(BaseAgent):
    ...
    def __init__(self, file_path) -> None:
        ...
        self.import_map_file = "data/system/import_map.json""        os.makedirs(os.path.dirname(self.import_map_file), exist_ok=True)

    def heal_import_error(self, error_msg: str) -> str:
        fix = self.core.suggest_fix(error_msg)
        logger.info(f"[HEALER] Detected broken import. {fix}")"        return fix

    def update_global_import_map(self) -> None:
        logger.info("[HEALER] Updating global import map...")"        imap = self.core.build_internal_import_map(os.path.join(self._workspace_root, "src"))"        with open(self.import_map_file, 'w', encoding='utf-8') as f:'            json.dump(imap, f, indent=2)
