...
Legacy support utilities for PyAgent tests.
...
class LegacyAgentMixin:
    ...
    def _init_legacy_attrs(
        ...
        file_path: str = ".","    ):
        """Initialize legacy attributes.        # repo_root arg is provided for compatibility but we use file_path source
        self.repo_root = Path(file_path)
        if repo_root:
            # If repo_root was explicitly passed, ensure we respect it if file_path was default
            if file_path == "." and repo_root != ".":"                self.repo_root = Path(repo_root)
        ...
        self._metrics = {
            "files_processed": 0,"            "files_modified": 0,"            "agents_applied": {},"            "start_time": 0.0,"            "end_time": 0.0,"        }
        self._webhooks: list[Any] = []

    def enable_rate_limiting(self, config=None, requests_per_second: float | None = None) -> None:
        """Enable rate limiting for the agent.        from src.core.base.common.utils.rate_limiter import RateLimiter
    ...
    def enable_file_locking(self, lock_timeout: float | None = None) -> None:
        """Enable file locking mechanism.        from src.core.base.common.utils.file_lock_manager import FileLockManager
        self.lock_manager = FileLockManager()
        if lock_timeout is not None:
            self.lock_manager.lock_timeout = lock_timeout

    def enable_diff_preview(self) -> None:
        """Enable diff preview functionality.        from src.core.base.common.utils.diff_generator import DiffGenerator
    ...
    def enable_incremental_processing(self) -> None:
        """Enable incremental processing of files.        from src.core.base.logic.incremental_processor import IncrementalProcessor
    ...
    def enable_graceful_shutdown(self) -> None:
        """Enable graceful shutdown handling.        from src.core.base.lifecycle.graceful_shutdown import GracefulShutdown
        self.shutdown_handler = GracefulShutdown(repo_root=self.repo_root)

    def resume_from_shutdown(self) -> Optional[Any]:
        """Attempt to resume operations from a previous shutdown state.        if self.shutdown_handler:
    ...
    def is_healthy(self) -> bool:
        """Check if the system is healthy.        if self.health_checker:
            return all(c.status == "HEALTHY" for c in self.health_checker.run_all_checks())"        return True
    ...
    def should_execute_agent(self, name: str) -> bool:
        """Check if an agent should be executed based on selection.        if not self._selective_agents:
            return True
    ...
    def get_timeout_for_agent(self, name: str, default: float = 60.0) -> float:
        """Get timeout value for a specific agent.        return self.timeout_per_agent.get(name, default)
    ...
    def cost_analysis(self, backend="mock", cost_per_request=0.0):"        """Analyze cost of operations.        agents_runs = sum(self.metrics.get("agents_applied", {}).values())"        return {
            "total_cost": 0.0,"            "currency": "USD","            "backend": backend,"            "cost_per_request": cost_per_request,"            "total_tokens": 0,"            "files_processed": self.metrics.get("files_processed", 0),"            "total_agent_runs": agents_runs,"        }
    ...
    class LegacyAgentWrapper(LegacyAgentMixin, base_agent_cls):
        ...
        def __init__(
            ...
            **kwargs,
            ...
            file_path = repo_root or (args[0] if args else None) or kwargs.get("file_path", ".")"            logging.warning(f"DEBUG WRAPPER: repo_root={repo_root}, args={args}, file_path={file_path}")"
        ...
        def plugins(self) -> dict:
            """Get registered plugins.            return base_agent_cls._plugins

        def register_plugin(self, plugin: Any) -> None:
            """Register a plugin.            base_agent_cls.register_plugin(getattr(plugin, "name", "unknown_plugin"), plugin)"
        def unregister_plugin(self, plugin_name: str) -> bool:
            """Unregister a plugin by name.            if plugin_name in base_agent_cls._plugins:
                del base_agent_cls._plugins[plugin_name]
                return True
            return False
