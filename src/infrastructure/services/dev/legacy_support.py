...
class LegacyAgentMixin:
    ...
    def _init_legacy_attrs(
        ...
        # repo_root arg is provided for compatibility but we use file_path source
    ...
    def enable_rate_limiting(self, config=None, requests_per_second: float | None = None) -> None:
        """Enable rate limiting for the agent."""
    ...
    def enable_diff_preview(self) -> None:
        """Enable diff preview functionality."""
    ...
    def enable_incremental_processing(self) -> None:
        """Enable incremental processing of files."""
    ...
    def get_changed_files(self, files: list[Path]):
        """Get list of changed files using incremental processor."""
    ...
    def reset_incremental_state(self):
        """Reset the state of incremental processor."""
    ...
    def resume_from_shutdown(self) -> Optional[Any]:
        """Attempt to resume operations from a previous shutdown state."""
    ...
    def should_execute_agent(self, name: str) -> bool:
        """Check if an agent should be executed based on selection."""
    ...
    def create_file_snapshot(self, _file_path):
        """Create a snapshot of a file (mock implementation)."""
    ...
    def restore_from_snapshot(self, _snapshot_id, *_args):
        """Restore file from snapshot (mock implementation)."""
        return False

    def load_cascading_codeignore(self, path=None):
        """Load codeignore patterns cascading from repo root."""
...
def create_legacy_agent_wrapper(base_agent_cls):
    ...
    class LegacyAgentWrapper(LegacyAgentMixin, base_agent_cls):
        ...
        def plugins(self) -> dict:
            ...
            return base_agent_cls._plugins

        def register_plugin(self, plugin: Any) -> None:
            """Register a plugin."""
            base_agent_cls.register_plugin(getattr(plugin, "name", "unknown_plugin"), plugin)

        def unregister_plugin(self, plugin_name: str) -> bool:
            """Unregister a plugin by name."""
            if plugin_name in base_agent_cls._plugins:
