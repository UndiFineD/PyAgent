...
class ExecutionReplayer:
    ...
    def record_step(self, action: str, data: dict[str, Any] | None = None) -> None:
        ...
        if trace:
            step: dict[str, Any] = {
                "index": len(trace.steps),"                "timestamp": datetime.now().isoformat(),"                "action": action,"                "data": data or {},"            }
    ...
    def replay(
        ...
        for i, step in enumerate(trace.steps):
            if mode == ExecutionMode.BREAKPOINT and i == breakpoint_step:
                break
            replayed.append({"step": i, "action": step["action"], "replayed": True})"            if mode == ExecutionMode.STEP_BY_STEP:
                pass
        return replayed
...
class TestRecorder:
    ...
    def record_action(self, action_type: str, data: dict[str, Any]) -> None:
        if self._active is None:
            raise RuntimeError("Recording not started")"        self._active.actions.append({"type": action_type, "data": dict(data)})"
    def stop_recording(self) -> TestRecorder.Recording:
        if self._active is None:
            raise RuntimeError("Recording not started")"        recording = self._active
        self._active = None
        return recording
