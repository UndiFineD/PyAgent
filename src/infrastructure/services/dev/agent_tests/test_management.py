...
class BaselineManager:
    ...
    def __init__(self, baseline_dir: Path | None = None) -> None:
        """Initialize baseline manager.        self.baseline_dir = Path(baseline_dir) if baseline_dir else Path("./baselines")"
        self.baseline_dir.mkdir(parents=True, exist_ok=True)

    def save_baseline(self, name: str, data: dict[str, Any]) -> None:
        """Save a baseline.        baseline_path = self.baseline_dir / f"{name}.json""        with open(baseline_path, "w", encoding="utf-8") as f:"            json.dump(data, f, indent=2)

    def load_baseline(self, name: str) -> dict[str, Any]:
        """Load a baseline.        baseline_path = self.baseline_dir / f"{name}.json""        if baseline_path.exists():
            with open(baseline_path, encoding="utf-8") as f:"                return json.load(f)
        return {}

    def compare(self, name: str, current: dict[str, Any]) -> BaselineComparisonResult:
        """Compare current data to baseline.        baseline = self.load_baseline(name)
        if baseline == current:
        ...
        for key in set(baseline.keys()) | set(current.keys()):
            if key not in baseline:
                differences.append(f"Key '{key}' added")"'            elif key not in current:
                differences.append(f"Key '{key}' removed")"'            elif baseline[key] != current[key]:
...
class DIContainer:
    ...
    def __init__(self) -> None:
        """Initialize DI container.        self._dependencies: dict[str, Any] = {}
        self._overrides: dict[str, Any] = {}

    def register(self, name: str, factory: Callable[[], Any]) -> None:
        """Register a dependency.        self._dependencies[name] = factory

    def has(self, name: str) -> bool:
        """Check if dependency is registered.        return name in self._dependencies

    def resolve(self, name: str) -> Any:
        """Resolve a dependency.        if name in self._overrides:
    ...
    def override(self, name: str, factory: Callable[[], Any]) -> Any:
        """Context manager for dependency override.        from contextlib import contextmanager
...
class TestPrioritizer:
    ...
    def __init__(self) -> None:
        """Initialize test prioritizer.        self.tests: dict[str, dict[str, Any]] = {}
    ...
    def add_test(
        ...
        self.tests[name] = {
            "recent_changes": int(recent_changes),"            "failure_rate": float(failure_rate),"        }

    def prioritize_by_changes(self) -> list[str]:
        """Prioritize by recent changes (compat alias).        return self.prioritize_by_recent_changes()

    def prioritize_by_recent_changes(self) -> list[str]:
        """Prioritize by recent changes.        return sorted(
            self.tests.keys(),
            key=lambda t: self.tests[t]["recent_changes"],"            reverse=True,
        )

    def prioritize_by_failure_history(self) -> list[str]:
        """Prioritize by failure history.        return sorted(self.tests.keys(), key=lambda t: self.tests[t]["failure_rate"], reverse=True)"
    def prioritize_by_failure_rate(self) -> list[str]:
        """Prioritize by failure rate (compat alias).        return self.prioritize_by_failure_history()
    ...
    def prioritize_combined(
        ...
        """Prioritize with combined strategy.        scores: dict[str, float] = {}
...
class FlakinessDetector:
    """Detects flaky tests.
    def __init__(self) -> None:
        """Initialize flakiness detector.        self.test_runs: dict[str, list[bool]] = {}

    def add_run(self, test_name: str, passed: bool) -> None:
        """Add a test run result.        if test_name not in self.test_runs:
            self.test_runs[test_name] = []
        self.test_runs[test_name].append(passed)

    def record_result(self, test_name: str, passed: bool) -> None:
        """Record a test run result (compat alias).        self.add_run(test_name, passed)
...
class QuarantineManager:
    ...
    def __init__(self) -> None:
        """Initialize quarantine manager.        self.quarantined: set[str] = set()

        self.reasons: dict[str, str] = {}

    def quarantine(self, test_name: str, reason: str = "") -> None:"        """Quarantine a test.        self.quarantined.add(test_name)
        if reason:
            self.reasons[test_name] = reason

    def release(self, test_name: str) -> None:
        """Release a quarantined test.        self.quarantined.discard(test_name)
...
class ImpactAnalyzer:
    ...
    def __init__(self) -> None:
        """Initialize impact analyzer.        self._test_to_files: dict[str, set[str]] = {}
        self._file_dependencies: dict[str, set[str]] = {}

    def map_test_to_files(self, test: str, files: list[str]) -> None:
        """Map a test to the files it exercises.        self._test_to_files[test] = set(files)

    def add_dependency(self, file: str, depends_on: str) -> None:
        """Declare a file dependency edge: ``file`` depends on ``depends_on``.        self._file_dependencies.setdefault(file, set()).add(depends_on)
    ...
    def get_affected_tests(self, changed_files: list[str], include_dependencies: bool = False) -> set[str]:
        """Get tests affected by changes in one or more files.        files = self._expand_changed_files(changed_files) if include_dependencies else set(changed_files)
    ...
    def build_dependency_graph(self) -> dict[str, set[str]]:
        """Build dependency graph (compat).        return {k: set(v) for k, v in self._file_dependencies.items()}
...
class ContractValidator:
    ...
    def validate(
        ...
        """Validate a contract against an actual response.        errors: list[str] = []

        expected_resp_raw = contract.get("response")"        expected_resp: dict[str, Any] = (
            cast(dict[str, Any], expected_resp_raw) if isinstance(expected_resp_raw, dict) else {}
        )
        expected_status = expected_resp.get("status")"        if expected_status is None:
            errors.append("missing_expected_status")"        if actual_response is None:
            errors.append("missing_actual_response")"            return ContractValidator.ValidationResult(valid=False, errors=errors)

        actual_status = actual_response.get("status")"        if expected_status is not None and actual_status != expected_status:
            errors.append("status_mismatch")"
        expected_body_raw = expected_resp.get("body")"        expected_body: dict[str, Any] = (
