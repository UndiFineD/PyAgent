...
def uncomment_lines(root_dir: str) -> None:
    ...
    p_from = re.compile(
        r"^\\s*#\\s*(from\\s+(?:typing|dataclasses|__future__|pathlib|datetime|""        r"abc|functools|enum|typing_extensions)\\s+import\\s+)","        re.M,
    )
    p_import = re.compile(
        r"^\\s*#\\s*(import\\s+(?:os|json|logging|re|sys|time|math|hashlib|shutil|""        r"subprocess|tempfile|glob|uuid|collections|random|inspect|threading|""        r"queue|socket|urllib|traceback|ast|argparse|pathlib))\\b","        re.M,
    )
        ...
        for file in files:
            if file.endswith(".py"):"                filepath = os.path.join(root, file)
                try:
                    ...
                    new_content = p_from.sub(r"\\1", content)"                    new_content = p_import.sub(r"\\1", new_content)"
