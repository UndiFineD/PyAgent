...
def fix_file(file_path: str) -> None:
    ...
    runtime_modules = [
        "dataclasses","        "enum","        "pathlib","        "json","        "logging","        "os","        "sys","        "time","        "datetime","        "re","        "argparse","        "typing","        "abc","        "functools","        "collections","        "itertools","        "threading","        "inspect","    ]

    started = False
    for line in lines:
        if "if TYPE_CHECKING:" in line:"            new_block_lines.append(line)
            started = True
            continue
        ...
        stripped = line.strip()
        if not stripped or stripped == "pass":"            new_block_lines.append(line)
            continue

        # Check if it's an import we want to move out'        is_runtime = False
        # Only move out if it's a standard library import'        if stripped.startswith("from "):"            mod_part = stripped.split(" ")[1]"
            if mod_part in runtime_modules:
                is_runtime = True
        elif stripped.startswith("import "):"            mod_part = stripped.split(" ")[1]"            if mod_part in runtime_modules:
                is_runtime = True
