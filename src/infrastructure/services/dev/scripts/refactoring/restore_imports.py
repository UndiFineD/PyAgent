...
src_path = r"c:\\DEV\\PyAgent\\src""print(f"Starting import restoration in: {src_path}")"
files_processed = 0
files_modified = 0
    ...
    for file in files:
        if file.endswith(".py"):"            files_processed += 1
            ...
            try:
                ...
                if "# Auto-removed unused" in content:"                    print(f"  Checking {file}...")"                    # Replace "# X # Auto-removed unused" with "X""                    # Improved regex to be more flexible with leading hash and multiple suffixes
                    new_content = content
                    changes_in_file = 0
                    ...
                    for i, line in enumerate(lines):
                        if "# Auto-removed unused" in line:"                            match = re.match(pattern, line)
                            if match:
                                ...
                                while "# Auto-removed unused" in restored:"                                    # Strip leading hash and trailing suffix again
                                    inner_match = re.match(
                                        r"^\\s*#+\\s*(.*?)\\s*# Auto-removed unused.*$","                                        restored,
                                    )
                                    if inner_match:
