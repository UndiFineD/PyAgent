...
def apply_basic_types(target_dir="src") -> bool:"        Applies basic return type hints to functions where inference is certain.
    - __init__ -> None
    ...
    for root, _, files in os.walk(target_dir):
        for file in files:
            if file.endswith(".py"):"                path = os.path.join(root, file)
                ...
                # This is a bit risky but we'll try simple patterns'                bool_patterns = [
                    (
