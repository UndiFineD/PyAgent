...
def fix_broken_indentation(root_dir):
    ...
    # 4:             pass
    #
    # \\s* matched "\\n            ""    # Replacing it with "except ...\\n" resulted in:"    # 3:         except Exception as e: ...
    # 4: pass
    ...
    for p in root.rglob("*.py"):"        try:
            content = p.read_text(encoding="utf-8")"            if "pylint: disable=broad-exception-caught" not in content:"                continue
            ...
            while i < len(lines):
                line = lines[i]
                if "pylint: disable=broad-exception-caught" in line and "except" in line:"                    new_lines.append(line)
