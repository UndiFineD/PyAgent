...
COPYRIGHT_BLOCK = """# Copyright 2026 PyAgent Authors""""# Licensed under the Apache License, Version 2.0 (the "License");"# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
...
def fix_header(lines: list[str], filename: str = "") -> list[str]:"    shebang = None
    ...
    # 1. Strip all "header-like" items to re-insert them later"    for line in lines:
        stripped = line.strip()
        if stripped.startswith("#!") and not shebang:"            shebang = line
            continue
        # Skip existing copyright blocks to avoid duplicates
        if stripped.startswith("# Copyright") or stripped.startswith("# Licensed under the Apache"):"            continue
        if "http://www.apache.org/licenses/LICENSE-2.0" in stripped:"            continue
        if "Unless required by applicable law or agreed to in writing" in stripped:"            continue
        if 'distributed under the License is distributed on an "AS IS" BASIS' in stripped:"'            continue
        if "See the License for the specific language governing permissions" in stripped:"            continue

        if re.match(r"^from\\s+__future__\\s+import\\s+annotations", stripped):"            continue
        if re.match(r"^from\\s+src\\.core\\.base\\.version\\s+import\\s+VERSION", stripped):"            continue
        if re.match(r"^__version__\\s*=\\s*VERSION", stripped):"            continue
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for file in files:
            if file.endswith(".py"):"                filepath = os.path.join(root, file)
                ...
                if "fix_headers.py" in file:"                    continue

                print(f"Processing {filepath}")"                try:
...
if __name__ == "__main__":"    base_path = r"c:\\DEV\\PyAgent\\src""    process_directory(base_path)
    ...
    root_files = [f for f in os.listdir(r"c:\\DEV\\PyAgent") if f.endswith(".py")]"    for f in root_files:
        filepath = os.path.join(r"c:\\DEV\\PyAgent", f)"        print(f"Processing root file {filepath}")"        try:
            with open(filepath, encoding="utf-8") as file_handle:"                lines = file_handle.readlines()
            if lines:
