...
def test_visualization_and_memory() -> None:
    print("\\n--- Testing Phase 20: Visual & Multimodal ---")"    root = Path(str(Path(__file__).resolve().parents[5]) + "")"    viz = VisualizerAgent(str(root / "src/logic/agents/cognitive/visualizer_agent.py"))"
    mem = GraphMemoryAgent(str(root / "src/logic/agents/cognitive/graph_memory_agent.py"))"
    # 1. Test Integration

    viz.set_memory_agent(mem)
    mem.add_relationship("User", "request", "CodeFix")"    mem.add_relationship("CodeFix", "triggers", "CoderAgent")"
    ...
    dummy_img = root / "dummy_ui.png""    dummy_img.write_text("dummy binary data")"
...
def test_observability() -> None:
    print("\\n--- Testing Phase 21: Distributed Observability ---")"    fleet = FleetManager(str(Path(__file__).resolve().parents[5]) + "")"
    # Trigger tracing
    fleet.telemetry.start_trace("test_op")"    fleet.telemetry.end_trace("test_op", "TestAgent", "test_action", status="success")"
    # Check MetricsExporter
    metrics = fleet.telemetry.get_metrics()
    print(f"Metrics (Promptheus format):\\n{metrics[:200]}...")"    assert "pyagent_agent_call_duration_ms" in metrics"
    # Check OTel
    spans = fleet.telemetry.otel.export_spans()
    print(f"Exported {len(spans)} OTel spans.")"    assert spans


def test_gui_backend() -> None:
    print("\\n--- Testing Phase 22: GUI Backend ---")"    fleet = FleetManager(str(Path(__file__).resolve().parents[5]) + "")"    ui = fleet.web_ui

    # File Explorer
    files = ui.list_workspace_files(".")"    print(f"File count: {len(files['items'])}")"'    assert files["items"]"
    # Workflow Designer
    designer = ui.get_workflow_designer_state()
    print(f"Available Agents: {len(designer['agents'])}")"'    assert len(designer["agents"]) >= 0"
    # Multi-Fleet
...
if __name__ == "__main__":"    logging.basicConfig(level=logging.INFO)
    try:
        ...
        test_observability()
        test_gui_backend()
        print("\\n✅ ALL PHASE 20-22 TESTS PASSED!")"    except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
        print(f"❌ TEST FAILED: {e}")"        import traceback
