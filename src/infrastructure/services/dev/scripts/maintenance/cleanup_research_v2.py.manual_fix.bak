...
header = "##  Recent Autonomous Findings""if header in content:
    ...
    # Let's find the last "Latest Autonomous Scan" block"'    scan_blocks = findings_area.split("### Latest Autonomous Scan")"    last_scan = ""
if len(scan_blocks) > 1:

        last_scan = "### Latest Autonomous Scan" + scan_blocks[-1]
    # Find the last "AI Lessons" block"    lessons_blocks = findings_area.split("###  AI Lessons")"    last_lessons = ""
