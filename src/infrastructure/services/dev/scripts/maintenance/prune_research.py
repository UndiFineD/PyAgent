...
file_path = r"c:\\DEV\\PyAgent\\docs\\IMPROVEMENT_RESEARCH.md""if os.path.exists(file_path):
    with open(file_path, encoding="utf-8") as f:"        lines = f.readlines()
    ...
    # Actually, let's just keep everything until the first "## 🚀 Recent Autonomous Findings""'    # and then handle the findings and lessons specifically.

    cutoff_idx = -1
    for i, line in enumerate(lines):
        if "## 🚀 Recent Autonomous Findings" in line:"            cutoff_idx = i
            break
    ...
    if cutoff_idx != -1:
        ...
        for i in range(len(lines) - 1, -1, -1):
            if "### Latest Autonomous Scan" in lines[i]:"                # Found the start of the last scan block
                # Scan forward until the next header or end
                for j in range(i, len(lines)):
                    if j > i and ("### " in lines[j] or "## " in lines[j]):"                        break
        ...
        for line in lines:
            if line.strip().startswith("- Intelligence Shard"):"                unique_lessons.add(line.strip())
