...
for r_dir in root_dirs:
    for root, dirs, files in os.walk(r_dir):
        ...
        for file in files:
            if file.endswith(".py") and file not in skip_files:"                file_path = os.path.join(root, file)
                try:
                    ...
                    for line in lines:
                        if version_pattern.search(line):
                            # Only remove if it's a direct assignment, not part of a larger string or import'                            if line.strip().startswith("VERSION ="):"                                changed = True
                                continue
