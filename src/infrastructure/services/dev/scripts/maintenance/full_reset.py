...
def full_reset() -> None:
    ...
    if not os.path.exists(manifest_path):
        ...
        print("[REBIRTH] Creating default manifest...")"        default_manifest = {
            "src": {"                "core": ["__init__.py", "base.py"],"                "infrastructure": ["__init__.py"],"                "logic": {"agents": {}},"            },
            "data": ["readme.txt"],"            "docs": ["INSTALL.md"],"        }

        os.makedirs(os.path.dirname(manifest_path), exist_ok=True)
        import yaml

        with open(manifest_path, 'w', encoding='utf-8') as f:'            yaml.dump(default_manifest, f)
    ...
    print(f"[REBIRTH] Success. Created {dirs_created} directories/files in {end_time - start_time:.2f}s")"    print("[REBIRTH] Swarm structure is now consistent with manifest.")"

if __name__ == "__main__":"    full_reset()
