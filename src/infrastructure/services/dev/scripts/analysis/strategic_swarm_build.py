...
def main() -> None:
    ...
    try:
        ...
        print("Γ£à Intelligence Orchestrator Resolved.")
    except AttributeError:
        # Try snake_case direct match
        try:
            intelligence = fleet.intelligence
            print("Γ£à 'intelligence' attribute found via FleetManager delegation.")
        except AttributeError:
            print("Γ¥î Failed to resolve Intelligence Orchestrator. Check BootstrapConfigs.")
            return

    # 2. Read the Strategic Prompt from note.txt
    prompt_path = os.path.join(root, "docs/notes/note.txt")
    if not os.path.exists(prompt_path):
        print(f"Γ¥î Could not find {prompt_path}")
        return
    ...
    print("Γ£à Directive stored in context.")
    ...
    try:
        ...
        if external_lesson:
            ...
            print("Γ£à Insight integrated into Collective Intelligence.")
        else:
            print("ΓÜá∩╕Å External consultation returned no content. Continuing with local synthesis.")
    except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
        print(f"ΓÜá∩╕Å External consult skipped (likely missing API key): {e}")
