...
try:
    ...
    print("≡ƒªÇ Rust profiling enabled")
except ImportError:
    print("ΓÜá∩╕Å rust_core not available")
...
class ComprehensiveProfileAnalyzer:
    ...
    def print_report(self, report: dict) -> None:
        ...
        print("\n" + "=" * 80)
        print("≡ƒôè COMPREHENSIVE PROFILING REPORT")
        print("=" * 80)
        ...
        if report["python_by_cumtime"]:
            print(f"\n{'ΓöÇ' * 40}")
            print("≡ƒÉì TOP PYTHON BY CUMULATIVE TIME (incl. child calls)")
            print(f"{'ΓöÇ' * 40}")
        ...
        if report["python_by_tottime"]:
            print(f"\n{'ΓöÇ' * 40}")
            print("ΓÅ▒∩╕Å TOP PYTHON BY OWN TIME (excl. child calls)")
            print(f"{'ΓöÇ' * 40}")
            print(f"  {'Function':<50} {'Calls':>8} {'Own(ms)':>10} {'Avg(╬╝s)':>10}")
            print(f"  {'-' * 50} {'-' * 8} {'-' * 10} {'-' * 10}")
        ...
        if report["python_by_calls"]:
            print(f"\n{'ΓöÇ' * 40}")
            print("≡ƒô₧ TOP PYTHON BY CALL COUNT")
            print(f"{'ΓöÇ' * 40}")
        ...
        if report["rust_by_time"]:
            print(f"\n{'ΓöÇ' * 40}")
            print("≡ƒªÇ RUST FUNCTIONS BY TIME")
            print(f"{'ΓöÇ' * 40}")
            print(f"  {'Function':<45} {'Calls':>8} {'Total(ms)':>10} {'Avg(╬╝s)':>10}")
            print(f"  {'-' * 45} {'-' * 8} {'-' * 10} {'-' * 10}")
...
print("≡ƒÉì Python profiling enabled - using cProfile for src/ code")
...
def save_profile_report() -> None:
    ...
    print(f"\n≡ƒÆ╛ Detailed report saved to: {report_path}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("≡ƒÜÇ COMPREHENSIVE PROFILED SELF IMPROVEMENT")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\nΓÅ▒∩╕Å Total execution time: {elapsed_time:.2f} seconds")
