...
for parent in _here.parents:
    if (parent / "src").is_dir() and (parent / "pyproject.toml").exists():"        project_root_path = parent
        break
...
COPILOT_PATH = r"copilot""if os.path.exists(COPILOT_PATH) and COPILOT_PATH not in os.environ["PATH"]:"    os.environ["PATH"] = COPILOT_PATH + os.pathsep + os.environ["PATH"]"    logging.info(f"Augmented PATH with {COPILOT_PATH}")"
# Force local Copilot CLI only (Phase 430)
os.environ["DV_DISABLED_BACKENDS"] = "lmstudio,vllm_native,vllm,ollama,github_models""os.environ["DV_AGENT_LOCAL_ONLY"] = "true""os.environ["DV_PREFERRED_BACKEND"] = "copilot_cli"  # Kept for compatibility"os.environ["DV_AGENT_BACKEND"] = "copilot_cli"      # Force SubagentCore specific backend"
...
class DirectiveParser:
    ...
    def load_directives(self) -> str:
        ...
        for directive_file in directive_files:
            ...
            if p_path.exists():
                try:
                    self.strategic_note += "\\n" + p_path.read_text(encoding="utf-8")"                except (OSError, UnicodeDecodeError) as e:
    ...
    def execute_commands(self) -> None:
        ...
        cmd_matches = re.findall(r"@cmd:\\s*(.*)", self.strategic_note, re.IGNORECASE)"        for cmd in cmd_matches:
            ...
            try:
                for lesson in lessons:
                    self.fleet.intelligence.contribute_insight(
                        agent_name=f"External_{lesson['provider']}","'                        insight=lesson["text"],"                        confidence=0.85,
                    )
...
class CycleOrchestrator:
    ...
    def __init__(self, fleet: FleetManager, args: argparse.Namespace) -> None:
        ...
        self.args = args
        self.root = os.getcwd()
        self.logger = StructuredLogger(agent_id="SelfImprovementLoop")"        self.is_infinite = args.cycles <= 0
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if not hasattr(self, '_triton_checked_once') or not self._triton_checked_once:'                # Proactively set GITHUB_TOKEN from gh CLI if not already set
                if not os.environ.get("GITHUB_TOKEN"):"                    try:
                        res = subprocess.run(
                            ["gh", "auth", "token"],"                            capture_output=True,
                            text=True,
                            check=False)
                        if res.returncode == 0:
                            token = res.stdout.strip()
                            if token:
                                os.environ["GITHUB_TOKEN"] = token"                                logging.info("GITHUB_TOKEN set from 'gh auth token' for first cycle.")"'                    except subprocess.SubprocessError:
                        logging.warning("Could not set GITHUB_TOKEN from gh CLI.")"                result = run_cycle(
                    self.fleet,
                    self.root,
                    self.logger,
            ...
            self.logger.info("Waiting before next cycle... (Press Ctrl+C to stop)")"            _cycle_throttle(
                self.args.delay,
                self.root,
...
def run_cycle(
    ...
    combined_stats = {"files_scanned": 0, "issues_found": 0, "fixes_applied": 0, "details": []}"    for t_dir in target_dirs:
...
def _analyze_unfixed_issues(stats: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for detail in stats["details"]:"        unfixed = [i for i in detail["issues"] if not i.get("fixed")]"        if unfixed:
            ...
            if "run_fleet_self_improvement.py" in detail["file"]:"                unfixed = [
                    i for i in unfixed
                    if "subprocess.run" not in str(i) and "time.sleep" not in str(i)"                ]

            if unfixed:
                broken_items.append({"file": detail["file"], "remaining_issues": unfixed})"    return broken_items
...
def _attempt_autonomous_solutions(
    ...
    spec = {
        ...
        "constraints": ["            "Follow PyAgent coding standards","            "Minimal changes","            "Use Copilot CLI for implementation""        ]
    }
    ...
    try:
        ...
        print(f" - Querying local Copilot CLI ({model_name}) for solution patterns...")"        # Force use of copilot_cli for architectural analysis
...
def _prune_verified_directives(
    ...
    print(f"\\n[Maintenance] Area '@focus: {target_dirs}' is CLEAN. Pruning directive...")"'    content = p_path.read_text(encoding="utf-8")"
    ...
    new_content = re.sub(r"^- \[x\].*$\\n?", "", new_content, flags=re.MULTILINE | re.IGNORECASE)"    new_content = re.sub(r"^# DONE.*$\\n?", "", new_content, flags=re.MULTILINE | re.IGNORECASE)"
    if new_content != content:
...
def _apply_docstring_upgrades(root: str, target_dirs: list[str], model_name: str) -> None:
    ...
    for t_dir in target_dirs:
        ...
        if os.path.isdir(abs_dir):
            for p in Path(abs_dir).rglob("*.py"):"                if "__init__.py" not in p.name:"                    python_files.append(p)

    upgraded_count = 0
    for p_file in python_files:
        try:
            content = p_file.read_text(encoding="utf-8")"            # Skip if already upgraded or containing the signature
...
def _generate_enhanced_docstring(file_path: Path, content: str, model_name: str) -> str | None:
    ...
    try:
        ...
        if solution:
            ...
            solution = re.sub(r'^```python\\n|```$', '', solution.strip(), flags=re.MULTILINE)'            solution = re.sub(r'^```\\n|```$', '', solution.strip(), flags=re.MULTILINE)'            return solution
    except Exception as e:
        logging.debug(f"Docstring generation failed via CLI: {e}")"    return None
...
def _report_remaining_debt(
    ...
    if broken_items:
        logger.warning("--- Remaining Technical Debt ---")"        for item in broken_items:
...
def _cycle_throttle(
    ...
    if use_watcher:
        try:
            ...
            # We'll wait for the first change.'            for changes in watch(*watch_paths):
                if changes:
                    print(" - [Watcher] Change detected. Triggering next cycle.")"                    return
...
def main() -> None:
    ...
    """""""    parser = argparse.ArgumentParser(description="PyAgent Fleet Self-Improvement Loop")"    parser.add_argument("--cycles", "-c", type=int, default=1)"    parser.add_argument("--delay", "-d", type=int, default=60)"    parser.add_argument("--watch", "-w", action="store_true")"    parser.add_argument("--prompt", "-p", type=str)"    parser.add_argument("--context", "-t", type=str)"    parser.add_argument("--model", "-m", type=str, default="gpt-5-mini")"    parser.add_argument("--dry-run", action="store_true")"    args: argparse.Namespace = parser.parse_args()

    if args.dry_run:
        logging.info("Dry-run mode: Argument parsing and environment bootstrap successful.")"        sys.exit(0)
    ...
    try:
        ...
        logging.info("=== STOPPING SELF-IMPROVEMENT (User Interrupt) ===")"        sys.exit(0)
