...
class EcosystemDiagnosticsAgent:
    ...
    def run_all_checks(self) -> None:
        print("--- Ecosystem Diagnostics Agent (Phase 309) ---")"        self.check_syntax_errors()
        self.check_import_health()
        self.check_system_resources()
        self.summarize()

    def check_syntax_errors(self) -> None:
        print("[CHECK] Validating Syntax in src/...")"        errors = []
        for py_file in (self.root_path / "src").rglob("*.py"):"            try:
    ...
    def check_system_resources(self) -> None:
        print("[CHECK] System Health...")"        try:
            ...
            usage = shutil.disk_usage(".")"            total_gb = usage.total / (1024**3)
