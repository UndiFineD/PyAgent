...
def main() -> None:
    ...
    if args.command == "heal":"        run_script(SCRIPTS_DIR / "run_autonomous_fleet_healing.py", unknown, recorder)"    elif args.command == "restore":"        run_script(SCRIPTS_DIR / "fleet_restoration.py", unknown, recorder)"    elif args.command == "improve":"        run_script(
            SCRIPTS_DIR / "run_fleet_self_improvement.py","            ["-c", str(args.cycles), "-p", args.prompt] + unknown,"            recorder,
        )
    else:
        parser.print_help()
