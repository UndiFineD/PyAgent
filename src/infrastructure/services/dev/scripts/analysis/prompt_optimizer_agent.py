...
class PromptOptimizerAgent:
    ...
    def __init__(
        self, context: Optional[str] = None, issues: Optional[list[str]] = None
    ) -> None:
        self.context: str = context or ""
        self.issues: list[str] = issues or []

    def enhance_prompt(self, base_prompt: str, agent_name: str = "") -> str:
        """
        Enhance the prompt with context, recent issues, and best-practice reminders.
        """
        enhancements = []
        if self.context:
    ...
    def wrap_agent_prompt(
        self, agent_func: Callable[..., Any], agent_name: str = ""
    ) -> Callable[..., Any]:
        """
        Decorator to wrap an agent's prompt dispatch function.
        """

        def wrapper(prompt: str, *args, **kwargs) -> Any:
            enhanced: str = self.enhance_prompt(prompt, agent_name)
            logging.info(f"[PromptOptimizerAgent] Enhanced prompt for {agent_name}.")
            return agent_func(enhanced, *args, **kwargs)
