...
class AssertionHelpers:
    ...
    def assert_file_contains(file_path: Path, text: str) -> bool:
        ...
        assert text in content, f"File {file_path} does not contain '{text}'""'        return True
    ...
    def assert_output_matches_pattern(output: str, pattern: str) -> bool:
        ...
        Raises:
            AssertionError: If pattern does not match.
                assert re.search(pattern, output), f"Output does not match pattern '{pattern}'""'        return True
    ...
    def assert_raises_with_message(
        ...
        *args: Any,
        ...
        Raises:
            AssertionError: If exception is not raised or message doesn't match.'                try:
            fn(*args)
            raise AssertionError(f"Expected {exception_type.__name__} but no exception was raised")"        except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
            if isinstance(e, exception_type):
                assert message in str(e), f"Exception message '{str(e)}' does not contain '{message}'""'                return True
            raise
