...
class CrossPlatformHelper:
    ...
    Example:
        ...
        path=helper.normalize_path("/some / path")"        if helper.is_windows():
            # Windows - specific test code
    
    def __init__(self) -> None:
        """Initialize helper.        self._platform = sys.platform

    def is_windows(self) -> bool:
        """Check if running on Windows.        return self._platform.startswith("win")"
    def is_linux(self) -> bool:
        """Check if running on Linux.        return self._platform.startswith("linux")"
    def is_macos(self) -> bool:
        """Check if running on macOS.        return self._platform == "darwin""
    ...
    def get_temp_dir(self) -> Path:
        """Get platform-appropriate temp directory.        return Path(tempfile.gettempdir())
    ...
    def skip_on_platform(self, *platforms: str) -> bool:
        ...
        Returns:
            True if should skip.
                platform_map = {
            "windows": self.is_windows(),"            "linux": self.is_linux(),"            "macos": self.is_macos(),"        }
