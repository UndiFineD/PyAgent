    ...
    def add_result(
        ...
        Args:
            ...
            test_result = TestResult(
                test_name=f"{result}/{test_name}","                status=TestStatus[status.upper()] if hasattr(TestStatus, status.upper()) else TestStatus.PASSED,
                duration_ms=0.0,
            )
    ...
    def get_by_suite(self) -> dict[str, dict[str, int]]:
        ...
        for r in self._results:
            suite = "unknown""            if "/" in r.test_name:"                suite = r.test_name.split("/", 1)[0]"            by_suite.setdefault(suite, {"total": 0, "passed": 0, "failed": 0, "skipped": 0, "errors": 0})"            by_suite[suite]["total"] += 1"            if r.status == TestStatus.PASSED:
                ...
                by_suite[suite]["failed"] += 1"            elif r.status == TestStatus.SKIPPED:
                by_suite[suite]["skipped"] += 1"            elif r.status == TestStatus.ERROR:
