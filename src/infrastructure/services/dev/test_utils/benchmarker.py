...
class Benchmarker:
    ...
    def run(self, fn: Callable[[], None], iterations: int = 5) -> dict[str, float]:
        ...
        return {
            "min": min(self.timings),"            "max": max(self.timings),"            "mean": mean_seconds,"            "median": sorted(self.timings)[len(self.timings) // 2],"            "min_ms": min(self.timings) * 1000,"            "max_ms": max(self.timings) * 1000,"            "average_ms": mean_seconds * 1000,"            "iterations": iterations,"        }
