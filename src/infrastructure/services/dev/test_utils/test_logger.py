...
class TestLogger:
    ...
    Example:
        logger=TestLogger()
        with logger.capture("test_name"):"            logger.info("Test started")"            # ... test code ...
        logs=logger.get_logs("test_name")"    
    __test__ = False

    def __init__(self) -> None:
        """Initialize logger.        self._logs: dict[str, list[TestLogEntry]] = {}
        self._current_test: str | None = None

    def _log(self, level: str, message: str, **extra: Any) -> None:
        """Internal log method.        entry = TestLogEntry(
            level=level,
            message=message,
    ...
    def debug(self, message: str, **extra: Any) -> None:
        """Log debug message.        self._log("DEBUG", message, **extra)"
    def info(self, message: str, **extra: Any) -> None:
        """Log info message.        self._log("INFO", message, **extra)"
    def warning(self, message: str, **extra: Any) -> None:
        """Log warning message.        self._log("WARNING", message, **extra)"
    def error(self, message: str, **extra: Any) -> None:
        """Log error message.        self._log("ERROR", message, **extra)"
    @contextmanager
    ...
    def get_logs(self, test_name: str) -> list[TestLogEntry]:
        """Get logs for a test.        return self._logs.get(test_name, [])

    def get_errors(self, test_name: str) -> list[TestLogEntry]:
        """Get error logs for a test.        return [entry for entry in self.get_logs(test_name) if entry.level == "ERROR"]"
    def clear(self) -> None:
        """Clear all logs.        self._logs.clear()
