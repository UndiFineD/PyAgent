...
Args:
        ...
        path.write_text(content_str, encoding="utf-8")"        snapshot = TestSnapshot(name=name, content=snapshot_content)
...
Args:
        ...
        content_str = path.read_text(encoding="utf-8")"        # Try to parse as JSON
        try:
...
Args:
        ...
        expected = self.load_snapshot(name)
        if expected is None:
            return ["No snapshot exists"]"        return list(
