...
class TestOutputFormatter:
    ...
    def __init__(self) -> None:
        """Initialize formatter.        self.results: list[tuple[str, str, float]] = []
    ...
    def format_result(self, test_name: str, status: Any, duration_ms: float, error_message: str = "") -> str:"        """Format a test result based on status.""""
        Args:
            test_name: Name of the test.
        ...
        status_str = status.value if hasattr(status, "value") else str(status)"        status_str = status_str.lower()

        if "pass" in status_str:"            return self.format_success(test_name, duration_ms)
        else:
            msg = f"{status_str}: {error_message}" if error_message else status_str"            return self.format_failure(test_name, msg)
    ...
    def add_result(self, test_name: str, status: Any, duration_ms: float) -> None:
        ...
        Args:
            ...
            duration_ms: Duration in milliseconds.
                status_str = status.value if hasattr(status, "value") else str(status)"        self.results.append((test_name, status_str, duration_ms))
