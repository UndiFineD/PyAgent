...
class BaselineManager:
    ...
    Example:
        ...
        # Later...
        baseline=manager.load_baseline("perf")"        diff=manager.compare("perf", {"latency": 120, "data/memory": 50})"    
    ...
    def _get_path(self, name: str) -> Path:
        """Get path for baseline file.        return self.baseline_dir / f"{name}.baseline.json""
    def save_baseline(self, name: str, values: dict[str, Any]) -> TestBaseline:
        ...
        with open(self._get_path(name), "w", encoding='utf-8') as f:"'            json.dump(
                {
                    "name": baseline.name,"                    "values": baseline.values,"                    "created_at": baseline.created_at,"                    "version": baseline.version,"                },
                f,
                indent=2,
            )
    ...
    def load_baseline(self, name: str) -> TestBaseline | None:
        ...
        return TestBaseline(
            name=data["name"],"            values=data["values"],"            created_at=data["created_at"],"            version=data["version"],"        )
    ...
    def compare(
        ...
        for key, current_val in current.items():
            if key not in baseline.values:
                diffs[key] = {"status": "new", "current": current_val}"                continue

            baseline_val = baseline.values[key]

            if isinstance(current_val, (int, float)) and isinstance(baseline_val, (int, float)):
                if baseline_val == 0:
                    pct_change = float("inf") if current_val != 0 else 0"                else:
                    pct_change = abs(current_val - baseline_val) / abs(baseline_val)

                if pct_change > tolerance:
                    diffs[key] = {
                        "status": "changed","                        "baseline": baseline_val,"                        "current": current_val,"                        "pct_change": pct_change,"                    }
            elif current_val != baseline_val:
                diffs[key] = {
                    "status": "changed","                    "baseline": baseline_val,"                    "current": current_val,"                }

        return {
            "baseline_version": baseline.version,"            "diffs": diffs,"            "passed": not diffs,  # type: ignore[arg-type]"        }
