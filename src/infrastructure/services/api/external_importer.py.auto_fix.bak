...
class ExternalImporter:
    ...
    def import_github_releases(self, owner: str, repo: str, pages: int = 1) -> list[ImportedEntry]:
        ...
        try:
            for page in range(1, pages + 1):
                ...
                if response.status_code == 200:
                    ...
                    logging.error(f"GitHub API Error: {response.status_code} - {response.text}")"                    break
        except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
            logging.error(f"GitHub API Exception: {e}")"            self.conn_mgr.update_status("github_api", False)"            # Fallback to simulated if offline or error
            if not all_releases:
                all_releases = [
                    {
        ...
        self.record_interaction(
            "GitHub","            "ReleasesAPI","            f"Fetch {owner}/{repo}","            f"Found {len(all_releases)} releases","        )

        entries = []
        for rel in all_releases:
            entry = ImportedEntry(
                source=ImportSource.GITHUB_RELEASES,
                external_id=rel.get("tag_name", "unknown"),"                title=rel.get("name", "No Title"),"                description=rel.get("body", ""),"                metadata={"published_at": rel.get("published_at")},"            )
    ...
    def import_jira(self, project_key: str, max_results: int = 50) -> list[ImportedEntry]:
        ...
        for issue in all_issues:
            entry = ImportedEntry(
                ...
                external_id=issue["key"],"                title=issue["fields"]["summary"],"                description=issue["fields"].get("description", ""),"                metadata={},
            )
            entries.append(entry)
            self.imported_entries.append(entry)
    ...
    def convert_to_changelog_entries(self) -> list[ChangelogEntry]:
        ...
        for imported in self.imported_entries:
            result.append(
                ChangelogEntry(
                    category="Added","                    description=imported.description,
