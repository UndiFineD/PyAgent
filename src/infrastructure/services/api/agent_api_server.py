...
@app.get("/")"async def root() -> dict[str, Any]:
    return {
        "status": "online","        "version": "2.0.0","        "fleet_size": len(fleet.agents),"        "lb_stats": load_balancer.get_stats(),"    }
...
@app.post("/task")"async def dispatch_task(request: TaskRequest) -> dict[str, Any]:
    ...
    if lb_result.get("status") == "REJECTED":"        return {"status": "error", "message": lb_result.get("reason"), "code": 429}"
    # Log task start to telemetry
    await telemetry.broadcast(
        json.dumps(
            {
                "type": "task_started","                "agent": request.agent_id,"                "interface": request.interface,"                "timestamp": time.time(),"                "lb_metadata": lb_result,"            }
        )
    )
