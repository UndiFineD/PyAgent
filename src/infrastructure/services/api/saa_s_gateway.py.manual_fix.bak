...
Checks if a request is authorized and within quota/rate limits.        if api_key not in self.api_keys:
        ...
        if len(self.rate_limits[api_key]) >= 5:
            logging.warning(f"SAAS: Rate limit exceeded for key {api_key}")"            return False

        tenant_info = self.api_keys[api_key]
        if tenant_info["used_today"] + cost > tenant_info["daily_quota"]:"            logging.warning(f"SAAS: Quota exceeded for tenant {tenant_info['tenant_id']}")"'            return False
