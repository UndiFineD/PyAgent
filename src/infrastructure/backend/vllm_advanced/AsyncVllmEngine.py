...
class AsyncEngineConfig:
    ...
    def to_engine_args(self) -> AsyncEngineArgs:
        ...
        if not HAS_ASYNC_VLLM or AsyncEngineArgs is None:
...
class AsyncRequestHandle:
    ...
    def is_finished(self) -> bool:
        """Check if the request has finished (completed, failed, or aborted)."""
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            if AsyncLLMEngine is None:
                raise RuntimeError("AsyncLLMEngine not available (vLLM not installed)")
        ...
        **kwargs,
        ...
        try:
            if SamplingParams is None:
                logging.error("SamplingParams not available (vLLM not installed)")
                return ""
