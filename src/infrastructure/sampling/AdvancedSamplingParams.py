...
class AdvancedSamplingParams(SamplingParams):
    ...
    def get_temperature(self, step: int) -> float:
        ...
        if step < self.temperature_warmup_steps:
            ...
            return self.temperature_decay_target + progress * (
                self.temperature - self.temperature_decay_target
            )
    ...
    def get_adaptive_top_k(self, entropy: float) -> int:
        ...
        k = int(
            self.min_adaptive_k
            + normalized * (self.max_adaptive_k - self.min_adaptive_k)
        )
        return max(self.min_adaptive_k, min(k, self.max_adaptive_k))

    def get_contextual_penalty(self, distance: int) -> float:
        """Get repetition penalty with distance decay."""
        if distance <= 0 or self.repetition_penalty == 1.0:
...
class TokenWhitelistProcessor:
    ...
    def apply_to_logits(
        self, logits: np.ndarray, vocab_size: Optional[int] = None
    ) -> np.ndarray:
...
class SamplingEngine:
    ...
    def sample(
        self, logits: np.ndarray, context_ids: Optional[List[int]] = None
    ) -> Tuple[int, float]:
        ...
        if (
            isinstance(self.params, AdvancedSamplingParams)
            and self.params.adaptive_top_k
        ):
            # Compute entropy
