            ...
            if num_to_evict > 0:
                ...
                for i, (block_hash, block) in enumerate(self.blocks.items()):
                    if block.ref_cnt == 0:
                        ...
                        priority = i + access_count * self.frequency_weight * len(
                            self.blocks
                        )
...
class PrefetchingLRUManager(LRUOffloadManager):
    ...
    def __init__(
        self, backend: Backend, enable_events: bool = False, prefetch_lookahead: int = 4
    ):
        super().__init__(backend, enable_events)
        ...
        with self._lock:
            for block_hash in block_hashes[: self.prefetch_lookahead]:
                if (
                    ...
                    and block_hash not in self._prefetch_in_progress
                ):
...
class LRUManagerFactory:
    ...
    @staticmethod
    def create_simple(
        num_blocks: int = 1000, block_size: int = 16
    ) -> LRUOffloadManager:
