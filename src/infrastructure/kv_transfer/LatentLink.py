...
class SynapticAdapter(nn.Module):
    ...
    def __init__(self, source_dim: int, target_dim: int):
        ...
        self.projector = nn.Sequential(
            nn.Linear(source_dim, target_dim), nn.LayerNorm(target_dim)
        )
...
class LatentLinkManager:
    ...
    def register_connection(
        self, source_id: str, target_id: str, source_dim: int, target_dim: int
    ):
        """Register a synaptic adapter between two agents."""
        key = (source_id, target_id)
        if key not in self.adapters:
            self.adapters[key] = SynapticAdapter(source_dim, target_dim)

    def transfer_latent(
        self, source_id: str, target_id: str, source_kv: torch.Tensor
    ) -> torch.Tensor:
        """transfer KV cache across agents via latent projection."""
        adapter = self.adapters.get((source_id, target_id))
        if adapter is None:
            raise ValueError(
                f"No LatentLink registered from {source_id} to {target_id}"
            )
...
class SynapticLink:
    ...
    def transmit(self, target_agent_id: str, context_kv: torch.Tensor):
        """Transmit context to another agent via the synaptic link."""
        return self.manager.transfer_latent(self.agent_id, target_agent_id, context_kv)
