...
class BlockTable:
    ...
    def __init__(self, config: BlockTableConfig):
        ...
        if config.kernel_block_size == config.block_size:
            ...
            if config.block_size % config.kernel_block_size != 0:
                raise ValueError(
                    f"kernel_block_size {config.kernel_block_size} must divide "
                    f"block_size {config.block_size} evenly"
                )
    ...
    def append_row(
        ...
        Args:
            ...
            num_tokens: Number of tokens (for slot calculation)
        """
        with self._lock:
    ...
    def compute_slot_mapping(
        ...
        if HAS_RUST:
            ...
            return rust_core.block_table_slot_mapping_rust(
    ...
    def allocate(
        ...
        with self._lock:
            ...
            for block_id in allocated:
                self._allocated[block_id] = BlockInfo(
                    block_id=block_id, ref_count=1, is_allocated=True
                )
...
class DistributedBlockTable:
    ...
    def allocate_blocks(
        self, row_idx: int, num_blocks: int, prefer_local: bool = True
    ) -> list[int]:
...
class BlockTableV2:
    ...
    def __init__(
        ...
        if use_sparse:
            self._impl = SparseBlockTable(config)
        else:
            self._impl = BlockTable(config)

        # Predictive allocator
        self.allocator = (
            PredictiveBlockAllocator(
                total_blocks=1000, block_size=config.block_size  # Default
            )
            if use_prediction
            else None
        )
