...
class ARCOffloadManager(OffloadingManager):
        ...
        with self._lock:
            for block_hash in reversed(list(block_hashes)):
                if block_hash in self.t1:
                    ...
                    delta = self.adaptation_speed * max(
                        1, len(self.b2) / max(1, len(self.b1))
                    )
                    ...
                    delta = self.adaptation_speed * max(
                        1, len(self.b1) / max(1, len(self.b2))
                    )
    ...
    def prepare_store(self, block_hashes: list[BlockHash]) -> PrepareStoreOutput | None:
        ...
        with self._lock:
            to_store = [
                h for h in block_hashes if h not in self.t1 and h not in self.t2
            ]
    ...
    def update_block_importance(
        self, block_hash: BlockHash, hidden_states: torch.Tensor
    ) -> None:
        ...
        if len(self._adaptation_history) >= 10:
            ...
            if recent_avg < overall_avg * 0.9:
                self.adaptation_speed = min(
                    self.adaptation_speed * 1.1, self.max_adaptation_speed
                )
            elif recent_avg > overall_avg * 1.1:
                self.adaptation_speed = max(
                    self.adaptation_speed * 0.9, self.min_adaptation_speed
                )
...
class AsyncARCManager:
    ...
    async def prepare_store_async(
        ...
        return await loop.run_in_executor(
            None, self.manager.prepare_store, block_hashes
        )
