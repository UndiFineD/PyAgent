...
class LogprobsAnalyzer:
    ...
    def rank_token_importance(
        ...
        lps = (
            ...
            else np.array([e.logprob for e in logprobs])
        )
        importance = -lps
    ...
    def compute_confidence(
        logprobs: Union[FlatLogprobs, List[LogprobEntry]], method: str = "mean"
    ) -> float:
        lps = (
            logprobs.logprobs
            if isinstance(logprobs, FlatLogprobs)
    ...
    def detect_anomalies(
        logprobs: Union[FlatLogprobs, List[LogprobEntry]], z_threshold: float = 2.5
    ) -> List[int]:
        lps = (
            logprobs.logprobs
            if isinstance(logprobs, FlatLogprobs)
    ...
    def compute_calibration(
        logprobs: Union[FlatLogprobs, List[LogprobEntry]], num_bins: int = 10
    ) -> Dict[str, Any]:
        probs = (
            np.exp(logprobs.logprobs)
            if isinstance(logprobs, FlatLogprobs)
            else np.array([e.probability for e in logprobs])
        )
        ...
        return {
            ...
            "bin_counts": np.bincount(indices, minlength=num_bins).tolist(),
