    ...
    def find_longest_prefix(
        ...
        with self._lock:
            ...
            while pos < len(tokens):
                ...
                matches = all(
                    child.prefix[i] == tokens[pos + i] for i in range(prefix_len)
                )
...
class PrefixCacheOptimizer:
    ...
    def _evict_batch(self) -> int:
        ...
        stale_entries = [
            (h, e)
            for h, e in self._warm_cache.items()
        ...
        # Evict from cold
        cold_entries = sorted(
            self._cold_cache.items(), key=lambda x: (x[1].hit_count, x[1].last_access)
        )
    ...
    def get_prewarm_candidates(self, limit: int = 10) -> list[tuple[int, ...]]:
        ...
        with self._lock:
            sorted_candidates = sorted(
                self._prewarm_candidates.items(), key=lambda x: x[1], reverse=True
            )[:limit]
