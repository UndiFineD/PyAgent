...
class BlockPool:
        ...
        with self._lock:
            for block_id in block_ids:
                ...
                if block.ref_count == 0:
                    ...
                    if (
                        block.block_hash
                        and block.block_hash in self._cached_block_hash_to_block
                    ):
    ...
    def _update_metrics(self) -> None:
        ...
        cached_count = sum(
            1 for b in self._blocks.values() if b.state == BlockState.CACHED
        )
        allocated_count = sum(
            1 for b in self._blocks.values() if b.state == BlockState.ALLOCATED
        )
