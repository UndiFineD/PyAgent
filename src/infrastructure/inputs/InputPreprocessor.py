...
SingletonPrompt = Union[
    TextPrompt, TokensPrompt, EmbedsPrompt, EncoderDecoderPrompt, ChatPrompt
]
...
class ProcessedInput:
    ...
    metadata: InputMetadata = field(
        default_factory=lambda: InputMetadata(
            prompt_type=PromptType.TEXT, estimated_tokens=0
        )
    )
...
class PromptValidator:
    ...
    def _validate_tokens(self, prompt: TokensPrompt) -> Tuple[bool, Optional[str]]:
        ...
        if len(prompt.prompt_token_ids) > self.max_length:
            return (
                False,
                f"Token prompt too long: {len(prompt.prompt_token_ids)} tokens",
            )
    ...
    def _validate_embeds(self, prompt: EmbedsPrompt) -> Tuple[bool, Optional[str]]:
        ...
        if prompt.prompt_embeds.shape[0] > self.max_length:
            return (
                False,
                f"Embedding sequence too long: {prompt.prompt_embeds.shape[0]}",
            )
        return True, None
    ...
    def _validate_encoder_decoder(
        self, prompt: EncoderDecoderPrompt
