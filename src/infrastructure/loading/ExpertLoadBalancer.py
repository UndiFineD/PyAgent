...
class DefaultEplbPolicy(AbstractEplbPolicy):
    ...
    def replicate_experts(
        ...
        # Initialize mappings
        phy_to_log = [
            [i if i < num_logical else -1 for i in range(num_physical)]
    ...
    def rebalance_experts(
        ...
        log_to_phy = [
            [[-1] * max_replicas for _ in range(num_logical)] for _ in range(num_layers)
        ]
        ...
        with self._lock:
            ...
            if not self.metrics.expert_load_pass:
                self.metrics.expert_load_pass = [
                    [0.0] * self.num_physical for _ in range(self.num_layers)
                ]
                ...
                for w in range(self.window_size):
                    if self.metrics.expert_load_window:
                        for e in range(self.num_physical):
                            layer_loads[e] += self.metrics.expert_load_window[w][layer][
                                e
                            ]
    ...
    def _should_rebalance(self) -> bool:
        ...
        for layer_loads in avg_load:
            if layer_loads:
                ...
                min_load = (
                    min(l for l in layer_loads if l > 0)
                    if any(l > 0 for l in layer_loads)
                    else 1.0
                )
