...
def atomic_writer(
    ...
    """
    writer = AtomicWriter(filepath, mode, encoding)
    try:
        f = writer.__enter__()
        yield f
    except Exception:
...
class WeightLoader(ABC):
    ...
    def get_weight_specs(self, file_paths: list[str]) -> list[WeightSpec]:
        """Get specifications for all weights without loading data."""
        pass
...
class SafetensorsLoader(WeightLoader):
    ...
    def iterate_weights(
        ...
        try:
            ...
            from safetensors import safe_open as safe_open_base
        except ImportError:
            raise ImportError("safetensors package required for SafetensorsLoader")
...
class MultiThreadWeightLoader(WeightLoader):
    ...
    def iterate_weights(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            futures = {
                executor.submit(self._load_file, f, device): f for f in file_paths
            }
