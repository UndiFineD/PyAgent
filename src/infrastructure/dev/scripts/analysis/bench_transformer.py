...
def run_performance_test(duration_secs=60):
    ...
    print(
        f"Config:   d_model={config.d_model}, d_ff={config.d_ff}, heads={config.n_heads}, layers={config.n_layers}"
    )
    print("-" * 50)
    ...
    print(
        f"Avg Latency:       {avg_latency:.2f}ms per batch ({tokens_per_call} tokens)"
    )
