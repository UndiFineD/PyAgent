...
class ImportRefactorer:
    ...
    def load_mappings(self, config_path: str):
        """Loads absolute and relative mappings from a JSON file."""
        ...
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
            self.absolute_mappings = data.get("absolute", {})
            self.relative_mappings = data.get("relative", {})

    def add_absolute_mapping(self, old_path: str, new_path: str):
        """Adds a single absolute import mapping (e.g., 'src.core.old' -> 'src.core.new')."""
        self.absolute_mappings[old_path.replace("\\", ".")] = new_path.replace("\\", ".")

    def add_relative_mapping(self, directory: str, old_ref: str, new_ref: str):
        """Adds a relative mapping scoped to a specific directory (e.g., '.enums' -> '.CoreEnums')."""
    ...
    def refactor_file(self, file_path: Path) -> bool:
        """Processes a single file to apply absolute and scoped relative mappings."""
    ...
    def run(self, search_paths: Optional[List[str]] = None):
        """Walks through search paths and refactors all Python files found."""
        if not search_paths:
                ...
                for file in files:
                    if file.endswith(".py"):
                        ...
                        if self.refactor_file(Path(root) / file):
                            files_updated += 1
                            action = "Updated" if not self.dry_run else "[DRY RUN] Would update"
                            print(f"{action}: {Path(root) / file}")
