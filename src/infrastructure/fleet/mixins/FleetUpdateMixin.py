...
class FleetUpdateMixin:
    ...
    def init_update_service(self, interval_seconds: int = 900):
        ...
        self._updater_thread = threading.Thread(
            target=self._update_loop, name="FleetAutoUpdater", daemon=True
        )
        self._updater_thread.start()
        logger.info(
            f"FleetUpdateMixin: Auto-update service started with {interval_seconds}s interval."
        )
    ...
    def _run_git_pull(self):
        ...
        if not git_dir.exists():
            logger.warning(
                f"FleetUpdateMixin: {workspace_path} is not a git repository. Skipping update."
            )
            return

        logger.info(
            "FleetUpdateMixin: Checking for updates from https://github.com/UndiFineD/PyAgent..."
        )
        ...
        try:
            ...
            if "Already up to date" in result.stdout:
                ...
                logger.info(
                    f"FleetUpdateMixin: Successfully updated PyAgent.\n{result.stdout}"
                )
                ...
                logger.error(
                    f"FleetUpdateMixin: Git pull failed (exit {result.returncode}): {result.stderr}"
                )

        except FileNotFoundError:
            logger.error(
                "FleetUpdateMixin: 'git' command not found. Cannot perform auto-update."
            )
        except Exception as e:
