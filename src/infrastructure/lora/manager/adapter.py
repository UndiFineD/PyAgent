...
class LoRAAdapter:
    ...
    def _load_from_dir(self, path: Path) -> LoRAWeights:
        for f in ["adapter_model.safetensors", "adapter_model.bin"]:
            if (path / f).exists():
                return (
                    self._load_st(path / f)
                    if f.endswith("safetensors")
                    else self._load_torch(path / f)
                )
    ...
    def apply_to_linear(
        self, module_name: str, hidden_states: np.ndarray
    ) -> np.ndarray:
        ...
        for name, weight in merged.items():
            for t in self.config.target_modules:
                if t in name and t in self.weights.lora_a:
                    delta = self.weights.scales.get(t, self.config.computed_scaling) * (
                        self.weights.lora_b[t] @ self.weights.lora_a[t]
                    )
...
def load_lora_adapter(
    path: str, name: Optional[str] = None, rank: int = 8, scale: float = 16.0, **kwargs
) -> LoRAAdapter:
