...
class CausalConv1d:
    ...
    def __init__(
        ...
        self.weight = np.random.randn(in_channels, kernel_size).astype(np.float32) * (
            1.0 / math.sqrt(kernel_size)
        )
    ...
    def forward(
        ...
        new_state = (
            x_padded[:, :, -(self.kernel_size - 1) :] if seq_len >= 1 else conv_state
        )
