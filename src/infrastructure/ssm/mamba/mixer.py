...
class MambaMixer:
    ...
    def __init__(self, config: MambaConfig) -> None:
        ...
        self.in_proj_bias = (
            np.zeros(2 * config.d_inner, dtype=np.float32) if config.use_bias else None
        )
        ...
        # dt projection
        self.dt_proj_weight = np.random.randn(config.d_inner, config.dt_rank).astype(
            np.float32
        ...
        self.out_proj_bias = (
            np.zeros(config.hidden_size, dtype=np.float32) if config.use_bias else None
        )
    ...
    def forward(
        ...
        dt = (
            F.softplus(torch.from_numpy(dt)).numpy()
            if HAS_TORCH
            else np.log1p(np.exp(dt))
        )
...
class Mamba2Mixer(MambaMixer):
    ...
    def __init__(
        ...
        # Multi-head A matrix
        self.ssm.A = -np.exp(
            np.random.randn(num_heads, self.head_dim, config.ssm_state_size).astype(
                np.float32
            )
            * 0.5
        )
