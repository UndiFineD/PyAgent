"""
Paged Attention Engine - Block-based KV cache attention system.

Inspired by vLLM's paged_attention_v1/v2 kernels for efficient memory management
in LLM inference. Implements block-based KV cache with slot mapping for
...
class AttentionConfig:
    ...
    def is_gqa(self) -> bool:
        """Whether using Grouped Query Attention."""
        return self.num_heads != self.num_kv_heads
    
    @property
    def is_mqa(self) -> bool:
        """Whether using Multi-Query Attention."""
...
class BlockTable:
    ...
    Maps logical blocks to physical blocks in the KV cache pool.
    
    Attributes:
        num_blocks: Total number of physical blocks
        block_size: Tokens per block
        block_tables: Mapping from sequence to list of physical block indices
        free_blocks: Set of available physical block indices
    """
    num_blocks: int
    block_size: int
    ...
    def copy_blocks(self, src_seq_id: int, dst_seq_id: int) -> int:
        """Copy block table from source to destination sequence.
        
        Note: This creates a copy-on-write reference, not a deep copy.
        ...
        if src_seq_id not in self.block_tables:
            return 0
        self.block_tables[dst_seq_id] = list(self.block_tables[src_seq_id])
        return len(self.block_tables[dst_seq_id])
...
class SlotMapping:
    ...
    Each token in a sequence is assigned a unique slot in the KV cache.
    
    Attributes:
        block_size: Tokens per block
        slots: Flat array of slot indices [seq_start, seq_start+seq_len)
    """
    block_size: int
    slots: NDArray[np.int64] = field(default_factory=lambda: np.array([], dtype=np.int64))
    
    def compute_slot(self, block_idx: int, offset: int) -> int:
        """Compute flat slot index from block and offset.
    ...
    def decode_slot(self, slot: int) -> tuple[int, int]:
        """Decode flat slot into block index and offset.
        
        Args:
            slot: Flat slot index
            
        Returns:
            Tuple of (block_idx, block_offset)
        """
        block_idx = slot // self.block_size
        offset = slot % self.block_size
        return block_idx, offset
    
    def map_sequence_slots(
        self,
        block_table: list[int],
        seq_len: int,
    ) -> NDArray[np.int64]:
        """Generate slot mapping for a sequence.
        ...
        for i in range(seq_len):
            ...
            if block_idx < len(block_table):
                ...
                slots[i] = -1  # Invalid slot
        return slots


@dataclass
class PagedKVCache:
    """Block-organized key/value cache for paged attention.
    
    Stores KV cache in blocks for memory-efficient attention.
    ...
    key_cache: NDArray | None = field(default=None, repr=False)
    value_cache: NDArray | None = field(default=None, repr=False)
    ...
    def write(
        ...
        """Write key/value tensors to cache using slot mapping.
        
        Args:
            key: Keys [num_tokens, num_kv_heads, head_size]
            value: Values [num_tokens, num_kv_heads, head_size]
