    ...
    def _paged_attention_kernel(
        ...
        for block_idx in range(num_blocks):
            ...
            phys_block = tl.load(
                block_tables_ptr
                + block_table_offset
                + block_idx * stride_block_tables_block
            )

            # Load key block
            k_offset = (
                phys_block * stride_k_cache_block + kv_head_idx * stride_k_cache_head
            )
            ...
            for seq_pos in range(block_size):
                ...
                if abs_pos < context_len:
                    ...
                    v = tl.load(
                        v_cache_ptr + v_offset + dim_offsets * stride_v_cache_dim
                    )
...
class SlidingWindowAttention(AttentionKernel):
    ...
    def forward(
        ...
        causal_mask = torch.ones(
            seq_len, seq_len, device=query.device, dtype=torch.bool
        )
...
class TritonAttentionOps:
    ...
    def forward(
        ...
        if (
            self._sliding_kernel
            and metadata.max_decode_seq_len > self.config.sliding_window_size
        ):
    ...
    def paged_attention_decode(
        ...
        if (
            max_context_len
            > self._kv_split.max_context_per_split * self._kv_split.num_splits
        ):
        ...
        for i in range(num_splits):
            ...
            split_metadata = AttentionMetadata(
                ...
                context_lens=torch.clamp(
                    metadata.context_lens - start_pos, min=0, max=end_pos - start_pos
                ),
