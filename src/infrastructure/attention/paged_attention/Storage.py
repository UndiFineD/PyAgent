...
class PagedKVCache:
    ...
    def write(
        self, key: np.ndarray, value: np.ndarray, slot_mapping: np.ndarray
    ) -> None:
    ...
    def read_blocks(
        ...
        values = np.zeros(
            (seq_len, self.num_kv_heads, self.head_size), dtype=self.dtype
        )
        for i in range(seq_len):
    ...
    def get_memory_usage(self) -> int:
        return (
            (self.key_cache.nbytes + self.value_cache.nbytes)
...
class AttentionMetadata:
    ...
    def from_seq_lens(
        ...
        block_tables_arr = np.full(
            (len(seq_lens), max_blocks_per_seq), -1, dtype=np.int32
        )
