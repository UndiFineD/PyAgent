...
class PagedAttentionOps:
    ...
    def paged_attention_v2(
        ...
        for seq_idx in range(ns):
            ...
            if sl == 0:
                continue
            keys, values = key_cache.read_blocks(
                [b for b in block_tables[seq_idx] if b >= 0], sl
            )
            if config.is_gqa:
                keys = PagedAttentionOps.expand_kv_for_gqa(
                    keys, config.num_queries_per_kv
    ...
    def expand_kv_for_gqa(kv: np.ndarray, num_queries_per_kv: int) -> np.ndarray:
        return (
            kv if num_queries_per_kv == 1 else np.repeat(kv, num_queries_per_kv, axis=1)
        )
