...
class PagedAttentionEngine:
    def __init__(self, config: AttentionConfig, num_blocks: int = 1024):
        ...
        self.kv_cache = PagedKVCache(
            num_blocks, config.block_size, config.num_kv_heads, config.head_size, dtype
        )
    ...
    def forward(
        self, query: np.ndarray, seq_ids: List[int], use_v2: bool = True
    ) -> np.ndarray:
        sl = np.array(
            [self._seq_positions.get(sid, 0) for sid in seq_ids], dtype=np.int32
        )
        mb = max(
            [self.block_table.num_allocated_blocks(sid) for sid in seq_ids], default=1
        )
        ...
        for i, sid in enumerate(seq_ids):
            ...
            for j, b in enumerate(tbl[:mb]):
                bt[i, j] = b
        return (
            PagedAttentionOps.paged_attention_v2(
                query, self.kv_cache, bt, sl, self.config
            )
            if use_v2
            else PagedAttentionOps.paged_attention_v1(
                query, self.kv_cache, bt, sl, self.config
            )
        )
    ...
    def get_stats(self) -> dict:
        return {
            ...
            "num_allocated_blocks": self.block_table.num_blocks
            - self.block_table.num_free_blocks,
