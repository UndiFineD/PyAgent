...
class MemoryPlacement(Enum):
    ...
    UVA = "uva"  # Unified Virtual Addressing
...
class CpuGpuBuffer:
    ...
    def cpu_to_gpu(self) -> None:
        ...
        np.copyto(self.gpu, self.cpu)
        self._cpu_dirty = False
        self._gpu_dirty = False
    
    def gpu_to_cpu(self) -> None:
        """Copy GPU buffer to CPU."""
        np.copyto(self.cpu, self.gpu)
        self._cpu_dirty = False
        self._gpu_dirty = False
    
    def sync(self) -> None:
        """Synchronize buffers based on dirty flags."""
        if self._cpu_dirty:
            self.cpu_to_gpu()
    ...
    def mark_cpu_dirty(self) -> None:
        """Mark CPU buffer as modified."""
        self._cpu_dirty = True
        self._gpu_dirty = False
    
    def mark_gpu_dirty(self) -> None:
        """Mark GPU buffer as modified."""
        self._gpu_dirty = True
        self._cpu_dirty = False
    
    def fill(self, value: Any) -> None:
        """Fill both buffers with a value."""
        self.cpu.fill(value)
        self.gpu.fill(value)
        self._cpu_dirty = False
        self._gpu_dirty = False
    ...
    def slice(self, *slices) -> Tuple[np.ndarray, np.ndarray]:
        """Get sliced views of both buffers."""
        return self.cpu[slices], self.gpu[slices]
    
    @property
    def nbytes(self) -> int:
        """Total bytes across both buffers."""
        return self.cpu.nbytes + self.gpu.nbytes
    
    def resize(self, new_shape: Tuple[int, ...]) -> None:
        """Resize buffers (reallocates memory)."""
...
class UvaBufferPool:
    ...
    def get(self, name: str) -> Optional[CpuGpuBuffer]:
        """Get a buffer by name (searches by prefix)."""
        with self._lock:
    ...
    def release(self, name: str) -> bool:
        """Release a buffer back to the pool (reset it)."""
        buf = self.get(name)
        if buf:
            buf.reset()
            return True
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "total_bytes": self._total_allocated,
                "total_mb": self._total_allocated / (1024 * 1024),
                "buffers": {
                    name: {
                        "shape": buf.shape,
                        "dtype": str(buf.dtype),
...
class PinnedMemoryManager:
    ...
    def available_bytes(self) -> int:
        """Available bytes."""
        return self.max_bytes - self._allocated
