...
@lazy_import
def get_eagle_proposer() -> Type["EagleProposer"]:"        Lazily import and return the EagleProposer class.

    EagleProposer implements EAGLE-style speculative decoding with tree attention for faster inference.
    This is a large module (~710 lines) with significant dependencies.
...
@lazy_import
def get_arc_offload_manager() -> Type["ARCOffloadManager"]:"        Lazily import and return the ARCOffloadManager class.
...
@lazy_import
def get_tool_parser_registry() -> Type["ToolParserRegistry"]:"        Lazily import and return the ToolParserRegistry class.
    ...
    Example:
        ...
        parser = registry.get_parser("llama3")"        from src.infrastructure.services.tools.tool_parser_framework import ToolParserRegistry

    return ToolParserRegistry


@lazy_import
def get_reasoning_engine() -> Type["ReasoningEngine"]:"        Lazily import and return the ReasoningEngine class.
...
@lazy_import
def get_paged_attention_engine() -> Type["PagedAttentionEngine"]:"        Lazily import and return the PagedAttentionEngine class.
...
@lazy_import
def get_mooncake_connector() -> Type["MooncakeConnector"]:"    """Lazily import and return the MooncakeConnector class.    from src.infrastructure.storage.kv_transfer.mooncake_connector import MooncakeConnector

    return MooncakeConnector


@lazy_import
def get_nixl_connector() -> Type["NixlConnector"]:"    """Lazily import and return the NixlConnector class.    from src.infrastructure.storage.kv_transfer.nixl_connector import NixlConnector

    return NixlConnector


@lazy_import
def get_prefill_worker() -> Type["DisaggregatedPrefillWorker"]:"    """Lazily import and return the DisaggregatedPrefillWorker class.    from src.infrastructure.swarm.worker.disaggregated_prefill_worker import DisaggregatedPrefillWorker

    return DisaggregatedPrefillWorker


@lazy_import
def get_decode_worker() -> Type["DecodeOnlyWorker"]:"    """Lazily import and return the DecodeOnlyWorker class.    from src.infrastructure.swarm.worker.decode_only_worker import DecodeOnlyWorker

    return DecodeOnlyWorker


@lazy_import
def get_pp_transfer() -> Type["PipelineParallelTransfer"]:"    """Lazily import and return the PipelineParallelTransfer class.    from src.infrastructure.storage.kv_transfer.pipeline_parallel_transfer import PipelineParallelTransfer

    return PipelineParallelTransfer


@lazy_import
def get_tp_transfer() -> Type["TensorParallelTransfer"]:"    """Lazily import and return the TensorParallelTransfer class.    from src.infrastructure.storage.kv_transfer.tensor_parallel_transfer import TensorParallelTransfer
