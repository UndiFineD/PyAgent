...
from .models import Response, ResponseOutput, TextContent, ResponseUsage
...
class SSEStream:
    ...
    async def send(self, event: str, data: Any) -> None:
        if not self._closed: await self._queue.put(SSEEvent(event=event, data=data))
...
class StreamingHandler:
    ...
    async def add_content_delta(self, text: str) -> None:
        if self._current_output is None:
            ...
            self._current_output = ResponseOutput(id=output_id, type=ResponseType.MESSAGE, content=[TextContent(text="")], status=ResponseStatus.IN_PROGRESS)
            self.response.output.append(self._current_output)
            await self.stream.send("response.output_item.added", {"output_index": len(self.response.output) - 1, "item": self._current_output.to_dict()})
        self._text_buffer.append(text)
        await self.stream.send("response.output_item.content_part.delta", {"output_index": len(self.response.output) - 1, "content_index": 0, "delta": {"type": "text", "text": text}})
    ...
    async def finish_output(self) -> None:
        if self._current_output:
            ...
            await self.stream.send("response.output_item.done", {"output_index": len(self.response.output) - 1, "item": self._current_output.to_dict()})
