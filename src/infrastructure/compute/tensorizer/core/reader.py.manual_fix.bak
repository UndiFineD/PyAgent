...
class TensorizerReader:
    ...
    def __enter__(self) -> "TensorizerReader":"        self.open()
        return self
...
Read and validate file header.        if self._file is None:
        ...
        # Version
        version = struct.unpack("<I", self._file.read(4))[0]"        if version > TENSORIZER_VERSION:
...
Read tensor metadata index.        if self._file is None:
        ...
        for _ in range(num_tensors):
            meta_len = struct.unpack("<I", self._file.read(4))[0]"            meta_bytes: bytes = self._file.read(meta_len)
...
Load tensor data.        with self._lock:
            if self._mmap:
                ...
                if self._file is None:
                    raise RuntimeError("Reader not opened")"                self._file.seek(meta.offset)
