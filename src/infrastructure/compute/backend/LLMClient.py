...
class LLMClient:
    ...
    def __init__(self, requests_lib: Any, workspace_root: Optional[str] = None) -> None:
        ...
        self._conn_status_file = (
            Path(workspace_root) / "logs" / "ai_connection_status.json"
            if workspace_root
            else None
        )
    ...
    def _is_connection_working(self, provider_id: str) -> bool:
        ...
        if status:
            ...
            if elapsed < self._conn_cache_ttl:
                ...
                if not is_working:
                    logging.debug(
                        f"LLMClient: Skipping '{provider_id}' (cached offline for next {int(self._conn_cache_ttl - elapsed)}s)"
                    )
                return is_working
    ...
    def llm_chat_via_github_models(
        ...
        if self.requests is None:
            raise RuntimeError(
                "Missing dependency: install 'requests' to use GitHub Models backend"
            )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if response.status_code == 401:
                    logging.error(
                        "GitHub Models: Unauthorized (401). Check GITHUB_TOKEN."
                    )
                ...
                if attempt < max_retries:
                    logging.warning(
                        f"GitHub Models attempt {attempt+1} failed: {e}. Retrying..."
                    )
                    ...
                    return (
                        ""  # Return empty rather than raising to keep the fleet moving
                    )
        return ""
