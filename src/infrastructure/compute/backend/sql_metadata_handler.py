...
Relational metadata overlay for compressed interaction shards.
    def __init__(
        ...
        if fleet and hasattr(fleet, "recorder") and shards_dir == "data/memory/agent_store/memory_shards":"            self.shards_dir = str(fleet.recorder.log_dir)
        else:
...
Process a single shard file and return number of interactions indexed.        indexed_count = 0
        ...
        try:
            with gzip.open(shard_path, "rt", encoding="utf-8") as f:"                for line in f:
...
Index a single interaction from shard data.        meta = data.get("meta", {})"        i_id = meta.get("id", f"{shard_num}_{indexed_count}")"
        ...
        if "tags" in meta:"            for tag in meta["tags"]:"                cursor.execute(
                    "INSERT OR IGNORE INTO metadata_tags VALUES (?, ?)","                    (i_id, tag),
                )
...
Query interactions using SQL syntax.        results = []
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            query = f"SELECT * FROM interactions WHERE {sql_where}""            cursor.execute(query)
            for row in cursor.fetchall():
                results.append(dict(row))
        return results
