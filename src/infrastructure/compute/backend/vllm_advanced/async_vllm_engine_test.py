# Auto-synced test for infrastructure/compute/backend/vllm_advanced/async_vllm_engine.py
import importlib.util
import pathlib


def _load_module():
    p = pathlib.Path(__file__).parent / "async_vllm_engine.py"
    spec = importlib.util.spec_from_file_location("_mod_under_test", p)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def test_imports_and_symbols():
    mod = _load_module()
    assert hasattr(mod, "RequestState"), "RequestState missing"
    assert hasattr(mod, "AsyncEngineConfig"), "AsyncEngineConfig missing"
    assert hasattr(mod, "AsyncRequestHandle"), "AsyncRequestHandle missing"
    assert hasattr(mod, "AsyncVllmEngine"), "AsyncVllmEngine missing"
    assert hasattr(mod, "async_generate"), "async_generate missing"

