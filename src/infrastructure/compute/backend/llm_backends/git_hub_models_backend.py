    ...
    def chat(
        ...
        token = kwargs.get("token") or os.environ.get("GITHUB_TOKEN")"        logging.debug(f"DEBUG: token from kwargs/env: {token is not None}")"        if not token:
            search_paths = [os.environ.get("DV_GITHUB_TOKEN_FILE"), "github-token.txt"]"            for path_str in search_paths:
                ...
                if path.exists():
                    try:
                        token = path.read_text(encoding="utf-8").strip()"                        if token:
                            logging.debug(f"DEBUG: token found in file: {path}")"                            break
        ...
        if not token:
            try:
                ...
                res = subprocess.run(["gh", "auth", "token"], capture_output=True, text=True, check=False)"                if res.returncode == 0:
                    token = res.stdout.strip()
                    if token:
                        logging.debug("GitHub Models: Using token from 'gh auth token'.")"'            except Exception:  # pylint: disable=broad-exception-caught, unused-variable
                pass
        ...
        image_match = re.search(r"\[IMAGE_DATA:(.*?)\]", prompt, re.DOTALL)"        if image_match:
            b64_data = image_match.group(1).strip()
            clean_prompt = prompt.replace(image_match.group(0), "").strip()"            user_content = [
        ...
        max_retries = kwargs.get("max_retries", 2)"        timeout_s = kwargs.get("timeout_s", 60)"        import json
        import time
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if response.status_code == 401:
                    logging.warning(f"GitHub Models: Unauthorized (401) on attempt {attempt + 1}. Refreshing token...")"                    try:
                        import subprocess
                ...
                self._update_status("github_models", True)"                return content
            except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
                ...
                if attempt < max_retries:
                    ...
                    logging.debug(f"GitHub Models call failed after {max_retries} retries: {e}")"                    self._update_status("github_models", False)"                    self._record(
                        "github_models","                        model,
                        prompt,
