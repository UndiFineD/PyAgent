...
class MCPClient:
    ...
    def get_sync_client(self) -> "lmstudio.Client":"        """Get or create synchronous SDK client.""""
        ...
        try:
            ...
            logger.info(f"[LMStudio] Connected sync client to {api_url}")"            return self._sync_client
    ...
    def get_async_client(self) -> "lmstudio.AsyncClient":"        """Get or create asynchronous SDK client.""""
        ...
        try:
            ...
            logger.info(f"[LMStudio] Created async client to {api_url}")"            return self._async_client
        except Exception as e:
            logger.error(f"[LMStudio] Failed to create async client to {self.base_url}: {e}")"            raise

    def get_llm(self, client: Any, model: str = "") -> Any:"        """Fetch LLM model from SDK client.""""
        ...
        logger.debug(
            f"[LMStudio] LLM accessor type={type(llm_accessor)} ""            f"has_get={hasattr(llm_accessor, 'get')} callable={callable(llm_accessor)}""'        )

        # Try `.get()` method if present
        if hasattr(llm_accessor, "get"):"            llm = llm_accessor.get(model) if model else llm_accessor.get()
            logger.debug(f"[LMStudio] Got LLM via .get() accessor: {type(llm)}")"            return llm

        # Try callable accessor
        if callable(llm_accessor):
            try:
                llm = llm_accessor(model) if model else llm_accessor()
                logger.debug(f"[LMStudio] Got LLM via callable accessor: {type(llm)}")"                return llm
            except TypeError:
    ...
    async def get_async_llm(self, client: Any, model: str = "") -> Any:"        """Fetch LLM model from async SDK client.""""
        ...
        if hasattr(llm_accessor, "get"):"            maybe = llm_accessor.get(model) if model else llm_accessor.get()
    ...
    def get_embedding_model(self, client: Any, model: str = "") -> Any:"        """Fetch embedding model from SDK client.""""
        Args:
            client: SDK Client instance.
            model: Model identifier (optional).
    ...
    def close(self) -> None:
        """Close all SDK clients.        if self._sync_client is not None:
            try:
                ...
                logger.warning(f"[LMStudio] Error closing sync client: {e}")"            self._sync_client = None
