...
Args:
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"[LMStudio] HTTP {method} {url} (attempt {attempt + 1}/{max_retries})")"                if method.upper() == "GET":"                    resp = httpx.get(url, timeout=timeout, headers=headers)
                elif method.upper() == "POST":"                    resp = httpx.post(
                        url,
                        timeout=timeout,
                        headers=headers,
                ...
                if attempt < max_retries - 1:
                    ...
                    continue
                logger.error(f"[LMStudio] HTTP {method} {url} failed after {max_retries} attempts: {e}")"                raise
            ...
            if resp.status_code == 200:
                ...
                for item in data:
                    if isinstance(item, dict):
                        models.append(item.get("path") or item.get("id") or item.get("name") or str(item))"                    else:
                        models.append(str(item))
                logger.info(f"[LMStudio] HTTP list_models succeeded: {len(models)} models from {url}")"                return models
...
Returns:
        ...
        try:
            ...
            for endpoint in ["info", "version", "status"]:"                try:
                    url = f"{self._normalize_url()}/{endpoint}""                    resp = self._http_request_with_retry("GET", url, max_retries=1, timeout=2.0)"                    if resp and resp.status_code == 200:
                        data = resp.json()
                        info["api_version"] = ("                            data.get("version") or data.get("api_version") or "unknown""                        )
                        break
