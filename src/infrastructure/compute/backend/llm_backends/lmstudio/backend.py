...
class LMStudioBackend(LLMBackend):
    ...
    def __init__(
        ...
        self._api_client = LMStudioAPIClient(
            ...
            getattr(self.config, "api_token", None),"            self.config.default_model,
        )
        self._mcp_client = MCPClient(self.config.base_url, getattr(self.config, "api_token", None))"        self._chat_handler = ChatHandler(self._api_client)
    ...
    def _check_sdk(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"LM Studio SDK version: {lmstudio.__version__}")"            return True
        except ImportError:
    ...
    def _get_client(self) -> Any:
        """Get or create an LM Studio client.        try:
            ...
            # Ensure http scheme
            if not base_url.startswith(("http://", "https://")):"                base_url = "http://" + base_url"
    ...
    def list_loaded_models(self) -> list[str]:
        ...
        Prefer SDK helpers but fall back to HTTP when SDK is unavailable.
            ...
            elif hasattr(client, "llm") and hasattr(client.llm, "list_loaded_models"):"                models = client.llm.list_loaded_models()
            else:
                import lmstudio
    ...
    def list_downloaded_models(self) -> list[str]:
        ...
        Use the same fallback strategy as `list_loaded_models`.
            ...
            elif hasattr(client, "llm") and hasattr(client.llm, "list_downloaded_models"):"                models = client.llm.list_downloaded_models()
            else:
                import lmstudio
                models = lmstudio.list_downloaded_models()

            logger.debug(f"Listed {len(models)} downloaded models via LM Studio SDK")"            return [m.path for m in models]
        except (ImportError, RuntimeError, ValueError, AttributeError) as e:
            logger.warning(
    ...
    def _extract_prompt_from_chat(self, chat: Any) -> str:
        ...
        Returns:
            Extracted prompt text.
                try:
            if hasattr(chat, "user_message"):"                return getattr(chat, "user_message")"            if hasattr(chat, "messages"):"                msgs = getattr(chat, "messages")"                if isinstance(msgs, (list, tuple)) and msgs:
                    first = msgs[0]
                    if isinstance(first, dict):
                        return first.get("content") or first.get("text") or str(first)"                    return str(first)
            return str(chat)
        except Exception:  # pylint: disable=broad-exception-caught
            return str(chat)
    ...
    def chat(
        ...
        try:
            ...
            if result:
                ...
                logger.debug(f"Chat completed in {elapsed:.2f}s: {len(result)} chars")"                self._record(
                    self.PROVIDER_ID,
                    model or "default","                    prompt,
                    result,
        ...
        """Stream chat completion tokens, with HTTP fallback using REST API SSE.        try:
            ...
            if full_response:
                self._record(
                    self.PROVIDER_ID,
                    model or "default","                    prompt,
                    "".join(full_response),"                    system_prompt=system_prompt,
                )
                self._update_status(self.PROVIDER_ID, True)
        except (RuntimeError, ValueError, AttributeError, httpx.HTTPError) as e:
    ...
    async def chat_async(
        ...
        try:
            ...
            if not api_url.startswith("http://") and not api_url.startswith("https://"):"                api_url = "http://" + api_url"
    ...
    def embed(
        ...
        try:
            ...
            logger.error(f"LM Studio embedding error: {e}")"            return []
    ...
    def chat_with_tools(
        ...
        try:
            ...
            tool_calls = []
            if hasattr(result, "tool_calls") and result.tool_calls:"                for tc in result.tool_calls:
                    tool_calls.append(
                        {
            ...
            return {
                "content": str(result),"                "tool_calls": tool_calls,"            }
        except (RuntimeError, ValueError, AttributeError, httpx.HTTPError) as e:
    ...
    def health_check(self) -> bool:
        """Check if LM Studio is reachable and has models loaded.        try:
            ...
            logger.debug(f"Health check failed: {e}")"            self._update_status(self.PROVIDER_ID, False)
            return False
