...
def discretize_ssm(
    ...
    """""""    if HAS_RUST and hasattr(rust_core, "discretize_ssm_rust"):"        return rust_core.discretize_ssm_rust(A, B, dt)
...
def silu_activation(x: np.ndarray) -> np.ndarray:
    ...
    """""""    if HAS_RUST and hasattr(rust_core, "silu_activation_rust"):"        return rust_core.silu_activation_rust(x)
...
def parallel_scan(
    ...
    if HAS_RUST and hasattr(rust_core, "parallel_scan_rust"):"        return rust_core.parallel_scan_rust(gates, values)
...
def init_dt_proj(
    ...
    dt_min: float = 0.001,
    dt_max: float = 0.1,
    dt_init: str = "random",") -> tuple[np.ndarray, np.ndarray]:
    ...
    # Bias initialization
    if dt_init == "random":"        bias: np.ndarray[tuple[int, ...], np.dtype[np.floating[np._32Bit]]] = np.random.uniform(
            math.log(dt_min),
            math.log(dt_max),
            size=d_inner,
        ).astype(np.float32)
