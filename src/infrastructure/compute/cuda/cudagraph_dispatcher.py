...
class DispatchStats:
    ...
    def graph_ratio(self) -> float:
        """Ratio of graph executions."""
        total = self.eager_count + self.graph_count + self.piecewise_count
        if total == 0:
            return 0.0
...
class AdaptiveDispatchPolicy(DispatchPolicy):
    ...
    def record(self, key: DispatchKey, mode: DispatchMode, latency: float) -> None:
        """Record dispatch decision and result."""
        with self._lock:
...
class CudagraphDispatcher:
    ...
    def enable_mode(self, mode: DispatchMode) -> None:
        """Enable a dispatch mode."""
        self._available_modes.add(mode)

    def disable_mode(self, mode: DispatchMode) -> None:
        """Disable a dispatch mode."""
        self._available_modes.discard(mode)
...
class CompositeDispatcher:
    ...
    def dispatch(self, key: DispatchKey, *args: Any, **kwargs: Any) -> Any:
        ...
        # Fallback to first dispatcher's eager mode
        if dispatchers:
...
def get_padded_key(key: DispatchKey, pad_to: int = 8) -> DispatchKey:
    ...
    padded_tokens = ((key.num_tokens + pad_to - 1) // pad_to) * pad_to
