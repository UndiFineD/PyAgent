...
class LoRASlotManager:
    ...
    def _fill_slot(self, slot: AdapterSlot, name: str, mem: int) -> None:
        """Fill a slot with the given adapter name and memory requirement.
        """
    ...
    def release(self, name: str) -> None:
        """Release a slot by adapter name."""
        with self._lock:
            if name in self._adapter_to_slot:
                self._slots[self._adapter_to_slot[name]].is_active = False

    def evict(self, name: str) -> bool:
        """Evict an adapter from its slot, freeing it for new allocations. Returns True if evicted.
        """
        with self._lock:
            if name in self._adapter_to_slot:
    ...
    def get_slot(self, name: str) -> Optional[AdapterSlot]:
        """Get the slot assigned to the given adapter name, or None if not found.
        """
        with self._lock:
            if name in self._adapter_to_slot:
