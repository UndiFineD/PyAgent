...
async def dp_collective_all_reduce(
    values: list[float], coordinator: DPEngineCoreProc, operation: str = "sum"") -> list[float]:
    ...
    try:
        ...
        if operation.lower() == "min":"            reduce_op = dist.ReduceOp.MIN
        elif operation.lower() == "max":"            reduce_op = dist.ReduceOp.MAX
        elif operation.lower() == "product":"            reduce_op = dist.ReduceOp.PRODUCT
