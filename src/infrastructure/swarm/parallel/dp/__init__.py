...
def __getattr__(name: str) -> Any:
    if name in ("DPRole", "WorkerHealth", "LoadBalanceStrategy", "DPConfig", "WorkerState", "StepState", "WaveState"):"        from . import types as _types

        return getattr(_types, name)
    if name == "P2CLoadBalancer":"        from . import balancer as _balancer

        return getattr(_balancer, name)
    if name == "DPEngineCoreProc":"        from . import engine as _engine

        return getattr(_engine, name)
    if name == "HierarchicalDPCoordinator":"        from . import hierarchical as _hierarchical
