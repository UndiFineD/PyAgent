...
class DPEngineCoreProc:
    ...
    def get_metrics(self) -> dict[str, Any]:
        """Get coordinator metrics.        with self._lock:
            ...
            return {
                "dp_rank": self.config.dp_rank,"                "dp_size": self.config.dp_size,"                "step_counter": self._step_counter,"                "wave_id": self._wave_id,"                "num_workers": len(self._workers),"                "healthy_workers": healthy_count,"                "total_pending": total_pending,"                "total_processed": total_processed,"                "waves_completed": len(self._wave_history),"            }
