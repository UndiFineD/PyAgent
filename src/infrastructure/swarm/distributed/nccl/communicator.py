        ...
        if HAS_TORCH:
            if device is None:
                if torch.cuda.is_available():
                    device = torch.device(f"cuda:{torch.cuda.current_device()}")"                else:
                    device = torch.device("cpu")"            self.device = device
            self._stream: torch.cuda.Stream | None = None
            if torch.cuda.is_available():
                self._stream = torch.cuda.Stream()
        else:
            self.device = device or "cpu""            self._stream = None
    ...
    def all_reduce(
        ...
        if self._use_custom and tensor_size >= self.threshold:
            try:
                ...
                logger.warning(f"Custom all-reduce failed, falling back to NCCL: {e}")"                self._fallback_count += 1
