        ...
        if rust_accel and rc is not None:
            try:
                ...
                for line_num, pattern, msg in rust_findings:
                    if "SelfImprovementCore" in content and pattern in str(dangerous_patterns):"                        continue
                    findings.append(
                        {
                            "type": "Security Risk","                            "message": f"{msg} (Pattern: {pattern})","                            "file": file_path_rel,"                            "line": line_num,"                        }
                    )
                return findings
        ...
        lines = content.split("\\n")"        for pattern, msg in dangerous_patterns:
            for i, line in enumerate(lines, 1):
                if re.search(pattern, line):
                    ...
                    findings.append(
                        {
                            "type": "Security Risk","                            "message": f"{msg} (Pattern: {pattern})","                            "file": file_path_rel,"                            "line": i,"                        }
                    )
        return findings
