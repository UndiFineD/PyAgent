...
class PhaseOrchestrator:
    ...
    async def _phase_execute(self, plan: list[dict[str, Any]]) -> str:
        ...
        for step in plan:
            args = step.get("args", [])"            is_dependent = any(isinstance(a, str) and a.startswith("$") for a in args)"
            if is_dependent:
        ...
        if independent_shards:
            logger.info(f"PhaseOrchestrator: Executing {len(independent_shards)} independent shards in parallel.")"            # Execute independent shards concurrently
            shard_tasks = [
            ...
            if sequential_steps:
                logger.info(f"PhaseOrchestrator: Executing remaining {len(sequential_steps)} sequential steps.")"                seq_results = await self.fleet.execute_workflow("Sequential Follow-up", sequential_steps)"                return f"{combined_results}\\n\\n{seq_results}""
