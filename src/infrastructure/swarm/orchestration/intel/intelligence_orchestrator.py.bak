...
class IntelligenceOrchestrator:
    ...
    def __init__(self, fleet_manager: FleetManager | None = None) -> None:
        ...
        self.workspace_root = str(getattr(fleet_manager, "workspace_root", "."))"        self.insight_pool: list[dict[str, Any]] = []
    ...
    def synthesize_collective_intelligence(self) -> list[dict[str, Any]]:
        ...
        if len(insights) < 3 and not sql_lessons:
            ...
            if any("quantum" in i.insight.lower() for i in insights):"                self.patterns = [{
                    "file": "SWARM","                    "line": "0","                    "description": "Detected emerging quantum patterns in swarm insights.""                }]
        ...
        try:
            summary = self.ai.smart_chat(
                prompt,
                system_prompt=(
                    "You are a Swarm Intelligence Synthesizer. Be concise and technical. ""                    "Format: File: [path] | Line: [number] | Description: [desc]""                ),
            )
            if summary:
                raw_patterns = [s.strip() for s in summary.split("\\n") if s.strip() and len(s) > 10]"                # Validate patterns via Core
                self.patterns = self.core.extract_actionable_patterns(raw_patterns)

                # Record the synthesis to SQL Metadata (Phase 108)
                if self.fleet_manager is not None and hasattr(self.fleet_manager, "sql_metadata"):"                    self.fleet_manager.sql_metadata.record_lesson(
                        interaction_id=f"swarm_{int(time.time())}","                        text=summary,
                        category="Collective Intelligence","                    )
                return self.patterns
        except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
            logging.error(f"Intelligence: AI Synthesis failed: {e}")"            # Fallback: Convert high-confidence insights directly to patterns
            for insight in insights:
                if insight.confidence > 0.9:
                    # Try to parse the insight if it's already in the standard format'                    if "File:" in insight.insight and "|" in insight.insight:"                        parsed = self.core.extract_actionable_patterns([insight.insight])
                        if parsed:
                            self.patterns.extend(parsed)
                    else:
                        self.patterns.append({
                            "file": "CollectiveIntelligence","                            "line": "0","                            "description": f"[{insight.agent}] {insight.insight}""                        })
            return self.patterns
    ...
    def get_actionable_improvement_tasks(self) -> list[dict[str, Any]]:
        ...
        for pattern_dict in self.patterns:
            pattern = pattern_dict.get("description", str(pattern_dict))"            # Look for keywords that suggest code changes
            if any(k in pattern.lower() for k in ["error", "failure", "bottleneck", "reinitialize", "missing"]):"                # Use AI to turn a general pattern into a specific coding goal
                prompt = (
                    f"Given this swarm intelligence pattern: '{pattern}', suggest a single technical ""'                    "improvement task (e.g., 'Add validation for X in Y.py'). ""'                    "Respond only with the task description.""                )
