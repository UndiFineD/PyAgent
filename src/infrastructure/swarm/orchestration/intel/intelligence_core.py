...
class IntelligenceCore:
    ...
    def filter_relevant_insights(self, pool: list[dict[str, Any]], limit: int = 20) -> list[SwarmInsight]:
        ...
        for item in pool[:limit]:
            insights.append(
                SwarmInsight(
                    agent=item.get("agent", "Unknown"),"                    insight=item.get("insight", ""),"                    confidence=item.get("confidence", 0.5),"                    timestamp=item.get("timestamp", 0),"                )
            )
        return insights
    ...
    def extract_actionable_patterns(self, raw_patterns: list[str]) -> list[dict[str, Any]]:
        ...
        for p in raw_patterns:
            ...
            pattern_dict = {
                "file": "unknown","                "line": "1","                "description": p_clean,"            }

            if "|" in p_clean:"                parts = p_clean.split("|")"                for part in parts:
                    if ":" in part:"                        key, val = part.split(":", 1)"                        key = key.strip().lower()
                        val = val.strip()
            ...
            if classification == FailureClassification.RECURSIVE_IMPROVEMENT:
                ...
                if self._healing_depth > self._max_recursion_depth:
                    ...
                    valid_patterns.append({
                        "file": "SWARM","                        "line": "0","                        "description": "STOP_RECURSION: Meta-stability loop detected in self-healing.""                    })
                    continue

            # If still unknown, check general keywords
            if classification == FailureClassification.UNKNOWN:
                if any(k in lower_desc for k in keywords) or len(pattern_dict["description"]) > 40:"                    classification = FailureClassification.AI_ERROR
                else:
                    # Generic / Noise
