...
class HolographicStateOrchestrator:
    ...
    def __init__(self, fleet=None) -> None:
        ...
        if self.fleet and hasattr(self.fleet, "backup_node"):"            self.backup_node = self.fleet.backup_node
        else:
    ...
    async def shard_hologram(self, hologram_id: str, hologram_data: dict, redundancy: int = 2) -> dict:
        ...
        for angle, data in perspectives.items():
            ...
            perspective_shards = self.backup_node.create_shards({
                "hologram_id": hologram_id,"                "perspective": angle,"                "data": data,"                "timestamp": hologram_data.get("timestamp", 0)"            }, custom_hash=f"{hologram_id}_{angle}")"
            # Record locally
            ...
            for shard in perspective_shards:
                if shard["mirror_index"] == 0:"                    self.backup_node.store_shard_locally(shard)

            # Mirror shards to neighbors
            if self.fleet and hasattr(self.fleet, "voyager_transport"):"                await self._mirror_to_neighbors(perspective_shards)

            shards_created += len(perspective_shards)

        # Broadcast "projection" (metadata) to neighborhood"        await self._broadcast_projection(hologram_id, list(perspectives.keys()))
    ...
    async def reconstruct_perspective(self, hologram_id: str, angle: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.fleet or not hasattr(self.fleet, "voyager_discovery"):"            logger.warning("Holographic: Swarm transport unavailable for reconstruction.")"            return None

        # Phase 330: Try to find local RAID shards first
        matching_shards = self.backup_node.get_local_shards_for_hash(hologram_id)
        if matching_shards:
            reconstructed = self.backup_node.reassemble_state({s["shard_id"]: s for s in matching_shards})"            if reconstructed and angle in reconstructed:
                logger.info(f"Holographic: Reconstructed '{angle}' from local shards.")"'                return reconstructed[angle]
    ...
    async def _mirror_to_neighbors(self, shards: List[Dict[str, Any]]):
        ...
        for i, shard in enumerate(shards):
            ...
            addr = peer.get("addr")"            port = peer.get("port", 5555)"
            if self.fleet and hasattr(self.fleet, "workspace_root"):"                sender_id = f"fleet-{self.fleet.workspace_root.name}""            else:
                sender_id = self.agent_id

            msg = {
                "type": "shard_store","                "sender_id": sender_id,"                "shard": shard"            }
    ...
    async def _broadcast_projection(self, hologram_id: str, angles: List[str]):
        ...
        projection = {
            "type": "hologram_projection","            "hologram_id": hologram_id,"            "angles": angles,"            "sender_id": sender_id"        }

        if self.fleet and hasattr(self.fleet, "voyager_transport"):"            peers = self.fleet.voyager_discovery.get_active_peers()
            for peer in peers:
