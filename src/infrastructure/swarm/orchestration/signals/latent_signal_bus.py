...
class LatentSignalBus:
    ...
    def transmit_latent(self, channel: str, state_payload: dict[str, Any]) -> str:
                Compresses a complex state payload into a 'latent signal' and transmits it.'                logging.info(f"LatentSignalBus: Encoding state for channel '{channel}'")"'
        ...
        raw_json = json.dumps(state_payload, separators=(",", ":"))"        latent_vector = base64.b64encode(raw_json.encode()).decode()

        self.latent_space[channel] = {
            "vector": latent_vector,"            "timestamp": datetime.now().isoformat(),"            "origin": "telepathic_compression_v1","        }

        # Emit signal to notify listeners of latent update
        if hasattr(self.fleet, "signals"):"            self.fleet.signals.emit(
                "LATENT_SIGNAL_RECEIVED","                {"channel": channel, "latent_checksum": hash(latent_vector)},"            )
    ...
    def receive_latent(self, channel: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logging.error(f"LatentSignalBus: Decompression failed: {e}")"            return None
