    ...
    async def fuse_outputs(
        ...
        if self.audit_logger and task_id:
            self.audit_logger.log_event(
                task_id=task_id,
                event_type="expert_fusion","                description=f"Merged expert results using {mode}","                data={"consensus_score": result.consensus_score, "experts": result.contributing_experts, "mode": mode},"            )
