...
class SwarmOptimizer:
    ...
    def monitor_efficiency(self) -> list[dict[str, Any]]:
        ...
        # Latency check
        avg_lat = summary.get("avg_latency_ms", 0)"        if avg_lat > 5000:
            suggestions.append(
                {
                    "type": "scaling","                    "reason": "High average fleet latency","                    "action": "Increase K8s replicas for specialized workers","                }
            )

        # Success rate check
        success_rate = summary.get("success_rate", 100)"        if success_rate < 80:
            suggestions.append(
                {
                    "type": "model_tuning","                    "reason": "Low success rate","                    "action": "Shift primary agents to gpt-4.1 from gpt-3.5","                }
            )
