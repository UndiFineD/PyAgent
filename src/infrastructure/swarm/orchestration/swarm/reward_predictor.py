...
Calculates a global bias for each expert based on general success.        biases = {}
    ...
    def adjust_routing(self, agent_id: str, base_similarity: float, domain: str = "general") -> float:"                Adjusts the raw similarity score with a historical reward bias.
                bias = self.expert_biases.get(agent_id, 0.0)

        # Check domain-specific baselines
        domain_baseline = self.wisdom.get("domain_baselines", {}).get(domain, 0.5)"        # If the domain itself is high-performing, we trust experts more (scaling)
