...
class ExpertEvolutionService:
    ...
    def process_evaluation(self, evaluation: ExpertEvaluation):
        ...
        if not current_profile:
            logger.warning(
                f"Evolution: Received evaluation for unknown expert {expert_id}"
            )
            return
        ...
        target_score = (
            ...
            else (evaluation.quality_score * 0.5)
        )

        # EMA update: score = (1 - alpha) * current + alpha * target
        new_score = ((1.0 - self.learning_rate) * current_score) + (
        ...
        logger.info(
            f"Evolution: Expert {expert_id} evolved: {current_score:.3f} -> {new_score:.3f}"
        )
