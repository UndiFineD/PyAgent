...
class FederatedMetaOptimizer:
    ...
    def run_optimization_step(self):
        ...
        if avg_vram_util > 0.8:
            new_target = min(0.9, self.config.get("distillation_ratio", 0.5) + 0.05)"            updates["distillation_ratio"] = new_target"            logger.info(
        ...
        acc_rate = metrics.get("speculative_acceptance_rate", 0.75)"        if acc_rate < 0.6:
            new_thresh = min(0.95, self.config.get("similarity_threshold", 0.85) + 0.01)"            updates["similarity_threshold"] = new_thresh"            logger.info(
