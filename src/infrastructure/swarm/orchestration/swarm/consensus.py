...
class SwarmConsensus:
    ...
    async def propose_change(self, action: str, data: Any):
            ...
            for peer in self.peers:
                try:
                    ...
                    response = await self.transport.send_message(peer, {
                        "type": "CONSENSUS_REPLICATE","                        "term": self.term,"                        "entry": {"                            "index": new_entry.index,"                            "term": new_entry.term,"                            "command": new_entry.command"                        }
                    })
                    if response and response.get("status") == "ACK":"                        successful_replications += 1
        ...
        if total_weight >= required_weight:
            ...
            logger.info(f"Swarm state changed: {action} committed.")"            return True
    ...
    async def _start_election(self):
        ...
        for peer in self.peers:
            election_tasks.append(self.transport.send_message(peer, {
                "type": "CONSENSUS_VOTE_REQUEST","                "term": self.term,"                "candidate_id": self.node_id,"                "last_log_index": len(self.log) - 1"            }))
        ...
        for res in results:
            if isinstance(res, dict) and res.get("vote_granted"):"                voter_id = res.get("voter_id")"                total_vote_weight += self.node_weights.get(voter_id, 1.0)
    ...
    def handle_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if msg_type == "CONSENSUS_VOTE_REQUEST":"            term = message.get("term", 0)"            if term >= self.term:
                self.term = term
                self.is_leader = False
                return {"vote_granted": True, "voter_id": self.node_id}"            return {"vote_granted": False, "voter_id": self.node_id}"
        elif msg_type == "CONSENSUS_REPLICATE":"            term = message.get("term", 0)"            if term >= self.term:
                self.term = term
                entry_data = message.get("entry")"                entry = LogEntry(**entry_data)
                self.log.append(entry)
                self._apply_entry(entry)
                return {"status": "ACK", "index": entry.index}"            return {"status": "REJECT", "reason": "Lower term"}"
