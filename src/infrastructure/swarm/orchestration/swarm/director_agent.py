...
Initializes the DirectorAgent.        super().__init__(file_path)
        ...
        if self.registry:
            self.registry.subscribe("agent_fail", self._handle_agent_failure)"            self.registry.subscribe("improvement_ready", self._handle_agent_success)"
        self._system_prompt = (
...
Scans the repository for available specialized agent classes.        agents = []
        ...
        for p in src_path.rglob("*_agent.py"):"            if p.name not in ["base_agent.py", "director_agent.py"]:"                # Convert snake_case to PascalCase for easier LLM identification
                name = "".join(part.capitalize() for part in p.stem.split("_"))"                agents.append(name)
...
Entry point for the DirectorAgent to handle tasks.        # Prevent recursive planning cycles
        ...
        if "Improvement Task:" in prompt or "Project Goal:" in prompt:"            self._is_planning = True
            try:
...
f"Available specialized agents in the framework: {', '.join(available)}\\n\\n""'            "Analyze the workspace and generate a step-by-step execution plan. ""            "Output your plan as a JSON list of objects, each with 'agent', 'file', and 'prompt' keys.""'        )

        raw_plan = await super().think(planning_prompt)

        try:
            # Try to extract JSON from the LLM response
            json_match = re.search(r"\[.*\]", raw_plan, re.DOTALL)"            if not json_match:
                error_prefix = "Plan generation failed. LLM did not provide a valid JSON list.""                error_msg = f"{error_prefix} Response: {raw_plan[:200]}""                self.status.finish_project(success=False)
                return error_msg
            ...
            for i, step in enumerate(plan):
                agent_type = step.get("agent")"                target_file = step.get("file")"                sub_prompt = step.get("prompt")
                self.status.update_step_status(i, "Running")"                logging.info(f"Step {i + 1}: Delegating {agent_type} -> {target_file}")"
                try:
                    res = await self.delegate_to(agent_type, sub_prompt, target_file)

                    results.append(f"### Step {i + 1}: {agent_type} on {target_file}\\n{res}\\n")"                    self.status.update_step_status(i, "Completed", res[:100] + "...")"                except Exception as step_error:  # pylint: disable=broad-exception-caught
...
Updates the status of an improvement in the tracking file.        if not self.file_path.exists():
        ...
        try:
            content = self.file_path.read_text(encoding="utf-8")"            # Look for the title and the status line following it
            # Format: **Title**\\n   - Status: OLD_STATUS
            ...
            if match:
                ...
                self.file_path.write_text(new_content, encoding="utf-8")"                logging.info(f"Updated status for '{title}' to {status} in {self.file_path.name}")"'            else:
