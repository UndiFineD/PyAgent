...
class ImmunizationOrchestrator:
    ...
    def scan_for_threats(self, prompt: str) -> bool:
            ...
            if re.search(signature, prompt, re.IGNORECASE):
                logging.warning(f"ImmunizationOrchestrator: Adversarial pattern detected: {signature}")"                return True
        return False
    ...
    def immunize(self, adversarial_example: str, label: str) -> str:
        ...
        if pattern not in self.threat_signatures:
            ...
            self.immunization_log.append(
                {
                    "label": label,"                    "pattern": pattern,"                    "timestamp": logging.time.time() if hasattr(logging, "time") else 0,"                }
            )

            # Broadcast the new antibody to the fleet
            if hasattr(self.fleet, "signals"):"                self.fleet.signals.emit("FLEET_IMMUNIZED", {"threat": label, "pattern": pattern})"
        return pattern
