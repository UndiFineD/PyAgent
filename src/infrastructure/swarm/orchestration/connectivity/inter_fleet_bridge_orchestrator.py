...
class InterFleetBridgeOrchestrator:
    ...
    async def send_signal(self, peer_name: str, signal_type: str, data: Any) -> Any:
        ...
        Bridges the legacy 'signal' API to the new Voyager transport.'                target = self.discovery_node.resolve_synapse_address(peer_name)
        if not target:
            logger.error(f"Voyager: Could not resolve peer '{peer_name}' for signal.")"'            return {"status": "error", "message": "Peer not found"}"
        peer_ip, peer_port = target
        payload = {"type": signal_type, "data": data, "sender_id": getattr(self.fleet_manager, "fleet_id", "unknown")}"        return await self.synapse.transport.send_to_peer(peer_ip, peer_port, payload)
    ...
    def find_best_offload_target(self, required_cpu: int, required_ram: float) -> Optional[tuple[str, int]]:
        ...
        for peer in peers:
            props = peer.get("properties", {})"            try:
                # Parse resources sent as strings
                peer_cpu = float(props.get("cpu_cores", "1"))"                peer_ram = float(props.get("ram_gb", "4.0"))"
                # Check constraints
                if peer_cpu >= required_cpu and peer_ram >= required_ram:
                    ...
                    if score > max_score:
                        max_score = score
                        addrs = peer.get("addresses", [])"                        port = int(props.get("transport_port", "5555"))"                        if addrs:
    ...
    async def offload_task(self, task_description: str, required_cpu: int = 1,
        ...
        if not target:
            logger.warning(
                f"Voyager: No suitable peer found for task offload (CPU:{required_cpu}, ""                f"RAM:{required_ram})""            )
            return None
    ...
    async def query_federated_memory(self, query: str, limit_per_peer: int = 3) -> List[Dict[str, Any]]:
        ...
        tasks = []
        for peer in peers:
            addrs = peer.get("addresses", [])"            if not addrs:
                continue
            addr = addrs[0]
            port = int(peer["properties"].get("transport_port", "5555"))"            tasks.append(self.synapse.transport.send_to_peer(addr, port, payload))
