...
class FederatedOrchestrator:
    ...
    def negotiate_privacy_boundaries(self, proposal_id: str, swarm_id: str, constraints: list[str]) -> bool:
                ...
                logging.info(f"FederatedOrchestrator: Negotiating constraints for {proposal_id} with {swarm_id}")"        for p in self.negotiation_history:
            if p["proposal_id"] == proposal_id:"                p["constraints"] = p.get("constraints", {})"                p["constraints"][swarm_id] = constraints"                return True
        return False
