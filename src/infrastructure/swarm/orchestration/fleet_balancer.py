...
class FleetBalancer:
    ...
    def add_node(self, node_id: str, capacity: int):
        """Adds or updates a node in the fleet."""
        self.nodes[node_id] = SwarmNode(node_id=node_id, capacity=capacity)
    ...
    def _update_balancer_params(self):
        """Recalculates GCD and max weight for the WRR algorithm."""
        if not self.nodes:
            self._max_weight = 0
            self._gcd_weight = 1
            return
    ...
    async def get_next_node(self) -> Optional[str]:
        """Provides the next node ID according to Weighted Round-Robin."""
    ...
    async def report_load(self, node_id: str, load: int):
        """Updates the current load of a node to adjust balancing dynamically."""
        if node_id in self.nodes:
