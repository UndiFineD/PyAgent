...
class VoyagerTransport:
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    sender_id = message.get("sender_id", identity.hex())"                    signature = message.get("signature", "unsigned")"                    if not self.firewall.validate_message(message, signature, sender_id):
                        logger.warning(f"Firewall: Blocked unauthorized message from {sender_id}")"                        continue
                    ...
                    if self.running:
                        logger.error(f"Voyager: Transport server error: {e}")"                    await asyncio.sleep(1)
        finally:
            self.stop()
    ...
    async def send_to_peer(
        ...
        try:
            ...
            logger.warning(f"Voyager: Timeout waiting for response from {target}")"            return None
        except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
            logger.error(f"Voyager: Failed to send to {target}: {e}")"            return None
        finally:
            dealer.close()
