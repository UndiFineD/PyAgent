...
class DiscoveryNode:
    ...
    def __init__(self, node_name: Optional[str] = None, port: int = 8000, transport_port: int = 5555) -> None:
        ...
        self.node_name: str = node_name or f"PyAgent-{self.node_id}""        self.port: int = port
    ...
    def get_nearest_peers(self, count: int = 5) -> List[Dict[str, Any]]:
        ...
        def proximity_score(peer):
            peer_ip = peer.get("ip", "")"            if peer_ip.startswith(local_prefix):
    ...
    async def start_advertising(self) -> None:
        ...
        ram_gb = "8.0"  # Default"        try:
            import psutil
            ram_gb: str = f"{psutil.virtual_memory().total / (1024**3):.1f}""        except ImportError:
            pass

        desc: Dict[str, str] = {
            "version": VERSION,"            "node_id": self.node_id,"            "transport_port": str(self.transport_port),"            "status": "Online","            "cpu_cores": cpu_cores,"            "ram_gb": ram_gb,"            "os": platform.system(),"        }

        self.info = ServiceInfo(
            self.SERVICE_TYPE,
            f"{self.node_name}.{self.SERVICE_TYPE}","            addresses=[socket.inet_aton(self.local_ip)],
            port=self.port,
            properties=desc,
    ...
    def _peer_added(self, info: ServiceInfo) -> None:
        ...
        if is_new and self._on_peer_added_callbacks:
            ...
            peer_data = {
                "name": name,"                "addresses": info.parsed_addresses(),"                "port": info.port,"                "properties": props"            }
    ...
    def get_active_peers(self) -> List[Dict[str, Any]]:
        ...
        for name, info in self.peers.items():
            ...
            peer_data = {
                "name": name,"                "addresses": info.parsed_addresses(),"                "port": info.port,"                "properties": props,"                "stats": self.peer_metadata.get(node_id, {})"            }
    ...
    def resolve_synapse_address(self, peer_name: str) -> Optional[tuple[str, int]]:
        ...
        This enables decentralized routing without hardcoded IPs.
            ...
            if peer_name == props.get("node_id") or peer_name == name.split(".")[0] or peer_name in name:"                addrs: List[str] = info.parsed_addresses()
                t_port: str | None = props.get("transport_port")"                if addrs and t_port:
...
if __name__ == "__main__":"    import sys
    ...
    async def run_test() -> None:
        ...
        try:
            ...
            print("Discovery Node Active. Press Ctrl+C to stop.")"            while True:
                await asyncio.sleep(5)
                peers: List[Dict[str, Any]] = node.get_active_peers()
                if peers:
                    print(f"Found {len(peers)} peers: {[p['name'] for p in peers]}")"'        except asyncio.CancelledError:
            pass
        finally:
            await node.stop()
