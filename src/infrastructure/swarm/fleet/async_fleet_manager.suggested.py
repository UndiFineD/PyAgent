...
class AsyncFleetManager(FleetManager):  # pylint: disable=too-many-ancestors
    ...
    async def execute_workflow_async(
        ...
        if workflow_id in self.active_workflows:
            ...
            logging.info(f"Resuming workflow {workflow_id} from step index {state.get('next_batch_idx', 0)}")"'        else:
            state = WorkflowState(task_id=workflow_id, original_request=task)
            state.set("next_batch_idx", 0)"            state.set("all_results", [])"            self.active_workflows[workflow_id] = state
        ...
        for i, step in enumerate(workflow_steps):
            ...
            step_id = step.get("id") or f"step_{i}""            step_map[step_id] = step
            # Phase 242: Add resource hints to graph for refined batching
            graph.add_node(step_id, resources=step.get("resources", []))"            for dep in step.get("depends_on", []):"                graph.add_dependency(step_id, dep)

        # 2. Resolve Batches
        try:
            batches = graph.resolve()
        except ValueError as e:
            logging.error(f"Workflow dependency resolution failed: {e}")"            return f"Error: Invalid workflow graph - {e}""
        logging.info(f"Resolved workflow into {len(batches)} parallel execution batches.")"
        all_results = state.get("all_results")"        start_idx = state.get("next_batch_idx")"
        ...
        for batch_idx in range(start_idx, len(batches)):
            ...
            if state.get("migration_pending"):"                logging.info(f"Migration signal received for {workflow_id}. Suspending at batch {batch_idx}.")"                state.set("next_batch_idx", batch_idx)"                if workflow_id in self._migration_events:
                    self._migration_events[workflow_id].set()
                return f"WORKFLOW_SUSPENDED: {workflow_id} at batch {batch_idx}""
            logging.info(f"Executing batch {batch_idx + 1}/{len(batches)}: {batch}")"            tasks = [self._run_single_step(step_map[step_id], workflow_id) for step_id in batch]
            ...
            for i, res in enumerate(responses):
                ...
                if isinstance(res, Exception):
                    logging.error(f"Async failure in batch {batch_idx + 1} for {agent_name}: {res}")"                    all_results.append(f"### Error from {agent_name} ({step_id})\\n{str(res)}\\n")"                else:
    ...
    async def migrate_workflow(self, workflow_id: str, remote_manager: AsyncFleetManager) -> bool:
        ...
        logging.info(f"Waiting for workflow {workflow_id} to suspend...")"        await event.wait()

        # Workflow is now suspended. Move state to remote.
        success = await remote_manager.handoff_state(state)

        if success:
            logging.info(f"Workflow {workflow_id} successfully migrated.")"            del self.active_workflows[workflow_id]
    ...
    async def _run_single_step(self, step: dict[str, Any], workflow_id: str) -> str:
        ...
        if not action_name:
            return "Error: Action name is required.""        action_fn = getattr(agent, action_name, None)
        if not action_fn:
            return f"Error: Action '{action_name}' not supported.""'
        trace_id = f"{workflow_id}_{agent_name}_{action_name}""        self.telemetry.start_trace(trace_id)

        try:
            from src.infrastructure.swarm.orchestration.system.lock_manager import LockManager
            ...
            async def run_with_async_locks(res_list: list[str]) -> Any:
                ...
                res = res_list[0]
                l_type = "file" if any(c in res for c in "/\\.") else "memory""                async with locker.acquire_async(res, lock_type=l_type):
    ...
    async def _pre_commit_audit(self, content: str, agent_name: str) -> str:
        ...
        try:
            ...
            audit_agent = self.agents.get("LegalAudit")"            if not audit_agent:
                return content

            # 1. License Check (Phase 238)
            compliance = audit_agent.check_license_compliance(content)
            if not compliance["is_compliant"]:"                violations_str = ", ".join(compliance["violations"])"                logging.warning(f"Legal Violation in {agent_name} output: {violations_str}")"                return (
                    f"[LEGAL_BLOCK]: Output from {agent_name} contains blacklisted licenses ""                    f"({violations_str}). REDACTED.""                )

            # 2. Liability Check
            liability = audit_agent.generate_liability_report(content)

            if "WARNING" in liability:"                logging.warning(f"Liability Risk in {agent_name} output: {liability}")"                # Append disclaimer instead of blocking
                return (
...
if __name__ == "__main__":"    # Test script
    ...
    ROOT_PATH = ".""    afleet = AsyncFleetManager(ROOT_PATH)
    afleet.register_agent("K1", KnowledgeAgent)"    afleet.register_agent("S1", SecurityGuardAgent)"
    wf = [
