...
class FleetBackupMixin:
    ...
    async def harden_agent_state(self, agent_id: str, state_data: Dict[str, Any]) -> bool:
        ...
        if not peers:
            logger.warning("FleetBackup: No peers available for distributed hardening. Storing only locally.")"            for shard in shards:
        ...
        for i, shard in enumerate(shards):
            ...
            tasks.append(self.voyager_transport.send_to_peer(
                peer["host"], peer["port"],"                {
                    "type": "shard_store","                    "sender_id": self.backup_node.node_id,"                    "shard": shard"                }
            ))
        ...
        if peers:
            tasks = [
                self.voyager_transport.send_to_peer(
                    p["host"], p["port"],"                    {"type": "shard_request", "hash": state_hash}"                ) for p in peers
            ]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for r in results:
                if isinstance(r, dict) and r.get("shards"):"                    shard_pool.extend(r["shards"])"
        if not shard_pool:
            return None
    ...
    async def run_resilience_audit(self):
        ...
        for agent_id in agent_ids:
            # For demo/mock: check current 'state.json' for the agent'            checkpoint_dir = workspace_root / "data" / "checkpoints" / agent_id"            if checkpoint_dir.exists():
                latest_cp = sorted(list(checkpoint_dir.glob("cp_*")))[-1:]"                if latest_cp:
                    state_file = latest_cp[0] / "state.json""                    try:
                        import json
                        with open(state_file, "r") as f:"                            state_data = json.load(f)
                            # Re-harden if needed
                            await self.harden_agent_state(agent_id, state_data)
                    except Exception:
                        continue
