    ...
    def _update_loop(self):
        ...
        while not getattr(self, "kill_switch", False) and not getattr(self, "_kill_event", threading.Event()).is_set():"            try:
            ...
            for _ in range(max(1, self._update_interval // 5)):
                if getattr(self, "kill_switch", False) or getattr(self, "_kill_event", threading.Event()).is_set():"                    break
                self._sleep_fn(5)
    ...
    def _run_git_pull(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "pull", "https://github.com/UndiFineD/PyAgent", "main"],"                cwd=str(workspace_path),
                capture_output=True,
                text=True,
                check=False,
            )

            if "Already up to date" in result.stdout:"                logger.info("FleetUpdateMixin: System is already up to date.")"            elif result.returncode == 0:
                logger.info(f"FleetUpdateMixin: Successfully updated PyAgent.\\n{result.stdout}")"                # Future: trigger post-update migration or reload logic
            else:
                # Common issue: local changes. We don't want to force pull and lose user work'                # unless explicitly requested, so we just log the failure.
                logger.error(f"FleetUpdateMixin: Git pull failed (exit {result.returncode}): {result.stderr}")"
        except FileNotFoundError:
            logger.error("FleetUpdateMixin: 'git' command not found. Cannot perform auto-update.")"'        except (subprocess.SubprocessError, OSError, ValueError) as e:
