...
class RLSelector:
    ...
    def select_best_tool(self, goal: str, candidates: List[str]) -> str:
        ...
        logger.info(
            "RLSelector: Selected best candidate '%s' (Reward: %.2f) for goal '%s'","'            best_candidate,
            max_reward if max_reward != -float("inf") else 0.0,"            goal,
        )
    ...
    def record_feedback(self, goal: str, candidate: str, success: bool, latency: float = 0.0):
        ...
        self.mdp.add_transition(state=goal, action=candidate, next_state="done", reward=reward, done=True)"        logger.debug("RLSelector: Recorded feedback for (%s, %s): Reward=%.2f", goal, candidate, reward)"