...
class FleetManager(
    ...
    def _safe_start_task(self, coro) -> None:
        """Starts a task if an event loop is running, otherwise logs a warning.        try:
            ...
            with contextlib.suppress(Exception):
                coro.close()
            task_name = getattr(getattr(coro, "cr_code", None), "co_name", "unknown")"            logging.debug(f"Fleet: Could not start task {task_name} - no event loop.")"
    def __init__(self, workspace_root: str) -> None:
        self.workspace_root = Path(workspace_root)
        self.manifest_repo = ManifestRepository()
        self.backup_node = DistributedBackup(node_id=f"node-{self.workspace_root.name}")"        self.evolution_loop: EvolutionLoop | None = None
        ...
        self.firewall = ZeroTrustFirewall(owner_key=f"node-key-{self.workspace_root.name}")"        self.infection_guard = InfectionGuard(workspace_root=str(self.workspace_root))
        ...
        self.capability_hints = {
            "articulate": "LinguisticAgent","            "reason": "ReasoningAgent","            "code": "CoderAgent","            "sql": "SqlQueryAgent","            "git": "PullRequestAgent","            "teleport": "ModalTeleportationOrchestrator",  # Orchestrator logic"            "byzantine_judge": "ByzantineConsensusAgent","            "governance": "GovernanceAgent","            "immune_system": "ImmuneSystemAgent","            "morphologicalevolution": "MorphologicalEvolution","            "signal_bus": "SignalBusOrchestrator","            "world_model": "WorldModelAgent","            "nas": "NetworkArchSearchAgent","            "heartbeat": "HeartbeatOrchestrator","            "entanglement": "EntanglementOrchestrator","            "core_expansion": "CoreExpansionAgent","            "temporal_shard": "TemporalShardAgent","            "reality_anchor": "RealityAnchorAgent","            "cognitive_borrowing": "CognitiveBorrowingOrchestrator","            "architect": "ArchitectAgent","            "fractal_knowledge": "FractalKnowledgeOrchestrator","            "dependency_graph": "DependencyGraphAgent","            "ui_architect": "UiArchitectAgent","            "evolution_guard": "CoreEvolutionGuard","            "explainability": "ExplainabilityAgent","            "Explainability": "ExplainabilityAgent","            "privacy_guard": "PrivacyGuardAgent","            "PrivacyGuard": "PrivacyGuardAgent","            "security_audit": "SecurityAuditAgent","            "SecurityAudit": "SecurityAuditAgent","            "model_router": "RouterModelAgent","            "search_mesh": "SemanticSearchMeshAgent","            "policy_enforcement": "PolicyEnforcementAgent","            "sovereignty_orchestrator": "SovereigntyOrchestrator","            "fractal_orchestrator": "FractalOrchestrator","            "intention_predictor": "IntentionPredictionAgent","            "cooperative_comm": "CooperativeCommunicationAgent","            "resource_curator": "ResourceCurationAgent","            "resource_arbitrator": "SwarmArbitratorAgent","            "honeypot": "HoneypotAgent","            "consensus_orchestrator": "ConsensusOrchestrator","            "speciation_orchestrator": "SpeciationOrchestrator","            "speciation": "SpeciationAgent","            "feature_store": "FeatureStoreAgent","            "inter_fleet_identity": "InterFleetIdentityAgent","            "inter_fleet_bridge": "InterFleetBridgeOrchestrator","            "synthetic_data": "SyntheticDataAgent","            "graph_relational": "GraphRelationalAgent","            "empathy_engine": "EmpathyAgent","            "neurosymbolic": "NeuroSymbolicAgent","            "neuro_symbolic": "NeuroSymbolicAgent","            "agent_identity": "IdentityAgent","            "ethics_guardrail": "EthicsGuardrailAgent","            "linguist": "LinguisticAgent","            "model_forge": "ModelForgeAgent","            "memorag": "MemoRAGAgent","            "process_synthesizer": "ProcessSynthesizerAgent","            "tool_synthesis": "ToolSynthesisAgent","            "memory_replay": "MemoryReplayAgent","            "swarm_distillation": "SwarmDistillationAgent","            "fleet_economy": "FleetEconomyAgent","            "swarm_visualizer": "SwarmVisualizerAgent","            "consensus_conflict": "ConsensusConflictAgent","            "visualizer": "VisualizerAgent","            "fleet_deployer": "FleetDeployerAgent","            "cloud_provider": "CloudProviderAgent","            "legal_audit": "LegalAuditAgent","            "memory_pruning": "MemoryPruningAgent","            "logic_prover": "LogicProverAgent","            "reward_model": "RewardModelAgent","            "audio_reasoning": "AudioReasoningAgent","            "entropy_guard": "EntropyGuardAgent","            "compliance_agent": "ComplianceAgent","            "sql_coder_agent": "SQLCoderAgent","            "telemetry_agent": "TelemetryAgent","            "workflow_agent": "WorkflowAgent","            "universal": "UniversalAgent","        }
    ...
    async def _on_voyager_peer_added(self, peer_data: Dict[str, Any]) -> None:
        ...
        if peer_id not in self.voyager_transport.sessions:
            logger.info(f"Voyager: Initiating Double Ratchet session with {peer_id}...")"            response = await self.voyager_transport.send_to_peer(
                addr, port,
                {
                    "type": "HANDSHAKE_INIT","                    "sender_id": f"node-{self.workspace_root.name}","                    "public_key": b"node-v4-init-pub""                },
                peer_id=peer_id
            )

            if response and response.get("type") == "HANDSHAKE_RESPONSE":"                from src.infrastructure.security.encryption.double_ratchet import \
                    DoubleRatchet
        ...
        current_peers = [p["properties"].get("node_id", p["name"])"                         for p in self.voyager_discovery.get_active_peers()]
    ...
    async def _handle_voyager_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif msg_type == "HANDSHAKE_INIT":"            # Start Double Ratchet Session (Phase 2.0 Security)
            remote_pub = message.get("public_key")"            # In a real implementation, we'd do X3DH or similar'            # For Phase 3.0, we simulate with a dummy root key
    ...
    async def _topology_loop(self) -> None:
        """Periodically refreshes the swarm topology visualization data (Pillar 9).        while True:
            try:
                ...
                stats = self.resource_monitor.get_latest_stats()
                traffic = stats.get("network_io", {}).get("bytes_sent", 0)"                self.topology_reporter.update_traffic("localhost", traffic)"
                self.topology_reporter.record_node(
                    node_id="localhost","                    group="gateway","                    metadata={"cpu": stats.get("cpu_usage"), "mem": stats.get("memory_usage")}"                )

                # 3. Active Local Agents (Synaptic Connections)
                for agent_id in self.agents.keys():
                    self.topology_reporter.record_node(agent_id, group="agent")"                    self.topology_reporter.record_link("localhost", agent_id, strength=1.2, type="memory_bus")"
                ...
                logger.error(f"FleetManager: Topology Loop Error: {e}")"                await asyncio.sleep(5)


if __name__ == "__main__":"    # Test script for FleetManager
    logging.basicConfig(level=logging.INFO)
    root = Path(str(Path(__file__).resolve().parents[3]) + "")"    fleet = FleetManager(str(root))
    ...
    # report = fleet.execute_workflow("Initial Audit", workflow) # Async call, requires await or asyncio.run"    # For now, just logging calls replacement
