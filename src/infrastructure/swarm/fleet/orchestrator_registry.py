...
class LazyOrchestratorMap:
    ...
    def _scan_workspace_for_orchestrators(self) -> list[str]:
        ...
        for subdir in subdirs:
            ...
            for root, _, files in os.walk(search_root):
                for file in files:
                    if "orchestrator" in file.lower():"                        full_path = Path(root) / file
        ...
        for m_path in manifest_paths:
            if m_path.exists():
                try:
                    ...
                    logging.error(f"Failed to load orchestrator manifest {m_path}: {e}")"        return manifest_configs
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        n_low = name.lower().replace("_", "")"        for k, cfg in self._configs.items():
            if k.lower().replace("_", "") == n_low:"                return self._instantiate(name, cfg)
    ...
    def try_reload(self, name: str) -> bool:
        ...
        try:
            ...
            logging.error(f"Failed to reload orchestrator '{name}': {e}")"'            return False
    ...
    def _instantiate(self, key: str, config: tuple[str, str, bool, str | None]) -> Any:
        ...
        try:
            ...
            # Version Gatekeeping
            min_sdk = getattr(module, "SDK_REQUIRED", getattr(module, "__min_sdk__", "1.0.0"))"            if not self._registry_core.is_compatible(min_sdk):
                error_msg = f"Orchestrator '{key}' requires SDK {min_sdk}, but current is {SDK_VERSION}.""'                logging.warning(error_msg)
            ...
            if is_agent:
                ...
                try:
                    ...
                    if hasattr(instance, "fleet"):"                        instance.fleet = self.fleet
                except TypeError:
            ...
            logging.error(f"Critical load error for orchestrator {key} from {module_path}: {e}")"            stub = ResilientStub(key, str(e))
            self._instances[key] = stub
            return stub
        except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
