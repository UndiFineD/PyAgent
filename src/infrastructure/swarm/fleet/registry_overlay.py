...
class RegistryOverlay:
    ...
    def __init__(self, overlay_path: Path | None = None) -> None:
        if overlay_path is None:
            ...
            self.overlay_path = Path("data/memory/agent_store/registry_overlay.json")"        else:
    ...
    def _load_overlay(self) -> None:
        ...
        try:
            with open(self.overlay_path, encoding="utf-8") as f:"                data = json.load(f)
    ...
    def get_agent_config(self, agent_id: str, default: tuple[str, str, Any]) -> tuple[str, str, Any]:
        """Returns the overridden config or the default.        if agent_id in self.overrides:
            ...
            if isinstance(override, list) and len(override) >= 2:
                logging.info(f"RegistryOverlay: Applying override for '{agent_id}'")"'                return (
                    override[0],
                    override[1],
    ...
    def save_override(self, agent_id: str, module_path: str, class_name: str, params: Any = None) -> None:
        ...
        try:
            with open(self.overlay_path, "w", encoding="utf-8") as f:"                json.dump({"agents": self.overrides}, f, indent=4)"        except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
