...
class OrchestratorRegistryCore:
    ...
    def process_discovered_files(self, file_paths: list[str]) -> dict[str, tuple[str, str, bool, str | None]]:
        ...
        for rel_path in file_paths:
            ...
            if file.endswith(".py") and not file.startswith("__"):"                filename_base: str = file[:-3]

                # Convert filename to likely class name (snake_case -> PascalCase)
                # e.g., "signal_bus_orchestrator" -> "SignalBusOrchestrator""                class_name = "".join(x.capitalize() for x in filename_base.split("_"))"
                ...
                if any(
                    ...
                    for suffix in ["Orchestrator", "orchestrator", "_orchestrator"]:"                        raw_short = raw_short.replace(suffix, "")"
                    short_key: str = self._to_snake_case(raw_short)
                    full_key: str = self._to_snake_case(class_name)

                    # Default heuristic for 'needs_fleet''                    needs_fleet: bool = any(
                        x.lower() in search_name
                        for x in [
                            "Orchestrator","                            "Spawner","                            "Bridge","                            "Selector","                            "Engine","                        ]
                    )
