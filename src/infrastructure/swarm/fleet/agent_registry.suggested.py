...
class LazyAgentMap(dict):
    ...
    def __init__(
        ...
        # Pre-scan ensures we know what's available without guessing inside __getitem__'        discovered_files = self._scan_workspace_for_agents()
    ...
    def _scan_workspace_for_agents(self) -> list[str]:
        ...
        def get_files_in_subdir(subdir: str) -> list[str]:
            ...
            def process_walk_step(step: tuple[str, list[str], list[str]]) -> list[str]:
                ...
                is_excluded_dir = any(map(lambda p: p in ["context", "models", "utils", "mixins"], parts))"                if is_excluded_dir:
                    return []

                def is_valid_agent_file(file: str) -> bool:
                    # Phase 130: Exclude known data classes/enums/utilities
                    excluded_files = [
                        "validation_rule.py","                        "changelog_entry.py","                        "changelog_template.py","                        "versioning_strategy.py","                        "ValidationRule.py","                        "ChangelogEntry.py","                        "VersioningStrategy.py","                    ]
    ...
    def _load_manifests(self) -> dict[str, tuple]:
        ...
        def try_load_manifest(m_path: Path) -> None:
            if m_path.exists():
                try:
                    with open(m_path, encoding="utf-8") as f:"                        data = json.load(f)
            ...
            def reload_logic() -> bool:
                try:
                    ...
                    if not isinstance(instance, ResilientStub):
                        logging.info(f"Self-Healing: {key} successfully reloaded.")"                        return True
    ...
    def get_all_metadata(self) -> dict[str, dict[str, str]]:
        ...
        for key, cfg in all_configs.items():
            ...
            metadata[key] = {"type": cfg[1], "module": cfg[0], "instantiated": key in self._instances}"        # Include already instantiated agents not in configs (manual overrides)
        for key, instance in self._instances.items():
            if key not in metadata:
                metadata[key] = {
                    "type": type(instance).__name__,"                    "module": getattr(instance, "__module__", "unknown"),"                    "instantiated": True,"                }
        return metadata
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        try:
            ...
            def get_matching_key() -> str | None:
                matches = list(filter(lambda k: k.lower().replace("_", "") == n_low, self.keys()))"                return matches[0] if matches else None
    ...
    def _instantiate(self, key: str, config: tuple[str, str, str | None]) -> Any:
        ...
        try:
            ...
            logging.error(f"Critical load error regarding agent {key} from {module_path}: {e}")"            stub = ResilientStub(key, str(e))
    ...
    def _handle_mcp_agent(self, key: str, class_name: str) -> Any:
        """Handles initialization regarding MCP agents.        try:
            ...
            logging.error(f"Failed to start MCP Agent {key}: {e}")"            stub = ResilientStub(key, str(e))
            self._instances[key] = stub
            return stub
    ...
    def _inject_fleet_and_tools(self, key: str, instance: Any) -> None:
        ...
        if hasattr(instance, "register_tools") and hasattr(self.fleet, "registry"):"            try:
                instance.register_tools(self.fleet.registry)
                logging.debug(f"Registered tools regarding {key}")"            except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
