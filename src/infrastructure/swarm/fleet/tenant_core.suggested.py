...
class TenantCore:
    ...
    def validate_and_translate_path(self, tenant_root: str, relative_path: str) -> str:
                Pure logic to ensure a path stays within the tenant's boundaries.'        Returns the absolute path if valid, raises PermissionError otherwise.
    ...
    def get_required_dirs(self) -> list[str]:
        """Standardised tenant folder structure.        return ["src", "data", "data/logs", "config"]"