...
try:
    from rust_core import to_snake_case_rust, topological_sort_rust, detect_cycles_rust  # type: ignore
    _RUST_CYCLES = True
    _RUST_ACCEL = True
except ImportError:
    _RUST_ACCEL = False    
...
class AgentRegistryCore:
    ...
    def _get_short_name(self, name: str) -> str | None:
        ...
        n_low = name.lower()
        if n_low.endswith("agent"):
            return name[:-5].rstrip("_")
        if n_low.endswith("orchestrator"):
    ...
    def validate_agent_structure(self, agent_instance: Any, required_methods: list[str] | None = None) -> list[str]:
        ...
        return [method for method in reqs if not hasattr(agent_instance, method)]
