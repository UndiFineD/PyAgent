...
class CheckpointManager:
    ...
    async def create_checkpoint(self, state_buffer: bytes) -> str:
        """
        Creates a checkpoint by 'teleporting' it to a peer rank via RDMA.
        This is a non-blocking operation for the main reasoning loop if backgrounded.
        """
        ...
        try:
            ...
            result = self.rust_bridge.execute(
                ...
                {
                    "target_rank": self.peer_rank,
                    "id": checkpoint_id,
                    "buffer_size": len(state_buffer),
                    "priority": "HIGH",
    ...
    def get_latest_checkpoint(self) -> Optional[CheckpointMetadata]:
        """Returns the most recent successful checkpoint."""
