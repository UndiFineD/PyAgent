...
class DistributedBackup:
    ...
    def __init__(self, node_id: str, replication_factor: int = 3):
        ...
        self.local_shards_dir = Path("data/shards")"        self.local_shards_dir.mkdir(parents=True, exist_ok=True)
    ...
    def create_shards(self, state_data: Dict[str, Any], custom_hash: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for part_idx, part in enumerate(parts):
            if hasattr(hashlib, "blake3"):"                part_hash = hashlib.blake3(part).hexdigest()
            else:
    ...
    def store_shard_locally(self, shard: Dict[str, Any]) -> bool:
        """Stores a shard from a peer and verifies integrity.        shard_id = shard["shard_id"]"        data_b64 = shard.get("data_b64", "")"        part_hash = shard.get("part_hash")"
        # Integrity check
        if part_hash:
            actual_data = self._decode(data_b64)
            if hasattr(hashlib, "blake3"):"                actual_hash = hashlib.blake3(actual_data).hexdigest()
            else:
                actual_hash = hashlib.md5(actual_data).hexdigest()

            if actual_hash != part_hash:
                logger.error(f"DistributedBackup: Corruption detected in shard {shard_id}")"                return False

        file_path = self.local_shards_dir / f"{shard_id}.json""        with open(file_path, "w", encoding="utf-8") as f:"            json.dump(shard, f)
        return True

    def get_local_shards_for_hash(self, full_hash: str) -> List[Dict[str, Any]]:
        """Retrieves locally stored shards belonging to a specific state hash.        shards = []
        for shard_file in self.local_shards_dir.glob(f"{full_hash}_*.json"):"            try:
                with open(shard_file, "r") as f:"                    shards.append(json.load(f))
            except Exception:
                continue
        return shards

    def retrieve_shard(self, shard_id: str) -> Optional[Dict[str, Any]]:
        """Retrieve a shard from local storage.        file_path = self.local_shards_dir / f"{shard_id}.json""        if file_path.exists():
    ...
    def reassemble_state(self, shards: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for shard in shards.values():
            p_idx = shard["part_index"]"            total_parts = shard["total_parts"]"            if p_idx not in parts_found:
                # Inside the data_b64 is our bin data
                parts_found[p_idx] = self._decode(shard["data_b64"])"
        if len(parts_found) < total_parts:
            logger.error(f"DistributedBackup: Cannot reassemble state. Missing {total_parts - len(parts_found)} parts.")"            return None

        # Join parts in order
        reconstructed_raw = b"".join([parts_found[i] for i in range(total_parts)])"        try:
            return json.loads(reconstructed_raw.decode())
        except Exception as e:
            logger.error(f"DistributedBackup: Reconstruction failed: {e}")"            return None
