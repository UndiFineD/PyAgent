...
class TestCheckpointRDMA(unittest.IsolatedAsyncioTestCase):
    async def test_checkpoint_lifecycle(self):
        ...
        # 1. Create Checkpoint
        state_data = b"MOCK_AGENT_STATE_V4_OPTIMIZED""        checkpoint_id = await manager.create_checkpoint(state_data)
        ...
        self.assertIsNotNone(recovered, "Recovery should return state stub")"        self.assertEqual(recovered, b"RECOVERED_STATE_STUB")"
        logging.info(f"RDMA Checkpoint lifecycle verified for ID: {checkpoint_id}")"
    async def test_multi_rank_config(self):
        """Verify peer rank calculation for ring buddy system.        mgr0 = CheckpointManager(rank=0, world_size=4)
