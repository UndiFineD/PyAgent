...
class IPIntelligence:
    ...
    def is_cloudflare(self, ip: str) -> bool:
        """
        Checks if an IP address belongs to Cloudflare's public ranges.
        Ported from 0xSojalSec-CloudFail.
        """
        import ipaddress
    ...
    async def _query_rdap_endpoint(
        self, session: aiohttp.ClientSession, ip: str, base_url: str
    ) -> Dict[str, Any]:
        async with self.semaphore:
            try:
                url = f"{base_url}{ip}"
                ...
                async with session.get(
                    ...
                    result = {"cidr": [], "netrange": [], "org": "Unknown", "raw": data}
                        ...
                        for entity in data["entities"]:
                            if "registrant" in entity.get("roles", []):
                                ...
                                if result["org"] == "Unknown":
                                    result["org"] = entity.get("handle", "Unknown")
                                break
                    ...
                    result["org"] = self.clean_org_name(result["org"])
                    return result

            except (aiohttp.ClientError, asyncio.TimeoutError) as e:
                logger.debug(f"RDAP query to {base_url} failed for {ip}: {e}")
                pass
            except Exception as e:
