...
class DomainIntelligence:
    ...
    async def fetch_chaos_index(self) -> List[Dict[str, Any]]:
        """Fetch the full index of bug bounty programs from Chaos."""
        session = self._session or aiohttp.ClientSession()
        try:
            async with session.get(self.CHAOS_URL) as response:
                if response.status == 200:
                    return await response.json()
                logger.error(f"Failed to fetch Chaos index: {response.status}")
                return []
        except Exception as e:
            logger.error(f"Error fetching Chaos index: {e}")
            return []
        finally:
            if not self._session:
                await session.close()
    ...
    async def get_program_subdomains(self, program_url: str) -> List[str]:
        ...
        Safety: Uses python's zipfile instead of shell commands.
        """
        session = self._session or aiohttp.ClientSession()
        subdomains = []
        try:
