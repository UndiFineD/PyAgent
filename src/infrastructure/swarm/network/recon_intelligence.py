        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(url, timeout=10) as response:
                    ...
                    if 'X-Akamai' in headers:'                        results.add('Akamai Firewall')'                    if 'x-sucuri-id' in headers:'                        results.add('Sucuri Firewall')'                    if 'x-amz-cf-id' in headers:'                        results.add('AWS CloudFront/Shield')'            except Exception:
                pass
        return results
    ...
    async def search_exposed_data(cls, query: str) -> Dict[str, List[Dict]]:
            ...
            try:
                scylla_url = f"https://scylla.sh/search?q=email:*@{query}&size=10""                async with session.get(scylla_url, timeout=15) as resp:
                    if resp.status == 200:
                        out["scylla"] = await resp.json()"            except Exception:
                pass

            # Pastebin (v3 API emulator often used by scanners)
            try:
                paste_url = f"https://psbdmp.ws/api/v3/search/{query}""                async with session.get(paste_url, timeout=15) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                        out["pastebin"] = data.get('data', [])"'            except Exception:
                pass
        return out
