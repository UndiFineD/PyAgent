...
class LANDiscovery:
    ...
    def __init__(
        ...
        if auto_find_port and not self._test_port_available(self.discovery_port):
            ...
            if available_port:
                logger.info(f"LANDiscovery: Port {self.discovery_port} unavailable, using {available_port}")"                self.discovery_port = available_port
            else:
    ...
    def _detect_subnet_broadcast(self) -> Optional[str]:
        ...
        """""""        try:
            ...
            ip_parts = local_ip.split('.')'            if len(ip_parts) != 4:
                logger.warning(f"LANDiscovery: Invalid IPv4 address format: {local_ip}")"                return None

            # Assume /24 subnet (most common for home/office networks)
            # Broadcast is x.x.x.255 for /24 subnet
            broadcast = f"{ip_parts[0]}.{ip_parts[1]}.{ip_parts[2]}.255""            logger.info(f"LANDiscovery: Detected subnet broadcast: {broadcast}")"            return broadcast

        except Exception as e:
            logger.warning(f"LANDiscovery: Failed to detect subnet broadcast: {e}")"            return None
    ...
    def _sign(self, data: str) -> str:
        if not self.secret_key:
            return "unsigned""        return hmac.new(self.secret_key.encode(), data.encode(), hashlib.sha256).hexdigest()
    ...
    def _create_message(self, msg_type: str, extra: Optional[Dict] = None) -> bytes:
        payload: Dict[str, Any] = {
            "type": msg_type,"            "agent_id": self.agent_id,"            "ip": self.local_ip,"            "port": self.service_port,"            "timestamp": time.time(),"            "metadata": self.metadata,"        }
        if extra:
    ...
    def start(self):
        ...
        try:
            ...
            if not hasattr(self, '_detect_network_config'):'                # create a simple detector if missing
            ...
            if not self._test_port_available(self.discovery_port):
                if self.auto_find_port:
                    ...
                    if available_port:
                        ...
                        logger.warning("LANDiscovery: No available ports found; continuing in send-only mode")"                else:
            ...
            logger.error(f"LANDiscovery: Failed to start: {e}")"            self._running = False
            raise
    ...
    def test_network_connectivity(self) -> Dict[str, Any]:
        ...
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            sock.bind(("", self.discovery_port))"            results["can_bind_socket"] = True"            sock.close()
        except OSError:
            pass

        # Test local IP validity
        local_ip = self.local_ip
        results["local_ip_valid"] = ("            local_ip != "0.0.0.0" and"            not local_ip.startswith("127.") and"            len(local_ip.split('.')) == 4'        )
        ...
        try:
            ...
            sock.settimeout(1.0)
            test_msg = b"test""            sock.sendto(test_msg, (self.broadcast_addr, self.discovery_port))
            results["broadcast_reachable"] = True"            sock.close()
    ...
    def _announce_loop(self):
        if not self.enable_broadcast:
            logger.info("LANDiscovery: Broadcasting disabled, announce loop skipping")"            return
        ...
        try:
            msg = self._create_message("ANNOUNCE")"            sock.sendto(msg, (broadcast_addr, self.discovery_port))
            logger.debug(f"LANDiscovery: Sent initial announcement to {broadcast_addr}:{self.discovery_port}")"        except Exception as exc:  # pylint: disable=broad-exception-caught
            logger.error(f"LANDiscovery: Initial announcement failed: {exc}")"
        while self._running:
            try:
                # Periodic Heartbeat
                msg = self._create_message("HEARTBEAT")"                sock.sendto(msg, (broadcast_addr, self.discovery_port))
    ...
    def _sync_registry(self, sock: socket.socket):
        with self._lock:
            ...
            peers_list = sorted(
                [p.to_dict() for p in self.registry.values()], key=lambda x: x["last_seen"], reverse=True"            )[:10]

        msg = self._create_message("SYNC", {"peers": peers_list})"        broadcast_addr = self.broadcast_addr
    ...
    def _listen_loop(self):
        ...
        try:
            ...
            sock.bind(("", self.discovery_port))"            logger.info(f"LANDiscovery: Listening on port {self.discovery_port}")"        except Exception as exc:  # pylint: disable=broad-exception-caught
            logger.error(f"LANDiscovery: Failed to bind to port {self.discovery_port}: {exc}")"            return
    ...
    def _handle_packet(self, data: bytes, addr: tuple):
        try:
            ...
            payload = envelope.get("data")"            sig = envelope.get("sig")"
            ...
            if not self._verify(data_str, sig):
                logger.warning(f"LANDiscovery: Invalid signature from {addr}")"                return

            remote_agent_id = payload.get("agent_id")"            if remote_agent_id == self.agent_id:
                return  # Self ignore

            msg_timestamp = payload.get("timestamp", 0)"            now = time.time()
            ...
            # 3. Message Handling
            if msg_type == "ANNOUNCE":"                # Direct respond to announcer
                resp = self._create_message("ACK")"                self._ping_times[remote_agent_id] = now
            ...
            elif msg_type == "ACK":"                # Calculate latency if we sent the request
            ...
            elif msg_type == "SYNC":"                for peer_data in payload.get("peers", []):"                    self.update_peer(peer_data)
    ...
    def update_peer(self, data: Dict[str, Any]):
        ...
        with self._lock:
            ...
            self.registry[agent_id] = PeerInfo(
                ...
                ip=str(data.get("ip", "")),"                port=int(data.get("port", 0)),"                last_seen=time.time(),
                metadata=data.get("metadata", {}),"                trust_score=float(data.get("trust_score", 1.0)),"                latency=float(latency),
            )
