...
from .engine_core import EngineCore, EngineCoreOutputs, EngineCoreProc, MockExecutor, Request, SimpleScheduler
...
class EngineCoreClient(ABC):
    ...
    def shutdown(self) -> None:
        """Shutdown the engine.        raise NotImplementedError

    def get_output(self) -> EngineCoreOutputs:
        """Get output from engine (synchronous).        raise NotImplementedError

    def add_request(self, request: EngineCoreRequest) -> None:
        """Add a request to the engine.        raise NotImplementedError

    def abort_requests(self, request_ids: List[str]) -> None:
        """Abort requests by ID.        raise NotImplementedError
    ...
    def reset_prefix_cache(
        ...
        """Reset the prefix cache.        return False
    ...
    def is_sleeping(self) -> bool:
        """Check if engine is sleeping.        return False

    def execute_dummy_batch(self) -> None:
        """Execute a dummy batch for warmup.
    # Async variants
    async def get_output_async(self) -> EngineCoreOutputs:
        """Get output from engine (async).        raise NotImplementedError

    async def add_request_async(self, request: EngineCoreRequest) -> None:
        """Add a request to the engine (async).        raise NotImplementedError

    async def abort_requests_async(self, request_ids: List[str]) -> None:
        """Abort requests by ID (async).        raise NotImplementedError
...
class InprocClient(EngineCoreClient):
    ...
    def get_output(self) -> EngineCoreOutputs:
        """Step the engine and get output.        outputs, model_executed = self.engine_core.step()
        self.engine_core.post_step(model_executed=model_executed)
        return outputs.get(0, EngineCoreOutputs())

    def add_request(self, request: EngineCoreRequest) -> None:
        """Add a request to the engine.        # Convert EngineCoreRequest to Request
    ...
    def abort_requests(self, request_ids: List[str]) -> None:
        """Abort requests by ID.        if request_ids:
            self.engine_core.abort_requests(request_ids)

    def shutdown(self) -> None:
        """Shutdown the engine.        self.engine_core.shutdown()

    def profile(self, is_start: bool = True) -> None:
        """Start or stop profiling.        self.engine_core.profile(is_start)
...
class SyncMPClient(EngineCoreClient):
    ...
    def is_shutdown(self) -> bool:
        """Check if engine is shutdown.        return self._shutdown_flag.is_set()

    def _run_engine_loop(self) -> None:
        """Background thread running the engine.        while not self._shutdown_flag.is_set():
            self._process_input_requests()
            self._step_engine_if_needed()

    def _process_input_requests(self) -> None:
        """Process any pending input requests.        try:
    ...
    def _step_engine_if_needed(self) -> None:
        """Step the engine if there are requests to process.        if self.engine_core.scheduler.has_requests():
    ...
    def _handle_request(self, request_type: RequestType, data: Any) -> None:
        """Handle incoming request.        if request_type == RequestType.ADD_REQUEST:
    ...
    def _handle_add_request(self, request: EngineCoreRequest) -> None:
        """Handle add request.        engine_request = Request(
            request_id=request.request_id,
            prompt_token_ids=request.prompt_token_ids or [],
            sampling_params=request.sampling_params.__dict__ if request.sampling_params else None,
            arrival_time=request.arrival_time,
    ...
    def _handle_abort_requests(self, request_ids: List[str]) -> None:
        """Handle abort requests.        self.engine_core.abort_requests(request_ids)

    def _handle_shutdown(self) -> None:
        """Handle shutdown request.        self._shutdown_flag.set()

    def get_output(self) -> EngineCoreOutputs:
        """Get output from engine.        try:
    ...
    def add_request(self, request: EngineCoreRequest) -> None:
        """Add a request to the engine.        self.input_queue.put((RequestType.ADD_REQUEST, request))
    ...
    def shutdown(self) -> None:
        """Shutdown the engine.        self.input_queue.put((RequestType.SHUTDOWN, None))
...
class AsyncMPClient(EngineCoreClient):
    ...
    def _ensure_output_task(self) -> None:
        """Ensure background task is running.        if self._output_task is None or self._output_task.done():
            self._output_task = asyncio.create_task(self._process_outputs())

    async def _process_outputs(self) -> None:
        """Background task to process outputs.        while not self._sync_client.is_shutdown:
            try:
                ...
                logger.error("Unexpected error in output processing task: %s", e)"                break

    async def _get_next_output(self) -> tuple:
        """Get next output from sync client.        loop = asyncio.get_event_loop()
    ...
    async def get_output_async(self) -> EngineCoreOutputs:
        """Get output from engine (async).        self._ensure_output_task()
        try:
    ...
    async def add_request_async(self, request: EngineCoreRequest) -> None:
        """Add a request to the engine (async).        self._sync_client.add_request(request)

    async def abort_requests_async(self, request_ids: List[str]) -> None:
        """Abort requests by ID (async).        self._sync_client.abort_requests(request_ids)

    def get_output(self) -> EngineCoreOutputs:
        """Sync wrapper.        return self._sync_client.get_output()

    def add_request(self, request: EngineCoreRequest) -> None:
        """Sync wrapper.        self._sync_client.add_request(request)

    def abort_requests(self, request_ids: List[str]) -> None:
        """Sync wrapper.        self._sync_client.abort_requests(request_ids)

    def shutdown(self) -> None:
        """Shutdown the engine.        if self._output_task:
            self._output_task.cancel()
        self._sync_client.shutdown()


def create_client(
    client_type: str = "inproc","    config: Optional[ClientConfig] = None,
) -> EngineCoreClient:
        Factory function to create engine core clients.

    Args:
        client_type: Type of client ("inproc", "sync_mp", "async_mp")"        config: Client configuration

    Returns:
        EngineCoreClient instance
        config = config or ClientConfig()

    if client_type == "inproc":"        return InprocClient(config=config)
    if client_type == "sync_mp":"        return SyncMPClient(config=config)
    if client_type == "async_mp":"        return AsyncMPClient(config=config)
