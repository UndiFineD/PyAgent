...
class PagedAttentionOps:
    ...
    def paged_attention_v1(
        ...
        """Basic paged attention implementation regarding sequence batches."""
    ...
    def paged_attention_v2(
        ...
        """Paged attention with partition-based reduction regarding sequences."""
        ...
        def _process_seq(sid: int) -> None:
            ...
            if sl > 0:
                ...
                def _partition_step(pi: int, exps: np.ndarray, maxs: np.ndarray, p_out: np.ndarray) -> np.ndarray:
                    ...
                    sc = np.einsum("hd,phd->hp", q, ks[s:e]) * config.scale
                    pm = np.max(sc, axis=1)
                    ...
                    ex += np.sum(es, axis=1)
                    po += np.einsum("hp,phd->hd", es, vs[s:e])
                    return _partition_step(pi + 1, ex, nm, po)

                out_buf[sid] = _partition_step(0, np.zeros(nh), np.full(nh, float("-inf")), np.zeros((nh, hs)))
            _process_seq(sid + 1)
