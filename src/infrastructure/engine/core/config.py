...
class Request:  # pylint: disable=too-many-instance-attributes
    ...
    def get_finished_reason(self) -> Optional[FinishReason]:
        """Get the reason why this request finished.        return self.finish_reason

    def is_finished(self) -> bool:
        """Check if request is finished.        return self.status in (RequestStatus.FINISHED, RequestStatus.ABORTED)
...
class SchedulerOutput:
    ...
    def is_empty(self) -> bool:
        """Check if no requests were scheduled.        return self.total_num_scheduled_tokens == 0
...
class ModelRunnerOutput:
    ...
    @classmethod
    def empty(cls) -> "ModelRunnerOutput":"        """Create an empty output.        return cls()
