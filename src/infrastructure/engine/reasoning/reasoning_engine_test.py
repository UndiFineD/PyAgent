...
"""
Test ReasoningFormat enum has expected values.        assert ReasoningFormat.DEEPSEEK_R1 is not None
...
"""
Test ToolCallFormat enum has expected values.        assert ToolCallFormat.OPENAI is not None
        assert ToolCallFormat.HERMES is not None
        assert ToolCallFormat.ANTHROPIC is not None

    def test_parse_state_values(self):
"""
Test ParseState enum has expected values.        assert ParseState.IDLE is not None
...
"""
Test creating ThinkingBlock.        block = ThinkingBlock(
            content="Let me think about this...","            start_position=10,
            end_position=50,
        )
        assert block.content == "Let me think about this...""        assert block.start_position == 10
        assert block.end_position == 50

    def test_thinking_block_defaults(self):
"""
Test ThinkingBlock default values.        block = ThinkingBlock(content="test", start_position=0, end_position=4)"        assert block.start_position == 0
...
"""
Test creating ToolCall.        call = ToolCall(
            id="call_123","            name="search","            arguments={"query": "test"},"        )
        assert call.id == "call_123""        assert call.name == "search""        assert call.arguments == {"query": "test"}
    def test_tool_call_with_format(self):
"""
Test ToolCall with format.        call = ToolCall(
...
"""
Test parsing DeepSeek thinking block.        parser = DeepSeekReasoningParser()
...
"""
Test parsing multiple thinking blocks.        parser = DeepSeekReasoningParser()
        text = "<think>First thought</think> middle <think>Second thought</think> end"
        content, blocks = parser.extract_thinking(text)
        assert len(blocks) == 2
        assert "First thought" in blocks[0].content"        assert "Second thought" in blocks[1].content"
    def test_extract_content_without_thinking(self):
"""
Test extracting content without thinking blocks.        parser = DeepSeekReasoningParser()
        text = "<think>Thinking...</think>The answer is 42"
        content, blocks = parser.extract_thinking(text)
        assert "42" in content
    def test_parser_reset(self):
"""
Test parser reset.        parser = DeepSeekReasoningParser()
...
Test Qwen3 reasoning parser.
    def test_parse_thinking_block(self):
"""
Test parsing Qwen thinking block.        parser = QwenReasoningParser()
...
"""
Test extracting content.        parser = QwenReasoningParser()
...
"""
Test custom delimiter configuration.        parser = GenericReasoningParser(
...
"""
Test parsing OpenAI format tool call.        parser = OpenAIToolParser()
        text = ''
...
"""
Test parsing function call.        parser = OpenAIToolParser()
        text = ''
...
"""
Test parsing Hermes XML format.        parser = HermesToolParser()
        text = '''<tool_call>'''
{"name": "search", "arguments": {"query": "test"}}"</tool_call>''''''
calls = parser.parse_tool_calls(text)
        assert isinstance(calls, list)
...
"""
Test creating reasoning engine.        engine = ReasoningEngine()
        assert engine is not None

    def test_create_with_format(self):
"""
Test creating engine with specific format.        engine = ReasoningEngine(
...
"""
Test parsing full response.        engine = ReasoningEngine(reasoning_format=ReasoningFormat.DEEPSEEK_R1)
        text = "<think>Let me think</think>The answer is 42"
        result = engine.parse(text)
        assert result is not None
        assert hasattr(result, 'thinking_blocks')'        assert len(result.thinking_blocks) == 1

    def test_parse_content(self):
"""
Test parsing extracts content correctly.        engine = ReasoningEngine(reasoning_format=ReasoningFormat.DEEPSEEK_R1)
        text = "<think>Thinking...</think>Final answer"
        result = engine.parse(text)
        assert "Final answer" in result.content
    def test_extract_thinking(self):
"""
Test extracting thinking content.        engine = ReasoningEngine(reasoning_format=ReasoningFormat.DEEPSEEK_R1)
...
"""
Test extracting tool calls.        engine = ReasoningEngine(
...
"""
Test creating DeepSeek engine.        engine = create_reasoning_engine(model_name="deepseek-r1")"        assert engine is not None

    def test_create_qwen_engine(self):
"""
Test creating Qwen engine.        engine = create_reasoning_engine(model_name="qwen-coder")"        assert engine is not None

    def test_create_with_tool_format(self):
"""
Test creating engine with tool format.        engine = create_reasoning_engine(
            model_name="generic","            tool_format=ToolCallFormat.HERMES,
        )
        assert engine is not None
