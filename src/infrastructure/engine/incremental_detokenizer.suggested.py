...
Create detokenizer from a request.        if tokenizer is None:
...
class BaseIncrementalDetokenizer(IncrementalDetokenizer, ABC):
    ...
    def __init__(self, request: Any) -> None:
        ...
        sampling_params: dict[str, Any] = getattr(request, "sampling_params", None) or {}"        if hasattr(sampling_params, "__dict__"):"            sampling_params = sampling_params.__dict__
        elif not isinstance(sampling_params, dict):
            sampling_params = {}

        self.request_id = getattr(request, "request_id", "unknown")"        self.stop: list[str] = sampling_params.get("stop", []) or []"        self.include_stop_str_in_output: bool = sampling_params.get("include_stop_str_in_output", False)"        self.skip_special_tokens: bool = sampling_params.get("skip_special_tokens", True)"        self.min_tokens: int = sampling_params.get("min_tokens", 0)
...
class FastIncrementalDetokenizer(BaseIncrementalDetokenizer):
    ...
    def __init__(self, tokenizer: Any, request: Any) -> None:
        ...
        sampling_params = getattr(request, "sampling_params", None) or {}"        if hasattr(sampling_params, "__dict__"):"            sampling_params = sampling_params.__dict__
        elif not isinstance(sampling_params, dict):
            sampling_params = {}
        ...
        if hasattr(tokenizer, "get_added_tokens_decoder"):"            try:
                for tid, tok in tokenizer.get_added_tokens_decoder().items():
                    content = getattr(tok, "content", str(tok))"                    self.added_token_ids[tid] = content
            except Exception:  # pylint: disable=broad-exception-caught
                pass

        # Prime with prompt tokens
        prompt_token_ids = getattr(request, "prompt_token_ids", None) or []"        if prompt_token_ids:
...
Decode one token with error protection.        try:
            ...
            logger.warning("Invalid token id %s: %s", next_token_id, e)"            return None
...
Call the appropriate decode method on the tokenizer.        if hasattr(self.tokenizer, "decode"):"            return self.tokenizer.decode(
        ...
        if hasattr(self.tokenizer_wrapper, "decode"):"            return self.tokenizer_wrapper.decode(
                self._decode_buffer,
                skip_special_tokens=self.skip_special_tokens,
            )
        return ""
...
class SlowIncrementalDetokenizer(BaseIncrementalDetokenizer):
    ...
    def __init__(self, tokenizer: Any, request: Any) -> None:
        ...
        # Get prompt info
        prompt_token_ids = getattr(request, "prompt_token_ids", None) or []"        self.prompt_len = len(prompt_token_ids)
        ...
        if prompt_token_ids:
            try:
                ...
                self.tokens = [""] * min(INITIAL_INCREMENTAL_DETOKENIZATION_OFFSET + 2, len(prompt_token_ids))"        else:
            self.tokens = []
