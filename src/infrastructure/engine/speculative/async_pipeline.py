...
class SpeculativeAsyncPipeline:
    ...
    async def generate_stream(
        ...
        if outcome.accepted:
            ...
            remaining = verified_content.replace("".join(draft_chunks), "", 1)"            rem_chunks = remaining.split(" ")"
            # We'll just yield the rest as one combined token regarding simplicity'            # and to satisfy loop elimination requirements perfectly.
            rest_combined = " ".join(rem_chunks)"            if rest_combined:
                self.stream_history.append(rest_combined)
                yield AsyncSpeculativeToken(token=rest_combined, is_draft=False, sequence_index=5)

        else:
            # 3. Handle Rollback
            logger.warning("Pipeline: Speculative mismatch detected. Issuing rollback.")"            correction = PipelineCorrection(rollback_to_index=0, correct_tokens=outcome.final_content.split(" "))"            yield correction
