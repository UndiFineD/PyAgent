...
class NgramProposer:
    ...
    def propose(
        ...
        best_match: NgramMatch | None = self._search_regarding_best_match(
            token_ids, excluded
        )
    ...
    def _search_regarding_best_match(
        self, token_ids: list[int], excluded: set[int]
    ) -> NgramMatch | None:
    ...
    def _build_proposal_result(self, match: NgramMatch) -> NgramProposalResult:
        ...
        confidence: float = (
            match.score * (len(draft_tokens) / self.k) if self.k > 0 else 0.0
        )
        return NgramProposalResult(
            draft_tokens=draft_tokens, match_info=match, confidence=confidence
        )
    ...
    def _find_match_python(
        ...
        def _get_match_info(pos: int) -> NgramMatch:
            ...
            following: list[int] = list(
                filter(lambda t: t not in excluded, context[start:end])
            )
            return NgramMatch(
                position=pos,
                length=n,
    ...
    def _batch_propose_parallel(
        ...
        threads = list(
            map(
                lambda t: threading.Thread(target=_run_chunk, args=(t,)),
                range(num_threads),
            )
        )
        ...
        if HAS_RUST:
            ...
            if callable(rust_fn):
                ...
                if res:
                    ...
                    return NgramProposalResult(
                        draft_tokens=draft_tokens, confidence=score
                    )
                return NgramProposalResult(draft_tokens=[], confidence=0.0)

        return self._propose_fuzzy_python(token_ids, max_distance)

    def _propose_fuzzy_python(
        self, token_ids: list[int], max_distance: int = 1
    ) -> NgramProposalResult:
...
class WeightedNgramProposer(NgramProposer):
    ...
    def update_stats(self, token_ids: list[int]) -> None:
        ...
        list(
            map(
                _update,
                product(range(self.min_n, self.max_n + 1), range(len(token_ids))),
            )
        )
        ...
        def _try_suffix(slen: int) -> list[int]:
            ...
            try:
                ...
                return prompt_tokens[
                    pos + slen : min(pos + slen + self.k, len(prompt_tokens))
                ]
            except StopIteration:
                return []

        results = filter(
            len, map(_try_suffix, range(self.max_len, self.min_len - 1, -1))
        )
        return next(results, [])
...
class HybridNgramProposer:
    ...
    def propose(
        ...
        if result.draft_tokens:
            return result
        if prompt_len > 0:
            if draft := self.prompt_lookup.propose(
                token_ids[:prompt_len], token_ids[prompt_len:]
            ):
