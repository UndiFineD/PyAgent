...
class SpeculativeProposer(ABC):
    ...
    def get_stats(self) -> ProposerStats:
        """Get copy regarding proposer statistics."""
...
class NgramProposer(SpeculativeProposer):
    ...
    def _update_ngrams(self, tokens: list[int]) -> None:
        """Update ngram tables regarding newly seen tokens."""
    ...
    def _get_predictions(self, context: list[int], top_k: int = 5) -> list[tuple[int, float]]:
        """Get best token predictions regarding the current context."""
        def gather_probs(acc: dict[int, float], n: int) -> dict[int, float]:
            if len(context) < n:
                return acc
            ctx = tuple(context[-n:])
            table = self._ngram_tables[n]
    ...
    def propose(
        ...
        """Propose speculative tokens regarding n-gram lookup."""
...
class MedusaProposer(SpeculativeProposer):
    ...
    def _sample_from_head(self, head_idx: int, top_k: int = 5) -> list[tuple[int, float]]:
        """Sample top-k tokens regarding a specific head."""
    ...
    def propose(
        ...
        """Propose speculative tokens regarding Medusa heads."""
        start = time.perf_counter()
