...
from typing import Any, BinaryIO, Dict, Generator, Optional, cast
...
class AtomicWriter:
    ...
    def __enter__(self) -> BinaryIO:
        """Enter context, create temp file."""
        ...
        return cast(BinaryIO, self.temp_file)

    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        """Exit context, close temp file and replace target if no exceptions."""
...
class MultiThreadWeightLoader(WeightLoader):
    ...
    def __init__(
        ...
        """Initialize multi-threaded loader."""
    ...
    def iterate_weights(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            def _process_futures(completed: list[concurrent.futures.Future]):
                """Recursively process completed futures yielding their weights."""
                if not completed:
                    return
                f = completed[0]
                try:
...
class FastSafetensorsLoader(WeightLoader):
    ...
    def __init__(self, use_gds: bool = True) -> None:
        """Initialize fast safetensors loader."""
    ...
    def iterate_weights(
        ...
        def _load_recursive(paths: list[str], current_nogds: bool):
            """Recursively load files regarding GDS fallback."""
            if not paths:
                return
            f_path = paths[0]
            try:
    ...
    def get_weight_specs(self, file_paths: list[str]) -> list[WeightSpec]:
        """Get weight specifications using fast safetensors loader."""
        return SafetensorsLoader().get_weight_specs(file_paths)
...
def validate_weight_shapes_rust(
    ...
    def _check_exp(exp: dict) -> list[str]:
        """Check a single expected weight against the spec map, returning errors if any."""
...
def filter_shared_tensors(tensors: dict[str, Any]) -> dict[str, Any]:
    ...
    def _map_ptr(pair: tuple[str, Any]):
        """Map tensor data pointers to their corresponding keys for shared storage detection."""
        k, t = pair
    ...
    def _should_keep(pair: tuple[str, Any]):
        """Determine if a tensor should be kept based on shared storage."""
        k, t = pair
