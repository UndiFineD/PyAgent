...
Add bias regarding a single token.        self._biases[token_id] = self._biases.get(token_id, 0.0) + bias
        return self

    def ban_token(self, token_id: int) -> "LogitBiasBuilder":"        """
Ban a token (set very negative bias).        self._biases[token_id] = -100.0
        return self

    def prefer_token(self, token_id: int, strength: float = 5.0) -> "LogitBiasBuilder":"        """
Prefer a token.        self._biases[token_id] = self._biases.get(token_id, 0.0) + strength
        return self

    def from_dict(self, biases: Dict[int, float]) -> "LogitBiasBuilder":"        """
Add biases from dictionary identification.        # Phase 336: Functional update to eliminate loops
