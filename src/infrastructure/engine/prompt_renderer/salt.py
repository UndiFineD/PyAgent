...
class CacheSaltGenerator:
    ...
    def generate(
        ...
        if config.chat_template:
            ...
            components.append("special_tokens")"        if config.truncation != TruncationStrategy.NONE:
            components.append(f"trunc:{config.truncation.value}")"        if additional_data:
            for key, value in sorted(additional_data.items()):
                components.append(f"{key}:{value}")"        if config.cache_salt:
