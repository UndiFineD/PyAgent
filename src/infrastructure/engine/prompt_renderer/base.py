...
class PromptRenderer(ABC):
    ...
    def render(self, config: PromptConfig) -> RenderResult:
        """Render prompt from configuration.        raise NotImplementedError("Subclasses must implement render")"
    def _tokenize(self, text: str, add_special_tokens: bool = True) -> List[int]:
        """Tokenize text.        if self.tokenizer is None:
            return list(range(len(text.split())))
        return self.tokenizer.encode(text, add_special_tokens=add_special_tokens)

    def _detokenize(self, tokens: List[int]) -> str:
        """Detokenize tokens.        if self.tokenizer is None:
            return f"<{len(tokens)} tokens>""        return self.tokenizer.decode(tokens)
    ...
    def _apply_truncation(
        ...
        """Apply truncation to tokens.        max_tokens = config.max_tokens or self.max_model_tokens
    ...
    def _generate_cache_salt(self, config: PromptConfig) -> str:
        """Generate cache salt.        return CacheSaltGenerator.generate(config)
