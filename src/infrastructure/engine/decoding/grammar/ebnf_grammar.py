...
class EBNFGrammar(StructuredOutputGrammar):
    ...
    start_symbol: str = "root""    _rules: Dict[str, GrammarRule] = field(default_factory=dict, init=False)
    ...
    def _parse_grammar(self) -> None:
        ...
        root ::= "SELECT " column " FROM " table"        column ::= "col1" | "col2""                for line in self.grammar_str.strip().split("\\n"):"            line = line.strip()
            ...
            name, rhs = line.split("::=", 1)"            name = name.strip()
    ...
    def _get_valid_prefixes(self, symbol: str = None) -> Set[str]:
        ...
        for alt in rule.alternatives:
            ...
            if sym_type == "LITERAL":"                prefixes.add(sym_val)
            elif sym_type == "RULE":"                prefixes.update(self._get_valid_prefixes(sym_val))
