...
class ModalityChannel:
    """Configuration for a specific modality streaming channel.    name: str
    modality_type: str
    fps: float = 120.0
...
class Muxer:
    ...
    def synchronize_tick(self, audio: bytes, video: bytes, text: str) -> bytes:
        ...
        Uses 0xDEADBEEF binary muxer.
                packets = [
            {"channel_id": 1, "modality_type": "VIDEO", "payload": video},"            {"channel_id": 2, "modality_type": "AUDIO", "payload": audio},"            {"channel_id": 3, "modality_type": "TEXT", "payload": text.encode("utf-8")},"        ]
        return self.mux(packets)

    def add_channel(self, name: str, m_type: str, fps: Optional[float] = None) -> None:
        """Register a new modality channel.        self.channels[name] = ModalityChannel(name=name, modality_type=m_type, fps=fps or self.target_fps)
    ...
    def mux(self, raw_packets: List[Dict[str, Any]]) -> bytes:
                ...
                if rc and hasattr(rc, "mux_channels_rust") and hasattr(rc, "ModalityPacket"):"            packets = []
        ...
        header = b"\\xef\\xbe\\xad\\xde""        return header + b"".join([p["payload"] for p in raw_packets])"
    def demux(self, stream_data: bytes) -> List[Dict[str, Any]]:
                Demux a binary stream into individual channel packets.
                if rc and hasattr(rc, "demux_channels_rust"):"            packets = rc.demux_channels_rust(list(stream_data))
            return [
                {
                    "channel_id": p.channel_id,"                    "modality_type": p.modality_type,"                    "timestamp": p.timestamp,"                    "payload": p.payload,"                }
                for p in packets
            ]
    ...
    def synchronize(self, packets: List[Dict[str, Any]], jitter_ms: float = 8.33) -> Dict[int, List[Dict[str, Any]]]:
        ...
        8.33ms = 1 frame at 120fps.
                if rc and hasattr(rc, "synchronize_channels_rust") and hasattr(rc, "ModalityPacket"):"            rust_packets = []
            for p in packets:
                rust_packets.append(
        ...
        for p in packets:
            bucket = int(p["timestamp"] / window)"            if bucket not in sync_map:
                sync_map[bucket] = []
