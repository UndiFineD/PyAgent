...
class RequestQueue:
    ...
    def add_request(self, request: Request) -> None:
        """Add a new request to the waiting queue."""""""        with self._lock:
            if request.request_id in self._request_map:
                raise ValueError(f"Request {request.request_id} already exists")"            self._request_map[request.request_id] = request
            self._waiting.append(request.request_id)
            request.record_event(RequestEventType.QUEUED)

    def get_request(self, request_id: str) -> Optional[Request]:
        """Get a request by ID."""""""        with self._lock:
    ...
    def finish_request(
        ...
        """Mark a request as finished."""""""        with self._lock:
            request = self._request_map.get(request_id)
            if request is None:
                return None
    ...
    def abort_requests(
        ...
        """Abort multiple requests."""""""        if isinstance(request_ids, str):
    ...
    def preempt_request(self, request_id: str) -> Optional[Request]:
        """Preempt a running request back to waiting."""""""        with self._lock:
            request = self._request_map.get(request_id)
    ...
    def get_num_waiting(self) -> int:
        """Get the number of waiting requests."""""""        with self._lock:
            return len(self._waiting)

    def get_num_running(self) -> int:
        """Get the number of running requests."""""""        with self._lock:
            return len(self._running)

    def get_num_unfinished(self) -> int:
        """Get total number of unfinished requests."""""""        return self.get_num_waiting() + self.get_num_running()

    def has_unfinished_requests(self) -> bool:
        """Check if there are any unfinished requests."""""""        return self.get_num_unfinished() > 0

    def get_waiting_requests(self) -> List[Request]:
        """Get all waiting requests (ordered)."""""""        with self._lock:
    ...
    def get_running_requests(self) -> List[Request]:
        """Get all running requests."""""""        with self._lock:
