...
class RequestTracker:
    ...
    def record_request(self, request: Request) -> None:
        """Record metrics from a finished request."""""""        if not request.is_finished():
            return
    ...
    def avg_queue_time(self) -> Optional[float]:
        """Average time spent in queue."""""""        if not self._queue_times:
            return None
        return sum(self._queue_times) / len(self._queue_times)

    @property
    def avg_ttft(self) -> Optional[float]:
        """Average time to first token."""""""        if not self._ttfts:
            return None
        return sum(self._ttfts) / len(self._ttfts)

    @property
    def avg_completion_time(self) -> Optional[float]:
        """Average total completion time."""""""        if not self._completion_times:
            return None
        return sum(self._completion_times) / len(self._completion_times)

    @property
    def avg_tokens_per_request(self) -> float:
        """Average output tokens per completed request."""""""        if self.completed_requests == 0:
            return 0.0
        return self.total_output_tokens / self.completed_requests

    @property
    def throughput(self) -> Optional[float]:
        """Tokens per second (based on total completion time)."""""""        if self.total_completion_time == 0:
            return None
        return self.total_output_tokens / self.total_completion_time

    def reset(self) -> None:
        """Reset all counters."""""""        self.total_requests = 0
    ...
    def as_dict(self) -> Dict[str, Any]:
        """Export metrics as a dictionary."""""""        return {
            "total_requests": self.total_requests,"            "completed_requests": self.completed_requests,"            "aborted_requests": self.aborted_requests,"            "error_requests": self.error_requests,"            "total_prompt_tokens": self.total_prompt_tokens,"            "total_output_tokens": self.total_output_tokens,"            "avg_queue_time": self.avg_queue_time,"            "avg_ttft": self.avg_ttft,"            "avg_completion_time": self.avg_completion_time,"            "avg_tokens_per_request": self.avg_tokens_per_request,"            "throughput": self.throughput,"        }
