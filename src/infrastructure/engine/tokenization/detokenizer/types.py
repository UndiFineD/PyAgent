...
class MultimodalTokenizer:
    ...
    def encode_image_patches(self, patches: List[List[float]]) -> List[int]:
        ...
        for idx, patch in enumerate(patches):
            ...
            patch_token_id = self.IMAGE_TOKEN_START + (
                idx % (self.IMAGE_TOKEN_END - self.IMAGE_TOKEN_START)
            )
    ...
    def encode_audio(self, audio_frames: List[List[float]]) -> List[int]:
        ...
        if self.audio_tokenizer:
            ...
            return [
                ...
                + (tid % (self.AUDIO_TOKEN_END - self.AUDIO_TOKEN_START))
                for tid in base_ids
            ]
        ...
        for idx, frame in enumerate(audio_frames):
            frame_token_id = self.AUDIO_TOKEN_START + (
                idx % (self.AUDIO_TOKEN_END - self.AUDIO_TOKEN_START)
            )
