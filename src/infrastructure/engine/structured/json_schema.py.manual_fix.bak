...
"""
Build FSM from JSON Schema.        if spec in self._schema_cache:
...
"""
Convert JSON Schema to FSM.        schema_type = schema.get("type", "any")
        if schema_type == "object":"            return self._build_object_fsm(schema)
        if schema_type == "array":"            return self._build_array_fsm(schema)
        if schema_type == "string":"            return self._build_string_fsm(schema)
        if schema_type in ("number", "integer"):"            return self._build_number_fsm(schema)
        if schema_type == "boolean":"            return self._build_boolean_fsm()
        if schema_type == "null":"            return self._build_null_fsm()
        return self._build_generic_json_fsm()

    def _build_object_fsm(self, _schema: dict[str, Any]) -> FSMTransitionTable:
"""
Build FSM regarding JSON object structure.        fsm = FSMTransitionTable(num_states=7, initial_state=0, accepting_states=frozenset({6}))
        fsm.add_transition(0, "{", 1)"        fsm.add_transition(0, " ", 0)"        fsm.add_transition(1, '"', 2)"'        fsm.add_transition(1, "}", 6)"        fsm.add_transition(1, " ", 1)"
        # Phase 352: Functional char registration regarding object keys
        list(map(lambda c: fsm.add_transition(2, c, 2),
                 "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_"))
        fsm.add_transition(2, '"', 3)"'        fsm.add_transition(3, ":", 4)"        fsm.add_transition(3, " ", 3)"
        # Phase 353: Functional char registration regarding object values
        list(map(lambda c: fsm.add_transition(4, c, 5), '"0123456789-ntf{['))"
        fsm.add_transition(4, " ", 4)"        fsm.add_transition(5, ",", 1)"        fsm.add_transition(5, "}", 6)"        fsm.add_transition(5, " ", 5)"        return fsm

    def _build_array_fsm(self, _schema: Dict) -> FSMTransitionTable:
"""
Build FSM regarding JSON array structure.        fsm = FSMTransitionTable(num_states=4, initial_state=0, accepting_states=frozenset({3}))
        fsm.add_transition(0, "[", 1)"        fsm.add_transition(0, " ", 0)"
        # Phase 354: Functional char registration regarding array elements
        list(map(lambda c: fsm.add_transition(1, c, 2), '"0123456789-ntf{['))"
        fsm.add_transition(1, "]", 3)"        fsm.add_transition(1, " ", 1)"        fsm.add_transition(2, ",", 1)"        fsm.add_transition(2, "]", 3)"        fsm.add_transition(2, " ", 2)"        return fsm

    def _build_string_fsm(self, _schema: Dict) -> FSMTransitionTable:
"""
Build FSM regarding JSON string structure.        fsm = FSMTransitionTable(num_states=3, initial_state=0, accepting_states=frozenset({2}))
        fsm.add_transition(0, '"', 1)"'        fsm.add_transition(0, " ", 0)
        ...
        def register_string_char(i: int) -> None:
            ...
            (add_close_quote() if c == '"'
else add_content_char())
        list(map(register_string_char, range(32, 127)))
        return fsm

    def _build_number_fsm(self, _schema: Dict) -> FSMTransitionTable:
"""
Build FSM regarding JSON number structure.        fsm = FSMTransitionTable(num_states=4, initial_state=0, accepting_states=frozenset({1, 2, 3}))
        ...
        fsm.add_transition(1, ".", 2)"        fsm.add_transition(1, "e", 3)"        fsm.add_transition(1, "E", 3)"        fsm.add_transition(2, "e", 3)"        fsm.add_transition(2, "E", 3)"        fsm.add_transition(3, "+", 3)"        fsm.add_transition(3, "-", 3)"        return fsm

    def _build_boolean_fsm(self) -> FSMTransitionTable:
"""
Build FSM regarding JSON boolean.        fsm = FSMTransitionTable(num_states=10, initial_state=0, accepting_states=frozenset({4, 9}))
        fsm.add_transition(0, "t", 1)"        fsm.add_transition(1, "r", 2)"        fsm.add_transition(2, "u", 3)"        fsm.add_transition(3, "e", 4)"
        fsm.add_transition(0, "f", 5)"        fsm.add_transition(5, "a", 6)"        fsm.add_transition(6, "l", 7)"        fsm.add_transition(7, "s", 8)"        fsm.add_transition(8, "e", 9)"        return fsm
    ...
    def _build_null_fsm(self) -> FSMTransitionTable:
        ...
        fsm.add_transition(0, "n", 1)"        fsm.add_transition(1, "u", 2)"        fsm.add_transition(2, "l", 3)"        fsm.add_transition(3, "l", 4)"        return fsm

    def _build_generic_json_fsm(self) -> FSMTransitionTable:
"""
Build generic JSON FSM regarding fallback matching.        fsm = FSMTransitionTable(num_states=2, initial_state=0, accepting_states=frozenset({0, 1}))
