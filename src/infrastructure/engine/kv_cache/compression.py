...
>>>>>>> 125558c4f (feat: implement Swarm Evolution Meta-Learning Phase 81-85)
                ...
                if vram_pressure > 0.9 and current_bits > 2:
                    ...
                    stats["scaled_down"] += 1
                # High pressure: Force INT4
                elif vram_pressure > 0.7 and current_bits > 4:
                    shard.precision = "int4"
                    stats["scaled_down"] += 1
                # Moderate pressure: Force FP8
...
>>>>>>> 125558c4f (feat: implement Swarm Evolution Meta-Learning Phase 81-85)
                if idle_time > self.idle_threshold_sec:
                    ...
                    if shard.is_cached:
                        logger.info(f"Compressor: Evicting cold shard {shard.shard_id} (idle {idle_time:.1f}s)")
                        shard.is_cached = False
                        stats["evicted"] += 1
                elif idle_time > 10.0:
                    # Idle: Compress to FP8
                    if shard.precision != "fp8":
                        logger.debug(f"Compressor: Compressing shard {shard.shard_id} to FP8")
                        shard.precision = "fp8"
                        stats["compressed"] += 1
                else:
                    # Active
