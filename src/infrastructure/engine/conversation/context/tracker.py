...
class TurnTracker:
    ...
    def turns(self) -> List[ConversationTurn]:
        """Return the list of conversation turns.        return self._turns

    @property
    def turn_count(self) -> int:
        """Return the total number of turns.        return len(self._turns)

    @property
    def total_tokens(self) -> TokenMetrics:
        """Return the total token metrics for all turns.        return self._total_tokens
    ...
    def add_turn(
        ...
        """Add a turn to the conversation.        turn_id = f"turn_{uuid.uuid4().hex[:12]}""
    ...
    def append_turn(self, turn: ConversationTurn) -> None:
        """Append an existing turn to the conversation.        self._turns.append(turn)
    ...
    def get_turn(self, turn_id: str) -> Optional[ConversationTurn]:
        """Get turn by ID.        return self._turn_index.get(turn_id)
    ...
    def get_messages(
        ...
        """Get turns as messages.        messages = []
    ...
    def get_recent(self, n: int) -> List[ConversationTurn]:
        """Get n most recent turns.        return self._turns[-n:]

    def clear(self) -> None:
        """Clear all turns.        self._turns.clear()
        self._turn_index.clear()
        self._total_tokens = TokenMetrics()

    def truncate(self, max_turns: Optional[int] = None) -> int:
        """Truncate old turns.        limit = max_turns or self.config.max_turns
