...
{% endif %}""",
    TemplateType.LLAMA2: (
        ...
        "{% for message in messages %}{% if loop.first and system_message %}[INST] <<SYS>>\n"
        "{{ system_message }}\n<</SYS>>\n\n{{ message['content'] }} [/INST]"
        "{% elif message['role'] == 'user' %}{% if not loop.first %} [INST] {{ message['content'] }} [/INST]"
    ...
    TemplateType.MISTRAL: (
        "{% if messages[0]['role'] == 'system' %}{% set system_message = messages[0]['content'] %}"
        "{% set messages = messages[1:] %}{% else %}{% set system_message = false %}{% endif %}"
        "{% for message in messages %}{% if message['role'] == 'user' %}{{ '[INST] ' }}"
        "{% if system_message and loop.first %}{{ system_message + '\n\n' }}{% endif %}"
        "{{ message['content'] + ' [/INST]' }}{% elif message['role'] == 'assistant' %}"
        "{{ message['content'] + '</s> ' }}{% endif %}{% endfor %}"
    ),
