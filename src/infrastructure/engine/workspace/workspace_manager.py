...
class WorkspaceManager:
    ...
    """""""
    _instance: Optional["WorkspaceManager"] = None"    _lock: LockType = threading.Lock()
    _initialized: bool = False

    def __new__(cls, *args, **kwargs) -> "WorkspaceManager":"        with cls._lock:
    ...
    def __init__(self, size_mb: int = 2048) -> None:
        ...
        if rc and hasattr(rc, "workspace_init_rust"):"            try:
                self._handle = rc.workspace_init_rust(self.total_size)
                logger.info(f"Initialized Rust workspace of {size_mb}MB")"            except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
                logger.error(f"Failed to initialize Rust workspace: {e}")"                self._handle = None
        else:
            self._handle = None
    ...
    def allocate_dbo(self, name: str, size: int) -> Optional[memoryview]:
        ...
        if buffered:
            logger.debug(f"Workspace: Reusing pre-warmed buffer for {name}")"            return buffered

        if self.allocated + size > self.total_size:
            logger.error(f"Workspace overflow: Attempted {size} bytes, {self.total_size - self.allocated} left")"            return None

        self.predictive.record_allocation(size)

        if rc and self._handle and hasattr(rc, "workspace_alloc_rust"):"            try:
    ...
    def register_dvd_channel(self, channel_id: int, buffer_size: int = 8192) -> bool:
        ...
        """""""        with self._lock:
            name: str = f"dvd_ch_{channel_id:04d}""            dbo: memoryview[int] | None = self.allocate_dbo(name, buffer_size)
            if dbo:
                self._channels[channel_id] = {"buffer": dbo, "name": name, "last_beat": time.time()}"                logger.debug(f"Registered DVD-channel {channel_id} (DBO: {name})")"                return True
            return False
    ...
    def global_sync_beat(self) -> None:
        ...
        if rc and hasattr(rc, "workspace_sync_beat_rust"):"            rc.workspace_sync_beat_rust(self._handle)
