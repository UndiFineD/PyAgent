...
Implements vLLM's PyNcclCommunicator patterns for distributed inference:
- NCCLConfig: Timeout and retry settings
- NCCLCommunicator: Wrapper with error handling
- Collective operations: all_reduce, all_gather, reduce_scatter, send/recv

Beyond vLLM: Automatic retry on transient failures with exponential backoff.
"""
...
class NCCLCommunicator:
    ...
    Beyond vLLM: Automatic retry with exponential backoff.
    """
    ...
    def all_gather(
        ...
        Returns:
            Gathered tensor, or handle if async_op
        """
    ...
    def reduce_scatterv(
        ...
        Args:
            tensor: Input tensor
            output_sizes: Size of output for each rank
            dim: Dimension to scatter along
            op: Reduction operation
        ...
        def do_reduce_scatterv():
            with self._timed_op("reduce_scatterv", tensor):
                # Split input according to output_sizes
    ...
    def stream_context(self):
        ...
        This separates communication from compute for overlap.
        """
...
class CustomAllReduce:
    ...
    Provides alternative all-reduce paths:
    - Symmetric memory for large tensors
    - Ring all-reduce for small groups
    ...
    def all_reduce(
        ...
        Chooses between:
        1. Custom implementation for large tensors
        2. NCCL for small tensors
    ...
    def _custom_all_reduce(
        ...
        # In practice, this would use symmetric memory or ring patterns
