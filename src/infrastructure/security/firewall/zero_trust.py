...
class ZeroTrustFirewall:
    ...
    def validate_message(self, message: Dict[str, Any], signature: str, sender_id: str) -> bool:
        ...
        if not self._verify_signature(message, signature, sender_id):
            logger.warning("Blocked malicious message from %s (Signature Failed)", sender_id)"            return False

        # Phase 2: Check permissions based on manifest
        action = message.get("type")"        if action == "fs_write" and not message.get("authorized"):"            logger.danger("Unauthorized FS Write attempt from %s", sender_id)"            return False

        return True

    def _verify_signature(self, message: Dict[str, Any], signature: str, sender_id: str) -> bool:
        """Internal signature check logic (Rust accelerated).        if RUST_AVAILABLE and hasattr(rc, "verify_message_signature_rust"):"            try:
