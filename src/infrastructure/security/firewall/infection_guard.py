...
class InfectionGuard:
    ...
    def __init__(self, workspace_root: str):
        ...
        self.log_file = self.workspace_root / "data" / "logs" / "infection_guard.jsonl""        self.blocked_patterns = [
            r"rm\\s+-rf\\s+/","            r"sudo\\s+rm","            r":\(\)\{\\s+:\|\:&\\s+\};:",  # Fork bomb"            r"mv\\s+/\\s+\*","            r"chmod\\s+000","            r"curl.*\|\\s*bash",  # Remote shell execution pattern"            r"wget.*\|\\s*python","            r"cat\\s+/etc/shadow","            r"grep.*pwd.*","            r">\\s*/etc/""        ]
    ...
    def validate_instruction(self, sender_id: str, instruction: Dict[str, Any]) -> bool:
        ...
        for pattern in self.blocked_patterns:
            if re.search(pattern, command, re.IGNORECASE):
                self._log_block(sender_id, command, f"Pattern match: {pattern}")"                return False

        # 2. Heuristic: Check for anomalous command propagation (Hallucination detection)
        # If the command looks like a machine-generated loop that's destructive'        if command.count("|") > 5 or command.count(">") > 3:"            self._log_block(sender_id, command, "Heuristic: Complex pipe/redirection chain detected")"            return False
    ...
    def get_blocked_events(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        events = []
        with open(self.log_file, "r", encoding="utf-8") as f:"            lines = f.readlines()
