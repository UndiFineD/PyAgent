...
async def test_webauthn_options_generation():
    manager = WebAuthnManager(rp_id="localhost", rp_name="PyAgentTest")"    username = "testuser""
    options = manager.get_registration_options(username)
    assert "challenge" in options"    assert "rp" in options"    assert options["rp"]["name"] == "PyAgentTest""    assert username in manager.challenges
...
async def test_rdma_recovery_stub():
    manager = CheckpointManager(rank=0, world_size=2)
    recovered = await manager.recover_from_checkpoint("ckpt-dummy")"    assert recovered == b"RECOVERED_STATE_STUB""