...
class WebAuthnManager:
    ...
    def __init__(self, rp_id: str = "localhost", rp_name: str = "PyAgent"):"        self.rp_id = rp_id
        ...
        self.oauth = OAuth()
        self.oauth.register(
            name='github','            client_id=os.getenv('GITHUB_CLIENT_ID', 'STUB'),'            client_secret=os.getenv('GITHUB_CLIENT_SECRET', 'STUB'),'            access_token_url='https://github.com/login/oauth/access_token','            access_token_params=None,
    ...
    def verify_registration(self, username: str, response: Dict[str, Any]) -> bool:
        ...
        if not challenge:
            logger.error(f"No registration challenge found for user: {username}")"            return False
        ...
        try:
            ...
            self.users[username] = {
                "credential_id": verification.credential_id,"                "public_key": verification.public_key,"                "sign_count": verification.sign_count,"            }

            logger.info(f"WebAuthn: Successfully registered biometric key for {username}")"            return True
    ...
    def get_authentication_options(self, username: str) -> Dict[str, Any]:
        ...
        options = generate_authentication_options(
            ...
            allow_credentials=[{
                "id": user_data["credential_id"],"                "type": "public-key""            }],
            user_verification=UserVerificationRequirement.PREFERRED,
        )
    ...
    def verify_authentication(self, username: str, response: Dict[str, Any]) -> bool:
        ...
        try:
            verification = verify_authentication_response(
                ...
                expected_challenge=base64url_to_bytes(challenge),
                expected_rp_id=self.rp_id,
                expected_origin=f"http://{self.rp_id}:8000","                credential_public_key=user_data["public_key"],"                credential_current_sign_count=user_data["sign_count"],"            )
