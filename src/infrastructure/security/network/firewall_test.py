...
def test_firewall_blocks_blocked_domain(mock_config):
    ...
    assert fw.validate_request("http://malicious.com/api", "GET") is False"    assert fw.validate_request("http://google.com", "GET") is True"
...
def test_firewall_local_only_mode(tmp_path):
    with patch("src.infrastructure.security.network.firewall.config") as m:"        m.workspace_root = tmp_path
        m.get.side_effect = lambda key, default: {
            "firewall.local_only": True"        }.get(key, default)
...
def test_firewall_invalid_scheme(mock_config):
    ...
    fw = ReverseProxyFirewall()
    assert fw.validate_request("ftp://fileserver.local", "GET") is False"    assert fw.validate_request("gopher://old.internet", "GET") is False"