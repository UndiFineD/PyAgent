...
class VRAMEstimator:
    ...
    @classmethod
    def estimate(
        cls, info: ModelInfo, ctx: int = 4096, batch: int = 1, dtype: str = "float16"
    ) -> VRAMEstimate:
        bpp = (
            1
        ...
        total_gb = (
            weights_gb + (kv_per_token_mb * ctx * batch / 1024) + (weights_gb * 0.15)
        )

        can_fit = [gpu for gpu, vram in cls.GPU_VRAM.items() if vram >= total_gb]
        rec = "RTX 4090" if total_gb <= 24 else "A100-80GB"
        return VRAMEstimate(
            weights_gb, kv_per_token_mb, weights_gb * 0.15, total_gb, rec, can_fit
        )
