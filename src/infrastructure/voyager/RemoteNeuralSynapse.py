...
from src.infrastructure.voyager.TransportLayer import VoyagerTransport
...
class RemoteNeuralSynapse:
    ...
    def __init__(
        self, fleet_manager: Any, transport_port: int = 5555, discovery_node: Any = None
    ):
    ...
    async def start(self):
        ...
        self._server_task = asyncio.create_task(
            self.transport.start_server(self._handle_incoming_synapse)
        )
    ...
    async def _handle_incoming_synapse(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Synapse: Incoming {msg_type} from {message.get('sender_id', 'unknown')}"
        )
        ...
        if msg_type == "teleport":
            ...
            if encoded_blob:
                ...
                return {
                    "status": "success",
                    "message": f"Agent {state.get('name')} assimilated.",
                }
    ...
    async def teleport_agent_to_peer(
        self, agent: Any, peer_address: str, transport_port: int
    ) -> bool:
        """
        ...
        payload = {
            ...
            "sender_id": (
                self.fleet_manager.fleet_id
                if hasattr(self.fleet_manager, "fleet_id")
                else "unknown"
            ),
        }

        logger.info(
            f"Synapse: Firing synaptic teleport of {agent.name} to {peer_address}:{transport_port}..."
        )

        response = await self.transport.send_to_peer(
            peer_address, transport_port, payload
        )
        if response and response.get("status") == "success":
            logger.info(
                f"Synapse: Teleportation confirmed by peer: {response.get('message')}"
            )
            return True
    ...
    async def teleport_to_peer_name(self, agent: Any, peer_name: str) -> bool:
        ...
        if not self.discovery_node:
            logger.error(
                "Synapse: Cannot teleport by name - discovery_node not initialized."
            )
            return False
        ...
        if not target:
            logger.error(
                f"Synapse: Could not resolve peer name '{peer_name}' on local network."
            )
            return False
    ...
    async def remote_invoke(
        self, thought_pattern: str, target_peer: Dict[str, Any]
        ...
        logger.info(
            f"Synapse: Remote invocation triggered on {target_peer.get('name')}"
        )
