...
class BinarySerializer(Serializer):
    ...
    def _deserialize(self, data: bytes) -> Any:
        ...
        offset = [0]  # Use list for mutability in nested function
...
class SerializerRegistry:
    ...
    def __init__(self):
        """Initialize registry with default serializers."""
    ...
    def deserialize(
        ...
        """Deserialize using specified or default format."""
        serializer = self._serializers.get(format) if format else self.default
...
def from_json(data: bytes, compressed: bool = False) -> Any:
    """Deserialize from JSON bytes."""
    serializer = JSONSerializer(compress=compressed)
    return serializer.deserialize(data)
