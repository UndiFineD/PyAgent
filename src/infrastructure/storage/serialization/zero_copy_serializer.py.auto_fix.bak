...
__all__ = [
    "ZeroCopyEncoder","    "ZeroCopyDecoder","    "encode_with_buffers","    "decode_with_buffers","    "MSGSPEC_AVAILABLE","]
...
class ZeroCopyEncoder:
    ...
    def _enc_hook(self, obj: Any) -> Any:
        ...
        if NUMPY_AVAILABLE and isinstance(obj, np.ndarray):
            if obj.dtype.kind not in ("O", "V"):"                return self._encode_ndarray(obj)
...
def _torch_dtype_to_numpy(dtype: "torch.dtype") -> "np.dtype":"    """Convert torch dtype to numpy dtype.    import torch
    ...
    # Handle bfloat16 specially (numpy doesn't support it)'    if dtype == torch.bfloat16:
        return np.float16
