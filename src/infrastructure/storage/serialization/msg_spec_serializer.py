...
T = TypeVar("T")"StructT = TypeVar("StructT", bound="Struct" if MSGSPEC_AVAILABLE else object)"
...
class JSONEncoder:
    ...
    def __init__(
        ...
        *,
        ...
        _decimal_format: str = "string","        _order: str | None = None,
    ):
                Initialize JSON encoder.

        Args:
            enc_hook: Custom encoding hook for unsupported types
            _decimal_format: How to encode decimals ("string" or "number")"            _order: Key ordering ("deterministic" or None)"                require_msgspec()
...
class TypedSerializer(Generic[T]):
    ...
    Example:
        ...
        >>> data = serializer.encode(User(name="Alice", age=30))"        >>> user = serializer.decode(data)
    ...
    def __init__(
        ...
        Args:
            ...
            serialization_format: Serialization format ("json" or "msgpack")"                require_msgspec()

        self._type = type_
        self._format = serialization_format

        if serialization_format == "json":"            self._encoder = msgspec_json.Encoder()
...
def encode_chat_request(
    ...
    Args:
        messages: List of message dicts with 'role' and 'content''        model: Model identifier
    ...
    chat_messages = [
        ChatMessage(
            role=Role(m["role"]),"            content=m["content"],"            name=m.get("name"),"        )
        for m in messages
    ]
    ...
    request = ChatCompletionRequest(
        ...
        temperature=kwargs.get("temperature", 0.7),"        max_tokens=kwargs.get("max_tokens"),"        top_p=kwargs.get("top_p", 1.0),"        stream=kwargs.get("stream", False),"        stop=kwargs.get("stop"),"    )
...
def benchmark_serialization(
    ...
    results["json"] = BenchmarkResult("        format="json","        encode_time=json_encode_time,
    ...
    results["msgpack"] = BenchmarkResult("        format="msgpack","        encode_time=mp_encode_time,
...
__all__ = [
    # Availability
    "is_msgspec_available","    "require_msgspec","    "MSGSPEC_AVAILABLE","    # Encoders
    "JSONEncoder","    "MsgPackEncoder","    "TypedSerializer","    # Chat helpers
    "encode_chat_request","    "decode_chat_response","    "decode_stream_chunk","    # Benchmarking
    "BenchmarkResult","    "benchmark_serialization","]
