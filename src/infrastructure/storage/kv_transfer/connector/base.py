...
class KVConnectorBase(ABC):
    ...
    def register_kv_caches(self, kv_caches: Dict[str, Any]) -> None:
        """Register KV caches for transfer operations.        with self._lock:
    ...
    def wait_for_save(self) -> None:
        """Wait for all KV cache saves to complete.        pass
    ...
    def health_check(self) -> bool:
        """Check connector health.        return self._health_status and self._initialized

    def reset_error_count(self) -> None:
        """Reset error count after successful operation.        self._error_count = 0

    def record_error(self) -> None:
        """Record an error occurrence.        self._error_count += 1
    ...
    def close(self) -> None:
        """Close the connector and release resources.        with self._lock:
