...
(Execute where the workspace root is the current working directory so LazyAgentMap/LazyOrchestratorMap can resolve paths.)

WHAT IT DOES:
Runs a lightweight diagnostic that instantiates LazyAgentMap and LazyOrchestratorMap (using the current working directory or a MockFleet) and prints counts and example names of discovered agent and orchestrator configurations, while logging errors and stack traces for import/OS/typing failures.

WHAT IT SHOULD DO BETTER:
- Avoid accessing protected members (e.g., _discovered_configs, _configs); use public APIs or add safe accessor methods to the registries.
- Accept explicit workspace/fleet path arguments and add CLI flags (--root, --limit, --verbose) instead of assuming os.getcwd(); return structured data (JSON) for machine consumption.
- Harden error handling (narrow exception handling, better messages) and add unit tests and mocks for registries to allow deterministic diagnostics in CI.
