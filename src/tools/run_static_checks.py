...
WHAT IT DOES:
Runs quick, local static-safety checks on a directory of extracted candidate Python packages. It performs fast AST-based heuristic checks (banned imports, banned names, dangerous attribute usage and calls) and attempts to run external tools (bandit and semgrep) if available, writing JSON reports under ./.external/static_checks/. Prefers using the current venv/python -m fallback if the tool executable is not on PATH.

WHAT IT SHOULD DO BETTER:
- Report missing external tool installations more clearly and collect install hints into a single summary output instead of returning exit codes only.  
- Improve AST heuristics to reduce false positives (context-aware detection for uses of open, eval-like names and more precise import resolution).  
- Add configurable rule sets, concurrency for large trees, and optional suppression/whitelist per-package; include better error handling and timeouts for subprocess runs; produce richer combined report (aggregated summary + per-file details) and machine-readable exit status.
