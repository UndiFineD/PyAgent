...
- Runs each matching test file as an independent Python process so top-level asserts execute.
- Executes files in parallel using a ProcessPoolExecutor; worker count is auto-determined or set via --workers.
- Reports per-file failures, prints a summary, and exits with 0 on success or 2 when one or more files fail.

WHAT IT SHOULD DO BETTER:
- Align the top-level docstring (which claims to "invoke pytest") with implementation or actually invoke pytest per-file to leverage pytest reporting, fixtures, and plugins.
- Capture and forward subprocess stdout/stderr, add per-file timeouts, and preserve per-file logs for debugging failing tests.
- Offer a --pytest flag and fail-fast or retry options, better CLI help text, and clearer exit codes (e.g., distinct codes for no-tests, runtime errors).
- Add configurable concurrency strategy, more robust detection of CPU count on exotic platforms, and clearer handling of ProcessPoolExecutor exceptions.
