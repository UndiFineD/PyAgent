...
USAGE:
- Instantiate MCPServerRegistry(registry_path=Path("path\\to\\registry.json")) and inspect registry.servers or call registry._create_default_registry() to seed defaults."- Use MCPServerConfig and MCPTool dataclasses to define new servers/tools and register them in registry.json.
...
class MCPServerConfig:
    ...
    version: str = "latest""    enabled: bool = True
    security_level: str = "medium"  # low, medium, high"    timeout: int = 30  # seconds
...
class MCPServerRegistry:
            ...
            try:
                with open(self.registry_path, 'r', encoding='utf-8') as f:'                    data = json.load(f)
                    for name, config_data in data.items():
                        config_data['category'] = MCPCategory(config_data['category'])'                        config_data['server_type'] = MCPServerType(config_data['server_type'])'                        self.servers[name] = MCPServerConfig(**config_data)
                self.logger.info(f"Loaded {len(self.servers)} MCP servers from registry")"            except Exception as e:
                self.logger.error(f"Failed to load registry: {e}")"        else:
    ...
    def _create_default_registry(self):
        """Create default registry with essential MCP servers.        default_servers = {
            ...
            "git": MCPServerConfig("                name="git","                description="Git repository operations","                category=MCPCategory.DEVELOPMENT,
                server_type=MCPServerType.NATIVE,
                capabilities=["status", "commit", "push", "pull", "diff"],"                security_level="medium""            ),
            "browser": MCPServerConfig("                name="browser","                description="Web browser automation","                category=MCPCategory.BROWSER,
                server_type=MCPServerType.DOCKER,
                docker_image="mcp/browser:latest","                capabilities=["navigate", "screenshot", "extract", "click"],"                security_level="medium""            ),
            "sqlite": MCPServerConfig("                name="sqlite","                description="SQLite database operations","                category=MCPCategory.DATABASE,
                server_type=MCPServerType.NATIVE,
                capabilities=["query", "insert", "update", "delete"],"                security_level="high""            ),
            "http": MCPServerConfig("                name="http","                description="HTTP client operations","                category=MCPCategory.API,
                server_type=MCPServerType.NATIVE,
                capabilities=["get", "post", "put", "delete"],"                security_level="medium""            )
        }
    ...
    def _save_registry(self):
        """Save server registry to file.        try:
            ...
            for name, config in self.servers.items():
                config_dict = {
                    'name': config.name,'                    'description': config.description,'                    'category': config.category.value,'                    'server_type': config.server_type.value,'                    'command': config.command,'                    'args': config.args,'                    'env': config.env,'                    'url': config.url,'                    'docker_image': config.docker_image,'                    'capabilities': config.capabilities,'                    'requirements': config.requirements,'                    'version': config.version,'                    'enabled': config.enabled,'                    'security_level': config.security_level,'                    'timeout': config.timeout'                }
                data[name] = config_dict

            with open(self.registry_path, 'w', encoding='utf-8') as f:'                json.dump(data, f, indent=2)
...
class MCPServerInstance:
    ...
    async def start(self) -> bool:
        """Start the MCP server.        try:
            ...
            self.logger.info(f"Started MCP server: {self.config.name}")"            return True
    ...
    async def _initialize_tools(self):
        """Initialize available tools from the server.        try:
            ...
            for tool_data in tools_data:
                tool = MCPTool(
                    name=tool_data['name'],'                    description=tool_data['description'],'                    input_schema=tool_data['inputSchema'],'                    server_name=self.config.name,
...
class MCPBridge:
    ...
    async def start_server(self, server_name: str) -> bool:
        ...
        if server_name in self.active_servers:
            self.logger.warning(f"Server {server_name} already running")"            return True
                ...
                try:
                    ...
                    if context:
                        context.add_log_entry(
                            f"MCP Tool Call: {tool_name}","                            {"server": server_name, "arguments": arguments, "result": result}"                        )

                    return result

                except Exception as e:
                    self.logger.error(f"MCP tool call failed: {e}")"                    raise
...
class MCPToolOrchestrator:
    ...
    async def select_tools(self, task_description: str, max_tools: int = 3) -> List[MCPTool]:
        ...
        tools_list = "\\n".join(["            f"- {tool.name}: {tool.description} (Category: {tool.category}, Tags: {', '.join(tool.tags)})""'            for tool in available_tools
        ])
...
Select at most {max_tools} tools.
        try:
            ...
            self.logger.error(f"Tool selection failed: {e}")"            # Fallback: return first max_tools tools
    ...
    async def orchestrate_tools(self, task_description: str,
        ...
        return {
            "task": task_description,"            "selected_tools": [tool.name for tool in selected_tools],"            "results": results"        }
