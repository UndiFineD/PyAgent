...
class ExternalToolSecurity:
    ...
    def _initialize_security_policies(self):
        ...
        self._security_policies["database"] = SecurityPolicy(
        ...
        self._security_policies["api"] = SecurityPolicy(
            allowed_domains={"api.github.com", "api.openai.com", "localhost"},
        ...
        self._security_policies["test"] = SecurityPolicy(
            allowed_domains={"localhost", "127.0.0.1"},
            blocked_patterns=set(),
            max_execution_time=30,
            max_memory_usage=100,
        ...
        self._security_policies["development"] = SecurityPolicy(
            allowed_domains={"localhost", "127.0.0.1"},
            blocked_patterns=set(),
    ...
    def _initialize_threat_patterns(self):
        ...
        self._threat_patterns = {
            ...
            ThreatCategory.DENIAL_OF_SERVICE: [
                r"while\s+True",
                r"for\s+i\s+in\s+range\s*\(\s*0",
                r"fork\s*\(\s*\)",
                r"threading\.Thread.*target.*while"
            ]
        }
    ...
    def _initialize_trusted_tools(self):
        ...
        self._trusted_tools.update([
            "python_compiler", "python_interpreter",
            "typescript_compiler", "typescript_interpreter",
            "javascript_compiler", "javascript_interpreter",
            "go_compiler", "go_interpreter",
            "rust_compiler", "rust_interpreter",
            "sql_executor", "rest_client",
            "code_formatter", "test_runner",
            "trusted_tool"  # For testing
        ])
    ...
    def _contains_suspicious_patterns(self, tool_name: str) -> bool:
        ...
        suspicious_patterns = [
            ...
            "crypto", "botnet", "untrusted", "malicious"
        ]
    ...
    def _validate_execution_context(self, context: Dict[str, Any]) -> bool:
        ...
        required_fields = ["category", "parameters"]
            ...
            for pattern in patterns:
                ...
                if matches:
                    threats.append({
                        ...
                        "pattern": pattern,
                        "matches": len(matches),
                        "severity": self._calculate_threat_severity(category, len(matches))
                    })
    ...
    def _calculate_threat_severity(self, category: ThreatCategory, match_count: int) -> str:
        ...
        base_severity = {
            ThreatCategory.MALWARE: "high",
            ThreatCategory.DATA_EXFILTRATION: "high",
            ThreatCategory.CODE_INJECTION: "critical",
            ThreatCategory.PRIVILEGE_ESCALATION: "critical",
            ThreatCategory.DENIAL_OF_SERVICE: "medium"
        }
    ...
    def create_secure_sandbox(self, tool_name: str, category: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "isolation_level": "high" if policy.security_level in [SecurityLevel.HIGH, SecurityLevel.TRUSTED] else "medium"
        }
    ...
    def audit_tool_execution(self, tool_name: str, execution_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        audit_report = {
            ...
            "network_calls": execution_result.get("network_calls", []),
            "file_access": execution_result.get("file_access", []),
            "security_violations": [],
    ...
    def _get_policy_for_tool(self, tool_name: str) -> Optional[SecurityPolicy]:
        ...
        # This would typically involve looking up the tool's category
    ...
    def _check_policy_violations(self, execution_result: Dict[str, Any], policy: SecurityPolicy) -> List[str]:
        ...
        if execution_result.get("execution_time", 0) > policy.max_execution_time:
            violations.append(f"Execution time exceeded {policy.max_execution_time}s limit")

        # Check memory usage
        if execution_result.get("memory_usage", 0) > policy.max_memory_usage:
            violations.append(f"Memory usage exceeded {policy.max_memory_usage}MB limit")

        # Check network access
        if not policy.network_access and execution_result.get("network_calls"):
            violations.append("Network access not permitted")

        # Check file access
        if not policy.file_access and execution_result.get("file_access"):
            violations.append("File access not permitted")
    ...
    def get_security_report(self) -> Dict[str, Any]:
        ...
        return {
            "trusted_tools_count": len(self._trusted_tools),
            "blocked_tools_count": len(self._blocked_tools),
            "registered_signatures": len(self._tool_signatures),
            "security_policies": len(self._security_policies),
            "threat_categories": len(self._threat_patterns),
            "verified_tools": sum(1 for sig in self._tool_signatures.values() if sig.verified)
        }