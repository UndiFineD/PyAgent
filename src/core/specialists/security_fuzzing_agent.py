...
target = task['target']'        target_type_str = task['target_type']'        target_type = FuzzingTarget(target_type_str)
...
session_ids = task['session_ids']'        result = await self.generate_security_report(session_ids, context)
        return result
