...
class EnvironmentSkill(SkillCore):
    ...
    def read_file(self, relative_path: str) -> str:
        ...
        if not str(full_path.resolve()).startswith(str(Path(self.workspace_root).resolve())):
            logger.error("Security: Attempted path traversal in read_file")
            return "ACCESS_DENIED"

        return full_path.read_text(encoding="utf-8")

    def write_file(self, relative_path: str, content: str) -> bool:
        """Securely write a file to the workspace."""
        if not self.agent.manifest.permissions.get("fs_write", False):
            logger.warning("Security: FS Write permission denied for role '%s'", self.agent.manifest.role)
            return False
