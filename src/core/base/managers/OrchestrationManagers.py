...
class AgentComposer:
    ...
    def _calculate_execution_order(self) -> None:
        ...
        def visit(agent_type: str) -> None:
            if agent_type in temp: raise ValueError(f"Circular dependency for {agent_type}")
            if agent_type in visited: return
            temp.add(agent_type)
            agent = next((a for a in self.agents if a.agent_type == agent_type), None)
            if agent:
                for dep in agent.depends_on: visit(dep)
        ...
        for agent in sorted(self.agents, key=lambda a: a.order):
            if agent.agent_type not in visited: visit(agent.agent_type)
    ...
    def execute(self, file_path: str, prompt: str, agent_factory: Callable[[str, str], BaseAgent]) -> Dict[str, str]:
        ...
        for agent_type in self.execution_order:
            ...
            for dep in agent_config.depends_on:
                if dep in self.results: enhanced_prompt += f"\\n\\nPrevious {dep} result:\\n{self.results[dep][:500]}"
            if current_content: agent.previous_content = current_content
            result = agent.improve_content(enhanced_prompt)
            self.results[agent_type] = result
            current_content = result
        return self.results
    def get_final_result(self) -> str:
        if not self.execution_order: return ""
...
class ABTest:
    ...
    def __post_init__(self) -> None:
        for variant in self.variants: self.variant_counts[variant] = 0
