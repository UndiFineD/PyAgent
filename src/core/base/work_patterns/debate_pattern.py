...
class DebateWorkPattern(WorkPattern):
    ...
    async def _generate_position(
        ...
        if actual_agent and hasattr(actual_agent, 'execute_task'):
            ...
            debate_context.work_state.update("debate_history", history)
    ...
    async def _generate_counter_argument(
        ...
        if actual_agent and hasattr(actual_agent, 'execute_task'):
            ...
            counter_context.work_state.update("debate_history", history)
    ...
    def _check_consensus(self, round_results: Dict[str, Any]) -> bool:
        ...
        arguments = round_results.get("arguments", [])
        if len(arguments) < 2:
            return False
    ...
    async def _synthesize_decision(
        ...
        elif self.config.synthesis_method == "weighted_vote":
            return await self._weighted_vote_synthesis(debate_history, participants)
        else:
    ...
    async def _weighted_vote_synthesis(
        ...
        for participant in participants:
            ...
            weight = 1.0
            if "auditor" in participant.role.lower():
                weight = 1.5  # Give more weight to auditors
            elif "advocate" in participant.role.lower():
                weight = 1.2
