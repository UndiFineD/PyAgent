...
def has_multi_modal_data(prompt: SingletonPrompt) -> bool:
    ...
    if isinstance(prompt, dict):
        ...
        return mm_data is not None and mm_data != {}
    return False
...
def make_tokens_prompt(
    ...
    *,
    ...
    if token_type_ids is not None:
        prompt["token_type_ids"] = token_type_ids
    if cache_salt is not None:
        prompt["cache_salt"] = cache_salt
    return prompt
...
def validate_prompt(prompt: PromptType) -> list[str]:
    ...
    if is_string_prompt(prompt):
        ...
        if not prompt.get("prompt", "").strip():
            errors.append("Empty prompt text in TextPrompt")
    elif is_tokens_prompt(prompt):
        token_ids = prompt.get("prompt_token_ids", [])
        if not token_ids:
            errors.append("Empty token_ids in TokensPrompt")
        elif not all(isinstance(t, int) for t in token_ids):
            errors.append("Non-integer token IDs in TokensPrompt")
    elif is_embeds_prompt(prompt):
        if prompt.get("prompt_embeds") is None:
            errors.append("Missing prompt_embeds in EmbedsPrompt")
    elif is_explicit_encoder_decoder_prompt(prompt):
