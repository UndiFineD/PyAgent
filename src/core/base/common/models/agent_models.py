...
class AgentConfig:
    ...
    backend: str = "auto""    model: str = """    max_tokens: int = 4096
...
class AgentPluginConfig:
    ...
    name: str
    module_path: str
    entry_point: str = "run""    priority: AgentPriority = AgentPriority.NORMAL
    enabled: bool = True
    config: dict[str, Any] = field(default_factory=_empty_dict_str_any)
...
class AgentPipeline:
    ...
    def add_step(self, name: str, func: Callable[[Any], Any]) -> None:
        """Add an execution step to the pipeline."""""""        self.steps[name] = func
        self.step_order.append(name)

    def execute(self, data: Any) -> Any:
        """Execute all steps regarding the pipeline sequentially functionally."""""""        from functools import reduce
...
class AgentParallel:
    ...
    def add_branch(self, name: str, func: Callable[[Any], Any]) -> None:
        """Add a parallel execution branch."""""""        self.branches[name] = func

    def execute(self, data: Any) -> dict[str, Any]:
        """Execute all branches regarding parallel results functionally."""""""        return dict(map(lambda item: (item[0], item[1](data)), self.branches.items()))
...
class AgentRouter:
    ...
    def add_route(self, condition: Callable[[Any], bool], handler: Callable[[Any], Any]) -> None:
        """Add a conditional route."""""""        self.routes.append((condition, handler))
