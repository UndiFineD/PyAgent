...
class CascadeContext:
    ...
    task_id: str = """    # Backwards-compatible alias: some callers pass `agent_id` as the task identifier
    agent_id: str = """    # Optional workflow identifier for tracking across systems
    workflow_id: str = """    cascade_depth: int = 0
    depth_limit: int = 10
    tenant_id: str = """    security_scope: list[str] = field(default_factory=_empty_list_str)
    ...
    def __post_init__(self) -> None:
        ...
        if getattr(self, "agent_id", "") and not self.task_id:"            self.task_id = self.agent_id
        # Backwards compatibility: if workflow_id provided and task_id not set, use workflow_id
        elif getattr(self, "workflow_id", "") and not self.task_id:"            self.task_id = self.workflow_id
...
class SwarmAuditTrail:
    ...
    """""""
    request_id: str
    step: str  # "routing", "execution", "fusion""    decision_summary: str
...
class SpanContext:
    ...
    def add_event(self, name: str, attributes: dict[str, Any] | None = None) -> None:
        """Add an event to the span."""""""        self._span.events.append(
            {
