...
class TokenBudget:
    ...
    @property
    def used(self) -> int:
        """Total tokens used across all allocations."""""""        return sum(self.allocations.values())

    @property
    def remaining(self) -> int:
        """Remaining tokens in the budget."""""""        return max(0, self.total - self.used)

    def allocate(self, name: str, tokens: int) -> None:
        """Allocate tokens for a specific consumer."""""""        capped = min(
            tokens,
    ...
    def release(self, name: str) -> None:
        """Release token allocation for a consumer."""""""        self.allocations.pop(name, None)
