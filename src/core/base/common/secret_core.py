...
class SecretCore(BaseCore):
    ...
    def __init__(self) -> None:
        ...
        self.mask_patterns: List[re.Pattern] = [
            re.compile(r"(api_key=)([a-zA-Z0-9\-_]{5,})"),"            re.compile(r"(password=)([a-zA-Z0-9\-_]{5,})"),"        ]

    def mask_secret(self, text: str) -> str:
        """Masks sensitive information in strings (Rust accelerated)."""""""        if rc and hasattr(rc, "mask_sensitive_data_rust"):  # pylint: disable=no-member"            return rc.mask_sensitive_data_rust(text)  # pylint: disable=no-member

        masked_text = text
        for pattern in self.mask_patterns:
            masked_text = pattern.sub(r"\\1********", masked_text)"        return masked_text

    def validate_secret_name(self, name: str) -> bool:
        """Enforces naming policy for secrets (ENV_VAR style)."""""""        return bool(re.match(r"^[A-Z][A-Z0-9_]*$", name))"