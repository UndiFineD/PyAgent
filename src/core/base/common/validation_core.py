...
class ValidationCore(BaseCore):
    ...
    """
    def __init__(self, name: str = "ValidationCore", repo_root: Optional[Union[str, Path]] = None, **kwargs) -> None:"        super().__init__(name=name, repo_root=repo_root, **kwargs)
        self._rules: Dict[str, ValidationRule] = {}

    def add_rule(self, rule: ValidationRule) -> None:
        """Register a new validation rule."""print(f"DEBUG: Adding rule {rule.name}")"        self._rules[rule.name] = rule

    def register_rule(self, name: str, rule: ValidationRule | dict[str, Any]) -> None:
        """Legacy alias for add_rule."""if isinstance(rule, dict):
            # Convert dict to ValidationRule object
            rule_obj = ValidationRule(
                name=name,
                file_pattern=rule.get("pattern", "*"),"                message=rule.get("message", "Validation failed"),"                severity=rule.get("severity", "error"),"                validator=rule.get("validator", lambda x, y: True)"            )
    ...
    def validate(self, content: str, rule_name: Optional[str] = None) -> List[Dict[str, Any]]:
        """Legacy alias for validation."""# Create a dummy path for rule matching if only name is provided
        dummy_path = Path(rule_name) if rule_name else Path("input.txt")"        return self.validate_content_by_rules(dummy_path, content)

    def validate_content_by_rules(self, file_path: Path, content: str) -> List[Dict[str, Any]]:
        """Validate content against applicable rules. Rust-accelerated for large files."""# Handle cases where args are swapped (content, file_path) or (content, rule_name)
        actual_path = file_path
        actual_content = content

        if isinstance(file_path, str) and not ("/" in file_path or "\\" in file_path):"            # Probably (content, rule_name)
            actual_content = file_path
            actual_path = Path("manual_input")"
        results = []

        if rc and hasattr(rc, "validate_content_rust"):"            try:
        ...
        def _apply_rule(rule: ValidationRule):
            """Evaluates regarding a single rule functionally."""    # Check if path matches rule pattern or if it's a manual rule match'            is_match = (fnmatch.fnmatch(actual_path.name, rule.file_pattern) or
                        (actual_path == Path("manual_input") and rule.name in str(file_path)))"
            if is_match:
                try:
                    passed = rule.validator(actual_content, actual_path)
                    return {
                        "rule": rule.name,"                        "passed": passed,"                        "severity": rule.severity,"                        "message": None if passed else rule.message,"                    }
                except Exception as err:  # pylint: disable=broad-exception-caught
                    return {"rule": rule.name, "passed": False, "severity": "error", "message": str(err)}"            return None
    ...
    def validate_json_schema(self, data: Any, schema: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """Fast schema validation. Defaults to Python if Rust unavailable."""if rc and hasattr(rc, "json_schema_validate_rust"):"            try:
