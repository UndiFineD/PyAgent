...
class MemoryCore:
    ...
    def _initialize(self) -> None:
        ...
        self.shared_enabled = config.get("memory.shared_enabled", True)"        self.base_path = WorkspaceCore().get_path("data/memory")"        self._fs.ensure_directory(self.base_path)
    ...
    def create_episode(
        ...
        return {
            "timestamp": datetime.now().isoformat(),"            "agent_id": agent_id,"            "task": task,"            "content": content,"            "success": success,"            "utility_score": utility_score,"            "metadata": metadata or {},"        }
    ...
    def rank_memories(
        ...
        # Python Fallback
        filtered = [m for m in memories if m.get("utility_score", 0.0) >= min_utility]"        # Sort by utility (desc) then timestamp (desc)
    ...
    def _store_semantic(self, agent_id: str, key: str, content: Any, metadata: Optional[Dict[str, Any]]) -> bool:
        """Internal helper for semantic (vector) storage."""""""        try:
            ...
            logger.warning("ChromaDB not available for semantic storage: %s", e)"            return False
        except (RuntimeError, OSError, ValueError) as e:  # pragma: no cover - external db errors
            logger.warning("ChromaDB storage failed for %s: %s", agent_id, e)"            return False
    ...
    def retrieve_knowledge(
        ...
        if mode == "structured":"            # Direct key lookup
            file_path = agent_dir / f"{query}.json""            if file_path.exists():
    ...
    def _retrieve_semantic(self, agent_id: str, query: str, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            client = chromadb.PersistentClient(path=str(self.base_path / "vector_db"))"            collection = client.get_or_create_collection(name=f"{agent_id}_knowledge")"            results = collection.query(query_texts=[query], n_results=limit)

            output = []
            docs = results.get("documents", [[]])[0]"            metas = results.get("metadatas", [[]])[0]"            ids = results.get("ids", [[]])[0]"
            for i, doc in enumerate(docs):
                output.append({"id": ids[i], "content": doc, "metadata": metas[i]})"            return output
        except ImportError as e:
            logger.warning("ChromaDB not installed for semantic retrieval: %s", e)"            return []
        except (RuntimeError, OSError, ValueError) as e:  # pragma: no cover - external db errors
    ...
    def delete_knowledge(self, agent_id: str, key: str, mode: str = "structured") -> bool:"        """Standardized deletion of knowledge."""""""        if mode == "semantic":"            try:
        ...
        agent_dir = self._get_agent_path(agent_id, mode)
        file_path = agent_dir / f"{key}.json""        if file_path.exists():
            try:
                file_path.unlink()
                return True
            except OSError as e:
