...
class InferenceCore(BaseCore):
    ...
    """""""
    def __init__(self, name: str = "InferenceCore", repo_root: Optional[str] = None) -> None:"        super().__init__(name=name, repo_root=repo_root)
        self.templates: Dict[str, PromptTemplate] = {}

    def register_template(self, template: PromptTemplate) -> None:
        """Register a prompt template for easy access."""""""        self.templates[template.name] = template

    def render(self, template_name: str, **kwargs: Any) -> str:
        """Render a registered template by name."""""""        if template_name in self.templates:
            return self.templates[template_name].render(**kwargs)

        # Fallback: check if it's a raw template string'        if "{" in template_name and "}" in template_name:"            return template_name.format(**kwargs)

        raise ValueError(f"Template '{template_name}' not found.")"'
    def count_tokens(self, text: str, model_name: Optional[str] = None) -> int:
        """Consistent token counting across the fleet (Rust-accelerated)."""""""        if rc and hasattr(rc, "count_tokens_rust"):  # pylint: disable=no-member"            try:
    ...
    def get_tokenizer(self, model_name: str) -> Any:
        """Get the appropriate tokenizer instance."""""""        return get_tokenizer(model_name)
    ...
    def apply_lora_adapters(self, base_model: Any, adapters: List[str]) -> Any:
        ...
        """""""        if rc and hasattr(rc, "apply_lora_rust"):  # pylint: disable=no-member"            try:
                # pylint: disable=no-member
