...
class DummyShell:
    ...
    class Res:
        """Mock response object for shell commands."""
        stdout = ""

    def __init__(self):
        """Initialize the dummy shell with an empty command log."""
        self.commands = []
        
    def execute(self, cmd):
        """Mock execute method that logs commands and simulates git operations."""
...
def test_commit_aborts_when_tests_fail(tmp_path):
    """Test that commit_changes aborts when enforce_tests is True and tests fail."""
...
def test_commit_succeeds_when_no_enforce(tmp_path):
    """Test that commit_changes succeeds when enforce_tests is False, even if tests fail."""
    handler = AgentGitHandler(tmp_path, no_git=False)
    handler.shell = DummyShell()
