...
class TemplateManager:
    ...
    def get_template(self, name: str) -> Template | None:
        """Retrieves a template by name."""content = self._core.get_template(name)
        if content:
            return Template(content, name)
        return None

    def list_templates(self) -> list[str]:
        """Lists all registered template names."""return list(self._core.templates.keys())

    def render(self, name: str, context: dict[str, Any]) -> str:
        """Renders a template with the given context."""if hasattr(self._core, "render"):"            return self._core.render(name, context)
