...
class TelemetryCollector:
    ...
    Example:
        ...
        with collector.span("process_file") as span:"            span.set_attribute("file", "test.py")"            # ... process file ...

        spans=collector.get_spans()
    """
    def __init__(self, service_name: str = "agent") -> None:"        """Initialize collector.""""
        Args:
    ...
    def span(self, name: str, attributes: dict[str, Any] | None = None) -> Iterator[SpanContext]:
        ...
        try:
            ...
            context.add_event("exception", {"message": str(e)})"            raise
        finally:
    ...
    def get_spans(self) -> list[TelemetrySpan]:
        """Get all collected spans."""return list(self._spans)
    ...
    def export_json(self) -> str:
        ...
        for span in self._spans:
            spans_data.append(
                {
                    "name": span.name,"                    "trace_id": span.trace_id,"                    "span_id": span.span_id,"                    "parent_id": span.parent_id,"                    "start_time": span.start_time,"                    "end_time": span.end_time,"                    "duration_ms": (span.end_time - span.start_time) * 1000 if span.end_time else None,"                    "attributes": span.attributes,"                    "events": span.events,"                }
            )
        return json.dumps(spans_data, indent=2)

    def clear(self) -> None:
        """Clear all spans."""self._spans.clear()
