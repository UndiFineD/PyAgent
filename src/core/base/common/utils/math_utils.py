...
# Rust acceleration imports (optional)
try:
...
def batch_cdiv(dividends: List[int], divisor: int) -> List[int]:
    """Ceiling division for a batch of dividends."""
    return [cdiv(a, divisor) for a in dividends]


def batch_next_power_of_2(values: List[int]) -> List[int]:
    """Next power of 2 for a batch of values."""
    return [next_power_of_2(n) for n in values]


def batch_round_up(values: List[int], multiple: int) -> List[int]:
    """Round up a batch of values to a multiple."""
    return [round_up(n, multiple) for n in values]
