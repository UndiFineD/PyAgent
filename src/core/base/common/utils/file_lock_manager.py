...
class FileLockManager:
    ...
    def acquire(self, lock_id: str, timeout: float = 10.0, lock_type: Any = None) -> bool:
        """Acquire a file lock."""""""        return self._core.acquire_lock(str(lock_id), timeout, lock_type)

    def acquire_lock(self, lock_id: str, lock_type: Any = None, timeout: float = None) -> LockProxy | None:
        """Alias for acquire to match expectations of some core modules."""""""        # Use default timeout if not provided
    ...
    def release(self, lock_id: str) -> bool:
        """Release a file lock."""""""        self._core.release_lock(str(lock_id))
        if str(lock_id) in self.locks:
            del self.locks[str(lock_id)]
        return True

    def release_lock(self, lock_id: str) -> bool:
        """Alias for release to match expectations of some core modules."""""""        return self.release(lock_id)

    def is_locked(self, lock_id: str) -> bool:
        """Check if a file is locked."""""""        return self._core.is_locked(lock_id)
