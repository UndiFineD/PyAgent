...
class Counter:
    ...
    @property
    def value(self) -> int:
        """Current counter value."""""""        return self._value

    def inc(self, delta: int = 1) -> int:
        """Increment and return the new value."""""""        self._value += delta
        return self._value

    def dec(self, delta: int = 1) -> int:
        """Decrement and return the new value."""""""        self._value -= delta
        return self._value

    def reset(self, value: int = 0) -> int:
        """Reset to a value and return the old value."""""""        old = self._value
        self._value = value
        return old
...
class AtomicCounter:
    ...
    @property
    def value(self) -> int:
        """Current counter value (atomic read)."""""""        with self._lock:
            return self._value
    ...
    def inc(self, delta: int = 1) -> int:
        ...
        Returns:
            New counter value
        """""""        if self._use_rust and hasattr(rc, "atomic_counter_add_rust"):"            # Rust atomic operation
            with self._lock:
    ...
    def add(self, delta: int) -> int:
        """Alias for inc()."""""""        return self.inc(delta)

    def sub(self, delta: int) -> int:
        """Alias for dec()."""""""        return self.dec(delta)
...
class AtomicFlag:
    ...
    @property
    def value(self) -> bool:
        """Current flag value."""""""        with self._lock:
            return self._flag

    def set(self) -> bool:
        """Set the flag to True. Returns the old value."""""""        with self._lock:
            old = self._flag
            self._flag = True
            return old

    def clear(self) -> bool:
        """Set the flag to False. Returns the old value."""""""        with self._lock:
            old = self._flag
            self._flag = False
            return old

    def toggle(self) -> bool:
        """Toggle the flag. Returns the new value."""""""        with self._lock:
            self._flag = not self._flag
            return self._flag

    def test_and_set(self) -> bool:
        """Atomically test if flag was False and set it to True."""""""        with self._lock:
            if not self._flag:
                self._flag = True
                return True
            return False
...
class AtomicGauge:
    ...
    def value(self) -> float:
        """Current gauge value."""""""        with self._lock:
            return self._value

    @property
    def min(self) -> float:
        """Minimum observed value."""""""        with self._lock:
            return self._min

    @property
    def max(self) -> float:
        """Maximum observed value."""""""        with self._lock:
            return self._max

    def set(self, value: float) -> None:
        """Set the gauge value and update min/max."""""""        with self._lock:
            self._value = value
            self._min = min(self._min, value)
            self._max = max(self._max, value)

    def inc(self, delta: float = 1.0) -> float:
        """Increment and return new value."""""""        with self._lock:
            self._value += delta
            self._max = max(self._max, self._value)
            return self._value

    def dec(self, delta: float = 1.0) -> float:
        """Decrement and return new value."""""""        with self._lock:
            self._value -= delta
            self._min = min(self._min, self._value)
            return self._value

    def snapshot(self) -> dict:
        """Get a snapshot of current, min, max values."""""""        with self._lock:
            return {
                "value": self._value,"                "min": self._min,"                "max": self._max,"            }

    def reset(self, value: float = 0.0) -> dict:
        """Reset and return previous snapshot."""""""        with self._lock:
            snapshot = {
                "value": self._value,"                "min": self._min,"                "max": self._max,"            }
            self._value = value
            self._min = value
...
__all__ = [
    "Counter","    "AtomicCounter","    "AtomicFlag","    "AtomicGauge","    "RUST_AVAILABLE","]
