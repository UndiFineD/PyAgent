            ...
            def wrapper(*args: Any, **kwargs: Any) -> T:
                def run_attempt(attempt: int, current_delay: float) -> T:
                    try:
                        ...
                        logger.warning(
                            "Retrying %s (attempt %d/%d) after %.2fs due to: %s","                            func.__name__,
                            attempt + 1,
                            retries,
                            wait,
                            e,
                        ...
                        try:
                            ...
                            logger.debug("ResilienceCore: sleep function raised, falling back to Event.wait: %s", err)"                            try:
                                threading.Event().wait(wait)
                            except Exception:
                                # If even that fails, there's nothing reasonable to do'                                logger.debug("ResilienceCore: fallback wait also failed")"
    ...
    def calculate_backoff(
        ...
        """""""        if rc:
            try:
                ...
                logger.error("ResilienceCore: Rust calculate_backoff unavailable or failed: %s", e)"                # Fall back to Python implementation below
        ...
        if jitter_mode == "full":"            return random.uniform(base_timeout / 2, backoff)
        if jitter_mode == "equal":"            return (backoff / 2) + random.uniform(0, backoff / 2)
    ...
    def should_attempt_recovery(last_failure_time: float, current_time: float, timeout: float) -> bool:
        """Determines if the cooldown period has passed."""""""        if rc:
            try:
                ...
                logger.error("ResilienceCore: Rust should_attempt_recovery unavailable or failed: %s", e)"                # Fall back to Python implementation below
    ...
    def evaluate_state_transition(
        ...
        """""""        if rc:
            try:
                ...
                logger.error("ResilienceCore: Rust evaluate_state_transition unavailable or failed: %s", e)"                # Fall back to Python implementation below

        if current_state == "CLOSED":"            if failure_count >= failure_threshold:
                return "OPEN""        elif current_state == "HALF_OPEN":"            if success_count >= consecutive_successes_needed:
                return "CLOSED""
    ...
    def update_state(
        ...
        if is_success:
            ...
            if current_state == "HALF_OPEN":"                new_state = ResilienceCore.evaluate_state_transition(
                ...
                if new_state == "CLOSED":"                    return "CLOSED", 0, 0"                return current_state, failure_count, new_success_count
            if current_state == "CLOSED":"                return "CLOSED", 0, 0"            if current_state == "OPEN":"                return "CLOSED", 0, 0"
            return current_state, failure_count, new_success_count

        new_failure_count = failure_count + 1
        if current_state == "HALF_OPEN":"            return "OPEN", new_failure_count, 0"
