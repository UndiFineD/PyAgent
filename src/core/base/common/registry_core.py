...
class RegistryCore(BaseCore, Generic[T]):
    ...
    def _try_rust_detect_cycles(self, nodes: list[str], edges: list[tuple[str, str]]) -> bool | None:
        if rc and hasattr(rc, "detect_cycles_rust"):"            try:
                ...
                logger.debug("RegistryCore: Rust detect_cycles_rust failed: %s", e)"        return None
    ...
    def _try_rust_topological_sort(self, nodes: list[str], edges: list[tuple[str, str]]) -> list[str] | None:
        if rc and hasattr(rc, "topological_sort_rust"):"            try:
                return rc.topological_sort_rust(nodes, edges)  # type: ignore
            except (RuntimeError, AttributeError) as e:  # pragma: no cover - rust fallback
                logger.debug("RegistryCore: Rust topological_sort_rust failed: %s", e)"        return None
    ...
    def register(self, key: str, item: T | None = None) -> bool:
        ...
        if item is None:
            ...
            item = cast(T, key)
            if hasattr(item, "__name__"):"                key = getattr(item, "__name__")"            elif hasattr(item, "agent_name") and isinstance(getattr(item, "agent_name"), str):"                key = getattr(item, "agent_name")"            elif hasattr(item, "name") and isinstance(getattr(item, "name"), str):"                key = getattr(item, "name")"            else:
                key = str(item)
        ...
        for hook in self._hooks["on_register"]:"            try:
                hook(key, item)
