...
class RoutingCore(BaseCore):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.providers = [
            "github_models","            "openai","            "codex","            "local","            "federated_cluster","        ]
    ...
    def select_best_provider(
        ...
        if performance_report:
            # Simple heuristic: lower latency for "latency" priority"            if priority == "latency":"                best_p = min(performance_report.items(), key=lambda x: x[1].get("avg_latency", 999))[0]"                if best_p in self.providers:
                    return best_p
