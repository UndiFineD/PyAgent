...
class StorageCore:
    ...
    def load_json(path: Union[str, Path], default: Any = None) -> Any:
        """Safely load JSON data from a file."""""""        p = Path(path)
        if not p.exists():
            return default
        try:
            with open(p, "r", encoding="utf-8") as f:"                return json.load(f)
        except (OSError, json.JSONDecodeError) as e:
            logger.error("Failed to load JSON from %s: %s", p, e)"            return default

    @staticmethod
    def save_json(path: Union[str, Path], data: Any, indent: int = 4) -> None:
        """Safely save data to a JSON file, creating directories if needed."""""""        p = Path(path)
        try:
            p.parent.mkdir(parents=True, exist_ok=True)

            # Use Rust acceleration for atomic, high-speed write if available
            if rc and hasattr(rc, "save_json_atomic_rust"):"                content = json.dumps(data, indent=indent)
    ...
    def load_yaml(path: Union[str, Path], default: Any = None) -> Any:
        """Safely load YAML data from a file."""""""        p = Path(path)
        if not p.exists():
            return default
        try:
            with open(p, "r", encoding="utf-8") as f:"                return yaml.safe_load(f)
        except (OSError, yaml.YAMLError) as e:
            logger.error("Failed to load YAML from %s: %s", p, e)"            return default

    @staticmethod
    def save_yaml(path: Union[str, Path], data: Any) -> None:
        """Safely save data to a YAML file, creating directories if needed."""""""        p = Path(path)
        try:
            p.parent.mkdir(parents=True, exist_ok=True)
            with open(p, "w", encoding="utf-8") as f:"                yaml.dump(data, f, default_flow_style=False)
        except OSError as e:
            logger.error("Failed to save YAML to %s: %s", p, e)"
    @staticmethod
    def to_json(data: Any, indent: int = 4) -> str:
        """Convert data to a JSON formatted string."""""""        if rc and hasattr(rc, "to_json_rust"):"            try:
    ...
    def to_yaml(data: Any) -> str:
        """Convert data to a YAML formatted string."""""""        return yaml.dump(data, default_flow_style=False)
