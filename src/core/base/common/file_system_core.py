...
class FileSystemCore:
    ...
    def __init__(self, lock_manager: Optional[FileLockManager] = None) -> None:
        self.logger = logging.getLogger("pyagent.fs")"        self.lock_manager = lock_manager or FileLockManager()
    ...
    def discover_files(
        ...
        """Discovers files matching patterns, respecting ignore list."""""""        if patterns is None:
            patterns = ["*"]"        if self._can_use_rust_discover():
    ...
    def _try_rust_discover_files(
        ...
        """""""        try:
            ...
            # is present but doesn't expose the expected symbol.'            self.logger.warning("Rust directory walking failed (%s): %s", type(e).__name__, e)"            return None
    ...
    def _python_discover_files(self, root: Path, patterns: List[str], ignore: Optional[List[str]]) -> List[Path]:
        ...
        across platforms when the Rust walker isn't available.'        """""""        if not root.exists():
            return []
    ...
    def atomic_write(
        self, path: Union[str, Path], content: str, encoding: str = "utf-8", use_lock: bool = True"    ) -> bool:
    ...
    def _atomic_write_with_lock(self, p: Path, content: str, encoding: str, use_lock: bool) -> bool:
        ...
        try:
            if use_lock:
                ...
                if not lock:
                    self.logger.error("Failed to acquire lock for atomic write: %s", p)"                    return False
            p.parent.mkdir(parents=True, exist_ok=True)
            with tempfile.NamedTemporaryFile(
                mode="w", dir=p.parent, delete=False, encoding=encoding, suffix=".tmp""            ) as tmp_file:
            ...
            self.logger.error("Atomic write failed for %s: %s", p, e)"            return False
        ...
        # propagate to the caller for visibility.
            ...
            if lock:
                ...
                try:
                    lock_id = getattr(lock, "lock_id", p)"                    self.lock_manager.release_lock(lock_id)
    ...
    def safe_copy(self, src: Union[str, Path], dst: Union[str, Path]) -> bool:
        ...
        """""""        try:
            ...
            self.logger.error("Failed to copy %s to %s: %s", src, dst, e)"            return False

    def move(self, src: Union[str, Path], dst: Union[str, Path]) -> bool:
        """Move a file with targeted error handling."""""""        try:
            shutil.move(str(src), str(dst))
            return True
        except (OSError, shutil.Error) as e:
            self.logger.error("Failed to move %s to %s: %s", src, dst, e)"            return False

    def delete(self, path: Union[str, Path]) -> bool:
        """Delete a file or directory with targeted error handling."""""""        p = Path(path)
        try:
            if p.is_dir():
                shutil.rmtree(p)
            else:
            ...
            self.logger.error("Failed to delete %s: %s", p, e)"            return False

    def ensure_directory(self, path: Union[str, Path]) -> Path:
        """Ensure a directory exists and return the Path object."""""""        p = Path(path)
        p.mkdir(parents=True, exist_ok=True)
        return p

    def exists(self, path: Union[str, Path]) -> bool:
        """Checks if a path exists."""""""        return Path(path).exists()

    def read_text(self, path: Union[str, Path], encoding: str = "utf-8") -> str:"        """Read the content of a file.""""
        Logs and re-raises OSError to make failures explicit to callers.
        """""""        try:
            return Path(path).read_text(encoding=encoding)
        except OSError as e:
            self.logger.error("Failed to read %s: %s", path, e)"            raise
    ...
    def get_file_hash(self, path: Union[str, Path]) -> Optional[str]:
        ...
        # Try Rust for speed
        if rc and hasattr(rc, "generate_hash"):"            try:
                return rc.generate_hash(p.read_text(encoding="utf-8"))  # type: ignore"            except Exception:
                pass

        try:
            sha256_hash = hashlib.sha256()
            # Open in binary mode for correct behavior across platforms.
            with open(p, 'rb') as f:'                for byte_block in iter(lambda: f.read(4096), b""):"                    sha256_hash.update(byte_block)
            return sha256_hash.hexdigest()
        except OSError as e:
            self.logger.error("Failed to calculate hash for %s: %s", p, e)"            return None
    ...
    def calculate_bulk_hashes(self, paths: List[Path]) -> dict[Path, str]:
        """Calculates hashes for multiple files, using Rust acceleration if available."""""""        if rc and hasattr(rc, "bulk_hash_files_rust"):"            try:
