...
class InputValidator:
    ...
    INJECTION_PATTERNS = [
        ...
        r'\$\([^)]*\)',  # Command injection
        r'`[^`]*`',  # Command injection
        r'\b(?:eval|exec|system|popen|subprocess)\b',  # Code execution
    ]
    ...
    def validate_resource_access(agent_id: str, resource_id: str, credentials: AgentCredentials) -> bool:
        """Validate if agent has access to resource (BOLA prevention)."""
        if agent_id != credentials.agent_id:
            return False
        # Check if resource belongs to agent's tenant
        if credentials.tenant_id and not resource_id.startswith(f"{credentials.tenant_id}_"):
            return False
        return True
...
class ErrorHandler:
    ...
    def mask_error(error: Exception) -> str:
        ...
        if "authentication" in str(error).lower() or "authorization" in str(error).lower():
            return f"Access denied: {error_type}"
...
class APISecurityCore:
    ...
    async def secure_communication(self, sender_id: str, receiver_id: str, message: Dict[str, Any], token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "resource_id" in sanitized_message:
                if not self.validator.validate_resource_access(sender_id, sanitized_message["resource_id"], creds):
                    self.error_handler.log_security_event(SecurityEvent(
                        timestamp=time.time(),
                        event_type="BOLA_ATTEMPT",
                        agent_id=sender_id,
                        details={"resource_id": sanitized_message["resource_id"]},
                        severity="ERROR"
                    ))
                    raise ValueError("Access to resource denied")

            # Authorize action
            action = sanitized_message.get("action", "")
            ...
            self.logger.error(f"Security violation in communication: {masked_error}")
            raise ValueError(masked_error) from e
