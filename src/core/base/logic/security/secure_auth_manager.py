...
@dataclass
class UserSession:
    """Authenticated user session with E2EE capabilities."""
    user_id: str
    oauth_token: str
    session_token: str
    created_at: float
...
class SecureAuthManager:
    ...
    def initiate_oauth_flow(self, provider: str = "github") -> Dict:
        """
    ...
    def decrypt_user_memory(self, session_token: str, encrypted_data: bytes) -> Optional[Dict]:
        """Decrypt user memory data using their personal encryption key."""
        session = self.verify_session(session_token)
        if not session or not session.e2e_enabled:
            return None
    ...
    def encrypt_user_chat(self, session_token: str, chat_data: Dict) -> Optional[bytes]:
        """Encrypt chat history with user's personal key."""
        session = self.verify_session(session_token)
        if not session:
            return None
    ...
    def decrypt_user_chat(self, session_token: str, encrypted_data: bytes) -> Optional[Dict]:
        """Decrypt chat history using user's personal key."""
        session = self.verify_session(session_token)
        if not session:
            return None
    ...
    def send_encrypted_message(
        ...
        if session_key not in self.e2e_core.sessions:
            ...
            recipient_bundle = self.e2e_core.get_public_prekey_bundle(recipient_user_id)
