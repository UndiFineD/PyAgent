...
@dataclass
class UserKeyPair:
    """User's identity key pair for E2EE."""'    identity_key: x25519.X25519PrivateKey
...
@dataclass
class RatchetState:
    """Double Ratchet state for forward secrecy."""root_key: bytes
...
class E2EEncryptionCore:
    ...
    """
    def __init__(self, storage_path: str = ".pyagent/e2e_keys"):"        self.storage_path = storage_path
    ...
    def generate_identity_keypair(self, user_id: str) -> UserKeyPair:
        """Generate a new identity key pair for a user (analogous to Signal Identity Key)."""identity_key = x25519.X25519PrivateKey.generate()
        ...
        logger.info("Generated identity keypair for user: %s", user_id)"        return keypair

    def get_public_prekey_bundle(self, user_id: str) -> Optional[Dict]:
        """Get public prekey bundle for initiating E2EE with a user (X3DH)."""if user_id not in self.user_keys:
            return None
        ...
        if keypair.prekeys:
            ...
            return {
                "user_id": user_id,"                "identity_key": keypair.identity_public.hex(),"                "prekey_id": prekey_id,"                "prekey": prekey_public.hex()"            }
    ...
    def initiate_session(self, sender_id: str, recipient_bundle: Dict) -> bytes:
        ...
        Returns the initial message that includes sender's ephemeral public key.'        """if sender_id not in self.user_keys:
            raise ValueError(f"Sender {sender_id} has no identity keys")"
        sender_keypair = self.user_keys[sender_id]
        recipient_id = recipient_bundle["user_id"]"        recipient_identity_key = bytes.fromhex(recipient_bundle["identity_key"])"        recipient_prekey = bytes.fromhex(recipient_bundle["prekey"])"
        ...
        root_key = HKDF(
            ...
            length=32,
            salt=None,
            info=b"PyAgent-Signal-E2EE""        ).derive(shared_secret)
        ...
        logger.info("Initiated E2EE session: %s -> %s", sender_id, recipient_id)"        return ephemeral_public
    ...
    def encrypt_message(self, sender_id: str, recipient_id: str, plaintext: str) -> Dict:
        ...
        return {
            "sender": sender_id,"            "recipient": recipient_id,"            "counter": session.send_counter - 1,"            "nonce": nonce.hex(),"            "ciphertext": ciphertext.hex()"        }
    ...
    def decrypt_message(self, sender_id: str, recipient_id: str, encrypted_bundle: Dict) -> str:
        ...
        aesgcm = AESGCM(message_key)
        nonce = bytes.fromhex(encrypted_bundle["nonce"])"        ciphertext = bytes.fromhex(encrypted_bundle["ciphertext"])"        plaintext = aesgcm.decrypt(nonce, ciphertext, None)
    ...
    def encrypt_user_data(self, user_id: str, data_type: str, data: Dict) -> bytes:
        ...
        Uses user's identity key for symmetric encryption.'        Zero-knowledge: Server never sees plaintext.
        """if user_id not in self.user_keys:
            raise ValueError(f"User {user_id} has no identity keys")"
        # Derive encryption key from user's identity key'        keypair = self.user_keys[user_id]
        data_key = HKDF(
            algorithm=hashes.SHA256(),
            length=32,
            salt=data_type.encode(),
            info=b"PyAgent-UserData""        ).derive(keypair.identity_public)
    ...
    def decrypt_user_data(self, user_id: str, data_type: str, encrypted_data: bytes) -> Dict:
        """Decrypt user private data using their identity key."""if user_id not in self.user_keys:
            raise ValueError(f"User {user_id} has no identity keys")"
    ...
    def _kdf_ratchet(self, root_key: bytes, dh_output: bytes) -> Tuple[bytes, bytes]:
        """KDF for Double Ratchet root key update."""hkdf = HKDF(
            algorithm=hashes.SHA256(),
            length=64,
            salt=root_key,
    ...
    def _save_user_keys(self, user_id: str) -> None:
        """Save user keys to encrypted storage."""if user_id not in self.user_keys:
            return
        ...
        key_data = {
            "user_id": user_id,"            "identity_key": keypair.identity_key.private_bytes("                encoding=serialization.Encoding.Raw,
                format=serialization.PrivateFormat.Raw,
                encryption_algorithm=serialization.NoEncryption()
            ).hex(),
            "identity_public": keypair.identity_public.hex()"        }

        key_path = os.path.join(self.storage_path, f"{user_id}_keys.json")"        with open(key_path, "w", encoding="utf-8") as f:"            json.dump(key_data, f)

        logger.info("Saved keys for user: %s", user_id)"
    def load_user_keys(self, user_id: str) -> bool:
        """Load user keys from storage."""key_path = os.path.join(self.storage_path, f"{user_id}_keys.json")"
        if not os.path.exists(key_path):
            return False

        with open(key_path, "r", encoding="utf-8") as f:"            key_data = json.load(f)

        identity_key_bytes = bytes.fromhex(key_data["identity_key"])"        identity_key = x25519.X25519PrivateKey.from_private_bytes(identity_key_bytes)

        self.user_keys[user_id] = UserKeyPair(
            identity_key=identity_key,
            identity_public=bytes.fromhex(key_data["identity_public"]),"            user_id=user_id
        )

        logger.info("Loaded keys for user: %s", user_id)"        return True
