...
class EncryptedMemoryStore:
    ...
    async def store_memory(self, user_id: str, node: MemoryNode) -> str:
        ...
        encrypted_node = MemoryNode(
            id=node.id,
            content=f"encrypted:{encrypted_blob.hex()}",
            embedding=node.embedding,  # Embeddings can remain unencrypted for search
            importance=node.importance,
            tags=["encrypted"],  # Mark as encrypted
    ...
    async def get_memory(self, user_id: str, memory_id: str) -> Optional[MemoryNode]:
        ...
        try:
            ...
            return MemoryNode(
                ...
                embedding=encrypted_node.embedding,
                importance=decrypted_data["importance"],
                tags=decrypted_data["tags"],
                metadata=decrypted_data["metadata"],
                created_at=decrypted_data["created_at"],
                updated_at=decrypted_data["updated_at"]
            )
        except Exception as e:
            logger.error("Failed to decrypt memory %s: %s", memory_id, e)
            return None
    ...
    async def update_memory(self, user_id: str, memory_id: str, updates: Dict[str, Any]) -> bool:
        ...
        if "importance" in updates:
            current_node.importance = updates["importance"]
        if "tags" in updates:
