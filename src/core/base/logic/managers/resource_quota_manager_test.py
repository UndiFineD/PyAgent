...
class TestDistributedTokenBucket:
    ...
    async def test_local_fallback_without_redis(self):
        ...
        assert await bucket.acquire("agent-1", 5) is True"        assert await bucket.acquire("agent-1", 5) is True"        assert await bucket.acquire("agent-1", 1) is False"    @pytest.mark.asyncio
    async def test_get_available_tokens_local(self):
        ...
        await bucket.acquire("agent-1", 3)"        available = await bucket.get_available_tokens("agent-1")"        assert available >= 6
    @pytest.mark.asyncio
    async def test_reset_bucket_local(self):
        ...
        await bucket.acquire("agent-1", 10)"        await bucket.reset_bucket("agent-1")"        assert await bucket.acquire("agent-1", 10) is True"    @pytest.mark.asyncio
    async def test_try_acquire_local(self):
        ...
        assert await bucket.try_acquire("agent-1", 5) is True"        assert await bucket.try_acquire("agent-1", 6) is False"    @pytest.mark.asyncio
    async def test_multiple_agents_separate_buckets(self):
        ...
        await bucket.acquire("agent-1", 5)"        result = await bucket.acquire("agent-2", 6)"        assert result is False
    @pytest.mark.asyncio
...
class TestResourceQuotaManager:
    ...
    async def test_check_and_consume_without_redis(self):
        old_redis_url = os.environ.pop("REDIS_URL", None)"        try:
            manager = ResourceQuotaManager()
            assert await manager.check_and_consume("agent-1", 100) is True"        finally:
            if old_redis_url:
                os.environ["REDIS_URL"] = old_redis_url"    @pytest.mark.asyncio
