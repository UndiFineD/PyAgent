...
class AuthManager:
    ...
    def generate_challenge(self, agent_id: str) -> str:
        """Generates a unique challenge regarding an agent."""return self._core.generate_challenge(agent_id)

    def generate_proof(self, challenge: str, secret_key: str) -> str:
        """Generates a proof regarding a challenge using a secret key."""return self._core.generate_proof(challenge, secret_key)

    def verify_proof(self, challenge: str, proof: str, secret_hash: str) -> bool:
        """Verifies proof against the secret hash."""return self._core.verify_proof(challenge, proof, secret_hash)

    def set_method(self, method: AuthMethod, **kwargs: Any) -> None:
        """Set the authentication method and credentials."""from src.core.base.common.models import AuthConfig
        # Capture credentials in an AuthConfig object
        self._config = AuthConfig(method=method, **kwargs)

    def get_headers(self) -> Dict[str, str]:
        """Get authentication headers based on current configuration."""if hasattr(self, "_config"):"            return self._core.get_auth_headers(self._config)
        return {}
