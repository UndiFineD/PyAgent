...
async def test_basic_functionality():
    """Test basic HybridMemoryCore functionality"""
    print("Testing HybridMemoryCore...")
    ...
    assert recalled is not None, "Memory recall failed"
    assert recalled.content == content, "Content mismatch"
    assert recalled.embedding == embedding, "Embedding mismatch"
    assert recalled.tags == tags, "Tags mismatch"
    assert recalled.importance == 0.8, "Importance mismatch"
    print("✓ Memory recall successful")

    # Test updating memory
    updates = {
        "content": "Updated: This is a test memory about Python programming",
        "importance": 0.9
    }
    ...
    assert updated.content == updates["content"], "Content update failed"
    assert updated.importance == 0.9, "Importance update failed"
    ...
    assert len(results) >= 2, "Search failed to find memories"
    print(f"✓ Search found {len(results)} memories")

    # Test memory graph
    graph = await memory_core.get_memory_graph(memory_id, max_depth=2)
    assert 'central_node' in graph, "Memory graph missing central node"
    assert 'related_nodes' in graph, "Memory graph missing related nodes"
    print("✓ Memory graph retrieval successful")

    # Test deletion
    success = await memory_core.delete_memory(memory_id)
    assert success, "Memory deletion failed"
    assert await memory_core.recall_memory(memory_id) is None, "Memory still exists after deletion"
