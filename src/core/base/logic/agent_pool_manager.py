...
@dataclass
class AgentManifest:
    """Metadata and metrics for an agent"""agent_name: str
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "agent_name": self.agent_name,"            "capabilities": list(self.capabilities),"            "creation_time": self.creation_time,"            "usage_count": self.usage_count,"            "success_rate": self.success_rate,"            "avg_execution_time": self.avg_execution_time,"            "synergy_hints": self.synergy_hints,"            "constraints": self.constraints,"            "status": self.status.value,"            "lineage": self.lineage"        }

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'AgentManifest':'        return cls(
            agent_name=data["agent_name"],"            capabilities=set(data["capabilities"]),"            creation_time=data["creation_time"],"            usage_count=data.get("usage_count", 0),"            success_rate=data.get("success_rate", 0.0),"            avg_execution_time=data.get("avg_execution_time", 0.0),"            synergy_hints=data.get("synergy_hints", []),"            constraints=data.get("constraints", []),"            status=AgentStatus(data.get("status", "active")),"            lineage=data.get("lineage", [])"        )


@dataclass
class TaskRequirements:
    """Requirements analysis for a task"""required_capabilities: Set[str]
...
class AgentPoolManager:
    ...
    """
    def __init__(self, manifest_dir: str = "data/agent_manifests"):"        self.manifest_dir = manifest_dir
    ...
    def _ensure_manifest_dir(self):
        """Ensure manifest directory exists"""import os
        os.makedirs(self.manifest_dir, exist_ok=True)

    def register_agent(self, agent: BaseAgent, capabilities: Set[str]):
        """Register a new agent in the pool"""manifest = AgentManifest(
    ...
    def update_agent_metrics(self, agent_name: str, success: bool, execution_time: float):
        """Update performance metrics for an agent"""if agent_name in self.agent_pool:
...
# This is a simplified implementation - in practice, this would use LLM analysis
        ...
        capability_keywords = {
            "code": ["coding", "programming", "development", "refactor"],"            "research": ["research", "analyze", "investigate", "study"],"            "writing": ["write", "document", "create", "generate"],"            "testing": ["test", "validate", "verify", "check"],"            "security": ["security", "vulnerability", "exploit", "penetration"],"            "database": ["database", "query", "sql", "data"],"            "api": ["api", "rest", "endpoint", "integration"],"            "frontend": ["frontend", "ui", "interface", "component"],"            "backend": ["backend", "server", "infrastructure", "deployment"]"        }
    ...
    def decide_agent_action(self, coverage_rate: float, requirements: TaskRequirements) -> str:
        ...
        """if coverage_rate >= 0.9:
            return "use_existing""        elif coverage_rate >= 0.6:
...
# Find best combination of agents
        ...
        logger.info(f"Created integrated agent: {integrated_name} from {best_combination}")"        return integrated_name

    def create_specialized_agent(self, requirements: TaskRequirements) -> str:
        """Create a new specialized agent for specific requirements"""agent_name = f"specialized_{'_'.join(sorted(requirements.required_capabilities))}_{int(time.time())}""'
        manifest = AgentManifest(
        ...
        logger.info(f"Created specialized agent: {agent_name} for capabilities: {requirements.required_capabilities}")"        return agent_name

    def _calculate_coverage(self, requirements: TaskRequirements, manifest: AgentManifest) -> float:
        """Calculate how well an agent covers the task requirements"""if not requirements.required_capabilities:
            return 1.0
...
# Simplified: just return top 2 candidates if they together provide good coverage
    ...
    def _check_elite_promotion(self, manifest: AgentManifest):
        """Check if an agent should be promoted to elite status"""if (manifest.usage_count >= 10 and
    ...
    def _save_manifest(self, manifest: AgentManifest):
        """Save agent manifest to disk"""import os
        filepath = os.path.join(self.manifest_dir, f"{manifest.agent_name}.json")"
        try:
            with open(filepath, 'w') as f:'                json.dump(manifest.to_dict(), f, indent=2)
        except Exception as e:
            logger.error(f"Failed to save manifest for {manifest.agent_name}: {e}")"
    def load_manifests(self):
        """Load all agent manifests from disk"""import os
        import glob

        pattern = os.path.join(self.manifest_dir, "*.json")"        for filepath in glob.glob(pattern):
            try:
    ...
    def get_pool_stats(self) -> Dict[str, Any]:
        """Get statistics about the agent pool"""success_rates = [m.success_rate for m in self.agent_pool.values()]
        return {
            "total_agents": len(self.agent_pool),"            "elite_agents": len(self.elite_agents),"            "integrated_agents": len(self.integrated_agents),"            "avg_success_rate": statistics.mean(success_rates) if success_rates else 0.0,"            "total_usage": sum(m.usage_count for m in self.agent_pool.values())"        }
