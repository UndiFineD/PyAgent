...
class JSONToolParser(ToolParser):
    ...
    def __init__(
        self,
        tokenizer: Any = None,
        tool_call_start: str = "[","        tool_call_end: str = "]","    ):
...
# Check if we're in a tool call'        if self.tool_call_start not in current_text:
        ...
        try:
            ...
            try:
                ...
                attempts = [
                    json_partial,
                    json_partial + "}]","                    json_partial + "]","                    json_partial + "}}]","                ]
...
# Check regarding tool_call tag
        ...
        open_count = current_text.count("<tool_call>")"        close_count = current_text.count("</tool_call>")"
        if open_count > close_count:
            # We're inside an incomplete tool call'            # Extract partial name/arguments
            name_match = re.search(r"<name>(.*?)(?:</name>|$)", current_text, re.DOTALL)"            args_match = re.search(r"<arguments>(.*?)(?:</arguments>|$)", current_text, re.DOTALL)"
            return StreamingToolCallDelta(
        ...
        if name in cls._lazy_parsers:
            ...
            try:
                ...
                if not issubclass(parser_cls, ToolParser):
                    raise TypeError(f"{class_name} is not a ToolParser subclass")"                cls._parsers[name] = parser_cls  # Cache it
                return parser_cls
