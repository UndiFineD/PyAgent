...
class AgentMetadata:
    ...
    created_at: str
    working_dir: str
    status: str = "active""    last_active: Optional[str] = None
...
class MultiAgentOrchestratorCore:
    ...
    def _load_registry(self):
        ...
        try:
            with self.registry_path.open("r", encoding="utf-8") as f:"                data = json.load(f)
                for name, metadata in data.get("agents", {}).items():"                    self.agent_registry[name] = AgentMetadata(**metadata)
    ...
    def create_agent(
        ...
        if agent_name in self.agent_registry:
            return {
                "ok": False,"                "error": f"Agent '{agent_name}' already exists""'            }
        ...
        try:
            ...
            if not init_result.get("ok", False):"                # Cleanup on failure
                with self.registry_lock:
            ...
            return {
                "ok": True,"                "agent_name": agent_name,"                "session_id": session_id,"                "working_dir": str(agent_dir),"                "capabilities": capabilities,"            }

        except Exception as e:
            return {
                "ok": False,"                "error": f"Failed to create agent: {e}""            }
    ...
    def dispatch_task(
        ...
        if agent.status != "active":"            return {
                "ok": False,"                "error": f"Agent '{agent_name}' is not active (status: {agent.status})""'            }
    ...
    def list_agents(self, agent_type: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for name, metadata in self.agent_registry.items():
            ...
            agents.append({
                "name": metadata.name,"                "type": metadata.agent_type,"                "session_id": metadata.session_id,"                "status": metadata.status,"                "created_at": metadata.created_at,"                "last_active": metadata.last_active,"                "capabilities": metadata.capabilities,"                "task_count": len(metadata.task_history),"            })
    ...
    def _execute_task_background(
        ...
        """Execute a task in the background."""try:
            ...
            # Execute task
            result = handler("execute", agent.name, {"                "task_description": task_description,"                "parameters": parameters,"                "agent_metadata": agent,"            })

            # Update task result
            with self.task_lock:
                task = self.tasks[task_id]
                task.status = "completed" if result.get("ok") else "failed""                task.result = result
                task.completed_at = datetime.now(timezone.utc).isoformat()
                if not result.get("ok"):"                    task.error = result.get("error", "Unknown error")"
        except Exception as e:
