...
class TestAgentPoolManager:
    ...
    def test_initialization(self):
        assert len(self.manager.agent_pool) == 0
    ...
    def test_task_requirements_analysis(self):
        requirements = self.manager.analyze_task_requirements(
            "Write a Python function to parse JSON data and handle database connections",
            None
        )
    ...
    def test_agent_registration(self):
        ...
        self.manager.register_agent(mock_agent, capabilities)
        assert "test_agent" in self.manager.agent_pool
        manifest = self.manager.agent_pool["test_agent"]
        assert manifest.capabilities == capabilities
        assert manifest.status == AgentStatus.ACTIVE
    def test_agent_metrics_update(self):
        mock_agent = Mock()
        mock_agent.name = "metrics_agent"
        self.manager.register_agent(mock_agent, {"coding"})
        self.manager.update_agent_metrics("metrics_agent", True, 2.0)
        self.manager.update_agent_metrics("metrics_agent", False, 1.5)
        self.manager.update_agent_metrics("metrics_agent", True, 3.0)
        manifest = self.manager.agent_pool["metrics_agent"]
        assert manifest.usage_count == 3
        assert manifest.success_rate == 2/3
        assert abs(manifest.avg_execution_time - 2.17) < 0.1
    def test_elite_promotion(self):
        mock_agent = Mock()
        mock_agent.name = "elite_candidate"
        self.manager.register_agent(mock_agent, {"coding"})
        for i in range(15):
            self.manager.update_agent_metrics("elite_candidate", True, 1.0)
        manifest = self.manager.agent_pool["elite_candidate"]
        assert manifest.status == AgentStatus.ELITE
        assert "elite_candidate" in self.manager.elite_agents
    def test_agent_selection(self):
        agents = [
            ("coder_agent", {"coding", "python"}),
            ("writer_agent", {"writing", "documentation"}),
            ("fullstack_agent", {"coding", "python", "writing", "testing"})
        ]
        ...
        assert best_agent == "fullstack_agent"
        assert coverage == 1.0
    ...
    def test_integrated_agent_creation(self):
        ...
        assert integrated_name.startswith("integrated_")
        assert integrated_name in self.manager.integrated_agents
        manifest = self.manager.integrated_agents[integrated_name]
        assert manifest.capabilities == {"coding", "testing"}
        assert manifest.lineage == ["agent_a", "agent_b"]
    def test_specialized_agent_creation(self):
        ...
        assert agent_name.startswith("specialized_")
        assert agent_name in self.manager.agent_pool
        manifest = self.manager.agent_pool[agent_name]
        assert manifest.capabilities == {"security", "networking"}
        assert manifest.status == AgentStatus.ACTIVE
    def test_manifest_persistence(self):
        ...
        assert "persistent_agent" in new_manager.agent_pool
        manifest = new_manager.agent_pool["persistent_agent"]
        assert manifest.capabilities == {"coding"}
    def test_pool_statistics(self):
        ...
        for name, caps in agents:
            ...
            mock_agent.name = name
            self.manager.register_agent(mock_agent, caps)
        self.manager.update_agent_metrics("agent1", True, 1.0)
        self.manager.update_agent_metrics("agent2", False, 2.0)
        stats = self.manager.get_pool_stats()
        assert stats["total_agents"] == 3
        assert stats["elite_agents"] == 0
        assert stats["integrated_agents"] == 0
