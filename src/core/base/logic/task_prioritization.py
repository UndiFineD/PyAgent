...
class TaskManager:
    ...
    def update_task(self, task_id: str, **updates) -> Optional[Task]:
        """Update an existing task."""with self._lock:
            ...
            if 'priority' in updates:'                self._requeue_task(task)
...
# Create task manager
    ...
    coder_agent = create_agent_capability(
        "coder_001", "Code Generator","        skills=[TaskType.CODE_GENERATION, TaskType.CODE_REVIEW],
    ...
    researcher_agent = create_agent_capability(
        "researcher_001", "Research Specialist","        skills=[TaskType.RESEARCH, TaskType.ANALYSIS],
    ...
    urgent_task = create_task(
        "Fix critical security vulnerability","        "Address the XSS vulnerability in login form","        TaskType.CODE_GENERATION,
    ...
    research_task = create_task(
        "Research AI trends","        "Analyze latest developments in AI for Q1 report","        TaskType.RESEARCH,
