...
class EfficientChangeDetector:
    ...
    def detect_changes(self) -> List[ChangeRecord]:
        ...
        for path_str, current_meta in current_metadata.items():
            if path_str not in self.metadata_cache:
                ...
                changes.append(ChangeRecord(
                    path=path_str,
                    change_type='created','                    timestamp=time.time(),
                    usn=self.current_usn,
                ...
                if (current_meta.mtime > cached_meta.mtime or
                    ...
                    changes.append(ChangeRecord(
                        path=path_str,
                        change_type='modified','                        timestamp=time.time(),
                        usn=self.current_usn,
                        metadata={
                            'old_size': cached_meta.size,'                            'new_size': current_meta.size,'                            'old_mtime': cached_meta.mtime,'                            'new_mtime': current_meta.mtime'                        }
                    ))
        ...
        for path_str, cached_meta in self.metadata_cache.items():
            if path_str not in current_metadata:
                changes.append(ChangeRecord(
                    path=path_str,
                    change_type='deleted','                    timestamp=time.time(),
                    usn=self.current_usn,
    ...
    def get_change_statistics(self) -> Dict[str, Any]:
        ...
        return {
            'total_changes': len(self.change_history),'            'change_types': change_types,'            'time_range': {'                'oldest': min(c.timestamp for c in self.change_history),'                'newest': max(c.timestamp for c in self.change_history)'            },
