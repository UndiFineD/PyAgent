...
class TestStrategyOptimizer:
    ...
    async def test_threshold_filtering(self):
        ...
        # Provide ground truth that matches the "good" strategy's expected output
        ground_truth = "Result from good: test"
        trial = await optimizer.optimize(strategies, "test", ground_truth=ground_truth)

        # Only the "good" strategy should pass threshold filtering
    ...
    async def test_pareto_frontier_selection(self):
        ...
        strategies = [
            MockStrategy("dominated", {"accuracy": 0.7, "latency": 0.5}, 0.5),  # Dominated
            MockStrategy("pareto1", {"accuracy": 0.8, "latency": 0.3}, 0.3),    # Pareto optimal
            MockStrategy("pareto2", {"accuracy": 0.9, "latency": 0.4}, 0.4),    # Pareto optimal
        ]
...
class TestPerformanceMeasurer:
    ...
    async def test_performance_measurement(self, measurer):
        ...
        assert result.execution_time >= 0.05  # At least the delay
