...
class TestReflectionLoopConfig:
    ...
    def test_default_config(self):
        ...
        assert config.critique_prompt.startswith("You are a senior software engineer")
...
class TestLLMReflectionAgent:
    ...
    def test_initial_generation(self):
        """Test initial content generation."""
        mock_llm = Mock(return_value="Generated content")
        agent = LLMReflectionAgent(mock_llm)
    ...
    def test_refinement_generation(self):
        ...
        mock_llm = Mock(return_value="Refined content")
        agent = LLMReflectionAgent(mock_llm)
    ...
    def test_critique_generation(self):
        ...
        mock_llm = Mock(return_value="This content needs improvement.")
        agent = LLMReflectionAgent(mock_llm)

        context = ReflectionContext(task_description="Test task")

        result = asyncio.run(agent.critique(context, "Test content"))
        assert result == "This content needs improvement."
        mock_llm.assert_called_once()
...
class TestCodeReflectionAgent:
    ...
    def test_code_generation(self):
        ...
        mock_llm = Mock(return_value="```python\nprint('hello')\n```")
    ...
    def test_code_critique(self):
        ...
        mock_llm = Mock(return_value="CODE_IS_PERFECT")
        agent = CodeReflectionAgent(mock_llm)

        context = ReflectionContext(task_description="Write factorial function")

        result = asyncio.run(agent.critique(context, "def factorial(n): return n * factorial(n-1) if n > 0 else 1"))
        assert result == "CODE_IS_PERFECT"
...
class TestReflectionLoopOrchestrator:
    ...
    def mock_agents(self):
        ...
        generator.critique = AsyncMock(return_value="Good content")
        generator.name = "Generator Agent"

        critic = Mock(spec=LLMReflectionAgent)
        critic.critique = AsyncMock(return_value="Good content")
        critic.name = "Critic Agent"
    ...
    def test_get_final_result(self, mock_agents):
        ...
        context = ReflectionContext(task_description="Test")
        context.history = [
            ReflectionResult(iteration=1, content="Content 1", critique="Good", is_satisfactory=True),
            ReflectionResult(iteration=2, content="Content 2", critique="Better", is_satisfactory=False)
        ]
    ...
    def test_get_reflection_summary(self, mock_agents):
        ...
        assert summary["final_content"] == "Content 1"
...
class TestConvenienceFunctions:
    ...
    def test_reflect_on_code(self):
        ...
        assert "factorial" in result["final_content"]
    ...
    def test_reflect_on_content(self):
        ...
        assert result["total_iterations"] == 1
        assert "summary" in result["final_content"]
...
class TestReflectionResult:
    ...
    def test_result_creation(self):
        ...
        assert result.critique == "Good work"
        assert result.is_satisfactory is True
...
class TestReflectionContext:
    ...
    def test_context_creation(self):
        """Test creating a reflection context."""
        context = ReflectionContext(task_description="Test task")
