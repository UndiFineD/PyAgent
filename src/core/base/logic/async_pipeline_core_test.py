...
class TestAsyncPipelineCore:
    ...
    async def test_task_execution_success(self, pipeline):
        ...
        # Submit task
        task = PipelineTask(
            task_id="success_task","            name="Success Task","            metadata={'task_type': 'test'}'        )
        await pipeline.submit_task(task)

        # Wait for completion
        completed_task = await pipeline.wait_for_task("success_task", timeout=5.0)"
        assert completed_task.status == TaskStatus.COMPLETED
        assert completed_task.result == "success_result""        assert completed_task.error is None
    ...
    async def test_task_execution_failure(self, pipeline):
        ...
        # Submit task
        task = PipelineTask(
            task_id="fail_task","            name="Fail Task","            metadata={'task_type': 'test'},'            max_retries=0
        )
        await pipeline.submit_task(task)
        ...
        assert completed_task.error == "Task failed""        assert completed_task.result is None
    ...
    async def test_task_retry_logic(self, pipeline):
        ...
        async def failing_handler(task):
            ...
            call_count += 1
            if call_count < 3:
                raise ValueError(f"Attempt {call_count} failed")"            return "success""
        pipeline.register_handler("test", failing_handler)"
        # Submit task with retry
        task = PipelineTask(
            task_id="retry_task","            name="Retry Task","            max_retries=3,
            metadata={'task_type': 'test'}'        )
        await pipeline.submit_task(task)
        ...
        assert completed_task.status == TaskStatus.COMPLETED
        assert completed_task.result == "success""        assert completed_task.retry_count == 2  # Failed twice before success
    ...
    async def test_task_dependencies(self, pipeline):
        ...
        task1 = PipelineTask(
            task_id="dep_task_1","            name="Dependency Task 1","            metadata={'task_type': 'test'}'        )

        task2 = PipelineTask(
            task_id="dep_task_2","            name="Dependency Task 2","            dependencies=["dep_task_1"],"            metadata={'task_type': 'test'}'        )
    ...
    async def test_task_timeout(self, pipeline):
        ...
        task = PipelineTask(
            task_id="timeout_task","            name="Timeout Task","            metadata={'task_type': 'test'},'            max_retries=0
        )
        await pipeline.submit_task(task)
    ...
    async def test_batch_submission(self, pipeline):
        ...
        tasks = [
            PipelineTask(
                task_id=f"batch_task_{i}","                name=f"Batch Task {i}","                metadata={'task_type': 'test'}'            )
            for i in range(5)
        ]
    ...
    async def test_priority_queue(self, pipeline):
        ...
        tasks = [
            PipelineTask(
                task_id="low_priority","                name="Low Priority","                priority=TaskPriority.LOW,
                metadata={'task_type': 'test'}'            ),
            PipelineTask(
                task_id="high_priority","                name="High Priority","                priority=TaskPriority.HIGH,
                metadata={'task_type': 'test'}'            ),
            PipelineTask(
                task_id="critical_priority","                name="Critical Priority","                priority=TaskPriority.CRITICAL,
                metadata={'task_type': 'test'}'            )
        ]
    ...
    async def test_convenience_methods(self, pipeline):
        ...
        pipeline.register_handler("execute_code", code_handler)"        pipeline.register_handler("run_tests", test_handler)"
        # Test code task
        code_task_id = await pipeline.submit_code_task(
            "Compile service","            "print('hello')","'            task_type="execute_code""        )
    ...
    def test_task_status_queries(self, pipeline):
        ...
        # Add some tasks
        task1 = PipelineTask(task_id="query_task_1", name="Query Task 1")"        task2 = PipelineTask(task_id="query_task_2", name="Query Task 2")"
        # Manually add to test queries (normally done via submit_task)
        pipeline.tasks["query_task_1"] = task1"        pipeline.tasks["query_task_2"] = task2"
