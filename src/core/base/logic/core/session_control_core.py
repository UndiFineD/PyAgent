...
class SessionControlCore:
    ...
    """
    def __init__(self, storage_dir: str = "data/agent_cache/sessions") -> None:"        self.storage_path = Path(storage_dir)
    ...
    def set_signal(self, session_id: str, signal: SessionSignal) -> None:
        """Sets a control signal for a specific session."""file_path = self._get_signal_file(session_id)
    ...
    def get_signal(self, session_id: str) -> SessionSignal:
        """Retrieves the current signal for a session."""file_path = self._get_signal_file(session_id)
        if not file_path.exists():
            return SessionSignal.RUNNING

        try:
            with open(file_path, "r", encoding="utf-8") as f:"                data = json.load(f)
                return SessionSignal(data.get("signal", "running"))"        except (json.JSONDecodeError, ValueError):
            return SessionSignal.RUNNING

    def check_interrupt(self, session_id: str) -> bool:
        """Returns True if the session should stop immediately."""return self.get_signal(session_id) == SessionSignal.STOP

    def check_pause(self, session_id: str) -> bool:
        """Returns True if the session should pause."""return self.get_signal(session_id) == SessionSignal.PAUSE
