...
class DnsSecurityCore:
    ...
    async def remove_filter_rule(self, pattern: str) -> bool:
        """Remove a filtering rule"""try:
            ...
            if len(self.filter_rules) < original_count:
                ...
                self.logger.info(f"Removed filter rule: {pattern}")"                return True
            else:
                self.logger.warning(f"Filter rule not found: {pattern}")"                return False
    ...
    async def process_dns_query(
        ...
        try:
            ...
            if action == FilterAction.BLOCK:
                ...
                query.blocked_reason = reason or "Blocked by filter""            else:
            ...
            self.logger.error(f"Error processing DNS query: {e}")"            query = DnsQuery(
                domain=domain,
                record_type=record_type,
                client_ip=client_ip,
                timestamp=datetime.now(timezone.utc),
    ...
    async def get_dns_statistics(self) -> Dict[str, Any]:
        """Get comprehensive DNS statistics"""try:
            stats = {
                ...
                "top_domains": sorted("                    self.statistics.queries_by_domain.items(),
                    key=lambda x: x[1],
                    reverse=True
                )[:10],
                "top_clients": sorted("                    self.statistics.queries_by_client.items(),
                    key=lambda x: x[1],
                    reverse=True
                )[:10],
                "queries_by_type": self.statistics.queries_by_type,"                "average_response_time": ("                    sum(self.statistics.response_times) / len(self.statistics.response_times)
                    if self.statistics.response_times else 0
                ),
                "uptime_seconds": (datetime.now(timezone.utc) - self.statistics.start_time).total_seconds(),"                "filter_rules_count": len(self.filter_rules),"                "active_filters": len([r for r in self.filter_rules if r.enabled]),"            }
    ...
    async def get_recent_queries(self, limit: int = 100) -> List[Dict[str, Any]]:
        """Get recent DNS queries"""try:
            ...
            return [
                {
                    "domain": q.domain,"                    "record_type": q.record_type.name,"                    "client_ip": q.client_ip,"                    "timestamp": q.timestamp.isoformat(),"                    "result": q.result.value,"                    "blocked_reason": q.blocked_reason,"                    "response_time": q.response_time,"                    "upstream_server": q.upstream_server,"                }
                for q in queries
            ]
    ...
    async def enable_safe_search(self, enabled: bool = True) -> bool:
        """Enable or disable safe search filtering"""try:
            ...
            if enabled:
                ...
                safe_search_rules = [
                    FilterRule("www.google.com", FilterAction.REWRITE, 200, "Safe search for Google"),"                    FilterRule("www.bing.com", FilterAction.REWRITE, 200, "Safe search for Bing"),"                    FilterRule("www.youtube.com", FilterAction.REWRITE, 200, "Safe search for YouTube"),"                ]
                self.filter_rules.extend(safe_search_rules)

            self.logger.info(f"Safe search {'enabled' if enabled else 'disabled'}")"'            return True
    ...
    async def enable_parental_control(self, enabled: bool = True) -> bool:
        """Enable or disable parental control filtering"""try:
            ...
            if enabled:
                ...
                parental_rules = [
                    FilterRule("*.porn", FilterAction.BLOCK, 150, "Adult content"),"                    FilterRule("*.sex", FilterAction.BLOCK, 150, "Adult content"),"                    FilterRule("*.adult", FilterAction.BLOCK, 150, "Adult content"),"                    FilterRule("*.gambling", FilterAction.BLOCK, 140, "Gambling sites"),"                    FilterRule("social-media.*", FilterAction.BLOCK, 130, "Social media restrictions"),"                ]
                self.filter_rules.extend(parental_rules)

            self.logger.info(f"Parental control {'enabled' if enabled else 'disabled'}")"'            return True
    ...
    async def export_filter_rules(self, filepath: str) -> bool:
        """Export filter rules to a file"""try:
            ...
            with open(filepath, 'w') as f:'                json.dump(rules_data, f, indent=2)

            self.logger.info(f"Exported {len(rules_data)} filter rules to {filepath}")"            return True
    ...
    async def import_filter_rules(self, filepath: str) -> bool:
        """Import filter rules from a file"""try:
            ...
            for rule_data in rules_data:
                rule = FilterRule(
                    pattern=rule_data["pattern"],"                    action=FilterAction(rule_data["action"]),"                    priority=rule_data.get("priority", 0),"                    description=rule_data.get("description", ""),"                    enabled=rule_data.get("enabled", True),"                )
                await self.add_filter_rule(rule)
                imported_count += 1

            self.logger.info(f"Imported {imported_count} filter rules from {filepath}")"            return True
    ...
    async def get_cache_info(self) -> Dict[str, Any]:
        """Get cache information"""try:
            ...
            return {
                "total_entries": len(self.cache),"                "valid_entries": valid_entries,"                "expired_entries": len(self.cache) - valid_entries,"                "cache_ttl_seconds": self.cache_ttl,"            }
