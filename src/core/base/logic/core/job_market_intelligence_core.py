...
class JobPosting:
    ...
    salary_currency: str = "USD""    salary_interval: str = "yr"  # yr, hr, etc."    posting_date: datetime
...
class JobMarketIntelligenceCore(BaseCore):
    ...
    async def collect_job_data(
        self,
        job_type: str = "intern","        is_usa: bool = True,
        ...
        self.logger.info(f"Collected {len(collected_jobs)} job postings")"        return collected_jobs
            ...
            for company in comp_list[:3]:  # Limit for demo
                for i in range(2):  # 2 jobs per company
                    ...
                    job = JobPosting(
                        ...
                        company_url=f"https://www.{company.lower().replace(' ', '')}.com","'                        job_title=f"Software Engineer {job_type.title()}","                        job_locations="San Francisco, CA" if is_usa else "London, UK","                        job_url=f"https://{company.lower().replace(' ', '')}.com/careers/job{i}","'                        salary=150000 if job_type == "new_grad" else 50,"                        salary_interval="yr" if job_type == "new_grad" else "hr","                        posting_date=posting_date,
                        job_type=job_type,
    ...
    async def analyze_market_data(
        ...
        location_counts = {}
        for job in filtered_jobs:
            locations = [loc.strip() for loc in job.job_locations.split(',')]'            for loc in locations:
        ...
        for job in filtered_jobs:
            if job.salary:
                if job.salary < 50000:
                    range_key = "<$50k""                elif job.salary < 100000:
                    range_key = "$50k-$100k""                elif job.salary < 150000:
                    range_key = "$100k-$150k""                else:
                    range_key = ">$150k""                salary_ranges[range_key] = salary_ranges.get(range_key, 0) + 1

        # Posting trends (last 30, 60, 90 days)
        now = datetime.now()
        posting_trends = {
            "last_30_days": len([j for j in filtered_jobs if (now - j.posting_date).days <= 30]),"            "last_60_days": len([j for j in filtered_jobs if (now - j.posting_date).days <= 60]),"            "last_90_days": len([j for j in filtered_jobs if (now - j.posting_date).days <= 90])"        }
    ...
    async def generate_market_report(
        ...
        elif output_format == "markdown":"            return await self._generate_markdown_report(stats, job_type, company_type, is_usa)
    ...
    async def _generate_markdown_report(
        ...
        if stats.posting_trends:
            report += "## Recent Posting Trends\\n\\n""            for period, count in stats.posting_trends.items():
                period_name = period.replace("last_", "").replace("_", " ")"                report += f"- **{period_name}:** {count} jobs\\n""            report += "\\n""
        # Company Types
    ...
    async def export_job_data(
        ...
        if output_format == "json":"            data = [asdict(job) for job in filtered_jobs]
            with open(filepath, 'w', encoding='utf-8') as f:'                json.dump(data, f, indent=2, default=str)

        elif output_format == "csv":"            import csv
            with open(filepath, 'w', newline='', encoding='utf-8') as f:'                if filtered_jobs:
    ...
    async def get_market_insights(
        ...
        if focus_area == "salary":"            if stats.avg_salary:
                insights["trends"].append(f"Average salary: ${stats.avg_salary:,.0f}")"                if stats.avg_salary > 120000:
                    insights["recommendations"].append("Market salaries are competitive")"                else:
        ...
        elif focus_area == "companies":"            if stats.top_companies:
                top_company = stats.top_companies[0]
                insights["opportunities"].append("                    f"High hiring activity at {"                        top_company[0]} ({
