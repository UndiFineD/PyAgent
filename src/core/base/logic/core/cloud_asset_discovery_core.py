...
class CloudAssetDiscoveryCore:
    ...
    def __init__(self, max_concurrent: int = 100, timeout: int = 4):
        ...
        self.db_path = "certificates.db"
        self._init_database()

        # Common cloud provider IP ranges (simplified)
        self.cloud_ranges = {
            "aws": [
                "52.0.0.0/8", "54.0.0.0/8", "13.0.0.0/8", "3.0.0.0/8",
                "18.0.0.0/8", "34.0.0.0/8", "35.0.0.0/8", "52.0.0.0/8"
            ],
            "azure": [
                "13.64.0.0/11", "13.96.0.0/13", "20.0.0.0/8", "40.0.0.0/8",
                "104.0.0.0/8", "168.61.0.0/16", "168.62.0.0/15"
            ],
            "gcp": [
                "8.8.0.0/16", "8.34.208.0/20", "23.236.0.0/16", "23.251.128.0/19",
                "34.64.0.0/10", "35.184.0.0/13", "35.192.0.0/14", "35.196.0.0/15"
            ]
        }

    def _init_database(self):
        """Initialize SQLite database for certificate storage"""
    ...
    async def _scan_ip_certificate(self, ip: str, port: int, keywords: List[str],
        ...
        async with semaphore:
            try:
                ...
                return AssetFinding(
                    ip_address=ip,
                    domain=cert_info.subject.get('CN', ip),
        ...
        try:
            ...
            with socket.create_connection((ip, port), timeout=self.timeout) as sock:
                with context.wrap_socket(sock, server_hostname=ip) as ssl_sock:
                    ...
                    subject_alt_names = []
                    if san_extension:
                        san_value = san_extension.get('value', '')
                        # Simple parsing of SAN extension
                        san_matches = re.findall(r'DNS:([^,\s]+)', san_value)
                        subject_alt_names.extend(san_matches)

                    # Extract organizations
                    organizations = []
                    for field in ['organizationName', 'O']:
                        if field in subject:
                            organizations.append(subject[field])

                    return CertificateInfo(
                        domain=subject.get('CN', ip),
                        ip_address=ip,
                        port=port,
                        subject=subject,
                        issuer=issuer,
            ...
            self.logger.debug(f"Failed to get certificate from {ip}:{port}: {e}")
            return None

    def _check_keywords(self, cert_info: CertificateInfo, keywords: List[str]) -> List[str]:
        """Check if certificate contains any of the specified keywords"""
        matched = []
        search_text = ' '.join([
            cert_info.domain,
            ' '.join(cert_info.subject_alt_names),
            ' '.join(cert_info.organizations),
            ' '.join(cert_info.subject.values()),
        ]).lower()
    ...
    def _classify_asset(self, cert_info: CertificateInfo, keywords: List[str]) -> str:
        """Classify the type of asset based on certificate information"""
        domain_lower = cert_info.domain.lower()

        # Development/staging indicators
        if any(word in domain_lower for word in ['dev', 'staging', 'test', 'qa', 'internal']):
            return "development"

        # API endpoints
        if 'api' in domain_lower or domain_lower.endswith('.api'):
            return "api_endpoint"

        # Admin panels
        if any(word in domain_lower for word in ['admin', 'manage', 'console', 'dashboard']):
            return "admin_panel"

        # Cloud storage
        if any(word in domain_lower for word in ['s3', 'blob', 'storage', 'cdn']):
    ...
    def _calculate_confidence(self, cert_info: CertificateInfo, keywords: List[str]) -> str:
        """Calculate confidence level for asset discovery"""
        confidence_score = 0
    ...
    async def _store_certificate(self, cert_info: CertificateInfo):
        ...
        try:
            ...
            ''', (
                ...
                ','.join(cert_info.subject_alt_names),
                cert_info.valid_from.isoformat(),
    ...
    async def search_stored_certificates(self, keywords: List[str],
        ...
        try:
            ...
            # Organization filter
            if organizations:
                org_conditions = ["organizations LIKE ?" for _ in organizations]
                conditions.append(f"({' OR '.join(org_conditions)})")
                params.extend([f'%{org}%' for org in organizations])
            ...
            for row in rows:
                cert = CertificateInfo(
                    ...
                    ip_address=row[1],
                    port=row[2],
                    subject={'CN': row[5]},  # Simplified
                    issuer={},  # Not stored
                    valid_from=datetime.fromisoformat(row[7]),
                    valid_until=datetime.fromisoformat(row[8]),
                    subject_alt_names=row[6].split(',') if row[6] else [],
                    organizations=row[4].split(',') if row[4] else [],
                    serial_number=row[9],
            ...
            self.logger.error(f"Failed to search certificates: {e}")
            return []
