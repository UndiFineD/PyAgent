...
class ActiveDirectoryThreatHuntingCore(BaseCore):
    ...
    async def load_threat_detection_rules(self) -> None:
        """Load threat detection rules and indicators"""
    ...
    async def initialize_baselines(self) -> None:
        ...
        self.baseline_data = {
            "max_group_membership": 50,
            "max_privileged_accounts": 10,
            "password_expiry_days": 90,
            "account_lockout_threshold": 5,
            "stale_account_threshold_days": 90,
            "service_account_indicators": ["svc-", "service-", "-svc", "-service"]
        }
    ...
    async def enumerate_ad_objects(
        ...
        for obj_data in mock_objects:
            ad_object = ADObject(
                distinguished_name=obj_data["dn"],
                object_class=obj_data["type"],
                sam_account_name=obj_data.get("sam_account_name"),
                user_principal_name=obj_data.get("upn"),
                member_of=obj_data.get("member_of", []),
                permissions=obj_data.get("permissions", {}),
                last_logon=obj_data.get("last_logon"),
                password_last_set=obj_data.get("password_last_set"),
                account_status=obj_data.get("status", "active")
            )

            self.ad_objects[ad_object.distinguished_name] = ad_object
            enumerated_objects.append(ad_object)

        self.logger.info(f"Enumerated {len(enumerated_objects)} AD objects")
    ...
    async def _generate_mock_ad_objects(self, object_types: List[ADObjectType]) -> List[Dict[str, Any]]:
            ...
            users = [
                {
                    ...
                    "permissions": {"GenericAll": True},
                    "last_logon": datetime.now() - timedelta(days=1),
                    "password_last_set": datetime.now() - timedelta(days=30),
                    "status": "active"
                },
                {
                    ...
                    "permissions": {},
                    "last_logon": datetime.now() - timedelta(days=5),
                    "password_last_set": datetime.now() - timedelta(days=60),
                    "status": "active"
                },
                {
                    "dn": "CN=svc-sql,CN=Users,DC=domain,DC=com",
                    "type": ADObjectType.USER,
                    "sam_account_name": "svc-sql",
                    "upn": "svc-sql@domain.com",
                    "member_of": ["Service Accounts"],
                    "permissions": {"ServicePrincipalNames": ["MSSQLSvc/server.domain.com"]},
                    "last_logon": datetime.now() - timedelta(days=1),
                    "password_last_set": datetime.now() - timedelta(days=365),
                    "status": "active"
                }
            ]
        ...
        if ADObjectType.COMPUTER in object_types:
            ...
            computers = [
                {
                    "dn": "CN=WORKSTATION01,CN=Computers,DC=domain,DC=com",
                    "type": ADObjectType.COMPUTER,
                    "sam_account_name": "WORKSTATION01$",
                    "last_logon": datetime.now() - timedelta(days=2),
                    "status": "active"
                }
            ]
            ...
            groups = [
                {
                    ...
                    "type": ADObjectType.GROUP,
                    "sam_account_name": "Domain Admins",
                    "member_of": ["Administrators"],
                    "permissions": {"GenericAll": True}
                }
            ]
    ...
    async def perform_threat_hunt(
        ...
        if hunt_type == "privileged_accounts":
            findings.extend(await self._hunt_privileged_accounts())
        elif hunt_type == "stale_accounts":
            findings.extend(await self._hunt_stale_accounts())
        elif hunt_type == "service_accounts":
            findings.extend(await self._hunt_service_accounts())
        elif hunt_type == "suspicious_permissions":
            findings.extend(await self._hunt_suspicious_permissions())
        elif hunt_type == "weak_security":
            findings.extend(await self._hunt_weak_security())
        else:
        ...
        result = HuntingResult(
            scan_id=f"hunt_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            scan_type=hunt_type,
            findings=findings,
        ...
        self.logger.info(f"Completed threat hunt '{hunt_type}': {len(findings)} findings, risk score: {risk_score:.2f}")
        return result

    async def _hunt_privileged_accounts(self) -> List[ThreatFinding]:
        """Hunt for privileged accounts and excessive permissions"""
        findings = []
                ...
                if privileged_memberships:
                    ...
                    findings.append(ThreatFinding(
                        ...
                        evidence={
                            ...
                            "last_logon": obj.last_logon.isoformat() if obj.last_logon else None
                        },
                        recommendations=[
                            "Review necessity of privileged access",
    ...
    async def _hunt_stale_accounts(self) -> List[ThreatFinding]:
        """Hunt for stale and inactive accounts"""
        findings = []
                ...
                if days_since_logon > threshold_days:
                    findings.append(ThreatFinding(
                        ...
                        recommendations=[
                            ...
                            "Review account lifecycle management",
                            "Implement automated account cleanup"
                        ]
                    ))
    ...
    async def _hunt_service_accounts(self) -> List[ThreatFinding]:
        """Hunt for service accounts and their security posture"""
        findings = []
        ...
        for obj in self.ad_objects.values():
            if obj.object_class == ADObjectType.USER:
                ...
                if is_service_account:
                    ...
                    # Check for SPNs
                    if "ServicePrincipalNames" in obj.permissions:
                        spns = obj.permissions["ServicePrincipalNames"]
                        if not spns:
    ...
    async def _hunt_suspicious_permissions(self) -> List[ThreatFinding]:
        """Hunt for suspicious permissions and access rights"""
        findings = []
    ...
    async def _hunt_weak_security(self) -> List[ThreatFinding]:
        """Hunt for weak security configurations"""
        findings = []
        ...
        for obj in self.ad_objects.values():
            if obj.object_class == ADObjectType.USER:
                ...
                if found_weaknesses:
                    findings.append(ThreatFinding(
                        id=f"weak_sec_{obj.sam_account_name}",
                        title="Weak Security Configuration",
                        description=f"Account has weak security settings: {', '.join(found_weaknesses)}",
                        threat_level=ThreatLevel.HIGH,
                        affected_objects=[obj.distinguished_name],
                        mitre_technique="T1110.003",  # Brute Force: Password Spraying
                        evidence={
                            "weak_configurations": found_weaknesses,
                            "account_status": obj.account_status
                        },
                        recommendations=[
                            "Enable pre-authentication requirements",
    ...
    async def _perform_general_hunt(self) -> List[ThreatFinding]:
        """Perform a general threat hunt combining multiple techniques"""
        findings = []
    ...
    async def generate_security_report(
        ...
        if output_format == "json":
            report_data: Dict[str, Any] = {
                ...
                "hunting_sessions": len(self.hunting_history),
            ...
            for hunt in self.hunting_history[-5:]:
                report_data["recent_hunts"].append({
                    "scan_id": hunt.scan_id,
                    "scan_type": hunt.scan_type,
                    "findings_count": len(hunt.findings),
                    "risk_score": hunt.risk_score,
                    "execution_time": hunt.execution_time
                })
                ...
                data["hunting_history"] = [
                    {
                        ...
                        "scanned_objects": h.scanned_objects,
    ...
    async def get_hunting_statistics(self) -> Dict[str, Any]:
        ...
        if self.hunting_history:
            ...
            stats["hunt_performance"] = {
                "average_execution_time": avg_execution_time,
                "average_risk_score": avg_risk_score,
                "average_findings_per_hunt": avg_findings
            }
