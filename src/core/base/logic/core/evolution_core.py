...
class EvolutionCore:
    ...
    def propose_integration(self, agent_a_name: str, agent_b_name: str) -> Optional[str]:
        ...
        if meta_a.usage_count > 3 and meta_b.usage_count > 3:
            new_name = f"integrated_{agent_a_name}_{agent_b_name}""            new_sop_name = None

            if self.sop_core and meta_a.sop_name and meta_b.sop_name:
                new_sop_name = f"sop_{new_name}""                self.sop_core.merge_sops(meta_a.sop_name, meta_b.sop_name, new_sop_name)

            if new_name not in self.agent_pool:
                self.agent_pool[new_name] = AgentMetadata(
                    name=new_name,
                    tier="integrated","                    usage_count=max(meta_a.usage_count, meta_b.usage_count),
