...
class AttackTechnique(Enum):
    ...
    RESOURCE_BASED_CONSTRAINED_DELEGATION = "resource_based_constrained_delegation"
    GPO_ABUSE = "gpo_abuse"
    ACL_ABUSE = "acl_abuse"
    TRUST_EXPLOITATION = "trust_exploitation"
    LAPS_ABUSE = "laps_abuse"
    SYSVOL_CREDENTIALS = "sysvol_credentials"
    DNSADMIN_ESCALATION = "dnsadmin_escalation"
...
class AttackVector:
    ...
    mitigation_complexity: str = "medium"
...
class DefenseAssessment:
    ...
    control: DefenseControl
    implemented: bool = False
    effectiveness: str = "unknown"  # high, medium, low, unknown
    coverage: str = "partial"  # full, partial, minimal, none
...
class ActiveDirectoryAttackDefenseCore:
    ...
    async def load_attack_vectors(self) -> None:
        """Load comprehensive attack vectors database"""
        self.attack_vectors = {
            AttackTechnique.KERBEROASTING: AttackVector(
                technique=AttackTechnique.KERBEROASTING,
                phase=KillChainPhase.RECONNAISSANCE,
                prerequisites=["Domain user account", "Service accounts with SPNs"],
                impact="Credential harvesting for offline cracking",
                detection_difficulty="low",
                mitigation_complexity="medium",
                affected_components=["Kerberos", "Service Accounts"],
                indicators=[
                    "TGS-REQ requests for service accounts",
                    "Unusual account enumeration patterns",
                    "Offline password cracking attempts"
                ],
                mitigations=[
                    "Use strong, complex service account passwords",
            ...
            AttackTechnique.ASREP_ROASTING: AttackVector(
                ...
                phase=KillChainPhase.RECONNAISSANCE,
                prerequisites=["Accounts with DONT_REQ_PREAUTH flag"],
                impact="Credential harvesting without pre-authentication",
                detection_difficulty="low",
                mitigation_complexity="low",
                affected_components=["Kerberos", "Account Settings"],
                indicators=[
                    "AS-REQ requests without pre-authentication",
                    "Accounts with DONT_REQ_PREAUTH set"
                ],
                mitigations=[
                    "Remove DONT_REQ_PREAUTH from accounts",
                    "Use strong passwords for vulnerable accounts",
                    "Monitor Kerberos pre-authentication failures"
                ]
            ),
            ...
            AttackTechnique.UNCONSTRAINED_DELEGATION: AttackVector(
                ...
                prerequisites=["Unconstrained delegation configured on computers"],
                impact="Complete domain compromise via ticket forwarding",
                detection_difficulty="medium",
                mitigation_complexity="medium",
                affected_components=["Kerberos", "Computer Objects"],
                indicators=[
                    "TRUSTED_FOR_DELEGATION flag on computer accounts",
                    "TGT forwarding to unauthorized services"
                ],
                mitigations=[
            ...
            AttackTechnique.CONSTRAINED_DELEGATION: AttackVector(
                ...
                phase=KillChainPhase.PRIVILEGE_ESCALATION,
                prerequisites=["Constrained delegation configured", "Service account compromise"],
                impact="Service ticket impersonation",
                detection_difficulty="high",
                mitigation_complexity="medium",
                affected_components=["Kerberos", "Service Accounts"],
                indicators=[
                    "S4U2self/S4U2proxy usage",
                    "Service tickets for unauthorized services"
                ],
                mitigations=[
                    "Use resource-based constrained delegation",
                    "Limit service ticket scope",
                    "Monitor S4U extensions usage"
                ]
            ),
            ...
            AttackTechnique.GPO_ABUSE: AttackVector(
                ...
                prerequisites=["GPO edit permissions", "Domain user account"],
                impact="Persistent code execution via group policy",
                detection_difficulty="medium",
                mitigation_complexity="low",
                affected_components=["Group Policy", "SYSVOL"],
                indicators=[
                    "Unauthorized GPO modifications",
                    "New scheduled tasks in GPOs",
                    "Modified login scripts"
                ],
                mitigations=[
                    "Restrict GPO creation/modification permissions",
                    "Audit GPO changes",
                    "Use GPO delegation carefully"
                ]
            ),
            ...
            AttackTechnique.ACL_ABUSE: AttackVector(
                ...
                phase=KillChainPhase.PRIVILEGE_ESCALATION,
                prerequisites=["Excessive permissions on AD objects"],
                impact="Privilege escalation via access rights",
                detection_difficulty="high",
                mitigation_complexity="high",
                affected_components=["Access Control Lists", "AD Objects"],
                indicators=[
                    "GenericAll/GenericWrite permissions on sensitive objects",
                    "DCSync rights on non-admin accounts"
                ],
                mitigations=[
            ...
            AttackTechnique.DCSYNC: AttackVector(
                ...
                prerequisites=["Replicating Directory Changes permissions"],
                impact="Complete domain credential extraction",
                detection_difficulty="medium",
                mitigation_complexity="medium",
                affected_components=["Directory Replication", "Domain Controllers"],
                indicators=[
                    "DS-Replication-Get-Changes requests",
                    "Unusual replication traffic"
                ],
                mitigations=[
            ...
            AttackTechnique.GOLDEN_TICKET: AttackVector(
                ...
                prerequisites=["KRBTGT hash compromise"],
                impact="Persistent domain-wide access",
                detection_difficulty="high",
                mitigation_complexity="medium",
                affected_components=["Kerberos", "Domain Controllers"],
                indicators=[
                    "Tickets with unrealistic lifetimes",
                    "TGT requests without recent AS requests"
                ],
                mitigations=[
                    "Regular KRBTGT password changes",
                    "Monitor for anomalous ticket patterns",
            ...
            AttackTechnique.TRUST_EXPLOITATION: AttackVector(
                ...
                phase=KillChainPhase.PRIVILEGE_ESCALATION,
                prerequisites=["Domain/forest trust relationships"],
                impact="Cross-domain/forest compromise",
                detection_difficulty="medium",
                mitigation_complexity="high",
                affected_components=["Domain Trusts", "Forest Trusts"],
                indicators=[
                    "SID history usage",
                    "Cross-domain ticket requests"
                ],
                mitigations=[
                    "Selective authentication on trusts",
                    "SID filtering",
            ...
            AttackTechnique.LAPS_ABUSE: AttackVector(
                ...
                phase=KillChainPhase.CREDENTIAL_DUMPING,
                prerequisites=["LAPS read permissions on computer objects"],
                impact="Local admin password retrieval",
                detection_difficulty="low",
                mitigation_complexity="low",
                affected_components=["LAPS", "Computer Objects"],
                indicators=[
                    "LDAP queries for ms-Mcs-AdmPwd attribute",
                    "Access to LAPS passwords"
                ],
                mitigations=[
                    "Restrict LAPS password read permissions",
                    "Audit LAPS password access",
                    "Use just-in-time access for local admin"
                ]
            )
        }
    ...
    async def initialize_defense_assessments(self) -> None:
        """Initialize defense control assessments"""
        for control in DefenseControl:
    ...
    async def assess_security_posture(
        ...
        posture = SecurityPosture(
            assessment_id=f"posture_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            domain=domain,
        ...
        self.logger.info(f"Completed security posture assessment for {domain}: score {overall_score:.1f}")
        return posture

    async def analyze_attack_vectors(self, ad_config: Dict[str, Any]) -> List[AttackVector]:
        """Analyze applicable attack vectors based on AD configuration"""
        applicable_vectors = []
        ...
        if ad_config.get("unconstrained_delegation_enabled", True):
            applicable_vectors.append(self.attack_vectors[AttackTechnique.UNCONSTRAINED_DELEGATION])

        # Check for service accounts
        if ad_config.get("service_accounts_exist", True):
            applicable_vectors.append(self.attack_vectors[AttackTechnique.KERBEROASTING])

        # Check for pre-auth disabled accounts
        if ad_config.get("asrep_roastable_accounts", True):
            applicable_vectors.append(self.attack_vectors[AttackTechnique.ASREP_ROASTING])

        # Check for GPO permissions
        if ad_config.get("weak_gpo_permissions", True):
            applicable_vectors.append(self.attack_vectors[AttackTechnique.GPO_ABUSE])

        # Check for ACL issues
        if ad_config.get("excessive_acl_permissions", True):
            applicable_vectors.append(self.attack_vectors[AttackTechnique.ACL_ABUSE])

        # Check for domain trusts
        if ad_config.get("domain_trusts_exist", True):
            applicable_vectors.append(self.attack_vectors[AttackTechnique.TRUST_EXPLOITATION])

        # Check for LAPS
        if ad_config.get("laps_deployed", False):
            applicable_vectors.append(self.attack_vectors[AttackTechnique.LAPS_ABUSE])

        return applicable_vectors

    async def assess_defenses(self, ad_config: Dict[str, Any]) -> Dict[DefenseControl, DefenseAssessment]:
        """Assess implemented defensive controls"""
        assessments = {}

        # Multi-factor authentication
        assessments[DefenseControl.MULTI_FACTOR_AUTH] = DefenseAssessment(
            control=DefenseControl.MULTI_FACTOR_AUTH,
            implemented=ad_config.get("mfa_enabled", False),
            effectiveness="high" if ad_config.get("mfa_enabled") else "low",
            coverage="full" if ad_config.get("mfa_enabled") else "none",
            last_assessed=datetime.now(),
            gaps=["MFA not enforced for service accounts"] if not ad_config.get("mfa_enabled") else [],
        ...
        assessments[DefenseControl.LEAST_PRIVILEGE] = DefenseAssessment(
            ...
            implemented=ad_config.get("least_privilege_implemented", False),
            effectiveness="medium",
            coverage="partial",
            last_assessed=datetime.now(),
            gaps=["Domain Admins group has excessive members"],
            recommendations=["Implement tiered administration", "Regular privilege audits"]
        )

        # Logging and monitoring
        assessments[DefenseControl.LOGGING_MONITORING] = DefenseAssessment(
            control=DefenseControl.LOGGING_MONITORING,
            implemented=ad_config.get("advanced_audit_enabled", False),
            effectiveness="high" if ad_config.get("siem_deployed") else "medium",
            coverage="full" if ad_config.get("siem_deployed") else "partial",
            last_assessed=datetime.now(),
            gaps=["No centralized log collection"] if not ad_config.get("siem_deployed") else [],
            recommendations=["Deploy SIEM solution", "Enable advanced audit policies"]
        )

        # Patch management
        assessments[DefenseControl.PATCH_MANAGEMENT] = DefenseAssessment(
            control=DefenseControl.PATCH_MANAGEMENT,
            implemented=ad_config.get("patch_management_active", True),
            effectiveness="medium",
            coverage="partial",
            last_assessed=datetime.now(),
            gaps=["Delayed patching of domain controllers"],
            recommendations=["Automate patch deployment", "Regular vulnerability scanning"]
        )
    ...
    async def analyze_kill_chains(self, attack_vectors: List[AttackVector]) -> List[KillChainAnalysis]:
        ...
        analysis = KillChainAnalysis(
            analysis_id=f"killchain_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            domain="example.com",
            ...
            attack_path=[
                "Initial reconnaissance via SPN scanning",
                "Kerberoasting to obtain service account hashes",
                "Password cracking to gain service account access",
    ...
    async def identify_critical_gaps(
        ...
        for vector in attack_vectors:
            if vector.detection_difficulty == "low" and vector.mitigation_complexity == "low":
                gaps.append(f"High-risk attack vector not properly mitigated: {vector.technique.value}")
    ...
    async def generate_priority_actions(
        ...
        for gap in critical_gaps:
            if "attack vector" in gap:
                actions.append("Implement mitigations for high-risk attack vectors")
            elif "defense" in gap:
                actions.append("Deploy critical security controls")
        ...
        actions.extend([
            ...
            "Train staff on AD security best practices",
    ...
    async def simulate_attack_chain(
        ...
        chain = KillChainAnalysis(
            analysis_id=f"sim_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            domain="simulation",
            start_time=datetime.now(),
            ...
            risk_score=5.0,
            attack_path=[
                f"Started with {start_technique.value}",
                "Escalated privileges",
                "Moved laterally",
                "Achieved domain admin access"
            ],
            recommendations=[
                "Block initial attack vector",
    ...
    async def generate_attack_report(
        ...
        if format == "json":
            report_data = {
                ...
                "assessment_date": posture.assessment_date.isoformat(),
                "overall_score": posture.overall_score,
                "attack_vectors": [
                    {
                        "technique": av.technique.value,
                        "phase": av.phase.value,
                        "impact": av.impact,
                        "detection_difficulty": av.detection_difficulty,
                        "mitigations": av.mitigations
                    }
                    for av in posture.attack_vectors
                ],
                "defense_assessments": [
                    {
                        "control": da.control.value,
                        "implemented": da.implemented,
                ...
                "critical_gaps": posture.critical_gaps,
                "priority_actions": posture.priority_actions
            }
            ...
            if posture.defense_assessments:
                ...
                for da in posture.defense_assessments:
                    status = "✅ Implemented" if da.implemented else "❌ Not Implemented"
                    report += f"### {da.control.value.replace('_', ' ').title()}\n\n"
    ...
    async def get_attack_statistics(self) -> Dict[str, Any]:
        ...
        stats: Dict[str, Any] = {
            ...
            "kill_chain_analyses": len(self.kill_chain_analyses),
            "most_common_attacks": [],
