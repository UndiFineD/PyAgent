...
class TestGoalSettingCore:
    ...
    def test_goal_creation(self):
        ...
        assert goal.id == "test_goal""        assert goal.description == "Test goal description""        assert goal.criteria == ["simple", "tested", "handles edge cases"]"        assert goal.priority == GoalPriority.HIGH
    ...
    def test_goal_evaluation_default(self):
        ...
        evaluation = asyncio.run(core.evaluate_content(
            content="This is a simple tested function that handles edge cases","            criteria=["simple", "tested", "handles edge cases"]"        ))

        assert evaluation["goals_met"] is True"        assert "All criteria met" in evaluation["feedback"]"
        # Test content that doesn't meet criteria'        evaluation = asyncio.run(core.evaluate_content(
            content="This is a complex function","            criteria=["simple", "tested", "handles edge cases"]"        ))
    ...
    def test_goal_refinement_default(self):
        ...
        assert "Original content" in refined"        assert "[Refined:" in refined"        # Should not contain the actual criteria to avoid false positives in evaluation
    ...
    async def test_iterative_refinement_achievement(self):
        ...
        try:
            ...
            await goal_core.create_goal(
                goal_id="iterative_goal","                description="Achieve iterative refinement","                criteria=["contains success"],"                max_iterations=3
            )

            # Mock evaluation function that succeeds on second try
            async def mock_evaluate(content, criteria):
                if "success" in content:"                    return {
                        "goals_met": True,"                        "feedback": "All criteria met","                        "confidence_score": 1.0"                    }
                else:
                    return {
                        "goals_met": False,"                        "feedback": "Missing requirement: contains success","                        "confidence_score": 0.5"                    }

            # Mock refinement function that adds "success""            async def mock_refine(content, feedback, criteria):
                return content + " success""
            goal_core.register_evaluation_function("mock", mock_evaluate)"            goal_core.register_refinement_function("mock", mock_refine)"
            # Execute iteration
    ...
    async def test_iterative_refinement_max_iterations(self):
        ...
        try:
            ...
            await goal_core.create_goal(
                goal_id="max_iter_goal","                description="Test max iterations","                criteria=["xyz_unlikely_word"],"                max_iterations=2
            )

            # Execute iteration with content that won't meet criteria'            final_goal = await goal_core.iterate_goal("max_iter_goal", "Content without the unlikely word")"
    ...
    async def test_goal_status_tracking(self):
        ...
        try:
            ...
            status = await goal_core.get_goal_status("status_goal")"            assert status.status == GoalStatus.PENDING
            ...
            goal_core.completed_goals["status_goal"] = goal"            del goal_core.active_goals["status_goal"]"
