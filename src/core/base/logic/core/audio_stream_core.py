...
class AudioStreamCore:
    ...
    def convert_ulaw_to_pcm(self, ulaw_data: bytes) -> bytes:
        """Converts u-law coded audio (8kHz, 8-bit) to linear PCM."""pcm_data = audioop.ulaw2lin(ulaw_data, self.target_width)
        return pcm_data

    def resample(self, pcm_data: bytes, source_rate: int) -> bytes:
        """Resamples PCM audio to the target sample rate."""if source_rate == self.target_sample_rate:
            return pcm_data
    ...
    def normalize_volume(self, pcm_data: bytes, target_rms: int = 2000) -> bytes:
        """Normalizes audio volume to a target RMS level."""rms = audioop.rms(pcm_data, self.target_width)
        if rms == 0:
            return pcm_data
    ...
    def detect_voice(self, pcm_data: bytes, threshold: int = 500) -> bool:
        """Simple RMS-based voice activity detection."""rms = audioop.rms(pcm_data, self.target_width)
