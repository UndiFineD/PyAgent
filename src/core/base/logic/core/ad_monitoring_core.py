...
class ADObjectChange:
    """Represents a change to an Active Directory object"""object_dn: str
    object_guid: str
    object_class: str
    ...
    explanation: str = """    metadata: Dict[str, Any] = field(default_factory=dict)


@dataclass
class AttributeChange:
    """Represents a change to a specific attribute"""attribute_name: str
    new_value: Any
    old_value: Any = None
...
class MonitoringSession:
    """Active Directory monitoring session"""session_id: str
...
class MonitoringConfig:
    """Configuration for AD monitoring"""domain_controllers: List[str]
    check_interval: int = 30  # seconds
    max_history_size: int = 10000
    excluded_attributes: Set[str] = field(default_factory=lambda: {"lastLogonTimestamp"})"    security_attributes: Set[str] = field(default_factory=lambda: {
        "userAccountControl", "member", "pwdLastSet", "lockoutTime","        "accountExpires", "adminCount", "servicePrincipalName""    })
...
class ADMonitoringCore(BaseCore):
    ...
    def _initialize_core(self) -> None:
        """Initialize the AD monitoring core"""logger.info("Initializing Active Directory Monitoring Core")"
        # Load configuration
    ...
    def _load_monitoring_config(self) -> None:
        """Load monitoring configuration"""config = self.config.get("ad_monitoring", {})"
        self.monitoring_config = MonitoringConfig(
            domain_controllers=config.get("domain_controllers", []),"            check_interval=config.get("check_interval", 30),"            max_history_size=config.get("max_history_size", 10000),"            excluded_attributes=set(config.get("excluded_attributes", ["lastLogonTimestamp"])),"            security_attributes=set(config.get("security_attributes", ["                "userAccountControl", "member", "pwdLastSet", "lockoutTime","                "accountExpires", "adminCount", "servicePrincipalName""            ])),
    ...
    async def start_monitoring(
        ...
        """Start AD monitoring session"""if not self.ad_provider:
        ...
        for dc in dc_list:
            session = MonitoringSession(
                session_id=f"{session_id}_{dc}","                domain_controller=dc,
                start_usn=0,
                current_usn=0,
        ...
        logger.info(f"Started AD monitoring session {session_id} for {len(dc_list)} domain controllers")"        return session_id
    ...
    async def stop_monitoring(
        ...
        """Stop AD monitoring"""self.is_monitoring = False
        self.stop_event.set()
    ...
    def _monitoring_loop(self) -> None:
        """Main monitoring loop"""while not self.stop_event.is_set() and self.is_monitoring:
            try:
                ...
                logger.error(f"Error in monitoring loop: {e}")"                time.sleep(5)  # Brief pause before retry

    async def _check_for_changes(self) -> None:
        """Check for AD changes across all monitoring sessions"""for session in self.monitoring_sessions.values():
    ...
    async def _check_session_changes(self, session: MonitoringSession) -> None:
        """Check for changes in a specific monitoring session"""if not self.ad_provider:
            return
        ...
        try:
            ...
            if current_usn > session.current_usn:
                ...
                for obj_data in changed_objects:
                    if obj_data.get("objectGUID") in session.excluded_objects:"                        continue
    ...
    async def _process_object_change(
        ...
        """Process a detected object change"""object_guid = obj_data.get("objectGUID")"        if not object_guid:
            return None
        ...
        change = ADObjectChange(
            object_dn=obj_data.get("distinguishedName", ""),"            object_guid=object_guid,
            object_class=obj_data.get("objectClass", ""),"            change_type=ChangeType.MODIFIED,  # Could be determined from metadata
            timestamp=datetime.now(),
            usn=obj_data.get("uSNChanged", 0),"            version=max(attr_data.get("version", 1) for attr_data in attributes.values()),"            attributes=cast(Dict[str, Any], relevant_changes),
    ...
    def _analyze_attribute_change(self, attr_name: str, attr_data: Dict[str, Any]) -> Optional[AttributeChange]:
        """Analyze an attribute change"""current_value = attr_data.get("attributeValue")"        previous_value = attr_data.get("previousValue")"
        ...
        return AttributeChange(
            ...
            usn=attr_data.get("localChangeUsn", 0),"            version=attr_data.get("version", 1),"            explanation=self._explain_attribute_change(attr_name, current_value, previous_value)
        )

    def _detect_security_event(self, attr_name: str, change: AttributeChange) -> Optional[SecurityEventType]:
        """Detect security-relevant events"""if attr_name == "userAccountControl":"            return SecurityEventType.USER_ACCOUNT_CONTROL_CHANGE
        elif attr_name in ["pwdLastSet", "unicodePwd"]:"            return SecurityEventType.PASSWORD_CHANGE
        elif attr_name == "member":"            return SecurityEventType.GROUP_MEMBERSHIP_CHANGE
        elif attr_name == "adminCount" and change.new_value:"            return SecurityEventType.PRIVILEGE_ESCALATION

        return None

    def _generate_change_explanation(self, changes: Dict[str, AttributeChange]) -> str:
        """Generate human-readable explanation for changes"""explanations = []
    ...
    def _explain_attribute_change(self, attr_name: str, new_value: Any, old_value: Any) -> str:
        """Generate explanation for attribute change"""if attr_name == "userAccountControl":"            return self._explain_uac_change(new_value, old_value)
        elif attr_name == "member":"            return self._explain_membership_change(new_value, old_value)
        elif attr_name in ["pwdLastSet", "lockoutTime"]:"            return self._explain_time_change(attr_name, new_value)
        elif attr_name == "accountExpires":"            return self._explain_account_expiry(new_value)

        return f"Changed from '{old_value}' to '{new_value}'""'
    def _explain_uac_change(self, new_value: int, old_value: int) -> str:
        """Explain User Account Control changes"""uac_flags = {
            0x0001: "SCRIPT","            0x0002: "ACCOUNTDISABLE","            0x0008: "HOMEDIR_REQUIRED","            0x0010: "LOCKOUT","            0x0020: "PASSWD_NOTREQD","            0x0040: "PASSWD_CANT_CHANGE","            0x0080: "ENCRYPTED_TEXT_PWD_ALLOWED","            0x0100: "TEMP_DUPLICATE_ACCOUNT","            0x0200: "NORMAL_ACCOUNT","            0x1000: "INTERDOMAIN_TRUST_ACCOUNT","            0x2000: "WORKSTATION_TRUST_ACCOUNT","            0x4000: "SERVER_TRUST_ACCOUNT","            0x8000: "DONT_EXPIRE_PASSWORD","            0x10000: "MNS_LOGON_ACCOUNT","            0x20000: "SMARTCARD_REQUIRED","            0x40000: "TRUSTED_FOR_DELEGATION","            0x80000: "NOT_DELEGATED","            0x100000: "USE_DES_KEY_ONLY","            0x200000: "DONT_REQ_PREAUTH","            0x400000: "PASSWORD_EXPIRED","            0x800000: "TRUSTED_TO_AUTH_FOR_DELEGATION""        }
        ...
        changes = []
        if added:
            changes.append(f"Added: {', '.join(added)}")"'        if removed:
    ...
    def _explain_time_change(self, attr_name: str, value: Any) -> str:
        """Explain time-based attribute changes"""if isinstance(value, int) and value > 0:
            try:
                ...
                return f"{attr_name}: {dt.isoformat()}""            except Exception:
                pass
        return f"{attr_name} changed""
    def _explain_account_expiry(self, value: Any) -> str:
        """Explain account expiry changes"""if value == 0 or (isinstance(value, int) and value > 9223372036854775807):
            return "Account set to never expire""        elif isinstance(value, int):
            try:
                dt = datetime.fromtimestamp(value / 10000000 - 11644473600)
                return f"Account expires: {dt.isoformat()}""            except Exception:
                pass
        return "Account expiry changed""
    def _determine_change_severity(self, change: ADObjectChange) -> str:
        """Determine severity level for change"""if any(event in [
    ...
    async def _initialize_session_usn(self, session: MonitoringSession) -> None:
        """Initialize USN for monitoring session"""if not self.ad_provider:
            return
    ...
    async def get_change_history(
        ...
        """Get change history with optional filtering"""history = self.change_history
        if session_id:
    ...
    async def get_security_events(
        ...
        """Get security events"""events = self.security_events
        if event_types:
    ...
    def _matches_filters(self, change: ADObjectChange, filters: Dict[str, Any]) -> bool:
        """Check if change matches filters"""for key, value in filters.items():
            if key == "object_class" and change.object_class != value:"                return False
            elif key == "change_type" and change.change_type != value:"                return False
            elif key == "security_events" and not any(e in change.security_events for e in value):"                return False
            elif key == "time_range":"                start_time, end_time = value
    ...
    def _check_group_membership(self, change: AttributeChange) -> Optional[SecurityEventType]:
        """Check for group membership changes"""return SecurityEventType.GROUP_MEMBERSHIP_CHANGE

    def _check_password_changes(self, change: AttributeChange) -> Optional[SecurityEventType]:
        """Check for password changes"""return SecurityEventType.PASSWORD_CHANGE

    def _check_privilege_escalation(self, change: AttributeChange) -> Optional[SecurityEventType]:
        """Check for privilege escalation indicators"""return None

    def _check_admin_access(self, change: AttributeChange) -> Optional[SecurityEventType]:
        """Check for admin access changes"""return None

    # Provider setters
    def set_ad_provider(self, provider: ADConnectionProvider) -> None:
        """Set the AD connection provider"""self.ad_provider = provider

    def set_alert_provider(self, provider: AlertProvider) -> None:
        """Set the alert provider"""self.alert_provider = provider

    async def cleanup(self) -> None:
        """Cleanup resources"""await self.stop_monitoring()
    ...
    async def shutdown(self, context: CascadeContext = None) -> bool:
        """Shutdown the core"""try:
            ...
            logger.error(f"Failed to shutdown AD Monitoring Core: {e}")"            return False

    async def health_check(self, context: CascadeContext = None) -> Dict[str, Any]:
        """Health check for the core"""active_sessions = sum(1 for s in self.monitoring_sessions.values() if s.is_active)

        return {
            "status": "healthy" if active_sessions > 0 or not self.monitoring_sessions else "idle","            "active_sessions": active_sessions,"            "total_sessions": len(self.monitoring_sessions),"            "change_history_size": len(self.change_history),"            "security_events_count": len(self.security_events),"            "providers_configured": {"                "ad_connection": self.ad_provider is not None,"                "alert": self.alert_provider is not None"            },
            "is_monitoring": self.is_monitoring"        }

    async def get_metrics(self, context: CascadeContext = None) -> Dict[str, Any]:
        """Get core metrics"""return {
            "monitoring_sessions": len(self.monitoring_sessions),"            "changes_detected": len(self.change_history),"            "security_events": len(self.security_events),"            "uptime": 0,  # Would track actual uptime"            "average_check_interval": self.monitoring_config.check_interval,"            "domain_controllers": len(self.monitoring_config.domain_controllers)"        }
    ...
    async def process_task(
        ...
        """Process a task through the monitoring core"""task_type = task_data.get("type", "unknown")"
        if task_type == "start_monitoring":"            dc_list = cast(List[str], task_data.get("domain_controllers"))"            session_id = await self.start_monitoring(dc_list, context)
            return {"session_id": session_id, "status": "started"}"
        elif task_type == "stop_monitoring":"            session_id = cast(str, task_data.get("session_id"))"            await self.stop_monitoring(session_id, context)
            return {"status": "stopped"}"
        elif task_type == "get_changes":"            session_id = cast(str, task_data.get("session_id"))"            limit = task_data.get("limit", 100)"            filters = task_data.get("filters", {})"            changes = await self.get_change_history(session_id, limit, filters)
            return {"changes": [self._change_to_dict(c) for c in changes]}"
        elif task_type == "get_security_events":"            event_types = cast(List[SecurityEventType], task_data.get("event_types"))"            limit = task_data.get("limit", 100)"            events = await self.get_security_events(event_types, limit)
    ...
    def _change_to_dict(self, change: ADObjectChange) -> Dict[str, Any]:
        """Convert change object to dictionary"""return {
            "object_dn": change.object_dn,"            "object_guid": change.object_guid,"            "object_class": change.object_class,"            "change_type": change.change_type.value,"            "timestamp": change.timestamp.isoformat(),"            "usn": change.usn,"            "version": change.version,"            "attributes": {k: str(v) for k, v in change.attributes.items()},"            "security_events": [e.value for e in change.security_events],"            "explanation": change.explanation"        }
