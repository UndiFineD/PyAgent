...
class TestErrorMappingCoreBasics:
    ...
    def test_get_code_for_known_exception(self) -> None:
        """Test getting error code for known exception."""
        code = ErrorMappingCore.get_code("FileSystemError")
        assert code == "PA-1001"

    def test_get_code_for_unknown_exception(self) -> None:
        """Test getting default code for unknown exception."""
        code = ErrorMappingCore.get_code("UnknownError")
        assert code == "PA-0000"

    def test_get_troubleshooting_link(self) -> None:
        """Test troubleshooting link generation."""
        link = ErrorMappingCore.get_troubleshooting_link("PA-1001")
        assert link == "https://docs.pyagent.ai/errors/PA-1001"
        assert "PA-1001" in link

    def test_describe_error_known(self) -> None:
        """Test error description for known code."""
        desc = ErrorMappingCore.describe_error("PA-1001")
        assert "FileSystemError" in desc
        assert "workspace" in desc.lower()

    def test_describe_error_unknown(self) -> None:
        """Test error description for unknown code."""
        desc = ErrorMappingCore.describe_error("PA-9999")
...
class TestErrorMappingCoreCategories:
    ...
    def test_infrastructure_error_codes(self) -> None:
        """Test 10xx infrastructure error codes."""
        infra_codes = [
    ...
    def test_model_error_codes(self) -> None:
        """Test 20xx model/AI error codes."""
        model_codes = [
    ...
    def test_security_error_codes(self) -> None:
        """Test 40xx security error codes."""
        security_codes = [
    ...
    def test_config_error_codes(self) -> None:
        """Test 50xx configuration error codes."""
        config_codes = [
...
class TestErrorMappingCorePropertyBased:
    ...
    def test_all_known_exceptions_map_to_valid_codes(self, exception_name: str) -> None:
        """Property: All known exceptions map to PA-xxxx codes."""
        code = ErrorMappingCore.get_code(exception_name)
        assert code.startswith("PA-")
        assert len(code) == 7  # PA-XXXX

    @given(st.sampled_from(list(ErrorMappingCore.ERROR_CODES.values())))
    def test_all_codes_format_is_consistent(self, code: str) -> None:
        """Property: All error codes follow PA-XXXX format."""
        assert code.startswith("PA-")
        assert len(code) == 7
        parts = code.split("-")
        assert len(parts) == 2
    ...
    @given(st.just("FileSystemError"))
    def test_consistent_code_retrieval(self, exception_name: str) -> None:
        """Property: Same exception always returns same code."""
        code1 = ErrorMappingCore.get_code(exception_name)
        code2 = ErrorMappingCore.get_code(exception_name)
        assert code1 == code2

    @given(st.sampled_from(list(ErrorMappingCore.ERROR_CODES.values())))
    def test_link_generation_format(self, code: str) -> None:
        """Property: Generated links are valid URLs."""
        link = ErrorMappingCore.get_troubleshooting_link(code)
...
class TestErrorMappingCoreEdgeCases:
    ...
    def test_empty_string_exception(self) -> None:
        """Test with empty string exception name."""
        code = ErrorMappingCore.get_code("")
        assert code == "PA-0000"

    def test_case_sensitive_matching(self) -> None:
        """Test that error code matching is case-sensitive."""
        # Exact match
        assert ErrorMappingCore.get_code("FileSystemError") == "PA-1001"
        # Case mismatch should return default
        assert ErrorMappingCore.get_code("filesystemerror") == "PA-0000"
        assert ErrorMappingCore.get_code("FILESYSTEMERROR") == "PA-0000"

    def test_whitespace_handling(self) -> None:
        """Test handling of whitespace in exception names."""
        code_with_space = ErrorMappingCore.get_code("FileSystemError ")
        assert code_with_space == "PA-0000"  # Should not match

    def test_all_codes_are_unique(self) -> None:
        """Test that all error codes are unique."""
        codes = list(ErrorMappingCore.ERROR_CODES.values())
        assert len(codes) == len(set(codes))

    def test_no_duplicate_exception_names(self) -> None:
        """Test that no exception name is duplicated."""
        exception_names = list(ErrorMappingCore.ERROR_CODES.keys())
        assert len(exception_names) == len(set(exception_names))

    def test_error_code_range_integrity(self) -> None:
        """Test that error codes follow category numbering (10xx, 20xx, etc)."""
        for code in ErrorMappingCore.ERROR_CODES.values():
