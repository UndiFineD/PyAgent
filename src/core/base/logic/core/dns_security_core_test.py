...
class TestDnsSecurityCore:
    ...
    async def test_add_filter_rule(self):
        rule = FilterRule("test.com", FilterAction.BLOCK, 100, "Test rule")
        result = await self.core.add_filter_rule(rule)
        assert result is True
        assert len(self.core.filter_rules) > 0
    ...
    async def test_remove_filter_rule(self):
        ...
        result = await self.core.remove_filter_rule("test.com")
        assert result is True
        assert "test.com" not in self.core.blocked_domains
    @pytest.mark.asyncio
    async def test_check_domain_filter_blocked(self):
        rule = FilterRule("blocked.com", FilterAction.BLOCK, 100, "Blocked domain")
        await self.core.add_filter_rule(rule)
        action, reason = await self.core.check_domain_filter("blocked.com")
        assert action == FilterAction.BLOCK
        assert reason == "Blocked domain"
    @pytest.mark.asyncio
    async def test_check_domain_filter_allowed(self):
        action, reason = await self.core.check_domain_filter("allowed.com")
        assert action == FilterAction.ALLOW
        assert reason is None
    @pytest.mark.asyncio
    async def test_process_dns_query_blocked(self):
        core = DnsSecurityCore()
        rule = FilterRule("malicious-site.com", FilterAction.BLOCK, 100, "Malicious site")
        await core.add_filter_rule(rule)
        ...
        assert query.blocked_reason == "Malicious site"
        assert query.domain == "malicious-site.com"
        assert query.client_ip == "192.168.1.100"
    @pytest.mark.asyncio
    async def test_process_dns_query_allowed(self):
        query = await self.core.process_dns_query("allowed.com", DnsRecordType.A, "192.168.1.100")
    ...
    async def test_get_dns_statistics(self):
        core = DnsSecurityCore()
        rule = FilterRule("ads.example.com", FilterAction.BLOCK, 100, "Ad blocking")
        await core.add_filter_rule(rule)
        await core.process_dns_query("ads.example.com", DnsRecordType.A, "192.168.1.100")
        await core.process_dns_query("allowed.com", DnsRecordType.A, "192.168.1.100")
        await core.process_dns_query("allowed.com", DnsRecordType.AAAA, "192.168.1.101")
        stats = await core.get_dns_statistics()
        assert stats["total_queries"] == 3
        assert stats["blocked_queries"] == 1
        assert stats["allowed_queries"] == 2
    ...
    async def test_get_recent_queries(self):
        await self.core.process_dns_query("test.com", DnsRecordType.A, "192.168.1.100")
        queries = await self.core.get_recent_queries(limit=10)
        assert len(queries) >= 1
        query_data = queries[-1]
        assert query_data["domain"] == "test.com"
        assert query_data["client_ip"] == "192.168.1.100"
        assert query_data["result"] == "allowed"
    @pytest.mark.asyncio
    async def test_enable_safe_search(self):
        result = await self.core.enable_safe_search(True)
        assert result is True
        assert self.core.safe_search_enabled is True
        safe_search_rules = [r for r in self.core.filter_rules if "Safe search" in r.description]
        assert len(safe_search_rules) > 0
    @pytest.mark.asyncio
    async def test_enable_parental_control(self):
        result = await self.core.enable_parental_control(True)
        assert result is True
        assert self.core.parental_control_enabled is True
        parental_rules = [r for r in self.core.filter_rules if "Adult content" in r.description]
        assert len(parental_rules) > 0
    @pytest.mark.asyncio
    ...
    async def test_export_import_filter_rules(self, tmp_path):
        rules = [
            FilterRule("test1.com", FilterAction.BLOCK, 100, "Test rule 1"),
            FilterRule("test2.com", FilterAction.ALLOW, 50, "Test rule 2"),
        ]
        for rule in rules:
            await self.core.add_filter_rule(rule)
        export_file = tmp_path / "rules.json"
        result = await self.core.export_filter_rules(str(export_file))
        assert result is True
    ...
    def test_matches_pattern_exact(self):
        assert self.core._matches_pattern("test.com", "test.com") is True
        assert self.core._matches_pattern("test.com", "other.com") is False
    def test_matches_pattern_wildcard(self):
        assert self.core._matches_pattern("sub.test.com", "*.test.com") is True
        assert self.core._matches_pattern("test.com", "*.test.com") is False
        assert self.core._matches_pattern("sub.domain.test.com", "*.test.com") is True
    @pytest.mark.asyncio
