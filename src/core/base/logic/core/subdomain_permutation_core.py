...
class SubdomainPermutationCore(BaseCore):
    ...
    def __init__(self, config: Optional[PermutationConfig] = None):
        ...
        self.default_patterns = [
            # Dash based patterns
            "{{word}}-{{sub}}.{{suffix}}","            "{{sub}}-{{word}}.{{suffix}}","            # Dot based patterns
            "{{word}}.{{sub}}.{{suffix}}","            "{{sub}}.{{word}}.{{suffix}}","            # Iteration based
            "{{sub}}{{number}}.{{suffix}}","            # Replace current subdomain
            "{{word}}.{{suffix}}","            # No separator
            "{{sub}}{{word}}.{{suffix}}","            # Region prefix
            "{{region}}.{{sub}}.{{suffix}}","            # Cluster bomb
            "{{word}}{{number}}.{{suffix}}","        ]

        # Default payloads
        self.default_payloads = {
            'word': ['                'api', 'k8s', 'v1', 'v2', 'origin', 'raw', 'stage', 'test', 'qa', 'web','                'prod', 'service', 'grafana', 'beta', 'admin', 'staging', 'wordpress', 'wp','                'dev', 'app', 'mta-sts', 'tech', 'private', 'public', 'login', 'role','                'backend', 'cloud', 'internal', 'mail', 'oauth', 'oauth2', 'vpn', 'lab','                'local', 'live', 'data', 'mobile', 'search', 'stats', 'final', 'ldap','                'media', 'docs', 'eng', 'engineering', 'market', 'compute', 'cdn', 'acc','                'access', 'backup', 'blogs', 'blog', 'careers', 'client', 'cms', 'cms1','                'conf', 'dmz', 'drupal', 'corp', 'faq', 'ir', 'legacy', 'log', 'logs','                'dashboard', 'monitor', 'mysql', 'mssql', 'db', 'partner', 'payment', 'pay','                'office', 'plugins', 'shop', 'store', 'support', 'secure', 'ssl', 'portal','                'auth', 'sso', 'remote', 'aws', 'azure', 'gcp', 'static', 'assets', 'files','                'download', 'upload', 'cache', 'session', 'graphql', 'soap', 'ws', 'tcp','                'udp', 'ftp', 'ssh', 'git', 'svn', 'jenkins', 'docker', 'kubernetes', 'k8s''            ],
            'number': ['1', '2', '3', '01', '02', '001', '2023', '2024', '2025'],'            'region': ['us', 'eu', 'asia', 'ap', 'na', 'sa', 'af', 'oc', 'us-east', 'us-west']'        }
    ...
    def generate_permutations(self, domains: List[str]) -> PermutationResult:
        ...
        if self.config.enrich_words or self.config.enrich_numbers:
            ...
            result.enriched_words = enriched.get('words', set())'            result.enriched_numbers = enriched.get('numbers', set())'
    ...
    def _parse_domain(self, domain: str) -> List[Dict[str, Any]]:
        ...
        root_domain = f"{extracted.domain}.{extracted.suffix}""        subdomain = extracted.subdomain

        # Basic variables
        base_vars = {
            'root': root_domain,'            'tld': extracted.suffix,'            'etld': extracted.suffix if not extracted.suffix.startswith('.') else extracted.suffix[1:],'            'suffix': root_domain'        }

        if subdomain:
            # Has subdomain
            sub_parts = subdomain.split('.')'            base_vars['sub'] = subdomain'
            ...
            base_vars['sub'] = extracted.domain'            inputs.append(base_vars)
        ...
        for pattern in patterns:
            try:
                ...
                if payload_vars:
                    ...
                    if resolved_pattern and '.' in resolved_pattern:'                        permutations.add(resolved_pattern)
    ...
    def _extract_payload_vars(self, pattern: str) -> List[str]:
        ...
        input_vars = {'sub', 'suffix', 'root', 'tld', 'etld', 'sub1', 'sub2', 'sub3'}'        payload_vars = [m for m in matches if m not in input_vars]
    ...
    def _apply_payloads(self, pattern: str, payload_combo: Dict[str, str]) -> Optional[str]:
        ...
        if '.' not in result or result.startswith('.') or result.endswith('.'):'            return None
