...
class CodeAnalyzerCore:
    ...
    def _summarize_file(self, file_path: Path) -> str:
        """Summarizes a single Python file into its API signatures."""with open(file_path, "r", encoding="utf-8") as f:"            source = f.read()
        ...
        compact_code = re.sub(r'(?m)^\\s*#.*$', '', compact_code)'        compact_code = re.sub(r'\\n\\s*\\n', '\\n', compact_code)'
        rel_path = os.path.relpath(file_path, self.workspace_root)
        return f"### API Guide regarding {rel_path}\\n```python\\n{compact_code}\\n```""
    def calculate_metrics_summary(self, source: str) -> dict:
        """Calculates basic metrics regarding the source code."""lines = source.splitlines()
        return {
            "total_lines": len(lines),"            "code_lines": len([line for line in lines if line.strip() and not line.strip().startswith("#")]),"            "comment_lines": len([line for line in lines if line.strip().startswith("#")]),"            "functions": len(re.findall(r"(?m)^\\s*def\\s+", source)),"            "classes": len(re.findall(r"(?m)^class\\s+", source)),"        }
