...
class AEMHackerCore(BaseCore):
    ...
    def __init__(self):
        ...
        self.registered_checks.update({
            'set_preferences': self.check_set_preferences,
            'querybuilder_servlet': self.check_querybuilder_servlet,
            'felix_console': self.check_felix_console,
            'groovy_console': self.check_groovy_console,
            'ssrf_salesforce_servlet': self.check_ssrf_salesforce_servlet,
        })

    def _generate_token(self, length: int = 10) -> str:
        """Generate a random token for SSRF detection."""
    ...
    def register_check(self, name: str):
        """Decorator to register vulnerability checks."""
        def decorator(func):
    ...
    async def perform_aem_assessment(self, config: AEMScanConfig) -> AEMScanResults:
        ...
        try:
            ...
            results.errors.append(f"Assessment failed: {str(e)}")
        finally:
            # Cleanup
            ...
            try:
                ...
                if resp and resp.status_code == 200:
                    findings.append(AEMFinding(
                        name="SetPreferencesServlet",
                        url=url,
                        description="SetPreferences servlet is exposed, may allow unauthorized access",
                        severity="medium"
                    ))
                    break
            except Exception as e:
    ...
    def check_querybuilder_servlet(self, base_url: str, my_host: str, debug: bool = False,
        ...
        paths = itertools.product(
            ...
            ['', '.css', '.ico', '.png', '.gif', '.html', '.1.json',
             '....4.2.1....json', ';%0a{0}.css', ';%0a{0}.png',
             ';%0a{0}.html', ';%0a{0}.ico', '.ico;%0a{0}.ico',
             '.css;%0a{0}.css', '.html;%0a{0}.html',
             '?{0}.css', '?{0}.ico']
        )
        ...
        for p1, p2 in paths:
            ...
            try:
                ...
                if resp and resp.status_code == 200:
                    ...
                    if not found_feed and '</feed>' in resp.text:
                        findings.append(AEMFinding(
    ...
    def check_felix_console(self, base_url: str, my_host: str, debug: bool = False,
        ...
        paths = itertools.product(
            ...
            ['', '.json', '.1.json', '.4.2.1...json', '.css', '.ico', '.png',
             '.gif', '.html', '.js', ';%0a{0}.css', ';%0a{0}.html',
             ';%0a{0}.png', '.json;%0a{0}.ico', '.servlet/{0}.css',
        ...
        for p1, p2 in paths:
            ...
            try:
                ...
                if resp and resp.status_code == 200 and 'Web Console - Bundles' in resp.text:
                    findings.append(AEMFinding(
                        name="FelixConsole",
                        url=url,
                        description="Felix OSGi console is exposed, allows RCE via bundle installation",
                        severity="critical",
                ...
                if resp and resp.status_code == 200:
                    if 'executionResult' in resp.text:
                        findings.append(AEMFinding(
                            name="GroovyConsole",
                            url=url,
                            description="Groovy console is exposed, allows remote code execution",
                            severity="critical",
    ...
    def check_ssrf_salesforce_servlet(self, base_url: str, my_host: str, debug: bool = False,
        ...
        for path_template in paths:
            ...
            back_url = f'http://{my_host}/{self.token}/salesforcesecret/{encoded_url}/'
        ...
        if 'salesforcesecret' in self.ssrf_detections:
            orig_url = base64.b16decode(self.ssrf_detections['salesforcesecret'][0]).decode()
            findings.append(AEMFinding(
                name="SalesforceSecretServlet-SSRF",
                url=orig_url,
                description="SSRF vulnerability in SalesforceSecretServlet",
                severity="high",
                cve="CVE-2018-5006",
                references=["https://helpx.adobe.com/security/products/experience-manager/apsb18-23.html"]
            ))
