...
class AISecurityValidationCore:
    ...
    def _init_toxic_patterns(self):
        """Initialize toxic content patterns"""
        self.toxic_patterns = [
            # Harmful content
            {
    ...
    def _scan_jailbreaks(self, text: str) -> List[SecurityIssue]:
        ...
        for jailbreak in self.jailbreak_patterns:
            if re.search(jailbreak.pattern, text_lower, re.IGNORECASE):
                ...
                issue = SecurityIssue(
                    issue_type="jailbreak_attempt",
                    severity=self._map_confidence_to_severity(jailbreak.confidence),
    ...
    def _scan_injections(self, text: str) -> List[SecurityIssue]:
        ...
        for injection in self.injection_patterns:
            if re.search(injection["pattern"], text_lower, re.IGNORECASE):
                matches = re.findall(injection["pattern"], text_lower, re.IGNORECASE)
                evidence = f"Pattern: '{injection['pattern']}' - Matches: {matches[:3]}"
    ...
    def _scan_toxic_content(self, text: str) -> List[SecurityIssue]:
        ...
        for toxic in self.toxic_patterns:
            if re.search(toxic["pattern"], text_lower, re.IGNORECASE):
                matches = re.findall(toxic["pattern"], text_lower, re.IGNORECASE)
                evidence = f"Pattern: '{toxic['pattern']}' - Matches: {matches[:3]}"

                issue = SecurityIssue(
                    issue_type="toxic_content",
                    severity=toxic["severity"],
                    description=toxic["description"],
                    evidence=evidence,
                    confidence=0.9,
    ...
    def _generate_recommendations(self, issues: List[SecurityIssue], overall_risk: str) -> List[str]:
        ...
        if "jailbreak" in categories:
            recommendations.append("Implement jailbreak-resistant prompt engineering")
            recommendations.append("Add rate limiting for suspicious patterns")

        if "injection" in categories:
            recommendations.append("Use input sanitization and validation")
            recommendations.append("Implement prompt firewalls and content filters")

        if "content_safety" in categories:
            recommendations.append("Add content moderation and toxicity filters")
