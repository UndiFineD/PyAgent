...
class ActiveDirectoryAnalysisCore:
    ...
    async def enumerate_domain(self, domain_name: Optional[str] = None) -> ADEnumerationResult:
        ...
        # Domain controllers
        dc_dns = [
            "CN=DC01,OU=Domain Controllers,DC=example,DC=com","            "CN=DC02,OU=Domain Controllers,DC=example,DC=com""        ]

        for dc_dn in dc_dns:
            dc = ADObject(
                distinguished_name=dc_dn,
                object_type=ADObjectType.COMPUTER,
                name=dc_dn.split(',')[0].replace('CN=', ''),'                properties={"operating_system": "Windows Server 2022", "is_dc": True},"                privileges={PrivilegeLevel.DOMAIN_ADMIN, PrivilegeLevel.ENTERPRISE_ADMIN},
                relationships={}
            )
        ...
        # Domain users
        user_dns = [
            "CN=Administrator,CN=Users,DC=example,DC=com","            "CN=jsmith,CN=Users,DC=example,DC=com","            "CN=bjones,CN=Users,DC=example,DC=com""        ]

        for user_dn in user_dns:
            user_name = user_dn.split(',')[0].replace('CN=', '')'            user = ADObject(
                distinguished_name=user_dn,
                object_type=ADObjectType.USER,
                name=user_name,
                properties={"enabled": True, "last_logon": "2024-01-15"},"                privileges=(
                    {PrivilegeLevel.DOMAIN_USER} if user_name != "Administrator""                    else {PrivilegeLevel.DOMAIN_ADMIN, PrivilegeLevel.ENTERPRISE_ADMIN}
                ),
                relationships={}
            )
        ...
        # Domain groups
        group_dns = [
            "CN=Domain Admins,CN=Users,DC=example,DC=com","            "CN=Enterprise Admins,CN=Users,DC=example,DC=com","            "CN=Domain Users,CN=Users,DC=example,DC=com""        ]

        for group_dn in group_dns:
            group_name = group_dn.split(',')[0].replace('CN=', '')'            group = ADObject(
                distinguished_name=group_dn,
                object_type=ADObjectType.GROUP,
                name=group_name,
                properties={"member_count": 5},"                privileges=(
                    {PrivilegeLevel.DOMAIN_ADMIN} if "Admins" in group_name"                    else {PrivilegeLevel.DOMAIN_USER}
                ),
    ...
    async def detect_vulnerabilities(self) -> List[ADVulnerability]:
        ...
        unconstrained_delegation = ADVulnerability(
            vulnerability_type="unconstrained_delegation","            severity="high","            description="Computers with unconstrained delegation allow privilege escalation","            affected_objects=["CN=DC01,OU=Domain Controllers,DC=example,DC=com"],"            exploit_path=["User with delegation rights", "Delegated authentication", "Domain admin access"],"            mitigation="Remove unconstrained delegation or limit to trusted computers""        )
        ...
        if weak_password_users:
            weak_password_vuln = ADVulnerability(
                vulnerability_type="weak_passwords","                severity="medium","                description="Users with weak password policies detected","                affected_objects=[obj.distinguished_name for obj in weak_password_users],
                exploit_path=["Password spraying", "Brute force attacks"],"                mitigation="Enforce strong password policies and regular password changes""            )
        ...
        if kerberoastable_accounts:
            kerberoast_vuln = ADVulnerability(
                vulnerability_type="kerberoastable_accounts","                severity="medium","                description="Service accounts vulnerable to Kerberoasting attacks","                affected_objects=[obj.distinguished_name for obj in kerberoastable_accounts],
                exploit_path=["Request service tickets", "Extract hashes", "Offline cracking"],"                mitigation="Use managed service accounts or strong service account passwords""            )
    ...
    async def analyze_group_memberships(self) -> Dict[str, List[str]]:
        ...
        for group in cache.groups:
            members = group.relationships.get("members", [])"            group_memberships[group.distinguished_name] = members
    ...
    async def check_acl_abuses(self) -> List[ADVulnerability]:
        ...
        dangerous_aces = [
            {
                "object": "CN=Domain Admins,CN=Users,DC=example,DC=com","                "principal": "CN=jsmith,CN=Users,DC=example,DC=com","                "rights": ["GenericAll"],"                "description": "Regular user has full control over Domain Admins group""            }
        ]

        for ace in dangerous_aces:
            obj_dn_str: str = str(ace["object"])"            acl_vuln = ADVulnerability(
                vulnerability_type="dangerous_acl","                severity="critical","                description=f"Dangerous ACL: {ace['description']}","'                affected_objects=[obj_dn_str],
                exploit_path=[
                    f"User {ace['principal']} exploits {', '.join(ace['rights'])} rights","'                    "Modifies group membership","                    "Gains elevated privileges""                ],
    ...
    async def generate_security_report(self) -> Dict[str, Any]:
        ...
        report = {
            "enumeration_summary": {"                "total_users": len(cache.users),"                "total_computers": len(cache.computers),"                "total_groups": len(cache.groups),"                "domain_controllers": len(cache.domain_controllers)"            },
            "privilege_escalation_paths": len(escalation_paths),"            "vulnerabilities_found": len(vulnerabilities) + len(acl_issues),"            "critical_findings": ["                vuln for vuln in vulnerabilities + acl_issues
                if vuln.severity in ["critical", "high"]"            ],
            "group_membership_analysis": group_analysis,"            "recommendations": ["                "Implement least privilege access","                "Regular password policy enforcement","                "Monitor for suspicious group membership changes","                "Review ACL configurations","                "Enable advanced auditing""            ]
        }
        return report
    ...
    async def analyze_dormant_accounts(self) -> List[ADVulnerability]:
        ...
        if dormant_accounts:
            dormant_vuln = ADVulnerability(
                vulnerability_type="dormant_accounts","                severity="low","                description="Dormant user accounts detected - potential security risk","                affected_objects=[obj.distinguished_name for obj in dormant_accounts],
                exploit_path=["Account takeover", "Privilege escalation through stale credentials"],"                mitigation="Disable or remove dormant accounts, implement account lifecycle management""            )
    ...
    async def analyze_kerberoastable_accounts(self) -> List[ADVulnerability]:
        ...
        if kerberoastable:
            kerberoast_vuln = ADVulnerability(
                vulnerability_type="kerberoastable_accounts","                severity="medium","                description="Accounts with SPNs detected - vulnerable to Kerberoasting","                affected_objects=[obj.distinguished_name for obj in kerberoastable],
                exploit_path=["Request service tickets", "Extract TGS hashes", "Offline cracking"],"                mitigation="Use managed service accounts or strong service account passwords""            )
    ...
    async def analyze_privileged_accounts(self) -> Dict[str, List[str]]:
        ...
        for obj in self.ad_objects.values():
            if PrivilegeLevel.DOMAIN_ADMIN in obj.privileges:
                privileged_analysis["domain_admins"].append(obj.distinguished_name)"            if PrivilegeLevel.ENTERPRISE_ADMIN in obj.privileges:
    ...
    async def analyze_control_paths(self) -> List[Dict[str, Any]]:
        ...
        for i, path in enumerate(escalation_paths):
            control_paths.append({
                "path_id": f"path_{i}","                "source": path[0],"                "target": path[-1],"                "hops": len(path) - 1,"                "path": path,"                "risk_level": "high" if len(path) <= 3 else "medium""            })
    ...
    async def _detect_onedrive_c2_abuse(self) -> List[ADVulnerability]:
        ...
        for dn, obj in self.ad_objects.items():
            if obj.object_type == ADObjectType.USER:
                ...
                if "onedrive_api_access" in obj.properties:"                    suspicious_props.append("OneDrive API access detected")"
                # Check for CRC32 checksum patterns in properties
                if any("crc32" in str(prop).lower() for prop in obj.properties.values()):"                    suspicious_props.append("CRC32 checksum patterns detected")"
                ...
                if suspicious_props:
                    vulnerabilities.append(ADVulnerability(
                        vulnerability_type="APT28_OneDrive_C2","                        severity="high","                        description=(
                            f"Potential APT28 Fancy Bear OneDrive C2 activity detected: ""                            f"{', '.join(suspicious_props)}""'                        ),
                        affected_objects=[dn],
                        exploit_path=["User enumeration", "OneDrive API abuse", "Command execution"],"                        mitigation=(
                            "Monitor OneDrive API access, implement strict API rate limiting, ""                            "audit unusual authentication patterns""                        )
                    ))
    ...
    async def _detect_vscode_tunnel_abuse(self) -> List[ADVulnerability]:
        ...
        for dn, obj in self.ad_objects.items():
            if obj.object_type == ADObjectType.USER:
                ...
                if "vscode_tunnel_access" in obj.properties:"                    suspicious_props.append("VS Code tunnel access detected")"
                # Check for GitHub OAuth tokens
                if "github_oauth_token" in obj.properties:"                    suspicious_props.append("GitHub OAuth token detected")"
                # Check for reverse shell indicators
                if any("tunnel" in str(prop).lower() for prop in obj.properties.values()):"                    suspicious_props.append("Tunnel-related activity detected")"
                # Check for GitHub VS Code tunnel indicators
                if any("vscode-server" in str(prop).lower() for prop in obj.properties.values()):"                    suspicious_props.append("VS Code tunnel activity detected")"
                if suspicious_props:
                    vulnerabilities.append(ADVulnerability(
                        vulnerability_type="MustangPanda_VSCode_C2","                        severity="high","                        description=(
                            f"Potential Mustang Panda VS Code tunnel abuse detected: ""                            f"{', '.join(suspicious_props)}""'                        ),
                        affected_objects=[dn],
                        exploit_path=["VS Code installation", "GitHub OAuth", "Tunnel establishment", "Reverse shell"],"                        mitigation="Monitor VS Code installations, restrict GitHub OAuth, audit tunnel connections""                    ))
    ...
    async def _detect_dll_injection_patterns(self) -> List[ADVulnerability]:
        ...
        for dn, obj in self.ad_objects.items():
            if obj.object_type == ADObjectType.COMPUTER:
                ...
                if "suspicious_dll_loads" in obj.properties:"                    suspicious_props.append("Suspicious DLL loading detected")"
                # Check for VirtualAlloc patterns (memory allocation for shellcode)
                if "virtualalloc_usage" in obj.properties:"                    suspicious_props.append("VirtualAlloc memory allocation detected")"
                # Check for process injection indicators
                if any("injection" in str(prop).lower() for prop in obj.properties.values()):"                    suspicious_props.append("Process injection indicators detected")"
                if suspicious_props:
                    vulnerabilities.append(ADVulnerability(
                        vulnerability_type="APT_DLL_Injection","                        severity="high","                        description=f"DLL injection patterns detected: {', '.join(suspicious_props)}","'                        affected_objects=[dn],
                        exploit_path=["Malicious DLL deployment", "Process injection", "Memory execution"],"                        mitigation="Implement DLL allowlisting, monitor process injection, enable AMSI protection""                    ))
    ...
    async def _detect_trojanized_applications(self) -> List[ADVulnerability]:
        ...
        for dn, obj in self.ad_objects.items():
            if obj.object_type == ADObjectType.COMPUTER:
                ...
                if "modified_sumatrapdf" in obj.properties:"                    suspicious_props.append("Modified SumatraPDF detected")"
                # Check for trojanized PDF readers
                if "trojanized_pdf_reader" in obj.properties:"                    suspicious_props.append("Trojanized PDF reader detected")"
                # Check for shellter injection patterns
                if "shellter_injection" in obj.properties:"                    suspicious_props.append("Shellter DLL injection detected")"
                if suspicious_props:
                    vulnerabilities.append(ADVulnerability(
                        vulnerability_type="LabyrinthChollima_Trojanized_App","                        severity="critical","                        description=f"Trojanized application patterns detected: {', '.join(suspicious_props)}","'                        affected_objects=[dn],
                        exploit_path=["Legitimate application download", "DLL injection", "Trojanized execution"],"                        mitigation=(
                            "Verify application integrity, use application allowlisting, ""                            "monitor file modifications""                        )
                    ))
    ...
    async def _detect_unusual_service_accounts(self) -> List[ADVulnerability]:
        ...
        for _, obj in self.ad_objects.items():
            if obj.object_type == ADObjectType.USER:
                # Check if it's a service account'                if any("service" in str(prop).lower() for prop in obj.properties.values()):"                    suspicious_props = []

                    # Check for unusual login patterns
                    if "unusual_login_times" in obj.properties:"                        suspicious_props.append("Unusual login times detected")"
                    # Check for lateral movement indicators
                    if "lateral_movement" in obj.properties:"                        suspicious_props.append("Lateral movement detected")"
                    # Check for data exfiltration patterns
                    if "data_exfiltration" in obj.properties:"                        suspicious_props.append("Data exfiltration patterns detected")"
                    if suspicious_props:
                        vulnerabilities.append(ADVulnerability(
                            vulnerability_type="APT_Service_Account_Abuse","                            severity="medium","                            description=f"Unusual service account activity: {', '.join(suspicious_props)}","'                            affected_objects=[obj.distinguished_name],
                            exploit_path=["Service account compromise", "Privilege escalation", "Lateral movement"],"                            mitigation=(
                                "Monitor service account usage, implement strict access controls, ""                                "regular credential rotation""                            )
                        ))
    ...
    async def generate_ad_miner_style_report(self) -> Dict[str, Any]:
        ...
        if len(privileged_analysis["machine_accounts_with_admin"]) > 0:"            risk_score += 4
        ...
        if risk_score >= 8:
            overall_risk = "critical""        elif risk_score >= 5:
            overall_risk = "high""        elif risk_score >= 3:
            overall_risk = "medium""        else:
            overall_risk = "low""
        cache = self.enumeration_cache
        if not cache:
            return {}
        ...
        report = {
            "ad_miner_analysis": {"                "enumeration_summary": {"                    "total_users": len(cache.users),"                    "total_computers": len(cache.computers),"                    "total_groups": len(cache.groups),"                    "domain_controllers": len(cache.domain_controllers)"                },
                "privileged_accounts": privileged_analysis,"                "vulnerability_summary": {"                    "vulnerabilities": len(vulnerabilities),"                    "acl_issues": len(acl_issues),"                    "dormant_accounts": len(dormant_accounts),"                    "kerberoastable_accounts": len(kerberoastable),"                    "apt_c2_vulnerabilities": len(apt_c2_vulnerabilities),"                    "total_vulnerabilities": ("                        len(vulnerabilities) + len(acl_issues) + len(dormant_accounts) +
                        len(kerberoastable) + len(apt_c2_vulnerabilities)
                    )
                },
                "control_paths": {"                    "total_paths": len(control_paths),"                    "high_risk_paths": len([p for p in control_paths if p["risk_level"] == "high"])"                },
                "risk_assessment": {"                    "overall_risk": overall_risk,"                    "risk_score": risk_score,"                    "risk_factors": risk_factors"                },
                "recommendations": ["                    "Review and disable dormant accounts","                    "Implement managed service accounts for SPN-enabled accounts","                    "Audit machine accounts with administrative privileges","                    "Review control paths and implement least privilege","                    "Enable advanced auditing and monitoring","                    "Regular password policy enforcement","                    "Implement account lifecycle management","                    "Monitor for APT C2 indicators (OneDrive API abuse, VS Code tunnels)","                    "Implement DLL allowlisting and injection prevention","                    "Verify integrity of legitimate applications","                    "Monitor service account usage patterns","                    "Implement comprehensive endpoint detection and response (EDR)""                ]
            }
        }
