...
class GoalSettingCore(BaseCore):
    ...
    async def evaluate_content(
        ...
        evaluation = {
            "goals_met": True,"            "criteria_results": {},"            "feedback": "","            "confidence_score": 0.0"        }
        ...
        for criterion in criteria:
            ...
            if any(keyword.lower() in content.lower() for keyword in criterion.split()):
                evaluation["criteria_results"][criterion] = True"            else:
                evaluation["criteria_results"][criterion] = False"                evaluation["goals_met"] = False"                feedback_parts.append(f"Missing requirement: {criterion}")"
        evaluation["feedback"] = "; ".join(feedback_parts) if feedback_parts else "All criteria met""        evaluation["confidence_score"] = 1.0 if evaluation["goals_met"] else 0.5"
        return evaluation
    ...
    async def refine_content(
        ...
        refinement_note = "\\n\\n[Refined: Improved content structure and clarity based on evaluation feedback]""        return f"{content}{refinement_note}""
    async def iterate_goal(
        ...
        while goal.should_continue_iteration():
            ...
            iteration_result = IterationResult(
                ...
                feedback=evaluation.get("feedback", ""),"                goals_met=evaluation.get("goals_met", False)"            )

            goal.evaluation_history.append({
                "iteration": iteration_result.iteration,"                "content": iteration_result.content,"                "evaluation": iteration_result.evaluation,"                "goals_met": iteration_result.goals_met,"                "timestamp": iteration_result.timestamp.isoformat()"            })

            # Check if goals are met
            if evaluation.get("goals_met", False):"                goal.status = GoalStatus.ACHIEVED
                goal.completed_at = datetime.now()
                self.logger.info(f"Goal {goal_id} achieved in {goal.current_iteration} iterations")"                break

            # Check if max iterations reached
            if goal.current_iteration >= goal.max_iterations:
                goal.status = GoalStatus.MAX_ITERATIONS_REACHED
                self.logger.warning(f"Goal {goal_id} reached max iterations without achievement")"                break

            # Refine content for next iteration
            current_content = await self.refine_content(
                current_content,
                evaluation.get("feedback", ""),"                goal.criteria,
                goal_type
            )
