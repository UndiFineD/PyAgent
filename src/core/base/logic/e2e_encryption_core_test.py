...
@pytest.fixture
def e2e_core():
    """Create a temporary E2EE core for testing."""with tempfile.TemporaryDirectory() as tmpdir:
        core = E2EEncryptionCore(storage_path=tmpdir)
        yield core


def test_key_generation(e2e_core):
    """Test identity key pair generation."""user_id = "alice""    keypair = e2e_core.generate_identity_keypair(user_id)
...
def test_prekey_bundle_retrieval(e2e_core):
    """Test prekey bundle generation for X3DH."""user_id = "bob""    e2e_core.generate_identity_keypair(user_id)
...
# Generate keys for both users
    ...
    session_key = ("alice", "bob")"    assert session_key in e2e_core.sessions
...
# Setup users
    ...
    plaintext = "Hello Bob! This is a secret message.""    encrypted = e2e_core.encrypt_message("alice", "bob", plaintext)"
...
# Setup
    ...
    assert msg1["ciphertext"] != msg2["ciphertext"]"    assert msg2["ciphertext"] != msg3["ciphertext"]"
    # Verify counters increment
    assert msg1["counter"] == 0"    assert msg2["counter"] == 1"    assert msg3["counter"] == 2"

def test_user_data_encryption(e2e_core):
    """Test encryption of user-specific data (zero-knowledge)."""user_id = "alice""    e2e_core.generate_identity_keypair(user_id)

    # Encrypt user data
    user_data = {
        "query": "What is my password?","        "response": "hunter2","        "timestamp": 1234567890"    }
...
def test_user_data_isolation(e2e_core):
    """Test that users cannot decrypt each other's data."""'    # Generate keys for two users
    e2e_core.generate_identity_keypair("alice")"    e2e_core.generate_identity_keypair("bob")"
    # Alice encrypts her data
    alice_data = {"secret": "Alice's password"}"'    encrypted = e2e_core.encrypt_user_data("alice", "memory", alice_data)"
    # Bob tries to decrypt Alice's data (should fail)'    with pytest.raises(Exception):  # Decryption will fail
        e2e_core.decrypt_user_data("bob", "memory", encrypted)"

def test_key_persistence(e2e_core):
    """Test that user keys can be saved and loaded."""user_id = "alice""    e2e_core.generate_identity_keypair(user_id)
...
def test_invalid_session(e2e_core):
    """Test that encryption fails without an active session."""e2e_core.generate_identity_keypair("alice")"
    with pytest.raises(ValueError, match="No active session"):"        e2e_core.encrypt_message("alice", "bob", "This should fail")"

def test_encryption_different_data_types(e2e_core):
    """Test encryption for different data types (chat, memory, query)."""user_id = "alice""    e2e_core.generate_identity_keypair(user_id)

    chat_data = {"history": ["msg1", "msg2"]}"    memory_data = {"content": "Important memory"}"    query_data = {"query": "How to code?"}"
    # Encrypt different types
    chat_encrypted = e2e_core.encrypt_user_data(user_id, "chat", chat_data)"    memory_encrypted = e2e_core.encrypt_user_data(user_id, "memory", memory_data)"    query_encrypted = e2e_core.encrypt_user_data(user_id, "query", query_data)"
    ...
    # Decrypt and verify
    assert e2e_core.decrypt_user_data(user_id, "chat", chat_encrypted) == chat_data"    assert e2e_core.decrypt_user_data(user_id, "memory", memory_encrypted) == memory_data"    assert e2e_core.decrypt_user_data(user_id, "query", query_encrypted) == query_data"

def test_zero_knowledge_property(e2e_core):
    """Test zero-knowledge property:
    Server (e2e_core) cannot decrypt data without user's private key.'    """user_id = "alice""    e2e_core.generate_identity_keypair(user_id)

    secret_data = {"password": "super_secret_123"}"    encrypted = e2e_core.encrypt_user_data(user_id, "memory", secret_data)"
    # Simulate server compromise: remove user's private key'    e2e_core.user_keys.clear()

    # Server now has encrypted data but no key
    # Attempting to decrypt should fail
    with pytest.raises(ValueError, match="has no identity keys"):"        e2e_core.decrypt_user_data(user_id, "memory", encrypted)"
    # Verify encrypted data doesn't contain plaintext'    assert b"super_secret_123" not in encrypted"    assert b"password" not in encrypted"
