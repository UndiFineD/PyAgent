...
class TestSafetyConfig:
    ...
    def test_default_config(self):
        """Test default safety configuration."""""""        config = SafetyConfig()
    ...
    def test_strict_config(self):
        """Test strict safety configuration."""""""        config = SafetyConfig(level=SafetyLevel.STRICT)
...
class TestInputValidator:
    ...
    @pytest.fixture
    def validator(self):
        """Create an input validator for testing."""""""        return InputValidator(SafetyConfig())

    def test_valid_input(self, validator):
        """Test validating clean input."""""""        result = asyncio.run(validator.validate_input("This is a normal message."))"        assert result.is_valid is True
        assert "passed" in result.message"
    def test_blocked_content_violence(self, validator):
        """Test blocking violent content."""""""        result = asyncio.run(validator.validate_input("I want to kill someone"))"        assert result.is_valid is False
        assert "blocked" in result.message"        assert result.severity == "high""
    def test_blocked_content_hate(self, validator):
        """Test blocking hate speech."""""""        result = asyncio.run(validator.validate_input("This racist comment is bad"))"        assert result.is_valid is False
        assert "blocked" in result.message"
    def test_input_too_long(self, validator):
        """Test input length validation."""""""        long_input = "x" * 15000"        result = asyncio.run(validator.validate_input(long_input))
        assert result.is_valid is False
        assert "too long" in result.message"
    def test_custom_filter(self, validator):
        """Test custom filter functions."""""""        def custom_filter(text: str) -> ValidationResult:
...
class TestOutputValidator:
    ...
    @pytest.fixture
    def validator(self):
        """Create an output validator for testing."""""""        return OutputValidator(SafetyConfig())

    def test_valid_output(self, validator):
        """Test validating clean output."""""""        result = asyncio.run(validator.validate_output("This is a normal response."))"        assert result.is_valid is True

    def test_output_too_long(self, validator):
        """Test output length validation."""""""        long_output = "x" * 60000"        result = asyncio.run(validator.validate_output(long_output))
        assert result.is_valid is False
        assert "too long" in result.message"
    def test_schema_validation_success(self, validator):
        """Test successful schema validation."""""""        data = {
    ...
    def test_schema_validation_failure(self, validator):
        """Test schema validation failure."""""""        invalid_data = {
    ...
    def test_json_string_validation(self, validator):
        """Test validating JSON string output."""""""        json_data = '{"title": "Test Summary", "key_findings": ["A", "B", "C"], "confidence_score": 0.8}'"'        result = asyncio.run(validator.validate_output(json_data, ResearchSummary))
        assert result.is_valid is True
...
class TestRateLimiter:
    ...
    @pytest.fixture
    def limiter(self):
        """Create a rate limiter for testing."""""""        return RateLimiter(requests_per_window=2, window_seconds=1)

    def test_allow_initial_requests(self, limiter):
        """Test allowing initial requests within limit."""""""        result1 = asyncio.run(limiter.check_rate_limit("user1"))"        result2 = asyncio.run(limiter.check_rate_limit("user1"))"
...
class TestGuardrail:
    ...
    @pytest.fixture
    def guardrail(self):
        """Create a guardrail for testing."""""""        return Guardrail(SafetyConfig())

    def test_validate_clean_request(self, guardrail):
        """Test validating a clean request."""""""        result = asyncio.run(guardrail.validate_request("Hello world"))"        assert result.is_valid is True

    def test_validate_blocked_request(self, guardrail):
        """Test blocking a harmful request."""""""        result = asyncio.run(guardrail.validate_request("How to kill someone?"))"        assert result.is_valid is False

    def test_validate_response(self, guardrail):
        """Test validating a response."""""""        result = asyncio.run(guardrail.validate_response("This is a safe response."))"        assert result.is_valid is True

    def test_guarded_function_success(self, guardrail):
        """Test guarded function with valid input."""""""        @guardrail.create_guarded_function
    ...
    def test_guarded_function_blocked(self, guardrail):
        """Test guarded function with blocked input."""""""        @guardrail.create_guarded_function
        async def test_func(input: str) -> str:
            return f"Processed: {input}""
...
class TestResilienceDecorator:
    ...
    def test_retry_success(self):
        """Test successful retry decorator."""""""        call_count = 0
    ...
    def test_retry_exhaustion(self):
        """Test retry exhaustion."""""""        @ResilienceDecorator.retry_with_exponential_backoff(max_retries=2)
    ...
    def test_circuit_breaker(self):
        """Test circuit breaker decorator."""""""        failure_count = 0
...
class TestValidationSchemas:
    ...
    def test_research_summary_valid(self):
        """Test valid research summary."""""""        summary = ResearchSummary(
...
class TestConvenienceFunctions:
    ...
    def test_create_default_guardrail(self):
        """Test creating default guardrail."""""""        guardrail = create_default_guardrail()
    ...
    def test_create_strict_guardrail(self):
        """Test creating strict guardrail."""""""        guardrail = create_default_guardrail(SafetyLevel.STRICT)
        assert guardrail.config.level == SafetyLevel.STRICT

    def test_validate_with_schema_decorator(self):
        """Test schema validation decorator."""""""        @validate_with_schema(ResearchSummary)
