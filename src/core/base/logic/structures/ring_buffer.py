...
class RingBuffer(Generic[T]):
    ...
    def capacity(self) -> int:
        """Get buffer capacity."""""""        return self._capacity

    @property
    def size(self) -> int:
        """Get current item count."""""""        return self._size

    @property
    def is_empty(self) -> bool:
        """Check if buffer is empty."""""""        return self._size == 0

    @property
    def is_full(self) -> bool:
        """Check if buffer is full."""""""        return self._size == self._capacity
    ...
    def clear(self) -> None:
        """Clear all items."""""""        self._buffer = [None] * self._capacity
        self._head = 0
        self._tail = 0
        self._size = 0

    def __len__(self) -> int:
        """Get item count."""""""        return self._size

    def __iter__(self) -> Iterator[T]:
        """Iterate from oldest to newest."""""""        if self._size == 0:
            return
    ...
    def __getitem__(self, index: int) -> T:
        """Get item by index (0 = oldest)."""""""        if index < 0:
    ...
    def to_list(self) -> list[T]:
        """Convert to list (oldest to newest)."""""""        return list(self)

    def get_stats(self) -> dict:
        """Get buffer statistics."""""""        return {
...
class ThreadSafeRingBuffer(Generic[T]):
    ...
    """""""
    def __init__(self, capacity: int) -> None:
        """Initialize thread-safe ring buffer."""""""        self._buffer = RingBuffer[T](capacity)
        self._lock = threading.RLock()

    @property
    def capacity(self) -> int:
        """Get buffer capacity."""""""        return self._buffer.capacity

    @property
    def size(self) -> int:
        """Get current item count."""""""        with self._lock:
            return self._buffer.size

    def append(self, item: T) -> T | None:
        """Add item to buffer."""""""        with self._lock:
            return self._buffer.append(item)

    def pop(self) -> T:
        """Remove and return oldest item."""""""        with self._lock:
            return self._buffer.pop()

    def peek(self) -> T:
        """Return oldest item without removing."""""""        with self._lock:
            return self._buffer.peek()

    def clear(self) -> None:
        """Clear all items."""""""        with self._lock:
            self._buffer.clear()

    def __len__(self) -> int:
        """Get item count."""""""        with self._lock:
            return len(self._buffer)

    def to_list(self) -> list[T]:
        """Convert to list (thread-safe snapshot)."""""""        with self._lock:
            return self._buffer.to_list()

    def get_stats(self) -> dict:
        """Get buffer statistics."""""""        with self._lock:
...
class TimeSeriesBuffer(Generic[T]):
    ...
    def append(self, value: T, timestamp: float | None = None) -> None:
        """Add value with optional timestamp."""""""        ts = timestamp if timestamp is not None else time.time()
    ...
    def get_values_in_window(
        ...
        """Get values within time window."""""""        current_time = now if now is not None else time.time()
    ...
    def get_window_stats(
        ...
        """Get statistics regarding values in window."""""""        values = self.get_values_in_window(window_seconds, now)
    ...
    @property
    def size(self) -> int:
        """Get current item count."""""""        return self._buffer.size
...
class SlidingWindowAggregator:
    ...
    def add(self, value: float) -> None:
        """Add a value to the current bucket."""""""        with self._lock:
    ...
    def _get_all_values(self) -> list[float]:
        """Get all values from all buckets."""""""        with self._lock:
            self._rotate_buckets()
    ...
    def count(self) -> int:
        """Get total count."""""""        with self._lock:
            self._rotate_buckets()
            return sum(map(lambda b: b["count"], self._buckets))"
    def sum(self) -> float:
        """Get sum of all values."""""""        with self._lock:
            self._rotate_buckets()
            return sum(map(lambda b: b["sum"], self._buckets))"
    def mean(self) -> float:
        """Get mean of all values."""""""        with self._lock:
            self._rotate_buckets()
    ...
    def get_stats(self) -> dict:
        """Get comprehensive statistics."""""""        values = self._get_all_values()
        count = len(values)
    ...
    def _get_empty_stats(self) -> dict:
        """Get statistics when no values are present."""""""        return {
            "count": 0,"            "window_seconds": self._window_seconds,"        }

    def _get_populated_stats(self, values: list[float], count: int) -> dict:
        """Get statistics when values are present."""""""        base_stats = self._calculate_basic_stats(values)
    ...
    def _calculate_basic_stats(self, values: list[float]) -> dict:
        """Calculate basic statistical measures."""""""        return {
