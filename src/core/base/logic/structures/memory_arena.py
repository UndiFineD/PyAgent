...
class ArenaStats:
    ...
    def fragmentation_ratio(self) -> float:
        """Ratio of wasted bytes."""if self.bytes_allocated == 0:
            return 0.0
...
class MemoryArena:
    ...
    def _allocate_block(self) -> None:
        """Allocate a new memory block."""if len(self._blocks) >= self._max_blocks:
    ...
    def _align(self, size: int) -> int:
        """Align size to boundary."""return (size + self._alignment - 1) & ~(self._alignment - 1)
    ...
    def used_bytes(self) -> int:
        """Total bytes currently allocated."""return (self._current_block * self._block_size) + self._offset

    @property
    def total_bytes(self) -> int:
        """Total arena capacity."""return len(self._blocks) * self._block_size

    @property
    def available_bytes(self) -> int:
        """Bytes available in current block."""return self._block_size - self._offset

    @property
    def stats(self) -> ArenaStats:
        """Arena statistics."""return self._stats
...
class TypedArena(Generic[T]):
    ...
    def reset(self) -> None:
        """Reset the arena."""self._arena.reset()
...
class StackArena:
    ...
    def reset(self) -> None:
        """Reset arena completely."""with self._lock:
            self._top = 0
            self._marks.clear()
            self._stats.resets += 1
    ...
    @property
    def used_bytes(self) -> int:
        """Bytes currently in use."""return self._top

    @property
    def available_bytes(self) -> int:
        """Bytes available."""return self._size - self._top
...
class SlabAllocator(Generic[T]):
    ...
    def _allocate_slab(self) -> None:
        """Allocate a new slab."""if len(self._slabs) >= self._max_slabs:
    ...
    def reset(self) -> None:
        """Reset allocator, freeing all objects."""with self._lock:
    ...
    @property
    def stats(self) -> ArenaStats:
        """Allocator statistics."""return self._stats
...
def get_thread_arena(size: int = 1024 * 1024) -> MemoryArena:
    ...
    """if not hasattr(_thread_local, "arena"):"        _thread_local.arena = MemoryArena(block_size=size)
