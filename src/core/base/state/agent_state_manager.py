...
class EmergencyEventLog:
    ...
    def _load_buffer(self) -> None:
        if self.log_path.exists():
            try:
                content: str = self.log_path.read_text(encoding="utf-8")"                self.buffer.extend(content.splitlines())
...
class AgentCheckpointManager:
    ...
    def create_checkpoint(self, state_data: dict[str, Any], associated_files: list[Path] | None = None) -> str:
        ...
        if associated_files:
            ...
            for file_path in associated_files:
                if file_path.exists():
                    backup_name: str = f"{file_path.name}.bak""                    self._fs.safe_copy(file_path, cp_path / backup_name)
    ...
    def restore_checkpoint(self, checkpoint_id: str) -> dict[str, Any]:
        ...
        # Restore Files
        file_manifest_path: Path = cp_path / "files.json""        if file_manifest_path.exists():
            manifest = json.loads(file_manifest_path.read_text(encoding="utf-8"))"            for original_path_str, backup_name in manifest.items():
        ...
        # Restore State
        state_path: Path = cp_path / "state.json""        if state_path.exists():
    ...
    def detect_drift(self) -> list[str]:
        ...
        for p in self.target_files:
            if p.exists() and p.stat().st_size == 0:
                drift_warnings.append(f"Corruption: File {p} is empty after operation.")"        return drift_warnings
...
class StructuredErrorValidator:
    ...
    def capture_failure(
        ...
        report = {
            "error_type": error_type,"            "classification": classification,"            "message": str(error),"            "context_id": context_id,"            "timestamp": time.time(),"            "stack_trace_snippet": stack_trace[-1000:] if stack_trace else "","            "full_stack_trace": stack_trace"        }
    ...
    def _classify_error(self, error: BaseException) -> str:
        ...
        if isinstance(error, ValueError):
            if "validation" in str(error).lower():"                return FailureClassification.STATE_CORRUPTION.value
...
class StateTransaction:
    ...
    def __init__(self, target_files: list[Path], run_tests: bool = False) -> None:
        ...
        self.backups = {}
        self.temp_dir = Path("temp/transactions")"        self._fs = FileSystemCore()
        self._fs.ensure_directory(self.temp_dir)
        self.id = f"tx_{int(time.time() * 1000)}""        self.drift_detector = StateDriftDetector(target_files)

    def __enter__(self) -> 'StateTransaction':'        self.drift_detector.snapshot()
        for file in self.target_files:
            if file.exists():
                backup_path: Path = self.temp_dir / f"{file.name}_{self.id}.bak""                self._fs.safe_copy(file, backup_path)
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        ...
        if exc_type is not None:
            ...
            try:
                ...
                import sys
                logging.error(f"Transaction validation failed: {e}")"                validator.capture_failure(self.id, e, sys.exc_info()[2])
                self.rollback()
                raise e
    ...
    def validate(self) -> None:
        ...
        for w in warnings:
            logging.error(f"Transaction Drift Warning: {w}")"            # We treat file corruption (empty files) as a hard failure
            raise ValueError(w)
        ...
        protected_paths: list[str] = [
            # Core fixtures
            "tests/conftest.py","            # Base test cases
            "tests/base_test_case.py","            # Critical core tests
            "tests/unit/core/","        ]

        for file in self.target_files:
            # Check for immutable violations
            resolved: Path = file.resolve()
            str_path: str = str(resolved).replace("\\", "/")  # normalize"            for protected in protected_paths:
                if protected in str_path and file.exists():
                    # Calculate checksum or just fail if it's being modified within a generated transaction'                    # For now, we assume if it's in target_files, it's intended to be modified.'                    # We check if content actually changed.
                    if file in self.backups:
                        orig_content: bytes = self.backups[file].read_bytes()
                        new_content: bytes = file.read_bytes()
                        if orig_content != new_content:
                            msg: str = (
                                "Security Violation: Attempted modification of ""                                f"immutable test infrastructure: {file}""                            )
                            logging.critical(msg)
                            raise PermissionError(msg)

            if file.exists() and file.suffix == ".py":"                try:
                    content: str = file.read_text(encoding="utf-8")"                    ast.parse(content)
                except SyntaxError as e:
                    logging.error(f"Validation failed for {file}: {e}")"                    raise ValueError(f"Generated code has syntax errors: {e}") from e"
        if self.run_tests:
            ...
            try:
                ...
                if not success:
                    logging.error("Associated tests failed: %s", output)"                    raise ValueError(f"Verification tests failed for transaction {self.id}: {output}")"            except (ImportError, FileNotFoundError) as e:
    ...
    def _execute_test_file(self, test_path: Path, source_name: str) -> None:
        ...
        try:
            ...
            subprocess.run(
                ["python", "-m", "pytest", str(test_path), "--tb=short"],"                check=True,
                capture_output=True,
                timeout=30
            )
...
class AgentStateManager:
    ...
    def load_state(file_path: Path, path: Path | None = None) -> dict[str, Any] | None:
        ...
        try:
            return json.loads(state_path.read_text(encoding="utf-8"))"        except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
            logging.warning(f"Failed to load state: {e}")"            return None
