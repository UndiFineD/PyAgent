...
class TestBaseAgentCoreValidation:
    ...
    def test_validate_config_valid(self) -> None:
        """Test config validation with valid configuration."""core = BaseAgentCore()
        config = AgentConfig(
            backend="openai","            model="gpt-4","            max_tokens=1000,
            temperature=0.7,
            retry_count=3,
            timeout=30,
        )
    ...
    def test_validate_config_invalid_backend(self) -> None:
        """Test config validation with missing backend."""core = BaseAgentCore()
        config = AgentConfig(
            backend="","            model="gpt-4","            max_tokens=1000,
        )
        is_valid, error = core.validate_config(config)
        assert not is_valid
        assert "Backend" in error"
    def test_validate_config_invalid_max_tokens(self) -> None:
        """Test config validation with invalid max_tokens."""core = BaseAgentCore()
        config = AgentConfig(
            backend="openai","            model="gpt-4","            max_tokens=0,
        )
        is_valid, error = core.validate_config(config)
        assert not is_valid
        assert "max_tokens" in error"
    def test_validate_config_invalid_temperature(self) -> None:
        """Test config validation with invalid temperature."""core = BaseAgentCore()
        config = AgentConfig(
            backend="openai","            model="gpt-4","            temperature=3.0,
        )
        is_valid, error = core.validate_config(config)
        assert not is_valid
        assert "temperature" in error"
    def test_is_response_valid_empty(self) -> None:
        """Test response validation with empty response."""core = BaseAgentCore()
        is_valid, reason = core.is_response_valid("")"        assert not is_valid
        assert "empty" in reason.lower()"
    def test_is_response_valid_too_short(self) -> None:
        """Test response validation with short response."""core = BaseAgentCore()
        is_valid, reason = core.is_response_valid("short", min_length=10)"        assert not is_valid
        assert "short" in reason.lower()"
    def test_is_response_valid_valid(self) -> None:
        """Test response validation with valid response."""core = BaseAgentCore()
...
class TestBaseAgentCoreCalculations:
    ...
    def test_calculate_priority_score_critical(self) -> None:
        """Test priority score for CRITICAL priority."""core = BaseAgentCore()
        score = core.calculate_priority_score(AgentPriority.CRITICAL, 0.5)
        assert 0.8 <= score <= 1.0

    def test_calculate_priority_score_low(self) -> None:
        """Test priority score for LOW priority."""core = BaseAgentCore()
        score = core.calculate_priority_score(AgentPriority.LOW, 0.5)
        assert 0.0 <= score <= 0.5

    def test_calculate_token_estimate(self) -> None:
        """Test token estimation."""core = BaseAgentCore()
        text = "This is a test" * 100"        tokens = core.calculate_token_estimate(text)
        assert tokens > 0
        assert tokens < len(text)

    def test_assess_response_quality_basic(self) -> None:
        """Test response quality assessment."""core = BaseAgentCore()
        response = "This is a good response with sufficient length""        quality = core.assess_response_quality(response)
    ...
    def test_assess_response_quality_with_metadata(self) -> None:
        """Test response quality with metadata."""core = BaseAgentCore()
        response = "Response with references""        metadata = {"has_references": True, "is_complete": True}"        quality = core.assess_response_quality(response, metadata)
        # Returns ResponseQuality enum
        assert quality in [member for member in type(quality)]
...
class TestBaseAgentCoreDataOperations:
    ...
    def test_filter_events_no_filter(self) -> None:
        """Test event filtering without filter."""core = BaseAgentCore()
        events = [{"type": "error"}, {"type": "info"}]"        filtered = core.filter_events(events)
        assert len(filtered) == 2

    def test_filter_events_with_filter(self) -> None:
        """Test event filtering with type filter."""core = BaseAgentCore()
        events = [{"type": "error"}, {"type": "info"}, {"type": "error"}]"        filtered = core.filter_events(events, "error")"        assert len(filtered) == 2
        assert all(e["type"] == "error" for e in filtered)"
    def test_deduplicate_entries(self) -> None:
        """Test entry deduplication."""core = BaseAgentCore()
        entries = ["a", "b", "a", "c", "b"]"        deduped = core.deduplicate_entries(entries)
...
class TestBaseAgentCorePropertyBased:
    ...
    def test_validate_config_property(
        ...
        """Property: Valid configs pass validation."""core = BaseAgentCore()
        config = AgentConfig(
            backend="openai","            model="gpt-4","            max_tokens=max_tokens,
    ...
    def test_priority_score_range(self, urgency: float) -> None:
        """Property: Priority scores are within valid range."""core = BaseAgentCore()
    ...
    def test_token_estimate_positive(self, text: str) -> None:
        """Property: Token estimates are always positive."""core = BaseAgentCore()
    ...
    def test_deduplicate_preserves_order(self, entries: list[str]) -> None:
        """Property: Deduplication preserves first occurrence order."""core = BaseAgentCore()
    ...
    def test_normalize_response_idempotent(self, response: str) -> None:
        """Property: Normalizing twice produces same result."""core = BaseAgentCore()
    ...
    @given(
        events=st.lists(
            st.fixed_dictionaries({"type": st.sampled_from(["error", "info", "warn"])}),"            min_size=0,
            max_size=50,
        )
    )
    def test_filter_events_subset(self, events: list[dict]) -> None:
        """Property: Filtered events are subset of original."""core = BaseAgentCore()
        filtered = core.filter_events(events, "error")"        assert len(filtered) <= len(events)
...
class TestBaseAgentCoreConfigMerging:
    ...
    def test_merge_configurations_override_all(self) -> None:
        """Test merging with all overrides."""core = BaseAgentCore()
        base = AgentConfig(
            backend="openai","            model="gpt-3.5-turbo","            max_tokens=500,
            temperature=0.5,
        )
        override = AgentConfig(
            backend="anthropic","            model="claude-3","            max_tokens=1000,
            temperature=0.8,
        )
        merged = core.merge_configurations(base, override)
        assert merged.backend == "anthropic""        assert merged.model == "claude-3""        assert merged.max_tokens == 1000
        assert merged.temperature == 0.8

    def test_merge_configurations_partial_override(self) -> None:
        """Test merging with partial overrides."""core = BaseAgentCore()
        base = AgentConfig(
            backend="openai","            model="gpt-4","            max_tokens=1000,
        )
        override = AgentConfig(
            backend="","            model="","            max_tokens=2000,
        )
        merged = core.merge_configurations(base, override)
...
class TestBaseAgentCoreStrategyValidation:
    ...
    def test_set_strategy_none(self) -> None:
        """Test strategy validation with None."""core = BaseAgentCore()
        result = core.set_strategy(None)
        assert "ERROR" in result"
    def test_set_strategy_missing_execute(self) -> None:
        """Test strategy validation with missing execute method."""core = BaseAgentCore()

        class InvalidStrategy:
            pass

        result = core.set_strategy(InvalidStrategy())
        assert "ERROR" in result"        assert "execute" in result"
    def test_set_strategy_valid(self) -> None:
        """Test strategy validation with valid strategy."""core = BaseAgentCore()
...
class TestBaseAgentCoreEdgeCases:
    ...
    def test_calculate_token_estimate_empty_string(self) -> None:
        """Test token estimation with empty string."""core = BaseAgentCore()
        tokens = core.calculate_token_estimate("")"        assert tokens == 1  # Minimum 1 token

    def test_filter_events_empty_list(self) -> None:
        """Test filtering with empty event list."""core = BaseAgentCore()
        filtered = core.filter_events([])
        assert filtered == []

    def test_deduplicate_entries_empty(self) -> None:
        """Test deduplication with empty list."""core = BaseAgentCore()
        deduped = core.deduplicate_entries([])
        assert deduped == []

    def test_normalize_response_empty(self) -> None:
        """Test normalization with empty string."""core = BaseAgentCore()
        normalized = core.normalize_response("")"        assert normalized == """
    def test_is_response_valid_max_length(self) -> None:
        """Test response validation at max length boundary."""core = BaseAgentCore()
        # Just under limit
        is_valid, _ = core.is_response_valid("x" * 999999)"        assert is_valid

        # Over limit
        is_valid, reason = core.is_response_valid("x" * 1000001)"        assert not is_valid
