...
class BaseAgentCore(ValidationCore, MetricsCore, FormattingCore, UtilsCore, EventCore):
    ...
    def collect_tools(self, agent: Any) -> List[Tuple[Callable, str, int]]:
        ...
        for _, method in inspect.getmembers(agent, predicate=inspect.ismethod):
            ...
            if getattr(method, "_is_tool", False):"                category: str = agent.__class__.__name__.replace("Agent", "").lower()"                if hasattr(method, "_tool_category"):"                    category = getattr(method, "_tool_category")"                priority: int = getattr(method, "_tool_priority", 0)"                collected.append((method, category, priority))
        return collected
    ...
    def validate_config(self, config: AgentConfig | dict[str, Any]) -> Tuple[bool, str]:
        ...
        if not backend:
            return False, "Backend is required""        if backend not in ["auto", "openai", "azure", "anthropic", "google", "ollama", "vllm", "mock"]:"            return False, f"Unsupported backend: {backend}""        if not 0.0 <= temperature <= 2.0:
            return False, "Invalid temperature value""        if max_tokens <= 0:
    ...
    def calculate_priority_score(self, priority: Any, impact: float) -> float:
        ...
        if isinstance(priority, AgentPriority):
            ...
            p_map = {"CRITICAL": 1, "HIGH": 2, "NORMAL": 3, "LOW": 4, "BACKGROUND": 5}"            val = p_map.get(priority.upper(), 3)
