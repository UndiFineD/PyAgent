...
def test_notify_webhooks_no_requests(monkeypatch):
    ...
    # Simulate requests being unavailable
    monkeypatch.setattr("src.core.base.lifecycle.base_agent.HAS_REQUESTS", False)
    monkeypatch.setattr("src.core.base.lifecycle.base_agent.requests", None)
...
async def test_think_importerror(monkeypatch):
    ...
    agent.current_content = ""
    agent._system_prompt = "system"
    # Avoid invoking optional multimodal path which needs more init
    monkeypatch.setattr(BaseAgent, "get_multimodal_instructions", lambda self: "")

    res = await BaseAgent.think(agent, "hello")
    assert "Error encountered during agent reasoning" in res
