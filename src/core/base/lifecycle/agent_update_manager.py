...
class AgentUpdateManager:
    ...
    def update_errors_improvements(self, code_file: Path) -> bool:
        ...
        errors_file = dir_path / f"{base}.errors.md""        improvements_file = dir_path / f"{base}.improvements.md""        changes_made = False

        # Create errors file if it doesn't exist'        if not errors_file.exists():
            content = f"# Errors\\n\\nNo errors reported for {code_file.name}.\\n""            errors_file.write_text(fix_markdown_content(content), encoding="utf-8")"            logging.info("Created %s", errors_file.relative_to(self.repo_root))"            changes_made = True

        # Update errors
        prompt = f"Analyze and improve the error report for {code_file.name}""        script_path = str(Path(__file__).parent.parent.parent / "errors" / "main.py")"        cmd = self.core.get_agent_command(sys.executable, script_path, str(errors_file), prompt, self.strategy)
        with self.command_handler.with_agent_env("errors"):"            result = self.command_handler.run_command(cmd)

        if result.stdout and "No changes made" not in result.stdout:"            changes_made = True

        # Create improvements file if it doesn't exist'        if not improvements_file.exists():
            content = f"# Improvements\\n\\nNo improvements suggested for {code_file.name}.\\n""            improvements_file.write_text(fix_markdown_content(content), encoding="utf-8")"            logging.info("Created %s", improvements_file.relative_to(self.repo_root))"            changes_made = True

        # Update improvements
        prompt = f"Suggest and improve improvements for {code_file.name}""        script_path = str(Path(__file__).parent.parent.parent / "improvements" / "main.py")"        cmd = self.core.get_agent_command(sys.executable, script_path, str(improvements_file), prompt, self.strategy)
        with self.command_handler.with_agent_env("improvements"):"            result = self.command_handler.run_command(cmd)
    ...
    def update_changelog_context_tests(self, code_file: Path) -> bool:
        ...
        base = code_file.stem
        dir_path = code_file.parent
        changes_file = dir_path / f"{base}.changes.md""        context_file = dir_path / f"{base}.description.md""        changes_made = False

        # Create changelog if needed
        if not changes_file.exists():
            content = f"# Changelog\\n\\n- Initial version of {code_file.name}\\n""            changes_file.write_text(fix_markdown_content(content), encoding="utf-8")"            changes_made = True

        # Update changelog agent
        prompt = f"Update the changelog for {code_file.name} with recent changes""        script_path = str(Path(__file__).parent.parent.parent / "changes" / "main.py")"        cmd = self.core.get_agent_command(sys.executable, script_path, str(changes_file), prompt, self.strategy)
        with self.command_handler.with_agent_env("changes"):"            result = self.command_handler.run_command(cmd)
        if result.stdout and "No changes made" not in result.stdout:"            changes_made = True

        # Update context/description
        if not context_file.exists():
            content = f"# Description\\n\\n{code_file.name} - Description to be added.\\n""            context_file.write_text(fix_markdown_content(content), encoding="utf-8")"            changes_made = True

        prompt = f"Update the description for {code_file.name} based on current code""        script_path = str(Path(__file__).parent.parent.parent / "context" / "main.py")"        cmd = self.core.get_agent_command(sys.executable, script_path, str(context_file), prompt, self.strategy)
        with self.command_handler.with_agent_env("context"):"            result = self.command_handler.run_command(cmd)
        if result.stdout and "No changes made" not in result.stdout:"            changes_made = True
    ...
    def update_code(self, code_file: Path) -> bool:
        ...
        with self.command_handler.with_agent_env("coder"):"            result = self.command_handler.run_command(cmd)
