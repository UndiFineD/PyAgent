...
class TestEnvironmentMixin:
    ...
    async def test_create_environment_config(self, agent):
        ...
        assert config.name == "test-env""        assert config.version == "1.0.0""        assert config.description == "Test environment""        assert config.isolation == EnvironmentIsolation.NONE
...
# Create config first
        ...
        async with agent.use_environment("test-env", "1.0.0") as env_instance:"            environment_used = env_instance
            assert env_instance.environment_name == "test-env@1.0.0""            assert env_instance.status.name == "RUNNING""
...
# Get initial count
        ...
        unique_name = f"test-env-{id(agent)}""        await agent.create_environment_config(name=unique_name, version="1.0.0")"
    ...
    async def test_cleanup_environments(self, agent):
        ...
        async with agent.use_environment("env1", "1.0.0") as inst1:"            instances.append(inst1)
            async with agent.use_environment("env2", "1.0.0") as inst2:"                instances.append(inst2)
