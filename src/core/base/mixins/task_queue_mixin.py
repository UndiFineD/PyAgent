...
class TaskQueueMixin:
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.max_queue_size: int = kwargs.get('max_queue_size', 10)'        self.task_ttl: int = kwargs.get('task_ttl', 600)  # 10 minutes'        self.cleanup_interval: int = kwargs.get('cleanup_interval', 300)  # 5 minutes'        self._cleanup_task: Optional[asyncio.Task[None]] = None
    ...
    async def submit_task(self, task_data: Dict[str, Any]) -> str:
        ...
        task_data.update({
            'job_id': job_id,'            'status': 'queued','            'submit_time': time.time(),'        })
    ...
    async def _task_worker(self) -> None:
        """Background worker to process tasks."""""""        while True:
            try:
                ...
                task_data['status'] = 'processing''                task_data['start_time'] = time.time()'
                try:
                    # Process the task - override in subclass
                    result = await self._process_task(task_data)
                    task_data.update({
                        'status': 'completed','                        'result': result,'                        'completion_time': time.time(),'                    })
                except Exception as e:
                    task_data.update({
                        'status': 'failed','                        'error': str(e),'                        'completion_time': time.time(),'                    })
        ...
        """Background worker to clean up old completed tasks."""""""        while True:
            try:
                ...
                for job_id, task_data in self.task_results.items():
                    if task_data.get('status') in ['completed', 'failed']:'                        submit_time = task_data.get('submit_time', 0)'                        completion_time = task_data.get('completion_time', current_time)'                        age = current_time - max(submit_time, completion_time)
