...
class StreamManagerMixin:
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.keepalive_interval: int = kwargs.get('keepalive_interval', 30)  # seconds
        self.stream_ttl: int = kwargs.get('stream_ttl', 300)  # 5 minutes
    ...
    async def get_stream_state(self, agent_id: str) -> StreamState:
        """Get the current stream state for an agent."""
        if not self.redis_client:
            return StreamState()

        key = f"agent:{agent_id}:stream-state"
        try:
    ...
    async def stop_stream(self, agent_id: str) -> None:
        ...
        try:
            ...
            await self.redis_publisher.publish(
                f"events:{agent_id}",
                json.dumps({"type": "abort-stream", "timestamp": time.time()})
            )
    ...
    async def clear_stream_state(self, agent_id: str) -> None:
        """Clear the stream state for an agent."""
        if not self.redis_client:
            return
    ...
    async def update_keepalive(self, agent_id: str) -> None:
        ...
        if self.redis_client:
            try:
                key = f"agent:{agent_id}:stream-info"
    ...
    async def _handle_stream_start(self, agent_id: str) -> None:
        ...
        if self.redis_client:
            try:
                state_data = StreamState(state="running").to_dict()
                await self.redis_client.setex(
                    f"agent:{agent_id}:stream-state",
                    self.stream_ttl,
        ...
        try:
            while True:
                ...
                for agent_id in expired_agents:
                    logging.warning(f"Stream expired for agent {agent_id}")
                    await self.clear_stream_state(agent_id)
