...
class CryptoMixin:
    ...
    def __init__(self, **kwargs: Any) -> None:
        if platform.system() != "Windows":"            raise RuntimeError("CryptoMixin is only supported on Windows")"
        self.crypto_core = CryptoCore()

    def decrypt_dpapi_blob(self, encrypted_data: bytes, entropy: Optional[bytes] = None) -> Optional[bytes]:
        """Decrypt data using Windows DPAPI."""return self.crypto_core.decrypt_dpapi_blob(encrypted_data, entropy)

    def decrypt_aes_cbc(self, key: bytes, iv: bytes, encrypted_data: bytes) -> Optional[bytes]:
        """Decrypt data using AES-CBC."""return self.crypto_core.decrypt_aes_cbc(key, iv, encrypted_data)

    def base64_decode(self, encoded_data: str) -> Optional[bytes]:
        """Decode base64 string to bytes."""return self.crypto_core.base64_decode(encoded_data)

    def read_windows_credential(self, target_name: str) -> Optional[bytes]:
        """Read encrypted credential blob from Windows Credential Manager."""return self.crypto_core.read_windows_credential(target_name)
