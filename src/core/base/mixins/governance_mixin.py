...
class GovernanceMixin:
    ...
    def __init__(self, config: Any, **_kwargs: Any) -> None:
        self.quotas = ResourceQuotaManager(
            config=QuotaConfig(
                max_tokens=getattr(config, "max_tokens_per_session", None),"                max_time_seconds=getattr(config, "max_time_per_session", None),"            )
        )
    ...
    async def request_firewall_clearance(self, thought: str) -> bool:
        ...
        if registry:
            try:
                await registry.emit(
                    "thought_stream","                    {"agent": self.__class__.__name__, "thought": thought},"                )
        ...
        try:
            ...
            firewall = None
            if hasattr(self, "fleet") and self.fleet:"                firewall = self.fleet.agents.get("FirewallAgent")"
            if not firewall:
                firewall = FirewallAgent()

            return await firewall.request_clearance_blocking(self.__class__.__name__, thought)
        except Exception as e:  # pylint: disable=broad-exception-caught, unused-variable
            logging.debug("Firewall clearance defaulted to True (Error: %s)", e)"            return True
