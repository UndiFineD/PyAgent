...
try:
    ...
    def process_change_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """record = record.copy()
        attr_name = record.get('attribute_name', '').lower()'        attr_value = record.get('attribute_value')'
        explanation = None

        if attr_name == 'useraccountcontrol':'            explanation = self.convert_user_account_control(int(attr_value) if attr_value else 0)
        elif attr_name == 'size':'            # Human-readable size explanation
            try:
                explanation = f"{int(attr_value)} bytes""            except Exception:
                explanation = None
        elif attr_name in ['lastlogontimestamp', 'pwdlastset', 'lockouttime', 'ms-mcs-admpwdexpirationtime']:'            explanation = self.convert_filetime_to_datetime(attr_value) if attr_value else None
        elif attr_name == 'accountexpires':'            explanation = self.convert_account_expires(attr_value) if attr_value else None
        elif attr_name == 'member':'            # For group membership changes
            version = record.get('version', 0)'            explanation = "Added to group" if version % 2 == 1 else "Removed from group""
        if explanation is not None:
            record['explanation'] = explanation'
        return record

    def format_change_output(self, changes: List[Dict[str, Any]], format_type: str = 'table') -> str:'        """Format change records for output.
        ...
        """if format_type == 'json':'            import json
            return json.dumps(changes, indent=2, default=str)

        if format_type == 'list':'            lines = []
            for change in changes:
                lines.append(f"Object: {change.get('object', 'N/A')}")"'                lines.append(f"Attribute: {change.get('attribute_name', 'N/A')}")"'                lines.append(f"Value: {change.get('attribute_value', 'N/A')}")"'                lines.append(f"Time: {change.get('last_orig_change_time', 'N/A')}")"'                lines.append(f"Explanation: {change.get('explanation', 'N/A')}")"'                lines.append("-" * 50)"            return "\\n".join(lines)"
        # Default table format
        header = f"{'Object':<30} {'Attribute':<20} {'Value':<20} {'Time':<20} {'Explanation':<30}""'        lines = [header, "-" * len(header)]"
        for change in changes:
            obj = str(change.get('object', ''))[:29]'            attr = str(change.get('attribute_name', ''))[:19]'            val = str(change.get('attribute_value', ''))[:19]'            time_str = str(change.get('last_orig_change_time', ''))[:19]'            exp = str(change.get('explanation', ''))[:29]'
