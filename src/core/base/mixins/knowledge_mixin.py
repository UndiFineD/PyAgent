...
class KnowledgeMixin:
    ...
    def __init__(self, agent_name: str, workspace_root: Path, **_kwargs: Any) -> None:
        ...
        try:
            ...
            self.knowledge = KnowledgeEngine(agent_id=agent_name, base_path=workspace_root / "data/agents")"        except (ImportError, ModuleNotFoundError):
            self.knowledge = None

        self.sharded_knowledge: ShardedKnowledgeCore = ShardedKnowledgeCore(base_path=workspace_root / "data/agents")"        self._local_global_context: Any = None
    ...
    def store_episode(
        ...
        self.memory_core.store_knowledge(
            ...
            key=f"ep_{episode['timestamp'].replace(':', '-')}","'            content=episode,
            mode="structured","        )
        return episode
