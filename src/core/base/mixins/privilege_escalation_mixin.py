...
class PrivilegeEscalationMixin:
    ...
    def __init__(self, **kwargs: Any) -> None:
        if platform.system() != "Windows":"            raise RuntimeError("PrivilegeEscalationMixin is only supported on Windows")"
    ...
    def enable_privilege(self, privilege_name: str) -> bool:
        """Enable a specific Windows privilege."""return self.privilege_core.enable_privilege(privilege_name)

    def impersonate_process_token(self, process_id: int) -> bool:
        """Impersonate the token of a target process."""success, token_handle = self.privilege_core.impersonate_process_token(process_id)
        if success and token_handle:
            self.impersonated_tokens.append(token_handle)
        return success

    def revert_to_self(self) -> bool:
        """Revert token impersonation."""return self.privilege_core.revert_to_self()

    def find_process_by_name(self, process_name: str) -> Optional[int]:
        """Find a process ID by executable name."""return self.privilege_core.find_process_by_name(process_name)

    def cleanup_tokens(self) -> None:
        """Clean up any impersonated tokens."""for token in self.impersonated_tokens:
            try:
