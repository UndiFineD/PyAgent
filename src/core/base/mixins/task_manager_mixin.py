...
class TaskItem:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "description": self.description,"            "completed": self.completed,"            "created_at": self.created_at,"            "completed_at": self.completed_at,"            "priority": self.priority"        }
...
class TaskManagerMixin:
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._workspace_root = kwargs.get('_workspace_root')'        self.task_file: Optional[Path] = None
        self.auto_save: bool = kwargs.get('auto_save_tasks', True)'        self.max_tasks: int = kwargs.get('max_tasks', 50)'
        ...
        if self._workspace_root:
            self.task_file = Path(self._workspace_root) / '.pyagent_tasks.json''            self._load_tasks()
        ...
        Inspired by Adorable's todo tool interface.'        """""""        try:
            ...
            if not items:
                ...
                for item_data in items:
                    description = item_data.get("description", "").strip()"                    if not description:
                        continue

                    completed = item_data.get("completed", False)"                    priority = item_data.get("priority", 1)"
            ...
            return {
                "success": True,"                "message": f"Updated {len(self.tasks)} tasks","                "task_count": len(self.tasks),"                "completed_count": sum(1 for t in self.tasks if t.completed)"            }

        except Exception as e:
            logging.error(f"Error updating task list: {e}")"            return {
    ...
    async def get_task_status(self, cascade_context: Optional[CascadeContext] = None) -> Dict[str, Any]:
        ...
        return {
            "total_tasks": total,"            "completed_tasks": completed,"            "pending_tasks": pending,"            "completion_rate": completed / total if total > 0 else 0,"            "priority_breakdown": {"                "high": high_priority,"                "medium": medium_priority,"                "low": low_priority"            },
            ...
            if task.description == description:
                if not task.completed:
                    ...
                    if self.auto_save:
                        self._save_tasks()
                    return {
                        "success": True,"                        "message": "Task completed","                        "task": task.to_dict()"                    }
                else:
    ...
    def _load_tasks(self) -> None:
        ...
        try:
            with open(self.task_file, 'r', encoding='utf-8') as f:'                data = json.load(f)

            self.tasks = TaskManagerMixin._ManagedTaskList(self)
            for item_data in data.get("tasks", []):"                try:
    ...
    def _save_tasks(self) -> None:
        ...
        try:
            ...
            data = {
                "version": "1.0","                "last_updated": time.time(),"                "tasks": [task.to_dict() for task in self.tasks]"            }

            with open(self.task_file, 'w', encoding='utf-8') as f:'                json.dump(data, f, indent=2, ensure_ascii=False)
