...
class DummyAgent(PersistenceMixin):
    def __init__(self):
        super().__init__()
        self.file_path = Path("temp_test_file.py")
        self.previous_content = "# original\n"
...
def test_update_file_runs_tests_and_rolls_back(monkeypatch, tmp_path):
    agent = DummyAgent()
    agent.file_path = tmp_path / "example.py"
    agent.file_path.write_text("print(1)\n")
...
def test_update_file_succeeds_when_tests_pass(monkeypatch, tmp_path):
    agent = DummyAgent()
    agent.file_path = tmp_path / "example2.py"
    agent.file_path.write_text("print(1)\n")
    agent.current_content = "print(2)\n"
