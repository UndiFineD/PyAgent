...
class UvaBuffer:
    ...
    def copy_to_gpu(
        self, stream: Optional[Any] = None, non_blocking: bool = True
    ) -> Any:
        ...
        with self._lock:
            ...
            if (
                ...
                or self._gpu_tensor.shape != self._cpu_tensor.shape
            ):
                self._gpu_tensor = torch.empty_like(
                    self._cpu_tensor, device=self._device
                )
        ...
        with self._lock:
            ...
            return {
                ...
                "avg_transfer_time_ms": (
                    (total_time / total_allocs / 1_000_000) if total_allocs > 0 else 0
                ),
                "throughput_gbps": (
                    (total_bytes / (1024**3)) / (total_time / 1_000_000_000)
                    if total_time > 0
                    else 0
                ),
                "contention_rate": (
