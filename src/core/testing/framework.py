...
class AgentTestingPyramidCore:
    ...
    async def run_test_pyramid(self, test_filter: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        results = {
            'unit': await self._run_unit_tests(test_filter),'            'integration': await self._run_integration_tests(test_filter),'            'e2e': await self._run_e2e_tests(test_filter),'            'performance': await self._run_performance_tests(test_filter),'            'security': await self._run_security_tests(test_filter)'        }
    ...
    def _matches_filter(self, test_path: Path, test_filter: Optional[Dict[str, Any]]) -> bool:
        ...
        # Check test type
        if 'type' in test_filter:'            if test_filter['type'] == 'unit' and 'unit' not in str(test_path):'                return False
            if test_filter['type'] == 'integration' and 'integration' not in str(test_path):'                return False
            if test_filter['type'] == 'e2e' and 'scenarios' not in str(test_path):'                return False
    ...
    async def _run_pytest_file(self, test_file: Path, test_type: TestType) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                ...
                status=status,
                duration=duration,
                output="",  # Would capture output in real implementation"                metadata={"exit_code": result}"            )
    ...
    async def _run_scenario_tests(self, scenario_file: Path) -> List[TestResult]:
        ...
        try:
            ...
            with open(scenario_file, 'r') as f:'                scenario_data = yaml.safe_load(f)
    ...
    async def _run_single_scenario(self, scenario: TestScenario) -> TestResult:
        ...
        try:
            ...
            module_name, class_name = scenario.agent_class.rsplit('.', 1)'            module = __import__(module_name, fromlist=[class_name])
            ...
            return TestResult(
                ...
                metadata={
                    "scenario": scenario.name,"                    "agent_class": scenario.agent_class,"                    "input": scenario.input_data,"                    "expected": scenario.expected_output,"                    "actual": result"                }
            )
...
scenarios:
  - name: example_scenario
    ...
    input_data:
      message: "Hello, agent!""      context: {{}}
    expected_output:
      response: "Hello, human!""      confidence: 0.9
    timeout: 30
    tags:
      - example
      - basic
...
class PromptVersioningSystem:
    ...
    def list_prompt_versions(self, prompt_name: str) -> List[Dict[str, Any]]:
        ...
        # Sort by timestamp
        versions.sort(key=lambda x: x['timestamp'], reverse=True)'        return versions
...
class EvaluationNotebookSystem:
    ...
    def create_evaluation_notebook(
        ...
        for r in test_results:
            results_list.append({
                'test_id': r.test_id,'                'test_type': r.test_type.value,'                'status': r.status.value,'                'duration': r.duration,'                'timestamp': r.timestamp'            })
        results_json = json.dumps(results_list, indent=2)

        notebook_content = {
            "cells": ["                {
                    "cell_type": "markdown","                    "metadata": {},"                    "source": ["                        "# Agent Testing Evaluation\\n","                        f"Generated: {time.strftime('%Y-%m-%d %H:%M:%S')}\\n","'                        f"Total Tests: {len(test_results)}\\n""                    ]
                },
                {
                    "cell_type": "code","                    "metadata": {},"                    "source": ["                        "import pandas as pd\\n","                        "import matplotlib.pyplot as plt\\n","                        "import json\\n","                        "\\n","                        "# Load test results\\n","                        f"results_data = {results_json}\\n","                        "\\n","                        "df = pd.DataFrame(results_data)\\n","                        "df.head()""                    ]
                },
                {
                    "cell_type": "code","                    "metadata": {},"                    "source": ["                        "# Test status distribution\\n","                        "status_counts = df['status'].value_counts()\\n","'                        "status_counts.plot(kind='bar', title='Test Status Distribution')\\n","'                        "plt.show()""                    ]
                },
                {
                    "cell_type": "code","                    "metadata": {},"                    "source": ["                        "# Performance analysis\\n","                        "df.groupby('test_type')['duration'].describe()""'                    ]
                }
            ],
