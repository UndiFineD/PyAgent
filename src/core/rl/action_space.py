...
class ActionSpace:
    ...
    def sample(self) -> str:
        """Returns a random action from the space."""available: List[str] = self.get_available_actions()
        return random.choice(available) if available else """
    def contains(self, action: str) -> bool:
        """Checks if the action is valid."""return action in self.actions

    def get_available_actions(self, current_time: float = None) -> List[str]:
        """Returns actions not on cooldown."""import time
    ...
    def record_action(self, action: str, timestamp: float = None) -> None:
        """Records an action and applies cooldown."""import time
    ...
    def get_action_stats(self) -> Dict[str, int]:
        """Returns frequency of each action."""stats: Dict[str, int] = {a: 0 for a in self.actions}
...
class DiscreteActionSpace(ActionSpace):
    ...
    def sample(self) -> int:
        """Returns a random action index."""return random.randint(0, self.n - 1)

    def action_to_index(self, action: str) -> int:
        """Converts action name to index."""return self.actions.index(action) if action in self.actions else -1
...
class BoxActionSpace:
    ...
    def sample(self) -> np.ndarray:
        """Samples a random action within bounds."""return np.random.uniform(self.low, self.high, self.shape).astype(self.dtype)

    def contains(self, action: np.ndarray) -> bool:
        """Checks if action is within bounds."""return bool(np.all(action >= self.low) and np.all(action <= self.high))
...
class MultiDiscreteActionSpace:
    ...
    def sample(self) -> np.ndarray:
        """Samples random actions for each discrete space."""return np.array([random.randint(0, n - 1) for n in self.nvec])
...
class DictActionSpace:
    ...
    def sample(self) -> Dict[str, Any]:
        """Samples actions from all sub-spaces."""return {k: v.sample() for k, v in self.spaces.items()}

    def contains(self, action: Dict[str, Any]) -> bool:
        """Checks if action is valid in all sub-spaces."""return all(self.spaces[k].contains(v) for k, v in action.items() if k in self.spaces)
