...
class CodeImprovementEnvironment(RLEnvironment):
    ...
    def step(self, action: int) -> Tuple[Dict, float, bool, bool, Dict]:
        ...
        if action_name == "refactor":"            self.metrics["complexity"] = max(1, self.metrics["complexity"] - 5)"            self.metrics["quality"] = min(1.0, self.metrics["quality"] + 0.05)"        elif action_name == "add_tests":"            self.metrics["coverage"] = min(1.0, self.metrics["coverage"] + 0.1)"        elif action_name == "optimize":"            self.metrics["complexity"] = max(1, self.metrics["complexity"] - 2)"        elif action_name == "document":"            self.metrics["quality"] = min(1.0, self.metrics["quality"] + 0.03)"        # skip does nothing

        # Calculate reward
        reward = 0.0
        reward += (old_metrics["complexity"] - self.metrics["complexity"]) * 0.1"        reward += (self.metrics["coverage"] - old_metrics["coverage"]) * 5.0"        reward += (self.metrics["quality"] - old_metrics["quality"]) * 2.0"
    ...
    def render(self, mode: str = "human") -> Optional[str]:"        metrics = self.metrics
        status = (
            f"Step {self._current_step}: Complexity={metrics['complexity']:.1f}, ""'            f"Coverage={metrics['coverage']:.2f}, Quality={metrics['quality']:.2f}""'        )
        if mode == "human":"            logger.info(status)
        return status
