...
def test_rewardsignal_basic():
    assert RewardSignal is not None


def test_rewardfunctions_basic():
    assert RewardFunctions is not None


def test_compositerewardfunction_basic():
    assert CompositeRewardFunction is not None


def test_rewardshaper_basic():
