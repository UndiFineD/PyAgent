...
class TestWebSecurityScannerCore:
    ...
    def test_init(self, scanner_core):
        ...
        assert scanner_core.timeout == 5
        assert scanner_core.concurrency == 2
        assert scanner_core.rate_limit == 10

    @pytest.mark.asyncio
    async def test_scan_hosts_empty(self, scanner_core):
        """Test scanning with no hosts."""
        results = await scanner_core.scan_hosts([], {})
        assert results == {}

    def test_normalize_url(self, scanner_core):
        """Test URL normalization."""
        assert scanner_core._normalize_url("example.com") == "http://example.com"
        assert scanner_core._normalize_url("https://example.com") == "https://example.com"
        assert scanner_core._normalize_url("") is None
    ...
    async def test_scan_single_host_match(self, mock_session_class, scanner_core):
        ...
        patterns = {'aem': r'href="/content/dam'}
        matches = await scanner_core._scan_single_host("http://example.com", patterns)
        assert 'aem' in matches

    @pytest.mark.asyncio
    @patch('aiohttp.ClientSession')
    async def test_scan_single_host_no_match(self, mock_session_class, scanner_core):
        """Test scanning single host with no pattern match."""
        mock_response = AsyncMock()
        mock_response.status = 200
...
class TestWebSecurityScannerAgent:
    ...
    async def test_detect_cms_instances(self, scanner_agent):
        ...
        assert "cms_detections" in results
        assert "analysis" in results
    ...
    async def test_analyze_scan_results(self, scanner_agent):
        ...
        results = {
            "http://example.com": ["aem", "wordpress"],
            "http://test.com": ["drupal"]
        }

        analysis = await scanner_agent._analyze_scan_results(results)

        assert analysis["pattern_distribution"]["aem"] == 1
        assert analysis["pattern_distribution"]["wordpress"] == 1
        assert analysis["pattern_distribution"]["drupal"] == 1
        assert analysis["total_matches"] == 3
        assert "http://example.com" in analysis["hosts_by_pattern"]["aem"]
