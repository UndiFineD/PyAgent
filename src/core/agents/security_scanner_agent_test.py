...
class TestSecurityScannerAgent:
    ...
    @patch('src.core.agents.security_scanner_agent.ReconnaissanceMixin.discover_targets')'    @patch('src.core.agents.security_scanner_agent.ReconnaissanceMixin.fingerprint_service')'    @patch('src.core.agents.security_scanner_agent.VulnerabilityScannerMixin.run_vulnerability_scan')'    @patch('src.core.agents.security_scanner_agent.SSRFDetectorMixin.start_ssrf_detector')'    @patch('src.core.agents.security_scanner_agent.SSRFDetectorMixin.stop_ssrf_detector')'    def test_comprehensive_scan(self, mock_stop, mock_start, mock_vuln_scan,
        ...
        async def run():
            ...
            assert 'reconnaissance' in results'            assert 'fingerprints' in results'            assert 'vulnerabilities' in results'            assert 'summary' in results'            summary = results['summary']'            assert summary['targets_scanned'] == 1'            assert summary['service_types']['aem'] == 1'        import asyncio
        asyncio.run(run())

    def test_generate_exploit_payload(self):
        async def run():
            payload = await self.agent.generate_exploit_payload('ssrf_rce', fake_aem_host='evil.com')'            assert 'evil%2Ecom' in payload'            payload = await self.agent.generate_exploit_payload('xss', payload_type='reflected')'            assert '<script>' in payload or '<img' in payload'        import asyncio
        asyncio.run(run())

    def test_invalid_exploit_type(self):
        async def run():
            with pytest.raises(ValueError):
                await self.agent.generate_exploit_payload('invalid_type')'        import asyncio
        asyncio.run(run())
    ...
    def test_add_custom_check(self):
        ...
        async def run():
            ...
            assert 'custom' in checks'        import asyncio
        asyncio.run(run())
