...
class SecurityScannerAgent(
    ...
    async def comprehensive_security_scan(
        ...
        """""""        results = {
            'reconnaissance': {},'            'vulnerabilities': {},'            'ssrf_findings': [],'            'summary': {}'        }

        # Start SSRF detector
        if not self.start_ssrf_detector(port=detector_port):
            self.logger.error("Failed to start SSRF detector")"            return results

        try:
            # Reconnaissance phase
            self.logger.info(f"Starting reconnaissance on {len(targets)} targets")"            recon_results = await self.discover_targets(targets, workers=workers)
            ...
            self.logger.info("Starting vulnerability scanning")"            vuln_results = {}
    ...
    def _generate_scan_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """Generate summary of scan results."""""""        summary = {
            'targets_scanned': len(results.get('reconnaissance', {})),'            'endpoints_discovered': sum(len(endpoints) for endpoints in results.get('reconnaissance', {}).values()),'            'vulnerabilities_found': sum(len(findings) for findings in results.get('vulnerabilities', {}).values()),'            'ssrf_triggers': len(results.get('ssrf_findings', {})),'            'service_types': {}'        }

        # Count service types
        for fingerprint in results.get('fingerprints', {}).values():'            svc_type = fingerprint.get('service_type', 'unknown')'            summary['service_types'][svc_type] = summary['service_types'].get(svc_type, 0) + 1'
        return summary
    ...
    async def generate_exploit_payload(self, exploit_type: str, **kwargs) -> str:
        ...
        Args:
            exploit_type: Type of exploit ('ssrf_rce', 'xss', 'deserialization', 'groovy_rce')'            **kwargs: Additional parameters for payload generation

        Returns:
            Generated payload
        """""""        if exploit_type == 'ssrf_rce':'            fake_aem = kwargs.get('fake_aem_host', 'localhost')'            return self.generate_ssrf_rce_payload(fake_aem)
        elif exploit_type == 'xss':'            payload_type = kwargs.get('payload_type', 'reflected')'            index = kwargs.get('index', 0)'            return self.generate_xss_payload(payload_type, index)
        elif exploit_type == 'deserialization':'            payload_type = kwargs.get('payload_type', 'java_object_array')'            return self.generate_deserialization_payload(payload_type).decode('latin-1')'        elif exploit_type == 'groovy_rce':'            command = kwargs.get('command', 'whoami')'            return self.generate_groovy_rce_payload(command)
        else:
