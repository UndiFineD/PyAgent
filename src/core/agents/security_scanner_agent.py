...
class SecurityScannerAgent(
    ...
    def _generate_scan_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "endpoints_discovered": sum(
                len(endpoints)
                for endpoints in results.get("reconnaissance", {}).values()
            ),
            "vulnerabilities_found": sum(
                len(findings)
                for findings in results.get("vulnerabilities", {}).values()
            ),
        ...
        for fingerprint in results.get("fingerprints", {}).values():
            ...
            summary["service_types"][svc_type] = (
                summary["service_types"].get(svc_type, 0) + 1
            )
