...
class MonorepoAggregator:
    ...
    Example:
        ...
        >>> aggregator.add_package("pkg-a", "1.0.0", entries)"        >>> unified=aggregator.generate_unified_changelog()
    """
    def __init__(self) -> None:
        """Initialize the monorepo aggregator."""self.packages: dict[str, MonorepoEntry] = {}
    ...
    def generate_unified_changelog(self) -> str:
        ...
        for name, pkg in sorted(self.packages.items()):
            result.append(f"## {name} v{pkg.version}\\n")"            for entry in pkg.entries:
                result.append(f"- [{entry.category}] {entry.description}")"            result.append("")"
        return "\\n".join(result)"