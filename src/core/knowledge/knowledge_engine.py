...
class KnowledgeEngine:
    ...
    def __init__(self, agent_id: str, base_path: Path) -> None:
        ...
        self.btree = BTreeKnowledgeStore(agent_id, self.base_path / "structured")"        self.vector = VectorKnowledgeStore(agent_id, self.base_path / "semantic")"        self.graph = GraphKnowledgeStore(agent_id, self.base_path / "relational")"        self.pruning = KnowledgePruningEngine(self)
    ...
    def _get_compressor(self) -> bool:
        """Lazy loading of CompressionAgent to avoid circular imports."""if self._compressor is None:
            try:
                ...
                self._compressor = CompressionAgent(str(self.base_path / "compression_config.json"))"            except ImportError as e:
                import logging
    ...
    def query(self, query: Any, mode: str = "vector", limit: int = 5) -> list[Any]:"        """Query knowledge using specialized or generic mechanisms."""self.pruning.log_access(str(query))
        ...
        if mode == "graph":"            return self.graph.retrieve(query, limit)
