...
class AutoMemCore:
    ...
    def get_bridge_connections(self, memory_id: str, max_depth: int = 3) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query = """
            MATCH path = (start:Memory {id: $memory_id})-[rels:*1..{max_depth}]-(end:Memory)
            WHERE start <> end
            AND ALL(r IN rels WHERE type(r) IN ['ASSOCIATED_WITH', 'SIMILAR_TO', 'CREATIVE_LINK', 'RELATED_TO'])
            ...
            for record in result.result_set:
                ...
                connections.append({
                    'start_memory': memory_id,
                    'end_memory': end_id,
                    'end_content': end_content,
                    'path_depth': depth,
                    'path_strength': path_strength,
                    'relationships': relationships,
                    'reasoning_path': self._extract_reasoning_path(path),
                    'neural_activation': path_strength * (1.0 / depth)  # Deeper paths have lower activation
                })
        ...
        try:
            ...
            for memory in initial_memories:
                ...
                if memory_id:
                    activation_network[memory_id] = {
                        ...
                        'activation': 1.0,  # Full activation for direct matches
                        'depth': 0,
                        'connections': []
                    }
            ...
            for hop in range(1, max_hops + 1):
                ...
                for memory_id in list(activation_network.keys()):
                    if activation_network[memory_id]['depth'] == hop - 1:
                        # Find bridge connections from this memory
                        bridges = self.get_bridge_connections(memory_id, max_depth=2)

                        for bridge in bridges:
                            target_id = bridge.get('end_memory')
                            if target_id and target_id not in visited_memories:
                                # Calculate activation decay
                                base_activation = activation_network[memory_id]['activation']
                                path_strength = bridge.get('path_strength', 0.5)
                                depth_penalty = 1.0 / (bridge.get('path_depth', 1) + 1)
                                ...
                                if new_activation > 0.1:  # Activation threshold
                                    new_activations[target_id] = {
                                        'content': bridge.get('end_content', ''),
                                        'activation': new_activation,
                                        'depth': hop,
                                        'connections': [memory_id],
                                        'relationship': bridge.get('relationships', [])
                                    }
            ...
            for memory_id, data in activation_network.items():
                ...
                if data['activation'] > 0.3:
                    insights.append({
                        ...
                        'activation': data['activation'],
                        'reasoning_depth': data['depth'],
                        'relevance_score': data['activation'] * (1.0 / (data['depth'] + 1))
                    })
            ...
            return {
                ...
                'activation_pattern': sorted(activation_pattern, key=lambda x: x['activation'], reverse=True),
                'reasoning_depth': max_hops,
                'total_activated_memories': len(activation_network),
                'confidence': min(1.0, len(insights) * 0.1)  # Simple confidence metric
            }

        except Exception as e:
            self.logger.error(f"Failed to perform neuroscience reasoning: {e}")
            return {
                'query': query,
                'insights': [],
                'activation_pattern': [],
    ...
    def benchmark_locomotivation(self) -> float:
        ...
        try:
            ...
            self.logger.info(
                f"🧠 LoCoMo benchmark complete: {total_score:.3f} "
                f"(Storage: {storage_score:.3f}, Recall: {recall_score:.3f}, "
                f"Stability: {stability_score:.3f}, Consolidation: {consolidation_score:.3f}, "
            ...
            self.logger.error(f"LoCoMo benchmark failed: {e}")
            return 0.0
    ...
    def _test_memory_storage_accuracy(self) -> float:
        ...
        try:
            ...
            test_memories = [
                ...
                "Project deadline is February 15th",
                "API key for service X is stored securely",
                "Last conversation was about machine learning",
                "User's favorite programming language is Python"
            ]

            stored_ids = []
            for memory in test_memories:
                mem_id = self.store_memory(memory, tags=["locomo_test"])
                stored_ids.append((mem_id, memory))

            # Verify storage by retrieving all memories
            all_memories = self.recall_memories("", tags=["locomo_test"], limit=10)
            stored_content = {mem['content'] for mem in all_memories}
            ...
            self.logger.warning(f"Storage accuracy test failed: {e}")
            return 0.0
    ...
    def _test_recall_performance(self) -> float:
        ...
        try:
            ...
            test_memories = []
            for i in range(50):
                content = f"LoCoMo test memory {i}: {'important ' * (i % 5)}data"
    ...
    def _test_consolidation_effectiveness(self) -> float:
        ...
        try:
            ...
            related_memories = [
                "User learned Python basics",
                "User completed first Python project",
                "User studied Python data structures",
                "User built a Python web application",
                "User mastered Python decorators"
            ]

            stored_ids = []
            for memory in related_memories:
                mem_id = self.store_memory(memory, tags=["python_learning", "locomo_consolidation"])
            ...
            pre_consolidation = self.recall_memories("Python", limit=10)
            post_consolidation = self.recall_memories("Python learning", limit=10)
            ...
            creative_results = self.recall_memories("programming journey", limit=5)
            creative_score = min(len(creative_results) / 3.0, 1.0)  # Expect some creative links
            ...
            self.logger.warning(f"Consolidation effectiveness test failed: {e}")
            return 0.0
    ...
    def _test_multi_hop_reasoning(self) -> float:
        ...
        try:
            ...
            chain_memories = [
                "User started learning machine learning",
                "Machine learning requires understanding statistics",
                "Statistics builds on probability theory",
                "Probability theory uses mathematical concepts",
                "Mathematics is fundamental to computer science"
            ]

            stored_ids = []
            for i, memory in enumerate(chain_memories):
                tags = ["ml_chain", f"step_{i}", "locomo_reasoning"]
                mem_id = self.store_memory(memory, tags=tags)
                stored_ids.append(mem_id)
            ...
            reasoning_result = self.neuroscience_reasoning(
                "How does machine learning connect to computer science?",
                max_hops=3
            )

            # Score based on whether reasoning found the connection chain
            if reasoning_result and 'insights' in reasoning_result:
                insights = reasoning_result['insights']
                # Check if reasoning connected the chain
                chain_keywords = ['statistics', 'probability', 'mathematics', 'computer science']
                found_connections = sum(1 for keyword in chain_keywords if
            ...
            self.logger.warning(f"Multi-hop reasoning test failed: {e}")
            return 0.0
...
class MemoryConsolidator:
        ...
        try:
            ...
            for record in result.result_set:
                ...
                try:
                    ...
                    self.logger.warning(f"Failed to decay memory {memory_id}: {e}")
                    continue
    ...
    def _creative_consolidation(self):
        ...
        try:
            ...
            # Find memories with high similarity that aren't already connected
            query = """
    ...
    def _forget_memories(self):
        ...
        try:
            ...
            AND (m.last_access IS NULL OR
                 datetime(m.last_access) < datetime() - duration('P90D'))
            AND NOT (m)-[:ASSOCIATED_WITH]-(:Memory)
            ...
            for record in result.result_set:
                ...
                SET m.archived = true,
                    ...
                    m.archive_reason = 'low_importance'
                """
                graph.query(archive_query, {
                    'id': memory_id,
                    'timestamp': datetime.now(timezone.utc).isoformat()
                })
