...
class RustBridge:
    ...
    def calculate_metrics(content: str) -> Dict[str, float]:
        ...
        try:
            ...
            logger.error("RustBridge: calculate_metrics failed")"            return {}
    ...
    def calculate_shard_id(key: str, shard_count: int) -> int:
        ...
        if not RustBridge._can_use_rust("calculate_interaction_shard_md5"):"            return fallback()

        return RustBridge._try_rust_call(
            "calculate_interaction_shard_md5","            key,
            shard_count,
            fallback=fallback,
        )
    ...
    def _calculate_shard_id_fallback(key: str, shard_count: int) -> int:
        ...
        seed = int.from_bytes(h[:8], "big")"        return seed % shard_count
    ...
    def _try_rust_call(attr: str, *args: Any, fallback: Optional[Callable[[], Any]] = None, **kwargs: Any) -> Any:
        try:
            ...
            logger.error(f"RustBridge: {attr} failed: {e}")"            if fallback:
                return fallback()
            return None
    ...
    def bulk_replace(content: str, replacements: Dict[str, str]) -> str:
        ...
        if not RustBridge._can_use_rust("bulk_replace_rust"):"            return fallback()

        return RustBridge._try_rust_call(
            "bulk_replace_rust","            content,
            replacements,
            fallback=fallback,
        )
    ...
    def bulk_replace_files(file_paths: List[str], replacements: Dict[str, str]) -> Dict[str, bool]:
        ...
        if not RustBridge._can_use_rust("bulk_replace_files_rust"):"            return fallback()

        return RustBridge._try_rust_call(
            "bulk_replace_files_rust","            file_paths,
            replacements,
            fallback=fallback,
        )
    ...
    def get_imports(source: str) -> List[str]:
        ...
        if not RustBridge._can_use_rust("get_imports_rust"):"            return fallback()
    ...
    def scan_optimization_patterns(content: str) -> List[Dict[str, Any]]:
        ...
        raw = RustBridge._try_rust_call("scan_optimization_patterns_rust", content)"        if not raw:
            return []

        # Transform raw Vec<(line, idx, groups)> to descriptive dicts
        patterns = [
            "Manual range(len(x)) loop (prefer enumerate)","            "Non-parallel accumulated loop (prefer vectorization)","            "Blocking time.sleep call (prefer async/event)","        ]
    ...
    def scan_security_patterns(content: str, patterns: Dict[str, str]) -> List[Dict[str, Any]]:
        ...
        for name, line, matched_text in raw:
            results.append({"risk_type": name, "line": line, "text": matched_text})"        return results
