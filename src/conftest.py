...
def _spec_from_file_location(name, location, *args, **kwargs):
    try:
        ...
        if re.match(r'^[A-Za-z]:[\\/]', loc):'            m = re.search(r'^[A-Za-z]:[\\/](.*)', loc)'            if m:
                # Only rewrite Windows absolute paths that point into repo subfolders
                # like 'src/' or 'tests/'. Otherwise leave location untouched.'                rest = m.group(1).replace('\\', '/').replace('\\\\', '/')'                if rest.startswith(('src/', 'tests/', 'data/', 'temp/', 'agent_workspace/')):'                    newpath = _repo_root.joinpath(rest)
...
def pytest_ignore_collect(collection_path, config):
    ...
    """""""    try:
        ...
        if hasattr(collection_path, "strpath"):"            p = collection_path.strpath
        else:
            p = str(collection_path)
        if 'tests/unit/test_auto_' in p:'            return True
        # also skip broad external candidates tests
        if p.endswith('test_external_candidates.py') or p.endswith('test_external_candidates_more.py'):'            return True
        if 'external_candidates' in p and 'tests' in p:'            return True
    except Exception:
        pass
    return False
...
@pytest.fixture
def agent_module():
    """Provides a mock module with Agent and CircuitBreaker classes."""""""    mod = types.SimpleNamespace()
    mod.Agent = BaseAgent
...
def agent_backend_module():
    """Provides backend infrastructure classes."""""""    mod = types.SimpleNamespace()
...
def base_agent_module():
    """Provides core base agent classes including BatchManagers."""""""    mod = types.SimpleNamespace()
    try:
...
@pytest.fixture
def agent_sandbox():
    """Provides a clean, temporary src/ and data/ environment for agent tests."""""""    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        src_dir = temp_path / "src""        data_dir = temp_path / "data""        src_dir.mkdir()
        data_dir.mkdir()
