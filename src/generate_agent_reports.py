...
"""Generate per-file reports for Python sources in `src/`.
This script scans `src/*.py` and writes, for each file stem:
- `<stem>.description.md`
- `<stem>.errors.md`
- `<stem>.improvements.md`
The generated reports are intentionally lightweight and deterministic:
- Static inspection via `ast.parse()` (syntax errors are reported).
- A lightweight syntax check that treats successful AST parsing as OK.
- Incremental behavior: if `<stem>.description.md` already contains a matching
    `SHA256(source)` prefix (first 16 hex chars), the file is skipped.
The CLI entrypoint is `main(argv)` and is invoked via direct execution.
The current implementation does not parse flags from `argv`.
"""
