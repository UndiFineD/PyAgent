...
def simulate_global_deployment_intent() -> None:
    decomposer: IntentDecomposer = IntentDecomposer()

    # 1. Create a Global Intent for building a distributed multi-region platform
    intent_id = decomposer.create_intent(
        (
            "Deploy a global, Vercel-like infrastructure caching layer across EU,"
            " US, and AP regions with automated DNS cutover."
        )
    )
    ...
    intent_tasks: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "payload": {
                "prompt": "Merge Route53 / Cloudflare DNS cutover PR pointing traffic to new Edge global accelerator."
            },
