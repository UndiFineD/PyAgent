...
pub fn calculate_agreement_score(votes: Vec<Bound<'_, pyo3::types::PyDict>>) -> PyResult<f64> {
    ...
    for vote in votes {
        let w: f64 = vote
            .get_item("weight")?
            .ok_or_else(|| pyo3::exceptions::PyKeyError::new_err("weight missing"))?
            .extract()?;
        let h: String = vote
            .get_item("hash")?
            .ok_or_else(|| pyo3::exceptions::PyKeyError::new_err("hash missing"))?
            .extract()?;
...
pub fn select_committee(
    agents_reliability: HashMap<String, f64>,
    min_size: usize,
) -> PyResult<Vec<String>> {
    let mut eligible: Vec<(String, f64)> = agents_reliability
        .iter()
    ...
    if committee.len() < min_size {
        ...
        committee = all.into_iter().take(min_size).map(|(n, _)| n).collect();
    }
...
#[pyfunction]
pub fn detect_deviating_hashes(
    votes: Vec<Bound<'_, pyo3::types::PyDict>>,
    ...
    for vote in votes {
        ...
        let id: String = vote
            .get_item("id")?
            .ok_or_else(|| pyo3::exceptions::PyKeyError::new_err("id missing"))?
            .extract()?;
