...
pub fn execute_dcap_cycle(topic: &str, content: &str) -> PyResult<HashMap<String, String>> {
    ...
    let deliberation = format!(
        "Deliberating on '{}': Assessing implications of {}...",
        topic, content_preview
    );

    // Phase 2: Conception
    let conception = format!(
        "Conceiving tool structure for '{}' based on extracted patterns.",
        topic
    );

    // Phase 3: Action
    let sanitized_topic = topic.to_lowercase().replace([' ', '-'], "_");
    ...
    // Phase 4: Perception
    let perception =
        "Validated tools against DCAP benchmarks (Self-Consistency, Logical Flow).".to_string();
...
pub fn analyze_paper(title: &str, summary: &str) -> PyResult<String> {
    ...
    Ok(format!(
        "Analysis of '{}': Identifies core logic: {}...",
        title, summary_preview
    ))
}
