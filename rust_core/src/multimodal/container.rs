...
pub fn parse_modality_stream_rust(
    ...
    if let Ok(b) = buffer.extract::<&[u8]>() {
        ...
        return Err(pyo3::exceptions::PyTypeError::new_err(
            "Argument 'buffer' must be bytes or str",
        ));
    }
    ...
    for frag in fragments {
        ...
        if type_ == "text" {
            ...
            for cap in re.captures_iter(text) {
                ...
                expanded.push(("modality".to_string(), tag_modality, tag_channel, tag_id));
...
pub fn project_modality_embeddings_rust(
    ...
    if embedding.is_empty() || projection_matrix.is_empty() {
        return embedding;
    }
...
pub fn synchronize_modalities_rust(
    ...
    // List of (video_idx, audio_idx) pairs match
...
pub fn calculate_multimodal_fusion_rust(
    ...
    match method {
        "concat" => features.into_iter().flatten().collect(),
        "weighted_sum" => {
...
#[pyfunction]
pub fn calculate_av_alignment_score_rust(audio_energy: Vec<f32>, visual_motion: Vec<f32>) -> f32 {
    // Cross-correlation check
    if audio_energy.is_empty() || visual_motion.is_empty() {
        return 0.0;
    }
