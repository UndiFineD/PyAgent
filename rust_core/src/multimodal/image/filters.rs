...
#[pyfunction]
pub fn apply_vision_filter_rust(pixels: Vec<u8>, filter_type: &str, intensity: f32) -> Vec<u8> {
...
#[pyfunction]
pub fn match_vision_color_profiles_rust(mut pixels: Vec<u8>, ref_pixels: Vec<u8>) -> Vec<u8> {
    if pixels.is_empty() || pixels.len() != ref_pixels.len() {
        return pixels;
    }
