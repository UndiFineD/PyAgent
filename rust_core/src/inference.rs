...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(arc_cache_balance_rust, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(compute_block_hashes_batched_rust, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(compute_diversity_penalty_rust, m)?)?;
