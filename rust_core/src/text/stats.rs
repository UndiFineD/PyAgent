...
#[pyfunction]
pub fn linear_forecast_rust(values: Vec<f64>, steps_ahead: usize) -> PyResult<Vec<f64>> {
    let n = values.len();
    if n < 2 {
        return Ok(Vec::new());
    }
...
#[pyfunction]
pub fn calculate_weighted_load_rust(loads: Vec<f64>, weights: Vec<f64>) -> PyResult<f64> {
