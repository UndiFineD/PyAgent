    ...
    candidate_tokens
        ...
        .map(|&token| {
            ...
            for seq in &existing_sequences {
                // Check recent window
                let start = if seq.len() > window_size {
                    seq.len() - window_size
                } else {
                    0
                };
...
#[pyfunction]
pub fn mirostat_sample_rust(logits: Vec<f64>, mu: f64, tau: f64, eta: f64) -> (usize, f64) {
...
#[pyfunction]
pub fn apply_top_k_rust(logits: Vec<Vec<f64>>, k: usize) -> Vec<Vec<f64>> {
    if k == 0 {
        return logits;
    }
...
#[pyfunction]
pub fn apply_top_p_rust(logits: Vec<Vec<f64>>, p: f64) -> Vec<Vec<f64>> {
    if p >= 1.0 {
        return logits;
    }
...
#[pyfunction]
pub fn apply_typical_sampling_rust(logits: Vec<Vec<f64>>, mass: f64) -> Vec<Vec<f64>> {
    logits
        .into_iter()
        .map(|mut row| {
            let max_val = row.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
            ...
            let mut deviations: Vec<(usize, f64)> = log_probs
                .iter()
                .enumerate()
            ...
            let keep_indices: HashSet<usize> = deviations
                .iter()
                .take(cutoff_idx)
                .map(|&(idx, _)| idx)
                .collect();
...
#[pyfunction]
pub fn apply_min_p_rust(logits: Vec<Vec<f64>>, min_p: f64) -> Vec<Vec<f64>> {
    if min_p <= 0.0 {
        return logits;
    }
...
#[pyfunction]
pub fn gumbel_noise_rust(shape: (usize, usize), seed: u64) -> Vec<Vec<f64>> {
    use std::collections::hash_map::DefaultHasher;
    use std::hash::{Hash, Hasher};
...
#[pyfunction]
pub fn min_p_threshold_rust(probs: Vec<f32>, min_p: f32) -> (f32, Vec<i32>) {
    if probs.is_empty() {
        return (0.0, Vec::new());
    }
...
#[pyfunction]
pub fn generate_sample_seeds_rust(base_seed: u64, n_samples: usize) -> Vec<u64> {
    use std::collections::hash_map::DefaultHasher;
    use std::hash::{Hash, Hasher};
