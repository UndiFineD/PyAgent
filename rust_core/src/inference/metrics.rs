...
#[pyfunction]
pub fn log_softmax_stable_rust(logits: Vec<f64>) -> Vec<f64> {
...
#[pyfunction]
pub fn extract_top_k_logprobs_rust(logprobs: Vec<f64>, k: usize) -> (Vec<usize>, Vec<f64>) {
...
#[pyfunction]
pub fn compute_perplexity_rust(logprobs: Vec<f64>) -> f64 {
...
#[pyfunction]
pub fn compute_entropy_rust(probs: Vec<f64>) -> f64 {
    probs
        .iter()
        .filter(|&&p| p > 1e-10)
        .map(|&p| -p * p.ln())
...
#[pyfunction]
pub fn counter_increment_rust(current: i64, increment: i64) -> i64 {
    current + increment
}
...
#[pyfunction]
pub fn extract_top_k_batch_rust(logprobs: Vec<Vec<f64>>, k: usize) -> Vec<(Vec<f64>, Vec<usize>)> {
    logprobs
        .into_iter()
        .map(|row| {
...
#[pyfunction]
pub fn sparse_logprobs_store_rust(logprobs: Vec<f64>, k: usize) -> (Vec<f64>, Vec<usize>) {
    let mut indexed: Vec<(usize, f64)> = logprobs
        .iter()
        .enumerate()
