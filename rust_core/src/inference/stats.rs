...
#[pyfunction]
pub fn compute_percentiles_rust(values: Vec<f64>, percentiles: Vec<f64>) -> Vec<f64> {
    if values.is_empty() {
...
#[pyfunction]
pub fn detect_anomalies_rust(values: Vec<f64>, z_threshold: f64) -> Vec<bool> {
    if values.len() < 2 {
...
#[pyfunction]
pub fn compute_cache_hit_rate_rust(window_queries: Vec<i64>, window_hits: Vec<i64>) -> f64 {
...
#[pyfunction]
pub fn analyze_trend_rust(timestamps: Vec<f64>, values: Vec<f64>) -> (String, f64) {
...
pub fn aggregate_token_metrics_rust(
    ...
    for key in keys {
        let sum: i64 = metrics_list.iter().filter_map(|m| m.get(key)).sum();
        result.insert(key.to_string(), sum);
    }

    let total =
        result.get("input_tokens").unwrap_or(&0) + result.get("output_tokens").unwrap_or(&0);
...
pub fn kv_cache_metrics_aggregate_rust(
    ...
    if !access_counts.is_empty() {
        let mean_access: f64 =
            access_counts.iter().sum::<i64>() as f64 / access_counts.len() as f64;
        let zero_access_rate: f64 =
            access_counts.iter().filter(|&&c| c == 0).count() as f64 / access_counts.len() as f64;
...
pub fn sliding_window_stats_rust(
    ...
    if !latencies.is_empty() {
        ...
        result.insert(
            ...
            latencies[(latencies.len() as f64 * 0.99) as usize] as f64,
        );
    }
...
#[pyfunction]
pub fn memory_pressure_rust(current_bytes: i64, peak_bytes: i64, eviction_rate: f64) -> f64 {
