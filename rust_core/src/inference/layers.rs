...
pub fn ssm_step_rust(
    ...
    for batch in 0..batch_size {
        for d in 0..d_inner {
            ...
            for s in 0..ssm_state {
                new_state[batch][d][s] =
                    dA[batch][d][s] * state[batch][d][s] + dB[batch][d][s] * x[batch][d];
            }
...
pub fn parallel_scan_rust(
    ...
    for batch in 0..batch_size {
        ...
        for t in 1..seq_len {
            for d in 0..dim {
                output[batch][t][d] =
                    gates[batch][t][d] * output[batch][t - 1][d] + values[batch][t][d];
            }
        }
    }
...
pub fn causal_conv1d_update_rust(
    ...
    let batch_size = x.len();
    let d_inner = if x.is_empty() { 0 } else { x[0].len() };
    let kernel_size = if weight.is_empty() {
        0
    } else {
        weight[0].len()
    };
...
pub fn mla_compress_kv_rust(
    ...
    let hidden_size = if kv_proj_weight.is_empty() {
        0
    } else {
        kv_proj_weight[0].len()
    };
...
#[pyfunction]
pub fn mla_head_mapping_rust(num_heads: usize, num_kv_heads: usize) -> Vec<usize> {
    if num_kv_heads == 0 {
...
#[pyfunction]
pub fn mean_pool_rust(embeddings: Vec<Vec<f64>>, mask: Vec<f64>) -> Vec<f64> {
...
#[pyfunction]
pub fn last_token_pool_rust(embeddings: Vec<Vec<f64>>, mask: Vec<f64>) -> Vec<f64> {
...
#[pyfunction]
pub fn attention_pool_rust(embeddings: Vec<Vec<f64>>, attention_scores: Vec<f64>) -> Vec<f64> {
    if embeddings.is_empty() {
        return Vec::new();
    }
