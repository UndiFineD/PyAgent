...
#[pyfunction]
pub fn top_k_mask_rust(logits: Vec<f32>, k: usize) -> PyResult<Vec<f32>> {
    if k >= logits.len() || k == 0 {
        return Ok(logits);
    }
...
#[pyfunction]
pub fn top_p_mask_rust(logits: Vec<f32>, p: f32) -> PyResult<Vec<f32>> {
...
#[pyfunction]
pub fn gumbel_sample_rust(logits: Vec<f32>, temperature: f32, seed: u64) -> PyResult<usize> {
    if logits.is_empty() {
