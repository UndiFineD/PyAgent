...
#[pyfunction]
pub fn quantize_symmetric_rust(weights: Vec<f32>, bits: u8) -> PyResult<(Vec<i8>, f32)> {
    if weights.is_empty() {
        return Ok((vec![], 1.0));
    }
...
#[pyfunction]
pub fn quantize_asymmetric_rust(weights: Vec<f32>, bits: u8) -> PyResult<(Vec<u8>, f32, i32)> {
    if weights.is_empty() {
        return Ok((vec![], 1.0, 0));
    }
...
#[pyfunction]
pub fn dequantize_int4_rust(packed: Vec<i8>, scale: f32, zero_point: i32) -> PyResult<Vec<f32>> {
