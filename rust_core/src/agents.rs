...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(pyo3::wrap_pyfunction!(
        economy::calculate_bid_priority_score,
        m
    )?)?;

    m.add_function(pyo3::wrap_pyfunction!(
        byzantine::detect_deviating_hashes,
        m
    )?)?;
    ...
    m.add_function(pyo3::wrap_pyfunction!(
        byzantine::calculate_agreement_score,
        m
    )?)?;
    ...
    m.add_function(pyo3::wrap_pyfunction!(
        consensus::calculate_consensus_winner,
        m
    )?)?;
    ...
    m.add_function(pyo3::wrap_pyfunction!(
        graph::extract_graph_entities_regex,
        m
    )?)?;
    ...
    m.add_function(pyo3::wrap_pyfunction!(
        graph::filter_active_topology_relationships,
        m
    )?)?;

    m.add_function(pyo3::wrap_pyfunction!(
        knowledge::extract_python_symbols,
        m
    )?)?;
    m.add_function(pyo3::wrap_pyfunction!(
        knowledge::extract_markdown_backlinks,
        m
    )?)?;
    ...
    m.add_function(pyo3::wrap_pyfunction!(
        localization::translate_key_terms,
        m
    )?)?;
    m.add_function(pyo3::wrap_pyfunction!(
        localization::detect_cultural_issues,
        m
    )?)?;
    ...
    m.add_function(pyo3::wrap_pyfunction!(
        quantum::calculate_superposition_weights,
        m
    )?)?;

    m.add_function(pyo3::wrap_pyfunction!(redqueen::evaluate_bypass, m)?)?;
    m.add_function(pyo3::wrap_pyfunction!(
        redqueen::filter_relevant_insights,
        m
    )?)?;
    ...
    m.add_function(pyo3::wrap_pyfunction!(
        stability::detect_failed_agents_rust,
        m
    )?)?;

    m.add_function(pyo3::wrap_pyfunction!(
        synapse::calculate_synaptic_weight,
        m
    )?)?;
