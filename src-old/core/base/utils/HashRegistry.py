...
## Source: src-old/core/base/utils/HashRegistry.description.md
...
## Source: src-old/core/base/utils/HashRegistry.improvements.md

# Improvements for HashRegistry
...
### Testing
- [!] **Missing test file** - Create `HashRegistry_test.py` with pytest tests
...
def get_hash_fn_by_name(name: str) -> Callable[[Union[str, bytes]], str]:
    ...
    if algorithm is None:
        raise ValueError(
            f"Unknown hash algorithm: {name}. " f"Available: {list(name_map.keys())}"
        )
