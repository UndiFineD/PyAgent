...
class ChannelProvider(ABC):
    ...
    def channel_type(self) -> ChannelType:
        """The type of channel this provider handles."""
        pass
...
class GatewayProtocol:
    ...
    async def handle_message(self, client_id: str, data: Dict[str, Any]):
        ...
        if msg_type == "presence_update":
            ...
            await self.handle_session_create(client_id, data)
        elif msg_type == "session_update":
            await self.handle_session_update(client_id, data)
        elif msg_type == "message_send":
            await self.handle_message_send(client_id, data)
        elif msg_type == "tool_call":
            await self.handle_tool_call(client_id, data)
        else:
    ...
    async def handle_presence_update(self, client_id: str, data: Dict[str, Any]):
        ...
        self.presence[client_id] = GatewayPresence(
            client_id=client_id,
            status=status,
    ...
    async def handle_session_update(self, client_id: str, data: Dict[str, Any]):
        ...
        for key, value in data.items():
            if key in ["activation_mode", "is_active", "metadata"]:
                if key == "activation_mode":
                    session.activation_mode = SessionActivationMode(value)
                elif key == "is_active":
    ...
    async def broadcast(self, message: Dict[str, Any], exclude_client: Optional[str] = None):
        """Broadcast message to all clients except optionally excluded one."""
        to_remove = []
