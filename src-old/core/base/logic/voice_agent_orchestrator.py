...
class VoiceAgentOrchestrator:
    ...
    def _process_transcription(
        ...
        if "create" in transcription.lower() and "agent" in transcription.lower():
            return self._handle_create_agent(transcription, context)
        elif "list agents" in transcription.lower():
            return self._handle_list_agents(context)
        elif "run task" in transcription.lower() or "execute" in transcription.lower():
            return self._handle_run_task(transcription, context)
        elif "check status" in transcription.lower():
            return self._handle_check_status(transcription, context)
        else:
            return {
                "ok": True,
    ...
    def _handle_create_agent(
        ...
        if "browser" in transcription.lower():
            agent_type = "browser"
        elif "voice" in transcription.lower():
            agent_type = "voice"
    ...
    def _voice_agent_handler(
        ...
        if action == "create":
            ...
            return {"ok": True, "message": f"Voice agent '{agent_name}' cleaned up"}
        elif action == "tools":
            return self.tool_specs
        elif action == "execute":
            # Voice agents don't execute tasks directly
            return {"ok": False, "error": "Voice agents coordinate other agents"}
        else:
        ...
        return [
            {
                "type": "function",
                "name": "create_agent",
                "description": "Create a new agent of specified type (coding, browser, voice)",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "agent_type": {
            ...
            },
            {
                "type": "function",
                "name": "dispatch_task",
                "description": "Send a task to a specific agent for execution",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "agent_name": {
    ...
    def _simulate_transcription(self, audio_data: bytes) -> str:
        """Simulate audio transcription (placeholder for real OpenAI API)."""
        # In real implementation, would send to OpenAI Realtime API
        # For now, return a simulated transcription that doesn't trigger actions
        return "Hello, how can you help me with my agents?"
