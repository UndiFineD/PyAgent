...
class ReconnaissanceCore:
    ...
    def __init__(self):
        """Initialize reconnaissance core with intelligence sources"""
        self.sources = {}
    ...
    async def enumerate_subdomains(self, config: ReconConfig) -> List[SubdomainResult]:
        ...
        for result_set in source_results:
            if isinstance(result_set, Exception):
                logger.error(f"Source enumeration failed: {result_set}")
                continue
        ...
        for var_name, values in payloads.items():
            ...
            for result in results:
                if f"{{{{{var_name}}}}}" in result:
                    for value in values:
                        new_results.append(result.replace(f"{{{{{var_name}}}}}", value))
                else:
