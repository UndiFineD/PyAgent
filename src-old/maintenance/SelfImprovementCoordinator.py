...
class SelfImprovementCoordinator:
    ...
    def __init__(self, workspace_root: str):
        ...
        self.budget = BudgetManager(
            daily_limit=25.0, monthly_limit=250.0  # Target from cloud_integration.md
        )
    ...
    def _init_discovery(self):
        ...
        try:
            ...
            if not secret_key:
                self.logger.warning(
                    "PYAGENT_SECRET not set; LAN discovery disabled for security."
                )
                return
    ...
    async def load_strategic_context(self):
        ...
        for f in files_to_check:
            if f.exists():
                ...
                fixed_prompts = re.findall(
                    r"-\s+(.*?)(?=\n-|\n\n|\n#|$)", content, re.DOTALL
                )
                self.directives["fixed_prompts"].extend(
        ...
        self.logger.info(
            f"Strategic context loaded: {len(self.directives['fixed_prompts'])} directives, {len(self.directives['target_peers'])} target peers."
        )
    ...
    async def discover_external_servers(self) -> List[Dict[str, Any]]:
        ...
        try:
                ...
                all_nodes.append(
                    {
                        ...
                        "type": "mcp_server",
                        "status": (
                            "connected" if name in registry._sessions else "registered"
                        ),
                    }
                )
            ...
            self.logger.error(
                f"Error during MCP server discovery from registry: {e}", exc_info=True
            )
        ...
        try:
            ...
            if status_file.exists():
                try:
                    ...
                    self.logger.warning(
                        f"Failed to parse connectivity_status.json: {jde}"
                    )
                except Exception as inner_e:
                    self.logger.error(
                        f"Error reading/processing connectivity_status.json: {inner_e}"
                    )
        except ImportError as e:
            self.logger.debug(
                f"ConnectivityManager not available for status check: {e}"
            )
        except Exception as e:
            self.logger.error(
                f"Unexpected error in ConnectivityManager discovery block: {e}",
                exc_info=True,
            )

        self.logger.info(
            f"Discovery Cycle: Found {len(all_nodes)} total available/connected servers/nodes."
        )
        return all_nodes
    ...
    async def run_healing_cycle(self):
        ...
        if failed_agents:
            self.logger.warning(
                f"Self-Healing: {len(failed_agents)} failures detected. Reviewing roadmap items..."
            )
            for agent in failed_agents:
                # Prioritize healing for agents mentioned in the fixed prompts or roadmap
                is_priority = any(
                    agent.lower() in p.lower() for p in self.directives["fixed_prompts"]
                )
                if is_priority:
                    ...
                    results["actions_taken"].append(
                        f"Queued priority recovery for {agent}"
                    )
    ...
    async def execute_remote_task(
        ...
        if not target or target.get("status") not in valid_statuses:
            return {
                "status": "failed",
                "error": f"Peer {target_peer} is offline or unknown",
            }
    ...
    async def run_discovery_cycle(self):
        ...
        high_priority_section = re.search(
            r"### High Priority\n(.*?)(?=\n###|\n==)", content, re.DOTALL
        )
    ...
    async def scan_for_research(self) -> List[str]:
        ...
        sciencedirect_links = re.findall(
            r"sciencedirect\.com/science/article/pii/(\w+)", content
        )

        links = [f"https://arxiv.org/abs/{l}" for l in arxiv_links]
        links.extend(
            [
                f"https://www.sciencedirect.com/science/article/pii/{l}"
                for l in sciencedirect_links
            ]
        )
    ...
    async def generate_action_plan(self, active_ideas: List[Dict[str, Any]]):
        ...
        for idea in active_ideas:
            print(
                f"[ACTION] Processing Improvement: {idea['title']} (Status: {idea['status']})"
            )
    ...
    async def trigger_agent_execution(self, item: Dict[str, Any]):
        ...
        try:
            if status == "PLANNED" or status == "PLANNING":
                from src.infrastructure.orchestration.swarm.DirectorAgent import (
                    DirectorAgent,
                )
