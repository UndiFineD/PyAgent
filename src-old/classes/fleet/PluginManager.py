...
class PluginManager:
                ...
                with open(self.registry_path, "r") as f:
                    ...
                    for key, meta in data.items():
                        ...
                        if self.validate_version(min_sdk):
                            ...
                            logging.warning(
                                f"PluginManager: Ignoring '{key}' (Incompatible SDK {min_sdk})"
                            )
        ...
        if self.plugins_dir.exists():
            for item in self.plugins_dir.iterdir():
                ...
                if item.is_file() and item.suffix == ".py":
                    ...
                    logging.debug(
                        f"PluginManager: Dynamically discovered '{plugin_name}'"
                    )
                    ...
                    logging.debug(
                        f"PluginManager: Dynamically discovered package '{plugin_name}'"
                    )
    ...
    def load_resource(self, plugin_name: str) -> Optional[Any]:
        ...
        module_path, class_name, needs_fleet, min_sdk = (
            meta if len(meta) >= 4 else (meta[0], meta[1], False, "1.0.0")
        )
