...
class FleetManager:
    ...
    def execute_reliable_task(self, task: str) -> str:
        ...
        try:
            ...
            fallback_model = self.fallback_engine.get_fallback_model(
                current_model, str(e)
            )
    ...
    def call_by_capability(self, goal: str, **kwargs) -> str:
            ...
            tools_metadata.append(
                {
                    ...
                    "sync": getattr(
                        t, "sync", True
                    ),  # Optimization: check if tool can be run async
                }
            )

        scored_candidates = self.core.score_tool_candidates(
        ...
        try:
            if is_essential:
                ...
                with ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(
                        self.registry.call_tool, best_tool, **kwargs
                    )
                    try:
            ...
            if hasattr(self, "telemetry"):
                self.telemetry.trace_workflow(
                    f"tool_{best_tool}", time.time() - start_time
                )

    def register_agent(
        self, name: str, agent_class: Type[BaseAgent], file_path: Optional[str] = None
    ) -> str:
    ...
    def cell_divide(self, agent_name: str) -> str:
        ...
        self.agents[clone_name] = (
            base_agent  # Simplified: sharing the instance for demo
        )
        ...
        for step in workflow_steps:
            ...
            while not success and attempts <= max_retries:
                ...
                try:
                    ...
                    logging.info(
                        f"Fleet executing (Attempt {attempts}): {agent_name}.{action_name}({processed_args}) using model {current_model}"
                    )
                    ...
                    if is_essential:
                        ...
                        with ThreadPoolExecutor(max_workers=1) as executor:
                            ...
                            try:
                                ...
                                logging.warning(
                                    f"Workflow agent '{agent_name}' timed out after 5 seconds."
                                )
                                raise Exception(
                                    f"Agent '{agent_name}' timed out after 5 seconds."
                                )

                    # Scaling & RL feedback
                    duration = time.time() - start_time
                    self.scaling.record_metric(agent_name, duration)
                    self.rl_selector.update_stats(
                    ...
                    token_info = getattr(
                        agent,
                        "_last_token_usage",
                        {"input": 0, "output": 0, "model": current_model or "unknown"},
                    )
                    ...
                    if hasattr(self, "self_healing"):
                        try:
                            heal_msg = self.self_healing.handle_failure(
                                agent, action_name, e, {"args": processed_args}
                            )
                    ...
                    if attempts <= max_retries:
                        current_model = getattr(agent, "get_model", lambda: "default")()
                        next_model = self.fallback_engine.get_fallback_model(
                            current_model, failure_reason=error_msg
                        )
                        ...
                        if next_model and hasattr(agent, "set_model"):
                            logging.warning(
                                f"Retrying {agent_name} with fallback model: {next_model}"
                            )
                            agent.set_model(next_model)
                            self.signals.emit(
                                "STEP_RETRYING",
                                {"agent": agent_name, "fallback_model": next_model},
                                sender="FleetManager",
                            )
                            import threading
...
if __name__ == "__main__":
    ...
    fleet.register_agent(
        "Knowledge", KnowledgeAgent, str(root / "src/classes/context/KnowledgeAgent.py")
    )
    fleet.register_agent(
        "Security",
        SecurityGuardAgent,
        str(root / "src/classes/coder/SecurityGuardAgent.py"),
    )
