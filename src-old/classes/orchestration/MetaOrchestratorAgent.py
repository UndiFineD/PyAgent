...
class MetaOrchestratorAgent(BaseAgent):
    ...
    def execute_by_goal(self, goal: str) -> str:
        ...
        for tool_meta in tools:
            if tool_meta.name.lower() in goal.lower() or any(
                k in goal.lower() for k in tool_meta.description.lower().split()
            ):
                logging.info(
                    f"Orchestrator matched goal '{goal}' to tool '{tool_meta.name}'"
                )
    ...
    def solve_complex_objective(self, objective: str, depth: int = 0) -> str:
        ...
        self.recorder.record_lesson(
            "meta_planning_init", {"objective": objective, "depth": depth}
        )

        # 1. Update Global Context with the new objective
        self.global_context.add_insight(
            f"Objective depth {depth}: {objective}", "MetaOrchestrator"
        )
        ...
        try:
            ...
            self.recorder.record_lesson(
                "meta_plan_received", {"plan": plan_data, "objective": objective}
            )
        except Exception as e:
            self.recorder.record_lesson(
                "meta_plan_parsing_error",
                {"error": str(e), "report_preview": plan_report[:500]},
            )
        ...
        for step in plan_data:
            ...
            res = self.fleet.execute_workflow(
