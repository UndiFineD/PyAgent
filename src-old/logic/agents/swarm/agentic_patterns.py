...
class SequentialAgentPattern:
    ...
    async def execute_sequential(
        ...
        **kwargs,
        ...
        for i, agent_config in enumerate(config.sub_agents):
            ...
            try:
                ...
                agent_context = context.next_level(
                    child_task_id=f"{context.task_id}_seq_{i}", agent_id=agent_name
                )
                ...
                if not config.continue_on_failure:
                    logger.error(
                        f"Stopping sequential execution due to failure in {agent_name}"
                    )
                    break
        ...
        logger.info(
            f"Completed sequential execution for {config.name}: "
        ...
        **kwargs,
        ...
        if agent_type == "pattern":
            ...
            if custom_logic and callable(custom_logic):
                return await custom_logic(
                    context, input_data, execution_state, **kwargs
                )
            else:
    ...
    async def execute_parallel(
        ...
        **kwargs,
        ...
        for i, agent_config in enumerate(agent_configs):
            agent_name = agent_config.get("name", f"agent_{i}")
            agent_context = context.next_level(
                child_task_id=f"{context.task_id}_par_{i}", agent_id=agent_name
            )
        ...
        logger.info(
            f"Completed parallel execution: {successful}/{len(agent_configs)} successful"
        )
