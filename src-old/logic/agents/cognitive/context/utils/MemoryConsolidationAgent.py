...
## Source: src-old/logic/agents/cognitive/context/utils/MemoryConsolidationAgent.description.md

# MemoryConsolidationAgent

**File**: `src\logic\agents\cognitive\context\utils\MemoryConsolidationAgent.py`  
**Type**: Python Module  
...
## Source: src-old/logic/agents/cognitive/context/utils/MemoryConsolidationAgent.improvements.md
...
class MemoryConsolidationAgent(BaseAgent):
    ...
    def consolidate_all(self) -> str:
        ...
        for ep in episodes:
            ...
            if "version" in task:
                self.context_engine.add_fact(
                    "project_version", ep["outcome"].split()[-1]
                )
                new_facts += 1
            ...
            if not success:
                if "import" in outcome or "module" in outcome:
                    self.context_engine.add_constraint(
                        "Always verify __init__.py exports before adding new sub-packages."
                    )
                    new_insights += 1
                elif "pos" in outcome or "argument" in outcome:
                    self.context_engine.add_insight(
                        "Agent signatures are prone to drift. Verify base class inheritance.",
                        "Consolidator",
                    )
                    new_insights += 1
            else:
                if "optimized" in task:
                    self.context_engine.add_insight(
                        f"Successfully optimized: {task}", "Consolidator"
                    )
                    new_insights += 1
