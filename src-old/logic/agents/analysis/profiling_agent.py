...
## Source: src-old/logic/agents/analysis/profiling_agent.improvements.md

# Improvements: src/logic/agents/analysis/profiling_agent.py
...
from src.observability.stats.core.profiling_core import ProfileStats, ProfilingCore
...
class ProfilingAgent(BaseAgent):
    ...
    def profile_file(self, file_path: str) -> list[ProfileStats]:
        ...
        try:
            ...
            if "os.remove" in code or "subprocess" in code:
                logging.info(
                    f"ProfilingAgent: Skipping execution profile for {file_path} due to risky calls. Using static fallback."
                )
    ...
    def static_profile(self, file_path: str) -> list[ProfileStats]:
        ...
        try:
            ...
            if not os.path.exists(file_path):
                logging.error(
                    f"ProfilingAgent: [Static] File does not exist: {file_path}"
                )
                return []
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    logging.info(
                        f"ProfilingAgent: Static scan of {node.name} found {loops} loops."
                    )
                    if loops > 5:
                        findings.append(
                            ProfileStats(
    ...
    def _handover_to_coder(
        self, file_path: str, slow_functions: List[ProfileStats]
    ) -> None:
        ...
        for func in slow_functions:
            ...
            if hasattr(self, "fleet") and self.fleet:
                ...
                self.fleet.intelligence.contribute_insight(
                    agent_name=self.agent_name, insight=insight_str, confidence=0.98
                )
                ...
                try:
                    ...
                    logging.debug(
                        f"ProfilingAgent: Could not directy inject task to orchestrator: {e}"
                    )

            logging.info(
                f"ProfilingAgent: Proposed Rust conversion for {func.function_name} ({func.total_time:.2f}s)"
            )
