...
def aggregate(results: dict) -> dict:
    ...
    for r in results.get("results", []):
        ...
        files[fn]["findings"].append(
            {"severity": sev, "test": test, "line": lineno, "text": text}
        )
    return files
...
def make_report(agg: dict, top_n: int = 50) -> None:
    ...
    for i, (fn, meta) in enumerate(entries[:top_n], 1):
        ...
        for f in meta["findings"]:
            report.append(
                f"  - [{f['severity']}] L{f.get('line')} {f['test']}: {f['text']}\n"
            )
        # create a patch proposal
        try:
            create_patch_proposal(fn, meta["findings"])
            report.append(
                f"- Patch proposal: .external/patches/{sanitize_name(Path(fn))}.patch\n"
            )
...
def main() -> int:
    ...
    make_report(agg)
    print(
        "Wrote report and patch proposals to .external/static_checks and .external/patches"
    )
    return 0
