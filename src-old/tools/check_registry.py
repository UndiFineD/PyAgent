...
## Source: src-old/tools/check_registry.description.md
...
## Source: src-old/tools/check_registry.improvements.md
...
def run_diagnostic() -> None:
    ...
    try:
        ...
        print(
            f"Discovered {len(agents._discovered_configs)} agent configurations."
        )  # pylint: disable=protected-access
        for name in list(agents._discovered_configs.keys())[
            :10
        ]:  # pylint: disable=protected-access
            print(f"  - {name}")
        if len(agents._discovered_configs) > 10:  # pylint: disable=protected-access
            print(
                f"  ... and {len(agents._discovered_configs) - 10} more."
            )  # pylint: disable=protected-access
    except (
        ImportError,
        OSError,
    ...
    try:
        ...
        print(
            f"Discovered {len(orchestrators._configs)} orchestrator configurations."
        )  # pylint: disable=protected-access
        for name in list(
            orchestrators._configs.keys()
        ):  # pylint: disable=protected-access
            print(f"  - {name}")
    except (
        ImportError,
