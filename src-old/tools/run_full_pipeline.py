        ...
        for node in ast.walk(mod):
            ...
            if isinstance(node, ast.Call):
                ...
                if (
                    ...
                    and fn.attr in dangerous_attrs
                ):
                    file_findings.append(f"dangerous_call: {fn.value.id}.{fn.attr}")
        py_out = out_dir / f"python_{sha}.json"
        if file_findings:
            py_out.write_text(
                json.dumps({str(p): sorted(set(file_findings))}, indent=2),
                encoding="utf-8",
            )
            results["python_only"] = {
                "code": 0,
                "files": {str(p): sorted(set(file_findings))},
            }
        else:
    ...
    if summary:
        for k, v in summary.items():
            content_lines.append(
                f'  - {k}: code={v.get("code")} result={v.get("result")}'
            )
    else:
...
def main():
    ...
    if report_changed:
        extracted = [
            p
            for p in (EXTRACT_TARGET.rglob("*.py") if EXTRACT_TARGET.exists() else [])
            if not _is_init(p)
        ]
    else:
        ...
        try:
            ...
            extracted = [
                p
                for p in (
                    EXTRACT_TARGET.rglob("*.py") if EXTRACT_TARGET.exists() else []
                )
                if not _is_init(p)
            ]
    try:
    ...
    for p in extracted:
        ...
        if row and abs(row[0] - st.st_mtime) < 0.0001 and row[1] == st.st_size:
            ...
            cur.execute(
                ...
                (int(datetime.utcnow().timestamp()), rel),
            )
            continue
    ...
    if to_hash:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as exc:
            futures = {
                exc.submit(compute_sha, str(p)): (p, rel, mtime, size)
                for (p, rel, mtime, size) in to_hash
            }
            ...
            for fut in concurrent.futures.as_completed(futures):
                ...
                if len(batch) >= 1000:
                    cur.executemany(
                        "REPLACE INTO files (path, mtime, size, sha256, last_seen) VALUES (?, ?, ?, ?, ?)",
                        batch,
                    )
                    conn.commit()
                    batch = []
    ...
    print(
        f"Changed files: {len(changed_files)} (unique content groups: {len(hash_map)})"
    )
    ...
    if changed_files:
        ...
        if shard_count > 1:
            ...
            selected = {
                k for i, k in enumerate(keys) if (i % shard_count) == shard_index
            }
            ...
            if missing_shas:
                ...
                try:
                    ...
                    run(
                        [
                            sys.executable,
                            str(SCRIPTS / "apply_safe_fixes.py"),
                            "--apply",
                            "--target",
                            str(tmpdir),
                            "--patch-dir",
                ...
                try:
                    ...
                    run(
                        [
                            sys.executable,
                            str(SCRIPTS / "run_static_checks.py"),
                            str(tmpdir),
                        ]
                    )
                ...
                if semgrep_json.exists():
                    try:
                        sj = json.loads(
                            semgrep_json.read_text(encoding="utf-8", errors="ignore")
                        )
                        for item in (
                            sj.get("results", []) if isinstance(sj, dict) else []
                        ):
                            path = item.get("path") or item.get("extra", {}).get(
                                "metadata", {}
                            ).get("path")
                            if not path:
                                path = item.get("path") or item.get("filename")
                            if path:
                                semgrep_by_file.setdefault(
                        ...
                        if isinstance(node, ast.Attribute) and isinstance(
                            ...
                            if node.attr in dangerous_attrs:
                                file_findings.append(
                                    f"dangerous_attr: {node.value.id}.{node.attr}"
                                )
                        if isinstance(node, ast.Call):
                            fn = node.func
                            ...
                            if (
                                ...
                                file_findings.append(
                                    f"dangerous_call: {fn.value.id}.{fn.attr}"
                                )
                    return (
                        path_str,
                        {
                            "files": (
                                {str(p): sorted(set(file_findings))}
                                if file_findings
                                else {}
                            )
                        },
                ...
                with concurrent.futures.ProcessPoolExecutor(
                    max_workers=min(len(sha_to_paths), (os.cpu_count() or 2))
                ) as pool:
                    futures = {
                        pool.submit(python_checks, str(p)): s
                ...
                for s, p in sha_to_paths.items():
                    ...
                    if bdata:
                        ...
                        try:
                            Path(bandit_path).write_text(
                                json.dumps(bdata, indent=2), encoding="utf-8"
                            )
                        except Exception:
                            bandit_path = ""
                    ...
                    if sdata:
                        ...
                        try:
                            Path(semgrep_path).write_text(
                                json.dumps(sdata, indent=2), encoding="utf-8"
                            )
                        except Exception:
                            semgrep_path = ""
                    ...
                    if pdata:
                        ...
                        try:
                            Path(python_path).write_text(
                                json.dumps(pdata, indent=2), encoding="utf-8"
                            )
                        except Exception:
                            python_path = ""
                ...
                if rows:
                    cur.executemany(
                        "REPLACE INTO tool_results (sha, bandit_path, semgrep_path, python_path, last_checked) VALUES (?, ?, ?, ?, ?)",
                        rows,
                    )
                    # also mark as static_checked
                    cur.executemany(
                        "REPLACE INTO static_checked (sha, last_checked) VALUES (?, ?)",
                        [(r[0], ts) for r in rows],
                    )
                    conn.commit()
            else:
                print(
                    "All unique content groups already static-checked; skipping apply/static checks"
                )
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)
    else:
        print(
            "No changed files detected; skipping apply-safe-fixes and per-file static checks"
        )
    ...
    try:
        ...
        rc.execute(
            "INSERT INTO runs (start_ts, duration_ms, changed_count, unique_groups) VALUES (?, ?, ?, ?)",
            (start_ts, duration_ms, len(changed_files), len(hash_map)),
        )
        rconn.commit()
        rconn.close()
    except Exception:
